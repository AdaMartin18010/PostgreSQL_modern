...
for md_file in base_path.rglob("*.md"):
    ...
    if md_file.name == 'README.md' and '参考资料' not in str(md_file) and '异步I-O机制' not in str(md_file):
        # 但异步I/O机制子目录的README.md需要TOC
        if '异步I-O机制' not in str(md_file):
            continue
    ...
    try:
        ...
        if re.search(toc_pattern, content, re.DOTALL):
            ...
            if toc_match:
                ...
                # 检查TOC项数（只统计顶级项，不包括嵌套项）
                toc_lines = current_toc.split('\n')
