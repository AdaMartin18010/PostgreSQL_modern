...
class ComprehensiveTaskScanner:
    ...
    def scan_document(self, file_path):
        ...
        if len(lines) < 50:
            ...
            self.tasks['short_docs'].append(rel_path)
        
        # 检查TOC
        toc_pattern = r'^##\s*📑\s*目录'
        has_toc = bool(re.search(toc_pattern, content, re.MULTILINE))
        
        if not has_toc and file_path.name == 'README.md':
            # 只检查README.md是否有TOC
            self.stats['docs_without_toc'] += 1
            self.tasks['no_toc'].append(rel_path)
...
if __name__ == '__main__':
    ...
    scanner = ComprehensiveTaskScanner(base_dir)
    scanner.scan_directory(base_dir)
    tasks = scanner.generate_report()
    
    # 生成任务清单文件
    with open(base_dir / 'TASK_LIST.md', 'w', encoding='utf-8') as f:
        f.write("# 📋 未完成任务清单\n\n")
        f.write("> **生成时间**: 2025年1月\n")
        f.write("> **状态**: 🔄 持续更新中\n\n")
        f.write("---\n\n")
        
        f.write("## 📊 任务统计\n\n")
        f.write(f"- 总文档数: {scanner.stats['total_docs']}\n")
        f.write(f"- 总任务数: {sum(len(v) for v in tasks.values())}\n\n")
        
        f.write("## 🎯 任务分类\n\n")
        for task_type, items in sorted(tasks.items()):
            if items:
                f.write(f"### {task_type} ({len(items)}个)\n\n")
                for item in items:
                    f.write(f"- [ ] {item}\n")
                f.write("\n")
        
        f.write("---\n\n")
        f.write("**说明**: 此清单由自动化工具生成，请根据实际情况更新。\n")
