...
BEGIN
    ...
    RAISE NOTICE '表创建成功: places (SRID=4326)';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION 'GEOMETRY类型不存在，请安装PostGIS扩展';
    WHEN duplicate_table THEN
        RAISE WARNING '表places已存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conname = 'places_location_check'
    ) THEN
        ALTER TABLE places
        ADD CONSTRAINT places_location_check
        CHECK (ST_IsValid(location));
        RAISE NOTICE '约束添加成功: places_location_check';
    ELSE
        RAISE WARNING '约束places_location_check已存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表places不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'ST_IsValid函数不存在，请检查PostGIS扩展安装';
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE proname = 'validate_geometry'
    ) THEN
        DROP FUNCTION validate_geometry();
        RAISE NOTICE '已删除现有函数: validate_geometry';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
