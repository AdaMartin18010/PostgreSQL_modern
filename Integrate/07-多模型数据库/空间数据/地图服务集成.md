# 地图服务集成

> **创建日期**: 2025年1月
> **技术版本**: PostgreSQL 18+, PostGIS 3.4+
> **难度等级**: ⭐⭐⭐ 中级

---

## 📋 目录

- [地图服务集成](#地图服务集成)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. Web地图服务（WMS）](#2-web地图服务wms)
    - [2.1 使用MapServer](#21-使用mapserver)
    - [2.2 使用GeoServer](#22-使用geoserver)
  - [3. Web要素服务（WFS）](#3-web要素服务wfs)
    - [3.1 WFS查询](#31-wfs查询)
    - [3.2 WFS-T（事务）](#32-wfs-t事务)
  - [4. Tile服务](#4-tile服务)
    - [4.1 使用PostGIS生成瓦片](#41-使用postgis生成瓦片)
    - [4.2 使用pg\_tileserv](#42-使用pg_tileserv)
  - [5. GeoJSON API](#5-geojson-api)
    - [5.1 创建GeoJSON视图](#51-创建geojson视图)
    - [5.2 RESTful API示例](#52-restful-api示例)
  - [6. 最佳实践](#6-最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [❌ 避免做法](#-避免做法)
  - [📚 相关文档](#-相关文档)

---

## 1. 概述

PostGIS可以与各种地图服务集成，提供地理数据的Web服务接口。常见的地图服务包括：

- **WMS** - Web地图服务（图片）
- **WFS** - Web要素服务（矢量数据）
- **Tile服务** - 瓦片地图服务
- **GeoJSON API** - RESTful地理数据API

---

## 2. Web地图服务（WMS）

### 2.1 使用MapServer

```sql
-- 准备数据
CREATE TABLE cities (
    id SERIAL PRIMARY KEY,
    name TEXT,
    population INT,
    geom GEOMETRY(POINT, 4326)
);

-- MapServer配置示例（mapfile）
MAP
    NAME "cities"
    STATUS ON
    SIZE 800 600
    EXTENT -180 -90 180 90
    UNITS DD
    SHAPEPATH "/data"

    LAYER
        NAME "cities"
        TYPE POINT
        STATUS ON
        CONNECTIONTYPE postgis
        CONNECTION "host=localhost dbname=mydb user=postgres password=xxx"
        DATA "geom FROM cities USING UNIQUE id USING srid=4326"
        CLASS
            STYLE
                COLOR 255 0 0
                SIZE 8
            END
        END
    END
END
```

### 2.2 使用GeoServer

```sql
-- GeoServer通过JDBC连接PostGIS
-- 配置数据存储：
-- 类型: PostGIS
-- 主机: localhost
-- 端口: 5432
-- 数据库: mydb
-- 用户: postgres
-- 密码: xxx
-- 架构: public
```

---

## 3. Web要素服务（WFS）

### 3.1 WFS查询

```sql
-- WFS GetFeature请求示例
-- URL: http://localhost:8080/geoserver/wfs?
--      service=WFS&version=1.1.0&request=GetFeature&
--      typeName=cities&outputFormat=application/json

-- 对应的SQL查询
SELECT
    id,
    name,
    population,
    ST_AsGeoJSON(geom) AS geometry
FROM cities
WHERE ST_Within(geom, ST_MakeEnvelope(116.0, 39.7, 116.7, 40.2, 4326));
```

### 3.2 WFS-T（事务）

```sql
-- WFS-T插入
INSERT INTO cities (name, population, geom)
VALUES (
    'Beijing',
    21540000,
    ST_SetSRID(ST_MakePoint(116.4074, 39.9042), 4326)
);

-- WFS-T更新
UPDATE cities
SET population = 22000000
WHERE name = 'Beijing';

-- WFS-T删除
DELETE FROM cities WHERE name = 'Beijing';
```

---

## 4. Tile服务

### 4.1 使用PostGIS生成瓦片

```sql
-- 创建瓦片表
CREATE TABLE tiles (
    z INT,
    x INT,
    y INT,
    tile_data BYTEA,
    PRIMARY KEY (z, x, y)
);

-- 生成瓦片函数
CREATE OR REPLACE FUNCTION generate_tile(
    z INT,
    x INT,
    y INT
)
RETURNS BYTEA AS $$
DECLARE
    bbox GEOMETRY;
    tile_data BYTEA;
BEGIN
    -- 计算瓦片边界框
    bbox := ST_TileEnvelope(z, x, y);

    -- 查询该区域的数据并生成瓦片
    -- 这里简化处理，实际需要使用地图渲染库
    SELECT ST_AsMVT(q, 'cities', 4096, 'geom')
    INTO tile_data
    FROM (
        SELECT
            id,
            name,
            ST_AsMVTGeom(geom, bbox, 4096, 256, true) AS geom
        FROM cities
        WHERE geom && bbox
    ) q;

    RETURN tile_data;
END;
$$ LANGUAGE plpgsql;
```

### 4.2 使用pg_tileserv

```bash
# pg_tileserv是PostGIS的瓦片服务
# 安装和使用：
# https://github.com/CrunchyData/pg_tileserv

# 配置连接
export DATABASE_URL="postgresql://postgres:password@localhost/mydb"

# 启动服务
pg_tileserv serve

# 访问瓦片
# http://localhost:7800/index.html
```

---

## 5. GeoJSON API

### 5.1 创建GeoJSON视图

```sql
-- 创建GeoJSON视图
CREATE OR REPLACE VIEW cities_geojson AS
SELECT
    json_build_object(
        'type', 'FeatureCollection',
        'features', json_agg(
            json_build_object(
                'type', 'Feature',
                'id', id,
                'geometry', ST_AsGeoJSON(geom)::json,
                'properties', json_build_object(
                    'name', name,
                    'population', population
                )
            )
        )
    ) AS geojson
FROM cities;
```

### 5.2 RESTful API示例

```sql
-- 使用PostgREST或自定义API
-- 查询所有城市
SELECT
    id,
    name,
    population,
    ST_AsGeoJSON(geom)::json AS geometry
FROM cities;

-- 按区域查询
SELECT
    id,
    name,
    population,
    ST_AsGeoJSON(geom)::json AS geometry
FROM cities
WHERE ST_Within(
    geom,
    ST_MakeEnvelope(116.0, 39.7, 116.7, 40.2, 4326)
);
```

---

## 6. 最佳实践

### ✅ 推荐做法

1. **使用标准服务协议** - WMS、WFS、Tile服务
2. **优化查询性能** - 使用空间索引和边界框过滤
3. **缓存瓦片** - 对于静态数据使用缓存
4. **使用连接池** - 提高并发性能
5. **监控服务性能** - 跟踪响应时间和错误率

### ❌ 避免做法

1. **直接暴露数据库** - 使用服务层封装
2. **返回过多数据** - 使用分页和限制
3. **忽略性能优化** - 空间查询需要优化
4. **不处理错误** - 提供友好的错误信息

---

## 📚 相关文档

- [PostGIS完整深化指南.md](./PostGIS完整深化指南.md) - PostGIS完整指南
- [地理信息查询优化.md](./地理信息查询优化.md) - 查询优化指南
- [空间数据建模指南.md](./空间数据建模指南.md) - 数据建模指南

---

**最后更新**: 2025年1月
