...
> **📋 文档来源**: `PostgreSQL_View\04-多模一体化\技术原理\统一查询接口.md`
> **📅 复制日期**: 2025-12-22
...
- [统一查询接口](#统一查询接口)
  ...
  - [8. 常见问题（FAQ）](#8-常见问题faq)
    - [8.1 统一查询接口性能相关问题](#81-统一查询接口性能相关问题)
      - [Q1: 如何优化统一查询接口性能？](#q1-如何优化统一查询接口性能)
      - [Q2: 如何提升统一查询接口准确率？](#q2-如何提升统一查询接口准确率)
    - [8.2 统一查询接口算法相关问题](#82-统一查询接口算法相关问题)
      - [Q3: 如何处理复杂多模查询？](#q3-如何处理复杂多模查询)
  - [9. 完整代码示例](#9-完整代码示例)
...
```sql
-- 示例 1: JSONB + 时序 + 向量混合查询（带性能测试和错误处理）
DO $$
DECLARE
    result_count INT;
BEGIN
...
-- 计划: Index Scan + Filter

-- 示例 2: 图 + 向量联合查询（带性能测试和错误处理）
DO $$
DECLARE
    graph_count INT;
    vector_count INT;
...
-- 计划: Function Scan + Index Scan

-- 示例 3: JSONB + 全文搜索 + 向量搜索（带性能测试和错误处理）
DO $$
DECLARE
    result_count INT;
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE WARNING 'pgvector扩展未安装，向量搜索可能不可用';
    END IF;
    ...
    RAISE NOTICE '找到 % 条符合条件的文档', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表documents不存在';
    WHEN OTHERS THEN
...
1. 解析查询语句
   - 提取 WHERE 子句中的条件
   - 识别数据类型（时序、JSONB、向量、图）

2. 识别查询类型
   FOR EACH 条件 IN WHERE 子句:
       IF 条件包含 time 列 AND 使用范围操作符:
           标记为时序查询
       IF 条件包含 JSONB 列 AND 使用 @> 或 -> 操作符:
           标记为 JSONB 查询
       IF 条件包含 vector 列 AND 使用 <=> 或 <-> 操作符:
           标记为向量查询
       IF 条件包含图查询语法:
           标记为图查询
...
| 操作 | 时间复杂度 | 空间复杂度 | 说明 |
| --- | --- | --- | --- |
| **查询解析** | O(n) | O(n) | n 为查询语句长度 |
| **类型识别** | O(m) | O(1) | m 为条件数量 |
| **执行计划生成** | O(m log m) | O(m) | 需要排序和优化 |
| **分区裁剪** | O(log p) | O(1) | p 为分区数量 |
| **索引选择** | O(k) | O(k) | k 为可用索引数量 |

**性能优化技巧**:

1. **条件重排序**: 将选择性高的条件放在前面
2. **索引组合**: 使用复合索引加速多条件查询
3. **结果缓存**: 缓存常用查询结果
4. **批量处理**: 批量处理相似查询
...
| 优化措施 | 优化前延迟 | 优化后延迟 | 提升 |
|---------|-----------|-----------|------|
| **优化路由** | 100ms | **<30ms** | **70%** ⬇️ |
| **并行执行** | 200ms | **<50ms** | **75%** ⬇️ |
