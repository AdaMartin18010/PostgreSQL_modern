...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND indexname = 'idx_user_profiles_gin'
    ) THEN
        CREATE INDEX idx_user_profiles_gin ON user_profiles USING GIN (profile);
        RAISE NOTICE '索引创建成功: idx_user_profiles_gin';
    END IF;
EXCEPTION
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_profiles') THEN
        RAISE WARNING '表user_profiles不存在';
        RETURN;
    END IF;
...
BEGIN
    ...
    RAISE NOTICE '关系创建成功，影响 % 行', result_count;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'cypher函数不存在，请检查AGE扩展安装';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建关系失败: %', SQLERRM;
END $$;
