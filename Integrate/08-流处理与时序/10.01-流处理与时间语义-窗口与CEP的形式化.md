...
流处理系统需要处理无界数据流，时间语义是流处理的核心概念。
本文档提供流处理时间语义和窗口操作的形式化模型，包括事件时间、处理时间、窗口操作和CEP（复杂事件处理）的形式化定义。
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_readings') THEN
        ...
        RAISE NOTICE '已删除现有表: sensor_readings';
    END IF;
    ...
    RAISE NOTICE '表创建成功: sensor_readings';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表sensor_readings已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表失败: %', SQLERRM;
END $$;

-- 转换为超表（TimescaleDB，带错误处理）
DO $$
DECLARE
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_readings') THEN
        RAISE EXCEPTION '表sensor_readings不存在，请先创建';
    END IF;
    ...
    IF NOT hypertable_exists THEN
        ...
        RAISE NOTICE 'Hypertable创建成功: sensor_readings';
    ELSE
        RAISE WARNING 'Hypertable sensor_readings已存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表sensor_readings不存在';
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Hypertable sensor_readings不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'sensor_hourly_stats'
    ) THEN
        DROP MATERIALIZED VIEW sensor_hourly_stats;
        RAISE NOTICE '已删除现有物化视图: sensor_hourly_stats';
    END IF;
    ...
    RAISE NOTICE '连续聚合视图创建成功: sensor_hourly_stats';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表sensor_readings不存在';
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '物化视图sensor_hourly_stats不存在，请先创建';
    END IF;

    SELECT EXISTS (
        SELECT 1 FROM timescaledb_information.jobs
        WHERE hypertable_name = 'sensor_hourly_stats'
        AND proc_name = 'policy_refresh_continuous_aggregate'
    ) INTO policy_exists;

    IF NOT policy_exists THEN
        PERFORM add_continuous_aggregate_policy('sensor_hourly_stats',
            start_offset => INTERVAL '3 hours',
            end_offset => INTERVAL '1 hour',
            schedule_interval => INTERVAL '1 hour'
        );
        RAISE NOTICE '连续聚合刷新策略添加成功: sensor_hourly_stats';
    ELSE
        RAISE WARNING '刷新策略已存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '物化视图sensor_hourly_stats不存在';
    WHEN undefined_function THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '物化视图sensor_hourly_stats不存在';
        RETURN;
    END IF;
    ...
    RAISE NOTICE '窗口聚合查询完成: % 条记录', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '物化视图sensor_hourly_stats不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询窗口聚合结果失败: %', SQLERRM;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'iot_sensors') THEN
        ...
        RAISE NOTICE '已删除现有表: iot_sensors';
    END IF;
    ...
    RAISE NOTICE '表创建成功: iot_sensors';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表iot_sensors已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表失败: %', SQLERRM;
END $$;
...
DECLARE
    hypertable_exists BOOLEAN;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'iot_sensors') THEN
        RAISE EXCEPTION '表iot_sensors不存在，请先创建';
    END IF;
    ...
    IF NOT hypertable_exists THEN
        PERFORM create_hypertable('iot_sensors', 'time', chunk_time_interval => INTERVAL '1 day');
        RAISE NOTICE 'Hypertable创建成功: iot_sensors';
    ELSE
        RAISE WARNING 'Hypertable iot_sensors已存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表iot_sensors不存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION 'create_hypertable函数不存在，请检查TimescaleDB扩展安装';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建Hypertable失败: %', SQLERRM;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
        AND tablename = 'iot_sensors'
        AND indexname = 'idx_iot_sensors_device_time'
    ) THEN
        CREATE INDEX idx_iot_sensors_device_time ON iot_sensors(device_id, time DESC);
        RAISE NOTICE '索引创建成功: idx_iot_sensors_device_time';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
        AND tablename = 'iot_sensors'
        AND indexname = 'idx_iot_sensors_type_time'
    ) THEN
        CREATE INDEX idx_iot_sensors_type_time ON iot_sensors(sensor_type, time DESC);
        RAISE NOTICE '索引创建成功: idx_iot_sensors_type_time';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表iot_sensors不存在';
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Hypertable iot_sensors不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'sensor_5min_sliding'
    ) THEN
        DROP MATERIALIZED VIEW sensor_5min_sliding;
        RAISE NOTICE '已删除现有物化视图: sensor_5min_sliding';
    END IF;
