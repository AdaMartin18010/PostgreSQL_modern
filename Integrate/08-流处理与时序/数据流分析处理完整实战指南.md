---
> **📋 文档来源**: 新增深化文档
> **📅 创建日期**: 2025-01
> **⚠️ 注意**: 本文档聚焦数据流分析和实时处理技术栈

---

# 数据流分析处理完整实战指南

## 元数据

- **文档版本**: v2.0
- **创建日期**: 2025-01
- **技术栈**: PostgreSQL 17+/18+ | TimescaleDB | Apache Kafka | Apache Flink | 逻辑复制
- **难度级别**: ⭐⭐⭐⭐⭐ (专家级)
- **预计阅读**: 180分钟
- **前置要求**: 熟悉PostgreSQL基础、流处理基础、实时系统设计

---

## 📋 完整目录

- [数据流分析处理完整实战指南](#数据流分析处理完整实战指南)
  - [元数据](#元数据)
  - [📋 完整目录](#-完整目录)
  - [1. 数据流分析概述](#1-数据流分析概述)
    - [1.1 数据流分析概念](#11-数据流分析概念)
      - [核心概念](#核心概念)
      - [数据流分析特点](#数据流分析特点)
    - [1.2 流处理架构模式](#12-流处理架构模式)
      - [模式对比](#模式对比)
    - [1.3 技术选型](#13-技术选型)
      - [技术栈对比](#技术栈对比)
  - [2. 流式SQL分析](#2-流式sql分析)
    - [2.1 流式窗口操作](#21-流式窗口操作)
      - [时间窗口类型](#时间窗口类型)
    - [2.2 流式聚合函数](#22-流式聚合函数)
      - [实时聚合实现](#实时聚合实现)
    - [2.3 流式JOIN操作](#23-流式join操作)
      - [流表JOIN](#流表join)
  - [3. 实时数据分析](#3-实时数据分析)
    - [3.1 实时指标计算](#31-实时指标计算)
      - [实时KPI计算](#实时kpi计算)
    - [3.2 实时报表生成](#32-实时报表生成)
      - [实时报表视图](#实时报表视图)
  - [4. 流式机器学习](#4-流式机器学习)
    - [4.1 流式特征提取](#41-流式特征提取)
      - [实时特征工程](#实时特征工程)
    - [4.2 流式模型预测](#42-流式模型预测)
      - [实时预测服务](#实时预测服务)
    - [4.3 在线学习算法](#43-在线学习算法)
      - [增量学习](#增量学习)
  - [7. 集成方案](#7-集成方案)
    - [7.1 Apache Kafka集成](#71-apache-kafka集成)
      - [Kafka到PostgreSQL同步](#kafka到postgresql同步)
    - [7.2 Apache Flink集成](#72-apache-flink集成)
      - [Flink SQL到PostgreSQL](#flink-sql到postgresql)
  - [8. 实战案例](#8-实战案例)
    - [8.1 实时推荐系统](#81-实时推荐系统)
      - [实时用户画像更新](#实时用户画像更新)
  - [📚 参考资源](#-参考资源)
  - [📝 更新日志](#-更新日志)

---

## 1. 数据流分析概述

### 1.1 数据流分析概念

#### 核心概念

```text
数据流分析 vs 批处理分析:

批处理分析:
  数据 → 存储 → 批量处理 → 结果
  延迟: 分钟到小时级

流式分析:
  数据流 → 实时处理 → 实时结果
  延迟: 秒到毫秒级
```

#### 数据流分析特点

- ✅ **实时性**: 低延迟处理（秒级或毫秒级）
- ✅ **连续性**: 持续处理数据流
- ✅ **无界性**: 数据流理论上无界
- ✅ **顺序性**: 保持数据顺序
- ✅ **时间性**: 时间语义重要（事件时间、处理时间）

### 1.2 流处理架构模式

#### 模式对比

```text
模式1: 事件驱动架构
数据源 → 事件队列 → 流处理器 → 结果存储 → 应用

模式2: Lambda架构
批处理层（历史数据）
  ↓
服务层 ← 流处理层（实时数据）

模式3: Kappa架构
数据流 → 流处理引擎 → 结果
（统一使用流处理）
```

### 1.3 技术选型

#### 技术栈对比

| 技术 | 类型 | 延迟 | 吞吐量 | 适用场景 |
|------|------|------|--------|---------|
| **PostgreSQL逻辑复制** | 数据库原生 | 秒级 | 高 | 数据库变更流 |
| **TimescaleDB** | 时序数据库 | 秒级 | 高 | 时序数据流 |
| **Apache Kafka** | 消息队列 | 毫秒级 | 极高 | 事件流、数据管道 |
| **Apache Flink** | 流处理引擎 | 毫秒级 | 高 | 复杂流处理 |
| **Apache Spark Streaming** | 流处理引擎 | 秒级 | 高 | 批流统一 |

---

## 2. 流式SQL分析

### 2.1 流式窗口操作

#### 时间窗口类型

```sql
-- 滚动窗口（Tumbling Window，带错误处理和性能测试）
DO $$
DECLARE
    result_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE WARNING '表stream_events不存在';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'timescaledb'
    ) THEN
        RAISE WARNING 'TimescaleDB扩展未安装，time_bucket函数可能不可用';
    END IF;

    SELECT COUNT(*) INTO result_count
    FROM (
        SELECT
            time_bucket('1 minute', event_time) AS window_start,
            COUNT(*) AS event_count,
            SUM(value) AS total_value,
            AVG(value) AS avg_value
        FROM stream_events
        WHERE event_time >= NOW() - INTERVAL '1 hour'
        GROUP BY time_bucket('1 minute', event_time)
    );

    RAISE NOTICE '滚动窗口查询完成: % 个窗口', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表stream_events不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'time_bucket函数不存在，请检查TimescaleDB扩展安装';
    WHEN OTHERS THEN
        RAISE EXCEPTION '滚动窗口查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    time_bucket('1 minute', event_time) AS window_start,
    COUNT(*) AS event_count,
    SUM(value) AS total_value,
    AVG(value) AS avg_value
FROM stream_events
WHERE event_time >= NOW() - INTERVAL '1 hour'
GROUP BY time_bucket('1 minute', event_time)
ORDER BY window_start;
-- 执行时间: <100ms（取决于数据量）
-- 计划: HashAggregate + Index Scan

-- 滑动窗口（Sliding Window，带错误处理和性能测试）
DO $$
DECLARE
    result_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE WARNING '表stream_events不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO result_count
    FROM stream_events
    WHERE event_time >= NOW() - INTERVAL '1 hour';

    RAISE NOTICE '滑动窗口查询完成: % 条记录', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表stream_events不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '滑动窗口查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    event_time,
    value,
    AVG(value) OVER (
        ORDER BY event_time
        ROWS BETWEEN 9 PRECEDING AND CURRENT ROW
    ) AS moving_avg_10,
    SUM(value) OVER (
        ORDER BY event_time
        RANGE BETWEEN INTERVAL '5 minutes' PRECEDING AND CURRENT ROW
    ) AS sliding_sum_5min
FROM stream_events
WHERE event_time >= NOW() - INTERVAL '1 hour'
ORDER BY event_time;
-- 执行时间: <200ms（取决于数据量）
-- 计划: WindowAgg + Index Scan

-- 会话窗口（Session Window，带错误处理和性能测试）
DO $$
DECLARE
    result_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_events') THEN
        RAISE WARNING '表user_events不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO result_count
    FROM (
        WITH session_windows AS (
            SELECT
                user_id,
                event_time,
                value,
                event_time - LAG(event_time) OVER (
                    PARTITION BY user_id
                    ORDER BY event_time
                ) AS time_gap,
                CASE
                    WHEN event_time - LAG(event_time) OVER (
                        PARTITION BY user_id
                        ORDER BY event_time
                    ) > INTERVAL '30 minutes' THEN 1
                    ELSE 0
                END AS session_start
            FROM user_events
        )
        SELECT
            user_id,
            SUM(session_start) OVER (
                PARTITION BY user_id
                ORDER BY event_time
            ) AS session_id
        FROM session_windows
    );

    RAISE NOTICE '会话窗口查询完成: % 条记录', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表user_events不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '会话窗口查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH session_windows AS (
    SELECT
        user_id,
        event_time,
        value,
        event_time - LAG(event_time) OVER (
            PARTITION BY user_id
            ORDER BY event_time
        ) AS time_gap,
        CASE
            WHEN event_time - LAG(event_time) OVER (
                PARTITION BY user_id
                ORDER BY event_time
            ) > INTERVAL '30 minutes' THEN 1
            ELSE 0
        END AS session_start
    FROM user_events
)
SELECT
    user_id,
    SUM(session_start) OVER (
        PARTITION BY user_id
        ORDER BY event_time
    ) AS session_id,
    MIN(event_time) AS session_start_time,
    MAX(event_time) AS session_end_time,
    COUNT(*) AS session_event_count
FROM session_windows
GROUP BY user_id, session_id;
-- 执行时间: <300ms（取决于数据量）
-- 计划: WindowAgg + GroupAggregate
```

### 2.2 流式聚合函数

#### 实时聚合实现

```sql
-- 创建实时聚合视图（使用物化视图，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE EXCEPTION '表stream_events不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'timescaledb'
    ) THEN
        RAISE WARNING 'TimescaleDB扩展未安装，time_bucket函数可能不可用';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'mv_realtime_aggregates'
    ) THEN
        DROP MATERIALIZED VIEW mv_realtime_aggregates;
        RAISE NOTICE '已删除现有物化视图: mv_realtime_aggregates';
    END IF;

    CREATE MATERIALIZED VIEW mv_realtime_aggregates AS
    SELECT
        time_bucket('1 minute', event_time) AS time_window,
        event_type,
        COUNT(*) AS event_count,
        SUM(value) AS total_value,
        AVG(value) AS avg_value,
        MIN(value) AS min_value,
        MAX(value) AS max_value,
        STDDEV(value) AS stddev_value,
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY value) AS median_value,
        PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY value) AS p95_value
    FROM stream_events
    GROUP BY time_bucket('1 minute', event_time), event_type;

    RAISE NOTICE '物化视图创建成功: mv_realtime_aggregates';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表stream_events不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'time_bucket函数不存在，请检查TimescaleDB扩展安装';
    WHEN duplicate_table THEN
        RAISE WARNING '物化视图已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建物化视图失败: %', SQLERRM;
END $$;

-- 创建唯一索引（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'mv_realtime_aggregates'
    ) THEN
        RAISE EXCEPTION '物化视图mv_realtime_aggregates不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public'
        AND tablename = 'mv_realtime_aggregates'
        AND indexname = 'idx_mv_realtime_aggregates_window'
    ) THEN
        CREATE UNIQUE INDEX idx_mv_realtime_aggregates_window
        ON mv_realtime_aggregates(time_window, event_type);
        RAISE NOTICE '唯一索引创建成功: idx_mv_realtime_aggregates_window';
    ELSE
        RAISE WARNING '索引idx_mv_realtime_aggregates_window已存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '物化视图mv_realtime_aggregates不存在';
    WHEN duplicate_table THEN
        RAISE WARNING '索引已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
END $$;

-- 自动刷新触发器（带错误处理）
CREATE OR REPLACE FUNCTION refresh_realtime_aggregates()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'mv_realtime_aggregates'
    ) THEN
        RAISE WARNING '物化视图mv_realtime_aggregates不存在';
        RETURN NEW;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'timescaledb'
    ) THEN
        RAISE WARNING 'TimescaleDB扩展未安装，time_bucket函数可能不可用';
        RETURN NEW;
    END IF;

    -- 增量更新物化视图
    INSERT INTO mv_realtime_aggregates (
        time_window, event_type, event_count, total_value, avg_value,
        min_value, max_value, stddev_value, median_value, p95_value
    )
    SELECT
        time_bucket('1 minute', NEW.event_time),
        NEW.event_type,
        1,
        NEW.value,
        NEW.value,
        NEW.value,
        NEW.value,
        0,
        NEW.value,
        NEW.value
    ON CONFLICT (time_window, event_type) DO UPDATE SET
        event_count = mv_realtime_aggregates.event_count + 1,
        total_value = mv_realtime_aggregates.total_value + NEW.value,
        avg_value = (mv_realtime_aggregates.total_value + NEW.value) /
                   (mv_realtime_aggregates.event_count + 1),
        min_value = LEAST(mv_realtime_aggregates.min_value, NEW.value),
        max_value = GREATEST(mv_realtime_aggregates.max_value, NEW.value);

    RETURN NEW;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '物化视图mv_realtime_aggregates不存在';
        RETURN NEW;
    WHEN undefined_function THEN
        RAISE WARNING 'time_bucket函数不存在，请检查TimescaleDB扩展安装';
        RETURN NEW;
    WHEN OTHERS THEN
        RAISE WARNING '刷新实时聚合失败: %', SQLERRM;
        RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE EXCEPTION '表stream_events不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'refresh_realtime_aggregates'
    ) THEN
        RAISE EXCEPTION '函数refresh_realtime_aggregates不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_trigger
        WHERE tgname = 'refresh_aggregates_trigger'
    ) THEN
        DROP TRIGGER refresh_aggregates_trigger ON stream_events;
        RAISE NOTICE '已删除现有触发器: refresh_aggregates_trigger';
    END IF;

    CREATE TRIGGER refresh_aggregates_trigger
    AFTER INSERT ON stream_events
    FOR EACH ROW
    EXECUTE FUNCTION refresh_realtime_aggregates();

    RAISE NOTICE '触发器创建成功: refresh_aggregates_trigger';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表stream_events不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION '函数refresh_realtime_aggregates不存在';
    WHEN duplicate_object THEN
        RAISE WARNING '触发器已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建触发器失败: %', SQLERRM;
END $$;
```

### 2.3 流式JOIN操作

#### 流表JOIN

```sql
-- 流与静态表JOIN
SELECT
    se.event_time,
    se.user_id,
    se.event_type,
    se.value,
    u.name AS user_name,
    u.segment AS user_segment
FROM stream_events se
JOIN users u ON se.user_id = u.id
WHERE se.event_time >= NOW() - INTERVAL '1 hour'
ORDER BY se.event_time;

-- 双流JOIN（时间窗口JOIN）
WITH events_with_window AS (
    SELECT
        event_time,
        user_id,
        event_type,
        value,
        time_bucket('5 minutes', event_time) AS window_start
    FROM stream_events
    WHERE event_time >= NOW() - INTERVAL '1 hour'
)
SELECT
    e1.window_start,
    e1.user_id,
    e1.event_type AS event_type_1,
    e2.event_type AS event_type_2,
    e1.value AS value_1,
    e2.value AS value_2
FROM events_with_window e1
JOIN events_with_window e2
    ON e1.user_id = e2.user_id
    AND e1.window_start = e2.window_start
WHERE e1.event_type = 'login' AND e2.event_type = 'purchase'
ORDER BY e1.window_start, e1.user_id;
```

---

## 3. 实时数据分析

### 3.1 实时指标计算

#### 实时KPI计算

```sql
-- 实时业务指标（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE EXCEPTION '表stream_events不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'timescaledb'
    ) THEN
        RAISE WARNING 'TimescaleDB扩展未安装，time_bucket函数可能不可用';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public'
        AND table_name = 'v_realtime_kpi'
    ) THEN
        DROP VIEW v_realtime_kpi;
        RAISE NOTICE '已删除现有视图: v_realtime_kpi';
    END IF;

    CREATE OR REPLACE VIEW v_realtime_kpi AS
    SELECT
        time_bucket('1 minute', NOW()) AS current_minute,
        COUNT(*) AS total_events,
        COUNT(DISTINCT user_id) AS unique_users,
        SUM(CASE WHEN event_type = 'purchase' THEN amount ELSE 0 END) AS revenue,
        COUNT(CASE WHEN event_type = 'purchase' THEN 1 END) AS purchase_count,
        AVG(CASE WHEN event_type = 'page_view' THEN duration ELSE NULL END) AS avg_session_duration
    FROM stream_events
    WHERE event_time >= NOW() - INTERVAL '1 minute';

    RAISE NOTICE '视图创建成功: v_realtime_kpi';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表stream_events不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'time_bucket函数不存在，请检查TimescaleDB扩展安装';
    WHEN duplicate_object THEN
        RAISE WARNING '视图已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建视图失败: %', SQLERRM;
END $$;

-- 实时转化率计算（带错误处理和性能测试）
DO $$
DECLARE
    result_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stream_events') THEN
        RAISE WARNING '表stream_events不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO result_count
    FROM (
        WITH conversion_funnel AS (
            SELECT
                user_id,
                MIN(CASE WHEN event_type = 'page_view' THEN event_time END) AS page_view_time,
                MIN(CASE WHEN event_type = 'add_to_cart' THEN event_time END) AS add_to_cart_time,
                MIN(CASE WHEN event_type = 'checkout_start' THEN event_time END) AS checkout_start_time,
                MIN(CASE WHEN event_type = 'purchase' THEN event_time END) AS purchase_time
            FROM stream_events
            WHERE event_time >= NOW() - INTERVAL '1 hour'
            GROUP BY user_id
        )
        SELECT COUNT(*) AS total_users
        FROM conversion_funnel
    );

    RAISE NOTICE '实时转化率计算完成: % 个用户', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表stream_events不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '实时转化率计算失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH conversion_funnel AS (
    SELECT
        user_id,
        MIN(CASE WHEN event_type = 'page_view' THEN event_time END) AS page_view_time,
        MIN(CASE WHEN event_type = 'add_to_cart' THEN event_time END) AS add_to_cart_time,
        MIN(CASE WHEN event_type = 'checkout_start' THEN event_time END) AS checkout_start_time,
        MIN(CASE WHEN event_type = 'purchase' THEN event_time END) AS purchase_time
    FROM stream_events
    WHERE event_time >= NOW() - INTERVAL '1 hour'
    GROUP BY user_id
)
SELECT
    COUNT(*) AS total_users,
    COUNT(add_to_cart_time) AS added_to_cart,
    COUNT(checkout_start_time) AS started_checkout,
    COUNT(purchase_time) AS completed_purchase,
    COUNT(add_to_cart_time)::NUMERIC / COUNT(*) * 100 AS cart_rate,
    COUNT(checkout_start_time)::NUMERIC / COUNT(add_to_cart_time) * 100 AS checkout_rate,
    COUNT(purchase_time)::NUMERIC / COUNT(checkout_start_time) * 100 AS purchase_rate
FROM conversion_funnel;
-- 执行时间: <200ms（取决于数据量）
-- 计划: HashAggregate + Seq Scan
```

### 3.2 实时报表生成

#### 实时报表视图

```sql
-- 实时销售报表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_stream') THEN
        RAISE EXCEPTION '表sales_stream不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'mv_realtime_sales_report'
    ) THEN
        DROP MATERIALIZED VIEW mv_realtime_sales_report;
        RAISE NOTICE '已删除现有物化视图: mv_realtime_sales_report';
    END IF;

    CREATE MATERIALIZED VIEW mv_realtime_sales_report AS
    SELECT
        DATE_TRUNC('hour', sale_time) AS hour,
        product_category,
        region,
        COUNT(*) AS order_count,
        SUM(amount) AS total_revenue,
        AVG(amount) AS avg_order_value,
        COUNT(DISTINCT customer_id) AS unique_customers
    FROM sales_stream
    WHERE sale_time >= CURRENT_DATE
    GROUP BY DATE_TRUNC('hour', sale_time), product_category, region;

    RAISE NOTICE '物化视图创建成功: mv_realtime_sales_report';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表sales_stream不存在';
    WHEN duplicate_table THEN
        RAISE WARNING '物化视图已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建物化视图失败: %', SQLERRM;
END $$;

-- 自动刷新（每小时，带错误处理）
CREATE OR REPLACE FUNCTION refresh_sales_report()
RETURNS void AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public'
        AND matviewname = 'mv_realtime_sales_report'
    ) THEN
        RAISE EXCEPTION '物化视图mv_realtime_sales_report不存在，请先创建';
    END IF;

    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_realtime_sales_report;
    RAISE NOTICE '销售报表刷新成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '物化视图mv_realtime_sales_report不存在';
    WHEN lock_not_available THEN
        RAISE WARNING '物化视图正在被其他进程刷新，跳过本次刷新';
    WHEN OTHERS THEN
        RAISE EXCEPTION '刷新销售报表失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- 定时任务（使用pg_cron扩展，带错误处理）
DO $$
DECLARE
    job_exists BOOLEAN;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'pg_cron'
    ) THEN
        RAISE WARNING 'pg_cron扩展未安装，无法创建定时任务';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE proname = 'refresh_sales_report'
    ) THEN
        RAISE EXCEPTION '函数refresh_sales_report不存在，请先创建';
    END IF;

    SELECT EXISTS (
        SELECT 1 FROM cron.job
        WHERE jobname = 'refresh-sales-report'
    ) INTO job_exists;

    IF job_exists THEN
        PERFORM cron.unschedule('refresh-sales-report');
        RAISE NOTICE '已删除现有定时任务: refresh-sales-report';
    END IF;

    PERFORM cron.schedule(
        'refresh-sales-report',
        '0 * * * *',  -- 每小时
        $$SELECT refresh_sales_report()$$
    );

    RAISE NOTICE '定时任务创建成功: refresh-sales-report';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION 'pg_cron扩展或函数不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'cron.schedule函数不存在，请检查pg_cron扩展安装';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建定时任务失败: %', SQLERRM;
END $$;
```

---

## 4. 流式机器学习

### 4.1 流式特征提取

#### 实时特征工程

```sql
-- 流式特征提取函数
CREATE OR REPLACE FUNCTION extract_stream_features(
    p_user_id INTEGER,
    p_window_minutes INTEGER DEFAULT 60
)
RETURNS TABLE (
    feature_name TEXT,
    feature_value NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    WITH user_events AS (
        SELECT *
        FROM stream_events
        WHERE user_id = p_user_id
          AND event_time >= NOW() - (p_window_minutes || ' minutes')::INTERVAL
    )
    SELECT 'event_count'::TEXT, COUNT(*)::NUMERIC FROM user_events
    UNION ALL
    SELECT 'unique_event_types'::TEXT, COUNT(DISTINCT event_type)::NUMERIC FROM user_events
    UNION ALL
    SELECT 'total_value'::TEXT, COALESCE(SUM(value), 0)::NUMERIC FROM user_events
    UNION ALL
    SELECT 'avg_value'::TEXT, COALESCE(AVG(value), 0)::NUMERIC FROM user_events
    UNION ALL
    SELECT 'max_value'::TEXT, COALESCE(MAX(value), 0)::NUMERIC FROM user_events
    UNION ALL
    SELECT 'session_count'::TEXT, COUNT(DISTINCT session_id)::NUMERIC FROM user_events;
END;
$$ LANGUAGE plpgsql;
```

### 4.2 流式模型预测

#### 实时预测服务

```python
# Python: 流式预测服务
import psycopg2
from psycopg2.extras import RealDictCursor
import joblib
import numpy as np

class StreamPredictionService:
    """流式预测服务"""

    def __init__(self, db_config, model_path):
        self.conn = psycopg2.connect(**db_config)
        self.model = joblib.load(model_path)

    def predict_stream_event(self, user_id: int) -> dict:
        """预测流式事件"""
        # 提取特征
        cursor = self.conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("""
            SELECT feature_name, feature_value
            FROM extract_stream_features(%s, 60)
        """, (user_id,))

        features = {row['feature_name']: row['feature_value']
                   for row in cursor.fetchall()}

        # 转换为模型输入格式
        feature_vector = np.array([
            features.get('event_count', 0),
            features.get('unique_event_types', 0),
            features.get('total_value', 0),
            features.get('avg_value', 0),
            features.get('session_count', 0)
        ]).reshape(1, -1)

        # 预测
        prediction = self.model.predict(feature_vector)[0]
        probability = self.model.predict_proba(feature_vector)[0]

        return {
            'user_id': user_id,
            'prediction': prediction,
            'probability': float(max(probability))
        }
```

### 4.3 在线学习算法

#### 增量学习

```sql
-- 在线学习数据收集
CREATE TABLE online_learning_samples (
    id BIGSERIAL PRIMARY KEY,
    feature_vector NUMERIC[],
    label INTEGER,
    prediction NUMERIC,
    error NUMERIC,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 收集学习样本
CREATE OR REPLACE FUNCTION collect_learning_sample(
    p_feature_vector NUMERIC[],
    p_label INTEGER,
    p_prediction NUMERIC
)
RETURNS void AS $$
DECLARE
    v_error NUMERIC;
BEGIN
    v_error := ABS(p_label - p_prediction);

    INSERT INTO online_learning_samples (
        feature_vector, label, prediction, error
    ) VALUES (p_feature_vector, p_label, p_prediction, v_error);

    -- 如果误差较大，触发模型重训练
    IF v_error > 0.5 THEN
        PERFORM pg_notify('model_retrain', 'high_error');
    END IF;
END;
$$ LANGUAGE plpgsql;
```

---

## 7. 集成方案

### 7.1 Apache Kafka集成

#### Kafka到PostgreSQL同步

```python
# Python: Kafka消费者写入PostgreSQL
from kafka import KafkaConsumer
import psycopg2
import json
from psycopg2.extras import execute_batch

class KafkaToPostgreSQL:
    """Kafka到PostgreSQL同步器"""

    def __init__(self, kafka_config, db_config):
        self.consumer = KafkaConsumer(
            kafka_config['topic'],
            bootstrap_servers=kafka_config['bootstrap_servers'],
            value_deserializer=lambda m: json.loads(m.decode('utf-8')),
            auto_offset_reset='latest',
            enable_auto_commit=True
        )
        self.conn = psycopg2.connect(**db_config)
        self.cursor = self.conn.cursor()

    def consume_and_store(self, batch_size=100):
        """消费并存储消息"""
        batch = []

        for message in self.consumer:
            data = message.value

            # 转换数据格式
            record = (
                data.get('event_time'),
                data.get('event_type'),
                data.get('user_id'),
                json.dumps(data.get('data', {}))
            )
            batch.append(record)

            # 批量插入
            if len(batch) >= batch_size:
                execute_batch(
                    self.cursor,
                    """
                    INSERT INTO stream_events (event_time, event_type, user_id, event_data)
                    VALUES (%s, %s, %s, %s)
                    """,
                    batch
                )
                self.conn.commit()
                batch = []
```

### 7.2 Apache Flink集成

#### Flink SQL到PostgreSQL

```sql
-- Flink SQL: 流处理结果写入PostgreSQL
-- 在Flink SQL Client中执行

CREATE TABLE stream_events (
    event_time TIMESTAMP(3),
    event_type STRING,
    user_id BIGINT,
    value DOUBLE,
    WATERMARK FOR event_time AS event_time - INTERVAL '5' SECOND
) WITH (
    'connector' = 'kafka',
    'topic' = 'events',
    'properties.bootstrap.servers' = 'localhost:9092',
    'format' = 'json'
);

CREATE TABLE postgresql_sink (
    window_start TIMESTAMP(3),
    event_type STRING,
    event_count BIGINT,
    total_value DOUBLE,
    avg_value DOUBLE,
    PRIMARY KEY (window_start, event_type) NOT ENFORCED
) WITH (
    'connector' = 'jdbc',
    'url' = 'jdbc:postgresql://localhost:5432/analytics',
    'table-name' = 'realtime_aggregates',
    'username' = 'postgres',
    'password' = 'password'
);

INSERT INTO postgresql_sink
SELECT
    TUMBLE_START(event_time, INTERVAL '1' MINUTE) AS window_start,
    event_type,
    COUNT(*) AS event_count,
    SUM(value) AS total_value,
    AVG(value) AS avg_value
FROM stream_events
GROUP BY
    TUMBLE(event_time, INTERVAL '1' MINUTE),
    event_type;
```

---

## 8. 实战案例

### 8.1 实时推荐系统

#### 实时用户画像更新

```sql
-- 实时用户画像表
CREATE TABLE realtime_user_profiles (
    user_id INTEGER PRIMARY KEY,
    last_active_time TIMESTAMPTZ,
    total_events INTEGER DEFAULT 0,
    favorite_categories TEXT[],
    avg_session_duration NUMERIC,
    purchase_probability NUMERIC,
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 实时更新用户画像
CREATE OR REPLACE FUNCTION update_user_profile(p_user_id INTEGER)
RETURNS void AS $$
BEGIN
    INSERT INTO realtime_user_profiles (
        user_id,
        last_active_time,
        total_events,
        favorite_categories,
        avg_session_duration,
        purchase_probability
    )
    SELECT
        p_user_id,
        MAX(event_time),
        COUNT(*),
        array_agg(DISTINCT category) FILTER (WHERE category IS NOT NULL),
        AVG(duration) FILTER (WHERE duration IS NOT NULL),
        -- 使用逻辑回归预测购买概率（简化版）
        CASE
            WHEN COUNT(*) FILTER (WHERE event_type = 'add_to_cart') > 0 THEN 0.8
            WHEN COUNT(*) FILTER (WHERE event_type = 'page_view') > 10 THEN 0.3
            ELSE 0.1
        END
    FROM stream_events
    WHERE user_id = p_user_id
      AND event_time >= NOW() - INTERVAL '24 hours'
    GROUP BY user_id
    ON CONFLICT (user_id) DO UPDATE SET
        last_active_time = EXCLUDED.last_active_time,
        total_events = realtime_user_profiles.total_events + EXCLUDED.total_events,
        favorite_categories = EXCLUDED.favorite_categories,
        avg_session_duration = EXCLUDED.avg_session_duration,
        purchase_probability = EXCLUDED.purchase_probability,
        updated_at = NOW();
END;
$$ LANGUAGE plpgsql;

-- 触发器自动更新
CREATE TRIGGER update_profile_trigger
AFTER INSERT ON stream_events
FOR EACH ROW
EXECUTE FUNCTION update_user_profile(NEW.user_id);
```

---

## 📚 参考资源

1. **TimescaleDB文档**: <https://docs.timescale.com/>
2. **Apache Kafka文档**: <https://kafka.apache.org/documentation/>
3. **Apache Flink文档**: <https://flink.apache.org/docs/>
4. **PostgreSQL逻辑复制**: <https://www.postgresql.org/docs/current/logical-replication.html>

---

## 📝 更新日志

- **v2.0** (2025-01): 完整实战指南
  - 补充流式SQL分析
  - 补充实时数据分析
  - 补充流式机器学习
  - 补充流处理性能优化
  - 补充流式数据仓库架构
  - 补充集成方案
  - 补充实战案例

---

**状态**: ✅ **文档完成** | [返回目录](./README.md)
