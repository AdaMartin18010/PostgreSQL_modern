---
> **📋 文档来源**: 新增深化文档
> **📅 创建日期**: 2025-01
> **⚠️ 注意**: 本文档聚焦时序数据分析和预测技术栈

---

# 时序数据分析完整实战指南

## 元数据

- **文档版本**: v2.0
- **创建日期**: 2025-01
- **技术栈**: PostgreSQL 17+/18+ | TimescaleDB | Python | Prophet | ARIMA | LSTM
- **难度级别**: ⭐⭐⭐⭐⭐ (专家级)
- **预计阅读**: 160分钟
- **前置要求**: 熟悉PostgreSQL基础、时序数据基础、机器学习基础

---

## 📋 完整目录

- [时序数据分析完整实战指南](#时序数据分析完整实战指南)
  - [元数据](#元数据)
  - [📋 完整目录](#-完整目录)
  - [1. 时序数据分析概述](#1-时序数据分析概述)
    - [1.1 时序数据特点](#11-时序数据特点)
      - [核心特征](#核心特征)
      - [时序数据模型](#时序数据模型)
    - [1.2 分析目标](#12-分析目标)
      - [分析目标分类](#分析目标分类)
    - [1.3 TimescaleDB概述](#13-timescaledb概述)
      - [TimescaleDB核心特性](#timescaledb核心特性)
  - [2. 时序分析方法](#2-时序分析方法)
    - [2.1 趋势分析](#21-趋势分析)
      - [线性趋势检测](#线性趋势检测)
      - [移动平均趋势](#移动平均趋势)
    - [2.2 季节性分析](#22-季节性分析)
      - [季节性分解](#季节性分解)
    - [2.3 周期性检测](#23-周期性检测)
      - [自相关分析](#自相关分析)
  - [3. 时序预测模型](#3-时序预测模型)
    - [3.1 ARIMA模型](#31-arima模型)
      - [ARIMA模型实现（PL/Python）](#arima模型实现plpython)
    - [3.3 Prophet模型](#33-prophet模型)
      - [Prophet预测实现](#prophet预测实现)
  - [5. TimescaleDB深度应用](#5-timescaledb深度应用)
    - [5.1 连续聚合（Continuous Aggregates）](#51-连续聚合continuous-aggregates)
      - [创建连续聚合](#创建连续聚合)
    - [5.2 数据保留策略](#52-数据保留策略)
      - [自动数据保留](#自动数据保留)
    - [5.3 自动压缩](#53-自动压缩)
      - [压缩配置](#压缩配置)
  - [6. 实战案例](#6-实战案例)
    - [6.1 监控指标分析](#61-监控指标分析)
      - [系统监控指标分析](#系统监控指标分析)
  - [📚 参考资源](#-参考资源)
  - [📝 更新日志](#-更新日志)

---

## 1. 时序数据分析概述

### 1.1 时序数据特点

#### 核心特征

```text
时序数据特征:
1. 时间顺序性 - 数据按时间排序
2. 相关性 - 相邻时间点数据相关
3. 趋势性 - 长期趋势
4. 季节性 - 周期性模式
5. 噪声 - 随机波动
```

#### 时序数据模型

```sql
-- 时序数据表设计
CREATE TABLE sensor_readings (
    time TIMESTAMPTZ NOT NULL,
    device_id TEXT NOT NULL,
    sensor_type TEXT NOT NULL,
    value NUMERIC NOT NULL,
    quality NUMERIC,  -- 数据质量分数
    metadata JSONB
);

-- 创建超表（TimescaleDB）
SELECT create_hypertable('sensor_readings', 'time');

-- 创建索引
CREATE INDEX idx_sensor_readings_device_time ON sensor_readings (device_id, time DESC);
CREATE INDEX idx_sensor_readings_type_time ON sensor_readings (sensor_type, time DESC);
```

### 1.2 分析目标

#### 分析目标分类

```text
1. 描述性分析
   - 趋势识别
   - 季节性识别
   - 异常检测

2. 预测性分析
   - 短期预测（小时、天）
   - 中期预测（周、月）
   - 长期预测（季度、年）

3. 诊断性分析
   - 变化原因分析
   - 影响因素识别
   - 相关性分析

4. 规范性分析
   - 优化建议
   - 决策支持
   - 自动调整
```

### 1.3 TimescaleDB概述

#### TimescaleDB核心特性

```text
✅ 自动分区（按时间）
   - 自动创建时间分区
   - 透明查询

✅ 连续聚合（Continuous Aggregates）
   - 自动维护聚合视图
   - 增量更新

✅ 数据压缩
   - 自动压缩旧数据
   - 节省存储空间

✅ 数据保留策略
   - 自动删除旧数据
   - 策略化管理

✅ 时序函数
   - time_bucket
   - first/last
   - histogram
```

---

## 2. 时序分析方法

### 2.1 趋势分析

#### 线性趋势检测

```sql
-- 线性趋势分析（使用最小二乘法）
CREATE OR REPLACE FUNCTION analyze_trend(
    p_device_id TEXT,
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ
)
RETURNS TABLE (
    slope NUMERIC,
    intercept NUMERIC,
    r_squared NUMERIC,
    trend_direction TEXT
) AS $$
DECLARE
    v_n BIGINT;
    v_sum_x NUMERIC;
    v_sum_y NUMERIC;
    v_sum_xy NUMERIC;
    v_sum_x2 NUMERIC;
    v_slope NUMERIC;
    v_intercept NUMERIC;
    v_ss_res NUMERIC;
    v_ss_tot NUMERIC;
    v_r_squared NUMERIC;
BEGIN
    -- 计算最小二乘法的参数
    SELECT
        COUNT(*)::NUMERIC,
        SUM(EXTRACT(EPOCH FROM (time - p_start_time)))::NUMERIC,
        SUM(value)::NUMERIC,
        SUM(EXTRACT(EPOCH FROM (time - p_start_time)) * value)::NUMERIC,
        SUM(POWER(EXTRACT(EPOCH FROM (time - p_start_time)), 2))::NUMERIC
    INTO v_n, v_sum_x, v_sum_y, v_sum_xy, v_sum_x2
    FROM sensor_readings
    WHERE device_id = p_device_id
      AND time BETWEEN p_start_time AND p_end_time;

    -- 计算斜率和截距
    v_slope := (v_n * v_sum_xy - v_sum_x * v_sum_y) /
               (v_n * v_sum_x2 - POWER(v_sum_x, 2));
    v_intercept := (v_sum_y - v_slope * v_sum_x) / v_n;

    -- 计算R²（简化版）
    SELECT
        SUM(POWER(value - (v_intercept + v_slope * EXTRACT(EPOCH FROM (time - p_start_time))), 2)),
        SUM(POWER(value - AVG(value) OVER (), 2))
    INTO v_ss_res, v_ss_tot
    FROM sensor_readings
    WHERE device_id = p_device_id
      AND time BETWEEN p_start_time AND p_end_time;

    v_r_squared := 1 - (v_ss_res / NULLIF(v_ss_tot, 0));

    RETURN QUERY SELECT
        v_slope,
        v_intercept,
        v_r_squared,
        CASE
            WHEN v_slope > 0 THEN 'increasing'
            WHEN v_slope < 0 THEN 'decreasing'
            ELSE 'stable'
        END;
END;
$$ LANGUAGE plpgsql;
```

#### 移动平均趋势

```sql
-- 移动平均趋势分析
SELECT
    time_bucket('1 hour', time) AS hour,
    device_id,
    AVG(value) AS hourly_avg,
    -- 简单移动平均（7小时）
    AVG(AVG(value)) OVER (
        PARTITION BY device_id
        ORDER BY time_bucket('1 hour', time)
        ROWS BETWEEN 6 PRECEDING AND CURRENT ROW
    ) AS sma_7h,
    -- 指数移动平均（加权）
    AVG(value) * 0.3 + LAG(AVG(value)) OVER (
        PARTITION BY device_id
        ORDER BY time_bucket('1 hour', time)
    ) * 0.7 AS ema
FROM sensor_readings
WHERE time >= NOW() - INTERVAL '7 days'
GROUP BY time_bucket('1 hour', time), device_id
ORDER BY hour, device_id;
```

### 2.2 季节性分析

#### 季节性分解

```sql
-- 季节性分析（按小时、星期、月份）
WITH hourly_stats AS (
    SELECT
        EXTRACT(HOUR FROM time) AS hour_of_day,
        EXTRACT(DOW FROM time) AS day_of_week,
        EXTRACT(MONTH FROM time) AS month,
        AVG(value) AS avg_value,
        STDDEV(value) AS stddev_value
    FROM sensor_readings
    WHERE time >= NOW() - INTERVAL '1 year'
    GROUP BY EXTRACT(HOUR FROM time), EXTRACT(DOW FROM time), EXTRACT(MONTH FROM time)
),
overall_stats AS (
    SELECT AVG(value) AS overall_mean
    FROM sensor_readings
    WHERE time >= NOW() - INTERVAL '1 year'
)
SELECT
    h.hour_of_day,
    h.day_of_week,
    h.month,
    h.avg_value,
    h.avg_value - o.overall_mean AS seasonal_component,
    (h.avg_value - o.overall_mean) / o.overall_mean * 100 AS seasonal_percent
FROM hourly_stats h, overall_stats o
ORDER BY month, day_of_week, hour_of_day;
```

### 2.3 周期性检测

#### 自相关分析

```sql
-- 自相关函数（ACF）计算
CREATE OR REPLACE FUNCTION calculate_autocorrelation(
    p_device_id TEXT,
    p_lag INTEGER,
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ
)
RETURNS NUMERIC AS $$
DECLARE
    v_mean NUMERIC;
    v_n BIGINT;
    v_covariance NUMERIC;
    v_variance NUMERIC;
    v_acf NUMERIC;
BEGIN
    -- 计算均值
    SELECT AVG(value), COUNT(*) INTO v_mean, v_n
    FROM sensor_readings
    WHERE device_id = p_device_id
      AND time BETWEEN p_start_time AND p_end_time;

    -- 计算自协方差和方差
    WITH lagged_data AS (
        SELECT
            value,
            LAG(value, p_lag) OVER (ORDER BY time) AS lagged_value
        FROM sensor_readings
        WHERE device_id = p_device_id
          AND time BETWEEN p_start_time AND p_end_time
        ORDER BY time
    )
    SELECT
        AVG((value - v_mean) * (lagged_value - v_mean)),
        AVG(POWER(value - v_mean, 2))
    INTO v_covariance, v_variance
    FROM lagged_data
    WHERE lagged_value IS NOT NULL;

    -- 计算自相关系数
    v_acf := v_covariance / NULLIF(v_variance, 0);

    RETURN v_acf;
END;
$$ LANGUAGE plpgsql;

-- 计算不同滞后的自相关系数
SELECT
    lag,
    calculate_autocorrelation('device_001', lag, NOW() - INTERVAL '30 days', NOW()) AS acf
FROM generate_series(1, 24) AS lag  -- 检查24个时间点的滞后
ORDER BY lag;
```

---

## 3. 时序预测模型

### 3.1 ARIMA模型

#### ARIMA模型实现（PL/Python）

```sql
-- 使用statsmodels实现ARIMA
CREATE OR REPLACE FUNCTION forecast_arima(
    p_device_id TEXT,
    p_horizon INTEGER DEFAULT 24,
    p_start_time TIMESTAMPTZ DEFAULT NULL,
    p_end_time TIMESTAMPTZ DEFAULT NOW()
)
RETURNS TABLE (
    forecast_time TIMESTAMPTZ,
    forecast_value NUMERIC,
    lower_bound NUMERIC,
    upper_bound NUMERIC
) AS $$
import pandas as pd
from statsmodels.tsa.arima.model import ARIMA
import numpy as np
import plpy

# 获取历史数据
if p_start_time is None:
    query = f"""
        SELECT time, value
        FROM sensor_readings
        WHERE device_id = '{p_device_id}'
          AND time <= '{p_end_time}'
        ORDER BY time
    """
else:
    query = f"""
        SELECT time, value
        FROM sensor_readings
        WHERE device_id = '{p_device_id}'
          AND time BETWEEN '{p_start_time}' AND '{p_end_time}'
        ORDER BY time
    """

result = plpy.execute(query)

# 转换为DataFrame
df = pd.DataFrame([dict(row) for row in result])
df['time'] = pd.to_datetime(df['time'])
df = df.set_index('time')

# 拟合ARIMA模型（自动选择参数）
model = ARIMA(df['value'], order=(1, 1, 1))
fitted_model = model.fit()

# 预测
forecast = fitted_model.forecast(steps=p_horizon)
forecast_ci = fitted_model.get_forecast(steps=p_horizon).conf_int()

# 生成预测时间点
last_time = df.index[-1]
forecast_times = pd.date_range(start=last_time, periods=p_horizon + 1, freq='H')[1:]

# 返回结果
for i, (time, value, lower, upper) in enumerate(zip(
    forecast_times, forecast.values,
    forecast_ci.iloc[:, 0].values,
    forecast_ci.iloc[:, 1].values
)):
    yield (time, float(value), float(lower), float(upper))

$$ LANGUAGE plpython3u;
```

### 3.3 Prophet模型

#### Prophet预测实现

```sql
CREATE OR REPLACE FUNCTION forecast_prophet(
    p_device_id TEXT,
    p_horizon_days INTEGER DEFAULT 30,
    p_start_time TIMESTAMPTZ DEFAULT NULL,
    p_end_time TIMESTAMPTZ DEFAULT NOW()
)
RETURNS TABLE (
    forecast_date DATE,
    forecast_value NUMERIC,
    lower_bound NUMERIC,
    upper_bound NUMERIC
) AS $$
from prophet import Prophet
import pandas as pd
import plpy

# 获取历史数据
if p_start_time is None:
    query = f"""
        SELECT time::DATE AS ds, AVG(value) AS y
        FROM sensor_readings
        WHERE device_id = '{p_device_id}'
          AND time <= '{p_end_time}'
        GROUP BY time::DATE
        ORDER BY ds
    """
else:
    query = f"""
        SELECT time::DATE AS ds, AVG(value) AS y
        FROM sensor_readings
        WHERE device_id = '{p_device_id}'
          AND time BETWEEN '{p_start_time}' AND '{p_end_time}'
        GROUP BY time::DATE
        ORDER BY ds
    """

result = plpy.execute(query)
df = pd.DataFrame([dict(row) for row in result])
df['ds'] = pd.to_datetime(df['ds'])

# 拟合Prophet模型
model = Prophet(
    yearly_seasonality=True,
    weekly_seasonality=True,
    daily_seasonality=False,
    seasonality_mode='multiplicative'
)
model.fit(df)

# 创建未来日期
future = model.make_future_dataframe(periods=p_horizon_days)

# 预测
forecast = model.predict(future)

# 返回预测结果（只返回未来部分）
forecast_future = forecast.tail(p_horizon_days)

for _, row in forecast_future.iterrows():
    yield (
        row['ds'].date(),
        float(row['yhat']),
        float(row['yhat_lower']),
        float(row['yhat_upper'])
    )

$$ LANGUAGE plpython3u;
```

---

## 5. TimescaleDB深度应用

### 5.1 连续聚合（Continuous Aggregates）

#### 创建连续聚合

```sql
-- 创建小时级聚合
CREATE MATERIALIZED VIEW hourly_sensor_metrics
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', time) AS bucket,
    device_id,
    sensor_type,
    AVG(value) AS avg_value,
    MIN(value) AS min_value,
    MAX(value) AS max_value,
    STDDEV(value) AS stddev_value,
    COUNT(*) AS reading_count
FROM sensor_readings
GROUP BY bucket, device_id, sensor_type;

-- 添加刷新策略
SELECT add_continuous_aggregate_policy('hourly_sensor_metrics',
    start_offset => INTERVAL '3 hours',
    end_offset => INTERVAL '1 hour',
    schedule_interval => INTERVAL '1 hour');

-- 创建日级聚合（基于小时级聚合）
CREATE MATERIALIZED VIEW daily_sensor_metrics
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 day', bucket) AS bucket,
    device_id,
    sensor_type,
    AVG(avg_value) AS avg_value,
    MIN(min_value) AS min_value,
    MAX(max_value) AS max_value,
    SUM(reading_count) AS total_readings
FROM hourly_sensor_metrics
GROUP BY bucket, device_id, sensor_type;

SELECT add_continuous_aggregate_policy('daily_sensor_metrics',
    start_offset => INTERVAL '3 days',
    end_offset => INTERVAL '1 day',
    schedule_interval => INTERVAL '1 day');
```

### 5.2 数据保留策略

#### 自动数据保留

```sql
-- 添加数据保留策略
SELECT add_retention_policy('sensor_readings', INTERVAL '90 days');

-- 自定义保留策略（保留压缩数据）
CREATE OR REPLACE FUNCTION custom_retention_policy()
RETURNS void AS $$
BEGIN
    -- 压缩30天前的数据
    PERFORM compress_chunk(chunk)
    FROM timescaledb_information.chunks
    WHERE hypertable_name = 'sensor_readings'
      AND range_start < NOW() - INTERVAL '30 days'
      AND is_compressed = FALSE;

    -- 删除90天前的数据
    DELETE FROM sensor_readings
    WHERE time < NOW() - INTERVAL '90 days';
END;
$$ LANGUAGE plpgsql;

-- 定时执行
SELECT cron.schedule(
    'retention-policy',
    '0 2 * * *',  -- 每天凌晨2点
    $$SELECT custom_retention_policy()$$
);
```

### 5.3 自动压缩

#### 压缩配置

```sql
-- 添加压缩策略
SELECT add_compression_policy('sensor_readings', INTERVAL '7 days');

-- 自定义压缩策略
SELECT set_compression_settings('sensor_readings',
    segmentby => 'device_id, sensor_type',
    orderby => 'time DESC',
    compress_segmentby => 'device_id, sensor_type'
);

-- 查看压缩统计
SELECT
    chunk_name,
    pg_size_pretty(before_compression_total_bytes) AS before_size,
    pg_size_pretty(after_compression_total_bytes) AS after_size,
    (1 - after_compression_total_bytes::NUMERIC / before_compression_total_bytes) * 100 AS compression_ratio
FROM timescaledb_information.chunk_compression_stats
WHERE hypertable_name = 'sensor_readings'
ORDER BY compression_ratio DESC;
```

---

## 6. 实战案例

### 6.1 监控指标分析

#### 系统监控指标分析

```sql
-- CPU使用率分析
WITH cpu_metrics AS (
    SELECT
        time_bucket('5 minutes', time) AS bucket,
        device_id,
        AVG(value) AS avg_cpu,
        MAX(value) AS max_cpu,
        PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY value) AS p95_cpu
    FROM sensor_readings
    WHERE sensor_type = 'cpu_usage'
      AND time >= NOW() - INTERVAL '24 hours'
    GROUP BY bucket, device_id
)
SELECT
    bucket,
    device_id,
    avg_cpu,
    max_cpu,
    p95_cpu,
    CASE
        WHEN avg_cpu > 80 THEN 'critical'
        WHEN avg_cpu > 60 THEN 'warning'
        ELSE 'normal'
    END AS status
FROM cpu_metrics
ORDER BY bucket DESC, avg_cpu DESC;
```

---

## 📚 参考资源

1. **TimescaleDB文档**: <https://docs.timescale.com/>
2. **Prophet文档**: <https://facebook.github.io/prophet/>
3. **ARIMA模型**: <https://www.statsmodels.org/stable/generated/statsmodels.tsa.arima.model.ARIMA.html>
4. **时序分析**: <https://otexts.com/fpp3/>

---

## 📝 更新日志

- **v2.0** (2025-01): 完整实战指南
  - 补充时序分析方法（趋势、季节性、周期性、异常检测）
  - 补充时序预测模型（ARIMA、指数平滑、Prophet、LSTM）
  - 补充时序聚合与降采样
  - 补充TimescaleDB深度应用
  - 补充实战案例

---

**状态**: ✅ **文档完成** | [返回目录](./README.md)
