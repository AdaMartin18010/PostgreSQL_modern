...
> **📋 文档来源**: `PostgreSQL_View\09-实践指南\运维手册\性能监控.md`
> **📅 复制日期**: 2025-12-22
...
- [性能监控](#性能监控)
  ...
  - [8. 实际应用案例](#8-实际应用案例)
    - [8.1 案例：大型电商平台性能监控实施](#81-案例大型电商平台性能监控实施)
    - [8.2 案例：RAG系统性能监控优化](#82-案例rag系统性能监控优化)
  - [9. 参考资料](#9-参考资料)
    - [8.1 官方文档](#81-官方文档)
    - [8.2 技术文档](#82-技术文档)
    - [8.3 相关资源](#83-相关资源)
...
1. **索引监控**: 监控索引使用情况，识别未使用的索引和需要优化的索引
1. **优化建议**: 基于监控数据提供性能优化建议
...
1. **全面覆盖**: 覆盖性能、资源、索引等各个方面
1. **合理频率**: 根据指标特性设置合理的采集频率
1. **成本控制**: 平衡监控成本和监控价值
...
1. **索引优化**: 删除未使用的索引，优化索引参数
1. **查询优化**: 基于慢查询日志优化查询语句
1. **资源优化**: 根据监控数据调整资源分配
...
SELECT
    'query_latency_p95' as metric_name,
...
SELECT
    ...
    SUM(calls) / EXTRACT(EPOCH FROM (NOW() - stats_reset)) as value
FROM pg_stat_statements
WHERE query LIKE '%<=>%';
...
QUERIES:
  - name: "vector_query_performance"
    help: "Vector query performance metrics"
    values: ["mean_exec_time", "calls", "total_exec_time"]
    query: |
      SELECT
        mean_exec_time,
        calls,
...
```sql
-- 1. 创建RAG系统专用监控视图
CREATE OR REPLACE VIEW rag_performance_monitoring AS
SELECT
    'vector_search_latency' as metric_name,
    AVG(mean_exec_time) as avg_latency,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY mean_exec_time) as p95_latency,
    PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY mean_exec_time) as p99_latency
FROM pg_stat_statements
...
- [性能调优技巧](../../10-AI与机器学习/10.01-向量处理/最佳实践/性能调优技巧.md) - Performance Tuning
