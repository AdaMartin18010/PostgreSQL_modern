...
SET
    recommended_value = CASE
        ...
        WHEN config_name = 'work_mem' THEN 32 -- 32MB (假设max_connections=100)
...
SELECT
    ...
    CASE
        WHEN optimization_potential > 20 THEN '高优先级优化'
        WHEN optimization_potential > 10 THEN '中优先级优化'
        ELSE '低优先级优化'
    END AS priority
...
VALUES
('INSERT', 1000, 1000, 50), -- 单条插入1000条需要1000ms，批量插入需要50ms
...
BEGIN
    ...
    RAISE NOTICE '批量插入完成: % 条记录, 耗时: %',
        v_total_records,
        EXTRACT(EPOCH FROM (v_end_time - v_start_time)) || ' 秒';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '批量更新完成: % 条记录', v_updated_count;
END $$;
```
...
BEGIN
    ...
    SELECT
        ...
        '存在大量空闲连接，占用资源'::TEXT,
        v_idle_connections,
        '使用连接池（PgBouncer或pgpool-II）减少连接数'::TEXT,
        '可减少 ' || ROUND(v_idle_connections * 0.7) || ' 个连接'::TEXT
    ...
    SELECT
        ...
        '设置statement_timeout和idle_in_transaction_session_timeout',
        '可释放 ' || v_long_running_connections || ' 个连接'
...
SET
    optimization_status = CASE
        WHEN current_usage <= optimal_usage THEN '正常'
        WHEN current_usage <= optimal_usage * 1.2 THEN '需关注'
        ELSE '需优化'
    END,
    optimization_potential = CASE
