...
> **📋 文档来源**: `PostgreSQL_View\09-实践指南\快速开始\第一个向量搜索应用.md`
> **📅 复制日期**: 2025-12-22
...
- [8.2 技术文档](#82-技术文档)
- [8.3 相关资源](#83-相关资源)
- [9.1 案例：企业知识库向量搜索应用](#91-案例企业知识库向量搜索应用)
- [9.2 案例：电商商品推荐向量搜索应用](#92-案例电商商品推荐向量搜索应用)
---
...
1. **掌握 pgvector**: 学会使用 pgvector 进行向量存储和搜索
1. **开发实践**: 能够独立开发向量搜索应用
1. **性能优化**: 了解索引创建和查询优化技巧
...
1. 使用代理（如果需要）: 设置 `HTTP_PROXY` 环境变量
...
1. 检查连接参数: 确认 `.env` 文件中的配置正确
1. 检查防火墙: 确保端口 5432 未被阻止
...
1. 确保所有向量使用相同维度
1. 检查向量生成模型: 使用 `text-embedding-3-small` (1536 维)
1. 统一向量维度: `ALTER TABLE documents ALTER COLUMN embedding TYPE vector(1536);`
...
1. **索引选择策略**: 查看[索引选择策略](../../10-AI与机器学习/10.01-向量处理/最佳实践/索引选择策略.md)

   - 学习 HNSW、IVFFlat、SP-GiST 的区别
   - 根据场景选择最佳索引

1. **性能调优**: 查看[性能调优技巧](../../10-AI与机器学习/10.01-向量处理/最佳实践/性能调优技巧.md)
   - 索引参数优化
   - 查询优化技巧
   - 并发优化策略
...
- [索引选择策略](../../10-AI与机器学习/10.01-向量处理/最佳实践/索引选择策略.md) - Index Selection Strategy
...
| 指标 | 优化前 | 优化后 | 改善 |
| --- | --- | --- | --- |
| **查询延迟(P99)** | 200ms | **<40ms** | **80%** ⬇️ |
| **搜索准确率** | 75% | **92%** | **+23%** |
| **系统响应时间** | 500ms | **<100ms** | **80%** ⬇️ |
