...
> **📋 文档来源**: `PostgreSQL培训\09-高可用\逻辑复制详解.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    ...
    RAISE NOTICE '发布my_publication创建成功';
EXCEPTION
...
BEGIN
    IF EXISTS (
        ...
        RAISE WARNING '发布all_tables已存在';
        RETURN;
    END IF;

    CREATE PUBLICATION all_tables FOR ALL TABLES;
    RAISE NOTICE '发布all_tables创建成功';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布all_tables已存在';
    WHEN insufficient_privilege THEN
...
-- 3. 发布特定列（PostgreSQL 15+，带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_publication WHERE pubname = 'users_publication'
    ) THEN
        RAISE WARNING '发布users_publication已存在';
        RETURN;
    END IF;
    ...
    RAISE NOTICE '发布users_publication创建成功';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布users_publication已存在';
    WHEN undefined_table THEN
...
-- 4. 创建带过滤条件的发布（PostgreSQL 15+，带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_publication WHERE pubname = 'active_users_publication'
    ) THEN
        RAISE WARNING '发布active_users_publication已存在';
        RETURN;
    END IF;
    ...
    FOR TABLE users
    WHERE (status = 'active');
    RAISE NOTICE '发布active_users_publication创建成功';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布active_users_publication已存在';
    WHEN undefined_table THEN
...
BEGIN
    ...
    IF publication_count > 0 THEN
        RAISE NOTICE '发现 % 个发布', publication_count;
    ELSE
        RAISE NOTICE '未发现发布';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_publication表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看发布信息失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF table_count > 0 THEN
        RAISE NOTICE '发布my_publication包含 % 个表', table_count;
    ELSE
        RAISE NOTICE '发布my_publication未包含表';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_publication_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看发布包含的表失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表new_table不存在';
    END IF;

    ALTER PUBLICATION my_publication ADD TABLE new_table;
    RAISE NOTICE '表new_table已添加到发布my_publication';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '发布my_publication或表new_table不存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    RAISE NOTICE '表old_table已从发布my_publication中移除';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '发布my_publication不存在';
    WHEN OTHERS THEN
...
-- 9. 设置发布操作类型（只发布 INSERT 和 UPDATE，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication WHERE pubname = 'my_publication'
    ...
    RAISE NOTICE '发布my_publication操作类型已设置为insert,update';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '发布my_publication不存在';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '无效的发布操作类型';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置发布操作类型失败: %', SQLERRM;
END $$;

-- 10. 创建发布用户（需要 REPLICATION 权限，带错误处理）
DO $$
BEGIN
    IF EXISTS (
    ...
    RAISE NOTICE '用户replicator创建成功，已授予SELECT权限';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '用户replicator已存在';
    WHEN insufficient_privilege THEN
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建发布用户失败: %', SQLERRM;
END $$;
```
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看订阅信息失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '找到 % 个订阅状态记录', subscription_count;
EXCEPTION
    WHEN undefined_table THEN
