...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看主库连接数失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF connection_count > 0 THEN
        RAISE NOTICE '从库数据库mydb有 % 个连接', connection_count;
    ELSE
        RAISE NOTICE '从库数据库mydb没有连接';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_is_in_recovery函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看从库连接数失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_is_in_recovery函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查主从状态失败: %', SQLERRM;
END $$;
