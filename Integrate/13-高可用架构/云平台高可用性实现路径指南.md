...
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_publication WHERE pubname = 'cross_region_pub'
    ) THEN
        RAISE WARNING '发布cross_region_pub已存在';
        RETURN;
    END IF;

    CREATE PUBLICATION cross_region_pub FOR ALL TABLES;
    RAISE NOTICE '发布cross_region_pub创建成功';
EXCEPTION
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_subscription WHERE subname = 'cross_region_sub'
    ) THEN
        RAISE WARNING '订阅cross_region_sub已存在';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '发布cross_region_pub不存在，请先在主区域创建';
    END IF;
    ...
    RAISE NOTICE '订阅cross_region_sub创建成功';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '订阅cross_region_sub已存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '发布cross_region_pub不存在';
    WHEN connection_exception THEN
        RAISE EXCEPTION '无法连接到主区域数据库';
    WHEN insufficient_privilege THEN
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'predictive_analysis'
    ) THEN
        DROP VIEW predictive_analysis;
        RAISE NOTICE '已删除现有视图predictive_analysis';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表monitoring_metrics不存在，请先创建';
    END IF;
    ...
    RAISE NOTICE '视图predictive_analysis创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '视图predictive_analysis已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION '表monitoring_metrics不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建预测性分析视图失败: %', SQLERRM;
END $$;
```
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'sla_monitoring'
    ) THEN
        DROP VIEW sla_monitoring;
        RAISE NOTICE '已删除现有视图sla_monitoring';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'system_uptime'
    ) THEN
        RAISE EXCEPTION '表system_uptime不存在，请先创建';
    END IF;
    ...
    RAISE NOTICE '视图sla_monitoring创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '视图sla_monitoring已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION '表system_uptime不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建SLA监控视图失败: %', SQLERRM;
END $$;
```
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'cost_monitoring'
    ) THEN
        DROP VIEW cost_monitoring;
        RAISE NOTICE '已删除现有视图cost_monitoring';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'cost_metrics'
    ) THEN
        RAISE EXCEPTION '表cost_metrics不存在，请先创建';
    END IF;
    ...
    RAISE NOTICE '视图cost_monitoring创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '视图cost_monitoring已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION '表cost_metrics不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建成本监控视图失败: %', SQLERRM;
END $$;
```
