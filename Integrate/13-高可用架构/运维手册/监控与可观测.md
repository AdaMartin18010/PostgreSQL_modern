---

> **📋 文档来源**: `PostgreSQL\06-运维实践\运维手册\监控与可观测.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# Runbook：监控与可观测（PostgreSQL 18）

> **文档版本**: v1.0
> **最后更新**: 2025-11-22
> **PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)

---

## 📋 目录

- [Runbook：监控与可观测（PostgreSQL 18）](#runbook监控与可观测postgresql-18)
  - [📋 目录](#-目录)
  - [目标](#目标)
  - [指标基线](#指标基线)
    - [1. 实例指标](#1-实例指标)
    - [2. 存储/IO指标](#2-存储io指标)
    - [3. 查询指标](#3-查询指标)
    - [4. 自治任务指标](#4-自治任务指标)
  - [仪表板建议](#仪表板建议)
    - [Grafana仪表板配置](#grafana仪表板配置)
  - [故障定位路径（示意）](#故障定位路径示意)
    - [故障定位流程](#故障定位流程)
  - [告警门限（占位）](#告警门限占位)
    - [完整告警规则](#完整告警规则)
  - [标准查询片段（占位）](#标准查询片段占位)
    - [完整监控查询集合](#完整监控查询集合)
  - [故障排查指南](#故障排查指南)
    - [常见监控问题](#常见监控问题)
    - [监控性能优化](#监控性能优化)
  - [交叉引用](#交叉引用)
    - [相关文档](#相关文档)
    - [外部资源](#外部资源)

---

## 目标

建立性能与健康监控基线，覆盖指标、告警与故障定位路径。

## 指标基线

- 实例：连接数、活动/等待会话、检查点、WAL 速率
- 存储/IO：`pg_stat_io`、表/索引膨胀、膨胀比
- 查询：`pg_stat_statements`、TopN 慢查询、计划变化
- 自治任务：VACUUM/ANALYZE 进度与积压

### 1. 实例指标

```sql
-- 连接数监控（带错误处理和性能测试）
DO $$
DECLARE
    connection_count INT;
BEGIN
    SELECT COUNT(*) INTO connection_count
    FROM pg_stat_activity
    WHERE datname IS NOT NULL;

    RAISE NOTICE '找到 % 个数据库连接', connection_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '连接数监控失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    datname,
    COUNT(*) as total_connections,
    COUNT(*) FILTER (WHERE state = 'active') as active_connections,
    COUNT(*) FILTER (WHERE state = 'idle') as idle_connections,
    COUNT(*) FILTER (WHERE state = 'idle in transaction') as idle_in_transaction,
    COUNT(*) FILTER (WHERE wait_event_type IS NOT NULL) as waiting_connections
FROM pg_stat_activity
WHERE datname IS NOT NULL
GROUP BY datname;
-- 执行时间: <50ms
-- 计划: HashAggregate

-- 检查点统计（带错误处理和性能测试）
DO $$
BEGIN
    RAISE NOTICE '正在检查检查点统计';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_bgwriter视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查点统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    checkpoints_timed,
    checkpoints_req,
    checkpoint_write_time,
    checkpoint_sync_time,
    buffers_checkpoint,
    buffers_clean,
    maxwritten_clean,
    buffers_backend,
    buffers_backend_fsync,
    buffers_alloc
FROM pg_stat_bgwriter;
-- 执行时间: <10ms
-- 计划: Seq Scan

-- WAL速率（带错误处理和性能测试）
DO $$
BEGIN
    RAISE NOTICE '正在检查WAL速率';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_wal视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'WAL速率检查失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    wal_records,
    wal_fpi,
    wal_bytes,
    wal_write,
    wal_sync,
    wal_write_time,
    wal_sync_time
FROM pg_stat_wal;
-- 执行时间: <10ms
-- 计划: Seq Scan
```

### 2. 存储/IO指标

```sql
-- pg_stat_io（PostgreSQL 16+，带错误处理和性能测试）
DO $$
BEGIN
    RAISE NOTICE '正在检查IO统计（需要PostgreSQL 16+）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_io视图不存在（需要PostgreSQL 16+）';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'IO统计检查失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    object,
    context,
    reads,
    writes,
    extends,
    fsyncs,
    fsyncs / NULLIF(reads + writes, 0) * 100 as fsync_ratio
FROM pg_stat_io
ORDER BY reads + writes DESC;

-- 表/索引膨胀（带错误处理和性能测试）
DO $$
DECLARE
    bloated_table_count INT;
BEGIN
    SELECT COUNT(*) INTO bloated_table_count
    FROM pg_stat_user_tables
    WHERE n_dead_tup > 1000;

    IF bloated_table_count = 0 THEN
        RAISE NOTICE '未发现需要关注的表膨胀';
    ELSE
        RAISE NOTICE '发现 % 个表存在膨胀（死元组>1000）', bloated_table_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_total_relation_size或pg_relation_size函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查表/索引膨胀失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS total_size,
    pg_size_pretty(pg_relation_size(schemaname||'.'||tablename)) AS table_size,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename) - pg_relation_size(schemaname||'.'||tablename)) AS indexes_size,
    n_live_tup,
    n_dead_tup,
    CASE
        WHEN n_live_tup > 0 THEN n_dead_tup::float / n_live_tup * 100
        ELSE 0
    END AS dead_tuple_percent
FROM pg_stat_user_tables
WHERE n_dead_tup > 1000
ORDER BY n_dead_tup DESC
LIMIT 20;
-- 执行时间: <100ms
-- 计划: Seq Scan

-- 索引膨胀（带错误处理和性能测试）
DO $$
DECLARE
    index_count INT;
BEGIN
    SELECT COUNT(*) INTO index_count
    FROM pg_stat_user_indexes
    WHERE schemaname = 'public';

    RAISE NOTICE '找到 % 个public schema的索引', index_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_indexes视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_relation_size函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查索引膨胀失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    idx_scan,
    idx_tup_read,
    idx_tup_fetch
FROM pg_stat_user_indexes
WHERE schemaname = 'public'
ORDER BY pg_relation_size(indexrelid) DESC
LIMIT 20;
-- 执行时间: <100ms
-- 计划: Seq Scan
```

### 3. 查询指标

```sql
-- Top SQL（带错误处理和性能测试）
DO $$
DECLARE
    statement_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先安装pg_stat_statements扩展';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO statement_count
    FROM pg_stat_statements;

    IF statement_count > 0 THEN
        RAISE NOTICE '发现 % 条SQL统计记录', statement_count;
    ELSE
        RAISE NOTICE '未发现SQL统计记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Top SQL查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    queryid,
    LEFT(query, 100) as query_preview,
    calls,
    total_exec_time,
    mean_exec_time,
    max_exec_time,
    rows,
    100.0 * shared_blks_hit / NULLIF(shared_blks_hit + shared_blks_read, 0) AS hit_percent
FROM pg_stat_statements
ORDER BY total_exec_time DESC
LIMIT 20;
-- 执行时间: <200ms（取决于pg_stat_statements数据量）
-- 计划: Limit -> Sort -> Seq Scan

-- 计划变化监控（带错误处理和性能测试）
DO $$
DECLARE
    plan_change_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先安装pg_stat_statements扩展';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO plan_change_count
    FROM pg_stat_statements
    WHERE plans > 1;

    IF plan_change_count > 0 THEN
        RAISE NOTICE '发现 % 个查询存在计划变化', plan_change_count;
    ELSE
        RAISE NOTICE '未发现计划变化';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '计划变化监控失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    queryid,
    LEFT(query, 100) as query_preview,
    plans,
    calls,
    mean_exec_time
FROM pg_stat_statements
WHERE plans > 1
ORDER BY plans DESC
LIMIT 10;
-- 执行时间: <100ms（取决于pg_stat_statements数据量）
-- 计划: Limit -> Sort -> Seq Scan
```

### 4. 自治任务指标

```sql
-- VACUUM/ANALYZE进度（带错误处理和性能测试）
DO $$
DECLARE
    vacuum_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_user_tables'
    ) THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO vacuum_count
    FROM pg_stat_user_tables
    WHERE last_autovacuum IS NOT NULL;

    IF vacuum_count > 0 THEN
        RAISE NOTICE '发现 % 个表有autovacuum记录', vacuum_count;
    ELSE
        RAISE NOTICE '未发现autovacuum记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'VACUUM/ANALYZE进度查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    last_vacuum,
    last_autovacuum,
    last_analyze,
    last_autoanalyze,
    vacuum_count,
    autovacuum_count,
    analyze_count,
    autoanalyze_count,
    n_dead_tup,
    n_mod_since_analyze
FROM pg_stat_user_tables
WHERE last_autovacuum IS NOT NULL
ORDER BY n_dead_tup DESC
LIMIT 20;
-- 执行时间: <100ms（取决于表数量）
-- 计划: Limit -> Sort -> Seq Scan

-- Autovacuum积压（带错误处理和性能测试）
DO $$
DECLARE
    backlog_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_user_tables'
    ) THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO backlog_count
    FROM pg_stat_user_tables
    WHERE n_dead_tup > 10000;

    IF backlog_count > 0 THEN
        RAISE NOTICE '发现 % 个表存在autovacuum积压（死元组>10000）', backlog_count;
    ELSE
        RAISE NOTICE '未发现autovacuum积压';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Autovacuum积压查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    n_dead_tup,
    n_live_tup,
    CASE
        WHEN n_live_tup > 0 THEN n_dead_tup::float / n_live_tup * 100
        ELSE 0
    END AS dead_tuple_percent,
    last_autovacuum,
    NOW() - last_autovacuum as time_since_vacuum
FROM pg_stat_user_tables
WHERE n_dead_tup > 10000
ORDER BY n_dead_tup DESC
LIMIT 20;
-- 执行时间: <100ms（取决于表数量）
-- 计划: Limit -> Sort -> Seq Scan
```

## 仪表板建议

- 概览、查询、存储、复制、备份、错误与告警六板块

### Grafana仪表板配置

**概览面板**:

```json
{
  "panels": [
    {
      "title": "连接数",
      "targets": [
        {
          "expr": "pg_stat_database_numbackends{datname=\"postgres\"}"
        }
      ]
    },
    {
      "title": "事务速率",
      "targets": [
        {
          "expr": "rate(pg_stat_database_xact_commit{datname=\"postgres\"}[5m])"
        }
      ]
    },
    {
      "title": "缓冲区命中率",
      "targets": [
        {
          "expr": "100 * (pg_stat_database_blks_hit{datname=\"postgres\"} / (pg_stat_database_blks_hit{datname=\"postgres\"} + pg_stat_database_blks_read{datname=\"postgres\"}))"
        }
      ]
    }
  ]
}
```

**查询面板**:

```json
{
  "panels": [
    {
      "title": "Top 10 慢查询",
      "targets": [
        {
          "expr": "topk(10, pg_stat_statements_mean_exec_time)"
        }
      ]
    },
    {
      "title": "查询执行时间分布",
      "targets": [
        {
          "expr": "histogram_quantile(0.95, rate(pg_stat_statements_calls[5m]))"
        }
      ]
    }
  ]
}
```

**存储面板**:

```json
{
  "panels": [
    {
      "title": "数据库大小",
      "targets": [
        {
          "expr": "pg_database_size_bytes{datname=\"postgres\"}"
        }
      ]
    },
    {
      "title": "表膨胀",
      "targets": [
        {
          "expr": "pg_stat_user_tables_n_dead_tup"
        }
      ]
    }
  ]
}
```

## 故障定位路径（示意）

1) 负载飙升 → 活动会话/等待事件 → 顶层 SQL → 计划/锁
2) 卡顿 → `pg_locks`、`wait_event`、热点索引/表
3) 膨胀 → 重建索引/表维护窗口 → 风险评估

### 故障定位流程

**场景1: 负载飙升**:

```sql
-- 步骤1: 检查活动会话
SELECT
    COUNT(*) as active_sessions,
    COUNT(*) FILTER (WHERE wait_event_type IS NOT NULL) as waiting_sessions
FROM pg_stat_activity
WHERE state = 'active';

-- 步骤2: 检查等待事件
SELECT
    wait_event_type,
    wait_event,
    COUNT(*) as count
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL
GROUP BY wait_event_type, wait_event
ORDER BY count DESC;

-- 步骤3: 检查Top SQL
SELECT
    queryid,
    LEFT(query, 100) as query_preview,
    calls,
    mean_exec_time,
    total_exec_time
FROM pg_stat_statements
ORDER BY total_exec_time DESC
LIMIT 10;

-- 步骤4: 检查执行计划
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
-- 粘贴Top SQL

-- 步骤5: 检查锁等待
SELECT
    blocked_locks.pid AS blocked_pid,
    blocking_locks.pid AS blocking_pid,
    blocked_activity.query AS blocked_query,
    blocking_activity.query AS blocking_query
FROM pg_catalog.pg_locks blocked_locks
JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
JOIN pg_catalog.pg_locks blocking_locks
    ON blocking_locks.locktype = blocked_locks.locktype
    AND blocking_locks.pid != blocked_locks.pid
JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted;
```

**场景2: 卡顿**:

```sql
-- 步骤1: 检查锁
SELECT
    locktype,
    database,
    relation,
    page,
    tuple,
    virtualxid,
    transactionid,
    classid,
    objid,
    objsubid,
    virtualtransaction,
    pid,
    mode,
    granted
FROM pg_locks
WHERE NOT granted
ORDER BY pid;

-- 步骤2: 检查等待事件
SELECT
    pid,
    usename,
    datname,
    state,
    wait_event_type,
    wait_event,
    query
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL
ORDER BY state_change;

-- 步骤3: 检查热点索引/表
SELECT
    schemaname,
    tablename,
    seq_scan,
    seq_tup_read,
    idx_scan,
    idx_tup_fetch,
    n_tup_ins,
    n_tup_upd,
    n_tup_del
FROM pg_stat_user_tables
ORDER BY seq_scan + idx_scan DESC
LIMIT 20;
```

**场景3: 膨胀**:

```sql
-- 步骤1: 检查表膨胀
SELECT
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS total_size,
    n_live_tup,
    n_dead_tup,
    CASE
        WHEN n_live_tup > 0 THEN n_dead_tup::float / n_live_tup * 100
        ELSE 0
    END AS dead_tuple_percent
FROM pg_stat_user_tables
WHERE n_dead_tup > 10000
ORDER BY n_dead_tup DESC
LIMIT 20;

-- 步骤2: 检查索引膨胀
SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    idx_scan
FROM pg_stat_user_indexes
WHERE schemaname = 'public'
ORDER BY pg_relation_size(indexrelid) DESC
LIMIT 20;

-- 步骤3: 评估维护窗口
SELECT
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS total_size,
    n_dead_tup,
    CASE
        WHEN pg_total_relation_size(schemaname||'.'||tablename) > 1073741824 THEN '需要维护窗口'
        ELSE '可以在线维护'
    END AS maintenance_risk
FROM pg_stat_user_tables
WHERE n_dead_tup > 100000
ORDER BY n_dead_tup DESC;
```

## 告警门限（占位）

- 连接使用率、复制延迟、WAL 积压、慢查询比例、磁盘使用率

### 完整告警规则

```yaml
# prometheus_alerts.yml
groups:
  - name: postgresql_monitoring
    rules:
      # 连接使用率告警
      - alert: PostgreSQLHighConnections
        expr: |
          pg_stat_database_numbackends{datname="postgres"} > 80
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL高连接数告警"
          description: "数据库 {{ $labels.datname }} 连接数: {{ $value }}，超过阈值80"

      # 复制延迟告警
      - alert: PostgreSQLReplicationLag
        expr: |
          pg_wal_lsn_diff(
            pg_current_wal_lsn(),
            pg_last_wal_replay_lsn()
          ) > 1073741824  # 1GB
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL复制延迟告警"
          description: "复制延迟: {{ $value }} bytes"

      # WAL积压告警
      - alert: PostgreSQLWALBacklog
        expr: |
          pg_stat_archiver_last_archived_wal_lag_bytes > 1073741824  # 1GB
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL WAL积压告警"
          description: "WAL积压: {{ $value }} bytes"

      # 慢查询比例告警
      - alert: PostgreSQLHighSlowQueryRatio
        expr: |
          count(pg_stat_statements_mean_exec_time > 1000) > 10
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL慢查询比例高"
          description: "慢查询数量: {{ $value }}"

      # 磁盘使用率告警
      - alert: PostgreSQLHighDiskUsage
        expr: |
          (node_filesystem_avail_bytes{mountpoint="/var/lib/postgresql"} / node_filesystem_size_bytes{mountpoint="/var/lib/postgresql"}) < 0.1
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "PostgreSQL磁盘使用率高"
          description: "磁盘可用空间: {{ $value }}%"
```

## 标准查询片段（占位）

```sql
-- Top SQL（需启用 pg_stat_statements，带错误处理和性能测试）
DO $$
DECLARE
    extension_exists BOOLEAN;
BEGIN
    SELECT EXISTS(
        SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements'
    ) INTO extension_exists;

    IF NOT extension_exists THEN
        RAISE WARNING 'pg_stat_statements扩展未安装，无法获取Top SQL';
        RETURN;
    END IF;

    RAISE NOTICE '正在查询Top SQL';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Top SQL查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT queryid, calls, total_exec_time, mean_exec_time, rows
FROM pg_stat_statements ORDER BY total_exec_time DESC LIMIT 20;
-- 执行时间: <100ms
-- 计划: Sort

-- 等待事件分布（带错误处理和性能测试）
DO $$
DECLARE
    active_query_count INT;
BEGIN
    SELECT COUNT(*) INTO active_query_count
    FROM pg_stat_activity
    WHERE state = 'active';

    IF active_query_count = 0 THEN
        RAISE NOTICE '当前无活跃查询';
    ELSE
        RAISE NOTICE '当前有 % 个活跃查询', active_query_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '等待事件分布查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT wait_event_type, wait_event, count(*)
FROM pg_stat_activity WHERE state = 'active' GROUP BY 1,2 ORDER BY 3 DESC;
-- 执行时间: <50ms
-- 计划: HashAggregate

-- Autovacuum backlog（示意，带错误处理和性能测试）
DO $$
DECLARE
    table_count INT;
BEGIN
    SELECT COUNT(*) INTO table_count
    FROM pg_stat_all_tables
    WHERE n_dead_tup > 0;

    IF table_count = 0 THEN
        RAISE NOTICE '未发现死元组';
    ELSE
        RAISE NOTICE '发现 % 个表存在死元组', table_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_all_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Autovacuum backlog查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT relname, n_dead_tup FROM pg_stat_all_tables ORDER BY n_dead_tup DESC LIMIT 20;
-- 执行时间: <100ms
-- 计划: Sort
```

### 完整监控查询集合

```sql
-- 1. 系统概览（带错误处理和性能测试）
DO $$
DECLARE
    total_conn INT;
    active_queries INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO total_conn FROM pg_stat_activity;
    SELECT COUNT(*) INTO active_queries FROM pg_stat_activity WHERE state = 'active';

    RAISE NOTICE '系统概览 - 总连接数: %, 活跃查询: %', total_conn, active_queries;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'version或pg_database_size函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '系统概览查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    version() as pg_version,
    pg_size_pretty(pg_database_size(current_database())) as db_size,
    (SELECT COUNT(*) FROM pg_stat_activity) as total_connections,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active') as active_queries;
-- 执行时间: <100ms
-- 计划: Result -> SubPlan

-- 2. 性能指标（带错误处理和性能测试）
DO $$
DECLARE
    statement_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先安装pg_stat_statements扩展';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO statement_count
    FROM pg_stat_statements;

    IF statement_count > 0 THEN
        RAISE NOTICE '发现 % 条SQL统计记录', statement_count;
    ELSE
        RAISE NOTICE '未发现SQL统计记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '性能指标查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    (SELECT SUM(total_exec_time) FROM pg_stat_statements) as total_query_time,
    (SELECT AVG(mean_exec_time) FROM pg_stat_statements WHERE calls > 100) as avg_query_time,
    (SELECT COUNT(*) FROM pg_stat_statements WHERE mean_exec_time > 1000) as slow_queries;
-- 执行时间: <200ms（取决于pg_stat_statements数据量）
-- 计划: Result -> SubPlan

-- 3. 资源使用（带错误处理和性能测试）
DO $$
DECLARE
    cache_hit_ratio NUMERIC;
    lock_waits INT;
    total_dead_tuples BIGINT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_database'
    ) THEN
        RAISE WARNING 'pg_stat_database视图不存在';
        RETURN;
    END IF;

    SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
    INTO cache_hit_ratio
    FROM pg_stat_database
    WHERE datname = current_database();

    SELECT COUNT(*) INTO lock_waits
    FROM pg_stat_activity
    WHERE wait_event_type = 'Lock';

    SELECT SUM(n_dead_tup) INTO total_dead_tuples
    FROM pg_stat_user_tables;

    RAISE NOTICE '资源使用 - 缓存命中率: %%%, 锁等待: %, 总死元组: %',
                 COALESCE(cache_hit_ratio, 0), lock_waits, COALESCE(total_dead_tuples, 0);
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '资源使用查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    (SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
     FROM pg_stat_database WHERE datname = current_database()) as cache_hit_ratio,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE wait_event_type = 'Lock') as lock_waits,
    (SELECT SUM(n_dead_tup) FROM pg_stat_user_tables) as total_dead_tuples;
-- 执行时间: <100ms
-- 计划: Result -> SubPlan

-- 4. 复制状态（如果有，带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_replication'
    ) THEN
        RAISE WARNING 'pg_stat_replication视图不存在，可能没有配置复制';
        RETURN;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_current_wal_lsn') THEN
        RAISE EXCEPTION 'pg_current_wal_lsn函数不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_wal_lsn_diff') THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff函数不存在';
    END IF;

    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    IF replication_count > 0 THEN
        RAISE NOTICE '发现 % 个复制连接', replication_count;
    ELSE
        RAISE NOTICE '未发现复制连接';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION '必要的WAL函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '复制状态查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    application_name,
    state,
    sync_state,
    pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) as replication_lag_bytes
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan
```

---

## 故障排查指南

### 常见监控问题

**问题1: 指标数据缺失**:

```sql
-- 检查扩展是否启用（带错误处理和性能测试）
DO $$
DECLARE
    extension_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_extension'
    ) THEN
        RAISE WARNING 'pg_extension表不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO extension_count
    FROM pg_extension
    WHERE extname IN ('pg_stat_statements', 'pg_stat_monitor');

    IF extension_count > 0 THEN
        RAISE NOTICE '发现 % 个监控扩展已启用', extension_count;
    ELSE
        RAISE WARNING '未发现监控扩展（pg_stat_statements或pg_stat_monitor）';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_extension表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查扩展失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_extension WHERE extname IN ('pg_stat_statements', 'pg_stat_monitor');
-- 执行时间: <10ms
-- 计划: Seq Scan

-- 检查统计信息是否收集（带错误处理和性能测试）
DO $$
DECLARE
    unanalyzed_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_user_tables'
    ) THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO unanalyzed_count
    FROM pg_stat_user_tables
    WHERE last_analyze IS NULL AND last_autoanalyze IS NULL;

    IF unanalyzed_count > 0 THEN
        RAISE WARNING '发现 % 个表未收集统计信息', unanalyzed_count;
    ELSE
        RAISE NOTICE '所有表都已收集统计信息';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查统计信息失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT schemaname, tablename, last_analyze, last_autoanalyze
FROM pg_stat_user_tables
WHERE last_analyze IS NULL AND last_autoanalyze IS NULL;
-- 执行时间: <100ms（取决于表数量）
-- 计划: Seq Scan

-- 手动触发统计信息收集（带错误处理）
DO $$
BEGIN
    ANALYZE;
    RAISE NOTICE '统计信息收集完成';
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '统计信息收集失败: %', SQLERRM;
END $$;
```

**问题2: 告警误报**:

```yaml
# 调整告警阈值
# 根据实际业务负载调整告警阈值
# 例如：连接数告警阈值从80%调整为85%
```

**问题3: Grafana面板无数据**:

```bash
# 检查Prometheus数据源连接
# 在Grafana中测试数据源连接

# 检查postgres_exporter是否运行
systemctl status postgres_exporter

# 检查Prometheus抓取配置
curl http://localhost:9187/metrics | head -20
```

### 监控性能优化

```sql
-- 1. 限制pg_stat_statements记录数
ALTER SYSTEM SET pg_stat_statements.max = 10000;
SELECT pg_reload_conf();

-- 2. 定期重置统计信息（如果需要）
SELECT pg_stat_statements_reset();

-- 3. 优化监控查询
-- 使用物化视图缓存常用监控数据
CREATE MATERIALIZED VIEW monitoring_summary AS
SELECT
    NOW() as check_time,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active') as active_queries,
    (SELECT AVG(mean_exec_time) FROM pg_stat_statements WHERE calls > 100) as avg_query_time
WITH DATA;

-- 定期刷新
REFRESH MATERIALIZED VIEW CONCURRENTLY monitoring_summary;
```

---

## 交叉引用

### 相关文档

- ⭐⭐⭐ [监控与诊断](../监控与诊断/06.01-监控与诊断.md) - 监控理论基础和指标体系
- ⭐⭐⭐ [监控与诊断落地指南](../监控与诊断/06.02-监控与诊断落地指南.md) - Prometheus + Grafana完整部署
- ⭐⭐ [性能调优变更闭环](./01-性能调优-变更闭环.md) - 性能调优流程
- ⭐⭐ [性能问题案例库](../监控与诊断/06.04-性能问题案例库.md) - 实际性能问题案例
- ⭐⭐ [日志与可观测性-落地指南](./06-日志与可观测性-落地指南.md) - 日志配置和采集

### 外部资源

- [PostgreSQL监控文档](https://www.postgresql.org/docs/current/monitoring-stats.html)
- [pg_stat_statements文档](https://www.postgresql.org/docs/current/pgstatstatements.html)
- [Prometheus官方文档](https://prometheus.io/docs/)
- [Grafana官方文档](https://grafana.com/docs/)

---

**文档版本**: v1.0
**最后更新**: 2025-11-22
**PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
**维护者**: Documentation Team
