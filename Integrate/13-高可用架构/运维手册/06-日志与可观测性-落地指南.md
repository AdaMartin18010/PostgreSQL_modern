...
> **📋 文档来源**: `PostgreSQL\06-运维实践\运维手册\06-日志与可观测性-落地指南.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    ...
    IF NOT extension_exists THEN
        RAISE WARNING 'pg_stat_statements扩展未安装，无法更新慢SQL指纹';
        RETURN;
    END IF;
    ...
    RAISE NOTICE '慢SQL指纹更新完成，处理 % 条记录', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表slow_query_fingerprints或pg_stat_statements不存在';
    WHEN OTHERS THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'service_query_mapping'
    ) THEN
        CREATE TABLE service_query_mapping (
        ...
        RAISE NOTICE '表service_query_mapping创建成功';
    ELSE
        RAISE NOTICE '表service_query_mapping已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表service_query_mapping已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建服务-查询映射表失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表service_query_mapping不存在，请先创建';
    END IF;
    ...
    IF NOT extension_exists THEN
        RAISE WARNING 'pg_stat_statements扩展未安装，无法更新服务-查询映射';
        RETURN;
    END IF;
    ...
    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    RAISE NOTICE '成功插入 % 条服务-查询映射记录', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表service_query_mapping或pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '定期更新映射失败: %', SQLERRM;
END $$;

-- 查询服务级别的慢SQL（带错误处理和性能测试）
DO $$
DECLARE
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '表service_query_mapping不存在，请先创建';
        RETURN;
    END IF;
    ...
    IF slow_query_count > 0 THEN
        RAISE NOTICE '发现 % 条服务级别的慢SQL', slow_query_count;
    ELSE
        RAISE NOTICE '未发现服务级别的慢SQL';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表或视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询服务级别的慢SQL失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF activity_count > 0 THEN
        RAISE NOTICE '发现 % 个order-service相关的活动会话', activity_count;
    ELSE
        RAISE NOTICE '未发现order-service相关的活动会话';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询追踪信息失败: %', SQLERRM;
END $$;
