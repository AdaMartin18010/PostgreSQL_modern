...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING 'pg_stat_wal_summarizer视图不存在（可能需要PostgreSQL 18+）';
        RETURN;
    END IF;
    ...
    IF summarizer_count > 0 THEN
        RAISE NOTICE 'WAL Summarizer已启用，发现 % 条记录', summarizer_count;
    ELSE
        RAISE NOTICE 'WAL Summarizer未启用或未发现记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_wal_summarizer视图不存在（需要PostgreSQL 18+）';
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF is_in_recovery THEN
        RAISE NOTICE '当前节点处于恢复模式';
    ELSE
        RAISE NOTICE '当前节点已退出恢复模式';
    END IF;
EXCEPTION
...
BEGIN
    ...
    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库，无法检查恢复点';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_last_wal_replay_timestamp') THEN
        RAISE EXCEPTION 'pg_last_wal_replay_timestamp函数不存在';
    END IF;

    RAISE NOTICE '正在检查恢复点';
EXCEPTION
    WHEN undefined_function THEN
...
BEGIN
    ...
    RAISE NOTICE '系统一致性检查: pg_class记录数=%, pg_attribute记录数=%', class_count, attribute_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_class或pg_attribute表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '系统一致性检查失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '业务数据一致性检查失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'RTO/RPO记录表backup_rto_rpo_log创建成功';
    ELSE
        RAISE NOTICE 'RTO/RPO记录表backup_rto_rpo_log已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING 'RTO/RPO记录表backup_rto_rpo_log已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建RTO/RPO记录表失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表backup_rto_rpo_log不存在，请先创建';
    END IF;
    ...
    IF inserted_id IS NOT NULL THEN
        RAISE NOTICE 'RTO/RPO记录插入成功，log_id: %', inserted_id;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表backup_rto_rpo_log不存在';
    WHEN check_violation THEN
        RAISE WARNING 'RTO/RPO记录违反约束条件';
    WHEN OTHERS THEN
        RAISE EXCEPTION '插入RTO/RPO记录失败: %', SQLERRM;
END $$;
```
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE '备份记录表backup_records创建成功';
    ELSE
        RAISE NOTICE '备份记录表backup_records已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '备份记录表backup_records已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建备份记录表失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表backup_records不存在，请先创建';
    END IF;
    ...
    IF inserted_id IS NOT NULL THEN
        RAISE NOTICE '备份记录插入成功，backup_id: %', inserted_id;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表backup_records不存在';
    WHEN check_violation THEN
        RAISE WARNING '备份记录违反约束条件';
    WHEN OTHERS THEN
        RAISE EXCEPTION '插入备份记录失败: %', SQLERRM;
END $$;

-- 查询RTO/RPO趋势（带错误处理和性能测试）
DO $$
DECLARE
    trend_count INT;
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '表backup_records不存在，无法查询RTO/RPO趋势';
        RETURN;
    END IF;
    ...
    IF trend_count > 0 THEN
        RAISE NOTICE '发现 % 条成功的备份记录', trend_count;
    ELSE
        RAISE NOTICE '未发现成功的备份记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表backup_records不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询RTO/RPO趋势失败: %', SQLERRM;
END $$;
...
-- 执行时间: <100ms（取决于记录数量）
-- 计划: GroupAggregate -> Sort -> Seq Scan
```
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_wal_summarizer视图不存在（需要PostgreSQL 18+）';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查WAL Summarizer状态失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF wal_level_val NOT IN ('replica', 'logical') THEN
        RAISE WARNING 'wal_level应该是replica或logical，当前值: %', wal_level_val;
    END IF;

    IF archive_mode_val != 'on' THEN
        RAISE WARNING 'archive_mode应该是on，当前值: %', archive_mode_val;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查配置失败: %', SQLERRM;
END $$;
```
