---

> **📋 文档来源**: `PostgreSQL\06-运维实践\运维手册\01-性能调优-变更闭环.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 性能调优-变更闭环（Runbook）

> **文档版本**: v1.0
> **最后更新**: 2025-11-22
> **PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)

---

## 📋 目录

- [性能调优-变更闭环（Runbook）](#性能调优-变更闭环runbook)
  - [📋 目录](#-目录)
  - [1. 目标](#1-目标)
  - [2. 前置检查](#2-前置检查)
    - [2.1 业务窗口确认](#21-业务窗口确认)
    - [2.2 备份准备](#22-备份准备)
    - [2.3 回滚脚本准备](#23-回滚脚本准备)
    - [2.4 指标面板准备](#24-指标面板准备)
    - [2.5 变更前检查清单](#25-变更前检查清单)
  - [3. 诊断入口](#3-诊断入口)
    - [3.1 诊断SQL脚本](#31-诊断sql脚本)
    - [3.2 问题定位流程](#32-问题定位流程)
  - [4. 变更票据模板](#4-变更票据模板)
    - [4.1 变更记录表](#41-变更记录表)
    - [4.2 变更票据模板（详细）](#42-变更票据模板详细)
    - [4.3 变更审批流程](#43-变更审批流程)
  - [5. 执行步骤](#5-执行步骤)
    - [5.1 变更前审计脚本](#51-变更前审计脚本)
    - [5.2 参数变更执行脚本](#52-参数变更执行脚本)
    - [5.3 索引变更执行脚本](#53-索引变更执行脚本)
    - [5.4 观察期监控脚本](#54-观察期监控脚本)
  - [6. 常用参数建议（详细）](#6-常用参数建议详细)
    - [6.1 连接与并发参数](#61-连接与并发参数)
    - [6.2 缓冲参数](#62-缓冲参数)
    - [6.3 并行和JIT参数](#63-并行和jit参数)
    - [6.4 I/O参数](#64-io参数)
    - [6.5 检查点和WAL参数](#65-检查点和wal参数)
    - [6.6 维护参数](#66-维护参数)
    - [6.7 参数变更检查清单](#67-参数变更检查清单)
  - [7. 验证清单](#7-验证清单)
    - [7.1 验证脚本](#71-验证脚本)
    - [7.2 验证标准](#72-验证标准)
  - [8. 回滚与复盘](#8-回滚与复盘)
    - [8.1 回滚脚本](#81-回滚脚本)
    - [8.2 复盘模板](#82-复盘模板)
    - [8.3 复盘查询](#83-复盘查询)

---

## 1. 目标

- 在生产环境中以最小风险完成参数/SQL/结构优化，形成“监测→诊断→定位→变更→验证→回滚/固化”的闭环。
- 对齐文档：`../05-部署架构/单机部署/05.02-性能调优实践.md`、`../监控与诊断/06.01-监控与诊断.md`。

## 2. 前置检查

- 业务窗口：确认灰度范围与低峰时段；
- 备份/回滚：配置备份点/快照；准备回滚脚本（参数旧值、DDL 回滚方案）。
- 指标面板：P95/P99、TPS/QPS、命中率、WAL 速率、检查点、锁等待、Autovacuum 滞后。

### 2.1 业务窗口确认

```bash
# 检查当前业务负载
psql -c "
SELECT
    date_trunc('hour', now()) as hour,
    COUNT(*) as active_connections,
    COUNT(*) FILTER (WHERE state = 'active') as active_queries
FROM pg_stat_activity
WHERE datname = current_database()
GROUP BY 1
ORDER BY 1 DESC
LIMIT 24;
"

# 确认低峰时段（示例：凌晨2-6点）
LOW_TRAFFIC_WINDOW="02:00-06:00"

# 检查当前时间是否在低峰窗口
CURRENT_HOUR=$(date +%H)
if [ "${CURRENT_HOUR}" -ge 2 ] && [ "${CURRENT_HOUR}" -lt 6 ]; then
    echo "当前处于低峰时段，可以进行变更"
else
    echo "警告：当前不在低峰时段，建议在低峰时段执行变更"
fi
```

### 2.2 备份准备

```bash
# 1. 创建参数备份
psql -c "
SELECT name, setting, source
FROM pg_settings
WHERE name IN ('shared_buffers', 'work_mem', 'max_connections')
ORDER BY name;
" > /tmp/pg_params_backup_$(date +%Y%m%d_%H%M%S).txt

# 2. 创建配置备份
cp $PGDATA/postgresql.conf $PGDATA/postgresql.conf.backup_$(date +%Y%m%d_%H%M%S)

# 3. 创建执行计划基线
psql -c "
SELECT queryid, query, plan
FROM pg_stat_statements
ORDER BY total_exec_time DESC
LIMIT 10;
" > /tmp/query_plans_baseline_$(date +%Y%m%d_%H%M%S).txt

# 4. 创建数据库快照（如果支持）
# pg_dump -F c -d mydb -f mydb_backup_$(date +%Y%m%d_%H%M%S).dump
```

### 2.3 回滚脚本准备

```bash
# 创建回滚脚本模板
cat > /tmp/rollback_script_$(date +%Y%m%d_%H%M%S).sh <<'EOF'
#!/bin/bash
# 回滚脚本

# 1. 恢复参数
psql -c "ALTER SYSTEM SET shared_buffers = '256MB';"
psql -c "SELECT pg_reload_conf();"

# 2. 恢复配置
# cp $PGDATA/postgresql.conf.backup_* $PGDATA/postgresql.conf

# 3. 删除新创建的索引（如果变更涉及索引）
# DROP INDEX CONCURRENTLY IF EXISTS idx_new_name;
EOF

chmod +x /tmp/rollback_script_*.sh
```

### 2.4 指标面板准备

```sql
-- 创建性能基线视图
CREATE OR REPLACE VIEW performance_baseline AS
SELECT
    NOW() as baseline_time,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active') as active_queries,
    (SELECT SUM(total_exec_time) FROM pg_stat_statements) as total_exec_time,
    (SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
     FROM pg_stat_database WHERE datname = current_database()) as cache_hit_ratio,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE wait_event_type IS NOT NULL) as waiting_queries;

-- 创建性能监控表（如果不存在，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'performance_metrics'
    ) THEN
        CREATE TABLE performance_metrics (
            metric_id SERIAL PRIMARY KEY,
            metric_time TIMESTAMP DEFAULT NOW(),
            active_queries INTEGER,
            total_exec_time NUMERIC,
            cache_hit_ratio NUMERIC,
            waiting_queries INTEGER,
            tps NUMERIC,
            qps NUMERIC
        );
        RAISE NOTICE '表performance_metrics创建成功';
    ELSE
        RAISE NOTICE '表performance_metrics已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表performance_metrics已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建性能监控表失败: %', SQLERRM;
END $$;

-- 记录当前性能指标（带错误处理）
DO $$
DECLARE
    active_queries INT;
    total_exec_time NUMERIC;
    cache_hit_ratio NUMERIC;
    waiting_queries INT;
    tps_count INT;
    qps_count BIGINT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'performance_metrics'
    ) THEN
        RAISE EXCEPTION '表performance_metrics不存在，请先创建';
    END IF;

    SELECT COUNT(*) INTO active_queries
    FROM pg_stat_activity
    WHERE state = 'active';

    SELECT COALESCE(SUM(total_exec_time), 0) INTO total_exec_time
    FROM pg_stat_statements;

    SELECT COALESCE(
        SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100,
        0
    ) INTO cache_hit_ratio
    FROM pg_stat_database
    WHERE datname = current_database();

    SELECT COUNT(*) INTO waiting_queries
    FROM pg_stat_activity
    WHERE wait_event_type IS NOT NULL;

    SELECT COUNT(*) INTO tps_count
    FROM pg_stat_database
    WHERE datname = current_database();

    SELECT COALESCE(SUM(calls), 0) INTO qps_count
    FROM pg_stat_statements;

    INSERT INTO performance_metrics (
        active_queries,
        total_exec_time,
        cache_hit_ratio,
        waiting_queries,
        tps,
        qps
    ) VALUES (
        active_queries,
        total_exec_time,
        cache_hit_ratio,
        waiting_queries,
        tps_count,
        qps_count
    );

    RAISE NOTICE '性能指标记录成功: 活跃查询=%, 总执行时间=%, 缓存命中率=%, 等待查询=%, TPS=%, QPS=%',
        active_queries, total_exec_time, cache_hit_ratio, waiting_queries, tps_count, qps_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表performance_metrics或相关视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录性能指标失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    (SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active'),
    (SELECT SUM(total_exec_time) FROM pg_stat_statements),
    (SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
     FROM pg_stat_database WHERE datname = current_database()),
    (SELECT COUNT(*) FROM pg_stat_activity WHERE wait_event_type IS NOT NULL),
    (SELECT COUNT(*) FROM pg_stat_database WHERE datname = current_database()),
    (SELECT SUM(calls) FROM pg_stat_statements);
-- 执行时间: <100ms
-- 计划: SubPlan
```

### 2.5 变更前检查清单

```bash
# 完整检查清单脚本
cat > /tmp/pre_change_checklist.sh <<'EOF'
#!/bin/bash

echo "=== 变更前检查清单 ==="

# 1. 检查数据库连接
if ! psql -c "SELECT 1;" > /dev/null 2>&1; then
    echo "❌ 数据库连接失败"
    exit 1
else
    echo "✅ 数据库连接正常"
fi

# 2. 检查当前负载
ACTIVE_QUERIES=$(psql -t -c "SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active';")
if [ "${ACTIVE_QUERIES}" -gt 10 ]; then
    echo "⚠️  当前活跃查询数: ${ACTIVE_QUERIES}，建议在低峰时段执行"
else
    echo "✅ 当前活跃查询数: ${ACTIVE_QUERIES}"
fi

# 3. 检查长事务
LONG_TXNS=$(psql -t -c "SELECT COUNT(*) FROM pg_stat_activity WHERE state != 'idle' AND NOW() - xact_start > INTERVAL '5 minutes';")
if [ "${LONG_TXNS}" -gt 0 ]; then
    echo "⚠️  存在长事务: ${LONG_TXNS} 个"
else
    echo "✅ 无长事务"
fi

# 4. 检查锁等待
LOCK_WAITS=$(psql -t -c "SELECT COUNT(*) FROM pg_stat_activity WHERE wait_event_type = 'Lock';")
if [ "${LOCK_WAITS}" -gt 5 ]; then
    echo "⚠️  锁等待数: ${LOCK_WAITS}，建议等待锁释放后再执行变更"
else
    echo "✅ 锁等待数: ${LOCK_WAITS}"
fi

# 5. 检查备份状态
if [ -f "$PGDATA/postgresql.conf.backup_*" ]; then
    echo "✅ 配置文件已备份"
else
    echo "⚠️  配置文件未备份，建议先备份"
fi

echo "=== 检查完成 ==="
EOF

chmod +x /tmp/pre_change_checklist.sh
/tmp/pre_change_checklist.sh
```

## 3. 诊断入口

1) 慢查询/吞吐下降/延迟升高告警触发；
2) 执行 `sql/diagnostics.sql` 中 TopN + 等待事件；
3) 对疑似语句 `EXPLAIN (ANALYZE, BUFFERS, TIMING)`；
4) 评估策略：索引/统计/SQL/参数/架构。

### 3.1 诊断SQL脚本

**完整诊断脚本** (`sql/diagnostics.sql`):

```sql
-- ============================================
-- PostgreSQL 性能诊断脚本
-- ============================================

-- 1. 当前活跃会话和等待事件
SELECT
    pid,
    usename,
    application_name,
    state,
    wait_event_type,
    wait_event,
    query_start,
    state_change,
    LEFT(query, 100) as query_preview
FROM pg_stat_activity
WHERE state != 'idle'
ORDER BY query_start;

-- 2. Top 10 慢查询（需要pg_stat_statements扩展）
SELECT
    LEFT(query, 100) as query_preview,
    calls,
    round(mean_exec_time::numeric, 2) as mean_ms,
    round(max_exec_time::numeric, 2) as max_ms,
    round((100.0 * shared_blks_hit / NULLIF(shared_blks_hit + shared_blks_read, 0))::numeric, 2) as hit_ratio,
    rows
FROM pg_stat_statements
WHERE calls > 10
ORDER BY mean_exec_time DESC
LIMIT 10;

-- 3. 锁等待情况
SELECT
    blocked_locks.pid AS blocked_pid,
    blocked_activity.usename AS blocked_user,
    blocking_locks.pid AS blocking_pid,
    blocking_activity.usename AS blocking_user,
    blocked_activity.query AS blocked_statement,
    blocking_activity.query AS blocking_statement
FROM pg_catalog.pg_locks blocked_locks
JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
JOIN pg_catalog.pg_locks blocking_locks
    ON blocking_locks.locktype = blocked_locks.locktype
    AND blocking_locks.database IS NOT DISTINCT FROM blocked_locks.database
    AND blocking_locks.relation IS NOT DISTINCT FROM blocked_locks.relation
    AND blocking_locks.page IS NOT DISTINCT FROM blocked_locks.page
    AND blocking_locks.tuple IS NOT DISTINCT FROM blocked_locks.tuple
    AND blocking_locks.virtualxid IS NOT DISTINCT FROM blocked_locks.virtualxid
    AND blocking_locks.transactionid IS NOT DISTINCT FROM blocked_locks.transactionid
    AND blocking_locks.classid IS NOT DISTINCT FROM blocked_locks.classid
    AND blocking_locks.objid IS NOT DISTINCT FROM blocked_locks.objid
    AND blocking_locks.objsubid IS NOT DISTINCT FROM blocked_locks.objsubid
    AND blocking_locks.pid != blocked_locks.pid
JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted;

-- 4. 缓冲区命中率
SELECT
    datname,
    round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2) as hit_ratio
FROM pg_stat_database
GROUP BY datname;

-- 5. 表扫描统计
SELECT
    schemaname,
    tablename,
    seq_scan,
    idx_scan,
    round(100.0 * idx_scan / NULLIF(seq_scan + idx_scan, 0), 2) as idx_scan_ratio,
    n_live_tup,
    n_dead_tup,
    round(100.0 * n_dead_tup / NULLIF(n_live_tup + n_dead_tup, 0), 2) as dead_ratio
FROM pg_stat_user_tables
WHERE seq_scan + idx_scan > 0
ORDER BY seq_scan DESC
LIMIT 20;

-- 6. 索引使用情况
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan,
    pg_size_pretty(pg_relation_size(indexname::regclass)) as index_size
FROM pg_stat_user_indexes
WHERE idx_scan = 0
AND pg_relation_size(indexname::regclass) > 1000000
ORDER BY pg_relation_size(indexname::regclass) DESC;

-- 7. WAL和检查点统计
SELECT
    checkpoints_timed,
    checkpoints_req,
    checkpoint_write_time,
    checkpoint_sync_time,
    buffers_checkpoint,
    buffers_clean,
    buffers_backend
FROM pg_stat_bgwriter;

-- 8. Autovacuum状态
SELECT
    schemaname,
    tablename,
    last_vacuum,
    last_autovacuum,
    last_analyze,
    last_autoanalyze,
    n_dead_tup,
    n_live_tup
FROM pg_stat_user_tables
WHERE n_dead_tup > 1000
ORDER BY n_dead_tup DESC;
```

### 3.2 问题定位流程

**性能问题定位决策树**:

```sql
-- 步骤1: 检查等待事件
SELECT wait_event_type, wait_event, COUNT(*) as count
FROM pg_stat_activity
WHERE state != 'idle'
GROUP BY wait_event_type, wait_event
ORDER BY count DESC;

-- 如果等待事件是：
-- - I/O: 检查缓冲区命中率、磁盘I/O性能
-- - Lock: 检查锁等待、死锁
-- - CPU: 检查慢查询、索引使用情况
-- - LWLock: 检查WAL写入、检查点

-- 步骤2: 分析慢查询
SELECT query, calls, mean_exec_time, max_exec_time
FROM pg_stat_statements
WHERE mean_exec_time > 100  -- 平均执行时间>100ms
ORDER BY mean_exec_time DESC
LIMIT 10;

-- 步骤3: 对慢查询执行EXPLAIN
-- EXPLAIN (ANALYZE, BUFFERS, TIMING) <慢查询SQL>;

-- 步骤4: 根据EXPLAIN结果选择优化策略
-- - Seq Scan: 考虑创建索引
-- - 高cost: 考虑优化查询、更新统计信息
-- - 高buffers: 考虑增加shared_buffers
```

## 4. 变更票据模板

```text
变更项：<参数/SQL/索引/结构>
对象/范围：<库/表/索引/服务>
旧值→新值：<从> → <到>
预期影响：<延迟/吞吐/资源>
验证窗口：<起止时间/灰度比例>
回滚条件：<P95 恶化>10% / 错误率升 / 锁冲突升>
负责人/审批：<Owner/Reviewer>
```

### 4.1 变更记录表

```sql
-- 创建变更记录表
CREATE TABLE IF NOT EXISTS performance_change_log (
    change_id SERIAL PRIMARY KEY,
    change_time TIMESTAMP DEFAULT NOW(),
    change_type VARCHAR(50),  -- 'parameter', 'index', 'sql', 'structure'
    change_target TEXT,
    old_value TEXT,
    new_value TEXT,
    change_by VARCHAR(100),
    expected_impact TEXT,
    verification_window TEXT,
    rollback_condition TEXT,
    change_status VARCHAR(20),  -- 'pending', 'in_progress', 'completed', 'rolled_back'
    actual_impact TEXT,
    notes TEXT
);

-- 记录变更
INSERT INTO performance_change_log (
    change_type,
    change_target,
    old_value,
    new_value,
    change_by,
    expected_impact,
    verification_window,
    rollback_condition
)
VALUES (
    'parameter',
    'shared_buffers',
    '256MB',
    '512MB',
    current_user,
    '提高缓存命中率5-10%',
    '2025-11-22 02:00 - 06:00',
    'P95延迟增加>10%或错误率上升'
);

-- 查询变更历史
SELECT
    change_id,
    change_time,
    change_type,
    change_target,
    old_value,
    new_value,
    change_status,
    actual_impact
FROM performance_change_log
ORDER BY change_time DESC
LIMIT 20;
```

### 4.2 变更票据模板（详细）

```text
==========================================
变更票据
==========================================
变更ID: CHG-2025-11-22-001
变更时间: 2025-11-22 02:00:00
变更类型: [ ] 参数 [✓] 索引 [ ] SQL [ ] 结构

变更项: shared_buffers
对象/范围: 数据库实例 postgres
旧值: 256MB
新值: 512MB

预期影响:
- 缓存命中率提升: 5-10%
- 内存使用增加: 256MB
- 性能提升: P95延迟降低5-10%

验证窗口:
- 开始时间: 2025-11-22 02:00:00
- 结束时间: 2025-11-22 06:00:00
- 观察期: 4小时

回滚条件:
- P95延迟增加>10%
- 错误率上升>5%
- 锁等待数增加>20%
- 内存使用超过阈值

负责人: DBA Team
审批人: Tech Lead
状态: [ ] 待审批 [✓] 已审批 [ ] 执行中 [ ] 已完成 [ ] 已回滚

变更前指标基线:
- 活跃查询数: 5
- 总执行时间: 12345.67ms
- 缓存命中率: 95.2%
- 等待查询数: 0

==========================================
```

### 4.3 变更审批流程

```bash
# 变更审批检查脚本
cat > /tmp/change_approval_check.sh <<'EOF'
#!/bin/bash

CHANGE_ID=$1
if [ -z "$CHANGE_ID" ]; then
    echo "用法: $0 <变更ID>"
    exit 1
fi

# 检查变更是否已审批
APPROVED=$(psql -t -c "
SELECT COUNT(*)
FROM performance_change_log
WHERE change_id = $CHANGE_ID
AND change_status = 'approved';
")

if [ "$APPROVED" -eq 0 ]; then
    echo "❌ 变更 $CHANGE_ID 未审批，无法执行"
    exit 1
else
    echo "✅ 变更 $CHANGE_ID 已审批，可以执行"
fi

# 检查变更窗口
CURRENT_HOUR=$(date +%H)
if [ "${CURRENT_HOUR}" -ge 2 ] && [ "${CURRENT_HOUR}" -lt 6 ]; then
    echo "✅ 当前处于低峰时段，可以执行变更"
else
    echo "⚠️  当前不在低峰时段，建议在低峰时段执行"
fi
EOF

chmod +x /tmp/change_approval_check.sh
```

## 5. 执行步骤

1) 建立审计：记录参数旧值（`SHOW`/`pg_file_settings`）与计划基线；
2) 单变量小步变更；
3) 观察 15–30 分钟稳定窗口；
4) 达标则固化与文档化；不达标则回滚。

### 5.1 变更前审计脚本

```sql
-- 1. 记录当前参数值（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        CREATE TABLE pg_tuning_audit (
            change_id SERIAL PRIMARY KEY,
            change_time TIMESTAMP DEFAULT NOW(),
            parameter_name TEXT,
            old_value TEXT,
            new_value TEXT,
            change_type TEXT,  -- 'parameter', 'index', 'sql', 'structure'
            change_reason TEXT,
            change_by TEXT
        );
        RAISE NOTICE '审计表pg_tuning_audit创建成功';
    ELSE
        RAISE NOTICE '审计表pg_tuning_audit已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '审计表pg_tuning_audit已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建审计表失败: %', SQLERRM;
END $$;

-- 记录参数变更（带错误处理）
DO $$
DECLARE
    inserted_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE EXCEPTION '表pg_tuning_audit不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_settings'
    ) THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    END IF;

    INSERT INTO pg_tuning_audit (parameter_name, old_value, change_type, change_reason, change_by)
    SELECT name, setting, 'parameter', 'Performance tuning', current_user
    FROM pg_settings
    WHERE name IN (
        'shared_buffers', 'work_mem', 'maintenance_work_mem',
        'effective_cache_size', 'max_connections',
        'checkpoint_timeout', 'max_wal_size', 'wal_buffers',
        'random_page_cost', 'effective_io_concurrency'
    );

    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    RAISE NOTICE '成功记录 % 个参数变更', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表pg_tuning_audit或pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录参数变更失败: %', SQLERRM;
END $$;

-- 2. 记录当前性能基线（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_performance_baseline'
    ) THEN
        CREATE TABLE pg_performance_baseline (
            baseline_id SERIAL PRIMARY KEY,
            baseline_time TIMESTAMP DEFAULT NOW(),
            metric_name TEXT,
            metric_value NUMERIC,
            notes TEXT
        );
        RAISE NOTICE '性能基线表pg_performance_baseline创建成功';
    ELSE
        RAISE NOTICE '性能基线表pg_performance_baseline已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '性能基线表pg_performance_baseline已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建性能基线表失败: %', SQLERRM;
END $$;

-- 记录关键指标（带错误处理和性能测试）
DO $$
DECLARE
    inserted_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_performance_baseline'
    ) THEN
        RAISE EXCEPTION '表pg_performance_baseline不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_database'
    ) THEN
        RAISE EXCEPTION 'pg_stat_database视图不存在';
    END IF;

    INSERT INTO pg_performance_baseline (metric_name, metric_value, notes)
    SELECT 'buffer_hit_ratio',
           round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2),
           'Database: ' || datname
    FROM pg_stat_database
    WHERE datname = current_database()
    GROUP BY datname;

    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    RAISE NOTICE '成功记录 % 条性能基线指标', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表pg_performance_baseline或pg_stat_database视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算缓冲区命中率时出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录性能基线失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT 'buffer_hit_ratio',
       round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2),
       'Database: ' || datname
FROM pg_stat_database
WHERE datname = current_database()
GROUP BY datname;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 记录当前慢查询基线（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_query_baseline'
    ) THEN
        CREATE TABLE pg_query_baseline (
            baseline_id SERIAL PRIMARY KEY,
            baseline_time TIMESTAMP DEFAULT NOW(),
            query_id BIGINT,
            query TEXT,
            calls BIGINT,
            mean_exec_time NUMERIC,
            max_exec_time NUMERIC
        );
        RAISE NOTICE '慢查询基线表pg_query_baseline创建成功';
    ELSE
        RAISE NOTICE '慢查询基线表pg_query_baseline已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '慢查询基线表pg_query_baseline已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建慢查询基线表失败: %', SQLERRM;
END $$;

-- 记录Top 20慢查询（带错误处理和性能测试）
DO $$
DECLARE
    inserted_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_query_baseline'
    ) THEN
        RAISE EXCEPTION '表pg_query_baseline不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先安装pg_stat_statements扩展';
        RETURN;
    END IF;

    INSERT INTO pg_query_baseline (query_id, query, calls, mean_exec_time, max_exec_time)
    SELECT queryid, LEFT(query, 500), calls, mean_exec_time, max_exec_time
    FROM pg_stat_statements
    ORDER BY mean_exec_time DESC
    LIMIT 20;

    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    RAISE NOTICE '成功记录 % 条慢查询基线', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表pg_query_baseline或pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录慢查询基线失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT queryid, LEFT(query, 500), calls, mean_exec_time, max_exec_time
FROM pg_stat_statements
ORDER BY mean_exec_time DESC
LIMIT 20;
-- 执行时间: <100ms（取决于pg_stat_statements数据量）
-- 计划: Limit -> Sort -> Seq Scan
```

### 5.2 参数变更执行脚本

```sql
-- 示例：调整shared_buffers
-- 步骤1: 记录旧值
DO $$
DECLARE
    old_value TEXT;
BEGIN
    SELECT setting INTO old_value FROM pg_settings WHERE name = 'shared_buffers';
    INSERT INTO pg_tuning_audit (parameter_name, old_value, change_type, change_reason, change_by)
    VALUES ('shared_buffers', old_value, 'parameter', 'Increase buffer for better cache hit ratio', current_user);
END $$;

-- 步骤2: 应用新值（需要重启，带错误处理）
-- 方法1: 使用ALTER SYSTEM（推荐，带错误处理）
DO $$
DECLARE
    old_value TEXT;
BEGIN
    SELECT setting INTO old_value FROM pg_settings WHERE name = 'shared_buffers';

    ALTER SYSTEM SET shared_buffers = '4GB';
    RAISE NOTICE '参数shared_buffers已从%更新为4GB', old_value;

    PERFORM pg_reload_conf();
    RAISE NOTICE '配置已重新加载（注意：shared_buffers需要重启才能生效）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效，请检查shared_buffers的值';
    WHEN OTHERS THEN
        RAISE EXCEPTION '参数变更失败: %', SQLERRM;
END $$;

-- 方法2: 直接修改postgresql.conf
-- shared_buffers = 4GB

-- 步骤3: 验证变更（带错误处理和性能测试）
DO $$
DECLARE
    current_value TEXT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_settings'
    ) THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    END IF;

    SELECT setting INTO current_value
    FROM pg_settings
    WHERE name = 'shared_buffers';

    RAISE NOTICE '当前shared_buffers值: %', current_value;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证变更失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT name, setting, unit, source
FROM pg_settings
WHERE name = 'shared_buffers';
-- 执行时间: <10ms
-- 计划: Seq Scan

-- 步骤4: 更新审计记录（带错误处理）
DO $$
DECLARE
    updated_count INT;
    new_value TEXT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，跳过更新';
        RETURN;
    END IF;

    SELECT setting INTO new_value
    FROM pg_settings
    WHERE name = 'shared_buffers';

    UPDATE pg_tuning_audit
    SET new_value = new_value
    WHERE parameter_name = 'shared_buffers'
    AND change_id = (SELECT MAX(change_id) FROM pg_tuning_audit WHERE parameter_name = 'shared_buffers');

    GET DIAGNOSTICS updated_count = ROW_COUNT;
    IF updated_count > 0 THEN
        RAISE NOTICE '成功更新 % 条审计记录', updated_count;
    ELSE
        RAISE WARNING '未找到需要更新的审计记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表pg_tuning_audit或pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '更新审计记录失败: %', SQLERRM;
END $$;
```

### 5.3 索引变更执行脚本

```sql
-- 示例：创建新索引
-- 步骤1: 记录变更（带错误处理）
DO $$
DECLARE
    inserted_id INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，跳过记录';
        RETURN;
    END IF;

    INSERT INTO pg_tuning_audit (parameter_name, old_value, change_type, change_reason, change_by)
    VALUES ('idx_orders_customer_date', 'NOT EXISTS', 'index', 'Optimize customer order history query', current_user)
    RETURNING change_id INTO inserted_id;

    RAISE NOTICE '变更记录成功，change_id: %', inserted_id;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表pg_tuning_audit不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录变更失败: %', SQLERRM;
END $$;

-- 步骤2: 创建索引（使用CONCURRENTLY避免锁表，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND indexname = 'idx_orders_customer_date'
    ) THEN
        RAISE WARNING '索引idx_orders_customer_date已存在，跳过创建';
        RETURN;
    END IF;

    CREATE INDEX CONCURRENTLY idx_orders_customer_date ON orders (customer_id, order_date DESC);
    RAISE NOTICE '索引idx_orders_customer_date创建成功（并发模式）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_object THEN
        RAISE WARNING '索引idx_orders_customer_date已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建索引失败: %', SQLERRM;
END $$;

-- 步骤3: 验证索引创建（带错误处理和性能测试）
DO $$
DECLARE
    index_exists BOOLEAN;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_indexes'
    ) THEN
        RAISE WARNING 'pg_indexes视图不存在';
        RETURN;
    END IF;

    SELECT EXISTS(
        SELECT 1 FROM pg_indexes
        WHERE indexname = 'idx_orders_customer_date'
    ) INTO index_exists;

    IF index_exists THEN
        RAISE NOTICE '索引idx_orders_customer_date验证成功';
    ELSE
        RAISE WARNING '索引idx_orders_customer_date不存在';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证索引创建失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname,
    indexdef
FROM pg_indexes
WHERE indexname = 'idx_orders_customer_date';
-- 执行时间: <10ms
-- 计划: Seq Scan

-- 步骤4: 测试查询性能（带错误处理和性能测试）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;
    RAISE NOTICE '查询性能测试完成';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '测试查询性能失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM orders
WHERE customer_id = 100
ORDER BY order_date DESC
LIMIT 20;
-- 执行时间: <50ms（取决于数据量和索引）
-- 计划: Limit -> Index Scan (如果使用了索引) 或 Seq Scan
```

### 5.4 观察期监控脚本

```sql
-- 变更后15-30分钟观察期监控
-- 每5分钟执行一次，记录关键指标

-- 创建监控表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_monitoring'
    ) THEN
        CREATE TABLE pg_tuning_monitoring (
            monitor_id SERIAL PRIMARY KEY,
            monitor_time TIMESTAMP DEFAULT NOW(),
            change_id INTEGER REFERENCES pg_tuning_audit(change_id),
            buffer_hit_ratio NUMERIC,
            avg_query_time NUMERIC,
            active_connections INTEGER,
            lock_waits INTEGER,
            checkpoint_frequency NUMERIC
        );
        RAISE NOTICE '监控表pg_tuning_monitoring创建成功';
    ELSE
        RAISE NOTICE '监控表pg_tuning_monitoring已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '监控表pg_tuning_monitoring已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建监控表失败: %', SQLERRM;
END $$;

-- 监控脚本（每5分钟执行，带错误处理）
DO $$
DECLARE
    inserted_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_monitoring'
    ) THEN
        RAISE EXCEPTION '表pg_tuning_monitoring不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，change_id将为空';
    END IF;

    INSERT INTO pg_tuning_monitoring (
        change_id,
        buffer_hit_ratio,
        avg_query_time,
        active_connections,
        lock_waits,
        checkpoint_frequency
    )
    SELECT
        (SELECT MAX(change_id) FROM pg_tuning_audit) as change_id,
        (SELECT round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2)
         FROM pg_stat_database WHERE datname = current_database()) as buffer_hit_ratio,
        (SELECT round(avg(mean_exec_time), 2)
         FROM pg_stat_statements WHERE calls > 10) as avg_query_time,
        (SELECT count(*) FROM pg_stat_activity WHERE state != 'idle') as active_connections,
        (SELECT count(*) FROM pg_locks WHERE NOT granted) as lock_waits,
        (SELECT round(extract(epoch from now() - stats_reset) / NULLIF(checkpoints_timed + checkpoints_req, 0), 2)
         FROM pg_stat_bgwriter) as checkpoint_frequency;

    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    RAISE NOTICE '成功插入 % 条监控记录', inserted_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '相关表或视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '插入监控记录失败: %', SQLERRM;
END $$;

-- 查看监控趋势（带错误处理和性能测试）
DO $$
DECLARE
    trend_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_monitoring'
    ) THEN
        RAISE WARNING '表pg_tuning_monitoring不存在，无法查看监控趋势';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO trend_count
    FROM pg_tuning_monitoring
    WHERE change_id = (SELECT MAX(change_id) FROM pg_tuning_audit);

    IF trend_count > 0 THEN
        RAISE NOTICE '发现 % 条监控趋势记录', trend_count;
    ELSE
        RAISE NOTICE '未发现监控趋势记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表pg_tuning_monitoring或pg_tuning_audit不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看监控趋势失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    monitor_time,
    buffer_hit_ratio,
    avg_query_time,
    active_connections,
    lock_waits
FROM pg_tuning_monitoring
WHERE change_id = (SELECT MAX(change_id) FROM pg_tuning_audit)
ORDER BY monitor_time DESC
LIMIT 10;
-- 执行时间: <50ms（取决于监控记录数量）
-- 计划: Limit -> Sort -> Seq Scan
```

## 6. 常用参数建议（详细）

### 6.1 连接与并发参数

```sql
-- max_connections: 最大连接数（带错误处理）
-- 建议值：根据应用连接池配置，通常100-500
-- 注意：每个连接消耗约10MB内存
DO $$
BEGIN
    ALTER SYSTEM SET max_connections = 200;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'max_connections已设置为200（注意：需要重启才能生效）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'max_connections值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置max_connections失败: %', SQLERRM;
END $$;

-- work_mem: 每个操作的工作内存（带错误处理）
-- 建议值：根据并发数和可用内存计算
-- 公式：work_mem = (可用内存 - shared_buffers) / (max_connections * 2)
-- 示例：16GB内存，4GB shared_buffers，200连接
-- work_mem = (16GB - 4GB) / (200 * 2) = 30MB
DO $$
BEGIN
    ALTER SYSTEM SET work_mem = '30MB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'work_mem已设置为30MB';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'work_mem值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置work_mem失败: %', SQLERRM;
END $$;

-- maintenance_work_mem: 维护操作内存（带错误处理）
-- 建议值：1-2GB（用于VACUUM、CREATE INDEX等）
DO $$
BEGIN
    ALTER SYSTEM SET maintenance_work_mem = '1GB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'maintenance_work_mem已设置为1GB';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'maintenance_work_mem值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置maintenance_work_mem失败: %', SQLERRM;
END $$;
```

### 6.2 缓冲参数

```sql
-- shared_buffers: 共享缓冲区（带错误处理）
-- 建议值：系统内存的25%（Linux）或40%（Windows）
-- 示例：16GB内存系统，设置4GB
DO $$
BEGIN
    ALTER SYSTEM SET shared_buffers = '4GB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'shared_buffers已设置为4GB（注意：需要重启才能生效）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'shared_buffers值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置shared_buffers失败: %', SQLERRM;
END $$;

-- effective_cache_size: 有效缓存大小（带错误处理）
-- 建议值：系统内存的50-75%
-- 示例：16GB内存系统，设置12GB
DO $$
BEGIN
    ALTER SYSTEM SET effective_cache_size = '12GB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'effective_cache_size已设置为12GB';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'effective_cache_size值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置effective_cache_size失败: %', SQLERRM;
END $$;
```

### 6.3 并行和JIT参数

```sql
-- max_parallel_workers_per_gather: 每个Gather节点的最大工作进程数（带错误处理）
-- 建议值：CPU核心数的50%
DO $$
BEGIN
    ALTER SYSTEM SET max_parallel_workers_per_gather = 4;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'max_parallel_workers_per_gather已设置为4';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'max_parallel_workers_per_gather值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置max_parallel_workers_per_gather失败: %', SQLERRM;
END $$;

-- max_parallel_workers: 系统最大并行工作进程数（带错误处理）
-- 建议值：CPU核心数
DO $$
BEGIN
    ALTER SYSTEM SET max_parallel_workers = 8;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'max_parallel_workers已设置为8';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'max_parallel_workers值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置max_parallel_workers失败: %', SQLERRM;
END $$;

-- jit: 启用JIT编译（PostgreSQL 11+，带错误处理）
-- 建议：对于复杂查询，启用JIT可以提升性能
DO $$
BEGIN
    ALTER SYSTEM SET jit = on;
    ALTER SYSTEM SET jit_above_cost = 100000;  -- 只对高cost查询启用JIT
    PERFORM pg_reload_conf();
    RAISE NOTICE 'JIT已启用，jit_above_cost已设置为100000';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'JIT参数值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置JIT参数失败: %', SQLERRM;
END $$;
```

### 6.4 I/O参数

```sql
-- random_page_cost: 随机页面访问成本（带错误处理）
-- SSD: 1.1-1.5，NVMe: 1.0-1.1，HDD: 4.0（默认）
DO $$
BEGIN
    ALTER SYSTEM SET random_page_cost = 1.1;  -- SSD环境
    PERFORM pg_reload_conf();
    RAISE NOTICE 'random_page_cost已设置为1.1（SSD环境）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'random_page_cost值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置random_page_cost失败: %', SQLERRM;
END $$;

-- effective_io_concurrency: 有效I/O并发数（PostgreSQL 18增强，带错误处理）
-- SSD: 200-300，NVMe: 300-500，HDD: 50-100
DO $$
BEGIN
    ALTER SYSTEM SET effective_io_concurrency = 200;  -- SSD环境
    PERFORM pg_reload_conf();
    RAISE NOTICE 'effective_io_concurrency已设置为200（SSD环境）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'effective_io_concurrency值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置effective_io_concurrency失败: %', SQLERRM;
END $$;

-- maintenance_io_concurrency: 维护操作I/O并发数（PostgreSQL 18新增，带错误处理）
DO $$
BEGIN
    ALTER SYSTEM SET maintenance_io_concurrency = 200;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'maintenance_io_concurrency已设置为200';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'maintenance_io_concurrency值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置maintenance_io_concurrency失败: %', SQLERRM;
END $$;
```

### 6.5 检查点和WAL参数

```sql
-- checkpoint_timeout: 检查点超时（带错误处理）
-- 建议值：15-30分钟
DO $$
BEGIN
    ALTER SYSTEM SET checkpoint_timeout = '15min';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'checkpoint_timeout已设置为15min';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'checkpoint_timeout值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置checkpoint_timeout失败: %', SQLERRM;
END $$;

-- max_wal_size: 最大WAL大小（带错误处理）
-- 建议值：根据写入负载，通常2-8GB
DO $$
BEGIN
    ALTER SYSTEM SET max_wal_size = '4GB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'max_wal_size已设置为4GB';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'max_wal_size值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置max_wal_size失败: %', SQLERRM;
END $$;

-- checkpoint_completion_target: 检查点完成目标（带错误处理）
-- 建议值：0.9（平滑写入）
DO $$
BEGIN
    ALTER SYSTEM SET checkpoint_completion_target = 0.9;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'checkpoint_completion_target已设置为0.9';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'checkpoint_completion_target值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置checkpoint_completion_target失败: %', SQLERRM;
END $$;

-- wal_compression: WAL压缩（带错误处理）
-- 建议：启用可以减少WAL大小
DO $$
BEGIN
    ALTER SYSTEM SET wal_compression = on;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'wal_compression已启用';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'wal_compression值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置wal_compression失败: %', SQLERRM;
END $$;

-- wal_buffers: WAL缓冲区（带错误处理）
-- 建议值：16-32MB（高写入负载）
DO $$
BEGIN
    ALTER SYSTEM SET wal_buffers = '16MB';
    PERFORM pg_reload_conf();
    RAISE NOTICE 'wal_buffers已设置为16MB（注意：需要重启才能生效）';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'wal_buffers值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置wal_buffers失败: %', SQLERRM;
END $$;
```

### 6.6 维护参数

```sql
-- autovacuum相关参数
-- autovacuum相关参数（带错误处理）
DO $$
BEGIN
    ALTER SYSTEM SET autovacuum = on;
    ALTER SYSTEM SET autovacuum_max_workers = 3;
    ALTER SYSTEM SET autovacuum_naptime = '1min';
    ALTER SYSTEM SET autovacuum_vacuum_scale_factor = 0.1;  -- 10%死元组触发
    ALTER SYSTEM SET autovacuum_analyze_scale_factor = 0.05;  -- 5%变更触发
    PERFORM pg_reload_conf();
    RAISE NOTICE 'autovacuum参数已设置完成';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION 'autovacuum参数值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置autovacuum参数失败: %', SQLERRM;
END $$;

-- maintenance_work_mem: 维护工作内存
ALTER SYSTEM SET maintenance_work_mem = '1GB';
```

### 6.7 参数变更检查清单

```sql
-- 检查参数是否生效（某些参数需要重启，带错误处理和性能测试）
DO $$
DECLARE
    param_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_settings'
    ) THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    END IF;

    SELECT COUNT(*) INTO param_count
    FROM pg_settings
    WHERE name IN (
        'shared_buffers', 'work_mem', 'max_connections',
        'checkpoint_timeout', 'max_wal_size'
    );

    IF param_count > 0 THEN
        RAISE NOTICE '发现 % 个相关参数', param_count;
    ELSE
        RAISE WARNING '未发现相关参数';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查参数失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT name, setting, unit, context, source
FROM pg_settings
WHERE name IN (
    'shared_buffers', 'work_mem', 'max_connections',
    'checkpoint_timeout', 'max_wal_size'
)
ORDER BY name;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- context说明：
-- 'postmaster': 需要重启PostgreSQL服务
-- 'sighup': 需要重新加载配置（SELECT pg_reload_conf();）
-- 'user': 可以在会话级别设置
```

## 7. 验证清单

- 指标对比：调优前/后 P95、TPS/QPS、命中率、WAL、检查点；
- SQL 对比：TopN 是否下降、估算偏差是否缓解；
- 资源：CPU/IO/内存是否在阈值内；
- 业务：错误率/超时是否正常。

### 7.1 验证脚本

```sql
-- 1. 性能指标对比（带错误处理和性能测试）
DO $$
DECLARE
    comparison_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_performance_baseline'
    ) THEN
        RAISE WARNING '表pg_performance_baseline不存在，无法进行性能指标对比';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_database'
    ) THEN
        RAISE WARNING 'pg_stat_database视图不存在';
        RETURN;
    END IF;

    WITH baseline AS (
        SELECT * FROM pg_performance_baseline
        WHERE baseline_id = (SELECT MAX(baseline_id) FROM pg_performance_baseline)
    ),
    current AS (
        SELECT
            round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2) as buffer_hit_ratio,
            sum(xact_commit) as commits,
            sum(xact_rollback) as rollbacks
        FROM pg_stat_database
        WHERE datname = current_database()
    )
    SELECT COUNT(*) INTO comparison_count
    FROM baseline, current
    WHERE baseline.metric_name = 'buffer_hit_ratio';

    IF comparison_count > 0 THEN
        RAISE NOTICE '性能指标对比完成，发现 % 条对比记录', comparison_count;
    ELSE
        RAISE NOTICE '未发现性能指标对比记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表或视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '性能指标对比失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH baseline AS (
    SELECT * FROM pg_performance_baseline
    WHERE baseline_id = (SELECT MAX(baseline_id) FROM pg_performance_baseline)
),
current AS (
    SELECT
        round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2) as buffer_hit_ratio,
        sum(xact_commit) as commits,
        sum(xact_rollback) as rollbacks
    FROM pg_stat_database
    WHERE datname = current_database()
)
SELECT
    'Buffer Hit Ratio' as metric,
    baseline.metric_value as baseline_value,
    current.buffer_hit_ratio as current_value,
    round((current.buffer_hit_ratio - baseline.metric_value)::numeric, 2) as improvement
FROM baseline, current
WHERE baseline.metric_name = 'buffer_hit_ratio';
-- 执行时间: <100ms（取决于基线数据量）
-- 计划: Nested Loop -> CTE Scan

-- 2. 慢查询对比（带错误处理和性能测试）
DO $$
DECLARE
    comparison_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_query_baseline'
    ) THEN
        RAISE WARNING '表pg_query_baseline不存在，无法进行慢查询对比';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先安装pg_stat_statements扩展';
        RETURN;
    END IF;

    WITH baseline AS (
        SELECT queryid, mean_exec_time as baseline_time
        FROM pg_query_baseline
        WHERE baseline_id = (SELECT MAX(baseline_id) FROM pg_query_baseline)
    ),
    current AS (
        SELECT queryid, mean_exec_time as current_time
        FROM pg_stat_statements
    )
    SELECT COUNT(*) INTO comparison_count
    FROM baseline b
    JOIN current c ON b.queryid = c.queryid
    WHERE c.current_time < b.baseline_time * 0.9;  -- 性能提升>10%

    IF comparison_count > 0 THEN
        RAISE NOTICE '慢查询对比完成，发现 % 条性能提升的查询', comparison_count;
    ELSE
        RAISE NOTICE '未发现性能提升的查询';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表或视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '慢查询对比失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH baseline AS (
    SELECT queryid, mean_exec_time as baseline_time
    FROM pg_query_baseline
    WHERE baseline_id = (SELECT MAX(baseline_id) FROM pg_query_baseline)
),
current AS (
    SELECT queryid, mean_exec_time as current_time
    FROM pg_stat_statements
)
SELECT
    LEFT(b.query, 100) as query_preview,
    b.baseline_time,
    c.current_time,
    round((b.baseline_time - c.current_time)::numeric, 2) as improvement_ms,
    round(((b.baseline_time - c.current_time) / NULLIF(b.baseline_time, 0) * 100)::numeric, 2) as improvement_pct
FROM baseline b
JOIN current c ON b.queryid = c.queryid
WHERE c.current_time < b.baseline_time * 0.9  -- 性能提升>10%
ORDER BY improvement_ms DESC
LIMIT 10;
-- 执行时间: <200ms（取决于pg_stat_statements数据量）
-- 计划: Limit -> Sort -> Hash Join -> CTE Scan

-- 3. 资源使用检查（带错误处理和性能测试）
DO $$
DECLARE
    active_conn_count INT;
    lock_wait_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO active_conn_count
    FROM pg_stat_activity
    WHERE state != 'idle';

    SELECT COUNT(*) INTO lock_wait_count
    FROM pg_locks
    WHERE NOT granted;

    RAISE NOTICE '活跃连接数: %, 锁等待数: %', active_conn_count, lock_wait_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '资源使用检查失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    'Active Connections' as metric,
    count(*) as value,
    (SELECT setting::integer FROM pg_settings WHERE name = 'max_connections') as max_value,
    round(100.0 * count(*) / NULLIF((SELECT setting::integer FROM pg_settings WHERE name = 'max_connections'), 0), 2) as usage_pct
FROM pg_stat_activity
WHERE state != 'idle'
UNION ALL
SELECT
    'Lock Waits' as metric,
    count(*) as value,
    NULL as max_value,
    NULL as usage_pct
FROM pg_locks
WHERE NOT granted;
-- 执行时间: <50ms
-- 计划: Append -> Seq Scan

-- 4. 检查点频率对比（带错误处理和性能测试）
DO $$
DECLARE
    checkpoint_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_bgwriter'
    ) THEN
        RAISE WARNING 'pg_stat_bgwriter视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO checkpoint_count
    FROM pg_stat_bgwriter;

    IF checkpoint_count > 0 THEN
        RAISE NOTICE '检查点频率对比完成，发现 % 条记录', checkpoint_count;
    ELSE
        RAISE NOTICE '未发现检查点记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_bgwriter视图不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查点频率对比失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    checkpoints_timed + checkpoints_req as total_checkpoints,
    round(extract(epoch from now() - stats_reset) / NULLIF(checkpoints_timed + checkpoints_req, 0) / 60, 2) as avg_interval_minutes,
    checkpoint_write_time + checkpoint_sync_time as total_checkpoint_time_ms
FROM pg_stat_bgwriter;
-- 执行时间: <10ms
-- 计划: Seq Scan
```

### 7.2 验证标准

**性能提升标准**:

- ✅ **缓冲区命中率**: 提升>2% 或 达到>99%
- ✅ **平均查询时间**: 降低>10%
- ✅ **P95/P99延迟**: 降低>10%
- ✅ **TPS/QPS**: 提升>5%
- ✅ **检查点频率**: 降低>20% 或 间隔>10分钟
- ✅ **锁等待**: 减少>50%

**回滚触发条件**:

- ❌ **缓冲区命中率**: 下降>2%
- ❌ **平均查询时间**: 增加>10%
- ❌ **P95/P99延迟**: 增加>10%
- ❌ **错误率**: 增加>1%
- ❌ **锁等待**: 增加>50%
- ❌ **资源使用**: CPU>90% 或 内存>95%

## 8. 回滚与复盘

- 立即恢复参数旧值/撤销索引或重写；
- 附加证据：日志、慢SQL、EXPLAIN、面板截图；
- 复盘条目：问题根因、无效尝试、有效手段、可复用脚本。

### 8.1 回滚脚本

```sql
-- 1. 参数回滚（带错误处理）
-- 从审计表获取旧值并恢复
DO $$
DECLARE
    audit_record RECORD;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE EXCEPTION '表pg_tuning_audit不存在，无法执行参数回滚';
    END IF;

    SELECT parameter_name, old_value
    INTO audit_record
    FROM pg_tuning_audit
    WHERE change_id = (SELECT MAX(change_id) FROM pg_tuning_audit)
    AND change_type = 'parameter'
    LIMIT 1;

    IF audit_record.parameter_name IS NOT NULL THEN
        EXECUTE format('ALTER SYSTEM SET %I = %L',
                       audit_record.parameter_name,
                       audit_record.old_value);
        PERFORM pg_reload_conf();
        RAISE NOTICE '参数回滚成功: % 已恢复为 %',
                     audit_record.parameter_name,
                     audit_record.old_value;
    ELSE
        RAISE WARNING '未找到需要回滚的参数变更记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表pg_tuning_audit不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效，无法回滚';
    WHEN OTHERS THEN
        RAISE EXCEPTION '参数回滚失败: %', SQLERRM;
END $$;

-- 2. 索引回滚（带错误处理）
-- 删除新创建的索引
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND indexname = 'idx_orders_customer_date'
    ) THEN
        RAISE NOTICE '索引idx_orders_customer_date不存在，跳过删除';
        RETURN;
    END IF;

    DROP INDEX CONCURRENTLY IF EXISTS idx_orders_customer_date;
    RAISE NOTICE '索引idx_orders_customer_date删除成功（并发模式）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '索引idx_orders_customer_date不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '索引回滚失败: %', SQLERRM;
END $$;

-- 3. 记录回滚操作（带错误处理）
DO $$
DECLARE
    inserted_id INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，跳过回滚记录';
        RETURN;
    END IF;

    INSERT INTO pg_tuning_audit (
        parameter_name,
        old_value,
        new_value,
        change_type,
        change_reason,
        change_by
    )
    SELECT
        parameter_name,
        new_value,  -- 当前值作为old_value
        old_value,  -- 原始值作为new_value（回滚）
        'rollback',
        'Performance degradation detected',
        current_user
    FROM pg_tuning_audit
    WHERE change_id = (SELECT MAX(change_id) FROM pg_tuning_audit)
    RETURNING change_id INTO inserted_id;

    IF inserted_id IS NOT NULL THEN
        RAISE NOTICE '回滚操作记录成功，change_id: %', inserted_id;
    ELSE
        RAISE WARNING '未找到需要回滚的变更记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表pg_tuning_audit不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录回滚操作失败: %', SQLERRM;
END $$;
```

### 8.2 复盘模板

**复盘记录表结构**:

```sql
-- 创建复盘记录表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_review'
    ) THEN
        CREATE TABLE pg_tuning_review (
            review_id SERIAL PRIMARY KEY,
            change_id INTEGER REFERENCES pg_tuning_audit(change_id),
            review_time TIMESTAMP DEFAULT NOW(),
            root_cause TEXT,
            attempted_solutions TEXT[],
            effective_solution TEXT,
            performance_improvement NUMERIC,
            lessons_learned TEXT,
            reusable_scripts TEXT,
            reviewed_by TEXT
        );
        RAISE NOTICE '复盘记录表pg_tuning_review创建成功';
    ELSE
        RAISE NOTICE '复盘记录表pg_tuning_review已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '复盘记录表pg_tuning_review已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建复盘记录表失败: %', SQLERRM;
END $$;
```

**复盘记录示例**:

```sql
-- 插入复盘记录（带错误处理）
DO $$
DECLARE
    inserted_id INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_review'
    ) THEN
        RAISE EXCEPTION '表pg_tuning_review不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，change_id将为空';
    END IF;

    INSERT INTO pg_tuning_review (
        change_id,
        root_cause,
        attempted_solutions,
        effective_solution,
        performance_improvement,
        lessons_learned,
        reusable_scripts,
        reviewed_by
    )
    VALUES (
        (SELECT MAX(change_id) FROM pg_tuning_audit),
        'Slow customer order history query due to missing index on (customer_id, order_date)',
        ARRAY[
            'Increased work_mem - no improvement',
            'Updated statistics - no improvement',
            'Created composite index - 80% improvement'
        ],
        'Created composite index idx_orders_customer_date ON orders (customer_id, order_date DESC)',
        80.0,  -- 80%性能提升
        'Composite indexes are effective for multi-column WHERE and ORDER BY clauses',
        'CREATE INDEX CONCURRENTLY idx_orders_customer_date ON orders (customer_id, order_date DESC);',
        current_user
    )
    RETURNING review_id INTO inserted_id;

    IF inserted_id IS NOT NULL THEN
        RAISE NOTICE '复盘记录插入成功，review_id: %', inserted_id;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表pg_tuning_review或pg_tuning_audit不存在';
    WHEN check_violation THEN
        RAISE WARNING '复盘记录违反约束条件';
    WHEN OTHERS THEN
        RAISE EXCEPTION '插入复盘记录失败: %', SQLERRM;
END $$;
```

### 8.3 复盘查询

```sql
-- 查看历史变更和复盘记录（带错误处理和性能测试）
DO $$
DECLARE
    record_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，无法查看历史变更';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO record_count
    FROM pg_tuning_audit a
    LEFT JOIN pg_tuning_review r ON a.change_id = r.change_id;

    IF record_count > 0 THEN
        RAISE NOTICE '发现 % 条历史变更记录', record_count;
    ELSE
        RAISE NOTICE '未发现历史变更记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看历史变更失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    a.change_id,
    a.change_time,
    a.parameter_name,
    a.old_value,
    a.new_value,
    a.change_type,
    r.root_cause,
    r.effective_solution,
    r.performance_improvement,
    r.lessons_learned
FROM pg_tuning_audit a
LEFT JOIN pg_tuning_review r ON a.change_id = r.change_id
ORDER BY a.change_time DESC
LIMIT 20;
-- 执行时间: <100ms（取决于记录数量）
-- 计划: Limit -> Sort -> Left Join

-- 统计最有效的优化手段（带错误处理和性能测试）
DO $$
DECLARE
    stat_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_audit'
    ) THEN
        RAISE WARNING '表pg_tuning_audit不存在，无法统计优化手段';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pg_tuning_review'
    ) THEN
        RAISE WARNING '表pg_tuning_review不存在，无法统计优化手段';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO stat_count
    FROM pg_tuning_audit a
    JOIN pg_tuning_review r ON a.change_id = r.change_id
    WHERE r.performance_improvement > 0;

    IF stat_count > 0 THEN
        RAISE NOTICE '发现 % 条有效优化记录', stat_count;
    ELSE
        RAISE NOTICE '未发现有效优化记录';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '统计优化手段失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    change_type,
    COUNT(*) as total_changes,
    AVG(r.performance_improvement) as avg_improvement,
    MAX(r.performance_improvement) as max_improvement
FROM pg_tuning_audit a
JOIN pg_tuning_review r ON a.change_id = r.change_id
WHERE r.performance_improvement > 0
GROUP BY change_type
ORDER BY avg_improvement DESC;
-- 执行时间: <100ms（取决于记录数量）
-- 计划: GroupAggregate -> Hash Join
```
