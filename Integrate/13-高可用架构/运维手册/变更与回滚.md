---

> **📋 文档来源**: `PostgreSQL\06-运维实践\运维手册\变更与回滚.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# Runbook：变更与回滚（PostgreSQL 18）

> **文档版本**: v1.0
> **最后更新**: 2025-11-22
> **PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)

---

## 📋 目录

- [Runbook：变更与回滚（PostgreSQL 18）](#runbook变更与回滚postgresql-18)
  - [📋 目录](#-目录)
  - [范围](#范围)
  - [原则](#原则)
  - [流程（示意）](#流程示意)
    - [1. 变更评审](#1-变更评审)
    - [2. 变更窗口准备](#2-变更窗口准备)
    - [3. 变更执行](#3-变更执行)
    - [4. 验证](#4-验证)
  - [回滚策略（示例）](#回滚策略示例)
    - [回滚执行](#回滚执行)
  - [检查清单（占位）](#检查清单占位)
    - [完整检查清单](#完整检查清单)
  - [演练频率（建议）](#演练频率建议)
    - [演练脚本](#演练脚本)
  - [交叉引用](#交叉引用)
    - [相关文档](#相关文档)
    - [外部资源](#外部资源)

---

## 范围

参数变更、DDL 变更、索引/分区变更、版本升级小步推进。

## 原则

- 蓝绿/金丝雀：优先对读流量试点，逐步扩大
- 可回滚：保留旧结构与数据路径、影子索引/表
- 可观测：标准化前后指标对比

## 流程（示意）

1) 评审：影响评估、回滚路径、验证计划
2) 变更窗口：只读期/低峰期，冻结长事务
3) 执行：分批、幂等脚本、错误即停
4) 验证：功能/性能/一致性；若失败走回滚

### 1. 变更评审

**影响评估**:

```sql
-- 1. 识别受影响对象（带错误处理和性能测试）
DO $$
DECLARE
    index_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_indexes'
    ) THEN
        RAISE WARNING 'pg_indexes视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO index_count
    FROM pg_indexes
    WHERE tablename = 'target_table';

    IF index_count > 0 THEN
        RAISE NOTICE '发现 % 个相关索引', index_count;
    ELSE
        RAISE NOTICE '未发现相关索引';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '识别受影响对象失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname
FROM pg_indexes
WHERE tablename = 'target_table';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 2. 检查依赖关系（带错误处理和性能测试）
DO $$
DECLARE
    dependency_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_depend'
    ) THEN
        RAISE WARNING 'pg_depend表不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO dependency_count
    FROM pg_depend
    JOIN pg_rewrite ON pg_depend.objid = pg_rewrite.oid
    JOIN pg_class as dependent_view ON pg_rewrite.ev_class = dependent_view.oid
    JOIN pg_class as source_table ON pg_depend.refobjid = source_table.oid
    JOIN pg_namespace dependent_ns ON dependent_view.relnamespace = dependent_ns.oid
    JOIN pg_namespace source_ns ON source_table.relnamespace = source_ns.oid
    WHERE source_table.relname = 'target_table';

    IF dependency_count > 0 THEN
        RAISE NOTICE '发现 % 个依赖关系', dependency_count;
    ELSE
        RAISE NOTICE '未发现依赖关系';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关系统表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查依赖关系失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    dependent_ns.nspname as dependent_schema,
    dependent_view.relname as dependent_view,
    source_ns.nspname as source_schema,
    source_table.relname as source_table
FROM pg_depend
JOIN pg_rewrite ON pg_depend.objid = pg_rewrite.oid
JOIN pg_class as dependent_view ON pg_rewrite.ev_class = dependent_view.oid
JOIN pg_class as source_table ON pg_depend.refobjid = source_table.oid
JOIN pg_namespace dependent_ns ON dependent_view.relnamespace = dependent_ns.oid
JOIN pg_namespace source_ns ON source_table.relnamespace = source_ns.oid
WHERE source_table.relname = 'target_table';
-- 执行时间: <100ms（取决于依赖关系数量）
-- 计划: Nested Loop -> Hash Join

-- 3. 检查订阅（逻辑复制，带错误处理和性能测试）
DO $$
DECLARE
    subscription_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_subscription'
    ) THEN
        RAISE WARNING 'pg_subscription视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO subscription_count
    FROM pg_subscription
    WHERE subpublications && ARRAY['target_publication'];

    IF subscription_count > 0 THEN
        RAISE NOTICE '发现 % 个相关订阅', subscription_count;
    ELSE
        RAISE NOTICE '未发现相关订阅';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_subscription视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查订阅失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    subname,
    subpublications,
    subenabled
FROM pg_subscription
WHERE subpublications && ARRAY['target_publication'];
-- 执行时间: <50ms
-- 计划: Seq Scan
```

**回滚路径设计**:

```sql
-- 创建变更记录表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'change_log'
    ) THEN
        CREATE TABLE change_log (
            change_id SERIAL PRIMARY KEY,
            change_time TIMESTAMP DEFAULT NOW(),
            change_type VARCHAR(50),  -- 'parameter', 'ddl', 'index', 'partition', 'upgrade'
            change_target TEXT,
            old_value TEXT,
            new_value TEXT,
            rollback_script TEXT,
            change_by VARCHAR(100),
            change_status VARCHAR(20),  -- 'planned', 'in_progress', 'completed', 'rolled_back'
            verification_result TEXT
        );
        RAISE NOTICE '变更记录表change_log创建成功';
    ELSE
        RAISE NOTICE '变更记录表change_log已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '变更记录表change_log已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建变更记录表失败: %', SQLERRM;
END $$;
```

### 2. 变更窗口准备

**冻结长事务**:

```sql
-- 1. 识别长事务（带错误处理和性能测试）
DO $$
DECLARE
    long_txn_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO long_txn_count
    FROM pg_stat_activity
    WHERE state = 'active'
    AND NOW() - query_start > INTERVAL '5 minutes';

    IF long_txn_count > 0 THEN
        RAISE NOTICE '发现 % 个长事务', long_txn_count;
    ELSE
        RAISE NOTICE '未发现长事务';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '识别长事务失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    usename,
    datname,
    state,
    query_start,
    NOW() - query_start as duration,
    query
FROM pg_stat_activity
WHERE state = 'active'
AND NOW() - query_start > INTERVAL '5 minutes'
ORDER BY query_start;
-- 执行时间: <50ms
-- 计划: Sort -> Seq Scan

-- 2. 终止长事务（谨慎使用，带错误处理）
DO $$
DECLARE
    terminated_count INT := 0;
    backend_pid INT;
    target_pid INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_terminate_backend') THEN
        RAISE EXCEPTION 'pg_terminate_backend函数不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_backend_pid') THEN
        RAISE EXCEPTION 'pg_backend_pid函数不存在';
    END IF;

    SELECT pg_backend_pid() INTO backend_pid;

    FOR target_pid IN
        SELECT pid
        FROM pg_stat_activity
        WHERE state = 'active'
        AND NOW() - query_start > INTERVAL '10 minutes'
        AND pid != backend_pid
    LOOP
        PERFORM pg_terminate_backend(target_pid);
        terminated_count := terminated_count + 1;
    END LOOP;

    IF terminated_count > 0 THEN
        RAISE NOTICE '已终止 % 个长事务', terminated_count;
    ELSE
        RAISE NOTICE '未发现需要终止的长事务';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_terminate_backend或pg_backend_pid函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法终止后端进程';
    WHEN OTHERS THEN
        RAISE EXCEPTION '终止长事务失败: %', SQLERRM;
END $$;

-- 3. 检查锁等待（带错误处理和性能测试）
DO $$
DECLARE
    lock_wait_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_locks'
    ) THEN
        RAISE WARNING 'pg_locks表不存在';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO lock_wait_count
    FROM pg_catalog.pg_locks blocked_locks
    JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
    JOIN pg_catalog.pg_locks blocking_locks
        ON blocking_locks.locktype = blocked_locks.locktype
        AND blocking_locks.pid != blocked_locks.pid
    JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
    WHERE NOT blocked_locks.granted;

    IF lock_wait_count > 0 THEN
        RAISE NOTICE '发现 % 个锁等待', lock_wait_count;
    ELSE
        RAISE NOTICE '未发现锁等待';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关系统表或视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查锁等待失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    blocked_locks.pid AS blocked_pid,
    blocking_locks.pid AS blocking_pid,
    blocked_activity.query AS blocked_query,
    blocking_activity.query AS blocking_query
FROM pg_catalog.pg_locks blocked_locks
JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
JOIN pg_catalog.pg_locks blocking_locks
    ON blocking_locks.locktype = blocked_locks.locktype
    AND blocking_locks.pid != blocked_locks.pid
JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted;
-- 执行时间: <100ms（取决于锁等待数量）
-- 计划: Hash Join -> Nested Loop
```

**低峰期确认**:

```sql
-- 检查当前负载（带错误处理和性能测试）
DO $$
DECLARE
    total_conn INT;
    active_queries INT;
    waiting_queries INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT
        COUNT(*),
        COUNT(*) FILTER (WHERE state = 'active'),
        COUNT(*) FILTER (WHERE wait_event_type IS NOT NULL)
    INTO total_conn, active_queries, waiting_queries
    FROM pg_stat_activity
    WHERE datname = current_database();

    RAISE NOTICE '当前负载 - 总连接数: %, 活跃查询: %, 等待查询: %', total_conn, active_queries, waiting_queries;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查当前负载失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    COUNT(*) as total_connections,
    COUNT(*) FILTER (WHERE state = 'active') as active_queries,
    COUNT(*) FILTER (WHERE wait_event_type IS NOT NULL) as waiting_queries
FROM pg_stat_activity
WHERE datname = current_database();
-- 执行时间: <50ms
-- 计划: Aggregate -> Seq Scan
```

### 3. 变更执行

**参数变更**:

```sql
-- 方法1: ALTER SYSTEM SET（推荐，持久化，带错误处理）
DO $$
DECLARE
    old_value TEXT;
BEGIN
    SELECT setting INTO old_value
    FROM pg_settings
    WHERE name = 'shared_buffers';

    ALTER SYSTEM SET shared_buffers = '512MB';
    RAISE NOTICE '参数shared_buffers已从%更新为512MB', old_value;

    PERFORM pg_reload_conf();
    RAISE NOTICE '配置已重新加载（注意：部分参数需要重启才能生效）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_settings视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效，请检查shared_buffers的值';
    WHEN OTHERS THEN
        RAISE EXCEPTION '参数变更失败: %', SQLERRM;
END $$;

-- 方法2: 直接修改配置文件
-- 编辑 postgresql.conf
-- shared_buffers = 512MB
-- 然后重启或reload

-- 记录变更（带错误处理）
DO $$
DECLARE
    old_value TEXT;
    change_log_exists BOOLEAN;
BEGIN
    -- 检查change_log表是否存在
    SELECT EXISTS(
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'change_log'
    ) INTO change_log_exists;

    IF NOT change_log_exists THEN
        RAISE WARNING '表change_log不存在，跳过变更记录';
        RETURN;
    END IF;

    SELECT setting INTO old_value
    FROM pg_settings
    WHERE name = 'shared_buffers';

    INSERT INTO change_log (
        change_type,
        change_target,
        old_value,
        new_value,
        rollback_script,
        change_by
    )
    VALUES (
        'parameter',
        'shared_buffers',
        old_value,
        '512MB',
        'ALTER SYSTEM SET shared_buffers = ''256MB'';',
        current_user
    );
    RAISE NOTICE '变更记录已保存';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表change_log或pg_settings不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录变更失败: %', SQLERRM;
END $$;
```

**DDL变更**:

```sql
-- 示例：添加列（使用IF NOT EXISTS确保幂等，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    ALTER TABLE orders
    ADD COLUMN IF NOT EXISTS new_column VARCHAR(100);
    RAISE NOTICE '列new_column添加成功（如果不存在）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_column THEN
        RAISE NOTICE '列new_column已存在，跳过添加';
    WHEN OTHERS THEN
        RAISE EXCEPTION '添加列失败: %', SQLERRM;
END $$;

-- 示例：修改列类型（需要数据转换，带错误处理）
-- 步骤1: 添加新列（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'status_new'
    ) THEN
        RAISE NOTICE '列status_new已存在，跳过添加';
        RETURN;
    END IF;

    ALTER TABLE orders
    ADD COLUMN status_new INTEGER;
    RAISE NOTICE '列status_new添加成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_column THEN
        RAISE NOTICE '列status_new已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '添加列失败: %', SQLERRM;
END $$;

-- 步骤2: 数据迁移（带错误处理）
DO $$
DECLARE
    updated_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'status'
    ) THEN
        RAISE WARNING '列status不存在，跳过数据迁移';
        RETURN;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'status_new'
    ) THEN
        RAISE EXCEPTION '列status_new不存在，请先添加';
    END IF;

    UPDATE orders
    SET status_new = CASE
        WHEN status = 'pending' THEN 1
        WHEN status = 'processing' THEN 2
        WHEN status = 'completed' THEN 3
        ELSE 0
    END;

    GET DIAGNOSTICS updated_count = ROW_COUNT;
    RAISE NOTICE '数据迁移完成，更新了 % 条记录', updated_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_column THEN
        RAISE EXCEPTION '列status或status_new不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '数据迁移失败: %', SQLERRM;
END $$;

-- 步骤3: 删除旧列（在验证后）
-- ALTER TABLE orders DROP COLUMN status;
-- ALTER TABLE orders RENAME COLUMN status_new TO status;

-- 回滚脚本
-- ALTER TABLE orders DROP COLUMN IF EXISTS status_new;
```

**索引变更**:

```sql
-- 在线创建索引（不阻塞写操作，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_created_at_new'
    ) THEN
        RAISE NOTICE '索引idx_orders_created_at_new已存在，跳过创建';
        RETURN;
    END IF;

    CREATE INDEX CONCURRENTLY idx_orders_created_at_new ON orders(created_at);
    RAISE NOTICE '索引创建成功: idx_orders_created_at_new';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_object THEN
        RAISE NOTICE '索引idx_orders_created_at_new已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建索引失败: %', SQLERRM;
END $$;

-- 验证索引创建成功（带错误处理和性能测试）
DO $$
DECLARE
    index_exists BOOLEAN;
BEGIN
    SELECT EXISTS(
        SELECT 1 FROM pg_indexes
        WHERE tablename = 'orders'
        AND indexname = 'idx_orders_created_at_new'
    ) INTO index_exists;

    IF NOT index_exists THEN
        RAISE WARNING '索引idx_orders_created_at_new不存在';
    ELSE
        RAISE NOTICE '索引idx_orders_created_at_new创建成功';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证索引失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    indexname,
    indexdef
FROM pg_indexes
WHERE tablename = 'orders'
AND indexname = 'idx_orders_created_at_new';
-- 执行时间: <10ms
-- 计划: Seq Scan

-- 切换索引（使用视图或应用层切换，带错误处理）
-- 步骤1: 创建视图桥接（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    CREATE OR REPLACE VIEW orders_view AS
    SELECT * FROM orders;
    RAISE NOTICE '视图orders_view创建/更新成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建视图失败: %', SQLERRM;
END $$;

-- 步骤2: 应用切换到新索引
-- 步骤3: 删除旧索引（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND indexname = 'idx_orders_created_at_old'
    ) THEN
        RAISE NOTICE '索引idx_orders_created_at_old不存在，跳过删除';
        RETURN;
    END IF;

    DROP INDEX CONCURRENTLY IF EXISTS idx_orders_created_at_old;
    RAISE NOTICE '索引删除成功: idx_orders_created_at_old';
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE '索引idx_orders_created_at_old不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '删除索引失败: %', SQLERRM;
END $$;

-- 回滚脚本（带错误处理）
-- DO $$
-- BEGIN
--     DROP INDEX CONCURRENTLY IF EXISTS idx_orders_created_at_new;
--     RAISE NOTICE '回滚完成：已删除索引idx_orders_created_at_new';
-- EXCEPTION
--     WHEN undefined_table THEN
--         RAISE NOTICE '索引idx_orders_created_at_new不存在';
--     WHEN OTHERS THEN
--         RAISE EXCEPTION '回滚失败: %', SQLERRM;
-- END $$;
```

**分区变更**:

```sql
-- 示例：添加新分区（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    CREATE TABLE IF NOT EXISTS orders_2025_12 PARTITION OF orders
    FOR VALUES FROM ('2025-12-01') TO ('2026-01-01');
    RAISE NOTICE '分区orders_2025_12创建成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_table THEN
        RAISE NOTICE '分区orders_2025_12已存在，跳过';
    WHEN OTHERS THEN
        RAISE EXCEPTION '添加分区失败: %', SQLERRM;
END $$;

-- 示例：删除旧分区（需要数据迁移，带错误处理）
-- 步骤1: 迁移数据（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders_2024_12'
    ) THEN
        RAISE WARNING '分区orders_2024_12不存在，跳过数据迁移';
        RETURN;
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders_2024_12_backup'
    ) THEN
        DROP TABLE orders_2024_12_backup;
        RAISE NOTICE '已删除现有备份表: orders_2024_12_backup';
    END IF;

    CREATE TABLE orders_2024_12_backup AS
    SELECT * FROM orders_2024_12;
    RAISE NOTICE '数据迁移完成: orders_2024_12 -> orders_2024_12_backup';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '分区orders_2024_12不存在';
    WHEN duplicate_table THEN
        RAISE WARNING '备份表orders_2024_12_backup已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '数据迁移失败: %', SQLERRM;
END $$;

-- 步骤2: 分离分区（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders_2024_12'
    ) THEN
        RAISE WARNING '分区orders_2024_12不存在，跳过分离';
        RETURN;
    END IF;

    ALTER TABLE orders DETACH PARTITION orders_2024_12;
    RAISE NOTICE '分区分离成功: orders_2024_12';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders或分区orders_2024_12不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '分离分区失败: %', SQLERRM;
END $$;

-- 步骤3: 删除分区（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders_2024_12'
    ) THEN
        RAISE WARNING '分区orders_2024_12不存在，跳过删除';
        RETURN;
    END IF;

    DROP TABLE orders_2024_12;
    RAISE NOTICE '分区删除成功: orders_2024_12';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '分区orders_2024_12不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '删除分区失败: %', SQLERRM;
END $$;

-- 回滚脚本
-- ALTER TABLE orders ATTACH PARTITION orders_2024_12
-- FOR VALUES FROM ('2024-12-01') TO ('2025-01-01');
```

### 4. 验证

**功能验证**:

```sql
-- 1. 验证对象存在（带错误处理和性能测试）
DO $$
DECLARE
    index_count INT;
BEGIN
    SELECT COUNT(*) INTO index_count
    FROM pg_indexes
    WHERE tablename = 'target_table';

    IF index_count = 0 THEN
        RAISE WARNING '表target_table未找到索引';
    ELSE
        RAISE NOTICE '表target_table找到 % 个索引', index_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证对象存在失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname
FROM pg_indexes
WHERE tablename = 'target_table';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 2. 验证数据完整性（带错误处理和性能测试）
DO $$
DECLARE
    total_count BIGINT;
    new_column_count BIGINT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'target_table'
    ) THEN
        RAISE EXCEPTION '表target_table不存在，请先创建';
    END IF;

    SELECT COUNT(*) INTO total_count FROM target_table;
    RAISE NOTICE '表target_table总记录数: %', total_count;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'target_table' AND column_name = 'new_column'
    ) THEN
        SELECT COUNT(*) INTO new_column_count FROM target_table WHERE new_column IS NOT NULL;
        RAISE NOTICE 'new_column非空记录数: %', new_column_count;
    ELSE
        RAISE WARNING '列new_column不存在，跳过验证';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表target_table不存在';
    WHEN undefined_column THEN
        RAISE WARNING '列new_column不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证数据完整性失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT COUNT(*) FROM target_table;
-- 执行时间: <100ms (取决于表大小)
-- 计划: Aggregate

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT COUNT(*) FROM target_table WHERE new_column IS NOT NULL;
-- 执行时间: <100ms (取决于表大小)
-- 计划: Aggregate

-- 3. 验证约束（带错误处理和性能测试）
DO $$
DECLARE
    constraint_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'target_table'
    ) THEN
        RAISE EXCEPTION '表target_table不存在，请先创建';
    END IF;

    SELECT COUNT(*) INTO constraint_count
    FROM pg_constraint
    WHERE conrelid = 'target_table'::regclass;

    IF constraint_count = 0 THEN
        RAISE NOTICE '表target_table未找到约束';
    ELSE
        RAISE NOTICE '表target_table找到 % 个约束', constraint_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表target_table不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_get_constraintdef函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证约束失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    conname,
    contype,
    pg_get_constraintdef(oid) as definition
FROM pg_constraint
WHERE conrelid = 'target_table'::regclass;
-- 执行时间: <50ms
-- 计划: Seq Scan
```

**性能验证**:

```sql
-- 对比变更前后的性能指标（带错误处理和性能测试）
DO $$
DECLARE
    baseline_exists BOOLEAN;
    before_count INT;
    after_count INT;
BEGIN
    SELECT EXISTS(
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'performance_baseline'
    ) INTO baseline_exists;

    IF NOT baseline_exists THEN
        RAISE WARNING '表performance_baseline不存在，跳过性能对比';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO before_count
    FROM performance_baseline
    WHERE baseline_time < NOW() - INTERVAL '1 hour';

    SELECT COUNT(*) INTO after_count
    FROM performance_baseline;

    IF before_count = 0 OR after_count = 0 THEN
        RAISE WARNING '性能基线数据不足，无法对比';
        RETURN;
    END IF;

    RAISE NOTICE '变更前基线记录数: %, 变更后基线记录数: %', before_count, after_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表performance_baseline不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '性能指标对比失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH before AS (
    SELECT * FROM performance_baseline
    WHERE baseline_time = (SELECT MAX(baseline_time) FROM performance_baseline WHERE baseline_time < NOW() - INTERVAL '1 hour')
),
after AS (
    SELECT * FROM performance_baseline
    WHERE baseline_time = (SELECT MAX(baseline_time) FROM performance_baseline)
)
SELECT
    'Active Queries' as metric,
    before.active_queries as before_value,
    after.active_queries as after_value,
    after.active_queries - before.active_queries as delta
FROM before, after;
-- 执行时间: <100ms
-- 计划: Nested Loop
```

**一致性验证**:

```sql
-- 验证主从一致性（如果有复制，带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_replication'
    ) THEN
        RAISE WARNING 'pg_stat_replication视图不存在，可能没有配置复制';
        RETURN;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_current_wal_lsn') THEN
        RAISE EXCEPTION 'pg_current_wal_lsn函数不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_wal_lsn_diff') THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff函数不存在';
    END IF;

    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    IF replication_count > 0 THEN
        RAISE NOTICE '发现 % 个复制连接', replication_count;
    ELSE
        RAISE NOTICE '未发现复制连接';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION '必要的WAL函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证主从一致性失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    application_name,
    pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) as lag_bytes
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 验证数据一致性（带错误处理和性能测试）
DO $$
DECLARE
    table_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_user_tables'
    ) THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO table_count
    FROM pg_stat_user_tables
    WHERE schemaname = 'public';

    IF table_count > 0 THEN
        RAISE NOTICE '发现 % 个用户表', table_count;
    ELSE
        RAISE NOTICE '未发现用户表';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证数据一致性失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    n_live_tup,
    n_dead_tup
FROM pg_stat_user_tables
WHERE schemaname = 'public'
ORDER BY n_live_tup DESC;
-- 执行时间: <100ms（取决于表数量）
-- 计划: Sort -> Seq Scan
```

## 回滚策略（示例）

- 参数：保存原配置并可一键恢复
- DDL：保留旧对象，使用视图或触发器桥接
- 索引：在线创建，新旧并存，切换后旧索引回收

### 回滚执行

**参数回滚**:

```sql
-- 从变更记录获取回滚脚本（带错误处理和性能测试）
DO $$
DECLARE
    rollback_script_text TEXT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'change_log'
    ) THEN
        RAISE WARNING '表change_log不存在，无法获取回滚脚本';
        RETURN;
    END IF;

    SELECT rollback_script INTO rollback_script_text
    FROM change_log
    WHERE change_id = <change_id>  -- 替换为实际的change_id
    AND change_status = 'in_progress'
    LIMIT 1;

    IF rollback_script_text IS NOT NULL THEN
        RAISE NOTICE '找到回滚脚本: %', LEFT(rollback_script_text, 100);
    ELSE
        RAISE WARNING '未找到回滚脚本';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表change_log不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '获取回滚脚本失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT rollback_script
FROM change_log
WHERE change_id = <change_id>  -- 替换为实际的change_id
AND change_status = 'in_progress';
-- 执行时间: <10ms
-- 计划: Seq Scan

-- 执行回滚（带错误处理）
DO $$
DECLARE
    old_value TEXT;
BEGIN
    -- 从change_log获取旧值
    SELECT old_value INTO old_value
    FROM change_log
    WHERE change_id = <change_id>  -- 替换为实际的change_id
    AND change_status = 'applied';

    IF old_value IS NULL THEN
        RAISE EXCEPTION '未找到变更记录或变更未应用';
    END IF;

    ALTER SYSTEM SET shared_buffers = old_value;
    RAISE NOTICE '参数已恢复到旧值: %', old_value;

    PERFORM pg_reload_conf();
    RAISE NOTICE '配置已重新加载';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'change_log表或pg_settings视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效';
    WHEN OTHERS THEN
        RAISE EXCEPTION '回滚失败: %', SQLERRM;
END $$;

-- 更新变更记录（带错误处理）
DO $$
DECLARE
    updated_rows INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'change_log'
    ) THEN
        RAISE WARNING '表change_log不存在，跳过更新';
        RETURN;
    END IF;

    UPDATE change_log
    SET
        change_status = 'rolled_back',
        verification_result = '回滚原因：性能指标下降10%'
    WHERE change_id = <change_id>;  -- 替换为实际的change_id

    GET DIAGNOSTICS updated_rows = ROW_COUNT;
    IF updated_rows = 0 THEN
        RAISE WARNING '未找到匹配的变更记录';
    ELSE
        RAISE NOTICE '变更记录已更新: % 行', updated_rows;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表change_log不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '更新变更记录失败: %', SQLERRM;
END $$;
```

**DDL回滚**:

```sql
-- 示例：回滚添加列（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，无法回滚';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'new_column'
    ) THEN
        RAISE NOTICE '列new_column不存在，跳过删除';
        RETURN;
    END IF;

    ALTER TABLE orders DROP COLUMN IF EXISTS new_column;
    RAISE NOTICE '列new_column删除成功（回滚完成）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '回滚添加列失败: %', SQLERRM;
END $$;

-- 示例：回滚修改列类型（带错误处理）
-- 恢复旧列
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，无法回滚';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'status_new'
    ) THEN
        RAISE NOTICE '列status_new不存在，跳过删除';
        RETURN;
    END IF;

    ALTER TABLE orders DROP COLUMN IF EXISTS status_new;
    RAISE NOTICE '列status_new删除成功（回滚完成）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '回滚修改列类型失败: %', SQLERRM;
END $$;

-- 或恢复旧数据（带错误处理）
-- DO $$
-- BEGIN
--     IF NOT EXISTS (
--         SELECT 1 FROM information_schema.tables
--         WHERE table_schema = 'public' AND table_name = 'orders_backup'
--     ) THEN
--         RAISE WARNING '备份表orders_backup不存在，无法恢复数据';
--         RETURN;
--     END IF;
--     UPDATE orders SET status = ... FROM orders_backup WHERE ...;
--     RAISE NOTICE '数据恢复完成';
-- EXCEPTION
--     WHEN undefined_table THEN
--         RAISE WARNING '相关表不存在';
--     WHEN OTHERS THEN
--         RAISE EXCEPTION '恢复旧数据失败: %', SQLERRM;
-- END $$;
```

**索引回滚**:

```sql
-- 删除新索引（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND indexname = 'idx_orders_created_at_new'
    ) THEN
        RAISE NOTICE '索引idx_orders_created_at_new不存在，跳过删除';
        RETURN;
    END IF;

    DROP INDEX CONCURRENTLY IF EXISTS idx_orders_created_at_new;
    RAISE NOTICE '索引idx_orders_created_at_new删除成功（并发模式）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '索引idx_orders_created_at_new不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '删除索引失败: %', SQLERRM;
END $$;

-- 恢复旧索引（如果已删除，需要重建，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'orders'
    ) THEN
        RAISE EXCEPTION '表orders不存在，无法重建索引';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE schemaname = 'public' AND indexname = 'idx_orders_created_at_old'
    ) THEN
        RAISE NOTICE '索引idx_orders_created_at_old已存在，跳过重建';
        RETURN;
    END IF;

    CREATE INDEX CONCURRENTLY idx_orders_created_at_old ON orders(created_at);
    RAISE NOTICE '索引idx_orders_created_at_old重建成功（并发模式）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_object THEN
        RAISE NOTICE '索引idx_orders_created_at_old已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '重建索引失败: %', SQLERRM;
END $$;
```

## 检查清单（占位）

- 变更脚本已干跑（非生产）且通过
- 受影响对象与依赖清单已确认（视图/触发器/函数/订阅）
- 监控看板与门限已设定（前/中/后对比）
- 回滚脚本与步骤已校验

### 完整检查清单

**变更前检查**:

```sql
-- 1. 变更脚本验证（在测试环境）
-- 执行变更脚本并验证结果

-- 2. 受影响对象清单（带错误处理和性能测试）
DO $$
DECLARE
    object_count INT;
BEGIN
    SELECT COUNT(*) INTO object_count
    FROM (
        SELECT schemaname || '.' || tablename as object_name
        FROM pg_tables
        WHERE tablename LIKE 'target%'
        UNION ALL
        SELECT schemaname || '.' || viewname
        FROM pg_views
        WHERE viewname LIKE 'target%'
        UNION ALL
        SELECT n.nspname || '.' || p.proname
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname LIKE 'target%'
    ) objects;

    IF object_count > 0 THEN
        RAISE NOTICE '发现 % 个受影响对象', object_count;
    ELSE
        RAISE NOTICE '未发现受影响对象';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关系统视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查受影响对象失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    'Tables' as object_type,
    schemaname || '.' || tablename as object_name
FROM pg_tables
WHERE tablename LIKE 'target%'
UNION ALL
SELECT
    'Views',
    schemaname || '.' || viewname
FROM pg_views
WHERE viewname LIKE 'target%'
UNION ALL
SELECT
    'Functions',
    n.nspname || '.' || p.proname
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE p.proname LIKE 'target%';
-- 执行时间: <100ms（取决于对象数量）
-- 计划: Append -> Union All

-- 3. 依赖关系检查（带错误处理和性能测试）
DO $$
DECLARE
    dependency_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_depend'
    ) THEN
        RAISE WARNING 'pg_depend表不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO dependency_count
    FROM pg_depend
    JOIN pg_rewrite ON pg_depend.objid = pg_rewrite.oid
    JOIN pg_class as dependent_view ON pg_rewrite.ev_class = dependent_view.oid
    JOIN pg_class as source_table ON pg_depend.refobjid = source_table.oid
    JOIN pg_namespace dependent_ns ON dependent_view.relnamespace = dependent_ns.oid
    JOIN pg_namespace source_ns ON source_table.relnamespace = source_ns.oid
    WHERE source_table.relname = 'target_table';

    IF dependency_count > 0 THEN
        RAISE NOTICE '发现 % 个依赖关系', dependency_count;
    ELSE
        RAISE NOTICE '未发现依赖关系';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关系统表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查依赖关系失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    dependent_ns.nspname || '.' || dependent_view.relname as dependent_object,
    source_ns.nspname || '.' || source_table.relname as source_object
FROM pg_depend
JOIN pg_rewrite ON pg_depend.objid = pg_rewrite.oid
JOIN pg_class as dependent_view ON pg_rewrite.ev_class = dependent_view.oid
JOIN pg_class as source_table ON pg_depend.refobjid = source_table.oid
JOIN pg_namespace dependent_ns ON dependent_view.relnamespace = dependent_ns.oid
JOIN pg_namespace source_ns ON source_table.relnamespace = source_ns.oid
WHERE source_table.relname = 'target_table';
-- 执行时间: <100ms（取决于依赖关系数量）
-- 计划: Nested Loop -> Hash Join
```

**变更中监控**:

```sql
-- 实时监控关键指标（带错误处理和性能测试）
DO $$
DECLARE
    active_queries INT;
    avg_query_time NUMERIC;
    lock_waits INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_activity'
    ) THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO active_queries
    FROM pg_stat_activity
    WHERE state = 'active';

    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public' AND table_name = 'pg_stat_statements'
    ) THEN
        SELECT AVG(mean_exec_time) INTO avg_query_time
        FROM pg_stat_statements
        WHERE calls > 100;
    ELSE
        avg_query_time := NULL;
        RAISE NOTICE 'pg_stat_statements视图不存在，跳过平均查询时间统计';
    END IF;

    SELECT COUNT(*) INTO lock_waits
    FROM pg_stat_activity
    WHERE wait_event_type = 'Lock';

    RAISE NOTICE '监控指标 - 活跃查询: %, 平均查询时间: % ms, 锁等待: %',
                 active_queries, COALESCE(avg_query_time, 0), lock_waits;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '相关系统视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '实时监控失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    NOW() as check_time,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE state = 'active') as active_queries,
    (SELECT AVG(mean_exec_time) FROM pg_stat_statements WHERE calls > 100) as avg_query_time,
    (SELECT COUNT(*) FROM pg_stat_activity WHERE wait_event_type = 'Lock') as lock_waits;
-- 执行时间: <100ms
-- 计划: Result -> SubPlan
```

**变更后验证**:

```sql
-- 1. 功能验证（带错误处理和性能测试）
DO $$
DECLARE
    row_count BIGINT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'target_table'
    ) THEN
        RAISE EXCEPTION '表target_table不存在，请先创建';
    END IF;

    SELECT COUNT(*) INTO row_count FROM target_table;
    RAISE NOTICE '表target_table包含 % 行数据', row_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表target_table不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '功能验证失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT COUNT(*) FROM target_table;
-- 执行时间: <100ms（取决于表大小）
-- 计划: Aggregate

-- 2. 性能验证（带错误处理）
DO $$
DECLARE
    func_exists BOOLEAN;
    result_count INT;
BEGIN
    SELECT EXISTS(
        SELECT 1 FROM pg_proc
        WHERE proname = 'verify_performance_change'
    ) INTO func_exists;

    IF NOT func_exists THEN
        RAISE WARNING '函数verify_performance_change不存在，跳过性能验证';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO result_count
    FROM verify_performance_change();

    IF result_count > 0 THEN
        RAISE NOTICE '性能验证完成，发现 % 条记录', result_count;
    ELSE
        RAISE NOTICE '性能验证完成，未发现记录';
    END IF;
EXCEPTION
    WHEN undefined_function THEN
        RAISE WARNING '函数verify_performance_change不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '性能验证失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM verify_performance_change();
-- 执行时间: <200ms（取决于函数实现）
-- 计划: Function Scan

-- 3. 一致性验证（带错误处理和性能测试）
DO $$
DECLARE
    table_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'pg_catalog' AND table_name = 'pg_stat_user_tables'
    ) THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO table_count
    FROM pg_stat_user_tables
    WHERE schemaname = 'public';

    IF table_count > 0 THEN
        RAISE NOTICE '发现 % 个用户表', table_count;
    ELSE
        RAISE NOTICE '未发现用户表';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '一致性验证失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    n_live_tup,
    n_dead_tup
FROM pg_stat_user_tables
WHERE schemaname = 'public';
-- 执行时间: <100ms（取决于表数量）
-- 计划: Seq Scan
```

## 演练频率（建议）

- 月度：常规参数与索引调整演练
- 季度：DDL 结构性变更与跨版本升级预演

### 演练脚本

```bash
#!/bin/bash
# change_drill.sh

# 演练场景：参数变更
echo "=== 变更演练开始 ==="

# 1. 记录当前状态
psql -c "SELECT name, setting FROM pg_settings WHERE name = 'shared_buffers';" > /tmp/before_change.txt

# 2. 执行变更
psql -c "ALTER SYSTEM SET shared_buffers = '512MB';"
psql -c "SELECT pg_reload_conf();"

# 3. 验证变更
psql -c "SELECT name, setting FROM pg_settings WHERE name = 'shared_buffers';" > /tmp/after_change.txt

# 4. 观察期（5分钟）
sleep 300

# 5. 回滚
psql -c "ALTER SYSTEM SET shared_buffers = '256MB';"
psql -c "SELECT pg_reload_conf();"

# 6. 验证回滚
psql -c "SELECT name, setting FROM pg_settings WHERE name = 'shared_buffers';" > /tmp/after_rollback.txt

echo "=== 变更演练完成 ==="
```

---

## 交叉引用

### 相关文档

- ⭐⭐⭐ [性能调优变更闭环](./01-性能调优-变更闭环.md) - 性能调优变更流程
- ⭐⭐⭐ [监控与诊断](../../监控与诊断/06.01-监控与诊断.md) - 监控理论基础
- ⭐⭐ [性能调优实践](../../../11-部署架构/单机部署/05.02-性能调优实践.md) - 性能调优详细指南
- ⭐⭐ [集群与高可用演练SOP](./03-集群与高可用-演练SOP.md) - 高可用演练流程
- ⭐ [故障切换与订阅重建](./故障切换与订阅重建.md) - 故障切换流程

### 外部资源

- [PostgreSQL变更管理最佳实践](https://www.postgresql.org/docs/current/maintenance.html)
- [PostgreSQL DDL文档](https://www.postgresql.org/docs/current/ddl.html)
- [PostgreSQL参数配置](https://www.postgresql.org/docs/current/runtime-config.html)

---

**文档版本**: v1.0
**最后更新**: 2025-11-22
**PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
**维护者**: Documentation Team
