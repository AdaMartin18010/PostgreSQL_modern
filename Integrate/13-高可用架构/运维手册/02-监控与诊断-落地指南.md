...
> **📋 文档来源**: `PostgreSQL\06-运维实践\运维手册\02-监控与诊断-落地指南.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_roles WHERE rolname = 'metrics'
    ) THEN
        RAISE WARNING '角色metrics已存在，跳过创建';
        RETURN;
    END IF;

    CREATE ROLE metrics WITH LOGIN PASSWORD 'your_secure_password';
    RAISE NOTICE '监控用户metrics创建成功';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '角色metrics已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建监控用户失败: %', SQLERRM;
END $$;

-- 授予监控权限（PostgreSQL 10+，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_roles WHERE rolname = 'metrics'
    ) THEN
        RAISE EXCEPTION '角色metrics不存在，请先创建';
    END IF;

    GRANT pg_monitor TO metrics;
    RAISE NOTICE '已授予pg_monitor权限给metrics';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '角色metrics不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '授予监控权限失败: %', SQLERRM;
END $$;

-- 或者细粒度权限（PostgreSQL 9.6-，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_roles WHERE rolname = 'metrics'
    ...
    RAISE NOTICE '已授予细粒度监控权限给metrics';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '角色metrics不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '授予细粒度权限失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '已授予连接数据库权限给metrics';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '角色metrics或数据库postgres不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要数据库所有者权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '授予连接权限失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF role_exists THEN
        RAISE NOTICE '角色metrics存在，权限验证完成';
    ELSE
        RAISE WARNING '角色metrics不存在';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '验证权限失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '活跃连接数: % / %', active_connections, max_connections;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity或pg_settings视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'version、pg_database_size或current_database函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '获取系统概览失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF active_session_count = 0 THEN
        RAISE NOTICE '当前无活跃会话';
    ELSE
        RAISE NOTICE '当前有 % 个活跃会话', active_session_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '获取活跃会话失败: %', SQLERRM;
END $$;
