...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Top 10慢查询失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF wait_event_count > 0 THEN
        RAISE NOTICE '发现 % 个等待事件', wait_event_count;
    ELSE
        RAISE NOTICE '未发现等待事件';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
...
ORDER BY count DESC;
-- 执行时间: <100ms
-- 计划: WindowAgg -> GroupAggregate -> Seq Scan
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '锁等待情况查询失败: %', SQLERRM;
END $$;
...
EXCEPTION
    ...
    WHEN undefined_function THEN
        RAISE WARNING 'pg_total_relation_size函数不存在';
    WHEN division_by_zero THEN
        RAISE WARNING '计算过程中出现除零错误';
    WHEN OTHERS THEN
        RAISE EXCEPTION '表膨胀情况查询失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF index_count > 0 THEN
        RAISE NOTICE '发现 % 个public schema的索引', index_count;
    ELSE
        RAISE NOTICE '未发现索引';
    END IF;
EXCEPTION
...
BEGIN
    ...
    IF connection_count > 0 THEN
        RAISE NOTICE '发现 % 个数据库连接', connection_count;
    ELSE
        RAISE NOTICE '未发现数据库连接';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '连接统计查询失败: %', SQLERRM;
END $$;
...
EXCEPTION
    ...
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_database_size函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '数据库大小查询失败: %', SQLERRM;
END $$;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '复制延迟查询失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_bgwriter视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查点统计查询失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Autovacuum统计查询失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF slow_query_count > 0 THEN
        RAISE WARNING '发现 % 个慢查询（平均执行时间>1000ms）', slow_query_count;
    ELSE
        RAISE NOTICE '未发现慢查询';
    END IF;
EXCEPTION
...
-- 步骤2: 分析执行计划
-- 注意：EXPLAIN (ANALYZE, BUFFERS, VERBOSE) 需要粘贴具体的慢查询SQL
-- 示例：
-- EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
-- SELECT * FROM orders WHERE user_id = 12345;

-- 步骤3: 检查索引使用（带错误处理和性能测试）
DO $$
DECLARE
    index_count INT;
BEGIN
    ...
    IF index_count > 0 THEN
        RAISE NOTICE '发现 % 个用户索引', index_count;
    ELSE
        RAISE NOTICE '未发现用户索引';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查索引使用失败: %', SQLERRM;
END $$;
