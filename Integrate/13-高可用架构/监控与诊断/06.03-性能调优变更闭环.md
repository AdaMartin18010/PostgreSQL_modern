...
BEGIN
    ...
    IF activity_count > 0 THEN
        RAISE NOTICE '发现 % 个当前数据库的活动连接', activity_count;
    ELSE
        RAISE NOTICE '未发现活动连接';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查业务负载失败: %', SQLERRM;
END \$\$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RETURN;
    END IF;
    RAISE NOTICE '正在备份参数配置';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '参数备份失败: %', SQLERRM;
END \$\$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RETURN;
    END IF;
    RAISE NOTICE '正在创建执行计划基线';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建执行计划基线失败: %', SQLERRM;
END \$\$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '已删除现有视图performance_baseline';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '删除现有视图失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '性能基线: 活跃查询=%, 总执行时间=%, 缓存命中率=%, 等待查询=%',
        active_queries, total_exec_time, cache_hit_ratio, waiting_queries;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity或pg_stat_statements视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'current_database函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建性能基线失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表performance_baseline不存在，请先创建';
    END IF;
    ...
    RAISE NOTICE '性能基线记录完成';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表performance_baseline不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '记录基线失败: %', SQLERRM;
END $$;
```
...
BEGIN
    ...
    RAISE NOTICE '等待事件数: %', wait_event_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询等待事件分布失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF lock_wait_count > 0 THEN
        RAISE WARNING '发现 % 个锁等待', lock_wait_count;
    ELSE
        RAISE NOTICE '无锁等待';
    END IF;
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询锁等待情况失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF bloated_table_count > 0 THEN
        RAISE WARNING '发现 % 个表存在膨胀（死元组>1000）', bloated_table_count;
    ELSE
        RAISE NOTICE '无表膨胀问题';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询表膨胀情况失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_indexes视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询索引使用情况失败: %', SQLERRM;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '请替换为实际的慢查询SQL，然后执行EXPLAIN (ANALYZE, BUFFERS, VERBOSE, COSTS, TIMING)';
    RAISE NOTICE '示例: EXPLAIN (ANALYZE, BUFFERS, VERBOSE, COSTS, TIMING) SELECT * FROM orders WHERE ...';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '执行计划分析说明失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF plan_change_count > 0 THEN
        RAISE WARNING '发现 % 个查询存在计划变化（plans>1）', plan_change_count;
    ELSE
        RAISE NOTICE '无计划变化';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先创建扩展';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查计划缓存失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE '表performance_change_log创建成功';
    ELSE
        RAISE NOTICE '表performance_change_log已存在';
    END IF;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表performance_change_log已存在';
    WHEN OTHERS THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '表performance_change_log不存在，请先创建';
    END IF;
    ...
    RAISE NOTICE '变更记录插入成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表performance_change_log不存在，请先创建';
    WHEN check_violation THEN
        RAISE WARNING '变更记录数据违反约束';
    WHEN OTHERS THEN
...
BEGIN
    ...
    RAISE NOTICE '参数shared_buffers已设置为512MB并重新加载配置';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统参数';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效: shared_buffers = 512MB';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'ALTER SYSTEM SET失败: %', SQLERRM;
END $$;

-- 方法2: 直接修改配置文件（需要重启）
-- 编辑 postgresql.conf
-- shared_buffers = 512MB
-- 注意：此方法需要手动编辑文件并重启PostgreSQL

-- 方法3: 会话级设置（仅当前会话，带错误处理）
DO $$
BEGIN
    SET work_mem = '64MB';
    RAISE NOTICE '会话级参数work_mem已设置为64MB（仅当前会话有效）';
EXCEPTION
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效: work_mem = 64MB';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'SET work_mem失败: %', SQLERRM;
END $$;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '无法获取shared_buffers值: %', SQLERRM;
END $$;
```
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE WARNING '索引idx_orders_created_at已存在';
        RETURN;
    END IF;

    CREATE INDEX CONCURRENTLY idx_orders_created_at ON orders(created_at);
    RAISE NOTICE '索引idx_orders_created_at创建成功（在线创建，不阻塞写操作）';
EXCEPTION
    WHEN undefined_table THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE '索引idx_orders_old不存在，无需删除';
        RETURN;
    END IF;

    DROP INDEX CONCURRENTLY IF EXISTS idx_orders_old;
    RAISE NOTICE '索引idx_orders_old删除成功（在线删除，不阻塞写操作）';
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '索引idx_orders_created_at不存在，无法重建';
    END IF;

    REINDEX INDEX CONCURRENTLY idx_orders_created_at;
    RAISE NOTICE '索引idx_orders_created_at重建成功（在线重建，不阻塞写操作）';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '索引idx_orders_created_at不存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    ELSE
        ...
        RAISE NOTICE 'pg_stat_statements扩展未安装，无法获取平均查询时间';
    END IF;
    ...
    RAISE NOTICE '监控指标: 活跃查询=%, 平均查询时间=%, 缓存命中率=%',
        active_query_count, avg_query_time_val, cache_hit_ratio_val;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '必要的系统视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'current_database函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '实时监控关键指标失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF slow_query_count > 0 THEN
        RAISE NOTICE '发现 % 个慢查询（平均执行时间>100ms）', slow_query_count;
    ELSE
        RAISE NOTICE '无慢查询';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在，请先创建扩展';
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控慢查询变化失败: %', SQLERRM;
END $$;
...
LIMIT 10;
-- 执行时间: <100ms
-- 计划: Limit -> WindowAgg -> Sort -> Seq Scan
```
...
BEGIN
    ...
    RAISE NOTICE '参数shared_buffers已回滚到256MB并重新加载配置';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统参数';
    WHEN invalid_parameter_value THEN
        RAISE EXCEPTION '参数值无效: shared_buffers = 256MB';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN OTHERS THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE '索引idx_orders_created_at不存在，无需回滚';
        RETURN;
    END IF;

    DROP INDEX CONCURRENTLY IF EXISTS idx_orders_created_at;
    RAISE NOTICE '索引idx_orders_created_at回滚成功（在线删除，不阻塞写操作）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '索引回滚失败: %', SQLERRM;
END $$;

-- 更新变更记录（带错误处理）
DO $$
DECLARE
    change_id_val INT := <change_id>;  -- 需要替换为实际的change_id
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'performance_change_log'
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '变更记录change_id=%不存在', change_id_val;
    END IF;
    ...
    RAISE NOTICE '变更记录更新成功: change_id=%', change_id_val;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表performance_change_log不存在';
    WHEN OTHERS THEN
