...
BEGIN
    ...
    IF slow_query_count > 0 THEN
        RAISE WARNING '发现 % 个慢查询（平均执行时间>500ms）', slow_query_count;
    ELSE
        RAISE NOTICE '未发现慢查询';
    END IF;
EXCEPTION
...
BEGIN
    ...
    RAISE NOTICE '找到 % 条统计信息记录', stats_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stats视图不存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    RAISE NOTICE '统计信息精度已提升: created_at和status列设置为2000';
EXCEPTION
    WHEN undefined_table THEN
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '已删除现有索引: idx_orders_created_status';
    END IF;

    CREATE INDEX CONCURRENTLY idx_orders_created_status
    ON orders (created_at, status)
    WHERE status = 'pending';
    RAISE NOTICE '复合索引创建成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_object THEN
        RAISE WARNING '索引idx_orders_created_status已存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表%不存在', TG_TABLE_NAME;
            RETURN NEW;
        WHEN undefined_column THEN
            RAISE WARNING '列%不存在', NEW.column_name;
            RETURN NEW;
        WHEN OTHERS THEN
            RAISE WARNING '设置统计精度失败: %', SQLERRM;
            RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;

    RAISE NOTICE '函数set_column_statistics创建成功';
EXCEPTION
    WHEN duplicate_function THEN
        RAISE WARNING '函数set_column_statistics已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建函数set_column_statistics失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE WARNING '列new_field已存在，跳过添加';
        RETURN;
    END IF;

    ALTER TABLE orders ADD COLUMN new_field VARCHAR(100);
    RAISE NOTICE '列new_field添加成功';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN duplicate_column THEN
        RAISE WARNING '列new_field已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '添加列失败: %', SQLERRM;
END $$;

-- 步骤2: 批量更新默认值（在低峰期，带错误处理）
DO $$
DECLARE
    updated_count INT;
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '列new_field不存在，请先添加';
    END IF;

    UPDATE orders SET new_field = 'default_value' WHERE new_field IS NULL;
    GET DIAGNOSTICS updated_count = ROW_COUNT;
    RAISE NOTICE '更新了 % 条记录的默认值', updated_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_column THEN
        RAISE EXCEPTION '列new_field不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '批量更新默认值失败: %', SQLERRM;
END $$;

-- 步骤3: 设置NOT NULL约束（需要锁表，但时间短，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
...
EXCEPTION
    ...
    WHEN check_violation THEN
        RAISE EXCEPTION '存在NULL值，无法设置NOT NULL约束';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置NOT NULL约束失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_current_wal_lsn') THEN
        RAISE WARNING 'pg_current_wal_lsn函数不存在';
        RETURN;
    END IF;
    ...
    IF max_wal_size_bytes > 0 THEN
        ...
        RAISE NOTICE 'WAL使用率: %', wal_usage_pct;
        IF wal_usage_pct > 80 THEN
            RAISE WARNING 'WAL使用率超过80%，建议检查checkpoint配置';
        END IF;
    END IF;
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控WAL使用率失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '表log_entries不存在，跳过诊断';
        RETURN;
    END IF;

    RAISE NOTICE '执行EXPLAIN分析...';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表log_entries不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '分区裁剪问题诊断失败: %', SQLERRM;
END $$;
