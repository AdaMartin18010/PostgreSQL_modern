...
> **📋 文档来源**: `PostgreSQL_View\06-架构设计\高可用架构\灾难恢复方案.md`
> **📅 复制日期**: 2025-12-22
...
- [6.1 备份建议](#61-备份建议)
- [6.2 恢复建议](#62-恢复建议)
- [7.1 案例：金融系统灾难恢复方案实施](#71-案例金融系统灾难恢复方案实施)
- [7.2 案例：电商平台灾难恢复方案实施](#72-案例电商平台灾难恢复方案实施)
- [7.1 官方文档](#71-官方文档)
- [7.2 学术论文](#72-学术论文)
- [7.3 技术博客](#73-技术博客)
- [7.4 相关资源](#74-相关资源)
- [8.1 WAL 归档配置](#81-wal-归档配置)
- [8.2 时间点恢复 (PITR) 实现](#82-时间点恢复-pitr-实现)
- [8.3 pgBackRest 备份配置](#83-pgbackrest-备份配置)
---
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '发布cross_region_pub不存在，请先创建';
    END IF;
...
BEGIN
    ...
    IF pg_is_in_recovery() THEN
        PERFORM pg_promote();
        RAISE NOTICE '备份区域已提升为主区域';
    ELSE
        RAISE WARNING '当前节点不是从库，无需提升';
    END IF;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_promote或pg_is_in_recovery函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '提升备份区域失败: %', SQLERRM;
END $$;
