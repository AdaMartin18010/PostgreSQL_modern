...
> **📋 文档来源**: `DataBaseTheory\09-数据模型与规范化\05-反范式化策略.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (
            ...
            RAISE NOTICE '列 total_amount 添加成功';
        ELSE
            RAISE NOTICE '列 total_amount 已存在';
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '更新订单总额失败: %', SQLERRM;
                RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_stats') THEN
            ...
            RAISE NOTICE '表 daily_stats 创建成功';
        ELSE
            RAISE NOTICE '表 daily_stats 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 daily_stats 已存在';
...
BEGIN
    BEGIN
        DROP MATERIALIZED VIEW IF EXISTS daily_stats_mv;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_daily_stats_date') THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users_denorm') THEN
            CREATE TABLE users_denorm (
                id SERIAL PRIMARY KEY,
                username VARCHAR(50),
                addresses JSONB
            );
            RAISE NOTICE '表 users_denorm 创建成功';
        ELSE
            RAISE NOTICE '表 users_denorm 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 users_denorm 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_addresses_gin') THEN
...
BEGIN
    BEGIN
        ...
        ELSE
            RAISE NOTICE '表 user_profiles 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 user_profiles 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_stats') THEN
            ...
            RAISE NOTICE '表 user_stats 创建成功';
        ELSE
            RAISE NOTICE '表 user_stats 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 user_stats 已存在';
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '更新用户发帖数失败: %', SQLERRM;
                RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
        ...
        RAISE NOTICE '触发器 trg_post_created 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建触发器失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        CREATE OR REPLACE FUNCTION decrease_user_post_count()
        RETURNS TRIGGER AS $$
        BEGIN
            UPDATE user_stats
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '减少用户发帖数失败: %', SQLERRM;
                RETURN OLD;
        END;
        $$ LANGUAGE plpgsql;

        DROP TRIGGER IF EXISTS trg_post_deleted ON posts;
        CREATE TRIGGER trg_post_deleted
        AFTER DELETE ON posts
        FOR EACH ROW
        EXECUTE FUNCTION decrease_user_post_count();
        RAISE NOTICE '触发器 trg_post_deleted 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建触发器失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'orders_analytics') THEN
            ...
            RAISE NOTICE '表 orders_analytics 创建成功';
        ELSE
            RAISE NOTICE '表 orders_analytics 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 orders_analytics 已存在';
...
-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM orders_analytics WHERE year = 2024;
```
