...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '函数依赖检测失败: %', SQLERRM;
                RETURN QUERY SELECT false, 0::BIGINT, NULL::JSONB;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '函数 check_functional_dependency 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
-- PostgreSQL 18：虚拟生成列的索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_total_value') THEN
            CREATE INDEX idx_total_value ON products(total_value);
            RAISE NOTICE '索引 idx_total_value 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_total_value 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (
            ...
            CHECK (total_value >= 0);
            RAISE NOTICE '约束 check_total_positive 创建成功';
        ELSE
            RAISE NOTICE '约束 check_total_positive 已存在';
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        ...
        ELSE
            RAISE NOTICE '表 subscriptions 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 subscriptions 已存在';
...
-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM employees WHERE total_salary > 10000;
```
...
```sql
-- PostgreSQL 18：UUIDv7时间顺序性验证（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'test_uuid') THEN
            CREATE TABLE test_uuid (
                id UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
                data TEXT
            );
            RAISE NOTICE '表 test_uuid 创建成功';
        ELSE
            RAISE NOTICE '表 test_uuid 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 test_uuid 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'test_uuidv4') THEN
            CREATE TABLE test_uuidv4 (
                id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                data TEXT
            );
            RAISE NOTICE '表 test_uuidv4 创建成功';
        ELSE
            RAISE NOTICE '表 test_uuidv4 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 test_uuidv4 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'events_v4') THEN
            CREATE TABLE events_v4 (
                id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                event_type VARCHAR(50),
                event_data JSONB
            );
            RAISE NOTICE '表 events_v4 创建成功';
        ELSE
            RAISE NOTICE '表 events_v4 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 events_v4 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'events_v7') THEN
            CREATE TABLE events_v7 (
                id UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
                event_type VARCHAR(50),
                event_data JSONB
            );
            RAISE NOTICE '表 events_v7 创建成功';
        ELSE
            RAISE NOTICE '表 events_v7 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 events_v7 已存在';
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM events_v4 WHERE id > uuid_generate_v4() ORDER BY id LIMIT 100;

-- UUIDv7：利用时间顺序性，50ms（2倍提升）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM events_v7 WHERE id > uuid_generate_v7() ORDER BY id LIMIT 100;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'student_courses_bad') THEN
            ...
            RAISE NOTICE '表 student_courses_bad 创建成功';
        ELSE
            RAISE NOTICE '表 student_courses_bad 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 student_courses_bad 已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'order_details_bad') THEN
            ...
            RAISE NOTICE '表 order_details_bad 创建成功';
        ELSE
            RAISE NOTICE '表 order_details_bad 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 order_details_bad 已存在';
