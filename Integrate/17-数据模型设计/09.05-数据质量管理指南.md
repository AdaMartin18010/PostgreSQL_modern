...
BEGIN
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            ...
            SELECT jsonb_build_object(
                'completeness_rate', COALESCE(v_completeness_rate, 0),
                'status', CASE
                    WHEN COALESCE(v_completeness_rate, 0) >= 95.0 THEN 'pass'
                    ELSE 'fail'
                END
            ) INTO v_result;
        ...
        ELSE
            RAISE NOTICE 'users表不存在，跳过完整性检查';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            SELECT jsonb_build_object(
                'accuracy_rate', COALESCE(v_accuracy_rate, 0),
                'status', CASE
                    WHEN COALESCE(v_accuracy_rate, 0) >= 99.0 THEN 'pass'
                    ELSE 'fail'
                END
            ) INTO v_result;
            ...
            v_check_count := v_check_count + 1;
        ELSE
            RAISE NOTICE 'orders表不存在，跳过准确性检查';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    RAISE NOTICE '质量检查完成: 共执行 % 项检查', v_check_count;
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            RAISE NOTICE 'orders表不存在，跳过清洗';
        END IF;
    EXCEPTION
        WHEN numeric_value_out_of_range THEN
            RAISE WARNING 'orders表金额修正时数值溢出';
...
- ⭐ [数据科学实践](../../16-应用设计与开发/应用架构/07.06-数据科学实践.md) - 数据科学中的数据质量
