...
BEGIN
    ...
    BEGIN
        ...
        WHERE COALESCE(o.amount, 0) > 0  -- 过滤无效数据
          AND o.order_date >= '2024-01-01'  -- 只处理有效日期范围
          AND o.order_date IS NOT NULL
          AND o.user_id IS NOT NULL;

        GET DIAGNOSTICS v_transformed_count = ROW_COUNT;
        RAISE NOTICE '转换订单数据: % 条', v_transformed_count;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的订单ID';
        WHEN foreign_key_violation THEN
            RAISE EXCEPTION '违反外键约束，请检查date_dim和customer_dim数据';
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '金额数值溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '转换订单数据失败: %', SQLERRM;
    END;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_fact') THEN
        RAISE EXCEPTION 'sales_fact表不存在';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'staging_order_items') THEN
        RAISE EXCEPTION 'staging_order_items表不存在';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '加载销售事实数据: % 条', v_loaded_count;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的销售事实记录';
        WHEN foreign_key_violation THEN
            RAISE EXCEPTION '违反外键约束，请检查维度表数据';
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '金额或数量数值溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '加载销售事实数据失败: %', SQLERRM;
    END;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取上次更新时间失败: %', SQLERRM;
    ...
    IF v_last_update_date IS NULL THEN
        ...
        RAISE NOTICE '没有历史数据，从 % 开始增量更新', v_last_update_date;
    ELSE
    ...
    BEGIN
        ...
        RAISE NOTICE '增量更新销售数据: % 条', v_updated_count;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的销售事实记录';
        WHEN foreign_key_violation THEN
        ...
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '金额或数量数值溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '增量加载新数据失败: %', SQLERRM;
    END;
...
BEGIN
    ...
    BEGIN
        ...
        RAISE NOTICE '成功创建分区: % (日期范围: % 至 %)', v_partition_name, v_start_date, v_end_date;
    EXCEPTION
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'fact_sales_columnar') THEN
        RAISE EXCEPTION 'fact_sales_columnar表不存在';
    END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '获取归档日期ID失败: %', SQLERRM;
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE '归档到列存储: % 条记录', v_archived_count;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的归档记录';
        WHEN foreign_key_violation THEN
    ...
    -- 删除已迁移的数据（可选，根据需求决定是否启用）
    -- BEGIN
    --     DELETE FROM sales_fact
    --     WHERE date_id < v_archive_date_id
    --       AND sale_id IN (SELECT sale_id FROM fact_sales_columnar);
    --     RAISE NOTICE '删除已归档数据完成';
    -- EXCEPTION
    --     WHEN OTHERS THEN
    --         RAISE WARNING '删除已归档数据失败: %', SQLERRM;
    -- END;
EXCEPTION
    WHEN OTHERS THEN
