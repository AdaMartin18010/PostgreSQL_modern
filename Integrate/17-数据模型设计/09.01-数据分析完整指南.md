...
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'mv_daily_sales_summary') THEN
        RAISE EXCEPTION 'mv_daily_sales_summary表不存在，无法刷新汇总数据';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales') THEN
        RAISE EXCEPTION 'sales表不存在，无法刷新汇总数据';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '删除当天数据: % 条', v_deleted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '删除当天数据失败: %', SQLERRM;
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE '插入当天汇总数据: % 条', v_inserted_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'sales或mv_daily_sales_summary表不存在';
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的汇总记录';
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '汇总金额计算溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '插入汇总数据失败: %', SQLERRM;
    END;

    RAISE NOTICE '刷新完成。删除: % 条，插入: % 条', v_deleted_count, v_inserted_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'refresh_daily_sales_summary执行失败: %', SQLERRM;
END;
$$;
```
