...
> **📋 文档来源**: `PostgreSQL\09-应用设计\数据模型设计\09.02-数据建模完整指南.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    -- 检查NEW记录是否存在
    IF NEW IS NULL THEN
        RAISE WARNING 'NEW记录为空，无法更新订单总金额';
        RETURN NULL;
    END IF;

    -- 检查order_id是否存在
    IF NEW.order_id IS NULL THEN
        RAISE WARNING 'order_id为空，无法更新订单总金额';
        RETURN NEW;
    END IF;

    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE WARNING 'orders表不存在，无法更新订单总金额';
        RETURN NEW;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
        RAISE WARNING 'order_items表不存在，无法更新订单总金额';
        RETURN NEW;
    END IF;
    ...
    BEGIN
        ...
        -- 检查数值溢出
        IF v_total_amount > 9999999999.99 THEN
            RAISE WARNING '订单总金额超出范围，使用最大值';
            v_total_amount := 9999999999.99;
        END IF;
        ...
        RAISE NOTICE '订单总金额已更新: order_id=%, total_amount=%', NEW.order_id, v_total_amount;
    EXCEPTION
        WHEN numeric_value_out_of_range THEN
            RAISE WARNING '订单总金额计算溢出: order_id=%', NEW.order_id;
        WHEN OTHERS THEN
            RAISE WARNING '更新订单总金额失败: order_id=%, 错误: %', NEW.order_id, SQLERRM;
    END;

    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'update_order_total触发器函数执行失败: %', SQLERRM;
...
```mermaid
erDiagram
    ORDERS_HOT ||--o{ ORDER_ITEMS_HOT : "包含"
    ORDERS_COLD ||--o{ ORDER_ITEMS_COLD : "包含"
    ORDERS_HOT {
        bigint order_id PK
        bigint user_id FK
    ...
    ORDERS_HOT }o--|| USERS : "属于"
    ORDERS_COLD }o--|| USERS : "属于"
```
...
flowchart LR
    subgraph "数据写入"
        A[新订单数据] --> B{订单日期}
        B -->|最近3个月| C[orders_hot<br/>行存储]
        B -->|3个月以前| D[orders_cold<br/>列存储]
    end

    subgraph "数据迁移"
        E[定时任务] --> F{检查orders_hot}
        F -->|超过3个月| G[迁移到orders_cold]
        G --> H[转换为列存储]
    end

    subgraph "查询路由"
        I[查询请求] --> J{查询条件}
        J -->|最近3个月| K[查询orders_hot]
        J -->|历史数据| L[查询orders_cold]
        J -->|全量数据| M[UNION查询]
    end
...
BEGIN
    -- 检查必需表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders_hot') THEN
        RAISE EXCEPTION 'orders_hot表不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders_cold') THEN
        RAISE EXCEPTION 'orders_cold表不存在';
    END IF;

    -- 计算归档日期（3个月前）
    v_archive_date := CURRENT_DATE - INTERVAL '3 months';
    RAISE NOTICE '归档日期: %', v_archive_date;
    ...
    BEGIN
        ...
        WHERE order_date < v_archive_date
          ...
          AND order_id NOT IN (SELECT order_id FROM orders_cold WHERE order_id IS NOT NULL);

        GET DIAGNOSTICS v_archived_count = ROW_COUNT;
        RAISE NOTICE '归档订单: % 条', v_archived_count;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '存在重复的订单ID，跳过已存在的记录';
        WHEN foreign_key_violation THEN
            RAISE EXCEPTION '违反外键约束，请检查关联表数据';
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '金额或数量数值溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '归档订单失败: %', SQLERRM;
    END;
    ...
    IF v_archived_count > 0 THEN
        BEGIN
            ...
            WHERE order_date < v_archive_date
              AND order_id IN (SELECT order_id FROM orders_cold WHERE order_date < v_archive_date);

            GET DIAGNOSTICS v_deleted_count = ROW_COUNT;
            RAISE NOTICE '删除已归档订单: % 条', v_deleted_count;
        EXCEPTION
            WHEN foreign_key_violation THEN
                RAISE WARNING '删除失败：存在外键约束，可能需要先删除关联数据';
            WHEN OTHERS THEN
                RAISE WARNING '删除已归档订单失败: %', SQLERRM;
        END;
    END IF;

    RAISE NOTICE '归档完成。归档: % 条，删除: % 条', v_archived_count, v_deleted_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'archive_old_orders执行失败: %', SQLERRM;
END;
$$;
```
...
sequenceDiagram
    ...
    Query Router->>Query Router: 分析查询条件

    alt 查询最近3个月数据
        Query Router->>OrdersHot: 查询热数据表
        OrdersHot-->>Query Router: 返回热数据
    else 查询历史数据
        Query Router->>OrdersCold: 查询冷数据表（列存储）
        OrdersCold-->>Query Router: 返回冷数据
    else 查询跨时间段数据
        Query Router->>OrdersHot: 查询热数据
        Query Router->>OrdersCold: 查询冷数据
        OrdersHot-->>Result Merger: 热数据结果
        OrdersCold-->>Result Merger: 冷数据结果
        Result Merger->>Result Merger: UNION合并结果
        Result Merger-->>Query Router: 合并结果
    end

    Query Router-->>Client: 返回最终结果
```
...
BEGIN
    -- 检查NEW记录是否存在
    IF NEW IS NULL THEN
        RAISE WARNING 'NEW记录为空，无法记录订单创建事件';
        RETURN NULL;
    END IF;

    -- 检查必需字段
    IF NEW.order_id IS NULL THEN
        RAISE WARNING 'order_id为空，无法记录订单创建事件';
        RETURN NEW;
    END IF;

    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'domain_events') THEN
        RAISE WARNING 'domain_events表不存在，无法记录事件';
        RETURN NEW;
    END IF;
    ...
    BEGIN
        v_event_data := jsonb_build_object(
            ...
            'user_id', COALESCE(NEW.user_id, 0),
            'total_amount', COALESCE(NEW.total_amount, 0),
            'created_at', COALESCE(NEW.created_at, NOW())
        );
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '构建事件数据失败: %', SQLERRM;
            v_event_data := jsonb_build_object('error', 'Failed to build event data');
    END;

    -- 插入领域事件
    BEGIN
        INSERT INTO domain_events (aggregate_type, aggregate_id, event_type, event_data)
        VALUES ('Order', NEW.order_id, 'OrderCreated', v_event_data);

        RAISE NOTICE '订单创建事件已记录: order_id=%', NEW.order_id;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '事件记录已存在: order_id=%', NEW.order_id;
        WHEN foreign_key_violation THEN
            RAISE WARNING '违反外键约束，无法记录事件';
        WHEN OTHERS THEN
            RAISE WARNING '记录订单创建事件失败: order_id=%, 错误: %', NEW.order_id, SQLERRM;
    END;

    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'record_order_created_event触发器函数执行失败: %', SQLERRM;
        RETURN NEW;  -- 即使出错也返回NEW，避免影响主操作
END;
$$;
...
- ⭐⭐ [扩展系统](../00-归档-项目管理文档/README.md) - 列存储扩展（cstore_fdw）🆕
