...
flowchart TD
    A[选择访问控制模型] --> B{安全需求?}
    B -->|简单| C[ACL<br/>访问控制列表]
    B -->|中等| D[RBAC<br/>基于角色]
    B -->|高| E[ABAC<br/>基于属性]
    B -->|最高| F[MAC<br/>强制访问控制]

    C --> G{用户数量?}
    G -->|少| H[PostgreSQL 18<br/>用户权限]
    G -->|多| D

    D --> I{权限粒度?}
    I -->|粗| J[PostgreSQL 18<br/>角色权限]
    I -->|细| E

    E --> K{动态策略?}
    K -->|是| L[PostgreSQL 18<br/>RLS策略]
    K -->|否| D

    F --> M[PostgreSQL 18<br/>安全标签]
