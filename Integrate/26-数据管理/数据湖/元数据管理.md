# PostgreSQL数据湖元数据管理指南

> **创建日期**: 2025年1月
> **技术版本**: PostgreSQL 17+/18+
> **难度等级**: ⭐⭐⭐ 中级

---

## 📋 目录

- [PostgreSQL数据湖元数据管理指南](#postgresql数据湖元数据管理指南)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. 元数据模型](#2-元数据模型)
    - [2.1 元数据表](#21-元数据表)
  - [3. 元数据存储](#3-元数据存储)
    - [3.1 自动收集](#31-自动收集)
  - [4. 元数据查询](#4-元数据查询)
    - [4.1 数据发现](#41-数据发现)
    - [4.2 数据血缘](#42-数据血缘)
  - [5. 元数据维护](#5-元数据维护)
    - [5.1 定期更新](#51-定期更新)
    - [5.2 元数据验证](#52-元数据验证)
  - [6. 数据血缘管理](#6-数据血缘管理)
    - [6.1 数据血缘表](#61-数据血缘表)
    - [6.2 数据血缘查询](#62-数据血缘查询)
    - [6.3 影响分析](#63-影响分析)
  - [7. 元数据搜索和发现](#7-元数据搜索和发现)
    - [7.1 全文搜索](#71-全文搜索)
    - [7.2 数据发现](#72-数据发现)
  - [8. 元数据版本管理](#8-元数据版本管理)
    - [8.1 元数据版本表](#81-元数据版本表)
    - [8.2 版本对比](#82-版本对比)
  - [9. 元数据监控和告警](#9-元数据监控和告警)
    - [9.1 元数据监控](#91-元数据监控)
    - [9.2 元数据告警](#92-元数据告警)
  - [📚 相关文档](#-相关文档)

---

## 1. 概述

元数据管理是数据湖系统的核心，提供数据目录和发现功能。

**元数据内容**:

- 数据源信息
- 数据格式
- 数据模式
- 数据血缘

---

## 2. 元数据模型

### 2.1 元数据表

```sql
-- 数据源元数据
CREATE TABLE data_source_metadata (
    id SERIAL PRIMARY KEY,
    source_name TEXT NOT NULL,
    source_type TEXT,
    location TEXT,
    format TEXT,
    schema_info JSONB,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 数据表元数据
CREATE TABLE table_metadata (
    id SERIAL PRIMARY KEY,
    table_name TEXT NOT NULL,
    source_id INT REFERENCES data_source_metadata(id),
    columns JSONB,
    row_count BIGINT,
    size_bytes BIGINT,
    last_updated TIMESTAMPTZ
);
```

---

## 3. 元数据存储

### 3.1 自动收集

```sql
-- 自动收集表元数据
CREATE OR REPLACE FUNCTION collect_table_metadata()
RETURNS VOID AS $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN
        SELECT
            schemaname,
            tablename,
            pg_total_relation_size(schemaname||'.'||tablename) AS size
        FROM pg_tables
        WHERE schemaname = 'public'
    LOOP
        INSERT INTO table_metadata (table_name, size_bytes)
        VALUES (r.tablename, r.size)
        ON CONFLICT (table_name) DO UPDATE
        SET size_bytes = r.size, last_updated = NOW();
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

---

## 4. 元数据查询

### 4.1 数据发现

```sql
-- 查找数据表
SELECT
    table_name,
    columns,
    row_count,
    size_bytes
FROM table_metadata
WHERE columns @> '[{"name": "user_id"}]'::jsonb;
```

### 4.2 数据血缘

```sql
-- 数据血缘查询
SELECT
    source_table,
    target_table,
    transformation
FROM data_lineage
WHERE source_table = 'users';
```

---

## 5. 元数据维护

### 5.1 定期更新

```sql
-- 定期更新元数据
SELECT cron.schedule(
    'update-metadata',
    '0 2 * * *',
    'SELECT collect_table_metadata();'
);
```

### 5.2 元数据验证

```sql
-- 验证元数据一致性（带错误处理和性能测试）
CREATE OR REPLACE FUNCTION validate_metadata()
RETURNS TABLE (
    table_name TEXT,
    status TEXT,
    actual_count BIGINT,
    metadata_count BIGINT
) AS $$
DECLARE
    table_rec RECORD;
    actual_row_count BIGINT;
BEGIN
    FOR table_rec IN
        SELECT tm.table_name, tm.row_count
        FROM table_metadata tm
    LOOP
        BEGIN
            -- 获取实际行数
            EXECUTE format('SELECT COUNT(*) FROM %I', table_rec.table_name)
            INTO actual_row_count;

            RETURN QUERY SELECT
                table_rec.table_name,
                CASE
                    WHEN actual_row_count = table_rec.row_count THEN '一致'
                    ELSE '不一致'
                END,
                actual_row_count,
                table_rec.row_count;

        EXCEPTION
            WHEN undefined_table THEN
                RETURN QUERY SELECT
                    table_rec.table_name,
                    '表不存在'::TEXT,
                    NULL::BIGINT,
                    table_rec.row_count;
            WHEN OTHERS THEN
                RETURN QUERY SELECT
                    table_rec.table_name,
                    format('验证失败: %', SQLERRM)::TEXT,
                    NULL::BIGINT,
                    table_rec.row_count;
        END;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 执行元数据验证
SELECT * FROM validate_metadata();
```

---

## 6. 数据血缘管理

### 6.1 数据血缘表

```sql
-- 创建数据血缘表
CREATE TABLE IF NOT EXISTS data_lineage (
    id SERIAL PRIMARY KEY,
    source_table TEXT NOT NULL,
    target_table TEXT NOT NULL,
    transformation TEXT,
    transformation_type TEXT,  -- 'ETL', 'VIEW', 'MATERIALIZED_VIEW'
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(source_table, target_table)
);

-- 插入数据血缘关系
INSERT INTO data_lineage (source_table, target_table, transformation, transformation_type)
VALUES
    ('source_sales', 'staging_sales', 'Extract from source system', 'ETL'),
    ('staging_sales', 'fact_sales', 'Transform and load', 'ETL'),
    ('fact_sales', 'mv_sales_summary', 'Aggregate by time and product', 'MATERIALIZED_VIEW')
ON CONFLICT (source_table, target_table) DO NOTHING;
```

### 6.2 数据血缘查询

```sql
-- 查询数据血缘（递归查询）
WITH RECURSIVE lineage_path AS (
    -- 起始点
    SELECT
        source_table,
        target_table,
        transformation,
        ARRAY[source_table, target_table] AS path,
        1 AS depth
    FROM data_lineage
    WHERE source_table = 'source_sales'

    UNION ALL

    -- 递归扩展
    SELECT
        dl.source_table,
        dl.target_table,
        dl.transformation,
        lp.path || dl.target_table,
        lp.depth + 1
    FROM data_lineage dl
    JOIN lineage_path lp ON dl.source_table = lp.target_table
    WHERE dl.target_table != ALL(lp.path)  -- 避免循环
      AND lp.depth < 10  -- 限制深度
)
SELECT
    path,
    depth,
    transformation
FROM lineage_path
ORDER BY depth, path;
```

### 6.3 影响分析

```sql
-- 影响分析：查询依赖某个表的所有下游表
CREATE OR REPLACE FUNCTION analyze_impact(p_table_name TEXT)
RETURNS TABLE (
    dependent_table TEXT,
    transformation TEXT,
    depth INT
) AS $$
BEGIN
    RETURN QUERY
    WITH RECURSIVE impact_analysis AS (
        SELECT
            target_table,
            transformation,
            1 AS depth
        FROM data_lineage
        WHERE source_table = p_table_name

        UNION ALL

        SELECT
            dl.target_table,
            dl.transformation,
            ia.depth + 1
        FROM data_lineage dl
        JOIN impact_analysis ia ON dl.source_table = ia.target_table
        WHERE ia.depth < 10
    )
    SELECT DISTINCT
        target_table,
        transformation,
        MIN(depth) OVER (PARTITION BY target_table) AS depth
    FROM impact_analysis
    ORDER BY depth, target_table;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '影响分析失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- 执行影响分析
SELECT * FROM analyze_impact('source_sales');
```

---

## 7. 元数据搜索和发现

### 7.1 全文搜索

```sql
-- 创建全文搜索索引
ALTER TABLE table_metadata ADD COLUMN IF NOT EXISTS search_vector tsvector;

-- 更新搜索向量
UPDATE table_metadata
SET search_vector =
    to_tsvector('english',
        COALESCE(table_name, '') || ' ' ||
        COALESCE(columns::text, '')
    );

-- 创建GIN索引
CREATE INDEX IF NOT EXISTS idx_table_metadata_search
ON table_metadata USING GIN (search_vector);

-- 全文搜索
SELECT
    table_name,
    columns,
    ts_rank(search_vector, query) AS rank
FROM table_metadata, to_tsquery('english', 'user & sales') query
WHERE search_vector @@ query
ORDER BY rank DESC;
```

### 7.2 数据发现

```sql
-- 数据发现函数（带错误处理和性能测试）
CREATE OR REPLACE FUNCTION discover_data(
    p_search_term TEXT,
    p_column_name TEXT DEFAULT NULL
)
RETURNS TABLE (
    table_name TEXT,
    column_name TEXT,
    data_type TEXT,
    match_score NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        tm.table_name,
        col->>'name' AS column_name,
        col->>'type' AS data_type,
        CASE
            WHEN col->>'name' ILIKE '%' || p_search_term || '%' THEN 100.0
            WHEN col->>'type' ILIKE '%' || p_search_term || '%' THEN 50.0
            ELSE 10.0
        END AS match_score
    FROM table_metadata tm,
         jsonb_array_elements(tm.columns) AS col
    WHERE (p_column_name IS NULL OR col->>'name' = p_column_name)
      AND (
          col->>'name' ILIKE '%' || p_search_term || '%' OR
          col->>'type' ILIKE '%' || p_search_term || '%'
      )
    ORDER BY match_score DESC, table_name, column_name;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '数据发现失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- 执行数据发现
SELECT * FROM discover_data('user');
SELECT * FROM discover_data('amount', 'sales_amount');
```

---

## 8. 元数据版本管理

### 8.1 元数据版本表

```sql
-- 创建元数据版本表
CREATE TABLE IF NOT EXISTS table_metadata_history (
    id SERIAL PRIMARY KEY,
    table_name TEXT NOT NULL,
    columns JSONB,
    row_count BIGINT,
    size_bytes BIGINT,
    version_number INT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建版本管理函数
CREATE OR REPLACE FUNCTION create_metadata_version()
RETURNS TRIGGER AS $$
DECLARE
    v_version_number INT;
BEGIN
    -- 获取下一个版本号
    SELECT COALESCE(MAX(version_number), 0) + 1
    INTO v_version_number
    FROM table_metadata_history
    WHERE table_name = NEW.table_name;

    -- 插入历史版本
    INSERT INTO table_metadata_history (
        table_name,
        columns,
        row_count,
        size_bytes,
        version_number
    )
    VALUES (
        NEW.table_name,
        NEW.columns,
        NEW.row_count,
        NEW.size_bytes,
        v_version_number
    );

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器
CREATE TRIGGER metadata_version_trigger
AFTER UPDATE ON table_metadata
FOR EACH ROW
WHEN (OLD.columns IS DISTINCT FROM NEW.columns OR
      OLD.row_count IS DISTINCT FROM NEW.row_count)
EXECUTE FUNCTION create_metadata_version();
```

### 8.2 版本对比

```sql
-- 版本对比函数
CREATE OR REPLACE FUNCTION compare_metadata_versions(
    p_table_name TEXT,
    p_version1 INT,
    p_version2 INT
)
RETURNS TABLE (
    change_type TEXT,
    change_detail TEXT
) AS $$
DECLARE
    v1 RECORD;
    v2 RECORD;
BEGIN
    SELECT * INTO v1
    FROM table_metadata_history
    WHERE table_name = p_table_name AND version_number = p_version1;

    SELECT * INTO v2
    FROM table_metadata_history
    WHERE table_name = p_table_name AND version_number = p_version2;

    IF v1 IS NULL OR v2 IS NULL THEN
        RAISE EXCEPTION '版本不存在';
    END IF;

    -- 对比列变化
    IF v1.columns IS DISTINCT FROM v2.columns THEN
        RETURN QUERY SELECT 'COLUMN_CHANGE'::TEXT,
            format('列定义从 % 变为 %', v1.columns::text, v2.columns::text)::TEXT;
    END IF;

    -- 对比行数变化
    IF v1.row_count IS DISTINCT FROM v2.row_count THEN
        RETURN QUERY SELECT 'ROW_COUNT_CHANGE'::TEXT,
            format('行数从 % 变为 %', v1.row_count, v2.row_count)::TEXT;
    END IF;

    RETURN;
END;
$$ LANGUAGE plpgsql;

-- 执行版本对比
SELECT * FROM compare_metadata_versions('fact_sales', 1, 2);
```

---

## 9. 元数据监控和告警

### 9.1 元数据监控

```sql
-- 创建元数据监控视图
CREATE OR REPLACE VIEW v_metadata_monitoring AS
SELECT
    tm.table_name,
    tm.row_count,
    tm.size_bytes,
    tm.last_updated,
    NOW() - tm.last_updated AS age,
    CASE
        WHEN NOW() - tm.last_updated > INTERVAL '7 days' THEN 'STALE'
        WHEN NOW() - tm.last_updated > INTERVAL '1 day' THEN 'WARNING'
        ELSE 'FRESH'
    END AS freshness_status
FROM table_metadata tm;

-- 查询过时的元数据
SELECT * FROM v_metadata_monitoring
WHERE freshness_status IN ('STALE', 'WARNING')
ORDER BY age DESC;
```

### 9.2 元数据告警

```sql
-- 元数据告警函数（带错误处理和性能测试）
CREATE OR REPLACE FUNCTION check_metadata_alerts()
RETURNS TABLE (
    alert_type TEXT,
    alert_message TEXT,
    alert_level TEXT
) AS $$
DECLARE
    stale_count INT;
    inconsistent_count INT;
BEGIN
    -- 检查过时元数据
    SELECT COUNT(*) INTO stale_count
    FROM v_metadata_monitoring
    WHERE freshness_status = 'STALE';

    IF stale_count > 0 THEN
        RETURN QUERY SELECT
            'STALE_METADATA'::TEXT,
            format('发现 % 个过时元数据表', stale_count)::TEXT,
            'WARNING'::TEXT;
    END IF;

    -- 检查不一致元数据
    SELECT COUNT(*) INTO inconsistent_count
    FROM validate_metadata()
    WHERE status != '一致';

    IF inconsistent_count > 0 THEN
        RETURN QUERY SELECT
            'INCONSISTENT_METADATA'::TEXT,
            format('发现 % 个不一致元数据表', inconsistent_count)::TEXT,
            'CRITICAL'::TEXT;
    END IF;

    RETURN;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '元数据告警检查失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- 查询告警
SELECT * FROM check_metadata_alerts();
```

---

## 📚 相关文档

- [数据湖完整指南.md](./数据湖完整指南.md) - 数据湖完整指南
- [数据湖架构设计.md](./数据湖架构设计.md) - 数据湖架构设计
- [26-数据管理/README.md](../README.md) - 数据管理主题

---

**最后更新**: 2025年1月
