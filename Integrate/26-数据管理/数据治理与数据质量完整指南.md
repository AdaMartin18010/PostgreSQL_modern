...
> **📋 文档来源**: 网络对标新增文档
> **📅 创建日期**: 2025-01
> **⚠️ 注意**: 本文档聚焦数据治理与数据质量完整指南，对标网络最新实践
...
- [PostgreSQL数据治理与数据质量完整指南](#postgresql数据治理与数据质量完整指南)
  ...
  - [4. 数据血缘分析](#4-数据血缘分析)
    ...
    - [4.2 血缘分析实现](#42-血缘分析实现)
  - [5. 数据目录管理](#5-数据目录管理)
    - [5.1 元数据管理](#51-元数据管理)
    - [5.2 数据目录实现](#52-数据目录实现)
  - [6. 数据合规性管理](#6-数据合规性管理)
...
```mermaid
flowchart TD
    A[开始：评估数据治理需求] --> B{数据敏感度?}
    B -->|高敏感数据| C[完整数据治理框架]
    B -->|中敏感数据| D[核心数据治理]
    B -->|低敏感数据| E[基础数据治理]

    C --> C1[数据质量 + 数据安全 + 数据合规 + 数据血缘]
    D --> D1[数据质量 + 数据安全]
    E --> E1[数据质量]
...
- [数据治理模型](../26-数据管理/12.09-数据库数据治理模型-治理策略与合规性检查的形式化.md)
