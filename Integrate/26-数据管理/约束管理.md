...
flowchart TD
    ...
    G --> H{通过?}
    H -->|否| D
    H -->|是| I[检查PRIMARY KEY约束]
    I --> J{通过?}
    J -->|否| D
    J -->|是| K[检查FOREIGN KEY约束]
    K --> L{通过?}
    L -->|否| D
    L -->|是| M[执行数据修改]
    M --> N[提交事务]
...
flowchart TD
    ...
    F -->|RESTRICT| G[阻止操作]
    F -->|CASCADE| H[级联更新/删除]
    F -->|SET NULL| I[设置外键为NULL]
    F -->|SET DEFAULT| J[设置外键为默认值]
    G --> K[抛出错误]
    H --> L[执行级联操作]
    I --> L
    J --> L
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '表 users 创建成功（带主键约束）';
    EXCEPTION
...
BEGIN
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            RAISE WARNING '添加主键约束失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '主键约束删除成功';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE NOTICE '主键约束不存在';
        WHEN OTHERS THEN
            RAISE WARNING '删除主键约束失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables
                       WHERE table_schema = 'public' AND table_name = 'users') THEN
            RAISE WARNING '表 users 不存在，无法创建外键约束';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '表 orders 创建成功（带外键约束）';
    EXCEPTION
        WHEN duplicate_table THEN
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.table_constraints
                   ...
                   AND constraint_name = 'fk_user') THEN
            RAISE NOTICE '外键约束 fk_user 已存在';
        ELSE
            ALTER TABLE orders
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '外键约束删除成功';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE NOTICE '外键约束不存在';
        WHEN OTHERS THEN
            RAISE WARNING '删除外键约束失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
-- 2. 分析插入性能
EXPLAIN (ANALYZE, BUFFERS, TIMING) INSERT INTO orders (user_id, total_amount) VALUES (1, 100.00);
```
...
-- 2. 分析插入性能
EXPLAIN (ANALYZE, BUFFERS, TIMING) INSERT INTO products (name, price) VALUES ('Product', 100.00);
```
