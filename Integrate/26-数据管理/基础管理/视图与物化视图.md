...
INSERT INTO orders (user_id, total_amount, order_date, status) VALUES
    ((SELECT id FROM users WHERE email = 'alice@example.com'), 299.99, '2024-01-15', 'completed'),
    ((SELECT id FROM users WHERE email = 'alice@example.com'), 199.99, '2024-02-20', 'pending'),
    ((SELECT id FROM users WHERE email = 'bob@example.com'), 399.99, '2024-01-18', 'completed'),
    ((SELECT id FROM users WHERE email = 'diana@example.com'), 149.99, '2024-03-10', 'completed')
...
```sql
-- 1. 创建可更新视图（基于单表，或满足可更新条件的JOIN）
CREATE OR REPLACE VIEW user_orders AS
SELECT u.id AS user_id, u.name, u.email, o.id AS order_id, o.total_amount, o.order_date, o.status
FROM users u
JOIN orders o ON u.id = o.user_id;

-- 2. 查询视图数据
SELECT * FROM user_orders ORDER BY user_id, order_id;

-- 3. 更新视图（会更新底层表users）
-- 注意：只能更新来自单表的列，不能更新JOIN后的列
UPDATE user_orders
