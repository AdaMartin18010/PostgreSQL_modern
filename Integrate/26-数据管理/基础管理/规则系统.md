...
- [PostgreSQL 规则系统](#postgresql-规则系统)
  ...
  - [6. 常见问题（FAQ）](#6-常见问题faq)
    - [6.1 规则基础常见问题](#61-规则基础常见问题)
      - [Q1: 规则和触发器有什么区别？](#q1-规则和触发器有什么区别)
      - [Q2: 如何优化规则性能？](#q2-如何优化规则性能)
    - [6.2 规则管理常见问题](#62-规则管理常见问题)
      - [Q3: 规则执行顺序是什么？](#q3-规则执行顺序是什么)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 推荐做法](#71-推荐做法)
      - [✅ 规则使用建议](#-规则使用建议)
      - [✅ 规则管理建议](#-规则管理建议)
...
flowchart TD
    A[查询语句] --> B[解析查询类型]
    B --> C{查询类型}
    C -->|SELECT| D[匹配SELECT规则]
    C -->|INSERT| E[匹配INSERT规则]
    C -->|UPDATE| F[匹配UPDATE规则]
    C -->|DELETE| G[匹配DELETE规则]
    D --> H{找到规则?}
    E --> H
    F --> H
    G --> H
...
BEGIN
    BEGIN
        CREATE RULE IF NOT EXISTS rule_name AS
            ON SELECT TO table_name
            DO INSTEAD
                SELECT * FROM another_table;
        RAISE NOTICE 'SELECT 规则创建成功';
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '规则 rule_name 已存在';
        WHEN undefined_table THEN
            RAISE WARNING '表 table_name 或 another_table 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建 SELECT 规则失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        CREATE RULE IF NOT EXISTS insert_rule_name AS
            ON INSERT TO table_name
            DO INSTEAD
                INSERT INTO another_table VALUES (NEW.*);
        RAISE NOTICE 'INSERT 规则创建成功';
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '规则 insert_rule_name 已存在';
        WHEN undefined_table THEN
            RAISE WARNING '表 table_name 或 another_table 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建 INSERT 规则失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        CREATE RULE IF NOT EXISTS update_rule_name AS
            ON UPDATE TO table_name
            DO INSTEAD
                UPDATE another_table SET * = NEW.* WHERE id = OLD.id;
        RAISE NOTICE 'UPDATE 规则创建成功';
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '规则 update_rule_name 已存在';
        WHEN undefined_table THEN
            RAISE WARNING '表 table_name 或 another_table 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建 UPDATE 规则失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        CREATE RULE IF NOT EXISTS delete_rule_name AS
            ON DELETE TO table_name
            DO INSTEAD
                DELETE FROM another_table WHERE id = OLD.id;
        RAISE NOTICE 'DELETE 规则创建成功';
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '规则 delete_rule_name 已存在';
        WHEN undefined_table THEN
            RAISE WARNING '表 table_name 或 another_table 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建 DELETE 规则失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        CREATE OR REPLACE VIEW user_view AS
            ...
            FROM users
            WHERE status = 'active';
        RAISE NOTICE '视图 user_view 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        CREATE OR REPLACE RULE insert_user AS
    ...
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '视图 user_view 或表 users 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建 INSERT 规则失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    BEGIN
        CREATE OR REPLACE RULE update_user AS
...
-- 2. 分析规则查询计划
EXPLAIN (ANALYZE, BUFFERS, TIMING) SELECT * FROM my_view;
```
