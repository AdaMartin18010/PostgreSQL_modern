...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '序列 user_id_seq 创建成功';
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '序列 user_id_seq 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '表 users_simple 创建成功（使用SERIAL）';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 users_simple 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    ...
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '序列 user_id_seq 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '获取序列值失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        curr_val := currval('user_id_seq');
        RAISE NOTICE '当前序列值: %', curr_val;
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '序列 user_id_seq 不存在或未在当前会话中使用';
        WHEN OTHERS THEN
            RAISE WARNING '获取当前序列值失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT currval('user_id_seq') AS current_value;
...
BEGIN
    BEGIN
        set_val := setval('user_id_seq', 100);
        RAISE NOTICE '序列值已设置为: %', set_val;
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '序列 user_id_seq 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '设置序列值失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        reset_val := setval('user_id_seq', 1, false);
        RAISE NOTICE '序列已重置为: %', reset_val;
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '序列 user_id_seq 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '重置序列失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '公共模式中共有 % 个序列', seq_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询序列信息失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '序列 user_id_seq 删除成功';
    EXCEPTION
        WHEN dependent_objects_still_exist THEN
            RAISE WARNING '序列 user_id_seq 仍被其他对象引用，无法删除';
        WHEN OTHERS THEN
            RAISE WARNING '删除序列失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
