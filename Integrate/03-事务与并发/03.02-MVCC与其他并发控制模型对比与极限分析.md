...
> **📋 文档来源**: `DataBaseTheory\03-事务与并发控制\03.02-MVCC与其他并发控制模型对比与极限分析.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法插入数据';
            RETURN;
        END IF;
...
-- PostgreSQL 18 MVCC：读操作不阻塞（带错误处理和性能测试）
-- 事务1：长时间运行的读事务
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行读操作';
            RETURN;
        END IF;
        RAISE NOTICE '开始PostgreSQL 18 MVCC读操作测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '读操作准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 事务2：同时进行写操作（不阻塞，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行写操作';
            RETURN;
        END IF;
        RAISE NOTICE '开始写操作（不阻塞读）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '写操作准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 事务1继续读取（仍看到旧版本，带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM accounts WHERE id = 1;
-- 结果：balance = 1000.00（事务1的快照数据）

-- 事务1提交后，新事务看到新版本（带性能测试）
COMMIT;
BEGIN;
...
-- 查看版本信息（PostgreSQL 18，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法查看版本信息';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看版本信息';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看版本信息准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- PostgreSQL 18新特性：查询版本链信息（带错误处理）
DO $$
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法查询版本链信息';
            RETURN;
        END IF;
    EXCEPTION
...
```sql
-- 查看MVCC相关统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法查看MVCC统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看MVCC相关统计';
    EXCEPTION
        WHEN OTHERS THEN
...
-- PostgreSQL 18：监控版本链长度（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_name') THEN
            CREATE INDEX idx_products_name ON products(name);
            RAISE NOTICE '索引 idx_products_name 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_products_name 已存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_price') THEN
...
-- 高并发读场景（PostgreSQL 18 MVCC优势，带错误处理和性能测试）
-- 1000个并发读事务，互不阻塞
-- 每个事务看到一致的快照
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法执行高并发读';
            RETURN;
        END IF;
...
-- 写操作（创建新版本，不影响读，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法执行写操作';
            RETURN;
        END IF;
        RAISE NOTICE '开始写操作（创建新版本，不影响读）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '写操作准备失败: %', SQLERRM;
            RAISE;
...
- 📖 [MVCC与其他并发控制模型对比](./03.02-MVCC与其他并发控制模型对比与极限分析.md) - 并发控制理论的统一框架，包括MVCC、2PL、OCC、TO等模型的对比
- 📖 [MVCC高级分析与形式证明](./03.01-MVCC高级分析与形式证明.md) - PostgreSQL MVCC的完整数学证明和理论解析

**设计权衡分析**:

- 📖 [事务与并发优化场景分析指南](./事务与并发优化场景分析指南.md) - 并发控制机制选择的系统化决策流程
- 📖 [事务与并发README](./README.md) - 事务与并发相关内容

**可视化与思维模型**:

- 📖 [事务与并发README](./README.md) - 事务与并发相关内容
- 📖 [理论体系](../00-归档-项目管理文档/README.md) - 理论体系相关内容
