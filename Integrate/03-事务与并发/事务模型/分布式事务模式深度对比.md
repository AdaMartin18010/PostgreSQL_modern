...
> **📋 文档来源**: `MVCC-ACID-CAP\01-理论基础\事务模型\分布式事务模式深度对比.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE EXCEPTION '表 accounts 不存在，无法执行2PC事务';
        END IF;

        BEGIN;
        UPDATE accounts SET balance = balance - 100 WHERE id = 1;
        PREPARE TRANSACTION 'txn_001';  -- 准备事务
        RAISE NOTICE '事务 txn_001 准备成功';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        -- 检查准备好的事务是否存在（带性能测试说明）
        -- 注意：这是检查性查询，通常不需要性能测试
        IF EXISTS (SELECT 1 FROM pg_prepared_xacts WHERE transaction = 'txn_001') THEN
            COMMIT PREPARED 'txn_001';  -- 提交准备好的事务
            RAISE NOTICE '事务 txn_001 提交成功';
        ELSE
            RAISE WARNING '准备好的事务 txn_001 不存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            ...
            BEGIN
                ROLLBACK PREPARED 'txn_001';
                RAISE NOTICE '事务 txn_001 已回滚';
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '回滚事务失败: %', SQLERRM;
            END;
    END;
END $$;
...
```sql
-- XA事务示例（带错误处理）
-- 注意：PostgreSQL原生不支持XA命令，这里提供概念性示例
-- 实际实现需要使用外部事务管理器（如PostgreSQL的pg_xact扩展或应用层实现）

DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE EXCEPTION '表 accounts 不存在，无法执行XA事务';
        END IF;

        -- XA事务概念性实现（实际需要使用外部事务管理器）
        ...
        -- XA COMMIT 'xid_001';
        COMMIT;
        RAISE NOTICE 'XA事务 xid_001 提交成功';
    EXCEPTION
        WHEN OTHERS THEN
            -- 回滚XA事务（概念性）
            -- XA ROLLBACK 'xid_001';
            ROLLBACK;
            RAISE WARNING 'XA事务 xid_001 回滚: %', SQLERRM;
    END;
END $$;
