...
> **📋 文档来源**: `MVCC-ACID-CAP\25-理论体系\事务模型\分布式事务最佳实践.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法扣减库存';
            RETURN;
        END IF;
        ...
        IF NOT FOUND THEN
            ROLLBACK;
            RAISE WARNING '产品ID 1 不存在，事务回滚';
            RETURN;
        END IF;

        COMMIT;
        RAISE NOTICE '步骤2：库存扣减成功';
    EXCEPTION
        WHEN undefined_table THEN
        ...
        WHEN check_violation THEN
            RAISE WARNING '库存不足，事务回滚';
            IF FOUND THEN
                ROLLBACK;
            END IF;
        WHEN OTHERS THEN
            RAISE WARNING '扣减库存失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法扣减账户';
            RETURN;
        END IF;
        ...
        IF NOT FOUND THEN
            ROLLBACK;
            RAISE WARNING '账户ID 100 不存在，事务回滚';
            RETURN;
        END IF;

        COMMIT;
        RAISE NOTICE '步骤3：账户扣减成功';
    EXCEPTION
        WHEN undefined_table THEN
            ...
            END IF;
        WHEN check_violation THEN
            RAISE WARNING '余额不足，事务回滚';
            IF FOUND THEN
                ROLLBACK;
            END IF;
        WHEN OTHERS THEN
            RAISE WARNING '扣减账户失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
DO $$
DECLARE
    v_account_id INT := 100;
    v_amount NUMERIC := 1000;
    v_updated_rows INT;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行TCC Try阶段';
            RETURN;
        END IF;

        BEGIN;
        UPDATE accounts SET frozen_balance = frozen_balance + v_amount
        WHERE id = v_account_id AND balance >= v_amount;

        GET DIAGNOSTICS v_updated_rows = ROW_COUNT;

        IF v_updated_rows = 0 THEN
            ROLLBACK;
            RAISE WARNING 'Try阶段失败：账户不存在或余额不足';
            RETURN;
        END IF;

        COMMIT;
        RAISE NOTICE 'Try阶段成功：资源已预留';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
            IF FOUND THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行TCC Confirm阶段';
            RETURN;
        END IF;
        ...
        IF NOT FOUND THEN
            ROLLBACK;
            RAISE WARNING 'Confirm阶段失败：账户不存在';
            RETURN;
        END IF;

        COMMIT;
        RAISE NOTICE 'Confirm阶段成功：转账已确认';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
            IF FOUND THEN
            ...
            END IF;
        WHEN OTHERS THEN
            RAISE WARNING 'Confirm阶段失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行TCC Cancel阶段';
            RETURN;
        END IF;
...
2. **TCC模式**：
   - 阿里巴巴 (2011). "TCC分布式事务模式"

3. **事件溯源**：
   - Fowler, M. (2005). "Event Sourcing"
   - Greg Young (2010). "Event Sourcing"
...
2. **微服务框架文档**：
   ...
   - [Seata Documentation](https://seata.io/)
