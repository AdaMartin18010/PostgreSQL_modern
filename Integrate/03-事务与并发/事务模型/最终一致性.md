...
> **📋 文档来源**: `MVCC-ACID-CAP\01-理论基础\事务模型\最终一致性.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = 'replica_slot') THEN
            ...
            RAISE NOTICE '物理复制槽 replica_slot 创建成功';
        ELSE
            RAISE NOTICE '物理复制槽 replica_slot 已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建物理复制槽失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'my_publication') THEN
            RAISE EXCEPTION '发布 my_publication 不存在，无法创建订阅';
        END IF;
...
BEGIN
    -- 检查NEW记录
    IF NEW IS NULL THEN
        RAISE WARNING 'NEW记录为空，无法发送账户变更通知';
        RETURN NULL;
    END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取账户信息失败: %', SQLERRM;
            RETURN COALESCE(NEW, OLD);
    END;

    -- 验证必需字段
    IF v_account_id IS NULL THEN
        RAISE WARNING '账户ID为空，跳过通知';
        RETURN COALESCE(NEW, OLD);
    END IF;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '转换payload为文本失败: %', SQLERRM;
                RETURN COALESCE(NEW, OLD);
        END;

        -- 发送通知
        BEGIN
            PERFORM pg_notify('account_change', v_payload_text);
        EXCEPTION
            WHEN OTHERS THEN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '构建或发送通知失败: %', SQLERRM;
    END;

    RETURN COALESCE(NEW, OLD);
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'notify_account_change触发器函数执行失败: %', SQLERRM;
        RETURN COALESCE(NEW, OLD);  -- 即使出错也返回，避免阻塞主操作
END;
$$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = 'events_slot') THEN
            PERFORM pg_create_logical_replication_slot('events_slot', 'pgoutput');
            RAISE NOTICE '逻辑复制槽 events_slot 创建成功';
        ELSE
            RAISE NOTICE '逻辑复制槽 events_slot 已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建逻辑复制槽失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_replication_slots WHERE slot_name = 'events_slot') THEN
            RAISE WARNING '逻辑复制槽 events_slot 不存在，无法读取变更';
            RETURN;
        END IF;
        RAISE NOTICE '开始读取逻辑复制槽变更';
    EXCEPTION
        WHEN OTHERS THEN
