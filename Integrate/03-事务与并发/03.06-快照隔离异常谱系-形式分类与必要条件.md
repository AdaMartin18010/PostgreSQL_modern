...
> **📋 文档来源**: `DataBaseTheory\03-事务与并发控制\03.06-快照隔离异常谱系-形式分类与必要条件.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'mydb') THEN
            RAISE WARNING '数据库 mydb 不存在，无法设置默认隔离级别';
        ELSE
            BEGIN
                ALTER DATABASE mydb SET default_transaction_isolation = 'serializable';
                RAISE NOTICE '数据库 mydb 的默认隔离级别已设置为可串行化';
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '设置数据库默认隔离级别失败: %', SQLERRM;
                    RAISE;
            END;
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '会话级别的默认隔离级别已设置为可串行化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置会话默认隔离级别失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 检查SSI相关配置（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始检查SSI相关配置';
    EXCEPTION
        WHEN OTHERS THEN
...
-- PostgreSQL 18新特性：查看SSI详细统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看SSI详细统计';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        ELSE
            ...
            RAISE NOTICE '表 accounts 创建成功（账户余额系统）';
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        ...
        -- 检查是否已有数据
        SELECT COUNT(*) INTO v_count FROM accounts WHERE account_name IN ('Account A', 'Account B');

        IF v_count >= 2 THEN
            RAISE NOTICE '测试数据已存在（Account A 和 Account B）';
        ELSE
            BEGIN
                ...
                RAISE NOTICE '测试数据插入成功: 插入了 % 条记录', v_count;
            EXCEPTION
                WHEN unique_violation THEN
                    RAISE WARNING '部分测试数据已存在';
                WHEN check_violation THEN
                    RAISE WARNING '余额约束违反（余额不能小于最小余额）';
                    RAISE;
                WHEN OTHERS THEN
...
```sql
-- 查看SSI依赖信息（PostgreSQL 18，带错误处理和性能测试）
-- 需要启用相关扩展或使用系统视图
...
-- 查看当前SSI事务状态（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看当前SSI事务状态';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
-- PostgreSQL 18：查看SSI中止的事务历史（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看SSI中止的事务历史';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
