...
```sql
-- PostgreSQL 18：查看锁类型（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看锁类型';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- PostgreSQL 18：显式行级锁（FOR UPDATE）（带错误处理）
DO $$
BEGIN
    BEGIN
        ...
        BEGIN;
        RAISE NOTICE '开始执行FOR UPDATE（严格2PL）';
    EXCEPTION
        WHEN OTHERS THEN
...
-- PostgreSQL 18：显式行级锁（FOR SHARE）（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行FOR SHARE';
            RETURN;
        END IF;
...
-- PostgreSQL 18：显式表级锁（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行表级锁';
            RETURN;
        END IF;
...
```sql
-- PostgreSQL 18：查看当前锁（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
...
-- PostgreSQL 18：查看锁等待（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看锁等待';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
```sql
-- PostgreSQL 18：锁超时配置（带错误处理）
DO $$
BEGIN
    BEGIN
        ALTER SYSTEM SET lock_timeout = '5s';
        -- 锁等待超时时间
        RAISE NOTICE '锁超时配置已设置';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE WARNING '权限不足，无法设置系统参数';
        WHEN OTHERS THEN
            RAISE WARNING '锁超时配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- PostgreSQL 18：死锁检测配置（带错误处理）
DO $$
BEGIN
    BEGIN
        ALTER SYSTEM SET deadlock_timeout = '1s';
        -- 死锁检测间隔
        RAISE NOTICE '死锁检测配置已设置';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE WARNING '权限不足，无法设置系统参数';
        WHEN OTHERS THEN
            RAISE WARNING '死锁检测配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- PostgreSQL 18：查看锁配置（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看锁配置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
```sql
-- SQLite 3.45：文件级锁（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行文件级锁';
            RETURN;
        END IF;
    ...
    EXCEPTION
        WHEN undefined_table THEN
            ...
            END IF;
        WHEN OTHERS THEN
            RAISE WARNING 'SQLite文件级锁操作失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
-- SQLite 3.45：WAL模式（提高并发）（带错误处理）
DO $$
BEGIN
    BEGIN
        PRAGMA journal_mode = WAL;
        -- WAL模式允许多个读和一个写并发
        RAISE NOTICE 'SQLite WAL模式已设置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'SQLite WAL模式设置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    ...
    BEGIN
        ...
        IF NOT FOUND THEN
            ROLLBACK;
            RAISE EXCEPTION '源账户不存在: %', from_account_id;
        END IF;

        -- 检查余额
        IF from_balance < amount THEN
            ROLLBACK;
            RAISE EXCEPTION '余额不足: 当前余额 %, 转账金额 %', from_balance, amount;
        END IF;
        ...
        IF NOT FOUND THEN
            ROLLBACK;
            RAISE EXCEPTION '目标账户不存在: %', to_account_id;
        END IF;
...
BEGIN
    BEGIN
        PERFORM transfer_funds(1, 2, 100.00);
        RAISE NOTICE '转账测试执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '转账测试失败: %', SQLERRM;
    END;
END $$;
...
- 📖 [公理系统](../00-归档-项目管理文档/README.md) - 三大公理的严格数学证明
...
- 📖 [形式化证明](../00-归档-项目管理文档/README.md) - 形式化证明相关文档
- 📖 [事务模型](../00-归档-项目管理文档/README.md) - 事务模型相关文档
