...
```sql
-- PostgreSQL 18：行级锁（细粒度，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法测试行级锁';
            RETURN;
        END IF;

        BEGIN;
        PERFORM * FROM accounts WHERE id = 1 FOR UPDATE;
        -- 获取行级排他锁
        COMMIT;
        RAISE NOTICE '行级锁测试成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '行级锁测试失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
-- PostgreSQL 18：表级锁（粗粒度，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法测试表级锁';
            RETURN;
        END IF;
        ...
        COMMIT;
        RAISE NOTICE '表级锁测试成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '表级锁测试失败: %', SQLERRM;
            IF FOUND THEN
                ROLLBACK;
            END IF;
    END;
...
-- PostgreSQL 18：查看当前锁（带性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查询当前锁状态';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        BEGIN
            ...
            -- 检查表是否存在
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = upgrade_to_table_lock.table_name) THEN
            ...
            BEGIN
                ...
                WHERE pid = pg_backend_pid()
                  AND locktype = 'tuple'
                  AND relation = upgrade_to_table_lock.table_name::regclass;
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '检查行锁数量失败: %', SQLERRM;
                    lock_count := 0;
            END;
            ...
            IF lock_count > 100 THEN
                BEGIN
                    ...
                    RAISE NOTICE '锁已升级为表锁（行锁数量: %）', lock_count;
                EXCEPTION
                    WHEN OTHERS THEN
                        RAISE EXCEPTION '锁升级失败: %', SQLERRM;
                END;
            ELSE
                RAISE NOTICE '行锁数量（%）未超过阈值，无需升级', lock_count;
            END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '商品表 products 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        BEGIN
            -- 参数验证
            IF price_increase_rate IS NULL THEN
                RAISE EXCEPTION 'price_increase_rate不能为NULL';
            END IF;

            IF price_increase_rate < 0 THEN
                RAISE EXCEPTION 'price_increase_rate不能为负数';
            END IF;
