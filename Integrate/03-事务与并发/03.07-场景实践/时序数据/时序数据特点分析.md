...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
            RAISE WARNING '表 metrics 不存在，无法检查HOT更新率';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        ...
        ELSE
            ...
            RAISE NOTICE '表 metrics 创建成功（按时间范围分区）';
        END IF;
    EXCEPTION
    ...
    BEGIN
        ...
        ELSE
            BEGIN
                ...
                RAISE NOTICE '时序数据表 metrics 创建成功（列顺序优化）';
            EXCEPTION
                WHEN duplicate_table THEN
                    RAISE WARNING '表 metrics 已存在';
                WHEN OTHERS THEN
                    RAISE WARNING '创建时序数据表失败: %', SQLERRM;
                    RAISE;
            END;
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
            RAISE WARNING '表 metrics 不存在，无法创建时间戳索引';
            RETURN;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'metrics' AND indexname = 'idx_metrics_time') THEN
            ...
            RAISE NOTICE '时间戳索引 idx_metrics_time 创建成功';
        ELSE
            RAISE NOTICE '时间戳索引 idx_metrics_time 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 metrics 不存在';
        ...
        WHEN OTHERS THEN
            RAISE WARNING '创建时间戳索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
