...
BEGIN
    BEGIN
        ...
        BEGIN
            PERFORM cron.schedule('archive-old-data', '0 2 * * *', 'SELECT auto_archive();');
            RAISE NOTICE '定时任务 archive-old-data 创建成功（每天凌晨2点执行）';
        EXCEPTION
            WHEN duplicate_object THEN
                RAISE WARNING '定时任务 archive-old-data 已存在';
            WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'archive_progress') THEN
            RAISE NOTICE '表 archive_progress 已存在';
        ELSE
            BEGIN
                ...
                RAISE NOTICE '表 archive_progress 创建成功（归档进度表）';
            EXCEPTION
                WHEN duplicate_table THEN
                    RAISE WARNING '表 archive_progress 已存在';
                WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
            RAISE WARNING '表 metrics 不存在，无法恢复分区数据';
            RETURN;
        END IF;

        BEGIN
            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics_2024_01_restored') THEN
                RAISE WARNING '表 metrics_2024_01_restored 已存在，跳过创建';
            ELSE
                CREATE TABLE metrics_2024_01_restored (
                    LIKE metrics INCLUDING ALL
                );
                RAISE NOTICE '表 metrics_2024_01_restored 创建成功';
            END IF;
        EXCEPTION
            WHEN duplicate_table THEN
                RAISE WARNING '表 metrics_2024_01_restored 已存在';
            WHEN OTHERS THEN
                RAISE WARNING '创建表失败: %', SQLERRM;
                RAISE;
        END;
        ...
        BEGIN
            ...
            RAISE NOTICE '数据从归档文件恢复成功';
        EXCEPTION
            WHEN undefined_file THEN
                RAISE WARNING '归档文件 /archive/metrics_2024_01.csv 不存在';
            WHEN OTHERS THEN
                RAISE WARNING 'COPY数据失败: %', SQLERRM;
                RAISE;
        END;
        ...
        BEGIN
            ...
            RAISE NOTICE '分区 metrics_2024_01_restored 已附加到 metrics';
        EXCEPTION
            WHEN duplicate_table THEN
                RAISE WARNING '分区 metrics_2024_01_restored 已附加';
            WHEN OTHERS THEN
                RAISE WARNING '附加分区失败: %', SQLERRM;
                RAISE;
        END;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
            RAISE WARNING '表 metrics 不存在，无法创建临时表';
            RETURN;
        END IF;
        ...
        BEGIN
            ...
            RAISE NOTICE '临时表 metrics_temp 创建成功';
        EXCEPTION
            WHEN duplicate_table THEN
                RAISE WARNING '临时表 metrics_temp 已存在';
            WHEN OTHERS THEN
...
BEGIN
    BEGIN
        BEGIN
            ...
            RAISE NOTICE '部分数据恢复成功（2024-01-01 至 2024-01-15）';
        EXCEPTION
            WHEN undefined_file THEN
                RAISE WARNING '归档文件 /archive/metrics_2024_01.csv 不存在';
            WHEN OTHERS THEN
                RAISE WARNING 'COPY部分数据失败: %', SQLERRM;
                RAISE;
        END;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics_archive_2024_01') THEN
            RAISE WARNING '外部表 metrics_archive_2024_01 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始外部表查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
            RAISE WARNING '表 metrics 不存在，无法执行联合查询';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics_archive_2024_01') THEN
            RAISE WARNING '外部表 metrics_archive_2024_01 不存在，无法执行联合查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始联合查询';
    EXCEPTION
