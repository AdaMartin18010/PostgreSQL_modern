---

> **📋 文档来源**: `MVCC-ACID-CAP\03-场景实践\电商系统\库存扣减完整案例.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 电商系统库存扣减完整案例

> **文档编号**: SCENARIO-ECOMMERCE-001
> **主题**: 电商系统库存扣减
> **版本**: PostgreSQL 17 & 18

---

## 📑 目录

- [电商系统库存扣减完整案例](#电商系统库存扣减完整案例)
  - [📑 目录](#-目录)
  - [📋 概述](#-概述)
  - [🔍 第一部分：业务场景分析](#-第一部分业务场景分析)
    - [1.1 业务需求](#11-业务需求)
      - [核心需求](#核心需求)
      - [性能要求](#性能要求)
      - [一致性要求](#一致性要求)
    - [1.2 并发场景](#12-并发场景)
      - [高并发下单](#高并发下单)
      - [库存超卖问题](#库存超卖问题)
      - [死锁风险](#死锁风险)
  - [🚀 第二部分：数据库设计](#-第二部分数据库设计)
    - [2.1 表结构设计](#21-表结构设计)
      - [商品表设计](#商品表设计)
      - [库存表设计](#库存表设计)
      - [订单表设计](#订单表设计)
    - [2.2 索引设计](#22-索引设计)
      - [主键索引](#主键索引)
      - [唯一索引](#唯一索引)
      - [查询索引](#查询索引)
    - [2.3 MVCC优化设计](#23-mvcc优化设计)
      - [fillfactor设置](#fillfactor设置)
      - [表级VACUUM配置](#表级vacuum配置)
  - [📊 第三部分：事务实现方案](#-第三部分事务实现方案)
    - [3.1 方案1：SELECT FOR UPDATE（悲观锁）](#31-方案1select-for-update悲观锁)
      - [实现代码](#实现代码)
      - [性能分析](#性能分析)
      - [适用场景](#适用场景)
    - [3.2 方案2：乐观锁（版本号）](#32-方案2乐观锁版本号)
      - [实现代码](#实现代码-1)
      - [性能分析](#性能分析-1)
      - [适用场景](#适用场景-1)
    - [3.3 方案3：原子操作（UPDATE WHERE）](#33-方案3原子操作update-where)
      - [实现代码](#实现代码-2)
      - [性能分析](#性能分析-2)
      - [适用场景](#适用场景-2)
  - [🔧 第四部分：多语言实现](#-第四部分多语言实现)
    - [4.1 Python实现](#41-python实现)
      - [psycopg2实现](#psycopg2实现)
      - [asyncpg实现](#asyncpg实现)
    - [4.2 Java实现](#42-java实现)
      - [JDBC实现](#jdbc实现)
      - [Spring实现](#spring实现)
    - [4.3 Go实现](#43-go实现)
      - [pgx实现](#pgx实现)
  - [📈 第五部分：性能优化](#-第五部分性能优化)
    - [5.1 连接池优化](#51-连接池优化)
    - [5.2 批量操作优化](#52-批量操作优化)
    - [5.3 查询优化](#53-查询优化)
  - [🎯 第六部分：监控和告警](#-第六部分监控和告警)
    - [6.1 关键指标监控](#61-关键指标监控)
    - [6.2 告警规则](#62-告警规则)
    - [6.3 故障处理](#63-故障处理)
  - [📝 总结](#-总结)
    - [核心方案](#核心方案)
    - [最佳实践](#最佳实践)
    - [性能指标](#性能指标)

---

## 📋 概述

电商系统库存扣减是典型的并发控制场景，需要在高并发环境下保证数据一致性和性能。
本文档提供完整的解决方案，涵盖数据库设计、事务实现、多语言实现、性能优化和监控告警。

---

## 🔍 第一部分：业务场景分析

### 1.1 业务需求

#### 核心需求

```sql
-- 业务场景：用户下单时扣减库存
-- 要求：
-- 1. 防止超卖（库存不能为负）
-- 2. 高并发支持（1000+ QPS）
-- 3. 数据一致性（ACID保证）
-- 4. 低延迟（<100ms）
```

#### 性能要求

- **QPS**: 1000+ 并发下单
- **延迟**: P99 < 100ms
- **可用性**: 99.9%
- **数据一致性**: 100%（不允许超卖）

#### 一致性要求

- **原子性**: 库存扣减和订单创建必须同时成功或失败
- **一致性**: 库存数量必须准确
- **隔离性**: 并发下单不能相互干扰
- **持久性**: 扣减结果必须持久化

### 1.2 并发场景

#### 高并发下单

```sql
-- 场景：1000个用户同时下单同一商品
-- 商品库存：100件
-- 问题：如何保证只有100个订单成功？
```

#### 库存超卖问题

```sql
-- 问题场景：
-- 时间T1: 用户A查询库存=100
-- 时间T2: 用户B查询库存=100
-- 时间T3: 用户A扣减1，库存=99
-- 时间T4: 用户B扣减1，库存=99
-- 结果：实际卖出2件，但库存只减少1（超卖）
```

#### 死锁风险

```sql
-- 死锁场景：
-- 事务1: 扣减商品A库存，然后扣减商品B库存
-- 事务2: 扣减商品B库存，然后扣减商品A库存
-- 结果：可能发生死锁
```

---

## 🚀 第二部分：数据库设计

### 2.1 表结构设计

#### 商品表设计

```sql
-- 商品表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE NOTICE '表 products 已存在';
        ELSE
            CREATE TABLE products (
                id BIGSERIAL PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                sku VARCHAR(100) UNIQUE NOT NULL,
                price DECIMAL(10, 2) NOT NULL,
                status VARCHAR(20) DEFAULT 'active',
                created_at TIMESTAMP DEFAULT NOW(),
                updated_at TIMESTAMP DEFAULT NOW()
            );
            RAISE NOTICE '表 products 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 products 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_sku') THEN
            CREATE INDEX idx_products_sku ON products(sku);
            RAISE NOTICE '索引 idx_products_sku 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_products_sku 已存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_status') THEN
            CREATE INDEX idx_products_status ON products(status);
            RAISE NOTICE '索引 idx_products_status 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_products_status 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 products 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '部分索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 库存表设计

```sql
-- 库存表（关键表，带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE NOTICE '表 inventory 已存在';
        ELSE
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
                RAISE WARNING '表 products 不存在，无法创建外键约束';
            END IF;

            CREATE TABLE inventory (
                product_id BIGINT PRIMARY KEY REFERENCES products(id),
                stock INT NOT NULL DEFAULT 0,
                reserved_stock INT NOT NULL DEFAULT 0,  -- 预留库存（已下单未支付）
                version INT NOT NULL DEFAULT 0,  -- 乐观锁版本号
                created_at TIMESTAMP DEFAULT NOW(),
                updated_at TIMESTAMP DEFAULT NOW(),

                -- 约束：库存不能为负
                CONSTRAINT chk_stock_non_negative CHECK (stock >= 0),
                CONSTRAINT chk_reserved_stock_non_negative CHECK (reserved_stock >= 0)
            );
            RAISE NOTICE '表 inventory 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 inventory 已存在';
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保表 products 存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'inventory' AND indexname = 'idx_inventory_stock') THEN
            CREATE INDEX idx_inventory_stock ON inventory(stock) WHERE stock > 0;
            RAISE NOTICE '索引 idx_inventory_stock 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_inventory_stock 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 inventory 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_inventory_stock 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 订单表设计

```sql
-- 订单表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE NOTICE '表 orders 已存在';
        ELSE
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
                RAISE WARNING '表 products 不存在，无法创建外键约束';
            END IF;

            CREATE TABLE orders (
                id BIGSERIAL PRIMARY KEY,
                user_id BIGINT NOT NULL,
                product_id BIGINT NOT NULL REFERENCES products(id),
                quantity INT NOT NULL,
                price DECIMAL(10, 2) NOT NULL,
                total_amount DECIMAL(10, 2) NOT NULL,
                status VARCHAR(20) DEFAULT 'pending',  -- pending, paid, cancelled
                created_at TIMESTAMP DEFAULT NOW(),
                paid_at TIMESTAMP,
                cancelled_at TIMESTAMP
            );
            RAISE NOTICE '表 orders 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 orders 已存在';
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保表 products 存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_user_id') THEN
            CREATE INDEX idx_orders_user_id ON orders(user_id);
            RAISE NOTICE '索引 idx_orders_user_id 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_product_id') THEN
            CREATE INDEX idx_orders_product_id ON orders(product_id);
            RAISE NOTICE '索引 idx_orders_product_id 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_status') THEN
            CREATE INDEX idx_orders_status ON orders(status);
            RAISE NOTICE '索引 idx_orders_status 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_created_at') THEN
            CREATE INDEX idx_orders_created_at ON orders(created_at);
            RAISE NOTICE '索引 idx_orders_created_at 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 orders 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '部分索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 2.2 索引设计

#### 主键索引

```sql
-- 主键自动创建索引
-- products.id, inventory.product_id, orders.id
```

#### 唯一索引

```sql
-- SKU唯一索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法创建唯一索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_sku_unique') THEN
            CREATE UNIQUE INDEX idx_products_sku_unique ON products(sku);
            RAISE NOTICE '唯一索引 idx_products_sku_unique 创建成功';
        ELSE
            RAISE NOTICE '唯一索引 idx_products_sku_unique 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 products 不存在';
        WHEN unique_violation THEN
            RAISE WARNING '唯一索引创建失败：存在重复的SKU值';
        WHEN duplicate_table THEN
            RAISE WARNING '唯一索引 idx_products_sku_unique 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建唯一索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 查询索引

```sql
-- 常用查询索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法创建索引';
        ELSE
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'inventory' AND indexname = 'idx_inventory_stock_low') THEN
                CREATE INDEX idx_inventory_stock_low ON inventory(stock) WHERE stock < 10;  -- 低库存告警
                RAISE NOTICE '索引 idx_inventory_stock_low 创建成功';
            ELSE
                RAISE NOTICE '索引 idx_inventory_stock_low 已存在';
            END IF;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法创建索引';
        ELSE
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'orders' AND indexname = 'idx_orders_user_status') THEN
                CREATE INDEX idx_orders_user_status ON orders(user_id, status);  -- 用户订单查询
                RAISE NOTICE '索引 idx_orders_user_status 创建成功';
            ELSE
                RAISE NOTICE '索引 idx_orders_user_status 已存在';
            END IF;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '部分索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 2.3 MVCC优化设计

#### fillfactor设置

```sql
-- 库存表：预留更新空间（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法设置fillfactor';
        ELSE
            ALTER TABLE inventory SET (fillfactor = 80);
            RAISE NOTICE '库存表 fillfactor 设置为 80';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 inventory 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '设置fillfactor失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 订单表：只插入，不更新（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法设置fillfactor';
        ELSE
            ALTER TABLE orders SET (fillfactor = 100);
            RAISE NOTICE '订单表 fillfactor 设置为 100';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 orders 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '设置fillfactor失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 表级VACUUM配置

```sql
-- 库存表：更新频繁，需要频繁VACUUM（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法配置VACUUM';
        ELSE
            ALTER TABLE inventory SET (
                autovacuum_vacuum_scale_factor = 0.05,  -- 5%死亡元组触发
                autovacuum_analyze_scale_factor = 0.02  -- 2%变化触发ANALYZE
            );
            RAISE NOTICE '库存表 VACUUM 配置完成';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 inventory 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '配置VACUUM失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 订单表：插入为主，较少VACUUM（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法配置VACUUM';
        ELSE
            ALTER TABLE orders SET (
                autovacuum_vacuum_scale_factor = 0.1,
                autovacuum_analyze_scale_factor = 0.05
            );
            RAISE NOTICE '订单表 VACUUM 配置完成';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 orders 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '配置VACUUM失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 📊 第三部分：事务实现方案

### 3.1 方案1：SELECT FOR UPDATE（悲观锁）

#### 3.1.1 实现代码

```sql
-- 悲观锁实现：扣减库存（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 inventory 或 orders 不存在，无法执行库存扣减';
            RETURN;
        END IF;
        RAISE NOTICE '开始悲观锁实现：扣减库存';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '操作准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

BEGIN;

-- 1. 锁定库存行（带错误处理）
BEGIN
    EXPLAIN (ANALYZE, BUFFERS, TIMING)
    SELECT stock, reserved_stock
    FROM inventory
    WHERE product_id = $1
    FOR UPDATE;
EXCEPTION
    WHEN lock_not_available THEN
        RAISE WARNING '无法获取锁，行已被其他事务锁定';
        RAISE;
    WHEN no_data_found THEN
        RAISE WARNING '商品 ID % 不存在', $1;
        RAISE;
    WHEN OTHERS THEN
        RAISE WARNING '查询库存失败: %', SQLERRM;
        RAISE;
END;

-- 2. 检查库存
-- 如果 stock - reserved_stock < quantity，则回滚

-- 3. 扣减库存（带错误处理和性能测试）
BEGIN
    EXPLAIN (ANALYZE, BUFFERS, TIMING)
    UPDATE inventory
    SET stock = stock - $2,
        updated_at = NOW()
    WHERE product_id = $1
      AND stock >= $2;

    IF NOT FOUND THEN
        RAISE WARNING '库存不足或商品不存在';
        RAISE EXCEPTION 'Insufficient stock';
    END IF;
EXCEPTION
    WHEN check_violation THEN
        RAISE WARNING '库存约束违反（库存不能为负）';
        RAISE;
    WHEN OTHERS THEN
        RAISE WARNING '更新库存失败: %', SQLERRM;
        RAISE;
END;

-- 4. 创建订单（带错误处理）
BEGIN
    INSERT INTO orders (user_id, product_id, quantity, price, total_amount)
    VALUES ($3, $1, $2, $4, $4 * $2);
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE WARNING '外键约束失败，请确保商品和用户存在';
        RAISE;
    WHEN OTHERS THEN
        RAISE WARNING '创建订单失败: %', SQLERRM;
        RAISE;
END;

COMMIT;
```

#### 性能分析

```sql
-- 性能特点：
-- 优点：
-- 1. 保证强一致性
-- 2. 实现简单
-- 3. 不会超卖
--
-- 缺点：
-- 1. 锁竞争激烈（高并发下）
-- 2. 可能死锁
-- 3. 性能较低（串行化）
--
-- 性能指标：
-- QPS: 500-800（单实例）
-- P99延迟: 50-100ms
```

#### 适用场景

- 库存数量较少（<1000）
- 并发量中等（<500 QPS）
- 对一致性要求极高

### 3.2 方案2：乐观锁（版本号）

#### 3.2.1 实现代码

```sql
-- 乐观锁实现：扣减库存（带错误处理）
DO $$
DECLARE
    current_stock INT;
    current_version INT;
    update_count INT;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法执行乐观锁扣减';
            RETURN;
        END IF;

        -- 1. 读取当前库存和版本号（带错误处理）
        BEGIN
            SELECT stock, version INTO current_stock, current_version
            FROM inventory
            WHERE product_id = $1;

            IF NOT FOUND THEN
                RAISE WARNING '商品 ID % 不存在', $1;
                RAISE EXCEPTION 'Product not found';
            END IF;
        EXCEPTION
            WHEN no_data_found THEN
                RAISE WARNING '商品 ID % 不存在', $1;
                RAISE;
            WHEN OTHERS THEN
                RAISE WARNING '读取库存失败: %', SQLERRM;
                RAISE;
        END;

        -- 2. 检查库存
        IF current_stock < $2 THEN
            RAISE WARNING '库存不足：当前库存 %，需要 %', current_stock, $2;
            RAISE EXCEPTION 'Insufficient stock';
        END IF;

    -- 3. 更新库存（版本号检查）
    UPDATE inventory
    SET stock = stock - $2,
        version = version + 1,
        updated_at = NOW()
    WHERE product_id = $1
      AND version = current_version;

    GET DIAGNOSTICS update_count = ROW_COUNT;

    -- 4. 检查更新是否成功
    IF update_count = 0 THEN
        RAISE EXCEPTION 'Version mismatch, please retry';
    END IF;

    -- 5. 创建订单
    INSERT INTO orders (user_id, product_id, quantity, price, total_amount)
    VALUES ($3, $1, $2, $4, $4 * $2);
END $$;
```

#### 3.2.2 性能分析

```sql
-- 性能特点：
-- 优点：
-- 1. 高并发性能好
-- 2. 无锁竞争
-- 3. 不会死锁
--
-- 缺点：
-- 1. 需要重试机制
-- 2. 冲突时性能下降
--
-- 性能指标：
-- QPS: 2000-3000（低冲突）
-- P99延迟: 20-50ms（无冲突）
-- 重试率: 5-10%（高并发）
```

#### 3.2.3 适用场景

- 库存数量较大（>1000）
- 高并发（>1000 QPS）
- 可以接受重试

### 3.3 方案3：原子操作（UPDATE WHERE）

#### 3.3.1 实现代码

```sql
-- 原子操作实现：扣减库存（推荐，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 inventory 或 orders 不存在，无法执行原子操作';
            RETURN;
        END IF;
        RAISE NOTICE '开始原子操作实现：扣减库存';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '操作准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

BEGIN;

-- 1. 原子扣减库存（带错误处理和性能测试）
BEGIN
    DECLARE
        v_updated INTEGER;
    BEGIN
        EXPLAIN (ANALYZE, BUFFERS, TIMING)
        UPDATE inventory
        SET stock = stock - $2,
            updated_at = NOW()
        WHERE product_id = $1
          AND stock >= $2;

        GET DIAGNOSTICS v_updated = ROW_COUNT;

        IF v_updated = 0 THEN
            RAISE WARNING '库存不足或商品不存在';
            RAISE EXCEPTION 'Insufficient stock';
        END IF;
    EXCEPTION
        WHEN check_violation THEN
            RAISE WARNING '库存约束违反（库存不能为负）';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '更新库存失败: %', SQLERRM;
            RAISE;
    END;
END;

-- 2. 检查更新结果
-- 如果 ROW_COUNT = 0，说明库存不足，回滚（已在上面处理）

-- 3. 创建订单（带错误处理）
BEGIN
    INSERT INTO orders (user_id, product_id, quantity, price, total_amount)
    VALUES ($3, $1, $2, $4, $4 * $2);
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE WARNING '外键约束失败，请确保商品和用户存在';
        RAISE;
    WHEN OTHERS THEN
        RAISE WARNING '创建订单失败: %', SQLERRM;
        RAISE;
END;

COMMIT;
```

#### 性能分析

```sql
-- 性能特点：
-- 优点：
-- 1. 性能最优
-- 2. 实现最简单
-- 3. 无锁竞争
-- 4. 不会死锁
--
-- 缺点：
-- 1. 需要检查更新结果
--
-- 性能指标：
-- QPS: 3000-5000（单实例）
-- P99延迟: 10-30ms
```

#### 3.3.3 适用场景

- **推荐方案**：适用于大多数场景
- 高并发（>1000 QPS）
- 简单可靠

---

## 🔧 第四部分：多语言实现

### 4.1 Python实现

#### psycopg2实现

```python
import psycopg2
from psycopg2 import pool
from contextlib import contextmanager

# 连接池
connection_pool = psycopg2.pool.ThreadedConnectionPool(
    minconn=5,
    maxconn=20,
    host="localhost",
    database="ecommerce",
    user="postgres",
    password="password"
)

def deduct_inventory(product_id, quantity, user_id, price):
    """扣减库存（原子操作方案）"""
    conn = connection_pool.getconn()
    try:
        cur = conn.cursor()

        # 原子扣减库存
        cur.execute("""
            UPDATE inventory
            SET stock = stock - %s,
                updated_at = NOW()
            WHERE product_id = %s
              AND stock >= %s
        """, (quantity, product_id, quantity))

        if cur.rowcount == 0:
            raise ValueError("Insufficient stock")

        # 创建订单
        cur.execute("""
            INSERT INTO orders (user_id, product_id, quantity, price, total_amount)
            VALUES (%s, %s, %s, %s, %s)
        """, (user_id, product_id, quantity, price, price * quantity))

        conn.commit()
        return True

    except Exception as e:
        conn.rollback()
        raise
    finally:
        cur.close()
        connection_pool.putconn(conn)
```

#### asyncpg实现

```python
import asyncpg
import asyncio

async def deduct_inventory_async(pool, product_id, quantity, user_id, price):
    """异步扣减库存"""
    async with pool.acquire() as conn:
        async with conn.transaction():
            # 原子扣减库存
            result = await conn.execute("""
                UPDATE inventory
                SET stock = stock - $1,
                    updated_at = NOW()
                WHERE product_id = $2
                  AND stock >= $1
            """, quantity, product_id)

            if result == "UPDATE 0":
                raise ValueError("Insufficient stock")

            # 创建订单
            await conn.execute("""
                INSERT INTO orders (user_id, product_id, quantity, price, total_amount)
                VALUES ($1, $2, $3, $4, $5)
            """, user_id, product_id, quantity, price, price * quantity)

            return True
```

### 4.2 Java实现

#### JDBC实现

```java
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import javax.sql.DataSource;

public class InventoryService {
    private final DataSource dataSource;

    public boolean deductInventory(long productId, int quantity,
                                   long userId, double price) throws SQLException {
        Connection conn = dataSource.getConnection();
        try {
            conn.setAutoCommit(false);

            PreparedStatement stmt = conn.prepareStatement(
                "UPDATE inventory SET stock = stock - ?, updated_at = NOW() " +
                "WHERE product_id = ? AND stock >= ?"
            );
            stmt.setInt(1, quantity);
            stmt.setLong(2, productId);
            stmt.setInt(3, quantity);

            int rows = stmt.executeUpdate();
            if (rows == 0) {
                conn.rollback();
                throw new SQLException("Insufficient stock");
            }

            // 创建订单
            PreparedStatement orderStmt = conn.prepareStatement(
                "INSERT INTO orders (user_id, product_id, quantity, price, total_amount) " +
                "VALUES (?, ?, ?, ?, ?)"
            );
            orderStmt.setLong(1, userId);
            orderStmt.setLong(2, productId);
            orderStmt.setInt(3, quantity);
            orderStmt.setDouble(4, price);
            orderStmt.setDouble(5, price * quantity);
            orderStmt.executeUpdate();

            conn.commit();
            return true;

        } catch (SQLException e) {
            conn.rollback();
            throw e;
        } finally {
            conn.setAutoCommit(true);
            conn.close();
        }
    }
}
```

#### Spring实现

```java
import org.springframework.transaction.annotation.Transactional;
import org.springframework.stereotype.Service;

@Service
public class InventoryService {

    @Transactional
    public boolean deductInventory(long productId, int quantity,
                                   long userId, double price) {
        // 原子扣减库存
        int rows = inventoryRepository.deductStock(productId, quantity);
        if (rows == 0) {
            throw new InsufficientStockException();
        }

        // 创建订单
        Order order = new Order();
        order.setUserId(userId);
        order.setProductId(productId);
        order.setQuantity(quantity);
        order.setPrice(price);
        order.setTotalAmount(price * quantity);
        orderRepository.save(order);

        return true;
    }
}

// Repository方法
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    @Modifying
    @Query("UPDATE Inventory i SET i.stock = i.stock - :quantity, i.updatedAt = NOW() " +
           "WHERE i.productId = :productId AND i.stock >= :quantity")
    int deductStock(@Param("productId") long productId, @Param("quantity") int quantity);
}
```

### 4.3 Go实现

#### pgx实现

```go
package main

import (
    "context"
    "github.com/jackc/pgx/v5/pgxpool"
)

func DeductInventory(ctx context.Context, pool *pgxpool.Pool,
                     productID int64, quantity int, userID int64, price float64) error {
    tx, err := pool.Begin(ctx)
    if err != nil {
        return err
    }
    defer tx.Rollback(ctx)

    // 原子扣减库存
    result, err := tx.Exec(ctx,
        "UPDATE inventory SET stock = stock - $1, updated_at = NOW() "+
        "WHERE product_id = $2 AND stock >= $1",
        quantity, productID)
    if err != nil {
        return err
    }

    if result.RowsAffected() == 0 {
        return fmt.Errorf("insufficient stock")
    }

    // 创建订单
    _, err = tx.Exec(ctx,
        "INSERT INTO orders (user_id, product_id, quantity, price, total_amount) "+
        "VALUES ($1, $2, $3, $4, $5)",
        userID, productID, quantity, price, price*float64(quantity))
    if err != nil {
        return err
    }

    return tx.Commit(ctx)
}
```

---

## 📈 第五部分：性能优化

### 5.1 连接池优化

```python
# Python连接池优化
connection_pool = psycopg2.pool.ThreadedConnectionPool(
    minconn=10,      # 最小连接数
    maxconn=50,      # 最大连接数（根据并发调整）
    host="localhost",
    database="ecommerce",
    user="postgres",
    password="password",
    # MVCC优化参数
    connect_timeout=10,
    keepalives=1,
    keepalives_idle=30,
)
```

### 5.2 批量操作优化

```python
# 批量扣减库存（多个商品）
def batch_deduct_inventory(deductions):
    """批量扣减库存"""
    conn = connection_pool.getconn()
    try:
        cur = conn.cursor()

        # 批量更新
        for product_id, quantity in deductions:
            cur.execute("""
                UPDATE inventory
                SET stock = stock - %s
                WHERE product_id = %s AND stock >= %s
            """, (quantity, product_id, quantity))

            if cur.rowcount == 0:
                raise ValueError(f"Insufficient stock for product {product_id}")

        conn.commit()
        return True

    except Exception as e:
        conn.rollback()
        raise
    finally:
        cur.close()
        connection_pool.putconn(conn)
```

### 5.3 查询优化

```sql
-- 使用覆盖索引优化库存查询（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法创建覆盖索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'inventory' AND indexname = 'idx_inventory_product_stock') THEN
            CREATE INDEX idx_inventory_product_stock ON inventory(product_id, stock);
            RAISE NOTICE '覆盖索引 idx_inventory_product_stock 创建成功';
        ELSE
            RAISE NOTICE '覆盖索引 idx_inventory_product_stock 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 inventory 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_inventory_product_stock 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建覆盖索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查询时使用索引（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始使用覆盖索引查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT stock FROM inventory WHERE product_id = 1;
-- 使用索引扫描，避免表扫描
```

---

## 🎯 第六部分：监控和告警

### 6.1 关键指标监控

```sql
-- 监控库存扣减成功率（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_logs') THEN
            RAISE WARNING '表 order_logs 不存在，无法监控库存扣减成功率';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控库存扣减成功率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '监控准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    COUNT(*) FILTER (WHERE status = 'success') * 100.0 / COUNT(*) as success_rate
FROM order_logs
WHERE created_at > NOW() - interval '1 minute';

-- 监控低库存商品（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法监控低库存商品';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控低库存商品';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '监控准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    product_id,
    stock,
    reserved_stock,
    stock - reserved_stock as available_stock
FROM inventory
WHERE stock - reserved_stock < 10
ORDER BY available_stock;
```

### 6.2 告警规则

```yaml
# Prometheus告警规则
- alert: LowInventory
  expr: inventory_available_stock < 10
  for: 5m
  labels:
    severity: warning
  annotations:
    summary: "Low inventory alert"
    description: "Product {{ $labels.product_id }} has only {{ $value }} items left"

- alert: InventoryDeductionFailure
  expr: rate(inventory_deduction_failures[5m]) > 0.1
  for: 2m
  labels:
    severity: critical
  annotations:
    summary: "High inventory deduction failure rate"
    description: "Failure rate: {{ $value }}"
```

### 6.3 故障处理

```sql
-- 故障场景1：库存扣减失败
-- 处理：检查库存是否真的不足，如果是，提示用户；如果是系统错误，重试

-- 故障场景2：死锁
-- 处理：自动重试（指数退避）

-- 故障场景3：库存不一致
-- 处理：定期对账，修正库存
```

---

## 📝 总结

### 核心方案

1. **推荐方案**：原子操作（UPDATE WHERE）
   - 性能最优
   - 实现简单
   - 无锁竞争

2. **备选方案**：乐观锁（版本号）
   - 高并发性能好
   - 需要重试机制

3. **保守方案**：SELECT FOR UPDATE
   - 强一致性
   - 性能较低

### 最佳实践

1. **数据库设计**：合理设置fillfactor和VACUUM参数
2. **事务设计**：使用原子操作，避免长事务
3. **连接池**：合理配置连接池大小
4. **监控告警**：监控关键指标，及时告警
5. **故障处理**：实现重试机制和故障恢复

### 性能指标

- **QPS**: 3000-5000（单实例）
- **P99延迟**: 10-30ms
- **成功率**: >99.9%
- **超卖率**: 0%

PostgreSQL 17/18的MVCC机制在电商库存扣减场景下表现优异，通过合理的数据库设计和事务实现，可以实现高性能、高可靠性的库存管理系统。
