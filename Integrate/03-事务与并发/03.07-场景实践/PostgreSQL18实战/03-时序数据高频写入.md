...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '单行INSERT成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 sensor_data 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '单行INSERT失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '批量INSERT成功，插入 % 条记录', inserted_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 sensor_data 不存在';
        WHEN array_subscript_error THEN
            RAISE WARNING '数组长度不匹配';
        WHEN OTHERS THEN
            RAISE WARNING '批量INSERT失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '批量写入成功，插入 % 条记录', inserted_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 sensor_data 不存在';
        WHEN OTHERS THEN
