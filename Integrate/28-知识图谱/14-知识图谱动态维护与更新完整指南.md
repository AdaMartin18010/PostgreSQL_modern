...
BEGIN
    ...
    IF p_version_id < 0 THEN
        RAISE EXCEPTION '版本ID必须>=0: %', p_version_id;
    END IF;

    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kg_changes') THEN
        RAISE EXCEPTION 'kg_changes表不存在';
    END IF;
    ...
    BEGIN
        ...
        SELECT
            ...
            COALESCE(es.current_data->>'name', '')::TEXT AS entity_name,
            COALESCE(es.current_data->>'type', '')::TEXT AS entity_type,
            COALESCE(es.current_data->'properties', '{}'::JSONB) AS properties
        FROM entity_states es
        WHERE es.current_data IS NOT NULL
        ORDER BY es.entity_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '查询知识图谱版本失败: %', SQLERRM;
    END;
END;
$$;
```
