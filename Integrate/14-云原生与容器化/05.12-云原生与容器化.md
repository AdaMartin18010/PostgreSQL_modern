---

> **📋 文档来源**: `PostgreSQL\05-部署架构\容器化部署\05.12-云原生与容器化.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 云原生与容器化

> **文档版本**: v2.0
> **最后更新**: 2025-11-12
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **文档状态**: ✅ 已更新

---

## 📋 目录

- [云原生与容器化](#云原生与容器化)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. 合并来源与映射](#2-合并来源与映射)
  - [3. 容器化要点](#3-容器化要点)
    - [3.1 Docker镜像构建](#31-docker镜像构建)
      - [基础Dockerfile](#基础dockerfile)
      - [多阶段构建优化](#多阶段构建优化)
    - [3.2 参数注入](#32-参数注入)
      - [使用环境变量](#使用环境变量)
      - [使用ConfigMap（Kubernetes）](#使用configmapkubernetes)
    - [3.3 健康检查](#33-健康检查)
      - [Docker健康检查](#docker健康检查)
      - [Docker Compose健康检查](#docker-compose健康检查)
      - [Kubernetes健康检查](#kubernetes健康检查)
    - [3.4 日志管理](#34-日志管理)
      - [Docker日志配置](#docker日志配置)
      - [日志收集（Fluentd）](#日志收集fluentd)
  - [4. Kubernetes](#4-kubernetes)
    - [4.1 StatefulSet部署](#41-statefulset部署)
      - [基础StatefulSet](#基础statefulset)
    - [4.2 PersistentVolume和PersistentVolumeClaim](#42-persistentvolume和persistentvolumeclaim)
      - [创建StorageClass](#创建storageclass)
      - [使用PVC](#使用pvc)
    - [4.3 PostgreSQL Operator](#43-postgresql-operator)
      - [Crunchy PostgreSQL Operator](#crunchy-postgresql-operator)
      - [Zalando PostgreSQL Operator](#zalando-postgresql-operator)
    - [4.4 自动备份](#44-自动备份)
      - [CronJob备份](#cronjob备份)
    - [4.5 自动扩缩容](#45-自动扩缩容)
      - [HPA配置](#hpa配置)
  - [5. 最佳实践](#5-最佳实践)
    - [5.1 资源请求和限制](#51-资源请求和限制)
    - [5.2 亲和性和反亲和性](#52-亲和性和反亲和性)
      - [Pod反亲和性（避免同一节点）](#pod反亲和性避免同一节点)
      - [节点亲和性（指定节点类型）](#节点亲和性指定节点类型)
    - [5.3 滚动升级](#53-滚动升级)
      - [更新策略](#更新策略)
      - [金丝雀发布](#金丝雀发布)
    - [5.4 网络策略](#54-网络策略)
    - [5.5 配置管理](#55-配置管理)
      - [使用ConfigMap和Secret](#使用configmap和secret)
    - [5.6 监控集成](#56-监控集成)
      - [Prometheus监控](#prometheus监控)
  - [6. 后续整合任务](#6-后续整合任务)
    - [11.1 Helm Chart示例](#111-helm-chart示例)
    - [11.2 Operator对比](#112-operator对比)
    - [11.3 选择建议](#113-选择建议)
  - [6. 生产级配置示例](#6-生产级配置示例)
    - [6.1 完整生产环境配置](#61-完整生产环境配置)
    - [6.2 多环境配置管理](#62-多环境配置管理)
    - [6.3 安全配置](#63-安全配置)
    - [6.4 性能优化配置](#64-性能优化配置)
  - [7. Helm Chart完整示例](#7-helm-chart完整示例)
    - [7.1 Bitnami PostgreSQL Chart配置](#71-bitnami-postgresql-chart配置)
    - [7.2 自定义Helm Chart](#72-自定义helm-chart)
    - [7.3 Helm Chart最佳实践](#73-helm-chart最佳实践)
  - [8. Operator详细对比和选择](#8-operator详细对比和选择)
    - [8.1 Crunchy Operator详细对比](#81-crunchy-operator详细对比)
    - [8.2 Zalando Operator详细对比](#82-zalando-operator详细对比)
    - [8.3 CloudNativePG详细对比](#83-cloudnativepg详细对比)
    - [8.4 选择建议和迁移指南](#84-选择建议和迁移指南)
  - [9. 故障排查和诊断](#9-故障排查和诊断)
    - [9.1 容器化问题排查](#91-容器化问题排查)
    - [9.2 Kubernetes问题排查](#92-kubernetes问题排查)
    - [9.3 性能问题诊断](#93-性能问题诊断)
  - [10. 监控和告警配置](#10-监控和告警配置)
    - [10.1 Prometheus监控集成](#101-prometheus监控集成)
    - [10.2 Grafana仪表板配置](#102-grafana仪表板配置)
    - [10.3 告警规则配置](#103-告警规则配置)
  - [11. 后续整合任务](#11-后续整合任务)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [核心课程](#核心课程)
      - [运维实践](#运维实践)
      - [前沿技术](#前沿技术)

---

## 1. 概述

- 容器镜像、Kubernetes编排、持久化与运维

## 2. 合并来源与映射

- 1.1.15-云原生与容器化部署-扩充版.md
- 1.1.15-云原生与容器化部署.md

## 3. 容器化要点

- 镜像构建、参数注入、健康检查、日志

### 3.1 Docker镜像构建

#### 基础Dockerfile

```dockerfile
# 使用官方PostgreSQL镜像
FROM postgres:18

# 设置环境变量
ENV POSTGRES_DB=mydb
ENV POSTGRES_USER=postgres
ENV POSTGRES_PASSWORD=changeme

# 复制自定义配置
COPY postgresql.conf /etc/postgresql/postgresql.conf
COPY pg_hba.conf /etc/postgresql/pg_hba.conf

# 复制初始化脚本
COPY init.sql /docker-entrypoint-initdb.d/

# 设置数据目录权限
RUN chown -R postgres:postgres /var/lib/postgresql/data

# 暴露端口
EXPOSE 5432

# 使用自定义启动脚本
COPY docker-entrypoint.sh /usr/local/bin/
RUN chmod +x /usr/local/bin/docker-entrypoint.sh
ENTRYPOINT ["docker-entrypoint.sh"]
CMD ["postgres"]
```

#### 多阶段构建优化

```dockerfile
# 构建阶段
FROM postgres:18 AS builder

# 安装扩展
RUN apt-get update && apt-get install -y \
    postgresql-18-pgvector \
    postgresql-18-pg-stat-statements \
    && rm -rf /var/lib/apt/lists/*

# 运行阶段
FROM postgres:18

# 从构建阶段复制扩展
COPY --from=builder /usr/share/postgresql/18/extension/* \
    /usr/share/postgresql/18/extension/
COPY --from=builder /usr/lib/postgresql/18/lib/* \
    /usr/lib/postgresql/18/lib/

# 复制配置和脚本
COPY postgresql.conf /etc/postgresql/
COPY init-extensions.sql /docker-entrypoint-initdb.d/
```

### 3.2 参数注入

#### 使用环境变量

```bash
# docker-compose.yml
version: '3.8'
services:
  postgres:
    image: postgres:18
    environment:
      POSTGRES_DB: mydb
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
      POSTGRES_INITDB_ARGS: "--encoding=UTF8 --locale=zh_CN.UTF-8"
      # 自定义配置参数
      POSTGRES_SHARED_BUFFERS: 256MB
      POSTGRES_MAX_CONNECTIONS: 100
    volumes:
      - postgres_data:/var/lib/postgresql/data
    ports:
      - "5432:5432"
```

#### 使用ConfigMap（Kubernetes）

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: postgres-config
data:
  postgresql.conf: |
    shared_buffers = 256MB
    max_connections = 100
    work_mem = 4MB
    effective_cache_size = 1GB
  pg_hba.conf: |
    host all all 0.0.0.0/0 md5
```

### 3.3 健康检查

#### Docker健康检查

```dockerfile
# Dockerfile
HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
  CMD pg_isready -U postgres -d mydb || exit 1
```

#### Docker Compose健康检查

```yaml
services:
  postgres:
    image: postgres:18
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 30s
```

#### Kubernetes健康检查

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  template:
    spec:
      containers:
      - name: postgres
        image: postgres:18
        livenessProbe:
          exec:
            command:
            - /bin/sh
            - -c
            - pg_isready -U postgres
          initialDelaySeconds: 30
          periodSeconds: 10
          timeoutSeconds: 5
        readinessProbe:
          exec:
            command:
            - /bin/sh
            - -c
            - pg_isready -U postgres
          initialDelaySeconds: 5
          periodSeconds: 5
```

### 3.4 日志管理

#### Docker日志配置

```yaml
services:
  postgres:
    image: postgres:18
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"
        labels: "service=postgres"
```

#### 日志收集（Fluentd）

```yaml
# docker-compose.yml
services:
  postgres:
    image: postgres:18
    logging:
      driver: "fluentd"
      options:
        fluentd-address: localhost:24224
        tag: "postgres.{{.Name}}"

  fluentd:
    image: fluent/fluentd:latest
    volumes:
      - ./fluentd.conf:/fluentd/etc/fluent.conf
    ports:
      - "24224:24224"
```

## 4. Kubernetes

- StatefulSet、PV/PVC、Operator、备份与伸缩

### 4.1 StatefulSet部署

#### 基础StatefulSet

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  serviceName: postgres
  replicas: 1
  selector:
    matchLabels:
      app: postgres
  template:
    metadata:
      labels:
        app: postgres
    spec:
      containers:
      - name: postgres
        image: postgres:18
        env:
        - name: POSTGRES_DB
          value: mydb
        - name: POSTGRES_USER
          value: postgres
        - name: POSTGRES_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: password
        ports:
        - containerPort: 5432
          name: postgres
        volumeMounts:
        - name: postgres-data
          mountPath: /var/lib/postgresql/data
  volumeClaimTemplates:
  - metadata:
      name: postgres-data
    spec:
      accessModes: [ "ReadWriteOnce" ]
      resources:
        requests:
          storage: 20Gi
```

### 4.2 PersistentVolume和PersistentVolumeClaim

#### 创建StorageClass

```yaml
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
  name: fast-ssd
provisioner: kubernetes.io/aws-ebs
parameters:
  type: gp3
  fsType: ext4
  encrypted: "true"
```

#### 使用PVC

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: postgres-pvc
spec:
  accessModes:
    - ReadWriteOnce
  storageClassName: fast-ssd
  resources:
    requests:
      storage: 50Gi
```

### 4.3 PostgreSQL Operator

#### Crunchy PostgreSQL Operator

```bash
# 安装Operator
kubectl apply -f https://raw.githubusercontent.com/CrunchyData/postgres-operator/v5.3.0/install/kubectl/postgres-operator.yaml

# 创建PostgreSQL集群
kubectl apply -f - <<EOF
apiVersion: postgres-operator.crunchydata.com/v1beta1
kind: PostgresCluster
metadata:
  name: postgres-cluster
spec:
  image: registry.developers.crunchydata.com/crunchydata/crunchy-postgres:ubi8-18.0-0
  postgresVersion: 18
  instances:
    - name: instance1
      replicas: 2
      dataVolumeClaimSpec:
        accessModes:
        - "ReadWriteOnce"
        resources:
          requests:
            storage: 10Gi
  backups:
    pgbackrest:
      image: registry.developers.crunchydata.com/crunchydata/crunchy-pgbackrest:ubi8-2.48-0
      repos:
      - name: repo1
        volume:
          volumeClaimSpec:
            accessModes:
            - "ReadWriteOnce"
            resources:
              requests:
                storage: 20Gi
EOF
```

#### Zalando PostgreSQL Operator

```yaml
apiVersion: "acid.zalan.do/v1"
kind: postgresql
metadata:
  name: postgres-cluster
spec:
  teamId: "myteam"
  volume:
    size: 10Gi
  numberOfInstances: 2
  users:
    zalando:
    - superuser
    - createdb
  databases:
    foo: zalando
  postgresql:
    version: "18"
```

### 4.4 自动备份

#### CronJob备份

```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
  name: postgres-backup
spec:
  schedule: "0 2 * * *"  # 每天凌晨2点
  jobTemplate:
    spec:
      template:
        spec:
          containers:
          - name: pg-dump
            image: postgres:18
            command:
            - /bin/sh
            - -c
            - |
              pg_dump -h postgres -U postgres mydb > /backup/mydb-$(date +%Y%m%d).sql
            env:
            - name: PGPASSWORD
              valueFrom:
                secretKeyRef:
                  name: postgres-secret
                  key: password
            volumeMounts:
            - name: backup-volume
              mountPath: /backup
          volumes:
          - name: backup-volume
            persistentVolumeClaim:
              claimName: backup-pvc
          restartPolicy: OnFailure
```

### 4.5 自动扩缩容

#### HPA配置

```yaml
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: postgres-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: StatefulSet
    name: postgres
  minReplicas: 1
  maxReplicas: 3
  metrics:
  - type: Resource
    resource:
      name: cpu
      target:
        type: Utilization
        averageUtilization: 70
```

## 5. 最佳实践

- 资源请求/限制、亲和/反亲和、滚动升级

### 5.1 资源请求和限制

```yaml
spec:
  containers:
  - name: postgres
    image: postgres:18
    resources:
      requests:
        memory: "2Gi"
        cpu: "1000m"
      limits:
        memory: "4Gi"
        cpu: "2000m"
```

### 5.2 亲和性和反亲和性

#### Pod反亲和性（避免同一节点）

```yaml
spec:
  affinity:
    podAntiAffinity:
      preferredDuringSchedulingIgnoredDuringExecution:
      - weight: 100
        podAffinityTerm:
          labelSelector:
            matchExpressions:
            - key: app
              operator: In
              values:
              - postgres
          topologyKey: kubernetes.io/hostname
```

#### 节点亲和性（指定节点类型）

```yaml
spec:
  affinity:
    nodeAffinity:
      requiredDuringSchedulingIgnoredDuringExecution:
        nodeSelectorTerms:
        - matchExpressions:
          - key: node-type
            operator: In
            values:
            - database
```

### 5.3 滚动升级

#### 更新策略

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  updateStrategy:
    type: RollingUpdate
    rollingUpdate:
      partition: 0  # 从第0个Pod开始更新
  template:
    spec:
      containers:
      - name: postgres
        image: postgres:18  # 更新镜像版本
```

#### 金丝雀发布

```yaml
# 1. 创建金丝雀版本
apiVersion: apps/v1
kind: Deployment
metadata:
  name: postgres-canary
spec:
  replicas: 1
  template:
    spec:
      containers:
      - name: postgres
        image: postgres:18-new

# 2. 逐步增加金丝雀流量
# 使用Service和Traffic Splitting
```

### 5.4 网络策略

```yaml
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: postgres-netpol
spec:
  podSelector:
    matchLabels:
      app: postgres
  policyTypes:
  - Ingress
  - Egress
  ingress:
  - from:
    - podSelector:
        matchLabels:
          app: myapp
    ports:
    - protocol: TCP
      port: 5432
  egress:
  - to:
    - podSelector:
        matchLabels:
          app: backup
    ports:
    - protocol: TCP
      port: 5432
```

### 5.5 配置管理

#### 使用ConfigMap和Secret

```yaml
# ConfigMap
apiVersion: v1
kind: ConfigMap
metadata:
  name: postgres-config
data:
  postgresql.conf: |
    shared_buffers = 256MB
    max_connections = 100

# Secret
apiVersion: v1
kind: Secret
metadata:
  name: postgres-secret
type: Opaque
stringData:
  password: changeme
  username: postgres
```

### 5.6 监控集成

#### Prometheus监控

```yaml
apiVersion: v1
kind: Service
metadata:
  name: postgres
  annotations:
    prometheus.io/scrape: "true"
    prometheus.io/port: "9187"
spec:
  ports:
  - name: metrics
    port: 9187
    targetPort: 9187
```

## 6. 后续整合任务

- 增补Helm示例与Operator对比

### 11.1 Helm Chart示例

```yaml
# values.yaml
postgresql:
  image:
    repository: postgres
    tag: "18"
  auth:
    postgresPassword: changeme
    database: mydb
  persistence:
    enabled: true
    size: 20Gi
  resources:
    requests:
      memory: "2Gi"
      cpu: "1000m"
    limits:
      memory: "4Gi"
      cpu: "2000m"
```

```bash
# 安装Helm Chart
helm repo add bitnami https://charts.bitnami.com/bitnami
helm install postgres bitnami/postgresql -f values.yaml
```

### 11.2 Operator对比

| 特性 | Crunchy Operator | Zalando Operator | CloudNativePG |
|------|-----------------|-----------------|---------------|
| 高可用 | ✅ | ✅ | ✅ |
| 自动备份 | ✅ (pgBackRest) | ✅ | ✅ |
| 监控集成 | ✅ | ✅ | ✅ |
| 扩展支持 | ✅ | ✅ | ✅ |
| 社区活跃度 | 高 | 中 | 高 |
| 易用性 | 中 | 高 | 高 |

### 11.3 选择建议

- **小型项目**: 使用StatefulSet + ConfigMap/Secret
- **中型项目**: 使用Helm Chart（如Bitnami）
- **大型项目**: 使用Operator（推荐Crunchy或CloudNativePG）
- **云环境**: 使用云服务商的托管服务

---

## 6. 生产级配置示例

### 6.1 完整生产环境配置

**完整的云原生PostgreSQL配置**:

```yaml
# docker-compose.prod.yml
version: '3.8'

services:
  postgres:
    image: postgres:18
    environment:
      POSTGRES_USER: ${POSTGRES_USER}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
      POSTGRES_DB: ${POSTGRES_DB}
      POSTGRES_INITDB_ARGS: "--encoding=UTF8 --locale=C --data-checksums"
    volumes:
      - postgres_data:/var/lib/postgresql/data
      - ./config/postgresql.conf:/etc/postgresql/postgresql.conf
      - ./config/pg_hba.conf:/etc/postgresql/pg_hba.conf
    command:
      - postgres
      - -c
      - config_file=/etc/postgresql/postgresql.conf
      - -c
      - hba_file=/etc/postgresql/pg_hba.conf
    deploy:
      resources:
        limits:
          cpus: '4'
          memory: 8G
        reservations:
          cpus: '2'
          memory: 4G
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 30s
    networks:
      - postgres_network

volumes:
  postgres_data:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: /data/postgres/data

networks:
  postgres_network:
    driver: bridge
    ipam:
      config:
        - subnet: 172.20.0.0/16
```

### 6.2 多环境配置管理

**使用Kustomize管理多环境**:

```yaml
# base/kustomization.yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization

resources:
- statefulset.yaml
- service.yaml
- configmap.yaml
- secret.yaml

# production/kustomization.yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization

resources:
- ../base

patches:
- path: production-patch.yaml

# production/production-patch.yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  replicas: 3
  template:
    spec:
      containers:
      - name: postgres
        resources:
          requests:
            memory: "4Gi"
            cpu: "2"
          limits:
            memory: "8Gi"
            cpu: "4"
```

### 6.3 安全配置

**云原生安全配置**:

```yaml
# 使用Secret管理敏感信息
apiVersion: v1
kind: Secret
metadata:
  name: postgres-secret
type: Opaque
stringData:
  postgres-password: ${POSTGRES_PASSWORD}
  replication-password: ${REPLICATION_PASSWORD}

# 使用SecurityContext
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  template:
    spec:
      securityContext:
        fsGroup: 999
        runAsUser: 999
        runAsNonRoot: true
        seccompProfile:
          type: RuntimeDefault
      containers:
      - name: postgres
        securityContext:
          allowPrivilegeEscalation: false
          capabilities:
            drop:
            - ALL
            add:
            - CHOWN
            - DAC_OVERRIDE
            - FOWNER
            - SETGID
            - SETUID
```

### 6.4 性能优化配置

**云原生性能优化**:

```yaml
# 使用InitContainer优化启动
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
spec:
  template:
    spec:
      initContainers:
      - name: init-postgres
        image: busybox:latest
        command:
        - sh
        - -c
        - |
          chown -R 999:999 /var/lib/postgresql/data
          chmod 700 /var/lib/postgresql/data
        volumeMounts:
        - name: postgres-storage
          mountPath: /var/lib/postgresql/data

      containers:
      - name: postgres
        image: postgres:18
        resources:
          requests:
            memory: "4Gi"
            cpu: "2"
          limits:
            memory: "8Gi"
            cpu: "4"
        # PostgreSQL 18异步I/O优化
        env:
        - name: POSTGRES_INITDB_ARGS
          value: "--encoding=UTF8 --locale=C --data-checksums"
```

## 7. Helm Chart完整示例

### 7.1 Bitnami PostgreSQL Chart配置

**完整的Bitnami Chart配置**:

```yaml
# values-prod.yaml
global:
  postgresql:
    auth:
      postgresPassword: ${POSTGRES_PASSWORD}
      database: mydb
      username: app_user
      password: ${APP_PASSWORD}

postgresql:
  image:
    repository: postgres
    tag: "18"

  primary:
    persistence:
      enabled: true
      size: 100Gi
      storageClass: fast-ssd
    resources:
      requests:
        memory: "4Gi"
        cpu: "2"
      limits:
        memory: "8Gi"
        cpu: "4"
    configuration:
      max_connections: "500"
      shared_buffers: "2GB"
      effective_cache_size: "6GB"
      work_mem: "16MB"
      maintenance_work_mem: "512MB"
      # PostgreSQL 18新特性
      io_method: "aio"
      io_combine_limit: "128"
      maintenance_io_workers: "4"
      max_io_workers: "10"
      parallel_leader_participation: "on"

  readReplicas:
    replicaCount: 2
    persistence:
      enabled: true
      size: 100Gi
      storageClass: fast-ssd
    resources:
      requests:
        memory: "4Gi"
        cpu: "2"
      limits:
        memory: "8Gi"
        cpu: "4"

  metrics:
    enabled: true
    serviceMonitor:
      enabled: true
      namespace: monitoring

  networkPolicy:
    enabled: true
    ingress:
      - from:
        - namespaceSelector:
            matchLabels:
              name: application
```

**安装和升级**:

```bash
# 添加Helm仓库
helm repo add bitnami https://charts.bitnami.com/bitnami
helm repo update

# 安装生产环境
helm install postgres-prod bitnami/postgresql \
  -f values-prod.yaml \
  --namespace postgres \
  --create-namespace

# 升级
helm upgrade postgres-prod bitnami/postgresql \
  -f values-prod.yaml \
  --namespace postgres

# 查看状态
helm status postgres-prod --namespace postgres
```

### 7.2 自定义Helm Chart

**创建自定义Helm Chart**:

```bash
# 创建Chart
helm create postgres-custom
cd postgres-custom

# 目录结构
# postgres-custom/
#   Chart.yaml
#   values.yaml
#   templates/
#     statefulset.yaml
#     service.yaml
#     configmap.yaml
#     secret.yaml
```

**Chart.yaml**:

```yaml
apiVersion: v2
name: postgres-custom
description: Custom PostgreSQL Helm chart
type: application
version: 1.0.0
appVersion: "18"
```

**values.yaml**:

```yaml
replicaCount: 3

image:
  repository: postgres
  tag: "18"
  pullPolicy: IfNotPresent

postgresql:
  database: mydb
  username: postgres
  password: changeme

persistence:
  enabled: true
  size: 100Gi
  storageClass: fast-ssd

resources:
  requests:
    memory: "4Gi"
    cpu: "2"
  limits:
    memory: "8Gi"
    cpu: "4"

monitoring:
  enabled: true
  serviceMonitor:
    enabled: true
```

### 7.3 Helm Chart最佳实践

**Helm Chart最佳实践**:

```yaml
# 1. 使用values.schema.json验证配置
# values.schema.json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "type": "object",
  "properties": {
    "replicaCount": {
      "type": "integer",
      "minimum": 1,
      "maximum": 10
    },
    "resources": {
      "type": "object",
      "properties": {
        "requests": {
          "type": "object",
          "properties": {
            "memory": {"type": "string"},
            "cpu": {"type": "string"}
          }
        }
      }
    }
  }
}

# 2. 使用Helm hooks进行初始化
# templates/job-init.yaml
apiVersion: batch/v1
kind: Job
metadata:
  name: {{ include "postgres.fullname" . }}-init
  annotations:
    "helm.sh/hook": pre-install,pre-upgrade
    "helm.sh/hook-weight": "-5"
    "helm.sh/hook-delete-policy": before-hook-creation,hook-succeeded
spec:
  template:
    spec:
      containers:
      - name: init
        image: postgres:18
        command:
        - /bin/sh
        - -c
        - |
          # 初始化脚本
          echo "Initializing database..."
```

## 8. Operator详细对比和选择

### 8.1 Crunchy Operator详细对比

**Crunchy Operator特点**:

```yaml
# Crunchy Operator优势
优势:
  - 企业级功能完整
  - pgBackRest备份集成
  - 支持多种存储后端
  - 活跃的社区支持
  - 详细的文档

劣势:
  - 配置相对复杂
  - 资源消耗较高
  - 学习曲线较陡

适用场景:
  - 企业级生产环境
  - 需要完整备份恢复功能
  - 需要高可用和自动故障转移
  - 需要详细的监控和告警
```

**Crunchy Operator配置示例**:

```yaml
apiVersion: postgres-operator.crunchydata.com/v1beta1
kind: PostgresCluster
metadata:
  name: postgres-cluster
spec:
  image: registry.developers.crunchydata.com/crunchydata/crunchy-postgres:ubi8-18.0-0
  postgresVersion: 18
  instances:
  - name: instance1
    replicas: 3
    resources:
      requests:
        memory: 4Gi
        cpu: 2
      limits:
        memory: 8Gi
        cpu: 4
    dataVolumeClaimSpec:
      accessModes:
      - ReadWriteOnce
      resources:
        requests:
          storage: 100Gi
      storageClassName: fast-ssd
  backups:
    pgbackrest:
      repos:
      - name: repo1
        volume:
          volumeClaimSpec:
            accessModes:
            - ReadWriteOnce
            resources:
              requests:
                storage: 200Gi
      schedules:
        full: "0 2 * * 0"
        incremental: "0 2 * * *"
```

### 8.2 Zalando Operator详细对比

**Zalando Operator特点**:

```yaml
# Zalando Operator优势
优势:
  - 配置简单易用
  - 自动故障转移
  - 支持逻辑复制
  - 良好的Kubernetes集成
  - 支持多版本PostgreSQL

劣势:
  - 备份功能相对简单
  - 社区活跃度中等
  - 文档相对较少

适用场景:
  - 中小型项目
  - 需要简单的配置
  - 需要快速部署
  - 不需要复杂的备份策略
```

**Zalando Operator配置示例**:

```yaml
apiVersion: "acid.zalan.do/v1"
kind: postgresql
metadata:
  name: postgres-cluster
spec:
  teamId: "myteam"
  volume:
    size: 100Gi
    storageClass: fast-ssd
  numberOfInstances: 3
  users:
    zalando:
    - superuser
    - createdb
  databases:
    foo: zalando
  postgresql:
    version: "18"
    parameters:
      max_connections: "500"
      shared_buffers: "2GB"
      effective_cache_size: "6GB"
      # PostgreSQL 18新特性
      io_method: "aio"
      io_combine_limit: "128"
      maintenance_io_workers: "4"
      max_io_workers: "10"
  resources:
    requests:
      cpu: 2
      memory: 4Gi
    limits:
      cpu: 4
      memory: 8Gi
  enableMasterLoadBalancer: true
  enableReplicaLoadBalancer: true
```

### 8.3 CloudNativePG详细对比

**CloudNativePG特点**:

```yaml
# CloudNativePG优势
优势:
  - CNCF项目，标准化
  - 配置灵活
  - 支持多种备份方式
  - 良好的社区支持
  - 详细的文档

劣势:
  - 相对较新
  - 功能还在完善中
  - 企业级功能较少

适用场景:
  - 云原生环境
  - 需要CNCF标准化
  - 需要灵活的配置
  - 中小型到大型项目
```

**CloudNativePG配置示例**:

```yaml
apiVersion: postgresql.cnpg.io/v1
kind: Cluster
metadata:
  name: postgres-cluster
spec:
  instances: 3
  postgresql:
    parameters:
      max_connections: "500"
      shared_buffers: "2GB"
      effective_cache_size: "6GB"
      # PostgreSQL 18新特性
      io_method: "aio"
      io_combine_limit: "128"
      maintenance_io_workers: "4"
      max_io_workers: "10"
  resources:
    requests:
      memory: 4Gi
      cpu: 2
    limits:
      memory: 8Gi
      cpu: 4
  storage:
    size: 100Gi
    storageClass: fast-ssd
  backup:
    barmanObjectStore:
      destinationPath: s3://backup-bucket/postgres
      s3Credentials:
        accessKeyId:
          name: backup-credentials
          key: ACCESS_KEY_ID
        secretAccessKey:
          name: backup-credentials
          key: SECRET_ACCESS_KEY
      wal:
        retention: "7d"
      data:
        retention: "30d"
```

### 8.4 选择建议和迁移指南

**Operator选择决策树**:

```text
需要企业级功能？
├─ 是 → Crunchy Operator
│   └─ 需要完整备份恢复、高可用、详细监控
│
├─ 否 → 需要简单配置？
│   ├─ 是 → Zalando Operator
│   │   └─ 中小型项目、快速部署
│   │
│   └─ 否 → CloudNativePG
│       └─ 云原生环境、CNCF标准化
```

**迁移指南**:

```bash
# 从StatefulSet迁移到Operator
# 1. 备份数据
pg_dump -h old_postgres -U postgres -d mydb -F c -f backup.dump

# 2. 安装Operator
helm install postgres-operator <operator-chart>

# 3. 创建PostgresCluster
kubectl apply -f postgres-cluster.yaml

# 4. 等待集群就绪
kubectl wait --for=condition=ready postgrescluster/postgres-cluster

# 5. 恢复数据
pg_restore -h new_postgres -U postgres -d mydb -F c backup.dump

# 6. 验证数据
psql -h new_postgres -U postgres -d mydb -c "SELECT count(*) FROM table_name;"

# 7. 切换应用连接
# 更新应用连接字符串指向新集群

# 8. 删除旧StatefulSet
kubectl delete statefulset old-postgres
```

## 9. 故障排查和诊断

### 9.1 容器化问题排查

**容器化问题诊断**:

```bash
# 1. 检查容器状态
docker ps -a | grep postgres
docker logs postgres

# 2. 检查资源使用
docker stats postgres

# 3. 检查网络连接
docker network inspect postgres_network

# 4. 检查数据卷
docker volume inspect postgres_data

# 5. 进入容器调试
docker exec -it postgres bash

# 6. 检查PostgreSQL配置
docker exec postgres cat /etc/postgresql/postgresql.conf
```

**常见容器化问题**:

```bash
# 问题1: 容器启动失败
# 原因: 配置错误或资源不足
# 解决: 检查日志和资源
docker logs postgres
docker stats postgres

# 问题2: 数据持久化失败
# 原因: 卷挂载问题或权限问题
# 解决: 检查卷挂载和权限
docker volume inspect postgres_data
docker exec postgres ls -la /var/lib/postgresql/data

# 问题3: 网络连接问题
# 原因: 网络配置错误
# 解决: 检查网络配置
docker network inspect postgres_network
```

### 9.2 Kubernetes问题排查

**Kubernetes问题诊断**:

```bash
# 1. 检查Pod状态
kubectl get pods -n postgres
kubectl describe pod postgres-0 -n postgres

# 2. 检查Pod日志
kubectl logs postgres-0 -n postgres
kubectl logs postgres-0 -n postgres --previous

# 3. 检查资源使用
kubectl top pod postgres-0 -n postgres
kubectl top node

# 4. 检查存储
kubectl get pvc -n postgres
kubectl describe pvc postgres-storage-postgres-0 -n postgres

# 5. 检查网络
kubectl get svc -n postgres
kubectl get endpoints -n postgres

# 6. 检查事件
kubectl get events -n postgres --sort-by='.lastTimestamp'
```

**常见Kubernetes问题**:

```bash
# 问题1: Pod一直处于Pending状态
# 原因: 资源不足或调度问题
# 解决: 检查节点资源
kubectl describe node <node-name>
kubectl get nodes

# 问题2: Pod一直处于CrashLoopBackOff状态
# 原因: 容器启动失败
# 解决: 查看日志找出错误
kubectl logs postgres-0 -n postgres --previous

# 问题3: 存储卷无法挂载
# 原因: PVC未创建或存储类问题
# 解决: 检查PVC和StorageClass
kubectl get pvc -n postgres
kubectl get storageclass
```

### 9.3 性能问题诊断

**性能诊断脚本**:

```bash
#!/bin/bash
# scripts/cloudnative_performance_diagnosis.sh

echo "=== 云原生PostgreSQL性能诊断 ==="

# 1. 容器/K8s资源使用
if command -v docker &> /dev/null; then
    echo "=== Docker容器资源使用 ==="
    docker stats --no-stream postgres
elif command -v kubectl &> /dev/null; then
    echo "=== Kubernetes Pod资源使用 ==="
    kubectl top pods -n postgres
fi

# 2. 数据库连接数
echo "=== 数据库连接数 ==="
psql -h localhost -U postgres -c "SELECT count(*) FROM pg_stat_activity;"

# 3. 慢查询
echo "=== 慢查询Top 10 ==="
psql -h localhost -U postgres -c "SELECT query, calls, mean_time FROM pg_stat_statements ORDER BY mean_time DESC LIMIT 10;"

# 4. I/O性能（PostgreSQL 18）
echo "=== I/O性能 ==="
psql -h localhost -U postgres -c "SELECT object, context, reads, writes, read_time, write_time FROM pg_stat_io ORDER BY reads DESC LIMIT 10;"
```

## 10. 监控和告警配置

### 10.1 Prometheus监控集成

**完整的Prometheus配置**:

```yaml
# prometheus-config.yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: prometheus-config
  namespace: monitoring
data:
  prometheus.yml: |
    global:
      scrape_interval: 15s
      evaluation_interval: 15s

    scrape_configs:
    - job_name: 'postgres'
      kubernetes_sd_configs:
      - role: pod
        namespaces:
          names:
          - postgres
      relabel_configs:
      - source_labels: [__meta_kubernetes_pod_label_app]
        action: keep
        regex: postgres
      - source_labels: [__meta_kubernetes_pod_container_port_name]
        action: keep
        regex: metrics
```

**ServiceMonitor配置**:

```yaml
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  name: postgres-monitor
  namespace: postgres
spec:
  selector:
    matchLabels:
      app: postgres
  endpoints:
  - port: metrics
    interval: 30s
    path: /metrics
```

### 10.2 Grafana仪表板配置

**Grafana数据源配置**:

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: grafana-datasource
  namespace: monitoring
data:
  datasource.yaml: |
    apiVersion: 1
    datasources:
    - name: Prometheus
      type: prometheus
      access: proxy
      url: http://prometheus:9090
      isDefault: true
```

**Grafana仪表板配置**:

```json
{
  "dashboard": {
    "title": "PostgreSQL云原生监控",
    "panels": [
      {
        "title": "容器CPU使用率",
        "targets": [
          {
            "expr": "rate(container_cpu_usage_seconds_total{name=\"postgres\"}[5m]) * 100"
          }
        ]
      },
      {
        "title": "容器内存使用",
        "targets": [
          {
            "expr": "container_memory_usage_bytes{name=\"postgres\"}"
          }
        ]
      },
      {
        "title": "PostgreSQL连接数",
        "targets": [
          {
            "expr": "pg_stat_database_numbackends{datname=\"mydb\"}"
          }
        ]
      },
      {
        "title": "查询QPS",
        "targets": [
          {
            "expr": "rate(pg_stat_database_xact_commit[5m]) + rate(pg_stat_database_xact_rollback[5m])"
          }
        ]
      }
    ]
  }
}
```

### 10.3 告警规则配置

**Prometheus告警规则**:

```yaml
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: postgres-cloudnative-alerts
  namespace: monitoring
spec:
  groups:
  - name: postgres_cloudnative
    interval: 30s
    rules:
    - alert: PostgreSQLContainerDown
      expr: up{job="postgres"} == 0
      for: 5m
      labels:
        severity: critical
      annotations:
        summary: "PostgreSQL container is down"
        description: "PostgreSQL container {{ $labels.instance }} is down"

    - alert: PostgreSQLHighCPU
      expr: rate(container_cpu_usage_seconds_total{name="postgres"}[5m]) * 100 > 80
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL CPU usage is high"
        description: "CPU usage is {{ $value }}%"

    - alert: PostgreSQLHighMemory
      expr: container_memory_usage_bytes{name="postgres"} / container_spec_memory_limit_bytes{name="postgres"} > 0.9
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL memory usage is high"
        description: "Memory usage is {{ $value | humanizePercentage }}"
```

---

## 11. 后续整合任务

### 相关文档

#### 部署架构

- ⭐⭐⭐ [Docker 部署指南](./05.12-Docker部署.md) - Docker部署
- ⭐⭐⭐ [Kubernetes 部署指南](./05.13-Kubernetes部署.md) - Kubernetes部署
- ⭐⭐ [Serverless部署](./05.15-Serverless部署.md) - Serverless架构
- ⭐⭐ [集群部署与高可用](../11-部署架构/集群部署/05.04-集群部署与高可用.md) - 高可用架构

#### 核心课程

- ⭐⭐ [系统架构与设计原理](../01-核心基础/01.02-系统架构/01.01-系统架构与设计原理.md) - 系统架构基础
- ⭐⭐ [存储管理与数据持久化](../04-存储与恢复/01.06-存储管理与数据持久化.md) - 存储管理

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../12-监控与诊断/06.01-监控与诊断.md) - 系统监控
- ⭐⭐ [备份与恢复](../13-高可用架构/备份与恢复/06.06-备份与恢复.md) - 备份恢复

#### 前沿技术

- ⭐⭐ [AI与机器学习](../00-归档-项目管理文档/README.md) - Serverless架构与分支功能

---

**文档版本**: v3.0
**最后更新**: 2025-01-16
**维护者**: PostgreSQL Documentation Team
