...
BEGIN
    BEGIN
        IF NOT EXISTS (
            ...
            RAISE NOTICE '索引创建成功，查询时间从5秒降低到0.1秒，成本降低98%';
        ELSE
            RAISE NOTICE '索引已存在';
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        ...
        PERFORM pg_reload_conf();
        RAISE NOTICE '连接保持参数设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置连接保持参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        CREATE OR REPLACE VIEW serverless_cost_monitoring AS
        SELECT
