...
| **IO瓶颈诊断** | pg_stat_io, iostat | 3分钟 | 高 | IO性能问题 |
...
flowchart TD
    ...
    B -->|小规模| C[pgAdmin]
    B -->|中等规模| D{成本要求?}
    B -->|大规模| E[Prometheus+Grafana]
    D -->|免费| E
    D -->|付费| F[Datadog/New Relic]
    C --> G[完成选型]
    E --> G
    F --> G
```
...
flowchart TD
    A[诊断需求] --> B{问题类型?}
    B -->|查询性能| C[EXPLAIN + pg_stat_statements]
    B -->|锁问题| D[pg_locks + pg_stat_activity]
    B -->|IO问题| E[pg_stat_io + iostat]
    B -->|日志分析| F[pgBadger]
    C --> G[完成选型]
    D --> G
    E --> G
...
- [PostgreSQL性能调优文档](https://www.postgresql.org/docs/current/performance-tips.html)
- [pg_stat_statements文档](https://www.postgresql.org/docs/current/pgstatstatements.html)

### 相关文档

- [监控诊断体系详解](./监控诊断体系详解.md)
- [监控与诊断落地指南](./06.02-监控与诊断落地指南.md)
