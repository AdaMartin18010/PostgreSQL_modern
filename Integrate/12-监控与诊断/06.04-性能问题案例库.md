...
> **📋 文档来源**: `PostgreSQL\06-运维实践\监控与诊断\06.04-性能问题案例库.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'pg_stat_statements扩展未安装，请先安装';
    END IF;
    ...
    RAISE NOTICE '找到 % 个慢查询（平均执行时间>500ms）', slow_query_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_stat_statements视图不存在，请先安装扩展';
    WHEN OTHERS THEN
        RAISE EXCEPTION '识别慢查询失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '表orders不存在，无法执行EXPLAIN分析';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE WARNING '表users不存在，无法执行EXPLAIN分析';
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE WARNING '表products不存在，无法执行EXPLAIN分析';
        RETURN;
    END IF;

    RAISE NOTICE '开始执行EXPLAIN分析';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'EXPLAIN分析准备失败: %', SQLERRM;
END $$;
...
WHERE o.created_at >= '2024-01-01' AND o.status = 'pending';
-- 执行时间: 取决于数据量和索引情况
-- 计划: 可能使用Seq Scan、Index Scan或Hash Join
...
BEGIN
    ...
    RAISE NOTICE '找到 % 条统计信息', stats_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stats视图不存在';
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '列created_at不存在';
    END IF;
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE NOTICE '已删除现有扩展统计: orders_created_status_dependencies';
    END IF;
    ...
    RAISE NOTICE '扩展统计创建成功: orders_created_status_dependencies';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '扩展统计orders_created_status_dependencies已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_column THEN
        RAISE EXCEPTION '列created_at或status不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建扩展统计失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF EXISTS (
        ...
        AND indexname = 'idx_orders_created_status'
    ) THEN
        RAISE NOTICE '索引idx_orders_created_status已存在';
        RETURN;
    END IF;

    RAISE NOTICE '开始创建索引（CONCURRENTLY模式，不会阻塞表）';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN OTHERS THEN
...
-- 注意：CONCURRENTLY模式创建索引不会阻塞表，但需要表上无未提交事务
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING '表orders不存在，无法验证优化效果';
        RETURN;
    END IF;

    RAISE NOTICE '开始验证优化效果';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '验证优化效果准备失败: %', SQLERRM;
END $$;
...
-- 执行时间: <150ms（优化后）
-- 计划: Index Scan using idx_orders_created_status
```
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING 'pg_stat_statements扩展未安装，无法监控验证';
        RETURN;
    END IF;
    ...
    RAISE NOTICE '找到 % 条相关查询', query_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控验证失败: %', SQLERRM;
END $$;
...
```sql
-- 1. 定期ANALYZE（通过cron或pg_cron，带错误处理）
-- 每天凌晨2点执行
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'pg_cron扩展未安装，请先安装';
    END IF;

    IF EXISTS (
        SELECT 1 FROM cron.job
        WHERE jobname = 'analyze-orders'
    ) THEN
        PERFORM cron.unschedule('analyze-orders');
        RAISE NOTICE '已删除现有定时任务: analyze-orders';
    END IF;

    PERFORM cron.schedule('analyze-orders', '0 2 * * *', 'ANALYZE orders;');
    RAISE NOTICE '定时任务创建成功: analyze-orders（每天凌晨2点执行）';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION 'pg_cron扩展未安装，请先安装';
    WHEN undefined_table THEN
        RAISE EXCEPTION 'cron.job表不存在，请先安装pg_cron扩展';
    WHEN insufficient_privilege THEN
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '已删除现有函数: set_column_statistics';
    END IF;
    ...
    RAISE NOTICE '函数创建成功: set_column_statistics';
EXCEPTION
    WHEN duplicate_function THEN
        RAISE WARNING '函数set_column_statistics已存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
...
-- 3. 监控统计信息准确性（带错误处理和性能测试）
DO $$
DECLARE
    stale_table_count INT;
BEGIN
    SELECT COUNT(*) INTO stale_table_count
    FROM pg_stat_user_tables
    WHERE last_analyze < NOW() - interval '7 days'
    OR last_autoanalyze < NOW() - interval '7 days';

    IF stale_table_count > 0 THEN
        RAISE WARNING '发现 % 个表的统计信息超过7天未更新', stale_table_count;
    ELSE
        RAISE NOTICE '所有表的统计信息都在7天内更新过';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控统计信息准确性失败: %', SQLERRM;
END $$;
...
| `EXPLAIN (ANALYZE, BUFFERS, TIMING)` | 执行计划分析 | 案例1, 案例4, 案例5, 案例6 |
...
- ⭐⭐⭐ [监控与诊断落地指南](./06.01-监控与诊断.md) - Prometheus + Grafana部署
- ⭐⭐⭐ [性能调优变更闭环](../30-性能调优/README.md) - 变更管理流程
- ⭐⭐ [执行计划与性能调优](../02-查询与优化/02.03-执行计划/02.04-执行计划与性能调优.md) - 查询优化
- ⭐⭐ [索引结构与优化](../02-查询与优化/02.02-索引结构/02.02-索引结构与优化.md) - 索引优化
- ⭐⭐ [统计信息与代价模型](../02-查询与优化/02.04-统计信息/02.03-统计信息与代价模型.md) - 统计信息
- ⭐⭐ [性能调优实践](../11-部署架构/单机部署/05.02-性能调优实践.md) - 性能调优指南
