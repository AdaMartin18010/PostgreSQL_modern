...
- [监控与诊断](#监控与诊断)
  ...
  - [2. PostgreSQL 18新增监控特性](#2-postgresql-18新增监控特性)
    ...
    - [2.4 PostgreSQL 18自我监测增强](#24-postgresql-18自我监测增强)
      - [2.4.1 pg\_stat\_io增强监控](#241-pg_stat_io增强监控)
      - [2.4.2 后端I/O追踪](#242-后端io追踪)
      - [2.4.3 连接性能监测](#243-连接性能监测)
...
### 2.4 PostgreSQL 18自我监测增强

**PostgreSQL 18** 显著增强了自我监测能力，通过内置的统计信息和监控视图，实现数据库的完全自我监测。

#### 2.4.1 pg_stat_io增强监控

PostgreSQL 18的pg_stat_io视图新增了read_bytes、write_bytes和extend_bytes列：
...
PostgreSQL 18新增pg_stat_get_backend_io()函数：
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '调整autovacuum参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
