...
- [PostgreSQL系统级调优指南](#postgresql系统级调优指南)
  ...
  - [5. CPU调优](#5-cpu调优)
    ...
    - [5.2 并行查询](#52-并行查询)
    - [5.3 PostgreSQL 18并行查询系统级优化](#53-postgresql-18并行查询系统级优化)
      - [5.3.1 并行查询系统配置](#531-并行查询系统配置)
      - [5.3.2 并行查询性能监控](#532-并行查询性能监控)
  - [6. 最佳实践](#6-最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [⚠️ 注意事项](#️-注意事项)
...
### 5.3 PostgreSQL 18并行查询系统级优化

**PostgreSQL 18** 显著增强了并行查询的系统级支持，通过系统级优化进一步提升并行查询性能。
...
BEGIN
    BEGIN
        ...
        -- 计算并行参数
        max_parallel_workers := LEAST(cpu_cores, 8);
        max_parallel_workers_per_gather := LEAST(cpu_cores / 2, 4);
        max_parallel_maintenance_workers := LEAST(cpu_cores / 4, 4);

        RAISE NOTICE '=== PostgreSQL 18并行查询系统配置 ===';
        RAISE NOTICE 'CPU核心数: %', cpu_cores;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行查询系统配置优化失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
```sql
-- PostgreSQL 18 并行查询性能监控（带错误处理和性能测试）
DO $$
DECLARE
    parallel_query_record RECORD;
BEGIN
    BEGIN
        RAISE NOTICE '=== PostgreSQL 18并行查询性能监控 ===';

        -- 查看并行查询统计
        FOR parallel_query_record IN
            SELECT
                datname,
                numbackends,
                xact_commit,
            ...
            WHERE datname = current_database()
        LOOP
            RAISE NOTICE '数据库: % | 连接数: % | 提交: % | 回滚: % | 缓存命中率: %%',
                parallel_query_record.datname,
                parallel_query_record.numbackends,
                parallel_query_record.xact_commit,
                parallel_query_record.xact_rollback,
                parallel_query_record.cache_hit_ratio;
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行查询性能监控失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
