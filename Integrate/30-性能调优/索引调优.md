...
- [PostgreSQL索引调优指南](#postgresql索引调优指南)
  ...
  - [6. PostgreSQL 18索引优化新特性](#6-postgresql-18索引优化新特性)
    ...
    - [6.3 PostgreSQL 18并行索引操作增强](#63-postgresql-18并行索引操作增强)
      - [6.3.1 并行索引构建](#631-并行索引构建)
      - [6.3.2 并行索引扫描](#632-并行索引扫描)
  - [7. 最佳实践](#7-最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [⚠️ 注意事项](#️-注意事项)
  - [📚 相关文档](#-相关文档)
...
### 6.3 PostgreSQL 18并行索引操作增强

**PostgreSQL 18** 显著增强了并行索引操作能力，支持并行索引构建、并行索引扫描和并行索引维护，大幅提升大规模索引操作的性能。
...
-- 并行索引构建
CREATE INDEX CONCURRENTLY idx_large_table_parallel ON large_table(column1, column2);
-- PostgreSQL 18: 并行索引构建速度提升2-4倍
```
...
ORDER BY column2
LIMIT 1000;
-- PostgreSQL 18: 并行索引扫描速度提升1.5-3倍
```
