# 9.1 PostgreSQL 18故障排查流程与脚本

> **所属主题**: 09-故障排查
> **章节编号**: 9.1
> **创建日期**: 2025年1月
> **PostgreSQL版本**: 18+
> **难度等级**: ⭐⭐⭐⭐
> **相关章节**: [8.3 混合负载系统优化](../08-性能调优案例/03-混合负载系统优化.md) | [10.1 推荐做法与注意事项](../10-最佳实践/01-推荐做法与注意事项.md)

---

## 📋 目录

- [9.1 PostgreSQL 18故障排查流程与脚本](#91-postgresql-18故障排查流程与脚本)
  - [📋 目录](#-目录)
  - [9.1.1 概述与背景](#911-概述与背景)
  - [9.1.2 故障排查流程](#912-故障排查流程)
    - [9.1.2.1 系统化故障排查流程](#9121-系统化故障排查流程)
  - [9.1.3 故障排查脚本库](#913-故障排查脚本库)
    - [9.1.3.1 PostgreSQL 18故障排查函数](#9131-postgresql-18故障排查函数)
  - [9.1.4 故障类型说明](#914-故障类型说明)
    - [9.1.4.1 性能问题排查](#9141-性能问题排查)
    - [9.1.4.2 连接问题排查](#9142-连接问题排查)
    - [9.1.4.3 锁问题排查](#9143-锁问题排查)
    - [9.1.4.4 I/O问题排查（PostgreSQL 18增强）](#9144-io问题排查postgresql-18增强)
  - [9.1.5 故障排查步骤](#915-故障排查步骤)
    - [9.1.5.1 步骤1: 问题分类](#9151-步骤1-问题分类)
    - [9.1.5.2 步骤2: 执行诊断](#9152-步骤2-执行诊断)
    - [9.1.5.3 步骤3: 问题定位](#9153-步骤3-问题定位)
    - [9.1.5.4 步骤4: 解决方案实施](#9154-步骤4-解决方案实施)
    - [9.1.5.5 步骤5: 效果验证](#9155-步骤5-效果验证)
  - [9.1.6 注意事项与最佳实践](#916-注意事项与最佳实践)
    - [9.1.6.1 PostgreSQL 18增强特性](#9161-postgresql-18增强特性)
    - [9.1.6.2 注意事项](#9162-注意事项)
    - [9.1.6.3 最佳实践](#9163-最佳实践)
  - [9.1.7 导航](#917-导航)
    - [9.1.7.1 章节导航](#9171-章节导航)
    - [9.1.7.2 相关章节](#9172-相关章节)
  - [📚 相关资源](#-相关资源)

---

## 9.1.1 概述与背景

PostgreSQL 18故障排查流程提供了系统化的故障诊断方法，包括性能问题、连接问题、锁问题、I/O问题等的诊断流程和脚本。

---

## 9.1.2 故障排查流程

### 9.1.2.1 系统化故障排查流程

```mermaid
flowchart TD
    A[故障报告] --> B{问题类型?}
    B -->|性能问题| C[性能诊断]
    B -->|连接问题| D[连接诊断]
    B -->|锁问题| E[锁诊断]
    B -->|I/O问题| F[I/O诊断]

    C --> C1[使用pg_stat_statements]
    C --> C2[使用EXPLAIN增强]
    C --> C3[检查并行查询效率]

    D --> D1[检查pg_stat_activity]
    D --> D2[检查log_connections]
    D --> D3[检查连接池配置]

    E --> E1[检查pg_locks]
    E --> E2[检查锁等待]
    E --> E3[检查死锁]

    F --> F1[检查pg_stat_io]
    F --> F2[使用pg_stat_get_backend_io]
    F --> F3[检查异步I/O配置]

    C1 --> G[问题定位]
    C2 --> G
    C3 --> G
    D1 --> G
    D2 --> G
    D3 --> G
    E1 --> G
    E2 --> G
    E3 --> G
    F1 --> G
    F2 --> G
    F3 --> G

    G --> H[解决方案实施]
    H --> I[效果验证]
```

---

## 9.1.3 故障排查脚本库

### 9.1.3.1 PostgreSQL 18故障排查函数

```sql
-- PostgreSQL 18 故障排查脚本库
CREATE OR REPLACE FUNCTION pg18_troubleshoot(issue_type TEXT)
RETURNS TABLE(
    check_item TEXT,
    check_result TEXT,
    status TEXT,
    recommendation TEXT
) AS $$
DECLARE
    pg_version int;
BEGIN
    SELECT current_setting('server_version_num')::int INTO pg_version;

    IF pg_version < 180000 THEN
        RAISE WARNING 'PostgreSQL 18故障排查需要PostgreSQL 18+';
        RETURN;
    END IF;

    CASE issue_type
        WHEN 'performance' THEN
            -- 性能问题排查
            RETURN QUERY
            SELECT
                '慢查询检查',
                COUNT(*)::TEXT || ' 个慢查询',
                CASE WHEN COUNT(*) > 10 THEN 'warning' ELSE 'normal' END,
                CASE WHEN COUNT(*) > 10 THEN '建议优化慢查询' ELSE '正常' END
            FROM pg_stat_statements
            WHERE mean_exec_time > 1000
            AND EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements')
            UNION ALL
            SELECT
                '缓存命中率',
                ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2)::TEXT || '%',
                CASE
                    WHEN ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) >= 95 THEN 'normal'
                    WHEN ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) >= 90 THEN 'warning'
                    ELSE 'critical'
                END,
                CASE
                    WHEN ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) < 90 THEN '建议增加shared_buffers'
                    ELSE '正常'
                END
            FROM pg_stat_database
            WHERE datname = current_database();

        WHEN 'connection' THEN
            -- 连接问题排查
            RETURN QUERY
            SELECT
                '连接数',
                COUNT(*)::TEXT || ' / ' || (SELECT setting FROM pg_settings WHERE name = 'max_connections'),
                CASE
                    WHEN COUNT(*) > (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') * 0.9 THEN 'critical'
                    WHEN COUNT(*) > (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') * 0.8 THEN 'warning'
                    ELSE 'normal'
                END,
                CASE
                    WHEN COUNT(*) > (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') * 0.9 THEN '连接数接近上限，建议检查连接池'
                    WHEN COUNT(*) > (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') * 0.8 THEN '连接数较高，建议监控'
                    ELSE '正常'
                END
            FROM pg_stat_activity
            WHERE datname = current_database()
            UNION ALL
            SELECT
                '空闲事务',
                COUNT(*)::TEXT || ' 个',
                CASE WHEN COUNT(*) > 10 THEN 'warning' ELSE 'normal' END,
                CASE WHEN COUNT(*) > 10 THEN '建议检查应用连接管理' ELSE '正常' END
            FROM pg_stat_activity
            WHERE datname = current_database() AND state = 'idle in transaction';

        WHEN 'lock' THEN
            -- 锁问题排查
            RETURN QUERY
            SELECT
                '锁等待',
                COUNT(*)::TEXT || ' 个',
                CASE WHEN COUNT(*) > 0 THEN 'warning' ELSE 'normal' END,
                CASE WHEN COUNT(*) > 0 THEN '建议检查锁等待查询' ELSE '正常' END
            FROM pg_catalog.pg_locks blocked_locks
            JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
            WHERE NOT blocked_locks.granted
            UNION ALL
            SELECT
                '死锁数',
                deadlocks::TEXT || ' 个',
                CASE WHEN deadlocks > 0 THEN 'critical' ELSE 'normal' END,
                CASE WHEN deadlocks > 0 THEN '检测到死锁，建议检查事务管理' ELSE '正常' END
            FROM pg_stat_database
            WHERE datname = current_database();

        WHEN 'io' THEN
            -- I/O问题排查（PostgreSQL 18增强）
            RETURN QUERY
            SELECT
                'I/O吞吐量',
                ROUND(SUM(read_bytes + write_bytes)::numeric / 1024 / 1024 / 1024, 2)::TEXT || ' GB',
                CASE
                    WHEN SUM(reads + writes) > 10000000 THEN 'warning'
                    ELSE 'normal'
                END,
                CASE
                    WHEN SUM(reads + writes) > 10000000 THEN 'I/O负载较高，建议检查查询和索引'
                    ELSE '正常'
                END
            FROM pg_stat_io
            UNION ALL
            SELECT
                '异步I/O配置',
                COALESCE((SELECT setting FROM pg_settings WHERE name = 'io_method'), 'sync'),
                CASE
                    WHEN (SELECT setting FROM pg_settings WHERE name = 'io_method') IN ('worker', 'io_uring') THEN 'normal'
                    ELSE 'warning'
                END,
                CASE
                    WHEN (SELECT setting FROM pg_settings WHERE name = 'io_method') NOT IN ('worker', 'io_uring') THEN '建议启用PostgreSQL 18异步I/O'
                    ELSE '正常'
                END;

        ELSE
            RAISE WARNING '未知的故障类型: %', issue_type;
    END CASE;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM pg18_troubleshoot('performance');
SELECT * FROM pg18_troubleshoot('connection');
SELECT * FROM pg18_troubleshoot('lock');
SELECT * FROM pg18_troubleshoot('io');
```

---

## 9.1.4 故障类型说明

### 9.1.4.1 性能问题排查

**检查项**：

- 慢查询检查
- 缓存命中率

**诊断工具**：

- pg_stat_statements（PostgreSQL 18增强）
- EXPLAIN增强
- 并行查询效率分析

### 9.1.4.2 连接问题排查

**检查项**：

- 连接数
- 空闲事务

**诊断工具**：

- pg_stat_activity
- log_connections（PostgreSQL 18增强）
- 连接池配置

### 9.1.4.3 锁问题排查

**检查项**：

- 锁等待
- 死锁数

**诊断工具**：

- pg_locks
- pg_stat_activity
- 锁等待检测脚本

### 9.1.4.4 I/O问题排查（PostgreSQL 18增强）

**检查项**：

- I/O吞吐量
- 异步I/O配置

**诊断工具**：

- pg_stat_io（PostgreSQL 18增强）
- pg_stat_get_backend_io()（PostgreSQL 18新增）
- 异步I/O配置检查

---

## 9.1.5 故障排查步骤

### 9.1.5.1 步骤1: 问题分类

根据故障现象确定问题类型：

- 性能问题：响应时间慢、吞吐量低
- 连接问题：连接数过高、连接超时
- 锁问题：查询阻塞、死锁
- I/O问题：I/O等待、磁盘瓶颈

### 9.1.5.2 步骤2: 执行诊断

使用对应的故障排查函数：

```sql
-- 性能问题
SELECT * FROM pg18_troubleshoot('performance');

-- 连接问题
SELECT * FROM pg18_troubleshoot('connection');

-- 锁问题
SELECT * FROM pg18_troubleshoot('lock');

-- I/O问题
SELECT * FROM pg18_troubleshoot('io');
```

### 9.1.5.3 步骤3: 问题定位

根据诊断结果定位具体问题：

- 查看check_result了解当前状态
- 查看status了解严重程度
- 查看recommendation了解解决建议

### 9.1.5.4 步骤4: 解决方案实施

根据recommendation实施解决方案：

- 优化慢查询
- 调整连接池配置
- 解决锁等待
- 优化I/O配置

### 9.1.5.5 步骤5: 效果验证

验证解决方案效果：

- 重新执行故障排查
- 对比优化前后的指标
- 确认问题已解决

---

## 9.1.6 注意事项与最佳实践

### 9.1.6.1 PostgreSQL 18增强特性

1. **I/O统计增强**：使用pg_stat_io的字节级别统计
2. **后端I/O追踪**：使用pg_stat_get_backend_io()进行后端级别追踪
3. **连接性能监测**：使用log_connections细粒度配置
4. **并行查询追踪**：使用pg_stat_statements的并行查询列

### 9.1.6.2 注意事项

⚠️ **重要提醒**：

1. **诊断频率**：合理设置诊断频率，避免过度诊断影响系统性能
2. **诊断影响**：诊断本身会有性能开销，注意对生产环境的影响
3. **结果分析**：综合分析多个指标，避免单一指标误判

### 9.1.6.3 最佳实践

✅ **推荐做法**：

1. **系统化诊断**：按照故障排查流程系统化诊断
2. **多维度分析**：结合多个指标综合判断问题
3. **及时记录**：记录诊断过程和结果，便于后续分析
4. **持续监控**：建立持续监控机制，及时发现新问题

---

## 9.1.7 导航

### 9.1.7.1 章节导航

- **上一节**：[8.3 混合负载系统优化](../08-性能调优案例/03-混合负载系统优化.md)
- **下一节**：[10.1 推荐做法与注意事项](../10-最佳实践/01-推荐做法与注意事项.md)
- **返回主题目录**：[09-故障排查](./README.md)
- **返回主文档**：[PostgreSQL-18-自动化运维与自我监测](../README.md)

### 9.1.7.2 相关章节

- [4.1 自动慢查询检测](../04-自动化诊断/01-自动慢查询检测.md) - 慢查询检测
- [4.2 自动锁等待检测](../04-自动化诊断/02-自动锁等待检测.md) - 锁等待检测
- [4.3 自动资源瓶颈检测](../04-自动化诊断/03-自动资源瓶颈检测.md) - 资源瓶颈检测
- [6.5 故障自动恢复](../06-综合方案/05-故障自动恢复.md) - 故障自动恢复

---

## 📚 相关资源

- [4.1 自动慢查询检测](../04-自动化诊断/01-自动慢查询检测.md)
- [4.2 自动锁等待检测](../04-自动化诊断/02-自动锁等待检测.md)
- [4.3 自动资源瓶颈检测](../04-自动化诊断/03-自动资源瓶颈检测.md)
- [PostgreSQL故障排查最佳实践](../10-最佳实践/01-推荐做法与注意事项.md)

---

**最后更新**: 2025年1月
**文档版本**: v2.0（已添加完整目录、章节编号、详细内容）
