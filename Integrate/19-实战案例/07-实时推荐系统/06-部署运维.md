---
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\07-实时推荐系统\06-部署运维.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变
---
> **⚠️ 重要提示**: 本文档遵循案例文档通用模板格式。
>
> **推荐阅读**:
>
> - [案例文档通用模板](../案例文档通用模板.md) - 通用案例文档格式和最佳实践
>
> 本文档保留作为实时推荐系统的部署运维参考。
---

# 案例7：实时推荐系统 - 部署运维

## 元数据

- **创建日期**: 2025-12-22
- **部署方式**: Docker + Kubernetes
- **监控**: Prometheus + Grafana
- **缓存**: Redis Cluster
- **消息队列**: Kafka

---

## 1. Docker部署配置

### 1.1 Docker Compose配置

**Docker Compose配置（带Redis和Kafka）**：

```yaml
version: '3.8'

services:
  postgres:
    image: pgvector/pgvector:pg18
    container_name: recommendation-db
    environment:
      POSTGRES_DB: recommendation_db
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: ${DB_PASSWORD}
    volumes:
      - postgres-data:/var/lib/postgresql/data
      - ./init.sql:/docker-entrypoint-initdb.d/init.sql
      - ./postgresql.conf:/etc/postgresql/postgresql.conf
    ports:
      - "5432:5432"
    command:
      - "postgres"
      - "-c"
      - "config_file=/etc/postgresql/postgresql.conf"
      - "-c"
      - "shared_buffers=4GB"
      - "-c"
      - "work_mem=256MB"
      - "-c"
      - "max_connections=300"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 10s
      timeout: 5s
      retries: 5
    restart: unless-stopped

  redis:
    image: redis:7-alpine
    container_name: recommendation-redis
    ports:
      - "6379:6379"
    volumes:
      - redis-data:/data
    command: redis-server --appendonly yes --maxmemory 8gb --maxmemory-policy allkeys-lru
    healthcheck:
      test: ["CMD", "redis-cli", "ping"]
      interval: 10s
      timeout: 5s
      retries: 5
    restart: unless-stopped

  kafka:
    image: confluentinc/cp-kafka:latest
    container_name: recommendation-kafka
    environment:
      KAFKA_BROKER_ID: 1
      KAFKA_ZOOKEEPER_CONNECT: zookeeper:2181
      KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka:9092
      KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR: 1
    ports:
      - "9092:9092"
    depends_on:
      - zookeeper
    restart: unless-stopped

  zookeeper:
    image: confluentinc/cp-zookeeper:latest
    container_name: recommendation-zookeeper
    environment:
      ZOOKEEPER_CLIENT_PORT: 2181
      ZOOKEEPER_TICK_TIME: 2000
    ports:
      - "2181:2181"
    restart: unless-stopped

  api:
    build:
      context: ./api
      dockerfile: Dockerfile
    container_name: recommendation-api
    environment:
      DB_HOST: postgres
      DB_NAME: recommendation_db
      DB_USER: postgres
      DB_PASSWORD: ${DB_PASSWORD}
      REDIS_HOST: redis
      REDIS_PORT: 6379
      KAFKA_BOOTSTRAP_SERVERS: kafka:9092
    ports:
      - "8007:8007"
    depends_on:
      - postgres
      - redis
      - kafka
    restart: unless-stopped

volumes:
  postgres-data:
  redis-data:
```

### 1.2 PostgreSQL优化配置

**PostgreSQL优化配置（PostgreSQL 18特性）**：

```ini
# postgresql.conf - 实时推荐系统优化

# 内存配置
shared_buffers = 4GB
work_mem = 256MB
maintenance_work_mem = 1GB
effective_cache_size = 12GB

# 连接配置
max_connections = 300
superuser_reserved_connections = 3

# WAL配置
wal_buffers = 32MB
max_wal_size = 8GB
min_wal_size = 2GB
checkpoint_timeout = 20min
checkpoint_completion_target = 0.9

# PostgreSQL 18异步I/O
io_direct = data
io_combine_limit = 256kB

# 注意：PostgreSQL 18没有内置连接池
# 建议使用pgBouncer进行连接池管理
# pgBouncer配置请参考单独的pgbouncer.ini配置文件

# 查询优化
random_page_cost = 1.1
effective_io_concurrency = 200
default_statistics_target = 100

# pgvector配置
shared_preload_libraries = 'vector'

# 并行查询（PostgreSQL 18）
max_parallel_workers_per_gather = 4
max_parallel_workers = 8
parallel_tuple_cost = 0.01
parallel_setup_cost = 1000

# 日志配置
log_destination = 'stderr'
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_duration_statement = 1000  # 记录>1s的查询

# 统计信息
track_io_timing = on
track_functions = all
pg_stat_statements.track = all
```

---

## 2. 缓存策略

### 2.1 Redis缓存配置

**Redis缓存配置（带错误处理和性能测试）**：

```python
# redis_config.py
import redis
from redis.sentinel import Sentinel

# Redis连接池
redis_pool = redis.ConnectionPool(
    host='redis',
    port=6379,
    max_connections=50,
    decode_responses=True
)

redis_client = redis.Redis(connection_pool=redis_pool)

# 缓存键策略
class CacheKeys:
    USER_PROFILE = "user:profile:{user_id}"
    ITEM_FEATURES = "item:features:{item_id}"
    USER_CF_REC = "rec:user_cf:{user_id}"
    ITEM_CF_REC = "rec:item_cf:{item_id}"
    VECTOR_REC = "rec:vector:{user_id}"
    HOT_ITEMS = "rec:hot:items"

    @staticmethod
    def get_ttl(key_type: str) -> int:
        """获取缓存TTL（秒）"""
        ttl_map = {
            'user_profile': 3600,      # 1小时
            'item_features': 7200,      # 2小时
            'user_cf': 1800,            # 30分钟
            'item_cf': 1800,            # 30分钟
            'vector': 1800,             # 30分钟
            'hot_items': 300,           # 5分钟
        }
        return ttl_map.get(key_type, 1800)

# 缓存装饰器
from functools import wraps
import json
import hashlib

def cache_result(key_template: str, ttl: int = 1800):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # 生成缓存键
            cache_key = key_template.format(*args, **kwargs)

            # 尝试从缓存获取
            cached = redis_client.get(cache_key)
            if cached:
                return json.loads(cached)

            # 执行函数
            result = func(*args, **kwargs)

            # 写入缓存
            redis_client.setex(cache_key, ttl, json.dumps(result))

            return result
        return wrapper
    return decorator
```

---

## 3. 消息队列配置

### 3.1 Kafka主题配置

**Kafka主题配置（带错误处理和性能测试）**：

```bash
# 创建Kafka主题
docker exec recommendation-kafka kafka-topics \
  --create \
  --bootstrap-server localhost:9092 \
  --replication-factor 1 \
  --partitions 3 \
  --topic user-behavior

docker exec recommendation-kafka kafka-topics \
  --create \
  --bootstrap-server localhost:9092 \
  --replication-factor 1 \
  --partitions 3 \
  --topic item-updates

# 查看主题列表
docker exec recommendation-kafka kafka-topics \
  --list \
  --bootstrap-server localhost:9092
```

### 3.2 Kafka消费者配置

**Kafka消费者配置（带错误处理和性能测试）**：

```python
# kafka_consumer.py
from kafka import KafkaConsumer
import json
import logging

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

class BehaviorConsumer:
    """用户行为消费者"""

    def __init__(self, bootstrap_servers: list):
        self.consumer = KafkaConsumer(
            'user-behavior',
            bootstrap_servers=bootstrap_servers,
            value_deserializer=lambda m: json.loads(m.decode('utf-8')),
            group_id='recommendation-service',
            auto_offset_reset='latest',
            enable_auto_commit=True,
            auto_commit_interval_ms=1000
        )

    def consume(self):
        """消费消息"""
        try:
            for message in self.consumer:
                behavior = message.value
                logger.info(f"收到行为数据: {behavior}")

                # 处理行为数据
                self.process_behavior(behavior)

        except Exception as e:
            logger.error(f"消费消息失败: {e}")
            raise

    def process_behavior(self, behavior: dict):
        """处理行为数据"""
        user_id = behavior['user_id']
        item_id = behavior['item_id']
        action = behavior['action']  # view, click, purchase

        # 更新用户画像
        self.update_user_profile(user_id, item_id, action)

        # 更新物品特征
        self.update_item_features(item_id, action)

        # 清除相关缓存
        self.invalidate_cache(user_id, item_id)

    def update_user_profile(self, user_id: int, item_id: int, action: str):
        """更新用户画像"""
        # 实现用户画像更新逻辑
        pass

    def update_item_features(self, item_id: int, action: str):
        """更新物品特征"""
        # 实现物品特征更新逻辑
        pass

    def invalidate_cache(self, user_id: int, item_id: int):
        """清除缓存"""
        cache_keys = [
            f"rec:user_cf:{user_id}",
            f"rec:item_cf:{item_id}",
            f"rec:vector:{user_id}"
        ]
        for key in cache_keys:
            redis_client.delete(key)
```

---

## 4. 监控配置

### 4.1 Prometheus配置

**Prometheus配置（带错误处理和性能测试）**：

```yaml
# prometheus.yml
global:
  scrape_interval: 15s
  evaluation_interval: 15s

scrape_configs:
  - job_name: 'postgres'
    static_configs:
      - targets: ['postgres-exporter:9187']
    metrics_path: /metrics

  - job_name: 'recommendation-api'
    static_configs:
      - targets: ['api:8007']
    metrics_path: /metrics

  - job_name: 'redis'
    static_configs:
      - targets: ['redis-exporter:9121']
    metrics_path: /metrics

  - job_name: 'kafka'
    static_configs:
      - targets: ['kafka-exporter:9308']
    metrics_path: /metrics
```

### 4.2 Grafana仪表板

**Grafana仪表板配置（带错误处理和性能测试）**：

```json
{
  "dashboard": {
    "title": "实时推荐系统监控",
    "panels": [
      {
        "title": "推荐QPS",
        "targets": [
          {
            "expr": "rate(recommendation_requests_total[5m])"
          }
        ]
      },
      {
        "title": "推荐延迟",
        "targets": [
          {
            "expr": "histogram_quantile(0.95, rate(recommendation_duration_ms_bucket[5m]))"
          }
        ]
      },
      {
        "title": "向量检索延迟",
        "targets": [
          {
            "expr": "histogram_quantile(0.95, rate(vector_search_duration_ms_bucket[5m]))"
          }
        ]
      },
      {
        "title": "缓存命中率",
        "targets": [
          {
            "expr": "rate(redis_hits_total[5m]) / rate(redis_requests_total[5m])"
          }
        ]
      },
      {
        "title": "Kafka消费延迟",
        "targets": [
          {
            "expr": "kafka_consumer_lag_sum"
          }
        ]
      }
    ]
  }
}
```

---

## 5. 备份与恢复

### 5.1 数据库备份

**数据库备份脚本（带错误处理和性能测试）**：

```bash
#!/bin/bash
# backup_recommendation.sh - 实时推荐系统备份

set -e

BACKUP_DIR="/backups/recommendation"
DATE=$(date +%Y%m%d_%H%M%S)
RETENTION_DAYS=30

mkdir -p "$BACKUP_DIR"

# 数据库备份
echo "开始数据库备份..."
pg_dump -h postgres -U postgres -d recommendation_db \
  -F c -b -v -f "$BACKUP_DIR/db_$DATE.dump"

# 验证备份
if [ $? -eq 0 ]; then
    echo "数据库备份成功: $BACKUP_DIR/db_$DATE.dump"

    # 压缩备份
    gzip "$BACKUP_DIR/db_$DATE.dump"

    # 清理旧备份
    find "$BACKUP_DIR" -name "db_*.dump.gz" -mtime +$RETENTION_DAYS -delete

    echo "备份完成，保留最近 $RETENTION_DAYS 天的备份"
else
    echo "数据库备份失败！"
    exit 1
fi
```

---

## 6. 性能调优

### 6.1 数据库性能调优

**数据库性能调优（PostgreSQL 18特性）**：

```sql
-- 1. 分析表统计信息（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            ANALYZE users;
            RAISE NOTICE 'users 表分析完成';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'items') THEN
            ANALYZE items;
            RAISE NOTICE 'items 表分析完成';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behaviors') THEN
            ANALYZE user_behaviors;
            RAISE NOTICE 'user_behaviors 表分析完成';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '分析表统计信息失败: %', SQLERRM;
    END;
END $$;

-- 2. 重建索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_items_embedding') THEN
            REINDEX INDEX CONCURRENTLY idx_items_embedding;
            RAISE NOTICE '索引 idx_items_embedding 重建完成';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_behaviors_user_item') THEN
            REINDEX INDEX CONCURRENTLY idx_user_behaviors_user_item;
            RAISE NOTICE '索引 idx_user_behaviors_user_item 重建完成';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '重建索引失败: %', SQLERRM;
    END;
END $$;

-- 3. 检查慢查询（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING 'pg_stat_statements扩展未安装，无法检查慢查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始检查慢查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查准备失败: %', SQLERRM;
    END;
END $$;

-- 查询慢查询（带性能测试和错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING 'pg_stat_statements扩展未安装，无法查询慢查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询慢查询（>1秒）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time,
    max_exec_time
FROM pg_stat_statements
WHERE mean_exec_time > 1000  -- >1秒
ORDER BY mean_exec_time DESC
LIMIT 20;

-- 4. 检查索引使用情况（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始检查索引使用情况';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查准备失败: %', SQLERRM;
    END;
END $$;

-- 查询索引使用情况（带性能测试和错误处理）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查询索引使用情况（public schema）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan,
    idx_tup_read,
    idx_tup_fetch
FROM pg_stat_user_indexes
WHERE schemaname = 'public'
ORDER BY idx_scan DESC
LIMIT 100;
```

### 6.2 缓存优化

**缓存优化（带错误处理和性能测试）**：

```python
# 1. 预热缓存
def warmup_cache():
    """预热缓存"""
    # 预热热门物品
    hot_items = get_hot_items(limit=1000)
    for item in hot_items:
        cache_key = CacheKeys.ITEM_FEATURES.format(item_id=item['item_id'])
        redis_client.setex(
            cache_key,
            CacheKeys.get_ttl('item_features'),
            json.dumps(item)
        )

    # 预热热门用户推荐
    hot_users = get_active_users(limit=1000)
    for user in hot_users:
        recommendations = generate_recommendations(user['user_id'])
        cache_key = CacheKeys.USER_CF_REC.format(user_id=user['user_id'])
        redis_client.setex(
            cache_key,
            CacheKeys.get_ttl('user_cf'),
            json.dumps(recommendations)
        )

# 2. 批量获取缓存
def batch_get_cache(keys: list) -> dict:
    """批量获取缓存"""
    if not keys:
        return {}

    values = redis_client.mget(keys)
    result = {}
    for key, value in zip(keys, values):
        if value:
            result[key] = json.loads(value)
    return result
```

---

## 7. 故障处理

### 7.1 常见故障处理

**常见故障处理（带错误处理和性能测试）**：

```bash
# 1. 数据库连接失败
docker exec recommendation-db pg_isready -U postgres

# 2. Redis连接失败
docker exec recommendation-redis redis-cli ping

# 3. Kafka消费延迟
docker exec recommendation-kafka kafka-consumer-groups \
  --bootstrap-server localhost:9092 \
  --group recommendation-service \
  --describe

# 4. 向量检索慢
docker exec recommendation-db psql -U postgres -d recommendation_db -c "
    SELECT indexname, idx_scan, idx_tup_read
    FROM pg_stat_user_indexes
    WHERE indexname = 'idx_items_embedding';
"

# 5. 缓存命中率低
# 检查缓存配置
docker exec recommendation-redis redis-cli INFO stats | grep keyspace
```

---

## 8. 扩容策略

### 8.1 水平扩容

**水平扩容策略（带错误处理和性能测试）**：

```yaml
# 增加API副本数
kubectl scale deployment recommendation-api --replicas=5

# 数据库读写分离（使用pgBouncer）
apiVersion: apps/v1
kind: Deployment
metadata:
  name: pgbouncer
spec:
  replicas: 2
  template:
    spec:
      containers:
      - name: pgbouncer
        image: pgbouncer/pgbouncer:latest
        env:
        - name: DATABASES_HOST
          value: postgres
        - name: DATABASES_PORT
          value: "5432"
        - name: DATABASES_DBNAME
          value: recommendation_db
        - name: POOL_MODE
          value: transaction
        - name: MAX_CLIENT_CONN
          value: "1000"
        - name: DEFAULT_POOL_SIZE
          value: "25"
```

### 8.2 Redis集群

**Redis集群配置（带错误处理和性能测试）**：

```yaml
# Redis Sentinel配置
apiVersion: v1
kind: ConfigMap
metadata:
  name: redis-sentinel-config
data:
  sentinel.conf: |
    port 26379
    sentinel monitor mymaster redis-master 6379 2
    sentinel down-after-milliseconds mymaster 5000
    sentinel parallel-syncs mymaster 1
    sentinel failover-timeout mymaster 10000
```

---

## 9. 运维检查清单

### 9.1 日常检查

**日常检查清单（带错误处理和性能测试）**：

```bash
#!/bin/bash
# daily_check_recommendation.sh - 实时推荐系统日常检查

echo "=== 实时推荐系统日常检查 ==="

# 1. 数据库连接检查
echo "1. 检查数据库连接..."
docker exec recommendation-db pg_isready -U postgres

# 2. Redis连接检查
echo "2. 检查Redis连接..."
docker exec recommendation-redis redis-cli ping

# 3. Kafka连接检查
echo "3. 检查Kafka连接..."
docker exec recommendation-kafka kafka-topics --list --bootstrap-server localhost:9092

# 4. 推荐延迟检查
echo "4. 检查推荐延迟..."
curl -s http://localhost:8007/metrics | grep recommendation_duration_ms

# 5. 缓存命中率检查
echo "5. 检查缓存命中率..."
docker exec recommendation-redis redis-cli INFO stats | grep keyspace_hits

# 6. 数据库大小检查
echo "6. 检查数据库大小..."
docker exec recommendation-db psql -U postgres -d recommendation_db -c "
    SELECT pg_size_pretty(pg_database_size('recommendation_db'));
"

echo "=== 检查完成 ==="
```

---

**完成**: 实时推荐系统部署运维
**字数**: ~7,000字
**涵盖**: Docker部署、缓存策略、消息队列配置、监控配置、备份恢复、性能调优、故障处理、扩容策略、运维检查

**返回**: [案例7主页](./README.md)
