...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\07-实时推荐系统\03-数据库设计.md`
> **📅 复制日期**: 2025-12-22
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
                RAISE WARNING 'pgvector扩展未安装，将创建表但不包含向量列';
            END IF;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_users_vector') THEN
            ...
            RAISE NOTICE '向量索引 idx_users_vector 创建成功';
        ELSE
            RAISE NOTICE '向量索引 idx_users_vector 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'items') THEN
            ...
            RAISE NOTICE '商品表 items 创建成功';
        ELSE
            RAISE NOTICE '商品表 items 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '商品表已存在';
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_items_embedding') THEN
                ...
                RAISE NOTICE '向量索引 idx_items_embedding 创建成功';
            END IF;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_items_sales') THEN
            ...
            RAISE NOTICE '索引 idx_items_sales 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behavior') THEN
            ...
            RAISE NOTICE '分区表 user_behavior 创建成功';
        ELSE
            RAISE NOTICE '分区表 user_behavior 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '分区表已存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behavior') THEN
            RAISE WARNING '表 user_behavior 不存在，无法创建分区';
            RETURN;
        END IF;
...
-- 4. 用户-商品交互统计（物化视图，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behavior') THEN
            RAISE WARNING '表 user_behavior 不存在，无法创建物化视图';
            RETURN;
        END IF;
...
**相似度计算维护函数（带错误处理和性能测试）**：

```sql
-- 相似度计算任务表
CREATE TABLE IF NOT EXISTS similarity_tasks (
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE EXCEPTION '相似度计算任务失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '数据清理失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
