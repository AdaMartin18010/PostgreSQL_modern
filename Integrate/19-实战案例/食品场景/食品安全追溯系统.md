...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\食品场景\食品安全追溯系统.md`
> **📅 复制日期**: 2025-12-22
...
- [食品安全追溯系统](#食品安全追溯系统)
  ...
  - [7. 参考资料](#7-参考资料)
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 追溯数据表创建](#81-追溯数据表创建)
    - [8.2 追溯系统实现](#82-追溯系统实现)
...
CREATE TABLE trace_data (
    ...
    batch_id TEXT NOT NULL,
    product_id TEXT NOT NULL,
    stage TEXT,  -- 'production', 'processing', 'packaging', 'distribution', 'retail'
    location TEXT,
    operator TEXT,
    status TEXT,  -- 'normal', 'suspicious', 'recalled'
...
CREATE TABLE quality_inspection (
    ...
    batch_id TEXT NOT NULL,
    product_id TEXT,
    image_vector vector(512),  -- 检测图像向量
    quality_score DECIMAL(10, 2),  -- 质量分数（0-100）
    defect_type TEXT,  -- 'contamination', 'spoilage', 'damage', 'none'
...
-- 转换为超表（用于时序数据）
SELECT create_hypertable('trace_data', 'time');
SELECT create_hypertable('quality_inspection', 'inspection_time');

-- 创建索引
CREATE INDEX idx_trace_data_batch_time ON trace_data (batch_id, time DESC);
CREATE INDEX idx_trace_data_product_time ON trace_data (product_id, time DESC);
CREATE INDEX idx_quality_inspection_vector ON quality_inspection USING hnsw (image_vector vector_cosine_ops);
```
...
class FoodTraceabilitySystem:
    ...
    def record_trace_data(self, batch_id: str, product_id: str, stage: str,
                         location: str, operator: str, status: str = 'normal'):
        """记录追溯数据"""
        self.cur.execute("""
            INSERT INTO trace_data
            (time, batch_id, product_id, stage, location, operator, status)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (
            datetime.now(), batch_id, product_id, stage, location, operator, status
        ))
    ...
    def trace_product(self, product_id: str) -> List[Dict]:
        ...
        self.cur.execute("""
            SELECT
                time, batch_id, stage, location, operator, status
            FROM trace_data
            WHERE product_id = %s
            ORDER BY time ASC
        """, (product_id,))
    ...
    def recall_batch(self, batch_id: str):
        """召回批次"""
        self.cur.execute("""
            UPDATE trace_data
            SET status = 'recalled'
...
# 使用示例
system = FoodTraceabilitySystem("host=localhost dbname=testdb user=postgres password=secret")

# 记录追溯数据
system.record_trace_data(
...
# 追溯产品
trace_history = system.trace_product('product_A')
for record in trace_history:
    print(f"{record['time']}: {record['stage']} at {record['location']}")
```
