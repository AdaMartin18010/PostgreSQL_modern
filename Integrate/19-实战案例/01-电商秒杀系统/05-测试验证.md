...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\01-电商秒杀系统\05-测试验证.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgtap') THEN
            ...
            RAISE NOTICE 'pgTAP扩展创建成功';
        ELSE
            RAISE NOTICE 'pgTAP扩展已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE 'pgTAP扩展已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建pgTAP扩展失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '开始执行单元测试（5个测试）';
        -- 注意：实际测试需要在事务块中执行
        -- BEGIN;
        -- SELECT plan(5);  -- 5个测试

        -- 准备测试数据（带错误处理）
        IF NOT EXISTS (SELECT 1 FROM flash_sales WHERE sale_id = 9999) THEN
...
```sql
-- 并发测试脚本（test_concurrent.sql，带错误处理说明）
-- 注意：以下脚本用于pgbench测试，需要在命令行执行
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'flash_sales') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'flash_orders') THEN
            RAISE WARNING '必需的表不存在，无法执行验证';
            RETURN;
        END IF;
        RAISE NOTICE '开始验证是否有超卖';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    ...
    (SELECT json_agg(row_to_json(t)) FROM (
        ...
        RAISE NOTICE '视图 performance_dashboard 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'performance_dashboard') THEN
            RAISE WARNING '视图 performance_dashboard 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询性能监控仪表板';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '连接池配置说明：需要在postgresql.conf中设置上述参数';
    EXCEPTION
        WHEN OTHERS THEN
...
-- 在低峰期手动VACUUM（带错误处理说明）
-- 注意：需要pg_cron扩展，实际执行需要在事务外
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
            RAISE WARNING 'pg_cron扩展未安装，无法调度VACUUM';
            RETURN;
        END IF;
        RAISE NOTICE 'VACUUM调度说明：可以使用pg_cron扩展调度定时VACUUM';
...
- [完整代码](../../../program/scripts/README.md)
