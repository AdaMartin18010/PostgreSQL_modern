...
BEGIN
    ...
    SELECT
        ev1.version as version1,
        ev2.version as version2,
        ev1.data != ev2.data as data_changed,
        ev1.data_hash != ev2.data_hash as hash_mismatch
    FROM experiment_versions ev1
    JOIN experiment_versions ev2 ON ev1.experiment_id = ev2.experiment_id
...
BEGIN
    -- 验证数据哈希
    IF NEW.data_hash != md5(NEW.data::TEXT) THEN
        RAISE EXCEPTION 'Data hash mismatch';
    END IF;
    ...
    IF NEW.version > 1 THEN
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'Version number not continuous';
        END IF;
    END IF;
