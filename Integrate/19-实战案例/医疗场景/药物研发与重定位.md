...
BEGIN
    ...
    vector_candidates AS (
        SELECT
            drug_id,
            disease_id,
            1 - (embedding <=> p_disease_vector) as similarity
        FROM drug_disease_associations
    ...
    fused_results AS (
        SELECT
            ...
            (COALESCE(gc.efficacy_score::NUMERIC, 0) * 0.6 +
             COALESCE(vc.similarity, 0) * 0.4) as prediction_score,
            CASE
                WHEN gc.efficacy_score::NUMERIC > 0.9 AND vc.similarity > 0.9 THEN 0.95
                WHEN gc.efficacy_score::NUMERIC > 0.8 AND vc.similarity > 0.8 THEN 0.85
                ELSE 0.75
            END as confidence
    ...
    SELECT drug_id, drug_name, prediction_score, confidence
    FROM fused_results
...
BEGIN
    ...
    vector_prediction AS (
        SELECT
            drug_id,
            1 - (embedding <=> p_disease_vector) as vector_score
        FROM drug_disease_associations
        WHERE disease_id = p_target_disease_id
        ORDER BY embedding <=> p_disease_vector
        LIMIT 30
    ...
    clinical_evidence AS (
        SELECT
            drug_id,
            AVG(efficacy_score) as clinical_score
    ...
    combined_prediction AS (
        SELECT
            ...
            (COALESCE(gnn.gnn_score::NUMERIC, 0) * 0.4 +
             ...
             COALESCE(clin.clinical_score, 0) * 0.3) as final_score
