...
class KnowledgeGraphQA:
    ...
    def __init__(self, conn_str, openai_api_key):
        ...
        try:
            ...
            print(f"警告: 加载AGE扩展失败: {e}")
            self.conn.rollback()

    def nl2cypher(self, question: str) -> str:
        """
        自然语言转Cypher查询（带完整错误处理和参数验证）
        """
...
"""
    ...
    def execute_cypher(self, cypher_query: str) -> list:
        """
        执行Cypher查询（带完整错误处理和参数验证）
        """

        # 参数验证
        if not cypher_query or not cypher_query.strip():
            raise ValueError("cypher_query不能为空")
...
"""
            ...
            # 返回基于结果的简单答案
            if results:
                return f"根据查询结果，找到了 {len(results)} 条相关信息。"
            return "抱歉，我暂时无法生成答案，请稍后再试。"
...
async def get_subgraph(entity: str, radius: int = 2):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"查询语法错误: {str(e)}")
    except psycopg2.OperationalError as e:
        raise HTTPException(status_code=503, detail=f"数据库连接失败: {str(e)}")
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"数据库操作失败: {str(e)}")
    ...
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kg_query_performance') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_kg_query_performance_hash') THEN
                ...
                RAISE NOTICE '索引 idx_kg_query_performance_hash 创建成功';
            ELSE
                RAISE NOTICE '索引 idx_kg_query_performance_hash 已存在';
            END IF;
            ...
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_kg_query_performance_created') THEN
                ...
                RAISE NOTICE '索引 idx_kg_query_performance_created 创建成功';
            ELSE
                RAISE NOTICE '索引 idx_kg_query_performance_created 已存在';
            END IF;
        ELSE
            RAISE WARNING '表 kg_query_performance 不存在，无法创建索引';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询性能统计失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_query_performance_stats') THEN
            RAISE WARNING '函数 get_query_performance_stats 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询性能统计';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'v_cypher_query_performance') THEN
            RAISE WARNING '视图 v_cypher_query_performance 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询Cypher性能统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
