...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\08-知识图谱问答系统\03-数据库设计.md`
> **📅 复制日期**: 2025-12-22
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '实体向量化失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        0.8::FLOAT AS similarity_score,  -- 图查询默认相似度
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE EXCEPTION 'NL2Cypher转换失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    ...
    BEGIN
        ...
        RETURN QUERY SELECT
            TRUE::BOOLEAN,
            NULL::TEXT,
            0::NUMERIC;  -- 简化处理，实际应该解析EXPLAIN结果
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Cypher查询验证失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
