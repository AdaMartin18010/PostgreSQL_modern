...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\家具场景\智能家具推荐系统.md`
> **📅 复制日期**: 2025-12-22
...
> **技术版本**: PostgreSQL 18+ (推荐) ⭐ | 17+ | pgvector 0.7.0+, PostGIS 3.4+
> **文档编号**: 08-34-01
...
- [智能家具推荐系统](#智能家具推荐系统)
  ...
  - [7. 参考资料](#7-参考资料)
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 家具数据表创建](#81-家具数据表创建)
    - [8.2 家具推荐实现](#82-家具推荐实现)
...
CREATE TABLE furniture (
    id SERIAL PRIMARY KEY,
    name TEXT NOT NULL,
    category TEXT,  -- 'sofa', 'table', 'chair', etc.
    style TEXT,  -- 'modern', 'classic', 'minimalist', etc.
    design_vector vector(512),  -- 设计向量
    size_width DECIMAL(10, 2),  -- 宽度（cm）
    size_height DECIMAL(10, 2),  -- 高度（cm）
    size_depth DECIMAL(10, 2),  -- 深度（cm）
    price DECIMAL(10, 2),
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE rooms (
    id SERIAL PRIMARY KEY,
    user_id INTEGER,
    room_type TEXT,  -- 'living_room', 'bedroom', 'kitchen', etc.
    room_shape GEOGRAPHY(POLYGON, 4326),  -- 房间形状
    area DECIMAL(10, 2),  -- 面积（m²）
    style_preference vector(512),  -- 风格偏好向量
    created_at TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB DEFAULT '{}'::JSONB
);

-- 创建向量索引
CREATE INDEX idx_furniture_vector ON furniture USING hnsw (design_vector vector_cosine_ops);
CREATE INDEX idx_rooms_shape ON rooms USING GIST (room_shape);
CREATE INDEX idx_rooms_vector ON rooms USING hnsw (style_preference vector_cosine_ops);
```
...
class FurnitureRecommender:
    ...
    def recommend_furniture(self, room_id: int, style_preference: List[float],
                           max_width: Optional[float] = None,
                           max_height: Optional[float] = None,
                           max_depth: Optional[float] = None,
                           limit: int = 10) -> List[Dict]:
        """推荐家具"""
        conditions = ["design_vector <=> %s < 0.5"]
        params = [style_preference, style_preference]

        if max_width:
            conditions.append("size_width <= %s")
            params.append(max_width)

        if max_height:
            conditions.append("size_height <= %s")
            params.append(max_height)

        if max_depth:
            conditions.append("size_depth <= %s")
        ...
        for row in self.cur.fetchall():
            recommendations.append({
                ...
                'size': {
                    ...
                    'depth': float(row[7]) if row[7] else None
                },
...
# 使用示例
recommender = FurnitureRecommender("host=localhost dbname=testdb user=postgres password=secret")

# 推荐家具（需要先获取风格偏好向量）
# style_vector = get_embedding("modern minimalist")  # 假设有获取向量的函数
# recommendations = recommender.recommend_furniture(
#     room_id=1,
#     style_preference=style_vector,
#     max_width=200.0,
#     max_height=100.0,
#     limit=10
# )
