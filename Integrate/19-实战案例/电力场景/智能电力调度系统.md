...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\电力场景\智能电力调度系统.md`
> **📅 复制日期**: 2025-12-22
...
- [智能电力调度系统](#智能电力调度系统)
  ...
  - [1. 概述](#1-概述)
    ...
    - [1.2 核心价值](#12-核心价值)
  - [2. 系统架构](#2-系统架构)
    - [2.1 智能电力调度体系思维导图](#21-智能电力调度体系思维导图)
    - [2.2 架构设计](#22-架构设计)
    - [2.3 技术栈](#23-技术栈)
  - [3. 数据模型设计](#3-数据模型设计)
    - [3.1 负荷数据时序表](#31-负荷数据时序表)
    - [3.2 发电数据时序表](#32-发电数据时序表)
  - [4. 电力调度](#4-电力调度)
    - [4.1 负荷预测](#41-负荷预测)
    - [4.2 发电调度](#42-发电调度)
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 案例: 智能电力调度系统（真实案例）](#51-案例-智能电力调度系统真实案例)
  ...
  - [7. 参考资料](#7-参考资料)
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 电力调度数据表创建](#81-电力调度数据表创建)
    - [8.2 电力调度实现](#82-电力调度实现)
...
CREATE TABLE load_data (
    ...
    predicted_load_mw DECIMAL(10, 2),  -- 预测负荷（MW）
...
CREATE TABLE generator_data (
    time TIMESTAMPTZ NOT NULL,
    generator_id TEXT NOT NULL,
    power_output_mw DECIMAL(10, 2),  -- 出力（MW）
    capacity_mw DECIMAL(10, 2),  -- 容量（MW）
    efficiency DECIMAL(5, 2),  -- 效率（%）
    fuel_cost DECIMAL(10, 2),  -- 燃料成本（元/MWh）
...
CREATE TABLE dispatch_plans (
    ...
    generator_id TEXT NOT NULL,
    scheduled_power_mw DECIMAL(10, 2),
    actual_power_mw DECIMAL(10, 2),
    status TEXT DEFAULT 'scheduled',  -- 'scheduled', 'executing', 'completed', 'cancelled'
...
-- 转换为超表（用于时序数据）
SELECT create_hypertable('load_data', 'time');
SELECT create_hypertable('generator_data', 'time');

-- 创建索引
CREATE INDEX idx_load_data_grid_time ON load_data (grid_id, time DESC);
CREATE INDEX idx_generator_data_generator_time ON generator_data (generator_id, time DESC);
CREATE INDEX idx_dispatch_plans_time ON dispatch_plans (plan_time DESC);
```
...
class PowerDispatchManager:
    ...
    def record_load_data(self, grid_id: str, load_mw: float,
        ...
        self.cur.execute("""
            ...
            (time, grid_id, load_mw, predicted_load_mw, frequency, voltage)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (
            datetime.now(), grid_id, load_mw, predicted_load_mw, frequency, voltage
        ))

        self.conn.commit()

    def record_generator_data(self, generator_id: str, power_output_mw: float,
                             capacity_mw: float, efficiency: Optional[float] = None,
                             fuel_cost: Optional[float] = None, status: str = 'running'):
        """记录发电机组数据"""
        self.cur.execute("""
            INSERT INTO generator_data
            (time, generator_id, power_output_mw, capacity_mw, efficiency, fuel_cost, status)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (
            datetime.now(), generator_id, power_output_mw, capacity_mw,
            efficiency, fuel_cost, status
        ))

        self.conn.commit()

    def create_dispatch_plan(self, plan_time: datetime, generator_id: str,
                            scheduled_power_mw: float):
        """创建调度计划"""
        self.cur.execute("""
            INSERT INTO dispatch_plans
            (plan_time, generator_id, scheduled_power_mw, status)
            VALUES (%s, %s, %s, %s)
        """, (plan_time, generator_id, scheduled_power_mw, 'scheduled'))

        self.conn.commit()

    def get_load_forecast(self, grid_id: str, hours: int = 24) -> List[Dict]:
        """获取负荷预测"""
        self.cur.execute("""
            SELECT
                time,
        ...
        for row in self.cur.fetchall():
            forecasts.append({
                'time': row[0],
                'load_mw': float(row[1]) if row[1] else None,
                'predicted_load_mw': float(row[2]) if row[2] else None
            })

        return forecasts

# 使用示例
manager = PowerDispatchManager("host=localhost dbname=testdb user=postgres password=secret")

# 记录负荷数据
manager.record_load_data(
    grid_id='grid_001',
    load_mw=1000.0,
    predicted_load_mw=1050.0,
    frequency=50.0,
    voltage=220.0
)

# 记录发电机组数据
manager.record_generator_data(
    generator_id='gen_001',
    power_output_mw=500.0,
    capacity_mw=600.0,
    efficiency=85.0,
    fuel_cost=300.0,
    status='running'
...
manager.create_dispatch_plan(plan_time, 'gen_001', 550.0)
```
