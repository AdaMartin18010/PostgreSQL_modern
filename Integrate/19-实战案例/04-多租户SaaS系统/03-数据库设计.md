...
BEGIN
    ...
    SELECT
        '用户数'::TEXT,
        (SELECT tenant_1_count FROM test_tenant_isolation_result WHERE test_item = '用户数'),
        (SELECT tenant_2_count FROM test_tenant_isolation_result WHERE test_item = '用户数'),
        CASE
            WHEN (SELECT tenant_1_count FROM test_tenant_isolation_result WHERE test_item = '用户数') !=
                 (SELECT tenant_2_count FROM test_tenant_isolation_result WHERE test_item = '用户数')
            THEN '隔离正常'
            ELSE '隔离异常'
        END;
