...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'content') AND
           EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes
                WHERE tablename = 'content' AND indexname LIKE '%feature_vector%'
            ) THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_behavior') THEN
        ...
        ELSE
            RAISE NOTICE '表 user_behavior 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表已存在';
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_behavior') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_behavior_user') THEN
                CREATE INDEX idx_user_behavior_user ON user_behavior(user_id, behavior_time DESC);
                RAISE NOTICE '索引 idx_user_behavior_user 创建成功';
            END IF;

            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_behavior_content') THEN
                CREATE INDEX idx_user_behavior_content ON user_behavior(content_id);
                RAISE NOTICE '索引 idx_user_behavior_content 创建成功';
            END IF;
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_preference') THEN
            -- 检查pgvector扩展
            IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
                CREATE EXTENSION IF NOT EXISTS vector;
                RAISE NOTICE '扩展 vector 创建成功';
            ...
            RAISE NOTICE '表 user_preference 创建成功';
        ELSE
            RAISE NOTICE '表 user_preference 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表已存在';
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_preference') AND
           EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes
                WHERE tablename = 'user_preference' AND indexname LIKE '%preference_vector%'
            ) THEN
...
SELECT
    c.content_id,
    c.title,
    AVG(ub.duration::FLOAT / NULLIF((c.metadata->>'duration')::FLOAT, 0)) AS avg_completion_rate,
    1 - (c.feature_vector <=> up.preference_vector) AS similarity
FROM content c
...
WHERE c.content_type = 'music'
  AND up.user_id = $1
GROUP BY c.content_id, c.title, c.feature_vector, up.preference_vector
ORDER BY (similarity * (1 + LOG(1 + COUNT(ub.behavior_id)))) DESC
LIMIT 20;
```
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '个性化推荐查询失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '相似内容推荐查询失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
