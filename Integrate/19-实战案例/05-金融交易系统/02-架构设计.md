---

> **📋 文档来源**: `DataBaseTheory\19-场景案例库\05-金融交易系统\02-架构设计.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 案例5：金融交易系统 - 架构设计

## 元数据

- **创建日期**: 2025-12-04
- **架构模式**: CQRS + Event Sourcing
- **ACID**: SERIALIZABLE隔离级别

---

## 1. 总体架构

```text
┌──────────────────────────────────────────────────────────┐
│              金融交易系统架构                             │
├──────────────────────────────────────────────────────────┤
│                                                            │
│  [交易请求]                                               │
│      │                                                     │
│  [API Gateway]                                            │
│      │                                                     │
│  [交易服务] ──> [PostgreSQL 18]                          │
│      │            (SERIALIZABLE)                          │
│      │                 │                                   │
│      │                 ├─> [账户表]                       │
│      │                 ├─> [交易表]                       │
│      │                 ├─> [审计日志]                     │
│      │                 └─> [事件表]                       │
│      │                                                     │
│  [对账服务] ──> 定时对账                                  │
│      │                                                     │
│  [审计服务] ──> 审计日志分析                              │
│                                                            │
│  [监控告警]                                               │
│  ├─ TPS监控                                               │
│  ├─ 延迟监控                                              │
│  └─ 异常检测                                              │
│                                                            │
└──────────────────────────────────────────────────────────┘
```

---

## 2. ACID保证设计

### 2.1 强一致性策略

**隔离级别**: SERIALIZABLE

```sql
-- 全局配置
ALTER SYSTEM SET default_transaction_isolation = 'serializable';

-- 或单个事务
BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;
-- 交易操作
COMMIT;
```

**原子性保证**:

- ✅ 转账操作要么全部成功，要么全部失败
- ✅ 使用事务确保原子性
- ✅ 乐观锁处理并发冲突

**持久性保证**:

- ✅ 同步提交WAL
- ✅ 主从同步复制
- ✅ 定期备份

---

## 3. 核心业务流程

### 3.1 转账流程

```text
转账请求
  │
  ├─> 参数验证
  │
  ├─> BEGIN TRANSACTION (SERIALIZABLE)
  │
  ├─> 检查余额
  │
  ├─> 扣款（乐观锁）
  │
  ├─> 入账
  │
  ├─> 记录交易
  │
  ├─> 记录审计日志
  │
  ├─> COMMIT
  │
  └─> 返回成功
```

---

## 4. 高可用设计

```text
主从架构:
  [Primary] ──同步复制──> [Standby 1]
      │
      └──异步复制──> [Standby 2]

故障转移:
  Primary故障 → Standby 1自动提升
  ├─ RPO: 0（同步复制）
  └─ RTO: <30秒（自动故障转移）
```

---

---

## 5. 详细架构组件

### 5.1 交易服务层

**交易服务层设计（带错误处理和性能测试）**：

```python
"""
交易服务层
技术栈: FastAPI + PostgreSQL 18
"""

from fastapi import FastAPI, HTTPException
from typing import Optional
import psycopg2
from psycopg2.extras import RealDictCursor
from decimal import Decimal

app = FastAPI()

class TransactionService:
    """交易服务"""

    def __init__(self, conn_str):
        self.conn = psycopg2.connect(conn_str, cursor_factory=RealDictCursor)
        self.cursor = self.conn.cursor()

    def transfer(
        self,
        from_account: str,
        to_account: str,
        amount: Decimal,
        user_id: int,
        request_id: str
    ):
        """转账（带幂等性）"""

        try:
            # 设置隔离级别
            self.cursor.execute("SET TRANSACTION ISOLATION LEVEL SERIALIZABLE")
            self.cursor.execute("BEGIN")

            # 检查幂等性
            self.cursor.execute("""
                SELECT transaction_id FROM transaction_requests
                WHERE request_id = %s
            """, (request_id,))

            existing = self.cursor.fetchone()
            if existing:
                return {'transaction_id': existing['transaction_id'], 'status': 'duplicate'}

            # 调用数据库转账函数
            self.cursor.execute("""
                SELECT transfer_money(%s, %s, %s, %s, %s)
            """, (request_id, from_account, to_account, amount, user_id))

            transaction_id = self.cursor.fetchone()[0]
            self.cursor.execute("COMMIT")

            return {
                'transaction_id': transaction_id,
                'status': 'success',
                'from_account': from_account,
                'to_account': to_account,
                'amount': float(amount)
            }

        except psycopg2.Error as e:
            self.cursor.execute("ROLLBACK")
            raise HTTPException(status_code=400, detail=str(e))

    def get_account_balance(self, account_id: str):
        """查询账户余额"""

        self.cursor.execute("""
            SELECT account_id, balance, currency, status
            FROM accounts
            WHERE account_id = %s
        """, (account_id,))

        account = self.cursor.fetchone()
        if not account:
            raise HTTPException(status_code=404, detail="账户不存在")

        return dict(account)

# API接口
transaction_service = TransactionService("dbname=finance_db user=postgres")

@app.post("/api/transfer")
async def transfer(
    from_account: str,
    to_account: str,
    amount: Decimal,
    user_id: int,
    request_id: str
):
    """转账接口"""
    return transaction_service.transfer(from_account, to_account, amount, user_id, request_id)

@app.get("/api/account/{account_id}/balance")
async def get_balance(account_id: str):
    """查询余额接口"""
    return transaction_service.get_account_balance(account_id)
```

### 5.2 对账服务

**对账服务设计（带错误处理和性能测试）**：

```sql
-- 对账服务函数
CREATE OR REPLACE FUNCTION daily_reconciliation(
    p_reconciliation_date DATE DEFAULT CURRENT_DATE
)
RETURNS TABLE (
    check_item TEXT,
    expected_value NUMERIC,
    actual_value NUMERIC,
    difference NUMERIC,
    status TEXT
) AS $$
DECLARE
    total_debit NUMERIC;
    total_credit NUMERIC;
    account_sum NUMERIC;
BEGIN
    BEGIN
        -- 参数验证
        IF p_reconciliation_date IS NULL THEN
            RAISE EXCEPTION 'reconciliation_date不能为NULL';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE EXCEPTION '必需的表不存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '参数验证失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        -- 检查借贷平衡
        SELECT
            COALESCE(SUM(CASE WHEN type = 'DEBIT' THEN amount ELSE 0 END), 0),
            COALESCE(SUM(CASE WHEN type = 'CREDIT' THEN amount ELSE 0 END), 0)
        INTO total_debit, total_credit
        FROM transactions
        WHERE DATE(created_at) = p_reconciliation_date;

        IF total_debit IS NULL THEN
            total_debit := 0;
        END IF;
        IF total_credit IS NULL THEN
            total_credit := 0;
        END IF;

        -- 检查账户余额总和
        SELECT COALESCE(SUM(balance), 0) INTO account_sum
        FROM accounts;

        IF account_sum IS NULL THEN
            account_sum := 0;
        END IF;

        -- 返回对账结果
        RETURN QUERY SELECT
            '借贷平衡'::TEXT,
            total_debit,
            total_credit,
            ABS(total_debit - total_credit),
            CASE
                WHEN ABS(total_debit - total_credit) < 0.01 THEN '平衡'
                ELSE '不平衡'
            END;

        RETURN QUERY SELECT
            '账户余额总和'::TEXT,
            account_sum,
            account_sum,
            0::NUMERIC,
            '正常'::TEXT;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '对账计算失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
```

---

## 6. 监控与告警

### 6.1 交易性能监控

**交易性能监控视图（带错误处理和性能测试）**：

```sql
-- 交易性能监控视图
CREATE OR REPLACE VIEW v_transaction_performance AS
SELECT
    date_trunc('hour', created_at) AS hour,
    COUNT(*) AS transaction_count,
    COUNT(*) FILTER (WHERE status = 'success') AS success_count,
    COUNT(*) FILTER (WHERE status = 'failed') AS failed_count,
    ROUND(AVG(EXTRACT(EPOCH FROM (updated_at - created_at)) * 1000), 2) AS avg_duration_ms,
    ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (updated_at - created_at)) * 1000), 2) AS p95_duration_ms,
    ROUND(COUNT(*) FILTER (WHERE status = 'success') * 100.0 / COUNT(*), 2) AS success_rate
FROM transactions
WHERE created_at > NOW() - INTERVAL '24 hours'
GROUP BY date_trunc('hour', created_at)
ORDER BY hour DESC;

-- 查询监控数据
SELECT * FROM v_transaction_performance;
```

### 6.2 异常检测

**异常检测函数（带错误处理和性能测试）**：

```sql
-- 异常检测函数
CREATE OR REPLACE FUNCTION detect_anomalies(
    p_hours INT DEFAULT 24
)
RETURNS TABLE (
    anomaly_type TEXT,
    description TEXT,
    severity TEXT,
    detected_at TIMESTAMPTZ
) AS $$
DECLARE
    avg_tps NUMERIC;
    current_tps NUMERIC;
    error_rate NUMERIC;
BEGIN
    BEGIN
        -- 参数验证
        IF p_hours IS NULL OR p_hours <= 0 THEN
            RAISE EXCEPTION 'hours必须大于0';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'v_transaction_performance') THEN
            RAISE EXCEPTION '视图 v_transaction_performance 不存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '参数验证失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        -- 检测TPS异常
        SELECT AVG(transaction_count) INTO avg_tps
        FROM v_transaction_performance
        WHERE hour > NOW() - INTERVAL '7 days';

        IF avg_tps IS NULL THEN
            avg_tps := 0;
        END IF;

        SELECT transaction_count INTO current_tps
        FROM v_transaction_performance
        WHERE hour = date_trunc('hour', NOW())
        LIMIT 1;

        IF current_tps IS NULL THEN
            current_tps := 0;
        END IF;

        IF current_tps > avg_tps * 2 THEN
            RETURN QUERY SELECT
                'TPS异常'::TEXT,
                format('当前TPS: %s, 平均TPS: %s', current_tps, avg_tps)::TEXT,
                '警告'::TEXT,
                NOW();
        END IF;

        -- 检测错误率异常
        SELECT AVG(100.0 - COALESCE(success_rate, 0)) INTO error_rate
        FROM v_transaction_performance
        WHERE hour > NOW() - (p_hours || ' hours')::INTERVAL;

        IF error_rate IS NULL THEN
            error_rate := 0;
        END IF;

        IF error_rate > 1.0 THEN
            RETURN QUERY SELECT
                '错误率异常'::TEXT,
                format('错误率: %.2f%%', error_rate)::TEXT,
                '严重'::TEXT,
                NOW();
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '异常检测失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
```

---

## 7. 安全设计

### 7.1 审计日志

**审计日志配置（带错误处理和性能测试）**：

```sql
-- 启用审计日志
ALTER SYSTEM SET log_statement = 'all';
ALTER SYSTEM SET log_connections = on;
ALTER SYSTEM SET log_disconnections = on;
ALTER SYSTEM SET log_duration = on;
SELECT pg_reload_conf();

-- 审计日志查询函数
CREATE OR REPLACE FUNCTION query_audit_logs(
    p_start_time TIMESTAMPTZ DEFAULT NOW() - INTERVAL '1 day',
    p_end_time TIMESTAMPTZ DEFAULT NOW(),
    p_user_id INT DEFAULT NULL
)
RETURNS TABLE (
    log_time TIMESTAMPTZ,
    user_id INT,
    operation TEXT,
    table_name TEXT,
    details JSONB
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        created_at AS log_time,
        user_id,
        operation,
        table_name,
        new_values AS details
    FROM audit_logs
    WHERE created_at BETWEEN p_start_time AND p_end_time
      AND (p_user_id IS NULL OR user_id = p_user_id)
    ORDER BY created_at DESC
    LIMIT 1000;

    RETURN;

        WHEN OTHERS THEN
            RAISE WARNING '查询审计日志失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
```

---

**完成日期**: 2025-12-04
**ACID级别**: 最高
**可用性**: 99.99%

**返回**: [案例5主页](./README.md)
