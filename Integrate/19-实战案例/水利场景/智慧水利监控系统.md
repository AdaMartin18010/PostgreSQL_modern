...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\水利场景\智慧水利监控系统.md`
> **📅 复制日期**: 2025-12-22
...
- [智慧水利监控系统](#智慧水利监控系统)
  ...
  - [7. 参考资料](#7-参考资料)
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 水利数据时序表创建](#81-水利数据时序表创建)
    - [8.2 水位数据采集实现](#82-水位数据采集实现)
    - [8.3 预警系统实现](#83-预警系统实现)
    - [8.4 水位趋势分析实现](#84-水位趋势分析实现)
---
...
CREATE TABLE water_level_data (
    ...
    station_id TEXT NOT NULL,
    station_name TEXT,
    location GEOGRAPHY(POINT, 4326),  -- 监测站位置
    water_level DECIMAL(10, 2),  -- 水位（米）
    flow_rate DECIMAL(10, 2),  -- 流量（m³/s）
    temperature DECIMAL(10, 2),  -- 水温（摄氏度）
...
CREATE TABLE water_quality_data (
    ...
    chemical_oxygen_demand DECIMAL(10, 2),  -- 化学需氧量（mg/L）
    metadata JSONB DEFAULT '{}'::JSONB
);
        ...
        for rule_id, threshold_value, severity in self.cur.fetchall():
            if water_level > threshold_value:
                alert = {
                    ...
                    'message': f"水位预警: {station_name} 当前水位 {water_level}米，超过阈值 {threshold_value}米"
                }
                alerts.append(alert)
    ...
    def check_water_quality_alerts(self, station_id: str) -> List[Dict]:
        ...
        if do and do < 5.0:
            alert = {
                ...
                'message': f"溶解氧过低: {do}mg/L（正常值应≥5.0mg/L）"
            }
            alerts.append(alert)
            self.create_alert(alert)
