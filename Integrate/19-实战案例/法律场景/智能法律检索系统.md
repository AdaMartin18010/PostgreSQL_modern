...
```sql
-- PostgreSQL 18：并行查询优化（适用于向量搜索和全文搜索）
ALTER SYSTEM SET max_parallel_workers_per_gather = 4;
ALTER SYSTEM SET max_parallel_workers = 8;
ALTER SYSTEM SET parallel_tuple_cost = 0.01;

-- PostgreSQL 18：GIN索引并行构建优化（全文搜索索引）
ALTER SYSTEM SET max_parallel_maintenance_workers = 8;
-- 性能提升：GIN索引构建速度提升3-4倍 ⭐

-- PostgreSQL 18：工作内存优化（向量相似度计算）
ALTER SYSTEM SET work_mem = '256MB';
-- 适用于大规模向量搜索和相似度计算

-- PostgreSQL 18：统计信息优化（查询计划优化）
ALTER SYSTEM SET default_statistics_target = 500;
-- 提升向量搜索和全文搜索的查询计划质量

-- 性能提升：
-- 向量搜索性能：+20-30% ⭐（pgvector与PostgreSQL 18优化）
-- 全文搜索性能：+30-40% ⭐（GIN并行构建和查询优化）
-- 混合搜索（向量+全文）性能：+25-35% ⭐
```
