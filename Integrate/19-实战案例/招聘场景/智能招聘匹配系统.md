...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\招聘场景\智能招聘匹配系统.md`
> **📅 复制日期**: 2025-12-22
...
- [智能招聘匹配系统](#智能招聘匹配系统)
  ...
  - [7. 参考资料](#7-参考资料)
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 招聘数据表创建](#81-招聘数据表创建)
    - [8.2 招聘匹配实现](#82-招聘匹配实现)
...
CREATE TABLE job_positions (
    ...
    company_id INTEGER,
    category TEXT,
    skills_vector vector(512),  -- 技能要求向量
    requirements_vector vector(512),  -- 职位要求向量
    salary_min DECIMAL(10, 2),
    salary_max DECIMAL(10, 2),
    location TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE resumes (
    id SERIAL PRIMARY KEY,
    candidate_id INTEGER NOT NULL,
    skills_vector vector(512),  -- 技能向量
    experience_vector vector(512),  -- 经验向量
    education_level TEXT,
    years_of_experience INTEGER,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB DEFAULT '{}'::JSONB
...
CREATE TABLE job_matches (
    ...
    skills_match_score DECIMAL(5, 2),  -- 技能匹配分数
    experience_match_score DECIMAL(5, 2),  -- 经验匹配分数
    overall_score DECIMAL(5, 2),  -- 综合匹配分数
