...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\02-OLAP分析系统\03-数据库设计.md`
> **📅 复制日期**: 2025-12-22
...
- [OLAP分析系统 - 数据库设计](#olap分析系统---数据库设计)
  - [📋 目录](#-目录)
  - [一、完整Schema](#一完整schema)
    - [1.1 事实表（分区）](#11-事实表分区)
    - [1.2 维度表](#12-维度表)
  - [二、聚合表设计](#二聚合表设计)
    - [2.1 多层聚合](#21-多层聚合)
  - [三、PostgreSQL 18特性应用](#三postgresql-18特性应用)
    ...
    - [3.2 统计信息优化](#32-统计信息优化)
    - [3.3 分区策略](#33-分区策略)
  - [四、查询优化](#四查询优化)
    - [4.1 典型OLAP查询优化](#41-典型olap查询优化)
    - [4.2 物化视图查询改写](#42-物化视图查询改写)
...
BEGIN
    ...
    IF referential_integrity_count > 0 THEN
        RETURN QUERY SELECT
            '参照完整性检查'::TEXT,
            format('发现 %s 条记录违反参照完整性', referential_integrity_count)::TEXT,
            'fail'::TEXT;
    ELSE
        RETURN QUERY SELECT
            '参照完整性检查'::TEXT,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'ETL性能统计失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
