---
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\10-金融反欺诈系统\06-部署运维.md`
> **📅 复制日期**: 2025-12-22
...
- **高可用**: Patroni + HAProxy + 主从复制
- **备份**: WAL归档 + 定期全量备份 + 规则备份
- **最后更新**: 2025年1月
...
**PostgreSQL优化配置（PostgreSQL 18特性，反欺诈系统）**：

```ini
# postgresql.conf - 金融反欺诈系统优化
...
- name: fraud_detection_alerts
  ...
  - alert: CriticalFraudDetectionLatency
    ...
    annotations:
      summary: "欺诈检测延迟严重超标"
      description: "P99延迟超过100ms: {{ $value }}ms"
  ...
  - alert: HighRuleMatchingLatency
    ...
    annotations:
      summary: "规则匹配延迟过高"
      description: "P95延迟超过20ms: {{ $value }}ms"
  ...
  - alert: HighDatabaseConnections
    ...
    annotations:
      summary: "数据库连接数过高"
      description: "当前连接数: {{ $value }}, 最大: 1000"
...
**规则备份脚本（backup_rules.sh）**：

```bash
#!/bin/bash
# backup_rules.sh - 规则备份脚本
...
echo "规则备份完成: $BACKUP_FILE.gz"
```

### 6.2 规则恢复

**规则恢复脚本（restore_rules.sh）**：

```bash
#!/bin/bash
# restore_rules.sh - 规则恢复脚本
...
BEGIN
    ...
    IF primary_count != replica_count THEN
        RAISE WARNING '数据不一致: 主库=% 从库=%', primary_count, replica_count;
    ELSE
        RAISE NOTICE '数据一致: %', primary_count;
    END IF;
END $$;
```
