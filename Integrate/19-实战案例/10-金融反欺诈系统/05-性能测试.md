...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\10-金融反欺诈系统\05-性能测试.md`
> **📅 复制日期**: 2025-12-22
    ...
    try:
        ...
        if latencies:
            ...
            if valid_latencies:
                ...
                print("欺诈检测性能:")
                print(f"  平均延迟: {sum(valid_latencies)/len(valid_latencies):.2f}ms")
                if len(valid_latencies) > 5000:
                    print(f"  P50: {valid_latencies[5000]:.2f}ms")
                if len(valid_latencies) > 9500:
                    print(f"  P95: {valid_latencies[9500]:.2f}ms")
                if len(valid_latencies) > 9900:
                    print(f"  P99: {valid_latencies[9900]:.2f}ms")
                print(f"  最大: {valid_latencies[-1]:.2f}ms")
    ...
    try:
        ...
        for i in range(1000):
            ...
            try:
                ...
                print(f"第 {i+1} 次规则匹配失败: {e}")
                latencies.append(0)
            except Exception as e:
                print(f"第 {i+1} 次规则匹配未知错误: {e}")
                latencies.append(0)
        ...
        if latencies:
            ...
            if valid_latencies:
                ...
                avg_matched = sum(len(r) for r in matched_rules_list) / len(matched_rules_list) if matched_rules_list else 0

                print(f"规则匹配性能:")
                print(f"  平均延迟: {avg:.2f}ms")
                print(f"  平均匹配: {avg_matched:.1f}条规则")
                print(f"  失败次数: {len(latencies) - len(valid_latencies)}")
    except Exception as e:
        print(f"测试执行失败: {e}")
    finally:
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'fraud_detection_logs') THEN
            ...
            RETURN;
        END IF;
        DROP VIEW IF EXISTS v_fraud_performance_metrics;
...
ORDER BY hour DESC;
        ...
        RAISE NOTICE '视图 v_fraud_performance_metrics 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
-- 查询性能指标（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM v_fraud_performance_metrics LIMIT 100;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'fraud_detection_logs') THEN
            RAISE EXCEPTION '表 fraud_detection_logs 不存在';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            RAISE WARNING '表 users 不存在，无法插入测试数据';
        ELSE
...
**完成**: 金融反欺诈系统性能测试
**字数**: ~12,000字
**涵盖**: 实时检测、规则引擎、ML模型、图算法、混合负载、准确率、资源使用、性能报告、PostgreSQL 18优化、性能监控、最佳实践
