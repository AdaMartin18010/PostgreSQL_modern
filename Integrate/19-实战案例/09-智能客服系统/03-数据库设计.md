---
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\09-智能客服系统\03-数据库设计.md`
> **📅 复制日期**: 2025-12-22
...
CREATE TABLE faqs (
    ...
    category VARCHAR(50),
    tags TEXT[],  -- 标签数组
    embedding vector(768),  -- pgvector向量（用于语义检索）
...
-- HNSW向量索引（PostgreSQL 18 + pgvector）
...
CREATE TABLE conversations (
    ...
    faq_ids INT[],  -- 关联的FAQ ID列表
    response_time_ms INT,  -- 响应时间（毫秒）
    user_rating INT CHECK (user_rating IN (1, 2, 3, 4, 5)),  -- 用户评分
    feedback TEXT,  -- 用户反馈
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (conv_id, created_at)
) PARTITION BY RANGE (created_at);

-- 按月分区（PostgreSQL 18自动分区）
...
CREATE TABLE sessions (
    ...
    metadata JSONB,  -- 会话元数据（IP、User-Agent等）
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE EXCEPTION '向量检索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE EXCEPTION '全文搜索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
**改进的VACUUM配置（PostgreSQL 18）**：

```sql
-- PostgreSQL 18改进的VACUUM配置
...
BEGIN
    ...
    LOOP
        BEGIN
            -- 调用AI服务生成向量（简化处理，实际应该调用pg_ai或外部API）
        ...
        EXCEPTION
            WHEN OTHERS THEN
                v_errors := v_errors + 1;
                RAISE WARNING '更新FAQ % 向量失败: %', v_faq.faq_id, SQLERRM;
        END;
    END LOOP;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '清理过期会话失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
**完成**: 智能客服系统数据库设计
**字数**: ~8,000字
**涵盖**: 核心表设计、辅助表、查询函数、统计视图、PostgreSQL 18特性、数据维护、性能优化
