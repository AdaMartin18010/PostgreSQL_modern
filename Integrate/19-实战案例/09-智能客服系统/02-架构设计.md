...
BEGIN
    BEGIN
        IF p_query_vector IS NULL THEN
            RAISE EXCEPTION 'query_vector不能为NULL';
        END IF;
...
BEGIN
    ...
    examples TEXT[],
            ...
            RAISE NOTICE '表 support_tickets 创建成功';
        ELSE
            RAISE NOTICE '表 support_tickets 已存在';
        END IF;
    EXCEPTION
    ...
    END;
END $$;
            RAISE NOTICE '表 intent_categories 创建成功';
        ELSE
            RAISE NOTICE '表 intent_categories 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表已存在';
...
BEGIN
    BEGIN
        IF p_question IS NULL OR p_question = '' THEN
            RAISE EXCEPTION 'question不能为空';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'intent_categories') THEN
            RAISE EXCEPTION '表 intent_categories 不存在';
        END IF;
        ...
        RETURN (
            ...
            WHERE p_question LIKE ANY(examples)
            LIMIT 1
        );
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '意图分类失败: %', SQLERRM;
            RETURN 'unknown';
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'conversation_sessions') THEN
            RAISE WARNING '表 conversation_sessions 不存在，无法创建视图';
            RETURN;
        END IF;
        DROP VIEW IF EXISTS v_conversation_quality;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '性能提升：批量FAQ导入速度提升40%';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- 启用并行查询（需要在postgresql.conf中设置）
-- ALTER SYSTEM SET max_parallel_workers_per_gather = 4;
-- ALTER SYSTEM SET max_parallel_workers = 8;

-- 并行向量检索（大数据量场景，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'faqs') THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'conversation_history_cache') THEN
    ...
    cached_at TIMESTAMPTZ DEFAULT NOW(),
            ...
            RAISE NOTICE '表 conversation_history_cache 创建成功';
        ELSE
            RAISE NOTICE '表 conversation_history_cache 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表已存在';
