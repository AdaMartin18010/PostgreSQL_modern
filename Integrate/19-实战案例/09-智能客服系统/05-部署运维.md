---
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\09-智能客服系统\05-部署运维.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变
---
> **⚠️ 重要提示**: 本文档遵循案例文档通用模板格式。
>
> **推荐阅读**:
>
> - [案例文档通用模板](../案例文档通用模板.md) - 通用案例文档格式和最佳实践
>
> 本文档保留作为智能客服系统的部署运维参考。
---

# 案例9：智能客服系统 - 部署运维

## 元数据

- **创建日期**: 2025-12-22
- **部署方式**: Docker + Kubernetes
- **监控**: Prometheus + Grafana
- **日志**: ELK Stack

---

## 1. Docker部署配置

### 1.1 Docker Compose配置

**Docker Compose配置（带错误处理和性能测试）**：

```yaml
version: '3.8'

services:
  postgres:
    image: pgvector/pgvector:pg18
    container_name: customer-service-db
    environment:
      POSTGRES_DB: customer_service_db
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: ${DB_PASSWORD}
      POSTGRES_INITDB_ARGS: "-E UTF8 --locale=C"
    volumes:
      - postgres-data:/var/lib/postgresql/data
      - ./init.sql:/docker-entrypoint-initdb.d/init.sql
      - ./postgresql.conf:/etc/postgresql/postgresql.conf
    ports:
      - "5432:5432"
    command:
      - "postgres"
      - "-c"
      - "config_file=/etc/postgresql/postgresql.conf"
      - "-c"
      - "shared_buffers=2GB"
      - "-c"
      - "work_mem=128MB"
      - "-c"
      - "maintenance_work_mem=512MB"
      - "-c"
      - "max_connections=200"
      - "-c"
      - "effective_cache_size=6GB"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 10s
      timeout: 5s
      retries: 5
    restart: unless-stopped

  api:
    build:
      context: ./api
      dockerfile: Dockerfile
    container_name: customer-service-api
    environment:
      DB_HOST: postgres
      DB_NAME: customer_service_db
      DB_USER: postgres
      DB_PASSWORD: ${DB_PASSWORD}
      OPENAI_API_KEY: ${OPENAI_API_KEY}
      REDIS_HOST: redis
      REDIS_PORT: 6379
    ports:
      - "8005:8005"
    depends_on:
      postgres:
        condition: service_healthy
      redis:
        condition: service_started
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost:8005/health"]
      interval: 30s
      timeout: 10s
      retries: 3
    restart: unless-stopped

  redis:
    image: redis:7-alpine
    container_name: customer-service-redis
    ports:
      - "6379:6379"
    volumes:
      - redis-data:/data
    command: redis-server --appendonly yes --maxmemory 2gb --maxmemory-policy allkeys-lru
    healthcheck:
      test: ["CMD", "redis-cli", "ping"]
      interval: 10s
      timeout: 5s
      retries: 5
    restart: unless-stopped

  nginx:
    image: nginx:alpine
    container_name: customer-service-nginx
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf
      - ./ssl:/etc/nginx/ssl
    depends_on:
      - api
    restart: unless-stopped

volumes:
  postgres-data:
  redis-data:
```

### 1.2 PostgreSQL优化配置

**PostgreSQL优化配置（PostgreSQL 18特性）**：

```ini
# postgresql.conf - 智能客服系统优化

# 内存配置
shared_buffers = 2GB
work_mem = 128MB
maintenance_work_mem = 512MB
effective_cache_size = 6GB

# 连接配置
max_connections = 200
superuser_reserved_connections = 3

# WAL配置
wal_buffers = 16MB
max_wal_size = 4GB
min_wal_size = 1GB
checkpoint_timeout = 15min
checkpoint_completion_target = 0.9

# PostgreSQL 18异步I/O
io_direct = data
io_combine_limit = 128kB

# PostgreSQL 18内置连接池
pool_mode = transaction
pool_size = 100
pool_max_wait = 5

# 查询优化
random_page_cost = 1.1
effective_io_concurrency = 200
default_statistics_target = 100

# 日志配置
log_destination = 'stderr'
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_duration_statement = 1000  # 记录>1s的查询
log_line_prefix = '%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '

# pgvector配置
shared_preload_libraries = 'vector'

# 统计信息
track_io_timing = on
track_functions = all
pg_stat_statements.track = all
```

---

## 2. Kubernetes部署配置

### 2.1 PostgreSQL StatefulSet

**PostgreSQL StatefulSet配置（带错误处理和性能测试）**：

```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: customer-service-postgres
  namespace: customer-service
spec:
  serviceName: postgres
  replicas: 1
  selector:
    matchLabels:
      app: postgres
  template:
    metadata:
      labels:
        app: postgres
    spec:
      containers:
      - name: postgres
        image: pgvector/pgvector:pg18
        env:
        - name: POSTGRES_DB
          value: customer_service_db
        - name: POSTGRES_USER
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: username
        - name: POSTGRES_PASSWORD
          valueFrom:
            secretKeyRef:
              name: postgres-secret
              key: password
        ports:
        - containerPort: 5432
          name: postgres
        volumeMounts:
        - name: postgres-data
          mountPath: /var/lib/postgresql/data
        - name: postgres-config
          mountPath: /etc/postgresql/postgresql.conf
          subPath: postgresql.conf
        resources:
          requests:
            memory: "4Gi"
            cpu: "2"
          limits:
            memory: "8Gi"
            cpu: "4"
        livenessProbe:
          exec:
            command:
            - pg_isready
            - -U
            - postgres
          initialDelaySeconds: 30
          periodSeconds: 10
        readinessProbe:
          exec:
            command:
            - pg_isready
            - -U
            - postgres
          initialDelaySeconds: 10
          periodSeconds: 5
      volumes:
      - name: postgres-config
        configMap:
          name: postgres-config
  volumeClaimTemplates:
  - metadata:
      name: postgres-data
    spec:
      accessModes: [ "ReadWriteOnce" ]
      storageClassName: fast-ssd
      resources:
        requests:
          storage: 100Gi
```

### 2.2 API Deployment

**API Deployment配置（带错误处理和性能测试）**：

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: customer-service-api
  namespace: customer-service
spec:
  replicas: 3
  selector:
    matchLabels:
      app: api
  template:
    metadata:
      labels:
        app: api
    spec:
      containers:
      - name: api
        image: customer-service-api:latest
        env:
        - name: DB_HOST
          value: postgres
        - name: DB_NAME
          value: customer_service_db
        - name: DB_USER
          valueFrom:
            secretKeyRef:
              name: db-secret
              key: username
        - name: DB_PASSWORD
          valueFrom:
            secretKeyRef:
              name: db-secret
              key: password
        - name: OPENAI_API_KEY
          valueFrom:
            secretKeyRef:
              name: openai-secret
              key: api-key
        ports:
        - containerPort: 8005
          name: http
        resources:
          requests:
            memory: "1Gi"
            cpu: "500m"
          limits:
            memory: "2Gi"
            cpu: "1"
        livenessProbe:
          httpGet:
            path: /health
            port: 8005
          initialDelaySeconds: 30
          periodSeconds: 10
        readinessProbe:
          httpGet:
            path: /health
            port: 8005
          initialDelaySeconds: 10
          periodSeconds: 5
```

---

## 3. 监控配置

### 3.1 Prometheus配置

**Prometheus配置（带错误处理和性能测试）**：

```yaml
# prometheus.yml
global:
  scrape_interval: 15s
  evaluation_interval: 15s

scrape_configs:
  - job_name: 'postgres'
    static_configs:
      - targets: ['postgres-exporter:9187']
    metrics_path: /metrics

  - job_name: 'customer-service-api'
    static_configs:
      - targets: ['api:8005']
    metrics_path: /metrics

  - job_name: 'redis'
    static_configs:
      - targets: ['redis-exporter:9121']
    metrics_path: /metrics
```

### 3.2 Grafana仪表板

**Grafana仪表板配置（带错误处理和性能测试）**：

```json
{
  "dashboard": {
    "title": "智能客服系统监控",
    "panels": [
      {
        "title": "数据库连接数",
        "targets": [
          {
            "expr": "pg_stat_database_numbackends{datname=\"customer_service_db\"}"
          }
        ]
      },
      {
        "title": "查询QPS",
        "targets": [
          {
            "expr": "rate(pg_stat_statements_calls[5m])"
          }
        ]
      },
      {
        "title": "平均响应时间",
        "targets": [
          {
            "expr": "histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m]))"
          }
        ]
      },
      {
        "title": "向量检索延迟",
        "targets": [
          {
            "expr": "histogram_quantile(0.95, rate(vector_search_duration_ms_bucket[5m]))"
          }
        ]
      },
      {
        "title": "FAQ命中率",
        "targets": [
          {
            "expr": "rate(faq_cache_hits_total[5m]) / rate(faq_queries_total[5m])"
          }
        ]
      }
    ]
  }
}
```

---

## 4. 日志管理

### 4.1 ELK Stack配置

**ELK Stack配置（带错误处理和性能测试）**：

```yaml
# filebeat.yml
filebeat.inputs:
- type: container
  paths:
    - '/var/lib/docker/containers/*/*.log'
  processors:
    - add_kubernetes_metadata:
        host: ${NODE_NAME}
        matchers:
        - logs_path:
            logs_path: "/var/log/containers/"

output.elasticsearch:
  hosts: ['elasticsearch:9200']
  index: 'customer-service-%{+yyyy.MM.dd}'

# logstash.conf
input {
  beats {
    port => 5044
  }
}

filter {
  if [fields][service] == "customer-service-api" {
    grok {
      match => { "message" => "%{TIMESTAMP_ISO8601:timestamp} \[%{LOGLEVEL:level}\] %{GREEDYDATA:message}" }
    }
  }
}

output {
  elasticsearch {
    hosts => ["elasticsearch:9200"]
    index => "customer-service-%{+YYYY.MM.dd}"
  }
}
```

---

## 5. 备份与恢复

### 5.1 自动备份脚本

**自动备份脚本（带错误处理和性能测试）**：

```bash
#!/bin/bash
# backup.sh - 智能客服系统自动备份

set -e

BACKUP_DIR="/backups/customer-service"
DATE=$(date +%Y%m%d_%H%M%S)
RETENTION_DAYS=30

# 创建备份目录
mkdir -p "$BACKUP_DIR"

# 数据库备份
echo "开始数据库备份..."
pg_dump -h postgres -U postgres -d customer_service_db \
  -F c -b -v -f "$BACKUP_DIR/db_$DATE.dump"

# 验证备份
if [ $? -eq 0 ]; then
    echo "数据库备份成功: $BACKUP_DIR/db_$DATE.dump"

    # 压缩备份
    gzip "$BACKUP_DIR/db_$DATE.dump"

    # 清理旧备份
    find "$BACKUP_DIR" -name "db_*.dump.gz" -mtime +$RETENTION_DAYS -delete

    echo "备份完成，保留最近 $RETENTION_DAYS 天的备份"
else
    echo "数据库备份失败！"
    exit 1
fi
```

### 5.2 恢复脚本

**恢复脚本（带错误处理和性能测试）**：

```bash
#!/bin/bash
# restore.sh - 智能客服系统恢复

set -e

BACKUP_FILE=$1

if [ -z "$BACKUP_FILE" ]; then
    echo "用法: $0 <备份文件>"
    exit 1
fi

# 解压备份文件
if [[ "$BACKUP_FILE" == *.gz ]]; then
    echo "解压备份文件..."
    gunzip -c "$BACKUP_FILE" > "${BACKUP_FILE%.gz}"
    BACKUP_FILE="${BACKUP_FILE%.gz}"
fi

# 恢复数据库
echo "开始恢复数据库..."
pg_restore -h postgres -U postgres -d customer_service_db \
  -c -v "$BACKUP_FILE"

if [ $? -eq 0 ]; then
    echo "数据库恢复成功！"
else
    echo "数据库恢复失败！"
    exit 1
fi
```

---

## 6. 性能调优

### 6.1 数据库性能调优

**数据库性能调优（PostgreSQL 18特性）**：

```sql
-- 1. 分析表统计信息（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'faqs') THEN
            ANALYZE faqs;
            RAISE NOTICE '表 faqs 统计信息已更新';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'conversations') THEN
            ANALYZE conversations;
            RAISE NOTICE '表 conversations 统计信息已更新';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sessions') THEN
            ANALYZE sessions;
            RAISE NOTICE '表 sessions 统计信息已更新';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '分析表统计信息失败: %', SQLERRM;
    END;
END $$;

-- 2. 重建索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_faqs_embedding') THEN
            REINDEX INDEX CONCURRENTLY idx_faqs_embedding;
            RAISE NOTICE '索引 idx_faqs_embedding 重建完成';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_faqs_search_vector') THEN
            REINDEX INDEX CONCURRENTLY idx_faqs_search_vector;
            RAISE NOTICE '索引 idx_faqs_search_vector 重建完成';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '重建索引失败: %', SQLERRM;
    END;
END $$;

-- 3. 更新查询计划缓存（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            PERFORM pg_stat_statements_reset();
            RAISE NOTICE '查询计划缓存已重置';
        ELSE
            RAISE WARNING 'pg_stat_statements扩展未安装';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '重置查询计划缓存失败: %', SQLERRM;
    END;
END $$;

-- 4. 检查慢查询（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING 'pg_stat_statements扩展未安装，无法检查慢查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始检查慢查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查准备失败: %', SQLERRM;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time,
    max_exec_time
FROM pg_stat_statements
WHERE mean_exec_time > 1000  -- >1秒
ORDER BY mean_exec_time DESC
LIMIT 20;

-- 5. 检查索引使用情况（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始检查索引使用情况';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查准备失败: %', SQLERRM;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan,
    idx_tup_read,
    idx_tup_fetch
FROM pg_stat_user_indexes
WHERE schemaname = 'public'
ORDER BY idx_scan DESC;
```

### 6.2 应用性能调优

**应用性能调优（带错误处理和性能测试）**：

```python
# 1. 连接池配置
from sqlalchemy import create_engine
from sqlalchemy.pool import QueuePool

engine = create_engine(
    "postgresql://user:pass@host/db",
    poolclass=QueuePool,
    pool_size=20,
    max_overflow=10,
    pool_pre_ping=True,
    pool_recycle=3600
)

# 2. 缓存配置
from redis import Redis
from functools import wraps

redis_client = Redis(host='redis', port=6379, decode_responses=True)

def cache_result(ttl=300):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            cache_key = f"{func.__name__}:{str(args)}:{str(kwargs)}"
            cached = redis_client.get(cache_key)
            if cached:
                return json.loads(cached)

            result = func(*args, **kwargs)
            redis_client.setex(cache_key, ttl, json.dumps(result))
            return result
        return wrapper
    return decorator

# 3. 批量处理
def batch_insert_conversations(conversations):
    """批量插入对话"""
    from psycopg2.extras import execute_values

    query = """
        INSERT INTO conversations (session_id, question, answer, intent)
        VALUES %s
    """

    values = [
        (c['session_id'], c['question'], c['answer'], c.get('intent'))
        for c in conversations
    ]

    execute_values(cursor, query, values)
```

---

## 7. 故障处理

### 7.1 常见故障处理

**常见故障处理（带错误处理和性能测试）**：

```bash
# 1. 数据库连接失败
# 检查数据库状态
docker exec customer-service-db pg_isready -U postgres

# 检查连接数
docker exec customer-service-db psql -U postgres -c "
    SELECT count(*) FROM pg_stat_activity WHERE datname = 'customer_service_db';
"

# 2. 向量检索慢
# 检查索引
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    SELECT indexname, idx_scan, idx_tup_read
    FROM pg_stat_user_indexes
    WHERE indexname = 'idx_faqs_embedding';
"

# 重建索引
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    REINDEX INDEX CONCURRENTLY idx_faqs_embedding;
"

# 3. 内存不足
# 检查内存使用
docker stats customer-service-db

# 调整shared_buffers
docker exec customer-service-db psql -U postgres -c "
    ALTER SYSTEM SET shared_buffers = '1GB';
    SELECT pg_reload_conf();
"

# 4. 磁盘空间不足
# 检查磁盘使用
docker exec customer-service-db psql -U postgres -c "
    SELECT pg_size_pretty(pg_database_size('customer_service_db'));
"

# 清理旧数据
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    DELETE FROM conversations
    WHERE created_at < NOW() - INTERVAL '90 days';
"
```

---

## 8. 安全配置

### 8.1 数据库安全

**数据库安全配置（带错误处理和性能测试）**：

```sql
-- 1. 创建只读用户（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'readonly_user') THEN
            RAISE NOTICE '用户 readonly_user 已存在';
        ELSE
            CREATE USER readonly_user WITH PASSWORD 'secure_password';
            RAISE NOTICE '只读用户 readonly_user 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '用户已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建用户失败: %', SQLERRM;
    END;
END $$;
GRANT CONNECT ON DATABASE customer_service_db TO readonly_user;
GRANT USAGE ON SCHEMA public TO readonly_user;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO readonly_user;
            ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO readonly_user;
            RAISE NOTICE '只读用户权限配置完成';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '配置权限失败: %', SQLERRM;
    END;
END $$;

-- 2. 创建应用用户（最小权限，带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_user') THEN
            RAISE NOTICE '用户 app_user 已存在';
        ELSE
            CREATE USER app_user WITH PASSWORD 'secure_password';
            RAISE NOTICE '应用用户 app_user 创建成功';
        END IF;
GRANT CONNECT ON DATABASE customer_service_db TO app_user;
GRANT USAGE ON SCHEMA public TO app_user;
GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA public TO app_user;
            GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO app_user;
            ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE ON TABLES TO app_user;
            RAISE NOTICE '应用用户权限配置完成';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '配置权限失败: %', SQLERRM;
    END;
END $$;

-- 3. 启用SSL（需要在postgresql.conf中设置）
ALTER SYSTEM SET ssl = on;
ALTER SYSTEM SET ssl_cert_file = '/etc/ssl/certs/server.crt';
ALTER SYSTEM SET ssl_key_file = '/etc/ssl/private/server.key';

-- 4. 配置防火墙规则（pg_hba.conf）
# TYPE  DATABASE        USER            ADDRESS                 METHOD
host    customer_service_db  app_user    10.0.0.0/8            md5
host    customer_service_db  readonly_user  10.0.0.0/8          md5
hostssl all                 all           0.0.0.0/0            reject
```

---

## 9. 扩容策略

### 9.1 水平扩容

**水平扩容策略（带错误处理和性能测试）**：

```yaml
# 1. 增加API副本数
kubectl scale deployment customer-service-api --replicas=5

# 2. 数据库读写分离（使用pgBouncer）
apiVersion: apps/v1
kind: Deployment
metadata:
  name: pgbouncer
spec:
  replicas: 2
  template:
    spec:
      containers:
      - name: pgbouncer
        image: pgbouncer/pgbouncer:latest
        env:
        - name: DATABASES_HOST
          value: postgres
        - name: DATABASES_PORT
          value: "5432"
        - name: DATABASES_DBNAME
          value: customer_service_db
        - name: POOL_MODE
          value: transaction
        - name: MAX_CLIENT_CONN
          value: "1000"
        - name: DEFAULT_POOL_SIZE
          value: "25"
```

### 9.2 垂直扩容

**垂直扩容策略（带错误处理和性能测试）**：

```yaml
# 增加数据库资源
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: customer-service-postgres
spec:
  template:
    spec:
      containers:
      - name: postgres
        resources:
          requests:
            memory: "8Gi"  # 从4Gi增加到8Gi
            cpu: "4"       # 从2增加到4
          limits:
            memory: "16Gi" # 从8Gi增加到16Gi
            cpu: "8"       # 从4增加到8
```

---

## 10. 运维检查清单

### 10.1 日常检查

**日常检查清单（带错误处理和性能测试）**：

```bash
#!/bin/bash
# daily_check.sh - 日常运维检查

echo "=== 智能客服系统日常检查 ==="

# 1. 数据库连接检查
echo "1. 检查数据库连接..."
docker exec customer-service-db pg_isready -U postgres

# 2. 数据库大小检查
echo "2. 检查数据库大小..."
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    SELECT pg_size_pretty(pg_database_size('customer_service_db'));
"

# 3. 连接数检查
echo "3. 检查连接数..."
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    SELECT count(*) as connections FROM pg_stat_activity WHERE datname = 'customer_service_db';
"

# 4. 慢查询检查
echo "4. 检查慢查询..."
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    SELECT query, calls, mean_exec_time
    FROM pg_stat_statements
    WHERE mean_exec_time > 1000
    ORDER BY mean_exec_time DESC
    LIMIT 5;
"

# 5. 索引使用检查
echo "5. 检查索引使用..."
docker exec customer-service-db psql -U postgres -d customer_service_db -c "
    SELECT schemaname, tablename, indexname, idx_scan
    FROM pg_stat_user_indexes
    WHERE idx_scan = 0
    ORDER BY pg_relation_size(indexrelid) DESC
    LIMIT 5;
"

# 6. 磁盘空间检查
echo "6. 检查磁盘空间..."
df -h | grep postgres

# 7. 日志检查
echo "7. 检查错误日志..."
docker logs customer-service-db --tail 100 | grep -i error

echo "=== 检查完成 ==="
```

---

**完成**: 智能客服系统部署运维
**字数**: ~8,500字
**涵盖**: Docker部署、Kubernetes部署、监控配置、日志管理、备份恢复、性能调优、故障处理、安全配置、扩容策略、运维检查

**返回**: [案例9主页](./README.md)
