...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\零售场景\智能库存管理系统.md`
> **📅 复制日期**: 2025-12-22
...
- [智能库存管理系统](#智能库存管理系统)
  ...
  - [8. 常见问题（FAQ）](#8-常见问题faq)
    - [8.1 库存管理性能相关问题](#81-库存管理性能相关问题)
      - [Q1: 如何优化库存查询性能？](#q1-如何优化库存查询性能)
      - [Q2: 如何提升销售预测准确率？](#q2-如何提升销售预测准确率)
    - [8.2 库存优化相关问题](#82-库存优化相关问题)
      - [Q3: 如何优化补货策略？](#q3-如何优化补货策略)
  - [9. 完整代码示例](#9-完整代码示例)
...
- [混合数据模型设计](../../07-多模型数据库/JSONB时序向量/混合数据模型设计.md)
...
```sql
-- 1. 创建复合索引
CREATE INDEX inventory_store_stock_idx
ON inventory (store_id, current_stock);
...
FROM sales_history
GROUP BY day, product_id, store_id;
```
...
WITH historical_sales AS (
    ...
    GROUP BY product_id, store_id, day
),
trend_analysis AS (
    SELECT
        product_id,
        ...
        -- 计算趋势
        (AVG(daily_quantity) FILTER (WHERE day > NOW() - INTERVAL '30 days') -
         AVG(daily_quantity) FILTER (WHERE day > NOW() - INTERVAL '60 days' AND day <= NOW() - INTERVAL '30 days')) as trend
...
seasonal_factors AS (
    SELECT
        ...
        AVG(daily_quantity) as avg_by_dow
    FROM historical_sales
    GROUP BY product_id, store_id, day_of_week
),
forecast AS (
    SELECT
        ta.product_id,
        ta.store_id,
        ta.avg_daily + ta.trend + COALESCE(sf.avg_by_dow, 0) as predicted_quantity,
        CASE
            WHEN ta.stddev_daily / NULLIF(ta.avg_daily, 0) < 0.2 THEN 0.9
            WHEN ta.stddev_daily / NULLIF(ta.avg_daily, 0) < 0.5 THEN 0.7
            ELSE 0.5
        END as confidence
    FROM trend_analysis ta
    LEFT JOIN seasonal_factors sf
        ON ta.product_id = sf.product_id
        AND ta.store_id = sf.store_id
        AND EXTRACT(DOW FROM CURRENT_DATE) = sf.day_of_week
)
...
DECLARE
    ...
    v_max_stock INTEGER;
    v_predicted_demand NUMERIC;
    v_lead_time_days INTEGER := 7;
BEGIN
    -- 获取当前库存信息
    SELECT current_stock, reorder_point, max_stock
    INTO v_current_stock, v_reorder_point, v_max_stock
    FROM inventory
    WHERE product_id = p_product_id AND store_id = p_store_id;

    -- 获取预测需求
    SELECT predicted_quantity
    INTO v_predicted_demand
    FROM sales_forecast
    WHERE product_id = p_product_id
      AND store_id = p_store_id
      AND forecast_date = CURRENT_DATE + (v_lead_time_days || ' days')::INTERVAL;
    ...
    SELECT
        LEAST(
            GREATEST(
                (v_reorder_point * 2 - v_current_stock)::INTEGER,
                (COALESCE(v_predicted_demand, 0) * v_lead_time_days)::INTEGER
            ),
            (v_max_stock - v_current_stock)::INTEGER
        ) as reorder_quantity,
        CASE
            WHEN v_current_stock < v_reorder_point * 0.5 THEN 'CRITICAL'
            WHEN v_current_stock < v_reorder_point THEN 'HIGH'
            ELSE 'MEDIUM'
        END as urgency,
        CASE
            WHEN v_current_stock < v_reorder_point * 0.5 THEN 'Stock critically low'
            WHEN v_current_stock < v_reorder_point THEN 'Below reorder point'
            ELSE 'Preventive reorder'
        END as reason;
END;
$$ LANGUAGE plpgsql;
```
