...
> **📋 文档来源**: `PostgreSQL_View\08-落地案例\金融场景\实时反欺诈系统.md`
> **📅 复制日期**: 2025-12-22
...
> **技术版本**: PostgreSQL 14+, Apache AGE 1.4.0+
> **文档编号**: 08-02-01
...
- [实时反欺诈系统](#实时反欺诈系统)
  ...
  - [8. 完整代码示例](#8-完整代码示例)
    - [8.1 图查询实现](#81-图查询实现)
    - [8.2 向量查询实现](#82-向量查询实现)
    - [8.3 混合查询实现](#83-混合查询实现)
    - [8.4 Python实时检测脚本](#84-python实时检测脚本)
...
erDiagram
    ...
    transaction_graph {
        text account_id PK "图节点"
        text transaction_id PK "图边"
        numeric amount
...
- [多模数据模型设计](../../07-多模型数据库/技术原理/多模数据模型设计.md) - 图+向量混合检索原理
- [图向量联合查询](../../07-多模型数据库/图向量混合检索/图向量联合查询.md) - 图向量联合查询机制
...
SELECT * FROM cypher('fraud_graph', $$
    CREATE (a1:Account {id: 'acc_001', name: 'Account A'}),
           (a2:Account {id: 'acc_002', name: 'Account B'}),
           (a3:Account {id: 'acc_003', name: 'Account C'})
$$) AS (a agtype);

-- 创建交易关系
SELECT * FROM cypher('fraud_graph', $$
    MATCH (a1:Account {id: 'acc_001'}), (a2:Account {id: 'acc_002'})
    CREATE (a1)-[t:TRANSFER {amount: 1000, timestamp: '2025-01-01 10:00:00'}]->(a2)
$$) AS (a agtype);

-- 查询可疑账户网络（2度关系）
SELECT * FROM cypher('fraud_graph', $$
    MATCH path = (a1:Account)-[*1..2]-(a2:Account)
    WHERE a1.id = 'acc_001'
    RETURN path
    LIMIT 10
$$) AS (path agtype);
...
-- 创建向量索引
CREATE INDEX ON transaction_vectors
USING ivfflat (behavior_vector vector_cosine_ops)
WITH (lists = 100);

-- 查询相似交易模式
SELECT
    tv1.transaction_id AS transaction1,
    tv2.transaction_id AS transaction2,
    1 - (tv1.behavior_vector <=> tv2.behavior_vector) AS similarity
FROM transaction_vectors tv1
CROSS JOIN transaction_vectors tv2
WHERE tv1.transaction_id != tv2.transaction_id
  AND tv1.behavior_vector <=> tv2.behavior_vector < 0.2  -- 相似度阈值
...
```sql
-- 图向量联合查询：查找相似交易模式的账户网络
WITH similar_transactions AS (
    -- 向量查询：找到相似交易
    SELECT
        tv1.transaction_id AS tx1,
        tv2.transaction_id AS tx2,
        1 - (tv1.behavior_vector <=> tv2.behavior_vector) AS similarity
    FROM transaction_vectors tv1
    JOIN transaction_vectors tv2 ON tv1.transaction_id < tv2.transaction_id
    WHERE tv1.behavior_vector <=> tv2.behavior_vector < 0.2
),
account_network AS (
    -- 图查询：查找账户关系
    SELECT * FROM cypher('fraud_graph', $$
        MATCH (a1:Account)-[t:TRANSFER]->(a2:Account)
        RETURN a1.id AS account1, a2.id AS account2, t.amount AS amount
    $$) AS (account1 agtype, account2 agtype, amount agtype)
)
-- 联合分析：结合相似交易和账户网络
SELECT
    an.account1,
    an.account2,
    COUNT(DISTINCT st.tx1) AS similar_transaction_count,
    AVG(st.similarity) AS avg_similarity
FROM account_network an
JOIN transactions t1 ON t1.from_account = an.account1::text
JOIN transactions t2 ON t2.from_account = an.account2::text
JOIN similar_transactions st ON st.tx1 = t1.id AND st.tx2 = t2.id
GROUP BY an.account1, an.account2
HAVING COUNT(DISTINCT st.tx1) >= 3  -- 至少3笔相似交易
ORDER BY similar_transaction_count DESC;
```
...
class RealTimeFraudDetector:
    ...
    def extract_transaction_features(self, transaction: Dict) -> np.ndarray:
        ...
        features = [
            transaction['amount'],
            transaction['hour'],  # 交易时间（小时）
            transaction['day_of_week'],
            transaction['merchant_category'],
            transaction['device_type'],
            transaction['location_distance'],  # 距离上次交易位置的距离
        ]
        return np.array(features, dtype=np.float32)

    def check_graph_anomaly(self, account_id: str, transaction_id: int) -> bool:
        """检查图异常：账户关系网络异常"""
        # 查询账户的2度关系网络大小
        query = """
            SELECT COUNT(*) as network_size
            FROM cypher('fraud_graph', $$
                MATCH path = (a:Account {id: %s})-[*1..2]-(connected)
        ...
        network_size = result['network_size'] if result else 0

        # 如果关系网络异常大，可能是异常账户
        return network_size > 100

    def check_vector_similarity(self, transaction_id: int, threshold: float = 0.2) -> List[Dict]:
        """检查向量相似度：查找相似交易模式"""
        query = """
            SELECT
                tv2.transaction_id,
            ...
            FROM transaction_vectors tv1
            JOIN transaction_vectors tv2 ON tv1.transaction_id != tv2.transaction_id
            WHERE tv1.transaction_id = %s
              AND tv1.behavior_vector <=> tv2.behavior_vector < %s
            ORDER BY similarity DESC
            LIMIT 10
        """

        self.cur.execute(query, (transaction_id, threshold))
    ...
    def calculate_risk_score(self, transaction: Dict) -> float:
        ...
        # 1. 金额异常检测
        if transaction['amount'] > transaction['avg_amount'] * 3:
            risk_score += 30.0

        # 2. 时间异常检测
        if transaction['hour'] < 2 or transaction['hour'] > 23:
            risk_score += 20.0

        # 3. 位置异常检测
        if transaction['location_distance'] > 1000:  # 距离超过1000km
            risk_score += 25.0

        # 4. 图异常检测
        if self.check_graph_anomaly(transaction['account_id'], transaction['id']):
            risk_score += 15.0

        # 5. 向量相似度检测
        similar_transactions = self.check_vector_similarity(transaction['id'])
        if len(similar_transactions) >= 5:
            risk_score += 10.0

        return min(risk_score, 100.0)  # 限制在0-100之间
    ...
    def detect_fraud(self, transaction: Dict) -> Dict:
        ...
        self.cur.execute("""
            ...
            (transaction_id, account_id, risk_score, decision, created_at)
            VALUES (%s, %s, %s, %s, %s)
        """, (
            transaction['id'],
    ...
    def batch_detect(self, transactions: List[Dict]) -> List[Dict]:
        """批量检测"""
        results = []
...
detector = RealTimeFraudDetector(
    "host=localhost dbname=fraud_db user=postgres password=secret"
)
...
transaction = {
    ...
    'device_type': 'mobile',
    'location_distance': 1500.0  # 距离上次交易1500km
}

# 实时检测
result = detector.detect_fraud(transaction)
print(f"Transaction {result['transaction_id']}: Risk Score = {result['risk_score']}, Decision = {result['decision']}")
```
