...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\06-全文搜索系统\01-需求分析.md`
> **📅 复制日期**: 2025-12-22
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '高级搜索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '获取搜索建议失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '记录搜索历史失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '刷新热门搜索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '索引维护查询失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '权限搜索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
