...
> **📋 文档来源**: `DataBaseTheory\19-场景案例库\03-IoT时序数据系统\03-数据库设计.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    ...
    IF null_count > 0 THEN
        RETURN QUERY SELECT
            'NULL值检查'::TEXT,
            format('发现 %s 条记录存在NULL值', null_count)::TEXT,
            'fail'::TEXT;
    ELSE
        RETURN QUERY SELECT
            'NULL值检查'::TEXT,
    ...
    IF outlier_count > 0 THEN
        RETURN QUERY SELECT
            '异常值检查'::TEXT,
            format('发现 %s 条记录超出正常范围', outlier_count)::TEXT,
            'warning'::TEXT;
    ELSE
        RETURN QUERY SELECT
            '异常值检查'::TEXT,
    ...
    IF duplicate_count > 0 THEN
        RETURN QUERY SELECT
            '重复数据检查'::TEXT,
            format('发现 %s 条重复记录', duplicate_count)::TEXT,
            'warning'::TEXT;
    ELSE
        RETURN QUERY SELECT
            '重复数据检查'::TEXT,
...
BEGIN
    ...
    expected_points := expected_devices * expected_points * 86400;  -- 每天86400秒
    ...
    RETURN QUERY SELECT
        ...
        CASE
            WHEN ABS(expected_points - actual_points) < expected_points * 0.05 THEN '正常'
            WHEN ABS(expected_points - actual_points) < expected_points * 0.1 THEN '警告'
            ELSE '异常'
        END;
...
SELECT
    ...
    CASE
        WHEN MAX(sd.timestamp) < NOW() - INTERVAL '1 hour' THEN '离线'
        WHEN MAX(sd.timestamp) < NOW() - INTERVAL '10 minutes' THEN '警告'
        ELSE '在线'
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '异常检测失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
