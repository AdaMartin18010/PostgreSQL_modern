...
# 1. 下载备份文件
aws s3 sync s3://$S3_BUCKET/full/$BACKUP_DATE/ $RESTORE_DIR/

# 2. 下载WAL文件
aws s3 sync s3://$S3_BUCKET/wal/ $RESTORE_DIR/pg_wal/

# 3. 恢复数据库
pg_ctl stop -D /var/lib/postgresql/18/main
rm -rf /var/lib/postgresql/18/main/*
cp -r $RESTORE_DIR/* /var/lib/postgresql/18/main/
...
# Barman恢复配置
barman recover \
  --target-time '2025-01-01 12:00:00' \
  --remote-ssh-command 'ssh postgres@restore-server' \
  postgresql-server \
  /var/lib/postgresql/18/restored \
  postgresql-restored
```
