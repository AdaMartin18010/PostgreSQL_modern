...
BEGIN
    BEGIN
        -- 创建数据表空间1
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_data1') THEN
            CREATE TABLESPACE ts_data1 LOCATION '/data/postgresql/data1';
            RAISE NOTICE '表空间 ts_data1 创建成功';
        ELSE
            RAISE WARNING '表空间 ts_data1 已存在';
        END IF;

        -- 创建数据表空间2
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_data2') THEN
            CREATE TABLESPACE ts_data2 LOCATION '/data/postgresql/data2';
            RAISE NOTICE '表空间 ts_data2 创建成功';
        ELSE
            RAISE WARNING '表空间 ts_data2 已存在';
        END IF;

        -- 创建索引表空间
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_index') THEN
            CREATE TABLESPACE ts_index LOCATION '/data/postgresql/index';
            RAISE NOTICE '表空间 ts_index 创建成功';
        ELSE
            RAISE WARNING '表空间 ts_index 已存在';
        END IF;
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '权限不足，无法创建表空间。请确保目录存在且PostgreSQL用户有权限访问';
        WHEN OTHERS THEN
            RAISE WARNING '创建表空间失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_data1') THEN
                RAISE EXCEPTION '表空间 ts_data1 不存在';
            END IF;
            ...
            RAISE NOTICE '表 orders 创建成功（表空间: ts_data1）';
        ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_data2') THEN
                RAISE EXCEPTION '表空间 ts_data2 不存在';
            END IF;
            ...
            RAISE NOTICE '表 order_items 创建成功（表空间: ts_data2）';
        ELSE
...
BEGIN
    BEGIN
        -- 创建orders表的索引
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'orders_customer_idx') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_index') THEN
                RAISE EXCEPTION '表空间 ts_index 不存在';
            END IF;
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            TABLESPACE ts_index;
            RAISE NOTICE '索引 orders_customer_idx 创建成功（表空间: ts_index）';
        ELSE
            RAISE WARNING '索引 orders_customer_idx 已存在';
        END IF;

        -- 创建order_items表的索引
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'order_items_order_idx') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_index') THEN
                RAISE EXCEPTION '表空间 ts_index 不存在';
            END IF;
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            ...
            TABLESPACE ts_index;
            RAISE NOTICE '索引 order_items_order_idx 创建成功（表空间: ts_index）';
        ELSE
            RAISE WARNING '索引 order_items_order_idx 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '索引已存在';
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询表空间数量失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        SELECT COUNT(*) INTO io_count FROM pg_stat_io WHERE object = 'relation';
        RAISE NOTICE '共有 % 条I/O统计记录', io_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING 'pg_stat_io 视图不存在（PostgreSQL 16+）';
        WHEN OTHERS THEN
            RAISE WARNING '查询I/O性能失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_fast') THEN
            ...
            RAISE NOTICE '表空间 ts_fast 创建成功';
        ELSE
            RAISE WARNING '表空间 ts_fast 已存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_slow') THEN
            CREATE TABLESPACE ts_slow LOCATION '/slow_disk/postgresql';
            RAISE NOTICE '表空间 ts_slow 创建成功';
        ELSE
            RAISE WARNING '表空间 ts_slow 已存在';
        END IF;
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '权限不足，无法创建表空间';
...
-- 2. 分离数据和索引：提升I/O性能（带错误处理）
-- 注意：示例代码，实际使用时需要替换表名和列名
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_data') THEN
            RAISE EXCEPTION '表空间 ts_data 不存在';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_index') THEN
            RAISE EXCEPTION '表空间 ts_index 不存在';
        END IF;
        ...
        RAISE NOTICE '表和索引应创建在指定表空间中';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '表空间不存在，请先创建';
        WHEN OTHERS THEN
            RAISE WARNING '操作失败: %', SQLERRM;
    END;
END $$;

-- 3. 分离热冷数据：优化存储成本（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_ssd') THEN
            RAISE EXCEPTION '表空间 ts_ssd 不存在';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'ts_hdd') THEN
            RAISE EXCEPTION '表空间 ts_hdd 不存在';
        END IF;
        -- CREATE TABLE hot_data (...) TABLESPACE ts_ssd;
        -- CREATE TABLE cold_data (...) TABLESPACE ts_hdd;
        RAISE NOTICE '热数据和冷数据应创建在对应表空间中';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '表空间不存在，请先创建';
        WHEN OTHERS THEN
...
```sql
-- 1. 检查表当前表空间（带错误处理和性能测试）
DO $$
DECLARE
    table_exists BOOLEAN;
    current_tablespace TEXT;
BEGIN
    BEGIN
        SELECT EXISTS (
        ...
        SELECT tablespace INTO current_tablespace
        FROM pg_tables
        WHERE schemaname = 'public' AND tablename = 'your_table';

        IF current_tablespace IS NULL THEN
            RAISE NOTICE '表 your_table 使用默认表空间';
        ELSE
            RAISE NOTICE '表 your_table 当前表空间: %', current_tablespace;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询表空间失败: %', SQLERRM;
...
BEGIN
    BEGIN
        ...
        SELECT pg_total_relation_size('public.your_table') INTO table_size;
        RAISE NOTICE '表 your_table 总大小: %', pg_size_pretty(table_size);
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 your_table 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'your_index') THEN
            RAISE EXCEPTION '索引 your_index 不存在';
        END IF;
        ...
        RAISE NOTICE '索引已移动到新表空间（注释已移除，可执行）';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '索引或表空间不存在';
        WHEN OTHERS THEN
            RAISE WARNING '移动索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 3. 批量移动（使用pg_repack，不阻塞，带错误处理）
DO $$
BEGIN
    BEGIN
        -- 检查pg_repack扩展是否存在
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_repack') THEN
            RAISE NOTICE 'pg_repack 扩展未安装，正在尝试安装...';
            CREATE EXTENSION IF NOT EXISTS pg_repack;
            RAISE NOTICE 'pg_repack 扩展安装成功';
        ELSE
            RAISE NOTICE 'pg_repack 扩展已存在';
        END IF;
        ...
        RAISE NOTICE 'pg_repack 移动表完成（注释已移除，可执行）';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '扩展、表或表空间不存在';
        WHEN OTHERS THEN
            RAISE WARNING 'pg_repack 操作失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        SELECT COALESCE(SUM(pg_total_relation_size(oid)), 0) INTO total_size
        FROM pg_class
        WHERE relname LIKE '%_toast%';

        RAISE NOTICE '共有 % 个TOAST表，总大小: %', toast_count, pg_size_pretty(total_size);
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询TOAST表大小失败: %', SQLERRM;
    END;
END $$;
...
DO $$
DECLARE
    large_column_count INT;
BEGIN
    BEGIN
        SELECT COUNT(*) INTO large_column_count
        FROM information_schema.columns
        WHERE table_schema = 'public'
            AND (data_type LIKE '%TEXT%' OR data_type LIKE '%BYTEA%');

        RAISE NOTICE 'public schema 共有 % 个大对象列', large_column_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询大对象列失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        -- 降低阈值，更多数据进入TOAST
        RAISE NOTICE 'TOAST阈值设置成功: 2000';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 your_table 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '调整TOAST阈值失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        -- 检查PostgreSQL版本是否支持compression选项（PostgreSQL 14+）
        IF (SELECT setting::int FROM pg_settings WHERE name = 'server_version_num') < 140000 THEN
            RAISE WARNING 'PostgreSQL版本不支持compression选项（需要14+）';
            RETURN;
        END IF;

        ALTER TABLE your_table SET (compression = 'lz4');
        -- 启用压缩，减少TOAST空间
        RAISE NOTICE '压缩设置成功: lz4';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 your_table 不存在';
        WHEN invalid_parameter_value THEN
            RAISE WARNING '压缩选项无效，请检查PostgreSQL版本和编译选项';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE WARNING '警告：VACUUM FULL 会锁定表，大表可能需要较长时间';
        -- VACUUM FULL your_table;
        -- 清理TOAST中的死元组
        RAISE NOTICE 'VACUUM FULL 执行完成（注释已移除，可执行）';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 your_table 不存在';
        WHEN lock_not_available THEN
            RAISE WARNING '无法获取表锁，表可能正在被使用';
        WHEN OTHERS THEN
            RAISE WARNING 'VACUUM FULL 失败: %', SQLERRM;
            RAISE;
    END;
END $$;
