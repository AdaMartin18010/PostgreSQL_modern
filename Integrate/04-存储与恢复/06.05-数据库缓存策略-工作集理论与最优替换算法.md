...
> **📋 文档来源**: `DataBaseTheory\06-存储与恢复\06.05-数据库缓存策略-工作集理论与最优替换算法.md`
> **📅 复制日期**: 2025-12-22
...
```mermaid
graph TD
    A[LRU竞争比定理] --> B[划分访问序列为块]
    B --> C[分析OPT未命中下界]
    C --> D[分析LRU未命中上界]
    D --> E[建立关系]
    E --> F[miss_LRU≤k·miss_OPT]
...
BEGIN
    ...
    RAISE NOTICE 'shared_buffers: % (默认: 128MB, 推荐: 系统内存的25%%, 但不超过8GB)', shared_buffers_val;

    SHOW effective_cache_size INTO effective_cache_size_val;
    RAISE NOTICE 'effective_cache_size: % (默认: 4GB, 推荐: 系统内存的50-75%%, 包括操作系统缓存)', effective_cache_size_val;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看缓存配置失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：查看缓存统计（带性能测试和错误处理）
DO $$
DECLARE
    bgwriter_count INT;
BEGIN
    ...
    IF bgwriter_count = 0 THEN
        RAISE WARNING '未找到缓存统计信息';
    ELSE
        RAISE NOTICE '找到缓存统计信息';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询缓存统计失败: %', SQLERRM;
END $$;
...
-- PostgreSQL 18：查看表缓存统计（带性能测试和错误处理）
DO $$
DECLARE
    table_count INT;
BEGIN
    ...
    IF table_count = 0 THEN
        RAISE WARNING '未找到表缓存统计信息';
    ELSE
        RAISE NOTICE '找到 % 个表的缓存统计信息', table_count;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询表缓存统计失败: %', SQLERRM;
END $$;
...
-- PostgreSQL 18：查看整体缓存命中率（带性能测试和错误处理）
DO $$
DECLARE
    total_tables INT;
BEGIN
    SELECT COUNT(*) INTO total_tables FROM pg_statio_user_tables;

    IF total_tables = 0 THEN
        RAISE WARNING '未找到表统计信息';
    ELSE
        RAISE NOTICE '找到 % 个表的统计信息', total_tables;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    RAISE NOTICE 'shared_buffers 已设置为 2GB (对于8GB内存系统，推荐2GB; 对于16GB内存系统，推荐4GB; 对于32GB+内存系统，推荐8GB上限)';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '调整共享缓冲区失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：调整有效缓存大小（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = current_user AND rolsuper = true) THEN
        RAISE EXCEPTION '需要超级用户权限来配置系统参数';
    ...
    RAISE NOTICE 'effective_cache_size 已设置为 6GB (应该设置为系统内存的50-75%%, 包括操作系统页面缓存)';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '调整有效缓存大小失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：调整后台写入器（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = current_user AND rolsuper = true) THEN
        RAISE EXCEPTION '需要超级用户权限来配置系统参数';
    ...
    RAISE NOTICE '后台写入器配置已更新 (bgwriter_delay: 200ms, bgwriter_lru_maxpages: 100, bgwriter_lru_multiplier: 2.0)';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '调整后台写入器失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：查看缓存配置（带性能测试和错误处理）
DO $$
DECLARE
    setting_count INT;
BEGIN
    ...
    IF setting_count = 0 THEN
        RAISE WARNING '未找到缓存配置';
    ELSE
        RAISE NOTICE '找到 % 个缓存配置项', setting_count;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询缓存配置失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'effective_io_concurrency 已设置为 200 (推荐值: SSD为200, HDD为2-4)';
...
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '启用顺序扫描预取失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：启用维护I/O并发（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = current_user AND rolsuper = true) THEN
        RAISE EXCEPTION '需要超级用户权限来配置系统参数';
    ...
    RAISE NOTICE 'maintenance_io_concurrency 已设置为 10 (用于VACUUM、CREATE INDEX等维护操作)';
...
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '启用维护I/O并发失败: %', SQLERRM;
END $$;

-- PostgreSQL 18：查看I/O统计（PostgreSQL 16+，带错误处理和性能测试）
DO $$
DECLARE
    io_count INT;
BEGIN
    SELECT COUNT(*) INTO io_count FROM pg_stat_io;

    IF io_count = 0 THEN
        RAISE WARNING '未找到I/O统计信息（可能需要PostgreSQL 16+）';
    ELSE
        RAISE NOTICE '找到I/O统计信息';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_io视图不存在（需要PostgreSQL 16+）';
    WHEN OTHERS THEN
...
-- 执行时间: <10ms（如果视图存在）
-- 计划: Limit -> Sort -> Seq Scan
-- 注意: pg_stat_io视图需要PostgreSQL 16+
...
BEGIN
    ...
    PERFORM pg_reload_conf();
    RAISE NOTICE '共享缓冲区配置已设置 (shared_buffers: 4GB, effective_cache_size: 12GB)';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '配置共享缓冲区失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    PERFORM pg_reload_conf();
    RAISE NOTICE '后台写入器配置已优化';
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '优化后台写入器失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '测试表创建成功: orders (包含索引)';
EXCEPTION
...
-- 6. 查看缓存命中率（带性能测试和错误处理）
DO $$
DECLARE
    table_exists BOOLEAN;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM pg_statio_user_tables WHERE relname = 'orders'
    ) INTO table_exists;

    IF NOT table_exists THEN
        RAISE WARNING '表 orders 不存在';
    ELSE
        RAISE NOTICE '找到表 orders 的缓存统计信息';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询缓存命中率失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    IF bgwriter_count = 0 THEN
        RAISE WARNING '未找到后台写入器统计信息';
    ELSE
        RAISE NOTICE '找到后台写入器统计信息';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控缓存性能失败: %', SQLERRM;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_fact') THEN
        ...
        RAISE NOTICE '已删除现有表: sales_fact';
    END IF;
    ...
    RAISE NOTICE '分析表创建成功: sales_fact (包含索引)';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表 sales_fact 已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建分析表失败: %', SQLERRM;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_prewarm') THEN
        ...
        RAISE NOTICE 'pg_prewarm扩展已创建';
    ELSE
        RAISE NOTICE 'pg_prewarm扩展已存在';
    END IF;
EXCEPTION
    WHEN duplicate_object THEN
        RAISE NOTICE 'pg_prewarm扩展已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建pg_prewarm扩展失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    SELECT pg_prewarm('sales_fact', 'buffer', 'main') INTO prewarm_result;
    RAISE NOTICE '表预热完成: sales_fact (预热块数: %)', prewarm_result;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表 sales_fact 不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '预热表失败: %', SQLERRM;
END $$;
...
DECLARE
    prewarm_result BIGINT;
BEGIN
    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_sales_date') THEN
        SELECT pg_prewarm('idx_sales_date', 'buffer', 'main') INTO prewarm_result;
        RAISE NOTICE '索引预热完成: idx_sales_date (预热块数: %)', prewarm_result;
    ELSE
        RAISE WARNING '索引 idx_sales_date 不存在';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_sales_product') THEN
        SELECT pg_prewarm('idx_sales_product', 'buffer', 'main') INTO prewarm_result;
        RAISE NOTICE '索引预热完成: idx_sales_product (预热块数: %)', prewarm_result;
    ELSE
        RAISE WARNING '索引 idx_sales_product 不存在';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_sales_customer') THEN
        SELECT pg_prewarm('idx_sales_customer', 'buffer', 'main') INTO prewarm_result;
        RAISE NOTICE '索引预热完成: idx_sales_customer (预热块数: %)', prewarm_result;
    ELSE
        RAISE WARNING '索引 idx_sales_customer 不存在';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '预热索引失败: %', SQLERRM;
END $$;
...
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM pg_statio_user_tables WHERE relname = 'sales_fact'
    ) INTO table_exists;

    IF NOT table_exists THEN
        RAISE WARNING '表 sales_fact 不存在';
    ELSE
        RAISE NOTICE '找到表 sales_fact 的缓存统计信息';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询预热效果失败: %', SQLERRM;
END $$;
...
LIMIT 100;
-- 执行时间: <500ms（取决于数据量）
-- 计划: Limit -> Sort -> HashAggregate -> Seq Scan

-- 6. 监控工作集大小（带性能测试和错误处理）
-- 工作集大小 = 最近访问的不同页面数
...
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM pg_stat_user_tables WHERE relname = 'sales_fact'
    ) INTO table_exists;

    IF NOT table_exists THEN
        RAISE WARNING '表 sales_fact 不存在';
    ELSE
        RAISE NOTICE '找到表 sales_fact 的统计信息';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控工作集大小失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'effective_cache_size 已设置为 12GB (推荐: 系统内存的50-75%%)';
...
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置有效缓存大小失败: %', SQLERRM;
END $$;
...
BEGIN
    ...
    ALTER SYSTEM SET effective_io_concurrency = '200';
    RAISE NOTICE 'effective_io_concurrency 已设置为 200 (SSD推荐值, HDD推荐2-4)';
...
EXCEPTION
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，无法修改系统配置';
    WHEN OTHERS THEN
        RAISE EXCEPTION '配置I/O并发失败: %', SQLERRM;
END $$;
