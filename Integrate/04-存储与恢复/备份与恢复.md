...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '共有 % 个表，总大小: %', table_count, pg_size_pretty(COALESCE(total_size, 0));
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查备份状态失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        SELECT COUNT(*) INTO archived_count FROM pg_stat_archiver;

        IF archived_count = 0 THEN
            RAISE WARNING 'pg_stat_archiver 视图无数据，WAL归档可能未启用';
            RETURN;
        END IF;

        SELECT archived_count, failed_count INTO archived_count, failed_count
        FROM pg_stat_archiver;

        RAISE NOTICE 'WAL归档: 成功 % 次，失败 % 次', archived_count, failed_count;

        IF failed_count > 0 THEN
            RAISE WARNING 'WAL归档存在失败记录，请检查配置';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING 'pg_stat_archiver 视图不存在';
        WHEN OTHERS THEN
            RAISE WARNING '检查 WAL 归档状态失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
command -v dropdb >/dev/null 2>&1 || error_exit "dropdb 命令未找到"
...
[ -s "$BACKUP_FILE" ] || error_exit "备份文件为空: $BACKUP_FILE"
echo "备份成功: $BACKUP_FILE"

# 2. 删除数据库（带错误处理）
echo "删除数据库..."
if ! dropdb -h localhost -U postgres mydb; then
    error_exit "删除数据库失败"
fi
echo "数据库删除成功"
...
# 3. 恢复（带错误处理）
echo "恢复数据库..."
if ! psql -h localhost -U postgres -d postgres < "$BACKUP_FILE"; then
    error_exit "恢复数据库失败"
fi
echo "数据库恢复成功"
```
...
-- 4. 恢复配置（postgresql.conf或recovery.conf）
-- restore_command = 'cp /backup/wal/%f %p'
