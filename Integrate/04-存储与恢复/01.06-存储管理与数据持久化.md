...
EXCEPTION
    ...
    WHEN duplicate_table THEN
        RAISE WARNING '分区已存在: %', partition_name;
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建分区失败: % (错误: %)', partition_name, SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    ...
    IF NOT tablespace_exists THEN
        RAISE WARNING '表空间不存在: %, 将使用默认表空间', tablespace_name;
    ...
    IF partition_exists THEN
        RAISE NOTICE '分区已存在: %, 跳过创建', partition_name;
        RETURN;
    END IF;
    ...
    RAISE NOTICE '分区创建成功: % (表空间: %)', partition_name, tablespace_name;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表不存在: application_logs';
    WHEN duplicate_table THEN
...
BEGIN
    IF retention_months <= 0 THEN
        RAISE EXCEPTION '保留月数必须大于0: %', retention_months;
    END IF;
    ...
    LOOP
        BEGIN
            EXECUTE format('DROP TABLE IF EXISTS %I', partition_record.tablename);
            RAISE NOTICE '已删除旧分区: %', partition_record.tablename;
            dropped_count := dropped_count + 1;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '删除分区失败: % (错误: %)', partition_record.tablename, SQLERRM;
        END;
    END LOOP;

    IF dropped_count = 0 THEN
        RAISE NOTICE '没有需要清理的分区';
    ELSE
