...
BEGIN
    BEGIN
        ...
        ALTER SYSTEM SET archive_command = 'test ! -f /backup/wal/%f && cp %p /backup/wal/%f && aws s3 cp /backup/wal/%f s3://postgresql-backups/wal/%f';
...
SELECT name, setting, source
FROM pg_settings
WHERE name IN ('wal_level', 'archive_mode', 'archive_command', 'archive_timeout', 'max_wal_size', 'min_wal_size');
```
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'backup_performance_test') THEN
            DROP TABLE backup_performance_test CASCADE;
            RAISE NOTICE '表 backup_performance_test 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '表 backup_performance_test 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 backup_performance_test 失败: %', SQLERRM;
    END;
END $$;

-- 测试备份期间的性能（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'backup_performance_test') THEN
            RAISE WARNING '表 backup_performance_test 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始测试备份期间的性能';
    EXCEPTION
        WHEN OTHERS THEN
