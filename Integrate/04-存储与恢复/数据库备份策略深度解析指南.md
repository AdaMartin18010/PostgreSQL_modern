...
# 4. 上传到S3
aws s3 sync $BACKUP_PATH s3://postgresql-backups/full/$DATE/
...
# 4. 上传到S3
aws s3 cp ${BACKUP_FILE}.gz s3://postgresql-backups/full/
```
...
```bash
# 使用pigz多线程压缩
pg_dump -h localhost -U postgres -F c -b -v -f - mydb | \
  pigz -p 4 > /backup/postgresql/full_backup_$(date +%Y%m%d).dump.gz
```
...
# 1. 获取上次备份的LSN
LAST_LSN=$(cat $FULL_BACKUP_PATH/backup_manifest | jq -r '.WAL-Ranges[0].End-LSN')
...
# 3. 验证增量备份
pg_verifybackup -D $INCREMENTAL_PATH

# 4. 上传到S3
aws s3 sync $INCREMENTAL_PATH s3://postgresql-backups/incremental/$INCREMENTAL_DATE/
```
...
# 1. 恢复全量备份
pg_combinebackup \
  --manifest /backup/full/backup_manifest \
  --pgdata /var/lib/postgresql/18/restored

# 2. 应用增量备份
pg_combinebackup \
  --manifest /backup/incremental/backup_manifest \
  --pgdata /var/lib/postgresql/18/restored

# 3. 启动数据库
pg_ctl start -D /var/lib/postgresql/18/restored
```
...
# 3. 清理本地WAL（保留1天）
find $ARCHIVE_DIR -name "*.wal" -mtime +1 -delete

# 4. 清理S3旧WAL（保留30天）
aws s3api list-objects --bucket $S3_BUCKET --prefix wal/ | \
  jq -r '.Contents[] | select(.LastModified < "'$(date -d '30 days ago' -Iseconds)'") | .Key' | \
...
# 使用pg_archivecleanup清理旧WAL
pg_archivecleanup -d $ARCHIVE_DIR $(psql -t -c "SELECT pg_walfile_name(pg_current_wal_lsn() - '1 day'::interval);")
...
# 使用高压缩比
pg_dump -h localhost -U postgres -F c -b -v -f - mydb | \
  gzip -9 > /backup/postgresql/backup_$(date +%Y%m%d).dump.gz
