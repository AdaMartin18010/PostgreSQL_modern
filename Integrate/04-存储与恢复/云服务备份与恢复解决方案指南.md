...
aws s3 cp ${BACKUP_FILE}.gz s3://$S3_BUCKET/$S3_KEY \
  ...
  --metadata "backup-date=$(date -Iseconds),backup-type=full"
...
```bash
# 创建Azure Database for PostgreSQL
...
```bash
# 创建Cloud SQL实例
gcloud sql instances create postgresql-prod \
...
# 3. 上传到Cloud Storage
gsutil cp ${BACKUP_FILE}.gz $BUCKET/$OBJECT

# 4. 设置存储类
gsutil rewrite -s NEARLINE $BUCKET/$OBJECT

# 5. 验证上传
gsutil ls -l $BUCKET/$OBJECT
```
...
aws s3 cp ${BACKUP_FILE}.gz \
  s3://postgresql-backups-aws/backups/backup_$(date +%Y%m%d).dump.gz

# 3. 上传到Azure Blob
az storage blob upload \
  --account-name postgresqlbackups \
  --container-name postgresql-backups \
  --name backups/backup_$(date +%Y%m%d).dump.gz \
  --file ${BACKUP_FILE}.gz

# 4. 上传到GCP Cloud Storage
