# 成本效益分析

> **创建日期**: 2025年1月
> **来源**: 数据建模经济学
> **状态**: ✅ 已完成
> **文档编号**: 03-03

---

## 📑 目录

- [成本效益分析](#成本效益分析)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 成本效益分析理论](#111-成本效益分析理论)
    - [1.1.2 成本分析理论](#112-成本分析理论)
    - [1.1.3 效益分析理论](#113-效益分析理论)
    - [1.1.4 ROI计算理论](#114-roi计算理论)
    - [1.1.5 决策理论](#115-决策理论)
    - [1.1.6 复杂度分析](#116-复杂度分析)
  - [2. 建模成本分析](#2-建模成本分析)
    - [2.1 成本分类](#21-成本分类)
    - [2.2 成本估算模型](#22-成本估算模型)
    - [2.3 不同建模方案成本对比](#23-不同建模方案成本对比)
  - [3. 性能效益分析](#3-性能效益分析)
    - [3.1 性能指标](#31-性能指标)
    - [3.2 性能效益量化](#32-性能效益量化)
    - [3.3 查询性能优化效益](#33-查询性能优化效益)
  - [4. 维护成本分析](#4-维护成本分析)
    - [4.1 维护成本构成](#41-维护成本构成)
    - [4.2 维护成本估算](#42-维护成本估算)
    - [4.3 不同建模方案的维护成本对比](#43-不同建模方案的维护成本对比)
  - [5. ROI计算方法](#5-roi计算方法)
    - [5.1 ROI基本公式](#51-roi基本公式)
    - [5.2 综合ROI计算](#52-综合roi计算)
    - [5.3 多方案ROI对比](#53-多方案roi对比)
  - [6. PostgreSQL实现](#6-postgresql实现)
    - [6.1 综合成本效益分析系统](#61-综合成本效益分析系统)
  - [7. 实际应用案例 / Practical Application Examples](#7-实际应用案例--practical-application-examples)
    - [7.1 案例1: 索引优化成本效益分析](#71-案例1-索引优化成本效益分析)
    - [7.2 案例2: 分区策略成本效益分析](#72-案例2-分区策略成本效益分析)
    - [7.3 案例3: 列存表成本效益分析](#73-案例3-列存表成本效益分析)
  - [8. 常见问题解答 / FAQ](#8-常见问题解答--faq)
    - [Q1: 如何量化建模成本？](#q1-如何量化建模成本)
    - [Q2: 如何评估性能效益？](#q2-如何评估性能效益)
    - [Q3: ROI计算应该考虑哪些因素？](#q3-roi计算应该考虑哪些因素)
    - [Q4: 如何比较多个建模方案？](#q4-如何比较多个建模方案)
    - [Q5: 成本效益分析如何持续跟踪？](#q5-成本效益分析如何持续跟踪)
  - [7. 相关资源](#7-相关资源)

---

## 1. 概述

成本效益分析帮助评估不同建模方案的投入产出比，为决策提供量化依据。
通过系统化的成本效益分析，可以在多个建模方案中选择最优解。

**核心价值**:

- **量化决策**：将主观判断转化为客观数据
- **资源优化**：合理分配建模资源
- **风险控制**：识别高成本低效益的方案
- **持续改进**：跟踪建模效果，持续优化

---

## 1.1 理论基础

### 1.1.1 成本效益分析理论

**成本效益分析（Cost-Benefit Analysis）**:

- **定义**: 评估不同方案的投入产出比
- **目的**: 为决策提供量化依据
- **方法**: 成本分析、效益分析、ROI计算

**分析原则**:

- **量化**: 尽可能量化成本和效益
- **全面**: 考虑所有成本和效益
- **时间**: 考虑时间价值

### 1.1.2 成本分析理论

**成本分析**:

- **成本分类**: 人力成本、时间成本、技术成本、机会成本、风险成本
- **成本估算**: 成本估算模型和方法
- **成本对比**: 不同方案的成本对比

**成本估算**:

- **人力成本**: $C_{human} = \sum_{i=1}^{n} D_i \times R_i$ where D is days, R is rate
- **时间成本**: $C_{time} = T \times R_t$ where T is time, R_t is time rate
- **总成本**: $C_{total} = C_{human} + C_{time} + C_{tech} + C_{risk}$

### 1.1.3 效益分析理论

**效益分析**:

- **效益分类**: 性能效益、维护效益、扩展效益
- **效益量化**: 效益量化方法
- **效益对比**: 不同方案的效益对比

**效益量化**:

- **性能效益**: $B_{perf} = \Delta T \times V$ where $\Delta T$ is time saved, V is value
- **维护效益**: $B_{maint} = \Delta M \times V$ where $\Delta M$ is maintenance saved
- **总效益**: $B_{total} = B_{perf} + B_{maint} + B_{ext}$

### 1.1.4 ROI计算理论

**ROI（Return on Investment）**:

- **定义**: 投资回报率
- **公式**: $ROI = \frac{B - C}{C} \times 100\%$ where B is benefit, C is cost
- **应用**: 评估投资回报

**ROI计算**:

- **简单ROI**: $ROI = \frac{B - C}{C}$
- **时间ROI**: $ROI_t = \frac{\sum_{i=1}^{n} \frac{B_i - C_i}{(1+r)^i}}{C_0}$ where r is discount rate
- **综合ROI**: 考虑多个因素的ROI

### 1.1.5 决策理论

**决策方法**:

- **多方案对比**: 对比多个方案的ROI
- **风险评估**: 评估方案风险
- **决策建议**: 基于ROI和风险做决策

**决策原则**:

- **ROI优先**: 优先选择ROI高的方案
- **风险控制**: 控制方案风险
- **综合评估**: 综合ROI和风险

### 1.1.6 复杂度分析

**分析复杂度**:

- **成本分析**: $O(C)$ where C is cost categories
- **效益分析**: $O(B)$ where B is benefit categories
- **ROI计算**: $O(1)$ (constant time)

---

## 2. 建模成本分析

### 2.1 成本分类

**建模成本构成**:

| 成本类型 | 说明 | 示例 |
|---------|------|------|
| 人力成本 | 建模师、开发人员工时 | 1000元/人天 |
| 时间成本 | 项目延期成本 | 5000元/天 |
| 技术成本 | 工具、培训费用 | 10000元 |
| 机会成本 | 选择A方案放弃B方案的成本 | 难以量化 |
| 风险成本 | 方案失败的风险 | 概率×损失 |

### 2.2 成本估算模型

```sql
-- 建模成本估算表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'modeling_cost_estimate') THEN
        CREATE TABLE modeling_cost_estimate (
            estimate_id SERIAL PRIMARY KEY,
            project_name VARCHAR(200) NOT NULL,
            -- 人力成本
            designer_days INT DEFAULT 0,
            developer_days INT DEFAULT 0,
            tester_days INT DEFAULT 0,
            daily_rate NUMERIC(10,2) DEFAULT 1000.00,
            -- 时间成本
            project_duration_days INT,
            delay_cost_per_day NUMERIC(10,2) DEFAULT 5000.00,
            -- 技术成本
            tool_license_cost NUMERIC(10,2) DEFAULT 0,
            training_cost NUMERIC(10,2) DEFAULT 0,
            -- 风险成本
            risk_probability NUMERIC(5,2) DEFAULT 0, -- 0-1
            risk_loss NUMERIC(10,2) DEFAULT 0,
            -- 计算总成本
            total_cost NUMERIC(12,2) GENERATED ALWAYS AS (
                -- 人力成本
                (designer_days + developer_days + tester_days) * daily_rate +
                -- 时间成本（延期）
                GREATEST(0, project_duration_days - 30) * delay_cost_per_day +
                -- 技术成本
                tool_license_cost + training_cost +
                -- 风险成本
                risk_probability * risk_loss
            ) STORED,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 modeling_cost_estimate 创建成功';
    ELSE
        RAISE NOTICE '表 modeling_cost_estimate 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 modeling_cost_estimate 失败: %', SQLERRM;
END $$;

-- 示例：电商系统建模成本估算
INSERT INTO modeling_cost_estimate (
    project_name, designer_days, developer_days, tester_days,
    project_duration_days, tool_license_cost, training_cost,
    risk_probability, risk_loss
) VALUES (
    '电商系统', 10, 20, 5,
    45, 5000, 3000,
    0.1, 50000
);

-- 查询成本明细
SELECT
    project_name,
    (designer_days + developer_days + tester_days) * daily_rate AS labor_cost,
    GREATEST(0, project_duration_days - 30) * delay_cost_per_day AS delay_cost,
    tool_license_cost + training_cost AS tech_cost,
    risk_probability * risk_loss AS risk_cost,
    total_cost
FROM modeling_cost_estimate;
```

### 2.3 不同建模方案成本对比

```sql
-- 建模方案成本对比表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'modeling_option_cost') THEN
        CREATE TABLE modeling_option_cost (
            option_id SERIAL PRIMARY KEY,
            project_name VARCHAR(200) NOT NULL,
            option_name VARCHAR(200) NOT NULL,
            -- 成本因素
            complexity_level VARCHAR(50), -- 'low', 'medium', 'high'
            estimated_days INT,
            risk_level VARCHAR(50), -- 'low', 'medium', 'high'
            -- 成本估算
            estimated_cost NUMERIC(12,2),
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 modeling_option_cost 创建成功';
    ELSE
        RAISE NOTICE '表 modeling_option_cost 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 modeling_option_cost 失败: %', SQLERRM;
END $$;

-- 成本估算函数
CREATE OR REPLACE FUNCTION estimate_modeling_cost(
    p_complexity VARCHAR,
    p_days INT,
    p_risk_level VARCHAR
)
RETURNS NUMERIC AS $$
DECLARE
    v_base_rate NUMERIC := 1000.00;
    v_complexity_multiplier NUMERIC;
    v_risk_multiplier NUMERIC;
BEGIN
    -- 复杂度系数
    v_complexity_multiplier := CASE p_complexity
        WHEN 'low' THEN 1.0
        WHEN 'medium' THEN 1.5
        WHEN 'high' THEN 2.0
        ELSE 1.0
    END;

    -- 风险系数
    v_risk_multiplier := CASE p_risk_level
        WHEN 'low' THEN 1.0
        WHEN 'medium' THEN 1.2
        WHEN 'high' THEN 1.5
        ELSE 1.0
    END;

    RETURN p_days * v_base_rate * v_complexity_multiplier * v_risk_multiplier;
END;
$$ LANGUAGE plpgsql;
```

---

## 3. 性能效益分析

### 3.1 性能指标

**关键性能指标**:

| 指标 | 说明 | 目标值 |
|------|------|--------|
| 查询响应时间 | 平均查询耗时 | <100ms |
| 写入吞吐量 | 每秒写入次数 | >1000 TPS |
| 并发用户数 | 同时在线用户 | >1000 |
| 数据压缩比 | 压缩后/压缩前 | >3:1 |
| 索引效率 | 索引命中率 | >95% |

### 3.2 性能效益量化

```sql
-- 性能效益分析表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'performance_benefit') THEN
        CREATE TABLE performance_benefit (
            benefit_id SERIAL PRIMARY KEY,
            project_name VARCHAR(200) NOT NULL,
            -- 性能指标
            metric_name VARCHAR(100) NOT NULL,
            baseline_value NUMERIC(12,2),
            improved_value NUMERIC(12,2),
            -- 效益计算
            improvement_percentage NUMERIC(5,2) GENERATED ALWAYS AS (
                CASE
                    WHEN baseline_value > 0 THEN
                        ((baseline_value - improved_value) / baseline_value * 100)
                    ELSE 0
                END
            ) STORED,
            -- 业务价值
            business_impact_score INT CHECK (business_impact_score BETWEEN 1 AND 10),
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 performance_benefit 创建成功';
    ELSE
        RAISE NOTICE '表 performance_benefit 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 performance_benefit 失败: %', SQLERRM;
END $$;

-- 示例：索引优化效益
INSERT INTO performance_benefit (project_name, metric_name, baseline_value, improved_value, business_impact_score) VALUES
    ('电商系统', '查询响应时间(ms)', 500, 50, 9),
    ('电商系统', '写入吞吐量(TPS)', 500, 1200, 8),
    ('电商系统', '并发用户数', 500, 1500, 10);

-- 查询性能效益汇总
SELECT
    project_name,
    metric_name,
    baseline_value,
    improved_value,
    improvement_percentage,
    business_impact_score,
    improvement_percentage * business_impact_score AS weighted_benefit
FROM performance_benefit
ORDER BY weighted_benefit DESC;
```

### 3.3 查询性能优化效益

```sql
-- 查询性能对比表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'query_performance_comparison') THEN
        CREATE TABLE query_performance_comparison (
            comparison_id SERIAL PRIMARY KEY,
            query_name VARCHAR(200) NOT NULL,
            -- 优化前
            before_execution_time_ms NUMERIC(10,2),
            before_index_size_mb NUMERIC(10,2),
            -- 优化后
            after_execution_time_ms NUMERIC(10,2),
            after_index_size_mb NUMERIC(10,2),
            -- 效益计算
            speedup_ratio NUMERIC(5,2) GENERATED ALWAYS AS (
                CASE
                    WHEN after_execution_time_ms > 0 THEN
                        before_execution_time_ms / after_execution_time_ms
                    ELSE 0
                END
            ) STORED,
            space_savings_mb NUMERIC(10,2) GENERATED ALWAYS AS (
                before_index_size_mb - after_index_size_mb
            ) STORED
        );
        RAISE NOTICE '表 query_performance_comparison 创建成功';
    ELSE
        RAISE NOTICE '表 query_performance_comparison 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 query_performance_comparison 失败: %', SQLERRM;
END $$;

-- 示例：订单查询优化
INSERT INTO query_performance_comparison (
    query_name, before_execution_time_ms, before_index_size_mb,
    after_execution_time_ms, after_index_size_mb
) VALUES (
    '订单查询', 500, 500,
    50, 200
);

-- 查询优化效益
SELECT
    query_name,
    before_execution_time_ms,
    after_execution_time_ms,
    speedup_ratio,
    before_index_size_mb,
    after_index_size_mb,
    space_savings_mb,
    CASE
        WHEN speedup_ratio > 5 THEN '优秀'
        WHEN speedup_ratio > 2 THEN '良好'
        ELSE '一般'
    END AS optimization_level
FROM query_performance_comparison;
```

---

## 4. 维护成本分析

### 4.1 维护成本构成

**维护成本类型**:

| 成本类型 | 说明 | 占比 |
|---------|------|------|
| 日常维护 | 监控、备份、优化 | 40% |
| 问题修复 | Bug修复、性能调优 | 30% |
| 功能扩展 | 新需求、功能增强 | 20% |
| 文档更新 | 文档维护、培训 | 10% |

### 4.2 维护成本估算

```sql
-- 维护成本估算表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'maintenance_cost') THEN
        CREATE TABLE maintenance_cost (
            cost_id SERIAL PRIMARY KEY,
            project_name VARCHAR(200) NOT NULL,
            -- 年度维护成本
            daily_maintenance_hours NUMERIC(5,2) DEFAULT 2.0,
            hourly_rate NUMERIC(10,2) DEFAULT 200.00,
            -- 问题修复成本
            avg_incidents_per_month INT DEFAULT 2,
            avg_fix_hours NUMERIC(5,2) DEFAULT 4.0,
            -- 功能扩展成本
            feature_requests_per_year INT DEFAULT 10,
            avg_feature_hours NUMERIC(5,2) DEFAULT 16.0,
            -- 年度总成本
            annual_cost NUMERIC(12,2) GENERATED ALWAYS AS (
                -- 日常维护：2小时/天 × 200元/小时 × 250工作日
                daily_maintenance_hours * hourly_rate * 250 +
                -- 问题修复：2次/月 × 4小时 × 200元/小时 × 12月
                avg_incidents_per_month * avg_fix_hours * hourly_rate * 12 +
                -- 功能扩展：10次/年 × 16小时 × 200元/小时
                feature_requests_per_year * avg_feature_hours * hourly_rate
            ) STORED,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 maintenance_cost 创建成功';
    ELSE
        RAISE NOTICE '表 maintenance_cost 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 maintenance_cost 失败: %', SQLERRM;
END $$;

-- 示例：电商系统维护成本
INSERT INTO maintenance_cost (
    project_name, daily_maintenance_hours, avg_incidents_per_month,
    feature_requests_per_year
) VALUES (
    '电商系统', 3.0, 3, 15
);

-- 查询维护成本
SELECT
    project_name,
    daily_maintenance_hours * hourly_rate * 250 AS daily_maintenance_cost,
    avg_incidents_per_month * avg_fix_hours * hourly_rate * 12 AS incident_cost,
    feature_requests_per_year * avg_feature_hours * hourly_rate AS feature_cost,
    annual_cost
FROM maintenance_cost;
```

### 4.3 不同建模方案的维护成本对比

```sql
-- 建模方案维护成本对比（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'modeling_maintenance_comparison') THEN
        CREATE TABLE modeling_maintenance_comparison (
            comparison_id SERIAL PRIMARY KEY,
            option_name VARCHAR(200) NOT NULL,
            -- 维护复杂度
            maintenance_complexity VARCHAR(50), -- 'low', 'medium', 'high'
            -- 预估维护成本
            estimated_annual_maintenance_cost NUMERIC(12,2),
            -- 可维护性评分（1-10）
            maintainability_score INT CHECK (maintainability_score BETWEEN 1 AND 10)
        );
        RAISE NOTICE '表 modeling_maintenance_comparison 创建成功';
    ELSE
        RAISE NOTICE '表 modeling_maintenance_comparison 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 modeling_maintenance_comparison 失败: %', SQLERRM;
END $$;

-- 维护成本估算函数
CREATE OR REPLACE FUNCTION estimate_maintenance_cost(
    p_complexity VARCHAR,
    p_base_cost NUMERIC
)
RETURNS NUMERIC AS $$
DECLARE
    v_multiplier NUMERIC;
BEGIN
    v_multiplier := CASE p_complexity
        WHEN 'low' THEN 1.0
        WHEN 'medium' THEN 1.5
        WHEN 'high' THEN 2.5
        ELSE 1.0
    END;

    RETURN p_base_cost * v_multiplier;
END;
$$ LANGUAGE plpgsql;
```

---

## 5. ROI计算方法

### 5.1 ROI基本公式

**ROI计算公式**:

```text
ROI = (收益 - 成本) / 成本 × 100%
```

### 5.2 综合ROI计算

```sql
-- ROI计算表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'roi_analysis') THEN
        CREATE TABLE roi_analysis (
            roi_id SERIAL PRIMARY KEY,
            project_name VARCHAR(200) NOT NULL,
            -- 成本
            initial_investment NUMERIC(12,2) NOT NULL,
            annual_maintenance_cost NUMERIC(12,2) DEFAULT 0,
            -- 收益
            annual_revenue_increase NUMERIC(12,2) DEFAULT 0,
            annual_cost_savings NUMERIC(12,2) DEFAULT 0,
            -- 时间周期
            analysis_period_years INT DEFAULT 3,
            -- 计算ROI
            total_cost NUMERIC(12,2) GENERATED ALWAYS AS (
                initial_investment + annual_maintenance_cost * analysis_period_years
            ) STORED,
            total_benefit NUMERIC(12,2) GENERATED ALWAYS AS (
                (annual_revenue_increase + annual_cost_savings) * analysis_period_years
            ) STORED,
            roi_percentage NUMERIC(10,2) GENERATED ALWAYS AS (
                CASE
                    WHEN total_cost > 0 THEN
                        ((total_benefit - total_cost) / total_cost * 100)
                    ELSE 0
                END
            ) STORED,
            payback_period_years NUMERIC(5,2) GENERATED ALWAYS AS (
                CASE
                    WHEN (annual_revenue_increase + annual_cost_savings) > 0 THEN
                        initial_investment / (annual_revenue_increase + annual_cost_savings)
                    ELSE NULL
                END
            ) STORED,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 roi_analysis 创建成功';
    ELSE
        RAISE NOTICE '表 roi_analysis 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 roi_analysis 失败: %', SQLERRM;
END $$;

-- 示例：电商系统ROI分析
INSERT INTO roi_analysis (
    project_name, initial_investment, annual_maintenance_cost,
    annual_revenue_increase, annual_cost_savings, analysis_period_years
) VALUES (
    '电商系统', 500000, 100000,
    300000, 150000, 3
);

-- 查询ROI分析结果
SELECT
    project_name,
    initial_investment,
    total_cost,
    total_benefit,
    roi_percentage,
    payback_period_years,
    CASE
        WHEN roi_percentage > 100 THEN '优秀投资'
        WHEN roi_percentage > 50 THEN '良好投资'
        WHEN roi_percentage > 0 THEN '一般投资'
        ELSE '不推荐'
    END AS investment_recommendation
FROM roi_analysis;
```

### 5.3 多方案ROI对比

```sql
-- 多方案ROI对比视图
CREATE VIEW option_roi_comparison AS
SELECT
    option_name,
    initial_investment,
    total_cost,
    total_benefit,
    roi_percentage,
    payback_period_years,
    RANK() OVER (ORDER BY roi_percentage DESC) AS roi_rank
FROM roi_analysis
ORDER BY roi_percentage DESC;
```

---

## 6. PostgreSQL实现

### 6.1 综合成本效益分析系统

```sql
-- 成本效益综合分析视图
CREATE VIEW comprehensive_cost_benefit AS
SELECT
    c.project_name,
    -- 成本
    c.total_cost AS modeling_cost,
    m.annual_cost AS maintenance_cost,
    -- 效益
    SUM(p.improvement_percentage * p.business_impact_score) AS performance_benefit_score,
    -- ROI
    r.roi_percentage,
    r.payback_period_years,
    -- 综合评分
    (
        CASE WHEN r.roi_percentage > 0 THEN 1 ELSE 0 END * 0.4 +
        CASE WHEN r.payback_period_years < 2 THEN 1 ELSE 0 END * 0.3 +
        CASE WHEN SUM(p.improvement_percentage) > 50 THEN 1 ELSE 0 END * 0.3
    ) * 10 AS overall_score
FROM modeling_cost_estimate c
LEFT JOIN maintenance_cost m ON c.project_name = m.project_name
LEFT JOIN performance_benefit p ON c.project_name = p.project_name
LEFT JOIN roi_analysis r ON c.project_name = r.project_name
GROUP BY c.project_name, c.total_cost, m.annual_cost, r.roi_percentage, r.payback_period_years;
```

---

## 7. 实际应用案例 / Practical Application Examples

### 7.1 案例1: 索引优化成本效益分析

**场景**: 为订单表添加索引优化查询性能

**成本分析**:

```sql
-- 成本计算
INSERT INTO modeling_cost (
    project_name, cost_type, cost_item, cost_amount, cost_unit
) VALUES
    ('index_optimization', 'development', '索引设计时间', 8, 'hours'),
    ('index_optimization', 'development', '索引创建时间', 2, 'hours'),
    ('index_optimization', 'maintenance', '索引维护时间', 1, 'hours_per_month'),
    ('index_optimization', 'storage', '索引存储空间', 5, 'GB');

-- 效益分析
INSERT INTO performance_benefit (
    project_name, metric_name, before_value, after_value, improvement_percent
) VALUES
    ('index_optimization', 'query_time', 1000, 50, 95),  -- 查询时间从1000ms降到50ms
    ('index_optimization', 'throughput', 100, 2000, 1900);  -- 吞吐量提升20倍

-- ROI计算
SELECT
    project_name,
    SUM(cost_amount) AS total_cost,
    AVG(improvement_percent) AS avg_improvement,
    (AVG(improvement_percent) / SUM(cost_amount)) AS roi_ratio
FROM modeling_cost mc
JOIN performance_benefit pb ON mc.project_name = pb.project_name
WHERE mc.project_name = 'index_optimization'
GROUP BY project_name;
```

### 7.2 案例2: 分区策略成本效益分析

**场景**: 为订单表实施时间分区

**成本效益对比**:

```sql
-- 方案对比（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'cost_benefit_comparison') THEN
        CREATE TABLE cost_benefit_comparison (
            option_name VARCHAR(100) PRIMARY KEY,
            development_cost NUMERIC(10,2),
            maintenance_cost_per_month NUMERIC(10,2),
            query_performance_score INT,
            storage_cost_per_month NUMERIC(10,2),
            total_cost_per_year NUMERIC(10,2)
        );
        RAISE NOTICE '表 cost_benefit_comparison 创建成功';
    ELSE
        RAISE NOTICE '表 cost_benefit_comparison 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 cost_benefit_comparison 失败: %', SQLERRM;
END $$;

INSERT INTO cost_benefit_comparison VALUES
    ('no_partitioning', 0, 0, 30, 1000, 12000),
    ('monthly_partitioning', 40, 2, 80, 800, 10240),
    ('quarterly_partitioning', 20, 1, 60, 900, 11120);

-- 选择最优方案
SELECT
    option_name,
    query_performance_score,
    total_cost_per_year,
    (query_performance_score / total_cost_per_year) AS efficiency_score
FROM cost_benefit_comparison
ORDER BY efficiency_score DESC;
```

### 7.3 案例3: 列存表成本效益分析

**场景**: 将OLAP事实表从行存迁移到列存

**ROI分析**:

```sql
-- 成本分析
INSERT INTO modeling_cost (
    project_name, cost_type, cost_item, cost_amount, cost_unit
) VALUES
    ('columnar_migration', 'development', '迁移开发', 40, 'hours'),
    ('columnar_migration', 'testing', '测试验证', 16, 'hours'),
    ('columnar_migration', 'deployment', '部署上线', 8, 'hours');

-- 效益分析
INSERT INTO performance_benefit (
    project_name, metric_name, before_value, after_value, improvement_percent
) VALUES
    ('columnar_migration', 'query_time', 5000, 500, 90),  -- 查询时间减少90%
    ('columnar_migration', 'storage_size', 1000, 200, 80);  -- 存储空间减少80%

-- ROI计算
SELECT
    'columnar_migration' AS project_name,
    SUM(cost_amount) AS total_cost_hours,
    AVG(improvement_percent) AS avg_improvement_percent,
    CASE
        WHEN AVG(improvement_percent) > 50 THEN 'High ROI'
        WHEN AVG(improvement_percent) > 20 THEN 'Medium ROI'
        ELSE 'Low ROI'
    END AS roi_assessment
FROM modeling_cost mc
JOIN performance_benefit pb ON mc.project_name = pb.project_name
WHERE mc.project_name = 'columnar_migration'
GROUP BY mc.project_name;
```

---

## 8. 常见问题解答 / FAQ

### Q1: 如何量化建模成本？

**A**: 成本量化方法：

```sql
-- 成本分类和量化（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'cost_breakdown') THEN
        CREATE TABLE cost_breakdown (
            cost_category VARCHAR(50),
            cost_item VARCHAR(100),
            unit_cost NUMERIC(10,2),
            unit_type VARCHAR(20)
        );
        RAISE NOTICE '表 cost_breakdown 创建成功';
    ELSE
        RAISE NOTICE '表 cost_breakdown 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 cost_breakdown 失败: %', SQLERRM;
END $$;

INSERT INTO cost_breakdown VALUES
    ('development', '数据库设计', 100, 'hour'),
    ('development', 'SQL开发', 80, 'hour'),
    ('development', '测试验证', 60, 'hour'),
    ('maintenance', '日常维护', 50, 'hour'),
    ('infrastructure', '存储成本', 0.1, 'GB_per_month'),
    ('infrastructure', '计算资源', 0.5, 'CPU_hour');
```

### Q2: 如何评估性能效益？

**A**: 性能效益评估：

```sql
-- 性能指标定义（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'performance_metrics') THEN
        CREATE TABLE performance_metrics (
            metric_name VARCHAR(50) PRIMARY KEY,
            metric_type VARCHAR(20),  -- 'query_time', 'throughput', 'storage'
            target_value NUMERIC(10,2),
            unit VARCHAR(20)
        );
        RAISE NOTICE '表 performance_metrics 创建成功';
    ELSE
        RAISE NOTICE '表 performance_metrics 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 performance_metrics 失败: %', SQLERRM;
END $$;

INSERT INTO performance_metrics VALUES
    ('query_time', 'query_time', 100, 'ms'),
    ('throughput', 'throughput', 1000, 'qps'),
    ('storage_size', 'storage', 100, 'GB');

-- 效益计算
SELECT
    project_name,
    metric_name,
    before_value,
    after_value,
    (before_value - after_value) / before_value * 100 AS improvement_percent,
    CASE
        WHEN (before_value - after_value) / before_value * 100 > 50 THEN 'Excellent'
        WHEN (before_value - after_value) / before_value * 100 > 20 THEN 'Good'
        ELSE 'Fair'
    END AS improvement_level
FROM performance_benefit;
```

### Q3: ROI计算应该考虑哪些因素？

**A**: ROI计算因素：

1. **开发成本**: 设计、开发、测试时间
2. **维护成本**: 日常维护、监控、优化时间
3. **基础设施成本**: 存储、计算资源成本
4. **性能效益**: 查询性能、吞吐量提升
5. **业务价值**: 用户体验、业务效率提升

```sql
-- 综合ROI计算
CREATE OR REPLACE FUNCTION calculate_comprehensive_roi(
    p_project_name VARCHAR
)
RETURNS TABLE(
    total_cost NUMERIC,
    total_benefit NUMERIC,
    roi_percent NUMERIC,
    payback_months NUMERIC
) AS $$
DECLARE
    v_cost NUMERIC;
    v_benefit NUMERIC;
BEGIN
    -- 计算总成本（转换为货币单位）
    SELECT SUM(cost_amount * unit_cost) INTO v_cost
    FROM modeling_cost mc
    JOIN cost_breakdown cb ON mc.cost_item = cb.cost_item
    WHERE mc.project_name = p_project_name;

    -- 计算总效益（性能提升带来的价值）
    SELECT SUM(improvement_percent * 1000) INTO v_benefit  -- 假设1%提升=1000元价值
    FROM performance_benefit
    WHERE project_name = p_project_name;

    RETURN QUERY SELECT
        v_cost,
        v_benefit,
        ((v_benefit - v_cost) / NULLIF(v_cost, 0) * 100) AS roi_percent,
        (v_cost / NULLIF(v_benefit / 12, 0)) AS payback_months;
END;
$$ LANGUAGE plpgsql;
```

### Q4: 如何比较多个建模方案？

**A**: 方案对比方法：

```sql
-- 多方案对比视图
CREATE VIEW modeling_option_comparison AS
SELECT
    option_name,
    SUM(development_cost) AS dev_cost,
    SUM(maintenance_cost) AS maint_cost,
    AVG(performance_score) AS perf_score,
    AVG(cost_score) AS cost_score,
    (AVG(performance_score) / NULLIF(SUM(development_cost + maintenance_cost), 0)) AS efficiency
FROM (
    SELECT
        'option_a' AS option_name,
        100 AS development_cost,
        10 AS maintenance_cost,
        80 AS performance_score,
        70 AS cost_score
    UNION ALL
    SELECT
        'option_b' AS option_name,
        150 AS development_cost,
        15 AS maintenance_cost,
        90 AS performance_score,
        60 AS cost_score
    UNION ALL
    SELECT
        'option_c' AS option_name,
        80 AS development_cost,
        8 AS maintenance_cost,
        70 AS performance_score,
        80 AS cost_score
) options
GROUP BY option_name
ORDER BY efficiency DESC;
```

### Q5: 成本效益分析如何持续跟踪？

**A**: 持续跟踪方法：

```sql
-- 成本效益跟踪表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'cost_benefit_tracking') THEN
        CREATE TABLE cost_benefit_tracking (
            tracking_id BIGSERIAL PRIMARY KEY,
            project_name VARCHAR(100) NOT NULL,
            tracking_date DATE NOT NULL,
            actual_cost NUMERIC(10,2),
            actual_benefit NUMERIC(10,2),
            performance_metrics JSONB,
            notes TEXT
        );
        RAISE NOTICE '表 cost_benefit_tracking 创建成功';
    ELSE
        RAISE NOTICE '表 cost_benefit_tracking 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 cost_benefit_tracking 失败: %', SQLERRM;
END $$;

-- 定期跟踪
INSERT INTO cost_benefit_tracking (
    project_name, tracking_date, actual_cost, actual_benefit, performance_metrics
) VALUES (
    'index_optimization',
    CURRENT_DATE,
    1000,
    5000,
    '{"query_time_ms": 45, "throughput_qps": 2100}'::jsonb
);

-- 生成跟踪报告
SELECT
    project_name,
    tracking_date,
    actual_cost,
    actual_benefit,
    (actual_benefit - actual_cost) AS net_benefit,
    ((actual_benefit - actual_cost) / NULLIF(actual_cost, 0) * 100) AS roi_percent
FROM cost_benefit_tracking
ORDER BY tracking_date DESC;
```

---

## 7. 相关资源

- [建模决策框架](./建模决策框架.md) - 决策框架方法
- [性能优化](../08-PostgreSQL建模实践/性能优化.md) - 性能优化指南
- [索引策略](../08-PostgreSQL建模实践/索引策略.md) - 索引优化策略
- [分区策略](../08-PostgreSQL建模实践/分区策略.md) - 分区优化策略

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
