...
CREATE UNIQUE INDEX ON mv_account_balance_summary(customer_id, account_type);
...
```sql
-- 活跃风控规则缓存
CREATE MATERIALIZED VIEW mv_active_risk_rules AS
SELECT
    rule_id,
    rule_name,
    rule_type,
...
CREATE INDEX ON mv_active_risk_rules(rule_type, risk_level);
...
SELECT
    ...
    CASE
        ...
        WHEN last_transaction_date < NOW() - INTERVAL '1 year' THEN 'inactive'
        ELSE 'normal'
    END AS alert_type
FROM accounts
WHERE status = 'active'
  AND (
    balance < 0 OR
    balance > 1000000 OR
    last_transaction_date < NOW() - INTERVAL '1 year'
  );
