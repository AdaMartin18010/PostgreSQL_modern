...
BEGIN
    BEGIN
        ...
        IF p_transaction_type_id IS NULL THEN
            RAISE EXCEPTION 'transaction_type_id不能为NULL';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') OR
    ...
    BEGIN
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION 'Transaction type % not found', p_transaction_type_id;
        END IF;
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION '更新账户余额失败，账户可能已被删除';
        END IF;

        RETURN v_transaction_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '执行交易失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        ...
        IF p_transaction_id IS NULL THEN
            RAISE EXCEPTION 'transaction_id不能为NULL';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'risk_rules') THEN
            RAISE EXCEPTION '必需的表不存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION '交易 % 不存在', p_transaction_id;
        END IF;

        IF v_account_id IS NULL OR v_amount IS NULL THEN
            RAISE EXCEPTION '交易数据不完整';
        END IF;
        ...
        LOOP
            BEGIN
                IF v_amount > COALESCE((v_rule.rule_config->>'max_amount')::NUMERIC, 0) THEN
                    RETURN QUERY SELECT
                        v_rule.rule_id,
                        v_rule.risk_level,
                        80,
                        format('交易金额 %s 超过限额 %s', v_amount, COALESCE(v_rule.rule_config->>'max_amount', '0'));
                END IF;
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '大额交易检测失败: %', SQLERRM;
            END;
        END LOOP;
        ...
        LOOP
            BEGIN
                IF (
                    ...
                    WHERE account_id = v_account_id
                      AND transaction_date > NOW() - COALESCE((v_rule.rule_config->>'time_window')::INTERVAL, INTERVAL '1 hour')
                ) > COALESCE((v_rule.rule_config->>'max_count')::INT, 0) THEN
                    RETURN QUERY SELECT
                        v_rule.rule_id,
                        v_rule.risk_level,
                        60,
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '频繁交易检测失败: %', SQLERRM;
            END;
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '风险检测失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') THEN
            RAISE WARNING '表 transactions 或 transaction_types 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询账户交易历史';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') THEN
            RAISE WARNING '必需的表不存在，无法执行账户交易历史查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询账户交易历史（JOIN查询）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'risk_events') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'risk_rules') THEN
            RAISE WARNING '表 risk_events 或 risk_rules 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询风险事件';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        ...
        IF p_transactions IS NULL THEN
            RAISE EXCEPTION 'transactions参数不能为NULL';
        END IF;

        IF jsonb_array_length(p_transactions) = 0 THEN
            RAISE EXCEPTION 'transactions数组不能为空';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE EXCEPTION '必需的表不存在（transactions或accounts）';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '参数验证失败: %', SQLERRM;
    ...
    BEGIN
        ...
        LOOP
            BEGIN
                -- 验证交易数据
                IF v_tx->>'account_id' IS NULL OR v_tx->>'amount' IS NULL THEN
                    RETURN QUERY SELECT NULL::BIGINT, 'failed'::VARCHAR, 'account_id或amount为空'::TEXT;
                    CONTINUE;
                END IF;

                -- 调用交易处理函数（如果存在）
                IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'process_transaction') THEN
                    ...
                    RETURN QUERY SELECT v_result.transaction_id, 'success'::VARCHAR, NULL::TEXT;
                ELSE
                    RETURN QUERY SELECT NULL::BIGINT, 'failed'::VARCHAR, 'process_transaction函数不存在'::TEXT;
                END IF;
            EXCEPTION WHEN OTHERS THEN
                RETURN QUERY SELECT NULL::BIGINT, 'failed'::VARCHAR, SQLERRM::TEXT;
            END;
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '批量处理交易失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;
...
-- 测试物化视图查询性能（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'mv_account_balance_summary') THEN
            RAISE WARNING '物化视图 mv_account_balance_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始测试物化视图查询性能';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 查询账户余额汇总物化视图（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE schemaname = 'public' AND matviewname = 'mv_account_balance_summary') THEN
            RAISE WARNING '物化视图 mv_account_balance_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询账户余额汇总物化视图';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    ...
    -- 使用缓存的规则（带性能测试说明）
    -- 注意：FOR循环中的查询性能测试通常在函数调用时进行
    -- EXPLAIN (ANALYZE, BUFFERS, TIMING)
    -- SELECT * FROM mv_active_risk_rules WHERE rule_type = 'transaction';
...
```sql
-- 监控：交易处理性能（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') THEN
            RAISE WARNING '必需的表不存在，无法执行交易处理性能监控';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控交易处理性能';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 监控：交易性能统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') THEN
            RAISE WARNING '必需的表不存在，无法执行交易性能统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控交易性能统计（最近1小时）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 监控：账户余额异常（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行账户余额异常监控';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控账户余额异常';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 审计：交易完整性检查（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_types') THEN
            RAISE WARNING '必需的表不存在，无法执行交易完整性检查';
            RETURN;
        END IF;
        RAISE NOTICE '开始审计交易完整性检查';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 数据一致性检查查询（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '必需的表不存在，无法执行数据一致性检查';
            RETURN;
        END IF;
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT t.transaction_id, a.account_id, a.balance
FROM transactions t
JOIN accounts a ON t.account_id = a.account_id
WHERE ABS(a.balance - (
...
```sql
-- 使用事务保证原子性（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
            RAISE WARNING '必需的表不存在，无法执行事务示例';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行事务示例（原子性保证）';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '插入准备失败: %', SQLERRM;
        END;
    END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行锁查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行锁查询（一致性保证）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行行级锁示例';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行行级锁示例';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行悲观锁查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行悲观锁查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法创建乐观锁表';
            RETURN;
        END IF;
        RAISE NOTICE '乐观锁示例：表 accounts 已存在，跳过创建';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法执行乐观锁更新';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_history') THEN
            RAISE EXCEPTION '必需的表不存在（transactions、accounts或transaction_history）';
        END IF;
        ...
        IF v_tx.status != 'completed' THEN
            RAISE EXCEPTION '交易 % 未完成，无法回滚', p_transaction_id;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '获取交易信息失败: %', SQLERRM;
    END;
    ...
    EXCEPTION
        WHEN foreign_key_violation THEN
            RAISE EXCEPTION '外键约束违反，请检查account_id是否存在';
        WHEN OTHERS THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'risk_check_queue') THEN
        ...
        RAISE NOTICE '表 risk_check_queue 创建成功';
    ELSE
        RAISE NOTICE '表 risk_check_queue 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 risk_check_queue 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_risk_queue_priority ON risk_check_queue(priority DESC, created_at);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '表 audit_log 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 audit_log 失败: %', SQLERRM;
END $$;
