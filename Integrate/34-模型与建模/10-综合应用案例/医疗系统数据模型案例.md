...
-- 从业者与设施关联表
CREATE TABLE practitioner_facility (
...
```sql
-- 诊疗期表
CREATE TABLE health_care_episode (
...
```sql
-- 医疗交付表（医疗服务或物品的提供）
CREATE TABLE health_care_delivery (
...
-- 交付从业者角色表
CREATE TABLE delivery_practitioner_role (
...
```sql
-- 医疗理赔表
CREATE TABLE health_care_claim (
...
```sql
-- 理赔结算表
CREATE TABLE health_care_claim_settlement (
...
-- 理赔支付表
CREATE TABLE health_care_claim_payment (
...
```sql
-- 患者索引
CREATE INDEX idx_patient_role_party ON patient_role(party_id);
CREATE INDEX idx_patient_role_number ON patient_role(patient_number);
CREATE INDEX idx_patient_information_party ON patient_information(party_id);
...
```sql
-- 医疗数据审计日志表
CREATE TABLE health_care_audit_log (
