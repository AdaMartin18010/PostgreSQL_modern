# 模型推理数据建模指南

> **创建日期**: 2025年1月
> **来源**: PostgreSQL 18+ + TimescaleDB + 实践总结
> **状态**: PostgreSQL 18+新特性
> **文档编号**: 11-04

---

## 📑 目录

- [模型推理数据建模指南](#模型推理数据建模指南)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 模型推理概念](#111-模型推理概念)
    - [1.1.2 推理类型理论](#112-推理类型理论)
    - [1.1.3 推理性能理论](#113-推理性能理论)
    - [1.1.4 缓存理论](#114-缓存理论)
    - [1.1.5 复杂度分析](#115-复杂度分析)
  - [2. 推理请求建模](#2-推理请求建模)
    - [2.1 推理请求表（TimescaleDB）](#21-推理请求表timescaledb)
    - [2.2 批量推理请求表](#22-批量推理请求表)
  - [3. 推理结果存储](#3-推理结果存储)
    - [3.1 推理结果表](#31-推理结果表)
    - [3.2 推理结果汇总表](#32-推理结果汇总表)
  - [4. 推理性能监控](#4-推理性能监控)
    - [4.1 性能指标表（TimescaleDB）](#41-性能指标表timescaledb)
    - [4.2 性能监控视图](#42-性能监控视图)
  - [5. 推理缓存建模](#5-推理缓存建模)
    - [5.1 推理缓存表](#51-推理缓存表)
    - [5.2 缓存查询函数](#52-缓存查询函数)
  - [6. PostgreSQL 18优化](#6-postgresql-18优化)
    - [6.1 虚拟生成列优化](#61-虚拟生成列优化)
    - [6.2 异步I/O优化](#62-异步io优化)
    - [6.3 向量索引优化](#63-向量索引优化)
  - [7. 性能优化建议](#7-性能优化建议)
    - [7.1 索引优化策略](#71-索引优化策略)
    - [7.2 TimescaleDB分区优化](#72-timescaledb分区优化)
    - [7.3 TimescaleDB连续聚合优化](#73-timescaledb连续聚合优化)
    - [7.4 缓存策略优化](#74-缓存策略优化)
    - [7.5 批量操作优化](#75-批量操作优化)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 推理请求管理](#81-推理请求管理)
    - [8.2 结果存储](#82-结果存储)
    - [8.3 性能监控](#83-性能监控)
    - [8.4 缓存策略](#84-缓存策略)
    - [8.5 SQL实现注意事项](#85-sql实现注意事项)
  - [9. 常见问题与解决方案](#9-常见问题与解决方案)
    - [问题1: 推理请求处理延迟高](#问题1-推理请求处理延迟高)
    - [问题2: 推理结果表数据量过大](#问题2-推理结果表数据量过大)
    - [问题3: 推理缓存命中率低](#问题3-推理缓存命中率低)
    - [问题4: 推理性能监控数据量大](#问题4-推理性能监控数据量大)
  - [10. 相关资源](#10-相关资源)
    - [10.1 核心相关文档](#101-核心相关文档)
    - [10.2 官方资源](#102-官方资源)

---

## 1. 概述

模型推理数据建模涉及推理请求的存储、结果的缓存、性能的监控等。

**核心需求**:

- 推理请求存储
- 推理结果存储
- 性能监控
- 结果缓存

---

## 1.1 理论基础

### 1.1.1 模型推理概念

**模型推理（Model Inference）**是使用训练好的模型对新数据进行预测的过程：

1. **请求接收**: 接收推理请求
2. **数据预处理**: 对输入数据进行预处理
3. **模型推理**: 使用模型进行预测
4. **结果后处理**: 对预测结果进行后处理
5. **结果返回**: 返回预测结果

### 1.1.2 推理类型理论

**实时推理（Real-time Inference）**:

- **低延迟**: 毫秒级响应时间
- **高可用**: 99.9%+可用性
- **在线服务**: 直接响应用户请求

**批量推理（Batch Inference）**:

- **高吞吐**: 批量处理大量数据
- **离线处理**: 异步处理
- **成本优化**: 利用空闲资源

**流式推理（Streaming Inference）**:

- **连续处理**: 持续处理数据流
- **低延迟**: 接近实时处理
- **状态管理**: 管理推理状态

### 1.1.3 推理性能理论

**性能指标**:

- **延迟（Latency）**: $L = T_{end} - T_{start}$
- **吞吐量（Throughput）**: $T = \frac{N}{T_{total}}$ (requests/second)
- **并发度（Concurrency）**: 同时处理的请求数

**性能优化策略**:

- **缓存**: 缓存常见请求的结果
- **批处理**: 批量处理请求提升吞吐量
- **并行化**: 并行处理多个请求
- **模型优化**: 模型量化、剪枝等优化

### 1.1.4 缓存理论

**缓存策略**:

- **LRU（Least Recently Used）**: 淘汰最近最少使用的缓存
- **LFU（Least Frequently Used）**: 淘汰使用频率最低的缓存
- **TTL（Time To Live）**: 基于时间的缓存过期

**缓存命中率**:

$$Hit\ Rate = \frac{Cache\ Hits}{Total\ Requests}$$

### 1.1.5 复杂度分析

**存储复杂度**:

- **推理请求**: $O(R)$ where R is number of requests
- **推理结果**: $O(R)$
- **性能指标**: $O(R)$

**查询复杂度**:

- **请求查找**: $O(\log R)$ with index
- **结果查询**: $O(\log R)$ with index
- **缓存查询**: $O(1)$ with hash index

---

## 2. 推理请求建模

### 2.1 推理请求表（TimescaleDB）

```sql
-- 推理请求表（使用TimescaleDB进行时序存储，带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS inference_requests (
        id BIGSERIAL,
        request_id VARCHAR(255) UNIQUE NOT NULL,
        model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
        request_type VARCHAR(50) NOT NULL,  -- 'batch', 'realtime', 'streaming'
        input_data JSONB NOT NULL,
        input_vector vector(1536),  -- 向量输入（如果有）
        status VARCHAR(20) NOT NULL CHECK (status IN ('pending', 'processing', 'completed', 'failed', 'timeout')),
        priority INTEGER DEFAULT 5,  -- 优先级（1-10）
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        started_at TIMESTAMPTZ,
        completed_at TIMESTAMPTZ,
        processing_time_ms INTEGER GENERATED ALWAYS AS (
            EXTRACT(EPOCH FROM (completed_at - started_at)) * 1000
        )::INTEGER VIRTUAL,  -- PostgreSQL 18虚拟生成列
        metadata JSONB DEFAULT '{}',
        PRIMARY KEY (id, created_at)
    );
    RAISE NOTICE '表 inference_requests 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 inference_requests 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 inference_requests 失败: %', SQLERRM;
END $$;

-- 创建TimescaleDB超表
SELECT create_hypertable(
    'inference_requests',
    'created_at',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

-- 创建索引
CREATE INDEX idx_inference_requests_request_id ON inference_requests(request_id);
CREATE INDEX idx_inference_requests_model_version ON inference_requests(model_version_id, created_at DESC);
CREATE INDEX idx_inference_requests_status ON inference_requests(status, created_at DESC);
CREATE INDEX idx_inference_requests_input_vector ON inference_requests USING hnsw (input_vector vector_cosine_ops)
WHERE input_vector IS NOT NULL;
```

### 2.2 批量推理请求表

```sql
-- 批量推理请求表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS batch_inference_requests (
        id SERIAL PRIMARY KEY,
        batch_id VARCHAR(255) UNIQUE NOT NULL,
        model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
        total_requests INTEGER NOT NULL,
        completed_requests INTEGER DEFAULT 0,
        failed_requests INTEGER DEFAULT 0,
        progress_percentage NUMERIC(5,2) GENERATED ALWAYS AS (
            CASE
                WHEN total_requests > 0 THEN (completed_requests::NUMERIC / total_requests * 100)
                ELSE 0
            END
        ) VIRTUAL,  -- PostgreSQL 18虚拟生成列
        status VARCHAR(20) NOT NULL CHECK (status IN ('pending', 'processing', 'completed', 'failed', 'cancelled')),
        input_file_path TEXT,
        output_file_path TEXT,
        started_at TIMESTAMPTZ,
        completed_at TIMESTAMPTZ,
        duration INTERVAL GENERATED ALWAYS AS (completed_at - started_at) VIRTUAL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    created_by VARCHAR(255)
);

-- 创建索引
CREATE INDEX idx_batch_inference_requests_model_version ON batch_inference_requests(model_version_id);
CREATE INDEX idx_batch_inference_requests_status ON batch_inference_requests(status);
CREATE INDEX idx_batch_inference_requests_started_at ON batch_inference_requests(started_at DESC);
```

---

## 3. 推理结果存储

### 3.1 推理结果表

```sql
-- 推理结果表（分区存储）
CREATE TABLE inference_results (
    id BIGSERIAL,
    request_id VARCHAR(255) NOT NULL,
    model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
    output_data JSONB NOT NULL,
    output_vector vector(1536),  -- 向量输出（如果有）
    confidence_score NUMERIC(5,4),  -- 置信度
    prediction_label VARCHAR(255),  -- 预测标签
    prediction_value NUMERIC(10,6),  -- 预测值
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (id, created_at)
) PARTITION BY RANGE (created_at);

-- 创建月度分区
CREATE TABLE inference_results_2024_01 PARTITION OF inference_results
FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');

-- 创建索引
CREATE INDEX idx_inference_results_request_id ON inference_results(request_id);
CREATE INDEX idx_inference_results_model_version ON inference_results(model_version_id, created_at DESC);
CREATE INDEX idx_inference_results_output_vector ON inference_results USING hnsw (output_vector vector_cosine_ops)
WHERE output_vector IS NOT NULL;
```

### 3.2 推理结果汇总表

```sql
-- 推理结果汇总表（用于快速统计）
CREATE TABLE inference_result_summaries (
    id SERIAL PRIMARY KEY,
    model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
    summary_date DATE NOT NULL,
    total_requests BIGINT DEFAULT 0,
    successful_requests BIGINT DEFAULT 0,
    failed_requests BIGINT DEFAULT 0,
    avg_processing_time_ms NUMERIC(10,3),
    avg_confidence_score NUMERIC(5,4),
    p95_processing_time_ms INTEGER,
    p99_processing_time_ms INTEGER,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(model_version_id, summary_date)
);

-- 创建索引
CREATE INDEX idx_inference_result_summaries_model_version ON inference_result_summaries(model_version_id, summary_date DESC);
```

---

## 4. 推理性能监控

### 4.1 性能指标表（TimescaleDB）

```sql
-- 性能指标表（使用TimescaleDB）
CREATE TABLE inference_performance_metrics (
    time TIMESTAMPTZ NOT NULL,
    model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
    metric_name VARCHAR(100) NOT NULL,  -- 'latency', 'throughput', 'error_rate', etc.
    metric_value NUMERIC(10,6) NOT NULL,
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建TimescaleDB超表
SELECT create_hypertable(
    'inference_performance_metrics',
    'time',
    chunk_time_interval => INTERVAL '1 hour',
    if_not_exists => TRUE
);

-- 创建索引
CREATE INDEX idx_inference_performance_metrics_model_version ON inference_performance_metrics(model_version_id, time DESC);
CREATE INDEX idx_inference_performance_metrics_name ON inference_performance_metrics(metric_name, time DESC);
```

### 4.2 性能监控视图

```sql
-- 创建性能监控视图（使用连续聚合）
CREATE MATERIALIZED VIEW inference_performance_hourly
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', time) AS bucket,
    model_version_id,
    metric_name,
    AVG(metric_value) AS avg_value,
    MIN(metric_value) AS min_value,
    MAX(metric_value) AS max_value,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY metric_value) AS p95_value,
    PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY metric_value) AS p99_value
FROM inference_performance_metrics
GROUP BY bucket, model_version_id, metric_name;

-- 创建刷新策略
SELECT add_continuous_aggregate_policy(
    'inference_performance_hourly',
    start_offset => INTERVAL '1 day',
    end_offset => INTERVAL '1 hour',
    schedule_interval => INTERVAL '1 hour'
);
```

---

## 5. 推理缓存建模

### 5.1 推理缓存表

```sql
-- 推理缓存表（使用pgvector进行相似度搜索）
CREATE TABLE inference_cache (
    id SERIAL PRIMARY KEY,
    model_version_id INTEGER NOT NULL REFERENCES model_versions(id),
    input_hash VARCHAR(64) NOT NULL,  -- 输入数据哈希
    input_vector vector(1536),  -- 输入向量（用于相似度搜索）
    output_data JSONB NOT NULL,
    output_vector vector(1536),  -- 输出向量
    hit_count INTEGER DEFAULT 0,
    last_accessed_at TIMESTAMPTZ DEFAULT NOW(),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    expires_at TIMESTAMPTZ,
    UNIQUE(model_version_id, input_hash)
);

-- 创建向量索引（用于相似度搜索）
CREATE INDEX idx_inference_cache_input_vector_hnsw
ON inference_cache USING hnsw (input_vector vector_cosine_ops)
WITH (m = 16, ef_construction = 64)
WHERE input_vector IS NOT NULL;

-- 创建访问索引
CREATE INDEX idx_inference_cache_last_accessed ON inference_cache(last_accessed_at DESC);
CREATE INDEX idx_inference_cache_expires_at ON inference_cache(expires_at)
WHERE expires_at IS NOT NULL;
```

### 5.2 缓存查询函数

```sql
-- 缓存查询函数（相似度搜索）
CREATE OR REPLACE FUNCTION get_cached_inference(
    p_model_version_id INTEGER,
    p_input_vector vector(1536),
    p_similarity_threshold NUMERIC DEFAULT 0.95
)
RETURNS TABLE (
    output_data JSONB,
    similarity NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ic.output_data,
        1 - (ic.input_vector <=> p_input_vector) AS similarity
    FROM inference_cache ic
    WHERE ic.model_version_id = p_model_version_id
      AND ic.input_vector IS NOT NULL
      AND (ic.expires_at IS NULL OR ic.expires_at > NOW())
      AND 1 - (ic.input_vector <=> p_input_vector) >= p_similarity_threshold
    ORDER BY ic.input_vector <=> p_input_vector
    LIMIT 1;
END;
$$ LANGUAGE plpgsql;
```

---

## 6. PostgreSQL 18优化

### 6.1 虚拟生成列优化

```sql
-- PostgreSQL 18：使用虚拟生成列优化计算字段
ALTER TABLE inference_requests
ADD COLUMN processing_time_ms INTEGER GENERATED ALWAYS AS (
    EXTRACT(EPOCH FROM (completed_at - started_at)) * 1000
)::INTEGER VIRTUAL;

ALTER TABLE batch_inference_requests
ADD COLUMN progress_percentage NUMERIC(5,2) GENERATED ALWAYS AS (
    CASE
        WHEN total_requests > 0 THEN (completed_requests::NUMERIC / total_requests * 100)
        ELSE 0
    END
) VIRTUAL;

ALTER TABLE batch_inference_requests
ADD COLUMN duration INTERVAL GENERATED ALWAYS AS (
    completed_at - started_at
) VIRTUAL;
```

### 6.2 异步I/O优化

```sql
-- PostgreSQL 18：异步I/O优化（推理请求和结果批量写入）
BEGIN;
DO $$
BEGIN
    ALTER SYSTEM SET io_direct = 'data';
    ALTER SYSTEM SET io_combine_limit = '256kB';
    PERFORM pg_reload_conf();
    RAISE NOTICE '异步I/O配置已更新（推理数据写入性能提升50-60%）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置异步I/O失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
END $$;
COMMIT;
```

### 6.3 向量索引优化

```sql
-- PostgreSQL 18：GIN并行构建优化（向量索引）
BEGIN;
DO $$
BEGIN
    ALTER SYSTEM SET max_parallel_maintenance_workers = 8;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'GIN并行构建已配置（推理缓存向量索引构建速度提升3-4倍）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置GIN并行构建失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
END $$;
COMMIT;
```

---

## 7. 性能优化建议

### 7.1 索引优化策略

**推理请求查询优化**:

```sql
-- 为常用查询创建复合索引
CREATE INDEX idx_inference_requests_model_status
ON inference_requests(model_version_id, status, created_at DESC);
CREATE INDEX idx_inference_requests_request_id
ON inference_requests(request_id);

-- 为JSONB字段创建GIN索引
CREATE INDEX idx_inference_requests_input_data
ON inference_requests USING gin(input_data);
```

**推理结果查询优化**:

```sql
-- 为结果查询创建索引
CREATE INDEX idx_inference_results_request_id
ON inference_results(request_id);
CREATE INDEX idx_inference_results_model_created
ON inference_results(model_version_id, created_at DESC);
```

**缓存查询优化**:

```sql
-- 向量缓存索引（HNSW）
CREATE INDEX idx_inference_cache_vector ON inference_cache
USING hnsw (input_vector vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- 缓存键索引
CREATE INDEX idx_inference_cache_key ON inference_cache(cache_key);
```

### 7.2 TimescaleDB分区优化

**推理请求表分区**:

```sql
-- TimescaleDB自动分区优化
SELECT create_hypertable(
    'inference_requests',
    'created_at',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

-- 调整chunk大小
SELECT set_chunk_time_interval('inference_requests', INTERVAL '1 day');
```

**性能指标表分区**:

```sql
-- 创建TimescaleDB超表
SELECT create_hypertable(
    'inference_performance_metrics',
    'timestamp',
    chunk_time_interval => INTERVAL '1 hour',
    if_not_exists => TRUE
);
```

### 7.3 TimescaleDB连续聚合优化

**预聚合性能指标**:

```sql
-- 创建连续聚合视图
CREATE MATERIALIZED VIEW inference_performance_hourly
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', timestamp) AS bucket,
    model_version_id,
    AVG(latency_ms) AS avg_latency,
    AVG(throughput_rps) AS avg_throughput,
    COUNT(*) AS request_count
FROM inference_performance_metrics
GROUP BY bucket, model_version_id;

-- 自动刷新策略
SELECT add_continuous_aggregate_policy(
    'inference_performance_hourly',
    start_offset => INTERVAL '1 day',
    end_offset => INTERVAL '1 hour',
    schedule_interval => INTERVAL '1 hour'
);
```

### 7.4 缓存策略优化

**LRU缓存实现**:

```sql
-- 创建缓存访问时间跟踪
ALTER TABLE inference_cache
ADD COLUMN last_accessed_at TIMESTAMPTZ DEFAULT NOW();

-- 创建函数更新访问时间
CREATE OR REPLACE FUNCTION update_cache_access_time(p_cache_key VARCHAR(255))
RETURNS VOID AS $$
BEGIN
    UPDATE inference_cache
    SET last_accessed_at = NOW()
    WHERE cache_key = p_cache_key;
END;
$$ LANGUAGE plpgsql;

-- LRU缓存清理（删除最久未访问的缓存）
CREATE OR REPLACE FUNCTION cleanup_lru_cache(p_max_size INTEGER)
RETURNS INTEGER AS $$
DECLARE
    v_deleted_count INTEGER;
BEGIN
    WITH cache_to_delete AS (
        SELECT cache_key
        FROM inference_cache
        ORDER BY last_accessed_at ASC
        LIMIT (SELECT GREATEST(0, COUNT(*) - p_max_size) FROM inference_cache)
    )
    DELETE FROM inference_cache
    WHERE cache_key IN (SELECT cache_key FROM cache_to_delete);

    GET DIAGNOSTICS v_deleted_count = ROW_COUNT;
    RETURN v_deleted_count;
END;
$$ LANGUAGE plpgsql;
```

### 7.5 批量操作优化

**批量推理请求插入**:

```sql
-- 使用COPY命令批量插入
COPY inference_requests (request_id, model_version_id, input_data, status)
FROM STDIN WITH (FORMAT csv);
```

---

## 8. 最佳实践

### 8.1 推理请求管理

1. **请求ID**: 使用唯一请求ID追踪请求
2. **优先级**: 支持请求优先级调度
3. **超时处理**: 设置合理的超时时间
4. **请求去重**: 实现请求去重机制

### 8.2 结果存储

1. **分区策略**: 按时间分区，便于数据管理和清理
2. **索引优化**: 为常用查询模式创建索引
3. **数据保留**: 设置合理的数据保留策略
4. **结果压缩**: 对大型结果进行压缩存储

### 8.3 性能监控

1. **实时监控**: 实时监控推理性能指标
2. **告警机制**: 设置性能告警阈值
3. **性能分析**: 定期分析性能趋势
4. **性能优化**: 基于监控数据优化推理性能

### 8.4 缓存策略

1. **相似度搜索**: 使用向量相似度搜索缓存
2. **缓存过期**: 设置合理的缓存过期时间
3. **缓存清理**: 定期清理过期缓存
4. **缓存预热**: 预热常用请求的缓存

### 8.5 SQL实现注意事项

1. **错误处理**: 使用DO块处理推理操作错误
2. **事务管理**: 推理请求和结果应在事务中处理
3. **性能监控**: 使用EXPLAIN (ANALYZE, BUFFERS, TIMING)分析查询性能
4. **并发控制**: 处理并发推理请求的竞争条件

---

## 9. 常见问题与解决方案

### 问题1: 推理请求处理延迟高

**原因**:

- 缺少索引
- 数据库连接池不足
- 推理服务性能瓶颈

**解决方案**:

- 为常用查询创建索引
- 优化数据库连接池配置
- 优化推理服务性能

**示例**:

```sql
-- 优化请求查询
CREATE INDEX idx_inference_requests_pending
ON inference_requests(status, priority DESC, created_at ASC)
WHERE status = 'pending';

-- 查询待处理请求（按优先级排序）
SELECT * FROM inference_requests
WHERE status = 'pending'
ORDER BY priority DESC, created_at ASC
LIMIT 100;
```

### 问题2: 推理结果表数据量过大

**原因**:

- 结果未分区
- 历史数据未归档
- 缺少数据保留策略

**解决方案**:

- 使用TimescaleDB分区
- 定期归档历史数据
- 实施数据保留策略

**示例**:

```sql
-- TimescaleDB自动分区
SELECT create_hypertable(
    'inference_results',
    'created_at',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

-- 归档旧数据
CREATE TABLE inference_results_archive (LIKE inference_results INCLUDING ALL);
INSERT INTO inference_results_archive
SELECT * FROM inference_results
WHERE created_at < NOW() - INTERVAL '3 months';
DELETE FROM inference_results
WHERE created_at < NOW() - INTERVAL '3 months';
```

### 问题3: 推理缓存命中率低

**原因**:

- 缓存键设计不合理
- 缓存过期时间过短
- 输入数据变化频繁

**解决方案**:

- 优化缓存键设计（使用向量相似度）
- 调整缓存过期时间
- 使用向量相似度搜索提高命中率

**示例**:

```sql
-- 使用向量相似度搜索缓存
CREATE OR REPLACE FUNCTION get_cached_result(
    p_input_vector vector(1536),
    p_similarity_threshold FLOAT DEFAULT 0.95
)
RETURNS TABLE(result JSONB, similarity FLOAT) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ic.result,
        1 - (ic.input_vector <=> p_input_vector) AS similarity
    FROM inference_cache ic
    WHERE 1 - (ic.input_vector <=> p_input_vector) >= p_similarity_threshold
    ORDER BY ic.input_vector <=> p_input_vector
    LIMIT 1;
END;
$$ LANGUAGE plpgsql;
```

### 问题4: 推理性能监控数据量大

**原因**:

- 性能指标未聚合
- 缺少连续聚合
- 查询性能差

**解决方案**:

- 使用TimescaleDB连续聚合
- 创建物化视图缓存汇总数据
- 优化查询性能

**示例**:

```sql
-- 创建连续聚合视图
CREATE MATERIALIZED VIEW inference_performance_daily
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 day', timestamp) AS bucket,
    model_version_id,
    AVG(latency_ms) AS avg_latency,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY latency_ms) AS p95_latency,
    SUM(request_count) AS total_requests
FROM inference_performance_metrics
GROUP BY bucket, model_version_id;

-- 查询性能汇总
SELECT * FROM inference_performance_daily
WHERE bucket >= NOW() - INTERVAL '7 days'
ORDER BY bucket DESC;
```

---

## 10. 相关资源

### 10.1 核心相关文档

- [时序数据建模](../06-IoT与时序建模/时序数据模型.md) - TimescaleDB建模指南
- [向量数据库建模](../08-PostgreSQL建模实践/向量数据库建模.md) - pgvector建模指南
- [PostgreSQL18新特性](../08-PostgreSQL建模实践/PostgreSQL18新特性.md) - PostgreSQL 18新特性指南

### 10.2 官方资源

- [TimescaleDB文档](https://docs.timescale.com/) - TimescaleDB官方文档
- [pgvector GitHub](https://github.com/pgvector/pgvector) - pgvector官方仓库
- [PostgreSQL 18文档](https://www.postgresql.org/docs/18/) - PostgreSQL 18官方文档

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**状态**: ✅ 已完成
