# 模型训练数据管理指南

> **创建日期**: 2025年1月
> **来源**: PostgreSQL 18+ + TimescaleDB + 实践总结
> **状态**: PostgreSQL 18+新特性
> **文档编号**: 11-03

---

## 📑 目录

- [模型训练数据管理指南](#模型训练数据管理指南)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 训练数据管理概念](#111-训练数据管理概念)
    - [1.1.2 数据集分割理论](#112-数据集分割理论)
    - [1.1.3 数据标注理论](#113-数据标注理论)
    - [1.1.4 数据版本管理理论](#114-数据版本管理理论)
    - [1.1.5 复杂度分析](#115-复杂度分析)
  - [2. 训练数据集管理](#2-训练数据集管理)
    - [2.1 数据集表](#21-数据集表)
    - [2.2 时序训练数据表（TimescaleDB）](#22-时序训练数据表timescaledb)
    - [2.3 向量训练数据表（pgvector）](#23-向量训练数据表pgvector)
  - [3. 数据标注管理](#3-数据标注管理)
    - [3.1 标注任务表](#31-标注任务表)
    - [3.2 标注记录表](#32-标注记录表)
  - [4. 数据版本管理](#4-数据版本管理)
    - [4.1 数据集版本表](#41-数据集版本表)
    - [4.2 数据变更日志](#42-数据变更日志)
  - [5. 数据质量监控](#5-数据质量监控)
    - [5.1 数据质量指标表](#51-数据质量指标表)
    - [5.2 数据质量报告](#52-数据质量报告)
  - [6. PostgreSQL 18优化](#6-postgresql-18优化)
    - [6.1 虚拟生成列优化](#61-虚拟生成列优化)
    - [6.2 异步I/O优化](#62-异步io优化)
    - [6.3 TimescaleDB连续聚合](#63-timescaledb连续聚合)
  - [7. 性能优化建议](#7-性能优化建议)
    - [7.1 索引优化策略](#71-索引优化策略)
    - [7.2 分区策略优化](#72-分区策略优化)
    - [7.3 TimescaleDB连续聚合优化](#73-timescaledb连续聚合优化)
    - [7.4 批量操作优化](#74-批量操作优化)
    - [7.5 PostgreSQL 18异步I/O优化](#75-postgresql-18异步io优化)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 数据集管理](#81-数据集管理)
    - [8.2 数据标注](#82-数据标注)
    - [8.3 数据质量](#83-数据质量)
    - [8.4 SQL实现注意事项](#84-sql实现注意事项)
  - [9. 常见问题与解决方案](#9-常见问题与解决方案)
    - [问题1: 训练数据查询性能慢](#问题1-训练数据查询性能慢)
    - [问题2: 时序训练数据存储空间过大](#问题2-时序训练数据存储空间过大)
    - [问题3: 数据标注一致性低](#问题3-数据标注一致性低)
    - [问题4: 数据集版本管理复杂](#问题4-数据集版本管理复杂)
  - [10. 相关资源](#10-相关资源)
    - [10.1 核心相关文档](#101-核心相关文档)
    - [10.2 官方资源](#102-官方资源)

---

## 1. 概述

模型训练数据管理涉及训练数据集的存储、版本控制、标注管理和质量监控。

**核心需求**:

- 训练数据集管理
- 数据标注管理
- 数据版本控制
- 数据质量监控

---

## 1.1 理论基础

### 1.1.1 训练数据管理概念

**训练数据管理**是机器学习生命周期中的关键环节，包括：

1. **数据收集**: 从各种数据源收集原始数据
2. **数据存储**: 高效存储大规模训练数据
3. **数据标注**: 为监督学习提供标签数据
4. **数据版本控制**: 跟踪数据集的变更历史
5. **数据质量监控**: 确保数据质量和一致性

### 1.1.2 数据集分割理论

**数据集分割原则**:

- **训练集（Training Set）**: 用于模型训练，通常占60-80%
- **验证集（Validation Set）**: 用于超参数调优，通常占10-20%
- **测试集（Test Set）**: 用于最终评估，通常占10-20%

**分割方法**:

- **随机分割**: 随机分配样本到不同集合
- **分层分割**: 保持类别分布的分割
- **时间分割**: 按时间顺序分割（时序数据）

### 1.1.3 数据标注理论

**标注质量评估**:

- **一致性（Consistency）**: 标注者内部一致性
- **准确性（Accuracy）**: 标注结果准确性
- **完整性（Completeness）**: 标注覆盖率

**标注一致性度量**:

- **Cohen's Kappa**: $\kappa = \frac{p_o - p_e}{1 - p_e}$
  - $p_o$: 观察一致性
  - $p_e$: 期望一致性

### 1.1.4 数据版本管理理论

**数据版本控制原则**:

1. **不可变性**: 已发布的数据集版本不可修改
2. **可追溯性**: 记录数据集的来源和变更历史
3. **可重现性**: 能够重现数据集的处理过程
4. **版本比较**: 支持版本间的差异分析

### 1.1.5 复杂度分析

**存储复杂度**:

- **数据集元数据**: $O(D)$ where D is number of datasets
- **训练数据**: $O(S)$ where S is total samples
- **标注数据**: $O(A)$ where A is number of annotations
- **版本历史**: $O(D \times V)$ where V is number of versions

**查询复杂度**:

- **数据集查找**: $O(\log D)$ with index
- **样本查询**: $O(\log S)$ with index
- **标注查询**: $O(\log A)$ with index

---

## 2. 训练数据集管理

### 2.1 数据集表

```sql
-- 训练数据集表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS training_datasets (
        id SERIAL PRIMARY KEY,
        dataset_name VARCHAR(255) NOT NULL,
        dataset_type VARCHAR(50) NOT NULL,  -- 'text', 'image', 'tabular', 'time_series', etc.
        description TEXT,
        data_source VARCHAR(255),  -- 数据来源
        total_samples BIGINT,
        train_samples BIGINT,
        validation_samples BIGINT,
        test_samples BIGINT,
        data_path TEXT,  -- 数据集文件路径
        metadata JSONB DEFAULT '{}',
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW(),
        UNIQUE(dataset_name)
    );
    RAISE NOTICE '表 training_datasets 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 training_datasets 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 training_datasets 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_training_datasets_type ON training_datasets(dataset_type);
    CREATE INDEX IF NOT EXISTS idx_training_datasets_created_at ON training_datasets(created_at DESC);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

### 2.2 时序训练数据表（TimescaleDB）

```sql
-- 时序训练数据（使用TimescaleDB，带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS time_series_training_data (
        time TIMESTAMPTZ NOT NULL,
        series_id VARCHAR(255) NOT NULL,
        features JSONB NOT NULL,  -- 特征数据
        label NUMERIC,  -- 标签（如果有）
        metadata JSONB DEFAULT '{}',
        created_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 time_series_training_data 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 time_series_training_data 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 time_series_training_data 失败: %', SQLERRM;
END $$;

-- 创建TimescaleDB超表（带错误处理）
DO $$
BEGIN
    PERFORM create_hypertable(
        'time_series_training_data',
        'time',
        chunk_time_interval => INTERVAL '1 day',
        if_not_exists => TRUE
    );
    RAISE NOTICE 'TimescaleDB超表创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建TimescaleDB超表失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_time_series_training_data_series ON time_series_training_data(series_id, time DESC);
    CREATE INDEX IF NOT EXISTS idx_time_series_training_data_features ON time_series_training_data USING GIN (features);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

### 2.3 向量训练数据表（pgvector）

```sql
-- 向量训练数据（使用pgvector，带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS vector_training_data (
        id SERIAL PRIMARY KEY,
        dataset_id INTEGER NOT NULL REFERENCES training_datasets(id),
        input_vector vector(1536) NOT NULL,  -- 输入向量
        output_vector vector(1536),  -- 输出向量（如果有）
        label INTEGER,  -- 标签
        metadata JSONB DEFAULT '{}',
        created_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 vector_training_data 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 vector_training_data 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 vector_training_data 失败: %', SQLERRM;
END $$;

-- 创建向量索引（HNSW，带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_vector_training_data_input_hnsw
    ON vector_training_data USING hnsw (input_vector vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);
    RAISE NOTICE '向量索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建向量索引失败: %', SQLERRM;
END $$;

-- 创建数据集索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_vector_training_data_dataset ON vector_training_data(dataset_id);
    RAISE NOTICE '数据集索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建数据集索引失败: %', SQLERRM;
END $$;
```

---

## 3. 数据标注管理

### 3.1 标注任务表

```sql
-- 标注任务表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS labeling_tasks (
        id SERIAL PRIMARY KEY,
        task_name VARCHAR(255) NOT NULL,
        dataset_id INTEGER NOT NULL REFERENCES training_datasets(id),
        task_type VARCHAR(50) NOT NULL,  -- 'classification', 'regression', 'segmentation', etc.
        status VARCHAR(20) NOT NULL CHECK (status IN ('pending', 'in_progress', 'completed', 'cancelled')),
        total_samples INTEGER,
        labeled_samples INTEGER DEFAULT 0,
        progress_percentage NUMERIC(5,2) GENERATED ALWAYS AS (
            CASE
                WHEN total_samples > 0 THEN (labeled_samples::NUMERIC / total_samples * 100)
                ELSE 0
            END
        ) VIRTUAL,  -- PostgreSQL 18虚拟生成列
        labeling_guidelines TEXT,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW(),
        completed_at TIMESTAMPTZ,
        created_by VARCHAR(255)
    );
    RAISE NOTICE '表 labeling_tasks 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 labeling_tasks 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 labeling_tasks 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_labeling_tasks_dataset ON labeling_tasks(dataset_id);
    CREATE INDEX IF NOT EXISTS idx_labeling_tasks_status ON labeling_tasks(status);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;

```

### 3.2 标注记录表

```sql
-- 标注记录表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS labeling_records (
        id BIGSERIAL PRIMARY KEY,
        task_id INTEGER NOT NULL REFERENCES labeling_tasks(id),
        sample_id VARCHAR(255) NOT NULL,
        label_value JSONB NOT NULL,  -- 标签值（支持各种类型）
        confidence_score NUMERIC(5,4),  -- 标注置信度
        annotator_id VARCHAR(255),  -- 标注员ID
        annotation_time INTERVAL,  -- 标注耗时
        quality_score NUMERIC(5,4),  -- 质量评分
        notes TEXT,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW(),
        UNIQUE(task_id, sample_id)
    );
    RAISE NOTICE '表 labeling_records 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 labeling_records 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 labeling_records 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_labeling_records_task ON labeling_records(task_id);
    CREATE INDEX IF NOT EXISTS idx_labeling_records_annotator ON labeling_records(annotator_id);
    CREATE INDEX IF NOT EXISTS idx_labeling_records_created_at ON labeling_records(created_at DESC);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

---

## 4. 数据版本管理

### 4.1 数据集版本表

```sql
-- 数据集版本表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS dataset_versions (
        id SERIAL PRIMARY KEY,
        dataset_id INTEGER NOT NULL REFERENCES training_datasets(id),
        version VARCHAR(50) NOT NULL,
        version_type VARCHAR(20) NOT NULL CHECK (version_type IN ('major', 'minor', 'patch')),
        changes_description TEXT,
        data_path TEXT NOT NULL,
        checksum VARCHAR(64),  -- 数据集校验和
        total_samples BIGINT,
        train_samples BIGINT,
        validation_samples BIGINT,
        test_samples BIGINT,
        is_active BOOLEAN DEFAULT FALSE,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        created_by VARCHAR(255),
        UNIQUE(dataset_id, version)
    );
    RAISE NOTICE '表 dataset_versions 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 dataset_versions 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 dataset_versions 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_dataset_versions_dataset ON dataset_versions(dataset_id);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;

-- 创建部分索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_dataset_versions_active ON dataset_versions(dataset_id, is_active) WHERE is_active = TRUE;
    RAISE NOTICE '部分索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建部分索引失败: %', SQLERRM;
END $$;

```

### 4.2 数据变更日志

```sql
-- 数据变更日志表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dataset_change_logs') THEN
        CREATE TABLE dataset_change_logs (
            id BIGSERIAL,
            dataset_id INTEGER NOT NULL REFERENCES training_datasets(id),
            version_id INTEGER REFERENCES dataset_versions(id),
            change_type VARCHAR(50) NOT NULL,  -- 'add', 'remove', 'modify', 'split', 'merge'
            change_details JSONB DEFAULT '{}',
            samples_affected BIGINT,
            created_at TIMESTAMPTZ DEFAULT NOW(),
            created_by VARCHAR(255),
            PRIMARY KEY (id, created_at)
        ) PARTITION BY RANGE (created_at);
        RAISE NOTICE '分区表 dataset_change_logs 创建成功';
    ELSE
        RAISE NOTICE '表 dataset_change_logs 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建分区表 dataset_change_logs 失败: %', SQLERRM;
END $$;

-- 创建月度分区（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS dataset_change_logs_2024_01 PARTITION OF dataset_change_logs
    FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');
    RAISE NOTICE '分区创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '分区已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE WARNING '创建分区失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_dataset_change_logs_dataset ON dataset_change_logs(dataset_id, created_at DESC);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

---

## 5. 数据质量监控

### 5.1 数据质量指标表

```sql
-- 数据质量指标表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS data_quality_metrics (
        id SERIAL PRIMARY KEY,
        dataset_id INTEGER NOT NULL REFERENCES training_datasets(id),
        version_id INTEGER REFERENCES dataset_versions(id),
        metric_name VARCHAR(100) NOT NULL,  -- 'completeness', 'accuracy', 'consistency', etc.
        metric_value NUMERIC(10,6) NOT NULL,
        threshold_value NUMERIC(10,6),  -- 阈值
        status VARCHAR(20) GENERATED ALWAYS AS (
            CASE
                WHEN threshold_value IS NULL THEN 'unknown'
                WHEN metric_value >= threshold_value THEN 'pass'
                ELSE 'fail'
            END
        ) VIRTUAL,  -- PostgreSQL 18虚拟生成列
        evaluated_at TIMESTAMPTZ DEFAULT NOW(),
        notes TEXT
    );
    RAISE NOTICE '表 data_quality_metrics 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 data_quality_metrics 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 data_quality_metrics 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_data_quality_metrics_dataset ON data_quality_metrics(dataset_id);
    CREATE INDEX IF NOT EXISTS idx_data_quality_metrics_status ON data_quality_metrics(status);
    CREATE INDEX IF NOT EXISTS idx_data_quality_metrics_evaluated_at ON data_quality_metrics(evaluated_at DESC);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

### 5.2 数据质量报告

```sql
-- 生成数据质量报告
CREATE OR REPLACE FUNCTION generate_data_quality_report(
    p_dataset_id INTEGER,
    p_version_id INTEGER DEFAULT NULL
)
RETURNS TABLE (
    metric_name VARCHAR(100),
    metric_value NUMERIC(10,6),
    threshold_value NUMERIC(10,6),
    status VARCHAR(20),
    evaluated_at TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        dqm.metric_name,
        dqm.metric_value,
        dqm.threshold_value,
        dqm.status,
        dqm.evaluated_at
    FROM data_quality_metrics dqm
    WHERE dqm.dataset_id = p_dataset_id
      AND (p_version_id IS NULL OR dqm.version_id = p_version_id)
    ORDER BY dqm.evaluated_at DESC;
END;
$$ LANGUAGE plpgsql;
```

---

## 6. PostgreSQL 18优化

### 6.1 虚拟生成列优化

```sql
-- PostgreSQL 18：使用虚拟生成列优化计算字段
ALTER TABLE labeling_tasks
ADD COLUMN progress_percentage NUMERIC(5,2) GENERATED ALWAYS AS (
    CASE
        WHEN total_samples > 0 THEN (labeled_samples::NUMERIC / total_samples * 100)
        ELSE 0
    END
) VIRTUAL;

ALTER TABLE data_quality_metrics
ADD COLUMN status VARCHAR(20) GENERATED ALWAYS AS (
    CASE
        WHEN threshold_value IS NULL THEN 'unknown'
        WHEN metric_value >= threshold_value THEN 'pass'
        ELSE 'fail'
    END
) VIRTUAL;
```

### 6.2 异步I/O优化

```sql
-- PostgreSQL 18：异步I/O优化（时序训练数据写入）
BEGIN;
DO $$
BEGIN
    ALTER SYSTEM SET io_direct = 'data';
    ALTER SYSTEM SET io_combine_limit = '256kB';
    PERFORM pg_reload_conf();
    RAISE NOTICE '异步I/O配置已更新（时序训练数据写入性能提升50-60%）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置异步I/O失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
END $$;
COMMIT;
```

### 6.3 TimescaleDB连续聚合

```sql
-- 创建时序训练数据的连续聚合（用于快速统计）
CREATE MATERIALIZED VIEW time_series_training_data_hourly
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', time) AS bucket,
    series_id,
    COUNT(*) AS sample_count,
    AVG((features->>'value')::NUMERIC) AS avg_value,
    MIN((features->>'value')::NUMERIC) AS min_value,
    MAX((features->>'value')::NUMERIC) AS max_value
FROM time_series_training_data
GROUP BY bucket, series_id;

-- 创建刷新策略
SELECT add_continuous_aggregate_policy(
    'time_series_training_data_hourly',
    start_offset => INTERVAL '1 day',
    end_offset => INTERVAL '1 hour',
    schedule_interval => INTERVAL '1 hour'
);
```

---

## 7. 性能优化建议

### 7.1 索引优化策略

**数据集查询优化**:

```sql
-- 为常用查询创建复合索引
CREATE INDEX idx_training_datasets_type_created ON training_datasets(dataset_type, created_at DESC);
CREATE INDEX idx_training_datasets_name_type ON training_datasets(dataset_name, dataset_type);

-- 为JSONB字段创建GIN索引
CREATE INDEX idx_training_datasets_metadata ON training_datasets USING gin(metadata);
```

**时序数据索引优化**:

```sql
-- TimescaleDB自动索引优化
CREATE INDEX idx_time_series_training_data_series_time
ON time_series_training_data(series_id, time DESC);

-- JSONB特征索引
CREATE INDEX idx_time_series_training_data_features
ON time_series_training_data USING gin(features);
```

**向量数据索引优化**:

```sql
-- pgvector HNSW索引优化
CREATE INDEX idx_vector_training_data_input_hnsw
ON vector_training_data USING hnsw (input_vector vector_cosine_ops)
WITH (m = 16, ef_construction = 64);
```

### 7.2 分区策略优化

**时序数据分区（TimescaleDB）**:

```sql
-- TimescaleDB自动分区优化
SELECT create_hypertable(
    'time_series_training_data',
    'time',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

-- 调整chunk大小以优化查询性能
SELECT set_chunk_time_interval('time_series_training_data', INTERVAL '1 day');
```

**向量数据分区**:

```sql
-- 按数据集ID分区
CREATE TABLE vector_training_data (
    id BIGSERIAL,
    dataset_id INTEGER NOT NULL,
    input_vector vector(1536) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (id, dataset_id)
) PARTITION BY HASH (dataset_id);

-- 创建分区
CREATE TABLE vector_training_data_0 PARTITION OF vector_training_data
FOR VALUES WITH (MODULUS 4, REMAINDER 0);
```

### 7.3 TimescaleDB连续聚合优化

**预聚合常用统计**:

```sql
-- 创建连续聚合视图
CREATE MATERIALIZED VIEW time_series_training_data_hourly
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', time) AS bucket,
    series_id,
    COUNT(*) AS sample_count,
    AVG((features->>'value')::NUMERIC) AS avg_value
FROM time_series_training_data
GROUP BY bucket, series_id;

-- 自动刷新策略
SELECT add_continuous_aggregate_policy(
    'time_series_training_data_hourly',
    start_offset => INTERVAL '1 day',
    end_offset => INTERVAL '1 hour',
    schedule_interval => INTERVAL '1 hour'
);
```

### 7.4 批量操作优化

**批量数据插入**:

```sql
-- 使用COPY命令批量插入（性能提升10-100倍）
COPY vector_training_data (dataset_id, input_vector, label)
FROM STDIN WITH (FORMAT csv);

-- 或使用INSERT ... VALUES批量插入
INSERT INTO vector_training_data (dataset_id, input_vector, label)
VALUES
    (1, '[0.1,0.2,0.3]'::vector, 0),
    (1, '[0.4,0.5,0.6]'::vector, 1);
```

### 7.5 PostgreSQL 18异步I/O优化

**异步I/O配置**:

```sql
-- PostgreSQL 18：启用异步I/O优化数据写入
ALTER SYSTEM SET io_direct = 'data';
ALTER SYSTEM SET io_combine_limit = '256kB';
SELECT pg_reload_conf();
```

---

## 8. 最佳实践

### 8.1 数据集管理

1. **版本控制**: 对数据集进行版本管理，记录变更历史
2. **数据分割**: 明确记录训练/验证/测试集分割
3. **数据校验**: 使用校验和确保数据完整性
4. **数据备份**: 定期备份重要数据集

### 8.2 数据标注

1. **标注指南**: 提供清晰的标注指南
2. **质量控制**: 对标注质量进行监控和评估
3. **多人标注**: 支持多人标注和一致性检查
4. **标注审核**: 建立标注审核机制

### 8.3 数据质量

1. **定期检查**: 定期检查数据质量指标
2. **阈值设置**: 设置合理的数据质量阈值
3. **异常处理**: 及时发现和处理数据异常
4. **质量报告**: 生成数据质量报告

### 8.4 SQL实现注意事项

1. **错误处理**: 使用DO块处理数据操作错误
2. **事务管理**: 数据集更新应在事务中执行
3. **性能监控**: 使用EXPLAIN ANALYZE分析查询性能
4. **数据一致性**: 确保数据集和样本数据的一致性

---

## 9. 常见问题与解决方案

### 问题1: 训练数据查询性能慢

**原因**:

- 缺少索引
- 全表扫描
- 数据量过大

**解决方案**:

- 为常用查询字段创建索引
- 使用TimescaleDB分区优化时序数据
- 使用pgvector索引优化向量查询

**示例**:

```sql
-- 优化前：全表扫描
SELECT * FROM vector_training_data
WHERE dataset_id = 1;

-- 优化后：使用索引和分区
CREATE INDEX idx_vector_training_data_dataset
ON vector_training_data(dataset_id);

SELECT * FROM vector_training_data
WHERE dataset_id = 1
LIMIT 1000;
```

### 问题2: 时序训练数据存储空间过大

**原因**:

- 数据未压缩
- 历史数据未归档
- 缺少数据保留策略

**解决方案**:

- 使用TimescaleDB压缩策略
- 定期归档历史数据
- 实施数据保留策略

**示例**:

```sql
-- TimescaleDB压缩策略
SELECT add_compression_policy('time_series_training_data', INTERVAL '7 days');

-- 归档旧数据
CREATE TABLE time_series_training_data_archive (LIKE time_series_training_data INCLUDING ALL);
INSERT INTO time_series_training_data_archive
SELECT * FROM time_series_training_data
WHERE time < NOW() - INTERVAL '1 year';
DELETE FROM time_series_training_data
WHERE time < NOW() - INTERVAL '1 year';
```

### 问题3: 数据标注一致性低

**原因**:

- 标注指南不清晰
- 缺少标注质量控制
- 多人标注未进行一致性检查

**解决方案**:

- 提供详细的标注指南
- 实施标注质量控制机制
- 计算标注一致性指标

**示例**:

```sql
-- 计算标注一致性（Cohen's Kappa）
WITH annotation_stats AS (
    SELECT
        sample_id,
        COUNT(DISTINCT annotator_id) AS annotator_count,
        COUNT(DISTINCT label) AS label_count,
        MODE() WITHIN GROUP (ORDER BY label) AS majority_label
    FROM labeling_records
    GROUP BY sample_id
)
SELECT
    AVG(CASE WHEN annotator_count > 1 AND label_count = 1 THEN 1.0 ELSE 0.0 END) AS consistency_rate
FROM annotation_stats;
```

### 问题4: 数据集版本管理复杂

**原因**:

- 版本信息分散
- 缺少版本比较工具
- 版本回滚困难

**解决方案**:

- 集中管理版本信息
- 创建版本比较视图
- 实现版本回滚机制

**示例**:

```sql
-- 创建版本比较视图
CREATE VIEW v_dataset_version_comparison AS
SELECT
    dv1.dataset_id,
    dv1.version AS version1,
    dv2.version AS version2,
    dv1.total_samples AS samples1,
    dv2.total_samples AS samples2,
    dv2.total_samples - dv1.total_samples AS sample_diff
FROM dataset_versions dv1
JOIN dataset_versions dv2 ON dv1.dataset_id = dv2.dataset_id
WHERE dv1.version != dv2.version;

-- 版本回滚函数
CREATE OR REPLACE FUNCTION rollback_dataset_version(
    p_dataset_id INTEGER,
    p_target_version VARCHAR(50)
)
RETURNS VOID AS $$
BEGIN
    -- 停用当前版本
    UPDATE dataset_versions
    SET is_active = FALSE
    WHERE dataset_id = p_dataset_id AND is_active = TRUE;

    -- 激活目标版本
    UPDATE dataset_versions
    SET is_active = TRUE
    WHERE dataset_id = p_dataset_id AND version = p_target_version;
END;
$$ LANGUAGE plpgsql;
```

---

## 10. 相关资源

### 10.1 核心相关文档

- [时序数据建模](../06-IoT与时序建模/时序数据模型.md) - TimescaleDB建模指南
- [向量数据库建模](../08-PostgreSQL建模实践/向量数据库建模.md) - pgvector建模指南
- [PostgreSQL18新特性](../08-PostgreSQL建模实践/PostgreSQL18新特性.md) - PostgreSQL 18新特性指南

### 10.2 官方资源

- [TimescaleDB文档](https://docs.timescale.com/) - TimescaleDB官方文档
- [pgvector GitHub](https://github.com/pgvector/pgvector) - pgvector官方仓库
- [PostgreSQL 18文档](https://www.postgresql.org/docs/18/) - PostgreSQL 18官方文档

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**状态**: ✅ 已完成
