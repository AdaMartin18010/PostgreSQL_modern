# AI应用场景建模指南

> **创建日期**: 2025年1月
> **来源**: PostgreSQL 18+ + pgvector + TimescaleDB + 实践总结
> **状态**: PostgreSQL 18+新特性
> **文档编号**: 11-05

---

## 📑 目录

- [AI应用场景建模指南](#ai应用场景建模指南)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 RAG（检索增强生成）理论](#111-rag检索增强生成理论)
    - [1.1.2 推荐系统理论](#112-推荐系统理论)
    - [1.1.3 语义搜索理论](#113-语义搜索理论)
    - [1.1.4 知识图谱理论](#114-知识图谱理论)
    - [1.1.5 复杂度分析](#115-复杂度分析)
  - [2. RAG应用建模](#2-rag应用建模)
    - [2.1 文档存储和向量化](#21-文档存储和向量化)
    - [2.2 RAG检索函数](#22-rag检索函数)
  - [3. 推荐系统建模](#3-推荐系统建模)
    - [3.1 用户和物品表](#31-用户和物品表)
    - [3.2 用户行为表（TimescaleDB）](#32-用户行为表timescaledb)
    - [3.3 推荐函数](#33-推荐函数)
  - [4. 语义搜索建模](#4-语义搜索建模)
    - [4.1 搜索内容表](#41-搜索内容表)
    - [4.2 语义搜索函数](#42-语义搜索函数)
  - [5. 知识图谱应用建模](#5-知识图谱应用建模)
    - [5.1 实体和关系表](#51-实体和关系表)
    - [5.2 知识图谱查询函数](#52-知识图谱查询函数)
  - [6. PostgreSQL 18优化](#6-postgresql-18优化)
    - [6.1 异步I/O优化](#61-异步io优化)
    - [6.2 GIN并行构建优化](#62-gin并行构建优化)
    - [6.3 虚拟生成列优化](#63-虚拟生成列优化)
  - [7. 性能优化建议](#7-性能优化建议)
    - [7.1 RAG应用优化](#71-rag应用优化)
    - [7.2 推荐系统优化](#72-推荐系统优化)
    - [7.3 语义搜索优化](#73-语义搜索优化)
    - [7.4 知识图谱优化](#74-知识图谱优化)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 RAG应用](#81-rag应用)
    - [8.2 推荐系统](#82-推荐系统)
    - [8.3 语义搜索](#83-语义搜索)
    - [8.4 知识图谱](#84-知识图谱)
    - [8.5 SQL实现注意事项](#85-sql实现注意事项)
  - [9. 常见问题与解决方案](#9-常见问题与解决方案)
    - [问题1: RAG检索延迟高](#问题1-rag检索延迟高)
    - [问题2: 推荐系统冷启动问题](#问题2-推荐系统冷启动问题)
    - [问题3: 语义搜索结果不准确](#问题3-语义搜索结果不准确)
    - [问题4: 知识图谱查询性能差](#问题4-知识图谱查询性能差)
  - [10. 相关资源](#10-相关资源)
    - [10.1 核心相关文档](#101-核心相关文档)
    - [10.2 官方资源](#102-官方资源)

---

## 1. 概述

本指南涵盖PostgreSQL在常见AI应用场景中的数据建模实践，包括RAG、推荐系统、语义搜索、知识图谱等。

**核心场景**:

- RAG（检索增强生成）应用
- 推荐系统
- 语义搜索
- 知识图谱应用

---

## 1.1 理论基础

### 1.1.1 RAG（检索增强生成）理论

**RAG架构**结合了检索和生成两个阶段：

1. **检索阶段**: 从知识库中检索相关文档
2. **生成阶段**: 基于检索到的文档生成回答

**检索策略**:

- **向量检索**: 使用向量相似度搜索相关文档
- **混合检索**: 结合向量检索和全文搜索
- **重排序**: 使用业务规则对结果重排序

**相似度计算**:

- **余弦相似度**: $\text{cos}(\theta) = \frac{\mathbf{A} \cdot \mathbf{B}}{||\mathbf{A}|| \times ||\mathbf{B}||}$
- **欧氏距离**: $d = ||\mathbf{A} - \mathbf{B}||_2$
- **点积**: $\mathbf{A} \cdot \mathbf{B}$

### 1.1.2 推荐系统理论

**推荐算法**:

1. **协同过滤（Collaborative Filtering）**:
   - **用户协同过滤**: 基于相似用户的偏好推荐
   - **物品协同过滤**: 基于相似物品推荐

2. **内容推荐（Content-Based）**:
   - 基于物品特征推荐
   - 基于用户画像推荐

3. **混合推荐（Hybrid）**:
   - 结合多种推荐策略
   - 加权融合推荐结果

**相似度计算**:

- **余弦相似度**: $\text{cos}(\theta) = \frac{\sum_{i} u_i v_i}{\sqrt{\sum_{i} u_i^2} \sqrt{\sum_{i} v_i^2}}$
- **皮尔逊相关系数**: $r = \frac{\sum_{i}(u_i - \bar{u})(v_i - \bar{v})}{\sqrt{\sum_{i}(u_i - \bar{u})^2}\sqrt{\sum_{i}(v_i - \bar{v})^2}}$

### 1.1.3 语义搜索理论

**语义搜索**使用向量表示理解查询和文档的语义：

- **查询向量化**: 将查询转换为向量
- **文档向量化**: 将文档转换为向量
- **相似度搜索**: 使用向量相似度搜索相关文档

**向量化方法**:

- **词嵌入（Word Embeddings）**: Word2Vec、GloVe
- **句子嵌入（Sentence Embeddings）**: BERT、Sentence-BERT
- **文档嵌入（Document Embeddings）**: Doc2Vec

### 1.1.4 知识图谱理论

**知识图谱**是结构化的知识表示：

- **实体（Entity）**: 图中的节点
- **关系（Relation）**: 图中的边
- **属性（Property）**: 实体和关系的属性

**图查询**:

- **路径查询**: 查找实体间的路径
- **模式匹配**: 匹配图模式
- **图算法**: PageRank、社区检测等

### 1.1.5 复杂度分析

**RAG检索复杂度**:

- **向量索引构建**: $O(N \log N)$ (HNSW)
- **向量搜索**: $O(\log N)$ (近似最近邻)
- **重排序**: $O(K)$ (K为候选数量)

**推荐系统复杂度**:

- **用户相似度计算**: $O(U^2)$ (U为用户数)
- **物品相似度计算**: $O(I^2)$ (I为物品数)
- **推荐生成**: $O(U \times I)$

---

## 2. RAG应用建模

### 2.1 文档存储和向量化

```sql
-- 文档表
CREATE TABLE documents (
    id SERIAL PRIMARY KEY,
    document_id VARCHAR(255) UNIQUE NOT NULL,
    title TEXT NOT NULL,
    content TEXT NOT NULL,
    content_type VARCHAR(50),  -- 'text', 'markdown', 'pdf', etc.
    source_url TEXT,
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 文档分块表
CREATE TABLE document_chunks (
    id SERIAL PRIMARY KEY,
    document_id VARCHAR(255) NOT NULL REFERENCES documents(document_id),
    chunk_index INTEGER NOT NULL,
    chunk_text TEXT NOT NULL,
        embedding vector(1536) NOT NULL,  -- OpenAI text-embedding-3-large
        token_count INTEGER,
        metadata JSONB DEFAULT '{}',
        created_at TIMESTAMPTZ DEFAULT NOW(),
        UNIQUE(document_id, chunk_index)
    );
    RAISE NOTICE '表 document_chunks 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 document_chunks 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 document_chunks 失败: %', SQLERRM;
END $$;

-- 创建向量索引（HNSW，带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_document_chunks_embedding_hnsw
    ON document_chunks USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);
    RAISE NOTICE '向量索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建向量索引失败: %', SQLERRM;
END $$;

-- 创建全文搜索索引（PostgreSQL 18 GIN并行构建，带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_document_chunks_text_gin
    ON document_chunks USING GIN (to_tsvector('english', chunk_text));
    RAISE NOTICE '全文搜索索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建全文搜索索引失败: %', SQLERRM;
END $$;

-- 创建文档索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_document_chunks_document_id ON document_chunks(document_id);
    RAISE NOTICE '文档索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建文档索引失败: %', SQLERRM;
END $$;
```

### 2.2 RAG检索函数

```sql
-- RAG检索函数（向量相似度 + 全文搜索，带性能测试）
CREATE OR REPLACE FUNCTION rag_retrieve(
    query_embedding vector(1536),
    query_text TEXT,
    top_k INTEGER DEFAULT 10,
    similarity_threshold FLOAT DEFAULT 0.7
)
RETURNS TABLE (
    chunk_id INTEGER,
    document_id VARCHAR(255),
    chunk_text TEXT,
    similarity FLOAT,
    text_rank FLOAT,
    combined_score FLOAT
) AS $$
BEGIN
    -- 参数验证
    IF query_embedding IS NULL THEN
        RAISE EXCEPTION 'query_embedding不能为NULL';
    END IF;
    IF query_text IS NULL OR query_text = '' THEN
        RAISE EXCEPTION 'query_text不能为空';
    END IF;
    IF top_k <= 0 OR top_k > 100 THEN
        RAISE EXCEPTION 'top_k必须在1-100之间';
    END IF;

    RETURN QUERY
    WITH vector_results AS (
        SELECT
            dc.id,
            dc.document_id,
            dc.chunk_text,
            1 - (dc.embedding <=> query_embedding) AS similarity
        FROM document_chunks dc
        WHERE 1 - (dc.embedding <=> query_embedding) >= similarity_threshold
        ORDER BY dc.embedding <=> query_embedding
        LIMIT top_k * 2
    ),
    text_results AS (
        SELECT
            dc.id,
            dc.document_id,
            dc.chunk_text,
            ts_rank(to_tsvector('english', dc.chunk_text), plainto_tsquery('english', query_text)) AS text_rank
        FROM document_chunks dc
        WHERE to_tsvector('english', dc.chunk_text) @@ plainto_tsquery('english', query_text)
        ORDER BY text_rank DESC
        LIMIT top_k * 2
    )
    SELECT
        COALESCE(v.id, t.id) AS chunk_id,
        COALESCE(v.document_id, t.document_id) AS document_id,
        COALESCE(v.chunk_text, t.chunk_text) AS chunk_text,
        COALESCE(v.similarity, 0) AS similarity,
        COALESCE(t.text_rank, 0) AS text_rank,
        COALESCE(v.similarity, 0) * 0.6 + COALESCE(t.text_rank, 0) * 0.4 AS combined_score
    FROM vector_results v
    FULL OUTER JOIN text_results t ON v.id = t.id
    ORDER BY combined_score DESC
    LIMIT top_k;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'RAG检索失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- 性能测试
EXPLAIN ANALYZE
SELECT * FROM rag_retrieve(
    '[0.1,0.2,...]'::vector(1536),
    'test query',
    10,
    0.7
);
```

---

## 3. 推荐系统建模

### 3.1 用户和物品表

```sql
-- 用户表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS users (
        id SERIAL PRIMARY KEY,
        user_id VARCHAR(255) UNIQUE NOT NULL,
        user_profile JSONB DEFAULT '{}',
        user_embedding vector(1536),  -- 用户向量表示
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 users 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 users 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 users 失败: %', SQLERRM;
END $$;

-- 物品表（商品、内容等，带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS items (
        id SERIAL PRIMARY KEY,
        item_id VARCHAR(255) UNIQUE NOT NULL,
        item_type VARCHAR(50) NOT NULL,  -- 'product', 'article', 'video', etc.
        title TEXT NOT NULL,
    description TEXT,
    item_embedding vector(1536),  -- 物品向量表示
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建向量索引
CREATE INDEX idx_users_embedding_hnsw
ON users USING hnsw (user_embedding vector_cosine_ops)
WHERE user_embedding IS NOT NULL;

CREATE INDEX idx_items_embedding_hnsw
ON items USING hnsw (item_embedding vector_cosine_ops)
WHERE item_embedding IS NOT NULL;
```

### 3.2 用户行为表（TimescaleDB）

```sql
-- 用户行为表（使用TimescaleDB）
CREATE TABLE user_behaviors (
    time TIMESTAMPTZ NOT NULL,
    user_id VARCHAR(255) NOT NULL,
    item_id VARCHAR(255) NOT NULL,
    behavior_type VARCHAR(50) NOT NULL,  -- 'view', 'click', 'purchase', 'like', etc.
    behavior_value NUMERIC(10,2),  -- 行为值（评分、金额等）
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建TimescaleDB超表
SELECT create_hypertable(
    'user_behaviors',
    'time',
    chunk_time_interval => INTERVAL '1 day',
    if_not_exists => TRUE
);

-- 创建索引
CREATE INDEX idx_user_behaviors_user_item ON user_behaviors(user_id, item_id, time DESC);
CREATE INDEX idx_user_behaviors_item ON user_behaviors(item_id, time DESC);
```

### 3.3 推荐函数

```sql
-- 基于向量相似度的推荐函数
CREATE OR REPLACE FUNCTION recommend_items(
    p_user_id VARCHAR(255),
    p_top_k INTEGER DEFAULT 20,
    p_exclude_items TEXT[] DEFAULT ARRAY[]::TEXT[]
)
RETURNS TABLE (
    item_id VARCHAR(255),
    title TEXT,
    similarity FLOAT,
    score FLOAT
) AS $$
DECLARE
    v_user_embedding vector(1536);
BEGIN
    -- 获取用户向量
    SELECT user_embedding INTO v_user_embedding
    FROM users
    WHERE user_id = p_user_id;

    IF v_user_embedding IS NULL THEN
        RAISE EXCEPTION 'User embedding not found for user_id: %', p_user_id;
    END IF;

    RETURN QUERY
    SELECT
        i.item_id,
        i.title,
        1 - (i.item_embedding <=> v_user_embedding) AS similarity,
        -- 综合评分：向量相似度 + 热度 + 时间衰减
        (1 - (i.item_embedding <=> v_user_embedding)) * 0.6 +
        COALESCE(ub.popularity_score, 0) * 0.3 +
        COALESCE(ub.recency_score, 0) * 0.1 AS score
    FROM items i
    LEFT JOIN LATERAL (
        SELECT
            COUNT(*)::FLOAT / (EXTRACT(EPOCH FROM (NOW() - MIN(time))) / 86400 + 1) AS popularity_score,
            EXP(-EXTRACT(EPOCH FROM (NOW() - MAX(time))) / 86400 / 7) AS recency_score
        FROM user_behaviors
        WHERE item_id = i.item_id
    ) ub ON TRUE
    WHERE i.item_embedding IS NOT NULL
      AND i.item_id != ALL(p_exclude_items)
    ORDER BY score DESC
    LIMIT p_top_k;
END;
$$ LANGUAGE plpgsql;
```

---

## 4. 语义搜索建模

### 4.1 搜索内容表

```sql
-- 搜索内容表
CREATE TABLE search_content (
    id SERIAL PRIMARY KEY,
    content_id VARCHAR(255) UNIQUE NOT NULL,
    content_type VARCHAR(50) NOT NULL,  -- 'article', 'product', 'faq', etc.
    title TEXT NOT NULL,
    content TEXT NOT NULL,
    embedding vector(1536) NOT NULL,
    search_vector tsvector,  -- 全文搜索向量
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建向量索引
CREATE INDEX idx_search_content_embedding_hnsw
ON search_content USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- 创建全文搜索索引（PostgreSQL 18 GIN并行构建）
CREATE INDEX idx_search_content_search_vector_gin
ON search_content USING GIN (search_vector);

-- 自动更新全文搜索向量
CREATE TRIGGER update_search_content_vector
BEFORE INSERT OR UPDATE ON search_content
FOR EACH ROW
EXECUTE FUNCTION tsvector_update_trigger(
    search_vector, 'pg_catalog.english', title, content
);
```

### 4.2 语义搜索函数

```sql
-- 语义搜索函数（混合搜索）
CREATE OR REPLACE FUNCTION semantic_search(
    query_embedding vector(1536),
    query_text TEXT,
    content_type VARCHAR(50) DEFAULT NULL,
    top_k INTEGER DEFAULT 20
)
RETURNS TABLE (
    content_id VARCHAR(255),
    title TEXT,
    content TEXT,
    similarity FLOAT,
    text_rank FLOAT,
    combined_score FLOAT
) AS $$
BEGIN
    RETURN QUERY
    WITH vector_results AS (
        SELECT
            sc.content_id,
            sc.title,
            sc.content,
            1 - (sc.embedding <=> query_embedding) AS similarity
        FROM search_content sc
        WHERE (content_type IS NULL OR sc.content_type = content_type)
        ORDER BY sc.embedding <=> query_embedding
        LIMIT top_k * 2
    ),
    text_results AS (
        SELECT
            sc.content_id,
            sc.title,
            sc.content,
            ts_rank(sc.search_vector, plainto_tsquery('english', query_text)) AS text_rank
        FROM search_content sc
        WHERE (content_type IS NULL OR sc.content_type = content_type)
          AND sc.search_vector @@ plainto_tsquery('english', query_text)
        ORDER BY text_rank DESC
        LIMIT top_k * 2
    )
    SELECT
        COALESCE(v.content_id, t.content_id) AS content_id,
        COALESCE(v.title, t.title) AS title,
        COALESCE(v.content, t.content) AS content,
        COALESCE(v.similarity, 0) AS similarity,
        COALESCE(t.text_rank, 0) AS text_rank,
        COALESCE(v.similarity, 0) * 0.7 + COALESCE(t.text_rank, 0) * 0.3 AS combined_score
    FROM vector_results v
    FULL OUTER JOIN text_results t ON v.content_id = t.content_id
    ORDER BY combined_score DESC
    LIMIT top_k;
END;
$$ LANGUAGE plpgsql;
```

---

## 5. 知识图谱应用建模

### 5.1 实体和关系表

```sql
-- 实体表
CREATE TABLE knowledge_entities (
    id SERIAL PRIMARY KEY,
    entity_id VARCHAR(255) UNIQUE NOT NULL,
    entity_type VARCHAR(50) NOT NULL,  -- 'person', 'organization', 'concept', etc.
    name TEXT NOT NULL,
    description TEXT,
    embedding vector(1536),  -- 实体向量表示
    properties JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 关系表
CREATE TABLE knowledge_relationships (
    id SERIAL PRIMARY KEY,
    source_entity_id VARCHAR(255) NOT NULL REFERENCES knowledge_entities(entity_id),
    target_entity_id VARCHAR(255) NOT NULL REFERENCES knowledge_entities(entity_id),
    relationship_type VARCHAR(100) NOT NULL,  -- 'works_for', 'located_in', 'related_to', etc.
    weight NUMERIC(5,4) DEFAULT 1.0,  -- 关系权重
    properties JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(source_entity_id, target_entity_id, relationship_type)
);

-- 创建向量索引
CREATE INDEX idx_knowledge_entities_embedding_hnsw
ON knowledge_entities USING hnsw (embedding vector_cosine_ops)
WHERE embedding IS NOT NULL;

-- 创建关系索引
CREATE INDEX idx_knowledge_relationships_source ON knowledge_relationships(source_entity_id);
CREATE INDEX idx_knowledge_relationships_target ON knowledge_relationships(target_entity_id);
CREATE INDEX idx_knowledge_relationships_type ON knowledge_relationships(relationship_type);
```

### 5.2 知识图谱查询函数

```sql
-- 实体相似度查询
CREATE OR REPLACE FUNCTION find_similar_entities(
    query_embedding vector(1536),
    entity_type VARCHAR(50) DEFAULT NULL,
    top_k INTEGER DEFAULT 10
)
RETURNS TABLE (
    entity_id VARCHAR(255),
    name TEXT,
    similarity FLOAT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        ke.entity_id,
        ke.name,
        1 - (ke.embedding <=> query_embedding) AS similarity
    FROM knowledge_entities ke
    WHERE ke.embedding IS NOT NULL
      AND (entity_type IS NULL OR ke.entity_type = entity_type)
    ORDER BY ke.embedding <=> query_embedding
    LIMIT top_k;
END;
$$ LANGUAGE plpgsql;

-- 关系路径查询（使用递归CTE）
CREATE OR REPLACE FUNCTION find_relationship_path(
    source_entity_id VARCHAR(255),
    target_entity_id VARCHAR(255),
    max_depth INTEGER DEFAULT 3
)
RETURNS TABLE (
    path TEXT[],
    depth INTEGER,
    total_weight NUMERIC
) AS $$
BEGIN
    RETURN QUERY
    WITH RECURSIVE entity_paths AS (
        -- 起始节点
        SELECT
            ARRAY[source_entity_id]::TEXT[] AS path,
            0 AS depth,
            0::NUMERIC AS total_weight
        WHERE source_entity_id != target_entity_id

        UNION ALL

        -- 递归查找
        SELECT
            ep.path || kr.target_entity_id,
            ep.depth + 1,
            ep.total_weight + kr.weight
        FROM entity_paths ep
        JOIN knowledge_relationships kr ON ep.path[array_length(ep.path, 1)] = kr.source_entity_id
        WHERE ep.depth < max_depth
          AND kr.target_entity_id != ALL(ep.path)  -- 避免循环
          AND kr.target_entity_id != source_entity_id
    )
    SELECT
        ep.path,
        ep.depth,
        ep.total_weight
    FROM entity_paths ep
    WHERE ep.path[array_length(ep.path, 1)] = target_entity_id
    ORDER BY ep.depth, ep.total_weight
    LIMIT 10;
END;
$$ LANGUAGE plpgsql;
```

---

## 6. PostgreSQL 18优化

### 6.1 异步I/O优化

```sql
-- PostgreSQL 18：异步I/O优化（向量检索和批量写入）
BEGIN;
DO $$
BEGIN
    ALTER SYSTEM SET io_direct = 'data';
    ALTER SYSTEM SET io_combine_limit = '256kB';
    PERFORM pg_reload_conf();
    RAISE NOTICE '异步I/O配置已更新（AI应用性能提升50-60%）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置异步I/O失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
END $$;
COMMIT;
```

### 6.2 GIN并行构建优化

```sql
-- PostgreSQL 18：GIN并行构建优化（全文搜索索引）
BEGIN;
DO $$
BEGIN
    ALTER SYSTEM SET max_parallel_maintenance_workers = 8;
    PERFORM pg_reload_conf();
    RAISE NOTICE 'GIN并行构建已配置（全文搜索索引构建速度提升3-4倍）';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置GIN并行构建失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
END $$;
COMMIT;
```

### 6.3 虚拟生成列优化

```sql
-- PostgreSQL 18：使用虚拟生成列优化计算字段
ALTER TABLE user_behaviors
ADD COLUMN behavior_score NUMERIC(10,6) GENERATED ALWAYS AS (
    CASE behavior_type
        WHEN 'purchase' THEN behavior_value * 10
        WHEN 'like' THEN behavior_value * 5
        WHEN 'click' THEN behavior_value * 2
        ELSE behavior_value
    END
) VIRTUAL;
```

---

## 7. 性能优化建议

### 7.1 RAG应用优化

**向量索引优化**:

```sql
-- HNSW索引优化文档向量检索
CREATE INDEX idx_document_chunks_embedding ON document_chunks
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64, ef_search = 100);

-- 全文搜索索引优化
CREATE INDEX idx_document_chunks_text_search ON document_chunks
USING gin(to_tsvector('english', chunk_text));
```

**混合检索优化**:

```sql
-- 创建函数实现混合检索
CREATE OR REPLACE FUNCTION hybrid_search(
    p_query_text TEXT,
    p_query_vector vector(1536),
    p_limit INTEGER DEFAULT 10
)
RETURNS TABLE(chunk_id INTEGER, score FLOAT) AS $$
BEGIN
    RETURN QUERY
    WITH vector_results AS (
        SELECT
            id,
            1 - (embedding <=> p_query_vector) AS vector_score
        FROM document_chunks
        ORDER BY embedding <=> p_query_vector
        LIMIT p_limit * 2
    ),
    text_results AS (
        SELECT
            id,
            ts_rank(to_tsvector('english', chunk_text), plainto_tsquery('english', p_query_text)) AS text_score
        FROM document_chunks
        WHERE to_tsvector('english', chunk_text) @@ plainto_tsquery('english', p_query_text)
        ORDER BY text_score DESC
        LIMIT p_limit * 2
    ),
    combined_results AS (
        SELECT
            COALESCE(v.id, t.id) AS id,
            COALESCE(v.vector_score, 0) * 0.6 + COALESCE(t.text_score, 0) * 0.4 AS combined_score
        FROM vector_results v
        FULL OUTER JOIN text_results t ON v.id = t.id
    )
    SELECT id, combined_score
    FROM combined_results
    ORDER BY combined_score DESC
    LIMIT p_limit;
END;
$$ LANGUAGE plpgsql;
```

### 7.2 推荐系统优化

**用户行为索引优化**:

```sql
-- TimescaleDB自动索引优化
CREATE INDEX idx_user_behaviors_user_time ON user_behaviors(user_id, time DESC);
CREATE INDEX idx_user_behaviors_item_time ON user_behaviors(item_id, time DESC);

-- 创建连续聚合视图
CREATE MATERIALIZED VIEW user_item_interactions_hourly
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 hour', time) AS bucket,
    user_id,
    item_id,
    COUNT(*) AS interaction_count,
    SUM(behavior_value) AS total_value
FROM user_behaviors
GROUP BY bucket, user_id, item_id;
```

**推荐计算优化**:

```sql
-- 使用物化视图缓存用户相似度
CREATE MATERIALIZED VIEW mv_user_similarity AS
SELECT
    u1.user_id AS user1_id,
    u2.user_id AS user2_id,
    (
        SELECT COUNT(*)
        FROM user_behaviors ub1
        JOIN user_behaviors ub2 ON ub1.item_id = ub2.item_id
        WHERE ub1.user_id = u1.user_id AND ub2.user_id = u2.user_id
    )::FLOAT / NULLIF(
        (SELECT COUNT(DISTINCT item_id) FROM user_behaviors WHERE user_id = u1.user_id), 0
    ) AS similarity
FROM (SELECT DISTINCT user_id FROM user_behaviors) u1
CROSS JOIN (SELECT DISTINCT user_id FROM user_behaviors) u2
WHERE u1.user_id < u2.user_id;
```

### 7.3 语义搜索优化

**向量索引优化**:

```sql
-- HNSW索引优化
CREATE INDEX idx_search_content_embedding ON search_content
USING hnsw (content_embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- 全文搜索索引
CREATE INDEX idx_search_content_text ON search_content
USING gin(to_tsvector('english', title || ' ' || content));
```

**缓存策略**:

```sql
-- 创建查询缓存表
CREATE TABLE search_query_cache (
    query_hash VARCHAR(64) PRIMARY KEY,
    query_text TEXT NOT NULL,
    results JSONB NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    expires_at TIMESTAMPTZ DEFAULT NOW() + INTERVAL '1 hour'
);

-- 创建索引
CREATE INDEX idx_search_query_cache_expires ON search_query_cache(expires_at);
```

### 7.4 知识图谱优化

**实体向量索引**:

```sql
-- 实体向量索引
CREATE INDEX idx_knowledge_entities_embedding ON knowledge_entities
USING hnsw (entity_embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);
```

**图查询优化**:

```sql
-- 使用递归CTE优化路径查询
CREATE OR REPLACE FUNCTION find_entity_path(
    p_start_entity_id INTEGER,
    p_end_entity_id INTEGER,
    p_max_depth INTEGER DEFAULT 5
)
RETURNS TABLE(path INTEGER[]) AS $$
BEGIN
    RETURN QUERY
    WITH RECURSIVE entity_path AS (
        SELECT
            ARRAY[start_entity_id, end_entity_id] AS path,
            1 AS depth
        FROM knowledge_relations
        WHERE start_entity_id = p_start_entity_id
          AND end_entity_id = p_end_entity_id
        UNION ALL
        SELECT
            ep.path || kr.end_entity_id,
            ep.depth + 1
        FROM entity_path ep
        JOIN knowledge_relations kr ON ep.path[array_length(ep.path, 1)] = kr.start_entity_id
        WHERE ep.depth < p_max_depth
          AND NOT (kr.end_entity_id = ANY(ep.path))
    )
    SELECT path FROM entity_path
    WHERE path[array_length(path, 1)] = p_end_entity_id
    ORDER BY array_length(path, 1)
    LIMIT 1;
END;
$$ LANGUAGE plpgsql;
```

---

## 8. 最佳实践

### 8.1 RAG应用

1. **文档分块**: 使用固定大小或语义分块
2. **混合搜索**: 结合向量相似度和全文搜索
3. **重排序**: 使用业务规则进行结果重排序
4. **缓存策略**: 缓存常见查询结果

### 8.2 推荐系统

1. **多策略融合**: 结合协同过滤、内容推荐、深度学习
2. **实时更新**: 使用TimescaleDB存储用户行为
3. **冷启动**: 使用内容特征处理新用户/新物品
4. **多样性**: 确保推荐结果的多样性

### 8.3 语义搜索

1. **索引优化**: 使用HNSW索引加速向量检索
2. **缓存策略**: 缓存热门查询结果
3. **结果多样性**: 确保结果多样性
4. **查询优化**: 优化查询性能

### 8.4 知识图谱

1. **实体向量化**: 使用实体描述生成向量
2. **关系权重**: 根据关系重要性设置权重
3. **路径查询**: 使用递归CTE进行路径查询
4. **图算法**: 使用图算法发现模式

### 8.5 SQL实现注意事项

1. **错误处理**: 使用DO块处理AI应用操作错误
2. **性能监控**: 使用EXPLAIN ANALYZE分析查询性能
3. **并发控制**: 处理并发请求的竞争条件
4. **资源管理**: 限制查询的内存和CPU使用

---

## 9. 常见问题与解决方案

### 问题1: RAG检索延迟高

**原因**:

- 向量索引未优化
- 查询向量未缓存
- 文档数量过大

**解决方案**:

- 使用HNSW索引优化向量检索
- 缓存查询向量
- 使用PCA降维减少向量维度

**示例**:

```sql
-- 优化向量检索
CREATE INDEX idx_document_chunks_embedding ON document_chunks
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64, ef_search = 100);

-- 查询优化
SELECT id, chunk_text, 1 - (embedding <=> $query_vector) AS similarity
FROM document_chunks
ORDER BY embedding <=> $query_vector
LIMIT 10;
```

### 问题2: 推荐系统冷启动问题

**原因**:

- 新用户/新物品缺少行为数据
- 协同过滤无法工作

**解决方案**:

- 使用内容特征推荐
- 使用热门推荐
- 使用混合推荐策略

**示例**:

```sql
-- 新用户推荐（基于内容特征）
CREATE OR REPLACE FUNCTION recommend_for_new_user(
    p_user_id INTEGER,
    p_limit INTEGER DEFAULT 10
)
RETURNS TABLE(item_id INTEGER, score FLOAT) AS $$
BEGIN
    RETURN QUERY
    -- 基于物品特征推荐
    SELECT
        i.item_id,
        AVG(ic.feature_value::FLOAT) AS content_score
    FROM items i
    JOIN item_features ic ON i.item_id = ic.item_id
    WHERE i.item_id NOT IN (
        SELECT item_id FROM user_behaviors WHERE user_id = p_user_id
    )
    GROUP BY i.item_id
    ORDER BY content_score DESC
    LIMIT p_limit;
END;
$$ LANGUAGE plpgsql;
```

### 问题3: 语义搜索结果不准确

**原因**:

- 向量质量差
- 查询理解不准确
- 缺少重排序

**解决方案**:

- 使用更好的向量模型
- 优化查询预处理
- 实现重排序机制

### 问题4: 知识图谱查询性能差

**原因**:

- 图规模过大
- 路径查询深度过大
- 缺少索引

**解决方案**:

- 限制路径查询深度
- 为常用查询创建索引
- 使用图数据库（如Apache AGE）

**示例**:

```sql
-- 限制路径查询深度
WITH RECURSIVE entity_path AS (
    SELECT
        ARRAY[start_entity_id, end_entity_id] AS path,
        1 AS depth
    FROM knowledge_relations
    WHERE start_entity_id = $start_id
    UNION ALL
    SELECT
        ep.path || kr.end_entity_id,
        ep.depth + 1
    FROM entity_path ep
    JOIN knowledge_relations kr ON ep.path[array_length(ep.path, 1)] = kr.start_entity_id
    WHERE ep.depth < 5  -- 限制最大深度
      AND NOT (kr.end_entity_id = ANY(ep.path))
)
SELECT path FROM entity_path
WHERE path[array_length(path, 1)] = $end_id;
```

---

## 10. 相关资源

### 10.1 核心相关文档

- [向量数据库建模](../08-PostgreSQL建模实践/向量数据库建模.md) - pgvector建模指南
- [时序数据建模](../06-IoT与时序建模/时序数据模型.md) - TimescaleDB建模指南
- [PostgreSQL18新特性](../08-PostgreSQL建模实践/PostgreSQL18新特性.md) - PostgreSQL 18新特性指南

### 10.2 官方资源

- [pgvector GitHub](https://github.com/pgvector/pgvector) - pgvector官方仓库
- [TimescaleDB文档](https://docs.timescale.com/) - TimescaleDB官方文档
- [PostgreSQL 18文档](https://www.postgresql.org/docs/18/) - PostgreSQL 18官方文档

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**状态**: ✅ 已完成
