# OLAP维度表技术完整指南

> **创建日期**: 2025年1月
> **来源**: Kimball《数据仓库工具箱》+ 权威资源对齐
> **状态**: 基于权威资源深化扩展
> **文档编号**: 05-03

---

## 📑 目录

- [OLAP维度表技术完整指南](#olap维度表技术完整指南)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 维度表基本概念](#111-维度表基本概念)
    - [1.1.2 缓慢变化维理论](#112-缓慢变化维理论)
    - [1.1.3 SCD Type 2理论](#113-scd-type-2理论)
    - [1.1.4 角色扮演维度理论](#114-角色扮演维度理论)
    - [1.1.5 杂项维度理论](#115-杂项维度理论)
    - [1.1.6 一致性维度理论](#116-一致性维度理论)
    - [1.1.7 复杂度分析](#117-复杂度分析)
  - [2. 缓慢变化维度（SCD）](#2-缓慢变化维度scd)
    - [2.1 SCD Type 1: 覆盖历史值](#21-scd-type-1-覆盖历史值)
    - [2.2 SCD Type 2: 保留完整历史](#22-scd-type-2-保留完整历史)
    - [2.3 SCD Type 3: 保留有限历史](#23-scd-type-3-保留有限历史)
    - [2.4 SCD Type 4: 快照表](#24-scd-type-4-快照表)
    - [2.5 SCD Type 6: 混合方法](#25-scd-type-6-混合方法)
  - [3. 角色扮演维度](#3-角色扮演维度)
  - [4. 杂项维度](#4-杂项维度)
  - [5. 退化维度](#5-退化维度)
  - [6. 一致性维度](#6-一致性维度)
  - [7. PostgreSQL实现](#7-postgresql实现)
    - [7.1 SCD Type 2自动处理](#71-scd-type-2自动处理)
    - [7.2 维度表索引策略](#72-维度表索引策略)
  - [8. 实际应用案例 / Practical Application Examples](#8-实际应用案例--practical-application-examples)
    - [8.1 案例1: SCD Type 2产品维度](#81-案例1-scd-type-2产品维度)
    - [8.2 案例2: 角色扮演维度应用](#82-案例2-角色扮演维度应用)
    - [8.3 案例3: 杂项维度应用](#83-案例3-杂项维度应用)
  - [9. 性能优化与监控 / Performance Optimization and Monitoring](#9-性能优化与监控--performance-optimization-and-monitoring)
    - [9.1 维度表性能优化](#91-维度表性能优化)
    - [9.2 SCD性能监控](#92-scd性能监控)
  - [10. 常见问题解答 / FAQ](#10-常见问题解答--faq)
    - [Q1: SCD Type 1、2、3如何选择？](#q1-scd-type-123如何选择)
    - [Q2: SCD Type 2会导致维度表很大吗？](#q2-scd-type-2会导致维度表很大吗)
    - [Q3: 角色扮演维度如何优化？](#q3-角色扮演维度如何优化)
    - [Q4: 杂项维度什么时候使用？](#q4-杂项维度什么时候使用)
    - [Q5: 如何优化维度表查询性能？](#q5-如何优化维度表查询性能)
  - [11. 相关资源 / Related Resources](#11-相关资源--related-resources)
    - [11.1 核心相关文档 / Core Related Documents](#111-核心相关文档--core-related-documents)
    - [11.2 理论基础 / Theoretical Foundation](#112-理论基础--theoretical-foundation)
    - [11.3 实践指南 / Practical Guides](#113-实践指南--practical-guides)
    - [11.4 应用案例 / Application Cases](#114-应用案例--application-cases)
    - [11.5 参考资源 / Reference Resources](#115-参考资源--reference-resources)

---

## 1. 概述

维度表是维度建模的核心组件，描述业务过程的上下文。
Kimball定义了多种维度表技术，包括缓慢变化维度（SCD）、角色扮演维度、杂项维度等。
正确使用这些技术对数据仓库设计至关重要。

---

## 1.1 理论基础

### 1.1.1 维度表基本概念

**维度表（Dimension Table）**描述业务过程的上下文：

- **维度属性**: 提供查询和过滤条件
- **维度键**: 主键，被事实表引用
- **业务键**: 业务系统中的唯一标识

**维度表特征**:

- **行数少**: 通常数千到数百万行
- **列数多**: 通常20-100列
- **可更新**: 维度属性可以更新

### 1.1.2 缓慢变化维理论

**缓慢变化维（Slowly Changing Dimension, SCD）**:

- **问题**: 维度属性会缓慢变化
- **挑战**: 如何保留历史值
- **解决方案**: SCD Type 1/2/3/4/6

**SCD类型对比**:

| 类型 | 历史保留 | 存储成本 | 查询复杂度 | 适用场景 |
|------|---------|---------|-----------|---------|
| **Type 1** | 不保留 | 低 | 低 | 错误更正 |
| **Type 2** | 完整保留 | 高 | 中 | 历史分析 |
| **Type 3** | 部分保留 | 中 | 中 | 有限历史 |
| **Type 4** | 快照保留 | 高 | 高 | 完整历史 |
| **Type 6** | 混合保留 | 高 | 高 | 复杂场景 |

### 1.1.3 SCD Type 2理论

**SCD Type 2**保留完整历史：

- **方法**: 添加新行保留历史值
- **标识**: 使用代理键（Surrogate Key）
- **时间戳**: 使用有效日期标识版本

**数学表示**:

- **维度版本**: $D_v = \{d_1, d_2, ..., d_n\}$
- **有效时间**: $T_{valid} = [T_{from}, T_{to})$
- **当前版本**: $D_{current} = \{d | T_{to} = NULL\}$

### 1.1.4 角色扮演维度理论

**角色扮演维度（Role-Playing Dimension）**:

- **定义**: 同一维度表在事实表中扮演多个角色
- **示例**: 日期维度可以扮演订单日期、发货日期、交付日期
- **实现**: 在事实表中使用多个外键引用同一维度表

**角色扮演维度优化**:

- **视图**: 为每个角色创建视图
- **别名**: 使用表别名区分角色
- **性能**: 不影响查询性能

### 1.1.5 杂项维度理论

**杂项维度（Junk Dimension）**:

- **定义**: 将多个低基数的标志位组合成一个维度
- **目的**: 减少事实表的列数
- **示例**: 将多个布尔标志组合成一个维度

**杂项维度设计**:

- **组合**: $D_{junk} = \{f_1, f_2, ..., f_n\}$ where $f_i$ is flag
- **基数**: $|D_{junk}| = \prod_{i=1}^{n} |f_i|$
- **优化**: 只包含实际存在的组合

### 1.1.6 一致性维度理论

**一致性维度（Conformed Dimension）**:

- **定义**: 在多个数据仓库中共享的维度
- **目的**: 支持跨数据仓库分析
- **要求**: 维度定义和属性一致

**一致性维度原则**:

- **定义一致**: 维度定义必须一致
- **属性一致**: 维度属性必须一致
- **键一致**: 维度键必须一致

### 1.1.7 复杂度分析

**存储复杂度**:

- **维度表**: $O(D)$ where D is number of dimension rows
- **SCD Type 2**: $O(D \times V)$ where V is average versions per dimension
- **杂项维度**: $O(C)$ where C is number of combinations

**查询复杂度**:

- **维度查询**: $O(\log D)$ with index
- **SCD Type 2查询**: $O(\log D \times V)$ with index
- **角色扮演查询**: $O(\log D)$ with index

---

## 2. 缓慢变化维度（SCD）

### 2.1 SCD Type 1: 覆盖历史值

**定义**: 直接更新维度记录，不保留历史值。

**特点**:

- 简单直接
- 不保留历史
- 适合不重要的属性变更

**适用场景**:

- 客户地址更正
- 产品描述更新
- 不重要的属性变更

**PostgreSQL实现**:

```sql
-- SCD Type 1：客户维度表
CREATE TABLE dim_customer (
    customer_key SERIAL PRIMARY KEY,
    customer_id INT NOT NULL UNIQUE,  -- 业务键
    customer_name VARCHAR(100) NOT NULL,
    email VARCHAR(100),
    phone VARCHAR(20),
    city VARCHAR(50),
    state VARCHAR(50),
    country VARCHAR(50) DEFAULT 'China',
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 更新客户信息（直接覆盖）
UPDATE dim_customer
SET
    email = 'newemail@example.com',
    phone = '13800138000',
    updated_at = NOW()
WHERE customer_id = 123;
```

---

### 2.2 SCD Type 2: 保留完整历史

**定义**: 为每个变化创建新记录，保留完整历史。

**特点**:

- 保留完整历史
- 支持历史分析
- 需要有效日期字段

**适用场景**:

- 客户地址变更
- 产品分类变更
- 员工部门变更

**PostgreSQL实现**:

```sql
-- SCD Type 2：客户维度表（保留历史，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dim_customer') THEN
        CREATE TABLE dim_customer (
            customer_key SERIAL PRIMARY KEY,
            customer_id INT NOT NULL,  -- 业务键（不唯一）
            customer_name VARCHAR(100) NOT NULL,
            email VARCHAR(100),
            phone VARCHAR(20),
            city VARCHAR(50),
            state VARCHAR(50),
            -- SCD Type 2字段
            valid_from DATE NOT NULL,
            valid_to DATE,
            is_current BOOLEAN DEFAULT TRUE,
            version_number INT DEFAULT 1,
            created_at TIMESTAMPTZ DEFAULT NOW(),
            CHECK (valid_to IS NULL OR valid_to >= valid_from)
        );
        RAISE NOTICE '表 dim_customer 创建成功';
    ELSE
        RAISE NOTICE '表 dim_customer 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 dim_customer 失败: %', SQLERRM;
END $$;

-- 创建唯一索引（业务键 + 有效日期，带错误处理）
DO $$
BEGIN
    CREATE UNIQUE INDEX IF NOT EXISTS idx_dim_customer_business_key
    ON dim_customer(customer_id, valid_from);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;

-- 插入新客户
INSERT INTO dim_customer (customer_id, customer_name, email, city, valid_from)
VALUES (123, '张三', 'zhangsan@example.com', '北京', '2024-01-01');

-- 客户地址变更（创建新记录）
INSERT INTO dim_customer (customer_id, customer_name, email, city, valid_from, version_number)
VALUES (123, '张三', 'zhangsan@example.com', '上海', '2024-06-01', 2);

-- 更新旧记录的valid_to和is_current
UPDATE dim_customer
SET
    valid_to = '2024-05-31',
    is_current = FALSE
WHERE customer_id = 123
  AND is_current = TRUE
  AND version_number = 1;

-- 查询当前有效记录
-- 查询维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_customer
WHERE customer_id = 123
  AND is_current = TRUE;

-- 查询历史记录
-- 查询维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_customer
WHERE customer_id = 123
ORDER BY valid_from DESC;
```

**自动处理SCD Type 2的函数**:

```sql
-- SCD Type 2更新函数
CREATE OR REPLACE FUNCTION update_dim_customer_scd2(
    p_customer_id INT,
    p_customer_name VARCHAR(100),
    p_email VARCHAR(100),
    p_city VARCHAR(50),
    p_change_date DATE DEFAULT CURRENT_DATE
)
RETURNS INT AS $$
DECLARE
    v_new_key INT;
    v_new_version INT;
BEGIN
    -- 获取当前版本号
    SELECT COALESCE(MAX(version_number), 0) + 1
    INTO v_new_version
    FROM dim_customer
    WHERE customer_id = p_customer_id;

    -- 关闭旧记录
    UPDATE dim_customer
    SET
        valid_to = p_change_date - INTERVAL '1 day',
        is_current = FALSE,
        updated_at = NOW()
    WHERE customer_id = p_customer_id
      AND is_current = TRUE;

    -- 插入新记录
    INSERT INTO dim_customer (
        customer_id, customer_name, email, city,
        valid_from, version_number, is_current
    )
    VALUES (
        p_customer_id, p_customer_name, p_email, p_city,
        p_change_date, v_new_version, TRUE
    )
    RETURNING customer_key INTO v_new_key;

    RETURN v_new_key;
END;
$$ LANGUAGE plpgsql;
```

---

### 2.3 SCD Type 3: 保留有限历史

**定义**: 在维度表中添加"旧值"列，仅保留上一次的值。

**特点**:

- 保留有限历史（仅上一次）
- 表结构简单
- 适合偶尔变化的属性

**适用场景**:

- 产品当前分类和上一分类
- 客户当前地区和上一地区

**PostgreSQL实现**:

```sql
-- SCD Type 3：产品维度表（保留上一值）
CREATE TABLE dim_product (
    product_key SERIAL PRIMARY KEY,
    product_id INT NOT NULL UNIQUE,
    product_name VARCHAR(200) NOT NULL,
    -- 当前值
    current_category VARCHAR(100),
    current_brand VARCHAR(100),
    -- 上一值（SCD Type 3）
    previous_category VARCHAR(100),
    previous_brand VARCHAR(100),
    -- 变更日期
    category_changed_date DATE,
    brand_changed_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 更新产品分类（保留上一值）
CREATE OR REPLACE FUNCTION update_product_category_scd3(
    p_product_id INT,
    p_new_category VARCHAR(100)
)
RETURNS VOID AS $$
BEGIN
    UPDATE dim_product
    SET
        previous_category = current_category,
        current_category = p_new_category,
        category_changed_date = CURRENT_DATE,
        updated_at = NOW()
    WHERE product_id = p_product_id;
END;
$$ LANGUAGE plpgsql;
```

---

### 2.4 SCD Type 4: 快照表

**定义**: 使用单独的迷你维度表存储频繁变化的属性。

**特点**:

- 主维度表稳定
- 迷你维度表存储变化
- 适合频繁变化的属性

**适用场景**:

- 产品价格历史
- 客户信用等级历史
- 库存水平历史

**PostgreSQL实现**:

```sql
-- 主维度表（稳定属性）
CREATE TABLE dim_product (
    product_key SERIAL PRIMARY KEY,
    product_id INT NOT NULL UNIQUE,
    product_name VARCHAR(200) NOT NULL,
    category VARCHAR(100),
    brand VARCHAR(100)
);

-- 迷你维度表（价格历史）
CREATE TABLE dim_product_price (
    price_key SERIAL PRIMARY KEY,
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    price NUMERIC(10,2) NOT NULL,
    effective_date DATE NOT NULL,
    expiry_date DATE,
    is_current BOOLEAN DEFAULT TRUE,
    UNIQUE(product_key, effective_date)
);

-- 查询产品当前价格
SELECT
    p.product_name,
    pp.price AS current_price,
    pp.effective_date
FROM dim_product p
JOIN dim_product_price pp ON p.product_key = pp.product_key
WHERE pp.is_current = TRUE;
```

---

### 2.5 SCD Type 6: 混合方法

**定义**: 结合SCD Type 1、Type 2、Type 3的混合方法。

**特点**:

- 灵活处理不同属性
- 部分属性保留历史，部分直接更新
- 适合复杂业务场景

**PostgreSQL实现**:

```sql
-- SCD Type 6：混合方法
CREATE TABLE dim_customer (
    customer_key SERIAL PRIMARY KEY,
    customer_id INT NOT NULL,
    customer_name VARCHAR(100) NOT NULL,
    -- Type 1：直接更新（不保留历史）
    email VARCHAR(100),
    phone VARCHAR(20),
    -- Type 2：保留完整历史
    city VARCHAR(50),
    state VARCHAR(50),
    valid_from DATE NOT NULL,
    valid_to DATE,
    is_current BOOLEAN DEFAULT TRUE,
    -- Type 3：保留上一值
    previous_city VARCHAR(50),
    city_changed_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
```

---

## 3. 角色扮演维度

**定义**: 同一个维度表在事实表中多次使用，扮演不同角色。

**特点**:

- 避免重复维度表
- 使用视图或别名
- 常见于时间维度

**PostgreSQL实现**:

```sql
-- 订单事实表（多个日期维度）
CREATE TABLE fact_orders (
    order_key BIGSERIAL PRIMARY KEY,
    order_date_key INT NOT NULL REFERENCES dim_date(date_key),
    ship_date_key INT REFERENCES dim_date(date_key),
    delivery_date_key INT REFERENCES dim_date(date_key),
    cancel_date_key INT REFERENCES dim_date(date_key),
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    customer_key INT NOT NULL REFERENCES dim_customer(customer_key),
    order_amount NUMERIC(10,2) NOT NULL
);

-- 创建角色扮演视图
CREATE VIEW dim_order_date AS SELECT * FROM dim_date;
CREATE VIEW dim_ship_date AS SELECT * FROM dim_date;
CREATE VIEW dim_delivery_date AS SELECT * FROM dim_date;

-- 查询（使用视图）
SELECT
    od.year_number AS order_year,
    sd.month_name AS ship_month,
    dd.day_name AS delivery_day,
    SUM(f.order_amount) AS total_amount
FROM fact_orders f
JOIN dim_order_date od ON f.order_date_key = od.date_key
JOIN dim_ship_date sd ON f.ship_date_key = sd.date_key
JOIN dim_delivery_date dd ON f.delivery_date_key = dd.date_key
GROUP BY od.year_number, sd.month_name, dd.day_name;
```

---

## 4. 杂项维度

**定义**: 将多个低基数的标志和指标组合成一个维度表。

**特点**:

- 减少事实表列数
- 提高查询性能
- 适合多个布尔标志

**PostgreSQL实现**:

```sql
-- 杂项维度表
CREATE TABLE dim_order_junk (
    junk_key SERIAL PRIMARY KEY,
    -- 组合多个标志
    is_online_order BOOLEAN NOT NULL,
    is_express_delivery BOOLEAN NOT NULL,
    is_gift_wrapped BOOLEAN NOT NULL,
    payment_method VARCHAR(20) NOT NULL,
    delivery_method VARCHAR(20) NOT NULL,
    -- 唯一约束
    UNIQUE(is_online_order, is_express_delivery, is_gift_wrapped, payment_method, delivery_method)
);

-- 订单事实表（使用杂项维度）
CREATE TABLE fact_orders (
    order_key BIGSERIAL PRIMARY KEY,
    order_date_key INT NOT NULL REFERENCES dim_date(date_key),
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    customer_key INT NOT NULL REFERENCES dim_customer(customer_key),
    junk_key INT NOT NULL REFERENCES dim_order_junk(junk_key),  -- 杂项维度
    order_amount NUMERIC(10,2) NOT NULL
);

-- 查询（使用杂项维度）
SELECT
    d.year_number,
    j.is_online_order,
    j.payment_method,
    SUM(f.order_amount) AS total_amount
FROM fact_orders f
JOIN dim_date d ON f.order_date_key = d.date_key
JOIN dim_order_junk j ON f.junk_key = j.junk_key
WHERE j.is_express_delivery = TRUE
GROUP BY d.year_number, j.is_online_order, j.payment_method;
```

---

## 5. 退化维度

**定义**: 维度属性直接存储在事实表中，不单独创建维度表。

**适用场景**:

- 交易编号
- 订单编号
- 发票编号

**PostgreSQL实现**:

```sql
-- 事实表（包含退化维度）
CREATE TABLE fact_sales (
    sale_id BIGSERIAL PRIMARY KEY,
    date_key INT NOT NULL REFERENCES dim_date(date_key),
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    -- 退化维度（直接存储在事实表中）
    transaction_number VARCHAR(50),  -- 交易编号
    receipt_number VARCHAR(50),      -- 收据编号
    -- 度量值
    sales_amount NUMERIC(10,2) NOT NULL
);

-- 为退化维度创建索引
CREATE INDEX idx_fact_sales_transaction_num ON fact_sales(transaction_number);
CREATE INDEX idx_fact_sales_receipt_num ON fact_sales(receipt_number);
```

---

## 6. 一致性维度

**定义**: 在不同事实表中使用相同的维度表，确保一致性。

**特点**:

- 维度键值一致
- 属性定义一致
- 支持跨事实表分析

**PostgreSQL实现**:

```sql
-- 一致性维度：时间维度
CREATE TABLE dim_date (
    date_key INT PRIMARY KEY,
    date_actual DATE NOT NULL UNIQUE,
    day_name VARCHAR(10),
    month_name VARCHAR(10),
    quarter_number INT,
    year_number INT
);

-- 多个事实表使用相同的时间维度
CREATE TABLE fact_sales (
    sale_id BIGSERIAL PRIMARY KEY,
    date_key INT NOT NULL REFERENCES dim_date(date_key),  -- 一致性维度
    ...
);

CREATE TABLE fact_orders (
    order_id BIGSERIAL PRIMARY KEY,
    date_key INT NOT NULL REFERENCES dim_date(date_key),  -- 一致性维度
    ...
);

-- 跨事实表分析
SELECT
    d.year_number,
    SUM(s.sales_amount) AS total_sales,
    SUM(o.order_amount) AS total_orders
FROM dim_date d
LEFT JOIN fact_sales s ON d.date_key = s.date_key
LEFT JOIN fact_orders o ON d.date_key = o.date_key
WHERE d.year_number = 2024
GROUP BY d.year_number;
```

---

## 7. PostgreSQL实现

### 7.1 SCD Type 2自动处理

```sql
-- 创建SCD Type 2处理函数
CREATE OR REPLACE FUNCTION process_scd_type2(
    p_table_name TEXT,
    p_business_key_col TEXT,
    p_business_key_val INT,
    p_change_date DATE DEFAULT CURRENT_DATE
)
RETURNS INT AS $$
DECLARE
    v_sql TEXT;
    v_new_key INT;
BEGIN
    -- 关闭旧记录
    v_sql := format('
        UPDATE %I
        SET valid_to = $1 - INTERVAL ''1 day'',
            is_current = FALSE,
            updated_at = NOW()
        WHERE %I = $2
          AND is_current = TRUE',
        p_table_name, p_business_key_col);

    EXECUTE v_sql USING p_change_date, p_business_key_val;

    -- 返回新键（需要根据实际情况实现）
    RETURN v_new_key;
END;
$$ LANGUAGE plpgsql;
```

---

### 7.2 维度表索引策略

```sql
-- SCD Type 2维度表索引
CREATE INDEX idx_dim_customer_business_key ON dim_customer(customer_id, valid_from);
CREATE INDEX idx_dim_customer_current ON dim_customer(customer_id) WHERE is_current = TRUE;
CREATE INDEX idx_dim_customer_valid_date ON dim_customer(valid_from, valid_to);

-- 维度表查询优化
-- 查询当前有效记录（使用部分索引）
-- 查询维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_customer
WHERE customer_id = 123
  AND is_current = TRUE;

-- 查询历史记录（使用日期索引）
-- 查询维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_customer
WHERE customer_id = 123
  AND valid_from <= '2024-06-01'
  AND (valid_to IS NULL OR valid_to >= '2024-06-01');
```

---

## 8. 实际应用案例 / Practical Application Examples

### 8.1 案例1: SCD Type 2产品维度

**完整SCD Type 2实现**:

```sql
-- SCD Type 2产品维度表
CREATE TABLE dim_product_scd2 (
    product_key SERIAL PRIMARY KEY,
    product_id INT NOT NULL,
    product_code VARCHAR(50) NOT NULL,
    product_name VARCHAR(200) NOT NULL,
    category_name VARCHAR(100),
    brand_name VARCHAR(100),
    unit_price NUMERIC(10,2),
    valid_from DATE NOT NULL,
    valid_to DATE,
    is_current BOOLEAN DEFAULT TRUE,
    UNIQUE(product_id, valid_from)
);

-- SCD Type 2更新函数
CREATE OR REPLACE FUNCTION update_product_scd2(
    p_product_id INT,
    p_new_data JSONB
)
RETURNS VOID AS $$
DECLARE
    v_current_key INT;
BEGIN
    -- 关闭当前版本
    UPDATE dim_product_scd2
    SET valid_to = CURRENT_DATE - INTERVAL '1 day',
        is_current = FALSE
    WHERE product_id = p_product_id
      AND is_current = TRUE;

    -- 创建新版本
    INSERT INTO dim_product_scd2 (
        product_id, product_code, product_name,
        category_name, brand_name, unit_price,
        valid_from, is_current
    )
    VALUES (
        p_product_id,
        p_new_data->>'product_code',
        p_new_data->>'product_name',
        p_new_data->>'category_name',
        p_new_data->>'brand_name',
        (p_new_data->>'unit_price')::NUMERIC,
        CURRENT_DATE,
        TRUE
    );
END;
$$ LANGUAGE plpgsql;

-- 查询历史版本
-- 查询SCD Type 2维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_product_scd2
WHERE product_id = 123
  AND valid_from <= '2024-06-01'
  AND (valid_to IS NULL OR valid_to >= '2024-06-01');
```

### 8.2 案例2: 角色扮演维度应用

**日期角色扮演维度**:

```sql
-- 事实表：订单事实（多个日期角色）
CREATE TABLE fact_orders (
    order_id BIGSERIAL,
    order_date_key INT NOT NULL REFERENCES dim_date(date_key),
    ship_date_key INT REFERENCES dim_date(date_key),
    delivery_date_key INT REFERENCES dim_date(date_key),
    customer_key INT NOT NULL REFERENCES dim_customer(customer_key),
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    order_amount NUMERIC(10,2) NOT NULL
);

-- 查询：使用角色扮演维度
-- 按订单日期分析
SELECT
    d1.year_number,
    d1.month_number,
    SUM(f.order_amount) AS total_orders
FROM fact_orders f
JOIN dim_date d1 ON f.order_date_key = d1.date_key
GROUP BY d1.year_number, d1.month_number;

-- 按发货日期分析
SELECT
    d2.year_number,
    d2.month_number,
    SUM(f.order_amount) AS total_shipped
FROM fact_orders f
JOIN dim_date d2 ON f.ship_date_key = d2.date_key
GROUP BY d2.year_number, d2.month_number;
```

### 8.3 案例3: 杂项维度应用

**订单杂项维度**:

```sql
-- 杂项维度：订单属性组合
CREATE TABLE dim_order_junk (
    junk_key SERIAL PRIMARY KEY,
    payment_method VARCHAR(50),
    shipping_method VARCHAR(50),
    order_source VARCHAR(50),
    promotion_type VARCHAR(50),
    is_gift BOOLEAN,
    is_express BOOLEAN,
    UNIQUE(payment_method, shipping_method, order_source, promotion_type, is_gift, is_express)
);

-- 事实表引用杂项维度
CREATE TABLE fact_order_detail (
    order_id BIGSERIAL,
    date_key INT NOT NULL REFERENCES dim_date(date_key),
    product_key INT NOT NULL REFERENCES dim_product(product_key),
    customer_key INT NOT NULL REFERENCES dim_customer(customer_key),
    junk_key INT REFERENCES dim_order_junk(junk_key),
    order_amount NUMERIC(10,2) NOT NULL
);

-- 查询：使用杂项维度
SELECT
    oj.payment_method,
    oj.shipping_method,
    SUM(f.order_amount) AS total_amount,
    COUNT(*) AS order_count
FROM fact_order_detail f
JOIN dim_order_junk oj ON f.junk_key = oj.junk_key
GROUP BY oj.payment_method, oj.shipping_method;
```

---

## 9. 性能优化与监控 / Performance Optimization and Monitoring

### 9.1 维度表性能优化

**索引优化**:

```sql
-- SCD Type 2维度表索引
CREATE INDEX idx_dim_product_id_current ON dim_product_scd2(product_id) WHERE is_current = TRUE;
CREATE INDEX idx_dim_product_valid_range ON dim_product_scd2(product_id, valid_from, valid_to);

-- 杂项维度索引
CREATE INDEX idx_dim_junk_lookup ON dim_order_junk(payment_method, shipping_method, order_source);
```

**查询优化**:

```sql
-- ✅ 优化：使用当前版本索引
-- 查询SCD Type 2维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_product_scd2
WHERE product_id = 123 AND is_current = TRUE;  -- 使用部分索引

-- ✅ 优化：历史版本查询
-- 查询SCD Type 2维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_product_scd2
WHERE product_id = 123
  AND valid_from <= '2024-06-01'
  AND (valid_to IS NULL OR valid_to >= '2024-06-01');  -- 使用范围索引
```

### 9.2 SCD性能监控

**监控SCD表大小**:

```sql
-- 监控维度表大小和版本数
SELECT
    'dim_product_scd2' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE is_current = TRUE) AS current_rows,
    COUNT(*) FILTER (WHERE is_current = FALSE) AS historical_rows,
    COUNT(DISTINCT product_id) AS unique_products,
    AVG(COUNT(*) OVER (PARTITION BY product_id)) AS avg_versions_per_product
FROM dim_product_scd2;

-- 监控SCD更新频率
SELECT
    DATE_TRUNC('month', valid_from) AS month,
    COUNT(*) AS new_versions,
    COUNT(DISTINCT product_id) AS products_updated
FROM dim_product_scd2
WHERE valid_from >= CURRENT_DATE - INTERVAL '12 months'
GROUP BY month
ORDER BY month DESC;
```

---

## 10. 常见问题解答 / FAQ

### Q1: SCD Type 1、2、3如何选择？

**A**: 选择原则：

| SCD类型 | 适用场景 | 特点 |
|---------|---------|------|
| Type 1 | 错误修正、不重要属性 | 覆盖历史，不保留历史 |
| Type 2 | 重要属性变更 | 保留完整历史 |
| Type 3 | 有限历史需求 | 保留有限历史列 |

**选择决策**:

- 需要完整历史 → SCD Type 2
- 只需要当前值 → SCD Type 1
- 需要有限历史 → SCD Type 3

### Q2: SCD Type 2会导致维度表很大吗？

**A**: 大小控制策略：

```sql
-- 监控维度表大小
SELECT
    product_id,
    COUNT(*) AS version_count,
    MAX(valid_from) AS latest_version
FROM dim_product_scd2
GROUP BY product_id
HAVING COUNT(*) > 10  -- 版本过多的产品
ORDER BY version_count DESC;

-- 优化：归档旧版本
CREATE TABLE dim_product_scd2_archive (LIKE dim_product_scd2 INCLUDING ALL);
INSERT INTO dim_product_scd2_archive
-- 查询SCD Type 2维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_product_scd2
WHERE valid_to < CURRENT_DATE - INTERVAL '5 years';
DELETE FROM dim_product_scd2
WHERE valid_to < CURRENT_DATE - INTERVAL '5 years';
```

### Q3: 角色扮演维度如何优化？

**A**: 优化策略：

1. **视图别名**: 使用视图创建别名
2. **索引优化**: 为每个角色创建索引
3. **查询优化**: 避免多次JOIN同一维度表

```sql
-- 创建视图别名
CREATE VIEW dim_order_date AS SELECT * FROM dim_date;
CREATE VIEW dim_ship_date AS SELECT * FROM dim_date;
CREATE VIEW dim_delivery_date AS SELECT * FROM dim_date;

-- 查询时使用视图
SELECT * FROM fact_orders f
JOIN dim_order_date d1 ON f.order_date_key = d1.date_key
JOIN dim_ship_date d2 ON f.ship_date_key = d2.date_key;
```

### Q4: 杂项维度什么时候使用？

**A**: 杂项维度适用场景：

- ✅ **低基数属性**: 属性值数量有限
- ✅ **属性组合**: 多个低基数属性组合
- ✅ **减少事实表列**: 减少事实表列数

**不适用场景**:

- ❌ **高基数属性**: 属性值很多
- ❌ **独立属性**: 属性独立使用，不需要组合

### Q5: 如何优化维度表查询性能？

**A**: 查询优化策略：

1. **当前版本索引**: 为is_current创建部分索引
2. **历史版本索引**: 为valid_from/valid_to创建范围索引
3. **物化视图**: 缓存常用维度查询结果

```sql
-- 当前版本物化视图
CREATE MATERIALIZED VIEW mv_dim_product_current AS
-- 查询SCD Type 2维度表（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM dim_product_scd2 WHERE is_current = TRUE;

CREATE UNIQUE INDEX ON mv_dim_product_current(product_id);

-- 定期刷新
REFRESH MATERIALIZED VIEW CONCURRENTLY mv_dim_product_current;
```

---

## 11. 相关资源 / Related Resources

### 11.1 核心相关文档 / Core Related Documents

- [维度建模基础](./维度建模基础.md) - 维度建模基础理论
- [事实表技术](./事实表技术.md) - 事实表设计技术
- [PostgreSQL列存实现](./PostgreSQL列存实现.md) - 列存表实现
- [索引策略](../08-PostgreSQL建模实践/索引策略.md) - 维度表索引设计
- [性能优化](../08-PostgreSQL建模实践/性能优化.md) - 维度表查询性能优化
- [约束设计](../08-PostgreSQL建模实践/约束设计.md) - 维度表约束设计

### 11.2 理论基础 / Theoretical Foundation

- [Kimball维度建模](../02-权威资源与标准/Kimball维度建模.md) - Kimball维度表设计理论
- [范式理论](../01-数据建模理论基础/范式理论.md) - 数据库范式理论（SCD设计）

### 11.3 实践指南 / Practical Guides

- [性能优化与监控](#9-性能优化与监控--performance-optimization-and-monitoring) - 本文档的性能监控章节
- [实际应用案例](#8-实际应用案例--practical-application-examples) - 本文档的应用案例章节
- [缓慢变化维度（SCD）](#2-缓慢变化维度scd) - 本文档的SCD章节

### 11.4 应用案例 / Application Cases

- [电商数据模型案例](../10-综合应用案例/电商数据模型案例.md) - 电商维度表设计案例
- [金融数据模型案例](../10-综合应用案例/金融数据模型案例.md) - 金融维度表设计案例

### 11.5 参考资源 / Reference Resources

- [权威资源索引](../00-导航与索引/权威资源索引.md) - 权威资源列表
- [术语对照表](../00-导航与索引/术语对照表.md) - 术语对照
- [快速查找指南](../00-导航与索引/快速查找指南.md) - 快速查找工具
- PostgreSQL官方文档: [Table Constraints](https://www.postgresql.org/docs/current/ddl-constraints.html)

- [Kimball维度建模](../02-权威资源与标准/Kimball维度建模.md) - 维度表技术来源
- [维度建模基础](./维度建模基础.md) - 维度建模基础
- [事实表技术](./事实表技术.md) - 事实表设计技术
- [索引策略](../08-PostgreSQL建模实践/索引策略.md) - 维度表索引策略
- [约束设计](../08-PostgreSQL建模实践/约束设计.md) - 维度表约束设计

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
