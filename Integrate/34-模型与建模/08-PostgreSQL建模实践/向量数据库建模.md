...
-- 创建月度分区
CREATE TABLE vector_data_2024_01 PARTITION OF vector_data
...
-- 创建类别分区
CREATE TABLE vector_data_text PARTITION OF vector_data
FOR VALUES IN ('text');

CREATE TABLE vector_data_image PARTITION OF vector_data
FOR VALUES IN ('image');
```
...
BEGIN
    ...
    -- 归一化
    RETURN (SELECT array_agg(unnest(v) / v_norm)::vector FROM generate_series(1, array_length(v, 1)));
END;
