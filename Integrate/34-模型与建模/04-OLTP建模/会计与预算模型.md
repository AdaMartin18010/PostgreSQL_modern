...
> **创建日期**: 2025年1月
> **来源**: Silverston《数据模型资源手册》卷1 Chapter 8 - Accounting and Budgeting + 实践总结
> **状态**: 基于权威资源深化扩展
> **文档编号**: 04-06
...
会计与预算模型是Silverston《数据模型资源手册》卷1（Volume 1 Chapter 8: Accounting and Budgeting）中的核心模型，用于管理企业的财务信息和预算控制。

**核心业务问题**:

- 企业的财务状况如何？与前期相比有何变化？
- 每个期间发生了哪些类型的交易？交易金额是多少？
- 每个期间发生了哪些费用？
- 折旧、资本化、摊销等交易对企业的影响如何？
- 设置了哪些预算？企业实际执行情况与预算的对比如何？

### 1.2 核心概念

**General Ledger Account（总账科目）**: 用于分类和汇总交易的财务报告"桶"

**Accounting Transaction（会计交易）**: 影响财务报表的交易

**Transaction Detail（交易明细）**: 复式记账中的借方和贷方分录

**Budget（预算）**: 监控资金支出的计划机制

**Budget Item（预算项）**: 预算的明细项目
...
CREATE TABLE organization_gl_account (
    ...
    organization_id INT NOT NULL, -- 引用内部组织（假设已有Party模型）
...
CREATE TABLE accounting_period (
    ...
    organization_id INT NOT NULL, -- 引用内部组织
...
CREATE TABLE accounting_transaction (
    ...
    organization_id INT, -- 引用内部组织（用于内部交易）
    from_party_id INT, -- 引用Party（用于外部交易）
    to_party_id INT, -- 引用Party（用于外部交易）
    transaction_date DATE NOT NULL,
    entry_date TIMESTAMPTZ DEFAULT NOW(),
    description TEXT,
    -- 业务规则：内部交易只需要organization_id，外部交易需要from_party和to_party
    CHECK (
...
BEGIN
    ...
    IF v_debit_total != v_credit_total THEN
        RAISE EXCEPTION 'Transaction不平衡: 借方总额 % 不等于贷方总额 %', v_debit_total, v_credit_total;
    END IF;
...
-- 辅助账关联（与Party、Product等）
ALTER TABLE organization_gl_account ADD COLUMN party_id INT; -- 用于应收账款、应付账款辅助账
ALTER TABLE organization_gl_account ADD COLUMN product_id INT REFERENCES product(product_id); -- 用于产品收入辅助账
ALTER TABLE organization_gl_account ADD COLUMN product_category_id INT; -- 用于产品分类收入辅助账
```
...
CREATE TABLE budget (
    ...
    organization_id INT NOT NULL, -- 引用内部组织（预算请求方）
...
CREATE TABLE budget_revision (
    ...
    budget_id INT NOT NULL REFERENCES budget(budget_id) ON DELETE CASCADE,
    revision_seq VARCHAR(50) NOT NULL, -- 如 "1.1", "1.2"
...
CREATE TABLE budget_revision_impact (
    ...
    revised_amount_change NUMERIC(15,2), -- 正数表示增加，负数表示减少
    revised_percent_change NUMERIC(5,2), -- 百分比变化
...
INSERT INTO budget_scenario (scenario_code, scenario_name, description) VALUES
    ('BEST_CASE', 'Best Case', '最佳情况'),
    ('WORST_CASE', 'Worst Case', '最坏情况'),
    ('BASELINE', 'Baseline', '基准情况'),
    ('EXCELLENT_MARKET', 'Excellent Market Conditions', '市场条件良好'),
    ('POOR_MARKET', 'Poor Market Conditions', '市场条件不佳');

-- 预算场景应用表（可以应用于整个预算或特定预算项）
...
```sql
-- 订单项预算分配表（订单承诺对预算的影响）
CREATE TABLE order_item_budget_allocation (
    allocation_id SERIAL PRIMARY KEY,
    order_item_id INT NOT NULL, -- 引用订单项（假设已有订单模型）
    budget_item_id INT NOT NULL REFERENCES budget_item(item_id),
    allocated_amount NUMERIC(15,2) NOT NULL CHECK (allocated_amount >= 0),
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(order_item_id, budget_item_id, from_date)
);

-- 需求预算分配表（需求对预算的分配）
CREATE TABLE requirement_budget_allocation (
    allocation_id SERIAL PRIMARY KEY,
    requirement_id INT NOT NULL, -- 引用需求（假设已有需求模型）
    budget_item_id INT NOT NULL REFERENCES budget_item(item_id),
    allocated_amount NUMERIC(15,2) NOT NULL CHECK (allocated_amount >= 0),
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(requirement_id, budget_item_id, from_date)
);

-- 付款预算分配表（付款对预算的影响）
CREATE TABLE payment_budget_allocation (
    allocation_id SERIAL PRIMARY KEY,
    payment_id INT NOT NULL, -- 引用付款（假设已有付款模型）
    budget_item_id INT NOT NULL REFERENCES budget_item(item_id),
    allocated_amount NUMERIC(15,2) NOT NULL CHECK (allocated_amount >= 0),
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(payment_id, budget_item_id, from_date)
);
```
...
CREATE TABLE gl_budget_xref (
    ...
    org_gl_account_id INT NOT NULL REFERENCES organization_gl_account(org_gl_account_id),
    budget_item_type_id INT NOT NULL REFERENCES budget_item_type(item_type_id),
    allocation_percentage NUMERIC(5,2) CHECK (allocation_percentage >= 0 AND allocation_percentage <= 100),
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(org_gl_account_id, budget_item_type_id, from_date)
);
```
...
INSERT INTO depreciation_method (method_code, method_name, formula) VALUES
    ('STRAIGHT_LINE', 'Straight Line Depreciation', '(Purchase cost - salvage cost) / estimated life in years'),
    ('DOUBLE_DECLINING', 'Double Declining Balance Depreciation', '(Purchase cost - salvage cost) * (1/estimated life in years) * 2'),
    ('SUM_OF_YEARS', 'Sum of Years Digits Depreciation', 'Complex formula based on remaining life');

-- 固定资产折旧方法关联表（多对多，支持时间有效性）
CREATE TABLE fixed_asset_depreciation_method (
    asset_depreciation_id SERIAL PRIMARY KEY,
    fixed_asset_id INT NOT NULL, -- 引用固定资产（假设已有固定资产表）
    depreciation_method_id INT NOT NULL REFERENCES depreciation_method(method_id),
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(fixed_asset_id, depreciation_method_id, from_date)
);

-- 折旧交易表（内部会计交易子类型）
CREATE TABLE depreciation_transaction (
    transaction_id INT PRIMARY KEY REFERENCES accounting_transaction(transaction_id) ON DELETE CASCADE,
    fixed_asset_id INT NOT NULL, -- 引用固定资产
    depreciation_method_id INT NOT NULL REFERENCES depreciation_method(method_id),
    depreciation_amount NUMERIC(15,2) NOT NULL CHECK (depreciation_amount >= 0),
    depreciation_period_start DATE NOT NULL,
    depreciation_period_end DATE NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    CHECK (depreciation_period_end >= depreciation_period_start)
);
```
...
**场景**: 企业完整的财务会计系统，包括总账、明细账、报表等。

**实现要点**:

- 使用general_ledger_account建立科目体系
- 使用accounting_transaction和transaction_detail实现复式记账
- 使用organization_gl_account_balance存储账户余额（可选）
- 使用accounting_period管理会计期间
...
- 使用budget和budget_item建立预算体系
- 使用budget_revision管理预算修订
- 使用budget_scenario支持多场景预算
- 使用order_item_budget_allocation关联订单和预算
...
**A**: 使用触发器在每次插入/更新/删除交易明细时验证：

```sql
-- 使用validate_transaction_balance()函数和触发器
```

### Q2: 如何处理预算修订的版本管理？

**A**: 使用budget_revision表管理预算修订历史，每个修订都有独立的revision_seq。

### Q3: 如何计算预算执行率？

**A**: 通过对比budget_item的金额和关联的实际支出（来自order_item_budget_allocation）计算。
...
- [工作努力模型](./工作努力模型.md) - 工作成本关联
- [订单管理模型](./订单管理模型.md) - 订单预算关联
- [产品管理模型](./产品管理模型.md) - 产品成本关联
- [Silverston数据模型资源手册](../02-权威资源与标准/Silverston数据模型资源手册.md) - 三卷本详解

### 12.2 归档资源

- [Volume 1完整内容](../../99-归档/参考资源/books/The Data Model Resource Book 1/full.md) - Chapter 8: Accounting and Budgeting

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**状态**: ✅ 文档完成，基于Volume 1 Chapter 8完整整合
