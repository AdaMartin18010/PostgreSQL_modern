...
> **创建日期**: 2025年1月
> **来源**: Silverston《数据模型资源手册》卷1 Chapter 6 - Work Efforts + 实践总结
> **状态**: 基于权威资源深化扩展
> **文档编号**: 04-05
...
- [工作努力模型完整指南](#工作努力模型完整指南)
  ...
  - [2. 工作需求与工作努力](#2-工作需求与工作努力)
    - [2.1 Work Requirement（工作需求）](#21-work-requirement工作需求)
    - [2.2 Work Effort（工作努力）](#22-work-effort工作努力)
    - [2.3 工作努力类型](#23-工作努力类型)
    - [2.4 工作努力层级结构](#24-工作努力层级结构)
  - [3. 工作努力关联](#3-工作努力关联)
    - [3.1 工作努力分解](#31-工作努力分解)
    - [3.2 工作努力依赖](#32-工作努力依赖)
    - [3.3 工作努力关联（递归模型）](#33-工作努力关联递归模型)
  - [4. 资源分配](#4-资源分配)
    - [4.1 人员分配（Party Assignment）](#41-人员分配party-assignment)
    - [4.2 库存分配（Inventory Assignment）](#42-库存分配inventory-assignment)
    - [4.3 固定资产分配（Fixed Asset Assignment）](#43-固定资产分配fixed-asset-assignment)
    - [4.4 技能标准](#44-技能标准)
  - [5. 时间跟踪](#5-时间跟踪)
    - [5.1 Time Entry（时间录入）](#51-time-entry时间录入)
    - [5.2 工作努力费率](#52-工作努力费率)
  - [6. 工作努力结果](#6-工作努力结果)
  - [7. PostgreSQL完整实现](#7-postgresql完整实现)
  - [8. 查询示例](#8-查询示例)
    - [查询1: 获取工作努力的完整层级结构](#查询1-获取工作努力的完整层级结构)
    - [查询2: 计算工作努力的成本](#查询2-计算工作努力的成本)
  - [9. 性能优化](#9-性能优化)
    - [9.1 索引策略](#91-索引策略)
    - [9.2 PostgreSQL 18优化](#92-postgresql-18优化)
  - [10. 实际应用案例](#10-实际应用案例)
    - [案例1: 项目管理](#案例1-项目管理)
    - [案例2: 生产管理](#案例2-生产管理)
  - [11. 常见问题解答 / FAQ](#11-常见问题解答--faq)
    - [Q1: 如何处理工作努力的时间冲突？](#q1-如何处理工作努力的时间冲突)
    - [Q2: 如何计算工作努力的完成百分比？](#q2-如何计算工作努力的完成百分比)
  - [12. 相关资源](#12-相关资源)
...
工作努力模型是Silverston《数据模型资源手册》卷1（Volume 1 Chapter 6: Work Efforts）中的核心模型，用于跟踪和管理企业内的各种工作活动，包括项目、任务、生产运行、维护等。
...
**Work Requirement（工作需求）**: 执行某种类型工作的需要

**Work Effort（工作努力）**: 工作需求的履行，包括设置、规划和实际执行的工作

**Work Effort Type（工作努力类型）**: Program、Project、Phase、Activity、Task等层级类型

**Work Effort Purpose（工作努力目的）**: Maintenance、Production Run、Work Flow、Research等

### 1.3 理论基础

**Work Requirement vs Work Effort**:

- **Work Requirement（工作需求）**: 表示执行某种类型工作的需要。可能是制造需求、维护需求、内部项目需求等。
- **Work Effort（工作努力）**: 工作需求的履行，包括设置、规划和实际执行的工作。
...
- **Program（项目群）**: 最高层级，包含多个项目
- **Project（项目）**: 独立的工作单元，包含多个阶段
- **Phase（阶段）**: 项目的特定阶段
- **Activity（活动）**: 阶段内的活动
- **Task（任务）**: 最细粒度的工作单元
...
- **人员分配**: 将人员分配到工作努力的不同角色（项目经理、团队成员等）
- **库存分配**: 为工作努力分配所需的原材料和库存
- **固定资产分配**: 分配设备、机器等固定资产

**时间跟踪理论**:

- **Time Entry（时间录入）**: 记录人员在特定时间段内对工作努力所花费的时间
- **Timesheet（时间表）**: 汇总一段时间内的时间录入
...
| Work Effort Associations | 3. 工作努力关联 | 100% |
| Work Effort Party Assignment | 4.1 人员分配 | 100% |
| Work Effort Time Tracking | 5. 时间跟踪 | 100% |
| Work Effort Results | 6. 工作努力结果 | 100% |

---

## 2. 工作需求与工作努力

### 2.1 Work Requirement（工作需求）

**核心实体设计**:

```sql
-- 工作需求表（Work Requirement是Requirement的子类型）
CREATE TABLE work_requirement (
    ...
    reason TEXT,
    -- 互斥关联：只能关联产品、交付物或固定资产之一
    product_id INT REFERENCES product(product_id),
    deliverable_id INT, -- 引用交付物（假设已有表）
    fixed_asset_id INT, -- 引用固定资产（假设已有表）
...
CREATE TABLE party_work_requirement_role (
    ...
    party_id INT NOT NULL, -- 引用Party（假设已有Party模型）
...
工作努力类型包括：Program、Project、Phase、Activity、Task、Job等，用于表示不同的工作层级。
...
CREATE TABLE work_effort_association (
    ...
    association_type_id INT, -- 关联类型（Breakdown, Dependency等）
    from_date DATE DEFAULT CURRENT_DATE,
    thru_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
工作努力可以分解为子工作努力，形成树状结构。

### 3.2 工作努力依赖

工作努力之间存在依赖关系：前置依赖（Precedency）和并发依赖（Concurrency）。

### 3.3 工作努力关联（递归模型）

使用递归关联模型支持灵活的工作努力分解和依赖关系。

---

## 4. 资源分配

### 4.1 人员分配（Party Assignment）
...
CREATE TABLE work_effort_inventory_assignment (
    assignment_id SERIAL PRIMARY KEY,
    work_effort_id INT NOT NULL REFERENCES work_effort(effort_id) ON DELETE CASCADE,
    inventory_item_id INT NOT NULL, -- 引用库存项（假设已有库存模型）
...
### 4.3 固定资产分配（Fixed Asset Assignment）
...
CREATE TABLE timesheet (
    ...
    worker_party_id INT NOT NULL, -- 引用工作人员（Party）
...
CREATE TABLE work_effort_deliverable_produced (
    ...
    work_effort_id INT NOT NULL REFERENCES work_effort(effort_id) ON DELETE CASCADE,
    deliverable_type_id INT, -- 引用交付物类型
    quantity_produced NUMERIC(10,2),
    produced_date DATE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE work_effort_inventory_produced (
    ...
    work_effort_id INT NOT NULL REFERENCES work_effort(effort_id) ON DELETE CASCADE,
    inventory_item_id INT NOT NULL, -- 引用库存项
...
WITH RECURSIVE effort_tree AS (
    ...
    WHERE we.effort_id != ALL(et.path) -- 防止循环
    AND et.level < 10
)
SELECT
    effort_id,
...
**场景**: 企业项目管理，跟踪项目、阶段、任务、活动。

**实现要点**:

- 使用work_effort表存储项目层级结构
- 使用work_effort_association表建立层级关系
- 使用time_entry跟踪时间
- 使用work_effort_party_assignment分配人员
...
- 使用work_requirement定义生产需求
- 使用work_effort跟踪生产运行
- 使用work_effort_inventory_assignment分配原材料
- 使用work_effort_fixed_asset_assignment分配设备
- 使用work_effort_inventory_produced记录产出
...
- [Party模型](./Party模型.md) - 人员与组织模型
- [产品管理模型](./产品管理模型.md) - 产品定义（用于生产需求）
- [订单管理模型](./订单管理模型.md) - 工作订单关联
- [Silverston数据模型资源手册](../02-权威资源与标准/Silverston数据模型资源手册.md) - 三卷本详解

### 12.2 归档资源

- [Volume 1完整内容](../../99-归档/参考资源/books/The Data Model Resource Book 1/full.md) - Chapter 6: Work Efforts

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**状态**: ✅ 文档完成，基于Volume 1 Chapter 6完整整合
