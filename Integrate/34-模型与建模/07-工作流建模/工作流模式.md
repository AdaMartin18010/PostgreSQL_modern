# 工作流模式

> **创建日期**: 2025年1月
> **来源**: 工作流模式研究
> **状态**: ✅ 已完成
> **文档编号**: 07-04

---

## 📑 目录

- [工作流模式](#工作流模式)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 工作流模式基本概念](#111-工作流模式基本概念)
    - [1.1.2 顺序模式理论](#112-顺序模式理论)
    - [1.1.3 并行模式理论](#113-并行模式理论)
    - [1.1.4 选择模式理论](#114-选择模式理论)
    - [1.1.5 循环模式理论](#115-循环模式理论)
    - [1.1.6 模式组合理论](#116-模式组合理论)
    - [1.1.7 复杂度分析](#117-复杂度分析)
  - [2. 顺序模式](#2-顺序模式)
    - [2.1 顺序模式定义](#21-顺序模式定义)
    - [2.2 PostgreSQL实现](#22-postgresql实现)
    - [2.3 顺序执行函数](#23-顺序执行函数)
  - [3. 并行模式](#3-并行模式)
    - [3.1 并行模式定义](#31-并行模式定义)
    - [3.2 PostgreSQL实现](#32-postgresql实现)
  - [4. 选择模式](#4-选择模式)
    - [4.1 选择模式定义](#41-选择模式定义)
    - [4.2 PostgreSQL实现](#42-postgresql实现)
  - [5. 循环模式](#5-循环模式)
    - [5.1 循环模式定义](#51-循环模式定义)
    - [5.2 PostgreSQL实现](#52-postgresql实现)
  - [6. PostgreSQL实现](#6-postgresql实现)
    - [6.1 综合工作流引擎](#61-综合工作流引擎)
  - [7. 更多实际案例 / More Practical Examples](#7-更多实际案例--more-practical-examples)
    - [7.1 案例1: 订单处理工作流](#71-案例1-订单处理工作流)
    - [7.2 案例2: 文档审批工作流](#72-案例2-文档审批工作流)
    - [7.3 案例3: 数据处理管道工作流](#73-案例3-数据处理管道工作流)
  - [8. 性能优化与监控 / Performance Optimization and Monitoring](#8-性能优化与监控--performance-optimization-and-monitoring)
    - [8.1 工作流性能优化](#81-工作流性能优化)
    - [8.2 工作流监控](#82-工作流监控)
  - [9. 常见问题解答 / FAQ](#9-常见问题解答--faq)
    - [Q1: 工作流应该使用顺序模式还是并行模式？](#q1-工作流应该使用顺序模式还是并行模式)
    - [Q2: 如何处理工作流失败和重试？](#q2-如何处理工作流失败和重试)
    - [Q3: 如何实现工作流的暂停和恢复？](#q3-如何实现工作流的暂停和恢复)
    - [Q4: 工作流性能如何优化？](#q4-工作流性能如何优化)
    - [Q5: 如何监控工作流的健康状态？](#q5-如何监控工作流的健康状态)
  - [9. 相关资源 / Related Resources](#9-相关资源--related-resources)
    - [9.1 核心相关文档 / Core Related Documents](#91-核心相关文档--core-related-documents)
    - [9.2 理论基础 / Theoretical Foundation](#92-理论基础--theoretical-foundation)
    - [9.3 实践指南 / Practical Guides](#93-实践指南--practical-guides)
    - [9.4 应用案例 / Application Cases](#94-应用案例--application-cases)
    - [9.5 参考资源 / Reference Resources](#95-参考资源--reference-resources)

---

## 1. 概述

工作流模式定义了常见的工作流控制结构，是构建复杂业务流程的基础。
这些模式是经过验证的、可重用的工作流设计解决方案。

**核心价值**:

- **标准化**：提供标准的工作流控制结构
- **可重用**：模式可以在不同场景中重用
- **可组合**：多个模式可以组合成复杂流程
- **可维护**：标准模式易于理解和维护

---

## 1.1 理论基础

### 1.1.1 工作流模式基本概念

**工作流模式（Workflow Patterns）**:

- **定义**: 常见的工作流控制结构
- **来源**: Workflow Patterns Initiative研究
- **目的**: 提供标准的工作流设计解决方案

**模式分类**:

- **控制流模式**: 控制任务执行顺序
- **数据流模式**: 控制数据流转
- **资源模式**: 控制资源分配
- **异常处理模式**: 处理异常情况

### 1.1.2 顺序模式理论

**顺序模式（Sequence Pattern）**:

- **定义**: 任务按固定顺序依次执行
- **数学表示**: $T_1 \rightarrow T_2 \rightarrow ... \rightarrow T_n$
- **执行条件**: 前一个任务完成后才能执行下一个任务

**顺序模式特点**:

- **严格顺序**: 任务必须按顺序执行
- **依赖关系**: 任务之间有依赖关系
- **串行执行**: 任务串行执行

### 1.1.3 并行模式理论

**并行模式（Parallel Pattern）**:

- **定义**: 多个任务并行执行
- **数学表示**: $T_1 || T_2 || ... || T_n$
- **同步点**: 所有任务完成后才能继续

**并行模式特点**:

- **并行执行**: 任务可以并行执行
- **同步**: 需要同步点
- **性能**: 提高执行性能

### 1.1.4 选择模式理论

**选择模式（Choice Pattern）**:

- **定义**: 根据条件选择执行分支
- **数学表示**: $if(C) then T_1 else T_2$
- **互斥性**: 分支互斥，只有一个分支执行

**选择模式特点**:

- **条件判断**: 根据条件选择分支
- **互斥性**: 分支互斥
- **灵活性**: 支持动态选择

### 1.1.5 循环模式理论

**循环模式（Loop Pattern）**:

- **定义**: 任务重复执行直到条件满足
- **数学表示**: $while(C) do T$
- **终止条件**: 循环终止条件

**循环模式特点**:

- **重复执行**: 任务重复执行
- **终止条件**: 需要终止条件
- **性能**: 注意避免无限循环

### 1.1.6 模式组合理论

**模式组合**:

- **顺序组合**: $P_1 \rightarrow P_2$
- **并行组合**: $P_1 || P_2$
- **嵌套组合**: $P_1(P_2)$

**组合原则**:

- **可组合性**: 模式可以组合
- **可重用性**: 模式可以重用
- **可维护性**: 组合模式易于维护

### 1.1.7 复杂度分析

**存储复杂度**:

- **工作流定义**: $O(P)$ where P is number of patterns
- **工作流实例**: $O(P \times I)$ where I is average instances per pattern

**执行复杂度**:

- **顺序模式**: $O(n)$ where n is number of tasks
- **并行模式**: $O(\max(t_i))$ where $t_i$ is task execution time
- **选择模式**: $O(1)$ (constant time)
- **循环模式**: $O(n \times m)$ where n is iterations, m is task time

---

## 2. 顺序模式

### 2.1 顺序模式定义

**顺序模式（Sequence Pattern）**：任务按固定顺序依次执行，前一个任务完成后才能执行下一个任务。

**特点**:

- 严格的执行顺序
- 前驱任务完成后才能开始后续任务
- 适用于有依赖关系的任务链

### 2.2 PostgreSQL实现

**顺序工作流表设计**:

```sql
-- 工作流定义表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS workflow_definition (
        workflow_id SERIAL PRIMARY KEY,
        workflow_name VARCHAR(200) NOT NULL,
        workflow_type VARCHAR(50) NOT NULL, -- 'sequence', 'parallel', 'choice', 'loop'
        definition JSONB NOT NULL,
        version INT DEFAULT 1,
        is_active BOOLEAN DEFAULT TRUE,
        created_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 workflow_definition 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 workflow_definition 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 workflow_definition 失败: %', SQLERRM;
END $$;

-- 工作流实例表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS workflow_instance (
        instance_id BIGSERIAL PRIMARY KEY,
        workflow_id INT NOT NULL REFERENCES workflow_definition(workflow_id),
        current_step INT DEFAULT 1,
        status VARCHAR(50) DEFAULT 'running', -- 'running', 'completed', 'failed', 'cancelled'
        context JSONB DEFAULT '{}',
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW(),
        completed_at TIMESTAMPTZ
    );
    RAISE NOTICE '表 workflow_instance 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 workflow_instance 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 workflow_instance 失败: %', SQLERRM;
END $$;

-- 工作流步骤表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS workflow_step (
        step_id SERIAL PRIMARY KEY,
        workflow_id INT NOT NULL REFERENCES workflow_definition(workflow_id),
        step_order INT NOT NULL,
        step_name VARCHAR(200) NOT NULL,
        step_type VARCHAR(50), -- 'task', 'sub_workflow'
        handler_function VARCHAR(200),
        is_required BOOLEAN DEFAULT TRUE,
        UNIQUE(workflow_id, step_order)
    );
    RAISE NOTICE '表 workflow_step 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 workflow_step 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 workflow_step 失败: %', SQLERRM;
END $$;

-- 工作流执行历史表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS workflow_execution_history (
        history_id BIGSERIAL PRIMARY KEY,
        instance_id BIGINT NOT NULL REFERENCES workflow_instance(instance_id),
        step_id INT NOT NULL REFERENCES workflow_step(step_id),
        status VARCHAR(50) NOT NULL, -- 'started', 'completed', 'failed'
        start_time TIMESTAMPTZ,
        end_time TIMESTAMPTZ,
        result JSONB,
        error_message TEXT
    );
    RAISE NOTICE '表 workflow_execution_history 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 workflow_execution_history 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 workflow_execution_history 失败: %', SQLERRM;
END $$;

-- 顺序工作流定义示例
INSERT INTO workflow_definition (workflow_name, workflow_type, definition) VALUES (
    '订单处理流程',
    'sequence',
    '{
        "steps": [
            {"order": 1, "name": "验证订单", "handler": "validate_order"},
            {"order": 2, "name": "处理支付", "handler": "process_payment"},
            {"order": 3, "name": "准备发货", "handler": "prepare_shipment"},
            {"order": 4, "name": "发送通知", "handler": "send_notification"}
        ]
    }'::JSONB
);
```

### 2.3 顺序执行函数

**顺序执行逻辑**:

```sql
-- 执行下一步
CREATE OR REPLACE FUNCTION execute_next_step(
    p_instance_id BIGINT
)
RETURNS BOOLEAN AS $$
DECLARE
    v_workflow_id INT;
    v_current_step INT;
    v_next_step RECORD;
    v_step_handler VARCHAR;
BEGIN
    -- 获取当前工作流信息
    SELECT workflow_id, current_step INTO v_workflow_id, v_current_step
    FROM workflow_instance
    WHERE instance_id = p_instance_id
      AND status = 'running';

    IF NOT FOUND THEN
        RETURN FALSE;
    END IF;

    -- 获取下一步
    SELECT * INTO v_next_step
    FROM workflow_step
    WHERE workflow_id = v_workflow_id
      AND step_order = v_current_step;

    IF NOT FOUND THEN
        -- 工作流完成
        UPDATE workflow_instance
        SET status = 'completed',
            completed_at = NOW()
        WHERE instance_id = p_instance_id;
        RETURN FALSE;
    END IF;

    -- 记录步骤开始
    INSERT INTO workflow_execution_history (
        instance_id, step_id, status, start_time
    ) VALUES (
        p_instance_id, v_next_step.step_id, 'started', NOW()
    );

    -- 执行步骤（这里调用handler函数）
    -- 实际实现中，handler_function会执行具体的业务逻辑
    -- 示例：EXECUTE format('SELECT %s($1)', v_next_step.handler_function) USING p_instance_id;

    -- 更新当前步骤
    UPDATE workflow_instance
    SET current_step = current_step + 1,
        updated_at = NOW()
    WHERE instance_id = p_instance_id;

    RETURN TRUE;
END;
$$ LANGUAGE plpgsql;
```

---

## 3. 并行模式

### 3.1 并行模式定义

**并行模式（Parallel Pattern）**：多个任务同时执行，所有任务完成后才能继续后续流程。

**特点**:

- 任务并行执行
- 需要等待所有并行任务完成
- 适用于独立的任务集合

### 3.2 PostgreSQL实现

**并行工作流定义**:

```sql
-- 并行工作流定义示例
INSERT INTO workflow_definition (workflow_name, workflow_type, definition) VALUES (
    '订单支付后处理',
    'parallel',
    '{
        "steps": [
            {"order": 1, "name": "发送邮件通知", "handler": "send_email", "parallel_group": 1},
            {"order": 1, "name": "发送短信通知", "handler": "send_sms", "parallel_group": 1},
            {"order": 1, "name": "更新库存", "handler": "update_inventory", "parallel_group": 1},
            {"order": 2, "name": "后续处理", "handler": "post_process"}
        ],
        "parallel_groups": [1]
    }'::JSONB
);

-- 并行步骤执行状态表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'parallel_step_status') THEN
        CREATE TABLE parallel_step_status (
            status_id BIGSERIAL PRIMARY KEY,
            instance_id BIGINT NOT NULL REFERENCES workflow_instance(instance_id),
            parallel_group INT NOT NULL,
            step_id INT NOT NULL REFERENCES workflow_step(step_id),
            status VARCHAR(50) DEFAULT 'pending', -- 'pending', 'running', 'completed', 'failed'
            start_time TIMESTAMPTZ,
            end_time TIMESTAMPTZ,
            result JSONB
        );
        RAISE NOTICE '表 parallel_step_status 创建成功';
    ELSE
        RAISE NOTICE '表 parallel_step_status 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '请先创建 workflow_instance 和 workflow_step 表';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 parallel_step_status 失败: %', SQLERRM;
END $$;

-- 检查并行组是否完成
CREATE OR REPLACE FUNCTION check_parallel_group_completed(
    p_instance_id BIGINT,
    p_parallel_group INT
)
RETURNS BOOLEAN AS $$
DECLARE
    v_total_count INT;
    v_completed_count INT;
BEGIN
    -- 统计并行组中的步骤总数
    SELECT COUNT(*) INTO v_total_count
    FROM workflow_step ws
    JOIN workflow_instance wi ON ws.workflow_id = wi.workflow_id
    WHERE wi.instance_id = p_instance_id
      AND (ws.definition->>'parallel_group')::INT = p_parallel_group;

    -- 统计已完成步骤数
    SELECT COUNT(*) INTO v_completed_count
    FROM parallel_step_status
    WHERE instance_id = p_instance_id
      AND parallel_group = p_parallel_group
      AND status = 'completed';

    RETURN v_completed_count = v_total_count;
END;
$$ LANGUAGE plpgsql;
```

---

## 4. 选择模式

### 4.1 选择模式定义

**选择模式（Choice Pattern）**：根据条件选择执行不同的分支，只有一个分支会被执行。

**特点**:

- 基于条件的分支选择
- 互斥执行
- 适用于条件判断场景

### 4.2 PostgreSQL实现

**选择工作流定义**:

```sql
-- 选择工作流定义示例
INSERT INTO workflow_definition (workflow_name, workflow_type, definition) VALUES (
    '订单处理分支',
    'choice',
    '{
        "condition": "order_amount > 1000",
        "branches": [
            {
                "condition": "order_amount > 1000",
                "steps": [
                    {"order": 1, "name": "VIP处理", "handler": "vip_process"}
                ]
            },
            {
                "condition": "order_amount <= 1000",
                "steps": [
                    {"order": 1, "name": "普通处理", "handler": "normal_process"}
                ]
            }
        ]
    }'::JSONB
);

-- 选择分支执行函数
CREATE OR REPLACE FUNCTION execute_choice_branch(
    p_instance_id BIGINT,
    p_condition JSONB
)
RETURNS INT AS $$
DECLARE
    v_workflow_id INT;
    v_context JSONB;
    v_branch RECORD;
    v_matched_branch INT;
BEGIN
    -- 获取工作流和上下文
    SELECT wi.workflow_id, wi.context INTO v_workflow_id, v_context
    FROM workflow_instance wi
    WHERE wi.instance_id = p_instance_id;

    -- 查找匹配的分支
    SELECT branch_id INTO v_matched_branch
    FROM jsonb_array_elements(
        (SELECT definition->'branches' FROM workflow_definition WHERE workflow_id = v_workflow_id)
    ) WITH ORDINALITY AS branches(branch, branch_id)
    WHERE evaluate_condition(branch->>'condition', v_context);

    IF v_matched_branch IS NULL THEN
        RAISE EXCEPTION 'No matching branch found';
    END IF;

    RETURN v_matched_branch;
END;
$$ LANGUAGE plpgsql;
```

---

## 5. 循环模式

### 5.1 循环模式定义

**循环模式（Loop Pattern）**：重复执行某些任务，直到满足退出条件。

**特点**:

- 重复执行
- 有退出条件
- 适用于需要重试或迭代的场景

### 5.2 PostgreSQL实现

**循环工作流定义**:

```sql
-- 循环工作流定义示例
INSERT INTO workflow_definition (workflow_name, workflow_type, definition) VALUES (
    '重试支付流程',
    'loop',
    '{
        "max_iterations": 3,
        "exit_condition": "payment_status = ''success''",
        "steps": [
            {"order": 1, "name": "尝试支付", "handler": "attempt_payment"},
            {"order": 2, "name": "检查结果", "handler": "check_result"}
        ]
    }'::JSONB
);

-- 循环执行函数
CREATE OR REPLACE FUNCTION execute_loop_workflow(
    p_instance_id BIGINT
)
RETURNS VOID AS $$
DECLARE
    v_workflow_def JSONB;
    v_max_iterations INT;
    v_current_iteration INT := 0;
    v_exit_condition TEXT;
    v_context JSONB;
    v_should_exit BOOLEAN := FALSE;
BEGIN
    -- 获取工作流定义
    SELECT wd.definition, wi.context INTO v_workflow_def, v_context
    FROM workflow_instance wi
    JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
    WHERE wi.instance_id = p_instance_id;

    v_max_iterations := (v_workflow_def->>'max_iterations')::INT;
    v_exit_condition := v_workflow_def->>'exit_condition';

    -- 循环执行
    WHILE v_current_iteration < v_max_iterations AND NOT v_should_exit LOOP
        v_current_iteration := v_current_iteration + 1;

        -- 执行循环体内的步骤
        PERFORM execute_workflow_steps(p_instance_id, v_workflow_def->'steps');

        -- 检查退出条件
        SELECT evaluate_condition(v_exit_condition, v_context) INTO v_should_exit;

        -- 更新上下文
        UPDATE workflow_instance
        SET context = context || jsonb_build_object('iteration', v_current_iteration),
            updated_at = NOW()
        WHERE instance_id = p_instance_id;
    END LOOP;

    -- 如果达到最大迭代次数仍未退出，标记为失败
    IF NOT v_should_exit THEN
        UPDATE workflow_instance
        SET status = 'failed',
            context = context || jsonb_build_object('error', 'Max iterations reached')
        WHERE instance_id = p_instance_id;
    END IF;
END;
$$ LANGUAGE plpgsql;
```

---

## 6. PostgreSQL实现

### 6.1 综合工作流引擎

**完整工作流系统**:

```sql
-- 工作流执行引擎主函数
CREATE OR REPLACE FUNCTION execute_workflow(
    p_instance_id BIGINT
)
RETURNS VOID AS $$
DECLARE
    v_workflow_type VARCHAR;
    v_workflow_def JSONB;
BEGIN
    -- 获取工作流类型和定义
    SELECT wd.workflow_type, wd.definition INTO v_workflow_type, v_workflow_def
    FROM workflow_instance wi
    JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
    WHERE wi.instance_id = p_instance_id
      AND wi.status = 'running';

    -- 根据类型执行不同的模式
    CASE v_workflow_type
        WHEN 'sequence' THEN
            PERFORM execute_sequence_workflow(p_instance_id);
        WHEN 'parallel' THEN
            PERFORM execute_parallel_workflow(p_instance_id);
        WHEN 'choice' THEN
            PERFORM execute_choice_workflow(p_instance_id);
        WHEN 'loop' THEN
            PERFORM execute_loop_workflow(p_instance_id);
        ELSE
            RAISE EXCEPTION 'Unknown workflow type: %', v_workflow_type;
    END CASE;
END;
$$ LANGUAGE plpgsql;

-- 工作流状态查询视图
CREATE VIEW workflow_status_view AS
SELECT
    wi.instance_id,
    wd.workflow_name,
    wi.status,
    wi.current_step,
    wi.created_at,
    wi.updated_at,
    wi.completed_at,
    -- 执行进度
    (
        SELECT COUNT(*)
        FROM workflow_execution_history weh
        WHERE weh.instance_id = wi.instance_id
          AND weh.status = 'completed'
    ) AS completed_steps,
    (
        SELECT COUNT(*)
        FROM workflow_step ws
        WHERE ws.workflow_id = wi.workflow_id
    ) AS total_steps
FROM workflow_instance wi
JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id;
```

---

## 7. 更多实际案例 / More Practical Examples

### 7.1 案例1: 订单处理工作流

**订单处理完整工作流**:

```sql
-- 订单处理工作流定义
INSERT INTO workflow_definition (workflow_name, workflow_type, description) VALUES
    ('order_processing', 'sequence', '订单处理工作流');

-- 工作流步骤
INSERT INTO workflow_step (workflow_id, step_name, step_type, step_order, handler_function) VALUES
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'order_processing'),
     'validate_order', 'task', 1, 'validate_order_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'order_processing'),
     'process_payment', 'task', 2, 'process_payment_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'order_processing'),
     'prepare_shipment', 'task', 3, 'prepare_shipment_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'order_processing'),
     'send_notification', 'task', 4, 'send_notification_function');

-- 订单处理函数
CREATE OR REPLACE FUNCTION validate_order_function(p_instance_id BIGINT)
RETURNS VOID AS $$
DECLARE
    v_order_id BIGINT;
BEGIN
    SELECT context->>'order_id' INTO v_order_id
    FROM workflow_instance WHERE instance_id = p_instance_id;

    -- 验证订单
    IF NOT EXISTS (SELECT 1 FROM orders WHERE order_id = v_order_id) THEN
        RAISE EXCEPTION 'Order % not found', v_order_id;
    END IF;

    -- 更新步骤状态
    UPDATE workflow_execution_history
    SET status = 'completed', completed_at = NOW()
    WHERE instance_id = p_instance_id AND step_name = 'validate_order';
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION process_payment_function(p_instance_id BIGINT)
RETURNS VOID AS $$
DECLARE
    v_order_id BIGINT;
    v_payment_result VARCHAR;
BEGIN
    SELECT context->>'order_id' INTO v_order_id
    FROM workflow_instance WHERE instance_id = p_instance_id;

    -- 处理支付
    SELECT change_order_state(v_order_id, 'pay', '{}'::jsonb) INTO v_payment_result;

    -- 更新步骤状态
    UPDATE workflow_execution_history
    SET status = 'completed', completed_at = NOW()
    WHERE instance_id = p_instance_id AND step_name = 'process_payment';
END;
$$ LANGUAGE plpgsql;
```

### 7.2 案例2: 文档审批工作流

**多级并行审批工作流**:

```sql
-- 文档审批工作流（并行模式）
INSERT INTO workflow_definition (workflow_name, workflow_type, description) VALUES
    ('document_approval', 'parallel', '文档审批工作流');

-- 并行审批步骤
INSERT INTO workflow_step (workflow_id, step_name, step_type, step_order, handler_function) VALUES
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'document_approval'),
     'legal_review', 'task', 1, 'legal_review_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'document_approval'),
     'finance_review', 'task', 1, 'finance_review_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'document_approval'),
     'technical_review', 'task', 1, 'technical_review_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'document_approval'),
     'final_approval', 'task', 2, 'final_approval_function');

-- 并行执行函数
CREATE OR REPLACE FUNCTION execute_parallel_approval(p_instance_id BIGINT)
RETURNS VOID AS $$
DECLARE
    v_step RECORD;
    v_all_completed BOOLEAN;
BEGIN
    -- 启动所有并行步骤
    FOR v_step IN
        SELECT step_name FROM workflow_step
        WHERE workflow_id = (
            SELECT workflow_id FROM workflow_instance WHERE instance_id = p_instance_id
        )
        AND step_order = 1
    LOOP
        INSERT INTO workflow_execution_history (instance_id, step_name, status)
        VALUES (p_instance_id, v_step.step_name, 'running');
    END LOOP;

    -- 等待所有步骤完成
    LOOP
        SELECT bool_and(status = 'completed') INTO v_all_completed
        FROM workflow_execution_history
        WHERE instance_id = p_instance_id AND step_order = 1;

        EXIT WHEN v_all_completed;

        PERFORM pg_sleep(1);  -- 等待1秒
    END LOOP;

    -- 执行最终审批
    PERFORM execute_workflow_step(p_instance_id, 'final_approval');
END;
$$ LANGUAGE plpgsql;
```

### 7.3 案例3: 数据处理管道工作流

**ETL数据处理工作流**:

```sql
-- 数据处理管道工作流
INSERT INTO workflow_definition (workflow_name, workflow_type, description) VALUES
    ('data_processing_pipeline', 'sequence', '数据处理管道');

-- 数据处理步骤
INSERT INTO workflow_step (workflow_id, step_name, step_type, step_order, handler_function) VALUES
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'data_processing_pipeline'),
     'extract_data', 'task', 1, 'extract_data_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'data_processing_pipeline'),
     'transform_data', 'task', 2, 'transform_data_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'data_processing_pipeline'),
     'load_data', 'task', 3, 'load_data_function'),
    ((SELECT workflow_id FROM workflow_definition WHERE workflow_name = 'data_processing_pipeline'),
     'validate_result', 'task', 4, 'validate_result_function');

-- ETL函数
CREATE OR REPLACE FUNCTION extract_data_function(p_instance_id BIGINT)
RETURNS VOID AS $$
DECLARE
    v_source_table TEXT;
    v_target_table TEXT;
BEGIN
    SELECT context->>'source_table', context->>'target_table'
    INTO v_source_table, v_target_table
    FROM workflow_instance WHERE instance_id = p_instance_id;

    -- 提取数据到临时表
    EXECUTE format('CREATE TEMP TABLE %I AS SELECT * FROM %I', v_target_table, v_source_table);

    UPDATE workflow_execution_history
    SET status = 'completed', completed_at = NOW()
    WHERE instance_id = p_instance_id AND step_name = 'extract_data';
END;
$$ LANGUAGE plpgsql;
```

---

## 8. 性能优化与监控 / Performance Optimization and Monitoring

### 8.1 工作流性能优化

**索引优化**:

```sql
-- 工作流实例索引
CREATE INDEX idx_workflow_instance_status
ON workflow_instance(status, created_at);

CREATE INDEX idx_workflow_instance_workflow
ON workflow_instance(workflow_id, status);

-- 执行历史索引
CREATE INDEX idx_execution_history_instance
ON workflow_execution_history(instance_id, step_order);

CREATE INDEX idx_execution_history_status
ON workflow_execution_history(status, completed_at)
WHERE status IN ('running', 'failed');
```

**批量处理优化**:

```sql
-- 批量启动工作流
CREATE OR REPLACE FUNCTION batch_start_workflow(
    p_workflow_name VARCHAR,
    p_contexts JSONB[]
)
RETURNS TABLE(instance_id BIGINT) AS $$
DECLARE
    v_context JSONB;
    v_instance_id BIGINT;
BEGIN
    FOREACH v_context IN ARRAY p_contexts
    LOOP
        INSERT INTO workflow_instance (workflow_id, status, context)
        SELECT workflow_id, 'running', v_context
        FROM workflow_definition
        WHERE workflow_name = p_workflow_name
        RETURNING instance_id INTO v_instance_id;

        RETURN QUERY SELECT v_instance_id;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

### 8.2 工作流监控

**工作流监控查询**:

```sql
-- 监控：工作流执行统计
SELECT
    wd.workflow_name,
    wi.status,
    COUNT(*) AS instance_count,
    AVG(EXTRACT(EPOCH FROM (COALESCE(wi.completed_at, NOW()) - wi.created_at))) AS avg_duration_seconds,
    MAX(EXTRACT(EPOCH FROM (COALESCE(wi.completed_at, NOW()) - wi.created_at))) AS max_duration_seconds
FROM workflow_instance wi
JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
GROUP BY wd.workflow_name, wi.status
ORDER BY instance_count DESC;

-- 监控：长时间运行的工作流
SELECT
    wi.instance_id,
    wd.workflow_name,
    wi.status,
    wi.current_step,
    NOW() - wi.created_at AS running_duration,
    wi.context
FROM workflow_instance wi
JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
WHERE wi.status = 'running'
  AND wi.created_at < NOW() - INTERVAL '1 hour'
ORDER BY running_duration DESC;

-- 监控：失败的步骤
SELECT
    weh.instance_id,
    weh.step_name,
    weh.status,
    weh.error_message,
    weh.created_at,
    wd.workflow_name
FROM workflow_execution_history weh
JOIN workflow_instance wi ON weh.instance_id = wi.instance_id
JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
WHERE weh.status = 'failed'
  AND weh.created_at >= NOW() - INTERVAL '24 hours'
ORDER BY weh.created_at DESC;
```

---

## 9. 常见问题解答 / FAQ

### Q1: 工作流应该使用顺序模式还是并行模式？

**A**: 选择原则：

- **顺序模式**: 步骤之间有依赖关系，必须按顺序执行
- **并行模式**: 步骤之间无依赖，可以同时执行以提高效率

**示例**:

```sql
-- ✅ 顺序模式：订单处理（支付必须在验证之后）
validate_order → process_payment → prepare_shipment

-- ✅ 并行模式：文档审批（多个部门可以同时审批）
legal_review || finance_review || technical_review → final_approval
```

### Q2: 如何处理工作流失败和重试？

**A**: 失败处理策略：

```sql
-- 添加重试配置到工作流步骤
ALTER TABLE workflow_step
ADD COLUMN max_retries INT DEFAULT 3,
ADD COLUMN retry_delay INTERVAL DEFAULT '5 minutes';

-- 重试函数
CREATE OR REPLACE FUNCTION retry_failed_step(
    p_instance_id BIGINT,
    p_step_name VARCHAR
)
RETURNS VOID AS $$
DECLARE
    v_retry_count INT;
    v_max_retries INT;
BEGIN
    SELECT retry_count, max_retries
    INTO v_retry_count, v_max_retries
    FROM workflow_execution_history
    WHERE instance_id = p_instance_id AND step_name = p_step_name
    ORDER BY created_at DESC LIMIT 1;

    IF v_retry_count >= v_max_retries THEN
        RAISE EXCEPTION 'Max retries exceeded for step %', p_step_name;
    END IF;

    -- 重置步骤状态并重试
    UPDATE workflow_execution_history
    SET status = 'running', retry_count = retry_count + 1
    WHERE instance_id = p_instance_id AND step_name = p_step_name;
END;
$$ LANGUAGE plpgsql;
```

### Q3: 如何实现工作流的暂停和恢复？

**A**: 暂停/恢复实现：

```sql
-- 暂停工作流
CREATE OR REPLACE FUNCTION pause_workflow(p_instance_id BIGINT)
RETURNS VOID AS $$
BEGIN
    UPDATE workflow_instance
    SET status = 'paused', paused_at = NOW()
    WHERE instance_id = p_instance_id AND status = 'running';
END;
$$ LANGUAGE plpgsql;

-- 恢复工作流
CREATE OR REPLACE FUNCTION resume_workflow(p_instance_id BIGINT)
RETURNS VOID AS $$
BEGIN
    UPDATE workflow_instance
    SET status = 'running', paused_at = NULL
    WHERE instance_id = p_instance_id AND status = 'paused';
END;
$$ LANGUAGE plpgsql;
```

### Q4: 工作流性能如何优化？

**A**: 性能优化建议：

1. **异步执行**: 使用消息队列异步执行工作流步骤
2. **批量处理**: 批量启动和处理工作流实例
3. **索引优化**: 为频繁查询的列创建索引
4. **分区**: 对历史表进行分区

```sql
-- 异步执行工作流步骤（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'workflow_queue') THEN
        CREATE TABLE workflow_queue (
            queue_id BIGSERIAL PRIMARY KEY,
            instance_id BIGINT NOT NULL,
            step_name VARCHAR(100) NOT NULL,
            priority INT DEFAULT 0,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 workflow_queue 创建成功';
    ELSE
        RAISE NOTICE '表 workflow_queue 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 workflow_queue 失败: %', SQLERRM;
END $$;

CREATE INDEX idx_workflow_queue_priority
ON workflow_queue(priority DESC, created_at);
```

### Q5: 如何监控工作流的健康状态？

**A**: 健康监控：

```sql
-- 工作流健康检查视图
CREATE VIEW workflow_health_check AS
SELECT
    wd.workflow_name,
    COUNT(*) FILTER (WHERE wi.status = 'running') AS running_count,
    COUNT(*) FILTER (WHERE wi.status = 'completed') AS completed_count,
    COUNT(*) FILTER (WHERE wi.status = 'failed') AS failed_count,
    AVG(EXTRACT(EPOCH FROM (COALESCE(wi.completed_at, NOW()) - wi.created_at)))
        FILTER (WHERE wi.status = 'completed') AS avg_completion_time,
    COUNT(*) FILTER (
        WHERE wi.status = 'running'
        AND wi.created_at < NOW() - INTERVAL '1 hour'
    ) AS stuck_count
FROM workflow_instance wi
JOIN workflow_definition wd ON wi.workflow_id = wd.workflow_id
GROUP BY wd.workflow_name;
```

---

## 9. 相关资源 / Related Resources

### 9.1 核心相关文档 / Core Related Documents

- [状态机建模](./状态机建模.md) - 状态机建模基础
- [JSONB状态机实现](./JSONB状态机实现.md) - JSONB实现状态机
- [BPMN建模](./BPMN建模.md) - BPMN标准建模
- [订单管理模型](../04-OLTP建模/订单管理模型.md) - 订单工作流应用案例

### 9.2 理论基础 / Theoretical Foundation

- [约束理论](../01-数据建模理论基础/约束理论.md) - 工作流约束理论

### 9.3 实践指南 / Practical Guides

- [性能优化与监控](#8-性能优化与监控--performance-optimization-and-monitoring) - 本文档的性能监控章节
- [更多实际案例](#7-更多实际案例--more-practical-examples) - 本文档的应用案例章节
- [性能优化](../08-PostgreSQL建模实践/性能优化.md) - 工作流性能优化

### 9.4 应用案例 / Application Cases

- [电商数据模型案例](../10-综合应用案例/电商数据模型案例.md) - 电商工作流案例
- [金融数据模型案例](../10-综合应用案例/金融数据模型案例.md) - 金融工作流案例

### 9.5 参考资源 / Reference Resources

- [权威资源索引](../00-导航与索引/权威资源索引.md) - 权威资源列表
- [术语对照表](../00-导航与索引/术语对照表.md) - 术语对照
- [快速查找指南](../00-导航与索引/快速查找指南.md) - 快速查找工具
- 工作流模式研究: [Workflow Patterns](https://www.workflowpatterns.com/)

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
