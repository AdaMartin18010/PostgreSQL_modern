# 状态机建模

> **创建日期**: 2025年1月
> **来源**: 状态机理论
> **状态**: ✅ 已完成
> **文档编号**: 07-02

---

## 📑 目录

- [状态机建模](#状态机建模)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 有限状态机理论](#111-有限状态机理论)
    - [1.1.2 状态转换理论](#112-状态转换理论)
    - [1.1.3 状态机类型理论](#113-状态机类型理论)
    - [1.1.4 状态验证理论](#114-状态验证理论)
    - [1.1.5 状态历史理论](#115-状态历史理论)
    - [1.1.6 复杂度分析](#116-复杂度分析)
  - [2. 状态机基础](#2-状态机基础)
    - [2.1 状态机要素](#21-状态机要素)
    - [2.2 状态机类型](#22-状态机类型)
    - [2.3 状态机表示](#23-状态机表示)
  - [3. 状态转换设计](#3-状态转换设计)
    - [3.1 状态定义](#31-状态定义)
    - [3.2 转换规则定义](#32-转换规则定义)
    - [3.3 状态转换历史](#33-状态转换历史)
  - [4. 状态验证](#4-状态验证)
    - [4.1 状态转换验证函数](#41-状态转换验证函数)
    - [4.2 状态转换执行函数](#42-状态转换执行函数)
  - [5. PostgreSQL实现](#5-postgresql实现)
    - [5.1 完整状态机系统](#51-完整状态机系统)
    - [5.2 状态机触发器](#52-状态机触发器)
    - [5.3 状态查询视图](#53-状态查询视图)
  - [6. 更多实际案例 / More Practical Examples](#6-更多实际案例--more-practical-examples)
    - [6.1 案例1: 订单状态机](#61-案例1-订单状态机)
    - [6.2 案例2: 审批流程状态机](#62-案例2-审批流程状态机)
    - [6.3 案例3: 支付状态机](#63-案例3-支付状态机)
  - [7. 性能优化与监控 / Performance Optimization and Monitoring](#7-性能优化与监控--performance-optimization-and-monitoring)
    - [7.1 状态机性能优化](#71-状态机性能优化)
    - [7.2 状态机监控](#72-状态机监控)
  - [8. 常见问题解答 / FAQ](#8-常见问题解答--faq)
    - [Q1: 状态机应该有多少个状态？](#q1-状态机应该有多少个状态)
    - [Q2: 如何处理并发状态转换？](#q2-如何处理并发状态转换)
    - [Q3: 状态转换历史表会很大，如何优化？](#q3-状态转换历史表会很大如何优化)
    - [Q4: 如何实现状态机的回滚？](#q4-如何实现状态机的回滚)
    - [Q5: 状态机性能如何优化？](#q5-状态机性能如何优化)
  - [8. 相关资源 / Related Resources](#8-相关资源--related-resources)
    - [8.1 核心相关文档 / Core Related Documents](#81-核心相关文档--core-related-documents)
    - [8.2 理论基础 / Theoretical Foundation](#82-理论基础--theoretical-foundation)
    - [8.3 实践指南 / Practical Guides](#83-实践指南--practical-guides)
    - [8.4 应用案例 / Application Cases](#84-应用案例--application-cases)
    - [8.5 参考资源 / Reference Resources](#85-参考资源--reference-resources)

---

## 1. 概述

状态机建模用于描述系统或对象的状态变化过程，是工作流建模的基础。
有限状态机（Finite State Machine, FSM）是一种计算模型，通过状态、事件和转换来描述系统行为。

**核心价值**:

- **行为建模**：清晰描述系统状态变化
- **规则验证**：确保状态转换符合业务规则
- **流程控制**：实现复杂业务流程的状态管理
- **错误处理**：定义异常状态和恢复机制

---

## 1.1 理论基础

### 1.1.1 有限状态机理论

**有限状态机（Finite State Machine, FSM）**:

- **定义**: $M = (Q, \Sigma, \delta, q_0, F)$
  - $Q$: 状态集合
  - $\Sigma$: 输入字母表（事件集合）
  - $\delta$: 转换函数 $Q \times \Sigma \rightarrow Q$
  - $q_0$: 初始状态
  - $F$: 终止状态集合

**状态机要素**:

- **状态（State）**: 系统在某一时刻的条件
- **事件（Event）**: 引起状态转换的触发器
- **转换（Transition）**: 状态之间的变化路径

### 1.1.2 状态转换理论

**状态转换**:

- **转换函数**: $\delta(q, e) = q'$ where $q, q' \in Q, e \in \Sigma$
- **转换条件**: 转换可以附加条件
- **转换动作**: 转换时可以执行动作

**状态转换图**:

- **有向图**: $G = (V, E)$ where V is states, E is transitions
- **路径**: 状态序列 $q_0 \rightarrow q_1 \rightarrow ... \rightarrow q_n$
- **可达性**: 状态$q_j$从状态$q_i$可达

### 1.1.3 状态机类型理论

**状态机分类**:

- **有限状态机（FSM）**: 状态数量有限
- **层次状态机（HSM）**: 支持状态的层次结构
- **并发状态机**: 多个状态机并行运行
- **状态图（Statechart）**: 扩展的状态机，支持复合状态

**层次状态机**:

- **父状态**: 包含子状态的复合状态
- **子状态**: 父状态内的状态
- **继承**: 子状态继承父状态的行为

### 1.1.4 状态验证理论

**状态转换验证**:

- **前置条件**: 转换前必须满足的条件
- **后置条件**: 转换后必须满足的条件
- **不变式**: 状态转换过程中保持不变的条件

**验证规则**:

- **合法性检查**: 检查转换是否合法
- **一致性检查**: 检查状态是否一致
- **完整性检查**: 检查状态是否完整

### 1.1.5 状态历史理论

**状态历史追踪**:

- **历史记录**: 记录所有状态转换
- **时间戳**: 记录状态转换的时间
- **事件记录**: 记录触发转换的事件

**历史查询**:

- **时间点查询**: 查询特定时间点的状态
- **变更历史**: 查询状态变更历史
- **审计追踪**: 记录操作审计信息

### 1.1.6 复杂度分析

**存储复杂度**:

- **状态表**: $O(S)$ where S is number of states
- **转换表**: $O(S \times E)$ where E is number of events
- **历史表**: $O(N \times H)$ where N is entities, H is average history per entity

**执行复杂度**:

- **状态转换**: $O(1)$ (constant time)
- **状态验证**: $O(V)$ where V is number of validation rules
- **历史查询**: $O(\log H)$ with index

---

## 2. 状态机基础

### 2.1 状态机要素

**状态机三要素**:

| 要素 | 说明 | 示例 |
|------|------|------|
| 状态（State） | 系统在某一时刻的条件 | 待处理、处理中、已完成 |
| 事件（Event） | 引起状态转换的触发器 | 开始处理、完成处理、取消 |
| 转换（Transition） | 状态之间的变化路径 | 待处理 → 处理中 |

### 2.2 状态机类型

**状态机分类**:

- **有限状态机（FSM）**：状态数量有限
- **层次状态机（HSM）**：支持状态的层次结构
- **并发状态机**：多个状态机并行运行
- **状态图（Statechart）**：扩展的状态机，支持复合状态

### 2.3 状态机表示

**状态转换图示例**:

```text
         [创建订单]
             |
             v
        [待处理] --[开始处理]--> [处理中]
             |                      |
             |                      v
             |                  [已完成]
             |
             v
        [已取消]
```

---

## 3. 状态转换设计

### 3.1 状态定义

**状态表设计**:

```sql
-- 状态定义表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS state_definition (
        state_id SERIAL PRIMARY KEY,
        state_machine_name VARCHAR(100) NOT NULL,
        state_name VARCHAR(100) NOT NULL,
        state_type VARCHAR(50), -- 'initial', 'normal', 'final', 'error'
        description TEXT,
        is_active BOOLEAN DEFAULT TRUE,
        UNIQUE(state_machine_name, state_name)
    );
    RAISE NOTICE '表 state_definition 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 state_definition 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 state_definition 失败: %', SQLERRM;
END $$;

-- 示例：订单状态机（带错误处理）
DO $$
BEGIN
    INSERT INTO state_definition (state_machine_name, state_name, state_type, description) VALUES
        ('order', 'pending', 'initial', '订单待处理'),
        ('order', 'processing', 'normal', '订单处理中'),
        ('order', 'completed', 'final', '订单已完成'),
        ('order', 'cancelled', 'final', '订单已取消'),
        ('order', 'error', 'error', '订单处理错误')
    ON CONFLICT DO NOTHING;
    RAISE NOTICE '状态定义数据插入成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '插入状态定义数据失败: %', SQLERRM;
END $$;
```

### 3.2 转换规则定义

**状态转换规则表**:

```sql
-- 状态转换规则表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS state_transition_rule (
        rule_id SERIAL PRIMARY KEY,
        state_machine_name VARCHAR(100) NOT NULL,
        from_state VARCHAR(100) NOT NULL,
        to_state VARCHAR(100) NOT NULL,
        event_name VARCHAR(100) NOT NULL,
        -- 转换条件（JSONB存储复杂条件）
        condition JSONB,
        -- 转换动作
        action TEXT,
        -- 权限检查
        required_role VARCHAR(100),
        is_enabled BOOLEAN DEFAULT TRUE,
        UNIQUE(state_machine_name, from_state, event_name)
    );
    RAISE NOTICE '表 state_transition_rule 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 state_transition_rule 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 state_transition_rule 失败: %', SQLERRM;
END $$;

-- 示例：订单状态转换规则（带错误处理）
DO $$
BEGIN
    INSERT INTO state_transition_rule (
        state_machine_name, from_state, to_state, event_name, condition, required_role
    ) VALUES
        ('order', 'pending', 'processing', 'start_processing', NULL, 'operator'),
        ('order', 'processing', 'completed', 'complete', '{"all_items_shipped": true}'::jsonb, 'operator'),
        ('order', 'pending', 'cancelled', 'cancel', NULL, 'customer'),
        ('order', 'processing', 'cancelled', 'cancel', '{"allow_cancel": true}'::jsonb, 'customer'),
        ('order', 'processing', 'error', 'error_occurred', NULL, 'system')
    ON CONFLICT DO NOTHING;
    RAISE NOTICE '状态转换规则数据插入成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '插入状态转换规则数据失败: %', SQLERRM;
END $$;
```

### 3.3 状态转换历史

**状态转换历史表**:

```sql
-- 状态转换历史表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS state_transition_history (
        history_id BIGSERIAL PRIMARY KEY,
        entity_type VARCHAR(100) NOT NULL, -- 'order', 'task', etc.
        entity_id BIGINT NOT NULL,
        from_state VARCHAR(100),
        to_state VARCHAR(100) NOT NULL,
        event_name VARCHAR(100) NOT NULL,
        -- 转换上下文
        context JSONB,
        -- 操作信息
        operator_id VARCHAR(100),
        operator_name VARCHAR(200),
        -- 时间戳
        transition_time TIMESTAMPTZ DEFAULT NOW(),
        -- 备注
        notes TEXT
    );
    RAISE NOTICE '表 state_transition_history 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 state_transition_history 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 state_transition_history 失败: %', SQLERRM;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    CREATE INDEX IF NOT EXISTS idx_transition_history_entity ON state_transition_history(entity_type, entity_id, transition_time DESC);
    CREATE INDEX IF NOT EXISTS idx_transition_history_state ON state_transition_history(to_state, transition_time DESC);
    RAISE NOTICE '索引创建成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建索引失败: %', SQLERRM;
END $$;
```

---

## 4. 状态验证

### 4.1 状态转换验证函数

**验证函数**:

```sql
-- 验证状态转换是否合法
CREATE OR REPLACE FUNCTION validate_state_transition(
    p_state_machine_name VARCHAR,
    p_from_state VARCHAR,
    p_to_state VARCHAR,
    p_event_name VARCHAR,
    p_context JSONB DEFAULT NULL
)
RETURNS BOOLEAN AS $$
DECLARE
    v_rule_exists BOOLEAN;
    v_condition_met BOOLEAN := TRUE;
BEGIN
    -- 检查转换规则是否存在
    SELECT EXISTS(
        SELECT 1 FROM state_transition_rule
        WHERE state_machine_name = p_state_machine_name
          AND from_state = p_from_state
          AND to_state = p_to_state
          AND event_name = p_event_name
          AND is_enabled = TRUE
    ) INTO v_rule_exists;

    IF NOT v_rule_exists THEN
        RETURN FALSE;
    END IF;

    -- 检查转换条件（如果有）
    IF p_context IS NOT NULL THEN
        SELECT EXISTS(
            SELECT 1 FROM state_transition_rule
            WHERE state_machine_name = p_state_machine_name
              AND from_state = p_from_state
              AND to_state = p_to_state
              AND event_name = p_event_name
              AND (condition IS NULL OR condition <@ p_context)
        ) INTO v_condition_met;
    END IF;

    RETURN v_rule_exists AND v_condition_met;
END;
$$ LANGUAGE plpgsql;
```

### 4.2 状态转换执行函数

**执行状态转换**:

```sql
-- 执行状态转换
CREATE OR REPLACE FUNCTION execute_state_transition(
    p_entity_type VARCHAR,
    p_entity_id BIGINT,
    p_state_machine_name VARCHAR,
    p_event_name VARCHAR,
    p_context JSONB DEFAULT NULL,
    p_operator_id VARCHAR DEFAULT NULL,
    p_operator_name VARCHAR DEFAULT NULL,
    p_notes TEXT DEFAULT NULL
)
RETURNS VARCHAR AS $$
DECLARE
    v_current_state VARCHAR;
    v_new_state VARCHAR;
    v_transition_valid BOOLEAN;
BEGIN
    -- 获取当前状态（需要根据实体类型查询对应的状态表）
    -- 这里假设有一个通用的状态表
    SELECT current_state INTO v_current_state
    FROM entity_state
    WHERE entity_type = p_entity_type
      AND entity_id = p_entity_id;

    IF v_current_state IS NULL THEN
        RAISE EXCEPTION 'Entity %:% not found', p_entity_type, p_entity_id;
    END IF;

    -- 查找目标状态
    SELECT to_state INTO v_new_state
    FROM state_transition_rule
    WHERE state_machine_name = p_state_machine_name
      AND from_state = v_current_state
      AND event_name = p_event_name
      AND is_enabled = TRUE
    LIMIT 1;

    IF v_new_state IS NULL THEN
        RAISE EXCEPTION 'Invalid transition from % with event %', v_current_state, p_event_name;
    END IF;

    -- 验证转换
    SELECT validate_state_transition(
        p_state_machine_name,
        v_current_state,
        v_new_state,
        p_event_name,
        p_context
    ) INTO v_transition_valid;

    IF NOT v_transition_valid THEN
        RAISE EXCEPTION 'State transition validation failed';
    END IF;

    -- 更新状态
    UPDATE entity_state
    SET current_state = v_new_state,
        updated_at = NOW()
    WHERE entity_type = p_entity_type
      AND entity_id = p_entity_id;

    -- 记录转换历史
    INSERT INTO state_transition_history (
        entity_type, entity_id, from_state, to_state,
        event_name, context, operator_id, operator_name, notes
    ) VALUES (
        p_entity_type, p_entity_id, v_current_state, v_new_state,
        p_event_name, p_context, p_operator_id, p_operator_name, p_notes
    );

    RETURN v_new_state;
END;
$$ LANGUAGE plpgsql;
```

---

## 5. PostgreSQL实现

### 5.1 完整状态机系统

**实体状态表**:

```sql
-- 通用实体状态表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entity_state') THEN
            CREATE TABLE entity_state (
                entity_type VARCHAR(100) NOT NULL,
                entity_id BIGINT NOT NULL,
                state_machine_name VARCHAR(100) NOT NULL,
                current_state VARCHAR(100) NOT NULL,
                -- 状态数据（JSONB存储状态相关的额外数据）
                state_data JSONB DEFAULT '{}',
                -- 时间戳
                created_at TIMESTAMPTZ DEFAULT NOW(),
                updated_at TIMESTAMPTZ DEFAULT NOW(),
                PRIMARY KEY (entity_type, entity_id)
            );
            RAISE NOTICE '表 entity_state 创建成功';
        ELSE
            RAISE NOTICE '表 entity_state 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 entity_state 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 entity_state 失败: %', SQLERRM;
    END;
END $$;

-- 订单状态表（具体实现，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            CREATE TABLE order_state (
                order_id BIGINT PRIMARY KEY,
                current_state VARCHAR(100) NOT NULL,
                state_data JSONB DEFAULT '{}',
                created_at TIMESTAMPTZ DEFAULT NOW(),
                updated_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 order_state 创建成功';
        ELSE
            RAISE NOTICE '表 order_state 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 order_state 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 order_state 失败: %', SQLERRM;
    END;
END $$;

-- 订单表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            CREATE TABLE orders (
                order_id BIGSERIAL PRIMARY KEY,
                customer_id BIGINT NOT NULL,
                order_amount NUMERIC(10,2) NOT NULL,
                order_date TIMESTAMPTZ DEFAULT NOW()
                -- 注意：外键约束需要在order_state表创建后添加
            );
            RAISE NOTICE '表 orders 创建成功';
        ELSE
            RAISE NOTICE '表 orders 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 orders 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 orders 失败: %', SQLERRM;
    END;
END $$;

-- 添加外键约束（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE WARNING '依赖表不存在，无法添加外键约束';
            RETURN;
        END IF;

        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints
            WHERE constraint_name = 'orders_order_id_fkey'
            AND table_schema = 'public'
            AND table_name = 'orders'
        ) THEN
            ALTER TABLE orders
            ADD CONSTRAINT orders_order_id_fkey
            FOREIGN KEY (order_id) REFERENCES order_state(order_id);
            RAISE NOTICE '外键约束添加成功';
        ELSE
            RAISE NOTICE '外键约束已存在，跳过添加';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '依赖表不存在';
        WHEN duplicate_object THEN
            RAISE NOTICE '外键约束已存在';
        WHEN OTHERS THEN
            RAISE WARNING '添加外键约束失败: %', SQLERRM;
    END;
END $$;
```

### 5.2 状态机触发器

**自动状态管理触发器**:

```sql
-- 创建订单时初始化状态（带错误处理）
CREATE OR REPLACE FUNCTION init_order_state()
RETURNS TRIGGER AS $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE EXCEPTION '表 order_state 不存在';
        END IF;

        INSERT INTO order_state (order_id, current_state)
        VALUES (NEW.order_id, 'pending');

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entity_state') THEN
            INSERT INTO entity_state (entity_type, entity_id, state_machine_name, current_state)
            VALUES ('order', NEW.order_id, 'order', 'pending');
        END IF;

        RETURN NEW;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '必需的表不存在（请检查 order_state 和 entity_state 表）';
        WHEN OTHERS THEN
            RAISE EXCEPTION '初始化订单状态失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法创建触发器';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'init_order_state') THEN
            RAISE WARNING '函数 init_order_state 不存在，无法创建触发器';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_init_order_state') THEN
            DROP TRIGGER trigger_init_order_state ON orders;
            RAISE NOTICE '已删除现有触发器 trigger_init_order_state';
        END IF;

        CREATE TRIGGER trigger_init_order_state
            AFTER INSERT ON orders
            FOR EACH ROW
            EXECUTE FUNCTION init_order_state();

        RAISE NOTICE '触发器 trigger_init_order_state 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 orders 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 init_order_state 不存在';
        WHEN duplicate_object THEN
            RAISE NOTICE '触发器已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建触发器失败: %', SQLERRM;
    END;
END $$;

-- 状态更新触发器（带错误处理）
CREATE OR REPLACE FUNCTION sync_order_state()
RETURNS TRIGGER AS $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entity_state') THEN
            RAISE WARNING '表 entity_state 不存在，跳过同步';
            RETURN NEW;
        END IF;

        UPDATE entity_state
        SET current_state = NEW.current_state,
            updated_at = NOW()
        WHERE entity_type = 'order'
          AND entity_id = NEW.order_id;

        RETURN NEW;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 entity_state 不存在';
            RETURN NEW;
        WHEN OTHERS THEN
            RAISE EXCEPTION '同步订单状态失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE WARNING '表 order_state 不存在，无法创建触发器';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'sync_order_state') THEN
            RAISE WARNING '函数 sync_order_state 不存在，无法创建触发器';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_sync_order_state') THEN
            DROP TRIGGER trigger_sync_order_state ON order_state;
            RAISE NOTICE '已删除现有触发器 trigger_sync_order_state';
        END IF;

        CREATE TRIGGER trigger_sync_order_state
            AFTER UPDATE ON order_state
            FOR EACH ROW
            WHEN (OLD.current_state IS DISTINCT FROM NEW.current_state)
            EXECUTE FUNCTION sync_order_state();

        RAISE NOTICE '触发器 trigger_sync_order_state 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 order_state 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 sync_order_state 不存在';
        WHEN duplicate_object THEN
            RAISE NOTICE '触发器已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建触发器失败: %', SQLERRM;
    END;
END $$;
```

### 5.3 状态查询视图

**状态查询视图**:

```sql
-- 订单状态查询视图（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE EXCEPTION '依赖表不存在（请先创建 orders 和 order_state 表）';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'order_status_view') THEN
            DROP VIEW order_status_view;
            RAISE NOTICE '已删除现有视图 order_status_view';
        END IF;

        CREATE VIEW order_status_view AS
        SELECT
            o.order_id,
            o.customer_id,
            o.order_amount,
            o.order_date,
            os.current_state,
            os.state_data,
            os.updated_at AS last_state_change,
            -- 状态持续时间
            NOW() - os.updated_at AS state_duration,
            -- 转换历史统计
            (
                SELECT COUNT(*)
                FROM state_transition_history
                WHERE entity_type = 'order'
                  AND entity_id = o.order_id
            ) AS transition_count
        FROM orders o
        JOIN order_state os ON o.order_id = os.order_id;

        RAISE NOTICE '视图 order_status_view 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '依赖表不存在（请先创建 orders 和 order_state 表）';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建视图 order_status_view 失败: %', SQLERRM;
    END;
END $$;
```

---

## 6. 更多实际案例 / More Practical Examples

### 6.1 案例1: 订单状态机

**完整订单状态机实现**:

```sql
-- 订单状态机定义（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_definition') THEN
            RAISE WARNING '表 state_definition 不存在，无法插入数据';
            RETURN;
        END IF;
        INSERT INTO state_definition (state_machine_name, state_name, state_type, description) VALUES
            ('order', 'created', 'initial', '订单已创建'),
            ('order', 'paid', 'normal', '订单已支付'),
            ('order', 'shipped', 'normal', '订单已发货'),
            ('order', 'delivered', 'final', '订单已送达'),
            ('order', 'cancelled', 'final', '订单已取消'),
            ('order', 'refunded', 'final', '订单已退款')
        ON CONFLICT DO NOTHING;
        RAISE NOTICE '订单状态机定义插入成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入状态机定义失败: %', SQLERRM;
    END;
END $$;

-- 订单状态转换规则（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_rule') THEN
            RAISE WARNING '表 state_transition_rule 不存在，无法插入数据';
            RETURN;
        END IF;
        INSERT INTO state_transition_rule (
            state_machine_name, from_state, to_state, event_name, condition, required_role
        ) VALUES
            ('order', 'created', 'paid', 'pay', NULL, 'customer'),
            ('order', 'paid', 'shipped', 'ship', '{"all_items_ready": true}', 'operator'),
            ('order', 'shipped', 'delivered', 'deliver', NULL, 'system'),
            ('order', 'created', 'cancelled', 'cancel', '{"allow_cancel": true}', 'customer'),
            ('order', 'paid', 'cancelled', 'cancel', '{"allow_cancel": true, "not_shipped": true}', 'customer'),
            ('order', 'paid', 'refunded', 'refund', '{"refund_reason": "valid"}', 'operator'),
            ('order', 'shipped', 'refunded', 'refund', '{"return_received": true}', 'operator')
        ON CONFLICT DO NOTHING;
        RAISE NOTICE '订单状态转换规则插入成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入状态转换规则失败: %', SQLERRM;
    END;
END $$;

-- 订单表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            CREATE TABLE orders (
                order_id BIGSERIAL PRIMARY KEY,
                customer_id BIGINT NOT NULL,
                total_amount NUMERIC(10,2) NOT NULL,
                created_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 orders 创建成功';
        ELSE
            RAISE NOTICE '表 orders 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 orders 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 orders 失败: %', SQLERRM;
    END;
END $$;

-- 订单状态表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE EXCEPTION '依赖表 orders 不存在，请先创建 orders 表';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            CREATE TABLE order_state (
                order_id BIGINT PRIMARY KEY REFERENCES orders(order_id),
                current_state VARCHAR(100) NOT NULL DEFAULT 'created',
                state_data JSONB DEFAULT '{}',
                updated_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 order_state 创建成功';
        ELSE
            RAISE NOTICE '表 order_state 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '依赖表 orders 不存在，请先创建 orders 表';
        WHEN duplicate_table THEN
            RAISE NOTICE '表 order_state 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 order_state 失败: %', SQLERRM;
    END;
END $$;

-- 订单状态转换函数（带完整错误处理）
CREATE OR REPLACE FUNCTION change_order_state(
    p_order_id BIGINT,
    p_event_name VARCHAR,
    p_context JSONB DEFAULT NULL,
    p_operator_id VARCHAR DEFAULT NULL
)
RETURNS VARCHAR AS $$
DECLARE
    v_current_state VARCHAR;
    v_new_state VARCHAR;
BEGIN
    BEGIN
        -- 参数验证
        IF p_order_id IS NULL THEN
            RAISE EXCEPTION '订单ID不能为NULL';
        END IF;

        IF p_event_name IS NULL OR length(trim(p_event_name)) = 0 THEN
            RAISE EXCEPTION '事件名称不能为空';
        END IF;

        -- 检查表是否存在
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE EXCEPTION '表 order_state 不存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_rule') THEN
            RAISE EXCEPTION '表 state_transition_rule 不存在';
        END IF;

        -- 获取当前状态
        SELECT current_state INTO v_current_state
        FROM order_state
        WHERE order_id = p_order_id;

        IF v_current_state IS NULL THEN
            RAISE EXCEPTION '订单 % 不存在', p_order_id;
        END IF;

        -- 查找目标状态
        SELECT to_state INTO v_new_state
        FROM state_transition_rule
        WHERE state_machine_name = 'order'
          AND from_state = v_current_state
          AND event_name = p_event_name
          AND is_enabled = TRUE
          AND (condition IS NULL OR condition <@ COALESCE(p_context, '{}'::jsonb))
        LIMIT 1;

        IF v_new_state IS NULL THEN
            RAISE EXCEPTION '无效的状态转换：从状态 % 使用事件 %', v_current_state, p_event_name;
        END IF;

        -- 更新状态
        UPDATE order_state
        SET current_state = v_new_state,
            state_data = COALESCE(p_context, '{}'::jsonb),
            updated_at = NOW()
        WHERE order_id = p_order_id;

        IF NOT FOUND THEN
            RAISE EXCEPTION '更新订单状态失败：订单 % 不存在', p_order_id;
        END IF;

        -- 记录历史
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_history') THEN
            INSERT INTO state_transition_history (
                entity_type, entity_id, from_state, to_state,
                event_name, context, operator_id
            ) VALUES (
                'order', p_order_id, v_current_state, v_new_state,
                p_event_name, p_context, p_operator_id
            );
        END IF;

        RETURN v_new_state;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '必需的表不存在（请检查 order_state、state_transition_rule、state_transition_history 表）';
        WHEN OTHERS THEN
            RAISE EXCEPTION '订单状态转换失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT change_order_state(1, 'pay', '{"payment_method": "credit_card"}'::jsonb, 'customer_123');
SELECT change_order_state(1, 'ship', '{"tracking_number": "TR123456"}'::jsonb, 'operator_456');
```

### 6.2 案例2: 审批流程状态机

**多级审批状态机**:

```sql
-- 审批状态机定义
INSERT INTO state_definition (state_machine_name, state_name, state_type, description) VALUES
    ('approval', 'draft', 'initial', '草稿'),
    ('approval', 'submitted', 'normal', '已提交'),
    ('approval', 'level1_approving', 'normal', '一级审批中'),
    ('approval', 'level2_approving', 'normal', '二级审批中'),
    ('approval', 'approved', 'final', '已批准'),
    ('approval', 'rejected', 'final', '已拒绝'),
    ('approval', 'withdrawn', 'final', '已撤回');

-- 审批转换规则
INSERT INTO state_transition_rule (
    state_machine_name, from_state, to_state, event_name, condition, required_role
) VALUES
    ('approval', 'draft', 'submitted', 'submit', NULL, 'applicant'),
    ('approval', 'submitted', 'level1_approving', 'assign_level1', NULL, 'admin'),
    ('approval', 'level1_approving', 'level2_approving', 'approve_level1', '{"amount": {"$gt": 10000}}', 'level1_approver'),
    ('approval', 'level1_approving', 'approved', 'approve_level1', '{"amount": {"$lte": 10000}}', 'level1_approver'),
    ('approval', 'level2_approving', 'approved', 'approve_level2', NULL, 'level2_approver'),
    ('approval', 'level1_approving', 'rejected', 'reject', NULL, 'level1_approver'),
    ('approval', 'level2_approving', 'rejected', 'reject', NULL, 'level2_approver'),
    ('approval', 'submitted', 'withdrawn', 'withdraw', NULL, 'applicant'),
    ('approval', 'level1_approving', 'withdrawn', 'withdraw', NULL, 'applicant');

-- 审批申请表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'approval_request') THEN
            CREATE TABLE approval_request (
                request_id BIGSERIAL PRIMARY KEY,
                applicant_id BIGINT NOT NULL,
                request_type VARCHAR(50) NOT NULL,
                amount NUMERIC(10,2),
                description TEXT,
                created_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 approval_request 创建成功';
        ELSE
            RAISE NOTICE '表 approval_request 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 approval_request 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表 approval_request 失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 审批状态表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'approval_request') THEN
            RAISE EXCEPTION '引用表 approval_request 不存在，无法创建外键约束表';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'approval_state') THEN
            CREATE TABLE approval_state (
                request_id BIGINT PRIMARY KEY REFERENCES approval_request(request_id),
                current_state VARCHAR(100) NOT NULL DEFAULT 'draft',
                current_approver_id BIGINT,
                state_data JSONB DEFAULT '{}',
                updated_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 approval_state 创建成功';
        ELSE
            RAISE NOTICE '表 approval_state 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '引用表 approval_request 不存在';
        WHEN duplicate_table THEN
            RAISE NOTICE '表 approval_state 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表 approval_state 失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 审批历史表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'approval_request') THEN
            RAISE EXCEPTION '引用表 approval_request 不存在，无法创建外键约束表';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'approval_history') THEN
            CREATE TABLE approval_history (
                history_id BIGSERIAL PRIMARY KEY,
                request_id BIGINT NOT NULL REFERENCES approval_request(request_id),
                from_state VARCHAR(100),
                to_state VARCHAR(100) NOT NULL,
                approver_id BIGINT,
    approver_name VARCHAR(200),
    comments TEXT,
    transition_time TIMESTAMPTZ DEFAULT NOW()
);
```

### 6.3 案例3: 支付状态机

**支付流程状态机**:

```sql
-- 支付状态机定义（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_definition') THEN
            RAISE WARNING '表 state_definition 不存在，无法插入数据';
            RETURN;
        END IF;
        INSERT INTO state_definition (state_machine_name, state_name, state_type, description) VALUES
            ('payment', 'pending', 'initial', '待支付'),
            ('payment', 'processing', 'normal', '处理中'),
            ('payment', 'succeeded', 'final', '支付成功'),
            ('payment', 'failed', 'final', '支付失败'),
            ('payment', 'refunding', 'normal', '退款中'),
            ('payment', 'refunded', 'final', '已退款'),
            ('payment', 'cancelled', 'final', '已取消')
        ON CONFLICT DO NOTHING;
        RAISE NOTICE '支付状态机定义插入成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入支付状态机定义失败: %', SQLERRM;
    END;
END $$;

-- 支付转换规则（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_rule') THEN
            RAISE WARNING '表 state_transition_rule 不存在，无法插入数据';
            RETURN;
        END IF;
        INSERT INTO state_transition_rule (
            state_machine_name, from_state, to_state, event_name, condition, required_role
        ) VALUES
            ('payment', 'pending', 'processing', 'process', NULL, 'system'),
            ('payment', 'processing', 'succeeded', 'success', NULL, 'payment_gateway'),
            ('payment', 'processing', 'failed', 'fail', NULL, 'payment_gateway'),
            ('payment', 'pending', 'cancelled', 'cancel', NULL, 'customer'),
            ('payment', 'succeeded', 'refunding', 'refund', NULL, 'operator'),
            ('payment', 'refunding', 'refunded', 'refund_success', NULL, 'payment_gateway'),
            ('payment', 'refunding', 'failed', 'refund_fail', NULL, 'payment_gateway')
        ON CONFLICT DO NOTHING;
        RAISE NOTICE '支付转换规则插入成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入支付转换规则失败: %', SQLERRM;
    END;
END $$;

-- 支付表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payments') THEN
            CREATE TABLE payments (
                payment_id BIGSERIAL PRIMARY KEY,
                order_id BIGINT NOT NULL,
                amount NUMERIC(10,2) NOT NULL,
                payment_method VARCHAR(50) NOT NULL,
                created_at TIMESTAMPTZ DEFAULT NOW()
            );
            RAISE NOTICE '表 payments 创建成功';
        ELSE
            RAISE NOTICE '表 payments 已存在，跳过创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 payments 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建表 payments 失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 支付状态表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payments') THEN
            RAISE EXCEPTION '引用表 payments 不存在，无法创建外键约束表';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payment_state') THEN
            CREATE TABLE payment_state (
                payment_id BIGINT PRIMARY KEY REFERENCES payments(payment_id),
    current_state VARCHAR(100) NOT NULL DEFAULT 'pending',
    gateway_transaction_id VARCHAR(200),
    state_data JSONB DEFAULT '{}',
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
```

---

## 7. 性能优化与监控 / Performance Optimization and Monitoring

### 7.1 状态机性能优化

**索引优化**:

```sql
-- 状态转换历史表索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_history') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_transition_history_entity_time') THEN
                CREATE INDEX idx_transition_history_entity_time
                ON state_transition_history(entity_type, entity_id, transition_time DESC);
                RAISE NOTICE '索引 idx_transition_history_entity_time 创建成功';
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_transition_history_state_time') THEN
                CREATE INDEX idx_transition_history_state_time
                ON state_transition_history(to_state, transition_time DESC);
                RAISE NOTICE '索引 idx_transition_history_state_time 创建成功';
            END IF;
        ELSE
            RAISE WARNING '表 state_transition_history 不存在，跳过索引创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建状态转换历史表索引失败: %', SQLERRM;
    END;
END $$;

-- 状态转换规则表索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'state_transition_rule') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_transition_rule_lookup') THEN
                CREATE INDEX idx_transition_rule_lookup
                ON state_transition_rule(state_machine_name, from_state, event_name, is_enabled);
                RAISE NOTICE '索引 idx_transition_rule_lookup 创建成功';
            END IF;
        ELSE
            RAISE WARNING '表 state_transition_rule 不存在，跳过索引创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建状态转换规则表索引失败: %', SQLERRM;
    END;
END $$;

-- 实体状态表索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entity_state') THEN
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_entity_state_lookup') THEN
                CREATE INDEX idx_entity_state_lookup
                ON entity_state(entity_type, entity_id);
                RAISE NOTICE '索引 idx_entity_state_lookup 创建成功';
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_entity_state_current') THEN
                CREATE INDEX idx_entity_state_current
                ON entity_state(state_machine_name, current_state);
                RAISE NOTICE '索引 idx_entity_state_current 创建成功';
            END IF;
        ELSE
            RAISE WARNING '表 entity_state 不存在，跳过索引创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建实体状态表索引失败: %', SQLERRM;
    END;
END $$;
```

**查询优化**:

```sql
-- 优化：使用物化视图缓存状态统计（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'entity_state') THEN
            RAISE WARNING '表 entity_state 不存在，无法创建物化视图';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'state_statistics') THEN
            CREATE MATERIALIZED VIEW state_statistics AS
            SELECT
                entity_type,
                state_machine_name,
                current_state,
                COUNT(*) AS entity_count,
    MAX(updated_at) AS last_update
FROM entity_state
GROUP BY entity_type, state_machine_name, current_state;

CREATE INDEX ON state_statistics(entity_type, state_machine_name, current_state);

-- 定期刷新
REFRESH MATERIALIZED VIEW CONCURRENTLY state_statistics;
```

### 7.2 状态机监控

**状态机监控查询**:

```sql
-- 监控：各状态的实体数量
SELECT
    entity_type,
    state_machine_name,
    current_state,
    COUNT(*) AS count,
    MIN(updated_at) AS oldest_entity,
    MAX(updated_at) AS newest_entity
FROM entity_state
GROUP BY entity_type, state_machine_name, current_state
ORDER BY count DESC;

-- 监控：状态转换频率
SELECT
    entity_type,
    from_state,
    to_state,
    event_name,
    COUNT(*) AS transition_count,
    AVG(EXTRACT(EPOCH FROM (transition_time - LAG(transition_time) OVER (PARTITION BY entity_type, entity_id ORDER BY transition_time)))) AS avg_duration_seconds
FROM state_transition_history
WHERE transition_time >= NOW() - INTERVAL '24 hours'
GROUP BY entity_type, from_state, to_state, event_name
ORDER BY transition_count DESC;

-- 监控：长时间停留在某个状态的实体
SELECT
    es.entity_type,
    es.entity_id,
    es.current_state,
    es.updated_at,
    NOW() - es.updated_at AS duration_in_state
FROM entity_state es
WHERE es.updated_at < NOW() - INTERVAL '7 days'
  AND es.current_state NOT IN ('completed', 'cancelled', 'closed')
ORDER BY duration_in_state DESC;
```

---

## 8. 常见问题解答 / FAQ

### Q1: 状态机应该有多少个状态？

**A**: 状态数量建议：

- **最少**: 3个状态（初始、进行中、终态）
- **推荐**: 5-10个状态
- **最多**: 不超过20个状态

**原则**: 遵循米勒定律（7±2），保持状态机简单易懂。

### Q2: 如何处理并发状态转换？

**A**: 并发控制策略：

```sql
-- 使用SELECT FOR UPDATE锁定实体（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE WARNING '表 order_state 不存在，无法执行锁定查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行SELECT FOR UPDATE锁定查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
END $$;

BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT current_state FROM order_state
WHERE order_id = 123 FOR UPDATE;

-- 执行状态转换（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'change_order_state') THEN
            RAISE WARNING '函数 change_order_state 不存在，无法执行状态转换';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行状态转换';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '状态转换准备失败: %', SQLERRM;
    END;
END $$;

-- SELECT change_order_state(123, 'ship', ...);
COMMIT;

-- 或使用乐观锁（版本号，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE WARNING '表 order_state 不存在，无法创建乐观锁表';
            RETURN;
        END IF;
        RAISE NOTICE '乐观锁示例：表 order_state 已存在，跳过创建';
        -- CREATE TABLE order_state (
        --     order_id BIGINT PRIMARY KEY,
        --     current_state VARCHAR(100),
        --     version INT DEFAULT 1,
        --     ...
        -- );
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '表检查失败: %', SQLERRM;
    END;
END $$;

-- 乐观锁更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_state') THEN
            RAISE WARNING '表 order_state 不存在，无法执行乐观锁更新';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行乐观锁更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新准备失败: %', SQLERRM;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
UPDATE order_state
SET current_state = 'shipped',
    version = version + 1
WHERE order_id = 123 AND version = 1;  -- 版本检查
```

### Q3: 状态转换历史表会很大，如何优化？

**A**: 优化策略：

1. **分区**: 按时间分区历史表
2. **归档**: 定期归档旧历史数据
3. **压缩**: 使用JSONB压缩存储上下文数据
4. **索引**: 创建合适的索引

```sql
-- 分区历史表
CREATE TABLE state_transition_history (
    ...
) PARTITION BY RANGE (transition_time);

-- 按月分区
CREATE TABLE state_transition_history_2024_01
PARTITION OF state_transition_history
FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');
```

### Q4: 如何实现状态机的回滚？

**A**: 回滚实现：

```sql
-- 添加回滚标志到转换规则
ALTER TABLE state_transition_rule
ADD COLUMN is_reversible BOOLEAN DEFAULT FALSE;

-- 回滚函数
CREATE OR REPLACE FUNCTION rollback_state_transition(
    p_entity_type VARCHAR,
    p_entity_id BIGINT,
    p_steps INT DEFAULT 1
)
RETURNS VARCHAR AS $$
DECLARE
    v_target_history RECORD;
BEGIN
    -- 查找要回滚到的历史记录
    SELECT * INTO v_target_history
    FROM state_transition_history
    WHERE entity_type = p_entity_type
      AND entity_id = p_entity_id
    ORDER BY transition_time DESC
    OFFSET p_steps
    LIMIT 1;

    IF v_target_history IS NULL THEN
        RAISE EXCEPTION 'Cannot rollback % steps', p_steps;
    END IF;

    -- 恢复到目标状态
    UPDATE entity_state
    SET current_state = v_target_history.to_state,
        updated_at = NOW()
    WHERE entity_type = p_entity_type
      AND entity_id = p_entity_id;

    -- 记录回滚历史
    INSERT INTO state_transition_history (
        entity_type, entity_id, from_state, to_state,
        event_name, context
    ) VALUES (
        p_entity_type, p_entity_id,
        (SELECT current_state FROM entity_state
         WHERE entity_type = p_entity_type AND entity_id = p_entity_id),
        v_target_history.to_state,
        'rollback',
        jsonb_build_object('rollback_to', v_target_history.history_id)
    );

    RETURN v_target_history.to_state;
END;
$$ LANGUAGE plpgsql;
```

### Q5: 状态机性能如何优化？

**A**: 性能优化建议：

1. **索引**: 为频繁查询的列创建索引
2. **物化视图**: 缓存状态统计信息
3. **批量操作**: 批量更新状态
4. **异步处理**: 使用消息队列异步处理状态转换

```sql
-- 批量状态更新
CREATE OR REPLACE FUNCTION batch_update_states(
    p_updates JSONB
)
RETURNS TABLE(entity_id BIGINT, new_state VARCHAR) AS $$
DECLARE
    v_update JSONB;
BEGIN
    FOR v_update IN SELECT * FROM jsonb_array_elements(p_updates)
    LOOP
        UPDATE entity_state
        SET current_state = v_update->>'new_state',
            updated_at = NOW()
        WHERE entity_type = v_update->>'entity_type'
          AND entity_id = (v_update->>'entity_id')::BIGINT;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

---

## 8. 相关资源 / Related Resources

### 8.1 核心相关文档 / Core Related Documents

- [JSONB状态机实现](./JSONB状态机实现.md) - JSONB实现状态机
- [工作流模式](./工作流模式.md) - 工作流模式指南
- [BPMN建模](./BPMN建模.md) - BPMN标准建模
- [订单管理模型](../04-OLTP建模/订单管理模型.md) - 订单状态机应用案例

### 8.2 理论基础 / Theoretical Foundation

- [约束理论](../01-数据建模理论基础/约束理论.md) - 状态转换约束理论

### 8.3 实践指南 / Practical Guides

- [性能优化与监控](#7-性能优化与监控--performance-optimization-and-monitoring) - 本文档的性能监控章节
- [更多实际案例](#6-更多实际案例--more-practical-examples) - 本文档的应用案例章节
- [索引策略](../08-PostgreSQL建模实践/索引策略.md) - 状态机索引设计

### 8.4 应用案例 / Application Cases

- [电商数据模型案例](../10-综合应用案例/电商数据模型案例.md) - 电商订单状态机案例

### 8.5 参考资源 / Reference Resources

- [权威资源索引](../00-导航与索引/权威资源索引.md) - 权威资源列表
- [术语对照表](../00-导航与索引/术语对照表.md) - 术语对照
- [快速查找指南](../00-导航与索引/快速查找指南.md) - 快速查找工具
- UML状态图: [UML State Machine Diagrams](https://www.uml-diagrams.org/state-machine-diagrams.html)

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
