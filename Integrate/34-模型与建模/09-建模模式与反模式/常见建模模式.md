# 常见建模模式

> **创建日期**: 2025年1月
> **来源**: 数据建模模式库
> **状态**: ✅ 已完成
> **文档编号**: 09-01

---

## 📑 目录

- [常见建模模式](#常见建模模式)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
  - [1.1 理论基础](#11-理论基础)
    - [1.1.1 建模模式基本概念](#111-建模模式基本概念)
    - [1.1.2 Party模式理论](#112-party模式理论)
    - [1.1.3 Order模式理论](#113-order模式理论)
    - [1.1.4 Product模式理论](#114-product模式理论)
    - [1.1.5 Account模式理论](#115-account模式理论)
    - [1.1.6 模式组合理论](#116-模式组合理论)
    - [1.1.7 复杂度分析](#117-复杂度分析)
  - [2. Party模式](#2-party模式)
    - [2.1 Party模式概述](#21-party模式概述)
    - [2.2 Party模式设计](#22-party模式设计)
  - [3. Order模式](#3-order模式)
    - [3.1 Order模式概述](#31-order模式概述)
    - [3.2 Order模式设计](#32-order模式设计)
  - [5. Account模式](#5-account模式)
    - [5.1 Account模式概述](#51-account模式概述)
    - [5.2 Account模式设计](#52-account模式设计)
  - [6. PostgreSQL实现](#6-postgresql实现)
    - [6.1 模式组合使用](#61-模式组合使用)
    - [6.2 模式查询视图](#62-模式查询视图)
  - [7. 模式应用案例 / Pattern Application Examples](#7-模式应用案例--pattern-application-examples)
    - [7.1 案例1: CRM系统（Party模式）](#71-案例1-crm系统party模式)
    - [7.2 案例2: 电商系统（Party+Order+Product模式）](#72-案例2-电商系统partyorderproduct模式)
    - [7.3 案例3: 银行系统（Account模式）](#73-案例3-银行系统account模式)
  - [8. 常见问题解答 / FAQ](#8-常见问题解答--faq)
    - [Q1: 什么时候应该使用Party模式？](#q1-什么时候应该使用party模式)
    - [Q2: Order模式和Account模式有什么区别？](#q2-order模式和account模式有什么区别)
    - [Q3: 如何组合使用多个模式？](#q3-如何组合使用多个模式)
    - [Q4: 模式设计如何平衡灵活性和性能？](#q4-模式设计如何平衡灵活性和性能)
    - [Q5: 如何迁移现有系统到标准模式？](#q5-如何迁移现有系统到标准模式)
  - [8. 相关资源 / Related Resources](#8-相关资源--related-resources)
    - [8.1 核心相关文档 / Core Related Documents](#81-核心相关文档--core-related-documents)
    - [8.2 理论基础 / Theoretical Foundation](#82-理论基础--theoretical-foundation)
    - [8.3 实践指南 / Practical Guides](#83-实践指南--practical-guides)
    - [8.4 应用案例 / Application Cases](#84-应用案例--application-cases)
    - [8.5 参考资源 / Reference Resources](#85-参考资源--reference-resources)

---

## 1. 概述

常见建模模式是经过验证的、可重用的数据模型设计解决方案。
这些模式来自Silverston的数据模型资源手册，是数据建模领域的经典模式。

**核心价值**:

- **可重用性**：模式可在多个项目中重用
- **标准化**：提供标准的数据模型结构
- **最佳实践**：基于行业最佳实践
- **扩展性**：模式易于扩展和定制

---

## 1.1 理论基础

### 1.1.1 建模模式基本概念

**建模模式（Data Modeling Pattern）**:

- **定义**: 经过验证的、可重用的数据模型设计解决方案
- **来源**: Silverston数据模型资源手册
- **目的**: 提供标准的数据模型结构

**模式特征**:

- **可重用性**: 模式可以在多个项目中重用
- **标准化**: 提供标准的数据模型结构
- **最佳实践**: 基于行业最佳实践
- **扩展性**: 模式易于扩展和定制

### 1.1.2 Party模式理论

**Party模式**:

- **定义**: 统一表示人员（Person）和组织（Organization）的通用模式
- **抽象**: $Party = Person \cup Organization$
- **优势**: 避免重复设计，支持复杂角色关系

**Party模式优势**:

- **统一管理**: 统一管理人员和组织
- **角色分离**: 角色通过Party Role表管理
- **关系管理**: 支持复杂的Party关系

### 1.1.3 Order模式理论

**Order模式**:

- **定义**: 订单头-订单行模式
- **结构**: Order Header + Order Lines
- **优势**: 支持复杂订单结构

**Order模式特点**:

- **一对多关系**: 一个订单头对应多个订单行
- **聚合根**: 订单头是聚合根
- **事务边界**: 订单头+订单行在同一事务中

### 1.1.4 Product模式理论

**Product模式**:

- **定义**: 产品分类和属性模式
- **结构**: Product + Product Category + Product Attribute
- **优势**: 支持灵活的产品属性管理

**Product模式特点**:

- **分类层次**: 支持产品分类层次结构
- **属性灵活**: 支持动态产品属性
- **版本管理**: 支持产品版本管理

### 1.1.5 Account模式理论

**Account模式**:

- **定义**: 账户余额和交易模式
- **结构**: Account + Account Balance + Transaction
- **优势**: 支持账户余额和交易历史

**Account模式特点**:

- **余额管理**: 支持账户余额管理
- **交易历史**: 记录所有交易历史
- **一致性**: 保证余额一致性

### 1.1.6 模式组合理论

**模式组合**:

- **组合原则**: 模式可以组合使用
- **组合方式**: Party + Order + Product
- **组合优势**: 构建完整的业务模型

**模式组合示例**:

- **CRM系统**: Party模式
- **电商系统**: Party + Order + Product模式
- **银行系统**: Party + Account模式

### 1.1.7 复杂度分析

**存储复杂度**:

- **Party模式**: $O(P)$ where P is number of parties
- **Order模式**: $O(O + L)$ where O is orders, L is order lines
- **Product模式**: $O(P + C)$ where P is products, C is categories

**查询复杂度**:

- **模式查询**: $O(\log N)$ with index
- **模式组合查询**: $O(\log N \times M)$ where M is number of patterns

---

## 2. Party模式

### 2.1 Party模式概述

**Party模式**：统一表示人员（Person）和组织（Organization）的通用模式。

**核心优势**:

- 统一管理人员和组织
- 支持复杂的角色关系
- 灵活的角色分配
- 易于扩展

### 2.2 Party模式设计

**PostgreSQL实现**:

```sql
-- Party父表（使用表继承，带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS party (
        party_id SERIAL PRIMARY KEY,
        party_type CHAR(1) NOT NULL CHECK (party_type IN ('P', 'O')),
        -- 通用属性
        name TEXT NOT NULL,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 party 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 party 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 party 失败: %', SQLERRM;
END $$;

-- Person子表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS person (
        party_id INT PRIMARY KEY REFERENCES party(party_id),
        first_name VARCHAR(100),
        last_name VARCHAR(100),
        date_of_birth DATE,
        gender CHAR(1) CHECK (gender IN ('M', 'F', 'O')),
        email VARCHAR(200),
        phone VARCHAR(50)
    ) INHERITS (party);
    RAISE NOTICE '表 person 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 person 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 person 失败: %', SQLERRM;
END $$;

-- Organization子表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS organization (
        party_id INT PRIMARY KEY REFERENCES party(party_id),
        legal_name VARCHAR(200) NOT NULL,
        tax_id VARCHAR(50),
        registration_number VARCHAR(100),
        website VARCHAR(200)
    ) INHERITS (party);
    RAISE NOTICE '表 organization 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 organization 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 organization 失败: %', SQLERRM;
END $$;

-- Party Role关联表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS party_role (
        role_id SERIAL PRIMARY KEY,
        party_id INT NOT NULL REFERENCES party(party_id),
        role_type VARCHAR(50) NOT NULL, -- 'customer', 'supplier', 'employee', etc.
        role_name VARCHAR(200),
        valid_from TIMESTAMPTZ DEFAULT NOW(),
        valid_to TIMESTAMPTZ,
        is_primary BOOLEAN DEFAULT FALSE,
        UNIQUE(party_id, role_type, valid_from)
    );
    RAISE NOTICE '表 party_role 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 party_role 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 party_role 失败: %', SQLERRM;
END $$;

-- Party关系表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS party_relationship (
        relationship_id SERIAL PRIMARY KEY,
        from_party_id INT NOT NULL REFERENCES party(party_id),
        to_party_id INT NOT NULL REFERENCES party(party_id),
        relationship_type VARCHAR(50) NOT NULL, -- 'employee_of', 'customer_of', 'parent_of'
        valid_from TIMESTAMPTZ DEFAULT NOW(),
        valid_to TIMESTAMPTZ,
        CHECK (from_party_id != to_party_id)
    );
    RAISE NOTICE '表 party_relationship 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 party_relationship 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 party_relationship 失败: %', SQLERRM;
END $$;
```

---

## 3. Order模式

### 3.1 Order模式概述

**Order模式**：订单管理系统的标准模式，支持复杂的订单处理流程。

**核心组件**:

- Order Header（订单头）
- Order Line（订单行）
- Order Status（订单状态）
- Payment（支付）
- Shipment（配送）

### 3.2 Order模式设计

**PostgreSQL实现**:

```sql
-- 订单头表（带错误处理）
DO $$
BEGIN
    CREATE TABLE IF NOT EXISTS order_header (
        order_id BIGSERIAL PRIMARY KEY,
        order_number VARCHAR(50) UNIQUE NOT NULL,
        -- Party关联
        customer_party_id INT NOT NULL REFERENCES party(party_id),
        -- 订单信息
        order_date TIMESTAMPTZ DEFAULT NOW(),
        required_date TIMESTAMPTZ,
        shipped_date TIMESTAMPTZ,
        -- 订单状态
        order_status VARCHAR(50) DEFAULT 'pending',
        -- 金额信息
        subtotal NUMERIC(10,2) NOT NULL DEFAULT 0,
        tax_amount NUMERIC(10,2) DEFAULT 0,
        shipping_amount NUMERIC(10,2) DEFAULT 0,
        total_amount NUMERIC(10,2) GENERATED ALWAYS AS (
            subtotal + tax_amount + shipping_amount
        ) STORED,
        -- 元数据
        notes TEXT,
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW()
    );
    RAISE NOTICE '表 order_header 创建成功';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表 order_header 已存在，跳过创建';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 order_header 失败: %', SQLERRM;
END $$;
```

-- 订单行表
CREATE TABLE order_line (
    line_id BIGSERIAL PRIMARY KEY,
    order_id BIGINT NOT NULL REFERENCES order_header(order_id),
    line_number INT NOT NULL,
    -- 产品关联
    product_id INT NOT NULL, -- 引用产品表
    -- 订单行信息
    quantity INT NOT NULL CHECK (quantity > 0),
    unit_price NUMERIC(10,2) NOT NULL,
    discount_amount NUMERIC(10,2) DEFAULT 0,
    line_total NUMERIC(10,2) GENERATED ALWAYS AS (
        quantity * unit_price - discount_amount
    ) STORED,
    -- 状态
    line_status VARCHAR(50) DEFAULT 'pending',
    -- 元数据
    notes TEXT,
    UNIQUE(order_id, line_number)
);

-- 订单状态历史表
CREATE TABLE order_status_history (
    history_id BIGSERIAL PRIMARY KEY,
    order_id BIGINT NOT NULL REFERENCES order_header(order_id),
    from_status VARCHAR(50),
    to_status VARCHAR(50) NOT NULL,
    changed_at TIMESTAMPTZ DEFAULT NOW(),
    changed_by VARCHAR(100),
    notes TEXT
);

-- 支付表
CREATE TABLE payment (
    payment_id BIGSERIAL PRIMARY KEY,
    order_id BIGINT NOT NULL REFERENCES order_header(order_id),
    payment_method VARCHAR(50) NOT NULL, -- 'credit_card', 'debit_card', 'paypal', etc.
    payment_status VARCHAR(50) DEFAULT 'pending',
    amount NUMERIC(10,2) NOT NULL,
    payment_date TIMESTAMPTZ,
    transaction_id VARCHAR(100),
    payment_details JSONB
);

```

---

## 4. Product模式

### 4.1 Product模式概述

**Product模式**：产品目录管理的标准模式，支持复杂的产品分类和属性。

**核心组件**:

- Product（产品）
- Product Category（产品分类）
- Product Attribute（产品属性）
- Product Price（产品价格）

### 4.2 Product模式设计

**PostgreSQL实现**:

```sql
-- 产品分类表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product_category') THEN
        CREATE TABLE product_category (
            category_id SERIAL PRIMARY KEY,
            parent_category_id INT REFERENCES product_category(category_id),
            category_code VARCHAR(50) UNIQUE NOT NULL,
            category_name VARCHAR(200) NOT NULL,
            description TEXT,
            display_order INT DEFAULT 0,
            is_active BOOLEAN DEFAULT TRUE
        );
        RAISE NOTICE '表 product_category 创建成功';
    ELSE
        RAISE NOTICE '表 product_category 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product_category 失败: %', SQLERRM;
END $$;

-- 产品表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product') THEN
        CREATE TABLE product (
            product_id SERIAL PRIMARY KEY,
            product_code VARCHAR(50) UNIQUE NOT NULL,
            product_name VARCHAR(200) NOT NULL,
            -- 分类关联
            category_id INT REFERENCES product_category(category_id),
            -- 产品信息
            description TEXT,
            brand VARCHAR(100),
            manufacturer VARCHAR(200),
            -- 状态
            status VARCHAR(50) DEFAULT 'active', -- 'active', 'inactive', 'discontinued'
            -- 元数据
            created_at TIMESTAMPTZ DEFAULT NOW(),
            updated_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 product 创建成功';
    ELSE
        RAISE NOTICE '表 product 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product 失败: %', SQLERRM;
END $$;

-- 产品属性定义表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product_attribute_definition') THEN
        CREATE TABLE product_attribute_definition (
            attribute_id SERIAL PRIMARY KEY,
            category_id INT REFERENCES product_category(category_id),
            attribute_name VARCHAR(100) NOT NULL,
            attribute_type VARCHAR(50) NOT NULL, -- 'text', 'number', 'boolean', 'date'
            is_required BOOLEAN DEFAULT FALSE,
            display_order INT DEFAULT 0,
            UNIQUE(category_id, attribute_name)
        );
        RAISE NOTICE '表 product_attribute_definition 创建成功';
    ELSE
        RAISE NOTICE '表 product_attribute_definition 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product_attribute_definition 失败: %', SQLERRM;
END $$;

-- 产品属性值表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product_attribute_value') THEN
        CREATE TABLE product_attribute_value (
            value_id BIGSERIAL PRIMARY KEY,
            product_id INT NOT NULL REFERENCES product(product_id),
            attribute_id INT NOT NULL REFERENCES product_attribute_definition(attribute_id),
            attribute_value TEXT NOT NULL,
            UNIQUE(product_id, attribute_id)
        );
        RAISE NOTICE '表 product_attribute_value 创建成功';
    ELSE
        RAISE NOTICE '表 product_attribute_value 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product_attribute_value 失败: %', SQLERRM;
END $$;

-- 产品价格表（支持多价格策略，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product_price') THEN
        CREATE TABLE product_price (
            price_id BIGSERIAL PRIMARY KEY,
            product_id INT NOT NULL REFERENCES product(product_id),
            price_type VARCHAR(50) NOT NULL, -- 'list', 'sale', 'wholesale', 'member'
            price_amount NUMERIC(10,2) NOT NULL,
            currency_code CHAR(3) DEFAULT 'USD',
            valid_from TIMESTAMPTZ DEFAULT NOW(),
            valid_to TIMESTAMPTZ,
            is_active BOOLEAN DEFAULT TRUE
        );
        RAISE NOTICE '表 product_price 创建成功';
    ELSE
        RAISE NOTICE '表 product_price 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product_price 失败: %', SQLERRM;
END $$;

-- 产品库存表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'product_inventory') THEN
        CREATE TABLE product_inventory (
            inventory_id BIGSERIAL PRIMARY KEY,
            product_id INT NOT NULL REFERENCES product(product_id),
            location_id INT, -- 仓库位置
            quantity_on_hand INT NOT NULL DEFAULT 0,
            quantity_reserved INT NOT NULL DEFAULT 0,
            quantity_available INT GENERATED ALWAYS AS (
                quantity_on_hand - quantity_reserved
            ) STORED,
            reorder_point INT DEFAULT 0,
            last_updated TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 product_inventory 创建成功';
    ELSE
        RAISE NOTICE '表 product_inventory 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 product_inventory 失败: %', SQLERRM;
END $$;
```

---

## 5. Account模式

### 5.1 Account模式概述

**Account模式**：账户管理的标准模式，支持金融账户、会员账户等多种账户类型。

**核心组件**:

- Account（账户）
- Account Type（账户类型）
- Account Balance（账户余额）
- Transaction（交易）

### 5.2 Account模式设计

**PostgreSQL实现**:

```sql
-- 账户类型表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'account_type') THEN
        CREATE TABLE account_type (
            type_id SERIAL PRIMARY KEY,
            type_code VARCHAR(50) UNIQUE NOT NULL,
            type_name VARCHAR(200) NOT NULL,
            description TEXT,
            allows_negative_balance BOOLEAN DEFAULT FALSE,
            interest_rate NUMERIC(5,4) DEFAULT 0
        );
        RAISE NOTICE '表 account_type 创建成功';
    ELSE
        RAISE NOTICE '表 account_type 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 account_type 失败: %', SQLERRM;
END $$;

-- 账户表（带错误处理，需要先创建party表）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'account') THEN
        CREATE TABLE account (
            account_id BIGSERIAL PRIMARY KEY,
            account_number VARCHAR(50) UNIQUE NOT NULL,
            -- Party关联
            party_id INT NOT NULL REFERENCES party(party_id),
            -- 账户类型
            account_type_id INT NOT NULL REFERENCES account_type(type_id),
            -- 账户信息
            account_name VARCHAR(200),
            currency_code CHAR(3) DEFAULT 'USD',
            -- 状态
            status VARCHAR(50) DEFAULT 'active', -- 'active', 'closed', 'frozen'
            -- 时间戳
            opened_date DATE DEFAULT CURRENT_DATE,
            closed_date DATE,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );
        RAISE NOTICE '表 account 创建成功';
    ELSE
        RAISE NOTICE '表 account 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '请先创建 party 表';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 account 失败: %', SQLERRM;
END $$;

-- 账户余额表（快照表，带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'account_balance') THEN
        CREATE TABLE account_balance (
            balance_id BIGSERIAL PRIMARY KEY,
            account_id BIGINT NOT NULL REFERENCES account(account_id),
            balance_date DATE NOT NULL,
            -- 余额信息
            opening_balance NUMERIC(15,2) NOT NULL DEFAULT 0,
            closing_balance NUMERIC(15,2) NOT NULL DEFAULT 0,
            -- 交易统计
            debit_count INT DEFAULT 0,
            credit_count INT DEFAULT 0,
            total_debits NUMERIC(15,2) DEFAULT 0,
            total_credits NUMERIC(15,2) DEFAULT 0,
            UNIQUE(account_id, balance_date)
        );
        RAISE NOTICE '表 account_balance 创建成功';
    ELSE
        RAISE NOTICE '表 account_balance 已存在，跳过创建';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表 account_balance 失败: %', SQLERRM;
END $$;
```

-- 交易表
CREATE TABLE transaction (
    transaction_id BIGSERIAL PRIMARY KEY,
    transaction_number VARCHAR(50) UNIQUE NOT NULL,
    -- 账户关联
    account_id BIGINT NOT NULL REFERENCES account(account_id),
    -- 交易信息
    transaction_type VARCHAR(50) NOT NULL, -- 'debit', 'credit', 'transfer'
    transaction_date TIMESTAMPTZ DEFAULT NOW(),
    amount NUMERIC(15,2) NOT NULL,
    -- 余额
    balance_after NUMERIC(15,2),
    -- 关联信息
    related_transaction_id BIGINT REFERENCES transaction(transaction_id),
    related_account_id BIGINT REFERENCES account(account_id),
    -- 描述
    description TEXT,
    reference_number VARCHAR(100),
    -- 状态
    status VARCHAR(50) DEFAULT 'posted', -- 'pending', 'posted', 'cancelled'
    posted_at TIMESTAMPTZ,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 创建索引
CREATE INDEX idx_transaction_account_date ON transaction(account_id, transaction_date DESC);
CREATE INDEX idx_transaction_type ON transaction(transaction_type, transaction_date DESC);
CREATE INDEX idx_account_balance_date ON account_balance(account_id, balance_date DESC);
```

---

## 6. PostgreSQL实现

### 6.1 模式组合使用

**综合应用示例**:

```sql
-- 电商系统：组合使用Party、Order、Product模式
-- 1. 客户（Party）
INSERT INTO person (party_id, party_type, name, first_name, last_name, email)
VALUES (1, 'P', 'John Doe', 'John', 'Doe', 'john@example.com');

INSERT INTO party_role (party_id, role_type, role_name, is_primary)
VALUES (1, 'customer', 'Customer', TRUE);

-- 2. 产品（Product）
INSERT INTO product (product_code, product_name, category_id, status)
VALUES ('PROD001', 'Laptop Computer', 1, 'active');

INSERT INTO product_price (product_id, price_type, price_amount)
VALUES (1, 'list', 999.99);

-- 3. 订单（Order）
INSERT INTO order_header (order_number, customer_party_id, order_status, subtotal)
VALUES ('ORD001', 1, 'pending', 999.99);

INSERT INTO order_line (order_id, line_number, product_id, quantity, unit_price)
VALUES (1, 1, 1, 1, 999.99);

-- 4. 账户（Account）- 如果支持账户余额
INSERT INTO account (account_number, party_id, account_type_id, account_name)
VALUES ('ACC001', 1, 1, 'Customer Account');
```

### 6.2 模式查询视图

**常用查询视图**:

```sql
-- 客户订单视图
CREATE VIEW customer_order_view AS
SELECT
    p.party_id,
    p.name AS customer_name,
    o.order_id,
    o.order_number,
    o.order_date,
    o.total_amount,
    o.order_status,
    COUNT(ol.line_id) AS line_count
FROM party p
JOIN party_role pr ON p.party_id = pr.party_id AND pr.role_type = 'customer'
JOIN order_header o ON p.party_id = o.customer_party_id
LEFT JOIN order_line ol ON o.order_id = ol.order_id
GROUP BY p.party_id, p.name, o.order_id, o.order_number, o.order_date, o.total_amount, o.order_status;

-- 产品销售统计视图
CREATE VIEW product_sales_view AS
SELECT
    p.product_id,
    p.product_code,
    p.product_name,
    SUM(ol.quantity) AS total_quantity_sold,
    SUM(ol.line_total) AS total_revenue,
    COUNT(DISTINCT ol.order_id) AS order_count
FROM product p
JOIN order_line ol ON p.product_id = ol.product_id
JOIN order_header o ON ol.order_id = o.order_id
WHERE o.order_status = 'completed'
GROUP BY p.product_id, p.product_code, p.product_name;
```

---

## 7. 模式应用案例 / Pattern Application Examples

### 7.1 案例1: CRM系统（Party模式）

**CRM系统完整实现**:

```sql
-- CRM系统：使用Party模式统一管理客户、联系人、供应商
CREATE TABLE party (
    party_id BIGSERIAL PRIMARY KEY,
    party_type CHAR(1) NOT NULL CHECK (party_type IN ('P', 'O')),
    name TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
) PARTITION BY LIST (party_type);

CREATE TABLE party_person PARTITION OF party FOR VALUES IN ('P');
CREATE TABLE party_organization PARTITION OF party FOR VALUES IN ('O');

-- 客户角色
CREATE TABLE party_role (
    party_role_id BIGSERIAL PRIMARY KEY,
    party_id BIGINT NOT NULL REFERENCES party(party_id),
    role_type VARCHAR(50) NOT NULL,
    valid_from TIMESTAMPTZ DEFAULT NOW(),
    valid_to TIMESTAMPTZ,
    UNIQUE(party_id, role_type, valid_from)
);

-- 客户联系信息
CREATE TABLE contact_mechanism (
    contact_id BIGSERIAL PRIMARY KEY,
    party_id BIGINT NOT NULL REFERENCES party(party_id),
    contact_type VARCHAR(50) NOT NULL, -- 'email', 'phone', 'address'
    contact_value TEXT NOT NULL,
    is_primary BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- CRM业务表
CREATE TABLE opportunity (
    opportunity_id BIGSERIAL PRIMARY KEY,
    customer_party_id BIGINT NOT NULL REFERENCES party(party_id),
    title VARCHAR(200) NOT NULL,
    amount NUMERIC(10,2),
    stage VARCHAR(50) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 查询：获取所有客户及其联系信息
SELECT
    p.party_id,
    p.name,
    pr.role_type,
    cm.contact_type,
    cm.contact_value
FROM party p
JOIN party_role pr ON p.party_id = pr.party_id
LEFT JOIN contact_mechanism cm ON p.party_id = cm.party_id AND cm.is_primary = TRUE
WHERE pr.role_type = 'customer'
  AND (pr.valid_to IS NULL OR pr.valid_to > NOW());
```

### 7.2 案例2: 电商系统（Party+Order+Product模式）

**电商系统完整实现**:

```sql
-- 电商系统：组合使用Party、Order、Product模式
-- Party模式：管理用户和商家
CREATE TABLE ecommerce_party (
    party_id BIGSERIAL PRIMARY KEY,
    party_type CHAR(1) NOT NULL CHECK (party_type IN ('P', 'O')),
    name TEXT NOT NULL,
    email VARCHAR(100) UNIQUE,
    phone VARCHAR(20),
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Product模式：商品管理
CREATE TABLE product (
    product_id BIGSERIAL PRIMARY KEY,
    product_code VARCHAR(50) UNIQUE NOT NULL,
    product_name VARCHAR(200) NOT NULL,
    description TEXT,
    category_id INT,
    price NUMERIC(10,2) NOT NULL,
    stock_quantity INT DEFAULT 0,
    status VARCHAR(20) DEFAULT 'active',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Order模式：订单管理
CREATE TABLE ecommerce_order (
    order_id BIGSERIAL PRIMARY KEY,
    order_number VARCHAR(50) UNIQUE NOT NULL,
    customer_party_id BIGINT NOT NULL REFERENCES ecommerce_party(party_id),
    seller_party_id BIGINT REFERENCES ecommerce_party(party_id),
    order_date TIMESTAMPTZ DEFAULT NOW(),
    total_amount NUMERIC(10,2) NOT NULL,
    order_status VARCHAR(20) DEFAULT 'pending',
    shipping_address JSONB,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE order_item (
    item_id BIGSERIAL PRIMARY KEY,
    order_id BIGINT NOT NULL REFERENCES ecommerce_order(order_id),
    product_id BIGINT NOT NULL REFERENCES product(product_id),
    quantity INT NOT NULL CHECK (quantity > 0),
    unit_price NUMERIC(10,2) NOT NULL,
    line_total NUMERIC(10,2) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 查询：获取客户订单统计
SELECT
    p.party_id,
    p.name,
    COUNT(DISTINCT o.order_id) AS order_count,
    SUM(o.total_amount) AS total_spent,
    AVG(o.total_amount) AS avg_order_amount
FROM ecommerce_party p
JOIN ecommerce_order o ON p.party_id = o.customer_party_id
WHERE o.order_status = 'completed'
GROUP BY p.party_id, p.name
ORDER BY total_spent DESC;
```

### 7.3 案例3: 银行系统（Account模式）

**银行系统完整实现**:

```sql
-- 银行系统：使用Account模式管理账户
CREATE TABLE bank_party (
    party_id BIGSERIAL PRIMARY KEY,
    party_type CHAR(1) NOT NULL CHECK (party_type IN ('P', 'O')),
    name TEXT NOT NULL,
    id_number VARCHAR(50) UNIQUE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Account模式：账户管理
CREATE TABLE account (
    account_id BIGSERIAL PRIMARY KEY,
    account_number VARCHAR(50) UNIQUE NOT NULL,
    party_id BIGINT NOT NULL REFERENCES bank_party(party_id),
    account_type VARCHAR(50) NOT NULL, -- 'savings', 'checking', 'credit'
    balance NUMERIC(15,2) NOT NULL DEFAULT 0,
    currency VARCHAR(3) DEFAULT 'CNY',
    status VARCHAR(20) DEFAULT 'active',
    opened_date DATE DEFAULT CURRENT_DATE,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 账户交易
CREATE TABLE account_transaction (
    transaction_id BIGSERIAL PRIMARY KEY,
    account_id BIGINT NOT NULL REFERENCES account(account_id),
    transaction_type VARCHAR(20) NOT NULL, -- 'deposit', 'withdrawal', 'transfer'
    amount NUMERIC(15,2) NOT NULL,
    balance_after NUMERIC(15,2) NOT NULL,
    transaction_date TIMESTAMPTZ DEFAULT NOW(),
    description TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 查询：获取账户余额和交易历史
SELECT
    a.account_number,
    p.name AS account_holder,
    a.account_type,
    a.balance,
    COUNT(at.transaction_id) AS transaction_count,
    MAX(at.transaction_date) AS last_transaction_date
FROM account a
JOIN bank_party p ON a.party_id = p.party_id
LEFT JOIN account_transaction at ON a.account_id = at.account_id
WHERE a.status = 'active'
GROUP BY a.account_id, a.account_number, p.name, a.account_type, a.balance;
```

---

## 8. 常见问题解答 / FAQ

### Q1: 什么时候应该使用Party模式？

**A**: Party模式适用场景：

- ✅ **统一管理**: 需要统一管理人员和组织
- ✅ **角色复杂**: 实体可能扮演多个角色
- ✅ **关系复杂**: 实体之间存在复杂关系
- ✅ **扩展需求**: 未来可能添加新的实体类型

**不适用场景**:

- ❌ **简单场景**: 只有单一类型的实体（如只有用户）
- ❌ **性能敏感**: 需要极致查询性能的场景
- ❌ **结构固定**: 实体结构非常固定，不会变化

### Q2: Order模式和Account模式有什么区别？

**A**: 区别对比：

| 特性 | Order模式 | Account模式 |
|------|-----------|-------------|
| 用途 | 管理订单和交易 | 管理账户和余额 |
| 核心实体 | Order（订单） | Account（账户） |
| 状态管理 | 订单状态流转 | 账户余额变化 |
| 适用场景 | 电商、零售 | 银行、金融 |

**选择原则**:

- 需要管理订单流程 → 使用Order模式
- 需要管理账户余额 → 使用Account模式

### Q3: 如何组合使用多个模式？

**A**: 模式组合策略：

```sql
-- 示例：电商系统组合Party+Order+Product模式
-- 1. Party模式：管理用户和商家
CREATE TABLE party (...);

-- 2. Product模式：管理商品
CREATE TABLE product (...);

-- 3. Order模式：管理订单（引用Party和Product）
CREATE TABLE order_header (
    order_id BIGSERIAL PRIMARY KEY,
    customer_party_id BIGINT REFERENCES party(party_id),  -- 引用Party
    ...
);

CREATE TABLE order_line (
    line_id BIGSERIAL PRIMARY KEY,
    order_id BIGINT REFERENCES order_header(order_id),
    product_id BIGINT REFERENCES product(product_id),  -- 引用Product
    ...
);
```

### Q4: 模式设计如何平衡灵活性和性能？

**A**: 平衡策略：

1. **核心表规范化**: 保持核心表规范化
2. **扩展表灵活化**: 使用JSONB存储扩展信息
3. **查询优化**: 为常用查询创建索引和视图
4. **物化视图**: 预计算复杂查询结果

```sql
-- 示例：平衡灵活性和性能
-- 核心表：规范化设计
CREATE TABLE party (
    party_id BIGSERIAL PRIMARY KEY,
    party_type CHAR(1) NOT NULL,
    name TEXT NOT NULL
);

-- 扩展表：灵活设计
CREATE TABLE party_extension (
    party_id BIGINT PRIMARY KEY REFERENCES party(party_id),
    custom_attributes JSONB,  -- 灵活存储
    metadata JSONB
);

-- 查询视图：优化查询
CREATE MATERIALIZED VIEW party_summary AS
SELECT
    p.party_id,
    p.name,
    jsonb_object_keys(pe.custom_attributes) AS attribute_keys
FROM party p
LEFT JOIN party_extension pe ON p.party_id = pe.party_id;
```

### Q5: 如何迁移现有系统到标准模式？

**A**: 迁移策略：

1. **分析现有结构**: 识别现有表结构
2. **映射到模式**: 将现有表映射到标准模式
3. **逐步迁移**: 分阶段迁移数据
4. **双写验证**: 新旧系统并行运行验证

```sql
-- 迁移示例：将用户表迁移到Party模式
-- 步骤1：创建Party表
CREATE TABLE party_new (
    party_id BIGSERIAL PRIMARY KEY,
    party_type CHAR(1) DEFAULT 'P',
    name TEXT NOT NULL
);

-- 步骤2：迁移数据
INSERT INTO party_new (party_id, name)
SELECT user_id, username FROM users;

-- 步骤3：创建映射表
CREATE TABLE user_party_mapping (
    old_user_id BIGINT PRIMARY KEY,
    new_party_id BIGINT REFERENCES party_new(party_id)
);

-- 步骤4：更新外键引用
UPDATE orders o
SET customer_id = (
    SELECT new_party_id FROM user_party_mapping
    WHERE old_user_id = o.customer_id
);
```

---

## 8. 相关资源 / Related Resources

### 8.1 核心相关文档 / Core Related Documents

- [Party模型](../04-OLTP建模/Party模型.md) - Party模型详细说明
- [订单管理模型](../04-OLTP建模/订单管理模型.md) - 订单模型详细说明
- [范式化设计](../04-OLTP建模/范式化设计.md) - OLTP范式化设计
- [建模反模式](./建模反模式.md) - 反模式识别
- [约束设计](../08-PostgreSQL建模实践/约束设计.md) - 模式约束设计

### 8.2 理论基础 / Theoretical Foundation

- [Silverston数据模型资源手册](../02-权威资源与标准/Silverston数据模型资源手册.md) - 权威资源
- [范式理论](../01-数据建模理论基础/范式理论.md) - 数据库范式理论
- [ER模型](../01-数据建模理论基础/ER模型.md) - 实体-关系模型

### 8.3 实践指南 / Practical Guides

- [模式应用案例](#7-模式应用案例--pattern-application-examples) - 本文档的应用案例章节
- [性能优化](../08-PostgreSQL建模实践/性能优化.md) - 模式性能优化

### 8.4 应用案例 / Application Cases

- [电商数据模型案例](../10-综合应用案例/电商数据模型案例.md) - 电商建模模式应用
- [金融数据模型案例](../10-综合应用案例/金融数据模型案例.md) - 金融建模模式应用

### 8.5 参考资源 / Reference Resources

- [权威资源索引](../00-导航与索引/权威资源索引.md) - 权威资源列表
- [术语对照表](../00-导航与索引/术语对照表.md) - 术语对照
- [快速查找指南](../00-导航与索引/快速查找指南.md) - 快速查找工具

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
