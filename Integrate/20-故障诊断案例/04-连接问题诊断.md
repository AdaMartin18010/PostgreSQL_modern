...
BEGIN
    BEGIN
        ...
        LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '终止进程 % 失败: %', backend_record.pid, SQLERRM;
            END;
        END LOOP;

        RAISE NOTICE '已终止 % 个空闲事务连接', terminated_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '终止空闲事务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- 1. 优化慢查询
-- 使用EXPLAIN (ANALYZE, BUFFERS, TIMING)找到慢查询
-- 添加索引、优化JOIN
...
BEGIN
    ...
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE WARNING '权限不足，无法设置数据库参数';
        WHEN OTHERS THEN
            RAISE WARNING '设置空闲连接超时参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        FROM pg_stat_activity
        GROUP BY time;
        RAISE NOTICE '连接统计视图创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建连接统计视图失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
```sql
-- 检查PgBouncer统计（如果启用pg_stat_statements）（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    datname,
    usename,
