...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '应用名称已设置: my_app_v1.2.3';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置应用名称失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'pg_stat_io') THEN
            RAISE WARNING 'pg_stat_io视图不存在（可能需要PostgreSQL 16+）';
            RETURN;
        END IF;
        RAISE NOTICE '开始检查I/O统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
