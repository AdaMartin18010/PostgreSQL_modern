...
- name: failover_alerts
  ...
  - alert: SubscriptionError
    ...
    annotations:
      summary: "订阅错误"
      description: "订阅 {{ $labels.subscription_name }} 出现错误"
```
...
```sql
-- 故障切换监控视图
CREATE OR REPLACE VIEW v_failover_status AS
SELECT
    '主库状态'::TEXT AS check_item,
    CASE WHEN pg_is_in_recovery() THEN '从库' ELSE '主库' END AS status,
    CASE WHEN pg_is_in_recovery() THEN '正常' ELSE '正常' END AS health_status
UNION ALL
SELECT
    '复制延迟'::TEXT,
    COALESCE(replay_lag::TEXT, '0')::TEXT,
    CASE WHEN replay_lag < INTERVAL '1 minute' THEN '正常' ELSE '警告' END
...
BEGIN
    ...
    SELECT
        '复制延迟检查'::TEXT,
        COALESCE(replay_lag::TEXT, '0')::TEXT,
        CASE WHEN replay_lag < INTERVAL '1 minute' THEN '通过' ELSE '失败' END
    FROM pg_stat_replication
    LIMIT 1;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '切换前检查失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    ...
    SELECT
        '主库状态验证'::TEXT,
        CASE WHEN pg_is_in_recovery() THEN '从库' ELSE '主库' END,
        CASE WHEN pg_is_in_recovery() THEN '失败' ELSE '通过' END;
    ...
    SELECT
        '订阅状态验证'::TEXT,
        subname::TEXT,
        CASE WHEN subenabled THEN '通过' ELSE '失败' END
    FROM pg_subscription
    LIMIT 1;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '切换后验证失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
