...
> **📋 文档来源**: `MVCC-ACID-CAP\01-理论基础\事务模型\两阶段提交深度分析.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN;
-- 数据库1操作
UPDATE accounts_db1.accounts SET balance = balance - 100 WHERE id = 1;
PREPARE TRANSACTION 'transfer_001';

-- 数据库2操作
BEGIN;
UPDATE accounts_db2.accounts SET balance = balance + 100 WHERE id = 2;
PREPARE TRANSACTION 'transfer_001';

-- 如果都成功，提交
COMMIT PREPARED 'transfer_001';  -- 数据库1
COMMIT PREPARED 'transfer_001';  -- 数据库2
