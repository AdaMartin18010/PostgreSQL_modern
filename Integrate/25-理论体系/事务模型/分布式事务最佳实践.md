...
-- 步骤3：扣减账户
BEGIN;
UPDATE accounts SET balance = balance - 1000 WHERE user_id = 100 AND balance >= 1000;
COMMIT;
...
-- 补偿1：取消订单
DELETE FROM orders WHERE user_id = 100 AND amount = 1000 ORDER BY id DESC LIMIT 1;
```
...
```sql
-- 数据准备：扩展账户表以支持TCC模式
ALTER TABLE accounts ADD COLUMN IF NOT EXISTS frozen_balance DECIMAL(10, 2) NOT NULL DEFAULT 0;

-- 更新示例数据
UPDATE accounts SET frozen_balance = 0 WHERE user_id = 100;
...
WHERE user_id = 100 AND balance >= 1000;
-- 返回结果：成功或失败（通过检查受影响的行数）
COMMIT;
...
WHERE user_id = 100 AND frozen_balance >= 1000;
COMMIT;

-- Cancel阶段：取消转账（如果Try阶段成功但Confirm失败）
-- BEGIN;
-- UPDATE accounts SET frozen_balance = frozen_balance - 1000
-- WHERE user_id = 100;
-- COMMIT;
