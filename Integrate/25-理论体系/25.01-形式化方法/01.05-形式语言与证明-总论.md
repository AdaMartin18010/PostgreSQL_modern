...
-- 2. 验证原子性
BEGIN;
INSERT INTO accounts (account_name, balance) VALUES ('TestAccount', 1000);
-- 模拟错误
ROLLBACK;
-- 验证：数据未插入
SELECT * FROM accounts WHERE account_name = 'TestAccount';  -- 应该返回空结果

-- 3. 验证一致性
BEGIN;
-- 先插入数据
INSERT INTO accounts (account_name, balance) VALUES ('TestAccount2', 1000);
-- 添加约束
ALTER TABLE accounts ADD CONSTRAINT check_balance CHECK (balance >= 0);
-- 尝试违反约束
UPDATE accounts SET balance = balance - 2000 WHERE account_name = 'TestAccount2';
-- 验证：余额不能为负（应该失败）
COMMIT;
...
BEGIN;
SELECT * FROM accounts WHERE account_name = 'TestAccount2';
COMMIT;
```
