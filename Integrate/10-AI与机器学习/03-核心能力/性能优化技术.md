...
flowchart TD
    ...
    F -->|小规模<100万| I[HNSW索引<br/>m=16, ef=64]
    F -->|大规模>1000万| J[IVFFlat索引<br/>lists=100]

    I --> K{召回率要求?}
    K -->|高>0.95| L[增加ef_search=100]
    K -->|中0.85-0.95| M[默认ef_search=40]
```
...
flowchart TD
    A[性能问题] --> B{检查CPU?}
    B -->|CPU高| C[查询优化<br/>索引优化]
    B -->|CPU低| D{检查IO?}
    D -->|IO高| E[存储优化<br/>缓存优化]
    D -->|IO低| F{检查内存?}
    F -->|内存不足| G[增加内存<br/>调整参数]
    F -->|内存充足| H[连接池优化<br/>并发优化]
```
