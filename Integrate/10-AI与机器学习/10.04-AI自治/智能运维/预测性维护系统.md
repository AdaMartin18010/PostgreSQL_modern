...
> **📋 文档来源**: `PostgreSQL_View\02-AI自治与自优化\智能运维\预测性维护系统.md`
> **📅 复制日期**: 2025-12-22
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '创建表metrics失败: %', SQLERRM;
        RAISE;
END $$;

-- 转换为 Hypertable（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Hypertable metrics 创建成功';
    ELSE
        RAISE NOTICE 'Hypertable metrics 已存在';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        AND indexname = 'idx_metrics_name_time'
    ) THEN
        CREATE INDEX idx_metrics_name_time ON metrics (metric_name, time DESC);
        RAISE NOTICE '索引 idx_metrics_name_time 创建成功';
    ELSE
        RAISE NOTICE '索引 idx_metrics_name_time 已存在';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
...
```sql
-- 使用 TimescaleDB 连续聚合（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_matviews
        WHERE schemaname = 'public' AND matviewname = 'metrics_5min'
    ) THEN
        DROP MATERIALIZED VIEW metrics_5min CASCADE;
        RAISE NOTICE '已删除现有物化视图: metrics_5min';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '删除物化视图失败: %', SQLERRM;
END $$;
