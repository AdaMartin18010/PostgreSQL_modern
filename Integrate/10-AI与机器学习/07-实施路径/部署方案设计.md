---
> **📋 文档来源**: `PostgreSQL_AI\21-最佳实践\部署方案设计.md`
> **📅 复制日期**: 2025-12-22
...
```mermaid
graph TD
    Start[开始选择部署方案] --> Q1{是否需要高可用?}
    Q1 -->|是| Q2{是否需要自动扩缩容?}
    Q1 -->|否| Q3{数据量规模?}
    Q2 -->|是| Serverless[Serverless部署<br/>Neon/Supabase]
    Q2 -->|否| HA[高可用部署<br/>主从+自动故障转移]
    Q3 -->|小规模<1TB| Standard[标准部署<br/>单机/容器]
    Q3 -->|大规模>1TB| Scale[高可用+分片<br/>Kubernetes]
...
graph TB
    App[应用层]
    LB[负载均衡器<br/>PgBouncer]
    Primary[(主库<br/>PostgreSQL)]
    Standby1[(从库1<br/>PostgreSQL)]
    Standby2[(从库2<br/>PostgreSQL)]
