...
- [PostgreSQL 逻辑回归完整指南](#postgresql-逻辑回归完整指南)
  ...
  - [逻辑回归概述](#逻辑回归概述)
    - [核心算法](#核心算法)
  - [1. 二分类逻辑回归](#1-二分类逻辑回归)
    - [1.1 Sigmoid函数实现](#11-sigmoid函数实现)
  - [2. 多分类逻辑回归](#2-多分类逻辑回归)
    - [2.1 Softmax函数实现](#21-softmax函数实现)
  - [3. 模型评估](#3-模型评估)
    - [3.1 混淆矩阵](#31-混淆矩阵)
    - [3.2 ROC曲线](#32-roc曲线)
  - [4. 实际应用案例](#4-实际应用案例)
    - [4.1 用户流失预测](#41-用户流失预测)

---

## 逻辑回归概述

**逻辑回归**用于二分类和多分类问题，通过Sigmoid函数将线性回归结果映射到概率空间。
