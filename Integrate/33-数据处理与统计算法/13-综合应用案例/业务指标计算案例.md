...
**PostgreSQL 18**引入了多项增强功能，可以显著提升业务指标计算的性能：

1. **Skip Scan优化**：
   - 对于包含指标值的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N指标查询和时间范围查询

2. **异步I/O增强**：
   - 对于大规模业务指标计算，异步I/O可以显著提升性能
   - 适用于批量指标计算和实时仪表板
...
**示例：使用Skip Scan优化Top-K指标查询**

```sql
-- 为业务指标创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_business_metrics_skip_scan
ON business_metrics(metric_date DESC, metric_value DESC);

-- Skip Scan优化查询：查找指标值最高的前10个日期
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    metric_date,
    metric_name,
...
对于实时仪表板，使用物化视图缓存关键指标：

```sql
-- 创建物化视图缓存实时仪表板数据
CREATE MATERIALIZED VIEW IF NOT EXISTS dashboard_metrics_cache AS
SELECT
    DATE_TRUNC('hour', order_date) AS metric_hour,
    DATE_TRUNC('day', order_date) AS metric_date,
    -- 收入指标
    SUM(order_amount) FILTER (WHERE order_status = 'completed') AS hourly_revenue,
    COUNT(*) FILTER (WHERE order_status = 'completed') AS hourly_orders,
    COUNT(DISTINCT user_id) FILTER (WHERE order_status = 'completed') AS hourly_active_users,
    -- 使用窗口函数计算累计指标（避免重复计算）
    SUM(SUM(order_amount) FILTER (WHERE order_status = 'completed')) OVER (
        PARTITION BY DATE_TRUNC('day', order_date)
        ORDER BY DATE_TRUNC('hour', order_date)
    ) AS daily_cumulative_revenue,
    -- 使用窗口函数计算增长率（避免重复计算）
    CASE
        WHEN LAG(SUM(order_amount) FILTER (WHERE order_status = 'completed')) OVER (
            PARTITION BY DATE_TRUNC('day', order_date)
            ORDER BY DATE_TRUNC('hour', order_date)
        ) > 0 THEN
            ROUND((
                (SUM(order_amount) FILTER (WHERE order_status = 'completed') -
                 LAG(SUM(order_amount) FILTER (WHERE order_status = 'completed')) OVER (
                     PARTITION BY DATE_TRUNC('day', order_date)
                     ORDER BY DATE_TRUNC('hour', order_date)
                 )) / LAG(SUM(order_amount) FILTER (WHERE order_status = 'completed')) OVER (
                     PARTITION BY DATE_TRUNC('day', order_date)
        ...
        ELSE NULL
    END AS hourly_revenue_growth_pct
FROM business_orders
WHERE order_date >= CURRENT_DATE - INTERVAL '7 days'
GROUP BY DATE_TRUNC('hour', order_date), DATE_TRUNC('day', order_date);

-- 创建索引加速物化视图查询
CREATE INDEX idx_dashboard_metrics_cache_hour ON dashboard_metrics_cache(metric_hour DESC);
...
**实时业务监控**：使用增量方法实时更新业务指标。

```sql
-- 实时业务监控：增量指标更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'business_metrics_state') THEN
            CREATE TABLE business_metrics_state (
            ...
            CREATE INDEX idx_business_metrics_state_date ON business_metrics_state(metric_date DESC);
            CREATE INDEX idx_business_metrics_state_name_value ON business_metrics_state(metric_name, metric_value DESC);

            RAISE NOTICE '业务指标状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量指标更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量指标更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH new_metrics AS (
    SELECT
        ...
        SUM(order_amount) FILTER (WHERE order_status = 'completed') AS metric_value
    FROM business_orders
    WHERE order_date > (SELECT COALESCE(MAX(metric_date), '1970-01-01') FROM business_metrics_state WHERE metric_name = 'Revenue')
      AND order_date <= CURRENT_DATE
    GROUP BY DATE_TRUNC('day', order_date)
)
-- 更新或插入业务指标
INSERT INTO business_metrics_state (metric_name, metric_date, metric_value, last_updated)
SELECT
    metric_name,
    metric_date,
    metric_value,
...
**业务指标预警**：检测业务指标异常并生成告警。

```sql
-- 业务指标预警：异常检测和告警（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'business_metrics') THEN
            RAISE WARNING '表 business_metrics 不存在，无法执行业务指标预警';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行业务指标预警';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '业务指标预警准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
metric_alerts AS (
    SELECT
        ...
        ms.q1_value,
        ms.q3_value,
        -- 使用窗口函数计算Z-score（避免重复计算）
        CASE
            WHEN ms.stddev_value > 0 THEN
                (bm.metric_value - ms.mean_value) / ms.stddev_value
            ELSE NULL
        END AS z_score,
        -- 使用IQR方法检测异常
        CASE
            WHEN bm.metric_value < ms.q1_value - 1.5 * (ms.q3_value - ms.q1_value) OR
                 bm.metric_value > ms.q3_value + 1.5 * (ms.q3_value - ms.q1_value) THEN 'Outlier'
            ELSE 'Normal'
        END AS iqr_status
    FROM business_metrics bm
    JOIN metric_statistics ms ON bm.metric_name = ms.metric_name
...
SELECT
    ...
    iqr_status,
    CASE
        WHEN ABS(z_score) > 3 THEN 'Critical Alert'  -- Z-score > 3
        WHEN ABS(z_score) > 2 THEN 'Warning Alert'   -- Z-score > 2
        WHEN iqr_status = 'Outlier' THEN 'Outlier Alert'
        ELSE 'Normal'
    END AS alert_level,
    CASE
        WHEN ABS(z_score) > 3 THEN '指标值显著偏离正常范围，需要立即关注'
        WHEN ABS(z_score) > 2 THEN '指标值偏离正常范围，需要关注'
        WHEN iqr_status = 'Outlier' THEN '指标值超出IQR范围，可能存在异常'
        ELSE '指标正常'
    END AS alert_message
FROM metric_alerts
WHERE ABS(z_score) > 2 OR iqr_status = 'Outlier'
