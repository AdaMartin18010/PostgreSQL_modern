...
**PostgreSQL 18**引入了多项增强功能，可以显著提升文本相似度计算的性能：

1. **Skip Scan优化**：
   - 对于包含相似度得分的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N相似文档查询

2. **异步I/O增强**：
   - 对于大规模文本相似度计算，异步I/O可以显著提升性能
   - 适用于批量相似度计算和文档聚类
...
**示例：使用Skip Scan优化相似文档查询**

```sql
-- 为文本相似度创建Skip Scan优化索引（使用表达式索引）
CREATE INDEX IF NOT EXISTS idx_text_similarity_skip_scan
ON documents USING btree((similarity(title, 'search_term')) DESC)
WHERE similarity(title, 'search_term') > 0.3;

-- Skip Scan优化查询：查找相似度最高的前10个文档
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    document_id,
    title,
...
对于频繁计算的文档相似度，使用物化视图缓存结果：

```sql
-- 创建物化视图缓存文档相似度矩阵
CREATE MATERIALIZED VIEW IF NOT EXISTS document_similarity_cache AS
WITH document_pairs AS (
    ...
    WHERE d1.document_id < d2.document_id
      AND d1.category = d2.category  -- 只在同类别内计算相似度
)
SELECT
    dp.doc1_id,
    dp.doc2_id,
    similarity(d1.title, d2.title) AS title_similarity,
    similarity(d1.content, d2.content) AS content_similarity,
    (similarity(d1.title, d2.title) * 0.4 + similarity(d1.content, d2.content) * 0.6) AS combined_similarity
FROM document_pairs dp
JOIN documents d1 ON dp.doc1_id = d1.document_id
JOIN documents d2 ON dp.doc2_id = d2.document_id;

-- 创建索引加速物化视图查询
CREATE INDEX idx_doc_similarity_combined ON document_similarity_cache(combined_similarity DESC);
CREATE INDEX idx_doc_similarity_doc1 ON document_similarity_cache(doc1_id, combined_similarity DESC);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY document_similarity_cache;
```
...
```sql
-- 为高相似度文档创建部分索引
CREATE INDEX IF NOT EXISTS idx_high_similarity_documents
ON documents USING GIN(to_tsvector('english', title))
WHERE similarity(title, 'common_term') > 0.7;

-- 为热门文档创建部分索引
CREATE INDEX IF NOT EXISTS idx_popular_documents_similarity
ON documents USING btree((similarity(title, 'popular_query')) DESC)
WHERE view_count > 1000;
```

**3. 语义相似度搜索：pgvector + 文本相似度混合搜索**

结合pgvector向量搜索和传统文本相似度的混合搜索：

```sql
-- 语义相似度搜索：pgvector + 文本相似度混合搜索（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 确保pgvector扩展已安装
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_vectors') THEN
            CREATE TABLE document_vectors (
                ...
                title TEXT NOT NULL,
                content TEXT,
                title_vector vector(384),  -- 使用384维向量（例如sentence-transformers）
                content_vector vector(384),
                view_count INTEGER DEFAULT 0,
                category VARCHAR(50)
            );

            CREATE INDEX idx_document_vectors_title_vector ON document_vectors
            USING ivfflat (title_vector vector_cosine_ops) WITH (lists = 100);

            RAISE NOTICE '文档向量表创建成功';
        END IF;

        RAISE NOTICE '开始执行语义相似度混合搜索';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '语义相似度搜索准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
vector_similarity_results AS (
    SELECT
        dv.document_id,
        dv.title,
        dv.content,
        1 - (dv.title_vector <=> (SELECT title_vector FROM target_vector)) AS vector_similarity
    FROM document_vectors dv
    WHERE dv.title_vector IS NOT NULL
    ORDER BY dv.title_vector <=> (SELECT title_vector FROM target_vector)
    LIMIT 50
),
text_similarity_results AS (
    SELECT
        dv.document_id,
        dv.title,
        dv.content,
        similarity(dv.title, (SELECT title FROM document_vectors WHERE document_id = 1 LIMIT 1)) AS text_similarity
    FROM document_vectors dv
    WHERE similarity(dv.title, (SELECT title FROM document_vectors WHERE document_id = 1 LIMIT 1)) > 0.3
    ORDER BY text_similarity DESC
    LIMIT 50
),
hybrid_results AS (
    SELECT
        COALESCE(vsr.document_id, tsr.document_id) AS document_id,
        COALESCE(vsr.title, tsr.title) AS title,
        COALESCE(vsr.content, tsr.content) AS content,
        COALESCE(vsr.vector_similarity, 0) AS vector_sim,
        COALESCE(tsr.text_similarity, 0) AS text_sim,
        -- 混合评分：向量相似度权重0.6，文本相似度权重0.4
        COALESCE(vsr.vector_similarity, 0) * 0.6 + COALESCE(tsr.text_similarity, 0) * 0.4 AS hybrid_score
    FROM vector_similarity_results vsr
    FULL OUTER JOIN text_similarity_results tsr ON vsr.document_id = tsr.document_id
)
SELECT
    document_id,
    title,
    ROUND(vector_sim::numeric, 4) AS vector_similarity,
    ROUND(text_sim::numeric, 4) AS text_similarity,
