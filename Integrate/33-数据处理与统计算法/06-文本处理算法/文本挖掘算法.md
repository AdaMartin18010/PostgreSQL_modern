...
- [PostgreSQL 文本挖掘算法完整指南](#postgresql-文本挖掘算法完整指南)
  ...
  - [4. 基于向量的主题聚类（pgvector）](#4-基于向量的主题聚类pgvector)
    - [4.1 向量主题聚类概述](#41-向量主题聚类概述)
      - [向量主题聚类原理](#向量主题聚类原理)
      - [pgvector扩展安装](#pgvector扩展安装)
    - [4.2 向量主题聚类实现](#42-向量主题聚类实现)
  - [5. 主题建模](#5-主题建模)
    - [5.1 主题建模原理](#51-主题建模原理)
      - [主题定义](#主题定义)
      - [LDA模型](#lda模型)
    - [5.2 简单主题提取](#52-简单主题提取)
  - [5. 文本挖掘优化策略](#5-文本挖掘优化策略)
    - [5.1 索引优化](#51-索引优化)
    - [5.2 批量处理优化](#52-批量处理优化)
    - [5.3 并行计算优化](#53-并行计算优化)
  - [6. PostgreSQL 18 并行文本挖掘增强](#6-postgresql-18-并行文本挖掘增强)
    - [6.1 并行文本挖掘原理](#61-并行文本挖掘原理)
    - [6.2 并行关键词提取](#62-并行关键词提取)
    - [6.3 并行TF-IDF计算](#63-并行tf-idf计算)
  - [7. 实际应用案例](#7-实际应用案例)
...
## 4. 基于向量的主题聚类（pgvector）

### 4.1 向量主题聚类概述

**基于向量的主题聚类（Vector-based Topic Clustering）**：使用pgvector扩展进行语义级别的文档聚类，通过向量相似度发现文档主题。

#### 向量主题聚类原理

**向量主题聚类**：将文档转换为向量嵌入（embedding），通过计算向量相似度进行主题聚类。
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_vectors') THEN
            RAISE WARNING '表 document_vectors 已存在，先删除';
...
```sql
-- 向量主题聚类：基于向量相似度聚类文档（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_vectors') THEN
            RAISE WARNING '表 document_vectors 不存在，无法执行向量主题聚类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行向量主题聚类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '向量主题聚类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 向量主题聚类：使用K-means思想进行聚类
...
nearest_neighbors AS (
    ...
    WHERE similarity > 0.7  -- 相似度阈值
    ORDER BY doc1_id, similarity DESC
),
-- 构建主题簇（简化版，实际应使用完整聚类算法）
topic_clusters AS (
    SELECT
...
## 6. PostgreSQL 18 并行文本挖掘增强

**PostgreSQL 18** 显著增强了并行文本挖掘能力，支持并行执行关键词提取、词频统计和TF-IDF计算，大幅提升大规模文本挖掘的性能。

### 6.1 并行文本挖掘原理

PostgreSQL 18 的并行文本挖掘通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描文本数据
2. **并行分词**：每个工作进程独立执行文本分词
3. **并行统计**：并行执行词频统计和TF-IDF计算
4. **结果合并**：主进程合并所有工作进程的挖掘结果

### 6.2 并行关键词提取

```sql
-- PostgreSQL 18 并行关键词提取（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'documents') THEN
            RAISE WARNING '表 documents 不存在，无法执行并行关键词提取';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行关键词提取';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行关键词提取准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- PostgreSQL 18 并行TF-IDF计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'documents') THEN
            RAISE WARNING '表 documents 不存在，无法执行并行TF-IDF计算';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行TF-IDF计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行TF-IDF计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升文本挖掘算法的性能：

1. **Skip Scan优化**：
   - 对于包含TF-IDF得分的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N关键词查询和热点话题查询

2. **异步I/O增强**：
   - 对于大规模文本挖掘计算，异步I/O可以显著提升性能
   - 适用于批量TF-IDF计算和主题建模
...
**1. 使用物化视图缓存TF-IDF计算结果**

对于频繁使用的TF-IDF结果，使用物化视图缓存：
...
WHERE tfidf_score > 0.1;  -- 只保留重要关键词
...
**2. 实时热点话题分析：增量TF-IDF计算**

**实时热点话题分析**：使用增量方法实时更新热点话题。

```sql
-- 实时热点话题分析：增量TF-IDF计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'topic_analysis_state') THEN
            ...
            RAISE NOTICE '话题分析状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行实时热点话题分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '实时热点话题分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
**3. 主题建模：基于TF-IDF的主题提取**

**主题建模**：使用TF-IDF进行简化的主题提取。

```sql
-- 主题建模：基于TF-IDF的主题提取（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_topics') THEN
            ...
            RAISE NOTICE '文档主题表创建成功';
        END IF;

        RAISE NOTICE '开始执行主题建模';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '主题建模准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH keyword_topics AS (
    SELECT
        keyword,
        -- 使用窗口函数计算关键词的重要性（避免重复计算）
        ROW_NUMBER() OVER (ORDER BY idf_score DESC, total_term_frequency DESC) AS topic_rank,
        CASE
            WHEN idf_score > 2.0 THEN 1  -- 高重要性主题
            WHEN idf_score > 1.0 THEN 2  -- 中等重要性主题
            ELSE 3  -- 低重要性主题
