...
- [PostgreSQL 文本分类算法完整指南](#postgresql-文本分类算法完整指南)
  ...
  - [3. 基于向量的分类（pgvector）](#3-基于向量的分类pgvector)
    - [3.1 向量分类概述](#31-向量分类概述)
      - [向量分类原理](#向量分类原理)
      - [pgvector扩展安装](#pgvector扩展安装)
    - [3.2 向量分类实现](#32-向量分类实现)
    - [3.3 向量分类性能优化](#33-向量分类性能优化)
  - [4. 文本特征提取](#4-文本特征提取)
    - [4.1 特征提取原理](#41-特征提取原理)
      - [特征定义](#特征定义)
      - [特征选择](#特征选择)
    - [4.2 特征向量提取实现](#42-特征向量提取实现)
    - [4.3 TF-IDF特征提取](#43-tf-idf特征提取)
  - [5. 分类优化策略](#5-分类优化策略)
    - [5.1 索引优化](#51-索引优化)
    - [5.2 规则优化](#52-规则优化)
    - [5.3 性能优化](#53-性能优化)
  - [6. 实际应用案例](#6-实际应用案例)
    - [6.1 文档自动分类](#61-文档自动分类)
    - [6.2 邮件分类系统](#62-邮件分类系统)
    - [6.3 新闻分类系统](#63-新闻分类系统)
    - [6.4 情感分析分类](#64-情感分析分类)
  - [7. 算法性能对比与优化](#7-算法性能对比与优化)
    - [7.1 分类方法对比](#71-分类方法对比)
    - [7.2 性能优化建议](#72-性能优化建议)
    - [7.3 常见问题与解决方案](#73-常见问题与解决方案)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 分类策略选择](#81-分类策略选择)
    - [8.2 特征工程](#82-特征工程)
    - [8.3 规则设计](#83-规则设计)
...
## 6. PostgreSQL 18 并行文本分类增强

**PostgreSQL 18** 显著增强了并行文本分类能力，支持并行执行关键词分类、规则分类和向量分类，大幅提升大规模文本分类的性能。

### 6.1 并行文本分类原理

PostgreSQL 18 的并行文本分类通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描文本数据
2. **并行特征提取**：每个工作进程独立提取文本特征
3. **并行分类**：并行执行分类算法
4. **结果合并**：主进程合并所有工作进程的分类结果

### 6.2 并行关键词分类

```sql
-- PostgreSQL 18 并行关键词分类（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'documents') THEN
            RAISE WARNING '表 documents 不存在，无法执行并行关键词分类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行关键词分类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行关键词分类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- PostgreSQL 18 并行向量分类（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        CREATE EXTENSION IF NOT EXISTS vector;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_classification') THEN
            RAISE WARNING '表 document_classification 不存在，无法执行并行向量分类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行向量分类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行向量分类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升文本分类算法的性能：

1. **Skip Scan优化**：
   - 对于包含分类标签的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N分类查询和类别筛选

2. **异步I/O增强**：
   - 对于大规模文本分类计算，异步I/O可以显著提升性能
   - 适用于批量文本分类和特征提取
...
WHERE max_score > 0.1;  -- 只保留置信度较高的分类
...
**增量文本分类**：对于流式文档，使用增量方法进行分类。

```sql
-- 增量文本分类：流式文档分类（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'document_classification_state') THEN
            CREATE TABLE document_classification_state (
                ...
                keyword_weights JSONB NOT NULL,  -- 存储关键词权重
                document_count INTEGER DEFAULT 0,
                last_updated TIMESTAMPTZ DEFAULT NOW()
            );

            RAISE NOTICE '文档分类状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量文本分类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量文本分类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
**多标签文本分类**：一个文档可以属于多个类别。

```sql
-- 多标签文本分类：文档多类别分类（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'documents') THEN
            RAISE WARNING '表 documents 不存在，无法执行多标签文本分类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行多标签文本分类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多标签文本分类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
