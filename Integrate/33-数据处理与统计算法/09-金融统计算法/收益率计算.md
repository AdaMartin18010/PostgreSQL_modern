# PostgreSQL 收益率计算完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 金融计算 | 收益率
> **难度级别**: ⭐⭐⭐ (中级)

---

## 📋 目录

- [PostgreSQL 收益率计算完整指南](#postgresql-收益率计算完整指南)
  - [📋 目录](#-目录)
  - [收益率计算概述](#收益率计算概述)
    - [理论基础](#理论基础)
      - [收益率问题定义](#收益率问题定义)
      - [收益率类型分类](#收益率类型分类)
      - [收益率计算方法选择](#收益率计算方法选择)
    - [核心指标](#核心指标)
  - [1. 简单收益率](#1-简单收益率)
    - [1.1 简单收益率原理](#11-简单收益率原理)
      - [简单收益率定义](#简单收益率定义)
      - [简单收益率公式](#简单收益率公式)
    - [1.2 日收益率计算实现](#12-日收益率计算实现)
    - [1.3 周收益率和月收益率](#13-周收益率和月收益率)
    - [1.4 多期简单收益率](#14-多期简单收益率)
  - [2. 对数收益率](#2-对数收益率)
    - [2.1 对数收益率原理](#21-对数收益率原理)
      - [对数收益率定义](#对数收益率定义)
      - [对数收益率优势](#对数收益率优势)
    - [2.2 对数收益率计算实现](#22-对数收益率计算实现)
    - [2.3 对数收益率累计](#23-对数收益率累计)
  - [3. 累计收益率](#3-累计收益率)
    - [3.1 累计收益率原理](#31-累计收益率原理)
      - [累计收益率定义](#累计收益率定义)
      - [累计收益率计算](#累计收益率计算)
    - [3.2 累计收益率计算实现](#32-累计收益率计算实现)
    - [3.3 累计收益率分解](#33-累计收益率分解)
  - [4. 年化收益率](#4-年化收益率)
    - [4.1 年化收益率原理](#41-年化收益率原理)
      - [年化收益率定义](#年化收益率定义)
      - [年化收益率公式](#年化收益率公式)
    - [4.2 年化收益率计算实现](#42-年化收益率计算实现)
    - [4.3 不同期限年化](#43-不同期限年化)
  - [5. 收益率计算优化策略](#5-收益率计算优化策略)
    - [5.1 索引优化](#51-索引优化)
    - [5.2 窗口函数优化](#52-窗口函数优化)
    - [5.3 批量计算优化](#53-批量计算优化)
  - [6. PostgreSQL 18 并行收益率计算增强](#6-postgresql-18-并行收益率计算增强)
    - [6.1 并行收益率计算原理](#61-并行收益率计算原理)
    - [6.2 并行简单收益率计算](#62-并行简单收益率计算)
    - [6.3 并行对数收益率计算](#63-并行对数收益率计算)
    - [6.4 并行累计收益率计算](#64-并行累计收益率计算)
  - [7. 实际应用案例](#7-实际应用案例)
    - [6.1 投资组合收益率分析](#61-投资组合收益率分析)
    - [6.2 股票收益率对比分析](#62-股票收益率对比分析)
    - [6.3 基金业绩评估](#63-基金业绩评估)
    - [6.4 资产配置收益率计算](#64-资产配置收益率计算)
  - [8. 算法性能对比与优化](#8-算法性能对比与优化)
    - [7.1 收益率计算方法对比](#71-收益率计算方法对比)
    - [7.2 性能优化建议](#72-性能优化建议)
    - [7.3 常见问题与解决方案](#73-常见问题与解决方案)
  - [9. 最佳实践](#9-最佳实践)
    - [8.1 计算方法选择](#81-计算方法选择)
    - [8.2 数据质量保证](#82-数据质量保证)
    - [8.3 精度控制](#83-精度控制)
    - [8.4 SQL实现注意事项](#84-sql实现注意事项)
    - [8.5 PostgreSQL 18 新特性应用](#85-postgresql-18-新特性应用)
    - [8.6 高级优化技巧](#86-高级优化技巧)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 收益率计算概述

**收益率计算（Return Calculation）**是衡量投资盈利能力的基础指标，是金融分析和投资决策的核心。

### 理论基础

#### 收益率问题定义

**收益率问题**：给定资产价格序列$P = \{P_0, P_1, \ldots, P_T\}$，计算收益率序列$R = \{r_1, r_2, \ldots, r_T\}$：
$$r_t = f(P_t, P_{t-1})$$

其中$f$是收益率计算函数。

#### 收益率类型分类

收益率类型分类：

1. **简单收益率（Simple Return）**：
   - 公式：$r_t = \frac{P_t - P_{t-1}}{P_{t-1}}$
   - 特点：直观、易于理解
   - 适用：单期收益计算

2. **对数收益率（Log Return）**：
   - 公式：$r_t = \ln\left(\frac{P_t}{P_{t-1}}\right)$
   - 特点：时间可加性、对称性
   - 适用：时间序列分析、多期收益

3. **累计收益率（Cumulative Return）**：
   - 公式：$R_T = \prod_{t=1}^{T}(1 + r_t) - 1$
   - 特点：多期收益汇总
   - 适用：投资组合评估

4. **年化收益率（Annualized Return）**：
   - 公式：$r_{annual} = (1 + r)^{n} - 1$（$n$为年化倍数）
   - 特点：标准化比较
   - 适用：不同期限收益比较

#### 收益率计算方法选择

选择计算方法的考虑因素：

1. **计算目的**：单期收益用简单收益率，多期收益用对数收益率
2. **数据特性**：价格数据用简单收益率，收益率数据用对数收益率
3. **分析需求**：时间序列分析用对数收益率，投资评估用累计收益率

### 核心指标

| 指标 | 数学定义 | 公式 | 用途 | 时间复杂度 | 适用场景 |
|------|---------|------|------|-----------|---------|
| **简单收益率** | $r_t = \frac{P_t - P_{t-1}}{P_{t-1}}$ | $(P_t - P_{t-1}) / P_{t-1}$ | 基础收益计算 | $O(1)$ | 单期收益 |
| **对数收益率** | $r_t = \ln\left(\frac{P_t}{P_{t-1}}\right)$ | $\ln(P_t / P_{t-1})$ | 时间序列分析 | $O(1)$ | 多期收益 |
| **累计收益率** | $R_T = \prod_{t=1}^{T}(1 + r_t) - 1$ | $\Pi(1 + r_i) - 1$ | 多期收益 | $O(n)$ | 投资评估 |
| **年化收益率** | $r_{annual} = (1 + r)^{n} - 1$ | $(1 + r)^n - 1$ | 标准化比较 | $O(1)$ | 收益比较 |

---

## 1. 简单收益率

### 1.1 简单收益率原理

**简单收益率（Simple Return）**是最直观的收益率计算方法。

#### 简单收益率定义

**简单收益率**：资产从时刻$t-1$到时刻$t$的收益率：
$$r_t = \frac{P_t - P_{t-1}}{P_{t-1}} = \frac{P_t}{P_{t-1}} - 1$$

**性质**：

- $r_t \in [-1, +\infty)$
- $r_t = 0$：价格不变
- $r_t > 0$：价格上涨
- $r_t < 0$：价格下跌

#### 简单收益率公式

**多期简单收益率**：从时刻$0$到时刻$T$的总收益率：
$$R_T = \prod_{t=1}^{T}(1 + r_t) - 1$$

### 1.2 日收益率计算实现

**日收益率**的实现和使用。

```sql
-- 创建股票价格表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 已存在，先删除';
            DROP TABLE stock_prices CASCADE;
        END IF;

        CREATE TABLE stock_prices (
            date DATE NOT NULL,
            symbol VARCHAR(10) NOT NULL,
            close_price NUMERIC(10, 2) NOT NULL,
            PRIMARY KEY (date, symbol)
        );

        -- 插入示例数据
        INSERT INTO stock_prices (date, symbol, close_price) VALUES
            ('2024-01-01', 'AAPL', 150.00),
            ('2024-01-02', 'AAPL', 152.50),
            ('2024-01-03', 'AAPL', 151.00),
            ('2024-01-04', 'AAPL', 153.75),
            ('2024-01-05', 'AAPL', 155.00),
            ('2024-01-01', 'GOOGL', 2800.00),
            ('2024-01-02', 'GOOGL', 2820.00),
            ('2024-01-03', 'GOOGL', 2810.00),
            ('2024-01-04', 'GOOGL', 2835.00),
            ('2024-01-05', 'GOOGL', 2850.00);

        RAISE NOTICE '表 stock_prices 创建成功，已插入10条数据';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 stock_prices 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- 计算日收益率（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算日收益率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 使用窗口函数计算日收益率
WITH price_changes AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price
    FROM stock_prices
)
SELECT
    date,
    symbol,
    close_price,
    prev_price,
    CASE
        WHEN prev_price IS NULL THEN NULL
        WHEN prev_price = 0 THEN NULL
        ELSE ROUND(((close_price - prev_price) / prev_price * 100)::numeric, 4)
    END AS daily_return_pct
FROM price_changes
WHERE prev_price IS NOT NULL
ORDER BY symbol, date;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    date,
    symbol,
    close_price,
    LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price
FROM stock_prices
ORDER BY symbol, date;
```

---

### 1.3 周收益率和月收益率

**周收益率和月收益率**：计算不同周期的收益率。

```sql
-- 周收益率和月收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算周收益率和月收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算周收益率和月收益率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '周收益率和月收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 周收益率：使用每周最后一个交易日
WITH weekly_prices AS (
    SELECT
        DATE_TRUNC('week', date) AS week_start,
        symbol,
        LAST_VALUE(close_price) OVER (
            PARTITION BY symbol, DATE_TRUNC('week', date)
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) AS week_end_price,
        FIRST_VALUE(close_price) OVER (
            PARTITION BY symbol, DATE_TRUNC('week', date)
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) AS week_start_price
    FROM stock_prices
),
weekly_returns AS (
    SELECT DISTINCT
        week_start,
        symbol,
        week_start_price,
        week_end_price,
        CASE
            WHEN week_start_price = 0 THEN NULL
            ELSE (week_end_price - week_start_price) / week_start_price
        END AS weekly_return
    FROM weekly_prices
)
SELECT
    week_start,
    symbol,
    week_start_price,
    week_end_price,
    ROUND(weekly_return::numeric, 6) AS weekly_return,
    ROUND((weekly_return * 100)::numeric, 2) AS weekly_return_pct
FROM weekly_returns
WHERE weekly_return IS NOT NULL
ORDER BY symbol, week_start;

-- 月收益率：使用每月最后一个交易日
WITH monthly_prices AS (
    SELECT
        DATE_TRUNC('month', date) AS month_start,
        symbol,
        LAST_VALUE(close_price) OVER (
            PARTITION BY symbol, DATE_TRUNC('month', date)
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) AS month_end_price,
        FIRST_VALUE(close_price) OVER (
            PARTITION BY symbol, DATE_TRUNC('month', date)
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) AS month_start_price
    FROM stock_prices
),
monthly_returns AS (
    SELECT DISTINCT
        month_start,
        symbol,
        month_start_price,
        month_end_price,
        CASE
            WHEN month_start_price = 0 THEN NULL
            ELSE (month_end_price - month_start_price) / month_start_price
        END AS monthly_return
    FROM monthly_prices
)
SELECT
    month_start,
    symbol,
    month_start_price,
    month_end_price,
    ROUND(monthly_return::numeric, 6) AS monthly_return,
    ROUND((monthly_return * 100)::numeric, 2) AS monthly_return_pct
FROM monthly_returns
WHERE monthly_return IS NOT NULL
ORDER BY symbol, month_start;
```

### 1.4 多期简单收益率

**多期简单收益率**：计算多期的总收益率。

```sql
-- 多期简单收益率（带错误处理和性能测试）
DO $$
DECLARE
    start_date DATE := '2024-01-01';
    end_date DATE := '2024-01-05';
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算多期简单收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算多期简单收益率，从 % 到 %', start_date, end_date;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多期简单收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 多期简单收益率：计算指定期间的总收益率
WITH period_prices AS (
    SELECT
        symbol,
        FIRST_VALUE(close_price) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) FILTER (WHERE date = start_date) AS start_price,
        LAST_VALUE(close_price) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
        ) FILTER (WHERE date = end_date) AS end_price
    FROM stock_prices
    WHERE date BETWEEN start_date AND end_date
)
SELECT DISTINCT
    symbol,
    start_price,
    end_price,
    CASE
        WHEN start_price = 0 THEN NULL
        ELSE (end_price - start_price) / start_price
    END AS period_return
FROM period_prices
WHERE start_price IS NOT NULL AND end_price IS NOT NULL
ORDER BY symbol;
```

---

## 2. 对数收益率

### 2.1 对数收益率原理

**对数收益率（Log Return）**使用自然对数计算收益率，具有时间可加性。

#### 对数收益率定义

**对数收益率**：资产从时刻$t-1$到时刻$t$的对数收益率：
$$r_t = \ln\left(\frac{P_t}{P_{t-1}}\right) = \ln(P_t) - \ln(P_{t-1})$$

**性质**：

- $r_t \in (-\infty, +\infty)$
- **时间可加性**：$r_{0,T} = \sum_{t=1}^{T} r_t$
- **对称性**：$r_t$和$-r_t$对称
- **近似性**：当$r_t$较小时，$\ln(1 + r_t) \approx r_t$

#### 对数收益率优势

**对数收益率优势**：

1. **时间可加性**：多期对数收益率等于各期对数收益率之和
2. **统计性质**：更接近正态分布
3. **计算便利**：多期收益计算更简单

### 2.2 对数收益率计算实现

**对数收益率**的实现和使用。

```sql
-- 对数收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算对数收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算对数收益率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '对数收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算对数收益率
WITH price_changes AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price
    FROM stock_prices
)
SELECT
    date,
    symbol,
    close_price,
    prev_price,
    CASE
        WHEN prev_price IS NULL THEN NULL
        WHEN prev_price <= 0 THEN NULL
        ELSE ROUND(LN(close_price / prev_price)::numeric, 6)
    END AS log_return
FROM price_changes
WHERE prev_price IS NOT NULL AND prev_price > 0
ORDER BY symbol, date;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    date,
    symbol,
    CASE
        WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
            LN(close_price / LAG(close_price) OVER (PARTITION BY symbol ORDER BY date))
        ELSE NULL
    END AS log_return
FROM stock_prices
ORDER BY symbol, date;
```

---

### 2.3 对数收益率累计

**对数收益率累计**：利用时间可加性计算多期收益。

```sql
-- 对数收益率累计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算对数收益率累计';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算对数收益率累计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '对数收益率累计计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 对数收益率累计：利用时间可加性
WITH log_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        LN(close_price / NULLIF(LAG(close_price) OVER (PARTITION BY symbol ORDER BY date), 0)) AS log_return
    FROM stock_prices
    WHERE LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0
),
cumulative_log_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        log_return,
        SUM(log_return) OVER (PARTITION BY symbol ORDER BY date) AS cumulative_log_return,
        EXP(SUM(log_return) OVER (PARTITION BY symbol ORDER BY date)) - 1 AS cumulative_return
    FROM log_returns
    WHERE log_return IS NOT NULL
)
SELECT
    date,
    symbol,
    close_price,
    ROUND(log_return::numeric, 6) AS log_return,
    ROUND(cumulative_log_return::numeric, 6) AS cumulative_log_return,
    ROUND(cumulative_return::numeric, 6) AS cumulative_return,
    ROUND((cumulative_return * 100)::numeric, 2) AS cumulative_return_pct
FROM cumulative_log_returns
ORDER BY symbol, date;
```

---

## 3. 累计收益率

### 3.1 累计收益率原理

**累计收益率（Cumulative Return）**计算多期投资的总体收益。

#### 累计收益率定义

**累计收益率**：从时刻$0$到时刻$T$的累计收益率：
$$R_T = \prod_{t=1}^{T}(1 + r_t) - 1$$

**对数形式**：使用对数收益率时：
$$R_T = \exp\left(\sum_{t=1}^{T} r_t\right) - 1$$

#### 累计收益率计算

**计算方法**：

1. **简单收益率累计**：$R_T = \prod_{t=1}^{T}(1 + r_t) - 1$
2. **对数收益率累计**：$R_T = \exp\left(\sum_{t=1}^{T} r_t\right) - 1$

### 3.2 累计收益率计算实现

**累计收益率**的实现和使用。

```sql
-- 累计收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算累计收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算累计收益率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '累计收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算累计收益率（使用简单收益率）
WITH daily_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) IS NULL THEN NULL
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
        END AS daily_return
    FROM stock_prices
),
cumulative_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        daily_return,
        EXP(SUM(LN(1 + COALESCE(daily_return, 0))) OVER (PARTITION BY symbol ORDER BY date)) - 1 AS cumulative_return
    FROM daily_returns
    WHERE daily_return IS NOT NULL
)
SELECT
    date,
    symbol,
    close_price,
    ROUND(COALESCE(daily_return, 0)::numeric, 6) AS daily_return,
    ROUND(cumulative_return::numeric, 6) AS cumulative_return,
    ROUND((cumulative_return * 100)::numeric, 2) AS cumulative_return_pct
FROM cumulative_returns
ORDER BY symbol, date;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    date,
    symbol,
    EXP(SUM(LN(1 + COALESCE(
        (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
        NULLIF(LAG(close_price) OVER (PARTITION BY symbol ORDER BY date), 0),
        0
    ))) OVER (PARTITION BY symbol ORDER BY date)) - 1 AS cumulative_return
FROM stock_prices
ORDER BY symbol, date;
```

---

### 3.3 累计收益率分解

**累计收益率分解**：分析各期对累计收益的贡献。

```sql
-- 累计收益率分解（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算累计收益率分解';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算累计收益率分解';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '累计收益率分解计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 累计收益率分解：分析各期贡献
WITH daily_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
        END AS daily_return
    FROM stock_prices
),
cumulative_analysis AS (
    SELECT
        date,
        symbol,
        close_price,
        daily_return,
        EXP(SUM(LN(1 + COALESCE(daily_return, 0))) OVER (PARTITION BY symbol ORDER BY date)) - 1 AS cumulative_return,
        ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY date) AS period_num
    FROM daily_returns
    WHERE daily_return IS NOT NULL
)
SELECT
    date,
    symbol,
    ROUND(daily_return::numeric, 6) AS daily_return,
    ROUND((daily_return * 100)::numeric, 2) AS daily_return_pct,
    ROUND(cumulative_return::numeric, 6) AS cumulative_return,
    ROUND((cumulative_return * 100)::numeric, 2) AS cumulative_return_pct,
    period_num
FROM cumulative_analysis
ORDER BY symbol, date;
```

---

## 4. 年化收益率

### 4.1 年化收益率原理

**年化收益率（Annualized Return）**将不同期限的收益率标准化为年度收益率，便于比较。

#### 年化收益率定义

**年化收益率**：将期间收益率$r$年化为年度收益率：
$$r_{annual} = (1 + r)^{n} - 1$$

其中$n$是年化倍数（如252个交易日、12个月等）。

#### 年化收益率公式

**年化公式**：

- **日收益率年化**：$r_{annual} = (1 + r_{daily})^{252} - 1$
- **月收益率年化**：$r_{annual} = (1 + r_{monthly})^{12} - 1$
- **对数收益率年化**：$r_{annual} = r_{log} \times 252$

### 4.2 年化收益率计算实现

**年化收益率**的实现和使用。

```sql
-- 年化收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算年化收益率';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算年化收益率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '年化收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算年化收益率
WITH period_stats AS (
    SELECT
        symbol,
        MIN(date) AS start_date,
        MAX(date) AS end_date,
        COUNT(*) - 1 AS trading_days,
        FIRST_VALUE(close_price) OVER (PARTITION BY symbol ORDER BY date) AS start_price,
        LAST_VALUE(close_price) OVER (PARTITION BY symbol ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS end_price
    FROM stock_prices
    GROUP BY symbol
),
total_return AS (
    SELECT
        symbol,
        start_date,
        end_date,
        trading_days,
        start_price,
        end_price,
        CASE
            WHEN start_price = 0 THEN NULL
            ELSE (end_price - start_price) / start_price
        END AS total_return,
        -- 假设一年252个交易日
        252.0 AS trading_days_per_year
    FROM period_stats
)
SELECT
    symbol,
    start_date,
    end_date,
    trading_days,
    start_price,
    end_price,
    ROUND(total_return::numeric, 6) AS total_return,
    ROUND((total_return * 100)::numeric, 2) AS total_return_pct,
    CASE
        WHEN trading_days = 0 THEN NULL
        ELSE ROUND((POWER(1 + total_return, trading_days_per_year / trading_days) - 1)::numeric, 6)
    END AS annualized_return,
    ROUND((POWER(1 + total_return, trading_days_per_year / NULLIF(trading_days, 0)) - 1) * 100, 2) AS annualized_return_pct
FROM total_return
WHERE total_return IS NOT NULL
ORDER BY symbol;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    symbol,
    COUNT(*) AS trading_days,
    MIN(close_price) AS min_price,
    MAX(close_price) AS max_price
FROM stock_prices
GROUP BY symbol;
```

---

### 4.3 不同期限年化

**不同期限年化**：处理不同期限的收益率年化。

```sql
-- 不同期限年化（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法计算不同期限年化';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算不同期限年化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '不同期限年化计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 不同期限年化：日、周、月收益率年化
WITH period_returns AS (
    SELECT
        symbol,
        -- 日收益率
        AVG(CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
        END) AS avg_daily_return,
        -- 周收益率（简化版）
        AVG(CASE
            WHEN LAG(close_price, 5) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price, 5) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price, 5) OVER (PARTITION BY symbol ORDER BY date)
        END) AS avg_weekly_return,
        -- 月收益率（简化版）
        AVG(CASE
            WHEN LAG(close_price, 20) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price, 20) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price, 20) OVER (PARTITION BY symbol ORDER BY date)
        END) AS avg_monthly_return
    FROM stock_prices
    GROUP BY symbol
)
SELECT
    symbol,
    ROUND(avg_daily_return::numeric, 6) AS avg_daily_return,
    ROUND((POWER(1 + COALESCE(avg_daily_return, 0), 252) - 1)::numeric, 6) AS annualized_from_daily,
    ROUND(avg_weekly_return::numeric, 6) AS avg_weekly_return,
    ROUND((POWER(1 + COALESCE(avg_weekly_return, 0), 52) - 1)::numeric, 6) AS annualized_from_weekly,
    ROUND(avg_monthly_return::numeric, 6) AS avg_monthly_return,
    ROUND((POWER(1 + COALESCE(avg_monthly_return, 0), 12) - 1)::numeric, 6) AS annualized_from_monthly
FROM period_returns
ORDER BY symbol;
```

---

## 5. 收益率计算优化策略

### 5.1 索引优化

**索引优化**：为收益率计算创建合适的索引。

```sql
-- 索引优化（带错误处理）
DO $$
BEGIN
    BEGIN
        -- 价格表索引：加速时间序列查询
        CREATE INDEX IF NOT EXISTS idx_stock_prices_date_symbol
        ON stock_prices(date, symbol);

        CREATE INDEX IF NOT EXISTS idx_stock_prices_symbol_date
        ON stock_prices(symbol, date);

        RAISE NOTICE '收益率计算索引创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '索引创建失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 5.2 窗口函数优化

**窗口函数优化**：使用窗口函数优化收益率计算。

```sql
-- 窗口函数优化：使用物化视图缓存收益率
CREATE MATERIALIZED VIEW IF NOT EXISTS daily_returns_cache AS
SELECT
    date,
    symbol,
    close_price,
    LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price,
    CASE
        WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
        ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
             LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
    END AS daily_return
FROM stock_prices;

CREATE INDEX IF NOT EXISTS idx_daily_returns_cache_date_symbol
ON daily_returns_cache(date, symbol);

-- 刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY daily_returns_cache;
```

### 5.3 批量计算优化

**批量计算优化**：使用批量操作提高性能。

```sql
-- 批量计算优化（带错误处理）
DO $$
BEGIN
    BEGIN
        SET work_mem = '512MB';
        RAISE NOTICE 'work_mem已设置为512MB';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '批量计算优化失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 6. PostgreSQL 18 并行收益率计算增强

**PostgreSQL 18** 显著增强了并行收益率计算能力，支持并行执行简单收益率、对数收益率和累计收益率计算，大幅提升大规模金融数据收益率计算的性能。

### 6.1 并行收益率计算原理

PostgreSQL 18 的并行收益率计算通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描价格数据
2. **并行计算**：每个工作进程独立计算收益率
3. **并行聚合**：每个工作进程独立计算累计收益率
4. **结果合并**：主进程合并所有工作进程的计算结果

### 6.2 并行简单收益率计算

```sql
-- PostgreSQL 18 并行简单收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行并行简单收益率计算';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行简单收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行简单收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行简单收益率计算：日收益率
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH price_changes AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price
    FROM stock_prices
)
SELECT
    date,
    symbol,
    close_price,
    prev_price,
    CASE
        WHEN prev_price IS NULL OR prev_price <= 0 THEN NULL
        ELSE ROUND(((close_price - prev_price) / prev_price)::numeric, 6)
    END AS simple_return
FROM price_changes
WHERE prev_price IS NOT NULL AND prev_price > 0
ORDER BY symbol, date;
```

### 6.3 并行对数收益率计算

```sql
-- PostgreSQL 18 并行对数收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行并行对数收益率计算';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行对数收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行对数收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行对数收益率计算
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH price_changes AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price
    FROM stock_prices
)
SELECT
    date,
    symbol,
    close_price,
    prev_price,
    CASE
        WHEN prev_price IS NULL OR prev_price <= 0 THEN NULL
        ELSE ROUND(LN(close_price / prev_price)::numeric, 6)
    END AS log_return
FROM price_changes
WHERE prev_price IS NOT NULL AND prev_price > 0
ORDER BY symbol, date;
```

### 6.4 并行累计收益率计算

```sql
-- PostgreSQL 18 并行累计收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行并行累计收益率计算';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行累计收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行累计收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行累计收益率计算：使用对数收益率的可加性
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH log_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
                LN(close_price / LAG(close_price) OVER (PARTITION BY symbol ORDER BY date))
            ELSE NULL
        END AS log_return
    FROM stock_prices
),
cumulative_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        log_return,
        SUM(log_return) OVER (PARTITION BY symbol ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_log_return
    FROM log_returns
    WHERE log_return IS NOT NULL
)
SELECT
    date,
    symbol,
    close_price,
    ROUND(log_return::numeric, 6) AS log_return,
    ROUND(cumulative_log_return::numeric, 6) AS cumulative_log_return,
    ROUND((EXP(cumulative_log_return) - 1)::numeric, 6) AS cumulative_simple_return
FROM cumulative_returns
ORDER BY symbol, date;
```

---

## 7. 实际应用案例

### 6.1 投资组合收益率分析

**投资组合收益率分析**计算多只股票组合的整体收益。

```sql
-- 投资组合收益率分析示例（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'portfolio_weights') THEN
            RAISE WARNING '表 portfolio_weights 不存在，创建示例表';

            CREATE TABLE portfolio_weights (
                symbol VARCHAR(10) PRIMARY KEY,
                weight NUMERIC(5, 4) NOT NULL CHECK (weight >= 0 AND weight <= 1)
            );

            INSERT INTO portfolio_weights (symbol, weight) VALUES
                ('AAPL', 0.4),
                ('GOOGL', 0.6);

            RAISE NOTICE '表 portfolio_weights 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，请先创建';
            RETURN;
        END IF;

        RAISE NOTICE '开始执行投资组合收益率分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '投资组合分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算投资组合日收益率
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH daily_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) IS NULL THEN NULL
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
        END AS daily_return
    FROM stock_prices
),
portfolio_returns AS (
    SELECT
        dr.date,
        SUM(dr.daily_return * pw.weight) AS portfolio_return
    FROM daily_returns dr
    JOIN portfolio_weights pw ON dr.symbol = pw.symbol
    WHERE dr.daily_return IS NOT NULL
    GROUP BY dr.date
)
SELECT
    date,
    ROUND(portfolio_return::numeric, 6) AS portfolio_daily_return,
    ROUND((portfolio_return * 100)::numeric, 4) AS portfolio_daily_return_pct,
    ROUND(EXP(SUM(LN(1 + portfolio_return)) OVER (ORDER BY date)) - 1, 6) AS cumulative_return,
    ROUND((EXP(SUM(LN(1 + portfolio_return)) OVER (ORDER BY date)) - 1) * 100, 2) AS cumulative_return_pct
FROM portfolio_returns
ORDER BY date;
```

### 6.2 股票收益率对比分析

**股票收益率对比分析**：对比多只股票的收益率表现。

```sql
-- 股票收益率对比分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行股票收益率对比分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行股票收益率对比分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '股票收益率对比分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 股票收益率对比：计算各股票的收益率统计
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH daily_returns AS (
    SELECT
        date,
        symbol,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) = 0 THEN NULL
            ELSE (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                 LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
        END AS daily_return
    FROM stock_prices
),
return_stats AS (
    SELECT
        symbol,
        COUNT(*) AS trading_days,
        AVG(daily_return) AS mean_return,
        STDDEV(daily_return) AS std_return,
        MIN(daily_return) AS min_return,
        MAX(daily_return) AS max_return,
        PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY daily_return) AS percentile_5,
        PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY daily_return) AS percentile_95
    FROM daily_returns
    WHERE daily_return IS NOT NULL
    GROUP BY symbol
),
cumulative_returns AS (
    SELECT DISTINCT
        symbol,
        EXP(SUM(LN(1 + COALESCE(daily_return, 0))) OVER (PARTITION BY symbol ORDER BY date)) - 1 AS cumulative_return
    FROM daily_returns
    WHERE daily_return IS NOT NULL
    ORDER BY symbol, date DESC
    LIMIT 1000
)
SELECT
    rs.symbol,
    rs.trading_days,
    ROUND(rs.mean_return::numeric, 6) AS mean_daily_return,
    ROUND((rs.mean_return * 252)::numeric, 6) AS annualized_return,
    ROUND(rs.std_return::numeric, 6) AS daily_volatility,
    ROUND((rs.std_return * SQRT(252))::numeric, 6) AS annualized_volatility,
    ROUND(cr.cumulative_return::numeric, 6) AS total_return,
    ROUND(rs.min_return::numeric, 6) AS worst_day,
    ROUND(rs.max_return::numeric, 6) AS best_day
FROM return_stats rs
LEFT JOIN (
    SELECT DISTINCT ON (symbol)
        symbol,
        cumulative_return
    FROM cumulative_returns
) cr ON rs.symbol = cr.symbol
ORDER BY annualized_return DESC;
```

### 6.3 基金业绩评估

**基金业绩评估**：评估基金的收益率表现。

```sql
-- 基金业绩评估（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'fund_nav') THEN
            RAISE WARNING '表 fund_nav 不存在，创建示例表';

            CREATE TABLE fund_nav (
                date DATE NOT NULL,
                fund_code VARCHAR(10) NOT NULL,
                nav NUMERIC(10, 4) NOT NULL,
                PRIMARY KEY (date, fund_code)
            );

            INSERT INTO fund_nav (date, fund_code, nav) VALUES
                ('2024-01-01', 'FUND001', 1.0000),
                ('2024-01-02', 'FUND001', 1.0100),
                ('2024-01-03', 'FUND001', 1.0080),
                ('2024-01-04', 'FUND001', 1.0120),
                ('2024-01-05', 'FUND001', 1.0150);

            RAISE NOTICE '表 fund_nav 创建成功';
        END IF;

        RAISE NOTICE '开始执行基金业绩评估';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '基金业绩评估准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 基金业绩评估：计算基金收益率和排名
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH fund_returns AS (
    SELECT
        date,
        fund_code,
        nav,
        CASE
            WHEN LAG(nav) OVER (PARTITION BY fund_code ORDER BY date) = 0 THEN NULL
            ELSE (nav - LAG(nav) OVER (PARTITION BY fund_code ORDER BY date)) /
                 LAG(nav) OVER (PARTITION BY fund_code ORDER BY date)
        END AS daily_return
    FROM fund_nav
),
fund_performance AS (
    SELECT
        fund_code,
        COUNT(*) AS trading_days,
        AVG(daily_return) AS mean_return,
        STDDEV(daily_return) AS std_return,
        EXP(SUM(LN(1 + COALESCE(daily_return, 0))) OVER (PARTITION BY fund_code ORDER BY date)) - 1 AS cumulative_return
    FROM fund_returns
    WHERE daily_return IS NOT NULL
    GROUP BY fund_code
),
fund_rankings AS (
    SELECT
        fund_code,
        trading_days,
        ROUND((mean_return * 252)::numeric, 6) AS annualized_return,
        ROUND((std_return * SQRT(252))::numeric, 6) AS annualized_volatility,
        ROUND(cumulative_return::numeric, 6) AS total_return,
        CASE
            WHEN std_return = 0 THEN NULL
            ELSE ROUND((mean_return / std_return * SQRT(252))::numeric, 4)
        END AS sharpe_ratio,
        RANK() OVER (ORDER BY (mean_return * 252) DESC) AS return_rank,
        RANK() OVER (ORDER BY CASE WHEN std_return = 0 THEN NULL ELSE (mean_return / std_return * SQRT(252)) END DESC) AS sharpe_rank
    FROM fund_performance
)
SELECT
    fund_code,
    annualized_return,
    annualized_volatility,
    total_return,
    sharpe_ratio,
    return_rank,
    sharpe_rank
FROM fund_rankings
ORDER BY sharpe_rank;
```

### 6.4 资产配置收益率计算

**资产配置收益率计算**：计算不同资产类别的收益率。

```sql
-- 资产配置收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'asset_prices') THEN
            RAISE WARNING '表 asset_prices 不存在，创建示例表';

            CREATE TABLE asset_prices (
                date DATE NOT NULL,
                asset_class VARCHAR(20) NOT NULL,
                asset_code VARCHAR(10) NOT NULL,
                price NUMERIC(10, 2) NOT NULL,
                PRIMARY KEY (date, asset_class, asset_code)
            );

            INSERT INTO asset_prices (date, asset_class, asset_code, price) VALUES
                ('2024-01-01', 'Stock', 'AAPL', 150.00),
                ('2024-01-02', 'Stock', 'AAPL', 152.00),
                ('2024-01-01', 'Bond', 'BOND001', 100.00),
                ('2024-01-02', 'Bond', 'BOND001', 100.50);

            RAISE NOTICE '表 asset_prices 创建成功';
        END IF;

        RAISE NOTICE '开始执行资产配置收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '资产配置收益率计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 资产配置收益率：按资产类别计算收益率
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH asset_returns AS (
    SELECT
        date,
        asset_class,
        asset_code,
        price,
        CASE
            WHEN LAG(price) OVER (PARTITION BY asset_class, asset_code ORDER BY date) = 0 THEN NULL
            ELSE (price - LAG(price) OVER (PARTITION BY asset_class, asset_code ORDER BY date)) /
                 LAG(price) OVER (PARTITION BY asset_class, asset_code ORDER BY date)
        END AS daily_return
    FROM asset_prices
),
class_returns AS (
    SELECT
        date,
        asset_class,
        AVG(daily_return) AS class_daily_return
    FROM asset_returns
    WHERE daily_return IS NOT NULL
    GROUP BY date, asset_class
),
class_performance AS (
    SELECT
        asset_class,
        COUNT(*) AS trading_days,
        AVG(class_daily_return) AS mean_return,
        STDDEV(class_daily_return) AS std_return,
        EXP(SUM(LN(1 + COALESCE(class_daily_return, 0))) OVER (PARTITION BY asset_class ORDER BY date)) - 1 AS cumulative_return
    FROM class_returns
    GROUP BY asset_class
)
SELECT
    asset_class,
    trading_days,
    ROUND((mean_return * 252)::numeric, 6) AS annualized_return,
    ROUND((std_return * SQRT(252))::numeric, 6) AS annualized_volatility,
    ROUND(cumulative_return::numeric, 6) AS total_return,
    CASE
        WHEN std_return = 0 THEN NULL
        ELSE ROUND((mean_return / std_return * SQRT(252))::numeric, 4)
    END AS sharpe_ratio
FROM class_performance
ORDER BY annualized_return DESC;
```

---

## 8. 算法性能对比与优化

### 7.1 收益率计算方法对比

| 方法 | 数学定义 | 时间复杂度 | 空间复杂度 | 适用场景 | 优点 | 缺点 |
|------|---------|-----------|-----------|---------|------|------|
| **简单收益率** | $r_t = \frac{P_t - P_{t-1}}{P_{t-1}}$ | $O(1)$ | $O(1)$ | 单期收益 | 直观、简单 | 多期计算复杂 |
| **对数收益率** | $r_t = \ln\left(\frac{P_t}{P_{t-1}}\right)$ | $O(1)$ | $O(1)$ | 时间序列 | 时间可加性 | 需要正价格 |
| **累计收益率** | $R_T = \prod_{t=1}^{T}(1 + r_t) - 1$ | $O(n)$ | $O(1)$ | 多期收益 | 准确 | 计算复杂 |
| **年化收益率** | $r_{annual} = (1 + r)^{n} - 1$ | $O(1)$ | $O(1)$ | 收益比较 | 标准化 | 需要期限信息 |

### 7.2 性能优化建议

1. **索引优化**：
   - 在(date, symbol)上创建复合索引
   - 为常用查询创建索引

2. **窗口函数优化**：
   - 使用窗口函数避免自连接
   - 使用物化视图缓存结果

3. **批量处理**：
   - 批量计算多个资产的收益率
   - 使用并行查询

### 7.3 常见问题与解决方案

**问题1**：除零错误

- **解决方案**：使用NULLIF处理零值、检查价格有效性

**问题2**：缺失值处理

- **解决方案**：使用LAG函数、填充缺失值、跳过缺失数据

**问题3**：精度问题

- **解决方案**：使用NUMERIC类型、控制小数位数、避免浮点误差

**问题4**：性能慢

- **解决方案**：创建索引、使用物化视图、优化窗口函数

---

## 9. 最佳实践

### 8.1 计算方法选择

1. **单期收益**：使用简单收益率
2. **多期收益**：使用对数收益率
3. **投资评估**：使用累计收益率
4. **收益比较**：使用年化收益率

### 8.2 数据质量保证

1. **价格验证**：确保价格为正数
2. **缺失值处理**：合理处理缺失数据
3. **异常值检测**：识别和处理异常值

### 8.3 精度控制

1. **数据类型**：使用NUMERIC类型
2. **小数位数**：统一小数位数
3. **舍入规则**：统一舍入规则

### 8.4 SQL实现注意事项

1. **边界检查**：检查除零和负数情况
2. **NULL处理**：正确处理NULL值
3. **性能优化**：使用索引和物化视图
4. **错误处理**：添加错误处理逻辑

### 8.5 PostgreSQL 18 新特性应用

**PostgreSQL 18**引入了多项增强功能，可以显著提升收益率计算的性能：

1. **Skip Scan优化**：
   - 对于包含收益率排序的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-K收益率查询和分页查询

2. **异步I/O增强**：
   - 对于大规模金融数据的收益率计算，异步I/O可以显著提升性能
   - 适用于批量收益率计算和多资产收益率计算

3. **并行查询增强**：
   - 收益率计算支持更好的并行执行
   - 适用于多资产并行收益率计算

**示例：使用Skip Scan优化收益率排名查询**

```sql
-- 为收益率计算创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_returns_rank_skip_scan
ON daily_returns_cache(symbol, daily_return DESC);

-- Skip Scan优化查询：查找收益率最高的前10只股票
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    symbol,
    date,
    daily_return,
    close_price
FROM daily_returns_cache
WHERE daily_return IS NOT NULL
ORDER BY daily_return DESC
LIMIT 10;
```

### 8.6 高级优化技巧

**1. 使用物化视图缓存收益率计算结果**

对于频繁计算的收益率，使用物化视图缓存结果：

```sql
-- 创建物化视图缓存收益率统计
CREATE MATERIALIZED VIEW IF NOT EXISTS return_statistics_cache AS
WITH daily_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) AS prev_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
                (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
            ELSE NULL
        END AS daily_return
    FROM stock_prices
)
SELECT
    symbol,
    COUNT(*) AS trading_days,
    AVG(daily_return) AS mean_return,
    STDDEV(daily_return) AS std_return,
    MIN(daily_return) AS min_return,
    MAX(daily_return) AS max_return,
    PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY daily_return) AS percentile_5,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY daily_return) AS percentile_95,
    EXP(SUM(LN(1 + COALESCE(daily_return, 0))) OVER (PARTITION BY symbol ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)) - 1 AS cumulative_return
FROM daily_returns
WHERE daily_return IS NOT NULL
GROUP BY symbol;

-- 创建索引加速物化视图查询
CREATE INDEX idx_return_stats_symbol ON return_statistics_cache(symbol);
CREATE INDEX idx_return_stats_mean_return ON return_statistics_cache(mean_return DESC);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY return_statistics_cache;
```

**2. 量化交易策略：动量策略收益率计算**

**动量策略**：买入近期表现好的股票，卖出近期表现差的股票。

```sql
-- 量化交易策略：动量策略收益率计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行动量策略分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行动量策略收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '动量策略分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 动量策略：计算过去N天的累计收益率，选择表现最好的股票
WITH momentum_returns AS (
    SELECT
        date,
        symbol,
        close_price,
        -- 计算过去5天的累计收益率（动量指标）
        EXP(SUM(LN(1 + COALESCE(
            (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
            NULLIF(LAG(close_price) OVER (PARTITION BY symbol ORDER BY date), 0),
            0
        ))) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN 4 PRECEDING AND CURRENT ROW
        )) - 1 AS momentum_5d,
        -- 计算过去20天的累计收益率（动量指标）
        EXP(SUM(LN(1 + COALESCE(
            (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
            NULLIF(LAG(close_price) OVER (PARTITION BY symbol ORDER BY date), 0),
            0
        ))) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN 19 PRECEDING AND CURRENT ROW
        )) - 1 AS momentum_20d
    FROM stock_prices
),
momentum_rankings AS (
    SELECT
        date,
        symbol,
        close_price,
        momentum_5d,
        momentum_20d,
        -- 计算综合动量得分
        (momentum_5d * 0.7 + momentum_20d * 0.3) AS momentum_score,
        -- 排名：动量得分越高，排名越靠前
        ROW_NUMBER() OVER (PARTITION BY date ORDER BY (momentum_5d * 0.7 + momentum_20d * 0.3) DESC) AS momentum_rank,
        -- 计算前20%股票数量
        COUNT(*) OVER (PARTITION BY date) AS total_stocks
    FROM momentum_returns
    WHERE momentum_5d IS NOT NULL AND momentum_20d IS NOT NULL
)
SELECT
    date,
    symbol,
    close_price,
    ROUND(momentum_5d::numeric, 6) AS momentum_5d_return,
    ROUND(momentum_20d::numeric, 6) AS momentum_20d_return,
    ROUND(momentum_score::numeric, 6) AS momentum_score,
    momentum_rank,
    total_stocks,
    CASE
        WHEN momentum_rank <= CEIL(total_stocks * 0.2) THEN '买入信号'  -- Top 20%
        WHEN momentum_rank >= FLOOR(total_stocks * 0.8) THEN '卖出信号'  -- Bottom 20%
        ELSE '持有'
    END AS trading_signal
FROM momentum_rankings
WHERE momentum_score IS NOT NULL
ORDER BY date DESC, momentum_rank
LIMIT 50;
```

**3. 均值回归策略：收益率回归分析**

**均值回归策略**：买入近期表现差的股票（预期回归均值），卖出近期表现好的股票。

```sql
-- 均值回归策略：收益率回归分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stock_prices') THEN
            RAISE WARNING '表 stock_prices 不存在，无法执行均值回归策略分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行均值回归策略收益率计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '均值回归策略分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 均值回归策略：计算收益率偏离均值的程度
WITH return_analysis AS (
    SELECT
        date,
        symbol,
        close_price,
        CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
                (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
            ELSE NULL
        END AS daily_return,
        -- 计算过去60天的平均收益率（长期均值）
        AVG(CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
                (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
            ELSE NULL
        END) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
        ) AS long_term_mean,
        -- 计算过去60天的收益率标准差
        STDDEV(CASE
            WHEN LAG(close_price) OVER (PARTITION BY symbol ORDER BY date) > 0 THEN
                (close_price - LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)) /
                LAG(close_price) OVER (PARTITION BY symbol ORDER BY date)
            ELSE NULL
        END) OVER (
            PARTITION BY symbol
            ORDER BY date
            ROWS BETWEEN 59 PRECEDING AND CURRENT ROW
        ) AS long_term_stddev
    FROM stock_prices
),
z_score_analysis AS (
    SELECT
        date,
        symbol,
        close_price,
        daily_return,
        long_term_mean,
        long_term_stddev,
        -- 计算Z-score（偏离均值的标准差倍数）
        CASE
            WHEN long_term_stddev > 0 THEN
                (daily_return - long_term_mean) / long_term_stddev
            ELSE NULL
        END AS z_score
    FROM return_analysis
    WHERE daily_return IS NOT NULL
      AND long_term_mean IS NOT NULL
      AND long_term_stddev IS NOT NULL
)
SELECT
    date,
    symbol,
    close_price,
    ROUND(daily_return::numeric, 6) AS daily_return,
    ROUND(long_term_mean::numeric, 6) AS long_term_mean_return,
    ROUND(long_term_stddev::numeric, 6) AS volatility,
    ROUND(z_score::numeric, 4) AS z_score,
    CASE
        WHEN z_score < -2 THEN '强烈买入信号'  -- 显著低于均值（2个标准差）
        WHEN z_score < -1 THEN '买入信号'      -- 低于均值（1个标准差）
        WHEN z_score > 2 THEN '强烈卖出信号'   -- 显著高于均值（2个标准差）
        WHEN z_score > 1 THEN '卖出信号'       -- 高于均值（1个标准差）
        ELSE '持有'
    END AS trading_signal,
    -- 预期收益率：回归到均值
    ROUND((long_term_mean - daily_return)::numeric, 6) AS expected_return_reversion
FROM z_score_analysis
WHERE z_score IS NOT NULL
ORDER BY date DESC, z_score
LIMIT 50;
```

---

## 📚 参考资源

### 学术文献

- 《金融计量学》（Financial Econometrics）- 收益率计算理论
- 《投资学》（Investments）- 收益率分析方法

### PostgreSQL官方文档

- [窗口函数](https://www.postgresql.org/docs/current/tutorial-window.html)
- [数学函数](https://www.postgresql.org/docs/current/functions-math.html)
- [聚合函数](https://www.postgresql.org/docs/current/functions-aggregate.html)

### 在线资源

- PostgreSQL金融计算指南
- 收益率计算方法详解
- 金融数据分析最佳实践

### 相关算法

- [风险评估算法](./风险评估算法.md) - 风险评估
- [投资组合优化](./投资组合优化.md) - 投资组合优化
- [金融时间序列分析](./金融时间序列分析.md) - 时间序列分析

---

**最后更新**: 2025年1月
**文档状态**: ✅ 已完成
