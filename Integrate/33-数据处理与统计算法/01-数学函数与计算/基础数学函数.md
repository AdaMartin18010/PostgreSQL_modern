...
- [PostgreSQL 基础数学函数完整指南](#postgresql-基础数学函数完整指南)
  ...
  - [6. PostgreSQL 18 并行数学函数计算增强](#6-postgresql-18-并行数学函数计算增强)
    - [6.1 并行数学函数计算原理](#61-并行数学函数计算原理)
    - [6.2 并行四则运算](#62-并行四则运算)
    - [6.3 并行幂运算和对数运算](#63-并行幂运算和对数运算)
    - [6.4 并行三角函数计算](#64-并行三角函数计算)
  - [7. 实际应用案例](#7-实际应用案例)
    - [6.1 金融计算：复利计算](#61-金融计算复利计算)
    - [6.2 几何计算：距离计算](#62-几何计算距离计算)
    - [6.3 科学计算：指数衰减](#63-科学计算指数衰减)
    - [6.4 统计分析：对数变换](#64-统计分析对数变换)
    - [6.5 工程计算：三角函数在信号处理中的应用](#65-工程计算三角函数在信号处理中的应用)
    - [6.6 数据科学：数学变换在特征工程中的应用](#66-数据科学数学变换在特征工程中的应用)
...
### 6.5 工程计算：三角函数在信号处理中的应用

**信号处理**：使用三角函数进行信号分析，包括傅里叶变换的基础计算。

```sql
-- 信号处理：正弦波生成和分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'signal_analysis') THEN
            CREATE TABLE signal_analysis (
                time_point NUMERIC NOT NULL,
                frequency NUMERIC NOT NULL,
                amplitude NUMERIC NOT NULL,
                phase NUMERIC NOT NULL,
                PRIMARY KEY (time_point, frequency)
            );

            -- 生成测试数据：正弦波信号
            INSERT INTO signal_analysis (time_point, frequency, amplitude, phase)
            SELECT
                t AS time_point,
                1.0 AS frequency,
                1.0 AS amplitude,
                0.0 AS phase
            FROM generate_series(0, 100, 0.1) AS t;

            RAISE NOTICE '信号分析表创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '信号分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算正弦波信号值: y(t) = A * sin(2πft + φ)
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    time_point,
    frequency,
    ...
    phase,
    -- 正弦波: A * sin(2πft + φ)
    amplitude * SIN(2 * PI() * frequency * time_point + phase) AS signal_value,
    -- 余弦波: A * cos(2πft + φ)
    amplitude * COS(2 * PI() * frequency * time_point + phase) AS cos_signal_value,
    -- 功率: A^2 / 2
    POWER(amplitude, 2) / 2.0 AS signal_power
FROM signal_analysis
ORDER BY time_point;
```

### 6.6 数据科学：数学变换在特征工程中的应用

**特征工程**：使用数学变换创建新的特征，提高机器学习模型性能。

```sql
-- 特征工程：数学变换创建新特征（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'feature_engineering') THEN
            ...
            -- 生成测试数据
            INSERT INTO feature_engineering (x1, x2, target)
            SELECT
                RANDOM() * 100 AS x1,
                RANDOM() * 100 AS x2,
            ...
            RAISE NOTICE '特征工程表创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '特征工程准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
SELECT
    ...
    target,
    -- 对数变换特征
    LN(ABS(x1) + 1) AS log_x1,
    LN(ABS(x2) + 1) AS log_x2,
    -- 幂变换特征
    POWER(x1, 2) AS x1_squared,
    POWER(x2, 2) AS x2_squared,
    -- 交互特征
    x1 * x2 AS x1_x2_interaction,
    SQRT(POWER(x1, 2) + POWER(x2, 2)) AS euclidean_norm,
    -- 三角函数特征（归一化后）
    SIN(2 * PI() * x1 / 100) AS sin_x1,
    COS(2 * PI() * x2 / 100) AS cos_x2,
    -- 比率特征
    CASE
        WHEN x2 != 0 THEN x1 / x2
        ELSE NULL
    END AS x1_x2_ratio
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升数学函数计算的性能：

1. **Skip Scan优化**：
   - 对于包含数学函数表达式的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于范围查询和分页查询

2. **异步I/O增强**：
   - 对于大数据量的数学计算，异步I/O可以显著提升性能
   - 适用于批量计算和聚合操作
...
**示例：使用Skip Scan优化数学函数查询**

```sql
-- 为数学表达式创建索引（Skip Scan优化）
CREATE INDEX IF NOT EXISTS idx_distance_skip_scan
ON locations USING btree (SQRT(POWER(x, 2) + POWER(y, 2)));

-- Skip Scan优化查询：查找距离原点最近的10个点
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    location_id,
    x,
...
SELECT
    ...
    LN(value + 1) AS log_value,
    SIN(value) AS sin_value,
    COS(value) AS cos_value,
    TAN(value) AS tan_value
FROM large_number_table
WHERE value > 0;
```
...
-- 为IMMUTABLE函数创建索引
CREATE INDEX idx_distances ON location_pairs
USING btree (calculate_distance(x1, y1, x2, y2));
```

**2. 物化视图缓存数学计算结果**

对于频繁计算的复杂数学表达式，使用物化视图缓存结果：

```sql
-- 创建物化视图缓存复杂数学计算
CREATE MATERIALIZED VIEW IF NOT EXISTS math_calculations_cache AS
SELECT
    id,
    POWER(value, 2) AS squared,
    SQRT(value) AS sqrt_value,
    ...
    COS(value) AS cos_value,
    -- 复杂计算
    SQRT(POWER(value, 2) + POWER(value, 3)) AS complex_calculation
FROM number_table
WHERE value > 0;

-- 创建索引加速物化视图查询
CREATE INDEX idx_math_cache_complex ON math_calculations_cache
USING btree (complex_calculation);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY math_calculations_cache;
```
...
SELECT
    ...
    squared,
    sqrt_value,
    -- 使用窗口函数计算累计统计量（避免重复计算）
    AVG(squared) OVER (ORDER BY id ROWS BETWEEN 10 PRECEDING AND CURRENT ROW) AS moving_avg_squared,
    STDDEV(sqrt_value) OVER (ORDER BY id ROWS BETWEEN 10 PRECEDING AND CURRENT ROW) AS moving_stddev_sqrt
FROM math_values;
```
