# PostgreSQL 非参数统计方法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 统计推断 | 非参数统计 | 秩检验
> **难度级别**: ⭐⭐⭐⭐ (高级)
> **参考标准**: Nonparametric Statistical Methods (Hollander et al.), Statistical Inference

---

## 📋 目录

- [PostgreSQL 非参数统计方法完整指南](#postgresql-非参数统计方法完整指南)
  - [📋 目录](#-目录)
  - [非参数统计概述](#非参数统计概述)
    - [理论基础](#理论基础)
    - [优势特点](#优势特点)
    - [应用场景](#应用场景)
  - [1. 秩检验](#1-秩检验)
    - [1.1 Wilcoxon符号秩检验](#11-wilcoxon符号秩检验)
  - [2. PostgreSQL 18 并行非参数统计增强](#2-postgresql-18-并行非参数统计增强)
    - [2.1 并行非参数统计原理](#21-并行非参数统计原理)
    - [2.2 并行Wilcoxon符号秩检验](#22-并行wilcoxon符号秩检验)
    - [2.3 并行Mann-Whitney U检验](#23-并行mann-whitney-u检验)
  - [📚 参考资源](#-参考资源)
    - [SQL实现注意事项](#sql实现注意事项)
    - [PostgreSQL 18 新特性应用（增强）](#postgresql-18-新特性应用增强)
    - [高级优化技巧（增强）](#高级优化技巧增强)

---

## 非参数统计概述

**非参数统计**不依赖于总体分布假设，适用于分布未知或非正态的数据。

### 理论基础

非参数方法基于**秩**、**符号**或**分布函数**，而非参数值本身。

### 优势特点

| 优势 | 说明 |
|------|------|
| **无分布假设** | 不要求正态分布 |
| **稳健性** | 对异常值不敏感 |
| **适用性广** | 适用于各种数据类型 |

### 应用场景

| 应用领域 | 具体应用 |
|---------|---------|
| **医学** | 临床试验、疗效评估 |
| **社会科学** | 调查数据分析 |
| **质量控制** | 非正态过程控制 |

---

## 1. 秩检验

### 1.1 Wilcoxon符号秩检验

**Wilcoxon符号秩检验**用于配对样本的非参数检验。

```sql
-- 非参数统计数据准备（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nonparametric_data') THEN
            RAISE WARNING '表 nonparametric_data 已存在，先删除';
            DROP TABLE nonparametric_data CASCADE;
        END IF;

        CREATE TABLE nonparametric_data (
            id SERIAL PRIMARY KEY,
            before_value NUMERIC NOT NULL,
            after_value NUMERIC NOT NULL
        );

        INSERT INTO nonparametric_data (before_value, after_value) VALUES
            (10, 12), (15, 18), (20, 19), (25, 28), (30, 32);

        RAISE NOTICE '表 nonparametric_data 创建成功';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 nonparametric_data 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- Wilcoxon符号秩检验
WITH differences AS (
    SELECT
        id,
        after_value - before_value AS difference,
        ABS(after_value - before_value) AS abs_difference
    FROM nonparametric_data
),
ranked_differences AS (
    SELECT
        id,
        difference,
        abs_difference,
        ROW_NUMBER() OVER (ORDER BY abs_difference) AS rank
    FROM differences
    WHERE difference != 0
),
signed_ranks AS (
    SELECT
        SUM(CASE WHEN difference > 0 THEN rank ELSE 0 END) AS positive_rank_sum,
        SUM(CASE WHEN difference < 0 THEN rank ELSE 0 END) AS negative_rank_sum,
        COUNT(*) AS n
    FROM ranked_differences
),
wilcoxon_statistic AS (
    SELECT
        LEAST(positive_rank_sum, negative_rank_sum) AS w_statistic,
        n
    FROM signed_ranks
)
SELECT
    w_statistic,
    n,
    CASE
        WHEN w_statistic <= 2 THEN 'Reject H0 (p < 0.05, n=5)'
        ELSE 'Fail to reject H0'
    END AS conclusion
FROM wilcoxon_statistic;
```

---

## 2. PostgreSQL 18 并行非参数统计增强

**PostgreSQL 18** 显著增强了并行非参数统计计算能力，支持并行执行秩检验、符号检验和分布检验，大幅提升大规模非参数统计检验的性能。

### 2.1 并行非参数统计原理

PostgreSQL 18 的并行非参数统计通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描数据
2. **并行秩计算**：每个工作进程独立计算部分秩
3. **并行统计量计算**：并行执行检验统计量计算
4. **结果合并**：主进程合并所有工作进程的检验结果

### 2.2 并行Wilcoxon符号秩检验

```sql
-- PostgreSQL 18 并行Wilcoxon符号秩检验（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nonparametric_data') THEN
            RAISE WARNING '表 nonparametric_data 不存在，无法执行并行Wilcoxon符号秩检验';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Wilcoxon符号秩检验';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Wilcoxon符号秩检验准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行Wilcoxon符号秩检验
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH differences AS (
    SELECT
        id,
        after_value - before_value AS difference,
        ABS(after_value - before_value) AS abs_difference
    FROM nonparametric_data
),
ranked_differences AS (
    SELECT
        id,
        difference,
        ROW_NUMBER() OVER (ORDER BY abs_difference) AS rank
    FROM differences
    WHERE difference != 0
),
signed_ranks AS (
    SELECT
        SUM(CASE WHEN difference > 0 THEN rank ELSE 0 END) AS positive_rank_sum,
        SUM(CASE WHEN difference < 0 THEN rank ELSE 0 END) AS negative_rank_sum,
        COUNT(*) AS n
    FROM ranked_differences
)
SELECT
    LEAST(positive_rank_sum, negative_rank_sum) AS w_statistic,
    n,
    CASE
        WHEN LEAST(positive_rank_sum, negative_rank_sum) <= 2 THEN 'Reject H0 (p < 0.05, n=5)'
        ELSE 'Fail to reject H0'
    END AS conclusion
FROM signed_ranks;
```

### 2.3 并行Mann-Whitney U检验

```sql
-- PostgreSQL 18 并行Mann-Whitney U检验（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'two_group_data') THEN
            RAISE WARNING '表 two_group_data 不存在，无法执行并行Mann-Whitney U检验';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Mann-Whitney U检验';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Mann-Whitney U检验准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行Mann-Whitney U检验：两独立样本
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH combined_ranks AS (
    SELECT
        value,
        group_id,
        ROW_NUMBER() OVER (ORDER BY value) AS rank
    FROM two_group_data
),
group_rank_sums AS (
    SELECT
        group_id,
        SUM(rank) AS rank_sum,
        COUNT(*) AS n
    FROM combined_ranks
    GROUP BY group_id
),
u_statistics AS (
    SELECT
        g1.group_id AS group1,
        g2.group_id AS group2,
        g1.n AS n1,
        g2.n AS n2,
        g1.rank_sum AS r1,
        g2.rank_sum AS r2,
        g1.n * g2.n + g1.n * (g1.n + 1) / 2 - g1.rank_sum AS u1,
        g1.n * g2.n + g2.n * (g2.n + 1) / 2 - g2.rank_sum AS u2
    FROM group_rank_sums g1
    CROSS JOIN group_rank_sums g2
    WHERE g1.group_id < g2.group_id
)
SELECT
    group1,
    group2,
    LEAST(u1, u2) AS u_statistic,
    CASE
        WHEN LEAST(u1, u2) <= 2 THEN 'Reject H0 (p < 0.05)'
        ELSE 'Fail to reject H0'
    END AS conclusion
FROM u_statistics;
```

---

## 📚 参考资源

1. **Hollander, M., et al. (2013)**: "Nonparametric Statistical Methods"

### SQL实现注意事项

1. **秩计算**: 需要对数据进行排序计算秩，注意排序性能
2. **结处理**: 相同值需要平均秩，注意结的处理
3. **数值精度**: 使用NUMERIC类型保持计算精度
4. **大样本近似**: 对于大样本，使用正态近似计算p值

### PostgreSQL 18 新特性应用（增强）

**PostgreSQL 18**引入了多项增强功能，可以显著提升非参数统计方法的性能：

1. **Skip Scan优化**：
   - 对于包含组别标签的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N秩检验查询和多组对比查询

2. **异步I/O增强**：
   - 对于大规模非参数统计，异步I/O可以显著提升性能
   - 适用于批量秩计算和并行检验统计量计算

3. **并行查询增强**：
   - 非参数统计支持更好的并行执行（已在2节详细说明）
   - 适用于大规模秩检验和并行分布检验

**示例：使用Skip Scan优化非参数统计查询**

```sql
-- 为非参数统计创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_nonparametric_skip_scan
ON nonparametric_data(id DESC, ABS(after_value - before_value) DESC);

-- Skip Scan优化查询：查找差异最大的配对
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT DISTINCT ON (ABS(after_value - before_value))
    id,
    before_value,
    after_value,
    ABS(after_value - before_value) AS abs_difference
FROM nonparametric_data
ORDER BY ABS(after_value - before_value) DESC, id DESC
LIMIT 50;
```

### 高级优化技巧（增强）

**1. 使用物化视图缓存非参数统计结果**

对于频繁使用的非参数统计结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存非参数统计结果
CREATE MATERIALIZED VIEW IF NOT EXISTS nonparametric_statistics_cache AS
WITH ranked_data AS (
    SELECT
        id,
        before_value,
        after_value,
        after_value - before_value AS difference,
        ABS(after_value - before_value) AS abs_difference,
        -- 使用窗口函数计算秩（避免重复排序）
        ROW_NUMBER() OVER (ORDER BY ABS(after_value - before_value)) AS rank
    FROM nonparametric_data
    WHERE after_value - before_value != 0
),
wilcoxon_stats AS (
    SELECT
        SUM(CASE WHEN difference > 0 THEN rank ELSE 0 END) AS positive_rank_sum,
        SUM(CASE WHEN difference < 0 THEN rank ELSE 0 END) AS negative_rank_sum,
        COUNT(*) AS n,
        LEAST(
            SUM(CASE WHEN difference > 0 THEN rank ELSE 0 END),
            SUM(CASE WHEN difference < 0 THEN rank ELSE 0 END)
        ) AS w_statistic
    FROM ranked_data
)
SELECT
    n,
    positive_rank_sum,
    negative_rank_sum,
    w_statistic,
    CASE
        WHEN w_statistic <= 2 AND n = 5 THEN 'Reject H0 (p < 0.05) - Significant'
        WHEN w_statistic <= 8 AND n = 10 THEN 'Reject H0 (p < 0.05) - Significant'
        ELSE 'Fail to reject H0 - Not Significant'
    END AS test_conclusion,
    CASE
        WHEN n < 10 THEN 'Use Exact Test'
        WHEN n < 30 THEN 'Use Normal Approximation'
        ELSE 'Use Asymptotic Test'
    END AS test_method_advice
FROM wilcoxon_stats;

-- 创建索引加速物化视图查询
CREATE INDEX idx_nonparametric_statistics_cache_conclusion ON nonparametric_statistics_cache(test_conclusion, w_statistic);
CREATE INDEX idx_nonparametric_statistics_cache_method ON nonparametric_statistics_cache(test_method_advice, n);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY nonparametric_statistics_cache;
```

**2. 实时非参数统计：增量秩更新**

**实时非参数统计**：对于实时数据，使用增量方法更新秩检验结果。

```sql
-- 实时非参数统计：增量秩更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nonparametric_statistics_state') THEN
            CREATE TABLE nonparametric_statistics_state (
                test_id VARCHAR(100) NOT NULL,
                sum_positive_ranks NUMERIC DEFAULT 0,
                sum_negative_ranks NUMERIC DEFAULT 0,
                count_pairs BIGINT DEFAULT 0,
                w_statistic NUMERIC,
                test_conclusion VARCHAR(50),
                last_updated TIMESTAMPTZ DEFAULT NOW(),
                PRIMARY KEY (test_id)
            );

            CREATE INDEX idx_nonparametric_statistics_state_test ON nonparametric_statistics_state(test_id, last_updated DESC);
            CREATE INDEX idx_nonparametric_statistics_state_updated ON nonparametric_statistics_state(last_updated DESC);

            RAISE NOTICE '非参数统计状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量非参数统计更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量非参数统计更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

**3. 智能非参数统计优化：自适应检验方法选择**

**智能非参数统计优化**：根据数据特征自动选择最优非参数检验方法。

```sql
-- 智能非参数统计优化：自适应检验方法选择（带错误处理和性能测试）
DO $$
DECLARE
    sample_size BIGINT;
    is_paired BOOLEAN;
    group_count INTEGER;
    recommended_test VARCHAR(50);
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nonparametric_data') THEN
            RAISE WARNING '表 nonparametric_data 不存在，无法执行智能非参数统计优化';
            RETURN;
        END IF;

        -- 计算数据特征
        SELECT
            COUNT(*),
            TRUE,  -- 配对数据
            2  -- 两个组（before/after）
        INTO sample_size, is_paired, group_count
        FROM nonparametric_data;

        -- 根据数据特征自适应选择非参数检验方法
        IF is_paired AND sample_size < 30 THEN
            recommended_test := 'WILCOXON_SIGNED_RANK';  -- 配对小样本：Wilcoxon符号秩
        ELSIF is_paired AND sample_size >= 30 THEN
            recommended_test := 'SIGN_TEST';  -- 配对大样本：符号检验
        ELSIF NOT is_paired AND group_count = 2 THEN
            recommended_test := 'MANN_WHITNEY_U';  -- 两独立组：Mann-Whitney U
        ELSIF group_count > 2 THEN
            recommended_test := 'KRUSKAL_WALLIS';  -- 多组：Kruskal-Wallis
        ELSE
            recommended_test := 'WILCOXON_SIGNED_RANK';  -- 默认：Wilcoxon符号秩
        END IF;

        RAISE NOTICE '样本大小: %, 配对: %, 组数: %, 推荐检验: %',
            sample_size, is_paired, group_count, recommended_test;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '智能非参数统计优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

**最后更新**: 2025年1月
**文档状态**: ✅ 已完成（包含完整理论推导、实现和PostgreSQL 18新特性支持）
