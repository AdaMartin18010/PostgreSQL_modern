# PostgreSQL 关联规则挖掘完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 关联规则 | 市场分析
> **难度级别**: ⭐⭐⭐⭐ (高级)

---

## 📋 目录

- [PostgreSQL 关联规则挖掘完整指南](#postgresql-关联规则挖掘完整指南)
  - [📋 目录](#-目录)
  - [关联规则挖掘概述](#关联规则挖掘概述)
    - [理论基础](#理论基础)
      - [关联规则定义](#关联规则定义)
      - [支持度（Support）](#支持度support)
      - [置信度（Confidence）](#置信度confidence)
      - [提升度（Lift）](#提升度lift)
      - [频繁项集（Frequent Itemset）](#频繁项集frequent-itemset)
      - [Apriori原理](#apriori原理)
    - [核心概念](#核心概念)
  - [1. Apriori算法](#1-apriori算法)
    - [1.1 Apriori算法原理](#11-apriori算法原理)
      - [算法步骤](#算法步骤)
      - [候选生成（连接步）](#候选生成连接步)
      - [剪枝（剪枝步）](#剪枝剪枝步)
      - [时间复杂度](#时间复杂度)
    - [1.2 单一项集统计实现](#12-单一项集统计实现)
    - [1.3 二项集统计实现](#13-二项集统计实现)
  - [2. FP-Growth算法](#2-fp-growth算法)
    - [2.1 FP-Growth算法原理](#21-fp-growth算法原理)
      - [算法步骤](#算法步骤-1)
      - [FP树结构](#fp树结构)
      - [优势](#优势)
      - [时间复杂度](#时间复杂度-1)
    - [2.2 FP-Growth算法实现](#22-fp-growth算法实现)
  - [3. 关联规则生成](#3-关联规则生成)
    - [3.1 规则生成实现](#31-规则生成实现)
  - [4. 规则评估与筛选](#4-规则评估与筛选)
    - [4.1 规则评估实现](#41-规则评估实现)
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 购物篮分析](#51-购物篮分析)
    - [5.2 推荐系统应用](#52-推荐系统应用)
    - [5.3 交叉销售分析](#53-交叉销售分析)
    - [5.4 商品组合优化](#54-商品组合优化)
    - [5.5 关联规则可视化分析](#55-关联规则可视化分析)
  - [6. PostgreSQL 18并行关联规则挖掘](#6-postgresql-18并行关联规则挖掘)
    - [6.1 并行关联规则挖掘概述](#61-并行关联规则挖掘概述)
      - [并行关联规则挖掘配置](#并行关联规则挖掘配置)
      - [并行关联规则挖掘实现](#并行关联规则挖掘实现)
  - [7. 算法性能对比与优化](#7-算法性能对比与优化)
    - [7.1 Apriori vs FP-Growth对比](#71-apriori-vs-fp-growth对比)
    - [7.2 性能优化建议](#72-性能优化建议)
    - [7.3 常见问题与解决方案](#73-常见问题与解决方案)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 参数选择](#81-参数选择)
    - [8.2 规则评估](#82-规则评估)
    - [8.3 应用场景](#83-应用场景)
    - [8.4 SQL实现注意事项](#84-sql实现注意事项)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 关联规则挖掘概述

**关联规则挖掘（Association Rule Mining）**是数据挖掘中的重要技术，用于发现数据项之间的关联关系。最经典的应用是购物篮分析（Market Basket Analysis），通过分析顾客的购买行为，发现商品之间的关联规则。

### 理论基础

#### 关联规则定义

**关联规则**的形式为：$X \Rightarrow Y$，其中：

- $X$：前件（Antecedent），条件项集
- $Y$：后件（Consequent），结果项集
- $X \cap Y = \emptyset$：前件和后件不相交

**示例**：$\{牛奶, 面包\} \Rightarrow \{黄油\}$ 表示"购买牛奶和面包的顾客，也倾向于购买黄油"

#### 支持度（Support）

**支持度**衡量项集在事务中出现的频率：

$$\text{Support}(X) = \frac{|\{t \in T : X \subseteq t\}|}{|T|}$$

其中：

- $T$：所有事务的集合
- $t$：单个事务
- $|T|$：事务总数

**规则支持度**：
$$\text{Support}(X \Rightarrow Y) = \text{Support}(X \cup Y) = \frac{|\{t \in T : (X \cup Y) \subseteq t\}|}{|T|}$$

#### 置信度（Confidence）

**置信度**衡量规则的可信度，即条件概率：

$$\text{Confidence}(X \Rightarrow Y) = P(Y|X) = \frac{\text{Support}(X \cup Y)}{\text{Support}(X)}$$

**置信度解释**：在包含$X$的事务中，有多少比例也包含$Y$。

#### 提升度（Lift）

**提升度**衡量规则的有效性，表示$Y$在$X$出现时的频率相对于$Y$的总体频率的提升：

$$\text{Lift}(X \Rightarrow Y) = \frac{\text{Confidence}(X \Rightarrow Y)}{\text{Support}(Y)} = \frac{\text{Support}(X \cup Y)}{\text{Support}(X) \times \text{Support}(Y)}$$

**提升度解释**：

- $\text{Lift} > 1$：正相关，规则有效
- $\text{Lift} = 1$：独立，无关联
- $\text{Lift} < 1$：负相关，规则无效

#### 频繁项集（Frequent Itemset）

**频繁项集**是支持度不低于最小支持度阈值（min_support）的项集：

$$F = \{X : \text{Support}(X) \geq \text{min\_support}\}$$

#### Apriori原理

**Apriori原理**（先验原理）：

- 如果一个项集是频繁的，那么它的所有子集也是频繁的
- 如果一个项集不是频繁的，那么它的所有超集也不是频繁的

**数学表达**：
$$\text{If } X \subseteq Y \text{ and } Y \text{ is frequent, then } X \text{ is frequent}$$
$$\text{If } X \text{ is not frequent, then } \forall Y \supseteq X, Y \text{ is not frequent}$$

### 核心概念

| 概念 | 数学定义 | 用途 |
|------|---------|------|
| **支持度** | $\text{Support}(X) = \frac{|\{t : X \subseteq t\}|}{|T|}$ | 衡量规则重要性 |
| **置信度** | $\text{Confidence}(X \Rightarrow Y) = \frac{\text{Support}(X \cup Y)}{\text{Support}(X)}$ | 衡量规则可靠性 |
| **提升度** | $\text{Lift}(X \Rightarrow Y) = \frac{\text{Confidence}(X \Rightarrow Y)}{\text{Support}(Y)}$ | 衡量规则有效性 |
| **频繁项集** | $\{X : \text{Support}(X) \geq \text{min\_support}\}$ | 候选规则基础 |

---

## 1. Apriori算法

### 1.1 Apriori算法原理

**Apriori算法**是关联规则挖掘的经典算法，基于Apriori原理，通过逐层搜索频繁项集。

#### 算法步骤

**阶段1：频繁项集生成**

1. **扫描数据库**，统计所有单项的支持度
2. **生成频繁1-项集** $L_1$（支持度 $\geq$ min_support）
3. **迭代生成频繁k-项集** $L_k$：
   - 从$L_{k-1}$生成候选k-项集$C_k$（连接和剪枝）
   - 扫描数据库，计算$C_k$中每个项集的支持度
   - 生成频繁k-项集$L_k$（支持度 $\geq$ min_support）
4. **重复步骤3**，直到$L_k$为空

**阶段2：关联规则生成**

1. 对每个频繁项集$L$，生成所有非空子集
2. 对每个子集$S \subset L$，生成规则$S \Rightarrow (L - S)$
3. 计算规则的置信度
4. 保留置信度 $\geq$ min_confidence的规则

#### 候选生成（连接步）

**连接操作**：将两个频繁(k-1)-项集连接生成候选k-项集

**条件**：两个项集的前(k-2)个项相同，最后一个项不同

**示例**：

- $L_2 = \{\{A,B\}, \{A,C\}, \{B,C\}\}$
- 连接$\{A,B\}$和$\{A,C\}$：$\{A,B,C\}$（前1项相同：A）
- 连接$\{A,B\}$和$\{B,C\}$：不连接（前1项不同：A vs B）

#### 剪枝（剪枝步）

**剪枝操作**：删除候选k-项集中包含非频繁(k-1)-项集的项集

**原理**：根据Apriori原理，如果项集的子集不是频繁的，那么该项集也不是频繁的

#### 时间复杂度

**时间复杂度**：$O(2^n)$（最坏情况，$n$是项的数量）

**优化**：

- 使用哈希树存储候选项集
- 减少数据库扫描次数
- 使用事务压缩

### 1.2 单一项集统计实现

```sql
-- 单一项集统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_items') THEN
            RAISE WARNING '表 transaction_items 不存在，无法执行单一项集统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行单一项集统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '单一项集统计准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    item,
    COUNT(DISTINCT transaction_id) AS support_count,
    COUNT(DISTINCT transaction_id) * 100.0 / (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) AS support_pct
FROM transaction_items
GROUP BY item
HAVING COUNT(DISTINCT transaction_id) >= 10  -- 最小支持度阈值
ORDER BY support_count DESC;
```

### 1.3 二项集统计实现

**二项集统计**统计两个项同时出现的频率，生成频繁2-项集。

```sql
-- 二项集统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_items') THEN
            RAISE WARNING '表 transaction_items 不存在，无法执行二项集统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行二项集统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '二项集统计准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    t1.item AS item1,
    t2.item AS item2,
    COUNT(DISTINCT t1.transaction_id) AS support_count,
    COUNT(DISTINCT t1.transaction_id) * 100.0 / (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) AS support_pct
FROM transaction_items t1
INNER JOIN transaction_items t2 ON t1.transaction_id = t2.transaction_id
WHERE t1.item < t2.item  -- 避免重复组合
GROUP BY t1.item, t2.item
HAVING COUNT(DISTINCT t1.transaction_id) >= 10  -- 最小支持度阈值
ORDER BY support_count DESC;
```

---

## 2. FP-Growth算法

### 2.1 FP-Growth算法原理

**FP-Growth（Frequent Pattern Growth）算法**是Apriori算法的改进，通过构建FP树（Frequent Pattern Tree）避免生成候选项集，提高效率。

#### 算法步骤

**阶段1：构建FP树**

1. **第一次扫描数据库**，统计所有项的支持度
2. **按支持度降序排序**，生成频繁项列表
3. **第二次扫描数据库**，构建FP树：
   - 对每个事务，按频繁项列表顺序排序
   - 将排序后的事务插入FP树
   - 更新节点计数和链接

**阶段2：从FP树挖掘频繁项集**

1. 从FP树的底部（支持度最低的项）开始
2. 对每个项，构建条件模式基（Conditional Pattern Base）
3. 构建条件FP树（Conditional FP-Tree）
4. 递归挖掘条件FP树，生成频繁项集

#### FP树结构

**FP树节点**：

- **项名**：项的名称
- **计数**：该项在路径中的出现次数
- **父节点指针**：指向父节点
- **子节点指针**：指向子节点
- **兄弟节点指针**：指向相同项的节点（用于快速遍历）

**头表（Header Table）**：

- 存储每个项的链表头指针
- 用于快速访问FP树中所有相同项的节点

#### 优势

1. **无需生成候选**：直接生成频繁项集
2. **减少扫描次数**：只需要扫描数据库2次
3. **压缩存储**：FP树压缩存储事务数据
4. **效率高**：时间复杂度通常优于Apriori

#### 时间复杂度

**时间复杂度**：$O(n \times m)$（$n$是事务数，$m$是平均事务长度）

**空间复杂度**：$O(n \times m)$（FP树大小）

### 2.2 FP-Growth算法实现

```sql
-- FP-Growth算法：构建频繁项列表（简化实现）
WITH item_supports AS (
    SELECT
        item,
        COUNT(DISTINCT transaction_id) AS support_count,
        COUNT(DISTINCT transaction_id) * 100.0 /
            (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) AS support_pct
    FROM transaction_items
    GROUP BY item
    HAVING COUNT(DISTINCT transaction_id) >= 10  -- min_support
),
sorted_items AS (
    SELECT
        item,
        support_count,
        ROW_NUMBER() OVER (ORDER BY support_count DESC) AS item_rank
    FROM item_supports
)
SELECT
    item,
    support_count,
    item_rank
FROM sorted_items
ORDER BY item_rank;
```

## 3. 关联规则生成

### 3.1 规则生成实现

**关联规则生成**从频繁项集生成关联规则，计算置信度和提升度。

```sql
-- 关联规则生成（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_items') THEN
            RAISE WARNING '表 transaction_items 不存在，无法生成关联规则';
            RETURN;
        END IF;
        RAISE NOTICE '开始生成关联规则';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '关联规则生成准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

WITH frequent_itemsets AS (
    SELECT
        t1.item AS item1,
        t2.item AS item2,
        COUNT(DISTINCT t1.transaction_id) AS support_count
    FROM transaction_items t1
    INNER JOIN transaction_items t2 ON t1.transaction_id = t2.transaction_id
    WHERE t1.item < t2.item
    GROUP BY t1.item, t2.item
    HAVING COUNT(DISTINCT t1.transaction_id) >= 10
),
item_supports AS (
    SELECT
        item,
        COUNT(DISTINCT transaction_id) AS item_support
    FROM transaction_items
    GROUP BY item
)
SELECT
    fi.item1,
    fi.item2,
    fi.support_count AS support_count,
    fi.support_count * 100.0 / (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) AS support_pct,
    fi.support_count * 100.0 / NULLIF(is1.item_support, 0) AS confidence_pct,
    fi.support_count * 100.0 / NULLIF(is2.item_support, 0) AS reverse_confidence_pct
FROM frequent_itemsets fi
JOIN item_supports is1 ON fi.item1 = is1.item
JOIN item_supports is2 ON fi.item2 = is2.item
ORDER BY confidence_pct DESC;
```

---

## 4. 规则评估与筛选

### 4.1 规则评估实现

**规则评估**计算规则的支持度、置信度和提升度，筛选有价值的规则。

```sql
-- 规则评估（带错误处理和性能测试）
DO $$
DECLARE
    total_transactions BIGINT;
    rule_support_count BIGINT;
    item1_support_count BIGINT;
    support_pct NUMERIC;
    confidence_pct NUMERIC;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_items') THEN
            RAISE WARNING '表 transaction_items 不存在，无法评估规则';
            RETURN;
        END IF;

        SELECT COUNT(DISTINCT transaction_id) INTO total_transactions FROM transaction_items;

        -- 计算规则 A -> B 的支持度和置信度
        SELECT COUNT(DISTINCT transaction_id) INTO rule_support_count
        FROM transaction_items
        WHERE transaction_id IN (
            SELECT transaction_id FROM transaction_items WHERE item = 'A'
        )
        AND transaction_id IN (
            SELECT transaction_id FROM transaction_items WHERE item = 'B'
        );

        SELECT COUNT(DISTINCT transaction_id) INTO item1_support_count
        FROM transaction_items
        WHERE item = 'A';

        support_pct := rule_support_count * 100.0 / NULLIF(total_transactions, 0);
        confidence_pct := rule_support_count * 100.0 / NULLIF(item1_support_count, 0);

        RAISE NOTICE '规则 A -> B 评估结果:';
        RAISE NOTICE '  支持度: %%%', support_pct;
        RAISE NOTICE '  置信度: %%%', confidence_pct;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '规则评估失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 5. 实际应用案例

### 5.1 购物篮分析

**购物篮分析**分析顾客购买商品的关联关系，发现商品之间的关联规则。

```sql
-- 购物篮分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            RAISE WARNING '表 order_items 不存在，无法执行购物篮分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行购物篮分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '购物篮分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

WITH product_pairs AS (
    SELECT DISTINCT
        o1.order_id,
        o1.product_id AS product1,
        o2.product_id AS product2
    FROM order_items o1
    INNER JOIN order_items o2 ON o1.order_id = o2.order_id
    WHERE o1.product_id < o2.product_id
),
pair_stats AS (
    SELECT
        product1,
        product2,
        COUNT(DISTINCT order_id) AS pair_count
    FROM product_pairs
    GROUP BY product1, product2
    HAVING COUNT(DISTINCT order_id) >= 5  -- 最小支持度
),
product_stats AS (
    SELECT
        product_id,
        COUNT(DISTINCT order_id) AS product_count
    FROM order_items
    GROUP BY product_id
)
SELECT
    ps.product1,
    ps.product2,
    ps.pair_count AS support_count,
    ps.pair_count * 100.0 / (SELECT COUNT(DISTINCT order_id) FROM order_items) AS support_pct,
    ps.pair_count * 100.0 / NULLIF(p1.product_count, 0) AS confidence_pct
FROM pair_stats ps
JOIN product_stats p1 ON ps.product1 = p1.product_id
JOIN product_stats p2 ON ps.product2 = p2.product_id
ORDER BY confidence_pct DESC
LIMIT 20;
```

### 5.2 推荐系统应用

**场景**：基于关联规则的商品推荐。

```sql
-- 推荐系统：基于关联规则推荐商品
WITH user_purchases AS (
    SELECT
        user_id,
        product_id,
        COUNT(*) AS purchase_count
    FROM order_items oi
    JOIN orders o ON oi.order_id = o.order_id
    GROUP BY user_id, product_id
),
association_rules AS (
    SELECT
        product1,
        product2,
        support_pct,
        confidence_pct,
        lift
    FROM product_association_rules
    WHERE confidence_pct >= 50  -- min_confidence
        AND lift > 1.2  -- 提升度阈值
),
user_recommendations AS (
    SELECT DISTINCT
        up.user_id,
        ar.product2 AS recommended_product,
        ar.confidence_pct,
        ar.lift,
        ROW_NUMBER() OVER (PARTITION BY up.user_id ORDER BY ar.lift DESC) AS rank
    FROM user_purchases up
    JOIN association_rules ar ON up.product_id = ar.product1
    WHERE NOT EXISTS (
        SELECT 1
        FROM user_purchases up2
        WHERE up2.user_id = up.user_id
            AND up2.product_id = ar.product2
    )
)
SELECT
    user_id,
    recommended_product,
    ROUND(confidence_pct::numeric, 2) AS confidence,
    ROUND(lift::numeric, 2) AS lift
FROM user_recommendations
WHERE rank <= 5  -- Top 5推荐
ORDER BY user_id, lift DESC;
```

### 5.3 交叉销售分析

**场景**：分析产品组合，优化交叉销售策略。

```sql
-- 交叉销售分析：分析产品组合的销售效果
WITH product_combinations AS (
    SELECT
        o1.product_id AS product1,
        o2.product_id AS product2,
        COUNT(DISTINCT o1.order_id) AS combination_count,
        COUNT(DISTINCT o1.order_id) * 100.0 /
            (SELECT COUNT(DISTINCT order_id) FROM order_items) AS support_pct
    FROM order_items o1
    INNER JOIN order_items o2 ON o1.order_id = o2.order_id
    WHERE o1.product_id < o2.product_id
    GROUP BY o1.product_id, o2.product_id
    HAVING COUNT(DISTINCT o1.order_id) >= 10
),
product_stats AS (
    SELECT
        product_id,
        COUNT(DISTINCT order_id) AS product_count,
        SUM(quantity * price) AS total_revenue
    FROM order_items
    GROUP BY product_id
),
combination_analysis AS (
    SELECT
        pc.product1,
        pc.product2,
        pc.combination_count,
        pc.support_pct,
        ps1.product_count AS product1_count,
        ps2.product_count AS product2_count,
        ps1.total_revenue AS product1_revenue,
        ps2.total_revenue AS product2_revenue,
        pc.combination_count * 100.0 / NULLIF(ps1.product_count, 0) AS confidence_pct,
        pc.combination_count * 100.0 / NULLIF(ps2.product_count, 0) AS reverse_confidence_pct
    FROM product_combinations pc
    JOIN product_stats ps1 ON pc.product1 = ps1.product_id
    JOIN product_stats ps2 ON pc.product2 = ps2.product_id
)
SELECT
    product1,
    product2,
    combination_count,
    ROUND(support_pct::numeric, 2) AS support_pct,
    ROUND(confidence_pct::numeric, 2) AS confidence_pct,
    ROUND(reverse_confidence_pct::numeric, 2) AS reverse_confidence_pct,
    CASE
        WHEN confidence_pct >= 60 THEN 'Strong cross-sell opportunity'
        WHEN confidence_pct >= 40 THEN 'Moderate cross-sell opportunity'
        ELSE 'Weak cross-sell opportunity'
    END AS cross_sell_potential
FROM combination_analysis
WHERE confidence_pct >= 30 OR reverse_confidence_pct >= 30
ORDER BY confidence_pct DESC, reverse_confidence_pct DESC;
```

### 5.4 商品组合优化

**场景**：优化商品组合，提升销售额。

```sql
-- 商品组合优化：分析商品组合的销售潜力
WITH product_pairs AS (
    SELECT
        o1.product_id AS product1,
        o2.product_id AS product2,
        COUNT(DISTINCT o1.order_id) AS pair_count,
        AVG(o1.quantity + o2.quantity) AS avg_quantity,
        AVG(o1.price + o2.price) AS avg_revenue
    FROM order_items o1
    INNER JOIN order_items o2 ON o1.order_id = o2.order_id
    WHERE o1.product_id < o2.product_id
    GROUP BY o1.product_id, o2.product_id
),
pair_metrics AS (
    SELECT
        product1,
        product2,
        pair_count,
        avg_quantity,
        avg_revenue,
        pair_count * avg_revenue AS total_potential_revenue,
        pair_count * 100.0 /
            (SELECT COUNT(DISTINCT order_id) FROM order_items) AS support_pct
    FROM product_pairs
    WHERE pair_count >= 5
),
optimization_recommendations AS (
    SELECT
        product1,
        product2,
        pair_count,
        ROUND(avg_revenue::numeric, 2) AS avg_revenue,
        ROUND(total_potential_revenue::numeric, 2) AS total_potential_revenue,
        ROUND(support_pct::numeric, 2) AS support_pct,
        CASE
            WHEN support_pct >= 10 AND avg_revenue > 100 THEN 'High priority bundle'
            WHEN support_pct >= 5 AND avg_revenue > 50 THEN 'Medium priority bundle'
            ELSE 'Low priority bundle'
        END AS bundle_priority
    FROM pair_metrics
)
SELECT
    product1,
    product2,
    pair_count,
    avg_revenue,
    total_potential_revenue,
    support_pct,
    bundle_priority
FROM optimization_recommendations
WHERE bundle_priority IN ('High priority bundle', 'Medium priority bundle')
ORDER BY total_potential_revenue DESC;
```

### 5.5 关联规则可视化分析

**场景**：分析关联规则的强度和方向。

```sql
-- 关联规则可视化分析：生成规则分析报告
WITH rule_analysis AS (
    SELECT
        item1,
        item2,
        support_count,
        support_pct,
        confidence_pct,
        reverse_confidence_pct,
        CASE
            WHEN confidence_pct > reverse_confidence_pct THEN item1 || ' -> ' || item2
            ELSE item2 || ' -> ' || item1
        END AS rule_direction,
        GREATEST(confidence_pct, reverse_confidence_pct) AS max_confidence,
        ABS(confidence_pct - reverse_confidence_pct) AS confidence_diff
    FROM association_rules
    WHERE confidence_pct >= 30 OR reverse_confidence_pct >= 30
),
rule_categories AS (
    SELECT
        item1,
        item2,
        rule_direction,
        support_count,
        ROUND(support_pct::numeric, 2) AS support_pct,
        ROUND(max_confidence::numeric, 2) AS max_confidence,
        ROUND(confidence_diff::numeric, 2) AS confidence_diff,
        CASE
            WHEN max_confidence >= 70 THEN 'Strong rule'
            WHEN max_confidence >= 50 THEN 'Moderate rule'
            ELSE 'Weak rule'
        END AS rule_strength,
        CASE
            WHEN confidence_diff < 10 THEN 'Bidirectional'
            WHEN confidence_diff < 30 THEN 'Unidirectional'
            ELSE 'Highly directional'
        END AS rule_direction_type
    FROM rule_analysis
)
SELECT
    rule_direction,
    support_count,
    support_pct,
    max_confidence,
    confidence_diff,
    rule_strength,
    rule_direction_type
FROM rule_categories
ORDER BY max_confidence DESC, support_count DESC;
```

---

## 6. PostgreSQL 18并行关联规则挖掘

### 6.1 并行关联规则挖掘概述

**PostgreSQL 18并行关联规则挖掘**：利用PostgreSQL 18的并行查询能力，显著提升大规模关联规则挖掘的性能。

#### 并行关联规则挖掘配置

```sql
-- 配置并行关联规则挖掘参数（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 设置并行工作进程数
        SET max_parallel_workers_per_gather = 4;

        -- 设置并行表扫描阈值
        SET min_parallel_table_scan_size = '8MB';

        -- 设置并行聚合成本阈值
        SET parallel_setup_cost = 0;
        SET parallel_tuple_cost = 0.0001;

        RAISE NOTICE '并行关联规则挖掘参数已配置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行关联规则挖掘配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 并行关联规则挖掘实现

```sql
-- 并行关联规则挖掘：大规模频繁项集统计（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 启用并行查询
        SET max_parallel_workers_per_gather = 4;
        SET parallel_setup_cost = 0;
        SET parallel_tuple_cost = 0.0001;

        RAISE NOTICE '开始执行并行关联规则挖掘';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行关联规则挖掘准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 并行频繁项集统计
EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH parallel_item_sets AS (
    SELECT
        transaction_id,
        item_id
    FROM transaction_items
    WHERE transaction_date >= CURRENT_DATE - INTERVAL '1 year'
),
parallel_item_counts AS (
    SELECT
        item_id,
        COUNT(DISTINCT transaction_id) AS support_count,
        COUNT(*) AS total_occurrences
    FROM parallel_item_sets
    GROUP BY item_id
),
parallel_frequent_items AS (
    SELECT
        item_id,
        support_count,
        total_occurrences,
        ROUND((support_count::numeric / (SELECT COUNT(DISTINCT transaction_id) FROM parallel_item_sets) * 100)::numeric, 2) AS support_percentage
    FROM parallel_item_counts
    WHERE support_count >= 100  -- 最小支持度阈值
)
SELECT
    item_id,
    support_count,
    total_occurrences,
    support_percentage,
    CASE
        WHEN support_percentage >= 50 THEN 'High Frequency'
        WHEN support_percentage >= 20 THEN 'Medium Frequency'
        ELSE 'Low Frequency'
    END AS frequency_category
FROM parallel_frequent_items
ORDER BY support_count DESC;
```

---

## 7. 算法性能对比与优化

### 7.1 Apriori vs FP-Growth对比

| 算法 | 时间复杂度 | 空间复杂度 | 数据库扫描次数 | 优势 | 劣势 |
|------|-----------|-----------|--------------|------|------|
| **Apriori** | $O(2^n)$ | $O(n)$ | 多次 | 实现简单、易于理解 | 生成大量候选、扫描次数多 |
| **FP-Growth** | $O(n \times m)$ | $O(n \times m)$ | 2次 | 效率高、无需候选 | 实现复杂、内存需求大 |

### 7.2 性能优化建议

1. **支持度阈值**：
   - 设置合理的最小支持度阈值
   - 过低：生成过多规则，计算量大
   - 过高：可能遗漏有价值的规则

2. **置信度阈值**：
   - 设置合理的最小置信度阈值
   - 根据业务需求调整
   - 考虑提升度指标

3. **数据预处理**：
   - 去除低频项
   - 数据清洗和标准化
   - 事务压缩

4. **算法选择**：
   - 小数据集：使用Apriori
   - 大数据集：使用FP-Growth
   - 稀疏数据：考虑其他算法

### 7.3 常见问题与解决方案

**问题1**：规则数量过多

- **解决方案**：提高支持度和置信度阈值、使用提升度筛选、限制规则长度

**问题2**：计算时间过长

- **解决方案**：使用FP-Growth算法、优化数据库查询、并行处理

**问题3**：规则质量不高

- **解决方案**：调整阈值、使用提升度、考虑规则的可解释性

**问题4**：内存不足

- **解决方案**：分批处理、使用事务压缩、优化数据结构

---

## 8. 最佳实践

### 8.1 参数选择

1. **最小支持度**：
   - 根据数据规模和业务需求设置
   - 通常设置为1%-10%
   - 考虑项的数量和事务数量

2. **最小置信度**：
   - 根据业务需求设置
   - 通常设置为50%-80%
   - 考虑规则的可靠性要求

3. **提升度阈值**：
   - 通常设置为>1.0（正相关）
   - 根据业务需求调整
   - 考虑规则的实用性

### 8.2 规则评估

1. **支持度**：规则的重要性，是否常见
2. **置信度**：规则的可信度，是否可靠
3. **提升度**：规则的有效性，是否有价值
4. **可解释性**：规则是否易于理解和应用

### 8.3 应用场景

1. **购物篮分析**：发现商品关联关系
2. **推荐系统**：基于关联规则推荐
3. **交叉销售**：优化产品组合
4. **库存管理**：预测商品需求
5. **营销策略**：制定促销方案

### 8.4 SQL实现注意事项

1. **性能优化**：
   - 创建适当的索引
   - 使用物化视图缓存结果
   - 分批处理大数据集

2. **数据质量**：
   - 确保数据完整性
   - 处理缺失值
   - 数据清洗和标准化

3. **结果验证**：
   - 验证规则的正确性
   - 检查规则的业务意义
   - 评估规则的实际效果

### 8.5 PostgreSQL 18 新特性应用（增强）

**PostgreSQL 18**引入了多项增强功能，可以显著提升关联规则挖掘的性能：

1. **Skip Scan优化**：
   - 对于包含频繁项集的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N频繁项集查询和规则评估查询

2. **异步I/O增强**：
   - 对于大规模关联规则挖掘计算，异步I/O可以显著提升性能
   - 适用于批量频繁项集统计和规则生成

3. **并行查询增强**：
   - 关联规则挖掘支持更好的并行执行（已在6节详细说明）
   - 适用于大规模频繁项集统计和Apriori算法

**示例：使用Skip Scan优化频繁项集查询**

```sql
-- 为频繁项集创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_frequent_itemsets_skip_scan
ON item_sets(itemset_size, support_count DESC);

-- Skip Scan优化查询：查找支持度最高的前10个频繁项集
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    itemset_id,
    itemset,
    itemset_size,
    support_count,
    support_percentage
FROM item_sets
WHERE itemset_size = 2  -- 二项集
ORDER BY support_count DESC
LIMIT 10;
```

### 8.6 高级优化技巧（增强）

**1. 使用物化视图缓存关联规则结果**

对于频繁使用的关联规则结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存关联规则结果
CREATE MATERIALIZED VIEW IF NOT EXISTS association_rules_cache AS
WITH frequent_itemsets AS (
    SELECT
        itemset_id,
        itemset,
        itemset_size,
        support_count,
        ROUND((support_count::numeric / (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) * 100)::numeric, 2) AS support_percentage
    FROM item_sets
    WHERE support_count >= 100  -- 最小支持度阈值
),
association_rules AS (
    SELECT
        f1.itemset AS antecedent,
        f2.itemset AS consequent,
        f1.itemset || f2.itemset AS combined_itemset,
        f1.support_count AS antecedent_support,
        f2.support_count AS consequent_support,
        -- 使用窗口函数计算支持度和置信度（避免重复计算）
        f1.support_count::numeric / NULLIF((SELECT COUNT(DISTINCT transaction_id) FROM transaction_items), 0) AS antecedent_support_pct,
        f2.support_count::numeric / NULLIF((SELECT COUNT(DISTINCT transaction_id) FROM transaction_items), 0) AS consequent_support_pct,
        -- 假设组合项集支持度（简化计算）
        GREATEST(f1.support_count, f2.support_count)::numeric / NULLIF((SELECT COUNT(DISTINCT transaction_id) FROM transaction_items), 0) AS combined_support_pct,
        -- 置信度计算
        GREATEST(f1.support_count, f2.support_count)::numeric / NULLIF(f1.support_count, 0) AS confidence,
        -- 提升度计算
        (GREATEST(f1.support_count, f2.support_count)::numeric / NULLIF(f1.support_count, 0)) /
        NULLIF(f2.support_count::numeric / NULLIF((SELECT COUNT(DISTINCT transaction_id) FROM transaction_items), 0), 0) AS lift
    FROM frequent_itemsets f1
    CROSS JOIN frequent_itemsets f2
    WHERE f1.itemset < f2.itemset  -- 避免重复和自组合
      AND f1.support_count >= 100
      AND f2.support_count >= 100
)
SELECT
    antecedent,
    consequent,
    ROUND(antecedent_support_pct::numeric, 4) AS antecedent_support,
    ROUND(consequent_support_pct::numeric, 4) AS consequent_support,
    ROUND(combined_support_pct::numeric, 4) AS combined_support,
    ROUND(confidence::numeric, 4) AS confidence,
    ROUND(lift::numeric, 4) AS lift,
    CASE
        WHEN lift > 1.5 THEN 'Strong Positive Association'
        WHEN lift > 1.0 THEN 'Positive Association'
        WHEN lift = 1.0 THEN 'Independent'
        ELSE 'Negative Association'
    END AS association_type
FROM association_rules
WHERE confidence >= 0.5  -- 最小置信度阈值
  AND lift > 1.0  -- 只保留正相关规则
ORDER BY lift DESC, confidence DESC;

-- 创建索引加速物化视图查询
CREATE INDEX idx_association_rules_cache_lift ON association_rules_cache(lift DESC);
CREATE INDEX idx_association_rules_cache_confidence ON association_rules_cache(confidence DESC);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY association_rules_cache;
```

**2. 实时关联规则挖掘：增量频繁项集更新**

**实时关联规则挖掘**：对于实时数据，使用增量方法更新频繁项集。

```sql
-- 实时关联规则挖掘：增量频繁项集更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'frequent_itemsets_state') THEN
            CREATE TABLE frequent_itemsets_state (
                itemset_hash VARCHAR(100) PRIMARY KEY,
                itemset INTEGER[] NOT NULL,
                itemset_size INTEGER NOT NULL,
                support_count BIGINT DEFAULT 0,
                last_updated TIMESTAMPTZ DEFAULT NOW()
            );

            CREATE INDEX idx_frequent_itemsets_state_size_support ON frequent_itemsets_state(itemset_size, support_count DESC);
            CREATE INDEX idx_frequent_itemsets_state_updated ON frequent_itemsets_state(last_updated DESC);

            RAISE NOTICE '频繁项集状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量频繁项集更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量频繁项集更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新频繁项集：实时关联规则挖掘
WITH new_transactions AS (
    SELECT
        transaction_id,
        array_agg(item_id ORDER BY item_id) AS itemset
    FROM transaction_items
    WHERE transaction_date > (SELECT COALESCE(MAX(last_updated), '1970-01-01') FROM frequent_itemsets_state)
      AND transaction_date <= CURRENT_DATE
    GROUP BY transaction_id
),
single_itemsets AS (
    SELECT DISTINCT
        unnest(itemset) AS item_id,
        COUNT(DISTINCT transaction_id) AS support_count
    FROM new_transactions
    GROUP BY unnest(itemset)
)
-- 更新或插入单项集（简化版，实际需要递归生成多频项集）
INSERT INTO frequent_itemsets_state (itemset_hash, itemset, itemset_size, support_count, last_updated)
SELECT
    item_id::text AS itemset_hash,
    ARRAY[item_id] AS itemset,
    1 AS itemset_size,
    COALESCE((SELECT support_count FROM frequent_itemsets_state WHERE itemset_hash = item_id::text), 0) + si.support_count AS new_support_count,
    NOW()
FROM single_itemsets si
WHERE si.support_count >= 10  -- 最小支持度阈值
ON CONFLICT (itemset_hash)
DO UPDATE SET
    support_count = EXCLUDED.support_count,
    last_updated = NOW();
```

**3. 智能关联规则挖掘：自适应参数选择**

**智能关联规则挖掘**：根据数据特征自动调整挖掘参数。

```sql
-- 智能关联规则挖掘：自适应参数选择（带错误处理和性能测试）
DO $$
DECLARE
    total_transactions BIGINT;
    avg_items_per_transaction NUMERIC;
    min_support_pct NUMERIC;
    min_confidence_pct NUMERIC;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_items') THEN
            RAISE WARNING '表 transaction_items 不存在，无法执行智能关联规则挖掘';
            RETURN;
        END IF;

        -- 计算数据特征
        SELECT COUNT(DISTINCT transaction_id) INTO total_transactions FROM transaction_items;
        SELECT AVG(item_count) INTO avg_items_per_transaction
        FROM (
            SELECT transaction_id, COUNT(*) AS item_count
            FROM transaction_items
            GROUP BY transaction_id
        ) AS transaction_stats;

        -- 根据数据特征自适应调整参数
        IF total_transactions < 10000 THEN
            min_support_pct := 5.0;  -- 小数据集使用较低支持度
            min_confidence_pct := 60.0;  -- 小数据集使用中等置信度
        ELSIF total_transactions < 100000 THEN
            min_support_pct := 2.0;  -- 中等数据集使用中等支持度
            min_confidence_pct := 70.0;  -- 中等数据集使用较高置信度
        ELSE
            min_support_pct := 1.0;  -- 大数据集使用较低支持度
            min_confidence_pct := 75.0;  -- 大数据集使用高置信度
        END IF;

        RAISE NOTICE '总事务数: %, 平均每事务项数: %, 最小支持度: %%%, 最小置信度: %%',
            total_transactions, avg_items_per_transaction, min_support_pct, min_confidence_pct;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '智能关联规则挖掘准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 智能关联规则挖掘：使用自适应参数
WITH adaptive_frequent_itemsets AS (
    SELECT
        item_id,
        COUNT(DISTINCT transaction_id) AS support_count,
        ROUND((COUNT(DISTINCT transaction_id)::numeric / NULLIF((SELECT COUNT(DISTINCT transaction_id) FROM transaction_items), 0) * 100)::numeric, 2) AS support_percentage
    FROM transaction_items
    GROUP BY item_id
    HAVING COUNT(DISTINCT transaction_id) >=
        (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) *
        CASE
            WHEN (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) < 10000 THEN 0.05
            WHEN (SELECT COUNT(DISTINCT transaction_id) FROM transaction_items) < 100000 THEN 0.02
            ELSE 0.01
        END  -- 自适应最小支持度
)
SELECT
    item_id,
    support_count,
    support_percentage,
    CASE
        WHEN support_percentage >= 50 THEN 'Very Frequent'
        WHEN support_percentage >= 20 THEN 'Frequent'
        WHEN support_percentage >= 10 THEN 'Moderate'
        ELSE 'Low Frequency'
    END AS frequency_category
FROM adaptive_frequent_itemsets
ORDER BY support_count DESC
LIMIT 50;
```

---

## 📚 参考资源

### 学术文献

1. **Agrawal, R., et al. (1993)**: "Mining Association Rules between Sets of Items in Large Databases", *ACM SIGMOD* - Apriori算法

2. **Han, J., et al. (2000)**: "Mining Frequent Patterns without Candidate Generation", *ACM SIGMOD* - FP-Growth算法

3. **《数据挖掘：概念与技术》**（Han et al., 2011）- 第6章 挖掘频繁模式、关联和相关性

4. **《机器学习》**（周志华, 2016）- 第13章 半监督学习

### PostgreSQL官方文档

- **窗口函数**: <https://www.postgresql.org/docs/current/tutorial-window.html>
- **聚合函数**: <https://www.postgresql.org/docs/current/functions-aggregate.html>
- **数组操作**: <https://www.postgresql.org/docs/current/functions-array.html>

### 在线资源

- **关联规则挖掘**: <https://en.wikipedia.org/wiki/Association_rule_learning>
- **Apriori算法**: <https://en.wikipedia.org/wiki/Apriori_algorithm>
- **FP-Growth算法**: <https://en.wikipedia.org/wiki/FP-growth_algorithm>

### 相关算法

- **聚类分析**：发现数据分组
- **分类算法**：预测类别
- **推荐算法**：个性化推荐
- **异常检测**：发现异常模式

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**文档状态**: ✅ 已完成
