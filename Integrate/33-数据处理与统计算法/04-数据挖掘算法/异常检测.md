...
- [PostgreSQL 异常检测完整指南](#postgresql-异常检测完整指南)
  ...
  - [7. PostgreSQL 18并行异常检测](#7-postgresql-18并行异常检测)
    - [7.1 并行异常检测概述](#71-并行异常检测概述)
      - [并行异常检测配置](#并行异常检测配置)
      - [并行异常检测实现](#并行异常检测实现)
  - [8. 算法性能对比与优化](#8-算法性能对比与优化)
    - [8.1 异常检测方法对比](#81-异常检测方法对比)
    - [8.2 性能优化建议](#82-性能优化建议)
    - [8.3 常见问题与解决方案](#83-常见问题与解决方案)
  - [9. 最佳实践](#9-最佳实践)
    - [9.1 异常检测方法选择](#91-异常检测方法选择)
    - [9.2 异常检测注意事项](#92-异常检测注意事项)
...
## 7. PostgreSQL 18并行异常检测

### 7.1 并行异常检测概述

**PostgreSQL 18并行异常检测**：利用PostgreSQL 18的并行查询能力，显著提升大规模异常检测的性能。

#### 并行异常检测配置

```sql
-- 配置并行异常检测参数（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 设置并行工作进程数
        ...
        RAISE NOTICE '并行异常检测参数已配置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行异常检测配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
```sql
-- 并行异常检测：大规模数据异常值识别（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 启用并行查询
        ...
        RAISE NOTICE '开始执行并行异常检测';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行异常检测准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
SELECT
    ...
    COUNT(*) FILTER (WHERE ABS(z_score) > 3) AS extreme_outliers,
    COUNT(*) FILTER (WHERE ABS(z_score) > 2) AS outliers,
    ROUND((COUNT(*) FILTER (WHERE ABS(z_score) > 2)::numeric / COUNT(*) * 100)::numeric, 2) AS outlier_percentage
FROM parallel_z_scores
GROUP BY column_name
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升异常检测的性能：

1. **Skip Scan优化**：
   - 对于包含异常得分的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N异常查询和异常阈值查询

2. **异步I/O增强**：
   - 对于大规模异常检测计算，异步I/O可以显著提升性能
   - 适用于批量异常检测和多维度异常检测

3. **并行查询增强**：
   - 异常检测支持更好的并行执行（已在7节详细说明）
   - 适用于大规模统计异常检测和Z-score计算

**示例：使用Skip Scan优化异常检测查询**

```sql
-- 为异常得分创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_anomaly_scores_skip_scan
ON anomaly_scores(variable_name, anomaly_score DESC);

-- Skip Scan优化查询：查找每个变量异常得分最高的前10个数据点
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT DISTINCT ON (variable_name)
    ...
    anomaly_type
FROM anomaly_scores
WHERE anomaly_score > 2.0  -- 异常阈值
ORDER BY variable_name, anomaly_score DESC
LIMIT 50;
```
...
anomaly_detection AS (
    SELECT
        ...
        -- 使用窗口函数计算异常排名（避免重复计算）
        ROW_NUMBER() OVER (
            PARTITION BY sd.variable_name
            ORDER BY ABS((sd.value - ss.mean_value) / NULLIF(ss.stddev_value, 0)) DESC
        ) AS anomaly_rank
...
SELECT
    ...
    anomaly_rank,
    CASE
        WHEN z_score > 3 THEN 'Extreme Outlier'
...
-- 创建索引加速物化视图查询
CREATE INDEX idx_anomaly_detection_cache_variable_score ON anomaly_detection_cache(variable_name, z_score DESC);
CREATE INDEX idx_anomaly_detection_cache_status ON anomaly_detection_cache(anomaly_status, anomaly_rank);
...
**实时异常检测**：对于流式数据，使用增量方法进行异常检测。

```sql
-- 实时异常检测：流式异常检测（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'anomaly_detection_state') THEN
            ...
            CREATE INDEX idx_anomaly_detection_state_variable ON anomaly_detection_state(variable_name);
            CREATE INDEX idx_anomaly_detection_state_updated ON anomaly_detection_state(last_updated DESC);

            RAISE NOTICE '异常检测状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行实时异常检测';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '实时异常检测准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH new_samples AS (
    SELECT
        ...
        value
    FROM statistical_data
    WHERE created_at > (SELECT COALESCE(MAX(last_updated), '1970-01-01') FROM anomaly_detection_state)
      AND value IS NOT NULL
),
updated_statistics AS (
    SELECT
        COALESCE(ads.variable_name, ns.variable_name) AS variable_name,
        COALESCE(ads.sample_count, 0) + COUNT(*) AS new_sample_count,
        COALESCE(ads.sum_value, 0) + SUM(ns.value) AS new_sum_value,
        COALESCE(ads.sum_squared_value, 0) + SUM(POWER(ns.value, 2)) AS new_sum_squared_value,
        LEAST(COALESCE(ads.min_value, ns.value), MIN(ns.value)) AS new_min_value,
        GREATEST(COALESCE(ads.max_value, ns.value), MAX(ns.value)) AS new_max_value
    FROM anomaly_detection_state ads
    FULL OUTER JOIN new_samples ns ON ads.variable_name = ns.variable_name
    GROUP BY ads.variable_name, ns.variable_name, ads.sample_count, ads.sum_value, ads.sum_squared_value, ads.min_value, ads.max_value
)
-- 更新或插入异常检测状态
...
**多维度异常检测**：组合多种异常检测方法提高准确性。

```sql
-- 多维度异常检测：组合异常检测方法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'statistical_data') THEN
            RAISE WARNING '表 statistical_data 不存在，无法执行多维度异常检测';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行多维度异常检测';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多维度异常检测准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 多维度异常检测：组合Z-score、IQR和移动平均方法
WITH statistical_summary AS (
    SELECT
        variable_name,
        AVG(value) AS mean_value,
...
multi_method_anomaly AS (
    SELECT
        ...
        CASE
            WHEN ss.stddev_value > 0 THEN
                ABS((ms.value - ss.mean_value) / ss.stddev_value)
            ELSE NULL
        END AS global_z_score,
        -- 移动Z-score方法
        CASE
            WHEN ms.moving_stddev_10 > 0 THEN
                ABS((ms.value - ms.moving_avg_10) / ms.moving_stddev_10)
            ELSE NULL
        END AS local_z_score,
        -- IQR方法
        CASE
            WHEN ms.value < ss.q1_value - 1.5 * ss.iqr_value OR
                 ms.value > ss.q3_value + 1.5 * ss.iqr_value THEN TRUE
            ELSE FALSE
        END AS is_iqr_outlier,
        -- 移动平均方法
        CASE
            WHEN ms.moving_stddev_10 > 0 THEN
                ABS(ms.value - ms.moving_avg_10) > 3 * ms.moving_stddev_10
            ELSE FALSE
        END AS is_moving_avg_outlier
    FROM moving_statistics ms
    JOIN statistical_summary ss ON ms.variable_name = ss.variable_name
)
SELECT
    sample_id,
    variable_name,
    ROUND(value::numeric, 4) AS value,
    ROUND(global_z_score::numeric, 4) AS global_z_score,
    ROUND(local_z_score::numeric, 4) AS local_z_score,
    is_iqr_outlier,
    is_moving_avg_outlier,
    -- 组合异常得分：多种方法的加权平均
    (
        COALESCE(global_z_score, 0) * 0.4 +
        COALESCE(local_z_score, 0) * 0.3 +
        CASE WHEN is_iqr_outlier THEN 2.0 ELSE 0.0 END * 0.2 +
        CASE WHEN is_moving_avg_outlier THEN 2.0 ELSE 0.0 END * 0.1
    ) AS combined_anomaly_score,
    CASE
        WHEN (COALESCE(global_z_score, 0) > 3 OR
              COALESCE(local_z_score, 0) > 3 OR
              is_iqr_outlier OR
              is_moving_avg_outlier) THEN 'Anomaly'
        ELSE 'Normal'
    END AS anomaly_status,
    -- 异常方法投票
    (
        CASE WHEN global_z_score > 3 THEN 1 ELSE 0 END +
        CASE WHEN local_z_score > 3 THEN 1 ELSE 0 END +
        CASE WHEN is_iqr_outlier THEN 1 ELSE 0 END +
        CASE WHEN is_moving_avg_outlier THEN 1 ELSE 0 END
    ) AS anomaly_votes
FROM multi_method_anomaly
WHERE (COALESCE(global_z_score, 0) > 2 OR
       COALESCE(local_z_score, 0) > 2 OR
       is_iqr_outlier OR
       is_moving_avg_outlier)
