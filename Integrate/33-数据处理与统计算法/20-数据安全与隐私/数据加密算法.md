# PostgreSQL 数据加密算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 数据安全 | 加密算法 | 哈希函数
> **难度级别**: ⭐⭐⭐⭐ (高级)
> **参考标准**: Cryptography, Data Encryption, Security

---

## 📋 目录

- [PostgreSQL 数据加密算法完整指南](#postgresql-数据加密算法完整指南)
  - [📋 目录](#-目录)
  - [数据加密概述](#数据加密概述)
  - [理论基础](#理论基础)
    - [数据加密问题定义](#数据加密问题定义)
    - [核心概念](#核心概念)
    - [算法分类](#算法分类)
    - [加密类型对比](#加密类型对比)
    - [技术栈](#技术栈)
  - [1. 对称加密](#1-对称加密)
    - [1.1 AES加密](#11-aes加密)
    - [1.2 加密函数封装](#12-加密函数封装)
  - [2. 非对称加密](#2-非对称加密)
    - [2.1 RSA加密](#21-rsa加密)
  - [3. 哈希函数](#3-哈希函数)
    - [3.1 MD5和SHA哈希](#31-md5和sha哈希)
    - [3.2 密码存储最佳实践](#32-密码存储最佳实践)
    - [3.3 数据完整性验证](#33-数据完整性验证)
  - [4. 加密性能分析](#4-加密性能分析)
    - [4.1 加密性能测试](#41-加密性能测试)
  - [5. PostgreSQL 18 并行数据加密增强](#5-postgresql-18-并行数据加密增强)
    - [5.1 并行数据加密原理](#51-并行数据加密原理)
    - [5.2 并行批量加密](#52-并行批量加密)
    - [5.3 并行数据完整性验证](#53-并行数据完整性验证)
  - [6. 实际应用案例](#6-实际应用案例)
    - [5.1 敏感数据加密存储](#51-敏感数据加密存储)
    - [5.2 密码管理系统](#52-密码管理系统)
    - [5.3 数据完整性验证系统](#53-数据完整性验证系统)
  - [📊 性能优化建议](#-性能优化建议)
    - [密钥管理](#密钥管理)
    - [加密算法选择](#加密算法选择)
    - [批量加密优化](#批量加密优化)
    - [索引优化](#索引优化)
  - [🎯 最佳实践](#-最佳实践)
    - [密钥安全](#密钥安全)
    - [密码策略](#密码策略)
    - [数据分类](#数据分类)
    - [SQL实现注意事项](#sql实现注意事项)
  - [📈 加密方法对比](#-加密方法对比)
  - [🔍 常见问题与解决方案](#-常见问题与解决方案)
    - [问题1：加密性能慢](#问题1加密性能慢)
    - [问题2：密钥泄露风险](#问题2密钥泄露风险)
    - [问题3：密码存储不安全](#问题3密码存储不安全)
  - [📚 参考资源](#-参考资源)

---

## 数据加密概述

**数据加密**保护数据的机密性和完整性，是数据安全的核心技术。

## 理论基础

### 数据加密问题定义

数据加密是通过数学算法将明文转换为密文的过程，确保只有拥有密钥的授权用户才能解密和访问原始数据。

### 核心概念

1. **加密类型**：
   - **对称加密**：加密和解密使用同一密钥，速度快，适合大数据量
   - **非对称加密**：使用公钥加密、私钥解密，安全性高，适合密钥交换
   - **哈希函数**：单向函数，不可逆，用于密码存储和数据完整性验证

2. **加密算法**：
   - **AES（Advanced Encryption Standard）**：对称加密标准，支持128/192/256位密钥
   - **RSA**：非对称加密算法，基于大数分解难题
   - **SHA系列**：安全哈希算法，用于数据完整性验证

3. **密钥管理**：
   - 密钥生成：使用安全的随机数生成器
   - 密钥存储：安全存储密钥，避免泄露
   - 密钥轮换：定期更新密钥

### 算法分类

1. **按加密方式分类**：
   - 对称加密：AES、DES、3DES
   - 非对称加密：RSA、ECC、ElGamal
   - 哈希函数：MD5、SHA-1、SHA-256、SHA-512

2. **按应用场景分类**：
   - 数据加密：保护存储数据
   - 传输加密：保护传输中的数据
   - 密码存储：安全存储用户密码

### 加密类型对比

| 类型 | 特点 | 应用场景 | 优势 | 局限性 |
|------|------|---------|------|--------|
| **对称加密** | 加密解密使用同一密钥 | 大数据量加密 | 速度快、效率高 | 密钥分发困难 |
| **非对称加密** | 公钥加密、私钥解密 | 密钥交换、数字签名 | 安全性高、密钥管理简单 | 速度慢、计算复杂 |
| **哈希函数** | 单向函数，不可逆 | 密码存储、数据完整性 | 不可逆、快速验证 | 无法解密 |

### 技术栈

- **pgcrypto扩展**：PostgreSQL加密扩展
- **加密函数**：encrypt、decrypt、digest、gen_random_bytes
- **哈希函数**：md5、sha1、sha256、sha512
- **密钥管理**：密钥生成、存储、轮换

---

## 1. 对称加密

### 1.1 AES加密

**AES（Advanced Encryption Standard）**是常用的对称加密算法。

**密钥长度**：128位、192位、256位

```sql
-- 数据加密准备（带错误处理）
DO $$
BEGIN
    BEGIN
        -- 检查pgcrypto扩展
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgcrypto') THEN
            RAISE WARNING 'pgcrypto扩展不存在，请先安装: CREATE EXTENSION pgcrypto;';
        ELSE
            RAISE NOTICE 'pgcrypto扩展已安装';
        END IF;

        -- 创建敏感数据表
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensitive_table') THEN
            DROP TABLE sensitive_table CASCADE;
        END IF;

        CREATE TABLE sensitive_table (
            id SERIAL PRIMARY KEY,
            sensitive_data TEXT NOT NULL,
            encrypted_data BYTEA,
            encryption_key TEXT DEFAULT 'my_secret_key_12345'  -- 实际应用中应使用更安全的密钥管理
        );

        -- 插入示例敏感数据
        INSERT INTO sensitive_table (sensitive_data) VALUES
            ('Credit Card: 1234-5678-9012-3456'),
            ('SSN: 123-45-6789'),
            ('Password: MySecret123'),
            ('Email: user@example.com');

        RAISE NOTICE '敏感数据表创建成功，共 % 条记录', (SELECT COUNT(*) FROM sensitive_table);
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- AES加密：使用pgcrypto扩展
-- 注意：需要先安装扩展 CREATE EXTENSION IF NOT EXISTS pgcrypto;
UPDATE sensitive_table
SET encrypted_data = encrypt(
    sensitive_data::bytea,
    encryption_key::bytea,
    'aes'
)
WHERE encrypted_data IS NULL;

-- 查看加密结果
SELECT
    id,
    sensitive_data,
    encrypted_data,
    LENGTH(encrypted_data) AS encrypted_length,
    encode(encrypted_data, 'hex') AS encrypted_hex
FROM sensitive_table
LIMIT 5;

-- AES解密
SELECT
    id,
    sensitive_data AS original_data,
    decrypt(encrypted_data, encryption_key::bytea, 'aes')::TEXT AS decrypted_data,
    CASE
        WHEN decrypt(encrypted_data, encryption_key::bytea, 'aes')::TEXT = sensitive_data
        THEN 'Decryption successful ✓'
        ELSE 'Decryption failed ✗'
    END AS decryption_status
FROM sensitive_table
WHERE encrypted_data IS NOT NULL
LIMIT 5;
```

### 1.2 加密函数封装

```sql
-- 创建加密函数
CREATE OR REPLACE FUNCTION encrypt_sensitive_data(
    p_data TEXT,
    p_key TEXT DEFAULT 'default_key'
)
RETURNS BYTEA AS $$
BEGIN
    -- 检查扩展
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgcrypto') THEN
        RAISE EXCEPTION 'pgcrypto扩展未安装';
    END IF;

    RETURN encrypt(p_data::bytea, p_key::bytea, 'aes');
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '加密失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 创建解密函数
CREATE OR REPLACE FUNCTION decrypt_sensitive_data(
    p_encrypted_data BYTEA,
    p_key TEXT DEFAULT 'default_key'
)
RETURNS TEXT AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgcrypto') THEN
        RAISE EXCEPTION 'pgcrypto扩展未安装';
    END IF;

    RETURN decrypt(p_encrypted_data, p_key::bytea, 'aes')::TEXT;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '解密失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 使用加密函数
SELECT
    id,
    encrypt_sensitive_data(sensitive_data, encryption_key) AS encrypted,
    decrypt_sensitive_data(
        encrypt_sensitive_data(sensitive_data, encryption_key),
        encryption_key
    ) AS decrypted
FROM sensitive_table
LIMIT 3;
```

---

## 2. 非对称加密

### 2.1 RSA加密

**RSA加密**使用公钥加密、私钥解密。

```sql
-- RSA密钥对生成（简化：使用pgcrypto）
-- 注意：实际应用中应使用专门的密钥管理工具
DO $$
DECLARE
    public_key TEXT;
    private_key TEXT;
BEGIN
    BEGIN
        -- 生成RSA密钥对（简化实现）
        -- 实际应用中应使用openssl或专门的密钥管理工具
        RAISE NOTICE 'RSA密钥对生成需要外部工具，这里仅演示概念';

        -- 示例：存储公钥和私钥（实际应用中应安全存储）
        CREATE TABLE IF NOT EXISTS rsa_keys (
            key_id SERIAL PRIMARY KEY,
            public_key TEXT NOT NULL,
            private_key TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );

        RAISE NOTICE 'RSA密钥表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION 'RSA密钥生成失败: %', SQLERRM;
    END;
END $$;

-- RSA加密示例（概念演示）
-- 实际实现需要使用pgcrypto的pgp_pub_encrypt函数
SELECT
    'RSA encryption requires pgcrypto extension and proper key management' AS note;
```

---

## 3. 哈希函数

### 3.1 MD5和SHA哈希

**哈希函数**用于密码存储和数据完整性验证。

```sql
-- 哈希函数：MD5和SHA
WITH hash_examples AS (
    SELECT
        'password123' AS plaintext,
        md5('password123') AS md5_hash,
        encode(digest('password123', 'sha1'), 'hex') AS sha1_hash,
        encode(digest('password123', 'sha256'), 'hex') AS sha256_hash
)
SELECT
    plaintext,
    md5_hash,
    sha1_hash,
    LEFT(sha256_hash, 32) AS sha256_hash_short,
    LENGTH(md5_hash) AS md5_length,
    LENGTH(sha1_hash) AS sha1_length,
    LENGTH(sha256_hash) AS sha256_length
FROM hash_examples;
```

### 3.2 密码存储最佳实践

```sql
-- 密码存储：使用加盐哈希
CREATE OR REPLACE FUNCTION hash_password(
    p_password TEXT,
    p_salt TEXT DEFAULT NULL
)
RETURNS TEXT AS $$
DECLARE
    salt_value TEXT;
    hashed_password TEXT;
BEGIN
    -- 生成随机盐（如果未提供）
    IF p_salt IS NULL THEN
        salt_value := encode(gen_random_bytes(16), 'hex');
    ELSE
        salt_value := p_salt;
    END IF;

    -- 使用SHA-256加盐哈希
    hashed_password := encode(
        digest(p_password || salt_value, 'sha256'),
        'hex'
    );

    -- 返回：盐:哈希值（便于存储和验证）
    RETURN salt_value || ':' || hashed_password;
END;
$$ LANGUAGE plpgsql;

-- 密码验证函数
CREATE OR REPLACE FUNCTION verify_password(
    p_password TEXT,
    p_stored_hash TEXT
)
RETURNS BOOLEAN AS $$
DECLARE
    salt_part TEXT;
    hash_part TEXT;
    computed_hash TEXT;
BEGIN
    -- 分离盐和哈希值
    salt_part := SPLIT_PART(p_stored_hash, ':', 1);
    hash_part := SPLIT_PART(p_stored_hash, ':', 2);

    -- 计算哈希值
    computed_hash := encode(
        digest(p_password || salt_part, 'sha256'),
        'hex'
    );

    -- 比较哈希值
    RETURN computed_hash = hash_part;
END;
$$ LANGUAGE plpgsql;

-- 使用密码哈希函数
WITH password_test AS (
    SELECT
        'MySecurePassword123' AS password,
        hash_password('MySecurePassword123') AS stored_hash
)
SELECT
    password,
    stored_hash,
    verify_password('MySecurePassword123', stored_hash) AS verification_result,
    verify_password('WrongPassword', stored_hash) AS wrong_password_result
FROM password_test;
```

### 3.3 数据完整性验证

```sql
-- 数据完整性：使用哈希验证数据是否被篡改
CREATE OR REPLACE FUNCTION compute_data_hash(
    p_data TEXT
)
RETURNS TEXT AS $$
BEGIN
    RETURN encode(digest(p_data, 'sha256'), 'hex');
END;
$$ LANGUAGE plpgsql;

-- 数据完整性检查
WITH data_integrity_check AS (
    SELECT
        id,
        sensitive_data,
        compute_data_hash(sensitive_data) AS data_hash,
        -- 模拟存储的哈希值
        compute_data_hash(sensitive_data) AS stored_hash
    FROM sensitive_table
)
SELECT
    id,
    LEFT(data_hash, 16) AS data_hash_short,
    LEFT(stored_hash, 16) AS stored_hash_short,
    CASE
        WHEN data_hash = stored_hash THEN 'Data integrity verified ✓'
        ELSE 'Data integrity check failed ✗'
    END AS integrity_status
FROM data_integrity_check
LIMIT 5;
```

---

## 4. 加密性能分析

### 4.1 加密性能测试

```sql
-- 加密性能测试
DO $$
DECLARE
    start_time TIMESTAMP;
    end_time TIMESTAMP;
    elapsed INTERVAL;
    test_data TEXT := 'This is a test data for encryption performance testing';
    encrypted_result BYTEA;
BEGIN
    start_time := clock_timestamp();

    -- 执行100次加密
    FOR i IN 1..100 LOOP
        encrypted_result := encrypt(test_data::bytea, 'test_key'::bytea, 'aes');
    END LOOP;

    end_time := clock_timestamp();
    elapsed := end_time - start_time;

    RAISE NOTICE 'AES加密100次，耗时: %', elapsed;
    RAISE NOTICE '平均每次加密耗时: %', elapsed / 100;
END $$;
```

---

## 5. PostgreSQL 18 并行数据加密增强

**PostgreSQL 18** 显著增强了并行数据加密计算能力，支持并行执行AES加密、哈希函数计算和数据完整性验证，大幅提升大规模数据加密处理的性能。

### 5.1 并行数据加密原理

PostgreSQL 18 的并行数据加密通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描待加密数据
2. **并行加密计算**：每个工作进程独立执行加密操作
3. **并行哈希计算**：并行执行哈希函数计算
4. **结果合并**：主进程合并所有工作进程的计算结果

### 5.2 并行批量加密

```sql
-- PostgreSQL 18 并行批量加密（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_sensitive_info') THEN
            RAISE WARNING '表 user_sensitive_info 不存在，无法执行并行批量加密';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行批量加密';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行批量加密准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行哈希计算
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    user_id,
    username,
    encode(digest(sensitive_data::text, 'sha256'), 'hex') AS hashed_data,
    encode(digest(sensitive_data::text, 'md5'), 'hex') AS md5_hash
FROM user_sensitive_info
ORDER BY user_id;
```

### 5.3 并行数据完整性验证

```sql
-- PostgreSQL 18 并行数据完整性验证（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_sensitive_info') THEN
            RAISE WARNING '表 user_sensitive_info 不存在，无法执行并行数据完整性验证';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行数据完整性验证';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行数据完整性验证准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行完整性校验
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    user_id,
    encode(digest(sensitive_data::text, 'sha256'), 'hex') AS current_hash,
    stored_hash,
    CASE
        WHEN encode(digest(sensitive_data::text, 'sha256'), 'hex') = stored_hash THEN 'Valid ✓'
        ELSE 'Invalid ✗'
    END AS integrity_status
FROM user_sensitive_info
ORDER BY user_id;
```

---

## 6. 实际应用案例

### 5.1 敏感数据加密存储

```sql
-- 敏感数据加密存储应用（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 创建用户敏感信息表
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_sensitive_info') THEN
            DROP TABLE user_sensitive_info CASCADE;
        END IF;

        CREATE TABLE user_sensitive_info (
            user_id SERIAL PRIMARY KEY,
            username VARCHAR(50) NOT NULL UNIQUE,
            email_encrypted BYTEA,
            phone_encrypted BYTEA,
            ssn_encrypted BYTEA,
            encryption_key TEXT DEFAULT 'user_encryption_key_12345'
        );

        -- 插入示例用户数据（加密存储）
        INSERT INTO user_sensitive_info (username, email_encrypted, phone_encrypted, ssn_encrypted) VALUES
            ('user1', encrypt('user1@example.com'::bytea, 'user_encryption_key_12345'::bytea, 'aes'),
                    encrypt('13800138000'::bytea, 'user_encryption_key_12345'::bytea, 'aes'),
                    encrypt('123-45-6789'::bytea, 'user_encryption_key_12345'::bytea, 'aes')),
            ('user2', encrypt('user2@example.com'::bytea, 'user_encryption_key_12345'::bytea, 'aes'),
                    encrypt('13900139000'::bytea, 'user_encryption_key_12345'::bytea, 'aes'),
                    encrypt('987-65-4321'::bytea, 'user_encryption_key_12345'::bytea, 'aes'));

        RAISE NOTICE '用户敏感信息表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '敏感数据加密存储准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查询加密数据（解密显示）
SELECT
    user_id,
    username,
    decrypt(email_encrypted, encryption_key::bytea, 'aes')::TEXT AS email,
    decrypt(phone_encrypted, encryption_key::bytea, 'aes')::TEXT AS phone,
    decrypt(ssn_encrypted, encryption_key::bytea, 'aes')::TEXT AS ssn
FROM user_sensitive_info
ORDER BY user_id;
```

### 5.2 密码管理系统

```sql
-- 密码管理系统应用（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 创建用户密码表
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_passwords') THEN
            DROP TABLE user_passwords CASCADE;
        END IF;

        CREATE TABLE user_passwords (
            user_id SERIAL PRIMARY KEY,
            username VARCHAR(50) NOT NULL UNIQUE,
            password_hash TEXT NOT NULL,
            salt TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );

        -- 插入示例用户密码（使用加盐哈希）
        INSERT INTO user_passwords (username, password_hash, salt) VALUES
            ('admin', hash_password('Admin123!', NULL), SPLIT_PART(hash_password('Admin123!', NULL), ':', 1)),
            ('user1', hash_password('User123!', NULL), SPLIT_PART(hash_password('User123!', NULL), ':', 1)),
            ('user2', hash_password('Password123!', NULL), SPLIT_PART(hash_password('Password123!', NULL), ':', 1));

        RAISE NOTICE '用户密码表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '密码管理系统准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 密码验证
SELECT
    username,
    CASE
        WHEN verify_password('Admin123!', password_hash) THEN 'Password correct ✓'
        ELSE 'Password incorrect ✗'
    END AS verification_result
FROM user_passwords
WHERE username = 'admin';
```

### 5.3 数据完整性验证系统

```sql
-- 数据完整性验证系统应用（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 创建数据完整性表
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_integrity_records') THEN
            DROP TABLE data_integrity_records CASCADE;
        END IF;

        CREATE TABLE data_integrity_records (
            record_id SERIAL PRIMARY KEY,
            data_content TEXT NOT NULL,
            data_hash TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );

        -- 插入示例数据（带哈希值）
        INSERT INTO data_integrity_records (data_content, data_hash) VALUES
            ('Important document content 1', compute_data_hash('Important document content 1')),
            ('Important document content 2', compute_data_hash('Important document content 2')),
            ('Important document content 3', compute_data_hash('Important document content 3'));

        RAISE NOTICE '数据完整性记录表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '数据完整性验证系统准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 数据完整性检查
SELECT
    record_id,
    LEFT(data_content, 30) AS data_preview,
    LEFT(data_hash, 16) AS stored_hash_short,
    LEFT(compute_data_hash(data_content), 16) AS computed_hash_short,
    CASE
        WHEN compute_data_hash(data_content) = data_hash THEN 'Integrity verified ✓'
        ELSE 'Integrity check failed ✗'
    END AS integrity_status
FROM data_integrity_records
ORDER BY record_id;
```

---

## 📊 性能优化建议

### 密钥管理

```sql
-- 密钥管理最佳实践
-- 1. 使用专门的密钥管理服务（如AWS KMS、Azure Key Vault）
-- 2. 定期轮换密钥
-- 3. 安全存储密钥，避免硬编码
```

### 加密算法选择

```sql
-- 根据性能和安全需求选择算法
-- AES-256：高安全性，适合敏感数据
-- AES-128：平衡性能和安全性
-- RSA：适合密钥交换，不适合大数据量加密
```

### 批量加密优化

```sql
-- 批量加密：使用事务批量处理
BEGIN;
UPDATE sensitive_table
SET encrypted_data = encrypt(sensitive_data::bytea, encryption_key::bytea, 'aes')
WHERE encrypted_data IS NULL;
COMMIT;
```

### 索引优化

```sql
-- 加密字段无法直接索引，需要创建函数索引
CREATE INDEX IF NOT EXISTS idx_username ON user_sensitive_info(username);
-- 注意：加密字段本身不能创建索引，只能对未加密字段创建索引
```

---

## 🎯 最佳实践

### 密钥安全

1. **密钥生成**：使用安全的随机数生成器
2. **密钥存储**：安全存储密钥，避免泄露
3. **密钥轮换**：定期更新密钥
4. **密钥分离**：不同数据使用不同密钥

### 密码策略

1. **加盐哈希**：使用加盐哈希存储密码
2. **密码强度**：要求强密码策略
3. **密码更新**：定期更新密码
4. **密码验证**：使用安全的密码验证方法

### 数据分类

1. **敏感度分级**：根据数据敏感度选择加密级别
2. **加密范围**：确定需要加密的数据范围
3. **加密时机**：在存储和传输时加密
4. **访问控制**：结合访问控制保护数据

### SQL实现注意事项

1. **错误处理**：使用DO块和EXCEPTION进行错误处理
2. **性能考虑**：加密操作可能影响性能，需要优化
3. **密钥管理**：安全管理加密密钥
4. **数据备份**：加密数据备份时注意密钥安全

---

## 📈 加密方法对比

| 方法 | 安全性 | 性能 | 适用场景 | 优势 | 局限性 |
|------|--------|------|---------|------|--------|
| AES-256 | 高 | 高 | 大数据量加密 | 速度快、安全性高 | 密钥管理复杂 |
| RSA-2048 | 高 | 低 | 密钥交换、数字签名 | 安全性高、密钥管理简单 | 速度慢、不适合大数据量 |
| SHA-256 | 高 | 高 | 密码存储、数据完整性 | 不可逆、快速验证 | 无法解密 |

---

## 🔍 常见问题与解决方案

### 问题1：加密性能慢

**原因**：

- 数据量大
- 加密算法选择不当
- 未使用批量处理

**解决方案**：

- 使用对称加密（AES）代替非对称加密
- 使用批量加密处理
- 优化加密算法参数

### 问题2：密钥泄露风险

**原因**：

- 密钥硬编码
- 密钥存储不安全
- 密钥未定期轮换

**解决方案**：

- 使用密钥管理服务
- 安全存储密钥
- 定期轮换密钥

### 问题3：密码存储不安全

**原因**：

- 使用明文存储
- 使用弱哈希算法
- 未使用加盐

**解决方案**：

- 使用加盐哈希
- 使用强哈希算法（SHA-256）
- 使用bcrypt或argon2

---

## 📚 参考资源

1. **Stallings, W. (2017)**: "Cryptography and Network Security: Principles and Practice", 7th Edition
2. **Menezes, A.J., et al. (2018)**: "Handbook of Applied Cryptography"
3. **NIST (2020)**: "Guidelines for Cryptography"

---

**最后更新**: 2025年1月
**文档状态**: ✅ 已完成
