...
- [PostgreSQL 网络流算法完整指南](#postgresql-网络流算法完整指南)
  ...
  - [2. 最小割问题](#2-最小割问题)
    - [2.1 最小割原理](#21-最小割原理)
  - [3. PostgreSQL 18 并行网络流增强](#3-postgresql-18-并行网络流增强)
    - [3.1 并行网络流原理](#31-并行网络流原理)
    - [3.2 并行Ford-Fulkerson算法](#32-并行ford-fulkerson算法)
    - [3.3 并行增广路径查找](#33-并行增广路径查找)
  - [4. 实际应用案例](#4-实际应用案例)
...
## 3. PostgreSQL 18 并行网络流增强

**PostgreSQL 18** 显著增强了并行网络流计算能力，支持并行执行Ford-Fulkerson算法、Edmonds-Karp算法和增广路径查找，大幅提升大规模网络流问题的性能。

### 3.1 并行网络流原理

PostgreSQL 18 的并行网络流通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描网络边数据
2. **并行增广路径查找**：每个工作进程独立查找增广路径
3. **并行流量更新**：并行执行流量更新
4. **结果合并**：主进程合并所有工作进程的计算结果

### 3.2 并行Ford-Fulkerson算法

```sql
-- PostgreSQL 18 并行Ford-Fulkerson算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'flow_network') THEN
            RAISE WARNING '表 flow_network 不存在，无法执行并行Ford-Fulkerson算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Ford-Fulkerson算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Ford-Fulkerson算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH residual_graph AS (
    SELECT
        ...
        to_node,
        capacity - flow AS residual_capacity,
        CASE WHEN capacity - flow > 0 THEN true ELSE false END AS is_forward_edge
    FROM flow_network
    UNION ALL
    SELECT
        to_node AS from_node,
...
SELECT
    from_node,
    to_node,
    ROUND(residual_capacity::numeric, 4) AS residual_cap,
    is_forward_edge
FROM residual_graph
WHERE residual_capacity > 0
ORDER BY from_node, to_node;
```
...
```sql
-- PostgreSQL 18 并行增广路径查找（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'flow_network') THEN
            RAISE WARNING '表 flow_network 不存在，无法执行并行增广路径查找';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行增广路径查找';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行增广路径查找准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH RECURSIVE augmenting_path AS (
    ...
    JOIN residual_graph rg ON ap.source_node = rg.from_node
    WHERE rg.to_node != ALL(ap.path)
      AND rg.residual_capacity > 0
      AND ap.depth < 10
)
SELECT DISTINCT ON (path[ARRAY_LENGTH(path, 1)])
    path,
    ARRAY_LENGTH(path, 1) AS path_length
FROM augmenting_path
WHERE path[ARRAY_LENGTH(path, 1)] = (SELECT MAX(to_node) FROM flow_network)
ORDER BY path[ARRAY_LENGTH(path, 1)], path_length
LIMIT 10;
```
