# PostgreSQL 推荐算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 推荐算法 | 协同过滤
> **难度级别**: ⭐⭐⭐⭐⭐ (专家级)

---

## 📋 目录

- [PostgreSQL 推荐算法完整指南](#postgresql-推荐算法完整指南)
  - [📋 目录](#-目录)
  - [推荐算法概述](#推荐算法概述)
    - [理论基础](#理论基础)
      - [推荐系统定义](#推荐系统定义)
      - [推荐系统类型](#推荐系统类型)
      - [推荐系统评估指标](#推荐系统评估指标)
    - [核心算法](#核心算法)
  - [1. 协同过滤](#1-协同过滤)
    - [1.1 协同过滤原理](#11-协同过滤原理)
      - [协同过滤假设](#协同过滤假设)
      - [协同过滤优势](#协同过滤优势)
      - [协同过滤挑战](#协同过滤挑战)
    - [1.2 基于用户的协同过滤实现](#12-基于用户的协同过滤实现)
    - [1.2 基于物品的协同过滤](#12-基于物品的协同过滤)
    - [1.3 协同过滤预测公式](#13-协同过滤预测公式)
  - [2. 相似度计算](#2-相似度计算)
    - [2.1 相似度计算原理](#21-相似度计算原理)
      - [相似度度量方法](#相似度度量方法)
    - [2.2 余弦相似度实现](#22-余弦相似度实现)
    - [2.2 皮尔逊相关系数](#22-皮尔逊相关系数)
    - [2.3 Jaccard相似度](#23-jaccard相似度)
    - [2.4 调整余弦相似度](#24-调整余弦相似度)
  - [3. 推荐生成](#3-推荐生成)
    - [3.1 推荐生成原理](#31-推荐生成原理)
      - [推荐生成方法](#推荐生成方法)
    - [3.2 Top-N推荐实现](#32-top-n推荐实现)
    - [3.3 加权推荐生成](#33-加权推荐生成)
  - [4. 基于向量的推荐（pgvector）](#4-基于向量的推荐pgvector)
    - [4.1 向量推荐概述](#41-向量推荐概述)
      - [向量推荐原理](#向量推荐原理)
      - [pgvector扩展安装](#pgvector扩展安装)
    - [4.2 向量推荐实现](#42-向量推荐实现)
    - [4.3 向量推荐性能优化](#43-向量推荐性能优化)
  - [5. 内容过滤推荐](#5-内容过滤推荐)
    - [5.1 内容过滤原理](#51-内容过滤原理)
      - [内容过滤优势](#内容过滤优势)
      - [内容过滤挑战](#内容过滤挑战)
    - [5.2 内容过滤实现](#52-内容过滤实现)
  - [6. 混合推荐](#6-混合推荐)
    - [6.1 混合推荐原理](#61-混合推荐原理)
      - [混合策略](#混合策略)
    - [6.2 混合推荐实现](#62-混合推荐实现)
  - [7. PostgreSQL 18 并行推荐与AI增强](#7-postgresql-18-并行推荐与ai增强)
    - [7.1 并行推荐原理](#71-并行推荐原理)
    - [7.2 并行协同过滤推荐](#72-并行协同过滤推荐)
    - [7.3 并行向量推荐（pgvector AI增强）](#73-并行向量推荐pgvector-ai增强)
    - [7.4 并行混合推荐（协同过滤 + 向量推荐）](#74-并行混合推荐协同过滤--向量推荐)
  - [8. 实际应用案例](#8-实际应用案例)
    - [8.1 商品推荐](#81-商品推荐)
    - [7.2 电影推荐系统](#72-电影推荐系统)
    - [7.3 新闻推荐系统](#73-新闻推荐系统)
    - [7.4 推荐系统评估](#74-推荐系统评估)
  - [8. 算法性能对比与优化](#8-算法性能对比与优化)
    - [8.1 推荐算法对比](#81-推荐算法对比)
    - [8.2 性能优化建议](#82-性能优化建议)
    - [8.3 常见问题与解决方案](#83-常见问题与解决方案)
  - [9. 最佳实践](#9-最佳实践)
    - [9.1 推荐系统设计](#91-推荐系统设计)
    - [9.2 推荐系统注意事项](#92-推荐系统注意事项)
    - [9.3 SQL实现注意事项](#93-sql实现注意事项)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 推荐算法概述

**推荐算法（Recommendation Algorithm）**用于预测用户可能感兴趣的物品，广泛应用于电商、内容平台、社交媒体等场景。推荐系统是现代互联网应用的核心功能之一，能够显著提升用户体验和业务转化率。

### 理论基础

#### 推荐系统定义

**推荐系统（Recommendation System）**是一个信息过滤系统，通过分析用户的历史行为、偏好和特征，预测用户可能感兴趣的物品或内容。

#### 推荐系统类型

1. **协同过滤（Collaborative Filtering）**：
   - 基于用户的协同过滤（User-Based CF）
   - 基于物品的协同过滤（Item-Based CF）

2. **内容过滤（Content-Based Filtering）**：
   - 基于物品特征
   - 基于用户画像

3. **混合推荐（Hybrid Recommendation）**：
   - 加权混合
   - 切换混合
   - 级联混合

#### 推荐系统评估指标

1. **准确率指标**：
   - **MAE（Mean Absolute Error）**：$MAE = \frac{1}{n}\sum_{i=1}^{n}|r_i - \hat{r}_i|$
   - **RMSE（Root Mean Square Error）**：$RMSE = \sqrt{\frac{1}{n}\sum_{i=1}^{n}(r_i - \hat{r}_i)^2}$

2. **排序指标**：
   - **Precision@K**：前K个推荐中相关的比例
   - **Recall@K**：前K个推荐覆盖的相关物品比例
   - **NDCG（Normalized Discounted Cumulative Gain）**：考虑排序位置的指标

### 核心算法

| 算法 | 用途 | 复杂度 | 数学表示 |
|------|------|--------|---------|
| **协同过滤** | 基于用户行为 | $O(n^2)$ | $sim(u,v) = \frac{\vec{u} \cdot \vec{v}}{|\vec{u}||\vec{v}|}$ |
| **内容过滤** | 基于物品特征 | $O(nm)$ | $score(u,i) = \sum_{f} w_{u,f} \cdot f_i$ |
| **混合推荐** | 组合多种方法 | $O(n^2)$ | $score = \alpha \cdot CF + (1-\alpha) \cdot CB$ |

---

## 1. 协同过滤

### 1.1 协同过滤原理

**协同过滤（Collaborative Filtering）**基于"相似用户有相似偏好"的假设，通过分析用户行为数据来预测用户兴趣。

#### 协同过滤假设

1. **用户相似性假设**：如果用户A和用户B对某些物品有相似的评分，那么他们对其他物品也可能有相似的评分。
2. **物品相似性假设**：如果物品X和物品Y被相似用户喜欢，那么喜欢X的用户也可能喜欢Y。

#### 协同过滤优势

1. **无需物品特征**：只需要用户行为数据
2. **发现潜在兴趣**：可以发现用户自己都不知道的兴趣
3. **适应性强**：能够适应用户兴趣的变化

#### 协同过滤挑战

1. **冷启动问题**：新用户和新物品缺乏数据
2. **数据稀疏性**：用户-物品矩阵通常很稀疏
3. **可扩展性**：计算复杂度高

### 1.2 基于用户的协同过滤实现

```sql
-- 创建用户评分表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 已存在，先删除';
            DROP TABLE user_ratings CASCADE;
        END IF;

        CREATE TABLE user_ratings (
            user_id INTEGER NOT NULL,
            item_id INTEGER NOT NULL,
            rating NUMERIC NOT NULL CHECK (rating >= 1 AND rating <= 5),
            PRIMARY KEY (user_id, item_id)
        );

        -- 插入示例数据
        INSERT INTO user_ratings (user_id, item_id, rating) VALUES
            (1, 1, 5), (1, 2, 4), (1, 3, 3), (1, 4, 2),
            (2, 1, 4), (2, 2, 5), (2, 3, 4), (2, 5, 3),
            (3, 2, 3), (3, 3, 5), (3, 4, 4), (3, 5, 5),
            (4, 1, 2), (4, 3, 4), (4, 4, 5), (4, 5, 4);

        RAISE NOTICE '表 user_ratings 创建成功，已插入16条评分数据';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 user_ratings 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- 基于用户的协同过滤（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行协同过滤';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行基于用户的协同过滤';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '协同过滤准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算用户相似度（余弦相似度）
WITH user_vectors AS (
    SELECT
        user_id,
        item_id,
        rating
    FROM user_ratings
),
user_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        SUM(u1.rating * u2.rating) AS dot_product,
        SQRT(SUM(u1.rating * u1.rating)) AS norm1,
        SQRT(SUM(u2.rating * u2.rating)) AS norm2
    FROM user_vectors u1
    JOIN user_vectors u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
    GROUP BY u1.user_id, u2.user_id
)
SELECT
    user1_id,
    user2_id,
    ROUND((dot_product / NULLIF(norm1 * norm2, 0))::numeric, 4) AS cosine_similarity
FROM user_similarity
ORDER BY cosine_similarity DESC;

-- 为用户1推荐物品（基于相似用户）
WITH target_user AS (
    SELECT 1 AS user_id
),
user_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        SUM(u1.rating * u2.rating) /
        NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity
    FROM user_ratings u1
    JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
    WHERE u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user)
    GROUP BY u1.user_id, u2.user_id
),
similar_users AS (
    SELECT
        CASE WHEN user1_id = (SELECT user_id FROM target_user) THEN user2_id ELSE user1_id END AS similar_user_id,
        similarity
    FROM user_similarity
    WHERE similarity > 0.5  -- 相似度阈值
),
recommendations AS (
    SELECT
        r.item_id,
        SUM(r.rating * s.similarity) / SUM(s.similarity) AS predicted_rating,
        COUNT(*) AS recommendation_count
    FROM user_ratings r
    JOIN similar_users s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
)
SELECT
    item_id,
    ROUND(predicted_rating::numeric, 2) AS predicted_rating,
    recommendation_count
FROM recommendations
ORDER BY predicted_rating DESC
LIMIT 10;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    u1.user_id,
    u2.user_id,
    COUNT(*) AS common_items
FROM user_ratings u1
JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
GROUP BY u1.user_id, u2.user_id
LIMIT 100;
```

### 1.2 基于物品的协同过滤

**基于物品的协同过滤**找到相似物品，推荐给用户。

```sql
-- 基于物品的协同过滤（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行物品协同过滤';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行基于物品的协同过滤';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '物品协同过滤准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算物品相似度
WITH item_vectors AS (
    SELECT
        item_id,
        user_id,
        rating
    FROM user_ratings
),
item_similarity AS (
    SELECT
        i1.item_id AS item1_id,
        i2.item_id AS item2_id,
        SUM(i1.rating * i2.rating) AS dot_product,
        SQRT(SUM(i1.rating * i1.rating)) AS norm1,
        SQRT(SUM(i2.rating * i2.rating)) AS norm2
    FROM item_vectors i1
    JOIN item_vectors i2 ON i1.user_id = i2.user_id AND i1.item_id < i2.item_id
    GROUP BY i1.item_id, i2.item_id
)
SELECT
    item1_id,
    item2_id,
    ROUND((dot_product / NULLIF(norm1 * norm2, 0))::numeric, 4) AS cosine_similarity
FROM item_similarity
ORDER BY cosine_similarity DESC;

-- 为用户1推荐物品（基于物品相似度）
WITH target_user AS (
    SELECT 1 AS user_id
),
user_items AS (
    SELECT item_id, rating
    FROM user_ratings
    WHERE user_id = (SELECT user_id FROM target_user)
),
item_similarity AS (
    SELECT
        i1.item_id AS item1_id,
        i2.item_id AS item2_id,
        SUM(i1.rating * i2.rating) /
        NULLIF(SQRT(SUM(i1.rating * i1.rating)) * SQRT(SUM(i2.rating * i2.rating)), 0) AS similarity
    FROM user_ratings i1
    JOIN user_ratings i2 ON i1.user_id = i2.user_id AND i1.item_id < i2.item_id
    GROUP BY i1.item_id, i2.item_id
),
recommendations AS (
    SELECT
        CASE WHEN isim.item1_id = ui.item_id THEN isim.item2_id ELSE isim.item1_id END AS item_id,
        SUM(ui.rating * isim.similarity) / SUM(isim.similarity) AS predicted_rating
    FROM user_items ui
    JOIN item_similarity isim ON (isim.item1_id = ui.item_id OR isim.item2_id = ui.item_id)
    WHERE CASE WHEN isim.item1_id = ui.item_id THEN isim.item2_id ELSE isim.item1_id END
          NOT IN (SELECT item_id FROM user_items)
    GROUP BY CASE WHEN isim.item1_id = ui.item_id THEN isim.item2_id ELSE isim.item1_id END
)
SELECT
    item_id,
    ROUND(predicted_rating::numeric, 2) AS predicted_rating
FROM recommendations
ORDER BY predicted_rating DESC
LIMIT 10;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    i1.item_id,
    i2.item_id,
    COUNT(*) AS common_users
FROM user_ratings i1
JOIN user_ratings i2 ON i1.user_id = i2.user_id AND i1.item_id < i2.item_id
GROUP BY i1.item_id, i2.item_id
LIMIT 100;
```

---

### 1.3 协同过滤预测公式

**预测评分**：
$$\hat{r}_{u,i} = \bar{r}_u + \frac{\sum_{v \in N(u)} sim(u,v) \cdot (r_{v,i} - \bar{r}_v)}{\sum_{v \in N(u)} |sim(u,v)|}$$

其中：

- $\hat{r}_{u,i}$：用户$u$对物品$i$的预测评分
- $\bar{r}_u$：用户$u$的平均评分
- $N(u)$：与用户$u$相似的用户集合
- $sim(u,v)$：用户$u$和$v$的相似度
- $r_{v,i}$：用户$v$对物品$i$的评分

---

## 2. 相似度计算

### 2.1 相似度计算原理

**相似度计算**是协同过滤的核心，用于衡量用户或物品之间的相似程度。

#### 相似度度量方法

1. **余弦相似度（Cosine Similarity）**：衡量向量夹角
2. **皮尔逊相关系数（Pearson Correlation）**：衡量线性相关性
3. **Jaccard相似度**：衡量集合重叠度
4. **欧氏距离**：衡量向量距离

### 2.2 余弦相似度实现

```sql
-- 余弦相似度计算函数（带错误处理）
CREATE OR REPLACE FUNCTION cosine_similarity(vec1 NUMERIC[], vec2 NUMERIC[])
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    dot_product NUMERIC := 0;
    norm1 NUMERIC := 0;
    norm2 NUMERIC := 0;
    i INTEGER;
BEGIN
    BEGIN
        IF vec1 IS NULL OR vec2 IS NULL OR
           array_length(vec1, 1) IS NULL OR array_length(vec2, 1) IS NULL THEN
            RAISE WARNING '输入向量为空';
            RETURN NULL;
        END IF;

        IF array_length(vec1, 1) != array_length(vec2, 1) THEN
            RAISE WARNING '向量维度不匹配';
            RETURN NULL;
        END IF;

        -- 计算点积和范数
        FOR i IN 1..array_length(vec1, 1) LOOP
            dot_product := dot_product + vec1[i] * vec2[i];
            norm1 := norm1 + vec1[i] * vec1[i];
            norm2 := norm2 + vec2[i] * vec2[i];
        END LOOP;

        norm1 := SQRT(norm1);
        norm2 := SQRT(norm2);

        IF norm1 = 0 OR norm2 = 0 THEN
            RETURN 0;
        END IF;

        RETURN dot_product / (norm1 * norm2);
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '余弦相似度计算失败: %', SQLERRM;
            RETURN NULL;
    END;
END;
$$;

-- 使用余弦相似度函数（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'cosine_similarity') THEN
            RAISE WARNING 'cosine_similarity函数不存在，请先创建';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算余弦相似度';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '余弦相似度计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 测试余弦相似度函数
SELECT
    cosine_similarity(ARRAY[1, 2, 3], ARRAY[4, 5, 6]) AS similarity1,
    cosine_similarity(ARRAY[1, 0, 0], ARRAY[0, 1, 0]) AS similarity2,
    cosine_similarity(ARRAY[1, 2, 3], ARRAY[2, 4, 6]) AS similarity3;
```

### 2.2 皮尔逊相关系数

**皮尔逊相关系数**衡量两个变量的线性相关性。

```sql
-- 皮尔逊相关系数计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法计算皮尔逊相关系数';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算皮尔逊相关系数';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '皮尔逊相关系数计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算用户间的皮尔逊相关系数
WITH user_stats AS (
    SELECT
        user_id,
        AVG(rating) AS avg_rating,
        STDDEV(rating) AS std_rating
    FROM user_ratings
    GROUP BY user_id
),
user_ratings_normalized AS (
    SELECT
        ur.user_id,
        ur.item_id,
        ur.rating,
        us.avg_rating,
        (ur.rating - us.avg_rating) / NULLIF(us.std_rating, 0) AS normalized_rating
    FROM user_ratings ur
    JOIN user_stats us ON ur.user_id = us.user_id
),
pearson_correlation AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        COUNT(*) AS common_items,
        SUM(u1.normalized_rating * u2.normalized_rating) /
        NULLIF(COUNT(*), 0) AS correlation
    FROM user_ratings_normalized u1
    JOIN user_ratings_normalized u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
    GROUP BY u1.user_id, u2.user_id
    HAVING COUNT(*) >= 2  -- 至少2个共同评分
)
SELECT
    user1_id,
    user2_id,
    common_items,
    ROUND(correlation::numeric, 4) AS pearson_correlation
FROM pearson_correlation
ORDER BY correlation DESC;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    u1.user_id,
    u2.user_id,
    COUNT(*) AS common_items
FROM user_ratings u1
JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
GROUP BY u1.user_id, u2.user_id
LIMIT 100;
```

---

### 2.3 Jaccard相似度

**Jaccard相似度**适用于二元评分（喜欢/不喜欢）。

```sql
-- Jaccard相似度计算：适用于二元评分
WITH user_items AS (
    SELECT
        user_id,
        ARRAY_AGG(DISTINCT item_id ORDER BY item_id) AS item_list
    FROM user_ratings
    WHERE rating >= 4  -- 只考虑高分（喜欢）
    GROUP BY user_id
),
jaccard_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        -- 交集大小
        (
            SELECT COUNT(*)
            FROM unnest(u1.item_list) AS item
            WHERE item = ANY(u2.item_list)
        ) AS intersection_size,
        -- 并集大小
        (
            SELECT COUNT(DISTINCT item)
            FROM (
                SELECT unnest(u1.item_list) AS item
                UNION
                SELECT unnest(u2.item_list) AS item
            ) AS union_items
        ) AS union_size
    FROM user_items u1
    JOIN user_items u2 ON u1.user_id < u2.user_id
)
SELECT
    user1_id,
    user2_id,
    intersection_size,
    union_size,
    -- Jaccard相似度 = 交集 / 并集
    ROUND((intersection_size::numeric / NULLIF(union_size, 0))::numeric, 4) AS jaccard_similarity
FROM jaccard_similarity
WHERE union_size > 0
ORDER BY jaccard_similarity DESC;
```

### 2.4 调整余弦相似度

**调整余弦相似度**考虑用户评分偏差。

```sql
-- 调整余弦相似度：减去用户平均评分
WITH user_means AS (
    SELECT
        user_id,
        AVG(rating) AS mean_rating
    FROM user_ratings
    GROUP BY user_id
),
adjusted_ratings AS (
    SELECT
        ur.user_id,
        ur.item_id,
        ur.rating - um.mean_rating AS adjusted_rating
    FROM user_ratings ur
    JOIN user_means um ON ur.user_id = um.user_id
),
adjusted_cosine AS (
    SELECT
        a1.user_id AS user1_id,
        a2.user_id AS user2_id,
        SUM(a1.adjusted_rating * a2.adjusted_rating) AS dot_product,
        SQRT(SUM(a1.adjusted_rating * a1.adjusted_rating)) AS norm1,
        SQRT(SUM(a2.adjusted_rating * a2.adjusted_rating)) AS norm2
    FROM adjusted_ratings a1
    JOIN adjusted_ratings a2 ON a1.item_id = a2.item_id AND a1.user_id < a2.user_id
    GROUP BY a1.user_id, a2.user_id
)
SELECT
    user1_id,
    user2_id,
    ROUND((dot_product / NULLIF(norm1 * norm2, 0))::numeric, 4) AS adjusted_cosine_similarity
FROM adjusted_cosine
ORDER BY adjusted_cosine_similarity DESC;
```

---

## 3. 推荐生成

### 3.1 推荐生成原理

**推荐生成**基于相似度计算结果，预测用户对未评分物品的评分，并生成推荐列表。

#### 推荐生成方法

1. **Top-N推荐**：推荐评分最高的N个物品
2. **阈值推荐**：推荐评分超过阈值的物品
3. **加权推荐**：根据相似度加权计算预测评分

### 3.2 Top-N推荐实现

```sql
-- Top-N推荐生成（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法生成Top-N推荐';
            RETURN;
        END IF;
        RAISE NOTICE '开始生成Top-N推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Top-N推荐生成准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 为用户1生成Top-5推荐
WITH target_user AS (
    SELECT 1 AS user_id, 5 AS top_n
),
user_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        SUM(u1.rating * u2.rating) /
        NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity
    FROM user_ratings u1
    JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
    WHERE u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user)
    GROUP BY u1.user_id, u2.user_id
),
similar_users AS (
    SELECT
        CASE WHEN user1_id = (SELECT user_id FROM target_user) THEN user2_id ELSE user1_id END AS similar_user_id,
        similarity
    FROM user_similarity
    WHERE similarity > 0.3  -- 相似度阈值
),
recommendations AS (
    SELECT
        r.item_id,
        SUM(r.rating * s.similarity) / SUM(s.similarity) AS predicted_rating,
        COUNT(*) AS recommendation_count,
        SUM(s.similarity) AS total_similarity
    FROM user_ratings r
    JOIN similar_users s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
    HAVING COUNT(*) >= 2  -- 至少2个相似用户推荐
)
SELECT
    item_id,
    ROUND(predicted_rating::numeric, 2) AS predicted_rating,
    recommendation_count,
    ROUND(total_similarity::numeric, 4) AS total_similarity
FROM recommendations
ORDER BY predicted_rating DESC, total_similarity DESC
LIMIT (SELECT top_n FROM target_user);

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    item_id,
    AVG(rating) AS avg_rating
FROM user_ratings
WHERE user_id = 1
GROUP BY item_id
LIMIT 10;
```

---

### 3.3 加权推荐生成

**加权推荐**使用相似度加权计算预测评分。

```sql
-- 加权推荐生成：考虑相似度和评分数量
WITH target_user AS (
    SELECT 1 AS user_id
),
user_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        SUM(u1.rating * u2.rating) /
        NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity,
        COUNT(*) AS common_items
    FROM user_ratings u1
    JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
    WHERE u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user)
    GROUP BY u1.user_id, u2.user_id
    HAVING COUNT(*) >= 2  -- 至少2个共同评分
),
similar_users AS (
    SELECT
        CASE WHEN user1_id = (SELECT user_id FROM target_user) THEN user2_id ELSE user1_id END AS similar_user_id,
        similarity,
        common_items,
        -- 加权相似度：考虑共同评分数量
        similarity * LOG(1 + common_items) AS weighted_similarity
    FROM user_similarity
    WHERE similarity > 0.3
),
recommendations AS (
    SELECT
        r.item_id,
        -- 加权平均预测评分
        SUM(r.rating * s.weighted_similarity) / SUM(s.weighted_similarity) AS predicted_rating,
        COUNT(*) AS recommendation_count,
        SUM(s.weighted_similarity) AS total_weight
    FROM user_ratings r
    JOIN similar_users s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
    HAVING COUNT(*) >= 2
)
SELECT
    item_id,
    ROUND(predicted_rating::numeric, 2) AS predicted_rating,
    recommendation_count,
    ROUND(total_weight::numeric, 4) AS total_weight
FROM recommendations
ORDER BY predicted_rating DESC, total_weight DESC
LIMIT 10;
```

---

## 4. 基于向量的推荐（pgvector）

### 4.1 向量推荐概述

**基于向量的推荐（Vector-based Recommendation）**：使用pgvector扩展进行语义级别的推荐，通过向量相似度实现更准确的物品推荐。

#### 向量推荐原理

**向量推荐**：将用户和物品转换为向量嵌入（embedding），通过计算向量相似度进行推荐。

**向量推荐优势**：

- 语义理解能力强
- 支持冷启动问题
- 可以结合预训练模型（如OpenAI、Cohere）

#### pgvector扩展安装

```sql
-- 安装pgvector扩展（需要PostgreSQL 11+）
DO $$
BEGIN
    BEGIN
        CREATE EXTENSION IF NOT EXISTS vector;
        RAISE NOTICE 'pgvector扩展已启用';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'pgvector扩展安装失败: %。请确保已安装pgvector扩展。', SQLERRM;
            RAISE;
    END;
END $$;
```

### 4.2 向量推荐实现

**创建向量推荐表**：

```sql
-- 创建物品向量表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_embeddings') THEN
            RAISE WARNING '表 item_embeddings 已存在，先删除';
            DROP TABLE item_embeddings CASCADE;
        END IF;

        -- 创建包含向量嵌入的物品表
        CREATE TABLE item_embeddings (
            item_id SERIAL PRIMARY KEY,
            item_name TEXT NOT NULL,
            description TEXT,
            -- 使用vector类型存储1536维向量（OpenAI text-embedding-3-large）
            embedding vector(1536),
            category TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );

        -- 创建向量相似度索引（HNSW索引，PostgreSQL 18+）
        CREATE INDEX idx_item_embeddings_vector
        ON item_embeddings
        USING hnsw (embedding vector_cosine_ops)
        WITH (m = 16, ef_construction = 64);

        RAISE NOTICE '物品向量表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建物品向量表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

**基于向量相似度的推荐**：

```sql
-- 向量推荐：基于用户历史物品的向量相似度推荐（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_embeddings') THEN
            RAISE WARNING '表 item_embeddings 不存在，无法执行向量推荐';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行向量推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '向量推荐准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 向量推荐：基于用户历史物品推荐相似物品
WITH user_history AS (
    SELECT
        u.user_id,
        AVG(ie.embedding) AS user_embedding
    FROM user_ratings ur
    JOIN item_embeddings ie ON ur.item_id = ie.item_id
    WHERE ur.rating >= 4  -- 只考虑高分物品
    GROUP BY u.user_id
),
recommendations AS (
    SELECT
        uh.user_id,
        ie.item_id,
        ie.item_name,
        ie.category,
        1 - (ie.embedding <=> uh.user_embedding) AS similarity_score
    FROM user_history uh
    CROSS JOIN item_embeddings ie
    WHERE NOT EXISTS (
        SELECT 1 FROM user_ratings ur
        WHERE ur.user_id = uh.user_id AND ur.item_id = ie.item_id
    )  -- 排除已评分物品
)
SELECT
    user_id,
    item_id,
    item_name,
    category,
    similarity_score
FROM recommendations
WHERE similarity_score > 0.7  -- 相似度阈值
ORDER BY user_id, similarity_score DESC
LIMIT 10;  -- 每个用户推荐10个物品
```

**混合推荐（向量 + 协同过滤）**：

```sql
-- 混合推荐：结合向量相似度和协同过滤（带错误处理和性能测试）
WITH vector_recommendations AS (
    -- 向量推荐分数
    SELECT
        u.user_id,
        ie.item_id,
        1 - (ie.embedding <=> (SELECT AVG(ie2.embedding)
                               FROM user_ratings ur2
                               JOIN item_embeddings ie2 ON ur2.item_id = ie2.item_id
                               WHERE ur2.user_id = u.user_id AND ur2.rating >= 4)) AS vector_score
    FROM users u
    CROSS JOIN item_embeddings ie
    WHERE NOT EXISTS (
        SELECT 1 FROM user_ratings ur
        WHERE ur.user_id = u.user_id AND ur.item_id = ie.item_id
    )
),
collaborative_recommendations AS (
    -- 协同过滤推荐分数
    SELECT
        ur.user_id,
        ur2.item_id,
        AVG(ur2.rating) AS cf_score
    FROM user_ratings ur
    JOIN user_ratings ur2 ON ur.item_id = ur2.item_id AND ur.user_id != ur2.user_id
    WHERE ur.rating >= 4
    GROUP BY ur.user_id, ur2.item_id
    HAVING COUNT(*) >= 2
)
SELECT
    COALESCE(vr.user_id, cf.user_id) AS user_id,
    COALESCE(vr.item_id, cf.item_id) AS item_id,
    COALESCE(vr.vector_score, 0) AS vector_score,
    COALESCE(cf.cf_score, 0) AS cf_score,
    -- 加权组合：向量推荐权重0.6，协同过滤权重0.4
    (COALESCE(vr.vector_score, 0) * 0.6 + COALESCE(cf.cf_score, 0) * 0.4) AS final_score
FROM vector_recommendations vr
FULL OUTER JOIN collaborative_recommendations cf
    ON vr.user_id = cf.user_id AND vr.item_id = cf.item_id
WHERE (COALESCE(vr.vector_score, 0) * 0.6 + COALESCE(cf.cf_score, 0) * 0.4) > 0.6
ORDER BY user_id, final_score DESC
LIMIT 10;
```

### 4.3 向量推荐性能优化

**索引优化**：

```sql
-- pgvector索引优化（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- HNSW索引：适合高维向量，查询速度快
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'item_embeddings' AND indexname = 'idx_item_embeddings_vector') THEN
            RAISE WARNING '向量索引已存在';
        ELSE
            CREATE INDEX idx_item_embeddings_vector
            ON item_embeddings
            USING hnsw (embedding vector_cosine_ops)
            WITH (m = 16, ef_construction = 64);
            RAISE NOTICE 'HNSW向量索引创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '向量索引创建失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 5. 内容过滤推荐

### 5.1 内容过滤原理

**内容过滤（Content-Based Filtering）**基于物品的特征和用户的历史偏好，推荐与用户喜欢过的物品相似的物品。

#### 内容过滤优势

1. **无需用户数据**：只需要物品特征
2. **可解释性强**：可以解释推荐原因
3. **无冷启动问题**：新物品可以立即推荐

#### 内容过滤挑战

1. **特征提取困难**：需要提取有效的物品特征
2. **过度专业化**：可能只推荐相似物品
3. **新用户问题**：新用户缺乏历史数据

### 5.2 内容过滤实现

```sql
-- 内容过滤推荐：基于物品特征相似度
WITH item_features AS (
    SELECT
        item_id,
        category,
        price_range,
        rating_avg,
        -- 特征向量（简化版）
        ARRAY[
            CASE category WHEN 'electronics' THEN 1 ELSE 0 END,
            CASE category WHEN 'books' THEN 1 ELSE 0 END,
            CASE category WHEN 'clothing' THEN 1 ELSE 0 END,
            CASE WHEN price_range < 50 THEN 1 ELSE 0 END,
            CASE WHEN price_range >= 50 AND price_range < 100 THEN 1 ELSE 0 END,
            CASE WHEN price_range >= 100 THEN 1 ELSE 0 END
        ] AS feature_vector
    FROM items
),
user_preferences AS (
    SELECT
        user_id,
        -- 用户偏好向量（基于历史评分）
        ARRAY[
            AVG(CASE WHEN category = 'electronics' THEN rating ELSE NULL END),
            AVG(CASE WHEN category = 'books' THEN rating ELSE NULL END),
            AVG(CASE WHEN category = 'clothing' THEN rating ELSE NULL END),
            AVG(CASE WHEN price_range < 50 THEN rating ELSE NULL END),
            AVG(CASE WHEN price_range >= 50 AND price_range < 100 THEN rating ELSE NULL END),
            AVG(CASE WHEN price_range >= 100 THEN rating ELSE NULL END)
        ] AS preference_vector
    FROM user_ratings ur
    JOIN items i ON ur.item_id = i.item_id
    GROUP BY user_id
),
content_recommendations AS (
    SELECT
        up.user_id,
        if.item_id,
        -- 使用余弦相似度计算物品与用户偏好的匹配度
        cosine_similarity(if.feature_vector, up.preference_vector) AS content_score
    FROM user_preferences up
    CROSS JOIN item_features if
    WHERE if.item_id NOT IN (
        SELECT item_id FROM user_ratings WHERE user_id = up.user_id
    )
)
SELECT
    user_id,
    item_id,
    ROUND(content_score::numeric, 4) AS content_score
FROM content_recommendations
WHERE content_score > 0.5
ORDER BY user_id, content_score DESC
LIMIT 10;
```

---

## 6. 混合推荐

### 6.1 混合推荐原理

**混合推荐（Hybrid Recommendation）**结合多种推荐方法，利用各自的优势，提高推荐质量。

#### 混合策略

1. **加权混合**：$score = \alpha \cdot CF + (1-\alpha) \cdot CB$
2. **切换混合**：根据情况选择不同方法
3. **级联混合**：先用一种方法筛选，再用另一种方法排序

### 6.2 混合推荐实现

```sql
-- 混合推荐：结合协同过滤和内容过滤
WITH target_user AS (
    SELECT 1 AS user_id, 0.6 AS cf_weight, 0.4 AS cb_weight
),
-- 协同过滤评分
cf_scores AS (
    SELECT
        r.item_id,
        SUM(r.rating * s.similarity) / SUM(s.similarity) AS cf_score
    FROM user_ratings r
    JOIN (
        SELECT
            CASE WHEN user1_id = (SELECT user_id FROM target_user) THEN user2_id ELSE user1_id END AS similar_user_id,
            similarity
        FROM (
            SELECT
                u1.user_id AS user1_id,
                u2.user_id AS user2_id,
                SUM(u1.rating * u2.rating) /
                NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity
            FROM user_ratings u1
            JOIN user_ratings u2 ON u1.item_id = u2.item_id AND u1.user_id < u2.user_id
            WHERE u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user)
            GROUP BY u1.user_id, u2.user_id
        ) sim
        WHERE similarity > 0.3
    ) s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
),
-- 内容过滤评分（简化版）
cb_scores AS (
    SELECT
        i.item_id,
        -- 基于类别匹配的简单评分
        CASE
            WHEN i.category IN (
                SELECT DISTINCT i2.category
                FROM user_ratings ur
                JOIN items i2 ON ur.item_id = i2.item_id
                WHERE ur.user_id = (SELECT user_id FROM target_user) AND ur.rating >= 4
            ) THEN 0.8
            ELSE 0.3
        END AS cb_score
    FROM items i
    WHERE i.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
),
-- 混合评分
hybrid_scores AS (
    SELECT
        COALESCE(cf.item_id, cb.item_id) AS item_id,
        COALESCE(cf.cf_score, 0) * (SELECT cf_weight FROM target_user) +
        COALESCE(cb.cb_score, 0) * (SELECT cb_weight FROM target_user) AS hybrid_score
    FROM cf_scores cf
    FULL OUTER JOIN cb_scores cb ON cf.item_id = cb.item_id
)
SELECT
    item_id,
    ROUND(hybrid_score::numeric, 4) AS hybrid_score
FROM hybrid_scores
ORDER BY hybrid_score DESC
LIMIT 10;
```

---

## 7. PostgreSQL 18 并行推荐与AI增强

**PostgreSQL 18** 显著增强了并行推荐能力，支持并行执行协同过滤、内容过滤和混合推荐操作，同时结合pgvector实现基于向量相似度的AI增强推荐，大幅提升推荐系统的性能和准确性。

### 7.1 并行推荐原理

PostgreSQL 18 的并行推荐通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描用户评分和物品数据
2. **并行相似度计算**：每个工作进程独立计算用户或物品相似度
3. **并行推荐生成**：每个工作进程独立生成部分推荐结果
4. **结果合并**：主进程合并所有工作进程的推荐结果

### 7.2 并行协同过滤推荐

```sql
-- PostgreSQL 18 并行协同过滤推荐（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行并行协同过滤推荐';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行协同过滤推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行协同过滤推荐准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行协同过滤推荐：基于用户相似度的推荐
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH target_user AS (
    SELECT 1 AS user_id
),
user_similarity AS (
    SELECT
        CASE WHEN u1.user_id = (SELECT user_id FROM target_user) THEN u2.user_id ELSE u1.user_id END AS similar_user_id,
        SUM(u1.rating * u2.rating) /
        NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity
    FROM user_ratings u1
    JOIN user_ratings u2 ON u1.item_id = u2.item_id
    WHERE (u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user))
      AND u1.user_id != u2.user_id
    GROUP BY u1.user_id, u2.user_id
    HAVING SUM(u1.rating * u2.rating) / NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) > 0.3
),
recommendations AS (
    SELECT
        r.item_id,
        SUM(r.rating * s.similarity) / SUM(s.similarity) AS predicted_rating
    FROM user_ratings r
    JOIN user_similarity s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
)
SELECT
    item_id,
    ROUND(predicted_rating::numeric, 4) AS predicted_rating
FROM recommendations
ORDER BY predicted_rating DESC
LIMIT 10;
```

### 7.3 并行向量推荐（pgvector AI增强）

```sql
-- PostgreSQL 18 并行向量推荐：基于pgvector的AI增强推荐（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 确保pgvector扩展已安装
        CREATE EXTENSION IF NOT EXISTS vector;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_embeddings') THEN
            RAISE WARNING '表 item_embeddings 不存在，无法执行并行向量推荐';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行并行向量推荐';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行向量推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行向量推荐准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行向量推荐：基于用户历史物品的向量相似度推荐
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH target_user AS (
    SELECT 1 AS user_id
),
user_history AS (
    SELECT DISTINCT item_id
    FROM user_ratings
    WHERE user_id = (SELECT user_id FROM target_user)
      AND rating >= 4  -- 只考虑高评分物品
),
user_profile_vector AS (
    SELECT
        AVG(embedding) AS profile_vector
    FROM item_embeddings ie
    JOIN user_history uh ON ie.item_id = uh.item_id
),
vector_recommendations AS (
    SELECT
        ie.item_id,
        ie.item_name,
        1 - (ie.embedding <=> (SELECT profile_vector FROM user_profile_vector)) AS similarity
    FROM item_embeddings ie
    WHERE ie.item_id NOT IN (SELECT item_id FROM user_history)
)
SELECT
    item_id,
    item_name,
    ROUND(similarity::numeric, 4) AS similarity_score
FROM vector_recommendations
ORDER BY similarity DESC
LIMIT 10;
```

### 7.4 并行混合推荐（协同过滤 + 向量推荐）

```sql
-- PostgreSQL 18 并行混合推荐：结合协同过滤和向量推荐（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行并行混合推荐';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_embeddings') THEN
            RAISE WARNING '表 item_embeddings 不存在，无法执行并行混合推荐';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行混合推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行混合推荐准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行混合推荐：加权结合协同过滤和向量推荐
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH target_user AS (
    SELECT 1 AS user_id, 0.6 AS cf_weight, 0.4 AS vector_weight
),
-- 协同过滤评分
cf_scores AS (
    SELECT
        r.item_id,
        SUM(r.rating * s.similarity) / SUM(s.similarity) AS cf_score
    FROM user_ratings r
    JOIN (
        SELECT
            CASE WHEN u1.user_id = (SELECT user_id FROM target_user) THEN u2.user_id ELSE u1.user_id END AS similar_user_id,
            SUM(u1.rating * u2.rating) / NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) AS similarity
        FROM user_ratings u1
        JOIN user_ratings u2 ON u1.item_id = u2.item_id
        WHERE (u1.user_id = (SELECT user_id FROM target_user) OR u2.user_id = (SELECT user_id FROM target_user))
          AND u1.user_id != u2.user_id
        GROUP BY u1.user_id, u2.user_id
        HAVING SUM(u1.rating * u2.rating) / NULLIF(SQRT(SUM(u1.rating * u1.rating)) * SQRT(SUM(u2.rating * u2.rating)), 0) > 0.3
    ) s ON r.user_id = s.similar_user_id
    WHERE r.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
    GROUP BY r.item_id
),
-- 向量推荐评分
vector_scores AS (
    SELECT
        ie.item_id,
        1 - (ie.embedding <=> (
            SELECT AVG(embedding)
            FROM item_embeddings
            WHERE item_id IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user) AND rating >= 4)
        )) AS vector_score
    FROM item_embeddings ie
    WHERE ie.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = (SELECT user_id FROM target_user))
),
-- 混合评分
hybrid_scores AS (
    SELECT
        COALESCE(cf.item_id, vs.item_id) AS item_id,
        COALESCE(cf.cf_score, 0) * (SELECT cf_weight FROM target_user) +
        COALESCE(vs.vector_score, 0) * (SELECT vector_weight FROM target_user) AS hybrid_score
    FROM cf_scores cf
    FULL OUTER JOIN vector_scores vs ON cf.item_id = vs.item_id
)
SELECT
    item_id,
    ROUND(hybrid_score::numeric, 4) AS hybrid_score
FROM hybrid_scores
ORDER BY hybrid_score DESC
LIMIT 10;
```

---

## 8. 实际应用案例

### 8.1 商品推荐

**商品推荐**为电商用户推荐可能感兴趣的商品。

```sql
-- 商品推荐示例（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'product_interactions') THEN
            RAISE WARNING '表 product_interactions 不存在，创建示例表';

            CREATE TABLE product_interactions (
                user_id INTEGER NOT NULL,
                product_id INTEGER NOT NULL,
                interaction_type VARCHAR(20) NOT NULL,  -- 'view', 'purchase', 'cart'
                interaction_score NUMERIC NOT NULL,  -- 1-5分
                interaction_time TIMESTAMP DEFAULT NOW(),
                PRIMARY KEY (user_id, product_id, interaction_type)
            );

            INSERT INTO product_interactions (user_id, product_id, interaction_type, interaction_score) VALUES
                (1, 101, 'purchase', 5), (1, 102, 'view', 4), (1, 103, 'cart', 3),
                (2, 101, 'purchase', 4), (2, 102, 'purchase', 5), (2, 104, 'view', 3),
                (3, 102, 'purchase', 4), (3, 103, 'purchase', 5), (3, 105, 'view', 4),
                (4, 101, 'view', 2), (4, 103, 'purchase', 5), (4, 105, 'purchase', 4);

            RAISE NOTICE '表 product_interactions 创建成功';
        END IF;
        RAISE NOTICE '开始执行商品推荐';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '商品推荐准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算加权评分（不同交互类型权重不同）
WITH weighted_ratings AS (
    SELECT
        user_id,
        product_id,
        SUM(CASE interaction_type
            WHEN 'purchase' THEN interaction_score * 3
            WHEN 'cart' THEN interaction_score * 2
            WHEN 'view' THEN interaction_score * 1
            ELSE interaction_score
        END) /
        SUM(CASE interaction_type
            WHEN 'purchase' THEN 3
            WHEN 'cart' THEN 2
            WHEN 'view' THEN 1
            ELSE 1
        END) AS weighted_rating
    FROM product_interactions
    GROUP BY user_id, product_id
),
user_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        SUM(u1.weighted_rating * u2.weighted_rating) /
        NULLIF(SQRT(SUM(u1.weighted_rating * u1.weighted_rating)) *
               SQRT(SUM(u2.weighted_rating * u2.weighted_rating)), 0) AS similarity
    FROM weighted_ratings u1
    JOIN weighted_ratings u2 ON u1.product_id = u2.product_id AND u1.user_id < u2.user_id
    GROUP BY u1.user_id, u2.user_id
),
recommendations AS (
    SELECT
        wr.product_id,
        SUM(wr.weighted_rating * us.similarity) / SUM(us.similarity) AS predicted_score
    FROM weighted_ratings wr
    JOIN user_similarity us ON wr.user_id = us.user2_id
    WHERE us.user1_id = 1  -- 为目标用户推荐
      AND wr.product_id NOT IN (SELECT product_id FROM weighted_ratings WHERE user_id = 1)
    GROUP BY wr.product_id
    HAVING COUNT(*) >= 1
)
SELECT
    product_id,
    ROUND(predicted_score::numeric, 2) AS predicted_score
FROM recommendations
ORDER BY predicted_score DESC
LIMIT 10;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    user_id,
    product_id,
    AVG(interaction_score) AS avg_score
FROM product_interactions
GROUP BY user_id, product_id
LIMIT 100;
```

---

### 7.2 电影推荐系统

**场景**：为视频平台用户推荐电影。

```sql
-- 电影推荐系统：结合评分、观看时长、收藏等行为
WITH user_movie_interactions AS (
    SELECT
        user_id,
        movie_id,
        -- 综合评分：考虑评分、观看时长、收藏
        (
            COALESCE(rating, 0) * 0.5 +
            CASE WHEN watch_duration > 0.8 THEN 4 ELSE 0 END * 0.3 +
            CASE WHEN favorited THEN 5 ELSE 0 END * 0.2
        ) AS interaction_score
    FROM movie_interactions
    WHERE rating IS NOT NULL OR watch_duration > 0 OR favorited = true
),
movie_similarity AS (
    SELECT
        m1.movie_id AS movie1_id,
        m2.movie_id AS movie2_id,
        -- 基于共同用户的相似度
        COUNT(DISTINCT m1.user_id) AS common_users,
        SUM(m1.interaction_score * m2.interaction_score) /
        NULLIF(
            SQRT(SUM(m1.interaction_score * m1.interaction_score)) *
            SQRT(SUM(m2.interaction_score * m2.interaction_score)),
            0
        ) AS similarity
    FROM user_movie_interactions m1
    JOIN user_movie_interactions m2 ON m1.user_id = m2.user_id AND m1.movie_id < m2.movie_id
    GROUP BY m1.movie_id, m2.movie_id
    HAVING COUNT(DISTINCT m1.user_id) >= 5  -- 至少5个共同用户
),
recommendations AS (
    SELECT
        target.user_id,
        CASE WHEN ms.movie1_id = um.movie_id THEN ms.movie2_id ELSE ms.movie1_id END AS recommended_movie_id,
        SUM(um.interaction_score * ms.similarity) / SUM(ms.similarity) AS predicted_score
    FROM (SELECT DISTINCT user_id FROM user_movie_interactions) target
    CROSS JOIN user_movie_interactions um
    JOIN movie_similarity ms ON (ms.movie1_id = um.movie_id OR ms.movie2_id = um.movie_id)
    WHERE um.user_id = target.user_id
      AND CASE WHEN ms.movie1_id = um.movie_id THEN ms.movie2_id ELSE ms.movie1_id END
          NOT IN (SELECT movie_id FROM user_movie_interactions WHERE user_id = target.user_id)
    GROUP BY target.user_id, CASE WHEN ms.movie1_id = um.movie_id THEN ms.movie2_id ELSE ms.movie1_id END
)
SELECT
    user_id,
    recommended_movie_id,
    ROUND(predicted_score::numeric, 2) AS predicted_score
FROM recommendations
ORDER BY user_id, predicted_score DESC
LIMIT 10;
```

### 7.3 新闻推荐系统

**场景**：为新闻平台用户推荐新闻文章。

```sql
-- 新闻推荐系统：基于阅读历史和文章相似度
WITH user_reading_history AS (
    SELECT
        user_id,
        article_id,
        read_duration,
        click_time,
        -- 阅读兴趣评分
        CASE
            WHEN read_duration > 60 THEN 5  -- 深度阅读
            WHEN read_duration > 30 THEN 4  -- 中等阅读
            WHEN read_duration > 10 THEN 3  -- 浅度阅读
            ELSE 2  -- 快速浏览
        END AS interest_score
    FROM article_reads
    WHERE click_time >= NOW() - INTERVAL '30 days'  -- 最近30天
),
article_similarity AS (
    SELECT
        a1.article_id AS article1_id,
        a2.article_id AS article2_id,
        -- 基于类别的相似度
        CASE
            WHEN a1.category = a2.category THEN 0.8
            WHEN a1.category IN (SELECT related_categories FROM articles WHERE article_id = a2.article_id) THEN 0.5
            ELSE 0.2
        END AS category_similarity,
        -- 基于标签的相似度
        (
            SELECT COUNT(*)
            FROM unnest(a1.tags) AS tag
            WHERE tag = ANY(a2.tags)
        )::numeric / NULLIF(
            (
                SELECT COUNT(DISTINCT tag)
                FROM (
                    SELECT unnest(a1.tags) AS tag
                    UNION
                    SELECT unnest(a2.tags) AS tag
                ) AS all_tags
            ),
            0
        ) AS tag_similarity
    FROM articles a1
    JOIN articles a2 ON a1.article_id < a2.article_id
),
recommendations AS (
    SELECT
        urh.user_id,
        CASE WHEN asim.article1_id = urh.article_id THEN asim.article2_id ELSE asim.article1_id END AS recommended_article_id,
        SUM(urh.interest_score * (asim.category_similarity * 0.6 + asim.tag_similarity * 0.4)) /
        SUM(asim.category_similarity * 0.6 + asim.tag_similarity * 0.4) AS predicted_score
    FROM user_reading_history urh
    JOIN article_similarity asim ON (asim.article1_id = urh.article_id OR asim.article2_id = urh.article_id)
    WHERE CASE WHEN asim.article1_id = urh.article_id THEN asim.article2_id ELSE asim.article1_id END
          NOT IN (SELECT article_id FROM user_reading_history WHERE user_id = urh.user_id)
    GROUP BY urh.user_id, CASE WHEN asim.article1_id = urh.article_id THEN asim.article2_id ELSE asim.article1_id END
)
SELECT
    user_id,
    recommended_article_id,
    ROUND(predicted_score::numeric, 2) AS predicted_score
FROM recommendations
ORDER BY user_id, predicted_score DESC
LIMIT 10;
```

### 7.4 推荐系统评估

**场景**：评估推荐系统的性能。

```sql
-- 推荐系统评估：计算MAE、RMSE等指标
WITH test_data AS (
    SELECT
        user_id,
        item_id,
        rating AS actual_rating,
        -- 预测评分（简化版，使用平均评分）
        (
            SELECT AVG(rating)
            FROM user_ratings ur2
            WHERE ur2.item_id = ur1.item_id
        ) AS predicted_rating
    FROM user_ratings ur1
    WHERE user_id = 1  -- 测试用户
),
evaluation_metrics AS (
    SELECT
        COUNT(*) AS total_predictions,
        AVG(ABS(actual_rating - predicted_rating)) AS mae,
        SQRT(AVG(POWER(actual_rating - predicted_rating, 2))) AS rmse,
        -- 准确率（预测误差小于1）
        SUM(CASE WHEN ABS(actual_rating - predicted_rating) < 1 THEN 1 ELSE 0 END)::numeric / COUNT(*) AS accuracy
    FROM test_data
)
SELECT
    total_predictions,
    ROUND(mae::numeric, 4) AS mae,
    ROUND(rmse::numeric, 4) AS rmse,
    ROUND(accuracy::numeric, 4) AS accuracy
FROM evaluation_metrics;
```

---

## 8. 算法性能对比与优化

### 8.1 推荐算法对比

| 算法 | 时间复杂度 | 空间复杂度 | 适用场景 | 优点 | 缺点 |
|------|-----------|-----------|---------|------|------|
| **用户协同过滤** | $O(n^2)$ | $O(n^2)$ | 用户多物品少 | 发现潜在兴趣 | 计算复杂 |
| **物品协同过滤** | $O(m^2)$ | $O(m^2)$ | 物品多用户少 | 稳定快速 | 需要物品相似度 |
| **内容过滤** | $O(nm)$ | $O(m)$ | 有物品特征 | 可解释性强 | 特征提取困难 |
| **混合推荐** | $O(n^2)$ | $O(n^2)$ | 综合场景 | 性能最优 | 实现复杂 |

### 8.2 性能优化建议

1. **计算优化**：
   - 使用矩阵分解（SVD、NMF）
   - 采样计算相似度
   - 并行处理

2. **存储优化**：
   - 缓存相似度矩阵
   - 使用物化视图
   - 增量更新

3. **算法优化**：
   - 使用LSH（Locality-Sensitive Hashing）
   - 降维技术
   - 近似算法

### 8.3 常见问题与解决方案

**问题1**：冷启动问题

- **解决方案**：使用内容过滤、热门推荐、用户画像

**问题2**：数据稀疏性

- **解决方案**：矩阵分解、降维技术、数据增强

**问题3**：计算性能问题

- **解决方案**：使用索引、采样计算、增量更新

**问题4**：推荐多样性不足

- **解决方案**：多样性优化、混合推荐、随机性引入

---

## 9. 最佳实践

### 9.1 推荐系统设计

1. **数据准备**：
   - 收集用户行为数据
   - 提取物品特征
   - 数据清洗和预处理

2. **算法选择**：
   - 根据场景选择算法
   - 考虑计算资源
   - 平衡准确性和多样性

3. **系统实现**：
   - 实时推荐vs离线推荐
   - 缓存策略
   - 增量更新

### 9.2 推荐系统注意事项

1. **冷启动**：
   - 新用户：使用热门推荐、内容过滤
   - 新物品：使用内容特征、相似物品

2. **多样性**：
   - 避免过度专业化
   - 引入随机性
   - 类别多样性

3. **实时性**：
   - 实时更新用户偏好
   - 快速响应
   - 增量计算

### 9.3 SQL实现注意事项

1. **性能考虑**：
   - 优化连接查询
   - 创建适当索引
   - 使用物化视图

2. **准确性**：
   - 处理NULL值
   - 避免除零错误
   - 验证计算结果

3. **可维护性**：
   - 使用清晰的变量名
   - 添加注释
   - 模块化设计

### 9.4 PostgreSQL 18 新特性应用（增强）

**PostgreSQL 18**引入了多项增强功能，可以显著提升推荐系统的性能：

1. **Skip Scan优化**：
   - 对于包含推荐得分的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N推荐查询和热门物品查询

2. **异步I/O增强**：
   - 对于大规模推荐计算，异步I/O可以显著提升性能
   - 适用于批量相似度计算和协同过滤推荐

3. **并行查询增强**：
   - 推荐系统计算支持更好的并行执行
   - 适用于大规模协同过滤和向量推荐

**示例：使用Skip Scan优化Top-N推荐查询**

```sql
-- 为推荐得分创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_recommendation_scores_skip_scan
ON user_item_scores(user_id, recommendation_score DESC);

-- Skip Scan优化查询：查找推荐得分最高的前10个物品
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    user_id,
    item_id,
    recommendation_score
FROM user_item_scores
WHERE user_id = 1
ORDER BY recommendation_score DESC
LIMIT 10;
```

### 9.5 高级优化技巧（增强）

**1. 使用物化视图缓存推荐结果**

对于频繁使用的推荐结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存热门推荐结果
CREATE MATERIALIZED VIEW IF NOT EXISTS popular_recommendations_cache AS
WITH user_item_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        COUNT(*) AS common_items,
        SQRT(SUM(POWER(u1.rating - u2.rating, 2))) AS similarity_distance
    FROM user_ratings u1
    JOIN user_ratings u2 ON u1.item_id = u2.item_id
    WHERE u1.user_id < u2.user_id
    GROUP BY u1.user_id, u2.user_id
    HAVING COUNT(*) >= 3
),
recommendation_scores AS (
    SELECT
        uis.user1_id AS user_id,
        u2.item_id,
        SUM(uis.similarity_distance * u2.rating) / NULLIF(SUM(ABS(uis.similarity_distance)), 0) AS recommendation_score
    FROM user_item_similarity uis
    JOIN user_ratings u2 ON uis.user2_id = u2.user_id
    WHERE u2.item_id NOT IN (
        SELECT item_id FROM user_ratings WHERE user_id = uis.user1_id
    )
    GROUP BY uis.user1_id, u2.item_id
)
SELECT
    user_id,
    item_id,
    ROUND(recommendation_score::numeric, 4) AS recommendation_score,
    ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY recommendation_score DESC) AS recommendation_rank
FROM recommendation_scores
WHERE recommendation_score > 0.5;

-- 创建索引加速物化视图查询
CREATE INDEX idx_popular_recommendations_user_score ON popular_recommendations_cache(user_id, recommendation_score DESC);
CREATE INDEX idx_popular_recommendations_rank ON popular_recommendations_cache(user_id, recommendation_rank);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY popular_recommendations_cache;
```

**2. 实时推荐更新：增量协同过滤**

**实时推荐更新**：使用增量方法更新协同过滤推荐。

```sql
-- 实时推荐更新：增量协同过滤（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_ratings') THEN
            RAISE WARNING '表 user_ratings 不存在，无法执行增量推荐更新';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行增量推荐更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量推荐更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新用户相似度矩阵
WITH new_ratings AS (
    SELECT
        user_id,
        item_id,
        rating
    FROM user_ratings
    WHERE rating_timestamp > (SELECT COALESCE(MAX(last_updated), '1970-01-01') FROM user_similarity_cache)
),
updated_similarity AS (
    SELECT
        u1.user_id AS user1_id,
        u2.user_id AS user2_id,
        COUNT(*) AS common_items,
        -- 使用窗口函数计算相似度（避免重复计算）
        CORR(u1.rating, u2.rating) OVER (PARTITION BY u1.user_id, u2.user_id) AS correlation_similarity
    FROM new_ratings u1
    JOIN new_ratings u2 ON u1.item_id = u2.item_id
    WHERE u1.user_id < u2.user_id
    GROUP BY u1.user_id, u2.user_id
    HAVING COUNT(*) >= 3
)
-- 更新或插入用户相似度缓存
INSERT INTO user_similarity_cache (user1_id, user2_id, similarity_score, common_items, last_updated)
SELECT
    user1_id,
    user2_id,
    correlation_similarity,
    common_items,
    NOW()
FROM updated_similarity
ON CONFLICT (user1_id, user2_id)
DO UPDATE SET
    similarity_score = EXCLUDED.similarity_score,
    common_items = EXCLUDED.common_items,
    last_updated = NOW();
```

**3. 混合推荐系统：协同过滤 + 内容过滤 + 向量推荐**

**混合推荐系统**：结合协同过滤、内容过滤和向量推荐的混合推荐。

```sql
-- 混合推荐系统：协同过滤 + 内容过滤 + 向量推荐（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        -- 确保pgvector扩展已安装
        CREATE EXTENSION IF NOT EXISTS vector;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_vectors') THEN
            CREATE TABLE item_vectors (
                item_id INTEGER PRIMARY KEY,
                item_features vector(384),  -- 使用384维向量
                category VARCHAR(50),
                popularity_score NUMERIC DEFAULT 0
            );

            CREATE INDEX idx_item_vectors_vector ON item_vectors
            USING ivfflat (item_features vector_cosine_ops) WITH (lists = 100);

            RAISE NOTICE '物品向量表创建成功';
        END IF;

        RAISE NOTICE '开始执行混合推荐系统';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '混合推荐系统准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 混合推荐：结合三种推荐方法
WITH target_user_vector AS (
    SELECT
        AVG(iv.item_features) AS user_preference_vector
    FROM user_ratings ur
    JOIN item_vectors iv ON ur.item_id = iv.item_id
    WHERE ur.user_id = 1 AND ur.rating >= 4
),
collaborative_filtering AS (
    SELECT
        item_id,
        SUM(similarity_score * rating) / NULLIF(SUM(ABS(similarity_score)), 0) AS cf_score
    FROM user_similarity_cache usc
    JOIN user_ratings ur ON usc.user2_id = ur.user_id
    WHERE usc.user1_id = 1
      AND ur.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = 1)
    GROUP BY item_id
),
content_filtering AS (
    SELECT
        iv.item_id,
        -- 基于物品特征的相似度
        COUNT(*) FILTER (WHERE ur.rating >= 4)::numeric / NULLIF(COUNT(*), 0) AS content_score
    FROM item_vectors iv
    LEFT JOIN user_ratings ur ON iv.item_id = ur.item_id
    WHERE iv.category IN (
        SELECT DISTINCT category
        FROM item_vectors iv2
        JOIN user_ratings ur2 ON iv2.item_id = ur2.item_id
        WHERE ur2.user_id = 1 AND ur2.rating >= 4
    )
    GROUP BY iv.item_id
),
vector_recommendation AS (
    SELECT
        iv.item_id,
        1 - (iv.item_features <=> (SELECT user_preference_vector FROM target_user_vector)) AS vector_score
    FROM item_vectors iv
    WHERE iv.item_id NOT IN (SELECT item_id FROM user_ratings WHERE user_id = 1)
    ORDER BY iv.item_features <=> (SELECT user_preference_vector FROM target_user_vector)
    LIMIT 50
),
hybrid_recommendations AS (
    SELECT
        COALESCE(cf.item_id, cont.item_id, vr.item_id) AS item_id,
        COALESCE(cf.cf_score, 0) AS collaborative_score,
        COALESCE(cont.content_score, 0) AS content_score,
        COALESCE(vr.vector_score, 0) AS vector_score,
        -- 混合评分：协同过滤40%，内容过滤30%，向量推荐30%
        COALESCE(cf.cf_score, 0) * 0.4 +
        COALESCE(cont.content_score, 0) * 0.3 +
        COALESCE(vr.vector_score, 0) * 0.3 AS hybrid_score
    FROM collaborative_filtering cf
    FULL OUTER JOIN content_filtering cont ON cf.item_id = cont.item_id
    FULL OUTER JOIN vector_recommendation vr ON COALESCE(cf.item_id, cont.item_id) = vr.item_id
)
SELECT
    item_id,
    ROUND(collaborative_score::numeric, 4) AS collaborative_score,
    ROUND(content_score::numeric, 4) AS content_score,
    ROUND(vector_score::numeric, 4) AS vector_score,
    ROUND(hybrid_score::numeric, 4) AS hybrid_score
FROM hybrid_recommendations
WHERE hybrid_score > 0.3
ORDER BY hybrid_score DESC
LIMIT 20;
```

---

## 📚 参考资源

### 学术文献

1. **《推荐系统实践》**（项亮, 2012）- 推荐系统经典教材

2. **《推荐系统》**（Ricci, F., et al., 2011）- 推荐系统理论

3. **《协同过滤推荐系统》**（Sarwar, B., et al., 2001）- 协同过滤算法

### PostgreSQL官方文档

- **聚合函数**: <https://www.postgresql.org/docs/current/functions-aggregate.html>
- **数组函数**: <https://www.postgresql.org/docs/current/functions-array.html>
- **窗口函数**: <https://www.postgresql.org/docs/current/tutorial-window.html>

### 在线资源

- **推荐系统**: <https://en.wikipedia.org/wiki/Recommender_system>
- **协同过滤**: <https://en.wikipedia.org/wiki/Collaborative_filtering>
- **Netflix Prize**: <https://en.wikipedia.org/wiki/Netflix_Prize>

### 相关算法

- **协同过滤算法**：用户/物品协同过滤
- **内容过滤算法**：基于特征的推荐
- **矩阵分解算法**：SVD、NMF推荐
- **深度学习推荐**：神经网络推荐

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**文档状态**: ✅ 已完成
