# PostgreSQL 决策树算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 决策树 | 分类算法
> **难度级别**: ⭐⭐⭐⭐⭐ (专家级)

---

## 📋 目录

- [PostgreSQL 决策树算法完整指南](#postgresql-决策树算法完整指南)
  - [📋 目录](#-目录)
  - [决策树概述](#决策树概述)
    - [理论基础](#理论基础)
      - [决策树定义](#决策树定义)
      - [决策树构建过程](#决策树构建过程)
      - [决策树算法类型](#决策树算法类型)
      - [决策树优缺点](#决策树优缺点)
    - [核心算法](#核心算法)
  - [1. 信息熵计算](#1-信息熵计算)
    - [1.1 信息熵原理](#11-信息熵原理)
      - [信息熵定义](#信息熵定义)
      - [熵的特性](#熵的特性)
    - [1.2 熵函数实现](#12-熵函数实现)
    - [1.3 条件熵计算](#13-条件熵计算)
  - [2. 信息增益](#2-信息增益)
    - [2.1 信息增益原理](#21-信息增益原理)
      - [信息增益定义](#信息增益定义)
    - [2.2 信息增益计算实现](#22-信息增益计算实现)
    - [2.3 信息增益率](#23-信息增益率)
    - [2.4 基尼不纯度](#24-基尼不纯度)
  - [3. 决策树构建](#3-决策树构建)
    - [3.1 决策树构建原理](#31-决策树构建原理)
      - [ID3算法步骤](#id3算法步骤)
    - [3.2 简单决策树实现](#32-简单决策树实现)
    - [3.3 决策树剪枝](#33-决策树剪枝)
    - [3.4 决策树可视化（简化版）](#34-决策树可视化简化版)
  - [4. 模型评估](#4-模型评估)
    - [4.1 准确率计算](#41-准确率计算)
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 客户分类](#51-客户分类)
    - [5.2 贷款风险评估](#52-贷款风险评估)
    - [5.3 产品推荐决策树](#53-产品推荐决策树)
    - [5.4 医疗诊断决策树](#54-医疗诊断决策树)
  - [6. PostgreSQL 18并行决策树](#6-postgresql-18并行决策树)
    - [6.1 并行决策树概述](#61-并行决策树概述)
      - [并行决策树配置](#并行决策树配置)
      - [并行决策树实现](#并行决策树实现)
  - [7. 算法性能对比与优化](#7-算法性能对比与优化)
    - [7.1 决策树算法对比](#71-决策树算法对比)
    - [7.2 性能优化建议](#72-性能优化建议)
    - [7.3 常见问题与解决方案](#73-常见问题与解决方案)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 决策树构建流程](#81-决策树构建流程)
    - [8.2 决策树注意事项](#82-决策树注意事项)
    - [8.3 SQL实现注意事项](#83-sql实现注意事项)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 决策树概述

**决策树（Decision Tree）**是一种树形结构的分类和回归算法，通过递归分割数据来构建决策规则。决策树具有可解释性强、易于理解和实现的优点，是机器学习中最常用的算法之一。

### 理论基础

#### 决策树定义

**决策树**是一个树形结构，其中：

- **内部节点**：表示特征（属性）
- **分支**：表示特征的取值
- **叶节点**：表示类别（分类）或值（回归）

#### 决策树构建过程

1. **特征选择**：选择最佳特征进行分割
2. **数据分割**：根据特征值分割数据
3. **递归构建**：对每个子集递归构建子树
4. **停止条件**：满足停止条件时创建叶节点

#### 决策树算法类型

1. **ID3算法**：使用信息增益选择特征
2. **C4.5算法**：使用信息增益率选择特征
3. **CART算法**：使用基尼不纯度选择特征
4. **CHAID算法**：使用卡方检验选择特征

#### 决策树优缺点

**优点**：

- 可解释性强
- 易于理解和实现
- 不需要特征缩放
- 可以处理非线性关系

**缺点**：

- 容易过拟合
- 对噪声敏感
- 不稳定（数据微小变化可能导致树结构变化）

### 核心算法

| 算法 | 用途 | 复杂度 | 数学表示 |
|------|------|--------|---------|
| **信息熵** | 衡量数据纯度 | $O(n)$ | $H(S) = -\sum p_i \log_2 p_i$ |
| **信息增益** | 选择最佳分割点 | $O(nm)$ | $IG(S,A) = H(S) - H(S\|A)$ |
| **基尼不纯度** | 衡量数据不纯度 | $O(n)$ | $Gini(S) = 1 - \sum p_i^2$ |
| **ID3算法** | 构建决策树 | $O(nm \log n)$ | 递归构建 |

---

## 1. 信息熵计算

### 1.1 信息熵原理

**信息熵（Information Entropy）**衡量数据集的混乱程度或不纯度。熵值越大，数据越混乱；熵值越小，数据越纯净。

#### 信息熵定义

**信息熵**：
$$H(S) = -\sum_{i=1}^{c} p_i \log_2 p_i$$

其中：

- $S$：数据集
- $c$：类别数
- $p_i$：类别$i$的概率

#### 熵的特性

1. **最大值**：当所有类别概率相等时，熵最大
2. **最小值**：当只有一个类别时，熵为0
3. **范围**：$0 \leq H(S) \leq \log_2 c$

### 1.2 熵函数实现

```sql
-- 创建决策树数据表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decision_tree_data') THEN
            RAISE WARNING '表 decision_tree_data 已存在，先删除';
            DROP TABLE decision_tree_data CASCADE;
        END IF;

        CREATE TABLE decision_tree_data (
            id SERIAL PRIMARY KEY,
            outlook VARCHAR(20) NOT NULL,
            temperature NUMERIC NOT NULL,
            humidity NUMERIC NOT NULL,
            windy BOOLEAN NOT NULL,
            play_tennis INTEGER NOT NULL CHECK (play_tennis IN (0, 1))
        );

        -- 插入示例数据
        INSERT INTO decision_tree_data (outlook, temperature, humidity, windy, play_tennis) VALUES
            ('sunny', 85, 85, false, 0), ('sunny', 80, 90, true, 0),
            ('overcast', 83, 86, false, 1), ('rain', 70, 96, false, 1),
            ('rain', 68, 80, false, 1), ('rain', 65, 70, true, 0),
            ('overcast', 64, 65, true, 1), ('sunny', 72, 95, false, 0),
            ('sunny', 69, 70, false, 1), ('rain', 75, 80, false, 1);

        RAISE NOTICE '表 decision_tree_data 创建成功，已插入10条数据';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 decision_tree_data 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- 信息熵计算函数（带错误处理）
CREATE OR REPLACE FUNCTION calculate_entropy(class_counts NUMERIC[])
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    total NUMERIC;
    entropy NUMERIC := 0;
    p NUMERIC;
    i INTEGER;
BEGIN
    BEGIN
        IF class_counts IS NULL OR array_length(class_counts, 1) IS NULL THEN
            RAISE WARNING '类别计数数组为空';
            RETURN 0;
        END IF;

        -- 计算总数
        SELECT SUM(unnest) INTO total FROM unnest(class_counts) AS unnest;

        IF total = 0 THEN
            RETURN 0;
        END IF;

        -- 计算熵: -Σ(p * log2(p))
        FOR i IN 1..array_length(class_counts, 1) LOOP
            p := class_counts[i] / total;
            IF p > 0 THEN
                entropy := entropy - p * LOG(2, p);
            END IF;
        END LOOP;

        RETURN entropy;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '熵计算失败: %', SQLERRM;
            RETURN NULL;
    END;
END;
$$;

-- 计算数据集的总熵（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decision_tree_data') THEN
            RAISE WARNING '表 decision_tree_data 不存在，无法计算熵';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'calculate_entropy') THEN
            RAISE WARNING 'calculate_entropy函数不存在，请先创建';
            RETURN;
        END IF;

        RAISE NOTICE '开始计算信息熵';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '熵计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算总熵
WITH class_distribution AS (
    SELECT
        play_tennis,
        COUNT(*) AS count
    FROM decision_tree_data
    GROUP BY play_tennis
),
class_counts AS (
    SELECT ARRAY_AGG(count ORDER BY play_tennis) AS counts
    FROM class_distribution
)
SELECT
    counts,
    calculate_entropy(counts) AS entropy
FROM class_counts;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    play_tennis,
    COUNT(*) AS count
FROM decision_tree_data
GROUP BY play_tennis;
```

---

### 1.3 条件熵计算

**条件熵**计算在给定特征条件下的熵。

```sql
-- 条件熵计算：给定特征A的条件熵
WITH feature_entropy AS (
    SELECT
        outlook,
        COUNT(*) AS outlook_count,
        calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS conditional_entropy
    FROM (
        SELECT outlook, play_tennis, COUNT(*) AS count
        FROM decision_tree_data
        GROUP BY outlook, play_tennis
    ) t
    GROUP BY outlook
)
SELECT
    outlook,
    outlook_count,
    ROUND(conditional_entropy::numeric, 4) AS conditional_entropy,
    -- 加权条件熵
    ROUND((outlook_count * conditional_entropy)::numeric, 4) AS weighted_entropy
FROM feature_entropy
ORDER BY outlook;
```

---

## 2. 信息增益

### 2.1 信息增益原理

**信息增益（Information Gain）**衡量使用某个特征分割数据后熵的减少量。信息增益越大，该特征对分类的贡献越大。

#### 信息增益定义

**信息增益**：
$$IG(S, A) = H(S) - H(S|A)$$

其中：

- $H(S)$：数据集$S$的熵
- $H(S|A)$：给定特征$A$的条件熵

**条件熵**：
$$H(S|A) = \sum_{v \in Values(A)} \frac{|S_v|}{|S|} H(S_v)$$

其中$S_v$是特征$A$取值为$v$的子集。

### 2.2 信息增益计算实现

```sql
-- 信息增益计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decision_tree_data') THEN
            RAISE WARNING '表 decision_tree_data 不存在，无法计算信息增益';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算信息增益';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '信息增益计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算outlook特征的信息增益
WITH total_entropy AS (
    SELECT calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS entropy
    FROM (
        SELECT play_tennis, COUNT(*) AS count
        FROM decision_tree_data
        GROUP BY play_tennis
    ) t
),
outlook_entropy AS (
    SELECT
        outlook,
        COUNT(*) AS outlook_count,
        calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS conditional_entropy
    FROM (
        SELECT outlook, play_tennis, COUNT(*) AS count
        FROM decision_tree_data
        GROUP BY outlook, play_tennis
    ) t
    GROUP BY outlook
),
weighted_entropy AS (
    SELECT
        SUM(outlook_count * conditional_entropy) / SUM(outlook_count) AS weighted_avg_entropy,
        SUM(outlook_count) AS total_count
    FROM outlook_entropy
)
SELECT
    t.entropy AS total_entropy,
    w.weighted_avg_entropy AS weighted_entropy,
    t.entropy - w.weighted_avg_entropy AS information_gain
FROM total_entropy t, weighted_entropy w;

-- 计算所有特征的信息增益
WITH total_entropy AS (
    SELECT calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS entropy
    FROM (
        SELECT play_tennis, COUNT(*) AS count
        FROM decision_tree_data
        GROUP BY play_tennis
    ) t
),
feature_gains AS (
    -- Outlook特征
    SELECT
        'outlook' AS feature,
        t.entropy - (
            SELECT SUM(outlook_count * conditional_entropy) / SUM(outlook_count)
            FROM (
                SELECT
                    outlook,
                    COUNT(*) AS outlook_count,
                    calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS conditional_entropy
                FROM (
                    SELECT outlook, play_tennis, COUNT(*) AS count
                    FROM decision_tree_data
                    GROUP BY outlook, play_tennis
                ) t2
                GROUP BY outlook
            ) t3
        ) AS information_gain
    FROM total_entropy t

    UNION ALL

    -- Temperature特征（离散化）
    SELECT
        'temperature' AS feature,
        t.entropy - (
            SELECT SUM(temp_count * conditional_entropy) / SUM(temp_count)
            FROM (
                SELECT
                    CASE
                        WHEN temperature < 70 THEN 'cool'
                        WHEN temperature < 80 THEN 'mild'
                        ELSE 'hot'
                    END AS temp_category,
                    COUNT(*) AS temp_count,
                    calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS conditional_entropy
                FROM (
                    SELECT
                        CASE
                            WHEN temperature < 70 THEN 'cool'
                            WHEN temperature < 80 THEN 'mild'
                            ELSE 'hot'
                        END AS temp_category,
                        play_tennis,
                        COUNT(*) AS count
                    FROM decision_tree_data
                    GROUP BY temp_category, play_tennis
                ) t2
                GROUP BY temp_category
            ) t3
        ) AS information_gain
    FROM total_entropy t
)
SELECT
    feature,
    ROUND(information_gain::numeric, 4) AS information_gain
FROM feature_gains
ORDER BY information_gain DESC;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    outlook,
    play_tennis,
    COUNT(*) AS count
FROM decision_tree_data
GROUP BY outlook, play_tennis;
```

---

### 2.3 信息增益率

**信息增益率**解决信息增益偏向多值特征的问题。

```sql
-- 信息增益率计算：IGR = IG / IV
WITH total_entropy AS (
    SELECT calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS entropy
    FROM (
        SELECT play_tennis, COUNT(*) AS count
        FROM decision_tree_data
        GROUP BY play_tennis
    ) t
),
feature_stats AS (
    SELECT
        'outlook' AS feature,
        COUNT(DISTINCT outlook) AS value_count,
        COUNT(*) AS total_count,
        -- 信息增益
        (SELECT entropy FROM total_entropy) - (
            SELECT SUM(outlook_count * conditional_entropy) / SUM(outlook_count)
            FROM (
                SELECT
                    outlook,
                    COUNT(*) AS outlook_count,
                    calculate_entropy(ARRAY_AGG(count ORDER BY play_tennis)) AS conditional_entropy
                FROM (
                    SELECT outlook, play_tennis, COUNT(*) AS count
                    FROM decision_tree_data
                    GROUP BY outlook, play_tennis
                ) t2
                GROUP BY outlook
            ) t3
        ) AS information_gain,
        -- 固有值（Intrinsic Value）
        calculate_entropy(ARRAY_AGG(outlook_count)) AS intrinsic_value
    FROM (
        SELECT outlook, COUNT(*) AS outlook_count
        FROM decision_tree_data
        GROUP BY outlook
    ) t4
)
SELECT
    feature,
    value_count,
    ROUND(information_gain::numeric, 4) AS information_gain,
    ROUND(intrinsic_value::numeric, 4) AS intrinsic_value,
    -- 信息增益率
    ROUND((information_gain / NULLIF(intrinsic_value, 0))::numeric, 4) AS gain_ratio
FROM feature_stats;
```

### 2.4 基尼不纯度

**基尼不纯度（Gini Impurity）**是另一种衡量数据不纯度的方法。

```sql
-- 基尼不纯度计算函数
CREATE OR REPLACE FUNCTION calculate_gini(class_counts NUMERIC[])
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    total NUMERIC;
    gini NUMERIC := 1;
    p NUMERIC;
    i INTEGER;
BEGIN
    BEGIN
        IF class_counts IS NULL OR array_length(class_counts, 1) IS NULL THEN
            RETURN 0;
        END IF;

        SELECT SUM(unnest) INTO total FROM unnest(class_counts) AS unnest;

        IF total = 0 THEN
            RETURN 0;
        END IF;

        -- 计算基尼不纯度: 1 - Σ(p_i^2)
        FOR i IN 1..array_length(class_counts, 1) LOOP
            p := class_counts[i] / total;
            gini := gini - p * p;
        END LOOP;

        RETURN gini;
    EXCEPTION
        WHEN OTHERS THEN
            RETURN NULL;
    END;
END;
$$;

-- 基尼不纯度计算
WITH class_distribution AS (
    SELECT
        play_tennis,
        COUNT(*) AS count
    FROM decision_tree_data
    GROUP BY play_tennis
),
class_counts AS (
    SELECT ARRAY_AGG(count ORDER BY play_tennis) AS counts
    FROM class_distribution
)
SELECT
    counts,
    calculate_gini(counts) AS gini_impurity,
    calculate_entropy(counts) AS entropy
FROM class_counts;
```

---

## 3. 决策树构建

### 3.1 决策树构建原理

**决策树构建**使用递归方式选择最佳特征进行分割，直到满足停止条件。

#### ID3算法步骤

1. **计算数据集熵**：$H(S)$
2. **对每个特征**：
   - 计算条件熵：$H(S|A)$
   - 计算信息增益：$IG(S, A) = H(S) - H(S|A)$
3. **选择信息增益最大的特征**作为分割特征
4. **递归构建子树**：对每个特征值创建子节点
5. **停止条件**：
   - 所有样本属于同一类别
   - 没有更多特征
   - 信息增益小于阈值

### 3.2 简单决策树实现

```sql
-- 简单决策树预测（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decision_tree_data') THEN
            RAISE WARNING '表 decision_tree_data 不存在，无法构建决策树';
            RETURN;
        END IF;
        RAISE NOTICE '开始构建决策树';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '决策树构建准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 使用简化的决策规则进行预测
WITH predictions AS (
    SELECT
        id,
        outlook,
        temperature,
        humidity,
        windy,
        play_tennis AS actual,
        CASE
            -- 第一层：根据outlook分割
            WHEN outlook = 'overcast' THEN 1
            WHEN outlook = 'sunny' THEN
                CASE
                    -- 第二层：根据humidity分割
                    WHEN humidity <= 75 THEN 1
                    ELSE 0
                END
            WHEN outlook = 'rain' THEN
                CASE
                    -- 第二层：根据windy分割
                    WHEN windy = false THEN 1
                    ELSE 0
                END
            ELSE 0
        END AS predicted
    FROM decision_tree_data
)
SELECT
    id,
    outlook,
    temperature,
    humidity,
    windy,
    actual,
    predicted,
    CASE WHEN actual = predicted THEN 'Correct' ELSE 'Wrong' END AS status
FROM predictions
ORDER BY id;

-- 计算准确率
WITH predictions AS (
    SELECT
        play_tennis AS actual,
        CASE
            WHEN outlook = 'overcast' THEN 1
            WHEN outlook = 'sunny' AND humidity <= 75 THEN 1
            WHEN outlook = 'rain' AND windy = false THEN 1
            ELSE 0
        END AS predicted
    FROM decision_tree_data
)
SELECT
    COUNT(*) AS total,
    SUM(CASE WHEN actual = predicted THEN 1 ELSE 0 END) AS correct,
    ROUND(SUM(CASE WHEN actual = predicted THEN 1 ELSE 0 END)::numeric / COUNT(*), 4) AS accuracy
FROM predictions;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    id,
    CASE
        WHEN outlook = 'overcast' THEN 1
        WHEN outlook = 'sunny' AND humidity <= 75 THEN 1
        WHEN outlook = 'rain' AND windy = false THEN 1
        ELSE 0
    END AS predicted
FROM decision_tree_data;
```

---

### 3.3 决策树剪枝

**剪枝**防止决策树过拟合。

```sql
-- 决策树剪枝：使用最小样本数限制
WITH tree_nodes AS (
    SELECT
        outlook,
        humidity,
        windy,
        play_tennis,
        COUNT(*) AS node_count
    FROM decision_tree_data
    GROUP BY outlook, humidity, windy, play_tennis
),
pruned_nodes AS (
    SELECT
        outlook,
        humidity,
        windy,
        play_tennis,
        node_count,
        -- 如果节点样本数小于阈值，则合并到父节点
        CASE
            WHEN node_count < 2 THEN 'pruned'
            ELSE 'kept'
        END AS pruning_status
    FROM tree_nodes
)
SELECT
    outlook,
    humidity,
    windy,
    play_tennis,
    node_count,
    pruning_status
FROM pruned_nodes
ORDER BY outlook, humidity, windy;
```

### 3.4 决策树可视化（简化版）

**可视化**展示决策树的结构。

```sql
-- 决策树结构可视化：展示决策路径
WITH decision_paths AS (
    SELECT
        id,
        outlook,
        humidity,
        windy,
        play_tennis,
        -- 决策路径
        CASE
            WHEN outlook = 'overcast' THEN 'Root -> Overcast -> Play'
            WHEN outlook = 'sunny' AND humidity <= 75 THEN 'Root -> Sunny -> Low Humidity -> Play'
            WHEN outlook = 'sunny' AND humidity > 75 THEN 'Root -> Sunny -> High Humidity -> Don''t Play'
            WHEN outlook = 'rain' AND windy = false THEN 'Root -> Rain -> Not Windy -> Play'
            WHEN outlook = 'rain' AND windy = true THEN 'Root -> Rain -> Windy -> Don''t Play'
            ELSE 'Unknown'
        END AS decision_path
    FROM decision_tree_data
)
SELECT
    decision_path,
    COUNT(*) AS path_count,
    COUNT(*) FILTER (WHERE play_tennis = 1) AS play_count,
    COUNT(*) FILTER (WHERE play_tennis = 0) AS dont_play_count
FROM decision_paths
GROUP BY decision_path
ORDER BY path_count DESC;
```

---

## 4. 模型评估

### 4.1 准确率计算

**准确率**衡量分类模型的性能。

```sql
-- 准确率、精确率、召回率计算
WITH predictions AS (
    SELECT
        play_tennis AS actual,
        CASE
            WHEN outlook = 'overcast' THEN 1
            WHEN outlook = 'sunny' AND humidity <= 75 THEN 1
            WHEN outlook = 'rain' AND windy = false THEN 1
            ELSE 0
        END AS predicted
    FROM decision_tree_data
),
confusion_matrix AS (
    SELECT
        SUM(CASE WHEN actual = 1 AND predicted = 1 THEN 1 ELSE 0 END) AS tp,
        SUM(CASE WHEN actual = 1 AND predicted = 0 THEN 1 ELSE 0 END) AS fn,
        SUM(CASE WHEN actual = 0 AND predicted = 1 THEN 1 ELSE 0 END) AS fp,
        SUM(CASE WHEN actual = 0 AND predicted = 0 THEN 1 ELSE 0 END) AS tn,
        COUNT(*) AS total
    FROM predictions
)
SELECT
    tp,
    fn,
    fp,
    tn,
    total,
    -- 准确率
    ROUND((tp + tn)::numeric / total, 4) AS accuracy,
    -- 精确率
    ROUND(tp::numeric / NULLIF(tp + fp, 0), 4) AS precision,
    -- 召回率
    ROUND(tp::numeric / NULLIF(tp + fn, 0), 4) AS recall,
    -- F1分数
    ROUND(2 * tp::numeric / NULLIF(2 * tp + fp + fn, 0), 4) AS f1_score
FROM confusion_matrix;
```

---

## 5. 实际应用案例

### 5.1 客户分类

**客户分类**使用决策树对客户进行分类。

```sql
-- 客户分类示例（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customer_data') THEN
            RAISE WARNING '表 customer_data 不存在，创建示例表';

            CREATE TABLE customer_data (
                customer_id SERIAL PRIMARY KEY,
                age INTEGER NOT NULL,
                income NUMERIC NOT NULL,
                credit_score INTEGER NOT NULL,
                purchase_history INTEGER NOT NULL,
                customer_segment VARCHAR(20) NOT NULL
            );

            INSERT INTO customer_data (age, income, credit_score, purchase_history, customer_segment) VALUES
                (25, 30000, 650, 5, 'low'), (35, 50000, 720, 15, 'medium'),
                (45, 80000, 780, 30, 'high'), (30, 40000, 680, 8, 'low'),
                (50, 100000, 800, 50, 'high'), (28, 35000, 660, 6, 'low'),
                (40, 70000, 750, 25, 'medium'), (55, 120000, 820, 60, 'high');

            RAISE NOTICE '表 customer_data 创建成功';
        END IF;
        RAISE NOTICE '开始执行客户分类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '客户分类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 使用决策树规则进行客户分类
WITH predictions AS (
    SELECT
        customer_id,
        age,
        income,
        credit_score,
        purchase_history,
        customer_segment AS actual_segment,
        CASE
            WHEN income >= 90000 THEN 'high'
            WHEN income >= 60000 AND credit_score >= 750 THEN 'high'
            WHEN income >= 40000 AND purchase_history >= 20 THEN 'medium'
            WHEN income >= 40000 THEN 'medium'
            ELSE 'low'
        END AS predicted_segment
    FROM customer_data
)
SELECT
    customer_id,
    age,
    income,
    credit_score,
    purchase_history,
    actual_segment,
    predicted_segment,
    CASE WHEN actual_segment = predicted_segment THEN 'Correct' ELSE 'Wrong' END AS status
FROM predictions
ORDER BY customer_id;

-- 计算混淆矩阵
WITH predictions AS (
    SELECT
        customer_segment AS actual,
        CASE
            WHEN income >= 90000 THEN 'high'
            WHEN income >= 60000 AND credit_score >= 750 THEN 'high'
            WHEN income >= 40000 AND purchase_history >= 20 THEN 'medium'
            WHEN income >= 40000 THEN 'medium'
            ELSE 'low'
        END AS predicted
    FROM customer_data
)
SELECT
    actual,
    predicted,
    COUNT(*) AS count
FROM predictions
GROUP BY actual, predicted
ORDER BY actual, predicted;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    customer_id,
    CASE
        WHEN income >= 90000 THEN 'high'
        WHEN income >= 60000 AND credit_score >= 750 THEN 'high'
        WHEN income >= 40000 AND purchase_history >= 20 THEN 'medium'
        WHEN income >= 40000 THEN 'medium'
        ELSE 'low'
    END AS predicted_segment
FROM customer_data;
```

---

### 5.2 贷款风险评估

**场景**：使用决策树评估贷款风险。

```sql
-- 贷款风险评估：使用决策树规则评估贷款风险
WITH loan_applications AS (
    SELECT
        application_id,
        age,
        income,
        credit_score,
        employment_years,
        loan_amount,
        CASE
            WHEN credit_score < 600 THEN 'high_risk'
            WHEN credit_score < 700 AND income < 50000 THEN 'high_risk'
            WHEN credit_score < 700 AND employment_years < 2 THEN 'high_risk'
            WHEN credit_score >= 700 AND income >= 50000 THEN 'low_risk'
            WHEN credit_score >= 650 AND income >= 40000 AND employment_years >= 3 THEN 'medium_risk'
            ELSE 'medium_risk'
        END AS risk_level
    FROM loan_applications_table
)
SELECT
    application_id,
    age,
    income,
    credit_score,
    employment_years,
    loan_amount,
    risk_level,
    -- 风险评分
    CASE
        WHEN risk_level = 'low_risk' THEN '批准'
        WHEN risk_level = 'medium_risk' THEN '条件批准'
        ELSE '拒绝'
    END AS decision
FROM loan_applications
ORDER BY credit_score DESC, income DESC;
```

### 5.3 产品推荐决策树

**场景**：使用决策树规则进行产品推荐。

```sql
-- 产品推荐决策树：根据用户特征推荐产品
WITH user_features AS (
    SELECT
        user_id,
        age,
        gender,
        purchase_frequency,
        avg_order_value,
        preferred_category
    FROM users
),
product_recommendations AS (
    SELECT
        uf.user_id,
        uf.age,
        uf.gender,
        uf.purchase_frequency,
        uf.avg_order_value,
        uf.preferred_category,
        -- 决策树推荐规则
        CASE
            WHEN uf.age < 25 AND uf.purchase_frequency > 10 THEN '电子产品'
            WHEN uf.age >= 25 AND uf.age < 40 AND uf.avg_order_value > 500 THEN '高端商品'
            WHEN uf.age >= 40 AND uf.preferred_category IS NOT NULL THEN uf.preferred_category
            WHEN uf.gender = 'female' AND uf.purchase_frequency > 5 THEN '美妆护肤'
            WHEN uf.gender = 'male' AND uf.purchase_frequency > 5 THEN '运动户外'
            ELSE '通用商品'
        END AS recommended_category
    FROM user_features uf
)
SELECT
    user_id,
    age,
    gender,
    purchase_frequency,
    avg_order_value,
    preferred_category,
    recommended_category,
    -- 推荐置信度
    CASE
        WHEN recommended_category = preferred_category THEN '高置信度'
        WHEN purchase_frequency > 10 THEN '中置信度'
        ELSE '低置信度'
    END AS confidence_level
FROM product_recommendations
ORDER BY user_id;
```

### 5.4 医疗诊断决策树

**场景**：使用决策树辅助医疗诊断。

```sql
-- 医疗诊断决策树：根据症状诊断疾病
WITH patient_symptoms AS (
    SELECT
        patient_id,
        fever,
        cough,
        headache,
        fatigue,
        age,
        CASE
            WHEN fever > 38.5 AND cough = true AND age > 60 THEN '高风险'
            WHEN fever > 38.5 AND cough = true THEN '中风险'
            WHEN fever > 37.5 AND (cough = true OR headache = true) THEN '低风险'
            WHEN fatigue = true AND age > 50 THEN '观察'
            ELSE '正常'
        END AS risk_level
    FROM patient_records
)
SELECT
    patient_id,
    fever,
    cough,
    headache,
    fatigue,
    age,
    risk_level,
    -- 建议措施
    CASE
        WHEN risk_level = '高风险' THEN '立即就医'
        WHEN risk_level = '中风险' THEN '建议就医'
        WHEN risk_level = '低风险' THEN '观察症状'
        WHEN risk_level = '观察' THEN '注意休息'
        ELSE '正常生活'
    END AS recommendation
FROM patient_symptoms
ORDER BY
    CASE risk_level
        WHEN '高风险' THEN 1
        WHEN '中风险' THEN 2
        WHEN '低风险' THEN 3
        WHEN '观察' THEN 4
        ELSE 5
    END;
```

---

## 6. PostgreSQL 18并行决策树

### 6.1 并行决策树概述

**PostgreSQL 18并行决策树**：利用PostgreSQL 18的并行查询能力，显著提升大规模决策树计算的性能。

#### 并行决策树配置

```sql
-- 配置并行决策树参数（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 设置并行工作进程数
        SET max_parallel_workers_per_gather = 4;

        -- 设置并行表扫描阈值
        SET min_parallel_table_scan_size = '8MB';

        -- 设置并行聚合成本阈值
        SET parallel_setup_cost = 0;
        SET parallel_tuple_cost = 0.0001;

        RAISE NOTICE '并行决策树参数已配置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行决策树配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

#### 并行决策树实现

```sql
-- 并行信息增益计算：大规模数据特征选择（PostgreSQL 18+）
DO $$
BEGIN
    BEGIN
        -- 启用并行查询
        SET max_parallel_workers_per_gather = 4;
        SET parallel_setup_cost = 0;
        SET parallel_tuple_cost = 0.0001;

        RAISE NOTICE '开始执行并行决策树计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行决策树准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 并行信息增益计算
EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH parallel_class_distribution AS (
    SELECT
        feature_value,
        class_label,
        COUNT(*) AS class_count
    FROM training_data
    GROUP BY feature_value, class_label
),
parallel_feature_stats AS (
    SELECT
        feature_value,
        SUM(class_count) AS feature_count,
        COUNT(DISTINCT class_label) AS class_variety
    FROM parallel_class_distribution
    GROUP BY feature_value
)
SELECT
    feature_value,
    feature_count,
    class_variety,
    -- 信息增益计算（简化版）
    -SUM((class_count::numeric / SUM(class_count) OVER ()) *
         LOG(2, NULLIF(class_count::numeric / SUM(class_count) OVER (), 0))) AS entropy
FROM parallel_class_distribution
GROUP BY feature_value, feature_count, class_variety
ORDER BY entropy DESC;
```

---

## 7. 算法性能对比与优化

### 7.1 决策树算法对比

| 算法 | 分割准则 | 适用场景 | 优点 | 缺点 |
|------|---------|---------|------|------|
| **ID3** | 信息增益 | 分类问题 | 简单直观 | 偏向多值特征 |
| **C4.5** | 信息增益率 | 分类问题 | 处理连续值 | 计算复杂 |
| **CART** | 基尼不纯度 | 分类/回归 | 处理回归 | 二叉树结构 |
| **CHAID** | 卡方检验 | 分类问题 | 多路分割 | 需要大样本 |

### 7.2 性能优化建议

1. **计算优化**：
   - 使用窗口函数减少重复计算
   - 缓存熵值计算结果
   - 并行处理特征

2. **查询优化**：
   - 创建特征索引
   - 使用物化视图
   - 限制数据范围

3. **算法优化**：
   - 使用剪枝策略
   - 特征选择
   - 增量学习

### 7.3 常见问题与解决方案

**问题1**：过拟合

- **解决方案**：使用剪枝、限制树深度、增加最小样本数

**问题2**：信息增益偏向多值特征

- **解决方案**：使用信息增益率、使用基尼不纯度

**问题3**：计算性能问题

- **解决方案**：优化SQL查询、使用索引、采样计算

**问题4**：连续特征处理

- **解决方案**：离散化、使用CART算法、使用分位数分割

---

## 8. 最佳实践

### 8.1 决策树构建流程

1. **数据准备**：
   - 处理缺失值
   - 特征编码
   - 数据清洗

2. **特征选择**：
   - 计算信息增益
   - 选择重要特征
   - 特征工程

3. **树构建**：
   - 选择分割准则
   - 递归构建
   - 设置停止条件

4. **模型评估**：
   - 计算准确率
   - 交叉验证
   - 混淆矩阵

### 8.2 决策树注意事项

1. **过拟合**：
   - 使用剪枝
   - 限制树深度
   - 增加最小样本数

2. **特征选择**：
   - 避免偏向多值特征
   - 考虑特征重要性
   - 特征工程

3. **数据质量**：
   - 处理缺失值
   - 去除异常值
   - 数据平衡

### 8.3 SQL实现注意事项

1. **性能考虑**：
   - 优化聚合查询
   - 创建特征索引
   - 使用窗口函数

2. **准确性**：
   - 处理NULL值
   - 避免数值精度问题
   - 验证计算结果

3. **可维护性**：
   - 使用清晰的变量名
   - 添加注释
   - 模块化设计

### 8.4 PostgreSQL 18 新特性应用

**PostgreSQL 18**引入了多项增强功能，可以显著提升决策树算法的性能：

1. **Skip Scan优化**：
   - 对于包含特征值的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于特征选择和分割点查找

2. **异步I/O增强**：
   - 对于大规模决策树计算，异步I/O可以显著提升性能
   - 适用于信息增益计算和批量预测

3. **并行查询增强**：
   - 决策树信息增益计算支持更好的并行执行
   - 适用于大规模特征选择和树构建

**示例：使用Skip Scan优化特征选择查询**

```sql
-- 为决策树特征创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_decision_tree_features_skip_scan
ON classification_data USING btree(feature1 DESC, label);

-- Skip Scan优化查询：查找信息增益最高的特征分割点
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH feature_splits AS (
    SELECT DISTINCT
        feature1 AS split_value,
        COUNT(*) FILTER (WHERE label = 1) AS positive_count,
        COUNT(*) FILTER (WHERE label = 0) AS negative_count
    FROM classification_data
    WHERE feature1 IS NOT NULL
    GROUP BY feature1
)
SELECT
    split_value,
    positive_count,
    negative_count,
    ROUND((positive_count::numeric / NULLIF(positive_count + negative_count, 0))::numeric, 4) AS positive_ratio
FROM feature_splits
ORDER BY ABS(positive_ratio - 0.5) ASC  -- 选择最接近0.5的分割点
LIMIT 10;
```

### 8.5 高级优化技巧

**1. 使用物化视图缓存决策树模型**

对于频繁使用的决策树模型，使用物化视图缓存：

```sql
-- 创建物化视图缓存决策树节点信息
CREATE MATERIALIZED VIEW IF NOT EXISTS decision_tree_nodes_cache AS
WITH tree_nodes AS (
    SELECT
        node_id,
        feature_name,
        split_value,
        parent_node_id,
        COUNT(*) FILTER (WHERE label = 1) AS positive_samples,
        COUNT(*) FILTER (WHERE label = 0) AS negative_samples,
        COUNT(*) AS total_samples
    FROM decision_tree_data
    GROUP BY node_id, feature_name, split_value, parent_node_id
)
SELECT
    node_id,
    feature_name,
    split_value,
    parent_node_id,
    positive_samples,
    negative_samples,
    total_samples,
    -- 计算节点纯度（Gini不纯度）
    1 - POWER(positive_samples::numeric / NULLIF(total_samples, 0), 2) -
        POWER(negative_samples::numeric / NULLIF(total_samples, 0), 2) AS gini_impurity
FROM tree_nodes;

-- 创建索引加速物化视图查询
CREATE INDEX idx_decision_tree_nodes_cache_parent ON decision_tree_nodes_cache(parent_node_id);
CREATE INDEX idx_decision_tree_nodes_cache_feature ON decision_tree_nodes_cache(feature_name, split_value);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY decision_tree_nodes_cache;
```

**2. 决策树预测：批量预测优化**

使用递归CTE优化决策树预测：

```sql
-- 决策树预测：批量预测优化（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decision_tree_model') THEN
            CREATE TABLE decision_tree_model (
                node_id INTEGER PRIMARY KEY,
                feature_name VARCHAR(50),
                split_value NUMERIC,
                parent_node_id INTEGER,
                is_leaf BOOLEAN DEFAULT false,
                predicted_label INTEGER,
                FOREIGN KEY (parent_node_id) REFERENCES decision_tree_model(node_id)
            );

            CREATE INDEX idx_decision_tree_model_parent ON decision_tree_model(parent_node_id);
            CREATE INDEX idx_decision_tree_model_feature ON decision_tree_model(feature_name, split_value);

            RAISE NOTICE '决策树模型表创建成功';
        END IF;

        RAISE NOTICE '开始执行决策树批量预测';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '决策树预测准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 决策树预测：使用递归CTE遍历决策树
WITH RECURSIVE tree_traversal AS (
    -- 起始：从根节点开始
    SELECT
        cd.sample_id,
        cd.feature1,
        cd.feature2,
        dtm.node_id AS current_node,
        dtm.feature_name,
        dtm.split_value,
        dtm.is_leaf,
        dtm.predicted_label,
        ARRAY[dtm.node_id] AS path
    FROM classification_data cd
    CROSS JOIN decision_tree_model dtm
    WHERE dtm.parent_node_id IS NULL  -- 根节点

    UNION ALL

    -- 递归：根据特征值选择子节点
    SELECT
        tt.sample_id,
        tt.feature1,
        tt.feature2,
        dtm.node_id AS current_node,
        dtm.feature_name,
        dtm.split_value,
        dtm.is_leaf,
        dtm.predicted_label,
        tt.path || dtm.node_id
    FROM tree_traversal tt
    JOIN decision_tree_model dtm ON dtm.parent_node_id = tt.current_node
    WHERE NOT dtm.node_id = ANY(tt.path)  -- 避免循环
      AND (
          (dtm.feature_name = 'feature1' AND
           CASE WHEN dtm.split_value IS NOT NULL THEN
               CASE WHEN tt.feature1 <= dtm.split_value THEN true ELSE false END
           ELSE true END)
          OR
          (dtm.feature_name = 'feature2' AND
           CASE WHEN dtm.split_value IS NOT NULL THEN
               CASE WHEN tt.feature2 <= dtm.split_value THEN true ELSE false END
           ELSE true END)
      )
)
SELECT DISTINCT ON (sample_id)
    sample_id,
    feature1,
    feature2,
    current_node AS final_node,
    predicted_label,
    path AS decision_path
FROM tree_traversal
WHERE is_leaf = true
ORDER BY sample_id, array_length(path, 1) DESC;
```

**3. 随机森林：集成决策树预测**

**随机森林**：使用多个决策树的集成预测。

```sql
-- 随机森林：集成决策树预测（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'random_forest_trees') THEN
            CREATE TABLE random_forest_trees (
                tree_id INTEGER NOT NULL,
                node_id INTEGER NOT NULL,
                feature_name VARCHAR(50),
                split_value NUMERIC,
                parent_node_id INTEGER,
                is_leaf BOOLEAN DEFAULT false,
                predicted_label INTEGER,
                PRIMARY KEY (tree_id, node_id)
            );

            CREATE INDEX idx_rf_trees_tree_node ON random_forest_trees(tree_id, parent_node_id);

            RAISE NOTICE '随机森林树表创建成功';
        END IF;

        RAISE NOTICE '开始执行随机森林预测';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '随机森林预测准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 随机森林预测：集成多个决策树的预测结果
WITH tree_predictions AS (
    SELECT
        cd.sample_id,
        rft.tree_id,
        rft.predicted_label,
        -- 使用窗口函数计算每个树的预测（简化版）
        ROW_NUMBER() OVER (PARTITION BY cd.sample_id, rft.tree_id ORDER BY rft.node_id) AS tree_prediction_rank
    FROM classification_data cd
    CROSS JOIN random_forest_trees rft
    WHERE rft.is_leaf = true
      AND rft.tree_id IN (SELECT DISTINCT tree_id FROM random_forest_trees LIMIT 10)  -- 使用10棵树
),
forest_votes AS (
    SELECT
        sample_id,
        predicted_label,
        COUNT(*) AS vote_count,
        COUNT(*)::numeric / (SELECT COUNT(DISTINCT tree_id) FROM random_forest_trees LIMIT 10) AS vote_ratio
    FROM tree_predictions
    WHERE tree_prediction_rank = 1
    GROUP BY sample_id, predicted_label
),
forest_predictions AS (
    SELECT DISTINCT ON (sample_id)
        sample_id,
        predicted_label AS forest_predicted_label,
        vote_count,
        ROUND(vote_ratio::numeric, 4) AS confidence
    FROM forest_votes
    ORDER BY sample_id, vote_count DESC
)
SELECT
    sample_id,
    forest_predicted_label,
    vote_count,
    confidence
FROM forest_predictions
ORDER BY sample_id;
```

---

## 📚 参考资源

### 学术文献

1. **《机器学习》**（Mitchell, T. M., 1997）- 决策树理论

2. **《数据挖掘：概念与技术》**（Han, J., et al., 2011）- 决策树算法

3. **《统计学习方法》**（李航, 2012）- 决策树方法

### PostgreSQL官方文档

- **聚合函数**: <https://www.postgresql.org/docs/current/functions-aggregate.html>
- **窗口函数**: <https://www.postgresql.org/docs/current/tutorial-window.html>
- **数组函数**: <https://www.postgresql.org/docs/current/functions-array.html>

### 在线资源

- **决策树**: <https://en.wikipedia.org/wiki/Decision_tree>
- **ID3算法**: <https://en.wikipedia.org/wiki/ID3_algorithm>
- **C4.5算法**: <https://en.wikipedia.org/wiki/C4.5_algorithm>

### 相关算法

- **分类算法**：决策树分类
- **随机森林**：基于决策树的集成方法
- **梯度提升树**：基于决策树的提升方法
- **特征选择算法**：信息增益特征选择

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**文档状态**: ✅ 已完成
