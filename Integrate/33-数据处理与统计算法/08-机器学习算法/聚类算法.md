# PostgreSQL 机器学习聚类算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 聚类算法 | 无监督学习
> **难度级别**: ⭐⭐⭐⭐ (高级)

---

## 📋 目录

- [PostgreSQL 机器学习聚类算法完整指南](#postgresql-机器学习聚类算法完整指南)
  - [📋 目录](#-目录)
  - [聚类算法概述](#聚类算法概述)
    - [理论基础](#理论基础)
      - [聚类定义](#聚类定义)
      - [聚类目标](#聚类目标)
      - [聚类算法分类](#聚类算法分类)
      - [距离度量](#距离度量)
    - [核心算法](#核心算法)
  - [1. K-means聚类](#1-k-means聚类)
    - [1.1 K-means原理](#11-k-means原理)
      - [K-means算法步骤](#k-means算法步骤)
      - [K-means数学定义](#k-means数学定义)
      - [K-means优缺点](#k-means优缺点)
    - [1.2 迭代K-means实现](#12-迭代k-means实现)
    - [1.3 K-means++初始化](#13-k-means初始化)
    - [1.4 多迭代K-means](#14-多迭代k-means)
  - [2. DBSCAN聚类](#2-dbscan聚类)
    - [2.1 DBSCAN原理](#21-dbscan原理)
      - [DBSCAN核心概念](#dbscan核心概念)
      - [DBSCAN算法步骤](#dbscan算法步骤)
      - [DBSCAN参数](#dbscan参数)
    - [2.2 密度聚类实现](#22-密度聚类实现)
    - [2.3 DBSCAN簇扩展](#23-dbscan簇扩展)
  - [3. 聚类评估](#3-聚类评估)
    - [3.1 聚类评估原理](#31-聚类评估原理)
      - [内部评估指标](#内部评估指标)
      - [外部评估指标](#外部评估指标)
    - [3.2 轮廓系数实现](#32-轮廓系数实现)
    - [3.2 肘部法则](#32-肘部法则)
    - [3.3 WCSS和BCSS计算](#33-wcss和bcss计算)
  - [4. 层次聚类](#4-层次聚类)
    - [4.1 层次聚类原理](#41-层次聚类原理)
      - [层次聚类类型](#层次聚类类型)
      - [链接准则](#链接准则)
    - [4.2 层次聚类实现（简化版）](#42-层次聚类实现简化版)
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 客户细分](#51-客户细分)
    - [5.2 用户行为聚类](#52-用户行为聚类)
    - [5.3 产品聚类分析](#53-产品聚类分析)
    - [5.4 异常检测聚类](#54-异常检测聚类)
  - [6. 算法性能对比与优化](#6-算法性能对比与优化)
    - [6.1 聚类算法对比](#61-聚类算法对比)
    - [6.2 性能优化建议](#62-性能优化建议)
    - [6.3 常见问题与解决方案](#63-常见问题与解决方案)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 聚类流程](#71-聚类流程)
    - [7.2 聚类注意事项](#72-聚类注意事项)
    - [7.3 SQL实现注意事项](#73-sql实现注意事项)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 聚类算法概述

**聚类算法（Clustering Algorithm）**是无监督学习的核心方法，用于发现数据中的隐藏模式和分组。聚类将相似的数据点归为一类，不同类的数据点尽可能不同。

### 理论基础

#### 聚类定义

**聚类（Clustering）**是将数据对象分成多个组（簇）的过程，使得：

- **簇内相似度高**：同一簇内的对象尽可能相似
- **簇间相似度低**：不同簇的对象尽可能不同

#### 聚类目标

**聚类目标函数**（以K-means为例）：
$$J = \sum_{i=1}^{k}\sum_{x \in C_i}||x - \mu_i||^2$$

其中：

- $k$：簇的数量
- $C_i$：第$i$个簇
- $\mu_i$：第$i$个簇的中心
- $||x - \mu_i||^2$：数据点到簇中心的距离平方

#### 聚类算法分类

1. **基于划分的聚类**：
   - K-means
   - K-medoids
   - K-means++

2. **基于密度的聚类**：
   - DBSCAN
   - OPTICS
   - Mean Shift

3. **基于层次的聚类**：
   - 凝聚聚类（Agglomerative）
   - 分裂聚类（Divisive）

4. **基于模型的聚类**：
   - 高斯混合模型（GMM）
   - 期望最大化（EM）

#### 距离度量

**欧氏距离**：
$$d(x, y) = \sqrt{\sum_{i=1}^{n}(x_i - y_i)^2}$$

**曼哈顿距离**：
$$d(x, y) = \sum_{i=1}^{n}|x_i - y_i|$$

**余弦距离**：
$$d(x, y) = 1 - \frac{x \cdot y}{||x|| \cdot ||y||}$$

### 核心算法

| 算法 | 用途 | 复杂度 | 数学表示 |
|------|------|--------|---------|
| **K-means** | 基于中心的聚类 | $O(nki)$ | $J = \sum_{i=1}^{k}\sum_{x \in C_i}||x - \mu_i||^2$ |
| **DBSCAN** | 基于密度的聚类 | $O(n \log n)$ | 密度可达性 |
| **层次聚类** | 树状聚类 | $O(n^2)$ | 链接准则 |

---

## 1. K-means聚类

### 1.1 K-means原理

**K-means聚类（K-means Clustering）**是最常用的聚类算法，通过迭代优化聚类中心来最小化簇内平方和（WCSS）。

#### K-means算法步骤

1. **初始化**：随机选择$k$个初始聚类中心
2. **分配**：将每个数据点分配到最近的聚类中心
3. **更新**：重新计算每个簇的聚类中心
4. **迭代**：重复步骤2-3直到收敛

#### K-means数学定义

**目标函数**：
$$J = \sum_{i=1}^{k}\sum_{x \in C_i}||x - \mu_i||^2$$

**聚类中心更新**：
$$\mu_i = \frac{1}{|C_i|}\sum_{x \in C_i}x$$

#### K-means优缺点

**优点**：

- 简单高效
- 适用于球形簇
- 可扩展性强

**缺点**：

- 需要预先指定$k$
- 对初始值敏感
- 只能处理球形簇

### 1.2 迭代K-means实现

```sql
-- 创建聚类数据表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_data') THEN
            RAISE WARNING '表 cluster_data 已存在，先删除';
            DROP TABLE cluster_data CASCADE;
        END IF;

        CREATE TABLE cluster_data (
            id SERIAL PRIMARY KEY,
            x NUMERIC NOT NULL,
            y NUMERIC NOT NULL
        );

        -- 插入示例数据（3个簇）
        INSERT INTO cluster_data (x, y) VALUES
            -- 簇1
            (2, 3), (2.5, 3.5), (3, 4), (2.2, 3.2), (2.8, 3.8),
            -- 簇2
            (8, 9), (8.5, 9.5), (9, 10), (8.2, 9.2), (8.8, 9.8),
            -- 簇3
            (5, 2), (5.5, 2.5), (6, 3), (5.2, 2.2), (5.8, 2.8);

        RAISE NOTICE '表 cluster_data 创建成功，已插入15条数据';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 cluster_data 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- K-means迭代实现（带错误处理和性能测试）
DO $$
DECLARE
    k INTEGER := 3;
    max_iterations INTEGER := 10;
    iteration INTEGER := 0;
    converged BOOLEAN := false;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_data') THEN
            RAISE WARNING '表 cluster_data 不存在，无法执行K-means聚类';
            RETURN;
        END IF;

        -- 创建临时表存储聚类中心
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_centers') THEN
            DROP TABLE cluster_centers CASCADE;
        END IF;

        -- 初始化聚类中心（随机选择k个点）
        CREATE TEMP TABLE cluster_centers AS
        SELECT
            ROW_NUMBER() OVER () AS cluster_id,
            x AS center_x,
            y AS center_y
        FROM (
            SELECT x, y FROM cluster_data ORDER BY RANDOM() LIMIT k
        ) t;

        RAISE NOTICE 'K-means聚类开始，k=%, 最大迭代次数=%', k, max_iterations;

        -- 迭代优化（简化版，实际需要循环）
        RAISE NOTICE '迭代完成（简化实现）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'K-means聚类失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- K-means聚类（使用窗口函数实现）
WITH initial_centers AS (
    SELECT
        ROW_NUMBER() OVER () AS cluster_id,
        x AS center_x,
        y AS center_y
    FROM (
        SELECT x, y FROM cluster_data ORDER BY RANDOM() LIMIT 3
    ) t
),
distances AS (
    SELECT
        d.id,
        d.x,
        d.y,
        c.cluster_id,
        SQRT(POWER(d.x - c.center_x, 2) + POWER(d.y - c.center_y, 2)) AS distance
    FROM cluster_data d
    CROSS JOIN initial_centers c
),
closest_clusters AS (
    SELECT
        id,
        x,
        y,
        cluster_id,
        ROW_NUMBER() OVER (PARTITION BY id ORDER BY distance) AS rn
    FROM distances
),
assignments AS (
    SELECT
        id,
        x,
        y,
        cluster_id
    FROM closest_clusters
    WHERE rn = 1
),
new_centers AS (
    SELECT
        cluster_id,
        AVG(x) AS center_x,
        AVG(y) AS center_y,
        COUNT(*) AS cluster_size
    FROM assignments
    GROUP BY cluster_id
)
SELECT
    a.id,
    a.x,
    a.y,
    a.cluster_id,
    nc.center_x,
    nc.center_y,
    nc.cluster_size
FROM assignments a
JOIN new_centers nc ON a.cluster_id = nc.cluster_id
ORDER BY a.cluster_id, a.id;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH initial_centers AS (
    SELECT ROW_NUMBER() OVER () AS cluster_id, x AS center_x, y AS center_y
    FROM (SELECT x, y FROM cluster_data ORDER BY RANDOM() LIMIT 3) t
)
SELECT
    d.id,
    c.cluster_id,
    SQRT(POWER(d.x - c.center_x, 2) + POWER(d.y - c.center_y, 2)) AS distance
FROM cluster_data d
CROSS JOIN initial_centers c
LIMIT 100;
```

---

### 1.3 K-means++初始化

**K-means++**改进初始化方法，选择更分散的初始中心。

```sql
-- K-means++初始化：选择更分散的初始中心
WITH first_center AS (
    SELECT id, x, y
    FROM cluster_data
    ORDER BY RANDOM()
    LIMIT 1
),
distance_to_first AS (
    SELECT
        d.id,
        d.x,
        d.y,
        SQRT(POWER(d.x - fc.x, 2) + POWER(d.y - fc.y, 2)) AS distance
    FROM cluster_data d
    CROSS JOIN first_center fc
),
second_center AS (
    SELECT id, x, y
    FROM distance_to_first
    ORDER BY distance DESC
    LIMIT 1
),
distance_to_centers AS (
    SELECT
        d.id,
        d.x,
        d.y,
        LEAST(
            SQRT(POWER(d.x - fc.x, 2) + POWER(d.y - fc.y, 2)),
            SQRT(POWER(d.x - sc.x, 2) + POWER(d.y - sc.y, 2))
        ) AS min_distance
    FROM cluster_data d
    CROSS JOIN first_center fc
    CROSS JOIN second_center sc
),
third_center AS (
    SELECT id, x, y
    FROM distance_to_centers
    ORDER BY min_distance DESC
    LIMIT 1
)
SELECT
    ROW_NUMBER() OVER () AS cluster_id,
    x AS center_x,
    y AS center_y
FROM (
    SELECT x, y FROM first_center
    UNION ALL
    SELECT x, y FROM second_center
    UNION ALL
    SELECT x, y FROM third_center
) centers;
```

### 1.4 多迭代K-means

**多迭代**实现完整的K-means迭代过程。

```sql
-- 多迭代K-means：实现完整的迭代过程
WITH RECURSIVE kmeans_iterations AS (
    -- 初始中心
    SELECT
        0 AS iteration,
        ROW_NUMBER() OVER () AS cluster_id,
        x AS center_x,
        y AS center_y
    FROM (
        SELECT x, y FROM cluster_data ORDER BY RANDOM() LIMIT 3
    ) initial_centers

    UNION ALL

    -- 迭代更新
    SELECT
        ki.iteration + 1,
        nc.cluster_id,
        nc.center_x,
        nc.center_y
    FROM kmeans_iterations ki
    JOIN (
        SELECT
            cluster_id,
            AVG(x) AS center_x,
            AVG(y) AS center_y
        FROM (
            SELECT
                d.id,
                d.x,
                d.y,
                c.cluster_id,
                ROW_NUMBER() OVER (PARTITION BY d.id ORDER BY
                    SQRT(POWER(d.x - c.center_x, 2) + POWER(d.y - c.center_y, 2))
                ) AS rn
            FROM cluster_data d
            CROSS JOIN kmeans_iterations c
            WHERE c.iteration = (SELECT MAX(iteration) FROM kmeans_iterations)
        ) assignments
        WHERE rn = 1
        GROUP BY cluster_id
    ) nc ON ki.cluster_id = nc.cluster_id
    WHERE ki.iteration < 10  -- 最大迭代次数
)
SELECT * FROM kmeans_iterations WHERE iteration = (SELECT MAX(iteration) FROM kmeans_iterations);
```

---

## 2. DBSCAN聚类

### 2.1 DBSCAN原理

**DBSCAN（Density-Based Spatial Clustering of Applications with Noise）**基于密度的聚类算法，可以发现任意形状的簇，并识别噪声点。

#### DBSCAN核心概念

1. **核心点（Core Point）**：邻域内至少有$minPts$个点的点
2. **边界点（Border Point）**：不是核心点，但在核心点的邻域内
3. **噪声点（Noise Point）**：既不是核心点也不是边界点

#### DBSCAN算法步骤

1. **标记核心点**：对每个点，计算$\epsilon$邻域内的点数
2. **构建簇**：从核心点开始，通过密度可达性扩展簇
3. **分配边界点**：将边界点分配到最近的簇
4. **标记噪声点**：剩余的点标记为噪声

#### DBSCAN参数

- **$\epsilon$（eps）**：邻域半径
- **$minPts$**：形成核心点的最小点数

### 2.2 密度聚类实现

```sql
-- DBSCAN聚类实现（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_data') THEN
            RAISE WARNING '表 cluster_data 不存在，无法执行DBSCAN聚类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行DBSCAN聚类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'DBSCAN聚类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- DBSCAN核心函数：计算邻域内的点数
WITH point_neighbors AS (
    SELECT
        p1.id AS point_id,
        p1.x,
        p1.y,
        COUNT(*) - 1 AS neighbor_count  -- 减去自己
    FROM cluster_data p1
    CROSS JOIN cluster_data p2
    WHERE SQRT(POWER(p1.x - p2.x, 2) + POWER(p1.y - p2.y, 2)) <= 2.0  -- eps = 2.0
    GROUP BY p1.id, p1.x, p1.y
),
core_points AS (
    SELECT
        point_id,
        x,
        y,
        neighbor_count
    FROM point_neighbors
    WHERE neighbor_count >= 3  -- min_pts = 3
),
dbscan_clusters AS (
    SELECT
        c.id,
        c.x,
        c.y,
        CASE
            WHEN cp.point_id IS NOT NULL THEN
                DENSE_RANK() OVER (ORDER BY cp.point_id)
            ELSE -1  -- 噪声点
        END AS cluster_id
    FROM cluster_data c
    LEFT JOIN core_points cp ON SQRT(POWER(c.x - cp.x, 2) + POWER(c.y - cp.y, 2)) <= 2.0
)
SELECT
    id,
    x,
    y,
    cluster_id,
    CASE WHEN cluster_id = -1 THEN 'Noise' ELSE 'Cluster' END AS point_type
FROM dbscan_clusters
ORDER BY cluster_id, id;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    p1.id,
    COUNT(*) - 1 AS neighbor_count
FROM cluster_data p1
CROSS JOIN cluster_data p2
WHERE SQRT(POWER(p1.x - p2.x, 2) + POWER(p1.y - p2.y, 2)) <= 2.0
GROUP BY p1.id
LIMIT 100;
```

---

### 2.3 DBSCAN簇扩展

**簇扩展**实现完整的DBSCAN簇构建过程。

```sql
-- DBSCAN簇扩展：实现密度可达性
WITH point_neighbors AS (
    SELECT
        p1.id AS point_id,
        p1.x,
        p1.y,
        ARRAY_AGG(p2.id) AS neighbor_ids,
        COUNT(*) - 1 AS neighbor_count
    FROM cluster_data p1
    CROSS JOIN cluster_data p2
    WHERE SQRT(POWER(p1.x - p2.x, 2) + POWER(p1.y - p2.y, 2)) <= 2.0
    GROUP BY p1.id, p1.x, p1.y
),
core_points AS (
    SELECT
        point_id,
        x,
        y,
        neighbor_ids
    FROM point_neighbors
    WHERE neighbor_count >= 3  -- minPts = 3
),
cluster_expansion AS (
    SELECT
        cp.point_id AS seed_point,
        cp.point_id AS cluster_point,
        cp.neighbor_ids
    FROM core_points cp

    UNION ALL

    SELECT
        ce.seed_point,
        unnest(ce.neighbor_ids) AS cluster_point,
        pn.neighbor_ids
    FROM cluster_expansion ce
    JOIN point_neighbors pn ON unnest(ce.neighbor_ids) = pn.point_id
    WHERE pn.neighbor_count >= 3  -- 只扩展核心点
),
dbscan_clusters AS (
    SELECT DISTINCT
        seed_point AS cluster_id,
        cluster_point AS point_id
    FROM cluster_expansion
)
SELECT
    COALESCE(dc.cluster_id, -1) AS cluster_id,
    d.id,
    d.x,
    d.y,
    CASE WHEN dc.cluster_id IS NULL THEN 'Noise' ELSE 'Cluster' END AS point_type
FROM cluster_data d
LEFT JOIN dbscan_clusters dc ON d.id = dc.point_id
ORDER BY cluster_id, d.id;
```

---

## 3. 聚类评估

### 3.1 聚类评估原理

**聚类评估**用于衡量聚类质量，帮助选择最优的聚类参数和算法。

#### 内部评估指标

1. **轮廓系数（Silhouette Coefficient）**：衡量簇内紧密度和簇间分离度
2. **WCSS（Within-Cluster Sum of Squares）**：簇内平方和
3. **BCSS（Between-Cluster Sum of Squares）**：簇间平方和

#### 外部评估指标

1. **调整兰德指数（ARI）**：与真实标签的相似度
2. **标准化互信息（NMI）**：信息论指标
3. **同质性（Homogeneity）**：每个簇只包含一个类的样本

### 3.2 轮廓系数实现

```sql
-- 轮廓系数计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_data') THEN
            RAISE WARNING '表 cluster_data 不存在，无法计算轮廓系数';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算轮廓系数';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '轮廓系数计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算轮廓系数
WITH kmeans_clusters AS (
    SELECT
        id,
        x,
        y,
        NTILE(3) OVER (ORDER BY x, y) AS cluster_id
    FROM cluster_data
),
intra_cluster_distances AS (
    SELECT
        c1.id,
        c1.cluster_id,
        AVG(SQRT(POWER(c1.x - c2.x, 2) + POWER(c1.y - c2.y, 2))) AS avg_intra_distance
    FROM kmeans_clusters c1
    JOIN kmeans_clusters c2 ON c1.cluster_id = c2.cluster_id AND c1.id != c2.id
    GROUP BY c1.id, c1.cluster_id
),
inter_cluster_distances AS (
    SELECT
        c1.id,
        c1.cluster_id,
        c2.cluster_id AS other_cluster,
        AVG(SQRT(POWER(c1.x - c2.x, 2) + POWER(c1.y - c2.y, 2))) AS avg_inter_distance
    FROM kmeans_clusters c1
    JOIN kmeans_clusters c2 ON c1.cluster_id != c2.cluster_id
    GROUP BY c1.id, c1.cluster_id, c2.cluster_id
),
min_inter_distances AS (
    SELECT
        id,
        cluster_id,
        MIN(avg_inter_distance) AS min_inter_distance
    FROM inter_cluster_distances
    GROUP BY id, cluster_id
),
silhouette_scores AS (
    SELECT
        i.id,
        i.cluster_id,
        i.avg_intra_distance AS a,
        m.min_inter_distance AS b,
        CASE
            WHEN GREATEST(a, b) = 0 THEN 0
            ELSE (b - a) / GREATEST(a, b)
        END AS silhouette
    FROM intra_cluster_distances i
    JOIN min_inter_distances m ON i.id = m.id
)
SELECT
    cluster_id,
    COUNT(*) AS cluster_size,
    ROUND(AVG(silhouette)::numeric, 4) AS avg_silhouette,
    ROUND(MIN(silhouette)::numeric, 4) AS min_silhouette,
    ROUND(MAX(silhouette)::numeric, 4) AS max_silhouette
FROM silhouette_scores
GROUP BY cluster_id
ORDER BY cluster_id;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    id,
    NTILE(3) OVER (ORDER BY x, y) AS cluster_id
FROM cluster_data;
```

### 3.2 肘部法则

**肘部法则**用于确定最优的K值。

```sql
-- 肘部法则计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cluster_data') THEN
            RAISE WARNING '表 cluster_data 不存在，无法计算肘部法则';
            RETURN;
        END IF;
        RAISE NOTICE '开始计算肘部法则';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '肘部法则计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 计算不同K值的SSE（误差平方和）
WITH k_values AS (
    SELECT unnest(ARRAY[1, 2, 3, 4, 5]) AS k
),
kmeans_results AS (
    SELECT
        k,
        id,
        x,
        y,
        NTILE(k) OVER (PARTITION BY k ORDER BY x, y) AS cluster_id
    FROM cluster_data
    CROSS JOIN k_values
),
cluster_centers AS (
    SELECT
        k,
        cluster_id,
        AVG(x) AS center_x,
        AVG(y) AS center_y
    FROM kmeans_results
    GROUP BY k, cluster_id
),
sse AS (
    SELECT
        kr.k,
        SUM(POWER(kr.x - cc.center_x, 2) + POWER(kr.y - cc.center_y, 2)) AS sse_value
    FROM kmeans_results kr
    JOIN cluster_centers cc ON kr.k = cc.k AND kr.cluster_id = cc.cluster_id
    GROUP BY kr.k
)
SELECT
    k,
    ROUND(sse_value::numeric, 2) AS sse,
    ROUND((sse_value - LAG(sse_value) OVER (ORDER BY k))::numeric, 2) AS sse_reduction
FROM sse
ORDER BY k;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    k,
    COUNT(*) AS cluster_count
FROM (
    SELECT
        unnest(ARRAY[1, 2, 3, 4, 5]) AS k,
        NTILE(unnest) OVER (PARTITION BY unnest ORDER BY x, y) AS cluster_id
    FROM cluster_data
    CROSS JOIN generate_series(1, 5) AS k
) t
GROUP BY k;
```

---

### 3.3 WCSS和BCSS计算

**WCSS和BCSS**评估聚类质量。

```sql
-- WCSS和BCSS计算：评估聚类质量
WITH kmeans_clusters AS (
    SELECT
        id,
        x,
        y,
        NTILE(3) OVER (ORDER BY x, y) AS cluster_id
    FROM cluster_data
),
cluster_centers AS (
    SELECT
        cluster_id,
        AVG(x) AS center_x,
        AVG(y) AS center_y
    FROM kmeans_clusters
    GROUP BY cluster_id
),
overall_center AS (
    SELECT
        AVG(x) AS overall_x,
        AVG(y) AS overall_y
    FROM cluster_data
),
wcss AS (
    SELECT
        SUM(POWER(kc.x - cc.center_x, 2) + POWER(kc.y - cc.center_y, 2)) AS wcss_value
    FROM kmeans_clusters kc
    JOIN cluster_centers cc ON kc.cluster_id = cc.cluster_id
),
bcss AS (
    SELECT
        SUM(
            COUNT(*) OVER (PARTITION BY kc.cluster_id) *
            (POWER(cc.center_x - oc.overall_x, 2) + POWER(cc.center_y - oc.overall_y, 2))
        ) AS bcss_value
    FROM kmeans_clusters kc
    JOIN cluster_centers cc ON kc.cluster_id = cc.cluster_id
    CROSS JOIN overall_center oc
    GROUP BY kc.cluster_id, cc.center_x, cc.center_y, oc.overall_x, oc.overall_y
)
SELECT
    ROUND(wcss_value::numeric, 2) AS wcss,
    ROUND(bcss_value::numeric, 2) AS bcss,
    ROUND((bcss_value / NULLIF(wcss_value + bcss_value, 0))::numeric, 4) AS explained_variance_ratio
FROM wcss, bcss;
```

---

## 4. 层次聚类

### 4.1 层次聚类原理

**层次聚类（Hierarchical Clustering）**构建一个树状的聚类层次结构，不需要预先指定簇的数量。

#### 层次聚类类型

1. **凝聚聚类（Agglomerative）**：自底向上，从单个点开始合并
2. **分裂聚类（Divisive）**：自顶向下，从所有点开始分裂

#### 链接准则

1. **单链接（Single Linkage）**：$d(C_i, C_j) = \min_{x \in C_i, y \in C_j} d(x, y)$
2. **完全链接（Complete Linkage）**：$d(C_i, C_j) = \max_{x \in C_i, y \in C_j} d(x, y)$
3. **平均链接（Average Linkage）**：$d(C_i, C_j) = \frac{1}{|C_i||C_j|}\sum_{x \in C_i}\sum_{y \in C_j} d(x, y)$

### 4.2 层次聚类实现（简化版）

```sql
-- 层次聚类：使用单链接准则（简化版）
WITH distance_matrix AS (
    SELECT
        d1.id AS id1,
        d2.id AS id2,
        SQRT(POWER(d1.x - d2.x, 2) + POWER(d1.y - d2.y, 2)) AS distance
    FROM cluster_data d1
    JOIN cluster_data d2 ON d1.id < d2.id
),
closest_pairs AS (
    SELECT
        id1,
        id2,
        distance,
        ROW_NUMBER() OVER (ORDER BY distance) AS merge_order
    FROM distance_matrix
)
SELECT
    merge_order,
    id1,
    id2,
    ROUND(distance::numeric, 4) AS merge_distance
FROM closest_pairs
ORDER BY merge_order
LIMIT 10;  -- 显示前10次合并
```

---

## 5. 实际应用案例

### 5.1 客户细分

**客户细分**使用聚类算法对客户进行分组。

```sql
-- 客户细分示例（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customer_features') THEN
            RAISE WARNING '表 customer_features 不存在，创建示例表';

            CREATE TABLE customer_features (
                customer_id SERIAL PRIMARY KEY,
                annual_spending NUMERIC NOT NULL,
                purchase_frequency NUMERIC NOT NULL,
                avg_order_value NUMERIC NOT NULL
            );

            INSERT INTO customer_features (annual_spending, purchase_frequency, avg_order_value) VALUES
                (500, 2, 250), (600, 3, 200), (800, 4, 200),
                (1200, 6, 200), (1500, 8, 187.5), (2000, 10, 200),
                (3000, 12, 250), (5000, 15, 333.33), (8000, 20, 400);

            RAISE NOTICE '表 customer_features 创建成功';
        END IF;
        RAISE NOTICE '开始执行客户细分';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '客户细分准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 使用K-means进行客户细分
WITH normalized_features AS (
    SELECT
        customer_id,
        annual_spending,
        purchase_frequency,
        avg_order_value,
        (annual_spending - AVG(annual_spending) OVER ()) / NULLIF(STDDEV(annual_spending) OVER (), 0) AS norm_spending,
        (purchase_frequency - AVG(purchase_frequency) OVER ()) / NULLIF(STDDEV(purchase_frequency) OVER (), 0) AS norm_frequency,
        (avg_order_value - AVG(avg_order_value) OVER ()) / NULLIF(STDDEV(avg_order_value) OVER (), 0) AS norm_order_value
    FROM customer_features
),
kmeans_clusters AS (
    SELECT
        customer_id,
        annual_spending,
        purchase_frequency,
        avg_order_value,
        NTILE(3) OVER (ORDER BY norm_spending, norm_frequency, norm_order_value) AS segment_id
    FROM normalized_features
),
segment_profiles AS (
    SELECT
        segment_id,
        COUNT(*) AS customer_count,
        ROUND(AVG(annual_spending)::numeric, 2) AS avg_spending,
        ROUND(AVG(purchase_frequency)::numeric, 2) AS avg_frequency,
        ROUND(AVG(avg_order_value)::numeric, 2) AS avg_order_value,
        CASE segment_id
            WHEN 1 THEN '低价值客户'
            WHEN 2 THEN '中价值客户'
            WHEN 3 THEN '高价值客户'
        END AS segment_name
    FROM kmeans_clusters
    GROUP BY segment_id
)
SELECT
    c.customer_id,
    c.annual_spending,
    c.purchase_frequency,
    c.avg_order_value,
    c.segment_id,
    sp.segment_name,
    sp.avg_spending AS segment_avg_spending
FROM kmeans_clusters c
JOIN segment_profiles sp ON c.segment_id = sp.segment_id
ORDER BY c.segment_id, c.annual_spending DESC;

-- 性能测试
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    customer_id,
    NTILE(3) OVER (ORDER BY annual_spending, purchase_frequency) AS segment_id
FROM customer_features;
```

---

### 5.2 用户行为聚类

**场景**：根据用户行为对用户进行聚类。

```sql
-- 用户行为聚类：基于用户活跃度和消费行为
WITH user_behavior_features AS (
    SELECT
        user_id,
        COUNT(DISTINCT DATE(activity_time)) AS active_days,
        COUNT(*) AS total_activities,
        SUM(CASE WHEN activity_type = 'purchase' THEN 1 ELSE 0 END) AS purchase_count,
        AVG(CASE WHEN activity_type = 'purchase' THEN amount ELSE NULL END) AS avg_purchase_amount
    FROM user_activities
    WHERE activity_time >= NOW() - INTERVAL '30 days'
    GROUP BY user_id
),
normalized_features AS (
    SELECT
        user_id,
        active_days,
        total_activities,
        purchase_count,
        avg_purchase_amount,
        (active_days - AVG(active_days) OVER ()) / NULLIF(STDDEV(active_days) OVER (), 0) AS norm_active_days,
        (total_activities - AVG(total_activities) OVER ()) / NULLIF(STDDEV(total_activities) OVER (), 0) AS norm_total_activities,
        (purchase_count - AVG(purchase_count) OVER ()) / NULLIF(STDDEV(purchase_count) OVER (), 0) AS norm_purchase_count
    FROM user_behavior_features
),
user_clusters AS (
    SELECT
        user_id,
        active_days,
        total_activities,
        purchase_count,
        avg_purchase_amount,
        NTILE(4) OVER (
            ORDER BY norm_active_days, norm_total_activities, norm_purchase_count
        ) AS cluster_id
    FROM normalized_features
),
cluster_profiles AS (
    SELECT
        cluster_id,
        COUNT(*) AS user_count,
        ROUND(AVG(active_days)::numeric, 2) AS avg_active_days,
        ROUND(AVG(total_activities)::numeric, 2) AS avg_total_activities,
        ROUND(AVG(purchase_count)::numeric, 2) AS avg_purchase_count,
        ROUND(AVG(avg_purchase_amount)::numeric, 2) AS avg_purchase_amount,
        CASE cluster_id
            WHEN 1 THEN '低活跃用户'
            WHEN 2 THEN '中等活跃用户'
            WHEN 3 THEN '高活跃用户'
            WHEN 4 THEN '超级用户'
        END AS cluster_label
    FROM user_clusters
    GROUP BY cluster_id
)
SELECT
    uc.user_id,
    uc.active_days,
    uc.total_activities,
    uc.purchase_count,
    uc.avg_purchase_amount,
    uc.cluster_id,
    cp.cluster_label,
    cp.avg_active_days,
    cp.avg_total_activities
FROM user_clusters uc
JOIN cluster_profiles cp ON uc.cluster_id = cp.cluster_id
ORDER BY uc.cluster_id, uc.total_activities DESC;
```

### 5.3 产品聚类分析

**场景**：根据产品特征对产品进行聚类。

```sql
-- 产品聚类分析：基于产品属性和销售表现
WITH product_features AS (
    SELECT
        product_id,
        category,
        price,
        rating_avg,
        sales_count,
        return_rate,
        -- 特征编码
        CASE category
            WHEN 'electronics' THEN 1
            WHEN 'books' THEN 2
            WHEN 'clothing' THEN 3
            ELSE 4
        END AS category_code
    FROM products
),
normalized_products AS (
    SELECT
        product_id,
        category,
        price,
        rating_avg,
        sales_count,
        return_rate,
        category_code,
        (price - AVG(price) OVER ()) / NULLIF(STDDEV(price) OVER (), 0) AS norm_price,
        (rating_avg - AVG(rating_avg) OVER ()) / NULLIF(STDDEV(rating_avg) OVER (), 0) AS norm_rating,
        (sales_count - AVG(sales_count) OVER ()) / NULLIF(STDDEV(sales_count) OVER (), 0) AS norm_sales
    FROM product_features
),
product_clusters AS (
    SELECT
        product_id,
        category,
        price,
        rating_avg,
        sales_count,
        return_rate,
        NTILE(5) OVER (
            ORDER BY norm_price, norm_rating, norm_sales
        ) AS cluster_id
    FROM normalized_products
),
cluster_analysis AS (
    SELECT
        cluster_id,
        COUNT(*) AS product_count,
        ROUND(AVG(price)::numeric, 2) AS avg_price,
        ROUND(AVG(rating_avg)::numeric, 2) AS avg_rating,
        ROUND(AVG(sales_count)::numeric, 2) AS avg_sales,
        ROUND(AVG(return_rate)::numeric, 4) AS avg_return_rate,
        -- 聚类特征
        CASE
            WHEN AVG(price) > (SELECT AVG(price) FROM product_features) AND
                 AVG(rating_avg) > (SELECT AVG(rating_avg) FROM product_features) THEN '高端优质'
            WHEN AVG(price) < (SELECT AVG(price) FROM product_features) AND
                 AVG(sales_count) > (SELECT AVG(sales_count) FROM product_features) THEN '性价比高'
            WHEN AVG(rating_avg) < (SELECT AVG(rating_avg) FROM product_features) THEN '需要改进'
            ELSE '普通产品'
        END AS cluster_characteristic
    FROM product_clusters
    GROUP BY cluster_id
)
SELECT
    pc.product_id,
    pc.category,
    pc.price,
    pc.rating_avg,
    pc.sales_count,
    pc.cluster_id,
    ca.cluster_characteristic,
    ca.avg_price AS cluster_avg_price,
    ca.avg_rating AS cluster_avg_rating
FROM product_clusters pc
JOIN cluster_analysis ca ON pc.cluster_id = ca.cluster_id
ORDER BY pc.cluster_id, pc.sales_count DESC;
```

### 5.4 异常检测聚类

**场景**：使用聚类识别异常数据点。

```sql
-- 异常检测聚类：使用DBSCAN识别异常点
WITH system_metrics AS (
    SELECT
        metric_id,
        cpu_usage,
        memory_usage,
        disk_io,
        network_io
    FROM system_monitoring
    WHERE timestamp >= NOW() - INTERVAL '1 hour'
),
point_neighbors AS (
    SELECT
        m1.metric_id,
        COUNT(*) - 1 AS neighbor_count
    FROM system_metrics m1
    CROSS JOIN system_metrics m2
    WHERE SQRT(
        POWER(m1.cpu_usage - m2.cpu_usage, 2) +
        POWER(m1.memory_usage - m2.memory_usage, 2) +
        POWER(m1.disk_io - m2.disk_io, 2) +
        POWER(m1.network_io - m2.network_io, 2)
    ) <= 10.0  -- eps阈值
    GROUP BY m1.metric_id
),
anomaly_detection AS (
    SELECT
        m.metric_id,
        m.cpu_usage,
        m.memory_usage,
        m.disk_io,
        m.network_io,
        COALESCE(pn.neighbor_count, 0) AS neighbor_count,
        CASE
            WHEN COALESCE(pn.neighbor_count, 0) < 3 THEN '异常点'
            ELSE '正常点'
        END AS anomaly_status
    FROM system_metrics m
    LEFT JOIN point_neighbors pn ON m.metric_id = pn.metric_id
)
SELECT
    metric_id,
    cpu_usage,
    memory_usage,
    disk_io,
    network_io,
    neighbor_count,
    anomaly_status
FROM anomaly_detection
WHERE anomaly_status = '异常点'
ORDER BY neighbor_count ASC;
```

---

## 6. 算法性能对比与优化

### 6.1 聚类算法对比

| 算法 | 时间复杂度 | 空间复杂度 | 适用场景 | 优点 | 缺点 |
|------|-----------|-----------|---------|------|------|
| **K-means** | $O(nki)$ | $O(n)$ | 球形簇 | 简单快速 | 需要指定k |
| **DBSCAN** | $O(n \log n)$ | $O(n)$ | 任意形状 | 自动确定簇数 | 参数敏感 |
| **层次聚类** | $O(n^2)$ | $O(n^2)$ | 树状结构 | 可视化好 | 计算复杂 |
| **GMM** | $O(nk^2i)$ | $O(nk)$ | 混合分布 | 概率模型 | 计算复杂 |

### 6.2 性能优化建议

1. **计算优化**：
   - 使用索引加速距离计算
   - 采样计算
   - 并行处理

2. **查询优化**：
   - 创建特征索引
   - 使用物化视图
   - 限制数据范围

3. **算法优化**：
   - K-means++初始化
   - Mini-batch K-means
   - 增量聚类

### 6.3 常见问题与解决方案

**问题1**：K值选择困难

- **解决方案**：使用肘部法则、轮廓系数、业务知识

**问题2**：初始值敏感

- **解决方案**：使用K-means++、多次运行取最优

**问题3**：计算性能问题

- **解决方案**：优化SQL查询、使用索引、采样计算

**问题4**：非球形簇

- **解决方案**：使用DBSCAN、核K-means、谱聚类

---

## 7. 最佳实践

### 7.1 聚类流程

1. **数据准备**：
   - 数据清洗
   - 特征选择
   - 数据标准化

2. **算法选择**：
   - 根据数据特征选择
   - 考虑计算资源
   - 评估算法性能

3. **参数调优**：
   - K值选择
   - 距离度量选择
   - 初始化方法

4. **结果评估**：
   - 使用多个指标
   - 业务验证
   - 可视化分析

### 7.2 聚类注意事项

1. **数据预处理**：
   - 标准化特征
   - 处理缺失值
   - 去除异常值

2. **参数选择**：
   - K值：肘部法则、轮廓系数
   - DBSCAN：eps和minPts
   - 距离度量：根据数据特征

3. **结果解释**：
   - 结合业务知识
   - 分析簇特征
   - 验证聚类合理性

### 7.3 SQL实现注意事项

1. **性能考虑**：
   - 优化距离计算
   - 创建特征索引
   - 使用窗口函数

2. **准确性**：
   - 处理NULL值
   - 避免数值精度问题
   - 验证计算结果

3. **可维护性**：
   - 使用清晰的变量名
   - 添加注释
   - 模块化设计

### 7.4 PostgreSQL 18 新特性应用

**PostgreSQL 18**引入了多项增强功能，可以显著提升聚类算法的性能：

1. **Skip Scan优化**：
   - 对于包含特征值的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于K-means初始中心点选择和Top-K最近邻查询

2. **异步I/O增强**：
   - 对于大规模聚类计算，异步I/O可以显著提升性能
   - 适用于批量距离计算和大规模数据聚类

3. **并行查询增强**：
   - 聚类距离计算支持更好的并行执行
   - 适用于大规模数据聚类和批量聚类分析

**示例：使用Skip Scan优化K-means初始中心点选择**

```sql
-- 为聚类特征创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_clustering_features_skip_scan
ON clustering_data USING btree(feature1 DESC, feature2 DESC);

-- Skip Scan优化查询：选择K-means初始中心点（K-means++简化版）
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH initial_centers AS (
    SELECT DISTINCT ON (feature1, feature2)
        sample_id,
        feature1,
        feature2,
        ROW_NUMBER() OVER (ORDER BY feature1 DESC, feature2 DESC) AS center_rank
    FROM clustering_data
    ORDER BY feature1 DESC, feature2 DESC
    LIMIT 5  -- K=5
)
SELECT
    center_rank AS cluster_id,
    feature1 AS center_x,
    feature2 AS center_y
FROM initial_centers
ORDER BY center_rank;
```

### 7.5 高级优化技巧

**1. 使用物化视图缓存聚类结果**

对于频繁使用的聚类结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存K-means聚类结果
CREATE MATERIALIZED VIEW IF NOT EXISTS kmeans_clusters_cache AS
WITH kmeans_centers AS (
    SELECT
        cluster_id,
        AVG(feature1) AS center_x,
        AVG(feature2) AS center_y
    FROM clustering_data
    GROUP BY cluster_id
),
sample_distances AS (
    SELECT
        cd.sample_id,
        cd.feature1,
        cd.feature2,
        kc.cluster_id,
        SQRT(POWER(cd.feature1 - kc.center_x, 2) + POWER(cd.feature2 - kc.center_y, 2)) AS distance_to_center
    FROM clustering_data cd
    CROSS JOIN kmeans_centers kc
),
closest_clusters AS (
    SELECT DISTINCT ON (sample_id)
        sample_id,
        feature1,
        feature2,
        cluster_id,
        distance_to_center
    FROM sample_distances
    ORDER BY sample_id, distance_to_center ASC
)
SELECT
    sample_id,
    feature1,
    feature2,
    cluster_id,
    ROUND(distance_to_center::numeric, 4) AS distance_to_center
FROM closest_clusters;

-- 创建索引加速物化视图查询
CREATE INDEX idx_kmeans_clusters_cache_cluster ON kmeans_clusters_cache(cluster_id);
CREATE INDEX idx_kmeans_clusters_cache_sample ON kmeans_clusters_cache(sample_id);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY kmeans_clusters_cache;
```

**2. 增量K-means聚类：流式数据聚类**

**增量K-means聚类**：对于流式数据，使用增量方法更新聚类中心。

```sql
-- 增量K-means聚类：流式数据聚类（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kmeans_cluster_state') THEN
            CREATE TABLE kmeans_cluster_state (
                cluster_id INTEGER PRIMARY KEY,
                center_x NUMERIC NOT NULL,
                center_y NUMERIC NOT NULL,
                sample_count BIGINT NOT NULL DEFAULT 0,
                last_updated TIMESTAMPTZ DEFAULT NOW()
            );

            RAISE NOTICE 'K-means聚类状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量K-means聚类';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量K-means聚类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新K-means聚类中心
WITH new_samples AS (
    SELECT
        feature1,
        feature2
    FROM clustering_data
    WHERE sample_id > (SELECT COALESCE(MAX(sample_id), 0) FROM kmeans_cluster_state)
),
sample_cluster_assignment AS (
    SELECT
        ns.feature1,
        ns.feature2,
        kcs.cluster_id,
        SQRT(POWER(ns.feature1 - kcs.center_x, 2) + POWER(ns.feature2 - kcs.center_y, 2)) AS distance
    FROM new_samples ns
    CROSS JOIN kmeans_cluster_state kcs
),
closest_cluster AS (
    SELECT DISTINCT ON (feature1, feature2)
        feature1,
        feature2,
        cluster_id
    FROM sample_cluster_assignment
    ORDER BY feature1, feature2, distance ASC
),
updated_centers AS (
    SELECT
        cc.cluster_id,
        -- 增量更新聚类中心：加权平均
        (kcs.center_x * kcs.sample_count + AVG(cc.feature1) * COUNT(*)) /
        NULLIF(kcs.sample_count + COUNT(*), 0) AS new_center_x,
        (kcs.center_y * kcs.sample_count + AVG(cc.feature2) * COUNT(*)) /
        NULLIF(kcs.sample_count + COUNT(*), 0) AS new_center_y,
        kcs.sample_count + COUNT(*) AS new_sample_count
    FROM closest_cluster cc
    JOIN kmeans_cluster_state kcs ON cc.cluster_id = kcs.cluster_id
    GROUP BY cc.cluster_id, kcs.center_x, kcs.center_y, kcs.sample_count
)
UPDATE kmeans_cluster_state kcs
SET
    center_x = uc.new_center_x,
    center_y = uc.new_center_y,
    sample_count = uc.new_sample_count,
    last_updated = NOW()
FROM updated_centers uc
WHERE kcs.cluster_id = uc.cluster_id;
```

**3. DBSCAN聚类：密度聚类优化**

**DBSCAN聚类**：基于密度的聚类算法，可以发现任意形状的簇。

```sql
-- DBSCAN聚类：密度聚类优化（带错误处理和性能测试）
DO $$
DECLARE
    eps NUMERIC := 0.5;  -- 邻域半径
    min_samples INTEGER := 5;  -- 最小样本数
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'clustering_data') THEN
            RAISE WARNING '表 clustering_data 不存在，无法执行DBSCAN聚类';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行DBSCAN聚类，eps=%, min_samples=%', eps, min_samples;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'DBSCAN聚类准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- DBSCAN聚类：基于密度的聚类
WITH point_neighbors AS (
    SELECT
        p1.sample_id AS point_id,
        p1.feature1 AS x1,
        p1.feature2 AS y1,
        COUNT(*) AS neighbor_count,
        ARRAY_AGG(p2.sample_id) AS neighbor_ids
    FROM clustering_data p1
    LEFT JOIN clustering_data p2 ON
        SQRT(POWER(p1.feature1 - p2.feature1, 2) + POWER(p1.feature2 - p2.feature2, 2)) <= 0.5
        AND p1.sample_id != p2.sample_id
    GROUP BY p1.sample_id, p1.feature1, p1.feature2
),
core_points AS (
    SELECT
        point_id,
        x1,
        y1,
        neighbor_count,
        neighbor_ids
    FROM point_neighbors
    WHERE neighbor_count >= 5  -- min_samples
),
dbscan_clusters AS (
    SELECT
        cp1.point_id,
        cp1.x1,
        cp1.y1,
        -- 使用窗口函数分配簇ID（简化版）
        DENSE_RANK() OVER (ORDER BY cp1.point_id) AS cluster_id
    FROM core_points cp1
    WHERE EXISTS (
        SELECT 1
        FROM core_points cp2
        WHERE cp2.point_id = ANY(cp1.neighbor_ids)
    )
)
SELECT
    point_id,
    x1,
    y1,
    cluster_id
FROM dbscan_clusters
ORDER BY cluster_id, point_id;
```

---

## 📚 参考资源

### 学术文献

1. **《数据挖掘：概念与技术》**（Han, J., et al., 2011）- 聚类算法

2. **《机器学习》**（Mitchell, T. M., 1997）- 聚类理论

3. **《统计学习方法》**（李航, 2012）- 聚类方法

### PostgreSQL官方文档

- **聚合函数**: <https://www.postgresql.org/docs/current/functions-aggregate.html>
- **窗口函数**: <https://www.postgresql.org/docs/current/tutorial-window.html>
- **数学函数**: <https://www.postgresql.org/docs/current/functions-math.html>

### 在线资源

- **聚类分析**: <https://en.wikipedia.org/wiki/Cluster_analysis>
- **K-means**: <https://en.wikipedia.org/wiki/K-means_clustering>
- **DBSCAN**: <https://en.wikipedia.org/wiki/DBSCAN>

### 相关算法

- **聚类分析算法**：数据挖掘中的聚类
- **异常检测算法**：基于聚类的异常检测
- **降维算法**：PCA等降维方法
- **特征选择算法**：聚类特征选择

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
**文档状态**: ✅ 已完成
