...
**PostgreSQL 18**引入了多项增强功能，可以显著提升哈希算法的性能：

1. **Skip Scan优化**：
   - 对于包含哈希键的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N哈希查询和哈希分布分析查询

2. **异步I/O增强**：
   - 对于大规模哈希操作，异步I/O可以显著提升性能
   - 适用于批量哈希索引构建和哈希分区操作

3. **并行查询增强**：
   - 哈希操作支持更好的并行执行（已在6节详细说明）
   - 适用于大规模哈希连接和并行哈希聚合

**示例：使用Skip Scan优化哈希索引查询**

```sql
-- 为哈希索引创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_hash_index_skip_scan
ON hash_test_table(email, id DESC);

-- Skip Scan优化查询：查找每个邮箱的最新记录
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
...
对于频繁使用的哈希分布分析结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存哈希分布分析结果
CREATE MATERIALIZED VIEW IF NOT EXISTS hash_distribution_cache AS
WITH hash_distribution_stats AS (
    SELECT
        table_name,
        column_name,
        hash_value,
        COUNT(*) AS bucket_count,
        AVG(row_count) AS avg_rows_per_bucket,
        STDDEV(row_count) AS stddev_rows_per_bucket,
        -- 使用窗口函数计算分布均匀性（避免重复计算）
        CASE
            WHEN AVG(row_count) OVER (PARTITION BY table_name, column_name) > 0 THEN
                STDDEV(row_count) OVER (PARTITION BY table_name, column_name) /
                NULLIF(AVG(row_count) OVER (PARTITION BY table_name, column_name), 0)
            ELSE NULL
        END AS distribution_coefficient
    FROM (
        SELECT
            'hash_test_table' AS table_name,
            'email' AS column_name,
            hashtext(email) % 100 AS hash_value,
            COUNT(*) AS row_count
        FROM hash_test_table
        GROUP BY hashtext(email) % 100
    ) AS bucket_stats
    GROUP BY table_name, column_name, hash_value, row_count
)
SELECT
    table_name,
    column_name,
    hash_value,
    bucket_count,
    ROUND(avg_rows_per_bucket::numeric, 2) AS avg_rows_per_bucket,
    ROUND(stddev_rows_per_bucket::numeric, 2) AS stddev_rows_per_bucket,
    ROUND(distribution_coefficient::numeric, 4) AS distribution_coefficient,
    CASE
        WHEN distribution_coefficient < 0.1 THEN 'Excellent Distribution'
        WHEN distribution_coefficient < 0.3 THEN 'Good Distribution'
        WHEN distribution_coefficient < 0.5 THEN 'Moderate Distribution'
        ELSE 'Poor Distribution'
    END AS distribution_quality
FROM hash_distribution_stats
ORDER BY table_name, column_name, hash_value;

-- 创建索引加速物化视图查询
CREATE INDEX idx_hash_distribution_cache_table_column ON hash_distribution_cache(table_name, column_name);
CREATE INDEX idx_hash_distribution_cache_quality ON hash_distribution_cache(distribution_quality, distribution_coefficient);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY hash_distribution_cache;
```

**2. 实时哈希分析：增量哈希统计更新**

**实时哈希分析**：对于实时数据，使用增量方法更新哈希统计结果。

```sql
-- 实时哈希分析：增量哈希统计更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'hash_analysis_state') THEN
            CREATE TABLE hash_analysis_state (
                table_name VARCHAR(100) NOT NULL,
                column_name VARCHAR(100) NOT NULL,
                hash_bucket INTEGER NOT NULL,
                row_count BIGINT DEFAULT 0,
                last_updated TIMESTAMPTZ DEFAULT NOW(),
                PRIMARY KEY (table_name, column_name, hash_bucket)
            );

            CREATE INDEX idx_hash_analysis_state_table_column ON hash_analysis_state(table_name, column_name);
            CREATE INDEX idx_hash_analysis_state_updated ON hash_analysis_state(last_updated DESC);

            RAISE NOTICE '哈希分析状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量哈希分析更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量哈希分析更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新哈希统计：实时哈希分析
WITH new_hash_data AS (
    SELECT
        'hash_test_table' AS table_name,
        'email' AS column_name,
        hashtext(email) % 100 AS hash_bucket,
        COUNT(*) AS new_row_count
    FROM hash_test_table
    WHERE id > (SELECT COALESCE(MAX(last_updated)::integer, 0) FROM hash_analysis_state WHERE table_name = 'hash_test_table')
    GROUP BY hashtext(email) % 100
),
updated_hash_stats AS (
    SELECT
        COALESCE(has.table_name, nhd.table_name) AS table_name,
        COALESCE(has.column_name, nhd.column_name) AS column_name,
        COALESCE(has.hash_bucket, nhd.hash_bucket) AS hash_bucket,
        COALESCE(has.row_count, 0) + COALESCE(nhd.new_row_count, 0) AS new_row_count
    FROM hash_analysis_state has
    FULL OUTER JOIN new_hash_data nhd ON has.table_name = nhd.table_name
                                        AND has.column_name = nhd.column_name
                                        AND has.hash_bucket = nhd.hash_bucket
)
-- 更新或插入哈希分析状态
INSERT INTO hash_analysis_state (
    table_name,
    column_name,
    hash_bucket,
    row_count,
...
SELECT
    ...
    new_row_count,
    NOW()
FROM updated_hash_stats
ON CONFLICT (table_name, column_name, hash_bucket)
DO UPDATE SET
...
**智能哈希优化**：根据数据特征自动选择最优哈希策略。

```sql
-- 智能哈希优化：自适应哈希策略选择（带错误处理和性能测试）
DO $$
DECLARE
    data_cardinality NUMERIC;
    data_distribution NUMERIC;
    recommended_strategy VARCHAR(50);
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'hash_test_table') THEN
            RAISE WARNING '表 hash_test_table 不存在，无法执行智能哈希优化';
            RETURN;
        END IF;

        -- 计算数据特征
        WITH hash_features AS (
            SELECT
                COUNT(DISTINCT email) AS unique_values,
                COUNT(*) AS total_rows,
                STDDEV(hashtext(email) % 100) / NULLIF(AVG(hashtext(email) % 100), 0) AS distribution_variance
            FROM hash_test_table
        )
        SELECT
            unique_values::numeric / NULLIF(total_rows, 0),
            distribution_variance
        INTO data_cardinality, data_distribution
        FROM hash_features;

        -- 根据数据特征自适应选择哈希策略
        IF data_cardinality > 0.8 AND data_distribution < 0.3 THEN
            recommended_strategy := 'HASH_INDEX';  -- 高基数低分布方差：哈希索引
        ELSIF data_cardinality > 0.5 AND data_distribution < 0.5 THEN
            recommended_strategy := 'HASH_PARTITION';  -- 中基数中分布方差：哈希分区
        ELSE
            recommended_strategy := 'B_TREE_INDEX';  -- 低基数高分布方差：B树索引
        END IF;

        RAISE NOTICE '数据基数: %, 数据分布方差: %, 推荐策略: %',
            data_cardinality, data_distribution, recommended_strategy;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '智能哈希优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 智能哈希优化：根据策略选择不同的哈希方法
WITH hash_characteristics AS (
    SELECT
        COUNT(DISTINCT email)::numeric / NULLIF(COUNT(*), 0) AS cardinality_ratio,
        STDDEV(hashtext(email) % 100) / NULLIF(AVG(hashtext(email) % 100), 0) AS distribution_variance
    FROM hash_test_table
),
adaptive_hash_strategy AS (
    SELECT
        cardinality_ratio,
        distribution_variance,
        CASE
            WHEN cardinality_ratio > 0.8 AND distribution_variance < 0.3 THEN 'HASH_INDEX'
            WHEN cardinality_ratio > 0.5 AND distribution_variance < 0.5 THEN 'HASH_PARTITION'
            ELSE 'B_TREE_INDEX'
        END AS recommended_strategy,
        -- 使用窗口函数计算哈希效率（避免重复计算）
        CASE
            WHEN cardinality_ratio > 0.8 AND distribution_variance < 0.3 THEN
                COUNT(*) FILTER (WHERE hashtext(email) % 100 = hashtext(email) % 100)::numeric / COUNT(*)
            ELSE NULL
        END AS hash_efficiency
    FROM hash_test_table
    CROSS JOIN hash_characteristics
    GROUP BY cardinality_ratio, distribution_variance
)
SELECT
    ROUND(cardinality_ratio::numeric, 4) AS cardinality_ratio,
    ROUND(distribution_variance::numeric, 4) AS distribution_variance,
    recommended_strategy,
    ROUND(COALESCE(hash_efficiency, 0)::numeric, 4) AS hash_efficiency,
    CASE
        WHEN recommended_strategy = 'HASH_INDEX' THEN 'Create hash index for high-cardinality columns'
        WHEN recommended_strategy = 'HASH_PARTITION' THEN 'Use hash partitioning for moderate-cardinality columns'
        ELSE 'Use B-tree index for low-cardinality columns'
    END AS optimization_advice
FROM adaptive_hash_strategy
LIMIT 1;
```
