...
- [PostgreSQL 数据库排序算法完整指南](#postgresql-数据库排序算法完整指南)
  ...
  - [5. PostgreSQL 18 并行排序增强](#5-postgresql-18-并行排序增强)
    - [5.1 并行排序原理](#51-并行排序原理)
    - [5.2 并行索引排序](#52-并行索引排序)
    - [5.3 并行内存排序](#53-并行内存排序)
    - [5.4 并行外部排序](#54-并行外部排序)
  - [6. 实际应用案例](#6-实际应用案例)
...
## 5. PostgreSQL 18 并行排序增强

**PostgreSQL 18** 显著增强了并行排序能力，支持并行执行索引排序、内存排序和外部排序，大幅提升大规模数据排序的性能。

### 5.1 并行排序原理

PostgreSQL 18 的并行排序通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描数据
2. **并行排序**：每个工作进程独立执行排序
3. **并行合并**：并行合并排序结果
4. **结果合并**：主进程合并所有工作进程的排序结果

### 5.2 并行索引排序

```sql
-- PostgreSQL 18 并行索引排序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行索引排序';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行索引排序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行索引排序准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- PostgreSQL 18 并行内存排序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行内存排序';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行内存排序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行内存排序准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- PostgreSQL 18 并行外部排序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行外部排序';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行外部排序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行外部排序准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;
SET work_mem = '64MB';  -- 较小的work_mem触发外部排序
...
SELECT
    ...
    created_at
FROM large_table
ORDER BY value DESC, category, created_at;
```
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升排序算法的性能：

1. **Skip Scan优化**：
   - 对于包含排序列的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N排序查询和分页排序查询

2. **异步I/O增强**：
   - 对于大规模排序操作，异步I/O可以显著提升性能
   - 适用于批量外部排序和并行排序

3. **并行查询增强**：
   - 排序操作支持更好的并行执行（已在5节详细说明）
   - 适用于大规模数据排序和并行排序分析

**示例：使用Skip Scan优化排序查询**

```sql
-- 为排序操作创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_sort_optimization_skip_scan
ON users(created_at DESC, user_id DESC);

-- Skip Scan优化查询：查找最新创建的10个用户
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT
    user_id,
    username,
...
对于频繁使用的排序结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存排序结果
CREATE MATERIALIZED VIEW IF NOT EXISTS sort_result_cache AS
WITH sorted_statistics AS (
    SELECT
        user_id,
        username,
        email,
        created_at,
        -- 使用窗口函数计算排名（避免重复计算）
        ROW_NUMBER() OVER (ORDER BY created_at DESC, user_id DESC) AS overall_rank,
        ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC('month', created_at) ORDER BY created_at DESC, user_id DESC) AS monthly_rank,
        -- 使用窗口函数计算排序得分（避免重复计算）
        EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 86400 AS days_since_creation,
        CASE
            WHEN EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 86400 <= 7 THEN 100
            WHEN EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 86400 <= 30 THEN 80
            WHEN EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 86400 <= 90 THEN 60
            ELSE 40
...
SELECT
    ...
    overall_rank,
    monthly_rank,
    ROUND(days_since_creation::numeric, 2) AS days_since_creation,
    recency_score,
    CASE
        WHEN overall_rank <= 100 THEN 'Top 100'
        WHEN overall_rank <= 1000 THEN 'Top 1000'
        WHEN monthly_rank <= 100 THEN 'Top 100 Monthly'
        ELSE 'Regular'
    END AS user_tier
FROM sorted_statistics
ORDER BY created_at DESC, user_id DESC;

-- 创建索引加速物化视图查询
CREATE INDEX idx_sort_result_cache_created ON sort_result_cache(created_at DESC, user_id DESC);
CREATE INDEX idx_sort_result_cache_tier ON sort_result_cache(user_tier, overall_rank);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY sort_result_cache;
```

**2. 实时排序优化：增量排序统计更新**

**实时排序优化**：对于实时数据，使用增量方法更新排序统计结果。

```sql
-- 实时排序优化：增量排序统计更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sort_optimization_state') THEN
            CREATE TABLE sort_optimization_state (
                table_name VARCHAR(100) NOT NULL,
                sort_column VARCHAR(100) NOT NULL,
                sort_direction VARCHAR(10) NOT NULL,  -- 'ASC', 'DESC'
                total_rows BIGINT DEFAULT 0,
                indexed_rows BIGINT DEFAULT 0,
                sort_efficiency NUMERIC,
                last_sorted_at TIMESTAMPTZ,
                last_updated TIMESTAMPTZ DEFAULT NOW(),
                PRIMARY KEY (table_name, sort_column, sort_direction)
            );

            CREATE INDEX idx_sort_optimization_state_table ON sort_optimization_state(table_name, last_updated DESC);
            CREATE INDEX idx_sort_optimization_state_updated ON sort_optimization_state(last_updated DESC);

            RAISE NOTICE '排序优化状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量排序优化更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量排序优化更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新排序统计：实时排序优化
WITH new_sort_data AS (
    SELECT
        'users' AS table_name,
        'created_at' AS sort_column,
    ...
    FROM users
    WHERE created_at > (SELECT COALESCE(MAX(last_sorted_at), '1970-01-01') FROM sort_optimization_state
                       WHERE table_name = 'users' AND sort_column = 'created_at' AND sort_direction = 'DESC')
      AND created_at <= CURRENT_TIMESTAMP
    GROUP BY 'users', 'created_at', 'DESC'
),
updated_sort_stats AS (
    SELECT
        COALESCE(sos.table_name, nsd.table_name) AS table_name,
        COALESCE(sos.sort_column, nsd.sort_column) AS sort_column,
        COALESCE(sos.sort_direction, nsd.sort_direction) AS sort_direction,
        COALESCE(sos.total_rows, 0) + COALESCE(nsd.new_total_rows, 0) AS new_total_rows,
        COALESCE(sos.indexed_rows, 0) + COALESCE(nsd.new_indexed_rows, 0) AS new_indexed_rows,
        (COALESCE(sos.indexed_rows, 0) + COALESCE(nsd.new_indexed_rows, 0))::numeric /
        NULLIF(COALESCE(sos.total_rows, 0) + COALESCE(nsd.new_total_rows, 0), 0) AS new_sort_efficiency
    FROM sort_optimization_state sos
    FULL OUTER JOIN new_sort_data nsd ON sos.table_name = nsd.table_name
                                       AND sos.sort_column = nsd.sort_column
                                       AND sos.sort_direction = nsd.sort_direction
)
-- 更新或插入排序优化状态
INSERT INTO sort_optimization_state (
    table_name,
    sort_column,
    sort_direction,
    total_rows,
...
SELECT
    ...
    new_indexed_rows,
    new_sort_efficiency,
    NOW() AS last_sorted_at,
    NOW()
FROM updated_sort_stats
ON CONFLICT (table_name, sort_column, sort_direction)
DO UPDATE SET
    total_rows = EXCLUDED.total_rows,
    indexed_rows = EXCLUDED.indexed_rows,
    sort_efficiency = EXCLUDED.sort_efficiency,
...
**智能排序优化**：根据数据特征自动选择最优排序策略。

```sql
-- 智能排序优化：自适应排序策略选择（带错误处理和性能测试）
DO $$
DECLARE
    table_size BIGINT;
    has_index_on_sort_column BOOLEAN;
    data_sortedness NUMERIC;
    recommended_strategy VARCHAR(50);
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            RAISE WARNING '表 users 不存在，无法执行智能排序优化';
            RETURN;
        END IF;
        ...
        SELECT
            ...
            CASE
                WHEN COUNT(*) > 0 THEN
                    (COUNT(*) FILTER (WHERE created_at = LAG(created_at) OVER (ORDER BY created_at))::numeric / COUNT(*))
                ELSE 0
            END
        INTO table_size, has_index_on_sort_column, data_sortedness
        FROM users;

        -- 根据数据特征自适应选择排序策略
        IF has_index_on_sort_column AND table_size < 1000000 THEN
            recommended_strategy := 'INDEX_SORT';  -- 有索引：索引排序
        ELSIF table_size < 100000 AND data_sortedness > 0.8 THEN
            recommended_strategy := 'QUICKSORT';  -- 小表已排序：快速排序
        ELSIF table_size < 100000 THEN
            recommended_strategy := 'HEAPSORT';  -- 小表未排序：堆排序
        ELSE
            recommended_strategy := 'EXTERNAL_SORT';  -- 大表：外部排序
        END IF;

        RAISE NOTICE '表大小: %, 有索引: %, 数据有序度: %, 推荐策略: %',
            table_size, has_index_on_sort_column, data_sortedness, recommended_strategy;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '智能排序优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 智能排序优化：根据策略选择不同的排序方法
WITH table_characteristics AS (
    SELECT
        'users' AS table_name,
        'created_at' AS sort_column,
        COUNT(*) AS table_size,
        ...
        CASE
            WHEN COUNT(*) > 0 THEN
                (COUNT(*) FILTER (WHERE created_at = LAG(created_at) OVER (ORDER BY created_at))::numeric / COUNT(*))
            ELSE 0
        END AS sortedness_ratio
    FROM users
),
adaptive_sort_strategy AS (
    SELECT
        table_name,
        sort_column,
        table_size,
        has_index,
        ROUND(sortedness_ratio::numeric, 4) AS sortedness_ratio,
        CASE
            WHEN has_index AND table_size < 1000000 THEN 'INDEX_SORT'
            WHEN table_size < 100000 AND sortedness_ratio > 0.8 THEN 'QUICKSORT'
            WHEN table_size < 100000 THEN 'HEAPSORT'
            ELSE 'EXTERNAL_SORT'
        END AS recommended_strategy,
        -- 使用窗口函数计算排序效率（避免重复计算）
        CASE
            WHEN has_index AND table_size < 1000000 THEN
                table_size  -- 索引排序：O(n)
            WHEN table_size < 100000 AND sortedness_ratio > 0.8 THEN
                table_size * LOG(table_size) * 0.5  -- 快速排序（已排序）：O(n log n) * 0.5
            WHEN table_size < 100000 THEN
                table_size * LOG(table_size)  -- 堆排序：O(n log n)
            ELSE
                table_size * LOG(table_size) * 1.5  -- 外部排序：O(n log n) * 1.5
        END AS estimated_complexity
...
SELECT
    ...
    CASE
        WHEN recommended_strategy = 'INDEX_SORT' THEN 'Create index on sort column for optimal performance'
        WHEN recommended_strategy = 'QUICKSORT' THEN 'Use quicksort for small sorted tables'
        WHEN recommended_strategy = 'HEAPSORT' THEN 'Use heapsort for small unsorted tables, ensure sufficient work_mem'
        ELSE 'Use external sort for large tables, ensure sufficient disk space and work_mem'
    END AS optimization_advice
FROM adaptive_sort_strategy;
```
