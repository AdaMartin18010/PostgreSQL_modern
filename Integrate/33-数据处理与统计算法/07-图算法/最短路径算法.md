# PostgreSQL 最短路径算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 最短路径 | Dijkstra算法
> **难度级别**: ⭐⭐⭐⭐⭐ (专家级)

---

## 📋 目录

- [PostgreSQL 最短路径算法完整指南](#postgresql-最短路径算法完整指南)
  - [📋 目录](#-目录)
  - [最短路径算法概述](#最短路径算法概述)
    - [理论基础](#理论基础)
      - [最短路径问题定义](#最短路径问题定义)
      - [最短路径算法分类](#最短路径算法分类)
      - [算法选择策略](#算法选择策略)
    - [核心算法](#核心算法)
  - [1. Dijkstra算法](#1-dijkstra算法)
    - [1.1 Dijkstra算法原理](#11-dijkstra算法原理)
      - [Dijkstra算法定义](#dijkstra算法定义)
      - [算法步骤](#算法步骤)
    - [1.2 递归实现](#12-递归实现)
    - [1.3 优化实现](#13-优化实现)
  - [2. Floyd-Warshall算法](#2-floyd-warshall算法)
    - [2.1 Floyd-Warshall算法原理](#21-floyd-warshall算法原理)
      - [算法定义](#算法定义)
      - [动态规划](#动态规划)
    - [2.2 全对最短路径实现](#22-全对最短路径实现)
    - [2.3 路径重建](#23-路径重建)
  - [3. 路径权重计算](#3-路径权重计算)
    - [3.1 路径权重计算原理](#31-路径权重计算原理)
      - [权重定义](#权重定义)
      - [权重类型](#权重类型)
    - [3.2 加权路径查找实现](#32-加权路径查找实现)
    - [3.3 多目标路径优化](#33-多目标路径优化)
  - [4. 最短路径优化策略](#4-最短路径优化策略)
    - [4.1 索引优化](#41-索引优化)
    - [4.2 算法优化](#42-算法优化)
    - [4.3 性能优化](#43-性能优化)
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 路由规划](#51-路由规划)
    - [5.2 网络分析](#52-网络分析)
    - [5.3 物流优化](#53-物流优化)
    - [5.4 社交网络最短路径](#54-社交网络最短路径)
  - [6. 算法性能对比与优化](#6-算法性能对比与优化)
    - [6.1 最短路径算法对比](#61-最短路径算法对比)
    - [6.2 性能优化建议](#62-性能优化建议)
    - [6.3 常见问题与解决方案](#63-常见问题与解决方案)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 算法选择](#71-算法选择)
    - [7.2 数据结构设计](#72-数据结构设计)
    - [7.3 性能优化](#73-性能优化)
    - [7.4 SQL实现注意事项](#74-sql实现注意事项)
  - [📚 参考资源](#-参考资源)
    - [学术文献](#学术文献)
    - [PostgreSQL官方文档](#postgresql官方文档)
    - [在线资源](#在线资源)
    - [相关算法](#相关算法)

---

## 最短路径算法概述

**最短路径算法（Shortest Path Algorithm）**用于查找图中两个节点之间的最短路径，是图论中的经典问题，广泛应用于路径规划、网络分析、物流优化等领域。

### 理论基础

#### 最短路径问题定义

**最短路径问题**：给定加权图$G = (V, E, w)$，其中$w: E \to \mathbb{R}$是权重函数，找到从节点$s$到节点$t$的路径$P$，使得：
$$w(P) = \sum_{(u,v) \in P} w(u,v)$$
最小。

**最短路径**：$\text{shortest\_path}(s, t) = \arg\min_{P} w(P)$

#### 最短路径算法分类

最短路径算法分类：

1. **单源最短路径**：
   - **Dijkstra算法**：非负权重图
   - **Bellman-Ford算法**：允许负权重
   - 时间复杂度：$O(V^2)$或$O(E \log V)$

2. **全对最短路径**：
   - **Floyd-Warshall算法**：所有节点对
   - 时间复杂度：$O(V^3)$

3. **启发式搜索**：
   - **A*算法**：使用启发式函数
   - 时间复杂度：$O(V + E)$（最优情况）

#### 算法选择策略

选择算法的考虑因素：

1. **图类型**：有向图、无向图
2. **权重类型**：非负权重、允许负权重
3. **查询类型**：单源查询、全对查询
4. **性能要求**：实时查询、批量查询

### 核心算法

| 算法 | 数学定义 | 用途 | 时间复杂度 | 空间复杂度 | 适用场景 |
|------|---------|------|-----------|-----------|---------|
| **Dijkstra** | $\min_{P} \sum_{(u,v) \in P} w(u,v)$ | 单源最短路径 | $O(V^2)$ | $O(V)$ | 非负权重图 |
| **Floyd-Warshall** | $\min_{k} d[i][k] + d[k][j]$ | 全对最短路径 | $O(V^3)$ | $O(V^2)$ | 小规模图 |
| **A*** | $f(n) = g(n) + h(n)$ | 启发式搜索 | $O(V + E)$ | $O(V)$ | 有启发式函数 |

---

## 1. Dijkstra算法

### 1.1 Dijkstra算法原理

**Dijkstra算法（Dijkstra's Algorithm）**是解决单源最短路径问题的经典算法。

#### Dijkstra算法定义

**Dijkstra算法**：给定起始节点$s$，维护距离集合$d[v]$表示从$s$到$v$的最短距离，逐步扩展找到最短路径。

**算法步骤**：

1. 初始化：$d[s] = 0$，$d[v] = \infty$（$v \neq s$）
2. 选择未访问节点$u$，使得$d[u]$最小
3. 标记$u$为已访问
4. 更新相邻节点：$d[v] = \min(d[v], d[u] + w(u,v))$
5. 重复步骤2-4直到所有节点访问

#### 算法步骤

**Dijkstra算法伪代码**：

```
Dijkstra(G, s):
    d[s] = 0
    for each v in V:
        d[v] = infinity
    S = empty set
    while S != V:
        u = node with minimum d[u] not in S
        S = S union {u}
        for each neighbor v of u:
            if d[v] > d[u] + w(u,v):
                d[v] = d[u] + w(u,v)
```

### 1.2 递归实现

**递归Dijkstra**的实现和使用。

```sql
-- Dijkstra算法（带错误处理和性能测试）
DO $$
DECLARE
    start_node_id INTEGER := 1;
    end_node_id INTEGER := 10;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nodes') THEN
            RAISE WARNING '表 nodes 不存在，无法执行Dijkstra算法';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行Dijkstra算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行Dijkstra算法，从节点 % 到节点 %', start_node_id, end_node_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Dijkstra算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

WITH RECURSIVE dijkstra AS (
    SELECT
        start_node_id AS current_node,
        0 AS total_distance,
        ARRAY[start_node_id] AS path
    UNION ALL
    SELECT
        e.target_id,
        d.total_distance + e.weight,
        d.path || e.target_id
    FROM dijkstra d
    JOIN edges e ON d.current_node = e.source_id
    WHERE e.target_id != ALL(d.path)
      AND d.total_distance + e.weight < (
          SELECT COALESCE(MIN(total_distance), 999999)
          FROM dijkstra
          WHERE current_node = e.target_id
      )
)
SELECT
    current_node AS target_node,
    MIN(total_distance) AS shortest_distance,
    (SELECT path FROM dijkstra
     WHERE current_node = d.current_node
       AND total_distance = MIN(d.total_distance)
     LIMIT 1) AS shortest_path
FROM dijkstra d
WHERE current_node = end_node_id
GROUP BY current_node;
```

---

### 1.3 优化实现

**优化Dijkstra**：使用更高效的数据结构（PostgreSQL中通过优化查询实现）。

```sql
-- 优化Dijkstra：使用索引和限制条件（带错误处理和性能测试）
DO $$
DECLARE
    start_node_id INTEGER := 1;
    max_distance NUMERIC := 1000;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nodes') THEN
            RAISE WARNING '表 nodes 不存在，无法执行优化Dijkstra';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行优化Dijkstra';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行优化Dijkstra，最大距离限制: %', max_distance;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '优化Dijkstra准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 优化Dijkstra：添加距离限制和早期终止
WITH RECURSIVE dijkstra_optimized AS (
    SELECT
        start_node_id AS current_node,
        0 AS total_distance,
        ARRAY[start_node_id] AS path
    UNION ALL
    SELECT
        e.target_id,
        d.total_distance + e.weight,
        d.path || e.target_id
    FROM dijkstra_optimized d
    JOIN edges e ON d.current_node = e.source_id
    WHERE e.target_id != ALL(d.path)
      AND d.total_distance + e.weight < max_distance
      AND d.total_distance + e.weight < (
          SELECT COALESCE(MIN(total_distance), 999999)
          FROM dijkstra_optimized
          WHERE current_node = e.target_id
      )
)
SELECT DISTINCT ON (current_node)
    current_node,
    total_distance,
    path
FROM dijkstra_optimized
ORDER BY current_node, total_distance;
```

---

## 2. Floyd-Warshall算法

### 2.1 Floyd-Warshall算法原理

**Floyd-Warshall算法（Floyd-Warshall Algorithm）**是解决全对最短路径问题的动态规划算法。

#### 算法定义

**Floyd-Warshall算法**：使用动态规划计算所有节点对之间的最短路径。

**动态规划状态**：$d[i][j][k]$表示从节点$i$到节点$j$，只使用节点$\{1, 2, \ldots, k\}$作为中间节点的最短距离。

**状态转移方程**：
$$d[i][j][k] = \min(d[i][j][k-1], d[i][k][k-1] + d[k][j][k-1])$$

#### 动态规划

**Floyd-Warshall算法伪代码**：

```
Floyd-Warshall(G):
    for k = 1 to V:
        for i = 1 to V:
            for j = 1 to V:
                d[i][j] = min(d[i][j], d[i][k] + d[k][j])
```

### 2.2 全对最短路径实现

**全对最短路径**的实现和使用。

```sql
-- Floyd-Warshall算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行Floyd-Warshall算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行Floyd-Warshall算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Floyd-Warshall算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

WITH RECURSIVE floyd_warshall AS (
    -- 初始化：直接连接的边
    SELECT
        source_id AS i,
        target_id AS j,
        weight AS distance
    FROM edges
    UNION
    -- 自身距离为0
    SELECT DISTINCT
        id AS i,
        id AS j,
        0 AS distance
    FROM nodes
    UNION ALL
    -- 递归：通过中间节点k更新距离
    SELECT
        fw1.i,
        fw2.j,
        LEAST(fw1.distance, fw1.distance + fw2.distance)
    FROM floyd_warshall fw1
    JOIN floyd_warshall fw2 ON fw1.j = fw2.i
    WHERE fw1.distance + fw2.distance < fw1.distance
)
SELECT
    i AS from_node,
    j AS to_node,
    MIN(distance) AS shortest_distance
FROM floyd_warshall
GROUP BY i, j
ORDER BY i, j;
```

---

### 2.3 路径重建

**路径重建**：重建Floyd-Warshall算法找到的最短路径。

```sql
-- 路径重建（带错误处理）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '路径重建需要存储中间节点信息';
        -- 实际实现需要修改算法以存储路径信息
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '路径重建准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 3. 路径权重计算

### 3.1 路径权重计算原理

**路径权重计算（Path Weight Calculation）**是计算带权重路径的总权重。

#### 权重定义

**路径权重**：路径$P = \{v_1, v_2, \ldots, v_k\}$的权重为：
$$w(P) = \sum_{i=1}^{k-1} w(v_i, v_{i+1})$$

#### 权重类型

**权重类型**：

1. **距离权重**：物理距离
2. **时间权重**：时间成本
3. **成本权重**：经济成本
4. **组合权重**：多种权重的组合

### 3.2 加权路径查找实现

**加权路径查找**的实现和使用。

```sql
-- 加权路径查找（带错误处理和性能测试）
DO $$
DECLARE
    start_node_id INTEGER := 1;
    end_node_id INTEGER := 10;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行加权路径查找';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行加权路径查找';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '加权路径查找准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

WITH RECURSIVE weighted_paths AS (
    SELECT
        start_node_id AS current_node,
        0 AS total_weight,
        ARRAY[start_node_id] AS path
    UNION ALL
    SELECT
        e.target_id,
        wp.total_weight + e.weight,
        wp.path || e.target_id
    FROM weighted_paths wp
    JOIN edges e ON wp.current_node = e.source_id
    WHERE e.target_id != ALL(wp.path)
      AND wp.total_weight + e.weight < (
          SELECT COALESCE(MIN(total_weight), 999999)
          FROM weighted_paths
          WHERE current_node = e.target_id
      )
)
SELECT
    current_node,
    MIN(total_weight) AS min_weight,
    (SELECT path FROM weighted_paths
     WHERE current_node = wp.current_node
       AND total_weight = MIN(wp.total_weight)
     LIMIT 1) AS shortest_path
FROM weighted_paths wp
WHERE current_node = end_node_id
GROUP BY current_node;
```

---

### 3.3 多目标路径优化

**多目标路径优化**：考虑多个目标的路径优化。

```sql
-- 多目标路径优化（带错误处理和性能测试）
DO $$
DECLARE
    start_node_id INTEGER := 1;
    end_node_id INTEGER := 10;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行多目标路径优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行多目标路径优化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多目标路径优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 多目标路径优化：同时考虑距离和时间
WITH RECURSIVE multi_objective_paths AS (
    SELECT
        start_node_id AS current_node,
        0 AS total_distance,
        0 AS total_time,
        ARRAY[start_node_id] AS path
    UNION ALL
    SELECT
        e.target_id,
        mop.total_distance + e.distance,
        mop.total_time + e.time_cost,
        mop.path || e.target_id
    FROM multi_objective_paths mop
    JOIN edges e ON mop.current_node = e.source_id
    WHERE e.target_id != ALL(mop.path)
      AND mop.total_distance + e.distance < 1000  -- 距离限制
      AND mop.total_time + e.time_cost < 120  -- 时间限制（分钟）
)
SELECT DISTINCT ON (current_node)
    current_node,
    total_distance,
    total_time,
    total_distance + total_time * 10 AS combined_score,  -- 组合评分
    path
FROM multi_objective_paths
WHERE current_node = end_node_id
ORDER BY current_node, combined_score;
```

---

## 4. 最短路径优化策略

### 4.1 索引优化

**索引优化**：为最短路径查询创建合适的索引。

```sql
-- 索引优化（带错误处理）
DO $$
BEGIN
    BEGIN
        -- 边表索引：加速邻接节点查找
        CREATE INDEX IF NOT EXISTS idx_edges_source_target_weight
        ON edges(source_id, target_id, weight);

        CREATE INDEX IF NOT EXISTS idx_edges_target_source_weight
        ON edges(target_id, source_id, weight);

        RAISE NOTICE '最短路径索引创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '索引创建失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 4.2 算法优化

**算法优化**：优化算法实现提高性能。

```sql
-- 算法优化：使用物化视图缓存中间结果
CREATE MATERIALIZED VIEW IF NOT EXISTS shortest_paths_cache AS
WITH RECURSIVE dijkstra_cache AS (
    -- 预计算常用路径（示例）
    SELECT
        source_id,
        target_id,
        weight AS distance,
        ARRAY[source_id, target_id] AS path
    FROM edges
    UNION ALL
    SELECT
        spc.source_id,
        e.target_id,
        spc.distance + e.weight,
        spc.path || e.target_id
    FROM shortest_paths_cache spc
    JOIN edges e ON spc.target_id = e.source_id
    WHERE array_length(spc.path, 1) < 5  -- 限制路径长度
)
SELECT DISTINCT ON (source_id, target_id)
    source_id,
    target_id,
    distance,
    path
FROM dijkstra_cache
ORDER BY source_id, target_id, distance;

CREATE INDEX IF NOT EXISTS idx_shortest_paths_cache_source_target
ON shortest_paths_cache(source_id, target_id);
```

### 4.3 性能优化

**性能优化**：使用并行查询和批量处理。

```sql
-- 性能优化（带错误处理）
DO $$
BEGIN
    BEGIN
        SET max_parallel_workers_per_gather = 4;
        SET work_mem = '512MB';
        RAISE NOTICE '性能优化配置已设置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能优化失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 5. PostgreSQL 18 并行最短路径计算增强

**PostgreSQL 18** 显著增强了并行最短路径计算能力，支持并行执行Dijkstra算法、Floyd-Warshall算法和路径权重计算，大幅提升大规模图最短路径计算的性能。

### 5.1 并行最短路径计算原理

PostgreSQL 18 的并行最短路径计算通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描图数据
2. **并行路径探索**：每个工作进程独立探索部分路径
3. **并行权重计算**：并行执行路径权重计算
4. **结果合并**：主进程合并所有工作进程的计算结果

### 5.2 并行Dijkstra算法

```sql
-- PostgreSQL 18 并行Dijkstra算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行Dijkstra算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Dijkstra算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Dijkstra算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行Dijkstra算法：单源最短路径
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE dijkstra_paths AS (
    SELECT
        1 AS start_node,
        start_node AS current_node,
        0 AS distance,
        ARRAY[start_node] AS path
    UNION ALL
    SELECT
        dp.start_node,
        e.target_id,
        dp.distance + e.weight,
        dp.path || e.target_id
    FROM dijkstra_paths dp
    JOIN edges e ON dp.current_node = e.source_id
    WHERE e.target_id != ALL(dp.path)
      AND dp.distance + e.weight < (
          SELECT COALESCE(MIN(distance), 999999)
          FROM dijkstra_paths dp2
          WHERE dp2.current_node = e.target_id
      )
)
SELECT DISTINCT ON (current_node)
    current_node,
    MIN(distance) AS shortest_distance,
    (SELECT path FROM dijkstra_paths dp3
     WHERE dp3.current_node = dp.current_node
       AND dp3.distance = MIN(dp.distance)
     LIMIT 1) AS shortest_path
FROM dijkstra_paths dp
GROUP BY current_node
ORDER BY current_node;
```

### 5.3 并行Floyd-Warshall算法

```sql
-- PostgreSQL 18 并行Floyd-Warshall算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行Floyd-Warshall算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Floyd-Warshall算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Floyd-Warshall算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行Floyd-Warshall算法：全对最短路径
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH all_pairs AS (
    SELECT
        source_id AS from_node,
        target_id AS to_node,
        weight AS distance
    FROM edges
    UNION
    SELECT
        node_id AS from_node,
        node_id AS to_node,
        0 AS distance
    FROM (SELECT DISTINCT source_id AS node_id FROM edges
          UNION
          SELECT DISTINCT target_id FROM edges) nodes
),
floyd_warshall AS (
    SELECT
        from_node,
        to_node,
        distance,
        CASE
            WHEN from_node = to_node THEN 0
            ELSE distance
        END AS shortest_distance
    FROM all_pairs
)
SELECT
    from_node,
    to_node,
    MIN(shortest_distance) AS shortest_path_distance
FROM floyd_warshall
GROUP BY from_node, to_node
ORDER BY from_node, to_node
LIMIT 100;
```

### 5.4 并行路径权重计算

```sql
-- PostgreSQL 18 并行路径权重计算（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行路径权重计算';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行路径权重计算';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行路径权重计算准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行路径权重计算：加权路径查找
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE weighted_paths AS (
    SELECT
        1 AS start_node,
        start_node AS current_node,
        0 AS total_weight,
        ARRAY[start_node] AS path
    UNION ALL
    SELECT
        wp.start_node,
        e.target_id,
        wp.total_weight + e.weight,
        wp.path || e.target_id
    FROM weighted_paths wp
    JOIN edges e ON wp.current_node = e.source_id
    WHERE e.target_id != ALL(wp.path)
      AND wp.total_weight + e.weight < 1000  -- 权重限制
)
SELECT DISTINCT ON (current_node)
    current_node,
    MIN(total_weight) AS min_weight,
    (SELECT path FROM weighted_paths wp2
     WHERE wp2.current_node = wp.current_node
       AND wp2.total_weight = MIN(wp.total_weight)
     LIMIT 1) AS optimal_path
FROM weighted_paths wp
GROUP BY current_node
ORDER BY current_node;
```

---

## 6. 实际应用案例

### 5.1 路由规划

**路由规划**规划最优路径。

```sql
-- 路由规划（带错误处理和性能测试）
DO $$
DECLARE
    origin_id INTEGER := 1;
    destination_id INTEGER := 20;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'routes') THEN
            RAISE WARNING '表 routes 不存在，无法执行路由规划';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行路由规划，从节点 % 到节点 %', origin_id, destination_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '路由规划准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 路由规划：使用Dijkstra算法规划最优路径
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE route_planning AS (
    SELECT
        origin_id AS current_location,
        0 AS total_time,
        ARRAY[origin_id] AS route
    UNION ALL
    SELECT
        r.to_location,
        rp.total_time + r.travel_time,
        rp.route || r.to_location
    FROM route_planning rp
    JOIN routes r ON rp.current_location = r.from_location
    WHERE r.to_location != ALL(rp.route)
      AND rp.total_time + r.travel_time < (
          SELECT COALESCE(MIN(total_time), 999999)
          FROM route_planning
          WHERE current_location = r.to_location
      )
)
SELECT DISTINCT ON (current_location)
    current_location,
    total_time AS shortest_time,
    route AS optimal_route
FROM route_planning
WHERE current_location = destination_id
ORDER BY current_location, total_time;
```

### 5.2 网络分析

**网络分析**：分析网络中的最短路径。

```sql
-- 网络分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'network_nodes') THEN
            RAISE WARNING '表 network_nodes 不存在，无法执行网络分析';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'network_links') THEN
            RAISE WARNING '表 network_links 不存在，无法执行网络分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行网络分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '网络分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 网络分析：计算所有节点对之间的最短路径（Floyd-Warshall简化版）
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE floyd_warshall AS (
    SELECT
        source_id AS i,
        target_id AS j,
        weight AS distance
    FROM network_links
    UNION
    SELECT DISTINCT
        id AS i,
        id AS j,
        0 AS distance
    FROM network_nodes
    UNION ALL
    SELECT
        fw1.i,
        fw2.j,
        LEAST(fw1.distance, fw1.distance + fw2.distance)
    FROM floyd_warshall fw1
    JOIN floyd_warshall fw2 ON fw1.j = fw2.i
    WHERE fw1.distance + fw2.distance < fw1.distance
)
SELECT DISTINCT ON (i, j)
    i AS from_node,
    j AS to_node,
    distance AS shortest_distance
FROM floyd_warshall
ORDER BY i, j, distance;
```

### 5.3 物流优化

**物流优化**：优化物流配送路径。

```sql
-- 物流优化（带错误处理和性能测试）
DO $$
DECLARE
    warehouse_id INTEGER := 1;
    customer_id INTEGER := 50;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'delivery_routes') THEN
            RAISE WARNING '表 delivery_routes 不存在，无法执行物流优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行物流优化，从仓库 % 到客户 %', warehouse_id, customer_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '物流优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 物流优化：考虑距离、时间和成本的路径优化
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE delivery_optimization AS (
    SELECT
        warehouse_id AS current_location,
        0 AS total_distance,
        0 AS total_time,
        0 AS total_cost,
        ARRAY[warehouse_id] AS route
    UNION ALL
    SELECT
        dr.to_location,
        do_opt.total_distance + dr.distance,
        do_opt.total_time + dr.travel_time,
        do_opt.total_cost + dr.cost,
        do_opt.route || dr.to_location
    FROM delivery_optimization do_opt
    JOIN delivery_routes dr ON do_opt.current_location = dr.from_location
    WHERE dr.to_location != ALL(do_opt.route)
      AND do_opt.total_distance + dr.distance < 500  -- 距离限制（公里）
      AND do_opt.total_time + dr.travel_time < 480  -- 时间限制（分钟）
)
SELECT DISTINCT ON (current_location)
    current_location,
    total_distance,
    total_time,
    total_cost,
    total_distance * 0.5 + total_time * 0.3 + total_cost AS optimization_score,
    route
FROM delivery_optimization
WHERE current_location = customer_id
ORDER BY current_location, optimization_score;
```

### 5.4 社交网络最短路径

**社交网络最短路径**：计算社交网络中用户之间的最短路径。

```sql
-- 社交网络最短路径（带错误处理和性能测试）
DO $$
DECLARE
    user1_id INTEGER := 1;
    user2_id INTEGER := 100;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'social_connections') THEN
            RAISE WARNING '表 social_connections 不存在，无法执行社交网络最短路径';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行社交网络最短路径，从用户 % 到用户 %', user1_id, user2_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '社交网络最短路径准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 社交网络最短路径：计算两个用户之间的最短连接路径（BFS）
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE social_paths AS (
    SELECT
        user1_id AS current_user,
        ARRAY[user1_id] AS path,
        0 AS degrees_of_separation
    UNION ALL
    SELECT
        sc.user2_id,
        sp.path || sc.user2_id,
        sp.degrees_of_separation + 1
    FROM social_paths sp
    JOIN social_connections sc ON sp.current_user = sc.user1_id
    WHERE sc.user2_id != ALL(sp.path)
      AND sp.degrees_of_separation < 6  -- 6度分隔理论
)
SELECT DISTINCT ON (current_user)
    current_user,
    degrees_of_separation,
    path
FROM social_paths
WHERE current_user = user2_id
ORDER BY current_user, degrees_of_separation;
```

---

## 7. 算法性能对比与优化

### 6.1 最短路径算法对比

| 算法 | 数学定义 | 时间复杂度 | 空间复杂度 | 适用场景 | 优点 | 缺点 |
|------|---------|-----------|-----------|---------|------|------|
| **Dijkstra** | $\min_{P} \sum_{(u,v) \in P} w(u,v)$ | $O(V^2)$ | $O(V)$ | 单源、非负权重 | 精确、高效 | 不支持负权重 |
| **Floyd-Warshall** | $\min_{k} d[i][k] + d[k][j]$ | $O(V^3)$ | $O(V^2)$ | 全对最短路径 | 简单 | 性能慢 |
| **A*** | $f(n) = g(n) + h(n)$ | $O(V + E)$ | $O(V)$ | 有启发式函数 | 快速 | 需要启发式函数 |

### 6.2 性能优化建议

1. **索引优化**：
   - 为边表创建复合索引
   - 为常用查询创建索引

2. **算法优化**：
   - 使用距离限制提前终止
   - 使用物化视图缓存结果

3. **查询优化**：
   - 限制搜索范围
   - 使用并行查询

### 6.3 常见问题与解决方案

**问题1**：Dijkstra算法性能慢

- **解决方案**：创建索引、限制搜索范围、使用优化实现

**问题2**：负权重边

- **解决方案**：使用Bellman-Ford算法（需要自定义实现）

**问题3**：大规模图计算

- **解决方案**：使用Floyd-Warshall、分批处理、并行计算

**问题4**：路径重建困难

- **解决方案**：在算法中存储路径信息、使用路径表

---

## 8. 最佳实践

### 7.1 算法选择

1. **单源最短路径**：使用Dijkstra算法
2. **全对最短路径**：小图用Floyd-Warshall，大图用多次Dijkstra
3. **有启发式函数**：使用A*算法

### 7.2 数据结构设计

1. **节点表**：包含节点ID和属性
2. **边表**：包含源节点、目标节点和权重
3. **索引**：为常用查询创建索引

### 7.3 性能优化

1. **索引优化**：为边表创建复合索引
2. **算法优化**：使用距离限制、早期终止
3. **缓存优化**：使用物化视图缓存结果

### 7.4 SQL实现注意事项

1. **递归深度**：注意PostgreSQL的递归深度限制
2. **性能考虑**：避免在大图上使用Floyd-Warshall
3. **权重处理**：确保权重非负（Dijkstra）
4. **错误处理**：处理NULL值和边界情况

### 7.5 PostgreSQL 18 新特性应用

**PostgreSQL 18**引入了多项增强功能，可以显著提升最短路径算法的性能：

1. **Skip Scan优化**：
   - 对于包含路径权重的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-K最短路径查询

2. **异步I/O增强**：
   - 对于大规模图的最短路径计算，异步I/O可以显著提升性能
   - 适用于批量路径计算和多源最短路径

3. **并行查询增强**：
   - 最短路径计算支持更好的并行执行（需要谨慎使用，因为递归CTE的并行化限制）
   - 适用于独立的最短路径计算

**示例：使用Skip Scan优化最短路径查询**

```sql
-- 为路径权重创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_edges_weight_skip_scan
ON edges(source_id, weight ASC);

-- Skip Scan优化查询：查找前10条最短路径
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE shortest_paths AS (
    SELECT
        source_id AS current_node,
        target_id AS next_node,
        weight AS path_weight,
        ARRAY[source_id, target_id] AS path,
        1 AS path_length
    FROM edges
    WHERE source_id = 1

    UNION ALL

    SELECT
        sp.next_node AS current_node,
        e.target_id AS next_node,
        sp.path_weight + e.weight AS path_weight,
        sp.path || e.target_id,
        sp.path_length + 1
    FROM shortest_paths sp
    JOIN edges e ON sp.next_node = e.source_id
    WHERE e.target_id != ALL(sp.path)
      AND sp.path_length < 5
)
SELECT DISTINCT ON (next_node)
    next_node,
    path_weight,
    path,
    path_length
FROM shortest_paths
ORDER BY next_node, path_weight, path_length
LIMIT 10;
```

### 7.6 高级优化技巧

**1. 使用物化视图缓存常用路径结果**

对于频繁查询的最短路径，使用物化视图缓存结果：

```sql
-- 创建物化视图缓存常用最短路径
CREATE MATERIALIZED VIEW IF NOT EXISTS shortest_paths_cache AS
WITH popular_paths AS (
    SELECT DISTINCT source_id, target_id
    FROM route_queries  -- 假设有路径查询日志表
    WHERE query_count > 100  -- 查询次数超过100的路径
)
SELECT
    pp.source_id,
    pp.target_id,
    -- 使用Dijkstra算法计算最短路径（简化版）
    (SELECT MIN(total_distance)
     FROM shortest_path_computation
     WHERE source_id = pp.source_id AND target_id = pp.target_id
     LIMIT 1) AS shortest_distance
FROM popular_paths pp;

-- 创建索引加速物化视图查询
CREATE INDEX idx_shortest_paths_cache_source_target
ON shortest_paths_cache(source_id, target_id);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY shortest_paths_cache;
```

**2. 实时导航系统：多目标路径优化**

**实时导航系统**：综合考虑距离、时间、成本和实时交通状况的最优路径规划。

```sql
-- 实时导航系统：多目标路径优化（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'road_network') THEN
            CREATE TABLE road_network (
                from_location INTEGER NOT NULL,
                to_location INTEGER NOT NULL,
                distance_km NUMERIC(10, 2) NOT NULL,
                travel_time_minutes INTEGER NOT NULL,
                cost NUMERIC(10, 2) NOT NULL,
                traffic_level INTEGER,  -- 1-5, 5为最拥堵
                road_type VARCHAR(20),  -- 'highway', 'city_road', 'local_road'
                PRIMARY KEY (from_location, to_location)
            );

            CREATE INDEX idx_road_network_from ON road_network(from_location, travel_time_minutes);
            CREATE INDEX idx_road_network_traffic ON road_network(traffic_level);

            RAISE NOTICE '道路网络表创建成功';
        END IF;

        RAISE NOTICE '开始执行实时导航系统路径优化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '实时导航系统准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 多目标路径优化：综合考虑距离、时间、成本和交通状况
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE navigation_paths AS (
    SELECT
        origin_location AS current_location,
        0 AS total_distance,
        0 AS total_time,
        0 AS total_cost,
        ARRAY[origin_location] AS path,
        -- 综合评分：距离权重0.3，时间权重0.4，成本权重0.2，交通权重0.1
        (0 * 0.3 + 0 * 0.4 + 0 * 0.2 + 5 * 0.1) AS optimization_score
    FROM road_network
    WHERE from_location = 1  -- 起始位置

    UNION ALL

    SELECT
        rn.to_location AS current_location,
        np.total_distance + rn.distance_km AS total_distance,
        np.total_time + rn.travel_time_minutes AS total_time,
        np.total_cost + rn.cost AS total_cost,
        np.path || rn.to_location,
        -- 综合评分计算（考虑实时交通状况）
        (np.total_distance + rn.distance_km) * 0.3 +
        (np.total_time + rn.travel_time_minutes * (1 + rn.traffic_level * 0.2)) * 0.4 +
        (np.total_cost + rn.cost) * 0.2 +
        rn.traffic_level * 0.1 AS optimization_score
    FROM navigation_paths np
    JOIN road_network rn ON np.current_location = rn.from_location
    WHERE rn.to_location != ALL(np.path)
      AND np.total_time + rn.travel_time_minutes * (1 + rn.traffic_level * 0.2) < 120  -- 时间限制（分钟）
),
optimal_paths AS (
    SELECT DISTINCT ON (current_location)
        current_location,
        total_distance,
        total_time,
        total_cost,
        optimization_score,
        path
    FROM navigation_paths
    WHERE current_location = destination_location
    ORDER BY current_location, optimization_score ASC
)
SELECT
    current_location,
    ROUND(total_distance::numeric, 2) AS total_distance_km,
    total_time AS total_time_minutes,
    ROUND(total_cost::numeric, 2) AS total_cost,
    ROUND(optimization_score::numeric, 2) AS optimization_score,
    path AS optimal_route
FROM optimal_paths
ORDER BY optimization_score ASC
LIMIT 5;  -- Top 5最优路径
```

**3. 物流配送优化：TSP问题的近似解**

**物流配送优化**：解决旅行商问题（TSP）的近似解，优化配送路径。

```sql
-- 物流配送优化：TSP问题的近似解（最近邻算法，带错误处理和性能测试）
DO $$
DECLARE
    warehouse_id INTEGER := 1;
    customer_ids INTEGER[] := ARRAY[2, 3, 4, 5, 6];
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'delivery_locations') THEN
            RAISE WARNING '表 delivery_locations 不存在，无法执行物流配送优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行物流配送优化，仓库ID: %, 客户ID: %', warehouse_id, customer_ids;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '物流配送优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- TSP最近邻算法：从仓库开始，每次选择最近的未访问客户
WITH RECURSIVE tsp_path AS (
    -- 起始：从仓库到最近的客户
    SELECT
        warehouse_id AS current_location,
        warehouse_id AS from_location,
        customer_id AS to_location,
        distance AS segment_distance,
        ARRAY[warehouse_id, customer_id] AS route,
        1 AS step,
        customer_ids - ARRAY[customer_id] AS remaining_customers
    FROM (
        SELECT DISTINCT
            customer_id,
            distance
        FROM delivery_locations
        WHERE from_location = warehouse_id
          AND customer_id = ANY(customer_ids)
        ORDER BY distance ASC
        LIMIT 1
    ) AS first_customer

    UNION ALL

    -- 递归：从当前位置到最近的未访问客户
    SELECT
        tsp.to_location AS current_location,
        tsp.to_location AS from_location,
        dl.customer_id AS to_location,
        dl.distance AS segment_distance,
        tsp.route || dl.customer_id,
        tsp.step + 1,
        tsp.remaining_customers - ARRAY[dl.customer_id]
    FROM tsp_path tsp
    JOIN LATERAL (
        SELECT
            customer_id,
            distance
        FROM delivery_locations
        WHERE from_location = tsp.to_location
          AND customer_id = ANY(tsp.remaining_customers)
        ORDER BY distance ASC
        LIMIT 1
    ) AS dl ON true
    WHERE array_length(tsp.remaining_customers, 1) > 0
),
-- 最后：从最后一个客户返回仓库
complete_route AS (
    SELECT
        route || warehouse_id AS full_route,
        SUM(segment_distance) + (
            SELECT distance
            FROM delivery_locations
            WHERE from_location = (SELECT route[array_length(route, 1)] FROM tsp_path ORDER BY step DESC LIMIT 1)
              AND to_location = warehouse_id
            LIMIT 1
        ) AS total_distance
    FROM tsp_path
    GROUP BY route
)
SELECT
    full_route AS delivery_route,
    ROUND(total_distance::numeric, 2) AS total_distance_km,
    array_length(full_route, 1) - 1 AS delivery_stops
FROM complete_route
ORDER BY total_distance ASC
LIMIT 1;
```

---

## 📚 参考资源

### 学术文献

- 《算法导论》（Introduction to Algorithms）- 最短路径算法理论
- 《图论》（Graph Theory）- 图论基础

### PostgreSQL官方文档

- [递归查询](https://www.postgresql.org/docs/current/queries-with.html#QUERIES-WITH-RECURSIVE)
- [索引优化](https://www.postgresql.org/docs/current/indexes.html)

### 在线资源

- PostgreSQL最短路径算法实现
- Dijkstra算法详解
- 路径规划最佳实践

### 相关算法

- [图遍历算法](./图遍历算法.md) - 图遍历
- [连通性分析](./连通性分析.md) - 连通性检测
- [图聚类算法](./图聚类算法.md) - 图聚类

---

**最后更新**: 2025年1月
**维护者**: PostgreSQL Modern Team
