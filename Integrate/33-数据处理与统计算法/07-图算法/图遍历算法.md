...
- [PostgreSQL 图遍历算法完整指南](#postgresql-图遍历算法完整指南)
  ...
  - [5. PostgreSQL 18 并行图遍历增强](#5-postgresql-18-并行图遍历增强)
    - [5.1 并行图遍历原理](#51-并行图遍历原理)
    - [5.2 并行深度优先搜索](#52-并行深度优先搜索)
    - [5.3 并行广度优先搜索](#53-并行广度优先搜索)
    - [5.4 并行路径查找](#54-并行路径查找)
  - [6. 实际应用案例](#6-实际应用案例)
...
## 5. PostgreSQL 18 并行图遍历增强

**PostgreSQL 18** 显著增强了并行图遍历能力，支持并行执行DFS、BFS和路径查找，大幅提升大规模图遍历的性能。

### 5.1 并行图遍历原理

PostgreSQL 18 的并行图遍历通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描图数据
2. **并行遍历**：每个工作进程独立执行图遍历
3. **并行路径查找**：并行执行路径查找算法
4. **结果合并**：主进程合并所有工作进程的遍历结果

### 5.2 并行深度优先搜索

```sql
-- PostgreSQL 18 并行深度优先搜索（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行深度优先搜索';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行深度优先搜索';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行深度优先搜索准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH RECURSIVE dfs_traversal AS (
    ...
    JOIN edges e ON dt.current_node = e.source_id
    WHERE e.target_id != ALL(dt.path)
      AND dt.depth < 10  -- 深度限制
)
SELECT DISTINCT ON (current_node)
    current_node,
    MIN(depth) AS shortest_depth,
    (SELECT path FROM dfs_traversal dt2
     WHERE dt2.current_node = dt.current_node
       AND dt2.depth = MIN(dt.depth)
     LIMIT 1) AS traversal_path
FROM dfs_traversal dt
...
```sql
-- PostgreSQL 18 并行广度优先搜索（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行广度优先搜索';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行广度优先搜索';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行广度优先搜索准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH RECURSIVE bfs_levels AS (
    ...
    SELECT
        ...
        bl.level + 1
    FROM bfs_levels bl
    JOIN edges e ON bl.current_node = e.source_id
    WHERE bl.level < 10  -- 层级限制
)
SELECT DISTINCT ON (current_node)
    current_node,
    MIN(level) AS shortest_level
FROM bfs_levels
GROUP BY current_node
ORDER BY current_node;
```
...
```sql
-- PostgreSQL 18 并行路径查找（带错误处理和性能测试）
DO $$
DECLARE
    start_node_id INTEGER := 1;
    end_node_id INTEGER := 10;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'edges') THEN
            RAISE WARNING '表 edges 不存在，无法执行并行路径查找';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行路径查找，从节点 % 到节点 %', start_node_id, end_node_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行路径查找准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH RECURSIVE all_paths AS (
    ...
    SELECT
        ...
        ap.path || e.target_id
    FROM all_paths ap
    JOIN edges e ON ap.current_node = e.source_id
    WHERE e.target_id != ALL(ap.path)
      AND array_length(ap.path, 1) < 20  -- 路径长度限制
)
SELECT
    path,
    array_length(path, 1) AS path_length
FROM all_paths
WHERE current_node = 10  -- 目标节点
...
**PostgreSQL 18**引入了多项增强功能，可以显著提升图遍历算法的性能：

1. **Skip Scan优化**：
   - 对于包含路径排序的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于分页查询和Top-K查询

2. **异步I/O增强**：
   - 对于大规模图数据的遍历，异步I/O可以显著提升性能
   - 适用于批量遍历和深度遍历

3. **并行查询增强**：
   - 图遍历支持更好的并行执行（需要谨慎使用，因为递归CTE的并行化限制）
   - 适用于独立的子图遍历

**示例：使用Skip Scan优化图遍历查询**

```sql
-- 为图遍历创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_edge_source_target
ON edges(source_id, target_id);

-- Skip Scan优化查询：查找前10个最短路径
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH RECURSIVE shortest_paths AS (
    SELECT
        source_id,
    ...
    SELECT
        ...
        sp.path || e.target_id,
        sp.path_length + 1,
        sp.total_weight + COALESCE(e.weight, 1)
    FROM shortest_paths sp
    JOIN edges e ON sp.target_id = e.source_id
    WHERE e.target_id != ALL(sp.path)
...
SELECT DISTINCT ON (target_id)
    ...
    total_weight
FROM shortest_paths
ORDER BY target_id, path_length, total_weight
LIMIT 10;
```
...
```sql
-- 社交网络影响力分析：使用图遍历查找影响范围（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'social_network') THEN
            ...
            -- 插入示例数据
            INSERT INTO social_network (user_id, follower_id, follow_date, influence_score) VALUES
                (1, 2, '2024-01-01', 0.8),
                (1, 3, '2024-01-02', 0.7),
                (2, 4, '2024-01-03', 0.6),
                (2, 5, '2024-01-04', 0.5),
            ...
            CREATE INDEX idx_social_network_user ON social_network(user_id);
            CREATE INDEX idx_social_network_follower ON social_network(follower_id);

            RAISE NOTICE '社交网络表创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '社交网络分析准备失败: %', SQLERRM;
...
WITH RECURSIVE influence_network AS (
    -- 起始用户（1度关系）
    SELECT
        user_id AS center_user,
        follower_id AS influenced_user,
        ARRAY[user_id, follower_id] AS influence_path,
        1 AS degree,
        influence_score AS path_influence
    FROM social_network
    WHERE user_id = 1
    ...
    SELECT
        inet.center_user,
        sn.follower_id AS influenced_user,
        inet.influence_path || sn.follower_id,
        inet.degree + 1,
        LEAST(inet.path_influence, sn.influence_score) AS path_influence
    FROM influence_network inet
    JOIN social_network sn ON inet.influenced_user = sn.user_id
    WHERE sn.follower_id != ALL(inet.influence_path)
      AND inet.degree < 3  -- 3度影响力
)
SELECT
    center_user,
    influenced_user,
    influence_path,
    degree,
    ROUND(path_influence::numeric, 2) AS influence_score
FROM influence_network
ORDER BY degree, influenced_user;
```
...
对于频繁遍历的图路径，使用物化视图缓存结果：

```sql
-- 创建物化视图缓存常用遍历路径
CREATE MATERIALIZED VIEW IF NOT EXISTS graph_paths_cache AS
WITH RECURSIVE all_paths AS (
    SELECT
        source_id,
        target_id,
        ...
        1 AS depth
    FROM edges
    WHERE source_id IN (SELECT DISTINCT source_id FROM edges LIMIT 100)  -- 限制起始节点
    ...
    SELECT
        ...
        ap.depth + 1
    FROM all_paths ap
    JOIN edges e ON ap.target_id = e.source_id
    WHERE e.target_id != ALL(ap.path)
      AND ap.depth < 3
)
...
-- 创建索引加速物化视图查询
CREATE INDEX idx_paths_cache_source_target ON graph_paths_cache(source_id, target_id);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY graph_paths_cache;
```
...
-- 为权重图创建部分索引
CREATE INDEX IF NOT EXISTS idx_weighted_edges
ON edges USING btree(weight)
WHERE weight > 0.5;
```
...
WITH RECURSIVE graph_traversal AS (
    SELECT
        ...
        ARRAY[source_id, target_id] AS path,
        1 AS depth,
        COALESCE(weight, 1) AS path_weight
    FROM edges
    WHERE source_id = 1
    ...
    SELECT
        gt.next_node AS current_node,
        e.target_id AS next_node,
        gt.path || e.target_id,
        gt.depth + 1,
        gt.path_weight + COALESCE(e.weight, 1)
    FROM graph_traversal gt
    JOIN edges e ON gt.next_node = e.source_id
    WHERE e.target_id != ALL(gt.path)
      AND gt.depth < 5
),
ranked_paths AS (
    SELECT
        ...
        path_weight,
        -- 使用窗口函数计算路径排名（避免重复计算）
        ROW_NUMBER() OVER (PARTITION BY next_node ORDER BY depth, path_weight) AS path_rank,
        DENSE_RANK() OVER (ORDER BY depth, path_weight) AS global_rank
...
SELECT
    ...
    global_rank
FROM ranked_paths
WHERE path_rank = 1  -- 每个目标节点的最短路径
ORDER BY global_rank, next_node;
```
