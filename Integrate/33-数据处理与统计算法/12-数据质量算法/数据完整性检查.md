...
**PostgreSQL 18**引入了多项增强功能，可以显著提升数据完整性检查的性能：

1. **Skip Scan优化**：
   - 对于包含外键的索引，Skip Scan可以跳过不必要的索引扫描
   - 特别适用于Top-N完整性检查查询和多表完整性对比查询

2. **异步I/O增强**：
   - 对于大规模完整性检查计算，异步I/O可以显著提升性能
   - 适用于批量完整性检查和完整性修复操作

3. **并行查询增强**：
   - 完整性检查支持更好的并行执行
   - 适用于大规模表完整性检查和多表并行完整性分析

**示例：使用Skip Scan优化完整性检查查询**

```sql
-- 为完整性检查创建Skip Scan优化索引
CREATE INDEX IF NOT EXISTS idx_integrity_check_skip_scan
ON orders(customer_id, order_id DESC);

-- Skip Scan优化查询：查找每个客户的最新订单完整性检查
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
SELECT DISTINCT ON (customer_id)
    ...
    order_date,
    CASE
        WHEN customer_id NOT IN (SELECT customer_id FROM customers) THEN 'Orphan Record'
        ELSE 'Valid'
    END AS integrity_status
FROM orders
ORDER BY customer_id, order_id DESC
LIMIT 50;
```
...
对于频繁使用的完整性检查结果，使用物化视图缓存：

```sql
-- 创建物化视图缓存完整性检查结果
CREATE MATERIALIZED VIEW IF NOT EXISTS integrity_check_cache AS
WITH integrity_statistics AS (
    SELECT
        'orders' AS table_name,
        'customer_id' AS column_name,
        COUNT(*) AS total_rows,
        COUNT(*) FILTER (WHERE customer_id NOT IN (SELECT customer_id FROM customers)) AS orphan_records,
        COUNT(*) FILTER (WHERE customer_id IS NULL) AS null_records,
        -- 使用窗口函数计算完整性得分（避免重复计算）
        CASE
            WHEN COUNT(*) > 0 THEN
                (COUNT(*) - COUNT(*) FILTER (WHERE customer_id NOT IN (SELECT customer_id FROM customers)) -
                 COUNT(*) FILTER (WHERE customer_id IS NULL))::numeric / COUNT(*)
            ELSE 0
    ...
    SELECT
        ...
        CASE
            WHEN COUNT(*) > 0 THEN
                (COUNT(*) - COUNT(*) FILTER (WHERE customer_id IS NULL))::numeric / COUNT(*)
            ELSE 0
        END AS integrity_score
    FROM customers
...
SELECT
    ...
    orphan_records,
    null_records,
    ROUND(integrity_score::numeric, 4) AS integrity_score,
    CASE
        WHEN integrity_score >= 0.99 THEN 'Excellent'
        WHEN integrity_score >= 0.95 THEN 'Good'
        WHEN integrity_score >= 0.90 THEN 'Moderate'
        ELSE 'Poor'
    END AS integrity_status
FROM integrity_statistics
ORDER BY table_name, column_name;

-- 创建索引加速物化视图查询
CREATE INDEX idx_integrity_check_cache_table_column ON integrity_check_cache(table_name, column_name);
CREATE INDEX idx_integrity_check_cache_status ON integrity_check_cache(integrity_status, integrity_score DESC);

-- 定期刷新物化视图
REFRESH MATERIALIZED VIEW CONCURRENTLY integrity_check_cache;
```

**2. 实时完整性检查：增量完整性更新**

**实时完整性检查**：对于实时数据，使用增量方法更新完整性检查结果。

```sql
-- 实时完整性检查：增量完整性更新（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'integrity_check_state') THEN
            CREATE TABLE integrity_check_state (
                table_name VARCHAR(100) NOT NULL,
                column_name VARCHAR(100) NOT NULL,
                constraint_type VARCHAR(50) NOT NULL,  -- 'FOREIGN_KEY', 'NOT_NULL', 'UNIQUE'
                total_rows BIGINT DEFAULT 0,
                violation_count BIGINT DEFAULT 0,
                last_checked_at TIMESTAMPTZ,
                last_updated TIMESTAMPTZ DEFAULT NOW(),
                PRIMARY KEY (table_name, column_name, constraint_type)
            );

            CREATE INDEX idx_integrity_check_state_table_column ON integrity_check_state(table_name, column_name);
            CREATE INDEX idx_integrity_check_state_updated ON integrity_check_state(last_updated DESC);

            RAISE NOTICE '完整性检查状态表创建成功';
        END IF;

        RAISE NOTICE '开始执行增量完整性检查更新';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '增量完整性检查更新准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 增量更新完整性检查统计：实时完整性检查
WITH new_integrity_data AS (
    SELECT
        'orders' AS table_name,
        'customer_id' AS column_name,
        'FOREIGN_KEY' AS constraint_type,
        COUNT(*) AS new_total_rows,
        COUNT(*) FILTER (WHERE customer_id NOT IN (SELECT customer_id FROM customers)) AS new_violation_count
    FROM orders
    WHERE order_id > (SELECT COALESCE(MAX(last_checked_at)::integer, 0) FROM integrity_check_state WHERE table_name = 'orders')
    GROUP BY 'orders', 'customer_id', 'FOREIGN_KEY'
),
updated_integrity_stats AS (
    SELECT
        COALESCE(ics.table_name, nid.table_name) AS table_name,
        COALESCE(ics.column_name, nid.column_name) AS column_name,
        COALESCE(ics.constraint_type, nid.constraint_type) AS constraint_type,
        COALESCE(ics.total_rows, 0) + COALESCE(nid.new_total_rows, 0) AS new_total_rows,
        COALESCE(ics.violation_count, 0) + COALESCE(nid.new_violation_count, 0) AS new_violation_count
    FROM integrity_check_state ics
    FULL OUTER JOIN new_integrity_data nid ON ics.table_name = nid.table_name
                                            AND ics.column_name = nid.column_name
                                            AND ics.constraint_type = nid.constraint_type
)
-- 更新或插入完整性检查状态
INSERT INTO integrity_check_state (
    table_name,
    column_name,
    constraint_type,
    total_rows,
...
SELECT
    ...
    NOW()
FROM updated_integrity_stats
ON CONFLICT (table_name, column_name, constraint_type)
DO UPDATE SET
...
**智能完整性检查**：根据数据特征自动选择最优完整性检查策略。

```sql
-- 智能完整性检查：自适应检查策略选择（带错误处理和性能测试）
DO $$
DECLARE
    table_size BIGINT;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行智能完整性检查';
            RETURN;
        END IF;

        -- 计算数据特征
        WITH integrity_features AS (
            SELECT
                COUNT(*) AS total_count,
                COUNT(*) FILTER (WHERE customer_id NOT IN (SELECT customer_id FROM customers))::numeric /
                NULLIF(COUNT(*), 0) AS violation_ratio
            FROM orders
        )
        SELECT
            total_count,
            violation_ratio
        INTO table_size, violation_rate
        FROM integrity_features;

        -- 根据数据特征自适应选择完整性检查策略
        IF table_size > 1000000 AND violation_rate < 0.01 THEN
            recommended_strategy := 'SAMPLING_CHECK';  -- 大表低违规率：采样检查
        ELSIF table_size > 100000 AND violation_rate < 0.05 THEN
            recommended_strategy := 'INCREMENTAL_CHECK';  -- 中表中违规率：增量检查
        ELSE
            recommended_strategy := 'FULL_CHECK';  -- 小表高违规率：全量检查
        END IF;

        RAISE NOTICE '表大小: %, 违规率: %, 推荐策略: %',
            table_size, violation_rate, recommended_strategy;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '智能完整性检查准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 智能完整性检查：根据策略选择不同的检查方式
WITH integrity_characteristics AS (
    SELECT
        'orders' AS table_name,
        COUNT(*) AS total_count,
        COUNT(*) FILTER (WHERE customer_id NOT IN (SELECT customer_id FROM customers))::numeric /
...
adaptive_integrity_check AS (
    SELECT
        table_name,
        total_count,
        violation_ratio,
        CASE
            WHEN total_count > 1000000 AND violation_ratio < 0.01 THEN 'SAMPLING_CHECK'
            WHEN total_count > 100000 AND violation_ratio < 0.05 THEN 'INCREMENTAL_CHECK'
            ELSE 'FULL_CHECK'
        END AS recommended_strategy,
        -- 根据推荐策略计算检查范围（避免重复计算）
        CASE
            WHEN total_count > 1000000 AND violation_ratio < 0.01 THEN
                GREATEST(1000, total_count / 100)  -- 采样1%
            WHEN total_count > 100000 AND violation_ratio < 0.05 THEN
                total_count / 10  -- 增量检查10%
            ELSE
                total_count  -- 全量检查
        END AS check_range
    FROM integrity_characteristics
)
SELECT
    table_name,
    total_count,
    ROUND(violation_ratio::numeric, 4) AS violation_ratio,
    recommended_strategy,
    check_range,
    CASE
        WHEN recommended_strategy = 'SAMPLING_CHECK' THEN 'Use TABLESAMPLE for large tables with low violation rate'
        WHEN recommended_strategy = 'INCREMENTAL_CHECK' THEN 'Use incremental check for medium tables'
        ELSE 'Use full table scan for small tables or high violation rate'
    END AS optimization_advice
FROM adaptive_integrity_check;
```
