# PostgreSQL 关联规则评估算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 数据挖掘 | 关联规则 | 规则评估
> **难度级别**: ⭐⭐⭐⭐ (高级)
> **参考标准**: Association Rule Evaluation, Rule Quality Metrics, Data Mining

---

## 📋 目录

- [PostgreSQL 关联规则评估算法完整指南](#postgresql-关联规则评估算法完整指南)
  - [📋 目录](#-目录)
  - [关联规则评估概述](#关联规则评估概述)
    - [评估指标](#评估指标)
  - [1. 提升度分析](#1-提升度分析)
    - [1.1 提升度计算](#11-提升度计算)
  - [2. 卡方检验](#2-卡方检验)
    - [2.1 卡方检验原理](#21-卡方检验原理)
    - [2.2 卡方检验实现](#22-卡方检验实现)
  - [3. 规则剪枝](#3-规则剪枝)
    - [3.1 基于提升度的剪枝](#31-基于提升度的剪枝)
    - [3.2 基于置信度的剪枝](#32-基于置信度的剪枝)
  - [4. PostgreSQL 18 并行关联规则评估增强](#4-postgresql-18-并行关联规则评估增强)
    - [4.1 并行关联规则评估原理](#41-并行关联规则评估原理)
    - [4.2 并行提升度分析](#42-并行提升度分析)
    - [4.3 并行卡方检验](#43-并行卡方检验)
  - [5. 综合评估指标](#5-综合评估指标)
    - [4.1 规则评分](#41-规则评分)
  - [📚 参考资源](#-参考资源)
  - [📊 性能优化建议](#-性能优化建议)
...
**关联规则评估**使用多种指标评估规则的质量和意义，筛选有价值的关联规则。

### 评估指标

| 指标 | 公式 | 含义 |
|------|------|------|
| **支持度** | $P(A \cup B)$ | 规则出现的频率 |
| **置信度** | $P(B\|A)$ | 规则的可信度 |
| **提升度** | $\frac{P(B\|A)}{P(B)}$ | 规则的有效性 |
| **卡方值** | $\chi^2$ | 统计显著性 |
...
BEGIN
    BEGIN
        ...
        CREATE TABLE association_rules (
            ...
            antecedent TEXT NOT NULL,  -- 前件
            consequent TEXT NOT NULL,  -- 后件
...
SELECT
    ...
    degrees_of_freedom,
    CASE
        WHEN chi_square_stat > 3.841 THEN 'Significant association (p < 0.05) ✓'
        WHEN chi_square_stat > 2.706 THEN 'Marginally significant (p < 0.10)'
        ELSE 'No significant association (p ≥ 0.10) ✗'
...
pruned_rules AS (
    SELECT
        ...
        ROUND(lift::numeric, 4) AS lift
    FROM rule_metrics
    WHERE lift > 1.0  -- 只保留提升度>1的规则
      AND support >= 0.3  -- 最小支持度
      AND confidence >= 0.5  -- 最小置信度
)
SELECT
    rule_id,
    rule,
...
WHERE confidence >= 0.5  -- 保留置信度≥0.5的规则
ORDER BY confidence DESC;
```

---

## 4. PostgreSQL 18 并行关联规则评估增强

**PostgreSQL 18** 显著增强了并行关联规则评估计算能力，支持并行执行提升度分析、卡方检验和规则剪枝，大幅提升大规模关联规则评估的性能。

### 4.1 并行关联规则评估原理

PostgreSQL 18 的并行关联规则评估通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描规则数据
2. **并行指标计算**：每个工作进程独立计算评估指标
3. **并行规则筛选**：并行执行规则剪枝和排序
4. **结果合并**：主进程合并所有工作进程的评估结果

### 4.2 并行提升度分析

```sql
-- PostgreSQL 18 并行提升度分析（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'association_rules') THEN
            RAISE WARNING '表 association_rules 不存在，无法执行并行提升度分析';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行提升度分析';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行提升度分析准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transaction_data') THEN
            RAISE WARNING '表 transaction_data 不存在，无法执行并行卡方检验';
            RETURN;
        END IF;
...
SELECT
    ...
    CASE
        WHEN (POWER(n11 - e11, 2) / NULLIF(e11, 0) +
              POWER(n10 - e10, 2) / NULLIF(e10, 0) +
              POWER(n01 - e01, 2) / NULLIF(e01, 0) +
              POWER(n00 - e00, 2) / NULLIF(e00, 0)) > 3.841 THEN 'Significant association ✓'
...
WITH rule_scoring AS (
    SELECT
        ar.rule_id,
        ar.antecedent || ' → ' || ar.consequent AS rule,
        ar.support,
        ar.confidence,
        is_cons.support AS consequent_support,
        ar.confidence / NULLIF(is_cons.support, 0) AS lift,
...
1. **索引优化**：在transaction_id和item_id上创建索引
2. **批量计算**：批量计算多个规则的指标
3. **缓存结果**：缓存频繁访问的规则评估结果

---

## 🎯 最佳实践

1. **多指标评估**：结合支持度、置信度、提升度等多个指标
2. **统计显著性**：使用卡方检验验证规则显著性
3. **业务验证**：结合业务知识验证规则合理性
4. **规则剪枝**：移除无效和冗余规则
