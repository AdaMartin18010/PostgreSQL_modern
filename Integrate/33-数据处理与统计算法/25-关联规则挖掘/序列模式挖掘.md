...
- [PostgreSQL 序列模式挖掘算法完整指南](#postgresql-序列模式挖掘算法完整指南)
  ...
  - [3. 复杂度分析](#3-复杂度分析)
    - [3.1 时间复杂度](#31-时间复杂度)
    - [3.2 空间复杂度](#32-空间复杂度)
  - [4. PostgreSQL 18 并行序列模式挖掘增强](#4-postgresql-18-并行序列模式挖掘增强)
    - [4.1 并行序列模式挖掘原理](#41-并行序列模式挖掘原理)
    - [4.2 并行GSP算法](#42-并行gsp算法)
    - [4.3 并行PrefixSpan算法](#43-并行prefixspan算法)
  - [5. 实际应用案例](#5-实际应用案例)
...
## 4. PostgreSQL 18 并行序列模式挖掘增强

**PostgreSQL 18** 显著增强了并行序列模式挖掘计算能力，支持并行执行GSP算法、PrefixSpan算法和序列模式匹配，大幅提升大规模序列模式挖掘的性能。

### 4.1 并行序列模式挖掘原理

PostgreSQL 18 的并行序列模式挖掘通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描序列数据
2. **并行支持度计算**：每个工作进程独立计算序列支持度
3. **并行模式生成**：并行执行候选序列生成
...
```sql
-- PostgreSQL 18 并行GSP算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sequence_data') THEN
            RAISE WARNING '表 sequence_data 不存在，无法执行并行GSP算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行GSP算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行GSP算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
WITH single_item_patterns AS (
    SELECT
        event_type,
        COUNT(DISTINCT sequence_id) AS support_count,
        COUNT(DISTINCT sequence_id)::NUMERIC / (SELECT COUNT(DISTINCT sequence_id) FROM sequence_data) AS support
    FROM sequence_data
    GROUP BY event_type
    HAVING COUNT(DISTINCT sequence_id) >= 2
)
SELECT
    event_type,
    support_count,
...
```sql
-- PostgreSQL 18 并行PrefixSpan算法（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sequence_data') THEN
            RAISE WARNING '表 sequence_data 不存在，无法执行并行PrefixSpan算法';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行PrefixSpan算法';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行PrefixSpan算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
projected_sequences AS (
    ...
    WHERE sequence_id IN (
        SELECT DISTINCT sequence_id
        FROM prefix_projection
        WHERE prefix_position IS NOT NULL
    )
      AND event_order > (SELECT MIN(event_order) FROM prefix_projection WHERE prefix_position IS NOT NULL)
)
SELECT
    event_type,
    COUNT(DISTINCT sequence_id) AS projected_support
FROM projected_sequences
GROUP BY event_type
ORDER BY projected_support DESC;
```
