# PostgreSQL 协整分析算法完整指南

> **创建日期**: 2025年1月
> **技术栈**: PostgreSQL 17+/18+ | 时间序列 | 协整分析 | 单位根检验
> **难度级别**: ⭐⭐⭐⭐⭐ (专家级)
> **参考标准**: Cointegration Analysis, Engle-Granger Test, Johansen Test

---

## 📋 目录

- [PostgreSQL 协整分析算法完整指南](#postgresql-协整分析算法完整指南)
  - [📋 目录](#-目录)
  - [协整分析概述](#协整分析概述)
    - [核心概念](#核心概念)
    - [应用场景](#应用场景)
  - [1. Engle-Granger检验](#1-engle-granger检验)
    - [1.1 Engle-Granger两步法](#11-engle-granger两步法)
    - [1.2 Engle-Granger实现](#12-engle-granger实现)
  - [2. Johansen检验](#2-johansen检验)
    - [2.1 Johansen检验原理](#21-johansen检验原理)
    - [2.2 Johansen检验实现](#22-johansen检验实现)
  - [3. PostgreSQL 18 并行协整分析增强](#3-postgresql-18-并行协整分析增强)
    - [3.1 并行协整分析原理](#31-并行协整分析原理)
    - [3.2 并行Engle-Granger检验](#32-并行engle-granger检验)
    - [3.3 并行残差平稳性检验](#33-并行残差平稳性检验)
  - [4. 误差修正模型（ECM）](#4-误差修正模型ecm)
    - [3.1 ECM原理](#31-ecm原理)
    - [3.2 ECM实现](#32-ecm实现)
  - [📚 参考资源](#-参考资源)
  - [📊 性能优化建议](#-性能优化建议)
  - [🎯 最佳实践](#-最佳实践)

---

## 协整分析概述

**协整**描述非平稳时间序列之间的长期均衡关系，是时间序列分析的重要工具。

### 核心概念

**协整定义**：如果两个非平稳序列的线性组合是平稳的，则它们协整。

**协整向量**：$\beta$ 使得 $y_t - \beta x_t$ 是平稳的。

### 应用场景

| 应用领域 | 具体应用 |
|---------|---------|
| **金融** | 配对交易、套利策略 |
| **经济** | 长期均衡关系分析 |
| **能源** | 价格关系分析 |

---

## 1. Engle-Granger检验

### 1.1 Engle-Granger两步法

**Engle-Granger两步法**：

1. **第一步**：估计协整关系 $y_t = \alpha + \beta x_t + \epsilon_t$
2. **第二步**：检验残差 $\epsilon_t$ 的平稳性（ADF检验）

### 1.2 Engle-Granger实现

```sql
-- 协整分析数据准备（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cointegration_data') THEN
            DROP TABLE cointegration_data CASCADE;
        END IF;

        CREATE TABLE cointegration_data (
            date DATE PRIMARY KEY,
            series1 NUMERIC NOT NULL,
            series2 NUMERIC NOT NULL
        );

        -- 插入示例数据（两个协整的时间序列）
        INSERT INTO cointegration_data (date, series1, series2)
        SELECT
            '2020-01-01'::DATE + (i || ' days')::INTERVAL,
            -- series1: 随机游走 + 趋势
            100 + i * 0.1 + SUM((RANDOM() - 0.5) * 2) OVER (ORDER BY i),
            -- series2: series1的线性变换 + 平稳误差
            (100 + i * 0.1 + SUM((RANDOM() - 0.5) * 2) OVER (ORDER BY i)) * 2 + (RANDOM() - 0.5) * 0.5
        FROM generate_series(1, 1000) i;

        CREATE INDEX idx_cointegration_date ON cointegration_data(date);

        RAISE NOTICE '协整数据表创建成功，共 % 条记录', (SELECT COUNT(*) FROM cointegration_data);
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- Engle-Granger检验：第一步 - 估计协整关系
WITH regression_coefficients AS (
    SELECT
        -- 使用最小二乘法估计系数
        REGR_SLOPE(series1, series2) AS beta,
        REGR_INTERCEPT(series1, series2) AS alpha,
        REGR_R2(series1, series2) AS r_squared
    FROM cointegration_data
),
regression_residuals AS (
    SELECT
        cd.date,
        cd.series1,
        cd.series2,
        cd.series1 - (rc.alpha + rc.beta * cd.series2) AS residual
    FROM cointegration_data cd
    CROSS JOIN regression_coefficients rc
)
SELECT
    ROUND(alpha::numeric, 4) AS intercept,
    ROUND(beta::numeric, 4) AS slope,
    ROUND(r_squared::numeric, 4) AS r_squared
FROM regression_coefficients;

-- Engle-Granger检验：第二步 - ADF检验残差
WITH regression_coefficients AS (
    SELECT
        REGR_SLOPE(series1, series2) AS beta,
        REGR_INTERCEPT(series1, series2) AS alpha
    FROM cointegration_data
),
residuals AS (
    SELECT
        date,
        series1 - (alpha + beta * series2) AS residual
    FROM cointegration_data
    CROSS JOIN regression_coefficients
),
adf_regression AS (
    SELECT
        -- ADF检验：残差对滞后残差回归
        REGR_SLOPE(residual, LAG(residual) OVER (ORDER BY date)) AS rho,
        REGR_INTERCEPT(residual, LAG(residual) OVER (ORDER BY date)) AS intercept,
        COUNT(*) AS n
    FROM residuals
    WHERE LAG(residual) OVER (ORDER BY date) IS NOT NULL
),
adf_statistic AS (
    SELECT
        rho,
        n,
        -- ADF统计量：t = (rho - 1) / SE(rho)
        (rho - 1) / NULLIF(
            SQRT((1 - POWER(rho, 2)) / NULLIF(n - 2, 0)),
            0
        ) AS adf_stat
    FROM adf_regression
)
SELECT
    ROUND(rho::numeric, 6) AS rho_coefficient,
    ROUND(adf_stat::numeric, 4) AS adf_statistic,
    CASE
        WHEN adf_stat < -3.34 THEN 'Cointegrated (1% level) ✓'
        WHEN adf_stat < -2.86 THEN 'Cointegrated (5% level) ✓'
        WHEN adf_stat < -2.57 THEN 'Cointegrated (10% level) ✓'
        ELSE 'Not cointegrated ✗'
    END AS cointegration_result
FROM adf_statistic;
```

---

## 2. Johansen检验

### 2.1 Johansen检验原理

**Johansen检验**用于多变量协整分析。

**检验统计量**：

- **迹统计量**：$\lambda_{trace} = -T \sum_{i=r+1}^{n} \ln(1-\hat{\lambda}_i)$
- **最大特征值统计量**：$\lambda_{max} = -T \ln(1-\hat{\lambda}_{r+1})$

### 2.2 Johansen检验实现

```sql
-- Johansen检验：多变量协整
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'multivariate_cointegration') THEN
            DROP TABLE multivariate_cointegration CASCADE;
        END IF;

        CREATE TABLE multivariate_cointegration (
            date DATE PRIMARY KEY,
            series1 NUMERIC NOT NULL,
            series2 NUMERIC NOT NULL,
            series3 NUMERIC NOT NULL
        );

        -- 插入多变量数据
        INSERT INTO multivariate_cointegration (date, series1, series2, series3)
        SELECT
            '2020-01-01'::DATE + (i || ' days')::INTERVAL,
            100 + i * 0.1 + SUM((RANDOM() - 0.5) * 2) OVER (ORDER BY i),
            (100 + i * 0.1) * 2 + SUM((RANDOM() - 0.5) * 2) OVER (ORDER BY i),
            (100 + i * 0.1) * 3 + SUM((RANDOM() - 0.5) * 2) OVER (ORDER BY i)
        FROM generate_series(1, 500) i;

        RAISE NOTICE '多变量协整数据表创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表失败: %', SQLERRM;
    END;
END $$;

-- Johansen检验：计算特征值（简化版）
WITH differences AS (
    SELECT
        date,
        series1 - LAG(series1) OVER (ORDER BY date) AS d_series1,
        series2 - LAG(series2) OVER (ORDER BY date) AS d_series2,
        series3 - LAG(series3) OVER (ORDER BY date) AS d_series3,
        LAG(series1) OVER (ORDER BY date) AS lag_series1,
        LAG(series2) OVER (ORDER BY date) AS lag_series2,
        LAG(series3) OVER (ORDER BY date) AS lag_series3
    FROM multivariate_cointegration
    WHERE LAG(series1) OVER (ORDER BY date) IS NOT NULL
),
-- 计算协方差矩阵（简化）
covariance_matrix AS (
    SELECT
        VAR_POP(d_series1) AS var1,
        VAR_POP(d_series2) AS var2,
        VAR_POP(d_series3) AS var3,
        COVAR_POP(d_series1, d_series2) AS cov12,
        COVAR_POP(d_series1, d_series3) AS cov13,
        COVAR_POP(d_series2, d_series3) AS cov23
    FROM differences
)
SELECT
    ROUND(var1::numeric, 4) AS variance_series1,
    ROUND(var2::numeric, 4) AS variance_series2,
    ROUND(var3::numeric, 4) AS variance_series3,
    ROUND(cov12::numeric, 4) AS covariance_12,
    ROUND(cov13::numeric, 4) AS covariance_13,
    ROUND(cov23::numeric, 4) AS covariance_23
FROM covariance_matrix;
```

---

## 3. PostgreSQL 18 并行协整分析增强

**PostgreSQL 18** 显著增强了并行协整分析计算能力，支持并行执行Engle-Granger检验、Johansen检验和误差修正模型，大幅提升大规模时间序列协整分析的性能。

### 3.1 并行协整分析原理

PostgreSQL 18 的并行协整分析通过以下方式实现：

1. **并行扫描**：多个工作进程并行扫描时间序列数据
2. **并行回归计算**：每个工作进程独立计算协整关系
3. **并行单位根检验**：并行执行ADF检验
4. **结果合并**：主进程合并所有工作进程的计算结果

### 3.2 并行Engle-Granger检验

```sql
-- PostgreSQL 18 并行Engle-Granger检验（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cointegration_data') THEN
            RAISE WARNING '表 cointegration_data 不存在，无法执行并行Engle-Granger检验';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行Engle-Granger检验';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行Engle-Granger检验准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行协整关系估计
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH regression_stats AS (
    SELECT
        AVG(series1) AS mean1,
        AVG(series2) AS mean2,
        SUM((series1 - AVG(series1) OVER ()) * (series2 - AVG(series2) OVER ())) AS covariance,
        SUM(POWER(series1 - AVG(series1) OVER (), 2)) AS variance1
    FROM cointegration_data
),
cointegration_coeff AS (
    SELECT
        covariance / NULLIF(variance1, 0) AS beta,
        mean2 - (covariance / NULLIF(variance1, 0)) * mean1 AS alpha
    FROM regression_stats
)
SELECT
    ROUND(alpha::numeric, 4) AS intercept,
    ROUND(beta::numeric, 4) AS cointegration_coefficient
FROM cointegration_coeff;
```

### 3.3 并行残差平稳性检验

```sql
-- PostgreSQL 18 并行残差平稳性检验（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cointegration_data') THEN
            RAISE WARNING '表 cointegration_data 不存在，无法执行并行残差平稳性检验';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行PostgreSQL 18并行残差平稳性检验';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行残差平稳性检验准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

SET max_parallel_workers_per_gather = 4;
SET parallel_setup_cost = 0;

-- 并行ADF检验：残差差分
EXPLAIN (ANALYZE, BUFFERS, TIMING, VERBOSE)
WITH residuals AS (
    SELECT
        date,
        series2 - 2 * series1 AS residual  -- 简化：假设beta=2
    FROM cointegration_data
),
residual_diff AS (
    SELECT
        date,
        residual,
        residual - LAG(residual) OVER (ORDER BY date) AS diff_residual
    FROM residuals
),
adf_statistic AS (
    SELECT
        AVG(diff_residual) / NULLIF(STDDEV(diff_residual), 0) AS adf_stat
    FROM residual_diff
    WHERE diff_residual IS NOT NULL
)
SELECT
    ROUND(adf_stat::numeric, 4) AS adf_statistic,
    CASE
        WHEN adf_stat < -2.86 THEN 'Stationary (p < 0.05) ✓'
        WHEN adf_stat < -2.57 THEN 'Stationary (p < 0.10)'
        ELSE 'Non-stationary ✗'
    END AS stationarity_test
FROM adf_statistic;
```

---

## 4. 误差修正模型（ECM）

### 3.1 ECM原理

**误差修正模型**描述短期动态调整到长期均衡的过程。

$$\Delta y_t = \alpha + \beta \Delta x_t + \gamma (y_{t-1} - \theta x_{t-1}) + \epsilon_t$$

其中 $(y_{t-1} - \theta x_{t-1})$ 是误差修正项。

### 3.2 ECM实现

```sql
-- 误差修正模型
WITH regression_coeffs AS (
    SELECT
        REGR_SLOPE(series1, series2) AS beta,
        REGR_INTERCEPT(series1, series2) AS alpha
    FROM cointegration_data
),
ecm_data AS (
    SELECT
        cd.date,
        cd.series1 - LAG(cd.series1) OVER (ORDER BY cd.date) AS delta_y,
        cd.series2 - LAG(cd.series2) OVER (ORDER BY cd.date) AS delta_x,
        cd.series1 - (rc.alpha + rc.beta * cd.series2) AS error_correction_term
    FROM cointegration_data cd
    CROSS JOIN regression_coeffs rc
    WHERE LAG(cd.series1) OVER (ORDER BY cd.date) IS NOT NULL
),
ecm_regression AS (
    SELECT
        REGR_SLOPE(delta_y, delta_x) AS short_term_coeff,
        REGR_SLOPE(delta_y, error_correction_term) AS ecm_coeff,
        REGR_R2(delta_y, delta_x) AS r_squared
    FROM ecm_data
)
SELECT
    ROUND(short_term_coeff::numeric, 4) AS short_term_coefficient,
    ROUND(ecm_coeff::numeric, 4) AS error_correction_coefficient,
    ROUND(r_squared::numeric, 4) AS r_squared,
    CASE
        WHEN ecm_coeff < 0 THEN 'Error correction mechanism present ✓'
        ELSE 'No error correction'
    END AS ecm_interpretation
FROM ecm_regression;
```

---

## 📚 参考资源

1. **Engle, R.F., Granger, C.W.J. (1987)**: "Co-integration and Error Correction: Representation, Estimation, and Testing"
2. **Johansen, S. (1991)**: "Estimation and Hypothesis Testing of Cointegration Vectors in Gaussian Vector Autoregressive Models"

---

## 📊 性能优化建议

1. **数据预处理**：确保数据是I(1)过程
2. **滞后选择**：使用信息准则选择最优滞后
3. **并行计算**：并行计算多个协整关系
4. **缓存结果**：缓存回归系数和检验统计量

---

## 🎯 最佳实践

1. **单位根检验**：先检验序列的平稳性
2. **协整检验**：使用Engle-Granger或Johansen检验
3. **ECM建模**：如果协整，建立误差修正模型
4. **结果解释**：解释协整向量的经济意义

---

**最后更新**: 2025年1月
**文档状态**: ✅ 已完成
