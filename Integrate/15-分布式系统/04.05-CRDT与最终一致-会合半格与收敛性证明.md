...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'crdt_state') THEN
            ...
            RAISE NOTICE '表 crdt_state 创建成功';
        ELSE
            RAISE NOTICE '表 crdt_state 已存在';
        END IF;
    EXCEPTION
...
```sql
-- 创建计数器CRDT表（G-Counter：增长计数器）（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'g_counter_crdt') THEN
            ...
            RAISE NOTICE '表 g_counter_crdt 创建成功';
        ELSE
            RAISE NOTICE '表 g_counter_crdt 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 g_counter_crdt 已存在';
...
```sql
-- SQLite 3.45：简化的CRDT实现（使用TEXT存储JSON）（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'crdt_state') THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'like_counter_crdt') THEN
            ...
            RAISE NOTICE '表 like_counter_crdt 创建成功';
        ELSE
            RAISE NOTICE '表 like_counter_crdt 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 like_counter_crdt 已存在';
...
```sql
-- 创建购物车CRDT（OR-Set实现）（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cart_crdt') THEN
            ...
            RAISE NOTICE '表 cart_crdt 创建成功';
        ELSE
            RAISE NOTICE '表 cart_crdt 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 cart_crdt 已存在';
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '验证会合半格性质失败: %', SQLERRM;
                RETURN FALSE;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '函数 verify_join_semilattice 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'CRDT状态监控查询成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '监控查询失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            -- 定期合并所有副本的状态
            -- 保证最终一致性
            RAISE NOTICE '定期合并执行中...';
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '定期合并失败: %', SQLERRM;
                RAISE;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '函数 periodic_merge 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建函数失败: %', SQLERRM;
            RAISE;
