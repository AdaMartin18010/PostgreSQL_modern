...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '最大准备事务数设置成功，已重新加载配置（需要重启PostgreSQL使配置生效）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置最大准备事务数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
-- PostgreSQL 18：查看2PC统计（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    datname,
    xact_commit,
...
```sql
-- 场景：协调者故障后的恢复（带错误处理）
-- 1. 查看准备的事务（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_prepared_xacts;

-- 2. 根据业务逻辑决定提交或中止（带错误处理）
DO $$
BEGIN
    BEGIN
        -- 如果所有参与者都准备成功，则提交
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '事务处理失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 3. PostgreSQL 18：自动清理过期的准备事务（带错误处理）
-- 设置准备事务超时
DO $$
BEGIN
    BEGIN
        ...
        PERFORM pg_reload_conf();
        RAISE NOTICE '准备事务超时设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置准备事务超时失败: %', SQLERRM;
            RAISE;
    END;
END $$;
-- 超过超时时间的准备事务会被自动清理
...
BEGIN
    BEGIN
        SET statement_timeout = '30s';
        -- 超过30秒自动回滚准备的事务
        RAISE NOTICE '超时机制设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置超时机制失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        PERFORM pg_reload_conf();
        -- 避免准备事务无限期阻塞
        RAISE NOTICE '超时设置成功，已重新加载配置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置超时失败: %', SQLERRM;
            RAISE;
    END;
END $$;
