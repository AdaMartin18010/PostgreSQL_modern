...
> **📋 文档来源**: `MVCC-ACID-CAP\04-形式化论证\CAP同构性论证\原子性与分区容错.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '超时配置设置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
        ...
        WHEN OTHERS THEN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    NULL;
            END;
            RAISE WARNING 'CP模式两阶段提交失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        UPDATE accounts SET balance = balance - 100 WHERE id = 1;
        COMMIT;  -- 本地立即提交
        RAISE NOTICE 'AP模式本地提交成功';
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE WARNING 'AP模式本地提交失败: %', SQLERRM;
            RAISE;
    END;
END $$;
