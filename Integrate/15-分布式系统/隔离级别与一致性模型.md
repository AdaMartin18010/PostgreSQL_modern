...
BEGIN
    BEGIN
        ...
        -- 3. 冲突检测与回滚
        COMMIT;
        RAISE NOTICE 'SERIALIZABLE事务执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE WARNING 'SERIALIZABLE事务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        -- 3. 快照隔离（SI）
        COMMIT;
        RAISE NOTICE 'REPEATABLE READ事务执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE WARNING 'REPEATABLE READ事务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        -- 3. 读已提交数据
        COMMIT;
        RAISE NOTICE 'READ COMMITTED事务执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE WARNING 'READ COMMITTED事务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
