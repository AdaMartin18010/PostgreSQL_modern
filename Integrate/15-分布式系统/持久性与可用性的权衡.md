...
DO $$
BEGIN
    RAISE NOTICE 'WAL配置需要在postgresql.conf中设置:';
    RAISE NOTICE 'wal_level = replica';
    RAISE NOTICE 'max_wal_size = 1GB';
    RAISE NOTICE 'min_wal_size = 80MB';
    RAISE NOTICE '设置后需要重启PostgreSQL或使用SELECT pg_reload_conf()';
END $$;
...
```sql
-- 同步刷新（强持久性，低可用性）（带错误处理）
DO $$
BEGIN
    BEGIN
        ALTER SYSTEM SET synchronous_commit = 'on';
        PERFORM pg_reload_conf();
        RAISE NOTICE '同步刷新设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置同步刷新失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 异步刷新（弱持久性，高可用性）（带错误处理）
DO $$
BEGIN
    BEGIN
        ALTER SYSTEM SET synchronous_commit = 'off';
        PERFORM pg_reload_conf();
        RAISE NOTICE '异步刷新设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置异步刷新失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ALTER SYSTEM SET synchronous_commit = 'on';
        PERFORM pg_reload_conf();
        RAISE NOTICE '金融场景：强持久性设置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '切换失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 动态切换到同步提交（提高持久性）（带错误处理）
DO $$
BEGIN
    BEGIN
        ALTER SYSTEM SET synchronous_commit = 'on';
        PERFORM pg_reload_conf();
        RAISE NOTICE '动态切换到同步提交成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '切换失败: %', SQLERRM;
            RAISE;
