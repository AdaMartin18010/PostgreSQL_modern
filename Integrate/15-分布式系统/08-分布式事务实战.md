...
> **📋 文档来源**: `docs\04-Distributed\08-分布式事务实战.md`
> **📅 复制日期**: 2025-12-22
...
- [PostgreSQL分布式事务实战](#postgresql分布式事务实战)
  - [📋 目录](#-目录)
  - [1. 两阶段提交（2PC）](#1-两阶段提交2pc)
    - [1.1 基础实现](#11-基础实现)
    - [1.2 Python实现](#12-python实现)
    - [1.3 监控prepared事务](#13-监控prepared事务)
  - [2. Saga模式](#2-saga模式)
    - [2.1 补偿事务](#21-补偿事务)
    - [2.2 Saga协调器](#22-saga协调器)
  - [3. 最终一致性](#3-最终一致性)
    - [3.1 异步消息队列](#31-异步消息队列)
  - [4. FDW（外部数据包装器）](#4-fdw外部数据包装器)
    - [4.1 postgres\_fdw](#41-postgres_fdw)
    - [4.2 性能优化](#42-性能优化)
  - [5. 幂等性设计](#5-幂等性设计)
    - [5.1 幂等操作](#51-幂等操作)
  - [6. 分布式锁](#6-分布式锁)
    - [6.1 PostgreSQL advisory lock](#61-postgresql-advisory-lock)
    - [6.2 分布式锁实现](#62-分布式锁实现)
  - [7. 容量规划](#7-容量规划)
    - [7.1 数据增长预测](#71-数据增长预测)
  - [8. 跨数据库查询](#8-跨数据库查询)
    - [8.1 dblink](#81-dblink)
  - [9. 分片策略](#9-分片策略)
    - [9.1 应用层分片](#91-应用层分片)
  - [10. 最终一致性验证](#10-最终一致性验证)
    - [10.1 数据对账](#101-数据对账)
...
-- 对账函数
-- 账户对账函数（带完整错误处理）
CREATE OR REPLACE FUNCTION reconcile_accounts(p_check_date DATE)
RETURNS TABLE (
    account_id BIGINT,
...
BEGIN
    ...
    IF p_check_date IS NULL THEN
        RAISE EXCEPTION '对账日期不能为空';
    END IF;

    IF p_check_date > CURRENT_DATE THEN
        RAISE EXCEPTION '对账日期不能是未来日期: %', p_check_date;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
        RAISE EXCEPTION 'transactions表不存在';
    END IF;
    ...
    BEGIN
        ...
        WITH computed AS (
            SELECT
                ...
                COALESCE(SUM(
                    CASE
                        WHEN transaction_type = 'credit' THEN COALESCE(amount, 0)
                        WHEN transaction_type = 'debit' THEN -COALESCE(amount, 0)
                        ELSE 0
                    END
                ), 0) AS balance
            FROM transactions
            WHERE DATE(created_at) <= p_check_date
        ...
        SELECT
            a.account_id,
            COALESCE(a.balance, 0) AS source_balance,
            COALESCE(c.balance, 0) AS computed_balance,
            COALESCE(a.balance, 0) - COALESCE(c.balance, 0) AS difference
        FROM accounts a
        LEFT JOIN computed c ON a.account_id = c.account_id
        WHERE a.account_id IS NOT NULL
          AND ABS(COALESCE(a.balance, 0) - COALESCE(c.balance, 0)) > 0.01
        ORDER BY ABS(COALESCE(a.balance, 0) - COALESCE(c.balance, 0)) DESC;
    EXCEPTION
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '对账计算溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '执行对账失败: %', SQLERRM;
    END;
END;
$$;
