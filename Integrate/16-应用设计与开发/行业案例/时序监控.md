...
> **📋 文档来源**: `PostgreSQL\09-应用设计\行业案例\时序监控.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    ...
    -- 生成分区名称和时间范围
    v_partition_name := p_table_name || '_' || to_char(p_partition_date, 'YYYY_MM_DD');
    v_start_date := p_partition_date::TIMESTAMPTZ;
    v_end_date := (p_partition_date + INTERVAL '1 day')::TIMESTAMPTZ;
    ...
    EXCEPTION
        ...
        WHEN invalid_table_definition THEN
            RAISE EXCEPTION '分区定义无效: 表=%，日期=%', p_table_name, p_partition_date;
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建分区失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'create_daily_partition执行失败: %', SQLERRM;
END;
$$;
...
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics_hourly') THEN
        RAISE EXCEPTION 'metrics_hourly表不存在，无法刷新';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'metrics') THEN
        RAISE EXCEPTION 'metrics表不存在，无法刷新';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '删除过期数据: % 条', v_deleted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '删除过期数据失败: %', SQLERRM;
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE '插入新数据: % 条', v_inserted_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'metrics或metrics_hourly表不存在';
        WHEN unique_violation THEN
            RAISE WARNING '插入数据时发生唯一约束冲突';
        WHEN OTHERS THEN
            RAISE EXCEPTION '插入新数据失败: %', SQLERRM;
    END;

    RAISE NOTICE '刷新完成。删除: % 条，插入: % 条', v_deleted_count, v_inserted_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'refresh_metrics_hourly执行失败: %', SQLERRM;
END;
$$;
```
...
- ⭐ [索引结构与优化](../../02-查询与优化/02.02-索引结构/02.02-索引结构与优化.md) - BRIN索引优化
- ⭐ [分区表设计](../../04-存储与恢复/01.06-存储管理与数据持久化.md) - 分区表最佳实践
