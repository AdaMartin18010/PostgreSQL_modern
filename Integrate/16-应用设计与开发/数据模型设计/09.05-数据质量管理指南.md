---

> **📋 文档来源**: `PostgreSQL\09-应用设计\数据模型设计\09.05-数据质量管理指南.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# PostgreSQL数据质量管理完整指南

> **版本**: v1.1
> **最后更新**: 2025-11-22
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **难度**: ⭐⭐⭐⭐
> **应用场景**: 数据质量、数据治理、数据清洗、数据验证、数据监控

---

## 📋 目录

- [PostgreSQL数据质量管理完整指南](#postgresql数据质量管理完整指南)
  - [📋 目录](#-目录)
  - [一、概述](#一概述)
    - [1.1 数据质量概念](#11-数据质量概念)
    - [1.2 数据质量维度](#12-数据质量维度)
    - [1.3 PostgreSQL数据质量优势](#13-postgresql数据质量优势)
    - [1.4 版本要求](#14-版本要求)
  - [二、数据质量维度](#二数据质量维度)
    - [2.1 完整性（Completeness）](#21-完整性completeness)
    - [2.2 准确性（Accuracy）](#22-准确性accuracy)
    - [2.3 一致性（Consistency）](#23-一致性consistency)
    - [2.4 及时性（Timeliness）](#24-及时性timeliness)
    - [2.5 有效性（Validity）](#25-有效性validity)
    - [2.6 唯一性（Uniqueness）](#26-唯一性uniqueness)
  - [三、数据质量检测](#三数据质量检测)
    - [3.1 完整性检测](#31-完整性检测)
    - [3.2 准确性检测](#32-准确性检测)
    - [3.3 一致性检测](#33-一致性检测)
    - [3.4 有效性检测](#34-有效性检测)
  - [四、数据清洗](#四数据清洗)
    - [4.1 缺失值处理](#41-缺失值处理)
    - [4.2 异常值处理](#42-异常值处理)
    - [4.3 重复数据处理](#43-重复数据处理)
    - [4.4 格式标准化](#44-格式标准化)
  - [五、数据质量监控](#五数据质量监控)
    - [5.1 质量指标监控](#51-质量指标监控)
    - [5.2 质量报告生成](#52-质量报告生成)
    - [5.3 质量告警](#53-质量告警)
    - [6.2 自定义函数](#62-自定义函数)
    - [6.3 外部工具集成](#63-外部工具集成)
  - [六、AI辅助数据质量管理 🆕](#六ai辅助数据质量管理-)
    - [6.4 AI辅助异常检测](#64-ai辅助异常检测)
    - [6.5 自动化数据质量规则生成](#65-自动化数据质量规则生成)
    - [6.6 数据质量可解释性](#66-数据质量可解释性)
  - [七、实践案例](#七实践案例)
    - [7.1 数据质量检查流程](#71-数据质量检查流程)
    - [7.2 自动化数据清洗](#72-自动化数据清洗)
    - [7.3 数据质量报告](#73-数据质量报告)
  - [九、最佳实践](#九最佳实践)
  - [十、参考资源](#十参考资源)
    - [9.1 官方文档](#91-官方文档)
    - [9.2 相关文档](#92-相关文档)
    - [9.3 外部资源](#93-外部资源)
  - [十一、交叉引用](#十一交叉引用)
    - [相关文档](#相关文档)
    - [外部资源](#外部资源)

---

## 一、概述

### 1.1 数据质量概念

**数据质量**是指数据满足特定用途的程度，包括数据的准确性、完整性、一致性、及时性等特征。

**数据质量的重要性**：

- **决策支持**：高质量数据支持准确决策
- **业务价值**：数据质量直接影响业务价值
- **合规要求**：满足法规和合规要求
- **成本控制**：减少数据质量问题带来的成本

### 1.2 数据质量维度

**数据质量六大维度**：

1. **完整性（Completeness）**：数据是否完整
2. **准确性（Accuracy）**：数据是否准确
3. **一致性（Consistency）**：数据是否一致
4. **及时性（Timeliness）**：数据是否及时
5. **有效性（Validity）**：数据是否符合规则
6. **唯一性（Uniqueness）**：数据是否唯一

### 1.3 PostgreSQL数据质量优势

**PostgreSQL在数据质量管理中的优势**：

- ✅ **强大的约束系统**：支持多种约束保证数据质量
- ✅ **触发器支持**：支持数据质量检查触发器
- ✅ **函数支持**：支持自定义数据质量检查函数
- ✅ **规则支持**：支持数据质量规则
- ✅ **审计支持**：支持数据变更审计

### 1.4 版本要求

- **PostgreSQL 18.x**（推荐）- 功能完整
- **PostgreSQL 17.x**（推荐）- 功能完整
- **PostgreSQL 16.x**（兼容）- 基础功能支持

---

## 二、数据质量维度

### 2.1 完整性（Completeness）

**完整性**指数据的完整程度。

```sql
-- 完整性检查：必填字段
SELECT
    'users' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE username IS NULL) AS null_username,
    COUNT(*) FILTER (WHERE email IS NULL) AS null_email,
    COUNT(*) FILTER (WHERE username IS NULL OR email IS NULL) AS incomplete_rows,
    (COUNT(*) FILTER (WHERE username IS NULL OR email IS NULL) * 100.0 / COUNT(*)) AS incompleteness_rate
FROM users;

-- 完整性检查：关联完整性
SELECT
    'orders' AS table_name,
    COUNT(*) AS total_orders,
    COUNT(*) FILTER (WHERE user_id NOT IN (SELECT user_id FROM users)) AS orphan_orders,
    (COUNT(*) FILTER (WHERE user_id NOT IN (SELECT user_id FROM users)) * 100.0 / COUNT(*)) AS orphan_rate
FROM orders;
```

### 2.2 准确性（Accuracy）

**准确性**指数据的准确程度。

```sql
-- 准确性检查：数值范围
SELECT
    'orders' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE amount < 0) AS negative_amount,
    COUNT(*) FILTER (WHERE amount > 1000000) AS excessive_amount,
    COUNT(*) FILTER (WHERE amount < 0 OR amount > 1000000) AS inaccurate_rows,
    (COUNT(*) FILTER (WHERE amount < 0 OR amount > 1000000) * 100.0 / COUNT(*)) AS inaccuracy_rate
FROM orders;

-- 准确性检查：逻辑一致性
SELECT
    'order_items' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE quantity <= 0) AS invalid_quantity,
    COUNT(*) FILTER (WHERE unit_price <= 0) AS invalid_price,
    COUNT(*) FILTER (WHERE subtotal != quantity * unit_price) AS calculation_error
FROM order_items;
```

### 2.3 一致性（Consistency）

**一致性**指数据的一致性。

```sql
-- 一致性检查：跨表一致性
SELECT
    'orders vs order_items' AS check_name,
    o.order_id,
    o.total_amount AS order_total,
    SUM(oi.subtotal) AS items_total,
    ABS(o.total_amount - SUM(oi.subtotal)) AS difference
FROM orders o
JOIN order_items oi ON oi.order_id = o.order_id
GROUP BY o.order_id, o.total_amount
HAVING ABS(o.total_amount - SUM(oi.subtotal)) > 0.01;

-- 一致性检查：时间一致性
SELECT
    'orders' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE created_at > updated_at) AS time_inconsistency,
    COUNT(*) FILTER (WHERE order_date > CURRENT_DATE) AS future_date
FROM orders;
```

### 2.4 及时性（Timeliness）

**及时性**指数据的及时程度。

```sql
-- 及时性检查：数据更新延迟
SELECT
    'orders' AS table_name,
    COUNT(*) AS total_rows,
    AVG(EXTRACT(EPOCH FROM (NOW() - updated_at))) / 3600 AS avg_update_delay_hours,
    MAX(EXTRACT(EPOCH FROM (NOW() - updated_at))) / 3600 AS max_update_delay_hours,
    COUNT(*) FILTER (WHERE updated_at < NOW() - INTERVAL '7 days') AS stale_rows
FROM orders;

-- 及时性检查：数据同步延迟
SELECT
    'replication' AS check_name,
    application_name,
    pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) AS lag_bytes,
    pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) / 1024 / 1024 AS lag_mb
FROM pg_stat_replication;
```

### 2.5 有效性（Validity）

**有效性**指数据是否符合规则。

```sql
-- 有效性检查：格式验证
SELECT
    'users' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE email !~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$') AS invalid_email,
    COUNT(*) FILTER (WHERE phone !~ '^[0-9]{11}$') AS invalid_phone
FROM users;

-- 有效性检查：枚举值验证
SELECT
    'orders' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) FILTER (WHERE status NOT IN ('pending', 'paid', 'shipped', 'delivered', 'cancelled')) AS invalid_status
FROM orders;
```

### 2.6 唯一性（Uniqueness）

**唯一性**指数据的唯一程度。

```sql
-- 唯一性检查：重复数据检测
SELECT
    'users' AS table_name,
    COUNT(*) AS total_rows,
    COUNT(*) - COUNT(DISTINCT email) AS duplicate_emails,
    COUNT(*) - COUNT(DISTINCT username) AS duplicate_usernames
FROM users;

-- 唯一性检查：重复记录检测
SELECT
    order_id,
    user_id,
    order_date,
    COUNT(*) AS duplicate_count
FROM orders
GROUP BY order_id, user_id, order_date
HAVING COUNT(*) > 1;
```

---

## 三、数据质量检测

### 3.1 完整性检测

**完整性检测函数**：

```sql
-- 完整性检测函数
CREATE OR REPLACE FUNCTION check_completeness(
    p_table_name TEXT,
    p_required_columns TEXT[]
)
RETURNS TABLE (
    column_name TEXT,
    total_rows BIGINT,
    null_count BIGINT,
    completeness_rate NUMERIC
) AS $$
DECLARE
    v_col TEXT;
    v_sql TEXT;
BEGIN
    FOREACH v_col IN ARRAY p_required_columns
    LOOP
        v_sql := format('
            SELECT
                %L AS column_name,
                COUNT(*) AS total_rows,
                COUNT(*) FILTER (WHERE %I IS NULL) AS null_count,
                (COUNT(*) FILTER (WHERE %I IS NULL) * 100.0 / COUNT(*)) AS completeness_rate
            FROM %I',
            v_col, v_col, v_col, p_table_name);

        RETURN QUERY EXECUTE v_sql;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM check_completeness('users', ARRAY['username', 'email', 'phone']);
```

### 3.2 准确性检测

**准确性检测函数**：

```sql
-- 准确性检测函数
CREATE OR REPLACE FUNCTION check_accuracy(
    p_table_name TEXT,
    p_column_name TEXT,
    p_min_value NUMERIC DEFAULT NULL,
    p_max_value NUMERIC DEFAULT NULL
)
RETURNS TABLE (
    total_rows BIGINT,
    accurate_rows BIGINT,
    inaccurate_rows BIGINT,
    accuracy_rate NUMERIC
) AS $$
DECLARE
    v_sql TEXT;
BEGIN
    v_sql := format('
        SELECT
            COUNT(*) AS total_rows,
            COUNT(*) FILTER (WHERE %I >= COALESCE(%L, -999999999) AND %I <= COALESCE(%L, 999999999)) AS accurate_rows,
            COUNT(*) FILTER (WHERE %I < COALESCE(%L, -999999999) OR %I > COALESCE(%L, 999999999)) AS inaccurate_rows,
            (COUNT(*) FILTER (WHERE %I >= COALESCE(%L, -999999999) AND %I <= COALESCE(%L, 999999999)) * 100.0 / COUNT(*)) AS accuracy_rate
        FROM %I',
        p_column_name, p_min_value, p_column_name, p_max_value,
        p_column_name, p_min_value, p_column_name, p_max_value,
        p_column_name, p_min_value, p_column_name, p_max_value,
        p_table_name);

    RETURN QUERY EXECUTE v_sql;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM check_accuracy('orders', 'amount', 0, 1000000);
```

### 3.3 一致性检测

**一致性检测函数**：

```sql
-- 一致性检测函数：订单总金额一致性
-- 订单一致性检查函数（带完整错误处理）
CREATE OR REPLACE FUNCTION check_order_consistency()
RETURNS TABLE (
    order_id BIGINT,
    order_total NUMERIC,
    items_total NUMERIC,
    difference NUMERIC,
    is_consistent BOOLEAN
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION 'orders表不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
        RAISE EXCEPTION 'order_items表不存在';
    END IF;

    -- 返回不一致的订单
    BEGIN
        RETURN QUERY
        SELECT
            o.order_id,
            o.total_amount AS order_total,
            COALESCE(SUM(oi.subtotal), 0) AS items_total,
            ABS(o.total_amount - COALESCE(SUM(oi.subtotal), 0)) AS difference,
            ABS(o.total_amount - COALESCE(SUM(oi.subtotal), 0)) < 0.01 AS is_consistent
        FROM orders o
        LEFT JOIN order_items oi ON oi.order_id = o.order_id
        WHERE o.total_amount IS NOT NULL
        GROUP BY o.order_id, o.total_amount
        HAVING ABS(o.total_amount - COALESCE(SUM(oi.subtotal), 0)) >= 0.01
        ORDER BY ABS(o.total_amount - COALESCE(SUM(oi.subtotal), 0)) DESC;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'orders或order_items表不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '检查订单一致性失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'check_order_consistency执行失败: %', SQLERRM;
END;
$$;
```

### 3.4 有效性检测

**有效性检测函数**：

```sql
-- 有效性检测函数：邮箱格式（带完整错误处理）
CREATE OR REPLACE FUNCTION check_email_validity()
RETURNS TABLE (
    user_id BIGINT,
    email TEXT,
    is_valid BOOLEAN
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
        RAISE EXCEPTION 'users表不存在';
    END IF;

    -- 返回无效邮箱记录
    BEGIN
        RETURN QUERY
        SELECT
            u.user_id,
            u.email,
            CASE
                WHEN u.email ~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN TRUE
                ELSE FALSE
            END AS is_valid
        FROM users u
        WHERE u.email IS NOT NULL
          AND length(trim(u.email)) > 0
          AND u.email !~ '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'
        ORDER BY u.user_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'users表不存在';
        WHEN invalid_regular_expression THEN
            RAISE EXCEPTION '邮箱格式正则表达式无效';
        WHEN OTHERS THEN
            RAISE EXCEPTION '检查邮箱有效性失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'check_email_validity执行失败: %', SQLERRM;
END;
$$;
```

---

## 四、数据清洗

### 4.1 缺失值处理

**缺失值处理策略**：

```sql
-- 策略1：删除缺失值
DELETE FROM users
WHERE username IS NULL OR email IS NULL;

-- 策略2：填充默认值
UPDATE users
SET phone = '00000000000'
WHERE phone IS NULL;

-- 策略3：填充平均值
UPDATE orders
SET amount = (SELECT AVG(amount) FROM orders WHERE amount IS NOT NULL)
WHERE amount IS NULL;

-- 策略4：填充中位数
UPDATE orders
SET amount = (
    SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY amount)
    FROM orders
    WHERE amount IS NOT NULL
)
WHERE amount IS NULL;

-- 策略5：使用前一个值填充（时间序列）
UPDATE metrics m1
SET value = (
    SELECT value
    FROM metrics m2
    WHERE m2.metric_name = m1.metric_name
      AND m2.timestamp < m1.timestamp
    ORDER BY m2.timestamp DESC
    LIMIT 1
)
WHERE m1.value IS NULL;
```

### 4.2 异常值处理

**异常值处理策略**：

```sql
-- 异常值检测：使用IQR方法
WITH stats AS (
    SELECT
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY amount) AS q1,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY amount) AS q3,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY amount) -
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY amount) AS iqr
    FROM orders
    WHERE amount IS NOT NULL
)
SELECT
    o.order_id,
    o.amount,
    CASE
        WHEN o.amount < stats.q1 - 1.5 * stats.iqr THEN 'Low Outlier'
        WHEN o.amount > stats.q3 + 1.5 * stats.iqr THEN 'High Outlier'
        ELSE 'Normal'
    END AS outlier_type
FROM orders o, stats
WHERE o.amount IS NOT NULL;

-- 异常值处理：删除异常值
DELETE FROM orders
WHERE amount < (
    SELECT PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY amount) -
           1.5 * (PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY amount) -
                  PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY amount))
    FROM orders
)
OR amount > (
    SELECT PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY amount) +
           1.5 * (PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY amount) -
                  PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY amount))
    FROM orders
);

-- 异常值处理：限制异常值
UPDATE orders
SET amount = LEAST(GREATEST(amount, 0), 1000000)
WHERE amount < 0 OR amount > 1000000;
```

### 4.3 重复数据处理

**重复数据处理策略**：

```sql
-- 重复数据检测
SELECT
    email,
    COUNT(*) AS duplicate_count,
    ARRAY_AGG(user_id ORDER BY created_at) AS user_ids
FROM users
GROUP BY email
HAVING COUNT(*) > 1;

-- 重复数据处理：保留最新记录
DELETE FROM users u1
WHERE EXISTS (
    SELECT 1 FROM users u2
    WHERE u2.email = u1.email
      AND u2.created_at > u1.created_at
);

-- 重复数据处理：合并重复记录
WITH duplicates AS (
    SELECT
        email,
        MIN(user_id) AS keep_user_id,
        ARRAY_AGG(user_id ORDER BY created_at) AS all_user_ids
    FROM users
    GROUP BY email
    HAVING COUNT(*) > 1
)
UPDATE orders o
SET user_id = d.keep_user_id
FROM duplicates d
WHERE o.user_id = ANY(d.all_user_ids[2:]);
```

### 4.4 格式标准化

**格式标准化处理**：

```sql
-- 格式标准化：去除空格
UPDATE users
SET username = TRIM(username),
    email = LOWER(TRIM(email));

-- 格式标准化：统一日期格式
UPDATE orders
SET order_date = DATE(order_date);

-- 格式标准化：统一状态格式
UPDATE orders
SET status = UPPER(TRIM(status));

-- 格式标准化：统一电话号码格式
UPDATE users
SET phone = REGEXP_REPLACE(phone, '[^0-9]', '', 'g')
WHERE phone IS NOT NULL;
```

---

## 五、数据质量监控

### 5.1 质量指标监控

**质量指标监控表**：

```sql
-- 数据质量指标表
CREATE TABLE data_quality_metrics (
    metric_id BIGSERIAL PRIMARY KEY,
    table_name TEXT NOT NULL,
    metric_name TEXT NOT NULL,
    metric_type VARCHAR(50) NOT NULL,  -- completeness, accuracy, consistency, etc.
    metric_value NUMERIC NOT NULL,
    threshold NUMERIC,
    status VARCHAR(20) NOT NULL,  -- pass, warning, fail
    check_time TIMESTAMPTZ DEFAULT NOW() NOT NULL,
    details JSONB
);

CREATE INDEX idx_dqm_table_time ON data_quality_metrics(table_name, check_time DESC);

-- 质量指标监控函数
CREATE OR REPLACE FUNCTION monitor_data_quality()
RETURNS INTEGER AS $$
DECLARE
    v_metric_count INTEGER := 0;
    v_completeness_rate NUMERIC;
    v_accuracy_rate NUMERIC;
BEGIN
    -- 监控完整性
    SELECT
        (COUNT(*) FILTER (WHERE username IS NOT NULL AND email IS NOT NULL) * 100.0 / COUNT(*))
    INTO v_completeness_rate
    FROM users;

    INSERT INTO data_quality_metrics (
        table_name, metric_name, metric_type, metric_value, threshold, status
    ) VALUES (
        'users', 'completeness', 'completeness', v_completeness_rate, 95.0,
        CASE
            WHEN v_completeness_rate >= 95.0 THEN 'pass'
            WHEN v_completeness_rate >= 90.0 THEN 'warning'
            ELSE 'fail'
        END
    );
    v_metric_count := v_metric_count + 1;

    -- 监控准确性
    SELECT
        (COUNT(*) FILTER (WHERE amount >= 0 AND amount <= 1000000) * 100.0 / COUNT(*))
    INTO v_accuracy_rate
    FROM orders;

    INSERT INTO data_quality_metrics (
        table_name, metric_name, metric_type, metric_value, threshold, status
    ) VALUES (
        'orders', 'accuracy', 'accuracy', v_accuracy_rate, 99.0,
        CASE
            WHEN v_accuracy_rate >= 99.0 THEN 'pass'
            WHEN v_accuracy_rate >= 95.0 THEN 'warning'
            ELSE 'fail'
        END
    );
    v_metric_count := v_metric_count + 1;

    RETURN v_metric_count;
END;
$$ LANGUAGE plpgsql;
```

### 5.2 质量报告生成

**质量报告生成函数**：

```sql
-- 质量报告生成函数
CREATE OR REPLACE FUNCTION generate_quality_report(
    p_table_name TEXT DEFAULT NULL,
    p_start_date TIMESTAMPTZ DEFAULT NOW() - INTERVAL '7 days',
    p_end_date TIMESTAMPTZ DEFAULT NOW()
)
RETURNS TABLE (
    table_name TEXT,
    metric_type TEXT,
    avg_metric_value NUMERIC,
    min_metric_value NUMERIC,
    max_metric_value NUMERIC,
    pass_count BIGINT,
    warning_count BIGINT,
    fail_count BIGINT,
    overall_status TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        dqm.table_name,
        dqm.metric_type,
        AVG(dqm.metric_value) AS avg_metric_value,
        MIN(dqm.metric_value) AS min_metric_value,
        MAX(dqm.metric_value) AS max_metric_value,
        COUNT(*) FILTER (WHERE dqm.status = 'pass') AS pass_count,
        COUNT(*) FILTER (WHERE dqm.status = 'warning') AS warning_count,
        COUNT(*) FILTER (WHERE dqm.status = 'fail') AS fail_count,
        CASE
            WHEN COUNT(*) FILTER (WHERE dqm.status = 'fail') > 0 THEN 'fail'
            WHEN COUNT(*) FILTER (WHERE dqm.status = 'warning') > 0 THEN 'warning'
            ELSE 'pass'
        END AS overall_status
    FROM data_quality_metrics dqm
    WHERE (p_table_name IS NULL OR dqm.table_name = p_table_name)
      AND dqm.check_time BETWEEN p_start_date AND p_end_date
    GROUP BY dqm.table_name, dqm.metric_type
    ORDER BY dqm.table_name, dqm.metric_type;
END;
$$ LANGUAGE plpgsql;
```

### 5.3 质量告警

**质量告警函数**：

```sql
-- 质量告警函数（带完整错误处理）
CREATE OR REPLACE FUNCTION check_quality_alerts(
    p_alert_window INTERVAL DEFAULT INTERVAL '1 hour'
)
RETURNS TABLE (
    alert_id BIGINT,
    table_name TEXT,
    metric_name TEXT,
    metric_value NUMERIC,
    threshold NUMERIC,
    status TEXT,
    alert_time TIMESTAMPTZ
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- 参数验证
    IF p_alert_window IS NULL OR p_alert_window <= INTERVAL '0' THEN
        RAISE EXCEPTION '告警时间窗口无效: %', p_alert_window;
    END IF;

    IF p_alert_window > INTERVAL '30 days' THEN
        RAISE EXCEPTION '告警时间窗口过大: % (最大30天)', p_alert_window;
    END IF;

    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_quality_metrics') THEN
        RAISE EXCEPTION 'data_quality_metrics表不存在，无法查询告警';
    END IF;

    -- 返回质量告警
    BEGIN
        RETURN QUERY
        SELECT
            dqm.metric_id,
            dqm.table_name,
            dqm.metric_name,
            dqm.metric_value,
            dqm.threshold,
            dqm.status,
            dqm.check_time
        FROM data_quality_metrics dqm
        WHERE dqm.status IN ('warning', 'fail')
          AND dqm.check_time >= NOW() - p_alert_window
          AND dqm.table_name IS NOT NULL
          AND dqm.metric_name IS NOT NULL
        ORDER BY
            CASE dqm.status
                WHEN 'fail' THEN 1
                WHEN 'warning' THEN 2
                ELSE 3
            END,
            dqm.check_time DESC;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'data_quality_metrics表不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '查询质量告警失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'check_quality_alerts执行失败: %', SQLERRM;
END;
$$;

---

## 六、数据质量工具

### 6.1 内置工具

**PostgreSQL内置数据质量工具**：

```sql
-- 1. 约束：保证数据质量
CREATE TABLE orders (
    order_id BIGSERIAL PRIMARY KEY,
    amount NUMERIC(10,2) NOT NULL CHECK (amount >= 0),
    status VARCHAR(20) NOT NULL CHECK (status IN ('pending', 'paid', 'shipped', 'delivered', 'cancelled'))
);

-- 2. 触发器：数据质量检查（带完整错误处理）
CREATE OR REPLACE FUNCTION check_order_quality()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    -- 检查NEW记录是否存在
    IF NEW IS NULL THEN
        RAISE EXCEPTION 'NEW记录为空，无法检查订单质量';
    END IF;

    -- 验证订单金额
    IF NEW.amount IS NULL THEN
        RAISE EXCEPTION '订单金额不能为空';
    END IF;

    IF NEW.amount < 0 THEN
        RAISE EXCEPTION '订单金额不能为负数: %', NEW.amount;
    END IF;

    IF NEW.amount > 1000000000 THEN
        RAISE EXCEPTION '订单金额超出限制: % (最大1000000000)', NEW.amount;
    END IF;

    -- 验证订单状态
    IF NEW.status IS NULL THEN
        RAISE EXCEPTION '订单状态不能为空';
    END IF;

    IF NEW.status NOT IN ('pending', 'paid', 'shipped', 'delivered', 'cancelled') THEN
        RAISE EXCEPTION '无效的订单状态: % (允许值: pending, paid, shipped, delivered, cancelled)', NEW.status;
    END IF;

    -- 自动设置时间戳
    IF TG_OP = 'INSERT' THEN
        IF NEW.created_at IS NULL THEN
            NEW.created_at := NOW();
        END IF;
    END IF;

    IF TG_OP IN ('INSERT', 'UPDATE') THEN
        NEW.updated_at := NOW();
    END IF;

    RETURN NEW;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '订单质量检查失败: %', SQLERRM;
END;
$$;

-- 创建触发器（带错误处理）
DO $$
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION 'orders表不存在，无法创建触发器';
    END IF;

    -- 检查触发器是否存在
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_check_order_quality') THEN
        DROP TRIGGER trigger_check_order_quality ON orders;
        RAISE NOTICE '已删除现有触发器: trigger_check_order_quality';
    END IF;

    -- 创建触发器
    CREATE TRIGGER trigger_check_order_quality
        BEFORE INSERT OR UPDATE ON orders
        FOR EACH ROW
        EXECUTE FUNCTION check_order_quality();

    RAISE NOTICE '触发器创建成功: trigger_check_order_quality';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION '触发器函数check_order_quality不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建触发器失败: %', SQLERRM;
END;
$$;

-- 3. 规则：数据质量规则
CREATE RULE check_order_amount AS
ON INSERT TO orders
WHERE amount < 0
DO INSTEAD NOTHING;
```

### 6.2 自定义函数

**自定义数据质量检查函数**：

```sql
-- 综合数据质量检查函数
CREATE OR REPLACE FUNCTION comprehensive_quality_check(p_table_name TEXT)
RETURNS TABLE (
    quality_dimension TEXT,
    metric_name TEXT,
    metric_value NUMERIC,
    threshold NUMERIC,
    status TEXT,
    details TEXT
) AS $$
BEGIN
    RETURN QUERY
    -- 完整性检查
    SELECT
        'Completeness'::TEXT,
        'Required Fields'::TEXT,
        (COUNT(*) FILTER (WHERE username IS NOT NULL AND email IS NOT NULL) * 100.0 / COUNT(*)),
        95.0,
        CASE
            WHEN (COUNT(*) FILTER (WHERE username IS NOT NULL AND email IS NOT NULL) * 100.0 / COUNT(*)) >= 95.0 THEN 'pass'
            ELSE 'fail'
        END,
        format('完整性: %.2f%%', (COUNT(*) FILTER (WHERE username IS NOT NULL AND email IS NOT NULL) * 100.0 / COUNT(*)))
    FROM users
    WHERE p_table_name = 'users'
    UNION ALL
    -- 准确性检查
    SELECT
        'Accuracy'::TEXT,
        'Value Range'::TEXT,
        (COUNT(*) FILTER (WHERE amount >= 0 AND amount <= 1000000) * 100.0 / COUNT(*)),
        99.0,
        CASE
            WHEN (COUNT(*) FILTER (WHERE amount >= 0 AND amount <= 1000000) * 100.0 / COUNT(*)) >= 99.0 THEN 'pass'
            ELSE 'fail'
        END,
        format('准确性: %.2f%%', (COUNT(*) FILTER (WHERE amount >= 0 AND amount <= 1000000) * 100.0 / COUNT(*)))
    FROM orders
    WHERE p_table_name = 'orders';
END;
$$ LANGUAGE plpgsql;
```

### 6.3 外部工具集成

**外部数据质量工具集成**：

```sql
-- 集成Great Expectations
-- 使用Python调用Great Expectations API

-- 集成Apache Griffin
-- 使用REST API调用Griffin数据质量服务

-- 集成自定义数据质量服务
-- 使用HTTP扩展调用外部服务
CREATE EXTENSION IF NOT EXISTS http;

CREATE OR REPLACE FUNCTION call_quality_service(p_data JSONB)
RETURNS JSONB AS $$
DECLARE
    v_response JSONB;
BEGIN
    SELECT content::jsonb INTO v_response
    FROM http((
        'POST',
        'http://quality-service:8000/check',
        ARRAY[http_header('Content-Type', 'application/json')],
        'application/json',
        p_data::text
    )::http_request);

    RETURN v_response;
END;
$$ LANGUAGE plpgsql;
```

---

## 六、AI辅助数据质量管理 🆕

### 6.4 AI辅助异常检测

**AI辅助异常检测**利用机器学习技术自动识别数据异常，提高数据质量检测的准确性和效率。

**PostgreSQL中的AI异常检测**：

```sql
-- 1. 统计异常检测：基于统计分布
CREATE OR REPLACE FUNCTION detect_statistical_anomalies(
    p_table_name TEXT,
    p_column_name TEXT,
    p_threshold NUMERIC DEFAULT 3.0  -- 3倍标准差
)
RETURNS TABLE (
    row_id BIGINT,
    column_value NUMERIC,
    z_score NUMERIC,
    is_anomaly BOOLEAN
) AS $$
DECLARE
    v_mean NUMERIC;
    v_stddev NUMERIC;
BEGIN
    -- 计算均值和标准差
    EXECUTE format('
        SELECT AVG(%I), STDDEV(%I)
        INTO v_mean, v_stddev
        FROM %I
        WHERE %I IS NOT NULL
    ', p_column_name, p_column_name, p_table_name, p_column_name);

    -- 检测异常值
    RETURN QUERY
    EXECUTE format('
        SELECT
            ctid::text::bigint AS row_id,
            %I AS column_value,
            ABS((%I - %L) / NULLIF(%L, 0)) AS z_score,
            ABS((%I - %L) / NULLIF(%L, 0)) > %L AS is_anomaly
        FROM %I
        WHERE %I IS NOT NULL
        ORDER BY z_score DESC
    ', p_column_name, p_column_name, v_mean, v_stddev,
       p_column_name, v_mean, v_stddev, p_threshold,
       p_table_name, p_column_name);
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM detect_statistical_anomalies('orders', 'amount', 3.0)
WHERE is_anomaly = TRUE;

-- 2. 模式异常检测：基于数据模式
CREATE OR REPLACE FUNCTION detect_pattern_anomalies(
    p_table_name TEXT,
    p_column_name TEXT
)
RETURNS TABLE (
    pattern TEXT,
    pattern_count BIGINT,
    anomaly_score NUMERIC,
    is_anomaly BOOLEAN
) AS $$
BEGIN
    RETURN QUERY
    EXECUTE format('
        WITH pattern_stats AS (
            SELECT
                %I::TEXT AS pattern,
                COUNT(*) AS pattern_count,
                COUNT(*) * 100.0 / SUM(COUNT(*)) OVER () AS pattern_percentage
            FROM %I
            WHERE %I IS NOT NULL
            GROUP BY %I
        ),
        avg_pattern_count AS (
            SELECT AVG(pattern_count) AS avg_count
            FROM pattern_stats
        )
        SELECT
            ps.pattern,
            ps.pattern_count,
            ABS(ps.pattern_count - apc.avg_count) / NULLIF(apc.avg_count, 0) AS anomaly_score,
            ABS(ps.pattern_count - apc.avg_count) / NULLIF(apc.avg_count, 0) > 2.0 AS is_anomaly
        FROM pattern_stats ps
        CROSS JOIN avg_pattern_count apc
        ORDER BY anomaly_score DESC
    ', p_column_name, p_table_name, p_column_name, p_column_name);
END;
$$ LANGUAGE plpgsql;

-- 3. 时间序列异常检测：基于时间序列模式
CREATE OR REPLACE FUNCTION detect_timeseries_anomalies(
    p_table_name TEXT,
    p_time_column TEXT,
    p_value_column TEXT,
    p_window_size INTEGER DEFAULT 7  -- 7天窗口
)
RETURNS TABLE (
    time_point TIMESTAMPTZ,
    value NUMERIC,
    expected_value NUMERIC,
    deviation NUMERIC,
    is_anomaly BOOLEAN
) AS $$
BEGIN
    RETURN QUERY
    EXECUTE format('
        WITH time_series AS (
            SELECT
                %I AS time_point,
                %I AS value,
                AVG(%I) OVER (
                    ORDER BY %I
                    ROWS BETWEEN %s PRECEDING AND 1 PRECEDING
                ) AS expected_value,
                STDDEV(%I) OVER (
                    ORDER BY %I
                    ROWS BETWEEN %s PRECEDING AND 1 PRECEDING
                ) AS stddev_value
            FROM %I
            ORDER BY %I
        )
        SELECT
            time_point,
            value,
            expected_value,
            ABS(value - expected_value) AS deviation,
            ABS(value - expected_value) > (2 * COALESCE(stddev_value, 0)) AS is_anomaly
        FROM time_series
        WHERE expected_value IS NOT NULL
        ORDER BY time_point
    ', p_time_column, p_value_column, p_value_column, p_time_column,
       p_window_size, p_value_column, p_time_column, p_window_size,
       p_table_name, p_time_column);
END;
$$ LANGUAGE plpgsql;

-- 使用示例：检测订单金额异常
SELECT * FROM detect_timeseries_anomalies('orders', 'order_date', 'amount', 7)
WHERE is_anomaly = TRUE;
```

### 6.5 自动化数据质量规则生成

**自动化规则生成**基于数据特征和历史模式自动生成数据质量规则。

**PostgreSQL中的自动化规则生成**：

```sql
-- 1. 自动生成完整性规则
CREATE OR REPLACE FUNCTION generate_completeness_rules(p_table_name TEXT)
RETURNS TABLE (
    column_name TEXT,
    null_percentage NUMERIC,
    suggested_rule TEXT,
    rule_type TEXT
) AS $$
BEGIN
    RETURN QUERY
    EXECUTE format('
        SELECT
            column_name::TEXT,
            (null_count * 100.0 / total_count) AS null_percentage,
            CASE
                WHEN null_count = 0 THEN format(''ALTER TABLE %s ALTER COLUMN %%s SET NOT NULL'', %L)
                WHEN null_count * 100.0 / total_count < 5 THEN format(''建议设置NOT NULL约束: %%s'', column_name)
                ELSE format(''建议检查数据完整性: %%s (空值比例: %.2f%%)'', column_name, null_count * 100.0 / total_count)
            END AS suggested_rule,
            CASE
                WHEN null_count = 0 THEN ''NOT NULL Constraint''
                WHEN null_count * 100.0 / total_count < 5 THEN ''Warning''
                ELSE ''Investigation Required''
            END AS rule_type
        FROM (
            SELECT
                column_name,
                COUNT(*) FILTER (WHERE column_value IS NULL) AS null_count,
                COUNT(*) AS total_count
            FROM (
                SELECT unnest(ARRAY[''column1'', ''column2'']) AS column_name,
                       unnest(ARRAY[column1, column2]) AS column_value
                FROM %I
            ) t
            GROUP BY column_name
        ) stats
    ', p_table_name, p_table_name);
END;
$$ LANGUAGE plpgsql;

-- 2. 自动生成范围规则
CREATE OR REPLACE FUNCTION generate_range_rules(
    p_table_name TEXT,
    p_column_name TEXT
)
RETURNS TABLE (
    min_value NUMERIC,
    max_value NUMERIC,
    avg_value NUMERIC,
    stddev_value NUMERIC,
    suggested_min NUMERIC,
    suggested_max NUMERIC,
    suggested_rule TEXT
) AS $$
BEGIN
    RETURN QUERY
    EXECUTE format('
        WITH stats AS (
            SELECT
                MIN(%I) AS min_val,
                MAX(%I) AS max_val,
                AVG(%I) AS avg_val,
                STDDEV(%I) AS stddev_val
            FROM %I
            WHERE %I IS NOT NULL
        )
        SELECT
            min_val,
            max_val,
            avg_val,
            stddev_val,
            GREATEST(avg_val - 3 * stddev_val, min_val) AS suggested_min,
            LEAST(avg_val + 3 * stddev_val, max_val) AS suggested_max,
            format(''CHECK (%s BETWEEN %%s AND %%s)'', %L, GREATEST(avg_val - 3 * stddev_val, min_val), LEAST(avg_val + 3 * stddev_val, max_val)) AS suggested_rule
        FROM stats
    ', p_column_name, p_column_name, p_column_name, p_column_name,
       p_table_name, p_column_name, p_column_name);
END;
$$ LANGUAGE plpgsql;

-- 3. 自动生成格式规则
CREATE OR REPLACE FUNCTION generate_format_rules(
    p_table_name TEXT,
    p_column_name TEXT
)
RETURNS TABLE (
    pattern TEXT,
    pattern_count BIGINT,
    pattern_percentage NUMERIC,
    suggested_rule TEXT
) AS $$
BEGIN
    RETURN QUERY
    EXECUTE format('
        WITH pattern_analysis AS (
            SELECT
                CASE
                    WHEN %I ~ ''^[0-9]+$'' THEN ''Numeric''
                    WHEN %I ~ ''^[A-Za-z]+$'' THEN ''Alphabetic''
                    WHEN %I ~ ''^[A-Za-z0-9]+$'' THEN ''Alphanumeric''
                    WHEN %I ~ ''^[0-9]{4}-[0-9]{2}-[0-9]{2}$'' THEN ''Date (YYYY-MM-DD)''
                    WHEN %I ~ ''^[0-9]{3}-[0-9]{3}-[0-9]{4}$'' THEN ''Phone (XXX-XXX-XXXX)''
                    WHEN %I ~ ''^[A-Za-z0-9._%%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$'' THEN ''Email''
                    ELSE ''Other''
                END AS pattern,
                COUNT(*) AS pattern_count
            FROM %I
            WHERE %I IS NOT NULL
            GROUP BY pattern
        )
        SELECT
            pattern::TEXT,
            pattern_count,
            (pattern_count * 100.0 / SUM(pattern_count) OVER ()) AS pattern_percentage,
            CASE
                WHEN pattern_percentage > 90 THEN format(''建议统一格式: %%s (占比: %.2f%%)'', pattern, pattern_percentage)
                ELSE format(''格式不一致: %%s (占比: %.2f%%)'', pattern, pattern_percentage)
            END AS suggested_rule
        FROM pattern_analysis
        ORDER BY pattern_count DESC
    ', p_column_name, p_column_name, p_column_name, p_column_name,
       p_column_name, p_column_name, p_table_name, p_column_name);
END;
$$ LANGUAGE plpgsql;
```

### 6.6 数据质量可解释性

**数据质量可解释性**提供数据质量问题的详细解释和修复建议。

**PostgreSQL中的数据质量可解释性**：

```sql
-- 1. 数据质量问题解释函数（带完整错误处理和参数验证）
CREATE OR REPLACE FUNCTION explain_quality_issue(
    p_table_name TEXT,
    p_column_name TEXT,
    p_issue_type TEXT
)
RETURNS TABLE (
    issue_description TEXT,
    root_cause TEXT,
    impact_assessment TEXT,
    recommended_action TEXT,
    priority TEXT
)
LANGUAGE plpgsql
AS $$
BEGIN
    -- 参数验证
    IF p_table_name IS NULL OR length(trim(p_table_name)) = 0 THEN
        RAISE EXCEPTION '表名不能为空';
    END IF;

    IF p_column_name IS NULL OR length(trim(p_column_name)) = 0 THEN
        RAISE EXCEPTION '列名不能为空';
    END IF;

    IF p_issue_type IS NULL OR length(trim(p_issue_type)) = 0 THEN
        RAISE EXCEPTION '问题类型不能为空';
    END IF;

    -- 验证问题类型
    IF p_issue_type NOT IN ('completeness', 'accuracy', 'consistency', 'validity', 'timeliness') THEN
        RAISE WARNING '未知的问题类型: %，使用默认处理', p_issue_type;
    END IF;

    -- 返回问题解释
    BEGIN
        RETURN QUERY
        SELECT
            CASE p_issue_type
                WHEN 'completeness' THEN format('表 %s 的列 %s 存在数据完整性问题', p_table_name, p_column_name)
                WHEN 'accuracy' THEN format('表 %s 的列 %s 存在数据准确性问题', p_table_name, p_column_name)
                WHEN 'consistency' THEN format('表 %s 的列 %s 存在数据一致性问题', p_table_name, p_column_name)
                WHEN 'validity' THEN format('表 %s 的列 %s 存在数据有效性问题', p_table_name, p_column_name)
                WHEN 'timeliness' THEN format('表 %s 的列 %s 存在数据及时性问题', p_table_name, p_column_name)
                ELSE format('表 %s 的列 %s 存在数据质量问题', p_table_name, p_column_name)
            END AS issue_description,
            CASE p_issue_type
                WHEN 'completeness' THEN '数据源缺失、ETL处理错误、业务规则未执行'
                WHEN 'accuracy' THEN '数据输入错误、计算错误、数据转换错误'
                WHEN 'consistency' THEN '数据同步延迟、多数据源不一致、业务规则冲突'
                WHEN 'validity' THEN '数据格式错误、范围超限、违反业务规则'
                WHEN 'timeliness' THEN '数据更新延迟、同步失败、处理超时'
                ELSE '未知原因'
            END AS root_cause,
            CASE p_issue_type
                WHEN 'completeness' THEN '影响数据分析和报表准确性，可能导致决策错误'
                WHEN 'accuracy' THEN '影响业务指标计算，可能导致财务损失'
                WHEN 'consistency' THEN '影响数据可信度，可能导致业务冲突'
                WHEN 'validity' THEN '影响业务流程，可能导致系统错误'
                WHEN 'timeliness' THEN '影响实时决策，可能导致业务延误'
                ELSE '影响数据整体质量'
            END AS impact_assessment,
            CASE p_issue_type
                WHEN 'completeness' THEN '1. 检查数据源完整性 2. 验证ETL流程 3. 添加NOT NULL约束'
                WHEN 'accuracy' THEN '1. 验证数据输入 2. 检查计算逻辑 3. 添加CHECK约束'
                WHEN 'consistency' THEN '1. 同步数据源 2. 统一业务规则 3. 添加外键约束'
                WHEN 'validity' THEN '1. 验证数据格式 2. 检查数据范围 3. 添加验证规则'
                WHEN 'timeliness' THEN '1. 优化数据同步 2. 检查处理流程 3. 设置超时机制'
                ELSE '1. 分析问题原因 2. 制定修复计划 3. 实施质量改进'
            END AS recommended_action,
            CASE p_issue_type
                WHEN 'completeness' THEN 'High'
                WHEN 'accuracy' THEN 'Critical'
                WHEN 'consistency' THEN 'Medium'
                WHEN 'validity' THEN 'High'
                WHEN 'timeliness' THEN 'Medium'
                ELSE 'Medium'
            END AS priority;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '生成问题解释失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'explain_quality_issue执行失败: %', SQLERRM;
END;
$$;

-- 2. 数据质量修复建议函数
CREATE OR REPLACE FUNCTION suggest_quality_fixes(
    p_table_name TEXT,
    p_column_name TEXT
)
RETURNS TABLE (
    fix_type TEXT,
    fix_description TEXT,
    fix_sql TEXT,
    estimated_impact TEXT,
    risk_level TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        'Constraint'::TEXT AS fix_type,
        format('为列 %s 添加NOT NULL约束', p_column_name) AS fix_description,
        format('ALTER TABLE %s ALTER COLUMN %s SET NOT NULL;', p_table_name, p_column_name) AS fix_sql,
        '防止未来插入空值，但需要先清理现有空值' AS estimated_impact,
        'Medium'::TEXT AS risk_level
    UNION ALL
    SELECT
        'Data Cleaning'::TEXT AS fix_type,
        format('清理列 %s 中的异常值', p_column_name) AS fix_description,
        format('UPDATE %s SET %s = NULL WHERE %s < 0 OR %s > 1000000;',
               p_table_name, p_column_name, p_column_name, p_column_name) AS fix_sql,
        '清理异常数据，但可能丢失部分信息' AS estimated_impact,
        'High'::TEXT AS risk_level
    UNION ALL
    SELECT
        'Validation'::TEXT AS fix_type,
        format('为列 %s 添加CHECK约束', p_column_name) AS fix_description,
        format('ALTER TABLE %s ADD CONSTRAINT check_%s CHECK (%s >= 0 AND %s <= 1000000);',
               p_table_name, p_column_name, p_column_name, p_column_name) AS fix_sql,
        '防止未来插入无效数据' AS estimated_impact,
        'Low'::TEXT AS risk_level;
END;
$$ LANGUAGE plpgsql;
```

**AI辅助数据质量管理优势**：

- **自动化检测**：自动识别数据异常和质量问题
- **智能规则生成**：基于数据特征自动生成质量规则
- **可解释性**：提供详细的问题解释和修复建议
- **效率提升**：减少手动检查和分析时间

---

## 七、实践案例

### 7.1 数据质量检查流程

**完整的数据质量检查流程**：

```sql
-- 1. 创建质量检查表
CREATE TABLE quality_check_results (
    check_id BIGSERIAL PRIMARY KEY,
    table_name TEXT NOT NULL,
    check_type TEXT NOT NULL,
    check_result JSONB NOT NULL,
    check_time TIMESTAMPTZ DEFAULT NOW() NOT NULL
);

-- 2. 执行质量检查
-- 执行质量检查函数（带完整错误处理）
CREATE OR REPLACE FUNCTION run_quality_checks()
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
    v_check_count INTEGER := 0;
    v_result JSONB;
    v_total_users INTEGER;
    v_valid_users INTEGER;
    v_total_orders INTEGER;
    v_valid_orders INTEGER;
BEGIN
    -- 检查质量检查结果表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'quality_check_results') THEN
        RAISE EXCEPTION 'quality_check_results表不存在，无法记录检查结果';
    END IF;

    -- 检查完整性
    BEGIN
        -- 检查users表是否存在
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            SELECT COUNT(*) INTO v_total_users FROM users;
            SELECT COUNT(*) INTO v_valid_users
            FROM users
            WHERE username IS NOT NULL AND email IS NOT NULL;

            IF v_total_users > 0 THEN
                v_result := jsonb_build_object(
                    'completeness_rate', ROUND((v_valid_users * 100.0 / v_total_users)::NUMERIC, 2),
                    'total_count', v_total_users,
                    'valid_count', v_valid_users,
                    'status', CASE
                        WHEN (v_valid_users * 100.0 / v_total_users) >= 95.0 THEN 'pass'
                        ELSE 'fail'
                    END
                );

                INSERT INTO quality_check_results (table_name, check_type, check_result)
                VALUES ('users', 'completeness', v_result);
                v_check_count := v_check_count + 1;
            ELSE
                RAISE NOTICE 'users表为空，跳过完整性检查';
            END IF;
        ELSE
            RAISE WARNING 'users表不存在，跳过完整性检查';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '完整性检查失败: %', SQLERRM;
    END;

    -- 检查准确性
    BEGIN
        -- 检查orders表是否存在
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            SELECT COUNT(*) INTO v_total_orders FROM orders;
            SELECT COUNT(*) INTO v_valid_orders
            FROM orders
            WHERE amount >= 0 AND amount <= 1000000;

            IF v_total_orders > 0 THEN
                v_result := jsonb_build_object(
                    'accuracy_rate', ROUND((v_valid_orders * 100.0 / v_total_orders)::NUMERIC, 2),
                    'total_count', v_total_orders,
                    'valid_count', v_valid_orders,
                    'status', CASE
                        WHEN (v_valid_orders * 100.0 / v_total_orders) >= 99.0 THEN 'pass'
                        ELSE 'fail'
                    END
                );

                INSERT INTO quality_check_results (table_name, check_type, check_result)
                VALUES ('orders', 'accuracy', v_result);
                v_check_count := v_check_count + 1;
            ELSE
                RAISE NOTICE 'orders表为空，跳过准确性检查';
            END IF;
        ELSE
            RAISE WARNING 'orders表不存在，跳过准确性检查';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '准确性检查失败: %', SQLERRM;
    END;

    IF v_check_count = 0 THEN
        RAISE WARNING '未执行任何质量检查';
    END IF;

    RETURN v_check_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'run_quality_checks执行失败: %', SQLERRM;
END;
$$;
```

### 7.2 自动化数据清洗

**自动化数据清洗流程**：

```sql
-- 自动化数据清洗函数
-- 自动化数据清洗函数（带完整错误处理）
CREATE OR REPLACE FUNCTION auto_clean_data()
RETURNS TABLE (
    table_name TEXT,
    cleaned_count INTEGER,
    error_count INTEGER
)
LANGUAGE plpgsql
AS $$
DECLARE
    v_cleaned_count INTEGER := 0;
    v_error_count INTEGER := 0;
BEGIN
    -- 清洗users表
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            -- 更新清洗数据
            UPDATE users
            SET username = TRIM(username),
                email = LOWER(TRIM(email)),
                phone = CASE
                    WHEN phone IS NOT NULL THEN REGEXP_REPLACE(phone, '[^0-9]', '', 'g')
                    ELSE phone
                END
            WHERE (username IS NOT NULL AND username != TRIM(username))
               OR (email IS NOT NULL AND email != LOWER(TRIM(email)))
               OR (phone IS NOT NULL AND phone != REGEXP_REPLACE(phone, '[^0-9]', '', 'g'));
            GET DIAGNOSTICS v_cleaned_count = ROW_COUNT;

            -- 删除无效数据
            DELETE FROM users
            WHERE username IS NULL OR length(trim(username)) = 0
               OR email IS NULL OR length(trim(email)) = 0;
            GET DIAGNOSTICS v_error_count = ROW_COUNT;

            RETURN QUERY SELECT 'users'::TEXT, v_cleaned_count, v_error_count;
        ELSE
            RAISE WARNING 'users表不存在，跳过清洗';
        END IF;
    EXCEPTION
        WHEN invalid_regular_expression THEN
            RAISE WARNING '清洗users表时正则表达式错误: %', SQLERRM;
            RETURN QUERY SELECT 'users'::TEXT, 0, 0;
        WHEN OTHERS THEN
            RAISE WARNING '清洗users表失败: %', SQLERRM;
            RETURN QUERY SELECT 'users'::TEXT, 0, 0;
    END;

    -- 清洗orders表
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            -- 修正负数金额
            UPDATE orders
            SET amount = GREATEST(amount, 0)
            WHERE amount < 0;
            GET DIAGNOSTICS v_cleaned_count = ROW_COUNT;

            -- 删除无效数据
            DELETE FROM orders
            WHERE amount < 0
               OR (order_date IS NOT NULL AND order_date > CURRENT_DATE);
            GET DIAGNOSTICS v_error_count = ROW_COUNT;

            RETURN QUERY SELECT 'orders'::TEXT, v_cleaned_count, v_error_count;
        ELSE
            RAISE WARNING 'orders表不存在，跳过清洗';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '清洗orders表失败: %', SQLERRM;
            RETURN QUERY SELECT 'orders'::TEXT, 0, 0;
    END;

EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'auto_clean_data执行失败: %', SQLERRM;
END;
$$;
```

### 7.3 数据质量报告

**数据质量报告生成**：

```sql
-- 数据质量报告
CREATE OR REPLACE FUNCTION generate_quality_report()
RETURNS TABLE (
    table_name TEXT,
    total_checks INTEGER,
    pass_count INTEGER,
    warning_count INTEGER,
    fail_count INTEGER,
    overall_score NUMERIC,
    report_time TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        qcr.table_name,
        COUNT(*) AS total_checks,
        COUNT(*) FILTER (WHERE qcr.check_result->>'status' = 'pass') AS pass_count,
        COUNT(*) FILTER (WHERE qcr.check_result->>'status' = 'warning') AS warning_count,
        COUNT(*) FILTER (WHERE qcr.check_result->>'status' = 'fail') AS fail_count,
        (COUNT(*) FILTER (WHERE qcr.check_result->>'status' = 'pass') * 100.0 / COUNT(*)) AS overall_score,
        MAX(qcr.check_time) AS report_time
    FROM quality_check_results qcr
    WHERE qcr.check_time >= NOW() - INTERVAL '1 day'
    GROUP BY qcr.table_name
    ORDER BY overall_score DESC;
END;
$$ LANGUAGE plpgsql;
```

---

## 九、最佳实践

1. **数据质量设计**：
   - 在数据库设计阶段考虑数据质量
   - 使用约束保证数据质量
   - 设计数据质量检查流程

2. **数据质量监控**：
   - 定期执行数据质量检查
   - 记录数据质量指标
   - 设置数据质量告警

3. **数据清洗**：
   - 建立数据清洗流程
   - 记录清洗操作日志
   - 验证清洗结果

4. **持续改进**：
   - 分析数据质量问题
   - 优化数据质量规则
   - 提升数据质量水平

---

## 十、参考资源

### 9.1 官方文档

- [PostgreSQL约束](https://www.postgresql.org/docs/current/ddl-constraints.html)
- [PostgreSQL触发器](https://www.postgresql.org/docs/current/triggers.html)
- [PostgreSQL规则](https://www.postgresql.org/docs/current/sql-createrule.html)

### 9.2 相关文档

- [ETL流程完整指南](./09.04-ETL流程完整指南.md) - ETL流程中的数据质量
- [数据建模完整指南](./09.02-数据建模完整指南.md) - 数据建模中的数据质量考虑
- [数据分析完整指南](./09.01-数据分析完整指南.md) - 数据分析中的数据质量

### 9.3 外部资源

- [数据质量管理最佳实践](https://www.postgresql.org/docs/current/)
- [Great Expectations文档](https://docs.greatexpectations.io/)
- [Apache Griffin文档](https://griffin.apache.org/)

---

## 十一、交叉引用

### 相关文档

- ⭐⭐⭐ [ETL流程完整指南](./09.04-ETL流程完整指南.md) - ETL流程中的数据质量
- ⭐⭐⭐ [数据建模完整指南](./09.02-数据建模完整指南.md) - 数据建模中的数据质量考虑
- ⭐⭐ [数据分析完整指南](./09.01-数据分析完整指南.md) - 数据分析中的数据质量
- ⭐⭐ [数据仓库设计指南](./09.03-数据仓库设计指南.md) - 数据仓库中的数据质量
- ⭐ [数据科学实践](../应用架构/07.06-数据科学实践.md) - 数据科学中的数据质量

### 外部资源

- [PostgreSQL约束文档](https://www.postgresql.org/docs/current/ddl-constraints.html)
- [PostgreSQL触发器文档](https://www.postgresql.org/docs/current/triggers.html)
- [PostgreSQL规则文档](https://www.postgresql.org/docs/current/sql-createrule.html)

---

**文档版本**: v1.1
**最后更新**: 2025-11-22
**PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
**维护者**: Documentation Team

```
