...
| **EXPLAIN (ANALYZE, BUFFERS, TIMING)** | 性能调试 | ✅ 性能分析 ❌ 仅查询 | 🟢 推荐 |
...
BEGIN
    ...
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'function_debug_log') THEN
        RAISE WARNING 'function_debug_log表不存在，无法记录日志';
        RETURN;
    END IF;

    -- 验证日志级别
    IF p_level NOT IN ('DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL') THEN
        RAISE WARNING '无效的日志级别: %, 使用默认值INFO', p_level;
        p_level := 'INFO';
    END IF;

    -- 检查消息长度（防止过长消息）
    IF LENGTH(p_message) > 10000 THEN
        RAISE WARNING '日志消息过长，已截断: % 字符', LENGTH(p_message);
        p_message := LEFT(p_message, 10000);
    END IF;
    ...
    EXCEPTION
        WHEN unique_violation THEN
            RAISE WARNING '日志记录已存在，跳过';
        WHEN foreign_key_violation THEN
            RAISE WARNING '违反外键约束，无法记录日志';
        WHEN check_violation THEN
            RAISE WARNING '违反检查约束，无法记录日志';
        WHEN OTHERS THEN
            RAISE WARNING '记录调试日志失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        -- 日志函数失败不应该影响主流程，只记录警告
        RAISE WARNING 'log_debug执行失败: %', SQLERRM;
END;
$$;
```
