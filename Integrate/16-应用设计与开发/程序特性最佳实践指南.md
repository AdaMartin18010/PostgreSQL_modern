...
BEGIN
    ...
    FROM user_settings
    WHERE user_id = get_user_setting.user_id
      AND key = setting_key;
...
CREATE OR REPLACE FUNCTION safe_parallel_function(value INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
...
CREATE OR REPLACE FUNCTION unsafe_parallel_function(value INTEGER)
RETURNS VOID
LANGUAGE plpgsql
VOLATILE
...
BEGIN
    -- 写操作
    INSERT INTO log_table (value) VALUES (value);
END;
$$;
```
