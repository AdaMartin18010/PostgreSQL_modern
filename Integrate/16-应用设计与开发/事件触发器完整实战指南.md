---
> **📋 文档来源**: 新增深化文档
> **📅 创建日期**: 2025-01
> **⚠️ 注意**: 本文档聚焦事件触发器（Event Triggers）完整技术栈

---

# PostgreSQL事件触发器完整实战指南

## 元数据

- **文档版本**: v2.0
- **创建日期**: 2025-01
- **技术栈**: PostgreSQL 17+/18+ | PL/pgSQL | pg_event_trigger
- **难度级别**: ⭐⭐⭐⭐⭐ (专家级)
- **预计阅读**: 120分钟
- **前置要求**: 熟悉PostgreSQL基础、触发器基础、DDL操作

---

## 📋 完整目录

- [PostgreSQL事件触发器完整实战指南](#postgresql事件触发器完整实战指南)
  - [元数据](#元数据)
  - [📋 完整目录](#-完整目录)
  - [1. 事件触发器概述](#1-事件触发器概述)
    - [1.1 事件触发器概念](#11-事件触发器概念)
      - [核心概念](#核心概念)
      - [事件触发器特点](#事件触发器特点)
    - [1.2 事件触发器vs普通触发器](#12-事件触发器vs普通触发器)
      - [对比表](#对比表)
    - [1.3 事件触发器应用场景](#13-事件触发器应用场景)
      - [典型应用场景](#典型应用场景)
  - [2. DDL事件触发器](#2-ddl事件触发器)
    - [2.1 DDL事件类型](#21-ddl事件类型)
      - [支持的事件](#支持的事件)
      - [事件触发时机](#事件触发时机)
    - [2.2 创建DDL事件触发器](#22-创建ddl事件触发器)
      - [基本创建语法](#基本创建语法)
      - [高级事件触发器函数](#高级事件触发器函数)
    - [2.3 获取DDL命令信息](#23-获取ddl命令信息)
      - [pg\_event\_trigger\_ddl\_commands()](#pg_event_trigger_ddl_commands)
      - [pg\_event\_trigger\_dropped\_objects()](#pg_event_trigger_dropped_objects)
    - [2.4 DDL事件触发器应用](#24-ddl事件触发器应用)
      - [条件触发](#条件触发)
  - [3. 数据库级事件触发器](#3-数据库级事件触发器)
    - [3.1 数据库事件类型](#31-数据库事件类型)
      - [PostgreSQL 15+支持的数据库事件](#postgresql-15支持的数据库事件)
    - [3.2 登录/登出事件触发器](#32-登录登出事件触发器)
      - [登录事件触发器](#登录事件触发器)
  - [4. 审计日志实现](#4-审计日志实现)
    - [4.1 DDL审计日志](#41-ddl审计日志)
      - [完整DDL审计系统](#完整ddl审计系统)
      - [审计日志查询](#审计日志查询)
  - [5. 安全策略实施](#5-安全策略实施)
    - [5.1 DDL权限控制](#51-ddl权限控制)
      - [监控敏感操作](#监控敏感操作)
    - [5.2 对象命名规范](#52-对象命名规范)
      - [命名规范检查](#命名规范检查)
  - [6. 性能监控](#6-性能监控)
    - [6.1 DDL操作监控](#61-ddl操作监控)
      - [DDL性能监控](#ddl性能监控)
  - [7. 实战案例](#7-实战案例)
    - [7.1 完整DDL审计系统](#71-完整ddl审计系统)
      - [系统架构](#系统架构)
  - [📚 参考资源](#-参考资源)
  - [📝 更新日志](#-更新日志)

---

## 1. 事件触发器概述

### 1.1 事件触发器概念

#### 核心概念

```text
事件触发器（Event Trigger）:
- 在数据库级别捕获DDL事件
- 响应CREATE、DROP、ALTER等操作
- 在操作执行前或后触发
- 可以阻止或记录DDL操作
```

#### 事件触发器特点

- ✅ **数据库级别**: 作用于整个数据库，不是特定表
- ✅ **DDL事件**: 捕获数据定义语言（DDL）事件
- ✅ **无法阻止**: 在事件发生后触发，无法阻止操作
- ✅ **信息访问**: 可以访问DDL命令的详细信息
- ✅ **安全性**: 需要超级用户权限创建

### 1.2 事件触发器vs普通触发器

#### 对比表

| 特性 | 事件触发器 | 普通触发器 |
| --- | --- | --- |
| **作用级别** | 数据库级别 | 表级别 |
| **触发事件** | DDL事件（CREATE、DROP、ALTER） | DML事件（INSERT、UPDATE、DELETE） |
| **是否可以阻止** | 不能阻止（在事件后触发） | 可以阻止（BEFORE触发器） |
| **信息访问** | `pg_event_trigger_ddl_commands()` | OLD/NEW记录 |
| **创建权限** | 需要超级用户 | 表所有者 |
| **使用场景** | 审计、监控、日志记录 | 数据验证、业务逻辑 |

### 1.3 事件触发器应用场景

#### 典型应用场景

```text
场景1: DDL审计日志
- 记录所有DDL操作
- 记录操作时间、用户、命令
- 生成审计报告

场景2: 数据库变更管理
- 跟踪数据库结构变更
- 记录变更历史
- 生成变更报告

场景3: 安全策略实施
- 监控敏感操作
- 记录安全事件
- 生成安全报告

场景4: 性能监控
- 监控DDL操作性能
- 记录长时间运行的操作
- 生成性能报告
```

---

## 2. DDL事件触发器

### 2.1 DDL事件类型

#### 支持的事件

```sql
-- DDL事件类型
CREATE TABLE event_types (
    event_type TEXT PRIMARY KEY,
    description TEXT
);

INSERT INTO event_types VALUES
('ddl_command_start', 'DDL命令执行前触发'),
('ddl_command_end', 'DDL命令执行后触发'),
('sql_drop', 'DROP命令执行后触发'),
('table_rewrite', '表重写操作时触发');
```

#### 事件触发时机

```text
ddl_command_start:
- 在DDL命令执行前触发
- 可以访问命令信息
- 无法阻止命令执行

ddl_command_end:
- 在DDL命令执行后触发
- 可以访问命令结果
- 通常用于日志记录

sql_drop:
- 在DROP命令执行后触发
- 可以访问被删除对象信息
- 通常用于审计和恢复

table_rewrite:
- 在表重写操作时触发
- 例如：ALTER TABLE ... SET DATA TYPE
- 通常用于性能监控
```

### 2.2 创建DDL事件触发器

#### 基本创建语法

```sql
-- 创建事件触发器函数
CREATE OR REPLACE FUNCTION log_ddl_command()
RETURNS event_trigger AS $$
BEGIN
    RAISE NOTICE 'DDL command executed';
END;
$$ LANGUAGE plpgsql;

-- 创建事件触发器
CREATE EVENT TRIGGER ddl_log_trigger
ON ddl_command_end
EXECUTE FUNCTION log_ddl_command();
```

#### 高级事件触发器函数

```sql
-- 获取DDL命令信息的事件触发器函数（带完整错误处理）
CREATE OR REPLACE FUNCTION audit_ddl_command()
RETURNS event_trigger
LANGUAGE plpgsql
AS $$
DECLARE
    r RECORD;
    v_command_text TEXT;
    v_inserted_count INTEGER := 0;
BEGIN
    -- 检查审计日志表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ddl_audit_log') THEN
        RAISE WARNING 'ddl_audit_log表不存在，跳过DDL审计记录';
        RETURN;
    END IF;

    -- 获取DDL命令信息并记录
    BEGIN
        FOR r IN
            SELECT * FROM pg_event_trigger_ddl_commands()
        LOOP
            BEGIN
                v_command_text := r.object_identity;

                -- 记录审计日志
                INSERT INTO ddl_audit_log (
                    event_time,
                    event_type,
                    command_tag,
                    object_type,
                    object_identity,
                    schema_name,
                    object_name,
                    username
                ) VALUES (
                    NOW(),
                    tg_event,
                    tg_tag,
            r.object_type,
            r.object_identity,
            r.schema_name,
            r.object_name,
            current_user
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建审计事件触发器
CREATE EVENT TRIGGER audit_ddl_trigger
ON ddl_command_end
EXECUTE FUNCTION audit_ddl_command();

-- 创建审计日志表
CREATE TABLE ddl_audit_log (
    id BIGSERIAL PRIMARY KEY,
    event_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    event_type TEXT NOT NULL,
    command_tag TEXT,
    object_type TEXT,
    object_identity TEXT,
    schema_name TEXT,
    object_name TEXT,
    username TEXT,
    command_text TEXT
);

CREATE INDEX idx_ddl_audit_log_time ON ddl_audit_log(event_time);
CREATE INDEX idx_ddl_audit_log_user ON ddl_audit_log(username);
CREATE INDEX idx_ddl_audit_log_object ON ddl_audit_log(object_identity);
```

### 2.3 获取DDL命令信息

#### pg_event_trigger_ddl_commands()

```sql
-- 使用pg_event_trigger_ddl_commands()获取命令信息
CREATE OR REPLACE FUNCTION log_ddl_details()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    cmd_text TEXT := '';
BEGIN
    FOR cmd IN
        SELECT
            object_type,
            schema_name,
            object_identity,
            in_extension,
            command_tag,
            command
        FROM pg_event_trigger_ddl_commands()
    LOOP
        -- 构建命令文本
        cmd_text := format(
            'Object: %s.%s (%s), Tag: %s, Extension: %s',
            cmd.schema_name,
            cmd.object_identity,
            cmd.object_type,
            cmd.command_tag,
            cmd.in_extension
        );

        RAISE NOTICE '%', cmd_text;

        -- 记录详细信息
        INSERT INTO ddl_audit_log (
            event_time,
            object_type,
            object_identity,
            schema_name,
            command_tag,
            command_text
        ) VALUES (
            NOW(),
            cmd.object_type,
            cmd.object_identity,
            cmd.schema_name,
            cmd.command_tag,
            cmd_text
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

#### pg_event_trigger_dropped_objects()

```sql
-- 获取被删除对象信息（用于sql_drop事件）
CREATE OR REPLACE FUNCTION log_dropped_objects()
RETURNS event_trigger AS $$
DECLARE
    obj RECORD;
BEGIN
    FOR obj IN
        SELECT
            object_type,
            schema_name,
            object_name,
            object_identity,
            address_names,
            address_args,
            original,
            normal
        FROM pg_event_trigger_dropped_objects()
    LOOP
        -- 记录删除的对象
        INSERT INTO drop_audit_log (
            event_time,
            object_type,
            schema_name,
            object_name,
            object_identity,
            original,
            username
        ) VALUES (
            NOW(),
            obj.object_type,
            obj.schema_name,
            obj.object_name,
            obj.object_identity,
            obj.original,
            current_user
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建DROP事件触发器
CREATE EVENT TRIGGER log_drop_trigger
ON sql_drop
EXECUTE FUNCTION log_dropped_objects();

-- 创建DROP审计日志表
CREATE TABLE drop_audit_log (
    id BIGSERIAL PRIMARY KEY,
    event_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    object_type TEXT NOT NULL,
    schema_name TEXT,
    object_name TEXT,
    object_identity TEXT,
    original BOOLEAN,
    username TEXT,
    restore_sql TEXT  -- 可用于恢复
);
```

### 2.4 DDL事件触发器应用

#### 条件触发

```sql
-- 只记录特定类型的DDL操作
CREATE OR REPLACE FUNCTION audit_table_ddl()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
BEGIN
    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
        WHERE object_type IN ('table', 'view', 'materialized view')
    LOOP
        INSERT INTO table_ddl_log (
            event_time,
            object_type,
            object_identity,
            command_tag,
            username
        ) VALUES (
            NOW(),
            cmd.object_type,
            cmd.object_identity,
            tg_tag,
            current_user
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;

CREATE EVENT TRIGGER audit_table_ddl_trigger
ON ddl_command_end
WHEN TAG IN ('CREATE TABLE', 'ALTER TABLE', 'DROP TABLE',
             'CREATE VIEW', 'ALTER VIEW', 'DROP VIEW')
EXECUTE FUNCTION audit_table_ddl();
```

---

## 3. 数据库级事件触发器

### 3.1 数据库事件类型

#### PostgreSQL 15+支持的数据库事件

```sql
-- 数据库级事件（PostgreSQL 15+）
-- login事件：用户登录时触发
-- 注意：需要pg_hba.conf配置支持
```

### 3.2 登录/登出事件触发器

#### 登录事件触发器

```sql
-- 登录事件触发器（PostgreSQL 15+）
-- 注意：此功能需要特定配置和权限

-- 创建登录审计日志表
CREATE TABLE login_audit_log (
    id BIGSERIAL PRIMARY KEY,
    login_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    username TEXT NOT NULL,
    database_name TEXT,
    application_name TEXT,
    client_addr INET,
    session_id TEXT
);

-- 创建登录事件触发器函数
CREATE OR REPLACE FUNCTION log_user_login()
RETURNS event_trigger AS $$
BEGIN
    INSERT INTO login_audit_log (
        username,
        database_name,
        application_name,
        client_addr,
        session_id
    ) VALUES (
        current_user,
        current_database(),
        current_setting('application_name', true),
        inet_client_addr(),
        pg_backend_pid()::TEXT
    );
END;
$$ LANGUAGE plpgsql;

-- 创建登录事件触发器（需要超级用户权限）
-- CREATE EVENT TRIGGER log_login_trigger
-- ON login
-- EXECUTE FUNCTION log_user_login();
```

---

## 4. 审计日志实现

### 4.1 DDL审计日志

#### 完整DDL审计系统

```sql
-- 完整的DDL审计日志系统
CREATE OR REPLACE FUNCTION comprehensive_ddl_audit()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    audit_record JSONB;
BEGIN
    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
    LOOP
        -- 构建审计记录
        audit_record := jsonb_build_object(
            'event_time', NOW(),
            'event_type', tg_event,
            'command_tag', tg_tag,
            'object_type', cmd.object_type,
            'schema_name', cmd.schema_name,
            'object_identity', cmd.object_identity,
            'username', current_user,
            'database', current_database(),
            'session_id', pg_backend_pid(),
            'transaction_id', txid_current()
        );

        -- 存储审计记录（JSONB格式，便于查询）
        INSERT INTO comprehensive_audit_log (
            event_time,
            event_type,
            audit_data
        ) VALUES (
            NOW(),
            tg_event,
            audit_record
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建综合审计日志表
CREATE TABLE comprehensive_audit_log (
    id BIGSERIAL PRIMARY KEY,
    event_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    event_type TEXT NOT NULL,
    audit_data JSONB NOT NULL
);

-- 创建GIN索引以支持JSONB查询
CREATE INDEX idx_audit_log_data ON comprehensive_audit_log USING GIN (audit_data);
CREATE INDEX idx_audit_log_time ON comprehensive_audit_log(event_time);
CREATE INDEX idx_audit_log_type ON comprehensive_audit_log(event_type);

-- 创建事件触发器
CREATE EVENT TRIGGER comprehensive_ddl_audit_trigger
ON ddl_command_end
EXECUTE FUNCTION comprehensive_ddl_audit();
```

#### 审计日志查询

```sql
-- 查询最近的表创建操作
SELECT
    event_time,
    audit_data->>'username' AS username,
    audit_data->>'object_identity' AS object,
    audit_data->>'command_tag' AS command
FROM comprehensive_audit_log
WHERE event_type = 'ddl_command_end'
  AND audit_data->>'object_type' = 'table'
  AND audit_data->>'command_tag' = 'CREATE TABLE'
ORDER BY event_time DESC
LIMIT 10;

-- 查询特定用户的所有DDL操作
SELECT
    event_time,
    audit_data->>'command_tag' AS command,
    audit_data->>'object_identity' AS object
FROM comprehensive_audit_log
WHERE audit_data->>'username' = 'admin'
ORDER BY event_time DESC;
```

---

## 5. 安全策略实施

### 5.1 DDL权限控制

#### 监控敏感操作

```sql
-- 监控敏感DDL操作
CREATE OR REPLACE FUNCTION monitor_sensitive_ddl()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    alert_level TEXT;
BEGIN
    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
    LOOP
        -- 判断敏感操作
        alert_level := CASE
            WHEN cmd.object_type = 'role' AND tg_tag LIKE 'CREATE%' THEN 'CRITICAL'
            WHEN cmd.object_type = 'database' THEN 'HIGH'
            WHEN cmd.schema_name = 'public' AND cmd.object_type = 'table' THEN 'MEDIUM'
            ELSE 'LOW'
        END;

        -- 记录敏感操作
        IF alert_level IN ('CRITICAL', 'HIGH') THEN
            INSERT INTO security_alert_log (
                alert_time,
                alert_level,
                object_type,
                object_identity,
                command_tag,
                username,
                action_taken
            ) VALUES (
                NOW(),
                alert_level,
                cmd.object_type,
                cmd.object_identity,
                tg_tag,
                current_user,
                'LOGGED'
            );

            -- 发送通知（可以使用pg_notify）
            PERFORM pg_notify('security_alert', jsonb_build_object(
                'level', alert_level,
                'object', cmd.object_identity,
                'command', tg_tag,
                'user', current_user
            )::TEXT);
        END IF;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建安全告警日志表
CREATE TABLE security_alert_log (
    id BIGSERIAL PRIMARY KEY,
    alert_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    alert_level TEXT NOT NULL,
    object_type TEXT,
    object_identity TEXT,
    command_tag TEXT,
    username TEXT,
    action_taken TEXT
);

-- 创建事件触发器
CREATE EVENT TRIGGER security_monitor_trigger
ON ddl_command_end
EXECUTE FUNCTION monitor_sensitive_ddl();
```

### 5.2 对象命名规范

#### 命名规范检查

```sql
-- 检查对象命名规范
CREATE OR REPLACE FUNCTION check_naming_convention()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    object_name TEXT;
    violation TEXT;
BEGIN
    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
        WHERE object_type IN ('table', 'view', 'function', 'procedure')
    LOOP
        object_name := cmd.object_name;

        -- 检查命名规范（示例：必须以字母开头，只能包含字母、数字、下划线）
        IF NOT (object_name ~ '^[a-z][a-z0-9_]*$') THEN
            violation := format(
                'Naming convention violation: %s does not match pattern ^[a-z][a-z0-9_]*$',
                object_name
            );

            -- 记录违规
            INSERT INTO naming_violation_log (
                violation_time,
                object_type,
                object_name,
                object_identity,
                violation_reason,
                username
            ) VALUES (
                NOW(),
                cmd.object_type,
                object_name,
                cmd.object_identity,
                violation,
                current_user
            );

            -- 可以选择抛出异常阻止创建
            -- RAISE EXCEPTION '%', violation;
        END IF;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建命名规范违规日志表
CREATE TABLE naming_violation_log (
    id BIGSERIAL PRIMARY KEY,
    violation_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    object_type TEXT NOT NULL,
    object_name TEXT NOT NULL,
    object_identity TEXT,
    violation_reason TEXT,
    username TEXT
);

-- 创建事件触发器
CREATE EVENT TRIGGER naming_convention_trigger
ON ddl_command_end
EXECUTE FUNCTION check_naming_convention();
```

---

## 6. 性能监控

### 6.1 DDL操作监控

#### DDL性能监控

```sql
-- DDL操作性能监控
CREATE OR REPLACE FUNCTION monitor_ddl_performance()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    start_time TIMESTAMPTZ;
    duration INTERVAL;
BEGIN
    -- 获取操作开始时间（需要配合ddl_command_start触发器）
    SELECT MAX(event_time) INTO start_time
    FROM ddl_performance_log
    WHERE command_id = txid_current();

    -- 如果没有开始时间记录，使用当前时间
    IF start_time IS NULL THEN
        start_time := NOW();
    END IF;

    duration := NOW() - start_time;

    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
    LOOP
        -- 记录性能数据
        INSERT INTO ddl_performance_log (
            event_time,
            command_tag,
            object_type,
            object_identity,
            duration,
            username
        ) VALUES (
            NOW(),
            tg_tag,
            cmd.object_type,
            cmd.object_identity,
            duration,
            current_user
        );

        -- 如果操作时间过长，记录告警
        IF duration > INTERVAL '1 minute' THEN
            INSERT INTO slow_ddl_alert (
                alert_time,
                object_identity,
                command_tag,
                duration,
                username
            ) VALUES (
                NOW(),
                cmd.object_identity,
                tg_tag,
                duration,
                current_user
            );
        END IF;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 创建性能日志表
CREATE TABLE ddl_performance_log (
    id BIGSERIAL PRIMARY KEY,
    event_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    command_tag TEXT,
    object_type TEXT,
    object_identity TEXT,
    duration INTERVAL,
    username TEXT,
    command_id BIGINT
);

CREATE INDEX idx_ddl_perf_time ON ddl_performance_log(event_time);
CREATE INDEX idx_ddl_perf_duration ON ddl_performance_log(duration);

-- 创建慢DDL告警表
CREATE TABLE slow_ddl_alert (
    id BIGSERIAL PRIMARY KEY,
    alert_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    object_identity TEXT,
    command_tag TEXT,
    duration INTERVAL,
    username TEXT
);

-- 创建事件触发器
CREATE EVENT TRIGGER ddl_performance_monitor
ON ddl_command_end
EXECUTE FUNCTION monitor_ddl_performance();
```

---

## 7. 实战案例

### 7.1 完整DDL审计系统

#### 系统架构

```sql
-- 完整的DDL审计系统实现

-- 1. 创建审计日志表
CREATE TABLE ddl_audit_system (
    id BIGSERIAL PRIMARY KEY,
    event_time TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    event_type TEXT NOT NULL,
    command_tag TEXT,
    object_type TEXT,
    schema_name TEXT,
    object_name TEXT,
    object_identity TEXT,
    username TEXT NOT NULL,
    database_name TEXT NOT NULL,
    application_name TEXT,
    client_addr INET,
    command_text TEXT,
    metadata JSONB
);

-- 2. 创建综合审计函数
CREATE OR REPLACE FUNCTION comprehensive_audit_function()
RETURNS event_trigger AS $$
DECLARE
    cmd RECORD;
    audit_metadata JSONB;
BEGIN
    FOR cmd IN
        SELECT * FROM pg_event_trigger_ddl_commands()
    LOOP
        -- 构建元数据
        audit_metadata := jsonb_build_object(
            'session_id', pg_backend_pid(),
            'transaction_id', txid_current(),
            'query', current_query(),
            'in_extension', cmd.in_extension
        );

        -- 插入审计记录
        INSERT INTO ddl_audit_system (
            event_type,
            command_tag,
            object_type,
            schema_name,
            object_name,
            object_identity,
            username,
            database_name,
            application_name,
            client_addr,
            metadata
        ) VALUES (
            tg_event,
            tg_tag,
            cmd.object_type,
            cmd.schema_name,
            cmd.object_name,
            cmd.object_identity,
            current_user,
            current_database(),
            current_setting('application_name', true),
            inet_client_addr(),
            audit_metadata
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 3. 创建事件触发器
CREATE EVENT TRIGGER comprehensive_audit_trigger
ON ddl_command_end
EXECUTE FUNCTION comprehensive_audit_function();

-- 4. 创建索引
CREATE INDEX idx_audit_time ON ddl_audit_system(event_time);
CREATE INDEX idx_audit_user ON ddl_audit_system(username);
CREATE INDEX idx_audit_object ON ddl_audit_system(object_identity);
CREATE INDEX idx_audit_type ON ddl_audit_system(object_type);

-- 5. 创建审计报告视图
CREATE OR REPLACE VIEW ddl_audit_report AS
SELECT
    DATE_TRUNC('day', event_time) AS audit_date,
    username,
    object_type,
    command_tag,
    COUNT(*) AS operation_count
FROM ddl_audit_system
GROUP BY DATE_TRUNC('day', event_time), username, object_type, command_tag
ORDER BY audit_date DESC, operation_count DESC;
```

---

## 📚 参考资源

1. **PostgreSQL官方文档**: <https://www.postgresql.org/docs/current/event-triggers.html>
2. **pg_event_trigger扩展**: <https://www.postgresql.org/docs/current/functions-event-triggers.html>
3. **DDL审计最佳实践**: <https://www.postgresql.org/docs/current/event-triggers.html>

---

## 📝 更新日志

- **v2.0** (2025-01): 完整实战指南
  - 补充DDL事件触发器完整实现
  - 补充数据库级事件触发器
  - 补充审计日志实现
  - 补充安全策略实施
  - 补充性能监控
  - 补充实战案例

---

**状态**: ✅ **文档完成** | [返回目录](./README.md)
