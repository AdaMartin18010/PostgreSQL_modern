...
- [PostgreSQL性能测试完整指南](#postgresql性能测试完整指南)
  ...
  - [3. 查询性能测试](#3-查询性能测试)
    - [3.1 EXPLAIN (ANALYZE, BUFFERS, TIMING)分析](#31-explain-analyze-buffers-timing分析)
...
### 3.1 EXPLAIN (ANALYZE, BUFFERS, TIMING)分析

**EXPLAIN (ANALYZE, BUFFERS, TIMING)使用**：

```sql
-- 基本EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
-- 方法2：使用EXPLAIN (ANALYZE, BUFFERS, TIMING)
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM users WHERE email = 'test@example.com';
