---
> **📋 文档来源**: 新增深化文档
> **📅 创建日期**: 2025-01
> **⚠️ 注意**: 本文档聚焦函数高级特性技术栈

---

# PostgreSQL函数高级特性指南

## 元数据

- **文档版本**: v2.0
- **创建日期**: 2025-01
- **技术栈**: PostgreSQL 17+/18+ | PL/pgSQL | PL/Python | PL/Perl | 自定义聚合
- **难度级别**: ⭐⭐⭐⭐⭐ (专家级)
- **预计阅读**: 200分钟
- **前置要求**: 熟悉PostgreSQL基础、PL/pgSQL基础、编程基础

---

## 📋 完整目录

- [PostgreSQL函数高级特性指南](#postgresql函数高级特性指南)
  - [元数据](#元数据)
  - [📋 完整目录](#-完整目录)
  - [1. 函数概述](#1-函数概述)
    - [1.1 函数类型](#11-函数类型)
      - [函数分类](#函数分类)
    - [1.2 函数特性分类](#12-函数特性分类)
      - [特性标记](#特性标记)
  - [2. 多语言函数](#2-多语言函数)
    - [2.1 PL/Python函数](#21-plpython函数)
      - [安装与配置](#安装与配置)
      - [PL/Python基础函数](#plpython基础函数)
      - [PL/Python数据处理](#plpython数据处理)
    - [2.2 PL/Perl函数](#22-plperl函数)
      - [PL/Perl基础](#plperl基础)
  - [3. 自定义聚合函数](#3-自定义聚合函数)
    - [3.1 聚合函数基础](#31-聚合函数基础)
      - [聚合函数结构](#聚合函数结构)
    - [3.2 状态函数](#32-状态函数)
      - [状态函数实现](#状态函数实现)
    - [3.3 最终函数](#33-最终函数)
      - [最终函数实现](#最终函数实现)
    - [3.4 组合函数](#34-组合函数)
      - [组合函数实现（用于并行执行）](#组合函数实现用于并行执行)
    - [3.5 完整示例](#35-完整示例)
      - [自定义SUM聚合函数](#自定义sum聚合函数)
      - [复杂聚合函数：中位数](#复杂聚合函数中位数)
  - [4. 函数安全特性](#4-函数安全特性)
    - [4.1 SECURITY DEFINER](#41-security-definer)
      - [安全定义者函数](#安全定义者函数)
    - [4.2 SECURITY INVOKER](#42-security-invoker)
      - [安全调用者函数（默认）](#安全调用者函数默认)
  - [5. 函数稳定性与并行](#5-函数稳定性与并行)
    - [5.1 IMMUTABLE](#51-immutable)
      - [不可变函数](#不可变函数)
    - [5.2 STABLE](#52-stable)
      - [稳定函数](#稳定函数)
    - [5.3 VOLATILE](#53-volatile)
      - [易变函数（默认）](#易变函数默认)
    - [5.4 并行执行](#54-并行执行)
      - [并行安全函数](#并行安全函数)
  - [6. 函数重载与参数匹配](#6-函数重载与参数匹配)
    - [6.1 函数重载](#61-函数重载)
      - [重载示例](#重载示例)
  - [7. 实战案例](#7-实战案例)
    - [7.1 自定义统计聚合函数](#71-自定义统计聚合函数)
      - [完整统计聚合函数](#完整统计聚合函数)
  - [📚 参考资源](#-参考资源)
  - [📝 更新日志](#-更新日志)

---

## 1. 函数概述

### 1.1 函数类型

#### 函数分类

```text
按返回类型分类:
1. 标量函数 - 返回单个值
2. 表函数 - 返回表（RETURNS TABLE）
3. 集合函数 - 返回集合（RETURNS SETOF）

按语言分类:
1. SQL函数 - LANGUAGE sql
2. PL/pgSQL函数 - LANGUAGE plpgsql
3. PL/Python函数 - LANGUAGE plpython3u
4. PL/Perl函数 - LANGUAGE plperl
5. PL/Java函数 - LANGUAGE java
6. C函数 - LANGUAGE c

按特性分类:
1. IMMUTABLE - 结果只依赖参数
2. STABLE - 结果在同一事务中不变
3. VOLATILE - 结果可能变化（默认）
```

### 1.2 函数特性分类

#### 特性标记

```sql
-- IMMUTABLE: 纯函数，结果只依赖参数
CREATE OR REPLACE FUNCTION add_numbers(a INTEGER, b INTEGER)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT a + b;
$$;

-- STABLE: 结果在同一事务中不变
CREATE OR REPLACE FUNCTION get_current_time()
RETURNS TIMESTAMPTZ
LANGUAGE sql
STABLE
AS $$
    SELECT NOW();
$$;

-- VOLATILE: 结果可能变化（默认）
CREATE OR REPLACE FUNCTION get_random_number()
RETURNS INTEGER
LANGUAGE sql
VOLATILE
AS $$
    SELECT floor(random() * 100)::INTEGER;
$$;
```

---

## 2. 多语言函数

### 2.1 PL/Python函数

#### 安装与配置

```bash
# Ubuntu/Debian
sudo apt-get install postgresql-17-plpython3

# 在数据库中启用
CREATE EXTENSION IF NOT EXISTS plpython3u;
```

#### PL/Python基础函数

```sql
-- 简单的Python函数
CREATE OR REPLACE FUNCTION python_hello(name TEXT)
RETURNS TEXT
LANGUAGE plpython3u
AS $$
    return f"Hello, {name}!"
$$;

-- 使用NumPy计算
CREATE OR REPLACE FUNCTION calculate_statistics(data NUMERIC[])
RETURNS TABLE (
    mean NUMERIC,
    stddev NUMERIC,
    median NUMERIC
)
LANGUAGE plpython3u
AS $$
import numpy as np

arr = np.array(data)
return [
    (float(np.mean(arr)),
     float(np.std(arr)),
     float(np.median(arr)))
]
$$;
```

#### PL/Python数据处理

```sql
-- 使用Pandas处理数据
CREATE OR REPLACE FUNCTION analyze_table_pandas(
    table_name TEXT,
    column_name TEXT
)
RETURNS TABLE (
    metric_name TEXT,
    metric_value NUMERIC
)
LANGUAGE plpython3u
AS $$
import pandas as pd
import plpy

query = f"SELECT {column_name} FROM {table_name}"
result = plpy.execute(query)

df = pd.DataFrame([dict(row) for row in result])

stats = {
    'count': len(df),
    'mean': float(df[column_name].mean()),
    'std': float(df[column_name].std()),
    'min': float(df[column_name].min()),
    'max': float(df[column_name].max()),
    'median': float(df[column_name].median())
}

for key, value in stats.items():
    yield (key, value)
$$;
```

### 2.2 PL/Perl函数

#### PL/Perl基础

```sql
-- 启用PL/Perl
CREATE EXTENSION IF NOT EXISTS plperl;

-- 简单的Perl函数
CREATE OR REPLACE FUNCTION perl_reverse(text)
RETURNS TEXT
LANGUAGE plperl
AS $$
    return reverse($_[0]);
$$;

-- Perl数据处理
CREATE OR REPLACE FUNCTION perl_process_array(data INTEGER[])
RETURNS INTEGER[]
LANGUAGE plperl
AS $$
    my @arr = @{$_[0]};
    @arr = map { $_ * 2 } @arr;
    return \@arr;
$$;
```

---

## 3. 自定义聚合函数

### 3.1 聚合函数基础

#### 聚合函数结构

```text
聚合函数组件:
1. 状态函数（State Function） - 处理每个输入值
2. 最终函数（Final Function） - 处理最终结果
3. 初始状态（Initial State） - 初始值
4. 组合函数（Combine Function） - 组合并行执行结果（可选）
```

### 3.2 状态函数

#### 状态函数实现

```sql
-- 状态函数：累积处理每个值
CREATE OR REPLACE FUNCTION sum_state(
    state INTEGER,
    value INTEGER
)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE(state, 0) + value;
END;
$$;
```

### 3.3 最终函数

#### 最终函数实现

```sql
-- 最终函数：处理最终状态
CREATE OR REPLACE FUNCTION sum_final(state INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE(state, 0);
END;
$$;
```

### 3.4 组合函数

#### 组合函数实现（用于并行执行）

```sql
-- 组合函数：组合两个状态
CREATE OR REPLACE FUNCTION sum_combine(
    state1 INTEGER,
    state2 INTEGER
)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE(state1, 0) + COALESCE(state2, 0);
END;
$$;
```

### 3.5 完整示例

#### 自定义SUM聚合函数

```sql
-- 创建完整的自定义聚合函数
CREATE AGGREGATE custom_sum(INTEGER) (
    SFUNC = sum_state,
    STYPE = INTEGER,
    INITCOND = 0,
    FINALFUNC = sum_final,
    COMBINEFUNC = sum_combine,
    PARALLEL = SAFE
);

-- 使用示例
SELECT custom_sum(value) FROM numbers;
```

#### 复杂聚合函数：中位数

```sql
-- 状态函数：累积所有值
CREATE OR REPLACE FUNCTION median_state(
    state INTEGER[],
    value INTEGER
)
RETURNS INTEGER[]
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN array_append(COALESCE(state, ARRAY[]::INTEGER[]), value);
END;
$$;

-- 最终函数：计算中位数
CREATE OR REPLACE FUNCTION median_final(state INTEGER[])
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    sorted INTEGER[];
    mid INTEGER;
BEGIN
    IF state IS NULL OR array_length(state, 1) IS NULL THEN
        RETURN NULL;
    END IF;

    sorted := state;
    SELECT INTO sorted array_agg(val ORDER BY val) FROM unnest(state) AS val;

    mid := array_length(sorted, 1) / 2;

    IF array_length(sorted, 1) % 2 = 0 THEN
        RETURN (sorted[mid] + sorted[mid + 1]) / 2.0;
    ELSE
        RETURN sorted[mid + 1];
    END IF;
END;
$$;

-- 组合函数
CREATE OR REPLACE FUNCTION median_combine(
    state1 INTEGER[],
    state2 INTEGER[]
)
RETURNS INTEGER[]
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE(state1, ARRAY[]::INTEGER[]) || COALESCE(state2, ARRAY[]::INTEGER[]);
END;
$$;

-- 创建聚合函数
CREATE AGGREGATE custom_median(INTEGER) (
    SFUNC = median_state,
    STYPE = INTEGER[],
    INITCOND = '{}',
    FINALFUNC = median_final,
    COMBINEFUNC = median_combine,
    PARALLEL = SAFE
);

-- 使用示例
SELECT custom_median(value) FROM numbers;
```

---

## 4. 函数安全特性

### 4.1 SECURITY DEFINER

#### 安全定义者函数

```sql
-- 使用SECURITY DEFINER以函数所有者权限执行
CREATE OR REPLACE FUNCTION admin_only_operation(user_id INTEGER)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp  -- 防止搜索路径攻击
AS $$
BEGIN
    -- 检查调用者
    IF current_user != 'app_user' THEN
        RAISE EXCEPTION 'Permission denied';
    END IF;

    -- 以函数所有者（admin）权限执行
    UPDATE users SET status = 'admin' WHERE id = user_id;
END;
$$;

-- 授予执行权限
GRANT EXECUTE ON FUNCTION admin_only_operation(INTEGER) TO app_user;
```

### 4.2 SECURITY INVOKER

#### 安全调用者函数（默认）

```sql
-- SECURITY INVOKER：以调用者权限执行（默认）
CREATE OR REPLACE FUNCTION user_operation(user_id INTEGER)
RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
AS $$
BEGIN
    -- 只能操作自己的数据
    IF user_id != current_setting('app.user_id', true)::INTEGER THEN
        RAISE EXCEPTION 'Permission denied: can only access own data';
    END IF;

    UPDATE users SET last_login = NOW() WHERE id = user_id;
END;
$$;
```

---

## 5. 函数稳定性与并行

### 5.1 IMMUTABLE

#### 不可变函数

```sql
-- IMMUTABLE函数：结果只依赖参数
CREATE OR REPLACE FUNCTION calculate_tax(price NUMERIC, rate NUMERIC DEFAULT 0.1)
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN price * rate;
END;
$$;

-- 可以在索引中使用
CREATE INDEX idx_products_tax ON products (calculate_tax(price, 0.1));
```

### 5.2 STABLE

#### 稳定函数

```sql
-- STABLE函数：结果在同一事务中不变
CREATE OR REPLACE FUNCTION get_user_balance(user_id INTEGER)
RETURNS NUMERIC
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
    v_balance NUMERIC;
BEGIN
    SELECT balance INTO v_balance
    FROM accounts
    WHERE user_id = get_user_balance.user_id;

    RETURN COALESCE(v_balance, 0);
END;
$$;
```

### 5.3 VOLATILE

#### 易变函数（默认）

```sql
-- VOLATILE函数：结果可能变化
CREATE OR REPLACE FUNCTION get_random_id()
RETURNS INTEGER
LANGUAGE plpgsql
VOLATILE
AS $$
BEGIN
    RETURN floor(random() * 1000000)::INTEGER;
END;
$$;
```

### 5.4 并行执行

#### 并行安全函数

```sql
-- PARALLEL SAFE：可以并行执行
CREATE OR REPLACE FUNCTION parallel_safe_function(value INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
PARALLEL SAFE
AS $$
BEGIN
    RETURN value * 2;
END;
$$;

-- PARALLEL UNSAFE：不能并行执行（默认）
CREATE OR REPLACE FUNCTION parallel_unsafe_function()
RETURNS INTEGER
LANGUAGE plpgsql
VOLATILE
PARALLEL UNSAFE
AS $$
BEGIN
    -- 访问共享状态，不能并行
    PERFORM pg_advisory_lock(1);
    RETURN 1;
END;
$$;
```

---

## 6. 函数重载与参数匹配

### 6.1 函数重载

#### 重载示例

```sql
-- 重载函数：不同参数类型
CREATE OR REPLACE FUNCTION add_numbers(a INTEGER, b INTEGER)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN a + b;
END;
$$;

CREATE OR REPLACE FUNCTION add_numbers(a NUMERIC, b NUMERIC)
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN a + b;
END;
$$;

-- 自动选择匹配的函数
SELECT add_numbers(1, 2);        -- 使用INTEGER版本
SELECT add_numbers(1.5, 2.5);    -- 使用NUMERIC版本
```

---

## 7. 实战案例

### 7.1 自定义统计聚合函数

#### 完整统计聚合函数

```sql
-- 状态类型：累积统计信息
CREATE TYPE stats_state AS (
    count BIGINT,
    sum NUMERIC,
    sum_sq NUMERIC,
    min_val NUMERIC,
    max_val NUMERIC
);

-- 状态函数
CREATE OR REPLACE FUNCTION stats_state_func(
    state stats_state,
    value NUMERIC
)
RETURNS stats_state
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    new_state stats_state;
BEGIN
    IF state IS NULL THEN
        new_state := (1, value, value * value, value, value);
    ELSE
        new_state := (
            state.count + 1,
            state.sum + value,
            state.sum_sq + (value * value),
            LEAST(state.min_val, value),
            GREATEST(state.max_val, value)
        );
    END IF;

    RETURN new_state;
END;
$$;

-- 最终函数：计算统计指标
CREATE OR REPLACE FUNCTION stats_final_func(state stats_state)
RETURNS TABLE (
    count BIGINT,
    mean NUMERIC,
    stddev NUMERIC,
    variance NUMERIC,
    min_val NUMERIC,
    max_val NUMERIC
)
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    v_mean NUMERIC;
    v_variance NUMERIC;
BEGIN
    IF state IS NULL OR state.count = 0 THEN
        RETURN;
    END IF;

    v_mean := state.sum / state.count;
    v_variance := (state.sum_sq / state.count) - (v_mean * v_mean);

    RETURN QUERY SELECT
        state.count,
        v_mean,
        sqrt(v_variance),
        v_variance,
        state.min_val,
        state.max_val;
END;
$$;

-- 组合函数
CREATE OR REPLACE FUNCTION stats_combine_func(
    state1 stats_state,
    state2 stats_state
)
RETURNS stats_state
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF state1 IS NULL THEN
        RETURN state2;
    END IF;

    IF state2 IS NULL THEN
        RETURN state1;
    END IF;

    RETURN (
        state1.count + state2.count,
        state1.sum + state2.sum,
        state1.sum_sq + state2.sum_sq,
        LEAST(state1.min_val, state2.min_val),
        GREATEST(state1.max_val, state2.max_val)
    );
END;
$$;

-- 创建聚合函数
CREATE AGGREGATE comprehensive_stats(NUMERIC) (
    SFUNC = stats_state_func,
    STYPE = stats_state,
    FINALFUNC = stats_final_func,
    COMBINEFUNC = stats_combine_func,
    INITCOND = '(0, 0, 0, NULL, NULL)',
    PARALLEL = SAFE
);

-- 使用示例
SELECT comprehensive_stats(amount) FROM orders;
```

---

## 📚 参考资源

1. **PostgreSQL官方文档**: <https://www.postgresql.org/docs/current/xfunc.html>
2. **PL/Python文档**: <https://www.postgresql.org/docs/current/plpython.html>
3. **自定义聚合函数**: <https://www.postgresql.org/docs/current/xaggr.html>

---

## 📝 更新日志

- **v2.0** (2025-01): 高级特性指南
  - 补充多语言函数（PL/Python、PL/Perl）
  - 补充自定义聚合函数完整实现
  - 补充函数安全特性
  - 补充函数稳定性与并行执行
  - 补充函数重载
  - 补充实战案例

---

**状态**: ✅ **文档完成** | [返回目录](./README.md)
