...
CREATE OR REPLACE PROCEDURE safe_operation(
    p_name TEXT DEFAULT 'John',
    p_email TEXT DEFAULT 'john@example.com'
)
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    BEGIN
        ...
        VALUES (trim(p_name), lower(trim(p_email)), NOW(), NOW());

        RAISE NOTICE '用户创建成功: % (%)', p_name, p_email;

    EXCEPTION
        WHEN unique_violation THEN
            RAISE NOTICE '用户已存在: %', p_email;
            -- 可以执行恢复操作
            BEGIN
                UPDATE users
                SET updated_at = NOW(),
                ...
                IF FOUND THEN
                    RAISE NOTICE '用户信息已更新: %', p_email;
                ELSE
                    RAISE WARNING '用户存在但更新失败';
                END IF;
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '更新用户信息失败: %', SQLERRM;
            END;
        WHEN check_violation THEN
            RAISE EXCEPTION '数据验证失败: %', SQLERRM;
        WHEN not_null_violation THEN
            RAISE EXCEPTION '必填字段为空: %', SQLERRM;
        WHEN foreign_key_violation THEN
            RAISE EXCEPTION '外键约束违反: %', SQLERRM;
        WHEN OTHERS THEN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'safe_operation执行失败: %', SQLERRM;
END;
$$;
```
...
BEGIN
    ...
    IF p_data IS NULL OR array_length(p_data, 1) IS NULL THEN
        RAISE EXCEPTION '数据数组不能为空';
    END IF;

    IF array_length(p_data, 1) = 0 THEN
        RAISE WARNING '数据数组为空，无需处理';
        RETURN;
    END IF;

    IF array_length(p_data, 1) > 100000 THEN
        RAISE EXCEPTION '数据量过大: % (最大支持100000条)', array_length(p_data, 1);
    END IF;
    ...
    LOOP
        ...
        BEGIN
            -- 数据提取和验证
            v_user_id := (v_item->>'user_id')::INTEGER;
            ...
            IF v_user_id IS NULL OR v_user_id <= 0 THEN
                RAISE EXCEPTION 'user_id无效: %', v_item->>'user_id';
            END IF;

            IF v_amount IS NULL OR v_amount <= 0 THEN
                RAISE EXCEPTION 'amount无效: %', v_item->>'amount';
            END IF;

            IF v_product_id IS NULL OR v_product_id <= 0 THEN
                RAISE EXCEPTION 'product_id无效: %', v_item->>'product_id';
            END IF;

            IF v_order_date IS NULL OR v_order_date > CURRENT_DATE THEN
                RAISE EXCEPTION 'order_date无效: %', v_item->>'order_date';
            END IF;

            -- 执行插入
            INSERT INTO orders (user_id, amount, product_id, order_date, created_at)
            VALUES (v_user_id, v_amount, v_product_id, v_order_date, NOW());
        ...
        EXCEPTION
            WHEN unique_violation THEN
                ...
                RAISE WARNING '第%条记录插入失败（重复）: %', v_count, SQLERRM;
            WHEN foreign_key_violation THEN
                v_fail_count := v_fail_count + 1;
                RAISE WARNING '第%条记录插入失败（外键约束）: %', v_count, SQLERRM;
            WHEN check_violation THEN
                v_fail_count := v_fail_count + 1;
                RAISE WARNING '第%条记录插入失败（检查约束）: %', v_count, SQLERRM;
            WHEN invalid_text_representation THEN
                v_fail_count := v_fail_count + 1;
                RAISE WARNING '第%条记录插入失败（数据类型转换错误）: %', v_count, SQLERRM;
            WHEN OTHERS THEN
                v_fail_count := v_fail_count + 1;
                v_error_count := v_error_count + 1;
                RAISE WARNING '第%条记录插入失败: %', v_count, SQLERRM;
                -- 如果错误过多，停止处理
                IF v_error_count > 100 THEN
                    RAISE EXCEPTION '错误过多，停止批量插入。成功: %，失败: %', v_success_count, v_fail_count;
                END IF;
        END;

        -- 每1000条提交一次（注意：在PROCEDURE中不能使用COMMIT，应该在外部事务中调用）
        -- 这里改为记录进度
        IF v_count % 1000 = 0 THEN
            RAISE NOTICE '已处理 % 条记录，成功: %，失败: %', v_count, v_success_count, v_fail_count;
        END IF;
    END LOOP;

    RAISE NOTICE '批量插入完成。总计: %，成功: %，失败: %', v_count, v_success_count, v_fail_count;
