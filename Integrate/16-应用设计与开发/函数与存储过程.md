...
BEGIN
    ...
    EXCEPTION
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '计算结果超出数值范围: % * %', p_price, p_quantity;
        WHEN OTHERS THEN
...
-- 使用函数
SELECT calculate_total(100, 5);  -- 500
```
...
BEGIN
    ...
    IF p_greeting IS NULL OR length(trim(p_greeting)) = 0 THEN
        RAISE EXCEPTION '问候语不能为空';
    END IF;

    -- 字符串长度验证（防止过长的字符串）
    IF length(p_name) > 100 THEN
        RAISE EXCEPTION '名称过长（最大100字符）: %', length(p_name);
    END IF;

    IF length(p_greeting) > 50 THEN
        RAISE EXCEPTION '问候语过长（最大50字符）: %', length(p_greeting);
    END IF;

    -- 返回问候语
    BEGIN
        RETURN trim(p_greeting) || ', ' || trim(p_name) || '!';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '字符串拼接失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'greet执行失败: %', SQLERRM;
END;
$$;
...
BEGIN
    ...
    IF p_max_age > 150 THEN
        RAISE EXCEPTION '最大年龄超出合理范围: % (最大150)', p_max_age;
    END IF;
    ...
    BEGIN
        ...
        FROM users
        WHERE users.age BETWEEN p_min_age AND p_max_age
        ORDER BY users.age;
    EXCEPTION
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'get_users_by_age执行失败: %', SQLERRM;
END;
$$;
...
BEGIN
    ...
    -- 防止栈溢出：限制最大输入值
    IF p_n > 170 THEN
        RAISE EXCEPTION '阶乘参数过大，可能导致栈溢出或数值溢出: % (最大170)', p_n;
    END IF;
    ...
    ELSE
        ...
        EXCEPTION
            WHEN numeric_value_out_of_range THEN
                RAISE EXCEPTION '阶乘计算结果超出数值范围: %!', p_n;
            WHEN OTHERS THEN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'factorial执行失败: %', SQLERRM;
END;
$$;
