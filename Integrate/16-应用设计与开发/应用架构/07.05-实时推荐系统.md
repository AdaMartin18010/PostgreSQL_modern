...
> **📋 文档来源**: `PostgreSQL\09-应用设计\应用架构\07.05-实时推荐系统.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behaviors') THEN
        ...
        RAISE NOTICE '已删除现有表: user_behaviors';
    END IF;
    ...
    RAISE NOTICE '用户行为事件主表创建成功: user_behaviors';
EXCEPTION
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_behaviors') THEN
        RAISE EXCEPTION 'user_behaviors表不存在，无法创建索引';
    END IF;

    -- user_id, item_id, behavior_time复合索引
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'user_behaviors' AND indexname = 'idx_user_behaviors_user_item') THEN
        ...
        RAISE NOTICE '索引创建成功: idx_user_behaviors_user_item';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'user_behaviors' AND indexname = 'idx_user_behaviors_item') THEN
        ...
        RAISE NOTICE '索引创建成功: idx_user_behaviors_item';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'user_behaviors' AND indexname = 'idx_user_behaviors_type') THEN
        ...
        RAISE NOTICE '索引创建成功: idx_user_behaviors_type';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'user_behaviors表不存在';
...
BEGIN
    ...
    IF p_item_id IS NULL OR p_item_id <= 0 THEN
        RAISE EXCEPTION '物品ID无效: %', p_item_id;
    END IF;

    IF p_limit IS NULL OR p_limit <= 0 OR p_limit > 10000 THEN
        RAISE EXCEPTION 'limit参数无效: % (必须在1-10000之间)', p_limit;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'item_features') THEN
        RAISE EXCEPTION 'item_features表不存在';
    END IF;
    ...
    BEGIN
        ...
        IF NOT FOUND THEN
            RAISE EXCEPTION '物品不存在: %', p_item_id;
        END IF;

        IF v_item_embedding IS NULL THEN
            RAISE EXCEPTION '物品向量为空: %', p_item_id;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'item_features表不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '查询物品向量失败: %', SQLERRM;
    END;
    ...
    EXCEPTION
        WHEN undefined_function THEN
            RAISE EXCEPTION '向量操作符不可用，请检查pgvector扩展';
        WHEN OTHERS THEN
            RAISE EXCEPTION '向量相似度计算失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_features') THEN
        RAISE EXCEPTION 'user_features表不存在';
    END IF;
    ...
    BEGIN
        ...
        IF v_user_embedding IS NULL THEN
            RAISE EXCEPTION '用户向量为空: %', p_user_id;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION 'user_features表不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '查询用户向量失败: %', SQLERRM;
    END;
...
- ⭐ [AI与机器学习](../../00-归档-项目管理文档/README.md) - RAG架构实践
