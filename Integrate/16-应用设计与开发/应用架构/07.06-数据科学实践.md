...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'raw_events') THEN
        ...
        RAISE NOTICE '已删除现有表: raw_events';
    END IF;
    ...
    RAISE NOTICE '原始数据主表创建成功: raw_events';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表raw_events已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建原始数据表失败: %', SQLERRM;
END $$;
...
BEGIN
    -- 删除现有分区（如果存在）
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'raw_events_2025_01_15') THEN
        DROP TABLE raw_events_2025_01_15;
        RAISE NOTICE '已删除现有分区: raw_events_2025_01_15';
    END IF;

    -- 检查主表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'raw_events') THEN
        RAISE EXCEPTION 'raw_events主表不存在，无法创建分区';
    END IF;

    CREATE TABLE raw_events_2025_01_15 PARTITION OF raw_events
        FOR VALUES FROM ('2025-01-15') TO ('2025-01-16');

    RAISE NOTICE '分区创建成功: raw_events_2025_01_15';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'raw_events主表不存在';
    WHEN duplicate_table THEN
        RAISE WARNING '分区已存在';
    WHEN OTHERS THEN
...
BEGIN
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'raw_events') THEN
        RAISE EXCEPTION 'raw_events表不存在';
    END IF;
    ...
    BEGIN
        ...
        v_cleaned_count := v_cleaned_count + v_deleted_count;
        RAISE NOTICE '删除重复数据: % 条', v_deleted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '删除重复数据失败: %', SQLERRM;
    END;
    ...
    BEGIN
        ...
        GET DIAGNOSTICS v_deleted_count = ROW_COUNT;
        v_cleaned_count := v_cleaned_count + v_deleted_count;
        RAISE NOTICE '删除异常数据: % 条', v_deleted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '删除异常数据失败: %', SQLERRM;
    END;

    RETURN v_cleaned_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'clean_raw_events失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
```sql
-- 清洗后的数据表（带完整错误处理，支持分区）
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cleaned_events') THEN
        DROP TABLE cleaned_events CASCADE;
        RAISE NOTICE '已删除现有表: cleaned_events';
    END IF;
    ...
    RAISE NOTICE '清洗后的数据主表创建成功: cleaned_events';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表cleaned_events已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建清洗数据表失败: %', SQLERRM;
END $$;
...
BEGIN
    -- 检查源表和目标表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'raw_events') THEN
        RAISE EXCEPTION '源表raw_events不存在';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cleaned_events') THEN
        RAISE EXCEPTION '目标表cleaned_events不存在';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE 'ETL转换完成: % 条记录', v_transformed_count;
    EXCEPTION
        WHEN undefined_table THEN
        ...
        WHEN OTHERS THEN
            RAISE EXCEPTION 'ETL转换失败: %', SQLERRM;
    END;

    RETURN v_transformed_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'etl_transform_events失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_features') THEN
        ...
        RAISE NOTICE '已删除现有表: user_features';
    END IF;
    ...
    CREATE TABLE user_features (
        ...
        days_since_first_event INTEGER CHECK (days_since_first_event >= 0),
        days_since_last_event INTEGER CHECK (days_since_last_event >= 0),
        -- 行为特征
        avg_session_duration NUMERIC(10,2) CHECK (avg_session_duration >= 0),
        avg_events_per_session NUMERIC(10,2) CHECK (avg_events_per_session >= 0),
        ...
        ctr_7d NUMERIC(5,4) GENERATED ALWAYS AS (
            CASE
                ...
                ELSE 0
            END
        ) STORED CHECK (ctr_7d >= 0 AND ctr_7d <= 1),
        activity_score NUMERIC(10,2) GENERATED ALWAYS AS (
            total_clicks * 0.5 + total_purchases * 2.0
        ) STORED CHECK (activity_score >= 0),
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW()
    );

    RAISE NOTICE '用户特征表创建成功: user_features';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表user_features已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建用户特征表失败: %', SQLERRM;
END $$;
...
BEGIN
    -- activity_score索引
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'user_features' AND indexname = 'idx_user_features_activity') THEN
        CREATE INDEX idx_user_features_activity ON user_features(activity_score DESC);
        RAISE NOTICE '索引创建成功: idx_user_features_activity';
    END IF;

    -- ctr_7d索引
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'user_features' AND indexname = 'idx_user_features_ctr') THEN
        CREATE INDEX idx_user_features_ctr ON user_features(ctr_7d DESC);
        RAISE NOTICE '索引创建成功: idx_user_features_ctr';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'user_features表不存在，无法创建索引';
...
- ⭐⭐ [AI与机器学习](../../00-归档-项目管理文档/README.md) - ML集成详细说明
