...
> **📋 文档来源**: `PostgreSQL培训\16-PostgreSQL17新特性\性能诊断改进.md`
> **📅 复制日期**: 2025-12-22
...
| **EXPLAIN (ANALYZE, BUFFERS, TIMING)** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | 3.8/5 |
...
flowchart TD
    ...
    E -->|是| I[EXPLAIN (ANALYZE, BUFFERS, TIMING)]
    F -->|是| J[自动化诊断]
...
**方案2：EXPLAIN (ANALYZE, BUFFERS, TIMING)**:

- **描述**：使用EXPLAIN (ANALYZE, BUFFERS, TIMING)进行执行计划诊断
- **优点**：
  - 准确性优秀（详细的执行计划）
  - 覆盖范围优秀（执行计划级别）
  - 适合执行计划分析
...
- EXPLAIN (ANALYZE, BUFFERS, TIMING)：5.0 × 0.3 + 3.0 × 0.25 + 4.0 × 0.2 + 2.0 × 0.15 + 5.0 × 0.1 = 3.8
...
3. 使用EXPLAIN (ANALYZE, BUFFERS, TIMING)进行执行计划分析
4. 组合使用多种工具，提高诊断准确性
5. 定期更新诊断规则和策略
...
**推荐方案**: pg_stat_statements（结合EXPLAIN (ANALYZE, BUFFERS, TIMING)）
...
3. **分析执行计划**
   ...
   ```sql
   EXPLAIN (ANALYZE, BUFFERS, TIMING) -- 查询语句
   ```
