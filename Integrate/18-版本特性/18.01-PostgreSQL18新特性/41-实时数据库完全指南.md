---

> **📋 文档来源**: `docs\01-PostgreSQL18\41-实时数据库完全指南.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# PostgreSQL 18 实时数据库完全指南

PostgreSQL LISTEN/NOTIFY机制实现实时数据推送，构建实时应用。

---

## 📋 目录

- [PostgreSQL 18 实时数据库完全指南](#postgresql-18-实时数据库完全指南)
  - [📋 目录](#-目录)
  - [LISTEN/NOTIFY基础](#listennotify基础)
    - [基本概念](#基本概念)
    - [核心特性](#核心特性)
  - [实时通知机制](#实时通知机制)
    - [1. 基础使用](#1-基础使用)
    - [2. 多通道监听](#2-多通道监听)
    - [3. Payload限制](#3-payload限制)
  - [实战案例](#实战案例)
    - [案例1: 实时订单监控](#案例1-实时订单监控)
      - [数据库端](#数据库端)
      - [Python监听端](#python监听端)
      - [Node.js监听端](#nodejs监听端)
    - [案例2: 实时聊天系统](#案例2-实时聊天系统)
    - [案例3: 缓存失效通知](#案例3-缓存失效通知)
  - [性能优化](#性能优化)
    - [1. 通知频率控制](#1-通知频率控制)
    - [2. 条件通知](#2-条件通知)
    - [3. 监听连接管理](#3-监听连接管理)
  - [最佳实践](#最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [❌ 避免的做法](#-避免的做法)
  - [完整实战项目](#完整实战项目)
    - [实时仪表板](#实时仪表板)
  - [监控与调试](#监控与调试)
    - [查看活跃监听](#查看活跃监听)
    - [性能监控](#性能监控)
  - [总结](#总结)
    - [优势](#优势)
    - [限制](#限制)
    - [适用场景](#适用场景)

---

## LISTEN/NOTIFY基础

### 基本概念

PostgreSQL提供轻量级的发布/订阅机制：

```sql
-- 性能测试：会话A：监听通道（带错误处理）
BEGIN;
DO $$
BEGIN
    PERFORM pg_listen('my_channel');
    RAISE NOTICE '已监听通道: my_channel';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '监听通道失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;

-- 性能测试：会话B：发送通知（带错误处理）
BEGIN;
DO $$
BEGIN
    PERFORM pg_notify('my_channel', '{"event": "new_order", "order_id": 123}');
    RAISE NOTICE '通知已发送';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '发送通知失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;
-- 会话A会收到通知
```

### 核心特性

| 特性 | 说明 |
|------|------|
| **异步通知** | 非阻塞，不影响事务 |
| **多对多** | 多个监听者，多个发送者 |
| **轻量级** | 内存中传递，无持久化 |
| **事务感知** | COMMIT后才发送 |

---

## 实时通知机制

### 1. 基础使用

```sql
-- 性能测试：创建通知函数（带错误处理）
BEGIN;
CREATE OR REPLACE FUNCTION notify_order_change()
RETURNS TRIGGER AS $$
BEGIN
    -- 发送通知
    PERFORM pg_notify(
        'order_events',
        json_build_object(
            'action', TG_OP,
            'order_id', COALESCE(NEW.id, OLD.id),
            'user_id', COALESCE(NEW.user_id, OLD.user_id),
            'timestamp', now()
        )::text
    );

    RETURN COALESCE(NEW, OLD);
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '通知函数执行失败: %', SQLERRM;
        RAISE;
END;
$$ LANGUAGE plpgsql;
COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '创建通知函数失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：创建触发器（带错误处理）
BEGIN;
DROP TRIGGER IF EXISTS order_change_trigger ON orders;
CREATE TRIGGER order_change_trigger
AFTER INSERT OR UPDATE OR DELETE ON orders
FOR EACH ROW
EXECUTE FUNCTION notify_order_change();
COMMIT;
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE '表orders不存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建触发器失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
```

### 2. 多通道监听

```sql
-- 性能测试：监听多个通道（带错误处理）
BEGIN;
DO $$
BEGIN
    PERFORM pg_listen('orders');
    PERFORM pg_listen('payments');
    PERFORM pg_listen('shipments');
    RAISE NOTICE '已监听多个通道';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '监听通道失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;

-- 性能测试：取消监听（带错误处理）
BEGIN;
DO $$
BEGIN
    PERFORM pg_unlisten('orders');
    RAISE NOTICE '已取消监听orders通道';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '取消监听失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;

-- 性能测试：取消所有监听（带错误处理）
BEGIN;
DO $$
BEGIN
    PERFORM pg_unlisten_all();
    RAISE NOTICE '已取消所有监听';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '取消所有监听失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;
```

### 3. Payload限制

```sql
-- Payload最大8000字节
SELECT length('very long string'::text);

-- 超过限制需要传递ID，再查询
PERFORM pg_notify(
    'large_data_event',
    json_build_object('id', NEW.id)::text
);
```

---

## 实战案例

### 案例1: 实时订单监控

#### 数据库端

```sql
-- 订单表
CREATE TABLE orders (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL,
    status VARCHAR(20) NOT NULL,
    total_amount NUMERIC(10, 2) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
);

-- 通知函数
CREATE OR REPLACE FUNCTION notify_order_event()
RETURNS TRIGGER AS $$
DECLARE
    payload JSON;
BEGIN
    -- 构建payload
    IF TG_OP = 'DELETE' THEN
        payload = json_build_object(
            'action', 'delete',
            'order_id', OLD.id,
            'timestamp', extract(epoch from now())
        );
    ELSE
        payload = json_build_object(
            'action', lower(TG_OP),
            'order_id', NEW.id,
            'user_id', NEW.user_id,
            'status', NEW.status,
            'total_amount', NEW.total_amount,
            'timestamp', extract(epoch from now())
        );
    END IF;

    -- 发送通知
    PERFORM pg_notify('order_events', payload::text);

    RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

-- 触发器
CREATE TRIGGER order_event_trigger
AFTER INSERT OR UPDATE OR DELETE ON orders
FOR EACH ROW
EXECUTE FUNCTION notify_order_event();
```

#### Python监听端

```python
import psycopg2
import json
import select

def listen_orders():
    """监听订单事件"""

    conn = psycopg2.connect(
        host='localhost',
        database='mydb',
        user='postgres'
    )

    conn.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)

    cursor = conn.cursor()
    cursor.execute("LISTEN order_events;")

    print("✓ 开始监听订单事件...")

    try:
        while True:
            # 等待通知（10秒超时）
            if select.select([conn], [], [], 10) == ([], [], []):
                print(".", end='', flush=True)
            else:
                conn.poll()

                while conn.notifies:
                    notify = conn.notifies.pop(0)

                    # 解析payload
                    payload = json.loads(notify.payload)

                    # 处理事件
                    handle_order_event(payload)

    except KeyboardInterrupt:
        print("\n监听已停止")

    finally:
        cursor.close()
        conn.close()

def handle_order_event(payload):
    """处理订单事件"""

    action = payload['action']
    order_id = payload['order_id']

    if action == 'insert':
        print(f"\n📦 新订单: #{order_id}")
        print(f"   用户: {payload['user_id']}")
        print(f"   金额: ${payload['total_amount']}")

        # 发送推送通知
        # send_push_notification(payload)

    elif action == 'update':
        print(f"\n🔄 订单更新: #{order_id}")
        print(f"   状态: {payload['status']}")

        # 更新前端
        # update_frontend(payload)

    elif action == 'delete':
        print(f"\n❌ 订单删除: #{order_id}")

if __name__ == '__main__':
    listen_orders()
```

#### Node.js监听端

```javascript
const { Client } = require('pg');

async function listenOrders() {
    const client = new Client({
        host: 'localhost',
        database: 'mydb',
        user: 'postgres'
    });

    await client.connect();

    // 监听通知
    client.on('notification', (msg) => {
        const payload = JSON.parse(msg.payload);
        handleOrderEvent(payload);
    });

    // 订阅通道
    await client.query('LISTEN order_events');

    console.log('✓ 开始监听订单事件...');
}

function handleOrderEvent(payload) {
    const { action, order_id } = payload;

    if (action === 'insert') {
        console.log(`📦 新订单: #${order_id}`);
        // WebSocket推送到前端
        io.emit('new_order', payload);
    } else if (action === 'update') {
        console.log(`🔄 订单更新: #${order_id}`);
        io.emit('order_updated', payload);
    }
}

listenOrders().catch(console.error);
```

### 案例2: 实时聊天系统

```sql
-- 消息表
CREATE TABLE messages (
    id BIGSERIAL PRIMARY KEY,
    room_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    content TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT now()
);

-- 通知函数
CREATE OR REPLACE FUNCTION notify_new_message()
RETURNS TRIGGER AS $$
BEGIN
    -- 按房间发送通知
    PERFORM pg_notify(
        'room_' || NEW.room_id,
        json_build_object(
            'message_id', NEW.id,
            'user_id', NEW.user_id,
            'content', NEW.content,
            'timestamp', extract(epoch from NEW.created_at)
        )::text
    );

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER new_message_trigger
AFTER INSERT ON messages
FOR EACH ROW
EXECUTE FUNCTION notify_new_message();
```

```python
def join_room(room_id: int):
    """加入聊天室"""

    cursor.execute(f"LISTEN room_{room_id};")
    print(f"✓ 加入房间 {room_id}")

def leave_room(room_id: int):
    """离开聊天室"""

    cursor.execute(f"UNLISTEN room_{room_id};")
    print(f"✓ 离开房间 {room_id}")
```

### 案例3: 缓存失效通知

```sql
-- 缓存失效通知
CREATE OR REPLACE FUNCTION notify_cache_invalidation()
RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify(
        'cache_invalidation',
        json_build_object(
            'table', TG_TABLE_NAME,
            'id', COALESCE(NEW.id, OLD.id),
            'action', TG_OP
        )::text
    );

    RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

-- 应用到多个表
CREATE TRIGGER cache_invalidation_trigger
AFTER INSERT OR UPDATE OR DELETE ON users
FOR EACH ROW
EXECUTE FUNCTION notify_cache_invalidation();

CREATE TRIGGER cache_invalidation_trigger
AFTER INSERT OR UPDATE OR DELETE ON products
FOR EACH ROW
EXECUTE FUNCTION notify_cache_invalidation();
```

```python
import redis

redis_client = redis.Redis()

def handle_cache_invalidation(payload):
    """处理缓存失效"""

    table = payload['table']
    id = payload['id']

    # 删除Redis缓存
    cache_key = f"{table}:{id}"
    redis_client.delete(cache_key)

    print(f"✓ 缓存失效: {cache_key}")
```

---

## 性能优化

### 1. 通知频率控制

```sql
-- 批量通知（避免每行触发）
CREATE OR REPLACE FUNCTION notify_batch_changes()
RETURNS TRIGGER AS $$
BEGIN
    -- 只在事务结束时通知
    PERFORM pg_notify(
        'batch_changes',
        json_build_object(
            'table', TG_TABLE_NAME,
            'timestamp', extract(epoch from now())
        )::text
    );

    RETURN NULL;
END;
$$ LANGUAGE plpgsql;

-- 使用AFTER语句级触发器
CREATE TRIGGER batch_changes_trigger
AFTER INSERT OR UPDATE OR DELETE ON large_table
FOR EACH STATEMENT
EXECUTE FUNCTION notify_batch_changes();
```

### 2. 条件通知

```sql
-- 只在重要变更时通知
CREATE OR REPLACE FUNCTION notify_important_changes()
RETURNS TRIGGER AS $$
BEGIN
    -- 只有状态变更时通知
    IF NEW.status != OLD.status THEN
        PERFORM pg_notify('order_status_changes',
            json_build_object('order_id', NEW.id, 'new_status', NEW.status)::text
        );
    END IF;

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
```

### 3. 监听连接管理

```python
from psycopg2.pool import SimpleConnectionPool

# 使用连接池
pool = SimpleConnectionPool(
    minconn=1,
    maxconn=5,
    host='localhost',
    database='mydb'
)

# 专用监听连接
listen_conn = pool.getconn()
listen_conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)

# 业务连接使用其他连接
business_conn = pool.getconn()
```

---

## 最佳实践

### ✅ 推荐做法

1. **Payload保持小巧**

   ```sql
   -- ✅ 只传递ID
   PERFORM pg_notify('event', json_build_object('id', NEW.id)::text);

   -- ❌ 传递整行数据
   PERFORM pg_notify('event', row_to_json(NEW)::text);
   ```

2. **事务感知**

   ```sql
   BEGIN;
   INSERT INTO orders VALUES (...);
   -- 通知在COMMIT后才发送
   COMMIT;
   ```

3. **错误处理**

   ```python
   try:
       conn.poll()
       # 处理通知
   except psycopg2.DatabaseError as e:
       print(f"数据库错误: {e}")
       # 重新连接
       reconnect()
   ```

4. **心跳检测**

   ```python
   import time

   last_notify_time = time.time()

   while True:
       # 10秒超时
       if select.select([conn], [], [], 10) == ([], [], []):
           # 发送心跳查询
           cursor.execute("SELECT 1;")

           if time.time() - last_notify_time > 60:
               print("⚠️  长时间未收到通知")
       else:
           last_notify_time = time.time()
           # 处理通知
   ```

### ❌ 避免的做法

1. ❌ 使用LISTEN做任务队列
   - 通知不持久化
   - 监听者断开会丢失通知
   - 使用专门的任务队列（如RabbitMQ）

2. ❌ 传递大量数据
   - Payload限制8KB
   - 应传递ID，再查询

3. ❌ 在长事务中NOTIFY
   - 通知延迟到COMMIT
   - 可能很久才发送

---

## 完整实战项目

### 实时仪表板

```sql
-- 统计表
CREATE TABLE dashboard_stats (
    id BIGSERIAL PRIMARY KEY,
    metric_name VARCHAR(50) NOT NULL,
    metric_value NUMERIC NOT NULL,
    updated_at TIMESTAMPTZ DEFAULT now()
);

-- 更新触发器
CREATE OR REPLACE FUNCTION notify_dashboard_update()
RETURNS TRIGGER AS $$
BEGIN
    PERFORM pg_notify(
        'dashboard_updates',
        json_build_object(
            'metric', NEW.metric_name,
            'value', NEW.metric_value,
            'timestamp', extract(epoch from NEW.updated_at)
        )::text
    );

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER dashboard_update_trigger
AFTER INSERT OR UPDATE ON dashboard_stats
FOR EACH ROW
EXECUTE FUNCTION notify_dashboard_update();
```

```javascript
// WebSocket服务器
const WebSocket = require('ws');
const { Client } = require('pg');

const wss = new WebSocket.Server({ port: 8080 });
const pgClient = new Client({ /* ... */ });

await pgClient.connect();
await pgClient.query('LISTEN dashboard_updates');

// 转发通知到WebSocket客户端
pgClient.on('notification', (msg) => {
    const payload = JSON.parse(msg.payload);

    // 广播到所有WebSocket客户端
    wss.clients.forEach(client => {
        if (client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify(payload));
        }
    });
});

console.log('✓ WebSocket服务器启动在 :8080');
```

```html
<!-- 前端页面 -->
<script>
const ws = new WebSocket('ws://localhost:8080');

ws.onmessage = (event) => {
    const data = JSON.parse(event.data);

    // 更新仪表板
    updateDashboard(data.metric, data.value);
};

function updateDashboard(metric, value) {
    document.getElementById(metric).textContent = value;
}
</script>
```

---

## 监控与调试

### 查看活跃监听

```sql
-- 查看当前监听的通道
SELECT
    pid,
    usename,
    application_name,
    client_addr,
    backend_start,
    state
FROM pg_stat_activity
WHERE backend_type = 'client backend'
  AND query LIKE '%LISTEN%';
```

### 性能监控

```sql
-- 通知统计（需要自定义）
CREATE TABLE notify_stats (
    channel_name VARCHAR(100),
    notify_count BIGINT,
    last_notify TIMESTAMPTZ
);

-- 在通知函数中记录
UPDATE notify_stats
SET notify_count = notify_count + 1,
    last_notify = now()
WHERE channel_name = 'order_events';
```

---

## 总结

### 优势

- ✅ 轻量级实时通知
- ✅ 与数据库事务集成
- ✅ 简单易用
- ✅ 支持多对多

### 限制

- ⚠️ 不持久化（监听者断开会丢失）
- ⚠️ Payload限制8KB
- ⚠️ 不适合做任务队列
- ⚠️ 需要保持连接

### 适用场景

- ✅ 实时仪表板
- ✅ 缓存失效
- ✅ 实时聊天
- ✅ 状态通知
- ✅ 事件驱动架构

---

**PostgreSQL LISTEN/NOTIFY - 构建高效实时应用！**
