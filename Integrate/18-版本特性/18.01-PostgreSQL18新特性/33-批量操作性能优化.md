...
- [PostgreSQL 18 批量操作性能优化](#postgresql-18-批量操作性能优化)
  - [📑 目录](#-目录)
  - [1. 批量INSERT](#1-批量insert)
    - [1.1 性能对比](#11-性能对比)
    - [1.2 SQL批量INSERT](#12-sql批量insert)
  - [2. 批量UPDATE](#2-批量update)
    - [2.1 VALUES方式](#21-values方式)
    - [2.2 临时表方式](#22-临时表方式)
    - [3.2 TRUNCATE vs DELETE](#32-truncate-vs-delete)
  - [4. COPY性能优化](#4-copy性能优化)
    - [4.1 最快导入](#41-最快导入)
  - [5. 批量UPSERT](#5-批量upsert)
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '单条插入失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：批量（快）（带错误处理和性能分析）
BEGIN;
