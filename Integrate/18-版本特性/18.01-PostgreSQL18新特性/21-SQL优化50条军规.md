...
- [PostgreSQL SQL优化50条军规](#postgresql-sql优化50条军规)
  - [📑 目录](#-目录)
  - [1. 索引优化（10条）](#1-索引优化10条)
    - [1.1 使用合适的索引类型](#11-使用合适的索引类型)
    - [1.2 覆盖索引](#12-覆盖索引)
    - [1.3 部分索引](#13-部分索引)
    - [1.4 表达式索引](#14-表达式索引)
    - [1.5 避免冗余索引](#15-避免冗余索引)
  - [2. 查询优化（15条）](#2-查询优化15条)
    - [2.1 避免SELECT \*](#21-避免select-)
    - [2.2 使用EXISTS代替IN](#22-使用exists代替in)
    - [2.3 避免OR，使用UNION](#23-避免or使用union)
    - [2.4 LIMIT早期化](#24-limit早期化)
    - [2.5 使用JOIN代替子查询](#25-使用join代替子查询)
    - [2.6 批量操作](#26-批量操作)
    - [2.7 使用COPY](#27-使用copy)
    - [2.8 避免函数包裹索引列](#28-避免函数包裹索引列)
    - [2.9 合理使用CTE](#29-合理使用cte)
    - [2.10 使用分区裁剪](#210-使用分区裁剪)
  - [3. 事务优化（8条）](#3-事务优化8条)
    - [3.1 保持事务短小](#31-保持事务短小)
    - [3.2 合适的隔离级别](#32-合适的隔离级别)
    - [3.3 避免空闲事务](#33-避免空闲事务)
  - [4. 连接优化（5条）](#4-连接优化5条)
    - [4.1 使用连接池](#41-使用连接池)
    - [4.2 Prepared Statements](#42-prepared-statements)
  - [5. 数据类型优化（5条）](#5-数据类型优化5条)
    - [5.1 选择合适的整数类型](#51-选择合适的整数类型)
    - [5.2 使用ENUM](#52-使用enum)
    - [5.3 TEXT vs VARCHAR](#53-text-vs-varchar)
  - [6. 配置优化（7条）](#6-配置优化7条)
    - [6.1 内存配置](#61-内存配置)
    - [6.2 SSD优化](#62-ssd优化)
    - [6.3 并行查询](#63-并行查询)
  - [7. VACUUM优化（5条）](#7-vacuum优化5条)
    - [7.1 定期VACUUM](#71-定期vacuum)
    - [7.2 针对性VACUUM](#72-针对性vacuum)
  - [8. 监控优化（总结）](#8-监控优化总结)
