...
> **📋 文档来源**: `docs\01-PostgreSQL18\12-时态约束与时间段完整性指南.md`
> **📅 复制日期**: 2025-12-22
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表room_booking_old已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 问题：同一天可以多个预订，时间段冲突！（带错误处理）
BEGIN;
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE NOTICE '违反主键约束：同一天同一房间只能有一个预订';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
-- 性能测试：✅ PostgreSQL 18时态约束（防止时间段重叠，带错误处理）
BEGIN;
...
EXCEPTION
    ...
    WHEN check_violation THEN
        RAISE NOTICE '时间段无效：check_out必须大于check_in';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：✅ 防止重叠（带错误处理和性能分析）
BEGIN;
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表child_table已存在';
    WHEN undefined_table THEN
        RAISE NOTICE '父表parent_table不存在，请先创建';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表hotel_bookings已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN exclusion_violation THEN
        RAISE NOTICE '时间段冲突：与现有预订重叠';
    WHEN check_violation THEN
        RAISE NOTICE '时间段无效：check_out必须大于check_in，且至少1小时';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：❌ 违反约束（时间段重叠，带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
INSERT INTO hotel_bookings (room_id, guest_name, check_in, check_out) VALUES
    (101, 'David', '2025-01-16 10:00', '2025-01-18 12:00');  -- 与Alice重叠
COMMIT;
EXCEPTION
    WHEN exclusion_violation THEN
        RAISE NOTICE '时间段冲突：与现有预订重叠';
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表lease_contracts已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN exclusion_violation THEN
        RAISE NOTICE '时间段冲突：与现有租期重叠';
    WHEN check_violation THEN
        RAISE NOTICE '时间段无效：lease_end必须大于lease_start';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：❌ 重叠检测（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
INSERT INTO lease_contracts (property_id, tenant_name, lease_start, lease_end, monthly_rent) VALUES
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表employee_contracts已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：子表：项目分配（必须在合同有效期内，带错误处理）
BEGIN;
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表project_assignments已存在';
    WHEN undefined_table THEN
        RAISE NOTICE '父表employee_contracts不存在，请先创建';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN exclusion_violation THEN
        RAISE NOTICE '时间段冲突：与现有合同重叠';
    WHEN check_violation THEN
        RAISE NOTICE '时间段无效：contract_end必须大于contract_start';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：✅ 有效的项目分配（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE NOTICE '外键约束违反：项目分配期不在员工合同期内';
    WHEN check_violation THEN
        RAISE NOTICE '时间段无效：assignment_end必须大于assignment_start';
    WHEN OTHERS THEN
        RAISE NOTICE '插入数据失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：❌ 无效的项目分配（跨合同期，带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
INSERT INTO project_assignments VALUES
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表rooms已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：2. 预订表（时态约束，带错误处理）
BEGIN;
...
```sql
-- 性能测试：无约束，应用层检查（带错误处理）
BEGIN;
CREATE TABLE IF NOT EXISTS bookings_app_check (
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表bookings_app_check已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION '触发器执行失败: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;
...
EXCEPTION
    WHEN duplicate_object THEN
        RAISE NOTICE '触发器booking_overlap_check已存在';
    WHEN undefined_table THEN
        RAISE NOTICE '表bookings_trigger_check不存在，请先创建';
    WHEN OTHERS THEN
...
-- 性能测试：排斥约束（带错误处理）
BEGIN;
CREATE TABLE IF NOT EXISTS bookings_exclude (
    booking_id SERIAL PRIMARY KEY,
    room_id INT,
    check_in TIMESTAMPTZ,
    check_out TIMESTAMPTZ,
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表bookings_exclude已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
```sql
-- 性能测试：PostgreSQL 18时态约束（最优，带错误处理）
BEGIN;
CREATE TABLE IF NOT EXISTS bookings_temporal (
    booking_id SERIAL,
    room_id INT,
    check_in TIMESTAMPTZ NOT NULL,
    check_out TIMESTAMPTZ NOT NULL,
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '表bookings_temporal已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建表失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '查询索引信息失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：2. 添加辅助索引（常见查询，带错误处理）
BEGIN;
CREATE INDEX IF NOT EXISTS idx_bookings_guest ON bookings(guest_name);
COMMIT;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_bookings_guest已存在';
    WHEN OTHERS THEN
...
BEGIN;
CREATE INDEX IF NOT EXISTS idx_bookings_dates ON bookings USING brin (check_in, check_out);
COMMIT;
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_bookings_dates已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建索引失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
