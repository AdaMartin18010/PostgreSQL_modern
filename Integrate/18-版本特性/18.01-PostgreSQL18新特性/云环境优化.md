...
AS $$
DECLARE
    v_current_connections NUMERIC;
    v_max_connections NUMERIC;
    v_threshold NUMERIC;
BEGIN
    BEGIN
        -- 获取当前连接数
        SELECT COUNT(*)::NUMERIC INTO v_current_connections
        FROM pg_stat_activity;

        IF v_current_connections IS NULL THEN
            v_current_connections := 0;
        END IF;

        -- 获取最大连接数
        SELECT setting::NUMERIC INTO v_max_connections
        FROM pg_settings
        WHERE name = 'max_connections';

        IF v_max_connections IS NULL OR v_max_connections <= 0 THEN
            RAISE WARNING '无法获取max_connections配置，使用默认值100';
            v_max_connections := 100;
        END IF;

        -- 计算阈值（80%）
        v_threshold := v_max_connections * 0.8;

        RETURN QUERY
        SELECT
            'connections'::TEXT as metric_name,
            v_current_connections as current_value,
            v_threshold as threshold,
            (v_current_connections > v_threshold) as needs_scale;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查扩展需求失败: %', SQLERRM;
            -- 返回默认值
            RETURN QUERY
            SELECT
                'connections'::TEXT,
