...
- [5.1 pg_aios视图详解](#51-pg_aios视图详解)
...
- [6.4 锁超时告警](#64-锁超时告警)
- [7.1 pg_stat_statements增强](#71-pg_stat_statements增强)
- [7.2 表级维护统计](#72-表级维护统计)
- [8.1 postgres_exporter配置](#81-postgres_exporter配置)
...
```mermaid
graph TB
    A[PostgreSQL 18<br/>可观测性增强] --> B[结构化日志]
    A --> C[精细化指标]
    A --> D[实时诊断]

    B --> B1[JSON格式日志<br/>机器可解析]
    B --> B2[连接阶段拆解<br/>setup_durations]
    B --> B3[锁失败详情<br/>log_lock_failures]

    C --> C1[I/O字节级统计<br/>pg_stat_io]
    C --> C2[AIO监控视图<br/>pg_aios]
    C --> C3[维护操作耗时<br/>total_vacuum_time]

    D --> D1[后端级I/O统计<br/>pg_stat_get_backend_io]
    D --> D2[查询进度跟踪<br/>pg_stat_progress_*]
    D --> D3[WAL活动整合<br/>统一指标模型]
...
sequenceDiagram
    ...
    Note over Client,Auth: PostgreSQL 18连接三阶段

    Client->>PG: 1. 连接请求

    rect rgb(255, 200, 200)
        Note over PG: 阶段1: Fork进程<br/>fork_duration_ms
        PG->>Backend: fork()新建后端进程
        Backend-->>PG: 进程创建完成
    end

    rect rgb(200, 255, 200)
        Note over Backend,Auth: 阶段2: 认证<br/>auth_duration_ms
        Backend->>Auth: 验证凭证
    ...
    rect rgb(200, 200, 255)
        Note over Backend: 阶段3: 初始化<br/>其他耗时
        Backend->>Backend: 加载配置
        Backend->>Backend: 初始化缓存
        Backend-->>Client: 连接就绪
    end

    Note over Client,Backend: 总耗时 = setup_duration_ms
```
...
SELECT
    ...
    -- 最大值![1766347347386](image/22-监控与可观测性完整体系指南/1766347347386.pdf)
...
```mermaid
graph TB
    subgraph "PostgreSQL 18集群"
        PG1[Primary<br/>JSON日志 + 统计视图]
        PG2[Replica 1<br/>JSON日志 + 统计视图]
        PG3[Replica 2<br/>JSON日志 + 统计视图]
    end

    subgraph "采集层"
        PE1[postgres_exporter<br/>指标采集]
        PE2[postgres_exporter<br/>指标采集]
        PE3[postgres_exporter<br/>指标采集]

        PL1[promtail<br/>日志采集]
        PL2[promtail<br/>日志采集]
        PL3[promtail<br/>日志采集]
    end

    subgraph "存储层"
        Prom[Prometheus<br/>时序数据库<br/>30天保留]
        Loki[Loki<br/>日志聚合<br/>7天保留]
    end
