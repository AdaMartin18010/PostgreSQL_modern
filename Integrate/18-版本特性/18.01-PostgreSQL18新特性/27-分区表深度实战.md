...
> **📋 文档来源**: `docs\01-PostgreSQL18\27-分区表深度实战.md`
> **📅 复制日期**: 2025-12-22
...
- [PostgreSQL 18 分区表深度实战](#postgresql-18-分区表深度实战)
  - [📑 目录](#-目录)
  - [1. 分区策略选择](#1-分区策略选择)
    - [1.1 范围分区（Range）](#11-范围分区range)
    - [1.2 列表分区（List）](#12-列表分区list)
    - [1.3 哈希分区（Hash）](#13-哈希分区hash)
  - [2. 多级分区](#2-多级分区)
    - [2.1 时间+地区](#21-时间地区)
  - [3. 分区维护](#3-分区维护)
    - [3.1 自动创建分区](#31-自动创建分区)
    - [3.2 分区附加/分离](#32-分区附加分离)
  - [4. 分区裁剪](#4-分区裁剪)
    - [4.1 自动裁剪](#41-自动裁剪)
  - [5. 分区索引](#5-分区索引)
    - [5.1 全局索引 vs 本地索引](#51-全局索引-vs-本地索引)
  - [6. 性能对比](#6-性能对比)
    - [6.1 分区 vs 单表](#61-分区-vs-单表)
  - [7. 分区最佳实践](#7-分区最佳实践)
    - [7.1 分区键选择](#71-分区键选择)
    - [7.2 分区大小](#72-分区大小)
  - [8. 分区与并行](#8-分区与并行)
    - [8.1 并行分区扫描](#81-并行分区扫描)
