...
> **📋 文档来源**: `docs\01-PostgreSQL18\28-云原生存储引擎适配指南.md`
> **📅 复制日期**: 2025-12-22
...
graph TB
    subgraph 传统架构
        A1[PostgreSQL实例] --> B1[本地磁盘<br/>RAID SSD]
        B1 --> C1[备份<br/>pg_dump]
        C1 --> D1[离线存储]
    end
...
```mermaid
sequenceDiagram
    participant Compute as 计算节点<br/>(PostgreSQL)
    participant Cache as 缓存层<br/>(SSD Cache)
    participant Storage as 存储层<br/>(S3/分布式存储)

    Compute->>Cache: 1. 读取Page请求

    alt 缓存命中
        Cache-->>Compute: 2a. 返回Page（快）
    else 缓存未命中
        Cache->>Storage: 2b. 从存储读取
        Storage-->>Cache: 3. 返回Page
        Cache-->>Compute: 4. 返回Page（慢）
    end

    Compute->>Cache: 5. 写入Page（异步）
    Cache->>Storage: 6. 刷盘到存储（批量）

    Note over Compute,Storage: 优势：<br/>1. 计算节点无状态<br/>2. 存储弹性扩展<br/>3. 秒级只读副本
```
...
```mermaid
graph TB
    A[Neon架构] --> B[计算层<br/>Compute Nodes]
    A --> C[存储层<br/>Pageserver]
    A --> D[持久层<br/>S3]

    B --> B1[PostgreSQL实例<br/>无状态]
    B --> B2[本地SSD缓存]
...
graph TB
    A[Aurora架构] --> B[计算层]
    A --> C[存储层<br/>Aurora Storage]

    B --> B1[主节点<br/>Read/Write]
    B --> B2[只读副本1]
    B --> B3[只读副本15<br/>最多15个]
...
```mermaid
flowchart TD
    Start[选择存储方案] --> Q1{云 vs 自建?}

    Q1 -->|自建| SelfHost[自建PostgreSQL]
    Q1 -->|云| Q2{云服务商?}
    ...
    Q3 -->|高可用优先| Aurora[Aurora PostgreSQL]
    Q3 -->|成本优先| RDS[RDS PostgreSQL]
    Q3 -->|Serverless| Neon[Neon Serverless]

    SelfHost --> Config1[优化配置]
    Aurora --> Config2[调优参数]
    RDS --> Config2
    Neon --> Config3[分支策略]
...
```mermaid
timeline
    title PostgreSQL存储引擎演进

    2010-2015 : 传统Heap存储
              : 本地磁盘
              : MVCC多版本

    2016-2020 : Pluggable Storage提出
              : FDW外部表
              : 云存储初步支持
