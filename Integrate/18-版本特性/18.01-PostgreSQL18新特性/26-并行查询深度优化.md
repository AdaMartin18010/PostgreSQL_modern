...
> **📋 文档来源**: `docs\01-PostgreSQL18\26-并行查询深度优化.md`
> **📅 复制日期**: 2025-12-22
...
- [PostgreSQL 18 并行查询深度优化](#postgresql-18-并行查询深度优化)
  - [📑 目录](#-目录)
  - [1. 并行查询架构](#1-并行查询架构)
  - [2. 配置参数](#2-配置参数)
    - [2.1 核心参数](#21-核心参数)
  - [3. 并行操作类型](#3-并行操作类型)
    - [3.1 并行顺序扫描](#31-并行顺序扫描)
    - [3.2 并行索引扫描](#32-并行索引扫描)
    - [3.3 并行Hash JOIN](#33-并行hash-join)
    - [3.4 并行聚合](#34-并行聚合)
  - [4. 并行度控制](#4-并行度控制)
    - [4.1 自动并行度](#41-自动并行度)
    - [4.2 强制并行度](#42-强制并行度)
  - [5. 性能分析](#5-性能分析)
    - [5.1 并行效率](#51-并行效率)
  - [6. 限制因素](#6-限制因素)
    - [6.1 不支持并行的操作](#61-不支持并行的操作)
    - [6.2 并行安全性](#62-并行安全性)
  - [7. PostgreSQL 18改进](#7-postgresql-18改进)
    - [7.1 并行性能提升](#71-并行性能提升)
  - [8. 调优实战](#8-调优实战)
    - [8.1 OLAP查询优化](#81-olap查询优化)
  - [9. 最佳实践](#9-最佳实践)
