...
> **📋 文档来源**: `docs\01-PostgreSQL18\08-性能调优实战指南.md`
> **📅 复制日期**: 2025-12-22
...
sudo fio --name=random-write --ioengine=libaio --iodepth=32 \
  ...
  --numjobs=4 --runtime=60 --group_reporting || error_exit "fio测试失败"

# NVMe优化
sudo nvme set-feature /dev/nvme0n1 -f 0x0c -v 1 || error_exit "NVMe优化失败"  # 启用write cache

# 文件系统挂载优化（需要手动添加到/etc/fstab）
echo "请手动添加以下行到/etc/fstab:"
...
# 备份原始配置
sudo cp /etc/sysctl.conf /etc/sysctl.conf.backup || error_exit "备份sysctl.conf失败"

# 添加配置到/etc/sysctl.conf
cat <<EOF | sudo tee -a /etc/sysctl.conf || error_exit "写入sysctl.conf失败"
# PostgreSQL优化配置
# 共享内存
...
# I/O调度
echo mq-deadline | sudo tee /sys/block/nvme0n1/queue/scheduler || error_exit "设置I/O调度器失败"

# 透明大页（关闭）
echo never | sudo tee /sys/kernel/mm/transparent_hugepage/enabled || error_exit "关闭透明大页失败"

# 应用
sudo sysctl -p || error_exit "应用sysctl配置失败"
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_active_users已存在';
    WHEN OTHERS THEN
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_orders_user_date已存在';
    WHEN OTHERS THEN
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_logs_timestamp已存在';
    WHEN OTHERS THEN
...
EXCEPTION
    ...
    WHEN undefined_object THEN
        RAISE NOTICE 'vector扩展未安装，请先执行: CREATE EXTENSION vector;';
    WHEN OTHERS THEN
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '查找缺失索引失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：查找未使用索引（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
