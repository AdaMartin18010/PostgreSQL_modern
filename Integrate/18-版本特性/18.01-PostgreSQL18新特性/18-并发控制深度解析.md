...
> **📋 文档来源**: `docs\01-PostgreSQL18\18-并发控制深度解析.md`
> **📅 复制日期**: 2025-12-22
...
- [PostgreSQL 18 并发控制深度解析](#postgresql-18-并发控制深度解析)
  - [📑 目录](#-目录)
  - [1. MVCC原理](#1-mvcc原理)
    - [1.1 多版本存储](#11-多版本存储)
    - [1.2 可见性判断](#12-可见性判断)
  ...
  - [5. 锁等待监控](#5-锁等待监控)
    ...
    - [5.2 终止阻塞](#52-终止阻塞)
  - [6. 队列处理](#6-队列处理)
    - [6.1 SKIP LOCKED](#61-skip-locked)
  - [7. 最佳实践](#7-最佳实践)
...
-- 性能测试：FOR NO KEY UPDATE（允许外键）（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE NOTICE 'FOR NO KEY UPDATE操作失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：FOR SHARE（共享锁）（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE NOTICE 'FOR SHARE操作失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：FOR KEY SHARE（允许更新非键列）（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE '表users不存在';
    WHEN OTHERS THEN
        RAISE NOTICE 'FOR KEY SHARE操作失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
```
...
EXCEPTION
    ...
    WHEN deadlock_detected THEN
        RAISE NOTICE '死锁检测: deadlock detected';
        RAISE NOTICE 'PostgreSQL自动检测并回滚一个事务';
        ROLLBACK;
    WHEN OTHERS THEN
...
DO $$
BEGIN
    PERFORM current_setting('deadlock_timeout');  -- 默认1s
    RAISE NOTICE 'deadlock_timeout查询成功';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '查询deadlock_timeout失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;
...
BEGIN
    ...
    RAISE NOTICE 'log_lock_waits已启用';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '配置log_lock_waits失败: %', SQLERRM;
        RAISE;
END $$;
COMMIT;
