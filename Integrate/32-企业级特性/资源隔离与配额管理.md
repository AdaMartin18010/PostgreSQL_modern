...
flowchart TD
    A[需要资源隔离?] --> B{隔离级别要求?}
    B -->|连接级隔离| C[连接池隔离 + 连接限制]
    B -->|查询级隔离| D[CPU隔离 + 内存隔离]
    B -->|存储级隔离| E[表空间隔离 + I/O隔离]
    B -->|全面隔离| F[多租户架构 + 资源隔离]

    C --> G[优势: 简单有效<br/>适用: 中小规模]
    D --> H[优势: 精细控制<br/>适用: 性能敏感]
    E --> I[优势: 存储隔离<br/>适用: 大规模]
    F --> J[优势: 全面隔离<br/>适用: 企业级]

    A --> K{需要配额管理?}
    K -->|是| L[存储配额 + 连接配额 + 查询配额]
    K -->|否| M[仅资源隔离]
```
...
BEGIN
    BEGIN
        RETURN QUERY
        SELECT
            COALESCE(r.rolname, 'unknown')::TEXT as tenant_name,
            COALESCE(COUNT(a.pid), 0)::BIGINT as connection_count,
            COALESCE(r.rolconnlimit, -1)::INT as max_connections,
            CASE
                WHEN r.rolconnlimit IS NULL OR r.rolconnlimit <= 0 THEN 0::NUMERIC
                WHEN COUNT(a.pid) IS NULL THEN 0::NUMERIC
                ELSE COALESCE(
        ...
        WHERE r.rolname IS NOT NULL
          AND r.rolname LIKE 'tenant%'
        GROUP BY r.rolname, r.rolconnlimit
        HAVING COALESCE(r.rolconnlimit, -1) > 0
           AND COALESCE(COUNT(a.pid), 0) > COALESCE(r.rolconnlimit, 0) * 0.8  -- 80%阈值
        ORDER BY usage_percent DESC;
    EXCEPTION
        WHEN division_by_zero THEN
            RAISE WARNING '检查资源使用情况时出现除零错误';
            RETURN;
        WHEN OTHERS THEN
