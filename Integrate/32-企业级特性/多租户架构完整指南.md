...
## 📊 国际多租户模型对标：Pool / Bridge / Silo

AWS、Azure 等云厂商常用 **Pool / Bridge / Silo** 三种模型描述多租户分区方式，与本指南上述模式的对应关系如下：

| 国际命名 | 本指南对应模式 | 说明 |
|----------|----------------|------|
| **Pool 模型** | 共享 Schema + RLS | 多租户共享同一实例与 Schema，靠 RLS 做行级隔离；成本低、扩展好，需注意“吵闹邻居”与租户级监控。 |
| **Bridge 模型** | 独立 Schema（或库内多库/多 Schema 混合） | 单实例内按租户分库或分 Schema，兼顾隔离与成本；适合中规模、定制化需求不一的情况。 |
| **Silo 模型** | 独立数据库（或单租户单实例） | 每租户独立实例或独立库，隔离与性能最佳，成本与运维最高；适合小规模或强合规场景。 |

**参考**：AWS Prescriptive Guidance（[Managed PostgreSQL for multi-tenant SaaS](https://docs.aws.amazon.com/prescriptive-guidance/latest/saas-multitenant-managed-postgresql/welcome.html)）、Azure [多租户方案中的 PostgreSQL](https://learn.microsoft.com/en-us/azure/architecture/guide/multitenant/service/postgresql)。无论采用哪种模型，均建议配合租户级监控与 SLA 指标（见本主题 [SLA管理完整指南](./SLA管理完整指南.md)）。
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行tenant_id索引查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM orders WHERE tenant_id = 1001 LIMIT 100;  -- 测试tenant_id索引性能
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM orders WHERE tenant_id = 1001 AND status = 'active' ORDER BY created_at DESC LIMIT 50;  -- 测试复合索引性能
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
EXECUTE get_orders(1001);  -- 测试参数化查询性能
