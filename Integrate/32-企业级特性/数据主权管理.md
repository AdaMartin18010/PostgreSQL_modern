...
BEGIN
    ...
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_classification') THEN
        RAISE EXCEPTION 'data_classification表不存在';
    END IF;
    ...
    BEGIN
        ...
        IF p_user_region != v_target_region AND v_target_region != 'GLOBAL' THEN
            RAISE EXCEPTION '数据必须存储在区域 % (用户区域: %)', v_target_region, p_user_region;
        END IF;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE EXCEPTION '未找到数据类型 % 的分类信息', p_data_type;
        WHEN OTHERS THEN
            RAISE EXCEPTION '路由数据到区域失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    -- 检查表是否存在
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_classification') THEN
        RAISE WARNING 'data_classification表不存在，默认不允许复制';
        RETURN false;
    END IF;
    ...
    BEGIN
        ...
        IF v_cross_border_allowed THEN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '检查法律依据失败: %', SQLERRM;
                    RETURN false;
            END;
        END IF;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cross_border_transfer_requests') THEN
        RAISE EXCEPTION 'cross_border_transfer_requests表不存在';
    END IF;
    ...
    BEGIN
        ...
        FOR i IN 1..3 LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    IF i = 3 THEN
                        RAISE EXCEPTION '生成请求ID失败（重试3次后仍然失败）';
                    END IF;
                    RAISE WARNING '生成请求ID失败，重试 %/%', i, 3;
            END;
        END LOOP;

        IF v_request_id IS NULL OR LENGTH(v_request_id) < 32 THEN
            RAISE EXCEPTION '生成的请求ID无效';
        END IF;
        ...
        IF NOT check_replication_allowed(p_source_region, p_target_region, p_table_name) THEN
            RAISE EXCEPTION '不允许跨境传输: 表=% (源区域: %, 目标区域: %)',
        ...
        BEGIN
            ...
            RAISE NOTICE '跨境传输请求创建成功: request_id=%, table=%, source=%, target=%',
...
BEGIN
    ...
    -- 检查pgp_sym_encrypt函数是否存在
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pgp_sym_encrypt') THEN
        RAISE EXCEPTION 'pgp_sym_encrypt函数不存在，请检查pgcrypto扩展';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '导出加密数据完成: 表=%, 区域=%, 行数=%', p_table_name, p_region, v_row_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表不存在: %', p_table_name;
        WHEN undefined_function THEN
            RAISE EXCEPTION 'pgcrypto扩展未安装或函数不可用';
        WHEN OTHERS THEN
            RAISE EXCEPTION '导出加密数据失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'cross_border_transfer_log') THEN
        RAISE EXCEPTION 'cross_border_transfer_log表不存在';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '跨境传输日志记录成功: request_id=%, status=%',
            COALESCE(p_request_id, 'NULL'), p_status;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表cross_border_transfer_log不存在';
        WHEN NOT_NULL_VIOLATION THEN
            RAISE EXCEPTION '必需字段不能为NULL';
        WHEN unique_violation THEN
            RAISE WARNING '日志记录已存在（可能重复记录）';
        WHEN OTHERS THEN
