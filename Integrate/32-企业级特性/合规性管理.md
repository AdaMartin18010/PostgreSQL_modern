...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'get_user_data') THEN
            RAISE WARNING '函数 get_user_data 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行数据访问权函数性能测试';
    EXCEPTION
        WHEN OTHERS THEN
...
SELECT * FROM get_user_data(1);  -- 示例：获取用户ID为1的数据
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'export_user_data') THEN
            RAISE WARNING '函数 export_user_data 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行数据可移植权函数性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
            RAISE;
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT export_user_data(1);  -- 示例：导出用户ID为1的数据
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'check_processing_consent') THEN
            RAISE WARNING '函数 check_processing_consent 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行检查处理同意函数性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
            RAISE;
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT check_processing_consent(1, 'marketing');  -- 示例：检查用户ID为1的营销处理同意

-- 性能测试：直接查询数据处理同意表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_processing_consent') THEN
            RAISE WARNING '表 data_processing_consent 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行数据处理同意表查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
            RAISE;
