...
> **📋 文档来源**: `PostgreSQL\05-部署架构\容器化部署\05.12-Docker部署.md`
> **📅 复制日期**: 2025-12-22
...
- [Docker 部署指南](#docker-部署指南)
  ...
  - [9. 生产级Docker Compose配置](#9-生产级docker-compose配置)
    - [9.1 完整生产环境配置](#91-完整生产环境配置)
    - [9.2 多容器编排配置](#92-多容器编排配置)
    - [9.3 网络和安全配置](#93-网络和安全配置)
    - [9.4 资源限制和优化](#94-资源限制和优化)
  - [10. 自定义镜像构建](#10-自定义镜像构建)
    - [10.1 多阶段构建优化](#101-多阶段构建优化)
    - [10.2 扩展安装和配置](#102-扩展安装和配置)
    - [10.3 性能优化配置](#103-性能优化配置)
    - [10.4 安全加固配置](#104-安全加固配置)
  - [11. 容器监控和日志](#11-容器监控和日志)
    - [11.1 Docker日志收集配置](#111-docker日志收集配置)
    - [11.2 容器健康检查配置](#112-容器健康检查配置)
    - [11.3 Prometheus监控集成](#113-prometheus监控集成)
    - [11.4 日志聚合和分析](#114-日志聚合和分析)
  - [12. 故障排查和诊断](#12-故障排查和诊断)
    - [12.1 容器启动问题排查](#121-容器启动问题排查)
    - [12.2 数据持久化问题](#122-数据持久化问题)
    - [12.3 网络连接问题](#123-网络连接问题)
    - [12.4 性能问题诊断](#124-性能问题诊断)
  - [13. 备份和恢复](#13-备份和恢复)
    - [13.1 数据卷备份](#131-数据卷备份)
    - [13.2 逻辑备份](#132-逻辑备份)
    - [13.3 自动备份脚本](#133-自动备份脚本)
  - [14. 深入阅读](#14-深入阅读)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [运维实践](#运维实践)
...
## 9. 生产级Docker Compose配置

### 9.1 完整生产环境配置

**生产级docker-compose.yml配置**:
...
**生产级postgresql.conf配置**:

```conf
# config/postgresql.conf - PostgreSQL 18生产级配置
...
```bash
# 错误1: "Error response from daemon: driver failed programming external connectivity"
# 原因: 端口已被占用
# 解决: 更改端口或停止占用端口的容器
docker-compose down
docker-compose up -d

# 错误2: "Permission denied" 数据目录
# 原因: 数据目录权限问题
# 解决: 修复权限
sudo chown -R 999:999 /data/postgres/data
sudo chmod 700 /data/postgres/data

# 错误3: "Container keeps restarting"
# 原因: 配置错误或启动失败
# 解决: 查看日志找出具体错误
docker logs postgres
