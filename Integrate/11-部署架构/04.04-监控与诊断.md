---

> **📋 文档来源**: `PostgreSQL\04-部署运维\04.04-监控与诊断.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# PostgreSQL监控与诊断完整指南

> **版本**: v3.0
> **最后更新**: 2025-01-15
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **难度**: ⭐⭐⭐⭐
> **应用场景**: 生产环境监控、性能诊断、问题排查、系统健康检查
> 🆕 **PostgreSQL 18监控增强**
>
> PostgreSQL 18引入多项监控改进：
>
> - ✅ **新增系统视图**: 增强的pg_stat_statements，新增标准差统计
> - ✅ **改进的进度报告**: pg_stat_progress_*视图更详细
> - ✅ **共享内存监控**: 新增pg_shmem_allocations视图
> - ✅ **更细粒度的等待事件**: 更准确的性能诊断

---

## 📑 目录

- [PostgreSQL监控与诊断完整指南](#postgresql监控与诊断完整指南)
  - [📑 目录](#-目录)
  - [一、概述](#一概述)
    - [1.1 监控目标](#11-监控目标)
    - [1.2 监控体系](#12-监控体系)
    - [1.3 诊断方法](#13-诊断方法)
  - [二、指标体系](#二指标体系)
    - [2.1 实例级指标](#21-实例级指标)
    - [2.2 表/索引级指标](#22-表索引级指标)
    - [2.3 查询级指标](#23-查询级指标)
    - [2.4 系统资源指标](#24-系统资源指标)
  - [2. PostgreSQL 18新增监控特性](#2-postgresql-18新增监控特性)
    - [2.1 增强的pg\_stat\_statements](#21-增强的pg_stat_statements)
    - [2.2 共享内存监控](#22-共享内存监控)
    - [2.3 改进的进度报告](#23-改进的进度报告)
  - [3. 常用视图与SQL](#3-常用视图与sql)
    - [3.1 活跃会话监控](#31-活跃会话监控)
    - [3.2 锁与等待](#32-锁与等待)
    - [3.3 表扫描/索引使用](#33-表扫描索引使用)
    - [3.4 I/O命中率](#34-io命中率)
    - [3.5 慢查询分析](#35-慢查询分析)
  - [五、监控实现](#五监控实现)
    - [5.1 Prometheus + Grafana](#51-prometheus--grafana)
      - [5.1.1 postgres\_exporter部署](#511-postgres_exporter部署)
      - [5.1.2 Prometheus配置](#512-prometheus配置)
      - [5.1.3 Grafana仪表盘](#513-grafana仪表盘)
    - [5.2 监控工具选择](#52-监控工具选择)
    - [5.3 告警配置](#53-告警配置)
  - [六、诊断流程（SOP）](#六诊断流程sop)
    - [6.1 性能劣化诊断](#61-性能劣化诊断)
    - [6.2 锁等待/死锁诊断](#62-锁等待死锁诊断)
    - [6.3 膨胀/垃圾诊断](#63-膨胀垃圾诊断)
    - [6.4 连接问题诊断](#64-连接问题诊断)
  - [七、知识矩阵对比](#七知识矩阵对比)
    - [7.1 监控工具对比](#71-监控工具对比)
    - [7.2 指标重要性对比](#72-指标重要性对比)
  - [八、实践案例](#八实践案例)
    - [8.1 案例1：慢查询诊断](#81-案例1慢查询诊断)
    - [8.2 案例2：锁等待诊断](#82-案例2锁等待诊断)
    - [8.3 案例3：性能劣化诊断](#83-案例3性能劣化诊断)
  - [九、最佳实践](#九最佳实践)
    - [9.1 监控最佳实践](#91-监控最佳实践)
    - [9.2 诊断最佳实践](#92-诊断最佳实践)
  - [十、参考资源](#十参考资源)
    - [10.1 官方文档](#101-官方文档)
    - [10.2 相关文档](#102-相关文档)
  - [十一、日志与可观测性增强](#十一日志与可观测性增强)
    - [11.1 日志配置](#111-日志配置)
    - [11.2 慢日志采集](#112-慢日志采集)
  - [十二、参考文献](#十二参考文献)

---

## 一、概述

### 1.1 监控目标

**监控的核心目标**：

- **预防问题**：提前发现潜在问题
- **快速诊断**：快速定位问题根源
- **性能优化**：识别性能瓶颈
- **容量规划**：预测资源需求

### 1.2 监控体系

**监控层次**：

1. **系统资源层**：CPU、内存、磁盘、网络
2. **数据库实例层**：连接、事务、缓冲、WAL
3. **对象层**：表、索引、查询
4. **应用层**：业务指标、用户体验

### 1.3 诊断方法

**诊断流程**：

1. **收集信息**：收集相关监控数据和日志
2. **分析问题**：分析问题特征和模式
3. **定位根因**：定位问题根本原因
4. **制定方案**：制定解决方案
5. **验证效果**：验证解决方案效果

## 二、指标体系

### 2.1 实例级指标

**核心指标**：

- **连接数**：当前连接数、最大连接数、连接使用率
- **事务**：TPS、事务提交/回滚率
- **缓冲命中率**：共享缓冲区命中率
- **WAL**：WAL写入速率、WAL大小
- **检查点**：检查点频率、检查点耗时
- **Autovacuum**：Autovacuum进程数、滞后情况

**监控SQL**：

```sql
-- 实例级概览（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看实例级概览';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    (SELECT count(*) FROM pg_stat_activity) AS total_connections,
    (SELECT count(*) FROM pg_stat_activity WHERE state = 'active') AS active_connections,
    (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') AS max_connections,
    (SELECT xact_commit FROM pg_stat_database WHERE datname = current_database()) AS xact_commit,
    (SELECT xact_rollback FROM pg_stat_database WHERE datname = current_database()) AS xact_rollback,
    (SELECT round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2)
     FROM pg_stat_database) AS cache_hit_ratio;
```

### 2.2 表/索引级指标

**核心指标**：

- **扫描统计**：顺序扫描次数、索引扫描次数
- **元组统计**：活元组数、死元组数、膨胀率
- **I/O统计**：堆块读取、堆块命中、索引块读取、索引块命中
- **维护统计**：最后VACUUM时间、最后ANALYZE时间

### 2.3 查询级指标

**核心指标**：

- **调用次数**：查询执行次数
- **执行时间**：平均时间、最小时间、最大时间、标准差
- **返回行数**：平均返回行数
- **缓存命中率**：共享块命中率

### 2.4 系统资源指标

**核心指标**：

- **CPU使用率**：用户态、内核态、I/O等待
- **内存使用率**：总内存、已用内存、缓存、交换
- **磁盘I/O**：读写速率、IOPS、利用率
- **网络I/O**：带宽使用、连接数

## 2. PostgreSQL 18新增监控特性

### 2.1 增强的pg_stat_statements

```sql
-- PostgreSQL 18+: 新增标准差统计（带错误处理）

-- 创建扩展（如果尚未安装）（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            CREATE EXTENSION pg_stat_statements;
            RAISE NOTICE '扩展 pg_stat_statements 创建成功';
        ELSE
            RAISE NOTICE '扩展 pg_stat_statements 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '扩展 pg_stat_statements 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建扩展失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查询性能统计（含标准差）（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING '扩展 pg_stat_statements 不存在，无法查询性能统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询性能统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    query,
    calls,
    mean_exec_time,
    stddev_exec_time,  -- ⭐ PostgreSQL 17新增
    min_exec_time,
    max_exec_time,
    rows,
    shared_blks_hit,
    shared_blks_read,
    -- 计算稳定性指标
    CASE
        WHEN mean_exec_time > 0
        THEN round((stddev_exec_time / mean_exec_time * 100)::numeric, 2)
        ELSE 0
    END AS cv_percent  -- 变异系数，衡量查询稳定性
FROM pg_stat_statements
WHERE calls > 10
ORDER BY mean_exec_time DESC
LIMIT 20;

-- 识别不稳定的查询（高变异系数）（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    left(query, 60) AS query_preview,
    calls,
    round(mean_exec_time::numeric, 2) AS mean_ms,
    round(stddev_exec_time::numeric, 2) AS stddev_ms,
    round((stddev_exec_time / mean_exec_time * 100)::numeric, 2) AS cv_percent
FROM pg_stat_statements
WHERE calls > 100
  AND mean_exec_time > 10
  AND stddev_exec_time / mean_exec_time > 0.5  -- 变异系数>50%
ORDER BY cv_percent DESC
LIMIT 10;
```

### 2.2 共享内存监控

```sql
-- PostgreSQL 18+: 监控动态共享内存使用（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始监控动态共享内存使用';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    name,
    allocated_size,
    free_size,
    used_size,
    dynamic,
    pg_size_pretty(allocated_size) AS allocated,
    pg_size_pretty(free_size) AS free,
    pg_size_pretty(used_size) AS used,
    round((used_size::float / allocated_size * 100)::numeric, 2) AS usage_percent
FROM pg_shmem_allocations
WHERE allocated_size > 0
ORDER BY allocated_size DESC
LIMIT 20;

-- 动态共享内存总览（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    dynamic,
    count(*) AS segment_count,
    pg_size_pretty(sum(allocated_size)) AS total_allocated,
    pg_size_pretty(sum(used_size)) AS total_used,
    round((sum(used_size)::float / sum(allocated_size) * 100)::numeric, 2) AS usage_percent
FROM pg_shmem_allocations
GROUP BY dynamic
ORDER BY dynamic;
```

### 2.3 改进的进度报告

```sql
-- PostgreSQL 18+: 增强的VACUUM进度监控（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始监控VACUUM进度';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    datname,
    relid::regclass AS table_name,
    phase,
    heap_blks_total,
    heap_blks_scanned,
    heap_blks_vacuumed,
    index_vacuum_count,
    max_dead_tuples,
    num_dead_tuples,
    -- 计算进度百分比
    round((heap_blks_scanned::float / NULLIF(heap_blks_total, 0) * 100)::numeric, 2) AS scan_progress,
    round((heap_blks_vacuumed::float / NULLIF(heap_blks_total, 0) * 100)::numeric, 2) AS vacuum_progress
FROM pg_stat_progress_vacuum
ORDER BY scan_progress DESC;

-- ANALYZE进度监控（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    datname,
    relid::regclass AS table_name,
    phase,
    sample_blks_total,
    sample_blks_scanned,
    round((sample_blks_scanned::float / NULLIF(sample_blks_total, 0) * 100)::numeric, 2) AS progress
FROM pg_stat_progress_analyze
ORDER BY progress DESC;
```

---

## 3. 常用视图与SQL

### 3.1 活跃会话监控

```sql
-- 活跃会话（PostgreSQL 18增强）（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看活跃会话';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    usename,
    application_name,
    client_addr,
    backend_start,
    xact_start,
    query_start,
    state_change,
    wait_event_type,
    wait_event,
    state,
    backend_xid,
    backend_xmin,
    left(query, 100) AS query_preview
FROM pg_stat_activity
WHERE state <> 'idle'
ORDER BY
    CASE state
        WHEN 'active' THEN 1
        WHEN 'idle in transaction' THEN 2
        ELSE 3
    END,
    query_start;

-- 长时间运行的事务（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    usename,
    application_name,
    state,
    now() - xact_start AS transaction_duration,
    now() - query_start AS query_duration,
    left(query, 80) AS query_preview
FROM pg_stat_activity
WHERE xact_start IS NOT NULL
  AND state NOT IN ('idle')
ORDER BY xact_start
LIMIT 20;
```

### 3.2 锁与等待

```sql
-- 锁与等待（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看锁与等待';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    locktype,
    relation::regclass,
    mode,
    granted,
    pid,
    page,
    tuple,
    virtualxid,
    transactionid,
    classid,
    objid
FROM pg_locks
WHERE NOT granted
ORDER BY relation, pid;

-- 锁等待详情（关联会话信息）（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    blocked_locks.pid AS blocked_pid,
    blocked_activity.usename AS blocked_user,
    blocking_locks.pid AS blocking_pid,
    blocking_activity.usename AS blocking_user,
    blocked_activity.query AS blocked_statement,
    blocking_activity.query AS blocking_statement
FROM pg_catalog.pg_locks blocked_locks
JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
JOIN pg_catalog.pg_locks blocking_locks
    ON blocking_locks.locktype = blocked_locks.locktype
    AND blocking_locks.database IS NOT DISTINCT FROM blocked_locks.database
    AND blocking_locks.relation IS NOT DISTINCT FROM blocked_locks.relation
    AND blocking_locks.page IS NOT DISTINCT FROM blocked_locks.page
    AND blocking_locks.tuple IS NOT DISTINCT FROM blocked_locks.tuple
    AND blocking_locks.virtualxid IS NOT DISTINCT FROM blocked_locks.virtualxid
    AND blocking_locks.transactionid IS NOT DISTINCT FROM blocked_locks.transactionid
    AND blocking_locks.classid IS NOT DISTINCT FROM blocked_locks.classid
    AND blocking_locks.objid IS NOT DISTINCT FROM blocked_locks.objid
    AND blocking_locks.objsubid IS NOT DISTINCT FROM blocked_locks.objsubid
    AND blocking_locks.pid != blocked_locks.pid
JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted;
```

### 3.3 表扫描/索引使用

```sql
-- 表扫描/索引使用（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看表扫描/索引使用';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    relname,
    seq_scan,
    idx_scan,
    n_tup_ins,
    n_tup_upd,
    n_tup_del,
    n_tup_hot_upd,
    n_live_tup,
    n_dead_tup,
    round((n_dead_tup::float / NULLIF(n_live_tup, 0) * 100)::numeric, 2) AS dead_tuple_percent,
    last_vacuum,
    last_autovacuum,
    last_analyze,
    last_autoanalyze
FROM pg_stat_user_tables
ORDER BY (seq_scan + idx_scan) DESC
LIMIT 20;

-- 识别缺少索引的表（顺序扫描过多）（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    relname,
    seq_scan,
    idx_scan,
    n_live_tup,
    round((seq_scan::float / NULLIF(seq_scan + idx_scan, 0) * 100)::numeric, 2) AS seq_scan_percent
FROM pg_stat_user_tables
WHERE seq_scan > 100
  AND n_live_tup > 1000
  AND seq_scan > idx_scan
ORDER BY seq_scan DESC
LIMIT 10;
```

### 3.4 I/O命中率

```sql
-- I/O 命中率（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看I/O命中率';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    relname,
    heap_blks_read,
    heap_blks_hit,
    idx_blks_read,
    idx_blks_hit,
    round(100.0 * heap_blks_hit / NULLIF(heap_blks_hit + heap_blks_read, 0), 2) AS heap_hit_ratio,
    round(100.0 * idx_blks_hit / NULLIF(idx_blks_hit + idx_blks_read, 0), 2) AS idx_hit_ratio
FROM pg_statio_user_tables
WHERE heap_blks_read + heap_blks_hit > 0
ORDER BY heap_hit_ratio ASC NULLS LAST
LIMIT 20;

-- 整体缓存命中率（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    sum(heap_blks_read) AS heap_read,
    sum(heap_blks_hit) AS heap_hit,
    round(100.0 * sum(heap_blks_hit) / NULLIF(sum(heap_blks_hit) + sum(heap_blks_read), 0), 2) AS cache_hit_ratio
FROM pg_statio_user_tables;
```

### 3.5 慢查询分析

```sql
-- 慢查询（需 pg_stat_statements）（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING '扩展 pg_stat_statements 不存在，无法查询慢查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看慢查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    queryid,
    left(query, 80) AS query_preview,
    calls,
    round(total_exec_time::numeric, 2) AS total_time_ms,
    round(mean_exec_time::numeric, 2) AS mean_time_ms,
    round(stddev_exec_time::numeric, 2) AS stddev_ms,  -- PG18+
    round(min_exec_time::numeric, 2) AS min_time_ms,
    round(max_exec_time::numeric, 2) AS max_time_ms,
    rows,
    round(100.0 * shared_blks_hit / NULLIF(shared_blks_hit + shared_blks_read, 0), 2) AS cache_hit_percent
FROM pg_stat_statements
WHERE calls > 10
ORDER BY mean_exec_time DESC
LIMIT 20;
```

## 五、监控实现

### 5.1 Prometheus + Grafana

#### 5.1.1 postgres_exporter部署

**创建监控用户**：

```sql
-- 创建监控专用用户（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'metrics') THEN
            CREATE ROLE metrics WITH LOGIN PASSWORD 'secure_password';
            RAISE NOTICE '监控用户 metrics 创建成功';
        ELSE
            RAISE NOTICE '监控用户 metrics 已存在';
        END IF;
        GRANT pg_monitor TO metrics;
        -- 如需WAL/复制细粒度指标
        GRANT pg_read_all_stats TO metrics;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '监控用户 metrics 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建监控用户失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

**启动postgres_exporter**：

```bash
# 方式1：环境变量
export DATA_SOURCE_NAME="postgresql://metrics:password@localhost:5432/postgres?sslmode=disable"
./postgres_exporter

# 方式2：命令行参数
./postgres_exporter \
  --extend.query-path=/etc/postgres_exporter/queries.yaml \
  --web.listen-address=:9187

# 方式3：systemd服务
[Unit]
Description=PostgreSQL Exporter
After=network.target

[Service]
Type=simple
User=postgres
Environment="DATA_SOURCE_NAME=postgresql://metrics:password@localhost:5432/postgres?sslmode=disable"
ExecStart=/usr/local/bin/postgres_exporter
Restart=always

[Install]
WantedBy=multi-user.target
```

#### 5.1.2 Prometheus配置

**prometheus.yml配置**：

```yaml
global:
  scrape_interval: 15s
  evaluation_interval: 15s

scrape_configs:
  - job_name: 'postgresql'
    scrape_interval: 15s
    static_configs:
      - targets:
        - 'db1:9187'
        - 'db2:9187'
        - 'db3:9187'
    relabel_configs:
      - source_labels: [__address__]
        target_label: instance
        regex: '([^:]+):\d+'
        replacement: '${1}'
```

#### 5.1.3 Grafana仪表盘

**核心面板**：

1. **概览面板**：
   - 连接数、TPS/QPS
   - 缓冲命中率
   - WAL写入速率
   - 检查点频率
   - Autovacuum状态
   - 锁等待

2. **明细面板**：
   - Top表与索引
   - Top查询
   - 复制延迟
   - 磁盘I/O
   - CPU与内存

**Grafana查询示例**：

```promql
# 连接数
pg_stat_database_numbackends{datname="postgres"}

# TPS
rate(pg_stat_database_xact_commit[5m])

# 缓冲命中率
sum(rate(pg_stat_database_blks_hit[5m])) /
(sum(rate(pg_stat_database_blks_hit[5m])) + sum(rate(pg_stat_database_blks_read[5m]))) * 100
```

### 5.2 监控工具选择

**工具对比**：

| 工具 | 类型 | 特点 | 适用场景 |
|------|------|------|---------|
| postgres_exporter | Exporter | 轻量、易部署 | Prometheus生态 |
| pg_stat_monitor | 扩展 | 内置、详细 | 深度分析 |
| pgwatch2 | 完整方案 | 功能全面 | 企业级监控 |
| Datadog | SaaS | 托管、易用 | 云原生 |

### 5.3 告警配置

**告警规则示例**：

```yaml
groups:
  - name: postgresql_alerts
    rules:
      # 连接数告警
      - alert: PostgreSQLHighConnections
        expr: pg_stat_database_numbackends / pg_settings_max_connections > 0.8
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL连接数过高"

      # 缓冲命中率告警
      - alert: PostgreSQLLowCacheHitRatio
        expr: (sum(rate(pg_stat_database_blks_hit[5m])) /
               (sum(rate(pg_stat_database_blks_hit[5m])) +
                sum(rate(pg_stat_database_blks_read[5m])))) < 0.95
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL缓冲命中率过低"

      # 复制延迟告警
      - alert: PostgreSQLReplicationLag
        expr: pg_replication_lag > 104857600  # 100MB
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "PostgreSQL复制延迟过高"
```

## 六、诊断流程（SOP）

### 6.1 性能劣化诊断

**诊断步骤**：

1. **检查实例压力**：连接数、CPU、I/O
2. **慢查询TopN**：使用pg_stat_statements
3. **执行计划分析**：EXPLAIN (ANALYZE, BUFFERS, TIMING)对比估算/实际基数
4. **索引/统计修复**：创建索引、更新统计信息
5. **复测验证**：验证优化效果

**诊断SQL**：

```sql
-- 步骤1：检查系统压力（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始检查系统压力';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    (SELECT count(*) FROM pg_stat_activity) AS connections,
    (SELECT setting FROM pg_settings WHERE name = 'max_connections') AS max_conn,
    (SELECT round(100.0 * sum(blks_hit) / NULLIF(sum(blks_hit) + sum(blks_read), 0), 2)
     FROM pg_stat_database) AS cache_hit_ratio;

-- 步骤2：慢查询TopN（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING '扩展 pg_stat_statements 不存在，无法查询慢查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看慢查询TopN';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    left(query, 100) AS query_preview,
    calls,
    mean_exec_time,
    max_exec_time,
    total_exec_time
FROM pg_stat_statements
ORDER BY mean_exec_time DESC
LIMIT 10;

-- 步骤3：执行计划分析（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, VERBOSE, TIMING)
SELECT * FROM orders WHERE user_id = 123;
```

### 6.2 锁等待/死锁诊断

**诊断步骤**：

1. **查看锁等待链**：使用pg_locks
2. **找出持锁长事务**：关联pg_stat_activity
3. **评估中止与DDL/事务拆分**：决定处理方案
4. **防再发**：索引优化、顺序调整、批量策略

**诊断SQL**：

```sql
-- 查看锁等待链（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看锁等待链';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH lock_tree AS (
    SELECT
        blocked_locks.pid AS blocked_pid,
        blocking_locks.pid AS blocking_pid,
        blocked_activity.query AS blocked_query,
        blocking_activity.query AS blocking_query
    FROM pg_locks blocked_locks
    JOIN pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
    JOIN pg_locks blocking_locks ON blocking_locks.locktype = blocked_locks.locktype
    JOIN pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
    WHERE NOT blocked_locks.granted
      AND blocking_locks.granted
)
SELECT * FROM lock_tree;

-- 找出持锁长事务（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH lock_tree AS (
    SELECT blocking_locks.pid AS blocking_pid
    FROM pg_locks blocked_locks
    JOIN pg_locks blocking_locks ON blocking_locks.locktype = blocked_locks.locktype
    WHERE NOT blocked_locks.granted AND blocking_locks.granted
)
SELECT
    pid,
    usename,
    application_name,
    now() - xact_start AS transaction_duration,
    query
FROM pg_stat_activity
WHERE pid IN (SELECT blocking_pid FROM lock_tree)
ORDER BY xact_start;
```

### 6.3 膨胀/垃圾诊断

**诊断步骤**：

1. **检查死元组**：查看n_dead_tup
2. **检查autovacuum频次**：查看last_autovacuum
3. **提升autovacuum参数**：或离峰VACUUM FULL
4. **长事务排查**：找出阻止VACUUM的长事务

**诊断SQL**：

```sql
-- 检查表膨胀（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始检查表膨胀';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    schemaname,
    relname,
    n_live_tup,
    n_dead_tup,
    round(100.0 * n_dead_tup / NULLIF(n_live_tup + n_dead_tup, 0), 2) AS dead_ratio,
    last_vacuum,
    last_autovacuum,
    last_analyze,
    last_autoanalyze
FROM pg_stat_user_tables
WHERE n_dead_tup > 1000
ORDER BY n_dead_tup DESC
LIMIT 20;

-- 检查阻止VACUUM的长事务（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    usename,
    application_name,
    xact_start,
    now() - xact_start AS transaction_age,
    query
FROM pg_stat_activity
WHERE xact_start IS NOT NULL
  AND now() - xact_start > interval '1 hour'
ORDER BY xact_start;
```

### 6.4 连接问题诊断

**诊断步骤**：

1. **检查连接数**：当前连接数、最大连接数
2. **检查连接状态**：活跃连接、空闲连接
3. **检查连接来源**：客户端IP、应用名称
4. **优化连接池**：使用pgbouncer

**诊断SQL**：

```sql
-- 连接数统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看连接数统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    state,
    count(*) AS connection_count
FROM pg_stat_activity
GROUP BY state;

-- 按应用统计连接（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    application_name,
    state,
    count(*) AS connection_count
FROM pg_stat_activity
GROUP BY application_name, state
ORDER BY connection_count DESC;

-- 长时间空闲连接（带错误处理和性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
    pid,
    usename,
    application_name,
    client_addr,
    state,
    state_change,
    now() - state_change AS idle_duration
FROM pg_stat_activity
WHERE state = 'idle'
  AND now() - state_change > interval '1 hour'
ORDER BY state_change;
```

## 七、知识矩阵对比

### 7.1 监控工具对比

| 工具 | 部署复杂度 | 功能完整性 | 性能开销 | 适用场景 |
|------|-----------|-----------|---------|---------|
| postgres_exporter | 低 | 中 | 低 | Prometheus生态 |
| pg_stat_monitor | 中 | 高 | 中 | 深度分析 |
| pgwatch2 | 中 | 高 | 中 | 企业级监控 |
| Datadog | 低 | 很高 | 低 | 云原生、SaaS |

### 7.2 指标重要性对比

| 指标类别 | 重要性 | 监控频率 | 告警阈值 |
|---------|--------|---------|---------|
| 连接数 | 高 | 实时 | >80% max_connections |
| 缓冲命中率 | 高 | 5分钟 | <95% |
| 慢查询 | 高 | 实时 | >1秒 |
| 锁等待 | 中 | 实时 | >10秒 |
| 复制延迟 | 高 | 1分钟 | >100MB |
| WAL大小 | 中 | 5分钟 | >80% max_wal_size |

## 八、实践案例

### 8.1 案例1：慢查询诊断

**问题**：P99延迟从50ms增加到500ms

**诊断过程**：

1. 检查pg_stat_statements，发现特定查询变慢
2. EXPLAIN (ANALYZE, BUFFERS, TIMING)显示顺序扫描
3. 检查统计信息，发现统计信息过期
4. 执行ANALYZE更新统计信息
5. 创建缺失的索引

**解决方案**：

```sql
-- 更新统计信息（带错误处理）
DO $$
BEGIN
    BEGIN
        ANALYZE orders;
        RAISE NOTICE '统计信息更新成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新统计信息失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 创建索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM pg_indexes
            WHERE indexname = 'idx_orders_user_created'
        ) THEN
            CREATE INDEX CONCURRENTLY idx_orders_user_created
            ON orders(user_id, created_at DESC);
            RAISE NOTICE '索引创建成功';
        ELSE
            RAISE NOTICE '索引已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 8.2 案例2：锁等待诊断

**问题**：大量查询被阻塞

**诊断过程**：

1. 查看pg_locks，发现大量未授予的锁
2. 找出持锁的长事务
3. 发现是DDL操作导致
4. 中止长事务，优化DDL执行时间

**解决方案**：

```sql
-- 查找持锁事务（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查找持锁事务';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
WITH lock_tree AS (
    SELECT blocking_locks.pid AS blocking_pid
    FROM pg_locks blocked_locks
    JOIN pg_locks blocking_locks ON blocking_locks.locktype = blocked_locks.locktype
    WHERE NOT blocked_locks.granted AND blocking_locks.granted
)
SELECT pid, query FROM pg_stat_activity
WHERE pid IN (SELECT blocking_pid FROM lock_tree);

-- 中止阻塞事务（谨慎操作）（带错误处理）
DO $$
DECLARE
    blocking_pid_var INTEGER;
BEGIN
    BEGIN
        -- 注意：实际使用时需要传入具体的blocking_pid
        -- SELECT blocking_pid INTO blocking_pid_var FROM lock_tree LIMIT 1;
        -- IF blocking_pid_var IS NOT NULL THEN
        --     PERFORM pg_terminate_backend(blocking_pid_var);
        --     RAISE NOTICE '已中止阻塞事务: %', blocking_pid_var;
        -- END IF;
        RAISE NOTICE '请谨慎使用 pg_terminate_backend，确保传入正确的进程ID';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '中止事务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 8.3 案例3：性能劣化诊断

**问题**：整体性能下降30%

**诊断过程**：

1. 检查系统资源，发现I/O饱和
2. 检查缓冲命中率，发现命中率下降
3. 检查表膨胀，发现多个表膨胀严重
4. 执行VACUUM和索引重建

**解决方案**：

```sql
-- 执行VACUUM（带错误处理）
DO $$
BEGIN
    BEGIN
        VACUUM ANALYZE large_table;
        RAISE NOTICE 'VACUUM ANALYZE 执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'VACUUM ANALYZE 执行失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 重建索引（带错误处理）
DO $$
BEGIN
    BEGIN
        REINDEX TABLE CONCURRENTLY large_table;
        RAISE NOTICE '索引重建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '索引重建失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

## 九、最佳实践

### 9.1 监控最佳实践

1. **分层监控**：系统资源、数据库实例、对象、查询
2. **关键指标优先**：重点关注连接数、命中率、慢查询
3. **合理告警**：避免告警疲劳，设置合理阈值
4. **定期审查**：定期审查监控配置和告警规则
5. **文档记录**：记录监控配置和告警处理流程

### 9.2 诊断最佳实践

1. **系统化方法**：遵循标准诊断流程
2. **数据驱动**：基于监控数据而非猜测
3. **快速定位**：使用TopN查询快速定位问题
4. **验证方案**：验证解决方案的有效性
5. **预防措施**：制定预防措施避免问题再次发生

## 十、参考资源

### 10.1 官方文档

- [PostgreSQL监控](https://www.postgresql.org/docs/current/monitoring.html)
- [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html)
- [PostgreSQL日志配置](https://www.postgresql.org/docs/current/runtime-config-logging.html)

### 10.2 相关文档

- [执行计划与性能调优](../02-查询与优化/02.03-执行计划/02.04-执行计划与性能调优.md) - 查询优化
- [性能调优实践](./04.06-性能调优实践.md) - 性能调优
- [查询优化器原理](../02-查询与优化/02.01-查询优化器/02.01-查询优化器原理.md) - 优化器原理

---

## 十一、日志与可观测性增强

### 11.1 日志配置

**postgresql.conf关键配置**：

```text
# 日志前缀
log_line_prefix = '%m [%p] %u@%d %r %a '

# 慢查询日志（生产按流量与存储调节）
log_min_duration_statement = 500ms

# 检查点日志
log_checkpoints = on

# Autovacuum日志
log_autovacuum_min_duration = 1s

# 锁等待日志
log_lock_waits = on

# I/O时间跟踪
track_io_timing = on

# 扩展加载
shared_preload_libraries = 'pg_stat_statements,auto_explain'

# auto_explain配置
auto_explain.log_min_duration = '200ms'
auto_explain.log_analyze = on
auto_explain.log_buffers = on
auto_explain.log_nested_statements = on
```

### 11.2 慢日志采集

**日志采集流程**：

1. **日志收集**：Filebeat/Vector收集PostgreSQL日志
2. **日志存储**：发送到Loki/Elasticsearch
3. **日志分析**：在Grafana建立慢SQL面板
4. **日志聚合**：生成fingerprint便于聚合统计

**日志分析示例**：

```sql
-- 从日志中提取慢查询（需要日志解析工具）
-- 生成fingerprint（正则归一化）
SELECT
    regexp_replace(query, '\d+', '?', 'g') AS fingerprint,
    count(*) AS occurrence_count,
    avg(duration) AS avg_duration
FROM slow_query_log
GROUP BY fingerprint
ORDER BY avg_duration DESC;
```

---

## 十二、参考文献

1. PostgreSQL Global Development Group. (2025). PostgreSQL 18 Documentation. <https://www.postgresql.org/docs/18/>
2. PostgreSQL官方文档 - [监控](https://www.postgresql.org/docs/current/monitoring.html)
3. PostgreSQL官方文档 - [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html)
4. Prometheus - [postgres_exporter](https://github.com/prometheus-community/postgres_exporter)
5. Grafana - [PostgreSQL Dashboard](https://grafana.com/grafana/dashboards/)

---

**最后更新**: 2025-01-15
**维护者**: Data Science Team
