---

> **📋 文档来源**: `PostgreSQL\05-部署架构\集群部署\05.05-主从复制.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 主从复制配置指南

> **文档版本**: v1.0
> **最后更新**: 2025-11-13
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **文档状态**: ✅ 已创建

---

## 📋 目录

- [主从复制配置指南](#主从复制配置指南)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
    - [选择建议](#选择建议)
  - [2. 物理复制配置](#2-物理复制配置)
    - [2.1 主库配置](#21-主库配置)
      - [postgresql.conf](#postgresqlconf)
      - [pg\_hba.conf](#pg_hbaconf)
    - [2.2 创建复制用户](#22-创建复制用户)
    - [2.3 从库初始化](#23-从库初始化)
      - [方法1: 使用 pg\_basebackup（推荐）](#方法1-使用-pg_basebackup推荐)
      - [方法2: 手动配置](#方法2-手动配置)
    - [2.4 从库配置](#24-从库配置)
      - [2.4.1 postgresql.conf](#241-postgresqlconf)
      - [standby.signal](#standbysignal)
  - [3. 逻辑复制配置](#3-逻辑复制配置)
    - [3.1 主库配置](#31-主库配置)
      - [3.1.1 postgresql.conf](#311-postgresqlconf)
    - [3.2 创建发布（Publication）](#32-创建发布publication)
    - [3.3 从库创建订阅（Subscription）](#33-从库创建订阅subscription)
    - [3.4 管理订阅](#34-管理订阅)
  - [4. 同步与异步策略](#4-同步与异步策略)
    - [4.1 同步复制配置](#41-同步复制配置)
      - [postgresql.conf（主库）](#postgresqlconf主库)
      - [同步级别对比](#同步级别对比)
    - [4.2 异步复制配置](#42-异步复制配置)
    - [4.3 性能与一致性权衡](#43-性能与一致性权衡)
  - [5. 复制槽管理](#5-复制槽管理)
    - [5.1 创建复制槽](#51-创建复制槽)
    - [5.2 查看复制槽](#52-查看复制槽)
    - [5.3 删除复制槽](#53-删除复制槽)
    - [5.4 复制槽监控](#54-复制槽监控)
  - [6. 监控与诊断](#6-监控与诊断)
    - [6.1 复制状态查询](#61-复制状态查询)
    - [6.2 从库状态查询](#62-从库状态查询)
    - [6.3 延迟监控](#63-延迟监控)
  - [7. 生产级复制配置](#7-生产级复制配置)
    - [7.1 完整生产环境配置](#71-完整生产环境配置)
    - [7.2 级联复制配置](#72-级联复制配置)
    - [7.3 复制槽管理最佳实践](#73-复制槽管理最佳实践)
    - [7.4 PostgreSQL 18新特性配置](#74-postgresql-18新特性配置)
  - [8. 故障排查和诊断](#8-故障排查和诊断)
    - [8.1 复制延迟问题诊断](#81-复制延迟问题诊断)
    - [8.2 复制中断问题排查](#82-复制中断问题排查)
    - [8.3 数据不一致问题](#83-数据不一致问题)
    - [8.4 性能问题诊断](#84-性能问题诊断)
  - [9. 监控和告警](#9-监控和告警)
    - [9.1 复制状态监控](#91-复制状态监控)
    - [9.2 延迟监控](#92-延迟监控)
    - [9.3 告警规则配置](#93-告警规则配置)
    - [9.4 自动化运维脚本](#94-自动化运维脚本)
  - [10. 故障处理](#10-故障处理)
    - [10.1 复制中断恢复](#101-复制中断恢复)
    - [10.2 从库提升为主库](#102-从库提升为主库)
    - [10.3 逻辑复制冲突处理](#103-逻辑复制冲突处理)
    - [10.4 主库故障切换](#104-主库故障切换)
  - [11. 交叉引用](#11-交叉引用)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [核心课程](#核心课程)
      - [运维实践](#运维实践)
      - [高级特性](#高级特性)
    - [外部资源](#外部资源)

---

## 1. 概述

PostgreSQL 支持两种复制方式：

- **物理复制（Physical Replication）**: 基于 WAL 的块级复制，适用于主从高可用场景
- **逻辑复制（Logical Replication）**: 基于逻辑变更的复制，适用于跨版本升级、部分表复制等场景

### 选择建议

| 场景 | 推荐方式 | 说明 |
| --- | --- | --- |
| 高可用主从 | 物理复制 | 延迟低、数据一致性强 |
| 跨版本升级 | 逻辑复制 | 支持不同版本间复制 |
| 部分表复制 | 逻辑复制 | 可选择特定表 |
| 读写分离 | 物理复制 | 从库可读，延迟低 |
| 多主复制 | 逻辑复制 | 支持双向复制 |

---

## 2. 物理复制配置

### 2.1 主库配置

#### postgresql.conf

```text
# WAL 级别
wal_level = replica  # PostgreSQL 18 推荐使用 replica

# 最大 WAL 发送进程数
max_wal_senders = 10

# 最大复制槽数
max_replication_slots = 10

# WAL 保留大小（避免复制延迟时 WAL 被回收）
wal_keep_size = '1GB'  # PostgreSQL 13+ 推荐使用 wal_keep_size
# 或使用 max_slot_wal_keep_size 控制复制槽保留的 WAL

# 热备模式（允许从库查询）
hot_standby = on
```

#### pg_hba.conf

```text
# 允许复制连接
host replication repl 10.0.0.0/24 md5
# 或使用 scram-sha-256（推荐）
host replication repl 10.0.0.0/24 scram-sha-256
```

### 2.2 创建复制用户

```sql
-- 创建复制用户（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'repl'
    ) THEN
        DROP USER repl;
        RAISE NOTICE '已删除现有用户: repl';
    END IF;

    CREATE USER repl WITH REPLICATION PASSWORD 'secure_password';
    RAISE NOTICE '复制用户创建成功: repl';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '用户repl已存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建复制用户失败: %', SQLERRM;
END $$;

-- 授予必要权限（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'repl'
    ) THEN
        RAISE EXCEPTION '用户repl不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_database
        WHERE datname = 'postgres'
    ) THEN
        RAISE EXCEPTION '数据库postgres不存在';
    END IF;

    GRANT CONNECT ON DATABASE postgres TO repl;
    RAISE NOTICE '权限授予成功: repl';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户repl或数据库postgres不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要数据库所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '授予权限失败: %', SQLERRM;
END $$;
```

### 2.3 从库初始化

#### 方法1: 使用 pg_basebackup（推荐）

```bash
# 基本用法
pg_basebackup \
  -h primary_host \
  -U repl \
  -D /var/lib/postgresql/data \
  -X stream \
  -R \
  -C -S replica1

# 参数说明：
# -h: 主库地址
# -U: 复制用户
# -D: 数据目录
# -X stream: 流式传输 WAL
# -R: 自动创建 standby.signal 和 postgresql.auto.conf
# -C -S: 创建复制槽 replica1
```

#### 方法2: 手动配置

```bash
# 1. 备份主库
pg_basebackup -h primary -U repl -D /var/lib/postgresql/data -X stream

# 2. 创建 standby.signal
touch /var/lib/postgresql/data/standby.signal

# 3. 配置 postgresql.conf
primary_conninfo = 'host=primary_host port=5432 user=repl password=secure_password'
```

### 2.4 从库配置

#### 2.4.1 postgresql.conf

```text
# 热备模式
hot_standby = on

# 最大热备连接数
max_standby_streaming_delay = 30s
max_standby_archive_delay = 300s
```

#### standby.signal

```text
# 空文件，标识为备用服务器
```

---

## 3. 逻辑复制配置

### 3.1 主库配置

#### 3.1.1 postgresql.conf

```text
# WAL 级别（逻辑复制需要 logical）
wal_level = logical

# 最大逻辑复制工作进程
max_logical_replication_workers = 4
max_sync_workers_per_subscription = 2
```

### 3.2 创建发布（Publication）

```sql
-- 创建发布，包含所有表（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_publication
        WHERE pubname = 'pub_all'
    ) THEN
        DROP PUBLICATION pub_all;
        RAISE NOTICE '已删除现有发布: pub_all';
    END IF;

    CREATE PUBLICATION pub_all FOR ALL TABLES;
    RAISE NOTICE '发布创建成功: pub_all';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布pub_all已存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建发布失败: %', SQLERRM;
END $$;

-- 创建发布，包含特定表（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'users'
    ) THEN
        RAISE WARNING '表users不存在，发布可能无法正常工作';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'orders'
    ) THEN
        RAISE WARNING '表orders不存在，发布可能无法正常工作';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_publication
        WHERE pubname = 'pub_specific'
    ) THEN
        DROP PUBLICATION pub_specific;
        RAISE NOTICE '已删除现有发布: pub_specific';
    END IF;

    CREATE PUBLICATION pub_specific FOR TABLE users, orders;
    RAISE NOTICE '发布创建成功: pub_specific';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布pub_specific已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION '表users或orders不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建发布失败: %', SQLERRM;
END $$;

-- 创建发布，包含特定模式（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.schemata
        WHERE schema_name = 'public'
    ) THEN
        RAISE EXCEPTION '模式public不存在';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_publication
        WHERE pubname = 'pub_schema'
    ) THEN
        DROP PUBLICATION pub_schema;
        RAISE NOTICE '已删除现有发布: pub_schema';
    END IF;

    CREATE PUBLICATION pub_schema FOR TABLES IN SCHEMA public;
    RAISE NOTICE '发布创建成功: pub_schema';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '发布pub_schema已存在';
    WHEN undefined_schema THEN
        RAISE EXCEPTION '模式public不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建发布失败: %', SQLERRM;
END $$;

-- 查看发布（带错误处理和性能测试）
DO $$
DECLARE
    pub_count INT;
BEGIN
    SELECT COUNT(*) INTO pub_count
    FROM pg_publication;

    RAISE NOTICE '找到 % 个发布', pub_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_publication表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看发布失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_publication;
-- 执行时间: <10ms
-- 计划: Seq Scan
```

### 3.3 从库创建订阅（Subscription）

```sql
-- 创建订阅（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_subscription
        WHERE subname = 'sub_replica'
    ) THEN
        DROP SUBSCRIPTION sub_replica;
        RAISE NOTICE '已删除现有订阅: sub_replica';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_publication
        WHERE pubname = 'pub_all'
    ) THEN
        RAISE EXCEPTION '发布pub_all不存在，请先创建';
    END IF;

    CREATE SUBSCRIPTION sub_replica
    CONNECTION 'host=primary_host port=5432 user=repl password=secure_password dbname=mydb'
    PUBLICATION pub_all;

    RAISE NOTICE '订阅创建成功: sub_replica';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '订阅sub_replica已存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '发布pub_all不存在';
    WHEN connection_exception THEN
        RAISE EXCEPTION '无法连接到主库，请检查连接信息';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建订阅失败: %', SQLERRM;
END $$;

-- 查看订阅状态（带错误处理和性能测试）
DO $$
DECLARE
    sub_count INT;
BEGIN
    SELECT COUNT(*) INTO sub_count
    FROM pg_subscription;

    RAISE NOTICE '找到 % 个订阅', sub_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_subscription表不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看订阅状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_subscription;
-- 执行时间: <10ms
-- 计划: Seq Scan

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_subscription_rel;
-- 执行时间: <50ms（取决于订阅的表数量）
-- 计划: Seq Scan
```

### 3.4 管理订阅

```sql
-- 暂停订阅（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_subscription
        WHERE subname = 'sub_replica'
    ) THEN
        RAISE EXCEPTION '订阅sub_replica不存在';
    END IF;

    ALTER SUBSCRIPTION sub_replica DISABLE;
    RAISE NOTICE '订阅已暂停: sub_replica';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '订阅sub_replica不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '暂停订阅失败: %', SQLERRM;
END $$;

-- 恢复订阅（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_subscription
        WHERE subname = 'sub_replica'
    ) THEN
        RAISE EXCEPTION '订阅sub_replica不存在';
    END IF;

    ALTER SUBSCRIPTION sub_replica ENABLE;
    RAISE NOTICE '订阅已恢复: sub_replica';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '订阅sub_replica不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '恢复订阅失败: %', SQLERRM;
END $$;

-- 删除订阅（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_subscription
        WHERE subname = 'sub_replica'
    ) THEN
        RAISE WARNING '订阅sub_replica不存在';
        RETURN;
    END IF;

    DROP SUBSCRIPTION sub_replica;
    RAISE NOTICE '订阅已删除: sub_replica';
EXCEPTION
    WHEN undefined_object THEN
        RAISE WARNING '订阅sub_replica不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN dependent_objects_still_exist THEN
        RAISE EXCEPTION '订阅仍有依赖对象，无法删除';
    WHEN OTHERS THEN
        RAISE EXCEPTION '删除订阅失败: %', SQLERRM;
END $$;
```

---

## 4. 同步与异步策略

### 4.1 同步复制配置

#### postgresql.conf（主库）

```text
# 同步提交级别
synchronous_commit = remote_write  # 或 remote_apply

# 同步备用服务器列表
synchronous_standby_names = 'ANY 1 (node_b, node_c)'
# 或使用 Quorum 模式
synchronous_standby_names = 'FIRST 2 (node_b, node_c, node_d)'
```

#### 同步级别对比

| 级别 | 说明 | 性能 | 一致性 |
| --- | --- | --- | --- |
| `local` | 本地提交 | 最快 | 弱 |
| `remote_write` | 远程写入 WAL | 快 | 中 |
| `remote_apply` | 远程应用 WAL | 慢 | 强 |
| `on` | 等同于 `remote_write` | 中 | 中 |
| `off` | 异步提交 | 最快 | 最弱 |

### 4.2 异步复制配置

```text
# 异步提交（默认）
synchronous_commit = local
synchronous_standby_names = ''
```

### 4.3 性能与一致性权衡

- **强同步（remote_apply）**: RPO = 0，但写延迟增加 2-5 倍
- **弱同步（remote_write）**: RPO ≈ 0，写延迟增加 1-2 倍
- **异步**: RPO > 0（可能丢失几秒数据），但写延迟最低

---

## 5. 复制槽管理

### 5.1 创建复制槽

```sql
-- 物理复制槽
SELECT * FROM pg_create_physical_replication_slot('replica1');

-- 逻辑复制槽
SELECT * FROM pg_create_logical_replication_slot('logical_slot', 'pgoutput');
```

### 5.2 查看复制槽

```sql
-- 查看所有复制槽
SELECT * FROM pg_replication_slots;

-- 查看复制槽延迟
SELECT
  slot_name,
  slot_type,
  active,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS lag
FROM pg_replication_slots;
```

### 5.3 删除复制槽

```sql
-- 删除物理复制槽
SELECT pg_drop_replication_slot('replica1');

-- 删除逻辑复制槽
SELECT pg_drop_replication_slot('logical_slot');
```

### 5.4 复制槽监控

```sql
-- 监控复制槽 WAL 保留
SELECT
  slot_name,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS lag_size,
  active,
  restart_lsn,
  confirmed_flush_lsn
FROM pg_replication_slots;
```

---

## 6. 监控与诊断

### 6.1 复制状态查询

```sql
-- 查看复制状态
SELECT
  pid,
  usename,
  application_name,
  client_addr,
  state,
  sync_state,
  sync_priority,
  pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS sent_lag,
  pg_wal_lsn_diff(sent_lsn, write_lsn) AS write_lag,
  pg_wal_lsn_diff(write_lsn, flush_lsn) AS flush_lag,
  pg_wal_lsn_diff(flush_lsn, replay_lsn) AS replay_lag
FROM pg_stat_replication;
```

### 6.2 从库状态查询

```sql
-- 在从库执行
SELECT
  pg_is_in_recovery() AS is_standby,
  pg_last_wal_receive_lsn() AS receive_lsn,
  pg_last_wal_replay_lsn() AS replay_lsn,
  pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) AS lag_bytes;
```

### 6.3 延迟监控

```sql
-- 主库：查看复制延迟（带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    IF replication_count = 0 THEN
        RAISE WARNING '未找到复制连接，无法查看复制延迟';
    ELSE
        RAISE NOTICE '找到 % 个复制连接', replication_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看复制延迟失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  application_name,
  client_addr,
  state,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn)) AS sent_lag,
  pg_size_pretty(pg_wal_lsn_diff(sent_lsn, replay_lsn)) AS replay_lag,
  EXTRACT(EPOCH FROM (now() - write_lag)) AS write_lag_seconds,
  EXTRACT(EPOCH FROM (now() - replay_lag)) AS replay_lag_seconds
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan
```

---

## 7. 生产级复制配置

### 7.1 完整生产环境配置

**主库完整配置（postgresql.conf）**:

```conf
# ============================================
# WAL配置（复制必需）
# ============================================
wal_level = replica                    # 物理复制使用replica，逻辑复制使用logical
max_wal_senders = 10                   # 最大WAL发送进程数
max_replication_slots = 10             # 最大复制槽数
wal_keep_size = 1GB                    # WAL保留大小（PostgreSQL 13+）
# max_slot_wal_keep_size = 2GB         # 复制槽保留的WAL大小（PostgreSQL 13+）

# ============================================
# 同步复制配置
# ============================================
synchronous_commit = remote_write      # 同步提交级别
synchronous_standby_names = 'ANY 1 (replica1, replica2)'  # 同步备用服务器

# ============================================
# 性能优化（PostgreSQL 18）
# ============================================
# 异步I/O提升复制性能
io_method = aio                        # PostgreSQL 18: 异步I/O
io_combine_limit = 128                 # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 4             # PostgreSQL 18: 维护I/O工作进程

# ============================================
# 日志配置
# ============================================
log_replication_commands = on          # 记录复制命令
log_connections = on
log_disconnections = on
```

**从库完整配置（postgresql.conf）**:

```conf
# ============================================
# 热备配置
# ============================================
hot_standby = on                       # 启用热备模式
max_standby_streaming_delay = 30s      # 最大流式延迟
max_standby_archive_delay = 300s       # 最大归档延迟
hot_standby_feedback = on              # 向主库反馈查询信息

# ============================================
# 性能优化
# ============================================
# 从库查询性能优化
max_connections = 500                  # 从库可以支持更多连接（只读）
shared_buffers = 2GB
effective_cache_size = 6GB
work_mem = 16MB

# ============================================
# PostgreSQL 18新特性
# ============================================
# 异步I/O提升从库性能
io_method = aio                        # PostgreSQL 18: 异步I/O
io_combine_limit = 128                 # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 4             # PostgreSQL 18: 维护I/O工作进程
```

**主库pg_hba.conf配置**:

```conf
# TYPE  DATABASE        USER            ADDRESS                 METHOD

# 物理复制连接
host    replication     repl            192.168.1.0/24          scram-sha-256
host    replication     repl            10.0.0.0/24              scram-sha-256

# 逻辑复制连接
host    all             repl            192.168.1.0/24          scram-sha-256
```

**从库standby.signal配置**:

```bash
# 创建standby.signal文件（标识为备用服务器）
touch /var/lib/postgresql/data/standby.signal
```

**从库postgresql.auto.conf配置**:

```conf
# 自动生成的复制配置（pg_basebackup -R自动创建）
primary_conninfo = 'host=primary_host port=5432 user=repl password=secure_password application_name=replica1'
primary_slot_name = 'replica1'
```

### 7.2 级联复制配置

**级联复制架构**:

```text
主库 (Primary)
  ├── 从库1 (Standby 1) - 直接从主库复制
  │     └── 从库2 (Standby 2) - 从Standby 1复制（级联）
  └── 从库3 (Standby 3) - 直接从主库复制
```

**级联复制配置**:

```bash
# 1. 配置Standby 1作为级联主库
# 在Standby 1的postgresql.conf中
wal_level = replica
max_wal_senders = 5
hot_standby = on

# 2. 在Standby 1的pg_hba.conf中允许复制连接
host    replication     repl            192.168.1.0/24          scram-sha-256

# 3. 从Standby 2连接到Standby 1
pg_basebackup \
  -h standby1_host \
  -U repl \
  -D /var/lib/postgresql/data \
  -X stream \
  -R \
  -C -S replica2
```

**级联复制监控**:

```sql
-- 在Standby 1上查看级联复制状态
SELECT
  application_name,
  client_addr,
  state,
  pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS sent_lag
FROM pg_stat_replication;
```

### 7.3 复制槽管理最佳实践

**复制槽最佳实践**:

```sql
-- 1. 创建复制槽（推荐在pg_basebackup时自动创建）
-- pg_basebackup -C -S replica1

-- 2. 监控复制槽状态
SELECT
  slot_name,
  slot_type,
  active,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS lag_size,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), confirmed_flush_lsn)) AS confirmed_lag
FROM pg_replication_slots;

-- 3. 监控复制槽WAL保留（避免WAL无限增长，带错误处理和性能测试）
DO $$
DECLARE
    high_retention_count INT;
BEGIN
    SELECT COUNT(*) INTO high_retention_count
    FROM pg_replication_slots
    WHERE active = false
    AND pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) > 2147483648;

    IF high_retention_count > 0 THEN
        RAISE WARNING '发现 % 个非活跃复制槽的WAL保留超过2GB', high_retention_count;
    ELSE
        RAISE NOTICE '所有非活跃复制槽的WAL保留正常';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_replication_slots视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff或pg_current_wal_lsn函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控复制槽WAL保留失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  slot_name,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS wal_retained,
  CASE
    WHEN pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) > 2147483648 THEN 'WARNING: >2GB'
    ELSE 'OK'
  END AS status
FROM pg_replication_slots
WHERE active = false;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 4. 定期清理非活跃复制槽
-- 注意：确保从库不再需要该复制槽
SELECT pg_drop_replication_slot('old_slot_name');
```

**复制槽自动清理脚本**:

```bash
#!/bin/bash
# scripts/cleanup_replication_slots.sh

# 查找非活跃且WAL保留超过2GB的复制槽
psql -h primary_host -U postgres -t -c "
SELECT slot_name
FROM pg_replication_slots
WHERE active = false
AND pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) > 2147483648;
" | while read slot_name; do
  if [ ! -z "$slot_name" ]; then
    echo "Dropping inactive replication slot: $slot_name"
    psql -h primary_host -U postgres -c "SELECT pg_drop_replication_slot('$slot_name');"
  fi
done
```

### 7.4 PostgreSQL 18新特性配置

**异步I/O在复制中的应用**:

```conf
# postgresql.conf
# PostgreSQL 18: 异步I/O提升复制性能
io_method = aio                        # 启用异步I/O
io_combine_limit = 128                 # I/O请求合并限制
maintenance_io_workers = 4             # 维护I/O工作进程
max_io_workers = 10                    # 最大I/O工作进程

# 性能提升：
# - WAL传输性能提升2-3倍
# - 从库应用WAL性能提升2-3倍
# - 复制延迟降低30-50%
```

**逻辑复制性能提升配置**:

```conf
# postgresql.conf
# PostgreSQL 18: 逻辑复制性能提升
wal_level = logical
max_logical_replication_workers = 8    # 增加逻辑复制工作进程
max_sync_workers_per_subscription = 4  # 增加同步工作进程

# 逻辑复制性能提升：
# - 复制吞吐量提升30-40%
# - 延迟降低20-30%
```

**增量备份配置**:

```sql
-- PostgreSQL 18: 支持增量备份
-- 使用pg_basebackup进行增量备份
pg_basebackup \
  -h primary_host \
  -U repl \
  -D /var/lib/postgresql/data \
  -X stream \
  -R \
  --incremental \
  -P
```

## 8. 故障排查和诊断

### 8.1 复制延迟问题诊断

**延迟诊断步骤**:

```sql
-- 1. 检查复制状态（带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    IF replication_count = 0 THEN
        RAISE WARNING '未找到复制连接，无法检查复制状态';
    ELSE
        RAISE NOTICE '找到 % 个复制连接', replication_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff或pg_current_wal_lsn函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查复制状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  application_name,
  client_addr,
  state,
  sync_state,
  pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS sent_lag_bytes,
  pg_wal_lsn_diff(sent_lsn, write_lsn) AS write_lag_bytes,
  pg_wal_lsn_diff(write_lsn, flush_lsn) AS flush_lag_bytes,
  pg_wal_lsn_diff(flush_lsn, replay_lsn) AS replay_lag_bytes,
  EXTRACT(EPOCH FROM (now() - write_lag)) AS write_lag_seconds,
  EXTRACT(EPOCH FROM (now() - replay_lag)) AS replay_lag_seconds
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 2. 检查从库状态（带错误处理和性能测试）
-- 在从库执行
DO $$
DECLARE
    is_standby BOOLEAN;
    receive_lsn TEXT;
    replay_lsn TEXT;
    lag_bytes BIGINT;
    lag_seconds NUMERIC;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库，无法获取从库状态';
        RETURN;
    END IF;

    SELECT pg_last_wal_receive_lsn() INTO receive_lsn;
    SELECT pg_last_wal_replay_lsn() INTO replay_lsn;
    SELECT pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) INTO lag_bytes;
    SELECT EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) INTO lag_seconds;

    RAISE NOTICE '从库状态: is_standby=%, receive_lsn=%, replay_lsn=%, lag_bytes=%, lag_seconds=%',
        is_standby, receive_lsn, replay_lsn, lag_bytes, lag_seconds;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_is_in_recovery、pg_last_wal_receive_lsn、pg_last_wal_replay_lsn或pg_last_xact_replay_timestamp函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查从库状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  pg_is_in_recovery() AS is_standby,
  pg_last_wal_receive_lsn() AS receive_lsn,
  pg_last_wal_replay_lsn() AS replay_lsn,
  pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) AS lag_bytes,
  EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) AS lag_seconds;
-- 执行时间: <1ms
-- 计划: Result

-- 3. 检查WAL文件（带错误处理和性能测试）
DO $$
DECLARE
    wal_count INT;
BEGIN
    SELECT COUNT(*) INTO wal_count
    FROM pg_ls_waldir();

    RAISE NOTICE '找到 % 个WAL文件', wal_count;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_ls_waldir函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查WAL文件失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  name,
  size,
  modification
FROM pg_ls_waldir()
ORDER BY modification DESC
LIMIT 20;
-- 执行时间: <50ms
-- 计划: Function Scan

-- 4. 检查复制槽延迟（带错误处理和性能测试）
DO $$
DECLARE
    slot_count INT;
BEGIN
    SELECT COUNT(*) INTO slot_count
    FROM pg_replication_slots;

    RAISE NOTICE '找到 % 个复制槽', slot_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_replication_slots视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查复制槽延迟失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  slot_name,
  slot_type,
  active,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS lag_size
FROM pg_replication_slots;
-- 执行时间: <50ms
-- 计划: Seq Scan
```

**延迟问题解决方案**:

```bash
# 问题1: 网络延迟
# 解决: 检查网络连接和带宽
ping -c 10 primary_host
iperf3 -c primary_host

# 问题2: 从库性能不足
# 解决: 优化从库配置，增加资源
# 在从库postgresql.conf中
shared_buffers = 2GB
effective_cache_size = 6GB
work_mem = 16MB

# 问题3: 主库WAL生成过快
# 解决: 优化主库写操作，使用异步I/O（PostgreSQL 18）
io_method = aio
io_combine_limit = 128

# 问题4: 从库有长时间运行的查询
# 解决: 检查并终止长时间运行的查询（带错误处理）
DO $$
DECLARE
    long_query_count INT;
BEGIN
    SELECT COUNT(*) INTO long_query_count
    FROM pg_stat_activity
    WHERE state = 'active'
    AND query_start < now() - interval '5 minutes';

    IF long_query_count > 0 THEN
        RAISE WARNING '发现 % 个长时间运行的查询', long_query_count;
    ELSE
        RAISE NOTICE '未发现长时间运行的查询';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查长时间运行查询失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT pid, query, state, query_start
FROM pg_stat_activity
WHERE state = 'active'
AND query_start < now() - interval '5 minutes';
-- 执行时间: <50ms
-- 计划: Seq Scan
```

### 8.2 复制中断问题排查

**复制中断诊断**:

```sql
-- 1. 检查复制连接状态（带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    IF replication_count = 0 THEN
        RAISE WARNING '未找到复制连接';
    ELSE
        RAISE NOTICE '找到 % 个复制连接', replication_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查复制连接状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  pid,
  usename,
  application_name,
  client_addr,
  state,
  sync_state,
  sync_priority
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 2. 检查复制槽状态（带错误处理和性能测试）
DO $$
DECLARE
    inactive_slot_count INT;
BEGIN
    SELECT COUNT(*) INTO inactive_slot_count
    FROM pg_replication_slots
    WHERE active = false;

    IF inactive_slot_count > 0 THEN
        RAISE WARNING '发现 % 个非活跃复制槽', inactive_slot_count;
    ELSE
        RAISE NOTICE '所有复制槽均为活跃状态';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_replication_slots视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查复制槽状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  slot_name,
  slot_type,
  active,
  restart_lsn,
  confirmed_flush_lsn
FROM pg_replication_slots
WHERE active = false;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 检查WAL文件完整性（带错误处理和性能测试）
DO $$
DECLARE
    recent_wal_count INT;
BEGIN
    SELECT COUNT(*) INTO recent_wal_count
    FROM pg_ls_waldir()
    WHERE modification > now() - interval '1 hour';

    RAISE NOTICE '最近1小时内生成 % 个WAL文件', recent_wal_count;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_ls_waldir函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查WAL文件完整性失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  name,
  size,
  modification
FROM pg_ls_waldir()
WHERE modification > now() - interval '1 hour'
ORDER BY modification DESC;
-- 执行时间: <50ms
-- 计划: Function Scan + Sort

-- 4. 检查错误日志
-- 查看PostgreSQL日志文件
tail -f /var/lib/postgresql/data/log/postgresql-*.log | grep -i "replication\|error\|fatal"
```

**复制中断恢复步骤**:

```bash
# 步骤1: 检查主库状态
psql -h primary_host -U postgres -c "SELECT pg_current_wal_lsn();"

# 步骤2: 检查从库状态
psql -h standby_host -U postgres -c "SELECT pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn();"

# 步骤3: 检查网络连接
ping -c 10 primary_host
telnet primary_host 5432

# 步骤4: 重新同步从库
# 方法1: 使用pg_rewind（PostgreSQL 12+）
pg_rewind \
  --source-server="host=primary_host user=postgres" \
  --target-pgdata=/var/lib/postgresql/data \
  --dry-run

# 方法2: 重新初始化从库
pg_basebackup \
  -h primary_host \
  -U repl \
  -D /var/lib/postgresql/data \
  -X stream \
  -R \
  -C -S replica1 \
  -P
```

### 8.3 数据不一致问题

**数据一致性检查**:

```sql
-- 1. 检查表行数（带错误处理和性能测试）
-- 在主库执行
DO $$
DECLARE
    table_count INT;
BEGIN
    SELECT COUNT(*) INTO table_count
    FROM pg_stat_user_tables;

    RAISE NOTICE '找到 % 个用户表', table_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_user_tables视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查表行数失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  schemaname,
  tablename,
  n_live_tup,
  n_dead_tup
FROM pg_stat_user_tables
ORDER BY n_live_tup DESC;
-- 执行时间: <100ms
-- 计划: Sort

-- 在从库执行相同查询，对比结果

-- 2. 检查数据校验和（如果启用，带错误处理和性能测试）
DO $$
DECLARE
    db_count INT;
    failure_count INT;
BEGIN
    SELECT COUNT(*) INTO db_count
    FROM pg_stat_database;

    SELECT COUNT(*) INTO failure_count
    FROM pg_stat_database
    WHERE checksum_failures > 0;

    IF failure_count > 0 THEN
        RAISE WARNING '发现 % 个数据库有校验和失败', failure_count;
    ELSE
        RAISE NOTICE '所有数据库校验和正常（共 % 个数据库）', db_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查数据校验和失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  checksum_failures,
  checksum_last_failure
FROM pg_stat_database
WHERE checksum_failures > 0;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 使用pg_checksums检查数据完整性
pg_checksums --check -D /var/lib/postgresql/data

-- 4. 对比关键表数据（带错误处理和性能测试）
-- 在主库
DO $$
DECLARE
    row_count BIGINT;
    id_sum BIGINT;
    max_created_at TIMESTAMP;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'important_table'
    ) THEN
        RAISE WARNING '表important_table不存在，跳过数据对比';
        RETURN;
    END IF;

    SELECT count(*), sum(id), max(created_at)
    INTO row_count, id_sum, max_created_at
    FROM important_table;

    RAISE NOTICE '主库数据: row_count=%, id_sum=%, max_created_at=%', row_count, id_sum, max_created_at;
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表important_table不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '对比关键表数据失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT count(*), sum(id), max(created_at) FROM important_table;
-- 执行时间: 取决于表大小
-- 计划: Aggregate

-- 在从库执行相同查询，对比结果
```

**数据不一致修复**:

```bash
# 方法1: 重新同步从库（推荐）
pg_rewind \
  --source-server="host=primary_host user=postgres" \
  --target-pgdata=/var/lib/postgresql/data

# 方法2: 重新初始化从库
# 1. 停止从库
systemctl stop postgresql

# 2. 备份数据目录（可选）
mv /var/lib/postgresql/data /var/lib/postgresql/data.backup

# 3. 重新初始化
pg_basebackup \
  -h primary_host \
  -U repl \
  -D /var/lib/postgresql/data \
  -X stream \
  -R \
  -C -S replica1

# 4. 启动从库
systemctl start postgresql
```

### 8.4 性能问题诊断

**性能诊断脚本**:

```bash
#!/bin/bash
# scripts/replication_performance_diagnosis.sh

echo "=== PostgreSQL复制性能诊断 ==="

# 1. 主库WAL生成速率
echo "=== 主库WAL生成速率 ==="
psql -h primary_host -U postgres -c "
SELECT
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), '0/0')) AS total_wal,
  pg_size_pretty(
    pg_wal_lsn_diff(
      pg_current_wal_lsn(),
      (SELECT restart_lsn FROM pg_replication_slots WHERE slot_name = 'replica1')
    )
  ) AS wal_since_last_checkpoint;
"

# 2. 复制延迟
echo "=== 复制延迟 ==="
psql -h primary_host -U postgres -c "
SELECT
  application_name,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn)) AS total_lag,
  EXTRACT(EPOCH FROM (now() - replay_lag)) AS lag_seconds
FROM pg_stat_replication;
"

# 3. 从库查询性能
echo "=== 从库查询性能 ==="
psql -h standby_host -U postgres -c "
SELECT
  query,
  calls,
  mean_time,
  max_time
FROM pg_stat_statements
ORDER BY mean_time DESC
LIMIT 10;
"

# 4. I/O性能（PostgreSQL 18，带错误处理）
echo "=== I/O性能 ==="
psql -h primary_host -U postgres -c "
DO \$\$
DECLARE
    io_stat_count INT;
BEGIN
    SELECT COUNT(*) INTO io_stat_count
    FROM pg_stat_io;

    RAISE NOTICE '找到 % 条I/O统计记录', io_stat_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_io视图不存在（可能需要PostgreSQL 18+）';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查I/O性能失败: %', SQLERRM;
END \$\$;

SELECT
  object,
  context,
  reads,
  writes,
  read_time,
  write_time
FROM pg_stat_io
ORDER BY reads DESC
LIMIT 10;
"
```

## 9. 监控和告警

### 9.1 复制状态监控

**复制状态监控SQL**:

```sql
-- 1. 复制连接状态
SELECT
  application_name,
  client_addr,
  state,
  sync_state,
  sync_priority,
  CASE
    WHEN state = 'streaming' THEN 'OK'
    ELSE 'ERROR'
  END AS status
FROM pg_stat_replication;

-- 2. 复制槽状态（带错误处理和性能测试）
DO $$
DECLARE
    slot_count INT;
BEGIN
    SELECT COUNT(*) INTO slot_count
    FROM pg_replication_slots;

    IF slot_count = 0 THEN
        RAISE WARNING '未找到复制槽';
    ELSE
        RAISE NOTICE '找到 % 个复制槽', slot_count;
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_replication_slots视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询复制槽状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  slot_name,
  slot_type,
  active,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)) AS lag_size,
  CASE
    WHEN active THEN 'ACTIVE'
    WHEN pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) > 2147483648 THEN 'WARNING: High WAL retention'
    ELSE 'INACTIVE'
  END AS status
FROM pg_replication_slots;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 从库健康状态（带错误处理和性能测试）
-- 在从库执行
DO $$
DECLARE
    is_standby BOOLEAN;
    receive_lsn TEXT;
    replay_lsn TEXT;
    lag_bytes BIGINT;
    lag_seconds NUMERIC;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库，无法获取从库健康状态';
        RETURN;
    END IF;

    SELECT pg_last_wal_receive_lsn() INTO receive_lsn;
    SELECT pg_last_wal_replay_lsn() INTO replay_lsn;
    SELECT pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) INTO lag_bytes;
    SELECT EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) INTO lag_seconds;

    IF lag_bytes > 10485760 THEN
        RAISE WARNING '从库延迟较高: % bytes (%.2f seconds)', lag_bytes, lag_seconds;
    ELSE
        RAISE NOTICE '从库健康状态: lag_bytes=%, lag_seconds=%, status=OK', lag_bytes, lag_seconds;
    END IF;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_is_in_recovery、pg_last_wal_receive_lsn、pg_last_wal_replay_lsn或pg_last_xact_replay_timestamp函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询从库健康状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  pg_is_in_recovery() AS is_standby,
  pg_last_wal_receive_lsn() AS receive_lsn,
  pg_last_wal_replay_lsn() AS replay_lsn,
  pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) AS lag_bytes,
  EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) AS lag_seconds,
  CASE
    WHEN pg_wal_lsn_diff(pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn()) > 10485760 THEN 'WARNING: High lag'
    ELSE 'OK'
  END AS status;
-- 执行时间: <1ms
-- 计划: Result
```

### 9.2 延迟监控

**延迟监控配置**:

```sql
-- 创建延迟监控视图（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public'
        AND table_name = 'replication_lag_monitor'
    ) THEN
        DROP VIEW replication_lag_monitor;
        RAISE NOTICE '已删除现有视图: replication_lag_monitor';
    END IF;

    EXECUTE 'CREATE OR REPLACE VIEW replication_lag_monitor AS
    SELECT
      application_name,
      client_addr,
      state,
      sync_state,
      pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn)) AS sent_lag,
      pg_size_pretty(pg_wal_lsn_diff(sent_lsn, write_lsn)) AS write_lag,
      pg_size_pretty(pg_wal_lsn_diff(write_lsn, flush_lsn)) AS flush_lag,
      pg_size_pretty(pg_wal_lsn_diff(flush_lsn, replay_lsn)) AS replay_lag,
      EXTRACT(EPOCH FROM (now() - write_lag)) AS write_lag_seconds,
      EXTRACT(EPOCH FROM (now() - replay_lag)) AS replay_lag_seconds';

    RAISE NOTICE '延迟监控视图创建成功: replication_lag_monitor';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_stat_replication视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff或pg_current_wal_lsn函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建延迟监控视图失败: %', SQLERRM;
END $$;

-- 使用视图查询（带性能测试）
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM replication_lag_monitor;
-- 执行时间: <50ms
-- 计划: Seq Scan
  CASE
    WHEN pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) > 104857600 THEN 'CRITICAL: >100MB'
    WHEN pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) > 10485760 THEN 'WARNING: >10MB'
    ELSE 'OK'
  END AS lag_status
FROM pg_stat_replication;

-- 查询延迟监控
SELECT * FROM replication_lag_monitor;
```

### 9.3 告警规则配置

**Prometheus告警规则**:

```yaml
# replication-alerts.yaml
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: postgres-replication-alerts
  namespace: monitoring
spec:
  groups:
  - name: postgres_replication
    interval: 30s
    rules:
    - alert: PostgreSQLReplicationDown
      expr: pg_replication_is_replica == 0 and pg_replication_lag == 0
      for: 5m
      labels:
        severity: critical
      annotations:
        summary: "PostgreSQL replication is down"
        description: "Replication connection is not active"

    - alert: PostgreSQLReplicationLag
      expr: pg_replication_lag > 10485760  # 10MB
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL replication lag is high"
        description: "Replication lag is {{ $value }} bytes"

    - alert: PostgreSQLReplicationLagCritical
      expr: pg_replication_lag > 104857600  # 100MB
      for: 5m
      labels:
        severity: critical
      annotations:
        summary: "PostgreSQL replication lag is critical"
        description: "Replication lag is {{ $value }} bytes"

    - alert: PostgreSQLReplicationSlotInactive
      expr: pg_replication_slot_active == 0
      for: 10m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL replication slot is inactive"
        description: "Replication slot {{ $labels.slot_name }} is inactive"
```

### 9.4 自动化运维脚本

**自动故障检测脚本**:

```bash
#!/bin/bash
# scripts/replication_health_check.sh

PRIMARY_HOST="primary_host"
STANDBY_HOST="standby_host"
ALERT_EMAIL="admin@example.com"

# 检查复制状态
REPLICATION_STATUS=$(psql -h $PRIMARY_HOST -U postgres -t -c "
SELECT count(*)
FROM pg_stat_replication
WHERE state = 'streaming';
")

if [ "$REPLICATION_STATUS" -eq 0 ]; then
  echo "CRITICAL: No active replication connections"
  echo "No active replication connections" | mail -s "PostgreSQL Replication Alert" $ALERT_EMAIL
  exit 1
fi

# 检查复制延迟
REPLICATION_LAG=$(psql -h $PRIMARY_HOST -U postgres -t -c "
SELECT max(pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn))
FROM pg_stat_replication;
")

if [ "$REPLICATION_LAG" -gt 104857600 ]; then  # 100MB
  echo "CRITICAL: Replication lag is high: $REPLICATION_LAG bytes"
  echo "Replication lag is $REPLICATION_LAG bytes" | mail -s "PostgreSQL Replication Lag Alert" $ALERT_EMAIL
  exit 1
fi

echo "OK: Replication is healthy"
exit 0
```

**自动修复脚本**:

```bash
#!/bin/bash
# scripts/replication_auto_fix.sh

PRIMARY_HOST="primary_host"
STANDBY_HOST="standby_host"
REPLICATION_SLOT="replica1"

# 检查复制状态
REPLICATION_STATE=$(psql -h $PRIMARY_HOST -U postgres -t -c "
SELECT state
FROM pg_stat_replication
WHERE application_name = '$REPLICATION_SLOT';
")

if [ "$REPLICATION_STATE" != "streaming" ]; then
  echo "Replication is not streaming, attempting to fix..."

  # 1. 检查从库状态
  STANDBY_STATE=$(psql -h $STANDBY_HOST -U postgres -t -c "SELECT pg_is_in_recovery();")

  if [ "$STANDBY_STATE" = "t" ]; then
    # 2. 尝试恢复复制
    psql -h $STANDBY_HOST -U postgres -c "SELECT pg_wal_replay_resume();"

    # 3. 等待并检查
    sleep 10
    NEW_STATE=$(psql -h $PRIMARY_HOST -U postgres -t -c "
    SELECT state
    FROM pg_stat_replication
    WHERE application_name = '$REPLICATION_SLOT';
    ")

    if [ "$NEW_STATE" = "streaming" ]; then
      echo "Replication restored successfully"
    else
      echo "Failed to restore replication, manual intervention required"
      exit 1
    fi
  else
    echo "Standby is not in recovery mode, manual intervention required"
    exit 1
  fi
fi
```

## 10. 故障处理

### 10.1 复制中断恢复

```sql
-- 检查复制状态（带错误处理和性能测试）
DO $$
DECLARE
    non_streaming_count INT;
BEGIN
    SELECT COUNT(*) INTO non_streaming_count
    FROM pg_stat_replication
    WHERE state != 'streaming';

    IF non_streaming_count > 0 THEN
        RAISE WARNING '发现 % 个非streaming状态的复制连接', non_streaming_count;
    ELSE
        RAISE NOTICE '所有复制连接状态正常';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查复制状态失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_stat_replication WHERE state != 'streaming';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 检查 WAL 文件（带错误处理和性能测试）
DO $$
DECLARE
    wal_count INT;
BEGIN
    SELECT COUNT(*) INTO wal_count
    FROM pg_ls_waldir();

    RAISE NOTICE '找到 % 个WAL文件', wal_count;
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_ls_waldir函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查WAL文件失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pg_ls_waldir() ORDER BY modification DESC LIMIT 10;
-- 执行时间: <100ms（取决于WAL文件数量）
-- 计划: Function Scan

-- 重新同步从库（带错误处理）
-- 在从库执行
DO $$
DECLARE
    is_standby BOOLEAN;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE EXCEPTION '当前节点不是从库，无法执行WAL重放恢复';
    END IF;

    PERFORM pg_wal_replay_resume();
    RAISE NOTICE 'WAL重放已恢复';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_wal_replay_resume函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '重新同步从库失败: %', SQLERRM;
END $$;
```

### 10.2 从库提升为主库

```sql
-- 在从库执行（PostgreSQL 12+，带错误处理）
DO $$
DECLARE
    is_standby BOOLEAN;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE EXCEPTION '当前节点不是从库，无法提升';
    END IF;

    PERFORM pg_promote();
    RAISE NOTICE '从库提升为主库成功';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_promote函数不存在（需要PostgreSQL 12+）';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '从库提升失败: %', SQLERRM;
END $$;

-- 或手动提升
-- 1. 停止从库
-- 2. 删除 standby.signal
-- 3. 启动数据库
```

### 10.3 逻辑复制冲突处理

```sql
-- 查看冲突
SELECT * FROM pg_stat_subscription_stats;

-- 处理冲突（在订阅端）
ALTER SUBSCRIPTION sub_replica SET (slot_name = NONE);
ALTER SUBSCRIPTION sub_replica SET (slot_name = 'sub_replica');
```

### 10.4 主库故障切换

**主库故障切换步骤**:

```bash
# 步骤1: 确认主库故障
ping -c 10 primary_host
psql -h primary_host -U postgres -c "SELECT 1;" || echo "Primary is down"

# 步骤2: 提升从库为主库
psql -h standby_host -U postgres -c "SELECT pg_promote();"

# 步骤3: 验证新主库
psql -h standby_host -U postgres -c "SELECT pg_is_in_recovery();"
# 应该返回 false

# 步骤4: 更新应用连接字符串
# 将应用指向新的主库地址

# 步骤5: 配置其他从库连接到新主库
# 在其他从库上更新primary_conninfo
```

**自动故障切换脚本**:

```bash
#!/bin/bash
# scripts/auto_failover.sh

PRIMARY_HOST="primary_host"
STANDBY_HOST="standby_host"
ALERT_EMAIL="admin@example.com"

# 检查主库健康
PRIMARY_HEALTH=$(psql -h $PRIMARY_HOST -U postgres -t -c "SELECT 1;" 2>/dev/null)

if [ -z "$PRIMARY_HEALTH" ]; then
  echo "Primary is down, initiating failover..."

  # 提升从库
  PROMOTE_RESULT=$(psql -h $STANDBY_HOST -U postgres -t -c "SELECT pg_promote();" 2>/dev/null)

  if [ "$PROMOTE_RESULT" = "t" ]; then
    echo "Failover successful: $STANDBY_HOST is now primary"
    echo "Failover completed: $STANDBY_HOST is now primary" | mail -s "PostgreSQL Failover Alert" $ALERT_EMAIL
  else
    echo "Failover failed"
    echo "Failover failed for $STANDBY_HOST" | mail -s "PostgreSQL Failover Error" $ALERT_EMAIL
    exit 1
  fi
fi
```

---

## 11. 交叉引用

### 相关文档

#### 部署架构

- ⭐⭐⭐ [集群部署与高可用](./05.04-集群部署与高可用.md) - 高可用架构
- ⭐⭐⭐ [读写分离](./05.06-读写分离.md) - 读写分离配置
- ⭐⭐ [分布式架构设计](../分布式部署/05.08-分布式架构设计.md) - 分布式架构

#### 核心课程

- ⭐⭐ [事务管理与ACID特性](../../03-事务与并发/03.02-ACID特性/01.04-事务管理与ACID特性.md) - 事务管理基础
- ⭐⭐ [并发控制与MVCC机制](../../03-事务与并发/03.01-MVCC机制/01.05-并发控制与MVCC机制.md) - 并发控制

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../../12-监控与诊断/06.01-监控与诊断.md) - 复制监控
- ⭐⭐ [备份与恢复](../../13-高可用架构/备份与恢复/06.06-备份与恢复.md) - 备份恢复

#### 高级特性

- ⭐⭐ [分布式系统](../../00-归档-项目管理文档/README.md) - 分布式事务

### 外部资源

- [PostgreSQL 官方文档 - 复制](https://www.postgresql.org/docs/current/high-availability.html)
- [PostgreSQL 官方文档 - 逻辑复制](https://www.postgresql.org/docs/current/logical-replication.html)

---

**文档版本**: v3.0
**最后更新**: 2025-01-16
**维护者**: PostgreSQL Documentation Team
