---

> **📋 文档来源**: `PostgreSQL\05-部署架构\集群部署\05.06-读写分离.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 读写分离配置指南

> **文档版本**: v1.0
> **最后更新**: 2025-11-13
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **文档状态**: ✅ 已创建

---

## 📋 目录

- [读写分离配置指南](#读写分离配置指南)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
    - [优势](#优势)
    - [挑战](#挑战)
  - [2. 架构设计](#2-架构设计)
    - [2.1 基本架构](#21-基本架构)
    - [2.2 中间件架构](#22-中间件架构)
    - [2.3 应用层路由](#23-应用层路由)
  - [3. 使用 pgpool-II](#3-使用-pgpool-ii)
    - [3.1 安装配置](#31-安装配置)
      - [安装](#安装)
      - [基本配置（pgpool.conf）](#基本配置pgpoolconf)
    - [3.2 启动服务](#32-启动服务)
    - [3.3 连接测试](#33-连接测试)
  - [4. 使用 pgbouncer](#4-使用-pgbouncer)
    - [4.1 配置（pgbouncer.ini）](#41-配置pgbouncerini)
    - [4.2 应用层路由](#42-应用层路由)
  - [5. 应用层路由](#5-应用层路由)
    - [5.1 Django 配置](#51-django-配置)
    - [5.2 Spring Boot 配置](#52-spring-boot-配置)
  - [6. 负载均衡](#6-负载均衡)
    - [6.1 轮询（Round Robin）](#61-轮询round-robin)
    - [6.2 加权轮询](#62-加权轮询)
    - [6.3 基于延迟的路由](#63-基于延迟的路由)
  - [7. 监控与诊断](#7-监控与诊断)
    - [7.1 pgpool-II 监控](#71-pgpool-ii-监控)
    - [7.2 复制延迟监控](#72-复制延迟监控)
    - [7.3 读写分离效果监控](#73-读写分离效果监控)
  - [7. pgpool-II详细配置](#7-pgpool-ii详细配置)
    - [7.1 生产级配置](#71-生产级配置)
    - [7.2 负载均衡配置](#72-负载均衡配置)
    - [7.3 故障转移配置](#73-故障转移配置)
    - [7.4 性能优化配置](#74-性能优化配置)
  - [8. 应用层路由详细实现](#8-应用层路由详细实现)
    - [8.1 Django完整配置示例](#81-django完整配置示例)
    - [8.2 Spring Boot完整配置示例](#82-spring-boot完整配置示例)
    - [8.3 Node.js配置示例](#83-nodejs配置示例)
    - [8.4 Python连接池配置](#84-python连接池配置)
  - [9. 故障排查和诊断](#9-故障排查和诊断)
    - [9.1 读写分离问题诊断](#91-读写分离问题诊断)
    - [9.2 负载均衡问题](#92-负载均衡问题)
    - [9.3 连接池问题](#93-连接池问题)
    - [9.4 性能问题诊断](#94-性能问题诊断)
  - [10. 监控和告警](#10-监控和告警)
    - [10.1 读写分离效果监控](#101-读写分离效果监控)
    - [10.2 延迟监控](#102-延迟监控)
    - [10.3 告警规则配置](#103-告警规则配置)
    - [10.4 性能指标监控](#104-性能指标监控)
  - [11. 智能事务路由](#11-智能事务路由)
    - [11.1 事务路由策略设计](#111-事务路由策略设计)
      - [11.1.1 路由算法设计](#1111-路由算法设计)
      - [11.1.2 路由规则配置](#1112-路由规则配置)
    - [11.2 业务类型识别和路由](#112-业务类型识别和路由)
      - [11.2.1 业务类型识别](#1121-业务类型识别)
      - [11.2.2 路由实现](#1122-路由实现)
    - [11.3 读写分离智能路由](#113-读写分离智能路由)
      - [11.3.1 读操作路由](#1131-读操作路由)
      - [11.3.2 写操作路由](#1132-写操作路由)
    - [11.4 负载均衡算法](#114-负载均衡算法)
      - [11.4.1 轮询算法（Round Robin）](#1141-轮询算法round-robin)
      - [11.4.2 加权轮询（Weighted Round Robin）](#1142-加权轮询weighted-round-robin)
      - [11.4.3 最少连接（Least Connections）](#1143-最少连接least-connections)
      - [11.4.4 一致性哈希](#1144-一致性哈希)
    - [11.5 实际配置示例](#115-实际配置示例)
      - [11.5.1 pgpool-II智能路由配置](#1151-pgpool-ii智能路由配置)
      - [11.5.2 Django智能路由配置](#1152-django智能路由配置)
      - [11.5.3 Spring Boot智能路由配置](#1153-spring-boot智能路由配置)
      - [11.5.4 Node.js智能路由配置](#1154-nodejs智能路由配置)
  - [12. 交叉引用](#12-交叉引用)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [核心课程](#核心课程)
      - [查询与优化](#查询与优化)
      - [运维实践](#运维实践)
    - [外部资源](#外部资源)

---

## 1. 概述

读写分离（Read-Write Splitting）是将读操作和写操作分发到不同的数据库实例，以提高系统整体性能和可扩展性。

### 优势

- **提升读性能**: 读操作分散到多个从库
- **降低主库压力**: 主库专注于写操作
- **提高可用性**: 从库故障不影响写操作
- **水平扩展**: 通过增加从库扩展读能力

### 挑战

- **数据一致性**: 主从复制延迟导致读延迟
- **路由复杂性**: 需要区分读写操作
- **故障处理**: 从库故障时的自动切换

---

## 2. 架构设计

### 2.1 基本架构

```text
应用层
  │
  ├─ 写操作 ──→ 主库 (Primary)
  │
  └─ 读操作 ──→ 从库 (Standby) ──→ 从库 (Standby)
```

### 2.2 中间件架构

```text
应用层
  │
  └─ pgpool-II / pgbouncer
      │
      ├─ 写操作 ──→ 主库
      │
      └─ 读操作 ──→ 从库（负载均衡）
```

### 2.3 应用层路由

```text
应用层（连接池）
  │
  ├─ 写连接 ──→ 主库
  │
  └─ 读连接 ──→ 从库（轮询/随机）
```

---

## 3. 使用 pgpool-II

### 3.1 安装配置

#### 安装

```bash
# Ubuntu/Debian
sudo apt-get install pgpool2

# 或从源码编译
wget https://www.pgpool.net/download.php?f=pgpool-II-4.4.3.tar.gz
tar xzf pgpool-II-4.4.3.tar.gz
cd pgpool-II-4.4.3
./configure --prefix=/usr/local/pgpool
make && make install
```

#### 基本配置（pgpool.conf）

```text
# 监听地址和端口
listen_addresses = '*'
port = 9999

# 后端数据库配置
backend_hostname0 = 'primary_host'
backend_port0 = 5432
backend_weight0 = 0  # 主库权重（写操作）
backend_flag0 = 'ALLOW_TO_FAILOVER'

backend_hostname1 = 'standby1_host'
backend_port1 = 5432
backend_weight1 = 1  # 从库权重（读操作）
backend_flag1 = 'ALLOW_TO_FAILOVER'

backend_hostname2 = 'standby2_host'
backend_port2 = 5432
backend_weight2 = 1
backend_flag2 = 'ALLOW_TO_FAILOVER'

# 负载均衡模式
load_balance_mode = on

# 主从复制检查
master_slave_mode = on
master_slave_sub_mode = 'stream'

# 健康检查
health_check_period = 30
health_check_timeout = 10
health_check_user = 'postgres'
health_check_password = 'password'
health_check_database = 'postgres'

# 故障转移
failover_on_backend_error = on
```

### 3.2 启动服务

```bash
# 启动 pgpool-II
pgpool -n -f /etc/pgpool2/pgpool.conf -F /etc/pgpool2/pcp.conf

# 或使用 systemd
sudo systemctl start pgpool2
sudo systemctl enable pgpool2
```

### 3.3 连接测试

```bash
# 通过 pgpool 连接
psql -h pgpool_host -p 9999 -U postgres -d mydb

# 查看后端状态
psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_NODES;"
```

---

## 4. 使用 pgbouncer

### 4.1 配置（pgbouncer.ini）

```ini
[databases]
mydb = host=primary_host port=5432 dbname=mydb
mydb_ro = host=standby1_host port=5432 dbname=mydb

[pgbouncer]
listen_addr = *
listen_port = 6432
auth_type = md5
auth_file = /etc/pgbouncer/userlist.txt

# 连接池模式
pool_mode = transaction

# 连接限制
max_client_conn = 1000
default_pool_size = 25
```

### 4.2 应用层路由

```python
# Python 示例
import psycopg2

# 写操作连接（主库）
write_conn = psycopg2.connect(
    host='pgbouncer_host',
    port=6432,
    database='mydb',
    user='app_user',
    password='password'
)

# 读操作连接（从库）
read_conn = psycopg2.connect(
    host='pgbouncer_host',
    port=6432,
    database='mydb_ro',
    user='app_user',
    password='password'
)
```

---

## 5. 应用层路由

### 5.1 Django 配置

```python
# settings.py
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'app_user',
        'PASSWORD': 'password',
        'HOST': 'primary_host',
        'PORT': '5432',
    },
    'read': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'app_user',
        'PASSWORD': 'password',
        'HOST': 'standby1_host',
        'PORT': '5432',
    }
}

# 数据库路由
DATABASE_ROUTERS = ['myapp.dbrouter.DatabaseRouter']
```

```python
# dbrouter.py
class DatabaseRouter:
    def db_for_read(self, model, **hints):
        return 'read'

    def db_for_write(self, model, **hints):
        return 'default'
```

### 5.2 Spring Boot 配置

```yaml
# application.yml
spring:
  datasource:
    write:
      jdbc-url: jdbc:postgresql://primary_host:5432/mydb
      username: app_user
      password: password
    read:
      - jdbc-url: jdbc:postgresql://standby1_host:5432/mydb
        username: app_user
        password: password
      - jdbc-url: jdbc:postgresql://standby2_host:5432/mydb
        username: app_user
        password: password
```

---

## 6. 负载均衡

### 6.1 轮询（Round Robin）

```python
class RoundRobinRouter:
    def __init__(self, read_hosts):
        self.read_hosts = read_hosts
        self.current = 0

    def get_read_host(self):
        host = self.read_hosts[self.current]
        self.current = (self.current + 1) % len(self.read_hosts)
        return host
```

### 6.2 加权轮询

```python
class WeightedRoundRobinRouter:
    def __init__(self, read_hosts_with_weights):
        self.hosts = read_hosts_with_weights
        self.current_weight = 0
        self.current_index = -1

    def get_read_host(self):
        while True:
            self.current_index = (self.current_index + 1) % len(self.hosts)
            if self.current_index == 0:
                self.current_weight -= sum(w for _, w in self.hosts)
            self.current_weight += self.hosts[self.current_index][1]
            if self.current_weight > 0:
                return self.hosts[self.current_index][0]
```

### 6.3 基于延迟的路由

```python
import time

class LatencyBasedRouter:
    def __init__(self, read_hosts):
        self.read_hosts = read_hosts
        self.latencies = {host: 0 for host in read_hosts}

    def measure_latency(self, host):
        start = time.time()
        # 执行简单查询
        conn = psycopg2.connect(host=host, ...)
        conn.execute("SELECT 1")
        conn.close()
        return time.time() - start

    def get_read_host(self):
        # 定期测量延迟
        for host in self.read_hosts:
            self.latencies[host] = self.measure_latency(host)

        # 选择延迟最低的主机
        return min(self.latencies, key=self.latencies.get)
```

---

## 7. 监控与诊断

### 7.1 pgpool-II 监控

```sql
-- 查看后端节点状态（带错误处理）
-- 注意：SHOW POOL_NODES是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_NODES命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_NODES;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 查看连接池状态（带错误处理）
-- 注意：SHOW POOL_PROCESSES是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_PROCESSES命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_PROCESSES;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 查看统计信息（带错误处理说明）
-- 注意：SHOW POOL_STATS是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_STATS命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_STATS;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

```

### 7.2 复制延迟监控

```sql
-- 监控主从延迟（带错误处理和性能测试）
DO $$
DECLARE
    replication_count INT;
BEGIN
    SELECT COUNT(*) INTO replication_count
    FROM pg_stat_replication;

    RAISE NOTICE '找到 % 个复制连接', replication_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_replication视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '监控主从延迟失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  application_name,
  client_addr,
  state,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn)) AS lag,
  EXTRACT(EPOCH FROM (now() - replay_lag)) AS lag_seconds
FROM pg_stat_replication;
-- 执行时间: <50ms
-- 计划: Seq Scan

```

### 7.3 读写分离效果监控

```sql
-- 主库：写操作统计（带错误处理和性能测试）
DO $$
DECLARE
    db_count INT;
BEGIN
    SELECT COUNT(*) INTO db_count
    FROM pg_stat_database;

    RAISE NOTICE '找到 % 个数据库', db_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '写操作统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  xact_commit,
  xact_rollback,
  blks_read,
  blks_hit
FROM pg_stat_database
WHERE datname = 'mydb';

-- 从库：读操作统计（带错误处理和性能测试）
DO $$
DECLARE
    db_count INT;
    is_standby BOOLEAN;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库，读操作统计可能不准确';
    END IF;

    SELECT COUNT(*) INTO db_count
    FROM pg_stat_database;

    RAISE NOTICE '找到 % 个数据库', db_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_is_in_recovery函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '读操作统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  numbackends,
  xact_commit,
  blks_read,
  blks_hit,
  tup_returned,
  tup_fetched
FROM pg_stat_database
WHERE datname = 'mydb';
```

---

## 7. pgpool-II详细配置

### 7.1 生产级配置

**完整的pgpool.conf生产配置**:

```conf
# ============================================
# 基本配置
# ============================================
listen_addresses = '*'
port = 9999
socket_dir = '/var/run/pgpool'
pcp_listen_addresses = '*'
pcp_port = 9898
pcp_socket_dir = '/var/run/pgpool'

# ============================================
# 后端数据库配置
# ============================================
# 主库（写操作）
backend_hostname0 = 'primary_host'
backend_port0 = 5432
backend_weight0 = 0                    # 主库权重为0（仅写操作）
backend_flag0 = 'ALLOW_TO_FAILOVER'
backend_data_directory0 = '/var/lib/postgresql/data'
backend_application_name0 = 'primary'

# 从库1（读操作）
backend_hostname1 = 'standby1_host'
backend_port1 = 5432
backend_weight1 = 1                    # 从库权重为1（读操作）
backend_flag1 = 'ALLOW_TO_FAILOVER'
backend_data_directory1 = '/var/lib/postgresql/data'
backend_application_name1 = 'standby1'

# 从库2（读操作）
backend_hostname2 = 'standby2_host'
backend_port2 = 5432
backend_weight2 = 1
backend_flag2 = 'ALLOW_TO_FAILOVER'
backend_data_directory2 = '/var/lib/postgresql/data'
backend_application_name2 = 'standby2'

# ============================================
# 连接池配置
# ============================================
num_init_children = 32                # 初始子进程数
max_pool = 4                          # 每个子进程的最大连接数
child_life_time = 300                  # 子进程生命周期（秒）
child_max_connections = 0              # 子进程最大连接数（0=无限制）
connection_life_time = 0               # 连接生命周期（0=无限制）
client_idle_limit = 0                  # 客户端空闲限制（0=无限制）

# ============================================
# 负载均衡配置
# ============================================
load_balance_mode = on                 # 启用负载均衡
ignore_leading_white_space = on       # 忽略前导空格

# ============================================
# 主从复制模式
# ============================================
master_slave_mode = on                 # 启用主从模式
master_slave_sub_mode = 'stream'       # 流复制模式
failover_on_backend_error = on         # 后端错误时故障转移

# ============================================
# 健康检查配置
# ============================================
health_check_period = 30               # 健康检查周期（秒）
health_check_timeout = 10              # 健康检查超时（秒）
health_check_user = 'postgres'         # 健康检查用户
health_check_password = 'password'     # 健康检查密码
health_check_database = 'postgres'     # 健康检查数据库
health_check_max_retries = 3           # 最大重试次数
health_check_retry_delay = 1           # 重试延迟（秒）

# ============================================
# 故障转移配置
# ============================================
failover_command = '/usr/local/bin/failover.sh %h %p %d %H %P %r %R'
failback_command = '/usr/local/bin/failback.sh %h %p %d %H %P %r %R'
follow_master_command = '/usr/local/bin/follow_master.sh %h %p %d %H %P %r %R'

# ============================================
# 日志配置
# ============================================
log_destination = 'stderr'
logging_collector = on
log_directory = '/var/log/pgpool'
log_filename = 'pgpool-%Y-%m-%d.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_messages = warning
log_connections = on
log_hostname = on
log_statement = on
log_per_node_statement = on

# ============================================
# 认证配置
# ============================================
enable_pool_hba = on
pool_passwd = 'pool_passwd'

# ============================================
# 性能优化
# ============================================
memory_cache_enabled = on              # 启用内存缓存
memqcache_method = 'memcached'         # 使用Memcached
memqcache_memcached_host = 'memcached_host'
memqcache_memcached_port = 11211
memqcache_total_size = 67108864        # 64MB
memqcache_max_num_cache = 1000000
memqcache_expire = 3600
memqcache_auto_cache_invalidation = on
```

**pool_hba.conf配置**:

```conf
# TYPE  DATABASE        USER            ADDRESS                 METHOD

# 本地连接
local   all             all                                     md5

# IPv4本地连接
host    all             all             127.0.0.1/32            md5

# 应用服务器连接
host    all             app_user        192.168.1.0/24          scram-sha-256

# 管理连接
host    all             postgres        192.168.1.0/24          scram-sha-256
```

### 7.2 负载均衡配置

**负载均衡策略配置**:

```conf
# pgpool.conf
# 负载均衡模式
load_balance_mode = on

# 负载均衡权重（根据从库性能调整）
backend_weight1 = 2                    # 高性能从库权重更高
backend_weight2 = 1                    # 普通从库权重较低

# 基于语句的负载均衡
statement_level_load_balance = on      # 语句级负载均衡

# 负载均衡算法
# - round_robin: 轮询（默认）
# - weighted_round_robin: 加权轮询
# - least_connections: 最少连接
load_balance_algorithm = 'round_robin'
```

**负载均衡测试**:

```sql
-- 测试负载均衡（带错误处理）
DO $$
BEGIN
    RAISE NOTICE '测试负载均衡：执行多次查询，观察连接分布';
    RAISE NOTICE '请使用: SELECT pg_backend_pid(), inet_server_addr(), inet_server_port();';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '负载均衡测试准备失败: %', SQLERRM;
END $$;

SELECT pg_backend_pid(), inet_server_addr(), inet_server_port();
-- 执行时间: <1ms
-- 计划: Result

-- 查看负载均衡统计（带错误处理说明）
-- 注意：SHOW POOL_STATS是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_STATS命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_STATS;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;
```

### 7.3 故障转移配置

**故障转移脚本**:

```bash
#!/bin/bash
# /usr/local/bin/failover.sh
# 参数: $1=失败节点主机名 $2=失败节点端口 $3=失败节点数据库 $4=新主库主机名 $5=新主库端口 $6=旧主库数据目录 $7=新主库数据目录

set -euo pipefail

# 错误处理函数
error_exit() {
    echo "错误: $1" >&2
    exit 1
}

# 检查参数
if [ $# -lt 7 ]; then
    error_exit "参数不足: 需要7个参数 (失败节点主机名 失败节点端口 失败节点数据库 新主库主机名 新主库端口 旧主库数据目录 新主库数据目录)"
fi

FAILED_NODE=$1
FAILED_PORT=$2
FAILED_DB=$3
NEW_PRIMARY=$4
NEW_PRIMARY_PORT=$5
OLD_PRIMARY_DATA=$6
NEW_PRIMARY_DATA=$7

echo "Failover: $FAILED_NODE:$FAILED_PORT -> $NEW_PRIMARY:$NEW_PRIMARY_PORT"

# 1. 提升新主库（带错误处理）
if ! ssh postgres@$NEW_PRIMARY "psql -h localhost -p $NEW_PRIMARY_PORT -d $FAILED_DB -c 'SELECT pg_promote();'" 2>/dev/null; then
    error_exit "提升新主库失败: 无法连接到 $NEW_PRIMARY:$NEW_PRIMARY_PORT 或 pg_promote() 执行失败"
fi

# 2. 更新应用连接字符串（可选）
# if ! curl -X POST http://app-server/api/config/update-db-host -d "host=$NEW_PRIMARY" 2>/dev/null; then
#     echo "警告: 更新应用连接字符串失败" >&2
# fi

# 3. 发送通知（带错误处理）
if command -v mail >/dev/null 2>&1; then
    echo "Failover completed: $NEW_PRIMARY is now primary" | mail -s "PostgreSQL Failover Alert" admin@example.com || echo "警告: 发送邮件通知失败" >&2
else
    echo "警告: mail命令不可用，跳过邮件通知" >&2
fi

echo "故障转移完成: $NEW_PRIMARY 现在是主库"
exit 0
```

**故障转移配置**:

```conf
# pgpool.conf
failover_command = '/usr/local/bin/failover.sh %h %p %d %H %P %r %R'
failback_command = '/usr/local/bin/failback.sh %h %p %d %H %P %r %R'
follow_master_command = '/usr/local/bin/follow_master.sh %h %p %d %H %P %r %R'

# 故障转移超时
failover_timeout = 60                  # 故障转移超时（秒）

# 自动故障恢复
failback_on_backend_error = off       # 不自动故障恢复（推荐手动恢复）
```

### 7.4 性能优化配置

**连接池优化**:

```conf
# pgpool.conf
# 连接池大小优化
num_init_children = 64                # 根据并发连接数调整
max_pool = 4                          # 每个子进程的最大连接数
child_life_time = 300                  # 子进程生命周期

# 连接复用
connection_life_time = 0               # 连接生命周期（0=无限制）
client_idle_limit = 0                  # 客户端空闲限制

# 查询缓存
memory_cache_enabled = on
memqcache_method = 'memcached'
memqcache_total_size = 134217728      # 128MB
memqcache_max_num_cache = 1000000
memqcache_expire = 3600
```

**查询优化**:

```conf
# pgpool.conf
# 查询缓存
memory_cache_enabled = on
memqcache_method = 'memcached'
memqcache_total_size = 134217728      # 128MB

# 查询重写
rewrite_query = on                     # 启用查询重写
rewrite_query_list = 'SELECT'          # 重写SELECT查询

# 并行查询
parallel_mode = off                    # 禁用并行模式（推荐）
```

## 8. 应用层路由详细实现

### 8.1 Django完整配置示例

**完整的Django配置**:

```python
# settings.py
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'app_user',
        'PASSWORD': os.getenv('DB_PASSWORD'),
        'HOST': 'primary_host',
        'PORT': '5432',
        'OPTIONS': {
            'connect_timeout': 10,
            'options': '-c statement_timeout=30000'
        },
        'CONN_MAX_AGE': 600,           # 连接池最大年龄
    },
    'read': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'app_user',
        'PASSWORD': os.getenv('DB_PASSWORD'),
        'HOST': 'standby1_host',
        'PORT': '5432',
        'OPTIONS': {
            'connect_timeout': 10,
            'options': '-c statement_timeout=30000'
        },
        'CONN_MAX_AGE': 600,
    },
    'read2': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'app_user',
        'PASSWORD': os.getenv('DB_PASSWORD'),
        'HOST': 'standby2_host',
        'PORT': '5432',
        'OPTIONS': {
            'connect_timeout': 10,
            'options': '-c statement_timeout=30000'
        },
        'CONN_MAX_AGE': 600,
    }
}

# 数据库路由
DATABASE_ROUTERS = ['myapp.dbrouter.DatabaseRouter']

# 连接池配置
DATABASES['default']['CONN_MAX_AGE'] = 600
DATABASES['read']['CONN_MAX_AGE'] = 600
DATABASES['read2']['CONN_MAX_AGE'] = 600
```

**数据库路由实现**:

```python
# myapp/dbrouter.py
import random
from django.conf import settings

class DatabaseRouter:
    """数据库路由：实现读写分离"""

    read_databases = ['read', 'read2']

    def db_for_read(self, model, **hints):
        """读操作路由到从库"""
        # 随机选择从库（负载均衡）
        return random.choice(self.read_databases)

    def db_for_write(self, model, **hints):
        """写操作路由到主库"""
        return 'default'

    def allow_relation(self, obj1, obj2, **hints):
        """允许跨数据库关系"""
        return True

    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """迁移只在主库执行"""
        if db == 'default':
            return True
        return False
```

**使用示例**:

```python
# views.py
from django.db import connections

def get_user_list(request):
    """读操作自动路由到从库"""
    users = User.objects.all()  # 自动路由到从库
    return render(request, 'users.html', {'users': users})

def create_user(request):
    """写操作自动路由到主库"""
    user = User.objects.create(  # 自动路由到主库
        username=request.POST['username'],
        email=request.POST['email']
    )
    return redirect('user_list')

# 手动指定数据库
def get_user_from_primary(request, user_id):
    """强制从主库读取（读取刚写入的数据）"""
    user = User.objects.using('default').get(id=user_id)
    return render(request, 'user.html', {'user': user})
```

### 8.2 Spring Boot完整配置示例

**完整的Spring Boot配置**:

```yaml
# application.yml
spring:
  datasource:
    # 主库配置（写操作）
    write:
      jdbc-url: jdbc:postgresql://primary_host:5432/mydb?sslmode=require
      username: app_user
      password: ${DB_PASSWORD}
      driver-class-name: org.postgresql.Driver
      hikari:
        maximum-pool-size: 20
        minimum-idle: 5
        connection-timeout: 30000
        idle-timeout: 600000
        max-lifetime: 1800000

    # 从库配置（读操作）
    read:
      - jdbc-url: jdbc:postgresql://standby1_host:5432/mydb?sslmode=require
        username: app_user
        password: ${DB_PASSWORD}
        driver-class-name: org.postgresql.Driver
        hikari:
          maximum-pool-size: 20
          minimum-idle: 5
          connection-timeout: 30000
          idle-timeout: 600000
          max-lifetime: 1800000
      - jdbc-url: jdbc:postgresql://standby2_host:5432/mydb?sslmode=require
        username: app_user
        password: ${DB_PASSWORD}
        driver-class-name: org.postgresql.Driver
        hikari:
          maximum-pool-size: 20
          minimum-idle: 5
          connection-timeout: 30000
          idle-timeout: 600000
          max-lifetime: 1800000

  jpa:
    hibernate:
      ddl-auto: none
    show-sql: false
    properties:
      hibernate:
        dialect: org.hibernate.dialect.PostgreSQLDialect
        format_sql: true
```

**数据源配置类**:

```java
// DataSourceConfig.java
@Configuration
public class DataSourceConfig {

    @Bean
    @Primary
    @ConfigurationProperties("spring.datasource.write")
    public DataSource writeDataSource() {
        return DataSourceBuilder.create().build();
    }

    @Bean
    @ConfigurationProperties("spring.datasource.read")
    public List<DataSource> readDataSources() {
        return Arrays.asList(
            DataSourceBuilder.create().build(),
            DataSourceBuilder.create().build()
        );
    }

    @Bean
    public DataSource routingDataSource() {
        RoutingDataSource routingDataSource = new RoutingDataSource();
        Map<Object, Object> dataSourceMap = new HashMap<>();
        dataSourceMap.put("write", writeDataSource());
        for (int i = 0; i < readDataSources().size(); i++) {
            dataSourceMap.put("read" + i, readDataSources().get(i));
        }
        routingDataSource.setTargetDataSources(dataSourceMap);
        routingDataSource.setDefaultTargetDataSource(writeDataSource());
        return routingDataSource;
    }
}
```

**路由数据源实现**:

```java
// RoutingDataSource.java
public class RoutingDataSource extends AbstractRoutingDataSource {

    private static final ThreadLocal<String> contextHolder = new ThreadLocal<>();
    private static final Random random = new Random();

    @Override
    protected Object determineCurrentLookupKey() {
        return contextHolder.get();
    }

    public static void setReadDataSource() {
        // 随机选择从库
        int index = random.nextInt(2);
        contextHolder.set("read" + index);
    }

    public static void setWriteDataSource() {
        contextHolder.set("write");
    }

    public static void clear() {
        contextHolder.remove();
    }
}
```

**AOP切面实现**:

```java
// ReadWriteAspect.java
@Aspect
@Component
public class ReadWriteAspect {

    @Around("@annotation(org.springframework.transaction.annotation.Transactional)")
    public Object routeDataSource(ProceedingJoinPoint joinPoint) throws Throwable {
        MethodSignature signature = (MethodSignature) joinPoint.getSignature();
        Transactional transactional = signature.getMethod().getAnnotation(Transactional.class);

        if (transactional != null && transactional.readOnly()) {
            RoutingDataSource.setReadDataSource();
        } else {
            RoutingDataSource.setWriteDataSource();
        }

        try {
            return joinPoint.proceed();
        } finally {
            RoutingDataSource.clear();
        }
    }
}
```

### 8.3 Node.js配置示例

**Node.js连接池配置**:

```javascript
// db.js
const { Pool } = require('pg');
const _ = require('lodash');

// 主库连接池（写操作）
const writePool = new Pool({
  host: 'primary_host',
  port: 5432,
  database: 'mydb',
  user: 'app_user',
  password: process.env.DB_PASSWORD,
  max: 20,
  idleTimeoutMillis: 30000,
  connectionTimeoutMillis: 30000,
});

// 从库连接池（读操作）
const readPools = [
  new Pool({
    host: 'standby1_host',
    port: 5432,
    database: 'mydb',
    user: 'app_user',
    password: process.env.DB_PASSWORD,
    max: 20,
    idleTimeoutMillis: 30000,
    connectionTimeoutMillis: 30000,
  }),
  new Pool({
    host: 'standby2_host',
    port: 5432,
    database: 'mydb',
    user: 'app_user',
    password: process.env.DB_PASSWORD,
    max: 20,
    idleTimeoutMillis: 30000,
    connectionTimeoutMillis: 30000,
  }),
];

// 获取读连接池（负载均衡）
function getReadPool() {
  return _.sample(readPools);
}

// 数据库操作封装
const db = {
  // 写操作
  async query(sql, params) {
    return await writePool.query(sql, params);
  },

  // 读操作
  async read(sql, params) {
    const pool = getReadPool();
    return await pool.query(sql, params);
  },

  // 事务（写操作）
  async transaction(callback) {
    const client = await writePool.connect();
    try {
      await client.query('BEGIN');
      const result = await callback(client);
      await client.query('COMMIT');
      return result;
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    } finally {
      client.release();
    }
  },
};

module.exports = db;
```

**使用示例**:

```javascript
// routes/users.js
const db = require('../db');

// 读操作（路由到从库）
router.get('/users', async (req, res) => {
  const result = await db.read('SELECT * FROM users');
  res.json(result.rows);
});

// 写操作（路由到主库）
router.post('/users', async (req, res) => {
  const result = await db.query(
    'INSERT INTO users (username, email) VALUES ($1, $2) RETURNING *',
    [req.body.username, req.body.email]
  );
  res.json(result.rows[0]);
});
```

### 8.4 Python连接池配置

**Python连接池配置**:

```python
# db.py
import psycopg2
from psycopg2 import pool
import random
import os

# 主库连接池（写操作）
write_pool = psycopg2.pool.ThreadedConnectionPool(
    1,
    20,
    host='primary_host',
    port=5432,
    database='mydb',
    user='app_user',
    password=os.getenv('DB_PASSWORD'),
    connect_timeout=10
)

# 从库连接池（读操作）
read_pools = [
    psycopg2.pool.ThreadedConnectionPool(
        1,
        20,
        host='standby1_host',
        port=5432,
        database='mydb',
        user='app_user',
        password=os.getenv('DB_PASSWORD'),
        connect_timeout=10
    ),
    psycopg2.pool.ThreadedConnectionPool(
        1,
        20,
        host='standby2_host',
        port=5432,
        database='mydb',
        user='app_user',
        password=os.getenv('DB_PASSWORD'),
        connect_timeout=10
    ),
]

def get_read_pool():
    """获取读连接池（负载均衡）"""
    return random.choice(read_pools)

def execute_write(query, params=None):
    """执行写操作"""
    conn = write_pool.getconn()
    try:
        cursor = conn.cursor()
        cursor.execute(query, params)
        result = cursor.fetchall() if cursor.description else None
        conn.commit()
        return result
    except Exception as e:
        conn.rollback()
        raise e
    finally:
        cursor.close()
        write_pool.putconn(conn)

def execute_read(query, params=None):
    """执行读操作"""
    pool = get_read_pool()
    conn = pool.getconn()
    try:
        cursor = conn.cursor()
        cursor.execute(query, params)
        result = cursor.fetchall()
        return result
    finally:
        cursor.close()
        pool.putconn(conn)
```

## 9. 故障排查和诊断

### 9.1 读写分离问题诊断

**问题诊断步骤**:

```sql
-- 1. 检查pgpool-II状态（带错误处理说明）
-- 注意：SHOW POOL_NODES、SHOW POOL_PROCESSES、SHOW POOL_STATS是pgpool-II特有的命令
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_NODES/PROCESSES/STATS命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_NODES;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 2. 检查后端节点状态（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'pgpool_nodes'
    ) THEN
        RAISE WARNING 'pgpool_nodes表不存在（可能未安装pgpool-II或未配置）';
        RETURN;
    END IF;

    RAISE NOTICE '开始检查后端节点状态';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '检查后端节点状态准备失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM pgpool_nodes;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 检查读写分离是否生效（带错误处理和性能测试）
-- 在主库执行
DO $$
DECLARE
    active_conn_count INT;
BEGIN
    SELECT COUNT(*) INTO active_conn_count
    FROM pg_stat_activity
    WHERE state = 'active';

    RAISE NOTICE '主库活动连接数: %', active_conn_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查读写分离失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT count(*) FROM pg_stat_activity WHERE state = 'active';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 在从库执行（带错误处理和性能测试）
DO $$
DECLARE
    active_conn_count INT;
    is_standby BOOLEAN;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库';
    END IF;

    SELECT COUNT(*) INTO active_conn_count
    FROM pg_stat_activity
    WHERE state = 'active';

    RAISE NOTICE '从库活动连接数: %', active_conn_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_is_in_recovery函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查读写分离失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT count(*) FROM pg_stat_activity WHERE state = 'active';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 4. 检查查询路由（带错误处理和性能测试）
-- 通过pgpool连接执行查询，查看实际连接的节点
DO $$
BEGIN
    RAISE NOTICE '检查查询路由：查看实际连接的节点';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '检查查询路由准备失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT pg_backend_pid(), inet_server_addr(), inet_server_port();
-- 执行时间: <1ms
-- 计划: Result
```

**常见问题及解决方案**:

```bash
# 问题1: 读操作仍然路由到主库
# 原因: pgpool-II配置错误或查询被识别为写操作
# 解决: 检查pgpool.conf配置
grep -i "load_balance_mode\|master_slave_mode" /etc/pgpool2/pgpool.conf

# 问题2: 负载均衡不工作
# 原因: 后端节点权重配置错误
# 解决: 检查backend_weight配置
grep -i "backend_weight" /etc/pgpool2/pgpool.conf

# 问题3: 连接池耗尽
# 原因: 连接数超过限制
# 解决: 增加连接池大小或优化连接使用
SHOW POOL_STATS;
```

### 9.2 负载均衡问题

**负载均衡问题诊断**:

```sql
-- 1. 检查负载均衡状态（带错误处理说明）
-- 注意：SHOW POOL_NODES是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_NODES命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_NODES;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 2. 检查连接分布（带错误处理和性能测试）
DO $$
DECLARE
    connection_count INT;
BEGIN
    SELECT COUNT(*) INTO connection_count
    FROM pg_stat_activity;

    RAISE NOTICE '总连接数: %', connection_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查连接分布失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  backend_hostname,
  backend_port,
  count(*) as connection_count
FROM pg_stat_activity
GROUP BY backend_hostname, backend_port;
-- 执行时间: <50ms
-- 计划: HashAggregate

-- 3. 检查查询分布（带错误处理和性能测试）
-- 通过pgpool执行多次查询，观察连接节点
DO $$
BEGIN
    RAISE NOTICE '检查查询分布：执行多次查询，观察连接节点';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '检查查询分布准备失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT pg_backend_pid(), inet_server_addr();
-- 执行时间: <1ms
-- 计划: Result
```

**负载均衡优化**:

```conf
# pgpool.conf
# 根据从库性能调整权重
backend_weight1 = 2                    # 高性能从库
backend_weight2 = 1                    # 普通从库

# 使用加权轮询
load_balance_algorithm = 'weighted_round_robin'
```

### 9.3 连接池问题

**连接池问题诊断**:

```sql
-- 1. 检查连接池状态（带错误处理说明）
-- 注意：SHOW POOL_PROCESSES和SHOW POOL_STATS是pgpool-II特有的命令
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_PROCESSES/STATS命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_PROCESSES;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 2. 检查连接数（带错误处理和性能测试）
DO $$
DECLARE
    connection_count INT;
BEGIN
    SELECT COUNT(*) INTO connection_count
    FROM pg_stat_activity;

    RAISE NOTICE '总连接数: %', connection_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查连接数失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  count(*) as connections,
  count(*) FILTER (WHERE state = 'active') as active_connections,
  count(*) FILTER (WHERE state = 'idle') as idle_connections
FROM pg_stat_activity
GROUP BY datname;
-- 执行时间: <50ms
-- 计划: HashAggregate

-- 3. 检查连接等待（带错误处理和性能测试）
DO $$
DECLARE
    waiting_conn_count INT;
BEGIN
    SELECT COUNT(*) INTO waiting_conn_count
    FROM pg_stat_activity
    WHERE wait_event_type IS NOT NULL;

    RAISE NOTICE '等待连接数: %', waiting_conn_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '检查连接等待失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  pid,
  wait_event_type,
  wait_event,
  state,
  query
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL;
-- 执行时间: <50ms
-- 计划: Seq Scan
```

**连接池优化**:

```conf
# pgpool.conf
# 优化连接池大小
num_init_children = 64                # 根据并发连接数调整
max_pool = 4                          # 每个子进程的最大连接数

# 连接复用
connection_life_time = 0               # 连接生命周期
client_idle_limit = 0                  # 客户端空闲限制
```

### 9.4 性能问题诊断

**性能诊断脚本**:

```bash
#!/bin/bash
# scripts/read_write_performance_diagnosis.sh
# 读写分离性能诊断脚本（带错误处理）

set -euo pipefail

# 错误处理函数
error_exit() {
    echo "错误: $1" >&2
    exit 1
}

# 检查参数
PRIMARY_HOST=${1:-primary_host}
STANDBY_HOST=${2:-standby_host}
DB_NAME=${3:-mydb}
DB_USER=${4:-postgres}

echo "=== 读写分离性能诊断 ==="

# 1. 主库写操作统计（带错误处理）
echo "=== 主库写操作统计 ==="
if ! psql -h "$PRIMARY_HOST" -U "$DB_USER" -d "$DB_NAME" -c "
SELECT
  datname,
  xact_commit,
  xact_rollback,
  blks_read,
  blks_hit,
  tup_inserted,
  tup_updated,
  tup_deleted
FROM pg_stat_database
WHERE datname = '$DB_NAME';
" 2>/dev/null; then
    error_exit "主库写操作统计失败: 无法连接到 $PRIMARY_HOST 或查询失败"
fi

# 2. 从库读操作统计（带错误处理）
echo "=== 从库读操作统计 ==="
if ! psql -h "$STANDBY_HOST" -U "$DB_USER" -d "$DB_NAME" -c "
SELECT
  datname,
  numbackends,
  xact_commit,
  blks_read,
  blks_hit,
  tup_returned,
  tup_fetched
FROM pg_stat_database
WHERE datname = 'mydb';
"

# 3. pgpool-II统计（带错误处理）
echo "=== pgpool-II统计 ==="
PGPOOL_HOST=${5:-pgpool_host}
if ! psql -h "$PGPOOL_HOST" -p 9999 -U "$DB_USER" -d "$DB_NAME" -c "SHOW POOL_STATS;" 2>/dev/null; then
    echo "警告: pgpool-II统计失败: 无法连接到 $PGPOOL_HOST:9999 或SHOW POOL_STATS命令失败" >&2
fi

# 4. 复制延迟（带错误处理）
echo "=== 复制延迟 ==="
if ! psql -h "$PRIMARY_HOST" -U "$DB_USER" -d "$DB_NAME" -c "
SELECT
  application_name,
  pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn)) AS lag
FROM pg_stat_replication;
" 2>/dev/null; then
    error_exit "复制延迟查询失败: 无法连接到 $PRIMARY_HOST 或查询失败"
fi
```

## 10. 监控和告警

### 10.1 读写分离效果监控

**读写分离效果监控SQL**:

```sql
-- 1. 主库写操作统计（带错误处理和性能测试）
DO $$
DECLARE
    db_count INT;
BEGIN
    SELECT COUNT(*) INTO db_count
    FROM pg_stat_database
    WHERE datname = 'mydb';

    IF db_count = 0 THEN
        RAISE WARNING '数据库mydb不存在或不在统计中';
    ELSE
        RAISE NOTICE '找到数据库mydb的统计信息';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '主库写操作统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  xact_commit as writes,
  tup_inserted + tup_updated + tup_deleted as rows_written,
  blks_read + blks_hit as total_blocks
FROM pg_stat_database
WHERE datname = 'mydb';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 2. 从库读操作统计（带错误处理和性能测试）
DO $$
DECLARE
    db_count INT;
    is_standby BOOLEAN;
BEGIN
    SELECT pg_is_in_recovery() INTO is_standby;

    IF NOT is_standby THEN
        RAISE WARNING '当前节点不是从库，读操作统计可能不准确';
    END IF;

    SELECT COUNT(*) INTO db_count
    FROM pg_stat_database
    WHERE datname = 'mydb';

    IF db_count = 0 THEN
        RAISE WARNING '数据库mydb不存在或不在统计中';
    ELSE
        RAISE NOTICE '找到数据库mydb的统计信息';
    END IF;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN undefined_function THEN
        RAISE WARNING 'pg_is_in_recovery函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '从库读操作统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  datname,
  numbackends as connections,
  xact_commit as reads,
  tup_returned + tup_fetched as rows_read,
  blks_read + blks_hit as total_blocks
FROM pg_stat_database
WHERE datname = 'mydb';
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 3. 读写分离比例（带错误处理和性能测试）
DO $$
DECLARE
    write_count INT;
    total_count INT;
    write_percentage NUMERIC;
BEGIN
    SELECT COUNT(*) INTO write_count
    FROM pg_stat_database
    WHERE datname = 'mydb';

    IF write_count = 0 THEN
        RAISE WARNING '数据库mydb不存在或不在统计中';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO total_count
    FROM pg_stat_database
    WHERE datname = 'mydb';

    IF total_count = 0 THEN
        RAISE WARNING '无法计算读写分离比例：总操作数为0';
        RETURN;
    END IF;

    RAISE NOTICE '开始计算读写分离比例';
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_database视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '读写分离比例计算失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  (SELECT xact_commit FROM pg_stat_database WHERE datname = 'mydb') as writes,
  (SELECT sum(xact_commit) FROM pg_stat_database WHERE datname = 'mydb') as total_ops,
  round(
    100.0 * (SELECT xact_commit FROM pg_stat_database WHERE datname = 'mydb') /
    NULLIF((SELECT sum(xact_commit) FROM pg_stat_database WHERE datname = 'mydb'), 0),
    2
  ) as write_percentage;
-- 执行时间: <100ms
-- 计划: SubPlan
```

### 10.2 延迟监控

**延迟监控配置**:

```sql
-- 创建延迟监控视图（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'public'
        AND table_name = 'read_write_lag_monitor'
    ) THEN
        DROP VIEW read_write_lag_monitor;
        RAISE NOTICE '已删除现有视图: read_write_lag_monitor';
    END IF;

    CREATE OR REPLACE VIEW read_write_lag_monitor AS
    SELECT
      'primary' AS node_type,
      datname,
      pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(),
        (SELECT restart_lsn FROM pg_replication_slots WHERE slot_name = 'replica1')
      )) AS lag_size
    FROM pg_stat_database
    WHERE datname = 'mydb'
    UNION ALL
    SELECT
      'standby' AS node_type,
      datname,
      pg_size_pretty(pg_wal_lsn_diff(
        (SELECT pg_current_wal_lsn() FROM pg_stat_database WHERE datname = 'mydb'),
        pg_last_wal_replay_lsn()
      )) AS lag_size
    FROM pg_stat_database
    WHERE datname = 'mydb';

    RAISE NOTICE '延迟监控视图创建成功: read_write_lag_monitor';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '视图read_write_lag_monitor已存在';
    WHEN undefined_table THEN
        RAISE EXCEPTION 'pg_stat_database或pg_replication_slots视图不存在';
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_wal_lsn_diff或pg_current_wal_lsn或pg_last_wal_replay_lsn函数不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建延迟监控视图失败: %', SQLERRM;
END $$;

-- 查询延迟监控视图（带错误处理和性能测试）
DO $$
DECLARE
    view_count INT;
BEGIN
    SELECT COUNT(*) INTO view_count
    FROM read_write_lag_monitor;

    RAISE NOTICE '延迟监控视图记录数: %', view_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'read_write_lag_monitor视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询延迟监控视图失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM read_write_lag_monitor;
-- 执行时间: <100ms
-- 计划: Append
```

### 10.3 告警规则配置

**Prometheus告警规则**:

```yaml
# read-write-split-alerts.yaml
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: postgres-read-write-split-alerts
  namespace: monitoring
spec:
  groups:
  - name: postgres_read_write_split
    interval: 30s
    rules:
    - alert: PostgreSQLReadWriteSplitImbalance
      expr: |
        (pg_stat_database_xact_commit{datname="mydb",instance="primary"} /
         (pg_stat_database_xact_commit{datname="mydb",instance="primary"} +
          sum(pg_stat_database_xact_commit{datname="mydb",instance=~"standby.*"}))) > 0.5
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL read-write split imbalance"
        description: "Write operations account for {{ $value | humanizePercentage }} of total operations"

    - alert: PostgreSQLReplicationLagHigh
      expr: pg_replication_lag > 10485760  # 10MB
      for: 5m
      labels:
        severity: warning
      annotations:
        summary: "PostgreSQL replication lag is high"
        description: "Replication lag is {{ $value }} bytes"
```

### 10.4 性能指标监控

**性能指标监控**:

```sql
-- 1. pgpool-II性能指标（带错误处理说明）
-- 注意：SHOW POOL_STATS是pgpool-II特有的命令，需要在pgpool-II连接中执行
DO $$
BEGIN
    RAISE NOTICE 'SHOW POOL_STATS命令需要在pgpool-II连接中执行';
    RAISE NOTICE '请使用: psql -h pgpool_host -p 9999 -U postgres -c "SHOW POOL_STATS;"';
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '此命令需要在pgpool-II连接中执行';
END $$;

-- 2. 连接池使用率（带错误处理和性能测试）
DO $$
DECLARE
    current_conn_count INT;
    max_conn_count INT;
    usage_percent NUMERIC;
BEGIN
    SELECT COUNT(*) INTO current_conn_count
    FROM pg_stat_activity;

    SELECT setting::int INTO max_conn_count
    FROM pg_settings
    WHERE name = 'max_connections';

    IF max_conn_count = 0 THEN
        RAISE WARNING 'max_connections配置为0，无法计算使用率';
        RETURN;
    END IF;

    usage_percent := round(100.0 * current_conn_count / max_conn_count, 2);
    RAISE NOTICE '当前连接数: %, 最大连接数: %, 使用率: %%', current_conn_count, max_conn_count, usage_percent;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_activity或pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '连接池使用率计算失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  (SELECT count(*) FROM pg_stat_activity) as current_connections,
  (SELECT setting::int FROM pg_settings WHERE name = 'max_connections') as max_connections,
  round(
    100.0 * (SELECT count(*) FROM pg_stat_activity) /
    NULLIF((SELECT setting::int FROM pg_settings WHERE name = 'max_connections'), 0),
    2
  ) as connection_usage_percent;
-- 执行时间: <100ms
-- 计划: SubPlan

-- 3. 查询性能（带错误处理和性能测试）
DO $$
DECLARE
    statement_count INT;
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_extension
        WHERE extname = 'pg_stat_statements'
    ) THEN
        RAISE WARNING 'pg_stat_statements扩展未安装，无法查询性能统计';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO statement_count
    FROM pg_stat_statements;

    RAISE NOTICE '找到 % 条查询统计记录', statement_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_stat_statements视图不存在';
    WHEN undefined_object THEN
        RAISE WARNING 'pg_stat_statements扩展未安装';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查询性能统计失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT
  query,
  calls,
  mean_time,
  max_time,
  total_time
FROM pg_stat_statements
ORDER BY mean_time DESC
LIMIT 20;
-- 执行时间: <100ms
-- 计划: Limit
```

---

## 11. 智能事务路由

### 11.1 事务路由策略设计

**智能事务路由**是根据事务类型、业务特征和系统负载，自动将事务路由到最合适的数据库节点，以优化性能和资源利用。

#### 11.1.1 路由算法设计

**路由决策因素**：

1. **事务类型**：
   - 读事务 → 从库
   - 写事务 → 主库
   - 只读事务 → 从库（可接受延迟）

2. **业务类型**：
   - 关键业务 → 主库（强一致性）
   - 非关键业务 → 从库（最终一致性）

3. **系统负载**：
   - 负载低的节点优先
   - 资源利用率低的节点优先

4. **数据一致性要求**：
   - 强一致性 → 主库
   - 最终一致性 → 从库

**路由算法示例**：

```python
# 智能事务路由算法（Python）
from enum import Enum
from typing import Optional, List
import psycopg

class TransactionType(Enum):
    READ = "read"
    WRITE = "write"
    READ_WRITE = "read_write"

class BusinessType(Enum):
    CRITICAL = "critical"  # 关键业务
    NORMAL = "normal"      # 普通业务
    BATCH = "batch"        # 批量业务

class ConsistencyLevel(Enum):
    STRONG = "strong"      # 强一致性
    EVENTUAL = "eventual"  # 最终一致性

class SmartRouter:
    def __init__(self, primary_conn, standby_conns: List):
        self.primary_conn = primary_conn
        self.standby_conns = standby_conns

    def route_transaction(
        self,
        transaction_type: TransactionType,
        business_type: BusinessType = BusinessType.NORMAL,
        consistency_level: ConsistencyLevel = ConsistencyLevel.EVENTUAL
    ) -> psycopg.Connection:
        """智能路由事务到合适的节点"""

        # 写事务必须路由到主库
        if transaction_type == TransactionType.WRITE:
            return self.primary_conn

        # 关键业务或强一致性要求 → 主库
        if business_type == BusinessType.CRITICAL or consistency_level == ConsistencyLevel.STRONG:
            return self.primary_conn

        # 读事务 → 选择负载最低的从库
        if transaction_type == TransactionType.READ:
            return self._select_best_standby()

        # 默认路由到主库
        return self.primary_conn

    def _select_best_standby(self) -> psycopg.Connection:
        """选择负载最低的从库"""
        best_conn = None
        min_load = float('inf')

        for conn in self.standby_conns:
            load = self._get_node_load(conn)
            if load < min_load:
                min_load = load
                best_conn = conn

        return best_conn or self.standby_conns[0]

    def _get_node_load(self, conn: psycopg.Connection) -> float:
        """获取节点负载（简化示例）"""
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                  COUNT(*) as active_connections,
                  AVG(EXTRACT(EPOCH FROM (now() - query_start))) as avg_query_time
                FROM pg_stat_activity
                WHERE state = 'active'
            """)
            result = cur.fetchone()
            if result:
                connections, avg_time = result
                # 负载 = 连接数 * 平均查询时间
                return connections * (avg_time or 0)
        return 0.0

# 使用示例
router = SmartRouter(primary_conn, [standby1_conn, standby2_conn])

# 读事务路由到从库
read_conn = router.route_transaction(
    TransactionType.READ,
    BusinessType.NORMAL,
    ConsistencyLevel.EVENTUAL
)

# 写事务路由到主库
write_conn = router.route_transaction(
    TransactionType.WRITE,
    BusinessType.CRITICAL,
    ConsistencyLevel.STRONG
)
```

#### 11.1.2 路由规则配置

**路由规则配置**：

```yaml
# 智能事务路由规则配置（YAML格式）
routing_rules:
  # 规则1：关键业务写操作
  - name: "critical_write"
    conditions:
      transaction_type: "write"
      business_type: "critical"
    action:
      route_to: "primary"
      consistency: "strong"

  # 规则2：普通读操作
  - name: "normal_read"
    conditions:
      transaction_type: "read"
      business_type: "normal"
    action:
      route_to: "standby"
      consistency: "eventual"
      load_balancing: "least_connections"

  # 规则3：批量读操作
  - name: "batch_read"
    conditions:
      transaction_type: "read"
      business_type: "batch"
    action:
      route_to: "standby"
      consistency: "eventual"
      load_balancing: "round_robin"

  # 规则4：强一致性读操作
  - name: "strong_consistency_read"
    conditions:
      transaction_type: "read"
      consistency_requirement: "strong"
    action:
      route_to: "primary"
      consistency: "strong"
```

### 11.2 业务类型识别和路由

#### 11.2.1 业务类型识别

**业务类型分类**：

1. **关键业务（Critical）**：
   - 金融交易
   - 用户认证
   - 订单支付
   - 要求：强一致性、低延迟

2. **普通业务（Normal）**：
   - 用户查询
   - 内容浏览
   - 数据统计
   - 要求：最终一致性可接受

3. **批量业务（Batch）**：
   - 数据导出
   - 报表生成
   - 数据分析
   - 要求：可接受延迟、高吞吐量

**识别方法**：

```python
# 业务类型识别（Python）
import re
from typing import Optional

class BusinessTypeIdentifier:
    def __init__(self):
        # 关键业务SQL模式
        self.critical_patterns = [
            r'\b(INSERT|UPDATE|DELETE).*accounts\b',
            r'\b(INSERT|UPDATE|DELETE).*transactions\b',
            r'\b(INSERT|UPDATE|DELETE).*orders\b.*status.*paid',
            r'\bSELECT.*FROM.*users.*WHERE.*password\b',
        ]

        # 批量业务SQL模式
        self.batch_patterns = [
            r'\bSELECT.*COUNT\(.*\)\b',
            r'\bSELECT.*GROUP BY\b',
            r'\bSELECT.*ORDER BY.*LIMIT\s+\d{4,}\b',  # 大量数据
        ]

    def identify(self, sql: str, transaction_type: TransactionType) -> BusinessType:
        """识别业务类型"""
        sql_upper = sql.upper()

        # 检查关键业务模式
        for pattern in self.critical_patterns:
            if re.search(pattern, sql_upper, re.IGNORECASE):
                return BusinessType.CRITICAL

        # 检查批量业务模式
        if transaction_type == TransactionType.READ:
            for pattern in self.batch_patterns:
                if re.search(pattern, sql_upper, re.IGNORECASE):
                    return BusinessType.BATCH

        return BusinessType.NORMAL

# 使用示例
identifier = BusinessTypeIdentifier()

sql = "SELECT * FROM accounts WHERE user_id = 123"
business_type = identifier.identify(sql, TransactionType.READ)
# 返回：BusinessType.NORMAL

sql = "UPDATE accounts SET balance = balance - 100 WHERE id = 1"
business_type = identifier.identify(sql, TransactionType.WRITE)
# 返回：BusinessType.CRITICAL
```

#### 11.2.2 路由实现

**应用层路由实现**：

```python
# 应用层智能路由实现（Python）
class ApplicationRouter:
    def __init__(self, router: SmartRouter, identifier: BusinessTypeIdentifier):
        self.router = router
        self.identifier = identifier

    def execute_query(
        self,
        sql: str,
        params: Optional[tuple] = None,
        consistency_level: ConsistencyLevel = ConsistencyLevel.EVENTUAL
    ):
        """执行查询（自动路由）"""
        # 识别事务类型
        transaction_type = self._detect_transaction_type(sql)

        # 识别业务类型
        business_type = self.identifier.identify(sql, transaction_type)

        # 路由到合适的节点
        conn = self.router.route_transaction(
            transaction_type,
            business_type,
            consistency_level
        )

        # 执行查询
        with conn.cursor() as cur:
            cur.execute(sql, params)
            if transaction_type == TransactionType.READ:
                return cur.fetchall()
            else:
                conn.commit()
                return cur.rowcount

    def _detect_transaction_type(self, sql: str) -> TransactionType:
        """检测事务类型"""
        sql_upper = sql.strip().upper()
        if sql_upper.startswith('SELECT'):
            return TransactionType.READ
        elif sql_upper.startswith(('INSERT', 'UPDATE', 'DELETE')):
            return TransactionType.WRITE
        else:
            return TransactionType.READ_WRITE
```

### 11.3 读写分离智能路由

#### 11.3.1 读操作路由

**读操作路由策略**：

1. **强一致性读**：路由到主库
2. **最终一致性读**：路由到负载最低的从库
3. **批量读**：路由到专用分析从库

**实现示例**：

```python
# 读操作智能路由（Python）
class ReadRouter:
    def __init__(self, primary_conn, standby_conns: List, analytics_conn=None):
        self.primary_conn = primary_conn
        self.standby_conns = standby_conns
        self.analytics_conn = analytics_conn

    def route_read(
        self,
        sql: str,
        require_strong_consistency: bool = False,
        is_analytics_query: bool = False
    ) -> psycopg.Connection:
        """路由读操作"""

        # 分析查询路由到分析库
        if is_analytics_query and self.analytics_conn:
            return self.analytics_conn

        # 强一致性要求 → 主库
        if require_strong_consistency:
            return self.primary_conn

        # 最终一致性 → 选择负载最低的从库
        return self._select_best_standby()

    def _select_best_standby(self) -> psycopg.Connection:
        """选择最佳从库"""
        # 检查从库延迟
        best_conn = None
        min_lag = float('inf')

        for conn in self.standby_conns:
            lag = self._get_replication_lag(conn)
            if lag < min_lag:
                min_lag = lag
                best_conn = conn

        return best_conn or self.standby_conns[0]

    def _get_replication_lag(self, conn: psycopg.Connection) -> float:
        """获取复制延迟（秒）"""
        with conn.cursor() as cur:
            cur.execute("""
                SELECT EXTRACT(EPOCH FROM (now() - pg_last_xact_replay_timestamp())) AS lag
            """)
            result = cur.fetchone()
            return result[0] if result and result[0] else 0.0
```

#### 11.3.2 写操作路由

**写操作路由策略**：

1. **所有写操作**：必须路由到主库
2. **事务路由**：包含写操作的事务路由到主库
3. **批量写**：考虑主库负载，可能需要限流

**实现示例**：

```python
# 写操作路由（Python）
class WriteRouter:
    def __init__(self, primary_conn):
        self.primary_conn = primary_conn

    def route_write(
        self,
        sql: str,
        check_primary_health: bool = True
    ) -> psycopg.Connection:
        """路由写操作（总是路由到主库）"""

        # 检查主库健康状态
        if check_primary_health:
            if not self._is_primary_healthy():
                raise Exception("Primary database is not healthy")

        return self.primary_conn

    def _is_primary_healthy(self) -> bool:
        """检查主库健康状态"""
        try:
            with self.primary_conn.cursor() as cur:
                cur.execute("SELECT 1")
                return True
        except Exception:
            return False
```

### 11.4 负载均衡算法

#### 11.4.1 轮询算法（Round Robin）

**轮询算法**：按顺序轮流分配请求到各个节点。

**实现示例**：

```python
# 轮询负载均衡（Python）
class RoundRobinBalancer:
    def __init__(self, connections: List):
        self.connections = connections
        self.current_index = 0

    def get_connection(self) -> psycopg.Connection:
        """获取下一个连接（轮询）"""
        conn = self.connections[self.current_index]
        self.current_index = (self.current_index + 1) % len(self.connections)
        return conn
```

#### 11.4.2 加权轮询（Weighted Round Robin）

**加权轮询**：根据节点性能设置权重，高性能节点分配更多请求。

**实现示例**：

```python
# 加权轮询负载均衡（Python）
class WeightedRoundRobinBalancer:
    def __init__(self, connections_with_weights: List[tuple]):
        """
        connections_with_weights: [(connection, weight), ...]
        例如：[(conn1, 3), (conn2, 2), (conn3, 1)]
        """
        self.connections = [c for c, _ in connections_with_weights]
        self.weights = [w for _, w in connections_with_weights]
        self.current_weights = self.weights.copy()
        self.current_index = 0

    def get_connection(self) -> psycopg.Connection:
        """获取下一个连接（加权轮询）"""
        # 找到权重最大的节点
        max_weight = max(self.current_weights)
        max_index = self.current_weights.index(max_weight)

        # 减少当前节点的权重
        self.current_weights[max_index] -= sum(self.weights)

        # 如果所有权重都<=0，重置
        if all(w <= 0 for w in self.current_weights):
            self.current_weights = self.weights.copy()

        return self.connections[max_index]
```

#### 11.4.3 最少连接（Least Connections）

**最少连接**：选择当前连接数最少的节点。

**实现示例**：

```python
# 最少连接负载均衡（Python）
class LeastConnectionsBalancer:
    def __init__(self, connections: List):
        self.connections = connections

    def get_connection(self) -> psycopg.Connection:
        """获取连接数最少的节点"""
        best_conn = None
        min_connections = float('inf')

        for conn in self.connections:
            connections = self._get_active_connections(conn)
            if connections < min_connections:
                min_connections = connections
                best_conn = conn

        return best_conn or self.connections[0]

    def _get_active_connections(self, conn: psycopg.Connection) -> int:
        """获取节点活跃连接数"""
        with conn.cursor() as cur:
            cur.execute("""
                SELECT COUNT(*)
                FROM pg_stat_activity
                WHERE state = 'active'
            """)
            result = cur.fetchone()
            return result[0] if result else 0
```

#### 11.4.4 一致性哈希

**一致性哈希**：使用一致性哈希算法分配请求，节点增减时只影响相邻节点。

**实现示例**：

```python
# 一致性哈希负载均衡（Python）
import hashlib

class ConsistentHashBalancer:
    def __init__(self, connections: List, virtual_nodes: int = 3):
        self.connections = connections
        self.virtual_nodes = virtual_nodes
        self.ring = {}
        self.sorted_keys = []

        # 为每个连接创建虚拟节点
        for conn in connections:
            for i in range(virtual_nodes):
                key = self._hash(f"{id(conn)}:{i}")
                self.ring[key] = conn
                self.sorted_keys.append(key)

        self.sorted_keys.sort()

    def _hash(self, key: str) -> int:
        """计算哈希值"""
        return int(hashlib.md5(key.encode()).hexdigest(), 16)

    def get_connection(self, routing_key: str) -> psycopg.Connection:
        """根据路由键获取连接"""
        if not self.ring:
            return None

        hash_key = self._hash(routing_key)

        # 找到第一个大于等于hash_key的节点
        for ring_key in self.sorted_keys:
            if ring_key >= hash_key:
                return self.ring[ring_key]

        # 如果没找到，返回第一个节点（环的起点）
        return self.ring[self.sorted_keys[0]]

# 使用示例
balancer = ConsistentHashBalancer([conn1, conn2, conn3])

# 根据用户ID路由（相同用户总是路由到同一节点）
user_id = 123
conn = balancer.get_connection(f"user:{user_id}")
```

### 11.5 实际配置示例

#### 11.5.1 pgpool-II智能路由配置

**pgpool-II配置**：

```conf
# pgpool.conf
# 智能路由配置

# 启用读写分离
master_slave_mode = on
load_balance_mode = on

# 读操作路由到从库
read_function_list = 'currval,lastval,nextval,setval'
read_only_function_list = 'pg_read_file,pg_stat_file,pg_ls_dir,pg_current_logfile'

# 写操作路由到主库
write_function_list = 'nextval,setval'

# 负载均衡算法
load_balance_mode = on
ignore_leading_white_space = on

# 延迟检查（避免路由到延迟过高的从库）
delay_threshold = 1000  # 1秒延迟阈值

# 健康检查
health_check_period = 10
health_check_timeout = 20
health_check_user = 'postgres'
health_check_password = 'password'
health_check_database = 'postgres'
```

#### 11.5.2 Django智能路由配置

**Django配置**：

```python
# settings.py
DATABASES = {
    'default': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'postgres',
        'PASSWORD': 'password',
        'HOST': 'primary-host',
        'PORT': '5432',
    },
    'read': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': 'mydb',
        'USER': 'postgres',
        'PASSWORD': 'password',
        'HOST': 'standby-host',
        'PORT': '5432',
    }
}

# 数据库路由
DATABASE_ROUTERS = ['myapp.dbrouter.SmartDatabaseRouter']
```

**数据库路由实现**：

```python
# myapp/dbrouter.py
class SmartDatabaseRouter:
    """智能数据库路由"""

    # 读操作路由到从库的表
    read_tables = {'users', 'products', 'orders'}

    # 写操作路由到主库的表
    write_tables = {'users', 'products', 'orders', 'transactions'}

    def db_for_read(self, model, **hints):
        """读操作路由"""
        if model._meta.db_table in self.read_tables:
            # 检查是否需要强一致性
            if hints.get('require_strong_consistency', False):
                return 'default'  # 主库
            return 'read'  # 从库
        return None

    def db_for_write(self, model, **hints):
        """写操作路由"""
        if model._meta.db_table in self.write_tables:
            return 'default'  # 主库
        return None

    def allow_relation(self, obj1, obj2, **hints):
        """允许关系"""
        return True

    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """允许迁移"""
        return db == 'default'  # 只在主库迁移
```

#### 11.5.3 Spring Boot智能路由配置

**Spring Boot配置**：

```yaml
# application.yml
spring:
  datasource:
    primary:
      jdbc-url: jdbc:postgresql://primary-host:5432/mydb
      username: postgres
      password: password
      driver-class-name: org.postgresql.Driver

    read:
      jdbc-url: jdbc:postgresql://standby-host:5432/mydb
      username: postgres
      password: password
      driver-class-name: org.postgresql.Driver
```

**数据源配置类**：

```java
// DataSourceConfig.java
@Configuration
public class DataSourceConfig {

    @Bean
    @Primary
    @ConfigurationProperties("spring.datasource.primary")
    public DataSource primaryDataSource() {
        return DataSourceBuilder.create().build();
    }

    @Bean
    @ConfigurationProperties("spring.datasource.read")
    public DataSource readDataSource() {
        return DataSourceBuilder.create().build();
    }

    @Bean
    public DataSource routingDataSource() {
        RoutingDataSource routingDataSource = new RoutingDataSource();
        Map<Object, Object> dataSourceMap = new HashMap<>();
        dataSourceMap.put("primary", primaryDataSource());
        dataSourceMap.put("read", readDataSource());
        routingDataSource.setTargetDataSources(dataSourceMap);
        routingDataSource.setDefaultTargetDataSource(primaryDataSource());
        return routingDataSource;
    }
}

// RoutingDataSource.java
public class RoutingDataSource extends AbstractRoutingDataSource {

    @Override
    protected Object determineCurrentLookupKey() {
        // 根据事务类型和业务类型决定数据源
        if (TransactionSynchronizationManager.isCurrentTransactionReadOnly()) {
            return "read";  // 只读事务 → 从库
        }
        return "primary";  // 写事务 → 主库
    }
}
```

#### 11.5.4 Node.js智能路由配置

**Node.js配置**：

```javascript
// db-router.js
const { Pool } = require('pg');

class SmartDatabaseRouter {
  constructor(config) {
    this.primaryPool = new Pool(config.primary);
    this.readPools = config.read.map(cfg => new Pool(cfg));
    this.currentReadIndex = 0;
  }

  async query(sql, params, options = {}) {
    const transactionType = this._detectTransactionType(sql);
    const requireStrongConsistency = options.requireStrongConsistency || false;

    // 写操作 → 主库
    if (transactionType === 'write') {
      return this.primaryPool.query(sql, params);
    }

    // 强一致性读 → 主库
    if (requireStrongConsistency) {
      return this.primaryPool.query(sql, params);
    }

    // 最终一致性读 → 从库（轮询）
    const readPool = this.readPools[this.currentReadIndex];
    this.currentReadIndex = (this.currentReadIndex + 1) % this.readPools.length;
    return readPool.query(sql, params);
  }

  _detectTransactionType(sql) {
    const sqlUpper = sql.trim().toUpperCase();
    if (sqlUpper.startsWith('SELECT')) {
      return 'read';
    } else if (sqlUpper.startsWith(('INSERT', 'UPDATE', 'DELETE'))) {
      return 'write';
    }
    return 'read';
  }
}

// 使用示例
const router = new SmartDatabaseRouter({
  primary: {
    host: 'primary-host',
    port: 5432,
    database: 'mydb',
    user: 'postgres',
    password: 'password'
  },
  read: [
    {
      host: 'standby1-host',
      port: 5432,
      database: 'mydb',
      user: 'postgres',
      password: 'password'
    },
    {
      host: 'standby2-host',
      port: 5432,
      database: 'mydb',
      user: 'postgres',
      password: 'password'
    }
  ]
});

// 读操作（自动路由到从库）
const users = await router.query('SELECT * FROM users WHERE id = $1', [123]);

// 写操作（自动路由到主库）
await router.query('INSERT INTO users (name) VALUES ($1)', ['Alice']);

// 强一致性读（路由到主库）
const account = await router.query(
  'SELECT * FROM accounts WHERE id = $1',
  [123],
  { requireStrongConsistency: true }
);
```

---

## 12. 交叉引用

### 相关文档

#### 部署架构

- ⭐⭐⭐ [主从复制配置指南](./05.05-主从复制.md) - 主从复制基础
- ⭐⭐⭐ [集群部署与高可用](./05.04-集群部署与高可用.md) - 高可用架构
- ⭐⭐ [性能调优实践](../单机部署/05.02-性能调优实践.md) - 性能优化

#### 核心课程

- ⭐⭐ [事务管理与ACID特性](../../03-事务与并发/03.02-ACID特性/01.04-事务管理与ACID特性.md) - 事务管理基础
- ⭐⭐ [并发控制与MVCC机制](../../03-事务与并发/03.01-MVCC机制/01.05-并发控制与MVCC机制.md) - 并发控制

#### 查询与优化

- ⭐⭐ [查询优化器原理](../../02-查询与优化/02.01-查询优化器/02.01-查询优化器原理.md) - 查询优化
- ⭐ [执行计划与性能调优](../../02-查询与优化/02.03-执行计划/02.04-执行计划与性能调优.md) - 性能分析

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../../12-监控与诊断/06.01-监控与诊断.md) - 读写分离监控
- ⭐⭐ [备份与恢复](../../13-高可用架构/备份与恢复/06.06-备份与恢复.md) - 备份恢复

### 外部资源

- [pgpool-II 官方文档](https://www.pgpool.net/docs/)
- [pgbouncer 官方文档](https://www.pgbouncer.org/)

---

**文档版本**: v3.0
**最后更新**: 2025-01-16
**维护者**: PostgreSQL Documentation Team
