---

> **📋 文档来源**: `PostgreSQL\05-部署架构\单机部署\05.01-单机部署与配置.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 单机部署与配置

> **文档版本**: v2.0
> **最后更新**: 2025-11-12
> **版本覆盖**: PostgreSQL 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> **文档状态**: ✅ 已更新

---

## 📋 目录

- [单机部署与配置](#单机部署与配置)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. 合并来源与映射](#2-合并来源与映射)
  - [3. 安装与初始化](#3-安装与初始化)
    - [3.1 包管理器安装](#31-包管理器安装)
      - [Ubuntu/Debian](#ubuntudebian)
      - [CentOS/RHEL](#centosrhel)
      - [macOS](#macos)
    - [3.2 源码编译安装](#32-源码编译安装)
    - [3.3 数据库初始化](#33-数据库初始化)
    - [3.4 服务管理](#34-服务管理)
    - [3.5 验证安装](#35-验证安装)
  - [4. 配置要点](#4-配置要点)
    - [4.1 postgresql.conf基础配置](#41-postgresqlconf基础配置)
      - [连接设置](#连接设置)
      - [内存设置](#内存设置)
      - [I/O设置](#io设置)
      - [WAL设置](#wal设置)
      - [日志设置](#日志设置)
    - [4.2 pg\_hba.conf认证配置](#42-pg_hbaconf认证配置)
      - [认证规则示例](#认证规则示例)
      - [认证方法说明](#认证方法说明)
    - [4.3 系统参数优化](#43-系统参数优化)
      - [Linux内核参数](#linux内核参数)
      - [资源限制](#资源限制)
    - [4.4 配置验证](#44-配置验证)
  - [5. 安全与合规](#5-安全与合规)
    - [5.1 用户和权限管理](#51-用户和权限管理)
    - [5.2 SSL/TLS加密](#52-ssltls加密)
    - [5.3 审计日志](#53-审计日志)
    - [5.4 行级安全策略](#54-行级安全策略)
    - [5.5 数据加密](#55-数据加密)
    - [5.6 安全最佳实践](#56-安全最佳实践)
  - [6. PostgreSQL 18生产级配置](#6-postgresql-18生产级配置)
    - [6.1 小规模部署配置（\<10GB数据，\<100连接）](#61-小规模部署配置10gb数据100连接)
    - [6.2 中规模部署配置（10-100GB数据，100-500连接）](#62-中规模部署配置10-100gb数据100-500连接)
    - [6.3 大规模部署配置（\>100GB数据，\>500连接）](#63-大规模部署配置100gb数据500连接)
    - [6.4 PostgreSQL 18新特性配置](#64-postgresql-18新特性配置)
  - [7. 故障排查与诊断](#7-故障排查与诊断)
    - [7.1 启动问题排查](#71-启动问题排查)
    - [7.2 连接问题诊断](#72-连接问题诊断)
    - [7.3 性能问题诊断](#73-性能问题诊断)
    - [7.4 日志分析方法](#74-日志分析方法)
  - [8. 性能优化实践](#8-性能优化实践)
    - [8.1 系统级优化](#81-系统级优化)
    - [8.2 数据库参数优化](#82-数据库参数优化)
    - [8.3 存储优化](#83-存储优化)
    - [8.4 网络优化](#84-网络优化)
  - [9. 监控与告警配置](#9-监控与告警配置)
    - [9.1 关键指标监控](#91-关键指标监控)
    - [9.2 Prometheus监控配置](#92-prometheus监控配置)
    - [9.3 告警规则配置](#93-告警规则配置)
  - [10. 安全加固详细指南](#10-安全加固详细指南)
    - [10.1 SSL/TLS完整配置](#101-ssltls完整配置)
    - [10.2 防火墙规则配置](#102-防火墙规则配置)
    - [10.3 审计日志配置](#103-审计日志配置)
    - [10.4 行级安全策略配置](#104-行级安全策略配置)
  - [11. 后续整合任务](#11-后续整合任务)
    - [11.1 不同操作系统差异](#111-不同操作系统差异)
      - [Ubuntu/Debian](#ubuntudebian-1)
      - [CentOS/RHEL](#centosrhel-1)
      - [macOS (Homebrew)](#macos-homebrew)
    - [11.2 系统参数推荐表](#112-系统参数推荐表)
    - [11.3 性能调优检查清单](#113-性能调优检查清单)
  - [12. OpenTelemetry集成](#12-opentelemetry集成)
    - [12.1 OpenTelemetry架构和原理](#121-opentelemetry架构和原理)
      - [12.1.1 OpenTelemetry架构](#1211-opentelemetry架构)
      - [12.1.2 遥测数据类型](#1212-遥测数据类型)
      - [12.1.3 OpenTelemetry优势](#1213-opentelemetry优势)
    - [12.2 PostgreSQL驱动集成配置](#122-postgresql驱动集成配置)
      - [12.2.1 Python驱动集成（psycopg3）](#1221-python驱动集成psycopg3)
      - [12.2.2 Java驱动集成（JDBC）](#1222-java驱动集成jdbc)
      - [12.2.3 Node.js驱动集成（pg）](#1223-nodejs驱动集成pg)
      - [12.2.4 配置最佳实践](#1224-配置最佳实践)
    - [12.3 跨服务请求跟踪配置](#123-跨服务请求跟踪配置)
      - [12.3.1 追踪配置](#1231-追踪配置)
      - [12.3.2 上下文传播](#1232-上下文传播)
      - [12.3.3 实际案例](#1233-实际案例)
    - [12.4 性能瓶颈识别方法](#124-性能瓶颈识别方法)
      - [12.4.1 追踪数据分析](#1241-追踪数据分析)
      - [12.4.2 瓶颈定位方法](#1242-瓶颈定位方法)
      - [12.4.3 优化建议](#1243-优化建议)
  - [13. ELK/Graylog日志管理](#13-elkgraylog日志管理)
    - [13.1 ELK Stack完整配置](#131-elk-stack完整配置)
      - [13.1.1 Elasticsearch配置](#1311-elasticsearch配置)
      - [13.1.2 Logstash配置](#1312-logstash配置)
      - [13.1.3 Kibana配置](#1313-kibana配置)
    - [13.2 Graylog配置和集成](#132-graylog配置和集成)
      - [13.2.1 Graylog安装配置](#1321-graylog安装配置)
      - [13.2.2 PostgreSQL集成](#1322-postgresql集成)
      - [13.2.3 日志采集配置](#1323-日志采集配置)
    - [13.3 日志分析和查询](#133-日志分析和查询)
      - [13.3.1 查询语法](#1331-查询语法)
      - [13.3.2 分析案例](#1332-分析案例)
      - [13.3.3 可视化配置](#1333-可视化配置)
    - [13.4 日志告警配置](#134-日志告警配置)
      - [13.4.1 告警规则配置](#1341-告警规则配置)
      - [13.4.2 通知配置](#1342-通知配置)
      - [13.4.3 实际案例](#1343-实际案例)
  - [14. PostgreSQL日志管理详细配置](#14-postgresql日志管理详细配置)
    - [14.1 日志参数详细配置](#141-日志参数详细配置)
      - [14.1.1 基础日志配置](#1411-基础日志配置)
      - [14.1.2 连接日志配置](#1412-连接日志配置)
      - [14.1.3 语句日志配置](#1413-语句日志配置)
      - [14.1.4 检查点和自动清理日志](#1414-检查点和自动清理日志)
      - [14.1.5 错误和警告日志](#1415-错误和警告日志)
    - [14.2 日志文件管理策略](#142-日志文件管理策略)
      - [14.2.1 日志轮转配置](#1421-日志轮转配置)
      - [14.2.2 日志归档策略](#1422-日志归档策略)
      - [14.2.3 日志清理策略](#1423-日志清理策略)
    - [14.3 日志分析工具](#143-日志分析工具)
      - [14.3.1 pgBadger使用](#1431-pgbadger使用)
      - [14.3.2 日志分析脚本](#1432-日志分析脚本)
      - [14.3.3 实时日志监控](#1433-实时日志监控)
  - [15. 交叉引用](#15-交叉引用)
    - [相关文档](#相关文档)
      - [部署架构](#部署架构)
      - [核心课程](#核心课程)
      - [运维实践](#运维实践)
      - [高级特性](#高级特性)

---

## 1. 概述

- 安装、初始化、基础配置与安全加固

## 2. 合并来源与映射

- 1.1.15-云原生与容器化部署.md（基础配置片段）
- 1.1.16-性能调优与监控-扩充版.md（相关配置）

## 3. 安装与初始化

- 包管理器/源码安装，initdb，pg_ctl/服务管理

### 3.1 包管理器安装

#### Ubuntu/Debian

```bash
# 添加PostgreSQL官方APT仓库
sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list'
wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
sudo apt-get update

# 安装PostgreSQL 18
sudo apt-get install -y postgresql-18 postgresql-contrib-18

# 启动服务
sudo systemctl start postgresql
sudo systemctl enable postgresql

# 检查状态
sudo systemctl status postgresql
```

#### CentOS/RHEL

```bash
# 安装PostgreSQL官方YUM仓库
sudo dnf install -y https://download.postgresql.org/pub/repos/yum/reporpms/EL-9-x86_64/pgdg-redhat-repo-latest.noarch.rpm

# 安装PostgreSQL 18
sudo dnf install -y postgresql18-server postgresql18-contrib

# 初始化数据库
sudo /usr/pgsql-18/bin/postgresql-18-setup initdb

# 启动服务
sudo systemctl start postgresql-18
sudo systemctl enable postgresql-18
```

#### macOS

```bash
# 使用Homebrew安装
brew install postgresql@18

# 启动服务
brew services start postgresql@18

# 创建数据库
createdb mydb
```

### 3.2 源码编译安装

```bash
# 1. 安装依赖
sudo apt-get install -y build-essential libreadline-dev zlib1g-dev libssl-dev libxml2-dev libxslt1-dev

# 2. 下载源码
wget https://ftp.postgresql.org/pub/source/v18.1/postgresql-18.1.tar.gz
tar -xzf postgresql-18.1.tar.gz
cd postgresql-18.1

# 3. 配置编译选项
./configure --prefix=/usr/local/pgsql \
    --with-openssl \
    --with-libxml \
    --with-libxslt \
    --enable-nls \
    --with-python \
    --with-perl

# 4. 编译和安装
make -j$(nproc)
sudo make install

# 5. 创建数据目录
sudo mkdir -p /usr/local/pgsql/data
sudo chown postgres:postgres /usr/local/pgsql/data

# 6. 初始化数据库
sudo -u postgres /usr/local/pgsql/bin/initdb -D /usr/local/pgsql/data
```

### 3.3 数据库初始化

```bash
# 使用initdb初始化数据库
sudo -u postgres /usr/pgsql-18/bin/initdb -D /var/lib/pgsql/18/data \
    --locale=zh_CN.UTF-8 \
    --encoding=UTF8 \
    --data-checksums

# 初始化参数说明：
# -D: 数据目录
# --locale: 区域设置
# --encoding: 字符编码
# --data-checksums: 启用数据校验和（PostgreSQL 9.3+）
```

### 3.4 服务管理

```bash
# 使用systemd管理（推荐）
sudo systemctl start postgresql-18
sudo systemctl stop postgresql-18
sudo systemctl restart postgresql-18
sudo systemctl status postgresql-18
sudo systemctl enable postgresql-18

# 使用pg_ctl管理
sudo -u postgres /usr/pgsql-18/bin/pg_ctl -D /var/lib/pgsql/18/data start
sudo -u postgres /usr/pgsql-18/bin/pg_ctl -D /var/lib/pgsql/18/data stop
sudo -u postgres /usr/pgsql-18/bin/pg_ctl -D /var/lib/pgsql/18/data restart
sudo -u postgres /usr/pgsql-18/bin/pg_ctl -D /var/lib/pgsql/18/data status
```

### 3.5 验证安装

```bash
# 检查PostgreSQL版本
psql --version

# 连接到数据库
sudo -u postgres psql

# 在psql中执行
SELECT version();
\conninfo
\l
```

## 4. 配置要点

- postgresql.conf / pg_hba.conf，内存/IO/连接数

### 4.1 postgresql.conf基础配置

```bash
# 编辑配置文件
sudo nano /var/lib/pgsql/18/data/postgresql.conf
```

#### 连接设置

```conf
# 监听地址（允许所有IP连接）
listen_addresses = '*'

# 端口
port = 5432

# 最大连接数
max_connections = 100

# 超级用户保留连接数
superuser_reserved_connections = 3
```

#### 内存设置

```conf
# 共享缓冲区（建议为系统内存的25%）
shared_buffers = 256MB

# 工作内存（用于排序、哈希等操作）
work_mem = 4MB

# 维护工作内存（用于VACUUM、CREATE INDEX等）
maintenance_work_mem = 64MB

# 有效缓存大小（建议为系统内存的50-75%）
effective_cache_size = 1GB
```

#### I/O设置

```conf
# 随机页面成本（SSD建议1.1，HDD建议4.0）
random_page_cost = 1.1

# 有效I/O并发数（SSD建议200，HDD建议2）
effective_io_concurrency = 200

# 异步I/O（PostgreSQL 18新特性）
backend_flush_after = 0
```

#### WAL设置

```conf
# WAL级别
wal_level = replica

# 最小WAL大小
min_wal_size = 1GB

# 最大WAL大小
max_wal_size = 4GB

# 检查点完成目标
checkpoint_completion_target = 0.9
```

#### 日志设置

```conf
# 日志收集
logging_collector = on

# 日志目录
log_directory = 'log'

# 日志文件名
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'

# 日志轮转
log_rotation_age = 1d
log_rotation_size = 100MB

# 慢查询日志
log_min_duration_statement = 500

# 记录检查点
log_checkpoints = on

# 记录连接
log_connections = on
log_disconnections = on
```

### 4.2 pg_hba.conf认证配置

```bash
# 编辑认证配置文件
sudo nano /var/lib/pgsql/18/data/pg_hba.conf
```

#### 认证规则示例

```conf
# TYPE  DATABASE        USER            ADDRESS                 METHOD

# 本地连接使用peer认证
local   all             all                                     peer

# IPv4本地连接使用md5认证
host    all             all             127.0.0.1/32            md5

# IPv4局域网连接使用md5认证
host    all             all             192.168.1.0/24          md5

# 复制连接使用md5认证
host    replication     replicator      192.168.1.0/24          md5

# SSL连接使用scram-sha-256认证
hostssl all             all             0.0.0.0/0               scram-sha-256
```

#### 认证方法说明

- **trust**: 无条件允许连接（仅用于本地开发）
- **md5**: 使用MD5加密密码
- **scram-sha-256**: 使用SCRAM-SHA-256加密密码（推荐）
- **peer**: 使用操作系统用户名认证（仅本地）
- **cert**: 使用SSL客户端证书认证

### 4.3 系统参数优化

#### Linux内核参数

```bash
# 编辑sysctl.conf
sudo nano /etc/sysctl.conf

# 添加以下参数
# 共享内存设置
kernel.shmmax = 68719476736
kernel.shmall = 4294967296

# 网络参数
net.core.somaxconn = 4096
net.ipv4.tcp_max_syn_backlog = 4096
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15

# 虚拟内存参数
vm.swappiness = 1
vm.dirty_ratio = 15
vm.dirty_background_ratio = 5

# 应用配置
sudo sysctl -p
```

#### 资源限制

```bash
# 编辑limits.conf
sudo nano /etc/security/limits.conf

# 添加以下限制
postgres soft nofile 65536
postgres hard nofile 65536
postgres soft nproc 32768
postgres hard nproc 32768
```

### 4.4 配置验证

```sql
-- 查看当前配置（带错误处理）
DO $$
DECLARE
    shared_buffers_val TEXT;
    max_connections_val TEXT;
    work_mem_val TEXT;
BEGIN
    SHOW shared_buffers INTO shared_buffers_val;
    SHOW max_connections INTO max_connections_val;
    SHOW work_mem INTO work_mem_val;

    RAISE NOTICE 'shared_buffers: %', shared_buffers_val;
    RAISE NOTICE 'max_connections: %', max_connections_val;
    RAISE NOTICE 'work_mem: %', work_mem_val;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '查看配置失败: %', SQLERRM;
END $$;

-- 查看所有配置（带错误处理和性能测试）
DO $$
DECLARE
    config_count INT;
BEGIN
    SELECT COUNT(*) INTO config_count
    FROM pg_settings;

    RAISE NOTICE '找到 % 个配置参数', config_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING 'pg_settings视图不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '查看配置失败: %', SQLERRM;
END $$;

EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT name, setting, unit, source FROM pg_settings ORDER BY name;
-- 执行时间: <50ms
-- 计划: Seq Scan

-- 重新加载配置（无需重启，带错误处理）
DO $$
BEGIN
    PERFORM pg_reload_conf();
    RAISE NOTICE '配置重新加载成功';
EXCEPTION
    WHEN undefined_function THEN
        RAISE EXCEPTION 'pg_reload_conf函数不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '重新加载配置失败: %', SQLERRM;
END $$;
```

## 5. 安全与合规

- 认证方式、加密、审计

### 5.1 用户和权限管理

```sql
-- 创建用户（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'app_user'
    ) THEN
        DROP USER app_user;
        RAISE NOTICE '已删除现有用户: app_user';
    END IF;

    CREATE USER app_user WITH PASSWORD 'secure_password';
    RAISE NOTICE '用户创建成功: app_user';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE WARNING '用户app_user已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建用户失败: %', SQLERRM;
END $$;

-- 创建数据库（带错误处理）
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_database
        WHERE datname = 'app_db'
    ) THEN
        DROP DATABASE app_db;
        RAISE NOTICE '已删除现有数据库: app_db';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'app_user'
    ) THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    END IF;

    CREATE DATABASE app_db OWNER app_user;
    RAISE NOTICE '数据库创建成功: app_db';
EXCEPTION
    WHEN duplicate_database THEN
        RAISE WARNING '数据库app_db已存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建数据库失败: %', SQLERRM;
END $$;

-- 授予权限（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_database
        WHERE datname = 'app_db'
    ) THEN
        RAISE EXCEPTION '数据库app_db不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'app_user'
    ) THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    END IF;

    GRANT CONNECT ON DATABASE app_db TO app_user;
    GRANT USAGE ON SCHEMA public TO app_user;
    GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO app_user;

    RAISE NOTICE '权限授予成功: app_user';
EXCEPTION
    WHEN undefined_database THEN
        RAISE EXCEPTION '数据库app_db不存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要数据库所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '授予权限失败: %', SQLERRM;
END $$;

-- 设置默认权限（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_database
        WHERE datname = current_database()
    ) THEN
        RAISE EXCEPTION '当前数据库不存在';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'app_user'
    ) THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    END IF;

    ALTER DEFAULT PRIVILEGES IN SCHEMA public
        GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO app_user;

    RAISE NOTICE '默认权限设置成功: app_user';
EXCEPTION
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要数据库所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '设置默认权限失败: %', SQLERRM;
END $$;
```

### 5.2 SSL/TLS加密

```bash
# 生成SSL证书
sudo -u postgres openssl req -new -x509 -days 365 -nodes \
    -text -out /var/lib/pgsql/18/data/server.crt \
    -keyout /var/lib/pgsql/18/data/server.key \
    -subj "/CN=postgresql-server"

# 设置权限
sudo chmod 600 /var/lib/pgsql/18/data/server.key
sudo chown postgres:postgres /var/lib/pgsql/18/data/server.key
sudo chown postgres:postgres /var/lib/pgsql/18/data/server.crt
```

```conf
# postgresql.conf中启用SSL
ssl = on
ssl_cert_file = 'server.crt'
ssl_key_file = 'server.key'
```

### 5.3 审计日志

```conf
# postgresql.conf中启用审计
log_statement = 'all'  # 记录所有SQL语句
# 或
log_statement = 'ddl'  # 仅记录DDL语句
log_statement = 'mod'  # 记录修改数据的语句

# 记录参数变更
log_parameter_max_length = 1024
```

### 5.4 行级安全策略

```sql
-- 启用行级安全（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'sensitive_data'
    ) THEN
        RAISE EXCEPTION '表sensitive_data不存在，请先创建';
    END IF;

    ALTER TABLE sensitive_data ENABLE ROW LEVEL SECURITY;
    RAISE NOTICE '行级安全已启用: sensitive_data';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表sensitive_data不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '启用行级安全失败: %', SQLERRM;
END $$;

-- 创建策略（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'sensitive_data'
    ) THEN
        RAISE EXCEPTION '表sensitive_data不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public'
        AND tablename = 'sensitive_data'
        AND policyname = 'user_data_policy'
    ) THEN
        DROP POLICY user_data_policy ON sensitive_data;
        RAISE NOTICE '已删除现有策略: user_data_policy';
    END IF;

    CREATE POLICY user_data_policy ON sensitive_data
        FOR ALL
    TO app_user
    USING (user_id = current_user_id());
```

### 5.5 数据加密

```sql
-- 使用pgcrypto扩展加密数据
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 加密存储
INSERT INTO users (username, password_hash)
VALUES ('user1', crypt('password123', gen_salt('bf')));

-- 验证密码
SELECT * FROM users
WHERE username = 'user1'
AND password_hash = crypt('password123', password_hash);
```

### 5.6 安全最佳实践

```bash
# 1. 定期更新PostgreSQL
sudo apt-get update && sudo apt-get upgrade postgresql-18

# 2. 限制网络访问
# 在pg_hba.conf中仅允许必要的IP访问

# 3. 使用强密码策略
# 在应用层或使用passwordcheck扩展

# 4. 定期备份
# 配置自动备份脚本

# 5. 监控异常访问
# 使用pg_stat_statements和日志分析
```

## 6. PostgreSQL 18生产级配置

### 6.1 小规模部署配置（<10GB数据，<100连接）

**适用场景**: 小型应用、开发测试环境、个人项目

**完整配置示例**:

```conf
# postgresql.conf - 小规模部署配置（2GB内存系统）

# ============================================
# 连接设置
# ============================================
listen_addresses = '*'
port = 5432
max_connections = 100
superuser_reserved_connections = 3

# ============================================
# 内存设置
# ============================================
shared_buffers = 512MB              # 系统内存的25%
effective_cache_size = 1GB          # 系统内存的50%
work_mem = 4MB                      # 每个操作的工作内存
maintenance_work_mem = 64MB         # 维护操作的工作内存
temp_buffers = 8MB                 # 临时缓冲区

# ============================================
# WAL设置
# ============================================
wal_level = replica
min_wal_size = 512MB
max_wal_size = 2GB
checkpoint_completion_target = 0.9
checkpoint_timeout = 15min
wal_compression = on                # PostgreSQL 18: WAL压缩

# ============================================
# I/O设置（PostgreSQL 18异步I/O）
# ============================================
random_page_cost = 1.1              # SSD
effective_io_concurrency = 200      # SSD
io_method = aio                     # PostgreSQL 18: 异步I/O
io_combine_limit = 64               # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 2          # PostgreSQL 18: 维护I/O工作进程

# ============================================
# 查询优化
# ============================================
default_statistics_target = 100
random_page_cost = 1.1
effective_cache_size = 1GB

# ============================================
# 日志设置
# ============================================
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_duration_statement = 1000  # 记录超过1秒的查询
log_checkpoints = on
log_connections = on
log_disconnections = on
log_lock_waits = on
log_temp_files = 0

# ============================================
# 自动清理
# ============================================
autovacuum = on
autovacuum_max_workers = 3
autovacuum_naptime = 1min
autovacuum_vacuum_threshold = 50
autovacuum_analyze_threshold = 50
```

**pg_hba.conf配置**:

```conf
# TYPE  DATABASE        USER            ADDRESS                 METHOD

# 本地连接
local   all             all                                     peer

# IPv4本地连接
host    all             all             127.0.0.1/32            scram-sha-256

# 局域网连接（仅允许特定网段）
host    all             all             192.168.1.0/24          scram-sha-256

# SSL连接（生产环境推荐）
hostssl all             all             0.0.0.0/0               scram-sha-256
```

**系统参数配置**:

```bash
# /etc/sysctl.conf
# 共享内存（2GB系统）
kernel.shmmax = 2147483648
kernel.shmall = 524288

# 网络参数
net.core.somaxconn = 1024
net.ipv4.tcp_max_syn_backlog = 2048
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15

# 虚拟内存
vm.swappiness = 1
vm.dirty_ratio = 15
vm.dirty_background_ratio = 5

# 文件系统
fs.file-max = 2097152
```

### 6.2 中规模部署配置（10-100GB数据，100-500连接）

**适用场景**: 中型应用、生产环境、多用户系统

**完整配置示例**:

```conf
# postgresql.conf - 中规模部署配置（8GB内存系统）

# ============================================
# 连接设置
# ============================================
listen_addresses = '*'
port = 5432
max_connections = 300
superuser_reserved_connections = 5

# ============================================
# 内存设置
# ============================================
shared_buffers = 2GB                # 系统内存的25%
effective_cache_size = 6GB          # 系统内存的75%
work_mem = 16MB                     # 每个操作的工作内存
maintenance_work_mem = 512MB        # 维护操作的工作内存
temp_buffers = 16MB                 # 临时缓冲区
max_prepared_transactions = 100     # 预准备事务

# ============================================
# WAL设置
# ============================================
wal_level = replica
min_wal_size = 1GB
max_wal_size = 4GB
checkpoint_completion_target = 0.9
checkpoint_timeout = 15min
wal_compression = on
wal_buffers = 16MB                  # WAL缓冲区

# ============================================
# I/O设置（PostgreSQL 18异步I/O）
# ============================================
random_page_cost = 1.1
effective_io_concurrency = 200
io_method = aio                     # PostgreSQL 18: 异步I/O
io_combine_limit = 128              # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 4          # PostgreSQL 18: 维护I/O工作进程
max_io_workers = 10                 # PostgreSQL 18: 最大I/O工作进程

# ============================================
# 查询优化
# ============================================
default_statistics_target = 100
random_page_cost = 1.1
effective_cache_size = 6GB
enable_hashjoin = on
enable_mergejoin = on
enable_nestloop = on

# ============================================
# 并行查询（PostgreSQL 18增强）
# ============================================
max_parallel_workers_per_gather = 4
max_parallel_workers = 8
max_parallel_maintenance_workers = 4
parallel_leader_participation = on  # PostgreSQL 18: Leader参与工作

# ============================================
# 日志设置
# ============================================
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_duration_statement = 500   # 记录超过500ms的查询
log_checkpoints = on
log_connections = on
log_disconnections = on
log_lock_waits = on
log_temp_files = 0
log_line_prefix = '%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '

# ============================================
# 自动清理
# ============================================
autovacuum = on
autovacuum_max_workers = 4
autovacuum_naptime = 30s
autovacuum_vacuum_threshold = 50
autovacuum_analyze_threshold = 50
autovacuum_vacuum_scale_factor = 0.1
autovacuum_analyze_scale_factor = 0.05
autovacuum_vacuum_cost_delay = 2ms
autovacuum_vacuum_cost_limit = 200

# ============================================
# 统计信息
# ============================================
track_activities = on
track_counts = on
track_io_timing = on
track_functions = all
pg_stat_statements.max = 10000
pg_stat_statements.track = all
```

**系统参数配置**:

```bash
# /etc/sysctl.conf
# 共享内存（8GB系统）
kernel.shmmax = 8589934592
kernel.shmall = 2097152

# 网络参数
net.core.somaxconn = 4096
net.ipv4.tcp_max_syn_backlog = 4096
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15
net.ipv4.tcp_fin_timeout = 30

# 虚拟内存
vm.swappiness = 1
vm.dirty_ratio = 15
vm.dirty_background_ratio = 5
vm.overcommit_memory = 2

# 文件系统
fs.file-max = 2097152
fs.aio-max-nr = 1048576
```

### 6.3 大规模部署配置（>100GB数据，>500连接）

**适用场景**: 大型应用、高并发系统、企业级生产环境

**完整配置示例**:

```conf
# postgresql.conf - 大规模部署配置（32GB内存系统）

# ============================================
# 连接设置
# ============================================
listen_addresses = '*'
port = 5432
max_connections = 1000
superuser_reserved_connections = 10

# ============================================
# 内存设置
# ============================================
shared_buffers = 8GB                # 系统内存的25%
effective_cache_size = 24GB         # 系统内存的75%
work_mem = 64MB                     # 每个操作的工作内存（注意：max_connections * work_mem不能太大）
maintenance_work_mem = 2GB          # 维护操作的工作内存
temp_buffers = 32MB                 # 临时缓冲区
max_prepared_transactions = 200     # 预准备事务
huge_pages = try                     # 尝试使用大页

# ============================================
# WAL设置
# ============================================
wal_level = replica
min_wal_size = 2GB
max_wal_size = 8GB
checkpoint_completion_target = 0.9
checkpoint_timeout = 15min
wal_compression = on
wal_buffers = 32MB                  # WAL缓冲区
wal_writer_delay = 200ms
wal_writer_flush_after = 1MB

# ============================================
# I/O设置（PostgreSQL 18异步I/O）
# ============================================
random_page_cost = 1.1
effective_io_concurrency = 300      # NVMe SSD
io_method = aio                     # PostgreSQL 18: 异步I/O
io_combine_limit = 256              # PostgreSQL 18: I/O合并限制
maintenance_io_workers = 8          # PostgreSQL 18: 维护I/O工作进程
max_io_workers = 16                 # PostgreSQL 18: 最大I/O工作进程

# ============================================
# 查询优化
# ============================================
default_statistics_target = 100
random_page_cost = 1.1
effective_cache_size = 24GB
enable_hashjoin = on
enable_mergejoin = on
enable_nestloop = on
join_collapse_limit = 12
from_collapse_limit = 12

# ============================================
# 并行查询（PostgreSQL 18增强）
# ============================================
max_parallel_workers_per_gather = 8
max_parallel_workers = 16
max_parallel_maintenance_workers = 8
parallel_leader_participation = on  # PostgreSQL 18: Leader参与工作
parallel_tuple_cost = 0.1
parallel_setup_cost = 1000

# ============================================
# 日志设置
# ============================================
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'
log_rotation_age = 1d
log_rotation_size = 100MB
log_min_duration_statement = 200    # 记录超过200ms的查询
log_checkpoints = on
log_connections = on
log_disconnections = on
log_lock_waits = on
log_temp_files = 0
log_line_prefix = '%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '
log_autovacuum_min_duration = 0     # 记录所有autovacuum操作

# ============================================
# 自动清理
# ============================================
autovacuum = on
autovacuum_max_workers = 6
autovacuum_naptime = 10s
autovacuum_vacuum_threshold = 50
autovacuum_analyze_threshold = 50
autovacuum_vacuum_scale_factor = 0.05
autovacuum_analyze_scale_factor = 0.02
autovacuum_vacuum_cost_delay = 2ms
autovacuum_vacuum_cost_limit = 2000

# ============================================
# 统计信息
# ============================================
track_activities = on
track_counts = on
track_io_timing = on
track_functions = all
pg_stat_statements.max = 50000
pg_stat_statements.track = all
pg_stat_statements.track_utility = on
```

**系统参数配置**:

```bash
# /etc/sysctl.conf
# 共享内存（32GB系统）
kernel.shmmax = 34359738368
kernel.shmall = 8388608

# 网络参数
net.core.somaxconn = 8192
net.ipv4.tcp_max_syn_backlog = 8192
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15
net.ipv4.tcp_fin_timeout = 30
net.ipv4.ip_local_port_range = 10000 65535

# 虚拟内存
vm.swappiness = 1
vm.dirty_ratio = 10
vm.dirty_background_ratio = 3
vm.overcommit_memory = 2
vm.overcommit_ratio = 80

# 文件系统
fs.file-max = 4194304
fs.aio-max-nr = 1048576
fs.nr_open = 1048576

# 大页支持（可选，提升性能）
vm.nr_hugepages = 4096
```

### 6.4 PostgreSQL 18新特性配置

**异步I/O配置**:

```conf
# postgresql.conf
# 启用异步I/O（PostgreSQL 18）
io_method = aio                     # 使用异步I/O
io_combine_limit = 128              # I/O请求合并限制
maintenance_io_workers = 4          # 维护操作的I/O工作进程数
max_io_workers = 10                 # 最大I/O工作进程数

# 性能提升：
# - 顺序扫描性能提升2-3倍
# - VACUUM性能提升2-3倍
# - 索引构建性能提升40%+
```

**虚拟生成列配置**:

```sql
-- 创建带虚拟生成列的表
CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    name VARCHAR(100),
    price DECIMAL(10,2),
    discount DECIMAL(5,2),
    final_price DECIMAL(10,2) GENERATED ALWAYS AS (price * (1 - discount/100)) VIRTUAL
);

-- 在虚拟生成列上创建索引（PostgreSQL 18支持）
CREATE INDEX idx_products_final_price ON products (final_price);

-- 查询优化器自动利用虚拟生成列索引
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM products WHERE final_price BETWEEN 50 AND 100;
```

**OAuth 2.0认证配置**:

```conf
# pg_hba.conf
# OAuth 2.0认证（PostgreSQL 18）
host    all             all             0.0.0.0/0               oauth

# postgresql.conf
# OAuth 2.0配置
oauth_issuer = 'https://auth.example.com'
oauth_client_id = 'postgresql-client'
oauth_client_secret = 'client-secret'
oauth_scope = 'openid profile email'
```

## 7. 故障排查与诊断

### 7.1 启动问题排查

**常见启动问题**:

```bash
# 1. 检查PostgreSQL进程
ps aux | grep postgres

# 2. 检查数据目录权限
ls -la /var/lib/pgsql/18/data
# 应该属于postgres用户

# 3. 检查配置文件语法
sudo -u postgres /usr/pgsql-18/bin/postgres -D /var/lib/pgsql/18/data --check-config

# 4. 检查端口占用
sudo netstat -tlnp | grep 5432
# 或
sudo ss -tlnp | grep 5432

# 5. 查看启动日志
sudo tail -f /var/lib/pgsql/18/data/log/postgresql-*.log

# 6. 手动启动测试（前台运行，查看详细错误）
sudo -u postgres /usr/pgsql-18/bin/postgres -D /var/lib/pgsql/18/data
```

**常见错误及解决方案**:

```bash
# 错误1: "could not create shared memory segment"
# 原因: 共享内存不足
# 解决: 增加kernel.shmmax和kernel.shmall
sudo sysctl -w kernel.shmmax=68719476736
sudo sysctl -w kernel.shmall=4294967296

# 错误2: "FATAL: lock file "postmaster.pid" already exists"
# 原因: 之前的PostgreSQL进程异常退出
# 解决: 检查并删除锁文件
sudo -u postgres rm /var/lib/pgsql/18/data/postmaster.pid

# 错误3: "FATAL: could not open directory "pg_wal": Permission denied"
# 原因: 目录权限问题
# 解决: 修复权限
sudo chown -R postgres:postgres /var/lib/pgsql/18/data
sudo chmod 700 /var/lib/pgsql/18/data

# 错误4: "FATAL: password authentication failed"
# 原因: 密码错误或pg_hba.conf配置问题
# 解决: 检查pg_hba.conf配置和用户密码
```

### 7.2 连接问题诊断

**连接问题排查步骤**:

```bash
# 1. 检查PostgreSQL服务状态
sudo systemctl status postgresql

# 2. 检查监听地址和端口
sudo netstat -tlnp | grep postgres
# 或
sudo ss -tlnp | grep postgres

# 3. 检查防火墙规则
sudo iptables -L -n | grep 5432
# 或
sudo firewall-cmd --list-all

# 4. 测试本地连接
psql -h localhost -U postgres -d postgres

# 5. 测试远程连接
psql -h <server_ip> -U postgres -d postgres

# 6. 检查pg_hba.conf配置
sudo cat /var/lib/pgsql/18/data/pg_hba.conf | grep -v "^#"

# 7. 检查连接日志
sudo tail -f /var/lib/pgsql/18/data/log/postgresql-*.log | grep -i connection
```

**常见连接问题及解决方案**:

```sql
-- 问题1: "too many connections"
-- 原因: 连接数达到max_connections限制
-- 解决: 增加max_connections或使用连接池

-- 查看当前连接数
SELECT count(*) FROM pg_stat_activity;

-- 查看连接详情
SELECT
    datname,
    usename,
    application_name,
    client_addr,
    state,
    query_start,
    state_change
FROM pg_stat_activity
ORDER BY query_start;

-- 查看连接数限制
SHOW max_connections;

-- 问题2: "connection refused"
-- 原因: PostgreSQL未启动或监听地址配置错误
-- 解决: 检查服务状态和listen_addresses配置

-- 问题3: "password authentication failed"
-- 原因: 密码错误或认证方法配置错误
-- 解决: 检查pg_hba.conf和用户密码

-- 重置密码
ALTER USER postgres WITH PASSWORD 'new_password';
```

### 7.3 性能问题诊断

**性能问题排查步骤**:

```sql
-- 1. 检查慢查询
SELECT
    query,
    calls,
    total_time,
    mean_time,
    max_time,
    stddev_time,
    rows
FROM pg_stat_statements
ORDER BY mean_time DESC
LIMIT 20;

-- 2. 检查锁等待
SELECT
    blocked_locks.pid AS blocked_pid,
    blocked_activity.usename AS blocked_user,
    blocking_locks.pid AS blocking_pid,
    blocking_activity.usename AS blocking_user,
    blocked_activity.query AS blocked_statement,
    blocking_activity.query AS blocking_statement
FROM pg_catalog.pg_locks blocked_locks
JOIN pg_catalog.pg_stat_activity blocked_activity ON blocked_activity.pid = blocked_locks.pid
JOIN pg_catalog.pg_locks blocking_locks ON blocking_locks.locktype = blocked_locks.locktype
JOIN pg_catalog.pg_stat_activity blocking_activity ON blocking_activity.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted
AND blocking_locks.granted;

-- 3. 检查I/O性能
SELECT * FROM pg_stat_io
ORDER BY reads DESC
LIMIT 20;

-- 4. 检查缓存命中率
SELECT
    'index hit rate' AS name,
    (sum(idx_blks_hit)) / nullif(sum(idx_blks_hit + idx_blks_read), 0) AS ratio
FROM pg_statio_user_indexes
UNION ALL
SELECT
    'table hit rate' AS name,
    sum(heap_blks_hit) / nullif(sum(heap_blks_hit) + sum(heap_blks_read), 0) AS ratio
FROM pg_statio_user_tables;

-- 5. 检查表大小和膨胀
SELECT
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS size,
    pg_size_pretty(pg_relation_size(schemaname||'.'||tablename)) AS table_size,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename) - pg_relation_size(schemaname||'.'||tablename)) AS indexes_size,
    n_dead_tup,
    n_live_tup,
    round(n_dead_tup * 100.0 / nullif(n_live_tup + n_dead_tup, 0), 2) AS dead_ratio
FROM pg_stat_user_tables
ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC
LIMIT 20;

-- 6. 检查索引使用情况
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan,
    idx_tup_read,
    idx_tup_fetch,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size
FROM pg_stat_user_indexes
WHERE idx_scan = 0
ORDER BY pg_relation_size(indexrelid) DESC;
```

**性能优化诊断脚本**:

```bash
#!/bin/bash
# performance_diagnosis.sh - PostgreSQL性能诊断脚本

echo "=== PostgreSQL性能诊断报告 ==="
echo "生成时间: $(date)"
echo ""

echo "=== 1. 系统资源 ==="
echo "内存使用:"
free -h
echo ""
echo "磁盘使用:"
df -h
echo ""
echo "I/O统计:"
iostat -x 1 3
echo ""

echo "=== 2. PostgreSQL连接数 ==="
psql -c "SELECT count(*) as total_connections, count(*) FILTER (WHERE state = 'active') as active_connections FROM pg_stat_activity;"
echo ""

echo "=== 3. 慢查询Top 10 ==="
psql -c "SELECT query, calls, mean_time, max_time FROM pg_stat_statements ORDER BY mean_time DESC LIMIT 10;"
echo ""

echo "=== 4. 锁等待 ==="
psql -c "SELECT count(*) as lock_waits FROM pg_stat_activity WHERE wait_event_type = 'Lock';"
echo ""

echo "=== 5. 缓存命中率 ==="
psql -c "SELECT 'index hit rate' AS name, (sum(idx_blks_hit)) / nullif(sum(idx_blks_hit + idx_blks_read), 0) AS ratio FROM pg_statio_user_indexes UNION ALL SELECT 'table hit rate' AS name, sum(heap_blks_hit) / nullif(sum(heap_blks_hit) + sum(heap_blks_read), 0) AS ratio FROM pg_statio_user_tables;"
echo ""

echo "=== 6. 表大小Top 10 ==="
psql -c "SELECT schemaname, tablename, pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS size FROM pg_stat_user_tables ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC LIMIT 10;"
```

### 7.4 日志分析方法

**日志配置优化**:

```conf
# postgresql.conf - 详细日志配置

# 基本日志设置
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'
log_rotation_age = 1d
log_rotation_size = 100MB

# 日志级别
log_min_messages = warning          # 记录警告及以上级别
log_min_error_statement = error     # 记录错误语句

# 查询日志
log_min_duration_statement = 500    # 记录超过500ms的查询
log_statement = 'ddl'               # 记录DDL语句
log_duration = off                  # 不记录每个语句的持续时间（避免日志过大）

# 连接日志
log_connections = on
log_disconnections = on
log_line_prefix = '%t [%p]: [%l-1] user=%u,db=%d,app=%a,client=%h '

# 检查点和WAL日志
log_checkpoints = on
log_autovacuum_min_duration = 0     # 记录所有autovacuum操作

# 锁等待日志
log_lock_waits = on

# 临时文件日志
log_temp_files = 0                  # 记录所有临时文件创建
```

**日志分析工具**:

```bash
# 1. 分析慢查询日志
grep "duration:" /var/lib/pgsql/18/data/log/postgresql-*.log | \
    awk '{print $NF}' | \
    sort -n | \
    tail -20

# 2. 分析错误日志
grep -i "error\|fatal\|panic" /var/lib/pgsql/18/data/log/postgresql-*.log | \
    tail -50

# 3. 分析连接日志
grep "connection" /var/lib/pgsql/18/data/log/postgresql-*.log | \
    awk '{print $1, $2, $NF}' | \
    sort | uniq -c | sort -rn | head -20

# 4. 分析锁等待日志
grep "lock" /var/lib/pgsql/18/data/log/postgresql-*.log | \
    tail -50

# 5. 使用pgBadger分析日志（推荐）
# 安装pgBadger
sudo apt-get install pgbadger

# 生成HTML报告
pgbadger /var/lib/pgsql/18/data/log/postgresql-*.log -o report.html

# 6. 实时监控日志
tail -f /var/lib/pgsql/18/data/log/postgresql-*.log | \
    grep -E "ERROR|FATAL|duration|connection"
```

## 8. 性能优化实践

### 8.1 系统级优化

**文件系统优化**:

```bash
# 1. 使用XFS文件系统（推荐用于PostgreSQL）
# 创建XFS文件系统
sudo mkfs.xfs /dev/sdb1

# 挂载时优化参数
sudo mount -o noatime,nodiratime /dev/sdb1 /var/lib/pgsql

# 2. 调整I/O调度器（SSD推荐使用none或noop）
echo none | sudo tee /sys/block/sdb/queue/scheduler

# 3. 禁用文件系统访问时间更新（提升性能）
# 在/etc/fstab中添加noatime选项
/dev/sdb1 /var/lib/pgsql xfs defaults,noatime,nodiratime 0 0
```

**网络优化**:

```bash
# /etc/sysctl.conf
# TCP参数优化
net.core.somaxconn = 4096
net.ipv4.tcp_max_syn_backlog = 4096
net.ipv4.tcp_keepalive_time = 600
net.ipv4.tcp_keepalive_probes = 3
net.ipv4.tcp_keepalive_intvl = 15
net.ipv4.tcp_fin_timeout = 30
net.ipv4.tcp_tw_reuse = 1
net.ipv4.tcp_max_tw_buckets = 2000000
net.ipv4.tcp_timestamps = 1
net.ipv4.tcp_sack = 1
net.ipv4.tcp_window_scaling = 1
```

### 8.2 数据库参数优化

**关键参数调优指南**:

```conf
# ============================================
# 内存参数调优
# ============================================
# shared_buffers: 系统内存的25%
# - 太小：频繁磁盘I/O
# - 太大：占用过多系统内存，影响其他进程
# 推荐值：2GB-8GB（根据系统内存调整）

# effective_cache_size: 系统内存的50-75%
# - 告诉优化器系统有多少内存可用于缓存
# - 不影响实际内存使用，仅用于查询规划
# 推荐值：系统内存的75%

# work_mem: 每个操作的工作内存
# - 注意：max_connections * work_mem不能超过系统内存
# - 用于排序、哈希连接、合并连接等操作
# 推荐值：4MB-64MB（根据连接数调整）

# ============================================
# I/O参数调优（PostgreSQL 18）
# ============================================
# io_method = aio: 启用异步I/O
# - 性能提升2-3倍（顺序扫描、VACUUM等）
# - 需要Linux内核支持io_uring

# effective_io_concurrency: I/O并发数
# - SSD: 200-300
# - NVMe: 300-500
# - HDD: 2-4

# ============================================
# WAL参数调优
# ============================================
# max_wal_size: 最大WAL大小
# - 影响检查点频率
# - 推荐值：系统内存的25-50%
# - 太大：恢复时间长
# - 太小：频繁检查点，影响性能

# checkpoint_completion_target: 检查点完成目标
# - 推荐值：0.9（在检查点时间窗口的90%内完成）
# - 平滑I/O负载
```

### 8.3 存储优化

**表空间优化**:

```sql
-- 1. 创建专用表空间（使用高性能存储）
CREATE TABLESPACE fast_storage
LOCATION '/fast_storage/postgresql';

-- 2. 在表空间上创建表
CREATE TABLE large_table (
    id SERIAL PRIMARY KEY,
    data TEXT
) TABLESPACE fast_storage;

-- 3. 移动现有表到新表空间
ALTER TABLE large_table SET TABLESPACE fast_storage;

-- 4. 查看表空间使用情况
SELECT
    spcname,
    pg_size_pretty(pg_tablespace_size(spcname)) AS size
FROM pg_tablespace;
```

**分区表优化**:

```sql
-- 1. 创建分区表（按时间分区）
CREATE TABLE sales (
    id SERIAL,
    sale_date DATE,
    amount DECIMAL(10,2)
) PARTITION BY RANGE (sale_date);

-- 2. 创建分区
CREATE TABLE sales_2024_01 PARTITION OF sales
FOR VALUES FROM ('2024-01-01') TO ('2024-02-01')
TABLESPACE fast_storage;

-- 3. 为分区创建索引
CREATE INDEX idx_sales_date ON sales (sale_date);

-- 4. 自动创建分区（使用函数）
-- 创建月度分区函数（带完整错误处理）
CREATE OR REPLACE FUNCTION create_monthly_partition(p_table_name text, p_start_date date)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
    v_partition_name text;
    v_end_date date;
BEGIN
    -- 参数验证
    IF p_table_name IS NULL OR TRIM(p_table_name) = '' THEN
        RAISE EXCEPTION '表名不能为空';
    END IF;

    IF p_start_date IS NULL THEN
        RAISE EXCEPTION '开始日期不能为空';
    END IF;

    -- 表名格式验证（防止SQL注入）
    IF p_table_name !~ '^[a-zA-Z_][a-zA-Z0-9_]*$' THEN
        RAISE EXCEPTION '表名格式无效: %', p_table_name;
    END IF;

    -- 检查主表是否存在
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = p_table_name
    ) THEN
        RAISE EXCEPTION '主表不存在: %', p_table_name;
    END IF;

    -- 计算分区日期范围
    v_partition_name := p_table_name || '_' || to_char(p_start_date, 'YYYY_MM');
    v_end_date := date_trunc('month', p_start_date) + INTERVAL '1 month';

    -- 检查分区是否已存在
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = v_partition_name
    ) THEN
        RAISE NOTICE '分区已存在: %', v_partition_name;
        RETURN;
    END IF;

    -- 创建分区
    BEGIN
        EXECUTE format(
            'CREATE TABLE %I PARTITION OF %I FOR VALUES FROM (%L) TO (%L)',
            v_partition_name, p_table_name, p_start_date, v_end_date
        );
        RAISE NOTICE '成功创建分区: % (日期范围: % 至 %)',
            v_partition_name, p_start_date, v_end_date;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '分区已存在: %', v_partition_name;
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建分区失败: % (错误: %)', v_partition_name, SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'create_monthly_partition执行失败: %', SQLERRM;
END;
$$;
```

### 8.4 网络优化

**连接池配置**:

```sql
-- 使用PgBouncer连接池
-- pgbouncer.ini配置
[databases]
mydb = host=localhost port=5432 dbname=mydb

[pgbouncer]
pool_mode = transaction          # 事务级连接池
max_client_conn = 1000           # 最大客户端连接数
default_pool_size = 25           # 默认连接池大小
min_pool_size = 5                # 最小连接池大小
reserve_pool_size = 5            # 保留连接池大小
reserve_pool_timeout = 3         # 保留连接池超时
max_db_connections = 100         # 每个数据库最大连接数
max_user_connections = 100       # 每个用户最大连接数
```

## 9. 监控与告警配置

### 9.1 关键指标监控

**核心监控指标**:

```sql
-- 1. 连接数监控
SELECT
    count(*) as total_connections,
    count(*) FILTER (WHERE state = 'active') as active_connections,
    count(*) FILTER (WHERE state = 'idle') as idle_connections,
    count(*) FILTER (WHERE state = 'idle in transaction') as idle_in_transaction,
    max_conn - count(*) as available_connections
FROM pg_stat_activity, (SELECT setting::int as max_conn FROM pg_settings WHERE name = 'max_connections') mc;

-- 2. 数据库大小监控
SELECT
    datname,
    pg_size_pretty(pg_database_size(datname)) AS size
FROM pg_database
ORDER BY pg_database_size(datname) DESC;

-- 3. 表大小和膨胀监控
SELECT
    schemaname,
    tablename,
    pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS total_size,
    pg_size_pretty(pg_relation_size(schemaname||'.'||tablename)) AS table_size,
    n_live_tup,
    n_dead_tup,
    round(n_dead_tup * 100.0 / nullif(n_live_tup + n_dead_tup, 0), 2) AS dead_ratio,
    last_vacuum,
    last_autovacuum
FROM pg_stat_user_tables
WHERE n_dead_tup > 1000
ORDER BY n_dead_tup DESC;

-- 4. 复制延迟监控（如果有从库）
SELECT
    client_addr,
    state,
    sync_state,
    pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS sent_lag_bytes,
    pg_wal_lsn_diff(pg_current_wal_lsn(), write_lsn) AS write_lag_bytes,
    pg_wal_lsn_diff(pg_current_wal_lsn(), flush_lsn) AS flush_lag_bytes,
    pg_wal_lsn_diff(pg_current_wal_lsn(), replay_lsn) AS replay_lag_bytes
FROM pg_stat_replication;

-- 5. 检查点监控
SELECT
    checkpoints_timed,
    checkpoints_req,
    checkpoint_write_time,
    checkpoint_sync_time,
    buffers_checkpoint,
    buffers_clean,
    maxwritten_clean,
    buffers_backend,
    buffers_backend_fsync,
    buffers_alloc
FROM pg_stat_bgwriter;

-- 6. I/O性能监控（PostgreSQL 18）
SELECT
    object,
    context,
    reads,
    writes,
    extends,
    fsyncs,
    read_time,
    write_time,
    sync_time
FROM pg_stat_io
ORDER BY reads DESC;
```

### 9.2 Prometheus监控配置

**PostgreSQL Exporter配置**:

```yaml
# docker-compose.yml
version: '3.8'
services:
  postgres:
    image: postgres:18
    environment:
      POSTGRES_PASSWORD: password
    volumes:
      - postgres_data:/var/lib/postgresql/data
    ports:
      - "5432:5432"

  postgres_exporter:
    image: prometheuscommunity/postgres-exporter
    environment:
      DATA_SOURCE_NAME: "postgresql://postgres:password@postgres:5432/postgres?sslmode=disable"
    ports:
      - "9187:9187"
    depends_on:
      - postgres

volumes:
  postgres_data:
```

**Prometheus配置**:

```yaml
# prometheus.yml
global:
  scrape_interval: 15s
  evaluation_interval: 15s

scrape_configs:
  - job_name: 'postgres'
    static_configs:
      - targets: ['postgres_exporter:9187']
    metrics_path: /metrics
```

**Grafana仪表板配置**:

```json
{
  "dashboard": {
    "title": "PostgreSQL监控",
    "panels": [
      {
        "title": "连接数",
        "targets": [
          {
            "expr": "pg_stat_database_numbackends{datname=\"postgres\"}"
          }
        ]
      },
      {
        "title": "查询QPS",
        "targets": [
          {
            "expr": "rate(pg_stat_database_xact_commit[5m]) + rate(pg_stat_database_xact_rollback[5m])"
          }
        ]
      },
      {
        "title": "缓存命中率",
        "targets": [
          {
            "expr": "sum(rate(pg_stat_database_blks_hit[5m])) / (sum(rate(pg_stat_database_blks_hit[5m])) + sum(rate(pg_stat_database_blks_read[5m])))"
          }
        ]
      }
    ]
  }
}
```

### 9.3 告警规则配置

**Prometheus告警规则**:

```yaml
# alerts.yml
groups:
  - name: postgresql_alerts
    interval: 30s
    rules:
      # 连接数告警
      - alert: PostgreSQLHighConnections
        expr: pg_stat_database_numbackends > 80
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL连接数过高"
          description: "数据库 {{ $labels.datname }} 连接数为 {{ $value }}，超过阈值80"

      # 慢查询告警
      - alert: PostgreSQLSlowQueries
        expr: rate(pg_stat_statements_mean_exec_time[5m]) > 1
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL慢查询"
          description: "查询平均执行时间 {{ $value }}秒，超过阈值1秒"

      # 复制延迟告警
      - alert: PostgreSQLReplicationLag
        expr: pg_replication_lag > 10485760  # 10MB
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "PostgreSQL复制延迟"
          description: "复制延迟 {{ $value }} 字节，超过阈值10MB"

      # 磁盘空间告警
      - alert: PostgreSQLDiskSpace
        expr: (node_filesystem_avail_bytes{mountpoint="/var/lib/postgresql"} / node_filesystem_size_bytes{mountpoint="/var/lib/postgresql"}) < 0.1
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "PostgreSQL磁盘空间不足"
          description: "磁盘使用率超过90%"

      # 检查点告警
      - alert: PostgreSQLCheckpointTooFrequent
        expr: rate(pg_stat_bgwriter_checkpoints_timed[5m]) > 0.1
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "PostgreSQL检查点过于频繁"
          description: "检查点频率 {{ $value }}，可能影响性能"
```

## 10. 安全加固详细指南

### 10.1 SSL/TLS完整配置

**生成SSL证书**:

```bash
# 1. 创建证书目录
sudo mkdir -p /var/lib/pgsql/18/data/ssl
sudo chown postgres:postgres /var/lib/pgsql/18/data/ssl
cd /var/lib/pgsql/18/data/ssl

# 2. 生成CA私钥
sudo -u postgres openssl genrsa -out ca.key 4096

# 3. 生成CA证书
sudo -u postgres openssl req -new -x509 -days 3650 -key ca.key -out ca.crt \
    -subj "/CN=PostgreSQL-CA"

# 4. 生成服务器私钥
sudo -u postgres openssl genrsa -out server.key 4096

# 5. 生成服务器证书请求
sudo -u postgres openssl req -new -key server.key -out server.csr \
    -subj "/CN=postgresql-server"

# 6. 生成服务器证书
sudo -u postgres openssl x509 -req -days 365 -in server.csr -CA ca.crt -CAkey ca.key \
    -CAcreateserial -out server.crt

# 7. 设置权限
sudo chmod 600 server.key
sudo chown postgres:postgres server.key server.crt

# 8. 清理临时文件
sudo rm server.csr
```

**PostgreSQL SSL配置**:

```conf
# postgresql.conf
ssl = on
ssl_cert_file = 'server.crt'
ssl_key_file = 'server.key'
ssl_ca_file = 'ca.crt'              # 可选：客户端证书验证
ssl_crl_file = ''                   # 可选：证书撤销列表
ssl_prefer_server_ciphers = on      # 优先使用服务器密码套件
ssl_ciphers = 'HIGH:!aNULL:!MD5'    # 允许的密码套件
ssl_min_protocol_version = 'TLSv1.2'  # 最小TLS版本
```

**pg_hba.conf SSL配置**:

```conf
# 强制SSL连接
hostssl all             all             0.0.0.0/0               scram-sha-256

# 客户端证书认证（更安全）
hostssl all             all             0.0.0.0/0               cert
```

**客户端连接测试**:

```bash
# 使用SSL连接
psql "postgresql://user:password@host:5432/dbname?sslmode=require"

# 验证SSL连接
psql -h host -U user -d dbname -c "\conninfo"
# 应该显示: SSL connection (protocol: TLSv1.3, cipher: ...)
```

### 10.2 防火墙规则配置

**iptables防火墙配置**:

```bash
# 1. 允许本地连接
sudo iptables -A INPUT -i lo -j ACCEPT

# 2. 允许特定IP访问PostgreSQL
sudo iptables -A INPUT -p tcp --dport 5432 -s 192.168.1.0/24 -j ACCEPT

# 3. 拒绝其他IP访问PostgreSQL
sudo iptables -A INPUT -p tcp --dport 5432 -j DROP

# 4. 保存规则
sudo iptables-save > /etc/iptables/rules.v4

# 5. 查看规则
sudo iptables -L -n -v | grep 5432
```

**firewalld防火墙配置**:

```bash
# 1. 添加PostgreSQL服务
sudo firewall-cmd --permanent --add-service=postgresql

# 2. 或添加自定义端口
sudo firewall-cmd --permanent --add-port=5432/tcp

# 3. 限制源IP（可选）
sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="192.168.1.0/24" port port="5432" protocol="tcp" accept'

# 4. 重新加载防火墙
sudo firewall-cmd --reload

# 5. 查看规则
sudo firewall-cmd --list-all
```

### 10.3 审计日志配置

**启用详细审计日志**:

```conf
# postgresql.conf
# 记录所有DDL语句
log_statement = 'ddl'

# 记录所有数据修改语句
log_statement = 'mod'

# 记录所有语句（生产环境谨慎使用，会产生大量日志）
# log_statement = 'all'

# 记录参数值
log_parameter_max_length = 1024
log_parameter = on

# 记录连接和断开
log_connections = on
log_disconnections = on

# 记录连接持续时间
log_duration = on

# 记录锁等待
log_lock_waits = on

# 记录临时文件
log_temp_files = 0
```

**使用pgAudit扩展（推荐）**:

```sql
-- 1. 安装pgAudit扩展
CREATE EXTENSION IF NOT EXISTS pgaudit;

-- 2. 配置审计级别
ALTER SYSTEM SET pgaudit.log = 'read,write,ddl';
ALTER SYSTEM SET pgaudit.log_catalog = off;
ALTER SYSTEM SET pgaudit.log_parameter = on;
ALTER SYSTEM SET pgaudit.log_statement_once = on;

-- 3. 重新加载配置
SELECT pg_reload_conf();

-- 4. 查看审计日志
SELECT * FROM pg_stat_statements WHERE query LIKE '%DELETE%' OR query LIKE '%UPDATE%';
```

### 10.4 行级安全策略配置

**行级安全策略详细配置**:

```sql
-- 1. 创建示例表（带错误处理）
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        DROP TABLE orders;
        RAISE NOTICE '已删除现有表: orders';
    END IF;

    CREATE TABLE orders (
        id SERIAL PRIMARY KEY,
        user_id INTEGER,
        order_date DATE,
        amount DECIMAL(10,2),
        status VARCHAR(20)
    );

    RAISE NOTICE '表创建成功: orders';
EXCEPTION
    WHEN duplicate_table THEN
        RAISE WARNING '表orders已存在';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建表失败: %', SQLERRM;
END $$;

-- 2. 启用行级安全（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    ALTER TABLE orders ENABLE ROW LEVEL SECURITY;
    RAISE NOTICE '行级安全已启用: orders';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '启用行级安全失败: %', SQLERRM;
END $$;

-- 3. 创建策略：用户只能查看自己的订单（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public'
        AND tablename = 'orders'
        AND policyname = 'user_orders_policy'
    ) THEN
        DROP POLICY user_orders_policy ON orders;
        RAISE NOTICE '已删除现有策略: user_orders_policy';
    END IF;

    CREATE POLICY user_orders_policy ON orders
        FOR SELECT
        TO app_user
        USING (user_id = current_setting('app.current_user_id', true)::INTEGER);

    RAISE NOTICE '策略创建成功: user_orders_policy';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建策略失败: %', SQLERRM;
END $$;

-- 4. 创建策略：用户只能插入自己的订单（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public'
        AND tablename = 'orders'
        AND policyname = 'user_orders_insert_policy'
    ) THEN
        DROP POLICY user_orders_insert_policy ON orders;
        RAISE NOTICE '已删除现有策略: user_orders_insert_policy';
    END IF;

    CREATE POLICY user_orders_insert_policy ON orders
        FOR INSERT
        TO app_user
        WITH CHECK (user_id = current_setting('app.current_user_id', true)::INTEGER);

    RAISE NOTICE '策略创建成功: user_orders_insert_policy';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建策略失败: %', SQLERRM;
END $$;

-- 5. 创建策略：用户只能更新自己的订单（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public'
        AND tablename = 'orders'
        AND policyname = 'user_orders_update_policy'
    ) THEN
        DROP POLICY user_orders_update_policy ON orders;
        RAISE NOTICE '已删除现有策略: user_orders_update_policy';
    END IF;

    CREATE POLICY user_orders_update_policy ON orders
        FOR UPDATE
        TO app_user
        USING (user_id = current_setting('app.current_user_id', true)::INTEGER)
        WITH CHECK (user_id = current_setting('app.current_user_id', true)::INTEGER);

    RAISE NOTICE '策略创建成功: user_orders_update_policy';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_object THEN
        RAISE EXCEPTION '用户app_user不存在，请先创建';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建策略失败: %', SQLERRM;
END $$;

-- 6. 创建策略：管理员可以查看所有订单（带错误处理）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE EXCEPTION '表orders不存在，请先创建';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_roles
        WHERE rolname = 'admin_user'
    ) THEN
        RAISE WARNING '用户admin_user不存在，策略将无法生效';
    END IF;

    IF EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public'
        AND tablename = 'orders'
        AND policyname = 'admin_orders_policy'
    ) THEN
        DROP POLICY admin_orders_policy ON orders;
        RAISE NOTICE '已删除现有策略: admin_orders_policy';
    END IF;

    CREATE POLICY admin_orders_policy ON orders
        FOR ALL
        TO admin_user
        USING (true)
        WITH CHECK (true);

    RAISE NOTICE '策略创建成功: admin_orders_policy';
EXCEPTION
    WHEN undefined_table THEN
        RAISE EXCEPTION '表orders不存在';
    WHEN undefined_object THEN
        RAISE WARNING '用户admin_user不存在，策略已创建但无法生效';
    WHEN insufficient_privilege THEN
        RAISE EXCEPTION '权限不足，需要表所有者或超级用户权限';
    WHEN OTHERS THEN
        RAISE EXCEPTION '创建策略失败: %', SQLERRM;
END $$;

-- 7. 测试策略（带错误处理）
DO $$
DECLARE
    result_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        RAISE WARNING '表orders不存在';
        RETURN;
    END IF;

    SET app.current_user_id = '123';
    SELECT COUNT(*) INTO result_count FROM orders;
    RAISE NOTICE '当前用户可查看 % 条订单（user_id=123）', result_count;
EXCEPTION
    WHEN undefined_table THEN
        RAISE WARNING '表orders不存在';
    WHEN OTHERS THEN
        RAISE WARNING '测试策略失败: %', SQLERRM;
END $$;
```

## 11. 后续整合任务

- 补充系统参数推荐表与不同OS差异

### 11.1 不同操作系统差异

#### Ubuntu/Debian

```bash
# 数据目录位置
/var/lib/postgresql/18/main

# 配置文件位置
/etc/postgresql/18/main/postgresql.conf
/etc/postgresql/18/main/pg_hba.conf

# 日志目录
/var/log/postgresql/
```

#### CentOS/RHEL

```bash
# 数据目录位置
/var/lib/pgsql/18/data

# 配置文件位置
/var/lib/pgsql/18/data/postgresql.conf
/var/lib/pgsql/18/data/pg_hba.conf

# 日志目录
/var/lib/pgsql/18/data/log
```

#### macOS (Homebrew)

```bash
# 数据目录位置
/opt/homebrew/var/postgresql@18

# 配置文件位置
/opt/homebrew/var/postgresql@18/postgresql.conf
/opt/homebrew/var/postgresql@18/pg_hba.conf
```

### 11.2 系统参数推荐表

| 系统内存 | shared_buffers | effective_cache_size | work_mem | maintenance_work_mem |
|---------|----------------|---------------------|----------|---------------------|
| 1GB     | 256MB         | 512MB              | 2MB      | 32MB                |
| 2GB     | 512MB         | 1GB                | 4MB      | 64MB                |
| 4GB     | 1GB           | 2GB                | 8MB      | 128MB               |
| 8GB     | 2GB           | 4GB                | 16MB     | 256MB               |
| 16GB    | 4GB           | 8GB                | 32MB     | 512MB               |
| 32GB    | 8GB           | 16GB               | 64MB     | 1GB                 |
| 64GB    | 16GB          | 32GB               | 128MB    | 2GB                 |

### 11.3 性能调优检查清单

```bash
# 1. 检查PostgreSQL版本
psql --version

# 2. 检查数据目录权限
ls -la /var/lib/pgsql/18/data

# 3. 检查配置文件语法
sudo -u postgres /usr/pgsql-18/bin/postgres -D /var/lib/pgsql/18/data -C config_file

# 4. 检查共享内存
ipcs -m

# 5. 检查系统资源
free -h
df -h
iostat -x 1

# 6. 检查连接数
psql -c "SELECT count(*) FROM pg_stat_activity;"

# 7. 检查慢查询
psql -c "SELECT * FROM pg_stat_statements ORDER BY mean_exec_time DESC LIMIT 10;"
```

---

## 12. OpenTelemetry集成

### 12.1 OpenTelemetry架构和原理

**OpenTelemetry**是一个开放标准的可观测性框架，用于生成、收集和管理遥测数据（指标、日志、追踪），帮助识别性能瓶颈和系统问题。

#### 12.1.1 OpenTelemetry架构

**核心组件**：

1. **Instrumentation（插桩）**：在应用程序中嵌入代码，收集遥测数据
2. **Collector（收集器）**：接收、处理和导出遥测数据
3. **Backend（后端）**：存储和分析遥测数据（如Jaeger、Zipkin、Prometheus）

**架构图**：

```text
应用程序
    ↓
OpenTelemetry SDK（插桩）
    ↓
OpenTelemetry Collector
    ↓
┌─────────┬─────────┬─────────┐
│Jaeger   │Prometheus│其他后端 │
│(追踪)   │(指标)   │(日志)   │
└─────────┴─────────┴─────────┘
```

#### 12.1.2 遥测数据类型

**三种遥测数据**：

1. **Traces（追踪）**：
   - 分布式请求的完整路径
   - 包含span（操作）和trace（请求）
   - 用于识别性能瓶颈

2. **Metrics（指标）**：
   - 数值型数据，如CPU使用率、请求数
   - 用于监控系统健康状态

3. **Logs（日志）**：
   - 文本型数据，记录事件和错误
   - 用于问题诊断

#### 12.1.3 OpenTelemetry优势

**优势**：

1. **标准化**：统一的API和SDK，支持多种语言
2. **可扩展**：支持多种后端系统
3. **可观测性**：提供完整的可观测性解决方案
4. **跨服务追踪**：追踪跨多个服务的请求

### 12.2 PostgreSQL驱动集成配置

#### 12.2.1 Python驱动集成（psycopg3）

**安装依赖**：

```bash
# 安装OpenTelemetry相关包
pip install opentelemetry-api opentelemetry-sdk
pip install opentelemetry-instrumentation-psycopg
pip install opentelemetry-exporter-jaeger
pip install opentelemetry-exporter-prometheus
```

**配置示例**：

```python
# PostgreSQL OpenTelemetry集成示例（Python）
from opentelemetry import trace
from opentelemetry.sdk.trace import TracerProvider
from opentelemetry.sdk.trace.export import BatchSpanProcessor
from opentelemetry.exporter.jaeger.thrift import JaegerExporter
from opentelemetry.instrumentation.psycopg import PsycopgInstrumentor
import psycopg

# 配置TracerProvider
trace.set_tracer_provider(TracerProvider())

# 配置Jaeger导出器
jaeger_exporter = JaegerExporter(
    agent_host_name="localhost",
    agent_port=6831,
)
trace.get_tracer_provider().add_span_processor(
    BatchSpanProcessor(jaeger_exporter)
)

# 自动插桩psycopg
PsycopgInstrumentor().instrument()

# 使用PostgreSQL连接（自动追踪）
conn = psycopg.connect(
    host="localhost",
    port=5432,
    dbname="mydb",
    user="postgres",
    password="password"
)

# 执行查询（自动创建span）
with conn.cursor() as cur:
    cur.execute("SELECT * FROM users WHERE id = %s", (123,))
    result = cur.fetchall()

conn.close()
```

#### 12.2.2 Java驱动集成（JDBC）

**Maven依赖**：

```xml
<!-- pom.xml -->
<dependencies>
    <!-- OpenTelemetry API -->
    <dependency>
        <groupId>io.opentelemetry</groupId>
        <artifactId>opentelemetry-api</artifactId>
        <version>1.32.0</version>
    </dependency>

    <!-- OpenTelemetry SDK -->
    <dependency>
        <groupId>io.opentelemetry</groupId>
        <artifactId>opentelemetry-sdk</artifactId>
        <version>1.32.0</version>
    </dependency>

    <!-- JDBC自动插桩 -->
    <dependency>
        <groupId>io.opentelemetry.instrumentation</groupId>
        <artifactId>opentelemetry-jdbc</artifactId>
        <version>1.32.0-alpha</version>
    </dependency>

    <!-- PostgreSQL JDBC驱动 -->
    <dependency>
        <groupId>org.postgresql</groupId>
        <artifactId>postgresql</artifactId>
        <version>42.7.1</version>
    </dependency>
</dependencies>
```

**配置示例**：

```java
// PostgreSQL OpenTelemetry集成示例（Java）
import io.opentelemetry.api.OpenTelemetry;
import io.opentelemetry.api.trace.Tracer;
import io.opentelemetry.instrumentation.jdbc.OpenTelemetryDriver;
import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.Statement;
import java.sql.ResultSet;

public class PostgreSQLOpenTelemetryExample {
    public static void main(String[] args) throws Exception {
        // 注册OpenTelemetry JDBC驱动
        Class.forName("io.opentelemetry.instrumentation.jdbc.OpenTelemetryDriver");

        // 配置连接URL（自动追踪）
        String url = "jdbc:otel:postgresql://localhost:5432/mydb";
        String user = "postgres";
        String password = "password";

        // 建立连接（自动创建span）
        try (Connection conn = DriverManager.getConnection(url, user, password);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM users WHERE id = 123")) {

            while (rs.next()) {
                System.out.println(rs.getString("name"));
            }
        }
    }
}
```

#### 12.2.3 Node.js驱动集成（pg）

**安装依赖**：

```bash
npm install @opentelemetry/api
npm install @opentelemetry/sdk-node
npm install @opentelemetry/instrumentation-pg
npm install @opentelemetry/exporter-jaeger
npm install pg
```

**配置示例**：

```javascript
// PostgreSQL OpenTelemetry集成示例（Node.js）
const { NodeSDK } = require('@opentelemetry/sdk-node');
const { getNodeAutoInstrumentations } = require('@opentelemetry/auto-instrumentations-node');
const { JaegerExporter } = require('@opentelemetry/exporter-jaeger');
const { Resource } = require('@opentelemetry/resources');
const { SemanticResourceAttributes } = require('@opentelemetry/semantic-resources');

// 配置OpenTelemetry SDK
const sdk = new NodeSDK({
  resource: new Resource({
    [SemanticResourceAttributes.SERVICE_NAME]: 'postgresql-app',
  }),
  traceExporter: new JaegerExporter({
    endpoint: 'http://localhost:14268/api/traces',
  }),
  instrumentations: [getNodeAutoInstrumentations()],
});

// 启动SDK
sdk.start();

// 使用PostgreSQL（自动追踪）
const { Pool } = require('pg');

const pool = new Pool({
  host: 'localhost',
  port: 5432,
  database: 'mydb',
  user: 'postgres',
  password: 'password',
});

// 执行查询（自动创建span）
async function queryUsers() {
  const result = await pool.query('SELECT * FROM users WHERE id = $1', [123]);
  return result.rows;
}

queryUsers().then(rows => {
  console.log(rows);
  pool.end();
});
```

#### 12.2.4 配置最佳实践

**配置建议**：

1. **采样率**：根据负载调整采样率，避免过度采样
2. **批量导出**：使用批量导出减少网络开销
3. **异步导出**：使用异步导出避免阻塞应用
4. **资源标签**：添加服务名、版本等标签便于过滤

**配置示例**：

```python
# OpenTelemetry配置最佳实践（Python）
from opentelemetry import trace
from opentelemetry.sdk.trace import TracerProvider
from opentelemetry.sdk.trace.export import BatchSpanProcessor
from opentelemetry.sdk.resources import Resource
from opentelemetry.exporter.otlp.proto.grpc.trace_exporter import OTLPSpanExporter

# 配置资源标签
resource = Resource.create({
    "service.name": "postgresql-app",
    "service.version": "1.0.0",
    "deployment.environment": "production",
})

# 配置TracerProvider
trace.set_tracer_provider(TracerProvider(resource=resource))

# 配置OTLP导出器（推荐）
otlp_exporter = OTLPSpanExporter(
    endpoint="http://localhost:4317",
    insecure=True,
)

# 配置批量处理器（优化性能）
span_processor = BatchSpanProcessor(
    otlp_exporter,
    max_queue_size=2048,  # 最大队列大小
    max_export_batch_size=512,  # 最大批量大小
    export_timeout_millis=30000,  # 导出超时
)

trace.get_tracer_provider().add_span_processor(span_processor)

# 配置采样率（生产环境建议0.1-0.5）
from opentelemetry.sdk.trace.sampling import TraceIdRatioBased

trace.get_tracer_provider().sampler = TraceIdRatioBased(0.1)  # 10%采样率
```

### 12.3 跨服务请求跟踪配置

#### 12.3.1 追踪配置

**分布式追踪**：

分布式追踪通过追踪ID（Trace ID）和跨度ID（Span ID）关联跨服务的请求。

**追踪流程**：

```text
客户端请求
    ↓
服务A（创建Trace ID: abc123）
    ↓
调用PostgreSQL（Span ID: span1，Parent: root）
    ↓
调用服务B（Span ID: span2，Parent: span1）
    ↓
调用PostgreSQL（Span ID: span3，Parent: span2）
    ↓
返回结果（所有span关联到Trace ID: abc123）
```

**配置示例**：

```python
# 跨服务请求跟踪配置（Python）
from opentelemetry import trace
from opentelemetry.propagate import inject, extract
from opentelemetry.trace.propagation.tracecontext import TraceContextTextMapPropagator
import requests

# 创建追踪器
tracer = trace.get_tracer(__name__)

# 服务A：创建span并传播上下文
def service_a():
    with tracer.start_as_current_span("service_a_operation") as span:
        span.set_attribute("service.name", "service-a")
        span.set_attribute("operation.type", "database_query")

        # 执行PostgreSQL查询
        with tracer.start_as_current_span("postgresql_query") as db_span:
            db_span.set_attribute("db.system", "postgresql")
            db_span.set_attribute("db.statement", "SELECT * FROM users")
            # 执行查询...

        # 调用服务B（传播追踪上下文）
        headers = {}
        inject(headers)  # 注入追踪上下文到HTTP头
        response = requests.get("http://service-b/api/data", headers=headers)

# 服务B：接收并继续追踪
def service_b():
    # 从HTTP头提取追踪上下文
    headers = request.headers
    context = extract(headers)

    with tracer.start_as_current_span("service_b_operation", context=context) as span:
        span.set_attribute("service.name", "service-b")
        # 继续处理...
```

#### 12.3.2 上下文传播

**上下文传播机制**：

OpenTelemetry通过上下文传播（Context Propagation）在服务间传递追踪信息。

**传播方式**：

1. **HTTP头传播**：通过HTTP头传递追踪上下文
2. **gRPC元数据传播**：通过gRPC元数据传递
3. **消息队列传播**：通过消息队列消息属性传递

**HTTP头传播示例**：

```python
# HTTP头传播配置
from opentelemetry.propagate import inject, extract
from opentelemetry.trace.propagation.tracecontext import TraceContextTextMapPropagator

# 发送请求时注入上下文
def send_request(url, data):
    headers = {}
    inject(headers)  # 注入追踪上下文
    response = requests.post(url, json=data, headers=headers)
    return response

# 接收请求时提取上下文
def handle_request(request):
    headers = dict(request.headers)
    context = extract(headers)  # 提取追踪上下文

    with tracer.start_as_current_span("handle_request", context=context):
        # 处理请求...
        pass
```

#### 12.3.3 实际案例

**案例：微服务架构中的PostgreSQL追踪**:

```python
# 微服务架构中的PostgreSQL追踪示例
from opentelemetry import trace
from opentelemetry.instrumentation.psycopg import PsycopgInstrumentor
import psycopg
from flask import Flask, request
from opentelemetry.propagate import extract

app = Flask(__name__)
tracer = trace.get_tracer(__name__)

# 自动插桩PostgreSQL
PsycopgInstrumentor().instrument()

# 用户服务
@app.route('/api/users/<user_id>', methods=['GET'])
def get_user(user_id):
    # 从请求头提取追踪上下文
    context = extract(dict(request.headers))

    with tracer.start_as_current_span("get_user", context=context) as span:
        span.set_attribute("user.id", user_id)

        # 连接PostgreSQL（自动创建span）
        conn = psycopg.connect(
            host="localhost",
            port=5432,
            dbname="mydb",
            user="postgres",
            password="password"
        )

        with conn.cursor() as cur:
            # 查询用户（自动追踪）
            cur.execute("SELECT * FROM users WHERE id = %s", (user_id,))
            user = cur.fetchone()

        conn.close()

        return {"user": user}
```

### 12.4 性能瓶颈识别方法

#### 12.4.1 追踪数据分析

**分析维度**：

1. **Span持续时间**：识别耗时最长的操作
2. **Span数量**：识别调用最频繁的操作
3. **错误率**：识别错误率最高的操作
4. **依赖关系**：识别服务间的依赖关系

**分析工具**：

1. **Jaeger UI**：可视化追踪数据
2. **Zipkin UI**：追踪数据可视化
3. **Grafana Tempo**：与Grafana集成

**分析示例**：

```sql
-- 使用Jaeger查询追踪数据（概念性示例）
-- 实际使用Jaeger API或UI

-- 查询最慢的PostgreSQL查询
SELECT
  operation_name,
  AVG(duration) AS avg_duration,
  MAX(duration) AS max_duration,
  COUNT(*) AS call_count
FROM traces
WHERE service_name = 'postgresql'
  AND operation_name LIKE '%query%'
GROUP BY operation_name
ORDER BY avg_duration DESC
LIMIT 10;
```

#### 12.4.2 瓶颈定位方法

**定位步骤**：

1. **识别慢操作**：通过追踪数据识别耗时最长的操作
2. **分析调用链**：分析完整的调用链，找出瓶颈环节
3. **检查依赖**：检查外部依赖（数据库、API等）的性能
4. **优化建议**：根据分析结果提出优化建议

**瓶颈识别示例**：

```python
# 性能瓶颈识别脚本（Python）
from opentelemetry import trace
from opentelemetry.sdk.trace import TracerProvider
from opentelemetry.sdk.trace.export import ConsoleSpanExporter

# 配置控制台导出器（用于调试）
trace.set_tracer_provider(TracerProvider())
trace.get_tracer_provider().add_span_processor(
    BatchSpanProcessor(ConsoleSpanExporter())
)

tracer = trace.get_tracer(__name__)

# 执行操作并分析性能
def analyze_performance():
    with tracer.start_as_current_span("complex_operation") as span:
        # 步骤1：数据库查询
        with tracer.start_as_current_span("db_query") as db_span:
            # 执行查询...
            db_span.set_attribute("db.query.duration", 100)  # 100ms

        # 步骤2：外部API调用
        with tracer.start_as_current_span("api_call") as api_span:
            # 调用API...
            api_span.set_attribute("http.duration", 500)  # 500ms（瓶颈！）

        # 步骤3：数据处理
        with tracer.start_as_current_span("data_processing") as proc_span:
            # 处理数据...
            proc_span.set_attribute("processing.duration", 50)  # 50ms

# 分析结果：
# - 总耗时：650ms
# - 瓶颈：API调用（500ms，77%）
# - 优化建议：优化API调用或使用缓存
```

#### 12.4.3 优化建议

**常见优化建议**：

1. **数据库优化**：
   - 添加索引
   - 优化查询语句
   - 使用连接池
   - 启用查询缓存

2. **API调用优化**：
   - 使用异步调用
   - 实现请求批处理
   - 使用缓存减少API调用

3. **服务优化**：
   - 减少服务间调用
   - 使用消息队列异步处理
   - 实现服务缓存

**优化示例**：

```python
# 基于追踪数据的优化建议（Python）
def generate_optimization_suggestions(trace_data):
    suggestions = []

    # 分析数据库查询
    db_spans = [s for s in trace_data.spans if s.attributes.get('db.system') == 'postgresql']
    if db_spans:
        avg_db_duration = sum(s.duration for s in db_spans) / len(db_spans)
        if avg_db_duration > 100:  # 超过100ms
            suggestions.append({
                'type': 'database',
                'issue': f'平均数据库查询时间过长: {avg_db_duration}ms',
                'suggestions': [
                    '检查查询是否有合适的索引',
                    '优化查询语句',
                    '考虑使用连接池',
                ]
            })

    # 分析API调用
    api_spans = [s for s in trace_data.spans if s.attributes.get('http.method')]
    if api_spans:
        avg_api_duration = sum(s.duration for s in api_spans) / len(api_spans)
        if avg_api_duration > 200:  # 超过200ms
            suggestions.append({
                'type': 'api',
                'issue': f'平均API调用时间过长: {avg_api_duration}ms',
                'suggestions': [
                    '考虑使用异步调用',
                    '实现请求批处理',
                    '使用缓存减少API调用',
                ]
            })

    return suggestions
```

---

## 13. ELK/Graylog日志管理

### 13.1 ELK Stack完整配置

**ELK Stack**是由Elasticsearch、Logstash和Kibana组成的日志管理解决方案，用于收集、存储、分析和可视化日志数据。

#### 13.1.1 Elasticsearch配置

**Elasticsearch**是分布式搜索和分析引擎，用于存储和索引日志数据。

**安装和配置**：

```bash
# 安装Elasticsearch（Ubuntu/Debian）
wget -qO - https://artifacts.elastic.co/GPG-KEY-elasticsearch | sudo apt-key add -
echo "deb https://artifacts.elastic.co/packages/8.x/apt stable main" | sudo tee /etc/apt/sources.list.d/elastic-8.x.list
sudo apt-get update && sudo apt-get install elasticsearch

# 配置Elasticsearch
sudo nano /etc/elasticsearch/elasticsearch.yml
```

**配置文件示例**：

```yaml
# /etc/elasticsearch/elasticsearch.yml
cluster.name: postgresql-logs
node.name: node-1
path.data: /var/lib/elasticsearch
path.logs: /var/log/elasticsearch
network.host: 0.0.0.0
http.port: 9200
discovery.type: single-node  # 单节点配置

# 生产环境配置
bootstrap.memory_lock: true
cluster.routing.allocation.disk.threshold_enabled: true
cluster.routing.allocation.disk.watermark.low: 85%
cluster.routing.allocation.disk.watermark.high: 90%
```

**启动和验证**：

```bash
# 启动Elasticsearch
sudo systemctl start elasticsearch
sudo systemctl enable elasticsearch

# 验证Elasticsearch运行
curl http://localhost:9200
```

#### 13.1.2 Logstash配置

**Logstash**是数据收集和处理管道，用于从PostgreSQL收集日志并发送到Elasticsearch。

**安装和配置**：

```bash
# 安装Logstash
sudo apt-get install logstash

# 创建PostgreSQL日志收集配置
sudo nano /etc/logstash/conf.d/postgresql.conf
```

**Logstash配置示例**：

```ruby
# /etc/logstash/conf.d/postgresql.conf
input {
  # 从PostgreSQL日志文件读取
  file {
    path => "/var/log/postgresql/postgresql-*.log"
    start_position => "beginning"
    sincedb_path => "/var/lib/logstash/sincedb"
    codec => multiline {
      pattern => "^%{TIMESTAMP_ISO8601}"
      negate => true
      what => "previous"
    }
  }

  # 从syslog读取（如果PostgreSQL日志发送到syslog）
  syslog {
    port => 514
    type => "postgresql"
  }
}

filter {
  if [type] == "postgresql" {
    # 解析PostgreSQL日志格式
    grok {
      match => {
        "message" => "%{TIMESTAMP_ISO8601:timestamp} \[%{NUMBER:pid}\] %{WORD:level}:  %{GREEDYDATA:message}"
      }
    }

    # 解析SQL语句
    if [message] =~ /statement:/ {
      grok {
        match => {
          "message" => "statement: %{GREEDYDATA:sql_statement}"
        }
      }
    }

    # 解析连接信息
    if [message] =~ /connection/ {
      grok {
        match => {
          "message" => "connection %{WORD:connection_type}: %{GREEDYDATA:connection_info}"
        }
      }
    }

    # 解析错误信息
    if [level] == "ERROR" {
      grok {
        match => {
          "message" => "ERROR:  %{GREEDYDATA:error_message}"
        }
      }
    }

    # 日期解析
    date {
      match => [ "timestamp", "yyyy-MM-dd HH:mm:ss.SSS" ]
    }

    # 添加字段
    mutate {
      add_field => {
        "database" => "postgresql"
        "environment" => "production"
      }
    }
  }
}

output {
  # 输出到Elasticsearch
  elasticsearch {
    hosts => ["localhost:9200"]
    index => "postgresql-logs-%{+YYYY.MM.dd}"
    template_name => "postgresql-logs"
    template => "/etc/logstash/templates/postgresql-logs.json"
    template_overwrite => true
  }

  # 调试输出（可选）
  stdout {
    codec => rubydebug
  }
}
```

**PostgreSQL日志配置**：

```conf
# postgresql.conf
# 启用日志记录
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d.log'
log_rotation_age = 1d
log_rotation_size = 100MB

# 记录所有SQL语句（用于分析）
log_statement = 'all'
log_min_duration_statement = 0

# 记录连接信息
log_connections = on
log_disconnections = on

# 记录锁等待
log_lock_waits = on

# 记录检查点
log_checkpoints = on

# 记录自动清理
log_autovacuum_min_duration = 0

# 日志格式（便于解析）
log_line_prefix = '%t [%p] %l: '
log_timezone = 'UTC'
```

#### 13.1.3 Kibana配置

**Kibana**是数据可视化工具，用于查询和可视化Elasticsearch中的日志数据。

**安装和配置**：

```bash
# 安装Kibana
sudo apt-get install kibana

# 配置Kibana
sudo nano /etc/kibana/kibana.yml
```

**Kibana配置示例**：

```yaml
# /etc/kibana/kibana.yml
server.port: 5601
server.host: "0.0.0.0"
elasticsearch.hosts: ["http://localhost:9200"]
kibana.index: ".kibana"

# 生产环境配置
server.maxPayloadBytes: 1048576
elasticsearch.requestTimeout: 300000
```

**创建Kibana仪表板**：

```json
// Kibana仪表板配置示例（JSON导出格式）
{
  "version": "8.0.0",
  "objects": [
    {
      "type": "dashboard",
      "id": "postgresql-logs-dashboard",
      "attributes": {
        "title": "PostgreSQL Logs Dashboard",
        "panelsJSON": "[...]"
      }
    }
  ]
}
```

**Kibana可视化配置**：

```text
Kibana可视化配置步骤：

1. 创建索引模式
   - 模式：postgresql-logs-*
   - 时间字段：@timestamp

2. 创建可视化
   - 错误日志数量（柱状图）
   - 慢查询趋势（折线图）
   - 连接数统计（饼图）
   - SQL语句分布（词云）

3. 创建仪表板
   - 组合所有可视化
   - 设置自动刷新
   - 配置筛选器
```

### 13.2 Graylog配置和集成

**Graylog**是开源的日志管理平台，提供日志收集、存储、搜索和分析功能。

#### 13.2.1 Graylog安装配置

**安装Graylog**：

```bash
# 安装MongoDB（Graylog元数据存储）
sudo apt-get install mongodb-org

# 安装Elasticsearch（Graylog数据存储）
sudo apt-get install elasticsearch

# 安装Graylog
wget https://packages.graylog2.org/repo/packages/graylog-5.1-repository_latest.deb
sudo dpkg -i graylog-5.1-repository_latest.deb
sudo apt-get update
sudo apt-get install graylog-server
```

**Graylog配置**：

```conf
# /etc/graylog/server/server.conf
is_master = true
node_id_file = /etc/graylog/server/node-id
password_secret = <generate-secret>
root_username = admin
root_password_sha2 = <generate-hash>
http_bind_address = 0.0.0.0:9000
http_external_uri = http://localhost:9000/
elasticsearch_hosts = http://localhost:9200
mongodb_uri = mongodb://localhost:27017/graylog
```

#### 13.2.2 PostgreSQL集成

**配置PostgreSQL日志输入**：

```text
Graylog输入配置：

1. System -> Inputs
2. 选择输入类型：
   - GELF UDP（推荐）
   - Syslog UDP
   - Beats（Filebeat）
3. 配置输入参数
4. 启动输入
```

**使用Filebeat收集PostgreSQL日志**：

```yaml
# /etc/filebeat/filebeat.yml
filebeat.inputs:
  - type: log
    enabled: true
    paths:
      - /var/log/postgresql/postgresql-*.log
    fields:
      logtype: postgresql
      environment: production
    fields_under_root: true
    multiline.pattern: '^%{TIMESTAMP_ISO8601}'
    multiline.negate: true
    multiline.match: after

processors:
  - add_host_metadata:
      when.not.contains.tags: forwarded
  - add_docker_metadata: ~
  - add_kubernetes_metadata: ~

output.logstash:
  hosts: ["localhost:5044"]

# 或直接输出到Graylog
output.gelf:
  hosts: ["localhost:12201"]
```

**PostgreSQL日志格式配置**：

```conf
# postgresql.conf
# 配置日志格式便于Graylog解析
log_line_prefix = '%t [%p] %l: '
log_timezone = 'UTC'
log_destination = 'stderr'
logging_collector = on
log_directory = 'log'
log_filename = 'postgresql-%Y-%m-%d.log'

# 记录详细信息
log_statement = 'all'
log_min_duration_statement = 0
log_connections = on
log_disconnections = on
log_lock_waits = on
```

#### 13.2.3 日志采集配置

**使用rsyslog转发日志**：

```conf
# /etc/rsyslog.d/30-postgresql.conf
# 转发PostgreSQL日志到Graylog
$ModLoad imfile
$InputFileName /var/log/postgresql/postgresql-*.log
$InputFileTag postgresql:
$InputFileStateFile postgresql-state
$InputFileSeverity info
$InputFileFacility local0
$InputRunFileMonitor

*.* @localhost:514;RSYSLOG_SyslogProtocol23Format
```

**使用GELF直接发送**：

```python
# 使用Python发送日志到Graylog（GELF格式）
import json
import socket
from datetime import datetime

def send_to_graylog(message, level='INFO', host='localhost', port=12201):
    """发送日志到Graylog（GELF格式）"""
    gelf_message = {
        "version": "1.1",
        "host": socket.gethostname(),
        "short_message": message,
        "full_message": message,
        "timestamp": datetime.now().timestamp(),
        "level": 6 if level == 'INFO' else 3,  # 6=INFO, 3=ERROR
        "_service": "postgresql",
        "_environment": "production"
    }

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.sendto(json.dumps(gelf_message).encode(), (host, port))
    sock.close()

# 使用示例
send_to_graylog("PostgreSQL connection established", "INFO")
send_to_graylog("Slow query detected: SELECT * FROM large_table", "WARNING")
```

### 13.3 日志分析和查询

#### 13.3.1 查询语法

**Elasticsearch查询语法（Kibana）**：

```text
Kibana查询示例：

1. 基本查询
   - 搜索所有错误：level:ERROR
   - 搜索特定时间：@timestamp:[2025-01-16 TO 2025-01-17]
   - 组合查询：level:ERROR AND message:connection

2. 字段查询
   - 搜索特定SQL：sql_statement:SELECT
   - 搜索特定用户：user:postgres
   - 搜索特定数据库：database:mydb

3. 正则表达式
   - 搜索模式：message:/timeout|deadlock/i

4. 范围查询
   - 搜索慢查询：duration:[100 TO *]
   - 搜索特定PID：pid:[1000 TO 2000]
```

**Graylog查询语法**：

```text
Graylog查询示例：

1. 基本查询
   - 搜索所有错误：level:ERROR
   - 搜索特定消息：message:"connection failed"
   - 组合查询：level:ERROR AND message:timeout

2. 字段查询
   - 搜索特定字段：source:postgresql-server-1
   - 搜索特定服务：service:postgresql

3. 时间范围
   - 最近1小时：timestamp:[now-1h TO now]
   - 特定日期：timestamp:2025-01-16

4. 函数查询
   - 统计错误数：count(level:ERROR)
   - 分组统计：groupby(level)
```

#### 13.3.2 分析案例

**案例1：慢查询分析**：

```text
Kibana查询：分析慢查询

查询条件：
- duration:[100 TO *]  # 查询时间超过100ms
- @timestamp:[now-1d TO now]  # 最近24小时

分析维度：
1. 按SQL语句分组统计
2. 按执行时间排序
3. 识别最慢的查询
4. 分析查询模式

可视化：
- 慢查询趋势图（折线图）
- 慢查询分布（饼图）
- SQL语句词云
```

**案例2：错误日志分析**：

```text
Graylog查询：分析错误日志

查询条件：
- level:ERROR
- timestamp:[now-7d TO now]  # 最近7天

分析维度：
1. 错误类型统计
2. 错误频率趋势
3. 错误来源分析
4. 错误影响评估

可视化：
- 错误数量趋势（折线图）
- 错误类型分布（饼图）
- 错误来源地图（地理分布）
```

**案例3：连接分析**：

```sql
-- 使用Kibana分析PostgreSQL连接
-- 查询条件：log_connections:true

-- 分析维度：
-- 1. 连接数趋势
-- 2. 连接来源IP分布
-- 3. 连接持续时间
-- 4. 连接失败原因

-- Kibana查询示例
{
  "query": {
    "bool": {
      "must": [
        {"match": {"message": "connection"}},
        {"range": {"@timestamp": {"gte": "now-1d"}}}
      ]
    }
  },
  "aggs": {
    "connections_by_hour": {
      "date_histogram": {
        "field": "@timestamp",
        "interval": "1h"
      }
    },
    "connections_by_ip": {
      "terms": {
        "field": "client_ip.keyword",
        "size": 10
      }
    }
  }
}
```

#### 13.3.3 可视化配置

**Kibana仪表板配置**：

```json
// PostgreSQL日志监控仪表板配置
{
  "dashboard": {
    "title": "PostgreSQL Logs Monitoring",
    "panels": [
      {
        "type": "visualization",
        "id": "error-count",
        "title": "Error Count",
        "visState": {
          "type": "histogram",
          "params": {
            "field": "@timestamp",
            "interval": "1h"
          },
          "aggs": [
            {
              "type": "filter",
              "params": {
                "query": "level:ERROR"
              }
            },
            {
              "type": "count"
            }
          ]
        }
      },
      {
        "type": "visualization",
        "id": "slow-queries",
        "title": "Slow Queries",
        "visState": {
          "type": "line",
          "params": {
            "field": "duration",
            "interval": "1h"
          }
        }
      }
    ]
  }
}
```

### 13.4 日志告警配置

#### 13.4.1 告警规则配置

**Kibana告警配置**：

```json
// Kibana告警规则配置
{
  "alert": {
    "name": "PostgreSQL Error Alert",
    "schedule": {
      "interval": "1m"
    },
    "conditions": {
      "aggType": "count",
      "aggField": "@timestamp",
      "termSize": 5,
      "termField": "level.keyword",
      "thresholdComparator": ">",
      "threshold": [10]
    },
    "actions": [
      {
        "type": "email",
        "subject": "PostgreSQL Error Alert",
        "to": ["admin@example.com"],
        "body": "Error count exceeded threshold: {{context.conditions}}"
      },
      {
        "type": "webhook",
        "url": "https://hooks.slack.com/services/YOUR/WEBHOOK/URL",
        "body": "{\"text\": \"PostgreSQL Error Alert: {{context.conditions}}\"}"
      }
    ]
  }
}
```

**Graylog告警配置**：

```text
Graylog告警配置步骤：

1. System -> Alerts
2. 创建告警条件：
   - 查询：level:ERROR
   - 时间范围：5分钟
   - 阈值：错误数 > 10
3. 配置通知：
   - Email通知
   - Slack通知
   - PagerDuty通知
4. 测试告警
```

**告警规则示例**：

```yaml
# Graylog告警规则配置（YAML格式）
alerts:
  - name: "PostgreSQL High Error Rate"
    condition:
      query: "level:ERROR"
      time_range: "5m"
      threshold: 10
    notifications:
      - type: email
        recipients: ["admin@example.com"]
      - type: slack
        webhook_url: "https://hooks.slack.com/services/YOUR/WEBHOOK/URL"

  - name: "PostgreSQL Slow Query Alert"
    condition:
      query: "duration:[1000 TO *]"
      time_range: "1m"
      threshold: 5
    notifications:
      - type: email
        recipients: ["dba@example.com"]

  - name: "PostgreSQL Connection Failure"
    condition:
      query: "message:connection*failed"
      time_range: "1m"
      threshold: 1
    notifications:
      - type: pagerduty
        service_key: "YOUR_PAGERDUTY_KEY"
```

#### 13.4.2 通知配置

**Email通知配置**：

```conf
# Graylog Email通知配置
# /etc/graylog/server/server.conf

transport_email_enabled = true
transport_email_hostname = smtp.example.com
transport_email_port = 587
transport_email_use_auth = true
transport_email_auth_username = alerts@example.com
transport_email_auth_password = <password>
transport_email_subject_prefix = [PostgreSQL Alert]
transport_email_from_email = alerts@example.com
```

**Slack通知配置**：

```python
# Slack通知脚本（Python）
import requests
import json

def send_slack_alert(message, webhook_url):
    """发送Slack告警"""
    payload = {
        "text": f"🚨 PostgreSQL Alert: {message}",
        "attachments": [
            {
                "color": "danger",
                "fields": [
                    {
                        "title": "Alert",
                        "value": message,
                        "short": False
                    }
                ]
            }
        ]
    }

    response = requests.post(
        webhook_url,
        data=json.dumps(payload),
        headers={'Content-Type': 'application/json'}
    )

    return response.status_code == 200

# 使用示例
send_slack_alert(
    "Error count exceeded threshold: 15 errors in last 5 minutes",
    "https://hooks.slack.com/services/YOUR/WEBHOOK/URL"
)
```

#### 13.4.3 实际案例

**案例：PostgreSQL慢查询告警**：

```text
告警场景：检测慢查询并告警

告警规则：
- 查询条件：duration:[5000 TO *]  # 查询时间超过5秒
- 时间窗口：5分钟
- 阈值：3次慢查询

告警动作：
1. 发送Email通知DBA团队
2. 发送Slack消息到#postgresql-alerts频道
3. 创建JIRA工单（自动）

告警消息示例：
"🚨 PostgreSQL Slow Query Alert
Time: 2025-01-16 10:30:00
Slow queries detected: 5
Top slow queries:
1. SELECT * FROM large_table WHERE id = ? (15.2s)
2. UPDATE orders SET status = ? WHERE user_id = ? (8.5s)
3. DELETE FROM logs WHERE created_at < ? (6.3s)
Action required: Review and optimize slow queries"
```

---

## 14. PostgreSQL日志管理详细配置

### 14.1 日志参数详细配置

#### 14.1.1 基础日志配置

**日志记录配置**：

```conf
# postgresql.conf
# ============================================
# 日志记录基础配置
# ============================================

# 启用日志收集器
logging_collector = on

# 日志目录
log_directory = 'log'

# 日志文件名格式
log_filename = 'postgresql-%Y-%m-%d_%H%M%S.log'

# 日志轮转配置
log_rotation_age = 1d      # 每天轮转
log_rotation_size = 100MB  # 或达到100MB时轮转

# 日志格式
log_line_prefix = '%t [%p] %l: '  # 时间戳 [进程ID] 日志级别:
log_timezone = 'UTC'               # 日志时区
```

#### 14.1.2 连接日志配置

**连接和断开日志**：

```conf
# 记录所有连接
log_connections = on

# 记录所有断开
log_disconnections = on

# 连接日志示例输出：
# 2025-01-16 10:30:00 UTC [12345] LOG: connection received: host=192.168.1.100 port=54321
# 2025-01-16 10:30:00 UTC [12345] LOG: connection authorized: user=postgres database=mydb
# 2025-01-16 10:35:00 UTC [12345] LOG: disconnection: session time: 0:05:00.123 user=postgres database=mydb
```

**连接详情配置**：

```conf
# 记录连接详情（PostgreSQL 14+）
log_connection_details = on

# 输出示例：
# 2025-01-16 10:30:00 UTC [12345] LOG: connection received: host=192.168.1.100 port=54321
# 2025-01-16 10:30:00 UTC [12345] DETAIL: connection parameters: application_name=psql, client_encoding=UTF8
```

#### 14.1.3 语句日志配置

**SQL语句日志**：

```conf
# 记录所有SQL语句
log_statement = 'all'  # 选项：none, ddl, mod, all

# 选项说明：
# - none: 不记录语句
# - ddl: 只记录DDL语句（CREATE, ALTER, DROP等）
# - mod: 记录DDL和DML语句（INSERT, UPDATE, DELETE等）
# - all: 记录所有语句（包括SELECT）

# 记录慢查询（执行时间超过指定毫秒）
log_min_duration_statement = 1000  # 1000ms = 1秒

# 输出示例：
# 2025-01-16 10:30:00 UTC [12345] LOG: duration: 1234.567 ms statement: SELECT * FROM large_table WHERE id = 123
```

**语句日志详细配置**：

```conf
# 记录语句参数（PostgreSQL 9.6+）
log_parameter_max_length = 0  # 0=不记录，-1=记录所有，>0=记录前N字节

# 记录语句执行计划（PostgreSQL 9.0+）
log_planner_stats = off
log_executor_stats = off
log_statement_stats = off

# 记录锁等待
log_lock_waits = on  # 记录等待锁超过deadlock_timeout的语句

# 输出示例：
# 2025-01-16 10:30:00 UTC [12345] LOG: process 12345 still waiting for ShareLock on transaction 123456 after 1000.123 ms
```

#### 14.1.4 检查点和自动清理日志

**检查点日志**：

```conf
# 记录检查点
log_checkpoints = on

# 输出示例：
# 2025-01-16 10:30:00 UTC [12345] LOG: checkpoint starting: time
# 2025-01-16 10:30:05 UTC [12345] LOG: checkpoint complete: wrote 1024 buffers (6.3%); 0 WAL file(s) added, 0 removed, 1 recycled; write=4.123 s, sync=0.456 s, total=5.234 s; sync files=1024, longest=0.123 s, average=0.004 s; distance=16384 kB, estimate=16384 kB
```

**自动清理日志**：

```conf
# 记录自动清理操作
log_autovacuum_min_duration = 0  # 记录所有自动清理操作（0=全部记录，-1=不记录）

# 输出示例：
# 2025-01-16 10:30:00 UTC [12345] LOG: automatic vacuum of table "public.users": index scans: 1
# 2025-01-16 10:30:00 UTC [12345] DETAIL: pages: 0 removed, 1000 remain, 0 skipped due to pins, 0 skipped frozen
# 2025-01-16 10:30:00 UTC [12345] DETAIL: tuples: 100 removed, 10000 remain, 0 are dead but not yet removable
```

#### 14.1.5 错误和警告日志

**错误日志级别**：

```conf
# 日志级别
log_min_messages = warning  # 选项：debug5, debug4, debug3, debug2, debug1, info, notice, warning, error, log, fatal, panic

# 推荐配置：
# - 开发环境：info 或 notice
# - 生产环境：warning 或 error

# 记录时间戳
log_timestamp = on

# 记录主机名
log_hostname = on

# 记录进程ID
log_process_id = on
```

**详细错误信息**：

```conf
# 记录详细错误信息
log_error_verbosity = default  # 选项：terse, default, verbose

# verbose模式输出更多调试信息
# 适用于问题诊断
```

### 14.2 日志文件管理策略

#### 14.2.1 日志轮转配置

**自动轮转**：

```conf
# postgresql.conf
# 日志轮转配置

# 按时间轮转
log_rotation_age = 1d      # 每天轮转
# 或
log_rotation_age = 1h      # 每小时轮转

# 按大小轮转
log_rotation_size = 100MB  # 达到100MB时轮转

# 保留的日志文件数
log_truncate_on_rotation = off  # 不截断，保留所有日志
```

**手动轮转**：

```bash
# 使用logrotate（Linux）
# /etc/logrotate.d/postgresql
/var/lib/pgsql/18/data/log/postgresql-*.log {
    daily
    rotate 30
    compress
    delaycompress
    notifempty
    missingok
    create 0640 postgres postgres
    sharedscripts
    postrotate
        /usr/bin/pg_ctl -D /var/lib/pgsql/18/data reload > /dev/null 2>&1 || true
    endscript
}
```

#### 14.2.2 日志归档策略

**日志归档配置**：

```bash
# 创建日志归档脚本
#!/bin/bash
# archive_logs.sh

LOG_DIR="/var/lib/pgsql/18/data/log"
ARCHIVE_DIR="/var/lib/pgsql/18/data/log/archive"
DATE=$(date +%Y%m%d)

# 创建归档目录
mkdir -p "$ARCHIVE_DIR"

# 归档昨天的日志
find "$LOG_DIR" -name "postgresql-*.log" -mtime +1 -exec mv {} "$ARCHIVE_DIR/" \;

# 压缩归档日志
find "$ARCHIVE_DIR" -name "*.log" -mtime +7 -exec gzip {} \;

# 删除30天前的归档日志
find "$ARCHIVE_DIR" -name "*.log.gz" -mtime +30 -delete

# 使用cron定期执行
# 0 2 * * * /usr/local/bin/archive_logs.sh
```

**日志压缩**：

```bash
# 压缩旧日志
find /var/lib/pgsql/18/data/log -name "postgresql-*.log" -mtime +1 -exec gzip {} \;

# 使用并行压缩（更快）
find /var/lib/pgsql/18/data/log -name "postgresql-*.log" -mtime +1 -print0 | \
    xargs -0 -P 4 -I {} gzip {}
```

#### 14.2.3 日志清理策略

**自动清理脚本**：

```sql
-- 创建日志清理函数
-- 清理旧日志函数（带完整错误处理）
CREATE OR REPLACE FUNCTION cleanup_old_logs(
    p_retention_days INTEGER DEFAULT 30
)
RETURNS TABLE (
    deleted_files INTEGER,
    freed_space TEXT
)
LANGUAGE plpgsql
AS $$
DECLARE
    v_log_dir TEXT;
    v_deleted_count INTEGER := 0;
    v_freed_bytes BIGINT := 0;
    v_file_rec RECORD;
BEGIN
    -- 参数验证
    IF p_retention_days IS NULL OR p_retention_days < 0 THEN
        RAISE EXCEPTION '保留天数必须>=0: %', p_retention_days;
    END IF;

    IF p_retention_days > 365 THEN
        RAISE WARNING '保留天数过大: %, 建议不超过365天', p_retention_days;
    END IF;

    -- 获取日志目录
    BEGIN
        SELECT setting INTO v_log_dir
        FROM pg_settings
        WHERE name = 'log_directory';

        IF v_log_dir IS NULL OR TRIM(v_log_dir) = '' THEN
            RAISE EXCEPTION '无法获取日志目录配置';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '获取日志目录失败: %', SQLERRM;
    END;

    -- 删除超过保留期的日志文件
    BEGIN
        FOR v_file_rec IN
            SELECT filename, size
            FROM pg_ls_logdir() AS f
            WHERE f.modification < NOW() - (p_retention_days || ' days')::INTERVAL
              AND f.filename IS NOT NULL
              AND f.filename LIKE 'postgresql-%.log'
        LOOP
            BEGIN
                PERFORM pg_file_unlink(v_log_dir || '/' || v_file_rec.filename);
                v_deleted_count := v_deleted_count + 1;
                v_freed_bytes := v_freed_bytes + COALESCE(v_file_rec.size, 0);
            EXCEPTION
                WHEN insufficient_privilege THEN
                    RAISE WARNING '权限不足，无法删除文件: %', v_file_rec.filename;
                WHEN OTHERS THEN
                    RAISE WARNING '删除文件失败: % (错误: %)', v_file_rec.filename, SQLERRM;
            END;
        END LOOP;
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '权限不足，无法访问日志目录';
        WHEN OTHERS THEN
            RAISE EXCEPTION '清理日志文件失败: %', SQLERRM;
    END;

    RETURN QUERY SELECT
        v_deleted_count,
        COALESCE(pg_size_pretty(v_freed_bytes), '0 bytes')::TEXT;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'cleanup_old_logs执行失败: %', SQLERRM;
END;
$$;

-- 使用示例
SELECT * FROM cleanup_old_logs(30);
```

### 14.3 日志分析工具

#### 14.3.1 pgBadger使用

**pgBadger安装**：

```bash
# Ubuntu/Debian
sudo apt-get install pgbadger

# CentOS/RHEL
sudo yum install pgbadger

# 或从源码安装
wget https://github.com/darold/pgbadger/archive/v12.0.tar.gz
tar -xzf v12.0.tar.gz
cd pgbadger-12.0
perl Makefile.PL
make && sudo make install
```

**pgBadger使用**：

```bash
# 基本使用
pgbadger /var/lib/pgsql/18/data/log/postgresql-*.log

# 输出HTML报告
pgbadger -o report.html /var/lib/pgsql/18/data/log/postgresql-*.log

# 分析多个日志文件
pgbadger /var/lib/pgsql/18/data/log/postgresql-*.log \
         /var/lib/pgsql/18/data/log/postgresql-*.log.1

# 增量分析（只分析新日志）
pgbadger -I /var/lib/pgsql/18/data/log/postgresql-*.log \
         --incremental \
         --outdir /var/lib/pgsql/18/data/log/reports

# 生成JSON报告
pgbadger -f json -o report.json /var/lib/pgsql/18/data/log/postgresql-*.log
```

**pgBadger配置**：

```bash
# 创建pgBadger配置文件
# ~/.pgbadgerrc
{
    "log_line_prefix": "%t [%p] %l: ",
    "log_timezone": "UTC",
    "outdir": "/var/lib/pgsql/18/data/log/reports",
    "incremental": true,
    "jobs": 4
}

# 使用配置文件
pgbadger -c ~/.pgbadgerrc /var/lib/pgsql/18/data/log/postgresql-*.log
```

#### 14.3.2 日志分析脚本

**慢查询分析脚本**：

```bash
#!/bin/bash
# analyze_slow_queries.sh

LOG_FILE="/var/lib/pgsql/18/data/log/postgresql-*.log"
OUTPUT_FILE="/tmp/slow_queries.txt"

# 提取慢查询（执行时间>1秒）
grep "duration:" "$LOG_FILE" | \
    awk '$NF > 1000 {print}' | \
    sort -k2 -n -r | \
    head -20 > "$OUTPUT_FILE"

echo "Top 20 slow queries saved to $OUTPUT_FILE"
```

**错误日志分析脚本**：

```bash
#!/bin/bash
# analyze_errors.sh

LOG_FILE="/var/lib/pgsql/18/data/log/postgresql-*.log"
OUTPUT_FILE="/tmp/errors.txt"

# 提取错误日志
grep -E "ERROR|FATAL|PANIC" "$LOG_FILE" | \
    awk '{print $1, $2, $3, $0}' | \
    sort > "$OUTPUT_FILE"

# 统计错误类型
echo "Error Statistics:"
grep -E "ERROR|FATAL|PANIC" "$LOG_FILE" | \
    awk '{print $NF}' | \
    sort | uniq -c | sort -rn

echo "Errors saved to $OUTPUT_FILE"
```

**连接分析脚本**：

```sql
-- 从日志中分析连接模式（需要log_connections = on）
-- 使用pg_read_file读取日志文件

-- 分析连接日志函数（带完整错误处理）
CREATE OR REPLACE FUNCTION analyze_connections(
    p_log_file_path TEXT
)
RETURNS TABLE (
    connection_time TIMESTAMP,
    user_name TEXT,
    database_name TEXT,
    client_host TEXT,
    session_duration INTERVAL
)
LANGUAGE plpgsql
AS $$
DECLARE
    v_log_content TEXT;
    v_file_size BIGINT;
BEGIN
    -- 参数验证
    IF p_log_file_path IS NULL OR TRIM(p_log_file_path) = '' THEN
        RAISE EXCEPTION '日志文件路径不能为空';
    END IF;

    -- 检查文件大小（防止读取过大文件）
    BEGIN
        SELECT pg_stat_file(p_log_file_path).size INTO v_file_size;

        IF v_file_size IS NULL THEN
            RAISE EXCEPTION '无法获取文件信息: %', p_log_file_path;
        END IF;

        IF v_file_size > 104857600 THEN  -- 100MB限制
            RAISE EXCEPTION '日志文件过大: % MB (限制: 100MB)',
                ROUND(v_file_size / 1048576.0, 2);
        END IF;
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '权限不足，无法访问日志文件: %', p_log_file_path;
        WHEN OTHERS THEN
            RAISE EXCEPTION '检查日志文件失败: %', SQLERRM;
    END;

    -- 读取日志文件
    BEGIN
        v_log_content := pg_read_file(p_log_file_path);

        IF v_log_content IS NULL THEN
            RAISE WARNING '日志文件内容为空: %', p_log_file_path;
        END IF;
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '权限不足，无法读取日志文件: %', p_log_file_path;
        WHEN OTHERS THEN
            RAISE EXCEPTION '读取日志文件失败: %', SQLERRM;
    END;

    -- 解析连接日志（简化示例）
    -- 实际实现需要更复杂的正则表达式解析
    -- 注意：这里返回示例数据，实际应该解析v_log_content

    RETURN QUERY
    SELECT
        NOW() AS connection_time,  -- 简化示例
        'postgres'::TEXT AS user_name,
        'mydb'::TEXT AS database_name,
        '192.168.1.100'::TEXT AS client_host,
        '5 minutes'::INTERVAL AS session_duration
    WHERE v_log_content IS NOT NULL;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'analyze_connections执行失败: %', SQLERRM;
END;
$$;
```

#### 14.3.3 实时日志监控

**实时日志监控脚本**：

```bash
#!/bin/bash
# monitor_logs.sh

LOG_FILE="/var/lib/pgsql/18/data/log/postgresql-$(date +%Y-%m-%d)*.log"

# 实时监控错误
tail -f "$LOG_FILE" | grep --line-buffered -E "ERROR|FATAL|PANIC" | \
    while read line; do
        echo "[$(date +%Y-%m-%d\ %H:%M:%S)] $line"
        # 发送告警（示例）
        # send_alert "$line"
    done

# 实时监控慢查询
tail -f "$LOG_FILE" | grep --line-buffered "duration:" | \
    awk '$NF > 1000 {print "[SLOW QUERY]", $0}'
```

**日志监控工具集成**：

```python
# 使用Python监控PostgreSQL日志
import subprocess
import re
from datetime import datetime

def monitor_postgresql_logs(log_file, callback):
    """监控PostgreSQL日志并调用回调函数"""
    process = subprocess.Popen(
        ['tail', '-f', log_file],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True
    )

    for line in process.stdout:
        # 检测错误
        if re.search(r'ERROR|FATAL|PANIC', line):
            callback('error', line)

        # 检测慢查询
        match = re.search(r'duration: (\d+\.\d+) ms', line)
        if match and float(match.group(1)) > 1000:
            callback('slow_query', line)

        # 检测连接问题
        if re.search(r'connection.*failed|connection.*refused', line):
            callback('connection_error', line)

# 使用示例
def handle_log_event(event_type, line):
    print(f"[{datetime.now()}] {event_type}: {line}")
    # 发送告警、记录到数据库等

monitor_postgresql_logs(
    '/var/lib/pgsql/18/data/log/postgresql-2025-01-16.log',
    handle_log_event
)
```

---

## 15. 交叉引用

### 相关文档

#### 部署架构

- ⭐⭐⭐ [性能调优实践](./05.02-性能调优实践.md) - 性能优化指南
- ⭐⭐ [集群部署与高可用](../集群部署/05.04-集群部署与高可用.md) - 高可用架构
- ⭐⭐ [Docker部署](../容器化部署/05.12-Docker部署.md) - 容器化部署

#### 核心课程

- ⭐⭐⭐ [系统架构与设计原理](../../01-核心基础/01.02-系统架构/01.01-系统架构与设计原理.md) - 系统架构基础
- ⭐⭐ [存储管理与数据持久化](../../04-存储与恢复/01.06-存储管理与数据持久化.md) - 存储管理
- ⭐⭐ [安全与合规](../../00-归档-项目管理文档/README.md) - 安全配置

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../../12-监控与诊断/06.01-监控与诊断.md) - 系统监控
- ⭐⭐ [备份与恢复](../../00-归档-项目管理文档/README.md) - 备份恢复

#### 高级特性

- ⭐⭐ [安全与合规](../../00-归档-项目管理文档/README.md) - 安全最佳实践

---

**文档版本**: v3.0
**最后更新**: 2025-01-16
**维护者**: PostgreSQL Documentation Team
