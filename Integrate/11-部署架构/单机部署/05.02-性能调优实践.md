...
- 步骤化SOP：
  ...
  4) 计划核查：`EXPLAIN (ANALYZE, BUFFERS, TIMING)` 与基数、并行度、I/O命中；
...
```sql
-- 1. 执行EXPLAIN (ANALYZE, BUFFERS, TIMING)
...
- EXPLAIN (ANALYZE, BUFFERS, TIMING)、pg_stat_statements、采样/剖析
- 必备扩展与设置：
  - `pg_stat_statements`：
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '列不存在: %.%', p_table_name, p_column_name;
    END IF;
    ...
    BEGIN
        ...
        IF v_total_val = 0 THEN
            RAISE WARNING '表为空: %', p_table_name;
            v_sel := 0;
        ELSE
    ...
    EXCEPTION
        ...
        WHEN numeric_value_out_of_range THEN
            RAISE EXCEPTION '选择性计算数值溢出';
        WHEN OTHERS THEN
            RAISE EXCEPTION '分析选择性失败: %', SQLERRM;
    END;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'analyze_selectivity执行失败: %', SQLERRM;
END;
$$;
...
BEGIN
    ...
    IF p_min_val IS NULL OR p_max_val IS NULL THEN
        RAISE EXCEPTION '最小值或最大值不能为空';
    END IF;

    IF p_min_val > p_max_val THEN
        RAISE EXCEPTION '最小值不能大于最大值: % > %', p_min_val, p_max_val;
    END IF;
    ...
    BEGIN
        ...
        IF v_hist_bounds IS NULL OR array_length(v_hist_bounds, 1) IS NULL THEN
            RAISE NOTICE '直方图数据不存在，返回默认估算: %', p_table_name || '.' || p_column_name;
            RETURN 0.1;  -- 默认估算
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取直方图失败: %', SQLERRM;
            RETURN 0.1;
    END;
    ...
    IF v_total_buckets IS NULL OR v_total_buckets <= 0 THEN
        RAISE WARNING '直方图桶数无效: %', v_total_buckets;
        RETURN 0.1;
    END IF;
    ...
    BEGIN
        FOR v_i IN 1..v_total_buckets LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '处理桶 % 时出错: %', v_i, SQLERRM;
            END;
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '计算匹配桶数失败: %', SQLERRM;
            RETURN 0.1;
    END;
    ...
    EXCEPTION
        WHEN division_by_zero THEN
            RETURN 0.1;
        WHEN OTHERS THEN
            RAISE WARNING '计算选择性失败: %', SQLERRM;
            RETURN 0.1;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'estimate_range_selectivity执行失败: %', SQLERRM;
        RETURN 0.1;
END;
$$;
...
BEGIN
    ...
    IF p_max_dead_percent IS NULL OR p_max_dead_percent < 0 OR p_max_dead_percent > 100 THEN
        RAISE EXCEPTION '最大死元组百分比必须在0-100之间: %', p_max_dead_percent;
    END IF;

    -- 检查pgstattuple扩展
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgstattuple') THEN
        RAISE WARNING 'pgstattuple扩展未安装，死元组百分比可能无法准确计算';
    END IF;
    ...
    BEGIN
        ...
        LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '处理索引 % 时出错: %', v_full_index_name, SQLERRM;
            END;
        END LOOP;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '查询膨胀索引失败: %', SQLERRM;
    END;
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'rebuild_bloated_indexes执行失败: %', SQLERRM;
END;
$$;
...
- ⭐⭐ [性能调优变更闭环](../../30-性能调优/README.md) - 变更管理
- ⭐⭐ [性能问题-案例库](../../16-应用设计与开发/行业案例/性能问题-案例库.md) - 性能问题案例

#### 前沿技术

- ⭐⭐ [多模型数据库](../../00-归档-项目管理文档/README.md) - 向量检索优化
