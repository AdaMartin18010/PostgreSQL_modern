---

> **📋 文档来源**: `PostgreSQL\05-部署架构\单机部署\05.02-性能调优实践.md`
> **📅 复制日期**: 2025-12-22
> **⚠️ 注意**: 本文档为复制版本，原文件保持不变

---

# 性能调优实践

**PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x, 15.x (兼容)
**难度**: ⭐⭐⭐⭐
**最后更新**: 2025-11-12

> 🆕 **PostgreSQL 18 + pgvector 2.0 性能优化要点** ⭐⭐⭐
>
> PostgreSQL 18 + pgvector 2.0 带来多项性能改进，调优时应重点关注：
>
> - ✅ **异步 I/O 子系统**: 向量检索 I/O 性能提升 2-3 倍 ⭐⭐⭐
> - ✅ **虚拟生成列**: 动态计算优化，查询性能提升 15-25% ⭐⭐
> - ✅ **并行查询增强**: 更智能的并行执行计划，建议调整max_parallel_workers
> - ✅ **索引优化**: B-tree去重优化，BRIN性能提升15-20%
> - ✅ **内存管理**: 动态共享内存，可减少预分配内存
> - ✅ **向量操作**: pgvector 2.0 SIMD优化，性能提升35-45%

---

## 目录

- [性能调优实践](#性能调优实践)
  - [目录](#目录)
  - [1. 概述](#1-概述)
  - [2. 合并来源与映射](#2-合并来源与映射)
  - [3. 基线与变更治理](#3-基线与变更治理)
    - [3.1 基线参数建立](#31-基线参数建立)
    - [3.2 SLA目标定义](#32-sla目标定义)
    - [3.3 指标阈值设置](#33-指标阈值设置)
    - [3.4 变更票据模板](#34-变更票据模板)
    - [3.5 预生产压测](#35-预生产压测)
  - [4. 调优流程](#4-调优流程)
    - [4.1 步骤1: 入口指征识别](#41-步骤1-入口指征识别)
    - [4.2 步骤2: 快速体检](#42-步骤2-快速体检)
    - [4.3 步骤3: TopN分析](#43-步骤3-topn分析)
    - [4.4 步骤4: 计划核查](#44-步骤4-计划核查)
    - [4.5 步骤5: 策略选择](#45-步骤5-策略选择)
    - [4.6 步骤6: 小步快跑](#46-步骤6-小步快跑)
    - [4.7 步骤7: 回归守护](#47-步骤7-回归守护)
  - [5. 典型场景](#5-典型场景)
  - [6. 工具与方法](#6-工具与方法)
  - [7. 参数调优指南（按子系统）](#7-参数调优指南按子系统)
    - [7.1 连接与并发参数](#71-连接与并发参数)
    - [7.2 缓冲与缓存参数](#72-缓冲与缓存参数)
    - [7.3 并行与JIT参数](#73-并行与jit参数)
    - [7.4 I/O与存储参数](#74-io与存储参数)
    - [7.5 WAL与持久化参数](#75-wal与持久化参数)
    - [7.6 维护与自清理参数](#76-维护与自清理参数)
  - [8. 查询层优化策略](#8-查询层优化策略)
    - [8.1 代价对齐优化](#81-代价对齐优化)
    - [8.2 访问路径优化](#82-访问路径优化)
    - [8.3 连接策略优化](#83-连接策略优化)
    - [8.4 查询重写优化](#84-查询重写优化)
  - [9. 存储与表设计](#9-存储与表设计)
  - [10. 典型工作负载基线](#10-典型工作负载基线)
  - [11. 验证与回归](#11-验证与回归)
  - [12. PostgreSQL 18 新特性应用](#12-postgresql-18-新特性应用)
    - [异步 I/O 优化](#异步-io-优化)
    - [虚拟生成列优化](#虚拟生成列优化)
  - [13. 索引优化深度分析](#13-索引优化深度分析)
    - [13.1 复合索引设计原则](#131-复合索引设计原则)
      - [13.1.1 复合索引基础](#1311-复合索引基础)
      - [13.1.2 设计原则](#1312-设计原则)
    - [13.2 列顺序选择策略](#132-列顺序选择策略)
      - [13.2.1 选择性评估](#1321-选择性评估)
      - [13.2.2 查询频率分析](#1322-查询频率分析)
      - [13.2.3 实际案例](#1323-实际案例)
    - [13.3 选择性评估方法](#133-选择性评估方法)
      - [13.3.1 统计信息查询](#1331-统计信息查询)
      - [13.3.2 直方图分析](#1332-直方图分析)
    - [13.4 索引维护最佳实践](#134-索引维护最佳实践)
      - [13.4.1 索引重建策略](#1341-索引重建策略)
      - [13.4.2 索引监控](#1342-索引监控)
    - [13.5 PostgreSQL 18索引优化](#135-postgresql-18索引优化)
      - [13.5.1 B-tree去重优化](#1351-b-tree去重优化)
      - [13.5.2 BRIN性能提升](#1352-brin性能提升)
      - [13.5.3 并行索引构建](#1353-并行索引构建)
  - [14. 统计信息维护最佳实践](#14-统计信息维护最佳实践)
    - [14.1 ANALYZE命令深度使用](#141-analyze命令深度使用)
      - [14.1.1 ANALYZE基础](#1411-analyze基础)
      - [14.1.2 ANALYZE参数详解](#1412-analyze参数详解)
      - [14.1.3 统计信息内容](#1413-统计信息内容)
    - [14.2 统计信息更新策略](#142-统计信息更新策略)
      - [14.2.1 自动ANALYZE配置](#1421-自动analyze配置)
      - [14.2.2 手动ANALYZE策略](#1422-手动analyze策略)
      - [14.2.3 统计信息监控](#1423-统计信息监控)
    - [14.3 查询规划器优化](#143-查询规划器优化)
      - [14.3.1 统计信息对规划器的影响](#1431-统计信息对规划器的影响)
      - [14.3.2 扩展统计信息](#1432-扩展统计信息)
      - [14.3.3 查询计划验证](#1433-查询计划验证)
    - [14.4 PostgreSQL 18统计信息新特性](#144-postgresql-18统计信息新特性)
      - [14.4.1 虚拟生成列统计](#1441-虚拟生成列统计)
      - [14.4.2 改进的多变量统计](#1442-改进的多变量统计)
      - [14.4.3 更精细的直方图](#1443-更精细的直方图)
      - [14.4.4 自适应统计信息更新](#1444-自适应统计信息更新)
  - [15. 请求性能跟踪](#15-请求性能跟踪)
    - [15.1 pg\_stat\_activity视图深度使用](#151-pg_stat_activity视图深度使用)
      - [15.1.1 视图字段详解](#1511-视图字段详解)
      - [15.1.2 连接状态分析](#1512-连接状态分析)
      - [15.1.3 等待事件分析](#1513-等待事件分析)
    - [15.2 并发问题识别](#152-并发问题识别)
      - [15.2.1 死锁检测](#1521-死锁检测)
      - [15.2.2 阻塞查询识别](#1522-阻塞查询识别)
      - [15.2.3 长时间运行查询](#1523-长时间运行查询)
    - [15.3 慢查询分析](#153-慢查询分析)
      - [15.3.1 使用pg\_stat\_statements](#1531-使用pg_stat_statements)
      - [15.3.2 查询优化建议](#1532-查询优化建议)
      - [15.3.3 实时慢查询监控](#1533-实时慢查询监控)
  - [16. 事务资源消耗分析](#16-事务资源消耗分析)
    - [16.1 事务内存使用监控](#161-事务内存使用监控)
      - [16.1.1 work\_mem监控](#1611-work_mem监控)
      - [16.1.2 内存使用分析](#1612-内存使用分析)
    - [16.2 资源消耗较高事务识别](#162-资源消耗较高事务识别)
      - [16.2.1 识别高资源消耗事务](#1621-识别高资源消耗事务)
      - [16.2.2 资源消耗分析工具](#1622-资源消耗分析工具)
    - [16.3 资源限制配置](#163-资源限制配置)
      - [16.3.1 work\_mem配置](#1631-work_mem配置)
      - [16.3.2 连接数限制](#1632-连接数限制)
      - [16.3.3 资源限制最佳实践](#1633-资源限制最佳实践)
  - [17. ML驱动的性能优化](#17-ml驱动的性能优化)
    - [17.1 机器学习在查询优化中的应用](#171-机器学习在查询优化中的应用)
      - [17.1.1 ML模型选择](#1711-ml模型选择)
      - [17.1.2 特征工程](#1712-特征工程)
      - [17.1.3 模型训练和评估](#1713-模型训练和评估)
    - [17.2 自动参数调优](#172-自动参数调优)
      - [17.2.1 参数调优算法](#1721-参数调优算法)
      - [17.2.2 自动化工具](#1722-自动化工具)
    - [17.3 智能索引推荐](#173-智能索引推荐)
      - [17.3.1 索引推荐算法](#1731-索引推荐算法)
      - [17.3.2 工具使用](#1732-工具使用)
    - [17.4 负载预测和资源调度](#174-负载预测和资源调度)
      - [17.4.1 负载预测模型](#1741-负载预测模型)
      - [17.4.2 资源调度策略](#1742-资源调度策略)
  - [18. 实战案例](#18-实战案例)
    - [案例1: OLTP系统调优](#案例1-oltp系统调优)
    - [案例2: OLAP分析系统调优](#案例2-olap分析系统调优)
    - [案例3: 混合负载调优](#案例3-混合负载调优)
  - [14. 深入阅读](#14-深入阅读)
    - [相关文档](#相关文档)
      - [查询与优化](#查询与优化)
      - [核心课程](#核心课程)
      - [运维实践](#运维实践)
      - [前沿技术](#前沿技术)
      - [版本特性](#版本特性)
    - [外部资源](#外部资源)
  - [15. 参考文献](#15-参考文献)

---

## 1. 概述

- 面向生产的“监测→诊断→定位→优化→验证→回归防护”闭环。
- 分层拆解：系统资源（CPU/内存/IO/网络）→ 数据库实例（连接/缓冲/检查点/WAL/Autovacuum）→ 对象（表/索引/膨胀）→ 查询与计划（代价/并行/基数）→ 应用（连接池/事务模型）。

## 2. 合并来源与映射

- 1.1.16-性能调优与监控-扩充版.md
- 1.1.16-性能调优与监控.md
- 03-查询与优化/02.04-执行计划与性能调优.md（交叉）

## 3. 基线与变更治理

- 建立"基线参数+SLA目标+指标阈值+回滚脚本"的变更四件套。
- 每次调优仅变更少量参数，记录变更票据：参数→旧值→新值→预期影响→验证窗口→回滚条件。
- 预生产压测与生产灰度：以典型工作负载（OLTP/OLAP/混合）重放/回放进行对比。

### 3.1 基线参数建立

```sql
-- 1. 创建参数基线表
CREATE TABLE IF NOT EXISTS parameter_baseline (
    baseline_id SERIAL PRIMARY KEY,
    baseline_time TIMESTAMP DEFAULT NOW(),
    parameter_name TEXT,
    parameter_value TEXT,
    parameter_source TEXT,
    notes TEXT
);

-- 2. 记录当前参数基线
INSERT INTO parameter_baseline (parameter_name, parameter_value, parameter_source)
SELECT name, setting, source
FROM pg_settings
WHERE source IN ('configuration file', 'command line', 'environment variable')
AND name IN (
    'shared_buffers', 'work_mem', 'maintenance_work_mem',
    'max_connections', 'effective_cache_size',
    'random_page_cost', 'effective_io_concurrency',
    'checkpoint_timeout', 'max_wal_size',
    'autovacuum_vacuum_scale_factor', 'autovacuum_analyze_scale_factor'
);

-- 3. 查询参数基线
SELECT * FROM parameter_baseline ORDER BY baseline_time DESC;
```

### 3.2 SLA目标定义

```sql
-- 创建SLA目标表
CREATE TABLE IF NOT EXISTS sla_targets (
    target_id SERIAL PRIMARY KEY,
    target_name TEXT,
    metric_name TEXT,
    target_value NUMERIC,
    unit TEXT,
    created_at TIMESTAMP DEFAULT NOW()
);

-- 定义SLA目标
INSERT INTO sla_targets (target_name, metric_name, target_value, unit) VALUES
('P95延迟', 'p95_latency', 100, 'ms'),
('P99延迟', 'p99_latency', 500, 'ms'),
('吞吐量', 'tps', 1000, 'transactions/sec'),
('缓存命中率', 'cache_hit_ratio', 95, 'percent'),
('错误率', 'error_rate', 0.1, 'percent');
```

### 3.3 指标阈值设置

```sql
-- 创建指标阈值表
CREATE TABLE IF NOT EXISTS metric_thresholds (
    threshold_id SERIAL PRIMARY KEY,
    metric_name TEXT,
    warning_threshold NUMERIC,
    critical_threshold NUMERIC,
    unit TEXT
);

-- 设置指标阈值
INSERT INTO metric_thresholds (metric_name, warning_threshold, critical_threshold, unit) VALUES
('active_connections', 80, 95, 'count'),
('cache_hit_ratio', 90, 85, 'percent'),
('replication_lag', 1048576, 10485760, 'bytes'),  -- 1MB, 10MB
('dead_tuples_ratio', 10, 20, 'percent'),
('checkpoint_frequency', 5, 10, 'minutes');
```

### 3.4 变更票据模板

```sql
-- 创建变更记录表
CREATE TABLE IF NOT EXISTS change_tickets (
    ticket_id SERIAL PRIMARY KEY,
    change_time TIMESTAMP DEFAULT NOW(),
    parameter_name TEXT,
    old_value TEXT,
    new_value TEXT,
    expected_impact TEXT,
    verification_window TEXT,
    rollback_condition TEXT,
    change_status TEXT,  -- 'pending', 'in_progress', 'completed', 'rolled_back'
    actual_impact TEXT,
    changed_by TEXT
);

-- 记录变更
INSERT INTO change_tickets (
    parameter_name, old_value, new_value, expected_impact,
    verification_window, rollback_condition, changed_by
) VALUES (
    'shared_buffers', '256MB', '512MB',
    '提高缓存命中率5-10%',
    '2025-11-22 02:00 - 06:00',
    'P95延迟增加>10%或错误率上升',
    current_user
);
```

### 3.5 预生产压测

```bash
# 使用pgbench进行压测
# 1. 初始化测试数据
pgbench -i -s 100 mydb  # -s: 缩放因子，100表示1000万行

# 2. 运行基准测试
pgbench -c 10 -j 2 -T 300 mydb
# -c: 客户端数
# -j: 线程数
# -T: 运行时间（秒）

# 3. 运行自定义测试
pgbench -f custom_script.sql -c 10 -j 2 -T 300 mydb

# 4. 对比测试结果
# 变更前
pgbench -c 10 -j 2 -T 300 mydb > baseline.txt

# 变更后
pgbench -c 10 -j 2 -T 300 mydb > after_change.txt

# 对比
diff baseline.txt after_change.txt
```

## 4. 调优流程

- 指标观察→瓶颈归因（CPU/IO/锁/计划）→变更验证
- 步骤化SOP：
  1) 入口指征：P99/吞吐/错误率异常、队列堆积、告警触发；
  2) 快速体检：连接、CPU利用、IO饱和、WAL/检查点、Autovacuum滞后；
  3) TopN：`pg_stat_statements`/活动会话/等待事件；
  4) 计划核查：`EXPLAIN (ANALYZE, BUFFERS, VERBOSE)` 与基数、并行度、I/O命中；
  5) 策略选择：索引/统计/SQL/参数/架构改造；
  6) 小步快跑：单变量变更→对比指标→保留或回滚；
  7) 回归守护：告警门槛、自动回滚开关（必要时）。

### 4.1 步骤1: 入口指征识别

```sql
-- 1. 检查P99延迟（需要pg_stat_statements）
SELECT
    queryid,
    LEFT(query, 100) as query_preview,
    calls,
    mean_exec_time,
    (SELECT percentile_cont(0.99) WITHIN GROUP (ORDER BY mean_exec_time)
     FROM pg_stat_statements) as p99_latency
FROM pg_stat_statements
WHERE mean_exec_time > (
    SELECT percentile_cont(0.99) WITHIN GROUP (ORDER BY mean_exec_time)
    FROM pg_stat_statements
)
ORDER BY mean_exec_time DESC
LIMIT 10;

-- 2. 检查吞吐量
SELECT
    datname,
    xact_commit + xact_rollback as total_transactions,
    xact_commit,
    xact_rollback
FROM pg_stat_database
WHERE datname = current_database();

-- 3. 检查错误率
SELECT
    datname,
    xact_rollback::float / NULLIF(xact_commit + xact_rollback, 0) * 100 as error_rate
FROM pg_stat_database
WHERE datname = current_database();

-- 4. 检查队列堆积
SELECT
    COUNT(*) FILTER (WHERE state = 'active') as active_queries,
    COUNT(*) FILTER (WHERE wait_event_type IS NOT NULL) as waiting_queries,
    COUNT(*) as total_connections
FROM pg_stat_activity
WHERE datname = current_database();
```

### 4.2 步骤2: 快速体检

```sql
-- 1. 连接数检查
SELECT
    COUNT(*) as total_connections,
    COUNT(*) FILTER (WHERE state = 'active') as active_connections,
    COUNT(*) FILTER (WHERE state = 'idle') as idle_connections,
    COUNT(*) FILTER (WHERE state = 'idle in transaction') as idle_in_transaction
FROM pg_stat_activity
WHERE datname = current_database();

-- 2. CPU利用率（通过等待事件推断）
SELECT
    wait_event_type,
    COUNT(*) as count,
    ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (), 2) as percentage
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL
GROUP BY wait_event_type
ORDER BY count DESC;

-- 3. I/O饱和检查
SELECT
    datname,
    blks_read,
    blks_hit,
    ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) as cache_hit_ratio
FROM pg_stat_database
WHERE datname = current_database();

-- 4. WAL/检查点检查
SELECT
    checkpoints_timed,
    checkpoints_req,
    checkpoint_write_time,
    checkpoint_sync_time,
    buffers_checkpoint,
    buffers_clean,
    buffers_backend
FROM pg_stat_bgwriter;

-- 5. Autovacuum滞后检查
SELECT
    schemaname,
    tablename,
    n_dead_tup,
    n_live_tup,
    last_autovacuum,
    CASE
        WHEN n_live_tup > 0 THEN ROUND(100.0 * n_dead_tup / n_live_tup, 2)
        ELSE 0
    END as dead_tuple_percent
FROM pg_stat_user_tables
WHERE n_dead_tup > 10000
ORDER BY n_dead_tup DESC
LIMIT 20;
```

### 4.3 步骤3: TopN分析

```sql
-- 1. TopN慢查询
SELECT
    queryid,
    LEFT(query, 200) as query_preview,
    calls,
    total_exec_time,
    mean_exec_time,
    max_exec_time,
    rows,
    ROUND(100.0 * shared_blks_hit / NULLIF(shared_blks_hit + shared_blks_read, 0), 2) as hit_ratio
FROM pg_stat_statements
ORDER BY total_exec_time DESC
LIMIT 10;

-- 2. TopN活动会话
SELECT
    pid,
    usename,
    application_name,
    state,
    wait_event_type,
    wait_event,
    query_start,
    NOW() - query_start as query_duration,
    LEFT(query, 100) as query_preview
FROM pg_stat_activity
WHERE state = 'active'
ORDER BY query_start
LIMIT 10;

-- 3. TopN等待事件
SELECT
    wait_event_type,
    wait_event,
    COUNT(*) as count
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL
GROUP BY wait_event_type, wait_event
ORDER BY count DESC
LIMIT 10;
```

### 4.4 步骤4: 计划核查

```sql
-- 1. 执行EXPLAIN ANALYZE
EXPLAIN (ANALYZE, BUFFERS, VERBOSE, COSTS, TIMING)
SELECT * FROM users WHERE email = 'user@example.com';

-- 2. 分析计划关键指标
-- - Planning Time: 计划时间
-- - Execution Time: 执行时间
-- - Buffers: shared hit/read/dirtied/written
-- - I/O Time: 读取时间
-- - Rows: 实际行数 vs 估算行数

-- 3. 检查基数估算误差
-- 如果实际行数与估算行数差异大，需要更新统计信息
ANALYZE users;

-- 4. 检查并行度
-- 查看计划中是否使用了并行工作进程
-- 如果应该并行但没有，检查max_parallel_workers_per_gather

-- 5. 检查I/O命中率
-- shared hit应该尽可能高（>95%）
```

### 4.5 步骤5: 策略选择

```sql
-- 策略1: 索引优化
-- 检查缺失的索引
SELECT
    schemaname,
    tablename,
    seq_scan,
    idx_scan,
    CASE
        WHEN seq_scan + idx_scan > 0
        THEN ROUND(100.0 * seq_scan / (seq_scan + idx_scan), 2)
        ELSE 0
    END as seq_scan_percent
FROM pg_stat_user_tables
WHERE seq_scan > 1000
AND seq_scan > idx_scan
ORDER BY seq_scan DESC
LIMIT 10;

-- 策略2: 统计信息优化
ALTER TABLE users ALTER COLUMN email SET STATISTICS 1000;
CREATE STATISTICS users_email_stats (dependencies) ON email, created_at FROM users;
ANALYZE users;

-- 策略3: SQL重写
-- 优化查询逻辑，减少数据扫描

-- 策略4: 参数调整
ALTER SYSTEM SET work_mem = '64MB';
SELECT pg_reload_conf();

-- 策略5: 架构改造
-- 分区表、读写分离、缓存层等
```

### 4.6 步骤6: 小步快跑

```bash
# 1. 记录变更前指标
psql -c "
SELECT
    'baseline' as stage,
    NOW() as timestamp,
    (SELECT SUM(total_exec_time) FROM pg_stat_statements) as total_exec_time,
    (SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
     FROM pg_stat_database WHERE datname = current_database()) as cache_hit_ratio
" > /tmp/baseline_metrics.txt

# 2. 执行单变量变更
psql -c "ALTER SYSTEM SET work_mem = '64MB';"
psql -c "SELECT pg_reload_conf();"

# 3. 等待稳定期（5-10分钟）
sleep 300

# 4. 记录变更后指标
psql -c "
SELECT
    'after_change' as stage,
    NOW() as timestamp,
    (SELECT SUM(total_exec_time) FROM pg_stat_statements) as total_exec_time,
    (SELECT SUM(blks_hit)::float / NULLIF(SUM(blks_hit) + SUM(blks_read), 0) * 100
     FROM pg_stat_database WHERE datname = current_database()) as cache_hit_ratio
" > /tmp/after_change_metrics.txt

# 5. 对比指标
diff /tmp/baseline_metrics.txt /tmp/after_change_metrics.txt

# 6. 决定保留或回滚
# 如果性能提升，保留变更
# 如果性能下降，回滚
```

### 4.7 步骤7: 回归守护

```sql
-- 1. 创建告警阈值表
CREATE TABLE IF NOT EXISTS alert_thresholds (
    threshold_id SERIAL PRIMARY KEY,
    metric_name TEXT,
    warning_value NUMERIC,
    critical_value NUMERIC,
    auto_rollback BOOLEAN DEFAULT false
);

-- 2. 设置告警阈值
INSERT INTO alert_thresholds (metric_name, warning_value, critical_value, auto_rollback) VALUES
('p95_latency', 100, 200, false),  -- 毫秒
('error_rate', 0.1, 1.0, true),    -- 百分比
('cache_hit_ratio', 90, 85, false); -- 百分比

-- 3. 监控查询（定期执行）
SELECT
    metric_name,
    current_value,
    warning_value,
    critical_value,
    CASE
        WHEN current_value >= critical_value THEN 'CRITICAL'
        WHEN current_value >= warning_value THEN 'WARNING'
        ELSE 'OK'
    END as status
FROM (
    SELECT
        'p95_latency' as metric_name,
        (SELECT percentile_cont(0.95) WITHIN GROUP (ORDER BY mean_exec_time)
         FROM pg_stat_statements) as current_value,
        (SELECT critical_value FROM alert_thresholds WHERE metric_name = 'p95_latency') as critical_value,
        (SELECT warning_value FROM alert_thresholds WHERE metric_name = 'p95_latency') as warning_value
) sub;
```

## 5. 典型场景

- 索引/统计信息、连接池、并行、存储与检查点
- 示例排查与处置：
  - 估算偏差→扩大统计目标或创建扩展统计：

    ```sql
    ALTER TABLE t ALTER COLUMN c SET STATISTICS 2000;
    CREATE STATISTICS s_t_multi (dependencies) ON c1, c2 FROM t;  -- 相关性
    ANALYZE t;
    ```

  - 索引未命中→谓词可索引化/表达式索引/类型一致：

    ```sql
    CREATE INDEX idx_t_lower_email ON t (lower(email));
    -- 确保查询用 lower(email) = ...
    ```

  - 锁等待→定位持锁长事务与冲突语句，必要时拆分DDL/批量：

    ```sql
    SELECT * FROM pg_locks l JOIN pg_stat_activity a USING(pid) WHERE NOT granted;
    ```

  - 膨胀→评估 `autovacuum`、重建索引/表：

    ```sql
    REINDEX TABLE CONCURRENTLY t;
    VACUUM (VERBOSE, ANALYZE) t;
    ```

## 6. 工具与方法

- EXPLAIN(ANALYZE, BUFFERS)、pg_stat_statements、采样/剖析
- 必备扩展与设置：
  - `pg_stat_statements`：

    ```sql
    CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
    SELECT query, calls, mean_time, rows
    FROM pg_stat_statements ORDER BY total_time DESC LIMIT 20;
    ```

  - `auto_explain`（开发/预生产启用，生产慎用高阈值）：

    ```text
    shared_preload_libraries = 'pg_stat_statements,auto_explain'
    auto_explain.log_min_duration = '200ms'
    auto_explain.log_analyze = on
    auto_explain.log_buffers = on
    auto_explain.log_nested_statements = on
    ```

  - 采样与等待分析：`pg_wait_sampling`、`pg_stat_kcache`、eBPF/bcc 工具（如 `profile`, `offcputime`）。

## 7. 参数调优指南（按子系统）

- 连接与并发：
- 应用优先连接池：`pgbouncer`（transaction 模式）→ 降低 `max_connections` 压力。
- `max_connections`：基于内存预算与工作负载并发度；过高会拉低命中率与上下文切换。
- `work_mem`：按并发排序/哈希操作估算；常见基线 4–64MB，避免全局放大导致内存爆。
- 缓冲与缓存：
- `shared_buffers`：常见 25% 物理内存起步（Linux+PageCache 情况），超大未必收益更高。
- `effective_cache_size`：估计 OS PageCache + shared_buffers（如内存 64GB → 48GB）。
- 并行与JIT：
- `max_parallel_workers_per_gather`：2–4 起步；`max_parallel_workers`≥总并行。
- `jit`：OLTP 多为关闭或阈值较高；OLAP 大查询收益明显。
- I/O 与存储：
- `random_page_cost`：SSD 通常 1.1–1.5；`effective_io_concurrency`：NVMe 可 256。
- 检查点：`checkpoint_timeout` 10–30min、`max_wal_size` 2–20GB、`checkpoint_completion_target` 0.7–0.9。
- WAL 与持久化：
- `wal_compression`=on（写放大下降，CPU ↑）；`synchronous_commit` 根据一致性需求调节（异步批量写可 off）。
- 维护与自清理：
- `autovacuum_vacuum_scale_factor` 降低以抑制膨胀（如 0.1→0.02 针对大表），结合 `autovacuum_vacuum_threshold`。
- `maintenance_work_mem`：索引重建/并行维护时提高（512MB–4GB）。

### 7.1 连接与并发参数

```sql
-- 1. max_connections
-- 计算：考虑应用连接数 + 管理连接 + 复制连接
-- 建议：100-300（使用连接池时）
-- 内存影响：每个连接约消耗 10MB（work_mem等）

-- 查看当前连接数
SELECT COUNT(*) FROM pg_stat_activity;

-- 查看连接数使用率
SELECT
    COUNT(*) as current_connections,
    current_setting('max_connections')::int as max_connections,
    ROUND(100.0 * COUNT(*) / current_setting('max_connections')::int, 2) as usage_percent
FROM pg_stat_activity;

-- 配置建议
ALTER SYSTEM SET max_connections = 200;
-- 如果使用pgbouncer，可以降低到100-150

-- 2. work_mem
-- 计算：work_mem * max_connections < 总内存的25%
-- 建议：4-64MB，根据查询复杂度调整

-- 查看work_mem使用情况
SELECT
    pid,
    usename,
    query,
    work_mem_used,
    work_mem_max
FROM pg_stat_activity
WHERE work_mem_used > 0
ORDER BY work_mem_used DESC;

-- 配置建议（根据并发查询数）
-- 10个并发：work_mem = 64MB
-- 50个并发：work_mem = 16MB
-- 100个并发：work_mem = 8MB
ALTER SYSTEM SET work_mem = '16MB';

-- 3. superuser_reserved_connections
-- 为超级用户保留的连接数
ALTER SYSTEM SET superuser_reserved_connections = 3;
```

### 7.2 缓冲与缓存参数

```sql
-- 1. shared_buffers
-- 建议：系统内存的25%（Linux），Windows可以更高
-- 64GB内存：16GB shared_buffers

-- 查看当前配置
SHOW shared_buffers;

-- 查看缓冲命中率
SELECT
    datname,
    ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) as hit_ratio
FROM pg_stat_database
WHERE datname = current_database();

-- 配置建议
ALTER SYSTEM SET shared_buffers = '16GB';  -- 64GB系统

-- 2. effective_cache_size
-- 建议：系统内存的50-75%
-- 64GB内存：32-48GB

ALTER SYSTEM SET effective_cache_size = '32GB';

-- 3. 验证配置
SELECT
    name,
    setting,
    unit,
    source
FROM pg_settings
WHERE name IN ('shared_buffers', 'effective_cache_size', 'work_mem');
```

### 7.3 并行与JIT参数

```sql
-- 1. 并行参数
-- max_parallel_workers_per_gather: 每个查询的最大并行工作进程数
-- max_parallel_workers: 系统最大并行工作进程总数
-- max_worker_processes: 系统最大工作进程数

-- 查看并行使用情况
SELECT
    pid,
    usename,
    query,
    parallel_workers
FROM pg_stat_activity
WHERE parallel_workers > 0;

-- 配置建议
ALTER SYSTEM SET max_parallel_workers_per_gather = 4;
ALTER SYSTEM SET max_parallel_workers = 8;
ALTER SYSTEM SET max_worker_processes = 16;

-- 2. JIT参数
-- OLTP: 关闭或设置高阈值
-- OLAP: 启用，阈值较低

-- 查看JIT使用情况
SELECT
    jit_functions,
    jit_generation_time,
    jit_inlining_count,
    jit_optimization_count
FROM pg_stat_statements
WHERE jit_functions > 0
LIMIT 10;

-- OLTP配置（关闭JIT）
ALTER SYSTEM SET jit = off;

-- OLAP配置（启用JIT）
ALTER SYSTEM SET jit = on;
ALTER SYSTEM SET jit_above_cost = 100000;
ALTER SYSTEM SET jit_optimize_above_cost = 500000;
ALTER SYSTEM SET jit_inline_above_cost = 500000;
```

### 7.4 I/O与存储参数

```sql
-- 1. random_page_cost
-- SSD: 1.1-1.5
-- HDD: 4.0（默认）
-- NVMe: 1.0-1.1

-- 查看I/O统计
SELECT
    datname,
    blks_read,
    blks_hit,
    ROUND(100.0 * blks_hit / NULLIF(blks_hit + blks_read, 0), 2) as hit_ratio
FROM pg_stat_database
WHERE datname = current_database();

-- SSD配置
ALTER SYSTEM SET random_page_cost = 1.1;

-- 2. effective_io_concurrency
-- SSD: 200
-- NVMe: 256
-- HDD: 2

ALTER SYSTEM SET effective_io_concurrency = 200;

-- 3. 检查点参数
-- checkpoint_timeout: 检查点间隔（10-30分钟）
-- max_wal_size: 最大WAL大小（2-20GB）
-- checkpoint_completion_target: 检查点完成目标（0.7-0.9）

-- 查看检查点统计
SELECT
    checkpoints_timed,
    checkpoints_req,
    checkpoint_write_time,
    checkpoint_sync_time
FROM pg_stat_bgwriter;

-- 配置建议
ALTER SYSTEM SET checkpoint_timeout = '15min';
ALTER SYSTEM SET max_wal_size = '4GB';
ALTER SYSTEM SET checkpoint_completion_target = 0.9;
```

### 7.5 WAL与持久化参数

```sql
-- 1. wal_compression
-- 启用压缩可以减少WAL大小，但增加CPU使用
ALTER SYSTEM SET wal_compression = on;

-- 2. synchronous_commit
-- local: 本地提交即可（最快，可能丢数据）
-- remote_write: 等待写入从库OS缓存（推荐）
-- remote_apply: 等待从库应用（最严格，最慢）
-- off: 等同于local（已废弃）

-- 查看当前配置
SHOW synchronous_commit;

-- 配置建议（根据RPO需求）
ALTER SYSTEM SET synchronous_commit = 'remote_write';

-- 3. wal_buffers
-- 建议：16MB（PostgreSQL 9.1+自动调整）
ALTER SYSTEM SET wal_buffers = '16MB';

-- 4. min_wal_size / max_wal_size
ALTER SYSTEM SET min_wal_size = '1GB';
ALTER SYSTEM SET max_wal_size = '4GB';
```

### 7.6 维护与自清理参数

```sql
-- 1. autovacuum参数
-- autovacuum_vacuum_scale_factor: 触发VACUUM的死元组比例
-- autovacuum_analyze_scale_factor: 触发ANALYZE的变更比例
-- autovacuum_vacuum_threshold: 触发VACUUM的最小死元组数

-- 查看autovacuum活动
SELECT
    schemaname,
    tablename,
    last_vacuum,
    last_autovacuum,
    last_analyze,
    last_autoanalyze,
    n_dead_tup,
    n_live_tup
FROM pg_stat_user_tables
WHERE n_dead_tup > 10000
ORDER BY n_dead_tup DESC
LIMIT 20;

-- 全局配置
ALTER SYSTEM SET autovacuum_vacuum_scale_factor = 0.1;
ALTER SYSTEM SET autovacuum_analyze_scale_factor = 0.05;
ALTER SYSTEM SET autovacuum_vacuum_threshold = 50;

-- 表级配置（大表）
ALTER TABLE large_table SET (
    autovacuum_vacuum_scale_factor = 0.02,
    autovacuum_analyze_scale_factor = 0.01
);

-- 2. maintenance_work_mem
-- 用于VACUUM、CREATE INDEX等维护操作
-- 建议：512MB-4GB

ALTER SYSTEM SET maintenance_work_mem = '1GB';

-- 3. autovacuum_max_workers
-- 同时运行的autovacuum工作进程数
ALTER SYSTEM SET autovacuum_max_workers = 3;
```

## 8. 查询层优化策略

- 代价对齐：校准 `default_statistics_target`（100–500）与扩展统计，降低错误选择率。
- 访问路径：覆盖索引、表达式/部分索引、BitmapScan 聚合回表成本。
- 连接策略：小表驱动、合适的连接顺序、避免隐式类型转换导致 Seq Scan。

### 8.1 代价对齐优化

```sql
-- 1. 调整统计目标
-- default_statistics_target: 默认100，可以提高到200-500

-- 查看当前统计目标
SHOW default_statistics_target;

-- 全局配置
ALTER SYSTEM SET default_statistics_target = 200;

-- 列级配置（重要列）
ALTER TABLE users ALTER COLUMN email SET STATISTICS 500;
ALTER TABLE orders ALTER COLUMN user_id SET STATISTICS 300;

-- 更新统计信息
ANALYZE users;
ANALYZE orders;

-- 2. 创建扩展统计
-- 相关性统计
CREATE STATISTICS users_email_created_stats (dependencies)
ON email, created_at FROM users;

-- 多列统计
CREATE STATISTICS orders_user_date_stats (dependencies, ndistinct)
ON user_id, order_date FROM orders;

-- 更新统计信息
ANALYZE users;
ANALYZE orders;

-- 3. 验证统计信息质量
SELECT
    schemaname,
    tablename,
    attname,
    n_distinct,
    correlation,
    most_common_vals,
    most_common_freqs
FROM pg_stats
WHERE schemaname = 'public'
AND tablename = 'users'
AND attname = 'email';
```

### 8.2 访问路径优化

```sql
-- 1. 覆盖索引（INCLUDE列）
-- 避免回表，提升查询性能
CREATE INDEX idx_users_email_covering ON users (email) INCLUDE (name, created_at);

-- 查询验证
EXPLAIN (ANALYZE, BUFFERS)
SELECT name, created_at FROM users WHERE email = 'user@example.com';
-- 应该显示Index Only Scan

-- 2. 表达式索引
-- 支持函数查询
CREATE INDEX idx_users_lower_email ON users (lower(email));

-- 查询必须使用相同的表达式
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM users WHERE lower(email) = 'user@example.com';
-- 应该使用索引

-- 3. 部分索引
-- 只索引部分数据，减少索引大小
CREATE INDEX idx_orders_active ON orders (user_id, order_date)
WHERE status = 'active';

-- 查询验证
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM orders WHERE user_id = 123 AND status = 'active';
-- 应该使用部分索引

-- 4. 复合索引优化
-- 考虑列的选择性和查询模式
CREATE INDEX idx_orders_user_date_status ON orders (user_id, order_date DESC, status);

-- 查询验证
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM orders
WHERE user_id = 123
AND order_date >= '2024-01-01'
AND status = 'active'
ORDER BY order_date DESC;
```

### 8.3 连接策略优化

```sql
-- 1. 小表驱动大表
-- 确保小表在JOIN的右侧（如果可能）

-- 错误示例：大表驱动小表
EXPLAIN (ANALYZE, BUFFERS)
SELECT u.name, o.total
FROM large_orders o
JOIN small_users u ON o.user_id = u.id;

-- 优化：调整JOIN顺序或使用子查询
EXPLAIN (ANALYZE, BUFFERS)
SELECT u.name, o.total
FROM small_users u
JOIN large_orders o ON u.id = o.user_id;

-- 2. 避免隐式类型转换
-- 确保JOIN键类型一致

-- 错误示例：类型不匹配
SELECT u.id, o.user_id
FROM users u
JOIN orders o ON u.id::text = o.user_id;  -- 类型转换导致Seq Scan

-- 正确示例：类型一致
SELECT u.id, o.user_id
FROM users u
JOIN orders o ON u.id = o.user_id::int;  -- 或修改表结构使类型一致

-- 3. 使用合适的连接算法
-- Hash Join: 适合大表JOIN
-- Nested Loop: 适合小表JOIN
-- Merge Join: 适合已排序的数据

-- 强制连接算法（仅用于测试）
SET enable_hashjoin = on;
SET enable_nestloop = on;
SET enable_mergejoin = on;

-- 4. 连接顺序优化
-- 使用JOIN_COLLAPSE_LIMIT控制连接顺序
SET join_collapse_limit = 8;  -- 默认8，可以调整
```

### 8.4 查询重写优化

```sql
-- 1. 避免SELECT *
-- 只选择需要的列
SELECT id, name, email FROM users WHERE id = 123;
-- 而不是
SELECT * FROM users WHERE id = 123;

-- 2. 使用LIMIT
-- 限制返回行数
SELECT * FROM orders WHERE user_id = 123 ORDER BY order_date DESC LIMIT 10;

-- 3. 使用EXISTS代替COUNT
-- EXISTS在找到第一条记录后立即返回
SELECT * FROM users u
WHERE EXISTS (SELECT 1 FROM orders o WHERE o.user_id = u.id);
-- 而不是
SELECT * FROM users u
WHERE (SELECT COUNT(*) FROM orders o WHERE o.user_id = u.id) > 0;

-- 4. 避免在WHERE子句中使用函数
-- 错误示例
SELECT * FROM users WHERE DATE(created_at) = '2024-01-01';

-- 正确示例
SELECT * FROM users
WHERE created_at >= '2024-01-01' AND created_at < '2024-01-02';

-- 5. 使用UNION ALL代替UNION（如果不需要去重）
SELECT id, name FROM users WHERE id < 100
UNION ALL
SELECT id, name FROM users WHERE id >= 100;
-- 而不是
SELECT id, name FROM users WHERE id < 100
UNION
SELECT id, name FROM users WHERE id >= 100;
```

- 分区与裁剪：时间/范围分区，确保谓词可裁剪；避免过多分区导致计划时间暴涨。
- 并行：确保运算符与函数可并行（immutable/parallel safe）。

## 9. 存储与表设计

- `fillfactor`：写多表降低（80–90）减少页分裂；读多保守提高空间利用。
- TOAST/压缩：9.5+ 自适应；PG15+ LZ4/ZSTD（需编译/设置）可降I/O。
- 冷热分层：热表热索引置高速盘，历史分区迁移冷盘；归档表使用压缩与只读策略。

## 10. 典型工作负载基线

- OLTP：
  - pgbouncer(transaction)、较小 `work_mem`（8–32MB）、`shared_buffers` 20–30%、JIT 关闭、`synchronous_commit=on`（重要交易）。
  - 强化 Autovacuum 与热点索引、低延迟检查点配置。
- OLAP：
  - 并行开启（2–4）、`work_mem` 64–512MB、JIT 打开（阈值 ≥200ms）、`random_page_cost` 低、`effective_io_concurrency` 高。
  - 宽表、列式外部引擎/FDW、分区裁剪与物化中间结果。
- 混合：分离读写池、OLAP 在只读副本跑、主库保守参数。

## 11. 验证与回归

- 基准数据：TPS/QPS、P95/P99、队列长度、命中率、WAL速率、检查点频率、Vacuum滞后、锁等待。
- 对比方法：调优前后 30–60 分钟稳定窗口；业务低峰灰度 5–10% 流量。
- 回滚触发：P95 恶化>10%、错误率升高、锁冲突上升、系统负载异常。

## 12. PostgreSQL 18 新特性应用

### 异步 I/O 优化

PostgreSQL 18 的异步 I/O 子系统可以显著提升I/O密集型操作的性能：

```sql
-- PostgreSQL 18: 异步 I/O 自动启用
-- 无需额外配置，系统自动优化I/O操作
-- 特别适用于：
-- - 大规模向量检索
-- - 批量数据导入
-- - 并行查询
```

**性能提升**:

- 向量索引构建速度提升 40%+
- 大规模查询I/O性能提升 2-3倍
- 更好的并发处理能力

### 虚拟生成列优化

PostgreSQL 18 的虚拟生成列可以优化计算密集型查询：

```sql
-- PostgreSQL 18: 使用虚拟生成列优化查询
CREATE TABLE products (
    id UUID PRIMARY KEY,
    price DECIMAL(10,2),
    discount_rate FLOAT,
    -- 虚拟生成列：自动计算最终价格
    final_price DECIMAL(10,2) GENERATED ALWAYS AS (
        price * (1 - discount_rate)
    ) STORED
);

-- 查询时直接使用，无需重复计算
SELECT id, final_price FROM products WHERE final_price > 100;
```

**优势**:

- 查询性能提升 15-25%
- 支持索引优化
- 减少重复计算

---

## 13. 索引优化深度分析

### 13.1 复合索引设计原则

**复合索引（Composite Index）**是在多个列上创建的索引，可以显著提升多列查询的性能。正确的复合索引设计是PostgreSQL性能优化的关键。

#### 13.1.1 复合索引基础

**复合索引结构**：

复合索引按照列的顺序组织数据，类似于电话簿的排序方式（先按姓氏，再按名字）。

```sql
-- 复合索引示例
CREATE INDEX idx_users_name_email ON users (last_name, first_name, email);

-- 索引结构（概念性）：
-- (Smith, John, john@example.com) -> row_id_1
-- (Smith, Jane, jane@example.com) -> row_id_2
-- (Doe, John, john.doe@example.com) -> row_id_3
```

**复合索引的优势**：

1. **多列查询优化**：可以同时优化多列条件查询
2. **覆盖索引**：如果索引包含所有查询列，可以避免回表
3. **排序优化**：可以优化ORDER BY多列排序

**复合索引的限制**：

1. **列顺序敏感**：列的顺序很重要，影响索引使用
2. **左前缀原则**：只能使用索引的最左前缀
3. **维护成本**：索引列越多，维护成本越高

#### 13.1.2 设计原则

**原则1：最左前缀匹配**:

PostgreSQL只能使用复合索引的最左前缀，因此列的顺序至关重要。

```sql
-- 索引：idx_users_name_email (last_name, first_name, email)

-- ✅ 可以使用索引的查询
SELECT * FROM users WHERE last_name = 'Smith';
SELECT * FROM users WHERE last_name = 'Smith' AND first_name = 'John';
SELECT * FROM users WHERE last_name = 'Smith' AND first_name = 'John' AND email = 'john@example.com';

-- ❌ 不能使用索引的查询（跳过了last_name）
SELECT * FROM users WHERE first_name = 'John';
SELECT * FROM users WHERE email = 'john@example.com';
SELECT * FROM users WHERE first_name = 'John' AND email = 'john@example.com';
```

**原则2：高选择性列在前**:

将选择性高的列（唯一值多的列）放在前面，可以提高索引效率。

```sql
-- 选择性分析
SELECT
    COUNT(DISTINCT last_name) AS distinct_last_names,
    COUNT(DISTINCT first_name) AS distinct_first_names,
    COUNT(DISTINCT email) AS distinct_emails,
    COUNT(*) AS total_rows
FROM users;

-- 结果示例：
-- distinct_last_names: 1000
-- distinct_first_names: 5000
-- distinct_emails: 10000
-- total_rows: 10000

-- 选择性：email > first_name > last_name
-- 因此索引顺序应该是：(email, first_name, last_name)
CREATE INDEX idx_users_email_name ON users (email, first_name, last_name);
```

**原则3：等值查询列在前，范围查询列在后**:

等值查询（=）的列应该放在范围查询（>, <, BETWEEN）的列之前。

```sql
-- 查询模式：WHERE status = 'active' AND created_at > '2024-01-01'
-- status是等值查询，created_at是范围查询
-- 索引顺序：status在前，created_at在后

CREATE INDEX idx_orders_status_date ON orders (status, created_at);

-- ✅ 可以使用索引
SELECT * FROM orders
WHERE status = 'active' AND created_at > '2024-01-01';

-- ❌ 如果顺序相反，索引效率降低
CREATE INDEX idx_orders_date_status ON orders (created_at, status);
-- 这个索引对上述查询效率较低
```

**原则4：覆盖索引设计**:

如果查询只需要索引列，可以设计覆盖索引避免回表。

```sql
-- 查询：SELECT user_id, status, created_at FROM orders WHERE status = 'active'
-- 设计覆盖索引
CREATE INDEX idx_orders_status_covering ON orders (status)
INCLUDE (user_id, created_at);

-- 或者使用包含所有列的复合索引
CREATE INDEX idx_orders_status_covering2 ON orders (status, user_id, created_at);

-- 查询执行计划（使用覆盖索引，避免回表）
EXPLAIN (ANALYZE, BUFFERS)
SELECT user_id, status, created_at
FROM orders
WHERE status = 'active';
-- 输出：Index Only Scan using idx_orders_status_covering
```

### 13.2 列顺序选择策略

#### 13.2.1 选择性评估

**选择性（Selectivity）**是索引列的唯一值比例，选择性越高，索引效率越高。

**选择性计算公式**：

```sql
-- 选择性 = DISTINCT值数量 / 总行数
-- 选择性范围：0到1，越接近1越好

-- 计算各列的选择性
SELECT
    'last_name' AS column_name,
    COUNT(DISTINCT last_name)::DECIMAL / COUNT(*) AS selectivity
FROM users
UNION ALL
SELECT
    'first_name' AS column_name,
    COUNT(DISTINCT first_name)::DECIMAL / COUNT(*) AS selectivity
FROM users
UNION ALL
SELECT
    'email' AS column_name,
    COUNT(DISTINCT email)::DECIMAL / COUNT(*) AS selectivity
FROM users;

-- 结果示例：
-- column_name | selectivity
-- ------------+------------
-- last_name   | 0.1000  (1000/10000)
-- first_name  | 0.5000  (5000/10000)
-- email       | 0.9999  (9999/10000)

-- 选择性排序：email > first_name > last_name
-- 索引顺序建议：(email, first_name, last_name)
```

**实际应用**：

```sql
-- 场景：用户表查询
-- 查询模式1：WHERE email = ? (最常见)
-- 查询模式2：WHERE email = ? AND first_name = ?
-- 查询模式3：WHERE email = ? AND first_name = ? AND last_name = ?

-- 选择性分析
SELECT
    COUNT(DISTINCT email) AS email_distinct,
    COUNT(DISTINCT first_name) AS first_name_distinct,
    COUNT(DISTINCT last_name) AS last_name_distinct,
    COUNT(*) AS total
FROM users;

-- 根据选择性和查询模式，设计索引
CREATE INDEX idx_users_email_name ON users (email, first_name, last_name);
```

#### 13.2.2 查询频率分析

**分析查询模式**：

使用`pg_stat_statements`分析实际查询模式，确定索引列顺序。

```sql
-- 启用pg_stat_statements
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- 分析查询模式
SELECT
    query,
    calls,
    mean_exec_time,
    (SELECT regexp_matches(query, 'WHERE\s+(\w+)\s*='))[1] AS first_condition,
    (SELECT regexp_matches(query, 'AND\s+(\w+)\s*='))[1] AS second_condition
FROM pg_stat_statements
WHERE query LIKE '%users%'
ORDER BY calls DESC
LIMIT 20;

-- 根据查询频率和条件顺序，设计索引
-- 如果大多数查询是 WHERE email = ? AND first_name = ?
-- 索引应该是：(email, first_name)
```

#### 13.2.3 实际案例

**案例1：订单表索引设计**：

```sql
-- 订单表结构
CREATE TABLE orders (
    id SERIAL PRIMARY KEY,
    user_id INTEGER,
    status VARCHAR(20),
    created_at TIMESTAMP,
    total_amount DECIMAL(10,2)
);

-- 查询模式分析
-- 1. WHERE user_id = ? AND status = ? (80%的查询)
-- 2. WHERE status = ? AND created_at > ? (15%的查询)
-- 3. WHERE user_id = ? (5%的查询)

-- 选择性分析
SELECT
    COUNT(DISTINCT user_id)::DECIMAL / COUNT(*) AS user_id_selectivity,
    COUNT(DISTINCT status)::DECIMAL / COUNT(*) AS status_selectivity,
    COUNT(*) AS total
FROM orders;

-- 结果：user_id选择性 > status选择性
-- 但查询模式1最频繁，且user_id在前

-- 设计索引
CREATE INDEX idx_orders_user_status ON orders (user_id, status);
CREATE INDEX idx_orders_status_date ON orders (status, created_at);

-- 验证索引使用
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM orders
WHERE user_id = 123 AND status = 'active';
-- 应该使用 idx_orders_user_status
```

**案例2：多条件查询优化**：

```sql
-- 产品表
CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    category_id INTEGER,
    brand_id INTEGER,
    price DECIMAL(10,2),
    stock INTEGER,
    created_at TIMESTAMP
);

-- 查询模式：
-- WHERE category_id = ? AND brand_id = ? AND price BETWEEN ? AND ?

-- 选择性分析
SELECT
    COUNT(DISTINCT category_id) AS cat_distinct,
    COUNT(DISTINCT brand_id) AS brand_distinct,
    COUNT(*) AS total
FROM products;
-- category_id: 100个不同值
-- brand_id: 50个不同值
-- 选择性：category_id > brand_id

-- 设计索引（等值查询在前，范围查询在后）
CREATE INDEX idx_products_cat_brand_price ON products (category_id, brand_id, price);

-- 验证
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM products
WHERE category_id = 1
  AND brand_id = 10
  AND price BETWEEN 100 AND 500;
```

### 13.3 选择性评估方法

#### 13.3.1 统计信息查询

**使用系统表查询选择性**：

```sql
-- 查询列的统计信息
SELECT
    schemaname,
    tablename,
    attname AS column_name,
    n_distinct AS distinct_values,
    correlation,
    most_common_vals,
    most_common_freqs
FROM pg_stats
WHERE schemaname = 'public'
  AND tablename = 'users'
  AND attname IN ('last_name', 'first_name', 'email');

-- n_distinct: 不同值的数量（负数表示比例）
-- correlation: 列值与物理存储顺序的相关性（-1到1）
-- most_common_vals: 最常见的值
-- most_common_freqs: 最常见值的频率
```

**计算实际选择性**：

```sql
-- 创建选择性分析函数
CREATE OR REPLACE FUNCTION analyze_selectivity(
    table_name TEXT,
    column_name TEXT
)
RETURNS TABLE (
    distinct_count BIGINT,
    total_count BIGINT,
    selectivity DECIMAL,
    recommendation TEXT
) AS $$
DECLARE
    distinct_val BIGINT;
    total_val BIGINT;
    sel DECIMAL;
BEGIN
    EXECUTE format('SELECT COUNT(DISTINCT %I), COUNT(*) FROM %I',
                   column_name, table_name)
    INTO distinct_val, total_val;

    sel := distinct_val::DECIMAL / NULLIF(total_val, 0);

    RETURN QUERY SELECT
        distinct_val,
        total_val,
        sel,
        CASE
            WHEN sel > 0.9 THEN 'Excellent - High selectivity'
            WHEN sel > 0.5 THEN 'Good - Medium-high selectivity'
            WHEN sel > 0.1 THEN 'Fair - Medium selectivity'
            ELSE 'Poor - Low selectivity, consider other columns first'
        END;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM analyze_selectivity('users', 'email');
SELECT * FROM analyze_selectivity('users', 'last_name');
```

#### 13.3.2 直方图分析

**查看直方图边界**：

```sql
-- 查看列的直方图边界（用于范围查询优化）
SELECT
    schemaname,
    tablename,
    attname,
    histogram_bounds
FROM pg_stats
WHERE schemaname = 'public'
  AND tablename = 'orders'
  AND attname = 'created_at';

-- histogram_bounds: 直方图的边界值数组
-- 用于估算范围查询的选择性
```

**选择性估算**：

```sql
-- 估算范围查询的选择性
CREATE OR REPLACE FUNCTION estimate_range_selectivity(
    table_name TEXT,
    column_name TEXT,
    min_val ANYELEMENT,
    max_val ANYELEMENT
)
RETURNS DECIMAL AS $$
DECLARE
    hist_bounds ANYARRAY;
    total_buckets INTEGER;
    matching_buckets INTEGER := 0;
    i INTEGER;
    bucket_min ANYELEMENT;
    bucket_max ANYELEMENT;
BEGIN
    -- 获取直方图边界
    SELECT histogram_bounds INTO hist_bounds
    FROM pg_stats
    WHERE schemaname = 'public'
      AND tablename = estimate_range_selectivity.table_name
      AND attname = estimate_range_selectivity.column_name;

    IF hist_bounds IS NULL THEN
        RETURN 0.1;  -- 默认估算
    END IF;

    total_buckets := array_length(hist_bounds, 1) - 1;

    -- 计算匹配的桶数
    FOR i IN 1..total_buckets LOOP
        bucket_min := hist_bounds[i];
        bucket_max := hist_bounds[i + 1];

        -- 检查桶是否与查询范围重叠
        IF bucket_max >= min_val AND bucket_min <= max_val THEN
            matching_buckets := matching_buckets + 1;
        END IF;
    END LOOP;

    -- 返回选择性估算
    RETURN matching_buckets::DECIMAL / total_buckets;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT estimate_range_selectivity('orders', 'created_at',
                                  '2024-01-01'::TIMESTAMP,
                                  '2024-12-31'::TIMESTAMP);
```

### 13.4 索引维护最佳实践

#### 13.4.1 索引重建策略

**何时重建索引**：

1. **索引膨胀**：索引占用空间过大
2. **性能下降**：查询性能明显下降
3. **大量更新**：表进行了大量UPDATE/DELETE操作

**检查索引膨胀**：

```sql
-- 检查索引膨胀
SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    idx_scan AS index_scans,
    idx_tup_read AS tuples_read,
    idx_tup_fetch AS tuples_fetched,
    -- 估算膨胀率（简化）
    CASE
        WHEN idx_scan = 0 THEN 'UNUSED'
        WHEN idx_tup_fetch::DECIMAL / NULLIF(idx_tup_read, 0) < 0.1 THEN 'BLOATED'
        ELSE 'OK'
    END AS status
FROM pg_stat_user_indexes
ORDER BY pg_relation_size(indexrelid) DESC;

-- 检查索引碎片（需要pgstattuple扩展）
CREATE EXTENSION IF NOT EXISTS pgstattuple;

SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    (pgstattuple(indexrelid::regclass)).dead_tuple_percent AS dead_tuple_percent
FROM pg_stat_user_indexes
WHERE pg_relation_size(indexrelid) > 100 * 1024 * 1024  -- 大于100MB
ORDER BY dead_tuple_percent DESC;
```

**重建索引方法**：

```sql
-- 方法1：REINDEX（PostgreSQL 12+，支持CONCURRENTLY）
REINDEX INDEX CONCURRENTLY idx_users_email_name;

-- 方法2：删除并重建（需要锁表）
DROP INDEX idx_users_email_name;
CREATE INDEX idx_users_email_name ON users (email, first_name, last_name);

-- 方法3：使用pg_repack（在线重建，无锁）
-- 需要安装pg_repack扩展
-- pg_repack -d mydb -t users --only-indexes
```

**自动重建索引脚本**：

```sql
-- 创建自动重建索引函数
CREATE OR REPLACE FUNCTION rebuild_bloated_indexes(
    min_size_mb INTEGER DEFAULT 100,
    max_dead_percent DECIMAL DEFAULT 20.0
)
RETURNS TABLE (
    index_name TEXT,
    index_size TEXT,
    dead_percent DECIMAL,
    rebuild_command TEXT
) AS $$
DECLARE
    rec RECORD;
BEGIN
    FOR rec IN
        SELECT
            schemaname || '.' || tablename || '.' || indexname AS full_index_name,
            pg_relation_size(indexrelid) AS size_bytes,
            (pgstattuple(indexrelid::regclass)).dead_tuple_percent AS dead_pct
        FROM pg_stat_user_indexes
        WHERE pg_relation_size(indexrelid) > min_size_mb * 1024 * 1024
          AND (pgstattuple(indexrelid::regclass)).dead_tuple_percent > max_dead_percent
    LOOP
        RETURN QUERY SELECT
            rec.full_index_name::TEXT,
            pg_size_pretty(rec.size_bytes)::TEXT,
            rec.dead_pct,
            format('REINDEX INDEX CONCURRENTLY %s;', rec.full_index_name)::TEXT;
    END LOOP;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM rebuild_bloated_indexes(100, 20.0);
```

#### 13.4.2 索引监控

**监控索引使用情况**：

```sql
-- 监控未使用的索引
SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    idx_scan AS index_scans,
    idx_tup_read AS tuples_read,
    idx_tup_fetch AS tuples_fetched
FROM pg_stat_user_indexes
WHERE idx_scan = 0  -- 从未使用
  AND pg_relation_size(indexrelid) > 10 * 1024 * 1024  -- 大于10MB
ORDER BY pg_relation_size(indexrelid) DESC;

-- 监控索引效率
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan AS scans,
    idx_tup_read AS tuples_read,
    idx_tup_fetch AS tuples_fetched,
    CASE
        WHEN idx_tup_read > 0 THEN
            ROUND((idx_tup_fetch::DECIMAL / idx_tup_read) * 100, 2)
        ELSE 0
    END AS efficiency_percent,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size
FROM pg_stat_user_indexes
WHERE idx_scan > 0
ORDER BY efficiency_percent ASC;  -- 效率低的在前
```

**索引维护计划**：

```sql
-- 创建索引维护视图
CREATE OR REPLACE VIEW index_maintenance_report AS
SELECT
    schemaname,
    tablename,
    indexname,
    pg_size_pretty(pg_relation_size(indexrelid)) AS index_size,
    idx_scan AS scans,
    CASE
        WHEN idx_scan = 0 THEN 'UNUSED - Consider dropping'
        WHEN idx_scan < 10 THEN 'RARELY USED - Review'
        WHEN idx_tup_fetch::DECIMAL / NULLIF(idx_tup_read, 0) < 0.1 THEN 'INEFFICIENT - Rebuild'
        ELSE 'OK'
    END AS recommendation
FROM pg_stat_user_indexes
ORDER BY pg_relation_size(indexrelid) DESC;

-- 查询维护报告
SELECT * FROM index_maintenance_report
WHERE recommendation != 'OK';
```

### 13.5 PostgreSQL 18索引优化

#### 13.5.1 B-tree去重优化

**PostgreSQL 18 B-tree去重**：

PostgreSQL 18引入了B-tree索引去重优化，可以显著减少索引大小和提高性能。

**去重原理**：

```sql
-- PostgreSQL 18自动去重重复的索引键
-- 对于重复值，只存储一次，提高索引效率

-- 示例：大量重复值的列
CREATE TABLE logs (
    id SERIAL PRIMARY KEY,
    level VARCHAR(10),  -- 只有几个不同值：INFO, WARNING, ERROR
    message TEXT,
    created_at TIMESTAMP
);

-- PostgreSQL 18自动优化重复键
CREATE INDEX idx_logs_level ON logs (level);
-- 索引大小显著减少（相比PostgreSQL 17）

-- 验证去重效果
SELECT
    pg_size_pretty(pg_relation_size('logs')) AS table_size,
    pg_size_pretty(pg_relation_size('idx_logs_level')) AS index_size;
```

**性能提升**：

```sql
-- 性能对比测试
-- PostgreSQL 17: 索引大小 500MB
-- PostgreSQL 18: 索引大小 50MB（去重后）
-- 查询性能提升：15-20%

-- 测试查询
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM logs
WHERE level = 'ERROR'
  AND created_at > NOW() - INTERVAL '1 day';
```

#### 13.5.2 BRIN性能提升

**BRIN索引优化**：

PostgreSQL 18对BRIN（Block Range Index）索引进行了性能优化。

**BRIN索引适用场景**：

```sql
-- BRIN索引适用于：
-- 1. 大表（GB级别）
-- 2. 数据按物理顺序存储（如时间序列）
-- 3. 范围查询为主

-- 创建BRIN索引
CREATE TABLE sensor_data (
    id BIGSERIAL,
    sensor_id INTEGER,
    timestamp TIMESTAMP,
    value DECIMAL(10,2)
) WITH (fillfactor = 100);

-- 按时间顺序插入数据
INSERT INTO sensor_data (sensor_id, timestamp, value)
SELECT
    (random() * 100)::INTEGER,
    NOW() - (random() * INTERVAL '365 days'),
    random() * 100
FROM generate_series(1, 10000000);

-- 创建BRIN索引（PostgreSQL 18优化）
CREATE INDEX idx_sensor_data_timestamp_brin
ON sensor_data USING BRIN (timestamp);

-- 性能提升：15-20%（相比PostgreSQL 17）
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM sensor_data
WHERE timestamp BETWEEN '2024-01-01' AND '2024-12-31';
```

#### 13.5.3 并行索引构建

**PostgreSQL 18并行索引构建**：

```sql
-- PostgreSQL 18支持更多索引类型的并行构建
-- 包括GIN索引的并行构建

-- 并行构建GIN索引
CREATE INDEX CONCURRENTLY idx_products_tags_gin
ON products USING GIN (tags);

-- 配置并行工作进程
SET max_parallel_maintenance_workers = 8;

-- 性能提升：大型GIN索引构建速度提升2-3倍
```

**最佳实践**：

```sql
-- 1. 使用CONCURRENTLY避免锁表
CREATE INDEX CONCURRENTLY idx_users_email ON users (email);

-- 2. 配置并行工作进程
SET max_parallel_maintenance_workers = 8;

-- 3. 监控索引构建进度
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    query,
    query_start,
    now() - query_start AS duration
FROM pg_stat_activity
WHERE query LIKE '%CREATE INDEX%';
```

---

## 14. 统计信息维护最佳实践

### 14.1 ANALYZE命令深度使用

**ANALYZE**是PostgreSQL收集表统计信息的关键命令，用于帮助查询规划器选择最优执行计划。

#### 14.1.1 ANALYZE基础

**ANALYZE的作用**：

1. **收集统计信息**：收集表的行数、列的唯一值数量、数据分布等
2. **更新系统表**：更新`pg_statistic`和`pg_class`系统表
3. **优化查询计划**：帮助查询规划器选择最优执行计划

**基本用法**：

```sql
-- 分析整个数据库
ANALYZE;

-- 分析特定表
ANALYZE users;

-- 分析特定列
ANALYZE users (email, created_at);

-- 分析特定表空间的所有表
ANALYZE VERBOSE;
```

#### 14.1.2 ANALYZE参数详解

**VERBOSE参数**：

```sql
-- 显示详细的统计信息收集过程
ANALYZE VERBOSE users;

-- 输出示例：
-- INFO:  analyzing "public.users"
-- INFO:  "users": scanned 10000 of 10000 pages, containing 100000 live rows and 0 dead rows; 10000 rows in sample, 100000 estimated total rows
```

**SKIP_LOCKED参数**（PostgreSQL 12+）：

```sql
-- 跳过被锁定的表，避免阻塞
ANALYZE SKIP_LOCKED users;

-- 适用于：
-- 1. 生产环境自动ANALYZE
-- 2. 避免与长时间运行的事务冲突
```

**采样控制**：

```sql
-- 设置统计信息目标（影响采样精度）
ALTER TABLE users ALTER COLUMN email SET STATISTICS 1000;

-- 默认值：default_statistics_target = 100
-- 范围：0-10000
-- 值越大，采样越多，统计信息越准确，但ANALYZE时间越长

-- 查看当前统计信息目标
SELECT
    tablename,
    attname,
    n_distinct,
    correlation,
    most_common_vals
FROM pg_stats
WHERE tablename = 'users'
  AND attname = 'email';
```

#### 14.1.3 统计信息内容

**pg_stats系统视图**：

```sql
-- 查看表的统计信息
SELECT
    schemaname,
    tablename,
    attname AS column_name,
    n_distinct AS distinct_values,  -- 不同值数量（负数表示比例）
    correlation,  -- 列值与物理存储顺序的相关性
    most_common_vals,  -- 最常见的值
    most_common_freqs,  -- 最常见值的频率
    histogram_bounds,  -- 直方图边界（用于范围查询估算）
    null_frac  -- NULL值比例
FROM pg_stats
WHERE schemaname = 'public'
  AND tablename = 'users'
ORDER BY attname;
```

**统计信息解读**：

```sql
-- 示例统计信息解读
SELECT
    attname,
    n_distinct,
    CASE
        WHEN n_distinct < 0 THEN
            '比例: ' || ABS(n_distinct)::TEXT || '%'
        ELSE
            '绝对数量: ' || n_distinct::TEXT
    END AS distinct_interpretation,
    correlation,
    CASE
        WHEN ABS(correlation) > 0.9 THEN 'Highly correlated with physical order'
        WHEN ABS(correlation) > 0.5 THEN 'Moderately correlated'
        ELSE 'Not correlated'
    END AS correlation_interpretation
FROM pg_stats
WHERE tablename = 'users';
```

### 14.2 统计信息更新策略

#### 14.2.1 自动ANALYZE配置

**autovacuum配置**：

```conf
# postgresql.conf
# 启用自动ANALYZE
autovacuum = on
autovacuum_analyze = on

# 自动ANALYZE触发条件
autovacuum_analyze_threshold = 50  # 表变更行数阈值
autovacuum_analyze_scale_factor = 0.1  # 表大小的比例因子

# 自动ANALYZE触发公式：
# 变更行数 > autovacuum_analyze_threshold + autovacuum_analyze_scale_factor * 表大小

# 自动ANALYZE延迟
autovacuum_analyze_delay = 10ms  # 每次ANALYZE后的延迟
```

**表级自动ANALYZE配置**：

```sql
-- 为特定表配置自动ANALYZE参数
ALTER TABLE large_table SET (
    autovacuum_analyze_threshold = 1000,
    autovacuum_analyze_scale_factor = 0.05
);

-- 禁用特定表的自动ANALYZE（手动管理）
ALTER TABLE static_table SET (autovacuum_analyze_enabled = false);
```

#### 14.2.2 手动ANALYZE策略

**更新频率策略**：

```sql
-- 策略1：高频更新表（每天ANALYZE）
-- 适用于：订单表、日志表等
CREATE OR REPLACE FUNCTION analyze_frequent_tables()
RETURNS void AS $$
BEGIN
    ANALYZE orders;
    ANALYZE order_items;
    ANALYZE logs;
END;
$$ LANGUAGE plpgsql;

-- 使用cron定期执行
-- 0 2 * * * psql -d mydb -c "SELECT analyze_frequent_tables();"

-- 策略2：中频更新表（每周ANALYZE）
CREATE OR REPLACE FUNCTION analyze_medium_tables()
RETURNS void AS $$
BEGIN
    ANALYZE users;
    ANALYZE products;
    ANALYZE categories;
END;
$$ LANGUAGE plpgsql;

-- 策略3：低频更新表（每月ANALYZE）
CREATE OR REPLACE FUNCTION analyze_rare_tables()
RETURNS void AS $$
BEGIN
    ANALYZE configurations;
    ANALYZE settings;
END;
$$ LANGUAGE plpgsql;
```

**智能ANALYZE脚本**：

```sql
-- 创建智能ANALYZE函数（只分析需要更新的表）
CREATE OR REPLACE FUNCTION smart_analyze(
    min_changes INTEGER DEFAULT 1000
)
RETURNS TABLE (
    table_name TEXT,
    changes_since_analyze BIGINT,
    last_analyze TIMESTAMP,
    analyze_command TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        schemaname || '.' || tablename::TEXT,
        n_mod_since_analyze,
        last_analyze,
        format('ANALYZE %I.%I;', schemaname, tablename)::TEXT
    FROM pg_stat_user_tables
    WHERE n_mod_since_analyze > min_changes
      AND (
          last_analyze IS NULL
          OR last_analyze < NOW() - INTERVAL '1 day'
      )
    ORDER BY n_mod_since_analyze DESC;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM smart_analyze(1000);
-- 返回需要ANALYZE的表列表
```

#### 14.2.3 统计信息监控

**监控统计信息过期情况**：

```sql
-- 创建统计信息监控视图
CREATE OR REPLACE VIEW statistics_monitoring AS
SELECT
    schemaname,
    tablename,
    n_live_tup AS live_rows,
    n_dead_tup AS dead_rows,
    n_mod_since_analyze AS changes_since_analyze,
    last_analyze,
    CASE
        WHEN last_analyze IS NULL THEN 'NEVER ANALYZED'
        WHEN last_analyze < NOW() - INTERVAL '7 days' THEN 'STALE (>7 days)'
        WHEN last_analyze < NOW() - INTERVAL '1 day' THEN 'STALE (>1 day)'
        ELSE 'FRESH'
    END AS analyze_status,
    CASE
        WHEN n_mod_since_analyze > n_live_tup * 0.1 THEN 'HIGH CHANGES'
        WHEN n_mod_since_analyze > n_live_tup * 0.05 THEN 'MEDIUM CHANGES'
        ELSE 'LOW CHANGES'
    END AS change_level
FROM pg_stat_user_tables
ORDER BY n_mod_since_analyze DESC;

-- 查询需要更新的表
SELECT * FROM statistics_monitoring
WHERE analyze_status IN ('NEVER ANALYZED', 'STALE (>7 days)')
   OR change_level = 'HIGH CHANGES';
```

**统计信息质量检查**：

```sql
-- 检查统计信息质量
CREATE OR REPLACE FUNCTION check_statistics_quality(
    table_name TEXT
)
RETURNS TABLE (
    column_name TEXT,
    n_distinct BIGINT,
    null_frac DECIMAL,
    correlation DECIMAL,
    quality_rating TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        attname::TEXT,
        n_distinct,
        null_frac,
        correlation,
        CASE
            WHEN n_distinct IS NULL THEN 'NO STATISTICS'
            WHEN null_frac > 0.5 THEN 'HIGH NULL RATIO'
            WHEN ABS(correlation) < 0.1 THEN 'LOW CORRELATION'
            WHEN n_distinct < 10 THEN 'LOW CARDINALITY'
            ELSE 'GOOD'
        END::TEXT
    FROM pg_stats
    WHERE schemaname = 'public'
      AND tablename = check_statistics_quality.table_name
    ORDER BY attname;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM check_statistics_quality('users');
```

### 14.3 查询规划器优化

#### 14.3.1 统计信息对规划器的影响

**基数估算**：

```sql
-- 统计信息影响查询规划器的基数估算
-- 基数估算不准确会导致：
-- 1. 选择错误的连接顺序
-- 2. 选择错误的连接算法
-- 3. 选择错误的索引

-- 示例：统计信息不准确的影响
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM users u
JOIN orders o ON u.id = o.user_id
WHERE u.email = 'test@example.com'
  AND o.status = 'active';

-- 如果统计信息不准确：
-- - 可能高估或低估连接结果集大小
-- - 导致选择Nested Loop而不是Hash Join
-- - 导致选择错误的索引
```

**改进统计信息精度**：

```sql
-- 提高统计信息目标（更精确的估算）
ALTER TABLE users ALTER COLUMN email SET STATISTICS 500;
ALTER TABLE orders ALTER COLUMN status SET STATISTICS 500;

-- 重新分析
ANALYZE users;
ANALYZE orders;

-- 验证改进效果
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM users u
JOIN orders o ON u.id = o.user_id
WHERE u.email = 'test@example.com'
  AND o.status = 'active';
```

#### 14.3.2 扩展统计信息

**多变量统计信息**（PostgreSQL 10+）：

```sql
-- 创建多变量统计信息（捕获列之间的相关性）
CREATE STATISTICS user_order_correlation
ON (user_id, status, created_at)
FROM orders;

-- 分析表
ANALYZE orders;

-- 查看多变量统计信息
SELECT
    stxname,
    stxkeys,
    stxkind
FROM pg_statistic_ext
WHERE stxrelid = 'orders'::regclass;

-- 多变量统计信息帮助规划器：
-- 1. 更准确地估算多列条件的选择性
-- 2. 识别列之间的相关性
-- 3. 优化连接顺序
```

**表达式统计信息**：

```sql
-- 为表达式创建统计信息
CREATE STATISTICS order_total_stats
ON (total_amount, (total_amount * 0.1))
FROM orders;

-- 分析表
ANALYZE orders;

-- 表达式统计信息帮助优化器：
-- 1. 估算表达式条件的选择性
-- 2. 优化包含表达式的查询
```

#### 14.3.3 查询计划验证

**对比ANALYZE前后的查询计划**：

```sql
-- 步骤1：记录ANALYZE前的查询计划
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
SELECT * FROM users WHERE email = 'test@example.com';
-- 记录：执行时间、行数估算、实际行数

-- 步骤2：更新统计信息
ANALYZE users;

-- 步骤3：记录ANALYZE后的查询计划
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
SELECT * FROM users WHERE email = 'test@example.com';
-- 对比：执行时间、行数估算准确性

-- 步骤4：分析改进
-- 如果估算行数更接近实际行数，说明统计信息改进有效
```

### 14.4 PostgreSQL 18统计信息新特性

#### 14.4.1 虚拟生成列统计

**PostgreSQL 18虚拟生成列统计**：

PostgreSQL 18支持在虚拟生成列上收集统计信息，提高查询优化效果。

```sql
-- 创建带虚拟生成列的表
CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    name VARCHAR(100),
    price DECIMAL(10,2),
    discount DECIMAL(5,2),
    final_price DECIMAL(10,2) GENERATED ALWAYS AS (price * (1 - discount/100)) STORED
);

-- PostgreSQL 18自动为虚拟生成列收集统计信息
ANALYZE products;

-- 查看虚拟生成列的统计信息
SELECT
    attname,
    n_distinct,
    correlation,
    most_common_vals
FROM pg_stats
WHERE tablename = 'products'
  AND attname = 'final_price';

-- 查询优化器可以使用虚拟生成列的统计信息
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM products
WHERE final_price BETWEEN 50 AND 100;
```

#### 14.4.2 改进的多变量统计

**更精确的多变量统计**：

```sql
-- PostgreSQL 18改进了多变量统计的准确性
CREATE STATISTICS user_order_mv_stats
ON (user_id, status, created_at, total_amount)
FROM orders;

-- 分析表（PostgreSQL 18使用更智能的采样策略）
ANALYZE orders;

-- 多变量统计信息更准确，查询计划更优
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM orders
WHERE user_id = 123
  AND status = 'active'
  AND created_at > '2024-01-01'
  AND total_amount > 100;
```

#### 14.4.3 更精细的直方图

**改进的直方图**：

```sql
-- PostgreSQL 18使用更精细的直方图
-- 提高范围查询的选择性估算准确性

-- 设置更高的统计信息目标
ALTER TABLE orders ALTER COLUMN created_at SET STATISTICS 500;

-- 分析表
ANALYZE orders;

-- 查看直方图边界（更精细）
SELECT
    attname,
    array_length(histogram_bounds, 1) AS bucket_count,
    histogram_bounds
FROM pg_stats
WHERE tablename = 'orders'
  AND attname = 'created_at';

-- 范围查询的选择性估算更准确
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM orders
WHERE created_at BETWEEN '2024-01-01' AND '2024-12-31';
```

#### 14.4.4 自适应统计信息更新

**自适应更新策略**：

```sql
-- PostgreSQL 18根据表的变化率自动调整ANALYZE频率
-- 高频变化的表：更频繁的ANALYZE
-- 低频变化的表：减少ANALYZE频率

-- 查看表的ANALYZE历史
SELECT
    schemaname,
    tablename,
    last_analyze,
    n_mod_since_analyze,
    n_live_tup,
    CASE
        WHEN n_mod_since_analyze::DECIMAL / NULLIF(n_live_tup, 0) > 0.1 THEN 'HIGH CHANGE RATE'
        WHEN n_mod_since_analyze::DECIMAL / NULLIF(n_live_tup, 0) > 0.05 THEN 'MEDIUM CHANGE RATE'
        ELSE 'LOW CHANGE RATE'
    END AS change_rate_category
FROM pg_stat_user_tables
ORDER BY n_mod_since_analyze DESC;
```

**最佳实践总结**：

```sql
-- 1. 定期监控统计信息质量
SELECT * FROM statistics_monitoring
WHERE analyze_status != 'FRESH';

-- 2. 为关键列设置更高的统计信息目标
ALTER TABLE orders ALTER COLUMN user_id SET STATISTICS 500;
ALTER TABLE orders ALTER COLUMN status SET STATISTICS 500;

-- 3. 创建多变量统计信息（捕获列相关性）
CREATE STATISTICS order_correlation
ON (user_id, status, created_at)
FROM orders;

-- 4. 定期执行ANALYZE（根据表的变化率）
-- 高频表：每天
-- 中频表：每周
-- 低频表：每月

-- 5. 监控查询计划的准确性
-- 对比估算行数和实际行数
-- 如果差异大，需要更新统计信息
```

---

## 15. 请求性能跟踪

### 15.1 pg_stat_activity视图深度使用

**pg_stat_activity**是PostgreSQL最重要的性能监控视图，提供当前所有数据库连接的实时信息。

#### 15.1.1 视图字段详解

**核心字段说明**：

```sql
-- 查看pg_stat_activity的所有字段
SELECT column_name, data_type, description
FROM information_schema.columns
WHERE table_name = 'pg_stat_activity'
ORDER BY ordinal_position;

-- 关键字段说明：
-- - pid: 进程ID
-- - datname: 数据库名
-- - usename: 用户名
-- - application_name: 应用名称
-- - client_addr: 客户端IP地址
-- - state: 连接状态（active, idle, idle in transaction等）
-- - query: 当前执行的查询
-- - query_start: 查询开始时间
-- - state_change: 状态最后变更时间
-- - wait_event_type: 等待事件类型
-- - wait_event: 等待事件名称
-- - backend_type: 后端类型（client, autovacuum, walsender等）
```

**基本查询**：

```sql
-- 查看所有活动连接
SELECT
    pid,
    datname,
    usename,
    application_name,
    client_addr,
    state,
    query_start,
    state_change,
    wait_event_type,
    wait_event,
    query
FROM pg_stat_activity
WHERE datname IS NOT NULL
ORDER BY query_start DESC;
```

#### 15.1.2 连接状态分析

**状态类型**：

```sql
-- 分析连接状态分布
SELECT
    state,
    COUNT(*) AS connection_count,
    COUNT(*) FILTER (WHERE state = 'active') AS active_count,
    COUNT(*) FILTER (WHERE state = 'idle') AS idle_count,
    COUNT(*) FILTER (WHERE state = 'idle in transaction') AS idle_in_transaction_count,
    COUNT(*) FILTER (WHERE state = 'idle in transaction (aborted)') AS idle_in_transaction_aborted_count
FROM pg_stat_activity
WHERE datname IS NOT NULL
GROUP BY state;

-- 状态说明：
-- - active: 正在执行查询
-- - idle: 空闲，等待客户端命令
-- - idle in transaction: 在事务中空闲（可能持有锁）
-- - idle in transaction (aborted): 在事务中空闲，但事务已中止
-- - fastpath function call: 快速路径函数调用
-- - disabled: 统计信息收集被禁用
```

**长时间运行的事务**：

```sql
-- 查找长时间运行的事务（可能导致锁等待）
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    xact_start,
    query_start,
    NOW() - xact_start AS transaction_duration,
    NOW() - query_start AS query_duration,
    query
FROM pg_stat_activity
WHERE state = 'idle in transaction'
  AND NOW() - xact_start > INTERVAL '5 minutes'
ORDER BY xact_start;
```

#### 15.1.3 等待事件分析

**等待事件类型**：

```sql
-- 分析等待事件
SELECT
    wait_event_type,
    wait_event,
    COUNT(*) AS wait_count,
    COUNT(*) FILTER (WHERE state = 'active') AS active_wait_count
FROM pg_stat_activity
WHERE wait_event_type IS NOT NULL
GROUP BY wait_event_type, wait_event
ORDER BY wait_count DESC;

-- 常见等待事件类型：
-- - Lock: 锁等待
-- - IO: I/O等待
-- - LWLock: 轻量级锁等待
-- - BufferPin: 缓冲区固定等待
-- - Activity: 后端活动等待
```

**锁等待分析**：

```sql
-- 查找锁等待的连接
SELECT
    blocked.pid AS blocked_pid,
    blocked.usename AS blocked_user,
    blocked.query AS blocked_query,
    blocking.pid AS blocking_pid,
    blocking.usename AS blocking_user,
    blocking.query AS blocking_query,
    blocked.wait_event_type,
    blocked.wait_event
FROM pg_stat_activity blocked
JOIN pg_locks blocked_locks ON blocked_locks.pid = blocked.pid
JOIN pg_locks blocking_locks ON blocking_locks.locktype = blocked_locks.locktype
    AND blocking_locks.database IS NOT DISTINCT FROM blocked_locks.database
    AND blocking_locks.relation IS NOT DISTINCT FROM blocked_locks.relation
    AND blocking_locks.page IS NOT DISTINCT FROM blocked_locks.page
    AND blocking_locks.tuple IS NOT DISTINCT FROM blocked_locks.tuple
    AND blocking_locks.virtualxid IS NOT DISTINCT FROM blocked_locks.virtualxid
    AND blocking_locks.transactionid IS NOT DISTINCT FROM blocked_locks.transactionid
    AND blocking_locks.classid IS NOT DISTINCT FROM blocked_locks.classid
    AND blocking_locks.objid IS NOT DISTINCT FROM blocked_locks.objid
    AND blocking_locks.objsubid IS NOT DISTINCT FROM blocked_locks.objsubid
    AND blocking_locks.pid != blocked_locks.pid
JOIN pg_stat_activity blocking ON blocking.pid = blocking_locks.pid
WHERE NOT blocked_locks.granted;
```

### 15.2 并发问题识别

#### 15.2.1 死锁检测

**死锁检测查询**：

```sql
-- 检测潜在死锁（简化版本）
SELECT
    a1.pid AS process1,
    a1.query AS query1,
    a2.pid AS process2,
    a2.query AS query2,
    l1.locktype,
    l1.relation::regclass AS table_name
FROM pg_locks l1
JOIN pg_locks l2 ON l1.locktype = l2.locktype
    AND l1.relation = l2.relation
    AND l1.pid != l2.pid
JOIN pg_stat_activity a1 ON a1.pid = l1.pid
JOIN pg_stat_activity a2 ON a2.pid = l2.pid
WHERE NOT l1.granted
  AND NOT l2.granted
  AND l1.pid < l2.pid;
```

**死锁日志分析**：

```sql
-- PostgreSQL自动记录死锁到日志
-- 日志格式：
-- ERROR: deadlock detected
-- DETAIL: Process 12345 waits for ShareLock on transaction 123456; blocked by process 12346.
-- Process 12346 waits for ShareLock on transaction 123457; blocked by process 12345.

-- 分析死锁日志的脚本
CREATE OR REPLACE FUNCTION analyze_deadlocks()
RETURNS TABLE (
    deadlock_time TIMESTAMP,
    process1_pid INTEGER,
    process2_pid INTEGER,
    details TEXT
) AS $$
BEGIN
    -- 从日志文件中提取死锁信息（需要pg_read_file）
    -- 实际实现需要解析日志文件
    RETURN QUERY
    SELECT
        NOW() AS deadlock_time,
        12345 AS process1_pid,
        12346 AS process2_pid,
        'Deadlock detected'::TEXT AS details;
END;
$$ LANGUAGE plpgsql;
```

#### 15.2.2 阻塞查询识别

**阻塞查询检测**：

```sql
-- 查找阻塞其他查询的进程
SELECT
    blocking.pid AS blocking_pid,
    blocking.usename AS blocking_user,
    blocking.application_name AS blocking_app,
    blocking.query AS blocking_query,
    blocking.query_start AS blocking_query_start,
    COUNT(blocked.pid) AS blocked_count,
    array_agg(blocked.pid) AS blocked_pids,
    array_agg(blocked.query) AS blocked_queries
FROM pg_stat_activity blocking
JOIN pg_locks blocking_locks ON blocking_locks.pid = blocking.pid
    AND blocking_locks.granted = true
JOIN pg_locks blocked_locks ON blocked_locks.locktype = blocking_locks.locktype
    AND blocked_locks.database IS NOT DISTINCT FROM blocking_locks.database
    AND blocked_locks.relation IS NOT DISTINCT FROM blocking_locks.relation
    AND blocked_locks.page IS NOT DISTINCT FROM blocking_locks.page
    AND blocked_locks.tuple IS NOT DISTINCT FROM blocking_locks.tuple
    AND blocked_locks.virtualxid IS NOT DISTINCT FROM blocking_locks.virtualxid
    AND blocked_locks.transactionid IS NOT DISTINCT FROM blocking_locks.transactionid
    AND blocked_locks.classid IS NOT DISTINCT FROM blocking_locks.classid
    AND blocked_locks.objid IS NOT DISTINCT FROM blocking_locks.objid
    AND blocked_locks.objsubid IS NOT DISTINCT FROM blocking_locks.objsubid
    AND blocked_locks.pid != blocking_locks.pid
    AND NOT blocked_locks.granted
JOIN pg_stat_activity blocked ON blocked.pid = blocked_locks.pid
WHERE blocking.datname IS NOT NULL
GROUP BY blocking.pid, blocking.usename, blocking.application_name,
         blocking.query, blocking.query_start
ORDER BY blocked_count DESC;
```

**终止阻塞查询**：

```sql
-- 终止阻塞查询（谨慎使用）
-- 1. 首先识别阻塞查询
SELECT pid, query, NOW() - query_start AS duration
FROM pg_stat_activity
WHERE pid IN (
    SELECT blocking.pid
    FROM pg_stat_activity blocking
    JOIN pg_locks blocking_locks ON blocking_locks.pid = blocking.pid
    WHERE blocking_locks.granted = true
      AND EXISTS (
          SELECT 1
          FROM pg_locks blocked_locks
          WHERE blocked_locks.locktype = blocking_locks.locktype
            AND NOT blocked_locks.granted
            AND blocked_locks.pid != blocking_locks.pid
      )
);

-- 2. 终止查询
-- SELECT pg_terminate_backend(pid);
-- 或
-- SELECT pg_cancel_backend(pid);  -- 更温和的方式
```

#### 15.2.3 长时间运行查询

**识别长时间运行查询**：

```sql
-- 查找长时间运行的查询
SELECT
    pid,
    datname,
    usename,
    application_name,
    client_addr,
    state,
    query_start,
    NOW() - query_start AS query_duration,
    wait_event_type,
    wait_event,
    LEFT(query, 100) AS query_preview
FROM pg_stat_activity
WHERE state = 'active'
  AND NOW() - query_start > INTERVAL '1 minute'
ORDER BY query_start;
```

**查询执行时间监控**：

```sql
-- 创建查询执行时间监控视图
CREATE OR REPLACE VIEW long_running_queries AS
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    query_start,
    NOW() - query_start AS duration,
    wait_event_type,
    wait_event,
    query
FROM pg_stat_activity
WHERE state = 'active'
  AND NOW() - query_start > INTERVAL '30 seconds'
ORDER BY query_start;

-- 查询长时间运行查询
SELECT * FROM long_running_queries;
```

### 15.3 慢查询分析

#### 15.3.1 使用pg_stat_statements

**启用pg_stat_statements**：

```sql
-- 1. 安装扩展
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- 2. 配置postgresql.conf
-- shared_preload_libraries = 'pg_stat_statements'
-- pg_stat_statements.max = 10000
-- pg_stat_statements.track = all

-- 3. 重启PostgreSQL
```

**慢查询分析**：

```sql
-- 查找最慢的查询
SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time,
    max_exec_time,
    stddev_exec_time,
    rows,
    100.0 * shared_blks_hit / nullif(shared_blks_hit + shared_blks_read, 0) AS hit_percent
FROM pg_stat_statements
WHERE query NOT LIKE '%pg_stat_statements%'
ORDER BY mean_exec_time DESC
LIMIT 20;

-- 查找执行次数最多的慢查询
SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time,
    (total_exec_time / calls) AS avg_time_per_call
FROM pg_stat_statements
WHERE mean_exec_time > 100  -- 平均执行时间超过100ms
ORDER BY calls DESC
LIMIT 20;
```

#### 15.3.2 查询优化建议

**生成优化建议**：

```sql
-- 创建查询优化建议函数
CREATE OR REPLACE FUNCTION generate_query_optimization_suggestions()
RETURNS TABLE (
    query_id BIGINT,
    query TEXT,
    mean_time DECIMAL,
    calls BIGINT,
    suggestions TEXT[]
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        s.queryid,
        LEFT(s.query, 200) AS query,
        s.mean_exec_time,
        s.calls,
        ARRAY[
            CASE
                WHEN s.shared_blks_hit::DECIMAL / NULLIF(s.shared_blks_hit + s.shared_blks_read, 0) < 0.9
                THEN '考虑增加shared_buffers或优化查询以减少I/O'
                ELSE NULL
            END,
            CASE
                WHEN s.temp_blks_read > 0
                THEN '查询使用临时文件，考虑增加work_mem'
                ELSE NULL
            END,
            CASE
                WHEN s.mean_exec_time > 1000
                THEN '查询执行时间过长，考虑添加索引或优化查询计划'
                ELSE NULL
            END
        ]::TEXT[] AS suggestions
    FROM pg_stat_statements s
    WHERE s.mean_exec_time > 100
      AND s.query NOT LIKE '%pg_stat_statements%'
    ORDER BY s.mean_exec_time DESC
    LIMIT 20;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM generate_query_optimization_suggestions();
```

#### 15.3.3 实时慢查询监控

**实时监控脚本**：

```sql
-- 创建实时慢查询监控函数
CREATE OR REPLACE FUNCTION monitor_slow_queries(
    min_duration_ms INTEGER DEFAULT 1000
)
RETURNS TABLE (
    pid INTEGER,
    datname TEXT,
    usename TEXT,
    query_start TIMESTAMP,
    duration INTERVAL,
    query TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        a.pid,
        a.datname,
        a.usename,
        a.query_start,
        NOW() - a.query_start AS duration,
        a.query
    FROM pg_stat_activity a
    WHERE a.state = 'active'
      AND a.query_start IS NOT NULL
      AND EXTRACT(EPOCH FROM (NOW() - a.query_start)) * 1000 > min_duration_ms
    ORDER BY a.query_start;
END;
$$ LANGUAGE plpgsql;

-- 使用示例（监控超过1秒的查询）
SELECT * FROM monitor_slow_queries(1000);
```

**自动告警脚本**：

```python
# 慢查询自动告警脚本（Python）
import psycopg2
import time
from datetime import datetime

def monitor_slow_queries(conn, min_duration_ms=1000, check_interval=10):
    """监控慢查询并发送告警"""
    while True:
        try:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT
                        pid,
                        datname,
                        usename,
                        query_start,
                        NOW() - query_start AS duration,
                        query
                    FROM pg_stat_activity
                    WHERE state = 'active'
                      AND query_start IS NOT NULL
                      AND EXTRACT(EPOCH FROM (NOW() - query_start)) * 1000 > %s
                    ORDER BY query_start
                """, (min_duration_ms,))

                slow_queries = cur.fetchall()

                if slow_queries:
                    print(f"[{datetime.now()}] Found {len(slow_queries)} slow queries:")
                    for query in slow_queries:
                        pid, datname, usename, query_start, duration, query_text = query
                        print(f"  PID {pid}: {duration} - {query_text[:100]}")
                        # 发送告警（示例）
                        # send_alert(f"Slow query detected: PID {pid}, Duration: {duration}")

        except Exception as e:
            print(f"Error monitoring slow queries: {e}")

        time.sleep(check_interval)

# 使用示例
conn = psycopg2.connect(
    host="localhost",
    port=5432,
    database="mydb",
    user="postgres",
    password="password"
)

monitor_slow_queries(conn, min_duration_ms=1000, check_interval=10)
```

---

## 16. 事务资源消耗分析

### 16.1 事务内存使用监控

**事务内存**是PostgreSQL中每个事务可以使用的内存，主要用于排序、哈希连接、聚合等操作。

#### 16.1.1 work_mem监控

**work_mem使用情况**：

```sql
-- 查看当前work_mem配置
SHOW work_mem;

-- 查看每个操作使用的work_mem
SELECT
    pid,
    datname,
    usename,
    query,
    query_start,
    state,
    -- 估算work_mem使用（需要pg_stat_statements）
    (SELECT sum(temp_blks_written) * current_setting('work_mem')::INTEGER / 1024 / 1024
     FROM pg_stat_statements
     WHERE query = a.query
     LIMIT 1) AS estimated_work_mem_mb
FROM pg_stat_activity a
WHERE state = 'active'
ORDER BY query_start;
```

**临时文件使用监控**：

```sql
-- 查看使用临时文件的操作（表示work_mem不足）
SELECT
    pid,
    datname,
    usename,
    query,
    temp_files,
    temp_bytes,
    pg_size_pretty(temp_bytes) AS temp_size
FROM pg_stat_activity
WHERE temp_files > 0
ORDER BY temp_bytes DESC;

-- 使用pg_stat_statements查看临时文件使用
SELECT
    query,
    calls,
    temp_blks_read,
    temp_blks_written,
    (temp_blks_read + temp_blks_written) * 8192 AS temp_bytes,
    pg_size_pretty((temp_blks_read + temp_blks_written) * 8192) AS temp_size
FROM pg_stat_statements
WHERE temp_blks_read > 0 OR temp_blks_written > 0
ORDER BY (temp_blks_read + temp_blks_written) DESC
LIMIT 20;
```

#### 16.1.2 内存使用分析

**事务内存使用估算**：

```sql
-- 创建事务内存使用监控视图
CREATE OR REPLACE VIEW transaction_memory_usage AS
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    query_start,
    xact_start,
    NOW() - query_start AS query_duration,
    NOW() - xact_start AS transaction_duration,
    -- 估算内存使用（基于查询类型）
    CASE
        WHEN query ILIKE '%ORDER BY%' THEN 'Sort operation - may use work_mem'
        WHEN query ILIKE '%GROUP BY%' THEN 'Aggregation - may use work_mem'
        WHEN query ILIKE '%JOIN%' THEN 'Join operation - may use work_mem'
        ELSE 'Unknown'
    END AS memory_usage_type,
    query
FROM pg_stat_activity
WHERE state = 'active'
  AND datname IS NOT NULL;

-- 查询内存使用情况
SELECT * FROM transaction_memory_usage
ORDER BY transaction_duration DESC;
```

**内存使用统计**：

```sql
-- 统计各数据库的内存使用情况
SELECT
    datname,
    COUNT(*) AS active_connections,
    COUNT(*) FILTER (WHERE state = 'active') AS active_queries,
    COUNT(*) FILTER (WHERE state = 'idle in transaction') AS idle_in_transaction,
    AVG(EXTRACT(EPOCH FROM (NOW() - query_start))) AS avg_query_duration_seconds
FROM pg_stat_activity
WHERE datname IS NOT NULL
GROUP BY datname
ORDER BY active_connections DESC;
```

### 16.2 资源消耗较高事务识别

#### 16.2.1 识别高资源消耗事务

**高资源消耗事务查询**：

```sql
-- 识别高资源消耗的事务
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    xact_start,
    query_start,
    NOW() - xact_start AS transaction_duration,
    NOW() - query_start AS query_duration,
    wait_event_type,
    wait_event,
    -- 资源消耗指标
    CASE
        WHEN NOW() - xact_start > INTERVAL '5 minutes' THEN 'HIGH - Long transaction'
        WHEN NOW() - query_start > INTERVAL '1 minute' THEN 'MEDIUM - Long query'
        ELSE 'LOW'
    END AS resource_consumption_level,
    LEFT(query, 200) AS query_preview
FROM pg_stat_activity
WHERE state IN ('active', 'idle in transaction')
  AND datname IS NOT NULL
ORDER BY transaction_duration DESC NULLS LAST;
```

**CPU和I/O使用分析**：

```sql
-- 使用pg_stat_statements分析资源消耗
SELECT
    query,
    calls,
    total_exec_time,
    mean_exec_time,
    -- I/O统计
    shared_blks_hit,
    shared_blks_read,
    shared_blks_dirtied,
    shared_blks_written,
    -- 临时文件使用
    temp_blks_read,
    temp_blks_written,
    -- 资源消耗评分
    (mean_exec_time * calls) AS total_time_score,
    (shared_blks_read + shared_blks_written) AS io_score,
    (temp_blks_read + temp_blks_written) AS temp_file_score
FROM pg_stat_statements
WHERE query NOT LIKE '%pg_stat_statements%'
ORDER BY (mean_exec_time * calls) DESC
LIMIT 20;
```

#### 16.2.2 资源消耗分析工具

**资源消耗分析函数**：

```sql
-- 创建资源消耗分析函数
CREATE OR REPLACE FUNCTION analyze_resource_consumption(
    min_duration_seconds INTEGER DEFAULT 60
)
RETURNS TABLE (
    pid INTEGER,
    datname TEXT,
    usename TEXT,
    transaction_duration INTERVAL,
    query_duration INTERVAL,
    resource_level TEXT,
    recommendations TEXT[]
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        a.pid,
        a.datname,
        a.usename,
        NOW() - a.xact_start AS transaction_duration,
        NOW() - a.query_start AS query_duration,
        CASE
            WHEN NOW() - a.xact_start > INTERVAL '10 minutes' THEN 'CRITICAL'
            WHEN NOW() - a.xact_start > INTERVAL '5 minutes' THEN 'HIGH'
            WHEN NOW() - a.xact_start > INTERVAL '1 minute' THEN 'MEDIUM'
            ELSE 'LOW'
        END::TEXT AS resource_level,
        ARRAY[
            CASE
                WHEN a.state = 'idle in transaction' THEN '事务空闲时间过长，考虑提交或回滚事务'
                ELSE NULL
            END,
            CASE
                WHEN NOW() - a.query_start > INTERVAL '5 minutes' THEN '查询执行时间过长，考虑优化查询或添加索引'
                ELSE NULL
            END,
            CASE
                WHEN a.wait_event_type = 'Lock' THEN '等待锁，检查是否有阻塞查询'
                ELSE NULL
            END
        ]::TEXT[] AS recommendations
    FROM pg_stat_activity a
    WHERE a.state IN ('active', 'idle in transaction')
      AND a.datname IS NOT NULL
      AND EXTRACT(EPOCH FROM (NOW() - COALESCE(a.xact_start, a.query_start))) > min_duration_seconds
    ORDER BY a.xact_start NULLS LAST;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM analyze_resource_consumption(60);
```

### 16.3 资源限制配置

#### 16.3.1 work_mem配置

**work_mem设置原则**：

```conf
# postgresql.conf
# work_mem配置原则：
# - 每个操作可以使用work_mem
# - 一个查询可能有多个操作，每个操作都可以使用work_mem
# - 总内存使用 = work_mem × 操作数 × 并发连接数
# - 建议：work_mem = (总内存 - shared_buffers) / (max_connections × 平均操作数)

# 小规模部署
work_mem = 4MB

# 中规模部署
work_mem = 16MB

# 大规模部署
work_mem = 64MB

# 注意：不要设置过大，可能导致内存溢出
```

**动态调整work_mem**：

```sql
-- 会话级调整
SET work_mem = '32MB';

-- 用户级调整
ALTER USER app_user SET work_mem = '64MB';

-- 数据库级调整
ALTER DATABASE mydb SET work_mem = '32MB';

-- 查看当前work_mem设置
SHOW work_mem;
SELECT current_setting('work_mem');
```

#### 16.3.2 连接数限制

**连接数配置**：

```conf
# postgresql.conf
# 最大连接数
max_connections = 100

# 超级用户保留连接数
superuser_reserved_connections = 3

# 实际可用连接数 = max_connections - superuser_reserved_connections
```

**连接数监控**：

```sql
-- 监控连接数使用情况
SELECT
    COUNT(*) AS total_connections,
    COUNT(*) FILTER (WHERE state = 'active') AS active_connections,
    COUNT(*) FILTER (WHERE state = 'idle') AS idle_connections,
    COUNT(*) FILTER (WHERE state = 'idle in transaction') AS idle_in_transaction_connections,
    current_setting('max_connections')::INTEGER AS max_connections,
    ROUND(COUNT(*)::DECIMAL / current_setting('max_connections')::INTEGER * 100, 2) AS connection_usage_percent
FROM pg_stat_activity
WHERE datname IS NOT NULL;
```

**连接池配置**：

```conf
# 使用pgbouncer连接池
# pgbouncer.ini
[databases]
mydb = host=localhost port=5432 dbname=mydb

[pgbouncer]
pool_mode = transaction
max_client_conn = 1000
default_pool_size = 25
reserve_pool_size = 5
```

#### 16.3.3 资源限制最佳实践

**资源限制配置总结**：

```sql
-- 创建资源限制配置视图
CREATE OR REPLACE VIEW resource_limits_config AS
SELECT
    name,
    setting,
    unit,
    CASE name
        WHEN 'max_connections' THEN '最大连接数'
        WHEN 'work_mem' THEN '每个操作的工作内存'
        WHEN 'maintenance_work_mem' THEN '维护操作的工作内存'
        WHEN 'shared_buffers' THEN '共享缓冲区'
        WHEN 'effective_cache_size' THEN '有效缓存大小'
        ELSE '其他'
    END AS description,
    CASE name
        WHEN 'max_connections' THEN
            '建议：根据应用需求设置，不要过大（100-500）'
        WHEN 'work_mem' THEN
            '建议：4MB-64MB，根据系统内存和并发数调整'
        WHEN 'maintenance_work_mem' THEN
            '建议：256MB-2GB，用于VACUUM、CREATE INDEX等'
        WHEN 'shared_buffers' THEN
            '建议：系统内存的25%'
        WHEN 'effective_cache_size' THEN
            '建议：系统内存的50-75%'
        ELSE NULL
    END AS recommendation
FROM pg_settings
WHERE name IN (
    'max_connections',
    'work_mem',
    'maintenance_work_mem',
    'shared_buffers',
    'effective_cache_size'
)
ORDER BY name;

-- 查询资源限制配置
SELECT * FROM resource_limits_config;
```

**资源使用告警**：

```sql
-- 创建资源使用告警函数
CREATE OR REPLACE FUNCTION check_resource_usage()
RETURNS TABLE (
    resource_type TEXT,
    current_usage TEXT,
    max_limit TEXT,
    usage_percent DECIMAL,
    status TEXT,
    recommendation TEXT
) AS $$
BEGIN
    RETURN QUERY
    WITH connection_stats AS (
        SELECT
            COUNT(*)::INTEGER AS current_connections,
            current_setting('max_connections')::INTEGER AS max_conn
        FROM pg_stat_activity
        WHERE datname IS NOT NULL
    )
    SELECT
        'Connections'::TEXT AS resource_type,
        cs.current_connections::TEXT AS current_usage,
        cs.max_conn::TEXT AS max_limit,
        ROUND(cs.current_connections::DECIMAL / cs.max_conn * 100, 2) AS usage_percent,
        CASE
            WHEN cs.current_connections::DECIMAL / cs.max_conn > 0.9 THEN 'CRITICAL'
            WHEN cs.current_connections::DECIMAL / cs.max_conn > 0.8 THEN 'WARNING'
            ELSE 'OK'
        END::TEXT AS status,
        CASE
            WHEN cs.current_connections::DECIMAL / cs.max_conn > 0.9 THEN
                '连接数接近上限，考虑增加max_connections或使用连接池'
            WHEN cs.current_connections::DECIMAL / cs.max_conn > 0.8 THEN
                '连接数较高，监控连接使用情况'
            ELSE '连接数正常'
        END::TEXT AS recommendation
    FROM connection_stats cs;
END;
$$ LANGUAGE plpgsql;

-- 使用示例
SELECT * FROM check_resource_usage();
```

---

## 17. ML驱动的性能优化

### 17.1 机器学习在查询优化中的应用

**机器学习（ML）**在PostgreSQL查询优化中的应用是一个前沿领域，可以通过学习历史查询模式来优化查询计划。

#### 17.1.1 ML模型选择

**适用的ML模型**：

1. **决策树/随机森林**：
   - 用于查询计划选择
   - 特征：查询类型、表大小、索引可用性等
   - 输出：最优查询计划

2. **神经网络**：
   - 用于执行时间预测
   - 特征：查询复杂度、数据分布、系统负载等
   - 输出：预测执行时间

3. **强化学习**：
   - 用于自适应查询优化
   - 通过试错学习最优策略

**模型选择示例**：

```python
# 查询优化ML模型选择（Python示例）
from sklearn.ensemble import RandomForestRegressor
from sklearn.neural_network import MLPRegressor
from sklearn.model_selection import train_test_split
import pandas as pd

def select_ml_model(query_features, execution_times):
    """选择最适合的ML模型"""
    X_train, X_test, y_train, y_test = train_test_split(
        query_features, execution_times, test_size=0.2
    )

    # 模型1：随机森林
    rf_model = RandomForestRegressor(n_estimators=100, random_state=42)
    rf_model.fit(X_train, y_train)
    rf_score = rf_model.score(X_test, y_test)

    # 模型2：神经网络
    nn_model = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=500)
    nn_model.fit(X_train, y_train)
    nn_score = nn_model.score(X_test, y_test)

    # 选择得分更高的模型
    if rf_score > nn_score:
        return rf_model, 'RandomForest'
    else:
        return nn_model, 'NeuralNetwork'
```

#### 17.1.2 特征工程

**查询特征提取**：

```python
# 查询特征提取（Python）
import re
import psycopg2

def extract_query_features(query, conn):
    """提取查询特征"""
    features = {}

    # 1. 查询类型
    query_upper = query.upper().strip()
    if query_upper.startswith('SELECT'):
        features['query_type'] = 'SELECT'
    elif query_upper.startswith('INSERT'):
        features['query_type'] = 'INSERT'
    elif query_upper.startswith('UPDATE'):
        features['query_type'] = 'UPDATE'
    elif query_upper.startswith('DELETE'):
        features['query_type'] = 'DELETE'
    else:
        features['query_type'] = 'OTHER'

    # 2. 查询复杂度
    features['query_length'] = len(query)
    features['join_count'] = query_upper.count('JOIN')
    features['subquery_count'] = query_upper.count('SELECT') - 1
    features['where_clause_count'] = query_upper.count('WHERE')
    features['order_by_count'] = query_upper.count('ORDER BY')
    features['group_by_count'] = query_upper.count('GROUP BY')

    # 3. 表大小（从PostgreSQL获取）
    with conn.cursor() as cur:
        # 提取表名（简化示例）
        table_match = re.search(r'FROM\s+(\w+)', query_upper)
        if table_match:
            table_name = table_match.group(1)
            cur.execute("""
                SELECT pg_total_relation_size(%s) / 1024 / 1024 AS size_mb
            """, (table_name,))
            result = cur.fetchone()
            features['table_size_mb'] = result[0] if result else 0
        else:
            features['table_size_mb'] = 0

    return features

# 使用示例
conn = psycopg2.connect(host="localhost", database="mydb", user="postgres")
query = "SELECT * FROM users WHERE id = 123"
features = extract_query_features(query, conn)
print(features)
```

#### 17.1.3 模型训练和评估

**模型训练流程**：

```python
# ML模型训练和评估（Python）
import pandas as pd
from sklearn.ensemble import RandomForestRegressor
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
import joblib

def train_query_optimization_model(training_data):
    """训练查询优化模型"""
    # 准备数据
    X = training_data[['query_length', 'join_count', 'table_size_mb',
                       'subquery_count', 'where_clause_count']]
    y = training_data['execution_time_ms']

    # 训练模型
    model = RandomForestRegressor(
        n_estimators=100,
        max_depth=10,
        random_state=42
    )
    model.fit(X, y)

    # 评估模型
    predictions = model.predict(X)
    mae = mean_absolute_error(y, predictions)
    rmse = mean_squared_error(y, predictions, squared=False)
    r2 = r2_score(y, predictions)

    print(f"Model Performance:")
    print(f"  MAE: {mae:.2f} ms")
    print(f"  RMSE: {rmse:.2f} ms")
    print(f"  R²: {r2:.4f}")

    # 保存模型
    joblib.dump(model, 'query_optimization_model.pkl')

    return model

# 使用示例
# training_data = pd.read_csv('query_training_data.csv')
# model = train_query_optimization_model(training_data)
```

### 17.2 自动参数调优

**自动参数调优**使用机器学习算法自动调整PostgreSQL配置参数，以优化性能。

#### 17.2.1 参数调优算法

**贝叶斯优化**：

```python
# 使用贝叶斯优化进行参数调优（Python）
from skopt import gp_minimize
from skopt.space import Real, Integer
import subprocess
import time

def evaluate_postgresql_config(params):
    """评估PostgreSQL配置参数"""
    shared_buffers, work_mem, max_connections = params

    # 更新PostgreSQL配置
    update_postgresql_config({
        'shared_buffers': f'{shared_buffers}MB',
        'work_mem': f'{work_mem}MB',
        'max_connections': max_connections
    })

    # 重启PostgreSQL（简化示例）
    # subprocess.run(['sudo', 'systemctl', 'restart', 'postgresql'])
    # time.sleep(10)  # 等待服务启动

    # 运行基准测试
    # benchmark_result = run_pgbench()

    # 返回负的性能指标（因为我们要最小化）
    # return -benchmark_result['tps']

    # 简化示例：返回模拟性能指标
    return -(shared_buffers * 0.1 + work_mem * 0.05 + max_connections * 0.01)

def auto_tune_postgresql():
    """自动调优PostgreSQL参数"""
    # 定义参数搜索空间
    space = [
        Integer(256, 2048, name='shared_buffers'),  # 256MB - 2GB
        Integer(4, 64, name='work_mem'),  # 4MB - 64MB
        Integer(50, 500, name='max_connections')  # 50 - 500
    ]

    # 使用贝叶斯优化
    result = gp_minimize(
        func=evaluate_postgresql_config,
        dimensions=space,
        n_calls=20,  # 评估次数
        random_state=42
    )

    print(f"Best parameters: {result.x}")
    print(f"Best performance: {-result.fun}")

    return result.x

# 使用示例
# best_params = auto_tune_postgresql()
```

#### 17.2.2 自动化工具

**pg_tune工具**：

```bash
# pg_tune - PostgreSQL自动调优工具
# 安装
pip install pg_tune

# 使用
pg_tune --dbname=mydb --host=localhost --port=5432

# pg_tune会：
# 1. 分析系统资源
# 2. 分析当前配置
# 3. 生成优化建议
# 4. 可选：自动应用优化
```

### 17.3 智能索引推荐

**智能索引推荐**使用机器学习分析查询模式，自动推荐最优索引。

#### 17.3.1 索引推荐算法

**基于查询模式的索引推荐**：

```python
# 智能索引推荐（Python）
import pandas as pd
from collections import Counter
import psycopg2

def recommend_indexes(conn, min_query_count=10):
    """推荐索引"""
    with conn.cursor() as cur:
        # 获取查询统计
        cur.execute("""
            SELECT
                query,
                calls,
                mean_exec_time,
                shared_blks_hit,
                shared_blks_read
            FROM pg_stat_statements
            WHERE calls >= %s
              AND mean_exec_time > 100  -- 只考虑慢查询
            ORDER BY mean_exec_time DESC
        """, (min_query_count,))

        queries = cur.fetchall()

    # 分析查询模式
    index_recommendations = []

    for query, calls, mean_time, hits, reads in queries:
        # 提取WHERE子句中的列
        where_columns = extract_where_columns(query)

        # 提取JOIN条件中的列
        join_columns = extract_join_columns(query)

        # 提取ORDER BY中的列
        order_columns = extract_order_columns(query)

        # 生成索引推荐
        if where_columns:
            index_recommendations.append({
                'table': extract_table_name(query),
                'columns': where_columns,
                'type': 'B-tree',
                'priority': calls * mean_time,  # 优先级 = 调用次数 × 平均时间
                'reason': 'WHERE clause optimization'
            })

        if join_columns:
            index_recommendations.append({
                'table': extract_table_name(query),
                'columns': join_columns,
                'type': 'B-tree',
                'priority': calls * mean_time,
                'reason': 'JOIN optimization'
            })

    # 按优先级排序
    index_recommendations.sort(key=lambda x: x['priority'], reverse=True)

    return index_recommendations

def extract_where_columns(query):
    """提取WHERE子句中的列（简化示例）"""
    import re
    where_match = re.search(r'WHERE\s+(.+?)(?:\s+GROUP|\s+ORDER|\s+LIMIT|$)', query, re.IGNORECASE)
    if where_match:
        where_clause = where_match.group(1)
        # 提取列名（简化）
        columns = re.findall(r'(\w+)\s*[=<>]', where_clause)
        return list(set(columns))
    return []

# 使用示例
# conn = psycopg2.connect(host="localhost", database="mydb", user="postgres")
# recommendations = recommend_indexes(conn)
# for rec in recommendations[:10]:  # 前10个推荐
#     print(f"CREATE INDEX ON {rec['table']} ({', '.join(rec['columns'])});")
```

#### 17.3.2 工具使用

**使用pg_qualstats和hypopg**：

```sql
-- 1. 安装扩展
CREATE EXTENSION IF NOT EXISTS pg_qualstats;
CREATE EXTENSION IF NOT EXISTS hypopg;

-- 2. 收集查询统计
-- pg_qualstats自动收集WHERE子句统计

-- 3. 查看统计信息
SELECT
    schemaname,
    tablename,
    attname,
    nbfiltered,
    qualnodeid
FROM pg_qualstats;

-- 4. 生成索引推荐
SELECT
    schemaname,
    tablename,
    array_agg(attname ORDER BY attname) AS columns,
    sum(nbfiltered) AS total_filtered
FROM pg_qualstats
GROUP BY schemaname, tablename, qualnodeid
HAVING sum(nbfiltered) > 1000
ORDER BY total_filtered DESC;

-- 5. 使用hypopg测试虚拟索引
SELECT * FROM hypopg_create_index('CREATE INDEX ON users (email)');
```

### 17.4 负载预测和资源调度

**负载预测**使用时间序列模型预测未来负载，提前调整资源分配。

#### 17.4.1 负载预测模型

**时间序列预测**：

```python
# 负载预测模型（Python）
import pandas as pd
import numpy as np
from sklearn.ensemble import RandomForestRegressor
from sklearn.preprocessing import StandardScaler
import matplotlib.pyplot as plt

def prepare_load_data(conn, days=30):
    """准备负载数据"""
    query = """
        SELECT
            date_trunc('hour', query_start) AS hour,
            COUNT(*) AS query_count,
            AVG(EXTRACT(EPOCH FROM (NOW() - query_start))) AS avg_duration,
            COUNT(*) FILTER (WHERE state = 'active') AS active_queries
        FROM pg_stat_activity
        WHERE query_start > NOW() - INTERVAL '%s days'
          AND datname IS NOT NULL
        GROUP BY hour
        ORDER BY hour
    """ % days

    df = pd.read_sql(query, conn)
    return df

def create_features(df):
    """创建特征"""
    df['hour'] = pd.to_datetime(df['hour']).dt.hour
    df['day_of_week'] = pd.to_datetime(df['hour']).dt.dayofweek
    df['is_weekend'] = (df['day_of_week'] >= 5).astype(int)

    # 滞后特征
    df['query_count_lag1'] = df['query_count'].shift(1)
    df['query_count_lag24'] = df['query_count'].shift(24)

    # 移动平均
    df['query_count_ma7'] = df['query_count'].rolling(window=7).mean()

    return df

def train_load_prediction_model(df):
    """训练负载预测模型"""
    # 准备特征和目标
    feature_cols = ['hour', 'day_of_week', 'is_weekend',
                     'query_count_lag1', 'query_count_lag24', 'query_count_ma7']
    X = df[feature_cols].dropna()
    y = df.loc[X.index, 'query_count']

    # 训练模型
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(X, y)

    # 预测
    predictions = model.predict(X)

    return model, predictions

# 使用示例
# conn = psycopg2.connect(host="localhost", database="mydb", user="postgres")
# df = prepare_load_data(conn, days=30)
# df = create_features(df)
# model, predictions = train_load_prediction_model(df)
```

#### 17.4.2 资源调度策略

**基于预测的资源调度**：

```python
# 基于预测的资源调度（Python）
def schedule_resources(predicted_load, current_resources):
    """根据预测负载调度资源"""
    recommendations = []

    # 预测负载阈值
    high_load_threshold = current_resources['max_connections'] * 0.8
    low_load_threshold = current_resources['max_connections'] * 0.3

    if predicted_load > high_load_threshold:
        recommendations.append({
            'action': 'scale_up',
            'resource': 'max_connections',
            'current': current_resources['max_connections'],
            'recommended': int(predicted_load * 1.2),
            'reason': 'Predicted load exceeds threshold'
        })
    elif predicted_load < low_load_threshold:
        recommendations.append({
            'action': 'scale_down',
            'resource': 'max_connections',
            'current': current_resources['max_connections'],
            'recommended': int(predicted_load * 1.5),
            'reason': 'Predicted load below threshold, can reduce resources'
        })

    return recommendations

# 使用示例
# predicted_load = model.predict(future_features)
# current_resources = {'max_connections': 100}
# recommendations = schedule_resources(predicted_load, current_resources)
```

---

## 18. 实战案例

### 案例1: OLTP系统调优

**场景**: 电商订单系统，高并发写入，低延迟查询

**调优方案**:

```sql
-- 连接池配置
-- pgbouncer transaction模式
max_client_conn = 1000
default_pool_size = 25

-- PostgreSQL参数
max_connections = 200
shared_buffers = 8GB  -- 25% of 32GB RAM
work_mem = 16MB
effective_cache_size = 24GB
random_page_cost = 1.1  -- SSD
effective_io_concurrency = 200  -- NVMe

-- 索引优化
CREATE INDEX CONCURRENTLY idx_orders_user_created
ON orders(user_id, created_at DESC);
```

**效果**: QPS提升40%，P99延迟降低30%

### 案例2: OLAP分析系统调优

**场景**: 数据仓库，复杂聚合查询，批量分析

**调优方案**:

```sql
-- PostgreSQL参数
max_parallel_workers_per_gather = 4
max_parallel_workers = 16
work_mem = 256MB
shared_buffers = 16GB
effective_cache_size = 48GB
jit = on
jit_above_cost = 200000

-- 分区表优化
CREATE TABLE sales (
    sale_date DATE,
    product_id INT,
    amount DECIMAL(10,2)
) PARTITION BY RANGE (sale_date);

-- 创建月度分区
CREATE TABLE sales_2025_01 PARTITION OF sales
FOR VALUES FROM ('2025-01-01') TO ('2025-02-01');
```

**效果**: 复杂查询性能提升60%，并行利用率提升50%

### 案例3: 混合负载调优

**场景**: 读写混合，需要平衡OLTP和OLAP性能

**调优方案**:

```sql
-- 读写分离
-- 主库（OLTP）
max_connections = 200
work_mem = 16MB
shared_buffers = 8GB
jit = off

-- 只读副本（OLAP）
max_connections = 100
work_mem = 256MB
shared_buffers = 16GB
jit = on
max_parallel_workers_per_gather = 4
```

**效果**: OLTP延迟稳定，OLAP查询性能提升50%

---

## 14. 深入阅读

### 相关文档

#### 查询与优化

- ⭐⭐⭐ [执行计划与性能调优](../../../02-查询与优化/02.03-执行计划/02.04-执行计划与性能调优.md) - 查询优化
- ⭐⭐ [查询优化器原理](../../../02-查询与优化/02.01-查询优化器/02.01-查询优化器原理.md) - 查询优化基础
- ⭐⭐ [索引结构与优化](../../../02-查询与优化/02.02-索引结构/02.02-索引结构与优化.md) - 索引优化
- ⭐⭐ [统计信息与代价模型](../../../02-查询与优化/02.04-统计信息/02.03-统计信息与代价模型.md) - 统计信息管理

#### 核心课程

- ⭐⭐ [系统架构与设计原理](../../../01-核心基础/01.02-系统架构/01.01-系统架构与设计原理.md) - 系统架构基础
- ⭐⭐ [存储管理与数据持久化](../../../04-存储与恢复/01.06-存储管理与数据持久化.md) - 存储优化（列存储架构、列压缩技术、异步I/O）🆕

#### 运维实践

- ⭐⭐⭐ [监控与诊断](../../../12-监控与诊断/06.01-监控与诊断.md) - 性能监控
- ⭐⭐ [性能调优变更闭环](../../../13-高可用架构/监控与诊断/06.03-性能调优变更闭环.md) - 变更管理
- ⭐⭐ [性能问题-案例库](../../../16-应用设计与开发/行业案例/性能问题-案例库.md) - 性能问题案例

#### 前沿技术

- ⭐⭐ [多模型数据库](../../../07-多模型数据库/README.md) - 向量检索优化

#### 版本特性

- ⭐⭐ [PostgreSQL 18新特性](../../../18-版本特性/02.01-PostgreSQL-18-新特性.md) - PostgreSQL 18性能优化

### 外部资源

- [PostgreSQL性能调优指南](https://www.postgresql.org/docs/current/performance-tips.html)
- [pg_stat_statements文档](https://www.postgresql.org/docs/current/pgstatstatements.html)
- [PostgreSQL配置参数](https://www.postgresql.org/docs/current/runtime-config.html)

---

## 15. 参考文献

1. PostgreSQL Global Development Group. (2025). PostgreSQL 18 Documentation. <https://www.postgresql.org/docs/18/>
2. PostgreSQL官方文档 - [性能调优](https://www.postgresql.org/docs/current/performance-tips.html)
3. PostgreSQL官方文档 - [配置参数](https://www.postgresql.org/docs/current/runtime-config.html)
4. PostgreSQL官方文档 - [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html)
5. PostgreSQL官方文档 - [EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html)
6. TimescaleDB - [性能调优最佳实践](https://docs.timescale.com/timescaledb/latest/how-to-guides/performance/)
7. PostgreSQL Global Development Group. (2024). PostgreSQL 17 Documentation. <https://www.postgresql.org/docs/17/>
