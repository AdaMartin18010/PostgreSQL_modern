...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '备份监控视图创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建备份监控视图失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'backup_audit') THEN
            ...
            RAISE NOTICE '备份审计表创建成功';
        ELSE
            RAISE NOTICE '备份审计表已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '备份审计表已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建备份审计表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
