...
> **📋 文档来源**: `PostgreSQL/01-核心课程/01.01-系统架构与设计原理.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '当前Backend进程PID: %', backend_pid;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取Backend进程PID失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '当前数据库: %, 活动进程数: %', current_db, process_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看系统进程状态准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看检查点配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        CHECKPOINT;
        RAISE NOTICE '检查点已成功触发';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '触发检查点失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看WAL配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- WAL Summarizer配置（PostgreSQL 18，带错误处理）
DO $$
DECLARE
...
BEGIN
    BEGIN
        ...
        IF v_wal_summarize IS NOT NULL THEN
            RAISE NOTICE 'WAL Summarizer配置（PostgreSQL 18）：';
            RAISE NOTICE '  wal_summarize: %, wal_summarize_keep_time: %',
                v_wal_summarize, v_wal_summarize_keep_time;
        ELSE
            RAISE WARNING 'WAL Summarizer功能仅在PostgreSQL 18+中可用';
        END IF;
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING 'WAL Summarizer配置参数不存在（可能PostgreSQL版本低于18）';
        WHEN OTHERS THEN
            RAISE WARNING '查看WAL Summarizer配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查看WAL汇总统计（PostgreSQL 18，带错误处理和性能测试）
DO $$
DECLARE
    stat_count INT;
BEGIN
    BEGIN
        ...
        IF stat_count > 0 THEN
            RAISE NOTICE '找到WAL汇总统计信息（PostgreSQL 18）';
        ELSE
            RAISE WARNING '未找到WAL汇总统计信息（可能PostgreSQL版本低于18或未启用）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING 'pg_stat_wal_summarizer视图不存在（可能PostgreSQL版本低于18）';
        WHEN OTHERS THEN
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看Autovacuum配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查看Autovacuum统计（带错误处理和性能测试）
DO $$
DECLARE
    stat_count INT;
BEGIN
    BEGIN
        ...
        IF stat_count > 0 THEN
            RAISE NOTICE '找到 % 个正在运行的VACUUM进程', stat_count;
        ELSE
            RAISE NOTICE '当前没有正在运行的VACUUM进程';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '统计信息：数据库数: %, 用户表数: %', db_count, table_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看统计信息准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
3. **Buffer区域（数据缓冲区）**
   ...
   - 大小由shared_buffers配置
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE WARNING '查看缓冲区配置失败: %', SQLERRM;
                RAISE;
        END;
    END $$;
...
PostgreSQL 18引入了动态共享内存管理，使共享内存分配更智能，内存效率提升20%。
  ...
  ```sql
  -- PostgreSQL 18新增：查看共享内存分配（带错误处理和性能测试）
  DO $$
  DECLARE
      shmem_count INT;
  BEGIN
      BEGIN
          ...
          IF shmem_count > 0 THEN
              RAISE NOTICE '找到 % 个共享内存分配项（PostgreSQL 18）', shmem_count;
          ELSE
              RAISE WARNING '未找到共享内存分配信息（可能PostgreSQL版本低于18）';
          END IF;
      EXCEPTION
          WHEN undefined_table THEN
              RAISE WARNING 'pg_shmem_allocations视图不存在（可能PostgreSQL版本低于18）';
          WHEN OTHERS THEN
              RAISE WARNING '查看共享内存分配准备失败: %', SQLERRM;
              RAISE;
      END;
  END $$;
  ...
  BEGIN
      BEGIN
          ...
          IF v_dynamic_shmem_type IS NOT NULL THEN
              RAISE NOTICE '动态共享内存类型: %', v_dynamic_shmem_type;
          ELSE
              RAISE WARNING 'dynamic_shared_memory_type参数不存在（可能PostgreSQL版本低于18）';
          END IF;
      EXCEPTION
          WHEN undefined_object THEN
              RAISE WARNING '动态共享内存配置参数不存在（可能PostgreSQL版本低于18）';
          WHEN OTHERS THEN
              RAISE WARNING '查看动态共享内存配置失败: %', SQLERRM;
              RAISE;
      END;
  END $$;
  -- postgresql.conf
...
BEGIN
    BEGIN
        ...
        IF setting_count > 0 THEN
            RAISE NOTICE '找到 % 个共享内存相关配置参数', setting_count;
        ELSE
            RAISE WARNING '未找到共享内存相关配置参数';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看查询内存配置失败: %', SQLERRM;
            RAISE;
    END;
END $$;
-- postgresql.conf
...
-- 查看临时文件使用（如果work_mem不足，带错误处理和性能测试）
DO $$
DECLARE
    current_db TEXT;
    db_count INT;
BEGIN
    BEGIN
        ...
        IF db_count > 0 THEN
            RAISE NOTICE '找到数据库 % 的统计信息', current_db;
        ELSE
            RAISE WARNING '未找到当前数据库的统计信息';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'slowspace') THEN
            ...
            RAISE NOTICE '提示：创建表空间 slowspace（需要确保目录存在且有权限）';
        ELSE
    ...
    BEGIN
        ...
        ELSE
            ...
            IF EXISTS (SELECT 1 FROM pg_tablespace WHERE spcname = 'fastspace') THEN
                ...
                RAISE NOTICE '表 large_table 在表空间 fastspace 中创建成功';
            ELSE
                RAISE WARNING '表空间 fastspace 不存在，无法在指定表空间创建表';
            END IF;
        END IF;
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '表空间 fastspace 不存在';
...
BEGIN
    BEGIN
        ...
        IF hit_ratio IS NULL THEN
            RAISE WARNING '无法计算缓冲区命中率，可能没有统计数据';
        ELSE
            RAISE NOTICE '缓冲区命中率: %%%', ROUND(hit_ratio * 100, 2);
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询缓冲区命中率失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
- [Docker部署指南](../../11-部署架构/容器化部署/05.12-Docker部署.md) - 容器化部署实践
...
- ⭐⭐⭐ [存储管理与数据持久化](../../04-存储与恢复/01.06-存储管理与数据持久化.md) - 存储管理详细说明
...
- ⭐⭐ [集群部署与高可用](../../11-部署架构/集群部署/05.04-集群部署与高可用.md) - 高可用架构设计
