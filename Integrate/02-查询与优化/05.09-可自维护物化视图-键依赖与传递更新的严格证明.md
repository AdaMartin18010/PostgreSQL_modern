...
> **📋 文档来源**: `DataBaseTheory\05-索引与查询优化\05.09-可自维护物化视图-键依赖与传递更新的严格证明.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
            DROP MATERIALIZED VIEW mv_account_summary CASCADE;
            RAISE NOTICE '物化视图 mv_account_summary 已存在，先删除';
        END IF;
        ...
        FROM transactions
        GROUP BY account_id;
        RAISE NOTICE '物化视图 mv_account_summary 创建成功';
    EXCEPTION
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
            RAISE EXCEPTION '物化视图 mv_account_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_account_summary' AND indexname = 'idx_mv_account_summary_account_id') THEN
            RAISE WARNING '索引 idx_mv_account_summary_account_id 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_account_summary_account_id
            ON mv_account_summary (account_id);
            RAISE NOTICE '唯一索引 idx_mv_account_summary_account_id 创建成功（键依赖）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_account_summary 不存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
            RAISE WARNING '物化视图 mv_account_summary 不存在，无法检查键依赖';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
            RAISE WARNING '表 transactions 不存在，无法创建触发器函数';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
            RAISE WARNING '物化视图 mv_account_summary 不存在，无法创建触发器函数';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'maintain_account_summary') THEN
            DROP FUNCTION maintain_account_summary() CASCADE;
            RAISE NOTICE '函数 maintain_account_summary 已存在，先删除';
        END IF;
        ...
        BEGIN
            BEGIN
                IF TG_OP = 'INSERT' THEN
                    ...
                    VALUES (COALESCE(NEW.account_id, 0), COALESCE(NEW.amount, 0), 1)
                    ON CONFLICT (account_id) DO UPDATE
                    SET
                        total_amount = mv_account_summary.total_amount + COALESCE(NEW.amount, 0),
                ...
                IF TG_OP = 'UPDATE' THEN
                    ...
                    SET
                        total_amount = total_amount - COALESCE(OLD.amount, 0) + COALESCE(NEW.amount, 0)
                    WHERE account_id = COALESCE(NEW.account_id, 0);
                END IF;
                ...
                IF TG_OP = 'DELETE' THEN
                    ...
                    WHERE account_id = COALESCE(OLD.account_id, 0);

                    -- 如果交易数为0，删除记录
                    DELETE FROM mv_account_summary
                    WHERE account_id = COALESCE(OLD.account_id, 0) AND transaction_count = 0;
                END IF;
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '传递更新函数 maintain_account_summary 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
            RAISE WARNING '表 transactions 不存在，无法创建触发器';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_maintain_account_summary') THEN
            DROP TRIGGER trigger_maintain_account_summary ON transactions;
            RAISE NOTICE '触发器 trigger_maintain_account_summary 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '触发器 trigger_maintain_account_summary 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 transactions 不存在';
        WHEN undefined_function THEN
            RAISE EXCEPTION '函数 maintain_account_summary 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法创建物化视图';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_inventory') THEN
            DROP MATERIALIZED VIEW mv_product_inventory CASCADE;
            RAISE NOTICE '物化视图 mv_product_inventory 已存在，先删除';
        END IF;
        ...
        FROM inventory
        GROUP BY product_id;
        RAISE NOTICE '可自维护物化视图 mv_product_inventory 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 inventory 不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建物化视图失败: %', SQLERRM;
    END;

    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_inventory') THEN
            RAISE EXCEPTION '物化视图 mv_product_inventory 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_product_inventory' AND indexname = 'idx_mv_product_inventory_product_id') THEN
            RAISE WARNING '索引 idx_mv_product_inventory_product_id 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_product_inventory_product_id
            ON mv_product_inventory(product_id);
            RAISE NOTICE '唯一索引 idx_mv_product_inventory_product_id 创建成功（键依赖）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_product_inventory 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_product_inventory_product_id 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inventory') THEN
            RAISE WARNING '表 inventory 不存在，无法创建触发器函数';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_inventory') THEN
            RAISE WARNING '物化视图 mv_product_inventory 不存在，无法创建触发器函数';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'maintain_product_inventory') THEN
            DROP FUNCTION maintain_product_inventory() CASCADE;
            RAISE NOTICE '函数 maintain_product_inventory 已存在，先删除';
        END IF;
        ...
        BEGIN
            BEGIN
                IF TG_OP = 'INSERT' THEN
                    ...
                    SET
                        total_quantity = mv_product_inventory.total_quantity + COALESCE(NEW.quantity, 0),
                        location_count = mv_product_inventory.location_count + 1,
                        avg_quantity = (mv_product_inventory.total_quantity + COALESCE(NEW.quantity, 0)) /
                ...
                IF TG_OP = 'UPDATE' THEN
                    ...
                    SET
                        ...
                        avg_quantity = (total_quantity - COALESCE(OLD.quantity, 0) + COALESCE(NEW.quantity, 0)) / location_count
                    WHERE product_id = COALESCE(NEW.product_id, 0);
                END IF;
                ...
                IF TG_OP = 'DELETE' THEN
                    ...
                    SET
                        ...
                        avg_quantity = CASE
                            ...
                            THEN (total_quantity - COALESCE(OLD.quantity, 0)) / (location_count - 1)
                            ELSE 0
                        END
                    WHERE product_id = COALESCE(OLD.product_id, 0);
...
3. **验证键依赖**：
   ...
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
               RAISE WARNING '物化视图 mv_account_summary 不存在，无法检查唯一约束';
               RETURN;
           END IF;
           RAISE NOTICE '开始检查唯一约束';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
       END;
...
1. **使用ON CONFLICT**：
   ...
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
               RAISE WARNING '物化视图 mv_account_summary 不存在，无法执行传递更新';
               RETURN;
           END IF;
           RAISE NOTICE '开始执行高效的传递更新';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
       END;
...
2. **批量更新优化**：
   ...
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_account_summary') THEN
               RAISE WARNING '物化视图 mv_account_summary 不存在，无法执行批量更新';
               RETURN;
           END IF;
           RAISE NOTICE '开始执行批量更新';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
       END;
...
3. **监控维护性能**：
   ...
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
               RAISE WARNING '表 transactions 不存在，无法监控触发器执行时间';
               RETURN;
           END IF;
