...
> **📋 文档来源**: `DataBaseTheory\05-索引与查询优化\05.04-增量物化视图-代数差分与正确性.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE EXCEPTION '表 orders 不存在，无法创建物化视图';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_order_summary') THEN
        ...
        ELSE
            ...
            RAISE NOTICE '物化视图 mv_order_summary 创建成功';
        END IF;
    EXCEPTION
...
-- 创建唯一索引（CONCURRENTLY刷新必需，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_order_summary') THEN
            RAISE EXCEPTION '物化视图 mv_order_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_order_summary' AND indexname = 'idx_mv_order_summary_customer_id') THEN
            RAISE WARNING '索引 idx_mv_order_summary_customer_id 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_order_summary_customer_id
            ON mv_order_summary (customer_id);
            RAISE NOTICE '唯一索引 idx_mv_order_summary_customer_id 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_order_summary 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_order_summary_customer_id 已存在';
        WHEN OTHERS THEN
...
```sql
-- 增量刷新（PostgreSQL 13+，带错误处理）
DO $$
DECLARE
    pg_version INT;
BEGIN
    BEGIN
        -- 检查PostgreSQL版本（CONCURRENTLY需要13+）
        SELECT current_setting('server_version_num')::INT INTO pg_version;
        IF pg_version < 130000 THEN
            RAISE EXCEPTION 'CONCURRENTLY刷新需要PostgreSQL 13+，当前版本: %', version();
        END IF;
        ...
        IF NOT EXISTS (
            ...
            WHERE tablename = 'mv_order_summary' AND indexdef LIKE '%UNIQUE%'
        ) THEN
            RAISE EXCEPTION 'CONCURRENTLY刷新需要唯一索引';
        END IF;

        RAISE NOTICE '开始增量刷新物化视图 mv_order_summary';
        -- REFRESH MATERIALIZED VIEW CONCURRENTLY mv_order_summary;
        -- 注意：REFRESH MATERIALIZED VIEW CONCURRENTLY不能在事务块中执行
        RAISE NOTICE '增量刷新命令已准备（请手动执行: REFRESH MATERIALIZED VIEW CONCURRENTLY mv_order_summary;）';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_order_summary 不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '增量刷新准备失败: %', SQLERRM;
    END;
END $$;
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT
                ...
                MAX(order_date) as last_order_date
            FROM orders
            WHERE order_date > COALESCE((SELECT MAX(last_order_date) FROM mv_order_summary), '1970-01-01'::TIMESTAMP)
            GROUP BY customer_id
            ...
            RAISE NOTICE '插入差分完成: 影响 % 行', insert_count;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '计算插入差分失败: %', SQLERRM;
        END;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表或物化视图不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '手动增量维护失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            ...
            RAISE NOTICE '删除差分完成: 影响 % 行', delete_count;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '计算删除差分失败: %', SQLERRM;
        END;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表或物化视图不存在';
   ...
   BEGIN
       ...
       BEGIN
           ...
           IF EXISTS (
               ...
               WHERE jobname = 'refresh-order-summary'
           ) THEN
               RAISE WARNING '定时任务 refresh-order-summary 已存在';
           ELSE
               -- 注意：cron.schedule需要在事务外执行
               RAISE NOTICE '定时任务命令已准备（请手动执行）:';
               RAISE NOTICE '  SELECT cron.schedule(';
               RAISE NOTICE '      ''refresh-order-summary'',';
               RAISE NOTICE '      ''0 * * * *'',';
               RAISE NOTICE '      $$REFRESH MATERIALIZED VIEW CONCURRENTLY mv_order_summary$$';
               RAISE NOTICE '  );';
           END IF;
       EXCEPTION
           WHEN undefined_table THEN
               RAISE EXCEPTION '物化视图 mv_order_summary 不存在';
           WHEN OTHERS THEN
               RAISE EXCEPTION '创建定时任务失败: %', SQLERRM;
       END;
   END $$;
   ```
...
3. **增量刷新监控**：
   ...
   ```sql
   -- 监控物化视图刷新时间（带错误处理和性能测试）
   DO $$
   BEGIN
       BEGIN
           RAISE NOTICE '开始监控物化视图刷新时间';
       EXCEPTION
           WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'PostgreSQL自动增量刷新命令已准备（请手动执行）:';
        RAISE NOTICE '  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_order_summary;';
        -- 注意：REFRESH MATERIALIZED VIEW CONCURRENTLY不能在事务块中执行
    EXCEPTION
        WHEN undefined_table THEN
...
-- SQLite: 手动实现（使用触发器，带错误处理）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE 'SQLite手动实现示例（PostgreSQL不支持此语法）:';
        RAISE NOTICE '  CREATE TRIGGER update_order_summary_after_insert';
        RAISE NOTICE '  AFTER INSERT ON orders';
        RAISE NOTICE '  BEGIN';
        RAISE NOTICE '      INSERT OR REPLACE INTO order_summary (customer_id, order_count, total_amount)';
        RAISE NOTICE '      SELECT';
        RAISE NOTICE '          customer_id,';
        RAISE NOTICE '          COUNT(*),';
        RAISE NOTICE '          SUM(total_amount)';
        RAISE NOTICE '      FROM orders';
        RAISE NOTICE '      WHERE customer_id = NEW.customer_id';
        RAISE NOTICE '      GROUP BY customer_id;';
        RAISE NOTICE '  END;';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_order_summary') THEN
            RAISE WARNING '物化视图 mv_customer_order_summary 已存在';
        ELSE
            ...
            FROM orders
            GROUP BY customer_id;
            RAISE NOTICE '物化视图 mv_customer_order_summary 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_order_summary') THEN
            RAISE EXCEPTION '物化视图 mv_customer_order_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_customer_order_summary' AND indexname = 'idx_mv_customer_order_summary_customer_id') THEN
            RAISE WARNING '索引 idx_mv_customer_order_summary_customer_id 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_customer_order_summary_customer_id
            ON mv_customer_order_summary (customer_id);
            RAISE NOTICE '唯一索引 idx_mv_customer_order_summary_customer_id 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_customer_order_summary 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_customer_order_summary_customer_id 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_order_summary') THEN
            RAISE WARNING '物化视图 mv_product_order_summary 已存在';
        ELSE
            ...
            FROM orders
            GROUP BY product_id;
            RAISE NOTICE '物化视图 mv_product_order_summary 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_order_summary') THEN
            RAISE EXCEPTION '物化视图 mv_product_order_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_product_order_summary' AND indexname = 'idx_mv_product_order_summary_product_id') THEN
            RAISE WARNING '索引 idx_mv_product_order_summary_product_id 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_product_order_summary_product_id
            ON mv_product_order_summary (product_id);
            RAISE NOTICE '唯一索引 idx_mv_product_order_summary_product_id 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_product_order_summary 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_product_order_summary_product_id 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
...
BEGIN
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_order_summary') THEN
            RAISE WARNING '物化视图 mv_customer_order_summary 不存在';
        ELSE
            -- 检查是否已存在定时任务
            IF EXISTS (
                SELECT 1 FROM cron.job
                WHERE jobname = 'refresh-customer-summary'
            ) THEN
                RAISE WARNING '定时任务 refresh-customer-summary 已存在';
            ELSE
                RAISE NOTICE '定时任务命令已准备（请手动执行）:';
                RAISE NOTICE '  SELECT cron.schedule(';
                RAISE NOTICE '      ''refresh-customer-summary'',';
                RAISE NOTICE '      ''0 * * * *'',';
                RAISE NOTICE '      $$REFRESH MATERIALIZED VIEW CONCURRENTLY mv_customer_order_summary$$';
                RAISE NOTICE '  );';
            END IF;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_order_summary') THEN
            RAISE WARNING '物化视图 mv_product_order_summary 不存在';
        ELSE
            IF EXISTS (
                SELECT 1 FROM cron.job
                WHERE jobname = 'refresh-product-summary'
            ) THEN
                RAISE WARNING '定时任务 refresh-product-summary 已存在';
            ELSE
                RAISE NOTICE '定时任务命令已准备（请手动执行）:';
                RAISE NOTICE '  SELECT cron.schedule(';
                RAISE NOTICE '      ''refresh-product-summary'',';
                RAISE NOTICE '      ''*/15 * * * *'',';
                RAISE NOTICE '      $$REFRESH MATERIALIZED VIEW CONCURRENTLY mv_product_order_summary$$';
                RAISE NOTICE '  );';
            END IF;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建定时任务失败: %', SQLERRM;
    END;
END $$;
...
```sql
-- 查询客户订单汇总（使用物化视图，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_order_summary') THEN
            RAISE WARNING '物化视图 mv_customer_order_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行客户订单汇总查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 查询产品订单汇总（使用物化视图，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_order_summary') THEN
            RAISE WARNING '物化视图 mv_product_order_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行产品订单汇总查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'logs') THEN
            RAISE EXCEPTION '表 logs 不存在，无法创建物化视图';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_app_log_summary') THEN
            RAISE WARNING '物化视图 mv_app_log_summary 已存在';
        ELSE
            ...
            RAISE NOTICE '物化视图 mv_app_log_summary 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_app_log_summary') THEN
            RAISE EXCEPTION '物化视图 mv_app_log_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_app_log_summary' AND indexname = 'idx_mv_app_log_summary_unique') THEN
            RAISE WARNING '索引 idx_mv_app_log_summary_unique 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_app_log_summary_unique
            ON mv_app_log_summary (app_id, log_level, log_date);
            RAISE NOTICE '唯一索引 idx_mv_app_log_summary_unique 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_app_log_summary 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_app_log_summary_unique 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '手动增量刷新命令已准备（请手动执行）:';
        RAISE NOTICE '  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_app_log_summary;';
        -- 注意：REFRESH MATERIALIZED VIEW CONCURRENTLY不能在事务块中执行
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_app_log_summary 不存在';
...
-- 或者使用触发器自动刷新（简化版，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'logs') THEN
            RAISE EXCEPTION '表 logs 不存在，无法创建触发器函数';
        END IF;
        ...
        IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'refresh_log_summary') THEN
            DROP FUNCTION refresh_log_summary() CASCADE;
            RAISE NOTICE '函数 refresh_log_summary 已存在，先删除';
        END IF;
        ...
        BEGIN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '刷新日志汇总失败: %', SQLERRM;
            END;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '函数 refresh_log_summary 创建成功';

        -- 创建触发器（注意：这会影响插入性能）
        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_refresh_log_summary') THEN
            RAISE WARNING '触发器 trigger_refresh_log_summary 已存在';
        ELSE
            -- CREATE TRIGGER trigger_refresh_log_summary
            -- AFTER INSERT ON logs
            -- FOR EACH ROW
            -- EXECUTE FUNCTION refresh_log_summary();
            RAISE NOTICE '触发器命令已准备（注意：会影响插入性能，请谨慎使用）:';
            RAISE NOTICE '  CREATE TRIGGER trigger_refresh_log_summary';
            RAISE NOTICE '  AFTER INSERT ON logs';
            RAISE NOTICE '  FOR EACH ROW';
            RAISE NOTICE '  EXECUTE FUNCTION refresh_log_summary();';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表或物化视图不存在';
...
```sql
-- 查询应用日志汇总（使用物化视图，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_app_log_summary') THEN
            RAISE WARNING '物化视图 mv_app_log_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行应用日志汇总查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_events') THEN
            RAISE WARNING '表 user_events 已存在';
        ELSE
            ...
            RAISE NOTICE '表 user_events 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 user_events 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 user_events 失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_events') THEN
            RAISE EXCEPTION '表 user_events 不存在，无法创建物化视图';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_user_event_summary') THEN
            RAISE WARNING '物化视图 mv_user_event_summary 已存在';
        ELSE
            ...
            RAISE NOTICE '物化视图 mv_user_event_summary 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_user_event_summary') THEN
            RAISE EXCEPTION '物化视图 mv_user_event_summary 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'mv_user_event_summary' AND indexname = 'idx_mv_user_event_summary_unique') THEN
            RAISE WARNING '索引 idx_mv_user_event_summary_unique 已存在';
        ELSE
            CREATE UNIQUE INDEX idx_mv_user_event_summary_unique
            ON mv_user_event_summary (user_id, event_type, event_date);
            RAISE NOTICE '唯一索引 idx_mv_user_event_summary_unique 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_user_event_summary 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_mv_user_event_summary_unique 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建唯一索引失败: %', SQLERRM;
...
BEGIN
    ...
    BEGIN
        ...
        IF EXISTS (
            SELECT 1 FROM cron.job
            WHERE jobname = 'refresh-user-event-summary'
        ) THEN
            RAISE WARNING '定时任务 refresh-user-event-summary 已存在';
        ELSE
            RAISE NOTICE '定时任务命令已准备（请手动执行）:';
            RAISE NOTICE '  SELECT cron.schedule(';
            RAISE NOTICE '      ''refresh-user-event-summary'',';
            RAISE NOTICE '      ''*/5 * * * *'',';
            RAISE NOTICE '      $$REFRESH MATERIALIZED VIEW CONCURRENTLY mv_user_event_summary$$';
            RAISE NOTICE '  );';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
...
```sql
-- 查询用户行为汇总（使用物化视图，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_user_event_summary') THEN
            RAISE WARNING '物化视图 mv_user_event_summary 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行用户行为汇总查询性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
