...
```sql
-- 场景1：前缀列选择性极高（几乎每个值都不同，带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'high_selectivity') THEN
            DROP TABLE high_selectivity CASCADE;
            RAISE NOTICE '表 high_selectivity 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '表 high_selectivity 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 high_selectivity 失败: %', SQLERRM;
    END;

    BEGIN
        CREATE INDEX idx_high ON high_selectivity(unique_col, status, created_at);
        RAISE NOTICE '索引 idx_high 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 high_selectivity 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_high 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'high_selectivity') THEN
            RAISE WARNING '表 high_selectivity 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行跳过扫描测试（前缀列选择性极高）';
    EXCEPTION
        WHEN OTHERS THEN
...
-- 场景2：前缀列选择性极低（只有1-2个不同值，带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'low_selectivity') THEN
            DROP TABLE low_selectivity CASCADE;
            RAISE NOTICE '表 low_selectivity 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '表 low_selectivity 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 low_selectivity 失败: %', SQLERRM;
    END;

    BEGIN
        CREATE INDEX idx_low ON low_selectivity(region, status, created_at);
        RAISE NOTICE '索引 idx_low 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 low_selectivity 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_low 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建索引失败: %', SQLERRM;
    END;
END $$;
...
```sql
-- PostgreSQL 18：查看查询计划（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '必需的表不存在，无法查看查询计划';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看查询计划';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- PostgreSQL 18：查看代价模型参数（带错误处理和性能测试）
DO $$
DECLARE
    param_count INT;
BEGIN
    BEGIN
        ...
        IF param_count = 0 THEN
            RAISE WARNING '未找到代价模型参数';
        ELSE
            RAISE NOTICE '找到 % 个代价模型参数', param_count;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询代价模型参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '代价参数已设置（需要重启或reload配置）';
    ...
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限来设置系统参数';
        WHEN OTHERS THEN
...
```sql
-- PostgreSQL 18：查看查询重写（带错误处理和性能测试）
-- 1. 选择下推（带错误处理）
DO $$
BEGIN
    BEGIN
...
-- PostgreSQL 18：强制连接顺序（用于测试，带错误处理）
DO $$
BEGIN
    BEGIN
        SET join_collapse_limit = 1;
        RAISE NOTICE 'join_collapse_limit已设置为1（强制连接顺序）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置join_collapse_limit失败: %', SQLERRM;
    END;
END $$;
...
```sql
-- PostgreSQL 18：查看优化器搜索空间（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
...
-- PostgreSQL 18：限制优化器搜索（避免组合爆炸，带错误处理）
DO $$
BEGIN
    BEGIN
        SET geqo_threshold = 12;  -- 超过12个表使用遗传算法
        SET geqo_effort = 5;  -- 遗传算法努力程度（1-10）
        RAISE NOTICE '优化器搜索限制已设置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置优化器搜索限制失败: %', SQLERRM;
    END;
END $$;

-- PostgreSQL 18：查看优化器统计（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING 'pg_stat_statements扩展未安装，无法查看优化器统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看优化器统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    ...
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            ...
            RAISE NOTICE '表 order_items 已存在，先删除';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') THEN
            RAISE EXCEPTION '表 customers 不存在，无法创建索引';
        END IF;
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'customers' AND indexname = 'idx_customers_city') THEN
            RAISE WARNING '索引 idx_customers_city 已存在';
        ELSE
            CREATE INDEX idx_customers_city ON customers(city);
            RAISE NOTICE '索引 idx_customers_city 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 customers 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_customers_city 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建索引失败: %', SQLERRM;
    ...
    BEGIN
        ...
        END IF;
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_orders_customer_date') THEN
            RAISE WARNING '索引 idx_orders_customer_date 已存在';
        ELSE
            CREATE INDEX idx_orders_customer_date ON orders(customer_id, order_date);
            RAISE NOTICE '索引 idx_orders_customer_date 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            RAISE EXCEPTION '表 order_items 不存在，无法创建索引';
        END IF;
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'order_items' AND indexname = 'idx_order_items_order') THEN
    ...
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 order_items 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_order_items_order 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建索引失败: %', SQLERRM;
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE EXCEPTION '表 products 不存在，无法创建索引';
        END IF;
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'products' AND indexname = 'idx_products_category') THEN
            RAISE WARNING '索引 idx_products_category 已存在';
        ELSE
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_orders_multi') THEN
            RAISE WARNING '索引 idx_orders_multi 已存在';
        ELSE
            CREATE INDEX idx_orders_multi ON orders(region, status, order_date);
            RAISE NOTICE '索引 idx_orders_multi 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
-- 3. 插入测试数据（10个区域，每个区域10万订单，带错误处理）
DO $$
DECLARE
    insert_count BIGINT;
BEGIN
    BEGIN
        ...
        RAISE NOTICE '测试数据插入成功: % 行', insert_count;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN OTHERS THEN
...
-- 4. 查询不包含第一列（region），可以使用跳过扫描（带性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
...
-- 5. 对比：不使用跳过扫描（如果禁用，带错误处理）
DO $$
BEGIN
    BEGIN
        SET enable_indexskipscan = off;
        RAISE NOTICE 'enable_indexskipscan已禁用（用于对比测试）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置enable_indexskipscan失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_fact') THEN
            DROP TABLE sales_fact CASCADE;
            RAISE NOTICE '表 sales_fact 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '表 sales_fact 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 sales_fact 失败: %', SQLERRM;
    END;

    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'date_dim') THEN
            DROP TABLE date_dim CASCADE;
            RAISE NOTICE '表 date_dim 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '表 date_dim 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建表 date_dim 失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_fact') THEN
            RAISE EXCEPTION '表 sales_fact 不存在，无法创建索引';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'sales_fact' AND indexname = 'idx_sales_date') THEN
            RAISE WARNING '索引 idx_sales_date 已存在';
        ELSE
            CREATE INDEX idx_sales_date ON sales_fact(date_id);
            RAISE NOTICE '索引 idx_sales_date 创建成功';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'sales_fact' AND indexname = 'idx_sales_product') THEN
            RAISE WARNING '索引 idx_sales_product 已存在';
        ELSE
            CREATE INDEX idx_sales_product ON sales_fact(product_id);
            RAISE NOTICE '索引 idx_sales_product 创建成功';
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'sales_fact' AND indexname = 'idx_sales_customer') THEN
            RAISE WARNING '索引 idx_sales_customer 已存在';
        ELSE
            CREATE INDEX idx_sales_customer ON sales_fact(customer_id);
            RAISE NOTICE '索引 idx_sales_customer 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 sales_fact 不存在';
        WHEN duplicate_table THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') THEN
            RAISE WARNING '表 customers 不存在，跳过ANALYZE';
        ELSE
            ANALYZE customers;
            RAISE NOTICE 'customers表统计信息已更新';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新customers统计信息失败: %', SQLERRM;
    END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新orders统计信息失败: %', SQLERRM;
    END;
END $$;
