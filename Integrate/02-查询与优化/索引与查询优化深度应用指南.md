...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始测试普通索引查询（需要回表）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询测试准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 覆盖索引（不需要回表，PostgreSQL 11+，带错误处理）
DO $$
DECLARE
    pg_version INT;
BEGIN
    BEGIN
        ...
        IF pg_version < 110000 THEN
            RAISE EXCEPTION '覆盖索引（INCLUDE子句）需要PostgreSQL 11+，当前版本: %', version();
        END IF;
        ...
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION '列不存在: amount';
        END IF;

        IF EXISTS (
            SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_orders_covering'
        ) THEN
    ...
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN undefined_column THEN
            RAISE EXCEPTION '列不存在（请检查customer_id、order_date、order_id和amount列）';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_orders_covering 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始测试覆盖索引查询（Index Only Scan，不需要回表，性能提升30-50%）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询测试准备失败: %', SQLERRM;
            RAISE;
