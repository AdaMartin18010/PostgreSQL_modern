...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_active_orders') THEN
            RAISE WARNING '索引 idx_active_orders 已存在';
        ELSE
            CREATE INDEX idx_active_orders
            ON orders(order_date)
            WHERE status = 'active';
            RAISE NOTICE '部分索引 idx_active_orders 创建成功';
        END IF;
    EXCEPTION
...
```sql
-- 场景：大部分订单是历史订单，只有少量活跃订单（带错误处理）
-- 完整索引：索引所有订单，存储大，维护慢（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_all_orders') THEN
            RAISE WARNING '索引 idx_all_orders 已存在';
        ELSE
            CREATE INDEX idx_all_orders ON orders(order_date);
            RAISE NOTICE '完整索引 idx_all_orders 创建成功（存储大，维护慢）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_all_orders 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建完整索引失败: %', SQLERRM;
    END;
END $$;

-- 部分索引：只索引活跃订单，存储小，维护快（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        ...
        ELSE
            ...
            RAISE NOTICE '部分索引 idx_active_orders 创建成功（存储小，维护快）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN undefined_column THEN
            RAISE EXCEPTION '列 status 或 order_date 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_active_orders 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建部分索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE EXCEPTION '表 orders 不存在，无法创建部分索引';
        END IF;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_recent_orders') THEN
            RAISE WARNING '索引 idx_recent_orders 已存在';
        ELSE
            CREATE INDEX idx_recent_orders
            ON orders(customer_id, order_date)
            WHERE order_date >= CURRENT_DATE - INTERVAL '30 days';
            RAISE NOTICE '部分索引 idx_recent_orders 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_high_value_orders') THEN
            RAISE WARNING '索引 idx_high_value_orders 已存在';
        ELSE
            CREATE INDEX idx_high_value_orders
            ON orders(customer_id)
            WHERE total_amount > 1000;
            RAISE NOTICE '部分索引 idx_high_value_orders 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN undefined_column THEN
            RAISE EXCEPTION '列 total_amount 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_high_value_orders 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建部分索引失败: %', SQLERRM;
...
```sql
-- 查询1：可以使用部分索引（status = 'active'，带性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询1性能测试（可以使用部分索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询2性能测试（不能使用部分索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询3性能测试（条件蕴含，可以使用部分索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_active_orders_date') THEN
            RAISE WARNING '索引 idx_active_orders_date 已存在';
        ELSE
            CREATE INDEX idx_active_orders_date
            ON orders(order_date)
            WHERE status = 'active';
            RAISE NOTICE '部分索引 idx_active_orders_date 创建成功（减少索引大小90%）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_recent_orders_customer') THEN
            RAISE WARNING '索引 idx_recent_orders_customer 已存在';
        ELSE
            CREATE INDEX idx_recent_orders_customer
            ON orders(customer_id, order_date)
            WHERE order_date >= CURRENT_DATE - INTERVAL '30 days'
              AND status != 'cancelled';
            RAISE NOTICE '部分索引 idx_recent_orders_customer 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行活跃订单查询性能测试（使用部分索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行客户最近订单查询性能测试（使用部分索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
3. **验证索引使用**：
   ...
   ```sql
   -- 验证部分索引使用情况（带错误处理和性能测试）
   DO $$
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
               RAISE WARNING '表 orders 不存在，无法验证部分索引使用情况';
               RETURN;
           END IF;
           RAISE NOTICE '开始验证部分索引使用情况';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
       END;
...
2. **监控分区裁剪**：
   ...
   ```sql
   -- 查看执行计划验证约束排除（带错误处理和性能测试）
   DO $$
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
               RAISE WARNING '表 orders 不存在，无法验证约束排除';
               RETURN;
           END IF;
           RAISE NOTICE '开始验证约束排除';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
       END;
