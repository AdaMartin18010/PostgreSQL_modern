...
> **📋 文档来源**: `DataBaseTheory\05-索引与查询优化\05.11-并行查询调度-负载均衡与通信代价的最优化.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        -- 检查PostgreSQL版本（并行查询需要9.6+）
        SELECT current_setting('server_version_num')::INT INTO pg_version;
        IF pg_version < 90600 THEN
            RAISE EXCEPTION '并行查询需要PostgreSQL 9.6+，当前版本: %', version();
        END IF;
        ...
        RAISE NOTICE '并行查询参数设置成功（会话级别）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '设置并行查询参数失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行扫描';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行扫描性能测试';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') THEN
            RAISE WARNING '表 large_table1 不存在，无法执行并行连接';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '表 large_table2 不存在，无法执行并行连接';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行连接性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行分区表查询性能测试（自动负载均衡）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行分区表并行查询性能测试（自动负载均衡）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        ELSE
            ...
            RAISE NOTICE '部分索引 idx_orders_customer_date 创建成功（减少通信）';
        END IF;
    EXCEPTION
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询性能测试（优先使用本地索引）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
