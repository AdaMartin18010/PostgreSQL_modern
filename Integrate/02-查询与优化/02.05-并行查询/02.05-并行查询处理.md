...
```sql
-- 查看parallel_leader_participation设置
SHOW parallel_leader_participation;

-- 启用leader参与（PostgreSQL 18默认启用）
SET parallel_leader_participation = on;

-- 禁用leader参与（仅协调，不参与实际工作）
...
```sql
-- 1. 启用parallel_leader_participation（PostgreSQL 18）
SET parallel_leader_participation = on;
