...
-- 或者使用SHOW命令（简单方式，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看并行查询参数设置';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '并行维护参数已设置: max_parallel_maintenance_workers=4, parallel_tuple_cost=0.1, parallel_setup_cost=1000';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置并行维护参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行顺序扫描';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行顺序扫描性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 性能测试：并行索引扫描（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行索引扫描';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行索引扫描性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 性能测试：并行位图扫描（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行位图扫描';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行位图扫描性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 性能测试：并行分区扫描（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales') THEN
            RAISE WARNING '表 sales 不存在，无法执行并行分区扫描';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行分区扫描性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '必需的表不存在，无法执行并行哈希连接';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行哈希连接性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        SET enable_parallel_hash = on;
        RAISE NOTICE '已启用并行哈希连接';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '启用并行哈希连接失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '必需的表不存在，无法执行并行嵌套循环连接';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行嵌套循环连接性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '必需的表不存在，无法执行并行合并连接';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行合并连接性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行哈希聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行哈希聚合性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行分组聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行分组聚合性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行排序聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行排序聚合性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行窗口函数';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行并行窗口函数性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
```sql
-- 查看parallel_leader_participation设置（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看parallel_leader_participation设置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
-- 性能对比：启用leader参与（PostgreSQL 18，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行性能对比测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行性能对比测试（启用leader参与）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        SET parallel_leader_participation = off;
        RAISE NOTICE '已禁用leader参与（仅worker工作）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '禁用leader参与失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 并行查询I/O性能对比（带错误处理和性能测试）
-- 传统同步I/O
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行同步I/O性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行同步I/O性能测试';
    EXCEPTION
        WHEN OTHERS THEN
...
-- 异步I/O（PostgreSQL 18，带错误处理和性能测试）
-- 配置异步I/O后
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行异步I/O性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行异步I/O性能测试（PostgreSQL 18）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '测试准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法执行并行INSERT';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '并行INSERT完成，插入1000000行数据';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '并行INSERT失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '并行查询内存配置已设置: work_mem=256MB, maintenance_work_mem=1GB';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales') THEN
            RAISE WARNING '表 sales 不存在，无法执行大数据分析查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行大数据分析查询（并行查询）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        SET parallel_tuple_cost = 0.1;
        SET parallel_setup_cost = 1000;
        RAISE NOTICE '并行查询成本参数已设置: parallel_tuple_cost=0.1, parallel_setup_cost=1000';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置并行查询成本参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        SET max_parallel_maintenance_workers = 4;
        RAISE NOTICE '已设置 max_parallel_maintenance_workers = 4';

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'large_table' AND indexname = 'idx_large_table_brin') THEN
            CREATE INDEX CONCURRENTLY idx_large_table_brin
            ON large_table USING BRIN (value);
            RAISE NOTICE 'BRIN索引 idx_large_table_brin 创建成功（并行构建）';
        ELSE
            RAISE NOTICE '索引 idx_large_table_brin 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 large_table 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_large_table_brin 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '并行维护操作失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
- ⭐⭐⭐ [执行计划与性能调优](../02.03-执行计划/02.04-执行计划与性能调优.md) - 执行计划分析
- ⭐⭐ [统计信息与代价模型](../02.04-统计信息/02.03-统计信息与代价模型.md) - 统计信息对并行查询的影响
- ⭐⭐ [索引结构与优化](../02.02-索引结构/02.02-索引结构与优化.md) - 并行索引扫描优化
...
- ⭐ [监控与诊断](../../00-归档-项目管理文档/README.md) - 并行查询监控方法
