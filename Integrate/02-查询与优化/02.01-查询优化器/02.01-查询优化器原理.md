...
-- 查看优化器配置参数（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        RAISE NOTICE '开始查看优化器配置参数';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法查看执行计划代价';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看实际执行计划代价';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '已禁用哈希连接和合并连接，强制使用嵌套循环连接';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置连接算法参数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        SET work_mem = '256MB';
        RAISE NOTICE '已设置 work_mem = 256MB（影响哈希表大小）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置 work_mem 失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            RAISE WARNING '必需的表不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'idx_emp_dept') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'idx_dept_id') THEN
            CREATE INDEX idx_dept_id ON departments(dept_id);
            RAISE NOTICE '索引 idx_dept_id 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_dept_id 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '索引已存在';
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '连接算法配置完成';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '配置连接算法失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects') THEN
            RAISE WARNING '必需的表不存在，无法查看连接算法使用情况';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看连接算法使用情况';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看连接算法准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           ...
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'clients') THEN
            RAISE WARNING '必需的表不存在，无法演示多表连接';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示多表连接（动态规划算法）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多表连接准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- join_collapse_limit: 控制连接顺序优化的表数量（带错误处理）
-- 默认值：8（PostgreSQL 18）
-- 如果表数超过此值，优化器会使用遗传算法
DO $$
DECLARE
    current_join_limit TEXT;
    current_from_limit TEXT;
BEGIN
    BEGIN
        SET join_collapse_limit = 8;  -- 默认值
        SET from_collapse_limit = 8;   -- 默认值
        RAISE NOTICE '连接顺序优化参数已设置';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置连接顺序优化参数失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        SELECT setting INTO current_join_limit FROM pg_settings WHERE name = 'join_collapse_limit';
        SELECT setting INTO current_from_limit FROM pg_settings WHERE name = 'from_collapse_limit';
        RAISE NOTICE '当前配置: join_collapse_limit=%, from_collapse_limit=%', current_join_limit, current_from_limit;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询配置失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        SHOW join_collapse_limit INTO v_join_collapse_limit;
        SHOW from_collapse_limit INTO v_from_collapse_limit;
        RAISE NOTICE 'join_collapse_limit: %', v_join_collapse_limit;
        RAISE NOTICE 'from_collapse_limit: %', v_from_collapse_limit;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects') THEN
            RAISE WARNING '必需的表不存在，无法演示连接顺序优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示优化器自动选择最优顺序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '连接顺序优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 示例2：强制连接顺序（调试用，带错误处理）
DO $$
BEGIN
    BEGIN
        SET join_collapse_limit = 1;
        RAISE NOTICE '已设置 join_collapse_limit = 1（强制使用查询中的连接顺序）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置连接顺序限制失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- 1. 使用子查询控制连接顺序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            RAISE WARNING '必需的表不存在，无法演示子查询控制连接顺序';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示使用子查询控制连接顺序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '子查询控制连接顺序准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 2. 使用CTE优化连接顺序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            RAISE WARNING '必需的表不存在，无法演示CTE优化连接顺序';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示使用CTE优化连接顺序';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'CTE优化连接顺序准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    ...
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_emp_dept 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects') THEN
            RAISE WARNING '表 projects 不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'projects' AND indexname = 'idx_proj_manager') THEN
            CREATE INDEX idx_proj_manager ON projects(manager_id);
            RAISE NOTICE '索引 idx_proj_manager 创建成功';
        ELSE
            RAISE NOTICE '索引 idx_proj_manager 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_proj_manager 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
...
-- BRIN索引：大表范围查询，块级索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'table1') THEN
            RAISE WARNING '表 table1 不存在，无法创建BRIN索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'table1' AND indexname = 'idx_brin') THEN
            CREATE INDEX idx_brin ON table1 USING brin(column1);
            RAISE NOTICE 'BRIN索引 idx_brin 创建成功';
        ELSE
            RAISE NOTICE 'BRIN索引 idx_brin 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 table1 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_brin 已存在';
        WHEN OTHERS THEN
...
-- HNSW索引：向量相似度搜索（pgvector，带错误处理）
DO $$
BEGIN
    BEGIN
        -- 检查pgvector扩展是否安装
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
            RAISE WARNING 'pgvector扩展未安装，无法创建HNSW索引。请先安装：CREATE EXTENSION vector;';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'table1') THEN
            RAISE WARNING '表 table1 不存在，无法创建HNSW索引';
            RETURN;
        END IF;
        ...
        IF NOT EXISTS (
            ...
            RAISE WARNING '表 table1 没有vector类型的列，无法创建HNSW索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'table1' AND indexname = 'idx_hnsw') THEN
            CREATE INDEX idx_hnsw ON table1 USING hnsw(embedding vector_l2_ops);
            RAISE NOTICE 'HNSW索引 idx_hnsw 创建成功';
        ELSE
            RAISE NOTICE 'HNSW索引 idx_hnsw 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 table1 不存在';
        WHEN undefined_column THEN
            RAISE WARNING '列 embedding 不存在或类型不正确';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_hnsw 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行Index Only Scan性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Index Only Scan性能测试准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示Hash聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示Hash聚合';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Hash聚合准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'work_mem 已设置为 256MB';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示Sort聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示Sort聚合';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'Sort聚合准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示混合聚合';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示混合聚合';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '混合聚合准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法查看聚合统计';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看聚合统计';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查看聚合统计准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示排名函数';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示排名函数';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示聚合窗口函数';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示聚合窗口函数';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '聚合窗口函数准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示窗口帧函数';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示窗口帧函数';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '窗口帧函数准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 窗口函数可以利用索引排序（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示窗口函数索引优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示窗口函数索引优化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '窗口函数索引优化准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法创建物化视图';
            RETURN;
        END IF;
...
-- 3. 使用CTE优化复杂窗口函数（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行CTE优化窗口函数';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示使用CTE优化复杂窗口函数';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'CTE优化窗口函数准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- PostgreSQL 18查询计划器改进示例（带错误处理和性能测试）
-- 1. 更准确的基数估计
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects') THEN
            RAISE WARNING '必需的表不存在，无法演示PostgreSQL 18查询计划器改进';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示PostgreSQL 18查询计划器改进（更准确的基数估计）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询计划器改进演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'products' AND indexname = 'idx_products_final_price') THEN
            ...
            RAISE NOTICE '索引 idx_products_final_price 创建成功（虚拟生成列索引）';
        ELSE
...
-- 查询优化器利用虚拟生成列索引（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法演示虚拟生成列索引优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示查询优化器利用虚拟生成列索引';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '虚拟生成列索引优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- 大表JOIN优化（PostgreSQL 18，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '必需的表不存在，无法演示大表JOIN优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示大表JOIN优化（PostgreSQL 18）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '大表JOIN优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- 多表连接优化（PostgreSQL 18，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           ...
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'clients') THEN
            RAISE WARNING '必需的表不存在，无法演示多表连接优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示多表连接优化（PostgreSQL 18）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '多表连接优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示复合条件查询优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示复合条件查询优化（PostgreSQL 18）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '复合条件查询优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            ANALYZE employees;
            RAISE NOTICE '表 employees 统计信息已更新';
        ELSE
            RAISE WARNING '表 employees 不存在，无法更新统计信息';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新表 employees 统计信息失败: %', SQLERRM;
    END;

    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            ANALYZE departments;
            RAISE NOTICE '表 departments 统计信息已更新';
        ELSE
            RAISE WARNING '表 departments 不存在，无法更新统计信息';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '更新表 departments 统计信息失败: %', SQLERRM;
    END;
END $$;

-- 3. 创建合适的索引支持查询优化（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
...
-- 4. 监控查询计划质量（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法监控查询计划质量';
            RETURN;
        END IF;
        RAISE NOTICE '开始监控查询计划质量';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '监控查询计划准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table1') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table2') THEN
            RAISE WARNING '必需的表不存在，无法演示工作内存配置效果';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示工作内存配置效果';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '工作内存配置演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'maintenance_work_mem 已设置为 1GB';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置maintenance_work_mem失败: %', SQLERRM;
            RAISE;
    END;
    ...
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            ...
            RAISE NOTICE '表 large_table 已执行VACUUM ANALYZE';
        ELSE
            RAISE WARNING '表 large_table 不存在，无法执行VACUUM ANALYZE';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '执行VACUUM ANALYZE失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE 'effective_cache_size 已设置为 8GB';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置effective_cache_size失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示有效缓存大小效果';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示有效缓存大小效果';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '有效缓存大小演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
```sql
-- 使用提示控制优化器（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            RAISE WARNING '必需的表不存在，无法演示提示控制优化器';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示使用提示控制优化器';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '提示控制优化器演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法演示强制索引使用';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示强制索引使用';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '强制索引使用演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        SET enable_seqscan = off;
        RAISE NOTICE 'enable_seqscan 已设置为 off（强制索引扫描）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置enable_seqscan失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'departments') THEN
            RAISE WARNING '必需的表不存在，无法执行复杂分析查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行复杂分析查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '复杂分析查询准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法演示分页查询优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示分页查询优化';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '分页查询优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'large_table') THEN
            RAISE WARNING '表 large_table 不存在，无法使用游标优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示使用游标优化分页查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '游标优化准备失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        BEGIN;
        OPEN cur_ref FOR SELECT * FROM large_table ORDER BY id;
        FETCH 20 FROM cur_ref;
        FETCH 20 FROM cur_ref;
        CLOSE cur_ref;
        COMMIT;
        RAISE NOTICE '游标优化执行成功';
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE WARNING '游标优化执行失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
- [统计信息与查询调优](../02.04-统计信息/02.03-统计信息与代价模型.md) - 统计信息管理
...
- [执行计划与性能调优](../02.03-执行计划/02.04-执行计划与性能调优.md) - 性能分析实践
...
- ⭐⭐⭐ [统计信息与代价模型](../02.04-统计信息/02.03-统计信息与代价模型.md) - 统计信息对优化器的影响
- ⭐⭐ [索引结构与优化](../02.02-索引结构/02.02-索引结构与优化.md) - 索引选择优化
