...
> **📋 文档来源**: `DataBaseTheory\05-索引与查询优化\05.10-查询重写等价性-基于同构的充分必要条件.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'courses') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行等价重写查询性能测试（通过同构检测）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询优化器同构检测性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'courses') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行等价重写查询性能测试（连接顺序改变，但同构）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_order_summary') THEN
            ...
            RAISE NOTICE '物化视图 mv_order_summary 已存在，先删除';
        END IF;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询性能测试（通过同构检测匹配物化视图）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
