...
```sql
-- PostgreSQL查询规划器自动选择索引（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行查询性能测试（规划器自动选择索引）';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- 使用pg_stat_statements分析查询模式（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_customer_date') THEN
            RAISE WARNING '索引 idx_customer_date 已存在';
        ELSE
            CREATE INDEX idx_customer_date
            ON orders(customer_id, order_date);
            RAISE NOTICE '复合索引 idx_customer_date 创建成功（读多写少场景）';
        END IF;
    EXCEPTION
...
-- 目标: 最小化更新代价
-- 使用部分索引或减少索引数量
-- 注意：此场景建议减少索引数量，不创建额外索引

-- 场景3: 存储受限（最小化存储空间，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_customer') THEN
            RAISE WARNING '索引 idx_customer 已存在';
        ELSE
            CREATE INDEX idx_customer
            ON orders(customer_id)
            WHERE order_date > '2024-01-01';  -- 部分索引
            RAISE NOTICE '部分索引 idx_customer 创建成功（存储受限场景）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        ...
        ELSE
            CREATE INDEX idx_customer_date
            ON orders(customer_id, order_date);
            RAISE NOTICE '复合索引 idx_customer_date 创建成功（查询代价: 极低, 存储代价: 高, 更新代价: 高）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (
            ...
            RAISE WARNING '列 status 不存在，跳过部分索引创建';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'orders' AND indexname = 'idx_customer_active') THEN
            RAISE WARNING '索引 idx_customer_active 已存在';
        ELSE
            CREATE INDEX idx_customer_active
            ON orders(customer_id)
            WHERE status = 'active';
            RAISE NOTICE '部分索引 idx_customer_active 创建成功（查询代价: 低, 存储代价: 低, 更新代价: 低）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
        WHEN undefined_column THEN
            RAISE WARNING '列 status 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_customer_active 已存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建部分索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        ...
        ELSE
            CREATE INDEX idx_customer
            ON orders(customer_id);
            RAISE NOTICE '单列索引 idx_customer 创建成功（查询代价: 低, 存储代价: 中, 更新代价: 中）';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 orders 不存在';
