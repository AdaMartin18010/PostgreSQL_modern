...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行性能对比测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行窗口函数性能测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行自连接性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行自连接性能测试（性能较差）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '自连接性能测试准备失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行相关子查询性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行相关子查询性能测试（性能最差）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '相关子查询性能测试准备失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行性能对比测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行窗口函数性能测试';
    EXCEPTION
...
-- 自连接+子查询方案（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行自连接+子查询性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行自连接+子查询性能测试（性能较差）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '自连接+子查询性能测试准备失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行时间序列分析查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行时间序列分析窗口函数查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行自连接时间序列查询（性能较差）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行1800天数据测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行窗口函数1800天数据测试';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行Top-N查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行Top-N窗口函数查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
```sql
-- 方案2：使用DISTINCT ON（PostgreSQL特有，有限制，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'employees') THEN
            RAISE WARNING '表 employees 不存在，无法执行DISTINCT ON查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行DISTINCT ON查询（有限制）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_employees_dept_salary') THEN
            RAISE NOTICE '索引 idx_employees_dept_salary 已存在，跳过创建';
        ELSE
            CREATE INDEX idx_employees_dept_salary
            ON employees(department_id, salary DESC);
            RAISE NOTICE '成功创建索引 idx_employees_dept_salary';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_orders_customer_date') THEN
            RAISE NOTICE '索引 idx_orders_customer_date 已存在，跳过创建';
        ELSE
            CREATE INDEX idx_orders_customer_date
            ON orders(customer_id, order_date);
            RAISE NOTICE '成功创建索引 idx_orders_customer_date';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_employees_covering') THEN
            RAISE NOTICE '索引 idx_employees_covering 已存在，跳过创建';
        ELSE
            CREATE INDEX idx_employees_covering
            ON employees(department_id, salary DESC)
            INCLUDE (employee_id, name);
            RAISE NOTICE '成功创建覆盖索引 idx_employees_covering';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
