...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') THEN
            RAISE WARNING '表 customers 不存在，无法创建物化视图';
            RETURN;
        END IF;
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_orders') THEN
            DROP MATERIALIZED VIEW mv_customer_orders CASCADE;
            RAISE NOTICE '物化视图 mv_customer_orders 已存在，先删除';
        END IF;
    ...
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 customers 或 orders 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_orders') THEN
            RAISE WARNING '物化视图 mv_customer_orders 不存在，无法执行查询';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_orders') THEN
            RAISE WARNING '物化视图 mv_customer_orders 不存在';
            RETURN;
        END IF;
        RAISE NOTICE '开始评估物化视图大小';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') THEN
            RAISE WARNING '表 customers 不存在，无法执行原始查询';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法执行原始查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始评估查询性能提升';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_customer_order_summary') THEN
            DROP MATERIALIZED VIEW mv_customer_order_summary CASCADE;
            RAISE NOTICE '物化视图 mv_customer_order_summary 已存在，先删除';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法创建物化视图';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_items') THEN
            RAISE WARNING '表 order_items 不存在，无法创建物化视图';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_sales_summary') THEN
            DROP MATERIALIZED VIEW mv_product_sales_summary CASCADE;
            RAISE NOTICE '物化视图 mv_product_sales_summary 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '物化视图 mv_product_sales_summary 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '表 products 或 order_items 不存在';
        WHEN OTHERS THEN
            RAISE EXCEPTION '创建物化视图失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_order_date_summary') THEN
            DROP MATERIALIZED VIEW mv_order_date_summary CASCADE;
            RAISE NOTICE '物化视图 mv_order_date_summary 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '物化视图 mv_order_date_summary 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
