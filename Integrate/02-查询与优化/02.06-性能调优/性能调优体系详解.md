...
-- 4. 使用LIMIT（带错误处理和性能测试）
-- 优化前（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法演示LIMIT优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示优化前查询（返回所有数据，不推荐）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'LIMIT优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始演示优化后查询（使用LIMIT，推荐）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING 'LIMIT优化演示准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法演示查询优化';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示优化前查询（使用LIKE，不推荐）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询优化演示准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'products') THEN
            RAISE WARNING '表 products 不存在，无法演示优化后查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始演示优化后查询（使用全文搜索，推荐）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询优化演示准备失败: %', SQLERRM;
            RAISE;
...
-- 3. 建立调优流程（说明）
-- 步骤1：识别性能瓶颈（使用pg_stat_statements和EXPLAIN (ANALYZE, BUFFERS, TIMING)）
...
-- 2. 查询优化（带性能测试）
-- 使用EXPLAIN (ANALYZE, BUFFERS, TIMING)分析查询（带错误处理）
DO $$
BEGIN
    BEGIN
...
EXPLAIN (ANALYZE, BUFFERS, TIMING) SELECT * FROM orders WHERE created_at > '2024-01-01';
