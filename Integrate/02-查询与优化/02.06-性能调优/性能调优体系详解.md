...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_matviews WHERE schemaname = 'public' AND matviewname = 'mv_sales_summary') THEN
            RAISE EXCEPTION '物化视图 mv_sales_summary 不存在';
        END IF;

        REFRESH MATERIALIZED VIEW CONCURRENTLY mv_sales_summary;
        RAISE NOTICE '物化视图 mv_sales_summary 刷新成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '物化视图 mv_sales_summary 不存在';
        WHEN OTHERS THEN
