...
> **📋 文档来源**: `DataBaseTheory\05-索引与查询优化\05.16-数据库联邦查询模型-多数据源集成与查询分解的形式化.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'foreign_server') THEN
            ...
            RAISE NOTICE '外部服务器 foreign_server 已存在，先删除';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'foreign_server') THEN
            RAISE EXCEPTION '外部服务器 foreign_server 不存在，无法创建外部表';
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_orders') THEN
            DROP FOREIGN TABLE remote_orders CASCADE;
            RAISE NOTICE '外部表 remote_orders 已存在，先删除';
        END IF;
        ...
        RAISE NOTICE '外部表 remote_orders 创建成功';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE EXCEPTION '外部服务器 foreign_server 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_orders') THEN
            RAISE WARNING '外部表 remote_orders 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询外部表性能测试';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'file_server') THEN
            ...
            RAISE NOTICE '外部服务器file_server创建成功';
        ELSE
            RAISE NOTICE '外部服务器file_server已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'csv_orders') THEN
            ...
            RAISE NOTICE '外部表csv_orders创建成功';
        ELSE
            RAISE NOTICE '外部表csv_orders已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'csv_orders') THEN
            RAISE WARNING '外部表csv_orders不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询CSV文件';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'remote_db1') THEN
            CREATE SERVER remote_db1
            FOREIGN DATA WRAPPER postgres_fdw
            OPTIONS (host 'db1.example.com', dbname 'sales');
            RAISE NOTICE '外部服务器remote_db1创建成功';
        ELSE
            RAISE NOTICE '外部服务器remote_db1已存在';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'remote_db2') THEN
            CREATE SERVER remote_db2
            FOREIGN DATA WRAPPER postgres_fdw
            OPTIONS (host 'db2.example.com', dbname 'inventory');
            RAISE NOTICE '外部服务器remote_db2创建成功';
        ELSE
            RAISE NOTICE '外部服务器remote_db2已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建外部服务器失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_user_mappings WHERE srvname = 'remote_db1' AND usename = current_user::text) THEN
            CREATE USER MAPPING FOR current_user
            SERVER remote_db1
            OPTIONS (user 'sales_user', password 'password1');
            RAISE NOTICE '用户映射remote_db1创建成功';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM pg_user_mappings WHERE srvname = 'remote_db2' AND usename = current_user::text) THEN
            CREATE USER MAPPING FOR current_user
            SERVER remote_db2
            OPTIONS (user 'inventory_user', password 'password2');
            RAISE NOTICE '用户映射remote_db2创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_sales') THEN
            ...
            RAISE NOTICE '外部表remote_sales创建成功';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_inventory') THEN
            ...
            RAISE NOTICE '外部表remote_inventory创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建外部表失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_sales') THEN
            RAISE WARNING '外部表remote_sales不存在，无法执行跨数据源查询';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_inventory') THEN
            RAISE WARNING '外部表remote_inventory不存在，无法执行跨数据源查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行跨数据源查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_orders') THEN
            RAISE WARNING '外部表remote_orders不存在，无法查看查询计划';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看查询计划（查询下推）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'remote_orders') THEN
            RAISE WARNING '外部表remote_orders不存在，无法更新统计信息';
            RETURN;
        END IF;
        ANALYZE remote_orders;
        RAISE NOTICE '成功更新外部表统计信息';
    EXCEPTION
        WHEN OTHERS THEN
...
-- 2. 创建PostgreSQL外部服务器（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'sales_db') THEN
            CREATE SERVER sales_db
            FOREIGN DATA WRAPPER postgres_fdw
            OPTIONS (host 'sales.example.com', dbname 'sales');
            RAISE NOTICE '外部服务器sales_db创建成功';
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'inventory_db') THEN
            CREATE SERVER inventory_db
            FOREIGN DATA WRAPPER postgres_fdw
            OPTIONS (host 'inventory.example.com', dbname 'inventory');
            RAISE NOTICE '外部服务器inventory_db创建成功';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建外部服务器失败: %', SQLERRM;
...
-- 3. 创建MySQL外部服务器（使用mysql_fdw，带错误处理）
-- 注意：需要先安装mysql_fdw扩展
-- DO $$
-- BEGIN
--     BEGIN
--         IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'customer_db') THEN
--             CREATE SERVER customer_db
--             FOREIGN DATA WRAPPER mysql_fdw
--             OPTIONS (host 'customer.example.com', dbname 'customer');
--             RAISE NOTICE '外部服务器customer_db创建成功';
--         END IF;
--     EXCEPTION
--         WHEN OTHERS THEN
--             RAISE WARNING '创建MySQL外部服务器失败: %', SQLERRM;
--     END;
-- END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_user_mappings WHERE srvname = 'sales_db' AND usename = current_user::text) THEN
            CREATE USER MAPPING FOR current_user
            SERVER sales_db
            OPTIONS (user 'sales_user', password 'password1');
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM pg_user_mappings WHERE srvname = 'inventory_db' AND usename = current_user::text) THEN
            CREATE USER MAPPING FOR current_user
            SERVER inventory_db
            OPTIONS (user 'inventory_user', password 'password2');
        END IF;
...
WHERE s.sale_date >= '2024-01-01'
  AND s.amount > 1000
GROUP BY s.product_id, i.quantity
ORDER BY total_sales DESC
LIMIT 100;
```
