...
- [概率数据库实现](#概率数据库实现)
  ...
  - [1. 概述](#1-概述)
    - [1.1 技术背景](#11-技术背景)
      - [**1.1.1 数据不确定性的挑战**](#111-数据不确定性的挑战)
      - [**1.1.2 传统数据库的局限性**](#112-传统数据库的局限性)
      - [**1.1.3 概率数据库的解决方案**](#113-概率数据库的解决方案)
    - [1.2 技术定位](#12-技术定位)
      - [**1.2.1 技术定位**](#121-技术定位)
      - [**1.2.2 与其他技术的对比**](#122-与其他技术的对比)
    - [1.3 核心价值](#13-核心价值)
      - [**1.3.1 核心价值**](#131-核心价值)
      - [**1.3.2 应用场景**](#132-应用场景)
      - [**1.3.3 技术优势**](#133-技术优势)
  - [2. 技术原理](#2-技术原理)
    - [2.1 概率数据库基础](#21-概率数据库基础)
      - [**2.1.1 概率数据库定义**](#211-概率数据库定义)
      - [**2.1.2 核心概念**](#212-核心概念)
      - [**2.1.3 概率数据库分类**](#213-概率数据库分类)
    - [2.2 不确定性数据模型](#22-不确定性数据模型)
      - [**2.2.1 不确定性数据模型类型**](#221-不确定性数据模型类型)
      - [**2.2.2 不确定性来源**](#222-不确定性来源)
      - [**2.2.3 概率值表示方法**](#223-概率值表示方法)
    - [2.3 概率查询处理](#23-概率查询处理)
      - [**2.3.1 概率查询基础**](#231-概率查询基础)
      - [**2.3.2 概率查询类型**](#232-概率查询类型)
      - [**2.3.4 概率查询优化**](#234-概率查询优化)
  - [3. PostgreSQL实现方案](#3-postgresql实现方案)
    ...
    - [3.2 数据类型扩展](#32-数据类型扩展)
      - [**3.2.1 概率值类型**](#321-概率值类型)
      - [**3.2.2 概率分布类型**](#322-概率分布类型)
      - [**3.2.3 不确定性元组类型**](#323-不确定性元组类型)
      - [**3.2.4 类型操作符和函数**](#324-类型操作符和函数)
      - [**3.2.5 类型索引支持**](#325-类型索引支持)
      - [**3.2.6 类型使用最佳实践**](#326-类型使用最佳实践)
    - [3.3 查询处理扩展](#33-查询处理扩展)
      - [**3.3.1 概率查询操作符**](#331-概率查询操作符)
      - [**3.3.2 自定义操作符实现**](#332-自定义操作符实现)
      - [**3.3.3 自定义函数实现**](#333-自定义函数实现)
      - [**3.3.4 查询优化器扩展**](#334-查询优化器扩展)
      - [**3.3.5 概率查询执行计划**](#335-概率查询执行计划)
  - [4. ProvSQL集成](#4-provsql集成)
    ...
    - [4.2 安装和配置](#42-安装和配置)
      ...
      - [**4.2.3 验证安装**](#423-验证安装)
    - [4.3 集成架构](#43-集成架构)
      - [**4.3.1 整体架构设计**](#431-整体架构设计)
      - [**4.3.2 各层职责详解**](#432-各层职责详解)
      - [**4.3.3 数据流详细说明**](#433-数据流详细说明)
      - [**4.3.4 接口设计**](#434-接口设计)
      - [**4.3.5 集成优势**](#435-集成优势)
  ...
  - [5. 实际应用案例](#5-实际应用案例)
    - [5.1 传感器数据不确定性处理](#51-传感器数据不确定性处理)
      - [**5.1.1 业务场景**](#511-业务场景)
      - [**5.1.2 数据模型设计**](#512-数据模型设计)
      - [**5.1.3 数据插入和概率计算**](#513-数据插入和概率计算)
      - [**5.1.4 查询和分析**](#514-查询和分析)
      - [**5.1.5 使用示例**](#515-使用示例)
    - [5.2 数据融合场景](#52-数据融合场景)
      - [**5.2.1 业务场景**](#521-业务场景)
      - [**5.2.2 数据模型设计**](#522-数据模型设计)
      - [**5.2.3 数据融合实现**](#523-数据融合实现)
      - [**5.2.4 数据质量评估**](#524-数据质量评估)
  ...
  - [8. 参考资料](#8-参考资料)
    ...
    - [8.9 参考资源使用建议](#89-参考资源使用建议)
      - [**8.9.1 不同角色的学习路径**](#891-不同角色的学习路径)
      - [**8.9.2 学习资源优先级**](#892-学习资源优先级)
      - [**8.9.3 学习时间规划**](#893-学习时间规划)
      - [**8.9.4 学习建议**](#894-学习建议)
  - [9. 完整代码示例](#9-完整代码示例)
    - [9.1 概率数据类型实现](#91-概率数据类型实现)
      - [**9.1.1 创建概率数据类型**](#911-创建概率数据类型)
      - [**9.1.2 创建概率数据表**](#912-创建概率数据表)
      - [**9.1.3 插入概率数据**](#913-插入概率数据)
      - [**9.1.4 查询概率数据**](#914-查询概率数据)
      - [**9.1.5 概率操作函数**](#915-概率操作函数)
      - [**9.1.6 概率数据更新**](#916-概率数据更新)
      - [**9.1.7 概率数据验证**](#917-概率数据验证)
      - [**9.1.8 完整使用示例**](#918-完整使用示例)
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_readings') THEN
            ...
            RAISE NOTICE '元组级不确定性表 sensor_readings 创建成功';
        ELSE
            RAISE NOTICE '元组级不确定性表 sensor_readings 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '元组级不确定性表 sensor_readings 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建元组级不确定性表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '成功插入 % 条不确定性元组', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入不确定性元组失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'uncertain_sensor_data') THEN
            ...
            RAISE NOTICE '属性级不确定性表 uncertain_sensor_data 创建成功';
        ELSE
            RAISE NOTICE '属性级不确定性表 uncertain_sensor_data 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '属性级不确定性表 uncertain_sensor_data 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建属性级不确定性表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'uncertain_sensor_data') THEN
            RAISE WARNING '表 uncertain_sensor_data 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        INSERT INTO uncertain_sensor_data VALUES (
            1,
            1,
            '{"25.5": 0.4, "25.6": 0.3, "25.7": 0.3}'::JSONB,  -- 温度的概率分布
            '{"60": 0.5, "61": 0.3, "62": 0.2}'::JSONB         -- 湿度的概率分布
        ) ON CONFLICT DO NOTHING;

        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条属性级不确定性数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入属性级不确定性数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'possible_events') THEN
            ...
            RAISE NOTICE '存在不确定性表 possible_events 创建成功';
        ELSE
            RAISE NOTICE '存在不确定性表 possible_events 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '存在不确定性表 possible_events 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建存在不确定性表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'possible_events') THEN
            RAISE WARNING '表 possible_events 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条存在不确定性数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入存在不确定性数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_measurements') THEN
            ...
            RAISE NOTICE '测量不确定性表 sensor_measurements 创建成功';
            ...
            RAISE NOTICE '测量不确定性表索引创建成功';
        ELSE
            RAISE NOTICE '测量不确定性表 sensor_measurements 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '测量不确定性表 sensor_measurements 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建测量不确定性表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_measurements') THEN
            RAISE WARNING '表 sensor_measurements 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条测量数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入测量数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'multi_source_data') THEN
            ...
            RAISE NOTICE '多源数据表 multi_source_data 创建成功';
            ...
            RAISE NOTICE '多源数据表索引创建成功';
        ELSE
            RAISE NOTICE '多源数据表 multi_source_data 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '多源数据表 multi_source_data 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建多源数据表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'multi_source_data') THEN
            RAISE WARNING '表 multi_source_data 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条多源数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入多源数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inferred_data') THEN
            ...
            RAISE NOTICE '推断数据表 inferred_data 创建成功';
            ...
            RAISE NOTICE '推断数据表索引创建成功';
        ELSE
            RAISE NOTICE '推断数据表 inferred_data 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '推断数据表 inferred_data 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建推断数据表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'inferred_data') THEN
            RAISE WARNING '表 inferred_data 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条推断数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入推断数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'time_uncertain_data') THEN
            ...
            RAISE NOTICE '时间不确定性表 time_uncertain_data 创建成功';
            ...
            RAISE NOTICE '时间不确定性表索引创建成功';
        ELSE
            RAISE NOTICE '时间不确定性表 time_uncertain_data 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '时间不确定性表 time_uncertain_data 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建时间不确定性表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'comprehensive_uncertain_data') THEN
            ...
            RAISE NOTICE '综合不确定性处理表创建成功';
        ELSE
            RAISE NOTICE '综合不确定性处理表已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建综合不确定性处理表失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '计算综合置信度失败: %', SQLERRM;
            END;
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '综合置信度计算函数 calculate_overall_confidence 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建综合置信度计算函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_readings') OR
           NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_metadata') THEN
            RAISE WARNING '表 sensor_readings 或 sensor_metadata 不存在，无法执行概率连接查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行概率连接查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
SELECT
    ...
    r.temperature,
    s.location,
    -- 注意：PROBABILITY()是示例函数，实际实现需要根据扩展调整
...
```sql
-- 概率聚合：计算加权平均（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_data') THEN
            RAISE WARNING '表 sensor_data 不存在，无法执行概率聚合查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行概率聚合查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_probability') THEN
            CREATE INDEX idx_probability ON sensor_data(probability);
            RAISE NOTICE '基础概率值索引 idx_probability 创建成功';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_high_probability') THEN
            ...
            RAISE NOTICE '部分索引 idx_high_probability 创建成功';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_prob_expr') THEN
            ...
            RAISE NOTICE '表达式索引 idx_prob_expr 创建成功';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_sensor_prob') THEN
            ...
            RAISE NOTICE '复合索引 idx_sensor_prob 创建成功';
        END IF;
    EXCEPTION
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF table_name IS NULL OR table_name = '' THEN
                RAISE EXCEPTION 'table_name不能为NULL或空字符串';
            END IF;

            IF sample_size IS NULL OR sample_size <= 0 THEN
                RAISE EXCEPTION 'sample_size必须大于0';
            END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'query_result_cache') THEN
            ...
            RAISE NOTICE '查询结果缓存表创建成功';
        ELSE
            RAISE NOTICE '查询结果缓存表已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建查询结果缓存表失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_data') THEN
            RAISE WARNING '表 sensor_data 不存在，无法创建部分索引';
            RETURN;
        END IF;
...
-- 2. 创建物化视图（预计算常见查询，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_data') THEN
            RAISE WARNING '表 sensor_data 不存在，无法创建物化视图';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'probability_value') THEN
            ...
            RAISE NOTICE '类型 probability_value 创建成功';
        ELSE
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'probability') THEN
            RAISE EXCEPTION '类型 probability 不存在，请先创建该类型';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_measurements') THEN
            ...
            RAISE NOTICE '表 sensor_measurements 创建成功';
        ELSE
            RAISE NOTICE '表 sensor_measurements 已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_measurements') THEN
            RAISE EXCEPTION '表 sensor_measurements 不存在，无法插入数据';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'probability_distribution') THEN
            RAISE EXCEPTION '类型 probability_distribution 不存在，请先创建该类型';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'weather_forecast') THEN
            ...
            RAISE NOTICE '表 weather_forecast 创建成功';
        ELSE
            RAISE NOTICE '表 weather_forecast 已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'weather_forecast') THEN
            RAISE EXCEPTION '表 weather_forecast 不存在，无法插入数据';
        END IF;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'product_reviews') THEN
            ...
            RAISE NOTICE '表 product_reviews 创建成功';
        ELSE
            RAISE NOTICE '表 product_reviews 已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建表失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'product_reviews') THEN
            RAISE EXCEPTION '表 product_reviews 不存在，无法插入数据';
        END IF;
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '概率值比较失败: %', SQLERRM;
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '概率值比较函数创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建概率值比较函数失败: %', SQLERRM;
    END;
END $$;
    ...
    BEGIN
        ...
        BEGIN
            IF conf < 0 OR conf > 1 THEN
                RAISE EXCEPTION '置信度必须在[0, 1]范围内';
            END IF;
            RETURN (val, conf)::probability;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '类型转换失败: %', SQLERRM;
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        ...
        RAISE NOTICE '类型转换函数创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建类型转换函数失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '概率值验证函数创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建概率值验证函数失败: %', SQLERRM;
    END;
END $$;
...
```sql
-- 为概率值类型创建GIST索引（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_measurements') THEN
            RAISE WARNING '表 sensor_measurements 不存在，无法创建索引';
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'weather_forecast') THEN
            RAISE WARNING '表 weather_forecast 不存在，无法创建索引';
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'product_reviews') THEN
            RAISE WARNING '表 product_reviews 不存在，无法创建索引';
...
BEGIN
    BEGIN
        ...
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '概率值比较操作符创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建概率值比较操作符失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '概率值乘法计算失败: %', SQLERRM;
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '概率值乘法操作符创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建概率值乘法操作符失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'probability_value') THEN
            RAISE EXCEPTION '类型 probability_value 不存在，请先创建该类型';
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                RAISE EXCEPTION '提取概率值失败: %', SQLERRM;
        END;
        $$ LANGUAGE plpgsql IMMUTABLE;
        RAISE NOTICE '概率值提取函数创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建概率值提取函数失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        ALTER SYSTEM SET random_page_cost = 1.1;  -- 降低随机I/O成本
        ALTER SYSTEM SET cpu_tuple_cost = 0.01;   -- 降低CPU成本
        ALTER SYSTEM SET effective_cache_size = '4GB';  -- 设置缓存大小
        ALTER SYSTEM SET max_parallel_workers_per_gather = 4;
        ALTER SYSTEM SET parallel_setup_cost = 1000;
        ALTER SYSTEM SET parallel_tuple_cost = 0.1;

        SELECT pg_reload_conf();
        RAISE NOTICE '查询优化器参数配置成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '配置查询优化器参数失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'query_rewrite_rules') THEN
            ...
            RAISE NOTICE '查询重写规则表创建成功';
        ELSE
            RAISE NOTICE '查询重写规则表已存在';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建查询重写规则表失败: %', SQLERRM;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'query_rewrite_rules') THEN
            RAISE EXCEPTION '表 query_rewrite_rules 不存在，无法插入规则';
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensor_data') THEN
            RAISE WARNING '表 sensor_data 不存在，无法查看执行计划';
            RETURN;
        END IF;
        RAISE NOTICE '开始查看基础执行计划';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
    END;
...
5. **性能分析工具**
   ...
   - **EXPLAIN (ANALYZE, BUFFERS, TIMING)**: PostgreSQL执行计划分析
     - 功能: 查询执行计划分析
     - 用途: 查询优化
