...
-- 3. 性能趋势（每小时平均，带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'query_audit_log') THEN
            RAISE WARNING '表 query_audit_log 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询性能趋势（每小时平均）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'medical_records_encrypted') THEN
            CREATE TABLE medical_records_encrypted (
            ...
            RAISE NOTICE '医疗记录加密表 medical_records_encrypted 创建成功';
        ELSE
            RAISE NOTICE '医疗记录加密表 medical_records_encrypted 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '医疗记录加密表 medical_records_encrypted 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建医疗记录加密表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'medical_records_encrypted') THEN
            RAISE WARNING '表 medical_records_encrypted 不存在，无法创建索引';
            RETURN;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_medical_records_patient_id') THEN
            ...
            RAISE NOTICE '索引 idx_medical_records_patient_id 创建成功';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_medical_records_age_ope') THEN
            CREATE INDEX idx_medical_records_age_ope ON medical_records_encrypted(age_ope);
            RAISE NOTICE '索引 idx_medical_records_age_ope 创建成功';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_medical_records_department') THEN
            CREATE INDEX idx_medical_records_department ON medical_records_encrypted(department);
            RAISE NOTICE '索引 idx_medical_records_department 创建成功';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_medical_records_visit_date') THEN
            CREATE INDEX idx_medical_records_visit_date ON medical_records_encrypted(visit_date);
            RAISE NOTICE '索引 idx_medical_records_visit_date 创建成功';
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_medical_records_doctor_id') THEN
            CREATE INDEX idx_medical_records_doctor_id ON medical_records_encrypted(doctor_id);
            RAISE NOTICE '索引 idx_medical_records_doctor_id 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 medical_records_encrypted 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '部分索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'medical_access_control') THEN
            CREATE TABLE medical_access_control (
            ...
            RAISE NOTICE '医疗访问控制表 medical_access_control 创建成功';
        ELSE
            RAISE NOTICE '医疗访问控制表 medical_access_control 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '医疗访问控制表 medical_access_control 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建医疗访问控制表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
            ...
            RAISE NOTICE '租户管理表 tenants 创建成功';
        ELSE
            RAISE NOTICE '租户管理表 tenants 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '租户管理表 tenants 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建租户管理表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 多租户数据表（使用行级安全策略，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_data_encrypted') THEN
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
                RAISE WARNING '表 tenants 不存在，无法创建外键约束';
            END IF;

            CREATE TABLE tenant_data_encrypted (
                id SERIAL PRIMARY KEY,
            ...
            RAISE NOTICE '多租户数据表 tenant_data_encrypted 创建成功';
        ELSE
            RAISE NOTICE '多租户数据表 tenant_data_encrypted 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '多租户数据表 tenant_data_encrypted 已存在';
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保表 tenants 存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建多租户数据表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_data_encrypted') THEN
            RAISE WARNING '表 tenant_data_encrypted 不存在，无法创建索引';
            RETURN;
        END IF;
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_tenant_data_created_at') THEN
            CREATE INDEX idx_tenant_data_created_at ON tenant_data_encrypted(created_at);
            RAISE NOTICE '索引 idx_tenant_data_created_at 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 tenant_data_encrypted 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '部分索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
...
-- 行级安全策略（PostgreSQL RLS，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_data_encrypted') THEN
            RAISE WARNING '表 tenant_data_encrypted 不存在，无法启用RLS';
            RETURN;
        END IF;

        ALTER TABLE tenant_data_encrypted ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE '表 tenant_data_encrypted 已启用行级安全';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 tenant_data_encrypted 不存在';
        WHEN OTHERS THEN
...
-- 策略：租户只能访问自己的数据（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_data_encrypted') THEN
            RAISE WARNING '表 tenant_data_encrypted 不存在，无法创建策略';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'tenant_data_encrypted' AND policyname = 'tenant_isolation_policy') THEN
            DROP POLICY tenant_isolation_policy ON tenant_data_encrypted;
        END IF;

        CREATE POLICY tenant_isolation_policy ON tenant_data_encrypted
            FOR ALL
            USING (tenant_id = current_setting('app.current_tenant_id')::INTEGER);
        RAISE NOTICE '租户隔离策略创建成功';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenant_key_mapping') THEN
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
                RAISE WARNING '表 tenants 不存在，无法创建外键约束';
            END IF;

            CREATE TABLE tenant_key_mapping (
            ...
            RAISE NOTICE '租户密钥映射表 tenant_key_mapping 创建成功';
        ELSE
            RAISE NOTICE '租户密钥映射表 tenant_key_mapping 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '租户密钥映射表 tenant_key_mapping 已存在';
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保表 tenants 存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建租户密钥映射表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'performance_metrics') THEN
            RAISE WARNING '表 performance_metrics 不存在，无法创建视图';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '性能监控视图 performance_metrics_view 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 performance_metrics 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'performance_metrics_view') THEN
            RAISE WARNING '视图 performance_metrics_view 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询性能趋势';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
