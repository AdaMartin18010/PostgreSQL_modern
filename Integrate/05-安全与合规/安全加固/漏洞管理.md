# 漏洞管理

> **创建日期**: 2025年1月
> **技术版本**: PostgreSQL 17+/18+
> **难度等级**: ⭐⭐⭐ 中级

---

## 📋 目录

- [漏洞管理](#漏洞管理)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. 漏洞发现](#2-漏洞发现)
    - [2.1 版本检查](#21-版本检查)
    - [2.2 安全公告监控](#22-安全公告监控)
    - [2.3 安全扫描工具](#23-安全扫描工具)
  - [3. 漏洞评估](#3-漏洞评估)
    - [3.1 严重程度分类](#31-严重程度分类)
    - [3.2 风险评估](#32-风险评估)
  - [4. 漏洞修复](#4-漏洞修复)
    - [4.1 补丁管理](#41-补丁管理)
    - [4.2 升级PostgreSQL](#42-升级postgresql)
    - [4.3 临时缓解措施](#43-临时缓解措施)
  - [5. 漏洞跟踪](#5-漏洞跟踪)
    - [5.1 漏洞跟踪表](#51-漏洞跟踪表)
    - [5.2 跟踪流程](#52-跟踪流程)
  - [6. 最佳实践](#6-最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [❌ 避免做法](#-避免做法)
  - [📚 相关文档](#-相关文档)

---

## 1. 概述

漏洞管理是PostgreSQL安全的重要组成部分。包括：

- **漏洞发现** - 识别安全漏洞
- **漏洞评估** - 评估漏洞严重程度
- **漏洞修复** - 修复安全漏洞
- **漏洞跟踪** - 跟踪漏洞状态

---

## 2. 漏洞发现

### 2.1 版本检查

```sql
-- 检查PostgreSQL版本（带错误处理）
DO $$
DECLARE
    pg_version TEXT;
    ext_count INT;
BEGIN
    BEGIN
        SELECT version() INTO pg_version;
        RAISE NOTICE 'PostgreSQL版本: %', pg_version;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取PostgreSQL版本失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        SELECT COUNT(*) INTO ext_count FROM pg_extension;
        RAISE NOTICE '已安装扩展数量: %', ext_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '获取扩展数量失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 检查扩展版本（带错误处理和性能测试）
DO $$
DECLARE
    ext_record RECORD;
    total_exts INT := 0;
BEGIN
    BEGIN
        FOR ext_record IN
            SELECT extname, extversion
            FROM pg_extension
            ORDER BY extname
        LOOP
            total_exts := total_exts + 1;
            RAISE NOTICE '扩展: %, 版本: %', ext_record.extname, ext_record.extversion;
        END LOOP;

        RAISE NOTICE '共找到 % 个扩展', total_exts;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '检查扩展版本失败: %', SQLERRM;
            RAISE;
    END;
END $$;

EXPLAIN ANALYZE
SELECT extname, extversion
FROM pg_extension
ORDER BY extname;
```

### 2.2 安全公告监控

```bash
# 订阅PostgreSQL安全公告
# https://www.postgresql.org/support/security/

# 检查已知漏洞
# CVE数据库: https://cve.mitre.org/
```

### 2.3 安全扫描工具

```bash
# 使用安全扫描工具
# - pgAudit
# - pg_security
# - 第三方安全扫描工具
```

---

## 3. 漏洞评估

### 3.1 严重程度分类

| 级别 | 说明 | 响应时间 |
|------|------|---------|
| **严重** | 远程代码执行、数据泄露 | 24小时内 |
| **高** | 权限提升、拒绝服务 | 7天内 |
| **中** | 信息泄露、配置错误 | 30天内 |
| **低** | 信息泄露、配置建议 | 90天内 |

### 3.2 风险评估

```sql
-- 评估漏洞影响范围
-- 1. 受影响的数据
-- 2. 受影响的用户
-- 3. 业务影响
-- 4. 合规影响
```

---

## 4. 漏洞修复

### 4.1 补丁管理

```bash
#!/bin/bash
# 补丁管理脚本（带错误处理）
set -e
set -u

error_exit() {
    echo "错误: $1" >&2
    exit 1
}

# 检查pg_dump是否可用
if ! command -v pg_dump &> /dev/null; then
    error_exit "pg_dump命令未找到，请确保PostgreSQL客户端已安装"
fi

# 1. 备份数据库
DB_NAME="${DB_NAME:-mydb}"
BACKUP_FILE="backup_$(date +%Y%m%d_%H%M%S).dump"

if pg_dump -Fc "$DB_NAME" > "$BACKUP_FILE"; then
    echo "数据库备份成功: $BACKUP_FILE"
else
    error_exit "数据库备份失败"
fi

# 2. 测试补丁（在测试环境）
echo "请在测试环境中测试补丁"

# 3. 应用补丁
echo "应用补丁..."

# 4. 验证修复
echo "验证修复..."

# 5. 监控系统
echo "监控系统状态..."
```

### 4.2 升级PostgreSQL

```bash
# 小版本升级（如17.1 -> 17.2）
# 通常只需要重启

# 大版本升级（如16 -> 17）
# 需要使用pg_upgrade或逻辑复制
```

### 4.3 临时缓解措施

```sql
-- 如果无法立即修复，实施临时缓解措施（带错误处理，需要超级用户权限）
DO $$
DECLARE
    current_user_super BOOLEAN;
BEGIN
    BEGIN
        -- 检查是否有超级用户权限
        SELECT rolsuper INTO current_user_super
        FROM pg_roles
        WHERE rolname = current_user;

        IF NOT current_user_super THEN
            RAISE EXCEPTION '需要超级用户权限来修改系统配置';
        END IF;

        -- 禁用受影响的功能（示例：清空shared_preload_libraries）
        ALTER SYSTEM SET shared_preload_libraries = '';

        -- 重新加载配置
        PERFORM pg_reload_conf();

        RAISE NOTICE '临时缓解措施已实施，shared_preload_libraries已清空';
        RAISE WARNING '注意：这是临时措施，请尽快应用正式补丁';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限';
        WHEN OTHERS THEN
            RAISE WARNING '实施临时缓解措施失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 5. 漏洞跟踪

### 5.1 漏洞跟踪表

```sql
-- 创建漏洞跟踪表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'vulnerability_tracking') THEN
            RAISE NOTICE '表 vulnerability_tracking 已存在，跳过创建';
            RETURN;
        END IF;

        CREATE TABLE vulnerability_tracking (
            id SERIAL PRIMARY KEY,
            cve_id TEXT UNIQUE,
            description TEXT NOT NULL,
            severity TEXT NOT NULL CHECK (severity IN ('critical', 'high', 'medium', 'low')),
            affected_version TEXT NOT NULL,
            fixed_version TEXT,
            status TEXT NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'in_progress', 'fixed', 'mitigated')),
            discovered_at TIMESTAMPTZ DEFAULT NOW(),
            fixed_at TIMESTAMPTZ,
            notes TEXT,
            created_at TIMESTAMPTZ DEFAULT NOW()
        );

        RAISE NOTICE '漏洞跟踪表创建成功';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 vulnerability_tracking 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建漏洞跟踪表失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        -- 创建索引
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'vuln_tracking_status_idx') THEN
            CREATE INDEX vuln_tracking_status_idx ON vulnerability_tracking (status);
            RAISE NOTICE '索引 vuln_tracking_status_idx 创建成功';
        ELSE
            RAISE NOTICE '索引 vuln_tracking_status_idx 已存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'vuln_tracking_severity_idx') THEN
            CREATE INDEX vuln_tracking_severity_idx ON vulnerability_tracking (severity);
            RAISE NOTICE '索引 vuln_tracking_severity_idx 创建成功';
        ELSE
            RAISE NOTICE '索引 vuln_tracking_severity_idx 已存在';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'vuln_tracking_cve_id_idx') THEN
            CREATE INDEX vuln_tracking_cve_id_idx ON vulnerability_tracking (cve_id);
            RAISE NOTICE '索引 vuln_tracking_cve_id_idx 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 5.2 跟踪流程

```sql
-- 记录新漏洞（带错误处理）
DO $$
DECLARE
    inserted_id INT;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'vulnerability_tracking') THEN
            RAISE EXCEPTION '表 vulnerability_tracking 不存在，请先创建该表';
        END IF;

        INSERT INTO vulnerability_tracking (
            cve_id, description, severity, affected_version, status
        ) VALUES (
            'CVE-2024-XXXX',
            'PostgreSQL安全漏洞描述',
            'high',
            '17.0',
            'open'
        )
        ON CONFLICT (cve_id) DO UPDATE
        SET description = EXCLUDED.description,
            severity = EXCLUDED.severity,
            affected_version = EXCLUDED.affected_version
        RETURNING id INTO inserted_id;

        RAISE NOTICE '漏洞记录成功，记录ID: %', inserted_id;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 vulnerability_tracking 不存在';
            RAISE;
        WHEN check_violation THEN
            RAISE WARNING '数据验证失败，请检查severity或status的值';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '记录新漏洞失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 更新状态（带错误处理）
DO $$
DECLARE
    updated_count INT;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'vulnerability_tracking') THEN
            RAISE EXCEPTION '表 vulnerability_tracking 不存在，请先创建该表';
        END IF;

        UPDATE vulnerability_tracking
        SET status = 'fixed',
            fixed_at = NOW(),
            fixed_version = '17.1'
        WHERE cve_id = 'CVE-2024-XXXX';

        GET DIAGNOSTICS updated_count = ROW_COUNT;

        IF updated_count = 0 THEN
            RAISE WARNING '未找到CVE ID为 CVE-2024-XXXX 的漏洞记录';
        ELSE
            RAISE NOTICE '漏洞状态更新成功，影响行数: %', updated_count;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 vulnerability_tracking 不存在';
            RAISE;
        WHEN check_violation THEN
            RAISE WARNING '状态值无效，必须是: open, in_progress, fixed, mitigated';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '更新漏洞状态失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 查询漏洞状态（带性能测试）
EXPLAIN ANALYZE
SELECT cve_id, severity, status, affected_version, fixed_version
FROM vulnerability_tracking
WHERE status IN ('open', 'in_progress')
ORDER BY
    CASE severity
        WHEN 'critical' THEN 1
        WHEN 'high' THEN 2
        WHEN 'medium' THEN 3
        WHEN 'low' THEN 4
    END,
    discovered_at DESC;
```

---

## 6. 最佳实践

### ✅ 推荐做法

1. **定期检查** - 定期检查安全公告
2. **及时修复** - 根据严重程度及时修复
3. **测试补丁** - 在测试环境测试补丁
4. **跟踪漏洞** - 记录所有漏洞和修复状态
5. **监控系统** - 修复后监控系统状态

### ❌ 避免做法

1. **忽略漏洞** - 安全漏洞必须修复
2. **不测试补丁** - 可能导致系统问题
3. **不备份** - 修复前必须备份
4. **不跟踪** - 无法管理漏洞状态

---

## 📚 相关文档

- [PostgreSQL安全加固完整指南.md](./PostgreSQL安全加固完整指南.md) - 完整加固指南
- [安全加固清单.md](./安全加固清单.md) - 安全检查清单
- [05-安全与合规/README.md](../README.md) - 安全与合规主题

---

**最后更新**: 2025年1月
