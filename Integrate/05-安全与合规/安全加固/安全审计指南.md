# 安全审计指南

> **创建日期**: 2025年1月
> **技术版本**: PostgreSQL 17+/18+
> **难度等级**: ⭐⭐⭐ 中级

---

## 📋 目录

- [安全审计指南](#安全审计指南)
  - [📋 目录](#-目录)
  - [1. 概述](#1-概述)
  - [2. pgAudit配置](#2-pgaudit配置)
    - [2.1 安装和配置](#21-安装和配置)
    - [2.2 审计级别](#22-审计级别)
  - [3. 自定义审计](#3-自定义审计)
    - [3.1 创建审计表](#31-创建审计表)
    - [3.2 创建审计触发器](#32-创建审计触发器)
    - [3.3 应用审计触发器](#33-应用审计触发器)
  - [4. 审计日志分析](#4-审计日志分析)
    - [4.1 查询审计日志](#41-查询审计日志)
    - [4.2 统计分析](#42-统计分析)
  - [5. 合规审计](#5-合规审计)
    - [5.1 GDPR合规](#51-gdpr合规)
    - [5.2 数据保留策略](#52-数据保留策略)
  - [6. 最佳实践](#6-最佳实践)
    - [✅ 推荐做法](#-推荐做法)
    - [❌ 避免做法](#-避免做法)
  - [📚 相关文档](#-相关文档)

---

## 1. 概述

安全审计是PostgreSQL安全的重要组成部分。通过审计可以：

- 跟踪所有数据库操作
- 发现安全事件
- 满足合规要求
- 进行安全分析

---

## 2. pgAudit配置

### 2.1 安装和配置

```sql
-- 安装pgAudit扩展（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
            RAISE EXCEPTION '需要超级用户权限来安装扩展';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgaudit') THEN
            -- 注意：pgaudit需要在shared_preload_libraries中配置后才能创建扩展
            CREATE EXTENSION pgaudit;
            RAISE NOTICE 'pgAudit扩展安装成功';
        ELSE
            RAISE NOTICE 'pgAudit扩展已存在';
        END IF;
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'pgAudit扩展不可用，可能需要在shared_preload_libraries中配置';
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限来安装扩展';
        WHEN OTHERS THEN
            RAISE WARNING '安装pgAudit扩展失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 配置审计级别（带错误处理，需要超级用户权限）
DO $$
BEGIN
    BEGIN
        IF NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
            RAISE EXCEPTION '需要超级用户权限来修改系统配置';
        END IF;

        -- 配置审计级别（审计所有操作）
        ALTER SYSTEM SET pgaudit.log = 'all';

        -- 配置审计选项
        ALTER SYSTEM SET pgaudit.log_catalog = off;
        ALTER SYSTEM SET pgaudit.log_parameter = on;
        ALTER SYSTEM SET pgaudit.log_statement_once = on;

        -- 重新加载配置
        PERFORM pg_reload_conf();

        RAISE NOTICE 'pgAudit配置已设置并重新加载';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限';
        WHEN OTHERS THEN
            RAISE WARNING '配置pgAudit失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 注意：也可以配置为只审计DDL和写入操作
-- ALTER SYSTEM SET pgaudit.log = 'ddl,write';
```

### 2.2 审计级别

```sql
-- 审计所有操作
ALTER SYSTEM SET pgaudit.log = 'all';

-- 只审计DDL
ALTER SYSTEM SET pgaudit.log = 'ddl';

-- 只审计写入操作
ALTER SYSTEM SET pgaudit.log = 'write';

-- 组合审计
ALTER SYSTEM SET pgaudit.log = 'ddl,write';
```

---

## 3. 自定义审计

### 3.1 创建审计表

```sql
-- 创建审计日志表（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
            RAISE NOTICE '表 audit_log 已存在，跳过创建';
            RETURN;
        END IF;

        CREATE TABLE audit_log (
            id BIGSERIAL PRIMARY KEY,
            table_name TEXT NOT NULL,
            operation TEXT NOT NULL,
            user_name TEXT NOT NULL,
            old_data JSONB,
            new_data JSONB,
            changed_fields TEXT[],
            ip_address INET,
            application_name TEXT,
            timestamp TIMESTAMPTZ DEFAULT NOW()
        );

        RAISE NOTICE '审计日志表创建成功';
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 audit_log 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建审计日志表失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        -- 创建索引
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'audit_log_table_idx') THEN
            CREATE INDEX audit_log_table_idx ON audit_log (table_name);
            RAISE NOTICE '索引 audit_log_table_idx 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'audit_log_user_idx') THEN
            CREATE INDEX audit_log_user_idx ON audit_log (user_name);
            RAISE NOTICE '索引 audit_log_user_idx 创建成功';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'audit_log_timestamp_idx') THEN
            CREATE INDEX audit_log_timestamp_idx ON audit_log (timestamp);
            RAISE NOTICE '索引 audit_log_timestamp_idx 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE NOTICE '索引已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

### 3.2 创建审计触发器

```sql
-- 创建审计触发器函数（带错误处理）
CREATE OR REPLACE FUNCTION audit_trigger_function()
RETURNS TRIGGER AS $$
DECLARE
    old_data JSONB;
    new_data JSONB;
    changed_fields TEXT[];
BEGIN
    BEGIN
        -- 检查audit_log表是否存在
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
            RAISE EXCEPTION '审计日志表 audit_log 不存在，请先创建该表';
        END IF;

        IF TG_OP = 'DELETE' THEN
            old_data := to_jsonb(OLD);
            INSERT INTO audit_log (
                table_name, operation, user_name,
                old_data, ip_address, application_name
            ) VALUES (
                TG_TABLE_NAME, TG_OP, current_user,
                old_data, inet_client_addr(), current_setting('application_name', true)
            );
            RETURN OLD;
        ELSIF TG_OP = 'UPDATE' THEN
            old_data := to_jsonb(OLD);
            new_data := to_jsonb(NEW);

            -- 计算变更字段
            BEGIN
                SELECT array_agg(key)
                INTO changed_fields
                FROM jsonb_each(old_data)
                WHERE (old_data->>key) IS DISTINCT FROM (new_data->>key);
            EXCEPTION
                WHEN OTHERS THEN
                    changed_fields := NULL;
                    RAISE WARNING '计算变更字段失败: %', SQLERRM;
            END;

            INSERT INTO audit_log (
                table_name, operation, user_name,
                old_data, new_data, changed_fields,
                ip_address, application_name
            ) VALUES (
                TG_TABLE_NAME, TG_OP, current_user,
                old_data, new_data, changed_fields,
                inet_client_addr(), current_setting('application_name', true)
            );
            RETURN NEW;
        ELSIF TG_OP = 'INSERT' THEN
            new_data := to_jsonb(NEW);
            INSERT INTO audit_log (
                table_name, operation, user_name,
                new_data, ip_address, application_name
            ) VALUES (
                TG_TABLE_NAME, TG_OP, current_user,
                new_data, inet_client_addr(), current_setting('application_name', true)
            );
            RETURN NEW;
        ELSE
            RAISE EXCEPTION '不支持的触发器操作: %', TG_OP;
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE EXCEPTION '审计日志表不存在';
            RETURN NULL;
        WHEN OTHERS THEN
            RAISE WARNING '审计触发器函数执行失败: %', SQLERRM;
            -- 触发器函数不能抛出异常，否则会影响原操作，所以返回适当的值
            IF TG_OP = 'DELETE' THEN
                RETURN OLD;
            ELSE
                RETURN NEW;
            END IF;
    END;
END;
$$ LANGUAGE plpgsql;
```

### 3.3 应用审计触发器

```sql
-- 为表创建审计触发器（带错误处理）
DO $$
BEGIN
    BEGIN
        -- 检查函数是否存在
        IF NOT EXISTS (
            SELECT 1 FROM pg_proc p
            JOIN pg_namespace n ON p.pronamespace = n.oid
            WHERE n.nspname = 'public' AND p.proname = 'audit_trigger_function'
        ) THEN
            RAISE EXCEPTION '函数 audit_trigger_function 不存在，请先创建该函数';
        END IF;

        -- 检查表是否存在
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            RAISE WARNING '表 users 不存在，跳过创建触发器';
        ELSE
            -- 删除已存在的触发器（如果存在）
            DROP TRIGGER IF EXISTS audit_users ON users;

            -- 创建触发器
            CREATE TRIGGER audit_users
            AFTER INSERT OR UPDATE OR DELETE ON users
            FOR EACH ROW
            EXECUTE FUNCTION audit_trigger_function();

            RAISE NOTICE '表 users 的审计触发器创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_function THEN
            RAISE WARNING '函数 audit_trigger_function 不存在';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '为 users 表创建审计触发器失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，跳过创建触发器';
        ELSE
            -- 删除已存在的触发器（如果存在）
            DROP TRIGGER IF EXISTS audit_orders ON orders;

            -- 创建触发器
            CREATE TRIGGER audit_orders
            AFTER INSERT OR UPDATE OR DELETE ON orders
            FOR EACH ROW
            EXECUTE FUNCTION audit_trigger_function();

            RAISE NOTICE '表 orders 的审计触发器创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_function THEN
            RAISE WARNING '函数 audit_trigger_function 不存在';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '为 orders 表创建审计触发器失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 4. 审计日志分析

### 4.1 查询审计日志

```sql
-- 查看最近的审计记录（带错误处理和性能测试）
DO $$
DECLARE
    record_count INT;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
        RAISE WARNING '审计日志表 audit_log 不存在';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO record_count FROM audit_log;
    RAISE NOTICE '审计日志表中共有 % 条记录', record_count;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '查询审计日志失败: %', SQLERRM;
END $$;

EXPLAIN ANALYZE
SELECT * FROM audit_log
ORDER BY timestamp DESC
LIMIT 100;

-- 查看特定表的操作（带性能测试）
EXPLAIN ANALYZE
SELECT * FROM audit_log
WHERE table_name = 'users'
ORDER BY timestamp DESC;

-- 查看特定用户的操作（带性能测试）
EXPLAIN ANALYZE
SELECT * FROM audit_log
WHERE user_name = 'app_user'
ORDER BY timestamp DESC;
```

### 4.2 统计分析

```sql
-- 按操作类型统计（带错误处理和性能测试）
DO $$
DECLARE
    stat_result RECORD;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
        RAISE WARNING '审计日志表 audit_log 不存在';
        RETURN;
    END IF;

    RAISE NOTICE '按操作类型统计（最近24小时）:';
    FOR stat_result IN
        SELECT
            operation,
            COUNT(*) AS count
        FROM audit_log
        WHERE timestamp >= NOW() - INTERVAL '24 hours'
        GROUP BY operation
        ORDER BY count DESC
    LOOP
        RAISE NOTICE '操作: %, 次数: %', stat_result.operation, stat_result.count;
    END LOOP;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '统计操作类型失败: %', SQLERRM;
END $$;

EXPLAIN ANALYZE
SELECT
    operation,
    COUNT(*) AS count
FROM audit_log
WHERE timestamp >= NOW() - INTERVAL '24 hours'
GROUP BY operation
ORDER BY count DESC;

-- 按用户统计（带错误处理和性能测试）
DO $$
DECLARE
    user_stat RECORD;
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
        RAISE WARNING '审计日志表 audit_log 不存在';
        RETURN;
    END IF;

    RAISE NOTICE '按用户统计（最近24小时）:';
    FOR user_stat IN
        SELECT
            user_name,
            COUNT(*) AS count
        FROM audit_log
        WHERE timestamp >= NOW() - INTERVAL '24 hours'
        GROUP BY user_name
        ORDER BY count DESC
    LOOP
        RAISE NOTICE '用户: %, 次数: %', user_stat.user_name, user_stat.count;
    END LOOP;
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING '按用户统计失败: %', SQLERRM;
END $$;

EXPLAIN ANALYZE
SELECT
    user_name,
    COUNT(*) AS count
FROM audit_log
WHERE timestamp >= NOW() - INTERVAL '24 hours'
GROUP BY user_name
ORDER BY count DESC;
```

---

## 5. 合规审计

### 5.1 GDPR合规

```sql
-- 记录数据访问（带错误处理）
DO $$
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_access_log') THEN
            RAISE NOTICE '表 data_access_log 已存在，跳过创建';
        ELSE
            CREATE TABLE data_access_log (
                id BIGSERIAL PRIMARY KEY,
                user_name TEXT NOT NULL,
                table_name TEXT NOT NULL,
                record_id BIGINT,
                access_type TEXT CHECK (access_type IN ('read', 'export', 'delete', 'update')),
                timestamp TIMESTAMPTZ DEFAULT NOW()
            );

            CREATE INDEX idx_data_access_log_user ON data_access_log (user_name);
            CREATE INDEX idx_data_access_log_table ON data_access_log (table_name);
            CREATE INDEX idx_data_access_log_timestamp ON data_access_log (timestamp);

            RAISE NOTICE '数据访问日志表创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE NOTICE '表 data_access_log 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建数据访问日志表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 创建访问日志触发器（带错误处理）
CREATE OR REPLACE FUNCTION data_access_trigger()
RETURNS TRIGGER AS $$
BEGIN
    BEGIN
        -- 检查表是否存在
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_access_log') THEN
            RAISE EXCEPTION '数据访问日志表 data_access_log 不存在，请先创建该表';
        END IF;

        -- 确保NEW存在（对于INSERT和UPDATE操作）
        IF TG_OP IN ('INSERT', 'UPDATE') AND NEW IS NULL THEN
            RAISE EXCEPTION 'NEW记录为空，无法记录访问日志';
        END IF;

        INSERT INTO data_access_log (
            user_name, table_name, record_id, access_type
        ) VALUES (
            current_user, TG_TABLE_NAME,
            CASE WHEN TG_OP = 'DELETE' THEN OLD.id ELSE NEW.id END,
            CASE
                WHEN TG_OP = 'INSERT' THEN 'read'
                WHEN TG_OP = 'UPDATE' THEN 'update'
                WHEN TG_OP = 'DELETE' THEN 'delete'
                ELSE 'read'
            END
        );

        RETURN COALESCE(NEW, OLD);
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '数据访问日志表不存在';
            RETURN COALESCE(NEW, OLD);
        WHEN OTHERS THEN
            RAISE WARNING '记录数据访问日志失败: %', SQLERRM;
            RETURN COALESCE(NEW, OLD);
    END;
END;
$$ LANGUAGE plpgsql;
```

### 5.2 数据保留策略

```sql
-- 自动清理旧审计日志（带错误处理）
CREATE OR REPLACE FUNCTION cleanup_audit_log()
RETURNS VOID AS $$
DECLARE
    deleted_count INT;
    retention_period INTERVAL := INTERVAL '7 years';  -- GDPR要求
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log') THEN
            RAISE EXCEPTION '审计日志表 audit_log 不存在';
        END IF;

        -- 删除旧记录
        DELETE FROM audit_log
        WHERE timestamp < NOW() - retention_period;

        GET DIAGNOSTICS deleted_count = ROW_COUNT;

        RAISE NOTICE '清理完成，删除了 % 条超过 % 的审计记录', deleted_count, retention_period;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '审计日志表不存在';
            RAISE;
        WHEN OTHERS THEN
            RAISE WARNING '清理审计日志失败: %', SQLERRM;
            RAISE;
    END;
END;
$$ LANGUAGE plpgsql;

-- 定期执行（使用pg_cron，带错误处理）
DO $$
DECLARE
    extension_exists BOOLEAN;
    job_id INT;
BEGIN
    -- 检查pg_cron扩展是否已安装
    SELECT EXISTS (
        SELECT 1 FROM pg_extension WHERE extname = 'pg_cron'
    ) INTO extension_exists;

    IF NOT extension_exists THEN
        BEGIN
            CREATE EXTENSION IF NOT EXISTS pg_cron;
            RAISE NOTICE 'pg_cron扩展安装成功';
        EXCEPTION
            WHEN insufficient_privilege THEN
                RAISE EXCEPTION '需要超级用户权限来安装pg_cron扩展';
            WHEN OTHERS THEN
                RAISE WARNING '安装pg_cron扩展失败: %', SQLERRM;
                RAISE;
        END;
    ELSE
        RAISE NOTICE 'pg_cron扩展已存在';
    END IF;

    -- 删除已存在的任务（如果存在）
    BEGIN
        PERFORM cron.unschedule('cleanup_audit') WHERE EXISTS (
            SELECT 1 FROM cron.job WHERE jobname = 'cleanup_audit'
        );
    EXCEPTION
        WHEN OTHERS THEN
            NULL;  -- 忽略错误
    END;

    -- 创建定时任务
    BEGIN
        SELECT cron.schedule('cleanup_audit', '0 3 * * *', 'SELECT cleanup_audit_log()') INTO job_id;
        RAISE NOTICE '定时清理任务创建成功，任务ID: %', job_id;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建定时任务失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```

---

## 6. 最佳实践

### ✅ 推荐做法

1. **启用pgAudit** - 使用标准审计扩展
2. **自定义审计** - 为关键表创建自定义审计
3. **定期分析** - 定期分析审计日志
4. **数据保留** - 根据合规要求保留审计日志
5. **监控异常** - 设置告警监控异常操作

### ❌ 避免做法

1. **不启用审计** - 无法追踪安全事件
2. **审计过多** - 影响性能
3. **不分析日志** - 审计日志需要分析才有价值
4. **不保留日志** - 无法满足合规要求

---

## 📚 相关文档

- [PostgreSQL安全加固完整指南.md](./PostgreSQL安全加固完整指南.md) - 完整加固指南
- [安全加固清单.md](./安全加固清单.md) - 安全检查清单
- [05-安全与合规/README.md](../README.md) - 安全与合规主题

---

**最后更新**: 2025年1月
