...
> **📋 文档来源**: `PostgreSQL_View\05-合规与可信\审计与脱敏\不可篡改审计日志.md`
> **📅 复制日期**: 2025-12-22
...
BEGIN
    BEGIN
        ...
        ELSE
            ...
            RAISE NOTICE '表 user_data_ledger 已创建';
    ...
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '表 user_data_ledger 已存在';
        WHEN OTHERS THEN
...
BEGIN
    ...
    IF sequence_number IS NULL OR sequence_number < 0 THEN
        RAISE EXCEPTION 'sequence_number不能为NULL或负数';
    END IF;

    IF operation IS NULL OR operation = '' THEN
        RAISE EXCEPTION 'operation不能为NULL或空字符串';
    END IF;
...
BEGIN
    ...
    IF p_operation IS NULL OR p_operation NOT IN ('INSERT', 'UPDATE', 'DELETE') THEN
        RAISE EXCEPTION 'operation必须为INSERT、UPDATE或DELETE';
    END IF;
...
BEGIN
        ...
        LOOP
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    -- 如果计算哈希失败，将该记录标记为无效
                    invalid_ids := array_append(invalid_ids, record.id);
                    RAISE WARNING '验证记录ID=%时出错: %', record.id, SQLERRM;
            END;
        END LOOP;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '验证完整性失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data_ledger') THEN
            RAISE WARNING '表 user_data_ledger 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始性能测试：验证完整性查询';
    EXCEPTION
        WHEN OTHERS THEN
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM user_data_ledger ORDER BY sequence_number LIMIT 1000;
```
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'verify_audit_integrity') THEN
            RAISE WARNING '函数 verify_audit_integrity 不存在，请先创建';
            RETURN;
        END IF;

        -- 检查是否已存在相同的调度任务
        IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'verify-audit-integrity') THEN
            RAISE NOTICE '调度任务 verify-audit-integrity 已存在';
        ELSE
            ...
            RAISE NOTICE '调度任务 verify-audit-integrity 已创建';
        END IF;
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'pg_cron扩展文件不存在，请确保已正确安装';
        WHEN insufficient_privilege THEN
            RAISE WARNING '权限不足，无法创建扩展或调度任务';
        WHEN OTHERS THEN
            RAISE WARNING '设置定时验证失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    -- 检查函数是否存在
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'verify_audit_integrity') THEN
        RAISE EXCEPTION '函数 verify_audit_integrity 不存在，请先创建';
    END IF;
    ...
    BEGIN
        ...
        IF NOT FOUND THEN
            RAISE WARNING 'verify_audit_integrity函数未返回结果';
            RETURN;
        END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '检查审计完整性告警失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;
```
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log_backup') THEN
        RAISE EXCEPTION '备份表 audit_log_backup 不存在';
    END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE '已备份 % 条审计记录', backup_count;

        -- 验证备份完整性
        PERFORM verify_audit_integrity();
        RAISE NOTICE '备份完整性验证通过';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION '备份审计日志失败: %', SQLERRM;
    END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    BEGIN
        ...
        ELSE
            ...
            RAISE NOTICE '调度任务 backup-audit-log 已创建';
        END IF;
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'pg_cron扩展文件不存在';
        WHEN insufficient_privilege THEN
            RAISE WARNING '权限不足';
        WHEN OTHERS THEN
            RAISE WARNING '设置定时备份失败: %', SQLERRM;
            RAISE;
    END;
END $$;
);
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data_ledger') THEN
            RAISE WARNING '表 user_data_ledger 不存在，无法启用行级安全';
            RETURN;
        END IF;

        ALTER TABLE user_data_ledger ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE '表 user_data_ledger 已启用行级安全';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data_ledger 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'compliance_officer') THEN
            RAISE WARNING '角色 compliance_officer 不存在，请先创建';
            RETURN;
        END IF;
        ...
        IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_data_ledger' AND policyname = 'audit_log_access') THEN
            ...
            RAISE NOTICE '已删除现有策略 audit_log_access';
        END IF;
        ...
        USING (TRUE);
        RAISE NOTICE '策略 audit_log_access 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data_ledger 不存在';
        WHEN undefined_object THEN
            RAISE WARNING '角色 compliance_officer 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_data_ledger' AND policyname = 'audit_log_no_write') THEN
            ...
            RAISE NOTICE '已删除现有策略 audit_log_no_write';
        END IF;
        ...
        USING (FALSE)
        WITH CHECK (FALSE);
        RAISE NOTICE '策略 audit_log_no_write 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data_ledger 不存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data_ledger') THEN
            RAISE WARNING '表 user_data_ledger 不存在，无法创建视图';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'audit_log_readonly') THEN
            RAISE NOTICE '视图 audit_log_readonly 已存在';
        ELSE
            CREATE VIEW audit_log_readonly AS
            SELECT * FROM user_data_ledger;
            RAISE NOTICE '视图 audit_log_readonly 已创建';
        END IF;

        -- 授予只读权限
        IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'data_analyst') THEN
            GRANT SELECT ON audit_log_readonly TO data_analyst;
            RAISE NOTICE '已授予data_analyst角色SELECT权限';
        ELSE
            RAISE WARNING '角色 data_analyst 不存在，无法授予权限';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data_ledger 不存在';
        WHEN duplicate_object THEN
            RAISE WARNING '视图 audit_log_readonly 已存在';
        WHEN undefined_object THEN
            RAISE WARNING '角色 data_analyst 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建视图或授予权限失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log_partitioned') THEN
            RAISE NOTICE '分区表 audit_log_partitioned 已存在';
        ELSE
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data_ledger') THEN
                RAISE WARNING '表 user_data_ledger 不存在，无法创建分区表';
                RETURN;
            END IF;
            ...
            RAISE NOTICE '分区表 audit_log_partitioned 已创建';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '分区表 audit_log_partitioned 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_log_partitioned') THEN
            RAISE WARNING '分区表 audit_log_partitioned 不存在，无法创建索引';
            RETURN;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'audit_log_partitioned' AND indexname = 'audit_log_partitioned_table_name_timestamp_idx') THEN
            ...
            RAISE NOTICE '索引 audit_log_partitioned_table_name_timestamp_idx 已创建';
        ELSE
            RAISE NOTICE '索引 audit_log_partitioned_table_name_timestamp_idx 已存在';
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'audit_log_partitioned' AND indexname = 'audit_log_partitioned_user_id_timestamp_idx') THEN
            ...
            RAISE NOTICE '索引 audit_log_partitioned_user_id_timestamp_idx 已创建';
        ELSE
            RAISE NOTICE '索引 audit_log_partitioned_user_id_timestamp_idx 已存在';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '分区表 audit_log_partitioned 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引已存在';
        WHEN OTHERS THEN
