...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'user_data_restricted') THEN
            RAISE WARNING '视图 user_data_restricted 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始性能测试：列权限视图查询性能';
    EXCEPTION
        WHEN OTHERS THEN
...
SELECT * FROM user_data_restricted WHERE id = 1;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.views WHERE table_schema = 'public' AND table_name = 'user_data_combined_access') THEN
            RAISE WARNING '视图 user_data_combined_access 不存在，无法执行性能测试';
            RETURN;
        END IF;
        RAISE NOTICE '开始性能测试：组合权限视图查询性能';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '性能测试准备失败: %', SQLERRM;
    END;
...
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT * FROM user_data_combined_access WHERE id = 1;
```
