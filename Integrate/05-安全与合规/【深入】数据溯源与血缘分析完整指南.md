...
- [数据溯源与血缘分析完整指南](#数据溯源与血缘分析完整指南)
  ...
  - [二、原理与理论](#二原理与理论)
    - [2.1 数据溯源理论](#21-数据溯源理论)
      - [**溯源的三种类型**](#溯源的三种类型)
      - [1. Why-Provenance（为什么溯源）](#1-why-provenance为什么溯源)
      - [2. Where-Provenance（哪里溯源）](#2-where-provenance哪里溯源)
      - [3. How-Provenance（如何溯源）](#3-how-provenance如何溯源)
    - [2.2 溯源模型](#22-溯源模型)
...
-- 1. 查询报表数据的血缘关系（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'data_lineage') THEN
            RAISE WARNING '表 data_lineage 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询报表数据的血缘关系';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- GDPR合规：追踪个人数据处理（带错误处理和性能测试）
-- 查询某个用户的数据处理轨迹
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_entities') THEN
            RAISE WARNING '表 provenance_entities 不存在，无法执行查询';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_activities') THEN
            RAISE WARNING '表 provenance_activities 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询用户数据处理轨迹';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
```sql
-- 查询表之间的依赖关系（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'table_dependencies') THEN
            RAISE WARNING '表 table_dependencies 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询表之间的依赖关系';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'column_lineage') THEN
            ...
            RAISE NOTICE '字段血缘表 column_lineage 创建成功';
        ELSE
            RAISE NOTICE '字段血缘表 column_lineage 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '字段血缘表 column_lineage 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建字段血缘表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 示例：sales_report.total_amount的血缘（带错误处理）
DO $$
DECLARE
    inserted_count INT;
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'column_lineage') THEN
            RAISE WARNING '表 column_lineage 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '成功插入 % 条字段血缘数据', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入字段血缘数据失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'row_provenance') THEN
            ...
            RAISE NOTICE '行级溯源表 row_provenance 创建成功';
        ELSE
            RAISE NOTICE '行级溯源表 row_provenance 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '行级溯源表 row_provenance 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建行级溯源表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'provsql') THEN
            ...
            RAISE NOTICE 'ProvSQL扩展安装成功';
        ELSE
            RAISE NOTICE 'ProvSQL扩展已存在';
        END IF;
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'ProvSQL扩展不可用，需要先安装';
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限来安装扩展';
        WHEN OTHERS THEN
            RAISE WARNING '安装ProvSQL扩展失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
            RAISE WARNING '表 users 不存在，无法启用溯源';
        ELSE
            PERFORM provsql.add_provenance('users');
            RAISE NOTICE '表 users 已启用溯源';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'orders') THEN
            RAISE WARNING '表 orders 不存在，无法启用溯源';
        ELSE
            PERFORM provsql.add_provenance('orders');
            RAISE NOTICE '表 orders 已启用溯源';
        END IF;
    EXCEPTION
        WHEN undefined_function THEN
            RAISE WARNING 'provsql.add_provenance函数不存在，请确保ProvSQL扩展已安装';
        WHEN OTHERS THEN
            RAISE WARNING '启用溯源失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 执行查询并获取溯源（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'results') THEN
            RAISE WARNING '表 results 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询溯源信息';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
```mermaid
flowchart TD
    A[数据源<br/>Data Sources] --> B[数据采集层<br/>Collection Layer]
    B --> C[溯源数据存储<br/>Provenance Storage]
    C --> D[血缘追踪引擎<br/>Lineage Engine]
    D --> E[分析处理层<br/>Analysis Layer]
    E --> F[可视化展示<br/>Visualization]

    A --> A1[PostgreSQL<br/>数据库]
    A --> A2[ETL工具<br/>ETL Tools]
    A --> A3[应用系统<br/>Applications]

    B --> B1[SQL解析器<br/>SQL Parser]
    B --> B2[变更捕获<br/>Change Capture]
    B --> B3[日志分析<br/>Log Analysis]

    C --> C1[PROV-DM模型<br/>PROV-DM Model]
    C --> C2[血缘关系表<br/>Lineage Tables]
    C --> C3[元数据存储<br/>Metadata Storage]

    D --> D1[正向追踪<br/>Forward Tracking]
    D --> D2[反向追踪<br/>Backward Tracking]
    D --> D3[影响分析<br/>Impact Analysis]

    E --> E1[血缘查询<br/>Lineage Query]
    E --> E2[依赖分析<br/>Dependency Analysis]
    E --> E3[影响评估<br/>Impact Assessment]

    F --> F1[血缘图谱<br/>Lineage Graph]
    F --> F2[依赖树<br/>Dependency Tree]
    F --> F3[影响报告<br/>Impact Report]
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_entities') THEN
            ...
            RAISE NOTICE '实体表 provenance_entities 创建成功';
        ELSE
            RAISE NOTICE '实体表 provenance_entities 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '实体表 provenance_entities 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建实体表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_activities') THEN
            ...
            RAISE NOTICE '活动表 provenance_activities 创建成功';
        ELSE
            RAISE NOTICE '活动表 provenance_activities 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '活动表 provenance_activities 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建活动表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_agents') THEN
            ...
            RAISE NOTICE '代理表 provenance_agents 创建成功';
        ELSE
            RAISE NOTICE '代理表 provenance_agents 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '代理表 provenance_agents 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建代理表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_activities') THEN
            RAISE NOTICE '表 provenance_activities 已存在，跳过创建';
        ELSE
            CREATE TABLE provenance_activities (
            ...
            RAISE NOTICE '分区表 provenance_activities 创建成功';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '分区表 provenance_activities 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'provenance_activities') THEN
            RAISE WARNING '分区表 provenance_activities 不存在，无法创建分区';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_inherits WHERE inhparent = 'provenance_activities'::regclass AND inhrelid = 'provenance_activities_2024_01'::regclass) THEN
            CREATE TABLE provenance_activities_2024_01 PARTITION OF provenance_activities
                FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');
            RAISE NOTICE '分区 provenance_activities_2024_01 创建成功';
        ELSE
            RAISE NOTICE '分区 provenance_activities_2024_01 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '分区 provenance_activities_2024_01 已存在';
        WHEN OTHERS THEN
