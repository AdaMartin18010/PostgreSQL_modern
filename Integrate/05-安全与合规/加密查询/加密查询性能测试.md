  ...
  BEGIN
      BEGIN
          IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
              ...
              RAISE NOTICE '测试数据表 test_data 创建成功';
          ELSE
              RAISE NOTICE '测试数据表 test_data 已存在';
          END IF;
      EXCEPTION
          WHEN duplicate_table THEN
              RAISE WARNING '测试数据表 test_data 已存在';
          WHEN OTHERS THEN
              RAISE WARNING '创建测试数据表失败: %', SQLERRM;
              RAISE;
      END;
  END $$;
  ```
...
1. **SELECT查询**：
   ...
   BEGIN
       BEGIN
           IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
               RAISE WARNING '表 test_data 不存在，无法执行查询';
               RETURN;
           END IF;
           RAISE NOTICE '开始执行等值查询';
       EXCEPTION
           WHEN OTHERS THEN
   ...
   BEGIN
       BEGIN
           ...
           IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
               RAISE WARNING '表 accounts 不存在，无法执行JOIN查询';
               RETURN;
           END IF;
           RAISE NOTICE '开始执行JOIN查询';
       EXCEPTION
           WHEN OTHERS THEN
               RAISE WARNING '查询准备失败: %', SQLERRM;
               RAISE;
     ...
     BEGIN
         BEGIN
             IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
                 ...
                 RAISE NOTICE '混合加密测试数据表 test_data 创建成功';
             ELSE
                 RAISE NOTICE '混合加密测试数据表 test_data 已存在';
             END IF;
         EXCEPTION
             WHEN duplicate_table THEN
                 RAISE WARNING '混合加密测试数据表 test_data 已存在';
             WHEN OTHERS THEN
                 RAISE WARNING '创建混合加密测试数据表失败: %', SQLERRM;
                 RAISE;
         END;
     END $$;
     ```
...
2. **索引优化**：
   ...
   - **实施**：
     ...
     BEGIN
         BEGIN
             IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
                 RAISE WARNING '表 test_data 不存在，无法创建索引';
                 RETURN;
             END IF;

             IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_account_id_hash') THEN
                 CREATE INDEX idx_account_id_hash ON test_data
                 USING hash(account_id);
                 RAISE NOTICE '哈希索引 idx_account_id_hash 创建成功';
             END IF;
         EXCEPTION
             WHEN undefined_table THEN
                 RAISE WARNING '表 test_data 不存在';
             WHEN duplicate_table THEN
                 RAISE WARNING '索引 idx_account_id_hash 已存在';
             WHEN OTHERS THEN
...
3. **混合方案**：
   ...
   - **实施**：
     ...
     ```sql
     -- 等值查询使用确定性加密，聚合查询使用Paillier加密（带错误处理）
     DO $$
     BEGIN
         BEGIN
             IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
                 ...
                 RAISE NOTICE '同态加密测试数据表 test_data 创建成功';
             ELSE
                 RAISE NOTICE '同态加密测试数据表 test_data 已存在';
             END IF;
         EXCEPTION
             WHEN duplicate_table THEN
                 RAISE WARNING '同态加密测试数据表 test_data 已存在';
             WHEN OTHERS THEN
                 RAISE WARNING '创建同态加密测试数据表失败: %', SQLERRM;
                 RAISE;
         END;
     END $$;
     ```
...
4. **查询优化**：
   ...
   - **实施**：
     ...
     BEGIN
         BEGIN
             IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'test_data') THEN
                 RAISE WARNING '表 test_data 不存在，无法创建物化视图';
                 RETURN;
             END IF;
