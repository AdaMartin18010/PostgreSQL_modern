...
- [混合加密查询原理](#混合加密查询原理)
  ...
  - [2. 技术原理](#2-技术原理)
    ...
    - [2.4 性能优化策略](#24-性能优化策略)
      - [1. 索引优化（Index Optimization）](#1-索引优化index-optimization)
      - [2. 缓存优化（Cache Optimization）](#2-缓存优化cache-optimization)
      - [3. 批量处理（Batch Processing）](#3-批量处理batch-processing)
      - [4. 硬件加速（Hardware Acceleration）](#4-硬件加速hardware-acceleration)
  - [3. 架构设计](#3-架构设计)
    - [3.1 整体架构](#31-整体架构)
    - [3.2 加密层设计](#32-加密层设计)
      - [1. 加密引擎（Encryption Engine）](#1-加密引擎encryption-engine)
      - [2. 密钥管理器（Key Manager）](#2-密钥管理器key-manager)
      - [3. 加密策略（Encryption Policy）](#3-加密策略encryption-policy)
    - [3.3 查询处理层](#33-查询处理层)
      - [1. 查询重写器（Query Rewriter）](#1-查询重写器query-rewriter)
      - [2. 查询优化器（Query Optimizer）](#2-查询优化器query-optimizer)
      - [3. 结果处理器（Result Processor）](#3-结果处理器result-processor)
    - [3.4 密钥管理](#34-密钥管理)
      - [1. 密钥存储（Key Storage）](#1-密钥存储key-storage)
      - [2. 密钥轮换（Key Rotation）](#2-密钥轮换key-rotation)
      - [3. 密钥访问控制（Key Access Control）](#3-密钥访问控制key-access-control)
  - [4. 实现细节](#4-实现细节)
  ...
  - [5. 性能分析](#5-性能分析)
    ...
    - [5.2 不同加密方案性能对比](#52-不同加密方案性能对比)
      - [1. AES-128加密](#1-aes-128加密)
      - [2. AES-256加密](#2-aes-256加密)
      - [3. 可搜索加密（Searchable Encryption）](#3-可搜索加密searchable-encryption)
      - [4. 同态加密（Homomorphic Encryption）](#4-同态加密homomorphic-encryption)
    - [5.3 优化效果](#53-优化效果)
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_email_encrypted_hash') THEN
            CREATE INDEX idx_email_encrypted_hash
            ON users_encrypted USING hash(email_encrypted);
            RAISE NOTICE '哈希索引 idx_email_encrypted_hash 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 users_encrypted 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_email_encrypted_hash 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users_encrypted') THEN
            RAISE WARNING '表 users_encrypted 不存在，无法执行查询';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行等值查询（可以使用索引，性能提升50-70%）';
    EXCEPTION
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_amount_encrypted_ope') THEN
            CREATE INDEX idx_amount_encrypted_ope
            ON transactions USING btree(amount_encrypted_ope);
            RAISE NOTICE 'OPE索引 idx_amount_encrypted_ope 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 transactions 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_amount_encrypted_ope 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建索引失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
            ...
            RETURN;
        END IF;
        RAISE NOTICE '开始执行范围查询（可以使用索引，性能提升40-60%）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_active_users_encrypted') THEN
            CREATE INDEX idx_active_users_encrypted
            ON users_encrypted(email_encrypted)
            WHERE is_active = TRUE;
            RAISE NOTICE '部分索引 idx_active_users_encrypted 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 users_encrypted 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_active_users_encrypted 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'decryption_cache') THEN
            ...
            RAISE NOTICE '解密缓存表 decryption_cache 创建成功';
        ELSE
            RAISE NOTICE '解密缓存表 decryption_cache 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '解密缓存表 decryption_cache 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建解密缓存表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF encrypted_value IS NULL THEN
                RAISE EXCEPTION 'encrypted_value不能为NULL';
            END IF;

            IF encryption_key IS NULL OR encryption_key = '' THEN
                RAISE EXCEPTION 'encryption_key不能为NULL或空字符串';
            END IF;
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '缓存解密函数 decrypt_with_cache 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建缓存解密函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF values IS NULL OR array_length(values, 1) IS NULL THEN
                RAISE EXCEPTION 'values数组不能为NULL或空';
            END IF;
            ...
            LOOP
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        RAISE WARNING '加密值 % 失败: %', val, SQLERRM;
                        -- 继续处理其他值
                END;
            END LOOP;
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '批量加密函数 encrypt_batch 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建批量加密函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            -- 参数验证
            IF encrypted_values IS NULL OR array_length(encrypted_values, 1) IS NULL THEN
                RAISE EXCEPTION 'encrypted_values数组不能为NULL或空';
            END IF;
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '批量解密函数 decrypt_batch 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建批量解密函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            ...
            IF algorithm IS NULL OR algorithm = '' THEN
                RAISE EXCEPTION 'algorithm不能为NULL或空字符串';
            END IF;
            ...
            IF algorithm = 'AES-256-CBC' THEN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        RAISE EXCEPTION 'AES-256-CBC加密失败: %', SQLERRM;
                END;
            ELSIF algorithm = 'AES-256-GCM' THEN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        RAISE EXCEPTION 'AES-256-GCM加密失败: %', SQLERRM;
                END;
            ELSE
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '加密引擎函数 encrypt_data 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建加密引擎函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'encryption_keys') THEN
            ...
            RAISE NOTICE '密钥管理表 encryption_keys 创建成功';
        ELSE
            RAISE NOTICE '密钥管理表 encryption_keys 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '密钥管理表 encryption_keys 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建密钥管理表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
-- 获取加密策略
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '加密策略获取函数 get_encryption_policy 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建加密策略获取函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'kms_keys') THEN
            ...
            RAISE NOTICE 'KMS密钥管理表 kms_keys 创建成功';
        ELSE
            RAISE NOTICE 'KMS密钥管理表 kms_keys 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING 'KMS密钥管理表 kms_keys 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建KMS密钥管理表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- KMS密钥获取函数（需要外部函数支持，带错误处理）
DO $$
BEGIN
    BEGIN
        -- 检查plpython3u扩展
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'plpython3u') THEN
            RAISE WARNING 'plpython3u扩展未安装，无法创建KMS密钥获取函数';
            RETURN;
        END IF;
        ...
        $$ LANGUAGE plpython3u;
        RAISE NOTICE 'KMS密钥获取函数 get_kms_key 创建成功';
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'plpython3u扩展不可用，需要先安装';
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限来安装扩展';
        WHEN OTHERS THEN
            RAISE WARNING '创建KMS密钥获取函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF p_table_name IS NULL OR p_table_name = '' THEN
                RAISE EXCEPTION 'p_table_name不能为NULL或空字符串';
            END IF;

            IF p_column_name IS NULL OR p_column_name = '' THEN
                RAISE EXCEPTION 'p_column_name不能为NULL或空字符串';
            END IF;

            IF p_old_key_id IS NULL OR p_old_key_id = '' THEN
                RAISE EXCEPTION 'p_old_key_id不能为NULL或空字符串';
            END IF;

            IF p_new_key_id IS NULL OR p_new_key_id = '' THEN
                RAISE EXCEPTION 'p_new_key_id不能为NULL或空字符串';
            END IF;
            ...
            -- 检查函数是否存在
            IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_active_key') THEN
                RAISE EXCEPTION '函数 get_active_key 不存在';
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '获取旧密钥失败: %', SQLERRM;
            END;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '获取新密钥失败: %', SQLERRM;
            END;
            ...
            LOOP
                ...
                BEGIN
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            RAISE WARNING '解密记录ID % 失败: %', v_record.id, SQLERRM;
                            CONTINUE;
                    END;
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            RAISE WARNING '更新记录ID % 失败: %', v_record.id, SQLERRM;
                            CONTINUE;
                    END;
                END;
            END LOOP;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE WARNING '更新密钥配置失败: %', SQLERRM;
            END;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '密钥轮换函数 rotate_encryption_key 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建密钥轮换函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF p_key_id IS NULL OR p_key_id = '' THEN
                RAISE EXCEPTION 'p_key_id不能为NULL或空字符串';
            END IF;

            IF p_permission IS NULL OR p_permission = '' THEN
                RAISE EXCEPTION 'p_permission不能为NULL或空字符串';
            END IF;

            -- 检查表是否存在
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'key_access_control') THEN
                RAISE EXCEPTION '表 key_access_control 不存在';
            END IF;
        ...
        END;
        $$ LANGUAGE plpgsql SECURITY DEFINER;
        RAISE NOTICE '密钥访问控制函数 check_key_access 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建密钥访问控制函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            ...
            -- 检查函数是否存在
            IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'check_key_access') THEN
                RAISE WARNING '函数 check_key_access 不存在，跳过访问控制检查';
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '获取密钥ID失败: %', SQLERRM;
            END;

            -- 检查访问权限
            IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'check_key_access') THEN
                BEGIN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        RAISE EXCEPTION '检查访问权限失败: %', SQLERRM;
                END;
            END IF;
            ...
            BEGIN
                ...
                IF v_key_data IS NULL THEN
                    RAISE EXCEPTION '密钥数据为空: %', v_key_id;
                END IF;
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '获取密钥数据失败: %', SQLERRM;
            END;

            RETURN v_key_data;
        END;
        $$ LANGUAGE plpgsql SECURITY DEFINER;
        RAISE NOTICE '密钥获取函数 get_active_key 创建成功（带访问控制）';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建密钥获取函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users_encrypted') THEN
            ...
            RAISE NOTICE '加密表 users_encrypted 创建成功';
        ELSE
            RAISE NOTICE '加密表 users_encrypted 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '加密表 users_encrypted 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建加密表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users_encrypted') THEN
            RAISE WARNING '表 users_encrypted 不存在，无法插入数据';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'encrypt_data') THEN
            RAISE WARNING '函数 encrypt_data 不存在';
            RETURN;
        END IF;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users_encrypted') THEN
            ...
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'decrypt_data') THEN
            RAISE WARNING '函数 decrypt_data 不存在';
            RETURN;
        END IF;
        RAISE NOTICE '开始执行重写后的加密查询';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_name_encrypted_search') THEN
            CREATE INDEX idx_name_encrypted_search
            ON users_encrypted
            USING hash(encrypt_data('John Doe', 'encryption_key'));
            RAISE NOTICE '可搜索加密索引 idx_name_encrypted_search 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 users_encrypted 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 encrypt_data 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_name_encrypted_search 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建可搜索加密索引失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users_encrypted') THEN
            RAISE WARNING '表 users_encrypted 不存在，无法创建视图';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'decrypt_data') THEN
            RAISE WARNING '函数 decrypt_data 不存在';
        ...
        SELECT
            ...
            created_at
        FROM users_encrypted;
        RAISE NOTICE '自动解密视图 users_decrypted 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 users_encrypted 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 decrypt_data 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建自动解密视图失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
