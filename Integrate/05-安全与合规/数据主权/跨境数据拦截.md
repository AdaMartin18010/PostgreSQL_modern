...
```sql
-- 创建拦截规则索引
CREATE INDEX IF NOT EXISTS idx_cross_border_rules_source_target
ON cross_border_rules USING GIN (source_sovereignty, target_sovereignty);

CREATE INDEX IF NOT EXISTS idx_cross_border_rules_priority
ON cross_border_rules (priority DESC);
