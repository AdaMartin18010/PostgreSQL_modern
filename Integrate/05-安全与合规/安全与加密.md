...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'readonly_user') THEN
            ...
            RAISE NOTICE '用户 readonly_user 创建成功';
        ELSE
            RAISE NOTICE '用户 readonly_user 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE WARNING '用户 readonly_user 已存在';
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        GRANT SELECT ON ALL TABLES IN SCHEMA public TO readonly_user;
        RAISE NOTICE '权限授予成功: readonly_user';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '数据库或Schema不存在';
        WHEN OTHERS THEN
            RAISE WARNING '授予权限失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_user') THEN
            ...
            RAISE WARNING '用户 app_user 创建成功（但权限过大，不推荐）';
        ELSE
    ...
    BEGIN
        GRANT ALL PRIVILEGES ON DATABASE mydb TO app_user;  -- 权限过大，不推荐
        RAISE WARNING '已授予所有权限（不推荐，权限过大）';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '数据库不存在';
        WHEN OTHERS THEN
            RAISE WARNING '授予权限失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    ...
    BEGIN
        ...
        GRANT SELECT ON ALL TABLES IN SCHEMA public TO app_readonly;
        RAISE NOTICE '基础权限授予成功: app_readonly';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '数据库或Schema不存在';
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        RAISE NOTICE '权限授予成功: app_readwrite';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '角色或表不存在';
        WHEN OTHERS THEN
            RAISE WARNING '授予权限失败: %', SQLERRM;
            RAISE;
    END;
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'user1') THEN
            ...
            RAISE NOTICE '用户 user1 创建成功';
        ELSE
            RAISE NOTICE '用户 user1 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE WARNING '用户 user1 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建用户失败: %', SQLERRM;
            RAISE;
    END;
    ...
    BEGIN
        ...
        RAISE NOTICE '角色分配成功: user1 -> app_readonly';
    EXCEPTION
        WHEN undefined_object THEN
            RAISE WARNING '用户或角色不存在';
        WHEN OTHERS THEN
            RAISE WARNING '分配角色失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
```sql
-- ✅ 好：使用RLS实现细粒度访问控制（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sensitive_data') THEN
            RAISE WARNING '表 sensitive_data 不存在，无法启用RLS';
            RETURN;
        END IF;
    ...
    BEGIN
        ...
        RAISE NOTICE 'RLS策略 user_data_policy 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 sensitive_data 不存在';
        WHEN OTHERS THEN
    ...
    BEGIN
        ...
        RAISE NOTICE '策略函数 current_user_id 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 users 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建策略函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
```sql
-- ✅ 好：定期审查用户权限（带错误处理和性能测试）
DO $$
DECLARE
    role_count INT;
BEGIN
    BEGIN
        SELECT COUNT(*) INTO role_count FROM pg_roles WHERE rolname NOT LIKE 'pg_%';
        RAISE NOTICE '找到 % 个用户角色', role_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询用户角色失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '密码加密方式已设置为: scram-sha-256';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '设置密码加密方式失败: %', SQLERRM;
            RAISE;
    END;

    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'secure_user') THEN
            CREATE USER secure_user WITH PASSWORD 'StrongP@ssw0rd!2024';
            RAISE NOTICE '用户 secure_user 创建成功（使用强密码）';
        ELSE
            RAISE NOTICE '用户 secure_user 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE WARNING '用户 secure_user 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建用户失败: %', SQLERRM;
            RAISE;
    END;
...
SELECT name, setting, unit, short_desc
FROM pg_settings
WHERE name IN ('ssl_cert_file', 'ssl_key_file');
```
...
SELECT name, setting, unit, short_desc
FROM pg_settings
WHERE name IN ('log_directory', 'log_filename');
```
