...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data') THEN
            ...
            RAISE NOTICE '带主权标签的表 user_data 创建成功';
        ELSE
            RAISE NOTICE '带主权标签的表 user_data 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '带主权标签的表 user_data 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建带主权标签的表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'current_user_sovereignty') THEN
            RAISE WARNING '函数 current_user_sovereignty 不存在，请先创建';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_data' AND policyname = 'sovereignty_policy') THEN
            DROP POLICY sovereignty_policy ON user_data;
            RAISE NOTICE '已删除现有策略 sovereignty_policy';
        END IF;
        ...
        RAISE NOTICE '主权访问策略 sovereignty_policy 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 current_user_sovereignty 不存在';
        WHEN duplicate_object THEN
            RAISE WARNING '策略 sovereignty_policy 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建主权访问策略失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sovereignty_tags') THEN
            CREATE TABLE sovereignty_tags (
                id SERIAL PRIMARY KEY,
            ...
            RAISE NOTICE '主权标签表 sovereignty_tags 创建成功';
        ELSE
            RAISE NOTICE '主权标签表 sovereignty_tags 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '主权标签表 sovereignty_tags 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建主权标签表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sovereignty_tags') THEN
            RAISE WARNING '表 sovereignty_tags 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '成功插入 % 条标准主权标签', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入标准主权标签失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF p_condition IS NULL OR p_condition = '' THEN
                RAISE EXCEPTION 'p_condition不能为NULL或空字符串';
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '执行标签分配失败: %', SQLERRM;
            END;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '自动标签分配函数 auto_assign_sovereignty_tags 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建自动标签分配函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        BEGIN
            -- 参数验证
            IF p_parent_tag_id IS NULL THEN
                RAISE EXCEPTION 'p_parent_tag_id不能为NULL';
            END IF;

            IF p_child_tag_id IS NULL THEN
                RAISE EXCEPTION 'p_child_tag_id不能为NULL';
            END IF;

            -- 检查表是否存在
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sovereignty_tags') THEN
                RAISE EXCEPTION '表 sovereignty_tags 不存在';
            END IF;

            -- 检查标签是否存在
            IF NOT EXISTS (SELECT 1 FROM sovereignty_tags WHERE id = p_parent_tag_id) THEN
                RAISE EXCEPTION '父标签ID % 不存在', p_parent_tag_id;
            END IF;

            IF NOT EXISTS (SELECT 1 FROM sovereignty_tags WHERE id = p_child_tag_id) THEN
                RAISE EXCEPTION '子标签ID % 不存在', p_child_tag_id;
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '更新标签继承关系失败: %', SQLERRM;
            END;
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '标签继承函数 inherit_sovereignty_tags 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建标签继承函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF p_tag_id IS NULL THEN
                RAISE EXCEPTION 'p_tag_id不能为NULL';
            END IF;

            IF p_new_value IS NULL OR p_new_value = '' THEN
                RAISE EXCEPTION 'p_new_value不能为NULL或空字符串';
            END IF;
            ...
            -- 检查标签是否存在
            IF NOT EXISTS (SELECT 1 FROM sovereignty_tags WHERE id = p_tag_id) THEN
                RAISE EXCEPTION '标签ID % 不存在', p_tag_id;
            END IF;
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION '更新标签失败: %', SQLERRM;
            END;
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '标签更新函数 update_sovereignty_tag 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建标签更新函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'access_policies') THEN
            ...
            RAISE NOTICE '访问策略表 access_policies 创建成功';
        ELSE
            RAISE NOTICE '访问策略表 access_policies 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '访问策略表 access_policies 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建访问策略表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'access_policies') THEN
            RAISE WARNING '表 access_policies 不存在，无法插入数据';
            RETURN;
        END IF;
        ...
        GET DIAGNOSTICS inserted_count = ROW_COUNT;
        RAISE NOTICE '成功插入 % 条访问策略', inserted_count;
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '插入访问策略失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_data') THEN
            RAISE WARNING '表 user_data 不存在，无法创建索引';
            RETURN;
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_sovereignty') THEN
            CREATE INDEX idx_sovereignty ON user_data USING GIN (data_sovereignty);
            RAISE NOTICE 'GIN索引 idx_sovereignty 创建成功';
        END IF;
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data 不存在';
        WHEN duplicate_table THEN
            RAISE WARNING '索引 idx_sovereignty 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'current_user_sovereignty_tags') THEN
            RAISE WARNING '函数 current_user_sovereignty_tags 不存在，请先创建';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '主权访问策略 sovereignty_access 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 user_data 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 current_user_sovereignty_tags 不存在';
        WHEN duplicate_object THEN
            RAISE WARNING '策略 sovereignty_access 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建主权访问策略失败: %', SQLERRM;
            RAISE;
    END;
