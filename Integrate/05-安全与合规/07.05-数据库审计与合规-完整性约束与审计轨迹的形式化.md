...
BEGIN
    BEGIN
        ...
        RAISE NOTICE '日志审计配置已更新并重新加载';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE WARNING '需要超级用户权限';
        WHEN OTHERS THEN
            RAISE WARNING '配置日志审计失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- PostgreSQL 18：查看日志（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_settings WHERE name = 'log_directory') THEN
            RAISE WARNING '无法确定日志目录位置';
            RETURN;
        END IF;
        RAISE NOTICE '开始读取日志文件';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '读取日志准备失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 日志文件位置：postgresql.conf中的log_directory配置
-- 注意：pg_read_file需要超级用户权限，且文件路径必须相对于data_directory
...
```sql
-- PostgreSQL 18：安装pgAudit扩展（带错误处理）
DO $$
BEGIN
    BEGIN
    ...
    EXCEPTION
        WHEN undefined_file THEN
            RAISE WARNING 'pgAudit扩展不可用，需要先安装';
        WHEN insufficient_privilege THEN
...
BEGIN
    BEGIN
        ...
        PERFORM pg_reload_conf();
        RAISE NOTICE 'pgAudit配置已更新并重新加载';
    EXCEPTION
        WHEN insufficient_privilege THEN
            RAISE WARNING '需要超级用户权限';
        WHEN OTHERS THEN
            RAISE WARNING '配置pgAudit失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- PostgreSQL 18：查看审计日志（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
            RAISE WARNING 'pg_stat_statements扩展未安装，无法查看统计信息';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询审计日志统计';
    EXCEPTION
        WHEN OTHERS THEN
...
```sql
-- PostgreSQL 18：主键约束（实体完整性，带错误处理）
DO $$
BEGIN
    BEGIN
...
-- PostgreSQL 18：外键约束（参照完整性，带错误处理）
DO $$
BEGIN
    BEGIN
...
-- PostgreSQL 18：CHECK约束（用户定义完整性，带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'accounts'::regclass AND conname = 'balance_check') THEN
            ALTER TABLE accounts
            ADD CONSTRAINT balance_check
            CHECK (balance >= 0);
            RAISE NOTICE 'CHECK约束 balance_check 添加成功';
        ELSE
            RAISE NOTICE 'CHECK约束 balance_check 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE WARNING 'CHECK约束 balance_check 已存在';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid = 'transactions'::regclass AND conname = 'amount_check') THEN
            ALTER TABLE transactions
            ADD CONSTRAINT amount_check
            CHECK (amount != 0);
            RAISE NOTICE 'CHECK约束 amount_check 添加成功';
        ELSE
            RAISE NOTICE 'CHECK约束 amount_check 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_object THEN
            RAISE WARNING 'CHECK约束 amount_check 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '添加CHECK约束失败: %', SQLERRM;
            RAISE;
    END;
...
-- PostgreSQL 18：NOT NULL约束（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法修改列';
            RETURN;
        END IF;

        ALTER TABLE accounts
        ALTER COLUMN account_number SET NOT NULL;
        RAISE NOTICE 'NOT NULL约束添加成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '添加NOT NULL约束失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- PostgreSQL 18：查看约束（带错误处理和性能测试）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法查询约束';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询约束信息';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '查询准备失败: %', SQLERRM;
            RAISE;
...
-- PostgreSQL 18：创建审计触发器（带错误处理）
DO $$
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
            RAISE WARNING '表 accounts 不存在，无法创建触发器';
            RETURN;
        END IF;
        ...
        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'accounts_audit_trigger') THEN
            DROP TRIGGER accounts_audit_trigger ON accounts;
        END IF;
        ...
        RAISE NOTICE '审计触发器 accounts_audit_trigger 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 accounts 不存在';
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_records') THEN
            ...
            RAISE NOTICE '表 financial_records 创建成功';
        ELSE
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_audit_log') THEN
            ...
            RAISE NOTICE '审计表 financial_audit_log 创建成功';
        ELSE
            RAISE NOTICE '审计表 financial_audit_log 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '审计表 financial_audit_log 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建审计表失败: %', SQLERRM;
            RAISE;
    END;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_records') THEN
            RAISE WARNING '表 financial_records 不存在，无法创建触发器';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'audit_trigger_function') THEN
            RAISE WARNING '函数 audit_trigger_function 不存在';
        ...
        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'financial_audit_trigger') THEN
            DROP TRIGGER financial_audit_trigger ON financial_records;
        END IF;
        ...
        FOR EACH ROW
        EXECUTE FUNCTION audit_trigger_function();
        RAISE NOTICE '审计触发器 financial_audit_trigger 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 financial_records 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 audit_trigger_function 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建审计触发器失败: %', SQLERRM;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_records') THEN
            RAISE WARNING '表 financial_records 不存在，无法插入数据';
            RETURN;
        END IF;
    ...
    EXCEPTION
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保account_id=1存在';
        WHEN check_violation THEN
            RAISE WARNING 'CHECK约束失败，请检查数据值';
        WHEN OTHERS THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_audit_log') THEN
            RAISE WARNING '表 financial_audit_log 不存在，无法查询审计日志';
            RETURN;
        END IF;
        RAISE NOTICE '开始查询审计日志';
    EXCEPTION
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_hash_chain') THEN
            IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_audit_log') THEN
                RAISE WARNING '表 financial_audit_log 不存在，无法创建外键约束';
            END IF;
            ...
            RAISE NOTICE '审计哈希链表 audit_hash_chain 创建成功';
        ELSE
            RAISE NOTICE '审计哈希链表 audit_hash_chain 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING '审计哈希链表 audit_hash_chain 已存在';
        WHEN foreign_key_violation THEN
            RAISE WARNING '外键约束失败，请确保表 financial_audit_log 存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建审计哈希链表失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE '哈希链函数 update_audit_hash_chain 创建成功';
    EXCEPTION
        WHEN undefined_file THEN
        ...
        WHEN insufficient_privilege THEN
            RAISE EXCEPTION '需要超级用户权限来安装扩展';
        WHEN OTHERS THEN
            RAISE WARNING '创建哈希链函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'financial_audit_log') THEN
            RAISE WARNING '表 financial_audit_log 不存在，无法创建触发器';
            RETURN;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_audit_hash_chain') THEN
            RAISE WARNING '函数 update_audit_hash_chain 不存在';
            RETURN;
        END IF;

        IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'audit_hash_chain_trigger') THEN
            DROP TRIGGER audit_hash_chain_trigger ON financial_audit_log;
        END IF;
        ...
        RAISE NOTICE '哈希链触发器 audit_hash_chain_trigger 创建成功';
    EXCEPTION
        WHEN undefined_table THEN
            RAISE WARNING '表 financial_audit_log 不存在';
        WHEN undefined_function THEN
            RAISE WARNING '函数 update_audit_hash_chain 不存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建哈希链触发器失败: %', SQLERRM;
            RAISE;
    END;
END $$;
```
...
BEGIN
    BEGIN
        ...
        ELSE
            RAISE NOTICE '表 patient_records 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
...
BEGIN
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'hipaa_audit_log') THEN
            ...
            RAISE NOTICE 'HIPAA审计表 hipaa_audit_log 创建成功';
        ELSE
            RAISE NOTICE 'HIPAA审计表 hipaa_audit_log 已存在';
        END IF;
    EXCEPTION
        WHEN duplicate_table THEN
            RAISE WARNING 'HIPAA审计表 hipaa_audit_log 已存在';
        WHEN OTHERS THEN
            RAISE WARNING '创建HIPAA审计表失败: %', SQLERRM;
            RAISE;
    END;
END $$;

-- 3. 创建HIPAA审计触发器（带错误处理）
DO $$
BEGIN
    BEGIN
        ...
        END;
        $$ LANGUAGE plpgsql;
        RAISE NOTICE 'HIPAA审计触发器函数 hipaa_audit_trigger_function 创建成功';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE WARNING '创建HIPAA审计触发器函数失败: %', SQLERRM;
            RAISE;
    END;
END $$;
