...
```mermaid
flowchart LR
    A[主库 Primary] -->|WAL 流复制| B[从库1 Standby]
    A -->|WAL 流复制| C[从库2 Standby]
    A -->|WAL 流复制| D[从库3 Standby]
    B -->|读请求| E[应用]
    C -->|读请求| E
    D -->|读请求| E
    A -->|写请求| E
...
flowchart TD
    ...
    A --> D[商品库 product_db]
    B --> E[用户表 users]
    B --> F[用户信息表 user_profiles]
    C --> G[订单表 orders]
    C --> H[订单详情表 order_items]
    D --> I[商品表 products]
    D --> J[商品分类表 categories]
