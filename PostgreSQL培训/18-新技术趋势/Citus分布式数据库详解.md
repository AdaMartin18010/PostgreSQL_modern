...
- [Citus 分布式数据库详解](#citus-分布式数据库详解)
  ...
  - [1. Citus 基础](#1-citus-基础)
    - [1.0 Citus分布式数据库知识体系思维导图](#10-citus分布式数据库知识体系思维导图)
    - [1.1 什么是 Citus](#11-什么是-citus)
    - [1.2 安装 Citus](#12-安装-citus)
    - [1.3 版本要求](#13-版本要求)
  - [2. 集群架构](#2-集群架构)
    - [2.0 Citus 分布式架构工作原理概述](#20-citus-分布式架构工作原理概述)
  ...
  - [10. 常见问题（FAQ）](#10-常见问题faq)
    - [10.1 Citus基础常见问题](#101-citus基础常见问题)
      - [Q1: 如何安装和配置Citus？](#q1-如何安装和配置citus)
      - [Q2: 如何创建分布式表？](#q2-如何创建分布式表)
    - [10.2 分布式查询常见问题](#102-分布式查询常见问题)
      - [Q3: 如何优化分布式查询性能？](#q3-如何优化分布式查询性能)
  - [📚 参考资料](#-参考资料)
    - [官方文档](#官方文档)
    - [技术论文](#技术论文)
    - [技术博客](#技术博客)
...
### 2.0 Citus 分布式架构工作原理概述

**Citus 分布式架构的本质**：

Citus 通过将数据分片（Shard）分布到多个 Worker 节点，实现水平扩展。
Coordinator 节点负责查询路由和元数据管理，Worker 节点存储实际数据。
查询时，Coordinator 将查询分解为多个子查询，并行发送到相关 Worker 节点执行，然后合并结果返回。

**Citus 集群架构图**：

```mermaid
flowchart TD
    A[应用] --> B[Coordinator 节点]
    B --> C[Worker 节点 1]
    B --> D[Worker 节点 2]
    B --> E[Worker 节点 3]
    B --> F[Worker 节点 N]
...
```mermaid
flowchart TD
    A[SQL 查询] --> B[Coordinator 解析]
    B --> C{查询类型}
    C -->|单分片查询| D[路由到单个 Worker]
    C -->|多分片查询| E[并行查询所有 Worker]
    C -->|JOIN 查询| F[分布式 JOIN]
    D --> G[Worker 执行]
    E --> H[并行执行]
    F --> I[分布式 JOIN 执行]
    G --> J[返回结果]
...
- **Hash 分片**：根据分片键的哈希值分布数据，适合均匀分布的数据
- **Range 分片**：根据分片键的范围分布数据，适合时间序列数据
- **引用表**：小表全量复制到所有节点，避免跨节点 JOIN

### 2.1 Coordinator 节点

**Coordinator 节点的职责**：

- **查询路由**：将查询路由到正确的 Worker 节点
- **元数据管理**：管理分片元数据和集群状态
- **查询规划**：规划分布式查询执行计划
- **结果合并**：合并 Worker 节点的查询结果
...
1. **分片键选择**（选择高基数、均匀分布的列）

   ```sql
   -- ✅ 好：选择高基数的列（customer_id）
   SELECT create_distributed_table('orders', 'customer_id');
   -- 高基数：数据分布均匀，查询性能好
...
3. **引用表**（小表使用引用表）
   ...
   -- ❌ 不好：小表也分片（不必要的开销）
   -- SELECT create_distributed_table('countries', 'country_code');
   ```
...
4. **分片数量**（根据数据量和节点数选择）
   ...
   -- ❌ 不好：分片数量过少（扩展性差）
   -- shard_count => 4  -- 4 个节点，只有 4 个分片

   -- ❌ 不好：分片数量过多（管理开销大）
   -- shard_count => 1000  -- 分片过多
   ```

**避免做法**：

1. **避免选择低基数的列作为分片键**（数据分布不均匀）
2. **避免不共置经常 JOIN 的表**（跨节点 JOIN 性能差）
3. **避免小表也分片**（使用引用表）
...
1. **查询优化**（使用分片键过滤查询）
   ...
   -- 先路由到单个 Worker，再过滤，性能好
   ```

2. **索引优化**（在分片键和查询列上创建索引）
...
2. **定期备份 Coordinator 和 Worker 节点**（保证数据安全）

   ```bash
   # ✅ 好：定期备份
   # Coordinator 节点备份（包含元数据）
   pg_dump -h coordinator_host -U postgres -d mydb > coordinator_backup.sql

   # Worker 节点备份（包含数据）
   pg_dump -h worker1_host -U postgres -d mydb > worker1_backup.sql
   ```

3. **根据负载添加 Worker 节点**（水平扩展）
...
1. **使用包管理器安装**：
    ...
    ```bash
    # Ubuntu/Debian
    sudo apt-get install postgresql-17-citus-12
...
-- ❌ 不好：不使用分片键
SELECT * FROM distributed_table WHERE data = 'value';
-- 需要查询所有分片，性能差
```
...
```sql
-- ✅ 好：创建索引
CREATE INDEX ON distributed_table (id);
-- 提升查询性能
```
