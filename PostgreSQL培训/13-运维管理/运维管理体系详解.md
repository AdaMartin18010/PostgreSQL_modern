...
- [PostgreSQL 运维管理体系详解](#postgresql-运维管理体系详解)
  ...
  - [6. 常见问题（FAQ）](#6-常见问题faq)
    - [6.1 运维管理基础常见问题](#61-运维管理基础常见问题)
      - [Q1: 如何建立完善的监控体系？](#q1-如何建立完善的监控体系)
      - [Q2: 如何实现自动化运维？](#q2-如何实现自动化运维)
...
SELECT cron.schedule('daily-backup', '0 2 * * *',
    $$pg_dump -Fc -d mydb -f /backup/mydb_$(date +\%Y\%m\%d).dump$$);
...
```sql
-- 1. 创建容量预测函数
CREATE OR REPLACE FUNCTION predict_capacity(days INTEGER DEFAULT 30)
RETURNS TABLE (
    metric TEXT,
    current_size TEXT,
    predicted_size TEXT,
...
BEGIN
    ...
    SELECT
        ...
        pg_size_pretty(pg_database_size(current_database())) AS current_size,
        pg_size_pretty(
            pg_database_size(current_database()) * POWER(1 + 0.01, days)
        ) AS predicted_size,
        0.01 AS growth_rate;
END;
$$ LANGUAGE plpgsql;
...
-- 2. 使用预测函数
SELECT * FROM predict_capacity(30);  -- 预测30天后
```
