...
- [PostgreSQL 事务管理详解](#postgresql-事务管理详解)
  ...
  - [3. 事务基础](#3-事务基础)
    - [3.0 事务工作原理概述](#30-事务工作原理概述)
    - [3.1 ACID 特性](#31-acid-特性)
    - [3.2 事务隔离级别算法分析](#32-事务隔离级别算法分析)
    - [3.3 事务基本操作](#33-事务基本操作)
  - [4. 事务隔离级别](#4-事务隔离级别)
  ...
  - [10. 常见问题（FAQ）](#10-常见问题faq)
    - [10.1 事务管理基础常见问题](#101-事务管理基础常见问题)
      - [Q1: 如何选择合适的事务隔离级别？](#q1-如何选择合适的事务隔离级别)
      - [Q2: 如何避免死锁？](#q2-如何避免死锁)
    - [10.2 事务性能常见问题](#102-事务性能常见问题)
      - [Q3: 事务执行很慢怎么办？](#q3-事务执行很慢怎么办)
  - [11. 参考资料](#11-参考资料)
    - [11.1 官方文档](#111-官方文档)
    - [11.2 技术论文](#112-技术论文)
    - [11.3 技术博客](#113-技术博客)
...
**定义1（隔离级别）**：
设 IsolationLevel = {READ UNCOMMITTED, READ COMMITTED, REPEATABLE READ, SERIALIZABLE}，每个级别有不同的可见性规则。

**定义2（快照隔离算法）**：
设 SnapshotIsolation(txid) → Snapshot：

```text
1. 获取当前事务ID：txid = txid_current()
2. 获取活跃事务列表：active_txids = get_active_transactions()
3. 创建快照：snapshot = {
     xmin = min(active_txids),
     xmax = next_txid(),
     xip_list = active_txids
   }
4. 返回 snapshot
```
...
- 快照创建时间复杂度：O(k)，k为活跃事务数
- 可见性判断时间复杂度：O(1)（平均情况）

**定义3（可串行化隔离算法）**：
设 SerializableIsolation(transactions) → {serializable, conflict}：
...
3. **合理使用隔离级别**（根据业务需求选择，默认 READ COMMITTED）
   ...
   -- ✅ 好：需要一致性快照时使用 REPEATABLE READ
   BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ;
   SELECT * FROM accounts WHERE id = 1;
   SELECT * FROM accounts WHERE id = 1;  -- 结果一致
   COMMIT;

   -- ✅ 好：金融系统使用 SERIALIZABLE
   BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;
   UPDATE accounts SET balance = balance - 100 WHERE id = 1;
   COMMIT;
   ```

4. **使用保存点处理复杂事务**（允许部分回滚）
   ...
   -- 如果订单创建失败，可以回滚到 after_user
   -- ROLLBACK TO SAVEPOINT after_user;
   COMMIT;
   ```

5. **使用函数封装事务逻辑**（保证事务边界清晰）
...
6. **检查约束保证数据一致性**
   ...
   -- 事务中违反约束会自动回滚
   BEGIN;
   UPDATE accounts SET balance = balance - 1000 WHERE id = 1;  -- 如果余额不足，自动回滚
   COMMIT;
   ```

**避免做法**：

1. **避免长事务**（长时间锁定资源，影响并发性能）

   ```sql
   -- ❌ 不好：长事务
   BEGIN;
   SELECT * FROM accounts WHERE id = 1 FOR UPDATE;
   -- 执行耗时操作（如处理业务逻辑、调用外部服务）
   -- 长时间持有锁，阻塞其他事务
   COMMIT;
   ```

2. **避免在事务中进行用户交互**（用户响应时间不确定）
...
3. **避免不必要的 SERIALIZABLE 隔离级别**（性能开销大）

   ```sql
   -- ❌ 不好：不需要最高一致性时使用 SERIALIZABLE
   BEGIN TRANSACTION ISOLATION LEVEL SERIALIZABLE;
   SELECT * FROM users WHERE id = 1;  -- 简单查询不需要 SERIALIZABLE
   COMMIT;
...
4. **避免嵌套事务**（PostgreSQL 不支持真正的嵌套事务）

   ```sql
   -- ❌ 不好：尝试嵌套事务（实际是保存点）
   BEGIN;
   BEGIN;  -- 这不会创建新事务，只是创建保存点
   COMMIT;
   COMMIT;
   ```
...
1. **利用 MVCC 提升并发性能**（读操作不阻塞写操作）
   ...
   -- 事务 1：读取
   BEGIN;
   SELECT * FROM accounts WHERE id = 1;  -- 不阻塞事务 2
   COMMIT;
...
3. **使用锁超时避免长时间等待**
   ...
   BEGIN;
   SELECT * FROM accounts WHERE id = 1 FOR UPDATE NOWAIT;  -- 不等待，立即返回错误
   COMMIT;
   ```
...
2. **避免死锁**（统一锁顺序，避免循环等待）
   ...
   ```sql
   -- ❌ 不好：不同事务以不同顺序获取锁（可能导致死锁）
   -- 事务 1：先锁 id=1，再锁 id=2
   -- 事务 2：先锁 id=2，再锁 id=1（可能死锁）
   ```

3. **避免不必要的锁**（只在必要时使用 FOR UPDATE）

   ```sql
   -- ❌ 不好：不需要修改时使用 FOR UPDATE
   BEGIN;
   SELECT * FROM accounts WHERE id = 1 FOR UPDATE;  -- 不需要修改，不需要锁
   COMMIT;

   -- ✅ 好：只在需要修改时使用锁
   BEGIN;
   SELECT * FROM accounts WHERE id = 1;  -- 只读取，不需要锁
   COMMIT;
   ```
...
```sql
-- 1. READ COMMITTED（默认，推荐）
BEGIN;  -- 默认READ COMMITTED
SELECT * FROM accounts WHERE id = 1;
COMMIT;
-- 适用场景：大多数OLTP应用，性能好

-- 2. REPEATABLE READ（需要一致性快照）
BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ;
SELECT * FROM accounts WHERE id = 1;
-- 同一事务中多次读取结果一致
COMMIT;
...
- READ COMMITTED：TPS **1000**，延迟 **10ms**
- REPEATABLE READ：TPS **800**，延迟 **15ms**
- SERIALIZABLE：TPS **500**，延迟 **30ms**
- **推荐：默认使用READ COMMITTED**
...
- 长事务：事务时间 **10秒**，锁持有时间 **10秒**
- 短事务：事务时间 **0.1秒**，锁持有时间 **0.1秒**
- **性能提升：100倍**
