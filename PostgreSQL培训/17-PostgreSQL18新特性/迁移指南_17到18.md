...
本文档提供从 PostgreSQL 17 升级到 PostgreSQL 18 的完整迁移指南，包括迁移步骤、注意事项、兼容性问题、性能优化建议等，帮助用户顺利完成版本升级。
...
- **异步 I/O 子系统**：性能提升高达 3 倍
- **文本处理改进**：PG_UNICODE_FAST 排序规则、casefold 函数
- **身份验证增强**：OAuth 2.0 支持、FIPS 模式、TLS v1.3
- **复制功能增强**：逻辑复制写入冲突报告、并行流式传输
- **生成列**：虚拟生成列支持
- **uuidv7() 函数**：基于时间的 UUID 生成
- **查询性能提升**：查询优化器和执行引擎改进
- **向量数据库增强**：pgvector 集成优化
...
- **性能提升**：查询性能提升 30-50%
- **新功能**：支持更多 SQL 标准和创新功能
- **安全性增强**：OAuth 2.0、FIPS 模式、TLS v1.3
- **可观测性增强**：更强大的监控和诊断能力
- **云原生特性**：云环境下的优化和特性
...
```bash
# 方法 1: 使用 pg_dump/pg_restore（推荐）
# 1. 从 PostgreSQL 17 导出
pg_dump -U postgres -F c -f database_backup.dump database_name

# 2. 导入到 PostgreSQL 18
pg_restore -U postgres -d database_name database_backup.dump

# 方法 2: 使用 pg_upgrade（快速升级）
# 1. 停止 PostgreSQL 17
...
```bash
# 1. 迁移配置文件
# 从 PostgreSQL 17 配置文件迁移到 PostgreSQL 18
