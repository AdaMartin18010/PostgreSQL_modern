...
- [【深入】PostgreSQL查询优化器深度解析 - CBO与统计信息](#深入postgresql查询优化器深度解析---cbo与统计信息)
  - [📑 目录](#-目录)
  - [1. 查询优化器架构](#1-查询优化器架构)
    - [1.1 优化器工作流程](#11-优化器工作流程)
    - [1.2 优化器关键数据结构](#12-优化器关键数据结构)
    - [1.3 优化器配置参数](#13-优化器配置参数)
  - [2. 基于成本的优化（CBO）](#2-基于成本的优化cbo)
    - [2.1 CBO原理](#21-cbo原理)
    - [2.2 顺序扫描代价计算](#22-顺序扫描代价计算)
    - [2.3 索引扫描代价计算](#23-索引扫描代价计算)
    - [2.4 连接代价计算](#24-连接代价计算)
    - [2.5 CBO调优技巧](#25-cbo调优技巧)
  - [3. 统计信息系统](#3-统计信息系统)
    - [3.1 统计信息表](#31-统计信息表)
    - [3.2 ANALYZE深度使用](#32-analyze深度使用)
    - [3.3 统计信息扩展（Extended Statistics）](#33-统计信息扩展extended-statistics)
  - [4. 代价估算模型](#4-代价估算模型)
    - [4.1 选择率估算](#41-选择率估算)
    - [4.2 行数估算](#42-行数估算)
    - [4.3 宽度估算](#43-宽度估算)
  - [5. 查询改写和优化](#5-查询改写和优化)
    - [5.1 谓词下推（Predicate Pushdown）](#51-谓词下推predicate-pushdown)
    - [5.2 连接消除（Join Elimination）](#52-连接消除join-elimination)
    - [5.3 子查询展开](#53-子查询展开)
    - [5.4 常量折叠](#54-常量折叠)
  - [6. 执行计划生成](#6-执行计划生成)
    - [6.1 动态规划算法](#61-动态规划算法)
    - [6.2 路径比较和选择](#62-路径比较和选择)
  - [7. 优化器调优实战](#7-优化器调优实战)
    - [7.1 案例1：错误的行数估算](#71-案例1错误的行数估算)
    - [7.2 案例2：过时的统计信息](#72-案例2过时的统计信息)
    - [7.3 案例3：相关性问题](#73-案例3相关性问题)
  - [8. PostgreSQL 18优化器增强](#8-postgresql-18优化器增强)
    - [8.1 增量排序（Incremental Sort）](#81-增量排序incremental-sort)
    - [8.2 Memoize（查询结果缓存）](#82-memoize查询结果缓存)
    - [8.3 并行哈希连接改进（PostgreSQL 18）](#83-并行哈希连接改进postgresql-18)
  - [📚 参考资源](#-参考资源)
    - [官方文档](#官方文档)
    - [源码分析](#源码分析)
    - [深入阅读](#深入阅读)
