# PostgreSQL分区表设计对MVCC的影响

> **文档编号**: DESIGN-PARTITION-001
> **主题**: 分区表设计
> **版本**: PostgreSQL 17 & 18

---

## 📑 目录

- [PostgreSQL分区表设计对MVCC的影响](#postgresql分区表设计对mvcc的影响)
  - [📑 目录](#-目录)
  - [📋 概述](#-概述)
  - [🔍 第一部分：分区策略](#-第一部分分区策略)
    - [1.1 范围分区](#11-范围分区)
      - [按时间分区](#按时间分区)
      - [按数值范围分区](#按数值范围分区)
    - [1.2 列表分区](#12-列表分区)
      - [按地区分区](#按地区分区)
      - [按状态分区](#按状态分区)
    - [1.3 哈希分区](#13-哈希分区)
      - [哈希分区原理](#哈希分区原理)
      - [MVCC影响](#mvcc影响)
  - [🚀 第二部分：分区表MVCC特性](#-第二部分分区表mvcc特性)
    - [2.1 分区裁剪](#21-分区裁剪)
      - [查询优化](#查询优化)
      - [VACUUM优化](#vacuum优化)
    - [2.2 分区级MVCC](#22-分区级mvcc)
      - [独立版本链](#独立版本链)
      - [独立VACUUM](#独立vacuum)
    - [2.3 分区维护](#23-分区维护)
      - [添加分区](#添加分区)
      - [删除分区](#删除分区)
      - [分区交换](#分区交换)
  - [📊 第三部分：性能优化](#-第三部分性能优化)
    - [3.1 分区键选择](#31-分区键选择)
    - [3.2 分区数量优化](#32-分区数量优化)
    - [3.3 索引设计](#33-索引设计)
  - [🔧 第四部分：实际案例分析](#-第四部分实际案例分析)
    - [4.1 时序数据分区](#41-时序数据分区)
    - [4.2 大表分区](#42-大表分区)
    - [4.3 多租户分区](#43-多租户分区)
  - [📝 总结](#-总结)

---

## 📋 概述

分区表是PostgreSQL处理大表的重要机制，对MVCC有重要影响。合理的分区设计可以提高查询性能、优化VACUUM效率、简化数据管理。本文档深入分析分区表设计对MVCC的影响。

---

## 🔍 第一部分：分区策略

### 1.1 范围分区

#### 按时间分区

```sql
-- 按时间范围分区（推荐用于时序数据）
CREATE TABLE logs (
    id BIGSERIAL,
    message TEXT,
    created_at TIMESTAMP NOT NULL,
    PRIMARY KEY (id, created_at)
) PARTITION BY RANGE (created_at);

-- 创建分区
CREATE TABLE logs_2024_01 PARTITION OF logs
    FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');

CREATE TABLE logs_2024_02 PARTITION OF logs
    FOR VALUES FROM ('2024-02-01') TO ('2024-03-01');

-- MVCC影响：
-- 1. 每个分区独立的版本链
-- 2. VACUUM可以并行处理不同分区
-- 3. 查询时只扫描相关分区（分区裁剪）
```

#### 按数值范围分区

```sql
-- 按数值范围分区
CREATE TABLE orders (
    id BIGSERIAL,
    user_id BIGINT,
    amount DECIMAL(10,2),
    created_at TIMESTAMP,
    PRIMARY KEY (id, user_id)
) PARTITION BY RANGE (user_id);

-- 创建分区
CREATE TABLE orders_0_1000 PARTITION OF orders
    FOR VALUES FROM (0) TO (1000);

CREATE TABLE orders_1000_2000 PARTITION OF orders
    FOR VALUES FROM (1000) TO (2000);
```

### 1.2 列表分区

#### 按地区分区

```sql
-- 按列表分区
CREATE TABLE users (
    id BIGSERIAL PRIMARY KEY,
    name VARCHAR(100),
    region VARCHAR(50)
) PARTITION BY LIST (region);

-- 创建分区
CREATE TABLE users_cn PARTITION OF users
    FOR VALUES IN ('CN');

CREATE TABLE users_us PARTITION OF users
    FOR VALUES IN ('US');
```

### 1.3 哈希分区

#### 哈希分区原理

```sql
-- 哈希分区：均匀分布数据
CREATE TABLE events (
    id BIGSERIAL PRIMARY KEY,
    event_data JSONB
) PARTITION BY HASH (id);

-- 创建分区
CREATE TABLE events_0 PARTITION OF events
    FOR VALUES WITH (MODULUS 4, REMAINDER 0);

CREATE TABLE events_1 PARTITION OF events
    FOR VALUES WITH (MODULUS 4, REMAINDER 1);

CREATE TABLE events_2 PARTITION OF events
    FOR VALUES WITH (MODULUS 4, REMAINDER 2);

CREATE TABLE events_3 PARTITION OF events
    FOR VALUES WITH (MODULUS 4, REMAINDER 3);

-- MVCC影响：
-- 1. 数据均匀分布
-- 2. 每个分区独立的版本链
-- 3. 并行VACUUM效率高
```

#### MVCC影响

```sql
-- 哈希分区对MVCC的影响：
-- 1. 数据均匀分布：每个分区大小相近
-- 2. VACUUM可以并行处理：提高效率
-- 3. 查询需要扫描多个分区：可能影响性能
```

---

## 🚀 第二部分：分区表MVCC特性

### 2.1 分区裁剪

#### 查询优化

```sql
-- 分区裁剪：只扫描相关分区
SELECT * FROM logs
WHERE created_at >= '2024-01-01'
  AND created_at < '2024-02-01';

-- 执行计划：
-- 只扫描 logs_2024_01 分区
-- 其他分区被裁剪掉

-- MVCC影响：
-- 1. 只检查相关分区的版本链
-- 2. 查询性能提升
-- 3. 锁竞争减少
```

#### VACUUM优化

```sql
-- VACUUM分区表
VACUUM logs;

-- PostgreSQL自动识别需要VACUUM的分区
-- 只VACUUM有死亡元组的分区

-- MVCC影响：
-- 1. VACUUM时间减少（只处理相关分区）
-- 2. 锁竞争减少（分区级锁）
-- 3. 并行VACUUM效率高
```

### 2.2 分区级MVCC

#### 独立版本链

```sql
-- 每个分区独立的版本链
-- 分区1的版本链不影响分区2

-- 查看分区版本链统计
SELECT
    schemaname,
    relname,
    n_live_tup,
    n_dead_tup,
    round(n_dead_tup * 100.0 / NULLIF(n_live_tup + n_dead_tup, 0), 2) as dead_ratio
FROM pg_stat_user_tables
WHERE schemaname = 'public'
  AND relname LIKE 'logs_%'
ORDER BY dead_ratio DESC;
```

#### 独立VACUUM

```sql
-- 可以单独VACUUM某个分区
VACUUM logs_2024_01;

-- MVCC影响：
-- 1. 只清理该分区的死亡元组
-- 2. 不影响其他分区
-- 3. VACUUM时间短
```

### 2.3 分区维护

#### 添加分区

```sql
-- 添加新分区
CREATE TABLE logs_2024_03 PARTITION OF logs
    FOR VALUES FROM ('2024-03-01') TO ('2024-04-01');

-- MVCC影响：
-- 1. 新分区独立的版本链
-- 2. 不影响现有分区
```

#### 删除分区

```sql
-- 删除旧分区（归档）
DROP TABLE logs_2023_01;

-- MVCC影响：
-- 1. 直接删除分区，无需VACUUM
-- 2. 释放空间快
-- 3. 不影响其他分区
```

#### 分区交换

```sql
-- 分区交换（PostgreSQL 17+）
-- 将分区转换为普通表（归档）
ALTER TABLE logs DETACH PARTITION logs_2023_01;

-- MVCC影响：
-- 1. 分区变为独立表
-- 2. 可以单独管理
-- 3. 不影响主表
```

---

## 📊 第三部分：性能优化

### 3.1 分区键选择

```sql
-- 分区键选择原则：
-- 1. 查询条件常用字段
-- 2. 数据分布均匀
-- 3. 分区数量合理（<1000）

-- ✅ 好的分区键：created_at（时间字段）
CREATE TABLE logs (...) PARTITION BY RANGE (created_at);

-- ❌ 不好的分区键：id（自增，分布不均匀）
CREATE TABLE logs (...) PARTITION BY RANGE (id);
```

### 3.2 分区数量优化

```sql
-- 分区数量优化：
-- 1. 太少：分区大，VACUUM慢
-- 2. 太多：元数据开销大，查询计划慢
-- 3. 推荐：100-1000个分区

-- 按月分区：12个分区/年（推荐）
-- 按周分区：52个分区/年（中等）
-- 按日分区：365个分区/年（大规模）
```

### 3.3 索引设计

```sql
-- 分区表索引设计：
-- 1. 主键必须包含分区键
-- 2. 每个分区自动创建索引
-- 3. 全局索引不支持（PostgreSQL限制）

-- 主键设计
CREATE TABLE logs (
    id BIGSERIAL,
    created_at TIMESTAMP NOT NULL,
    PRIMARY KEY (id, created_at)  -- 必须包含分区键
) PARTITION BY RANGE (created_at);

-- 索引自动创建在每个分区上
CREATE INDEX idx_logs_created_at ON logs(created_at);
-- 等价于在每个分区上创建索引
```

---

## 🔧 第四部分：实际案例分析

### 4.1 时序数据分区

```sql
-- 日志表：按时间分区
CREATE TABLE app_logs (
    id BIGSERIAL,
    app_name VARCHAR(100),
    level VARCHAR(20),
    message TEXT,
    created_at TIMESTAMP NOT NULL,
    PRIMARY KEY (id, created_at)
) PARTITION BY RANGE (created_at)
WITH (
    fillfactor = 100  -- 只插入，不更新
);

-- 自动创建分区函数
CREATE OR REPLACE FUNCTION create_monthly_partition(table_name TEXT, month_date DATE)
RETURNS VOID AS $$
DECLARE
    partition_name TEXT;
    start_date DATE;
    end_date DATE;
BEGIN
    start_date := date_trunc('month', month_date)::DATE;
    end_date := (start_date + interval '1 month')::DATE;
    partition_name := table_name || '_' || to_char(start_date, 'YYYY_MM');

    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I PARTITION OF %I
        FOR VALUES FROM (%L) TO (%L)',
        partition_name, table_name, start_date, end_date
    );
END;
$$ LANGUAGE plpgsql;

-- MVCC影响：
-- 1. 每个分区独立的版本链
-- 2. 旧分区可以归档（删除）
-- 3. VACUUM只处理活跃分区
```

### 4.2 大表分区

```sql
-- 大表分区：按用户ID范围分区
CREATE TABLE user_events (
    id BIGSERIAL,
    user_id BIGINT NOT NULL,
    event_type VARCHAR(50),
    event_data JSONB,
    created_at TIMESTAMP,
    PRIMARY KEY (id, user_id)
) PARTITION BY RANGE (user_id)
WITH (
    fillfactor = 80  -- 更新频繁
);

-- 创建分区（每1000万用户一个分区）
CREATE TABLE user_events_0_10m PARTITION OF user_events
    FOR VALUES FROM (0) TO (10000000);

CREATE TABLE user_events_10m_20m PARTITION OF user_events
    FOR VALUES FROM (10000000) TO (20000000);

-- MVCC影响：
-- 1. 每个分区独立的版本链
-- 2. VACUUM可以并行处理
-- 3. 查询只扫描相关分区
```

### 4.3 多租户分区

```sql
-- 多租户分区：按租户ID分区
CREATE TABLE tenant_data (
    id BIGSERIAL,
    tenant_id INT NOT NULL,
    data JSONB,
    created_at TIMESTAMP,
    PRIMARY KEY (id, tenant_id)
) PARTITION BY LIST (tenant_id);

-- 创建分区
CREATE TABLE tenant_data_1 PARTITION OF tenant_data
    FOR VALUES IN (1);

CREATE TABLE tenant_data_2 PARTITION OF tenant_data
    FOR VALUES IN (2);

-- MVCC影响：
-- 1. 每个租户独立的版本链
-- 2. 租户数据隔离
-- 3. 可以单独VACUUM某个租户
```

---

## 📝 总结

### 核心优势

1. **分区裁剪**：查询只扫描相关分区
2. **独立版本链**：每个分区独立的版本链
3. **并行VACUUM**：可以并行处理不同分区
4. **简化管理**：可以单独管理分区

### 最佳实践

1. **分区键选择**：查询常用字段，数据分布均匀
2. **分区数量**：100-1000个分区
3. **分区大小**：每个分区10-100GB
4. **自动管理**：使用函数自动创建分区

### MVCC影响

- ✅ 分区裁剪减少查询时间50-90%
- ✅ 独立版本链减少VACUUM时间30-50%
- ✅ 并行VACUUM提高效率2-4倍
- ✅ 分区删除释放空间快（无需VACUUM）

PostgreSQL 17/18的分区表机制对MVCC性能有重要影响，通过合理的分区设计，可以显著提升性能和简化数据管理。
