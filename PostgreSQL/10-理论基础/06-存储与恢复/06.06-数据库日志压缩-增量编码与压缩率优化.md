...
```sql
-- PostgreSQL 18：启用WAL压缩
ALTER SYSTEM SET wal_compression = 'on';
-- 或 'lz4'（PostgreSQL 14+支持lz4压缩）

-- PostgreSQL 18：查看WAL压缩配置
SHOW wal_compression;
-- 可选值：off, on, lz4
-- off：禁用压缩
-- on：使用pglz压缩（默认）
-- lz4：使用lz4压缩（PostgreSQL 14+）
