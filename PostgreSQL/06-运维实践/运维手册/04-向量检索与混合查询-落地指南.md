# 向量检索与混合查询-落地指南（Runbook）

> **文档版本**: v1.0
> **最后更新**: 2025-11-22
> **PostgreSQL版本**: 18.x (推荐) ⭐ | 17.x (推荐) | 16.x (兼容)
> 对齐：`../../04-高级特性/03.05-向量数据库支持.md`

---

## 📋 目录

- [向量检索与混合查询-落地指南（Runbook）](#向量检索与混合查询-落地指南runbook)
  - [📋 目录](#-目录)
  - [1. 目标](#1-目标)
  - [2. 前置](#2-前置)
    - [2.1 pgvector安装](#21-pgvector安装)
    - [2.2 启用扩展](#22-启用扩展)
    - [2.3 准备数据表](#23-准备数据表)
  - [3. 步骤](#3-步骤)
    - [3.1 索引类型选择](#31-索引类型选择)
    - [3.2 在线查询（结构化 + 向量）](#32-在线查询结构化--向量)
    - [3.3 全文 + 向量融合](#33-全文--向量融合)
  - [4. 运行参数](#4-运行参数)
    - [4.1 性能参数调优](#41-性能参数调优)
    - [4.2 性能监控](#42-性能监控)
    - [4.3 性能基准测试](#43-性能基准测试)
  - [5. 验证](#5-验证)
    - [5.1 召回率验证](#51-召回率验证)
    - [5.2 性能基线记录](#52-性能基线记录)
  - [6. 故障排查](#6-故障排查)
    - [6.1 常见问题](#61-常见问题)
  - [7. 生产运维实践](#7-生产运维实践)
    - [7.1 监控指标](#71-监控指标)
      - [7.1.1 核心指标定义](#711-核心指标定义)
      - [7.1.2 监控查询](#712-监控查询)
      - [7.1.3 Prometheus指标导出](#713-prometheus指标导出)
    - [7.2 告警规则](#72-告警规则)
      - [7.2.1 告警规则定义](#721-告警规则定义)
      - [7.2.2 Prometheus告警配置](#722-prometheus告警配置)
    - [7.3 性能监控](#73-性能监控)
      - [7.3.1 实时性能监控](#731-实时性能监控)
      - [7.3.2 Grafana仪表板配置](#732-grafana仪表板配置)
    - [7.4 索引健康度监控](#74-索引健康度监控)
      - [7.4.1 索引健康度检查](#741-索引健康度检查)
      - [7.4.2 索引重建建议](#742-索引重建建议)
    - [7.5 故障处理流程](#75-故障处理流程)
      - [7.5.1 故障分类](#751-故障分类)
      - [7.5.2 故障处理步骤](#752-故障处理步骤)
      - [7.5.3 故障处理清单](#753-故障处理清单)
  - [8. 最佳实践](#8-最佳实践)

---

## 1. 目标

- 建立"向量召回 + 结构化过滤 + 全文权重"的混合检索流水线，支持在线查询与离线重排。

## 2. 前置

- 安装 `pgvector`；准备 `docs(id, text, embedding vector(n))`；选择索引（HNSW/IVFFlat/PQ）并 ANALYZE。

### 2.1 pgvector安装

```bash
# 方法1: 使用apt安装（Ubuntu/Debian）
sudo apt-get install postgresql-14-pgvector

# 方法2: 从源码编译
git clone --branch v0.5.1 https://github.com/pgvector/pgvector.git
cd pgvector
make
sudo make install

# 方法3: 使用Docker
docker pull pgvector/pgvector:pg14
```

### 2.2 启用扩展

```sql
-- 创建扩展
CREATE EXTENSION IF NOT EXISTS vector;

-- 验证安装
SELECT * FROM pg_available_extensions WHERE name = 'vector';

-- 检查版本
SELECT extversion FROM pg_extension WHERE extname = 'vector';
```

### 2.3 准备数据表

```sql
-- 创建文档表
CREATE TABLE docs (
    id SERIAL PRIMARY KEY,
    text TEXT NOT NULL,
    embedding vector(1536),  -- OpenAI ada-002维度
    created_at TIMESTAMP DEFAULT NOW(),
    category VARCHAR(100),
    metadata JSONB
);

-- 创建全文搜索索引
CREATE INDEX docs_text_gin ON docs USING gin(to_tsvector('simple', text));

-- 创建结构化字段索引
CREATE INDEX docs_created_at_idx ON docs(created_at);
CREATE INDEX docs_category_idx ON docs(category);
```

## 3. 步骤

1) 创建索引：

    ```sql
    CREATE INDEX IF NOT EXISTS docs_hnsw ON docs USING hnsw (embedding vector_l2_ops) WITH (m=32, ef_construction=200);
    ```

### 3.1 索引类型选择

**HNSW索引（推荐用于高维向量）**:

```sql
-- HNSW索引创建
CREATE INDEX docs_hnsw ON docs
USING hnsw (embedding vector_l2_ops)
WITH (
    m = 32,              -- 每个节点的连接数（16-64）
    ef_construction = 200  -- 构建时的候选数（100-1000）
);

-- 查询时设置ef_search（影响召回率和性能）
SET ivfflat.probes = 100;  -- 注意：HNSW使用ef_search参数
```

**IVFFlat索引（推荐用于大规模数据）**:

```sql
-- IVFFlat索引创建
CREATE INDEX docs_ivfflat ON docs
USING ivfflat (embedding vector_l2_ops)
WITH (
    lists = 100  -- 聚类中心数（建议：rows/1000）
);

-- 查询时设置probes
SET ivfflat.probes = 10;  -- 搜索的聚类数（1-lists）
```

**索引选择建议**:

| 场景 | 推荐索引 | 原因 |
|------|---------|------|
| 高维向量(>100维) | HNSW | 更好的召回率 |
| 大规模数据(>100万) | IVFFlat | 更快的构建速度 |
| 实时查询 | HNSW | 更低的延迟 |
| 批量查询 | IVFFlat | 更好的吞吐量 |

### 3.2 在线查询（结构化 + 向量）

```sql
SELECT id, text
FROM docs
WHERE created_at >= now() - interval '7 day'
ORDER BY embedding <-> $1
LIMIT 50;
```

**优化版本**:

```sql
-- 方法1: 先过滤后排序（推荐）
SELECT id, text, embedding <-> $1 AS distance
FROM docs
WHERE created_at >= now() - interval '7 day'
  AND category = 'technology'
ORDER BY embedding <-> $1
LIMIT 50;

-- 方法2: 使用子查询优化
WITH filtered AS (
    SELECT id, text, embedding
    FROM docs
    WHERE created_at >= now() - interval '7 day'
      AND category = 'technology'
)
SELECT id, text, embedding <-> $1 AS distance
FROM filtered
ORDER BY embedding <-> $1
LIMIT 50;
```

### 3.3 全文 + 向量融合

```sql
WITH s AS (
    SELECT id, ts_rank(to_tsvector('simple', text), plainto_tsquery('simple',$2)) AS tr
    FROM docs
    WHERE to_tsvector('simple', text) @@ plainto_tsquery('simple',$2)
    ORDER BY tr DESC LIMIT 500
), v AS (
    SELECT id, embedding <-> $1 AS dist
    FROM docs
    WHERE id IN (SELECT id FROM s)
    ORDER BY dist ASC LIMIT 100
)
SELECT d.id, d.text, 0.6*(1.0/v.dist) + 0.4*s.tr AS score
FROM v d JOIN s ON d.id = s.id
ORDER BY score DESC LIMIT 50;
```

**优化版本**:

```sql
-- 使用加权融合
WITH text_results AS (
    SELECT
        id,
        text,
        ts_rank(to_tsvector('simple', text), plainto_tsquery('simple', $2)) AS text_score
    FROM docs
    WHERE to_tsvector('simple', text) @@ plainto_tsquery('simple', $2)
    ORDER BY text_score DESC
    LIMIT 500
),
vector_results AS (
    SELECT
        id,
        embedding <-> $1 AS vector_distance
    FROM docs
    WHERE id IN (SELECT id FROM text_results)
    ORDER BY embedding <-> $1
    LIMIT 100
)
SELECT
    d.id,
    d.text,
    -- 归一化分数并加权融合
    0.6 * (1.0 / (1.0 + v.vector_distance)) + 0.4 * t.text_score AS combined_score
FROM vector_results v
JOIN text_results t ON v.id = t.id
JOIN docs d ON v.id = d.id
ORDER BY combined_score DESC
LIMIT 50;
```

## 4. 运行参数

- `SET ivfflat.probes = 10;` 查询高召回时调大；
- 控制 `work_mem`，避免ANN与排序内存爆；
- 监控延迟TP95/TP99 与召回指标。

### 4.1 性能参数调优

```sql
-- 1. 调整work_mem（向量查询需要更多内存）
SET work_mem = '256MB';

-- 2. 调整IVFFlat probes（召回率 vs 性能权衡）
SET ivfflat.probes = 10;   -- 默认值，快速但召回率较低
SET ivfflat.probes = 50;   -- 平衡模式
SET ivfflat.probes = 100;  -- 高召回率，但较慢

-- 3. 调整HNSW ef_search（仅HNSW索引）
-- 注意：pgvector的HNSW实现可能使用不同参数名
-- 查询时在ORDER BY子句中指定LIMIT会影响搜索范围

-- 4. 启用并行查询
SET max_parallel_workers_per_gather = 4;
```

### 4.2 性能监控

```sql
-- 1. 查询执行时间
EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
SELECT id, text
FROM docs
WHERE created_at >= now() - interval '7 day'
ORDER BY embedding <-> $1
LIMIT 50;

-- 2. 索引使用情况
SELECT
    schemaname,
    tablename,
    indexname,
    idx_scan,
    idx_tup_read,
    idx_tup_fetch
FROM pg_stat_user_indexes
WHERE tablename = 'docs'
AND indexname LIKE '%vector%';

-- 3. 表大小和索引大小
SELECT
    pg_size_pretty(pg_relation_size('docs')) AS table_size,
    pg_size_pretty(pg_relation_size('docs_hnsw')) AS index_size;
```

### 4.3 性能基准测试

```sql
-- 创建测试函数
CREATE OR REPLACE FUNCTION benchmark_vector_search(
    query_vector vector(1536),
    limit_count INTEGER DEFAULT 50
)
RETURNS TABLE (
    execution_time_ms NUMERIC,
    result_count BIGINT
) AS $$
DECLARE
    start_time TIMESTAMP;
    end_time TIMESTAMP;
    result_count BIGINT;
BEGIN
    start_time := clock_timestamp();

    SELECT COUNT(*) INTO result_count
    FROM (
        SELECT id
        FROM docs
        ORDER BY embedding <-> query_vector
        LIMIT limit_count
    ) sub;

    end_time := clock_timestamp();

    RETURN QUERY SELECT
        EXTRACT(EPOCH FROM (end_time - start_time)) * 1000 AS execution_time_ms,
        result_count;
END;
$$ LANGUAGE plpgsql;

-- 执行基准测试
SELECT * FROM benchmark_vector_search(
    (SELECT embedding FROM docs LIMIT 1),
    50
);
```

## 5. 验证

- 与暴力真值对比 Recall@k；性能基线记录与回归检查。

### 5.1 召回率验证

```sql
-- 创建召回率验证函数
CREATE OR REPLACE FUNCTION verify_recall_at_k(
    query_vector vector(1536),
    k INTEGER DEFAULT 50,
    ground_truth_limit INTEGER DEFAULT 1000
)
RETURNS TABLE (
    recall_at_k NUMERIC,
    precision_at_k NUMERIC
) AS $$
DECLARE
    approximate_results INTEGER[];
    ground_truth_results INTEGER[];
    intersection_count INTEGER;
BEGIN
    -- 近似搜索结果
    SELECT ARRAY_AGG(id ORDER BY embedding <-> query_vector)
    INTO approximate_results
    FROM (
        SELECT id, embedding
        FROM docs
        ORDER BY embedding <-> query_vector
        LIMIT k
    ) sub;

    -- 真实结果（暴力搜索）
    SELECT ARRAY_AGG(id ORDER BY embedding <-> query_vector)
    INTO ground_truth_results
    FROM (
        SELECT id, embedding
        FROM docs
        ORDER BY embedding <-> query_vector
        LIMIT ground_truth_limit
    ) sub;

    -- 计算交集
    SELECT COUNT(*) INTO intersection_count
    FROM unnest(approximate_results) AS a(id)
    WHERE id = ANY(ground_truth_results[1:k]);

    RETURN QUERY SELECT
        intersection_count::NUMERIC / k AS recall_at_k,
        intersection_count::NUMERIC / k AS precision_at_k;
END;
$$ LANGUAGE plpgsql;

-- 执行验证
SELECT * FROM verify_recall_at_k(
    (SELECT embedding FROM docs LIMIT 1),
    50,
    1000
);
```

### 5.2 性能基线记录

```sql
-- 创建性能基线表
CREATE TABLE IF NOT EXISTS vector_search_performance_baseline (
    test_id SERIAL PRIMARY KEY,
    test_time TIMESTAMP DEFAULT NOW(),
    query_type VARCHAR(50),  -- 'vector_only', 'hybrid', 'fulltext'
    index_type VARCHAR(50),   -- 'hnsw', 'ivfflat', 'none'
    k_value INTEGER,
    execution_time_ms NUMERIC,
    recall_at_k NUMERIC,
    result_count BIGINT,
    notes TEXT
);

-- 记录性能基线
INSERT INTO vector_search_performance_baseline (
    query_type,
    index_type,
    k_value,
    execution_time_ms,
    recall_at_k
)
VALUES (
    'vector_only',
    'hnsw',
    50,
    25.5,
    0.95
);
```

## 6. 故障排查

### 6.1 常见问题

**问题1: 索引构建慢**:

```sql
-- 检查索引构建进度
SELECT
    pid,
    datname,
    usename,
    application_name,
    state,
    query
FROM pg_stat_activity
WHERE query LIKE '%CREATE INDEX%';

-- 解决方案：分批构建或使用CONCURRENTLY
CREATE INDEX CONCURRENTLY docs_hnsw ON docs USING hnsw (embedding vector_l2_ops);
```

**问题2: 查询性能差**:

```sql
-- 检查索引是否被使用
EXPLAIN (ANALYZE, BUFFERS)
SELECT id, text
FROM docs
ORDER BY embedding <-> $1
LIMIT 50;

-- 解决方案：
-- 1. 增加ivfflat.probes
-- 2. 检查work_mem设置
-- 3. 考虑重建索引
```

**问题3: 内存不足**:

```sql
-- 检查当前work_mem
SHOW work_mem;

-- 解决方案：增加work_mem或减少查询范围
SET work_mem = '512MB';
```

## 7. 生产运维实践

### 7.1 监控指标

#### 7.1.1 核心指标定义

**查询性能指标**:

- `p50_latency_ms`: 查询延迟中位数
- `p95_latency_ms`: 查询延迟95分位
- `p99_latency_ms`: 查询延迟99分位
- `query_qps`: 每秒查询数
- `error_rate`: 错误率

**召回率指标**:

- `recall_at_k`: Top-K召回率
- `precision_at_k`: Top-K精确率
- `mrr_score`: 平均倒数排名

**资源指标**:

- `index_size_gb`: 索引大小
- `memory_usage_percent`: 内存使用率
- `cpu_usage_percent`: CPU使用率
- `disk_io_read_mb_s`: 磁盘读IO
- `disk_io_write_mb_s`: 磁盘写IO

#### 7.1.2 监控查询

```sql
-- 创建监控视图
CREATE OR REPLACE VIEW vector_search_metrics AS
SELECT
    NOW() AS timestamp,
    'vector_search' AS metric_type,
    COUNT(*) AS total_queries,
    PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY execution_time_ms) AS p50_latency_ms,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY execution_time_ms) AS p95_latency_ms,
    PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY execution_time_ms) AS p99_latency_ms,
    AVG(execution_time_ms) AS avg_latency_ms,
    AVG(recall_at_k) AS avg_recall_at_k
FROM vector_search_performance_baseline
WHERE test_time >= NOW() - INTERVAL '1 hour';

-- 查询当前指标
SELECT * FROM vector_search_metrics;
```

#### 7.1.3 Prometheus指标导出

```sql
-- 创建扩展表用于Prometheus抓取
CREATE TABLE IF NOT EXISTS vector_search_metrics_export (
    timestamp TIMESTAMP DEFAULT NOW(),
    metric_name TEXT,
    metric_value NUMERIC,
    labels JSONB
);

-- 定期更新指标（通过定时任务）
CREATE OR REPLACE FUNCTION update_vector_metrics()
RETURNS void AS $$
BEGIN
    -- 插入查询延迟指标
    INSERT INTO vector_search_metrics_export (metric_name, metric_value, labels)
    SELECT
        'vector_search_p95_latency_ms',
        PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY execution_time_ms),
        jsonb_build_object('index_type', index_type, 'query_type', query_type)
    FROM vector_search_performance_baseline
    WHERE test_time >= NOW() - INTERVAL '5 minutes'
    GROUP BY index_type, query_type;
END;
$$ LANGUAGE plpgsql;
```

### 7.2 告警规则

#### 7.2.1 告警规则定义

**P0级告警（严重）**:

- 查询延迟 P95 > 100ms 持续5分钟
- 错误率 > 1% 持续5分钟
- 召回率 < 90% 持续10分钟
- 索引损坏检测

**P1级告警（警告）**:

- 查询延迟 P95 > 50ms 持续10分钟
- 内存使用率 > 80% 持续5分钟
- CPU使用率 > 85% 持续10分钟
- 索引大小增长异常

#### 7.2.2 Prometheus告警配置

```yaml
# prometheus-alerts.yml
groups:
  - name: vector_search_alerts
    interval: 30s
    rules:
      # P0: 查询延迟告警
      - alert: VectorSearchHighLatency
        expr: vector_search_p95_latency_ms > 100
        for: 5m
        labels:
          severity: critical
          component: vector_search
        annotations:
          summary: "向量查询延迟过高"
          description: "P95延迟 {{ $value }}ms 超过阈值 100ms"

      # P0: 错误率告警
      - alert: VectorSearchHighErrorRate
        expr: rate(vector_search_errors_total[5m]) / rate(vector_search_queries_total[5m]) > 0.01
        for: 5m
        labels:
          severity: critical
        annotations:
          summary: "向量查询错误率过高"
          description: "错误率 {{ $value | humanizePercentage }} 超过阈值 1%"

      # P1: 召回率告警
      - alert: VectorSearchLowRecall
        expr: vector_search_recall_at_k < 0.90
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: "向量查询召回率偏低"
          description: "召回率 {{ $value }} 低于阈值 90%"

      # P1: 内存使用告警
      - alert: VectorSearchHighMemory
        expr: vector_search_memory_usage_percent > 80
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: "向量搜索内存使用过高"
          description: "内存使用率 {{ $value }}% 超过阈值 80%"
```

### 7.3 性能监控

#### 7.3.1 实时性能监控

```sql
-- 创建性能监控函数
CREATE OR REPLACE FUNCTION monitor_vector_search_performance(
    time_window INTERVAL DEFAULT '1 hour'
)
RETURNS TABLE (
    metric_name TEXT,
    metric_value NUMERIC,
    threshold_value NUMERIC,
    status TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        'p95_latency_ms'::TEXT,
        PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY execution_time_ms),
        50.0,
        CASE
            WHEN PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY execution_time_ms) > 50 THEN 'ALERT'
            ELSE 'OK'
        END
    FROM vector_search_performance_baseline
    WHERE test_time >= NOW() - time_window
    GROUP BY 1;
END;
$$ LANGUAGE plpgsql;

-- 执行监控
SELECT * FROM monitor_vector_search_performance('1 hour');
```

#### 7.3.2 Grafana仪表板配置

```json
{
  "dashboard": {
    "title": "Vector Search Performance",
    "panels": [
      {
        "title": "Query Latency (P50/P95/P99)",
        "targets": [
          {
            "expr": "vector_search_p50_latency_ms",
            "legendFormat": "P50"
          },
          {
            "expr": "vector_search_p95_latency_ms",
            "legendFormat": "P95"
          },
          {
            "expr": "vector_search_p99_latency_ms",
            "legendFormat": "P99"
          }
        ]
      },
      {
        "title": "Query QPS",
        "targets": [
          {
            "expr": "rate(vector_search_queries_total[5m])",
            "legendFormat": "QPS"
          }
        ]
      },
      {
        "title": "Recall at K",
        "targets": [
          {
            "expr": "vector_search_recall_at_k",
            "legendFormat": "Recall@K"
          }
        ]
      }
    ]
  }
}
```

### 7.4 索引健康度监控

#### 7.4.1 索引健康度检查

```sql
-- 索引健康度检查函数
CREATE OR REPLACE FUNCTION check_vector_index_health()
RETURNS TABLE (
    index_name TEXT,
    table_name TEXT,
    index_size_gb NUMERIC,
    bloat_ratio NUMERIC,
    index_scan_count BIGINT,
    index_usage_ratio NUMERIC,
    health_status TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        i.indexname::TEXT,
        i.tablename::TEXT,
        pg_size_pretty(pg_relation_size(i.indexrelid))::NUMERIC / (1024*1024*1024) AS index_size_gb,
        -- 简化的膨胀率计算
        CASE
            WHEN pg_stat_user_indexes.idx_scan > 0 THEN
                (pg_relation_size(i.indexrelid) - pg_relation_size(i.indexrelid))::NUMERIC / pg_relation_size(i.indexrelid)
            ELSE 0
        END AS bloat_ratio,
        pg_stat_user_indexes.idx_scan AS index_scan_count,
        CASE
            WHEN pg_stat_user_tables.seq_scan + pg_stat_user_indexes.idx_scan > 0 THEN
                pg_stat_user_indexes.idx_scan::NUMERIC /
                (pg_stat_user_tables.seq_scan + pg_stat_user_indexes.idx_scan)
            ELSE 0
        END AS index_usage_ratio,
        CASE
            WHEN pg_stat_user_indexes.idx_scan = 0 THEN 'UNUSED'
            WHEN (pg_stat_user_indexes.idx_scan::NUMERIC /
                  NULLIF(pg_stat_user_tables.seq_scan + pg_stat_user_indexes.idx_scan, 0)) < 0.5 THEN 'UNDERUSED'
            ELSE 'HEALTHY'
        END AS health_status
    FROM pg_indexes i
    JOIN pg_stat_user_indexes ON i.indexname = pg_stat_user_indexes.indexname
    JOIN pg_stat_user_tables ON i.tablename = pg_stat_user_tables.relname
    WHERE i.indexname LIKE '%vector%' OR i.indexname LIKE '%hnsw%' OR i.indexname LIKE '%ivfflat%';
END;
$$ LANGUAGE plpgsql;

-- 执行检查
SELECT * FROM check_vector_index_health();
```

#### 7.4.2 索引重建建议

```sql
-- 生成索引重建建议
CREATE OR REPLACE FUNCTION get_index_rebuild_recommendations()
RETURNS TABLE (
    index_name TEXT,
    recommendation TEXT,
    priority TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        i.indexname::TEXT,
        CASE
            WHEN pg_stat_user_indexes.idx_scan = 0 THEN '索引未被使用，考虑删除'
            WHEN pg_relation_size(i.indexrelid) > pg_relation_size(i.tablename)::regclass * 2 THEN '索引大小异常，考虑重建'
            WHEN (pg_stat_user_indexes.idx_scan::NUMERIC /
                  NULLIF(pg_stat_user_tables.seq_scan + pg_stat_user_indexes.idx_scan, 0)) < 0.3 THEN '索引使用率低，考虑优化查询'
            ELSE '索引健康'
        END AS recommendation,
        CASE
            WHEN pg_stat_user_indexes.idx_scan = 0 THEN 'LOW'
            WHEN pg_relation_size(i.indexrelid) > pg_relation_size(i.tablename)::regclass * 2 THEN 'HIGH'
            ELSE 'MEDIUM'
        END AS priority
    FROM pg_indexes i
    JOIN pg_stat_user_indexes ON i.indexname = pg_stat_user_indexes.indexname
    JOIN pg_stat_user_tables ON i.tablename = pg_stat_user_tables.relname
    WHERE i.indexname LIKE '%vector%';
END;
$$ LANGUAGE plpgsql;
```

### 7.5 故障处理流程

#### 7.5.1 故障分类

**P0故障（严重）**:

- 服务完全不可用
- 数据损坏
- 索引损坏

**P1故障（高优先级）**:

- 查询性能严重下降（P95 > 100ms）
- 召回率显著下降（< 85%）
- 资源耗尽（内存/CPU）

**P2故障（中优先级）**:

- 查询性能轻微下降（P95 > 50ms）
- 资源使用率偏高

#### 7.5.2 故障处理步骤

**步骤1: 问题定位**:

```sql
-- 检查当前查询状态
SELECT
    pid,
    usename,
    application_name,
    state,
    query_start,
    query
FROM pg_stat_activity
WHERE query LIKE '%vector%' OR query LIKE '%embedding%'
ORDER BY query_start;

-- 检查索引状态
SELECT * FROM check_vector_index_health();

-- 检查资源使用
SELECT
    pg_size_pretty(pg_database_size(current_database())) AS db_size,
    pg_size_pretty(sum(pg_relation_size(indexrelid))) AS total_index_size
FROM pg_stat_user_indexes
WHERE indexrelname LIKE '%vector%';
```

**步骤2: 快速恢复**:

```sql
-- 方案1: 增加work_mem
SET work_mem = '1GB';

-- 方案2: 降低查询精度（临时）
SET ivfflat.probes = 5;  -- 降低probes以提升速度

-- 方案3: 终止慢查询
SELECT pg_terminate_backend(pid)
FROM pg_stat_activity
WHERE query LIKE '%vector%'
  AND state = 'active'
  AND query_start < NOW() - INTERVAL '30 seconds';
```

**步骤3: 根本解决**:

```sql
-- 重建索引（需要维护窗口）
REINDEX INDEX CONCURRENTLY docs_hnsw;

-- 或删除重建
DROP INDEX CONCURRENTLY docs_hnsw;
CREATE INDEX CONCURRENTLY docs_hnsw ON docs
USING hnsw (embedding vector_cosine_ops)
WITH (m=16, ef_construction=64);

-- 更新统计信息
ANALYZE docs;
```

#### 7.5.3 故障处理清单

**P0故障处理清单**:

- [ ] 确认故障影响范围
- [ ] 检查服务状态和错误日志
- [ ] 执行快速恢复操作
- [ ] 验证服务恢复
- [ ] 记录故障根因
- [ ] 制定预防措施

**P1故障处理清单**:

- [ ] 收集性能指标
- [ ] 分析性能下降原因
- [ ] 执行优化操作
- [ ] 验证性能改善
- [ ] 记录优化措施

## 8. 最佳实践

1. **索引选择**: 高维向量用HNSW，大规模数据用IVFFlat
2. **参数调优**: 根据召回率和性能需求平衡probes/ef_search
3. **混合查询**: 先结构化过滤，再向量搜索，最后融合排序
4. **监控指标**: 定期检查查询延迟、召回率、索引使用情况
5. **定期维护**: 定期VACUUM和ANALYZE，重建索引以优化性能
