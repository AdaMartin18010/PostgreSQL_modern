...
- [PostgreSQL语义搜索系统端到端实现完整指南](#postgresql语义搜索系统端到端实现完整指南)
  - [📑 目录](#-目录)
  - [📊 思维导图](#-思维导图)
  - [一、项目概述](#一项目概述)
    - [1.1 功能特性](#11-功能特性)
  - [二、知识矩阵对比](#二知识矩阵对比)
    - [2.1 向量数据库方案对比](#21-向量数据库方案对比)
    - [2.2 搜索架构方案对比](#22-搜索架构方案对比)
  - [三、系统架构](#三系统架构)
  - [四、快速开始](#四快速开始)
    - [4.1 环境要求](#41-环境要求)
    - [4.2 一键启动](#42-一键启动)
  ...
  - [六、数据库设计](#六数据库设计)
    ...
    - [6.2 视图和函数](#62-视图和函数)
  - [七、后端实现](#七后端实现)
    - [7.1 配置管理 (config.py)](#71-配置管理-configpy)
    - [7.2 数据库连接 (database.py)](#72-数据库连接-databasepy)
    - [7.3 嵌入服务 (services/embedding.py)](#73-嵌入服务-servicesembeddingpy)
    - [7.4 文档分块服务 (services/chunking.py)](#74-文档分块服务-serviceschunkingpy)
  - [八、前端实现](#八前端实现)
    - [8.1 搜索组件 (SearchBar.tsx)](#81-搜索组件-searchbartsx)
    - [8.2 搜索结果组件 (SearchResults.tsx)](#82-搜索结果组件-searchresultstsx)
  - [九、Docker部署](#九docker部署)
    - [9.1 docker-compose.yml配置](#91-docker-composeyml配置)
  - [十、性能监控](#十性能监控)
    - [10.1 Prometheus metrics](#101-prometheus-metrics)
  - [十一、完整API实现](#十一完整api实现)
    - [11.1 搜索API (api/search.py)](#111-搜索api-apisearchpy)
    - [11.2 文档上传API (api/documents.py)](#112-文档上传api-apidocumentspy)
  - [十二、测试用例](#十二测试用例)
    - [集成测试 (tests/test\_search.py)](#集成测试-teststest_searchpy)
  - [十三、使用指南](#十三使用指南)
    - [13.1 基本使用流程](#131-基本使用流程)
    - [13.2 Python SDK示例](#132-python-sdk示例)
  - [十四、性能优化](#十四性能优化)
    - [14.1 数据库优化](#141-数据库优化)
    - [14.2 应用层优化](#142-应用层优化)
    - [14.3 批量处理优化](#143-批量处理优化)
  - [十五、故障排查](#十五故障排查)
    - [15.1 常见问题](#151-常见问题)
  - [十六、监控仪表盘](#十六监控仪表盘)
    - [16.1 Grafana配置](#161-grafana配置)
  - [十七、总结](#十七总结)
    - [17.1 完成功能](#171-完成功能)
    - [17.2 性能指标](#172-性能指标)
    - [17.3 扩展建议](#173-扩展建议)
...
|-----|------|------|---------|---------------|
| pgvector | 原生集成、SQL支持、事务一致性 | 大规模性能限制 | 中小规模、需要ACID | ✅ 原生支持 |
| Pinecone | 托管服务、高性能 | 成本高、供应商锁定 | 大规模、无运维需求 | ❌ 外部服务 |
| Weaviate | 图数据库集成、多模态 | 学习曲线陡 | 复杂查询、图关系 | ⚠️ 需要集成 |
| Milvus | 高性能、分布式 | 运维复杂 | 超大规模、专业团队 | ⚠️ 需要集成 |
| Qdrant | Rust实现、高性能 | 功能相对简单 | 高性能需求 | ⚠️ 需要集成 |

### 2.2 搜索架构方案对比

| 架构方案 | 优势 | 劣势 | 适用场景 | 复杂度 |
|---------|------|------|---------|--------|
| 单机架构 | 简单、易维护 | 性能限制 | 小规模、开发测试 | ⭐ |
| 读写分离 | 性能提升 | 数据一致性 | 中等规模、读多写少 | ⭐⭐ |
| 分布式架构 | 高可用、可扩展 | 复杂度高 | 大规模、生产环境 | ⭐⭐⭐⭐ |
| 微服务架构 | 灵活、可扩展 | 运维复杂 | 大型系统、多团队 | ⭐⭐⭐⭐⭐ |
...
### 7.3 嵌入服务 (services/embedding.py)
...
### 7.4 文档分块服务 (services/chunking.py)
