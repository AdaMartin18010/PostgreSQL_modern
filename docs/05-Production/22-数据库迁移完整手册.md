...
从MySQL/Oracle/SQL Server迁移到PostgreSQL 18的完整指南。
...
1. [迁移规划](#迁移规划)
2. [从MySQL迁移](#从MySQL迁移)
3. [从Oracle迁移](#从Oracle迁移)
4. [从SQL Server迁移](#从SQL Server迁移)
5. [跨版本升级](#跨版本升级)
6. [验证与测试](#验证与测试)
...
SELECT user_id, name, MAX(created_at)
FROM orders
GROUP BY user_id, name;  -- 必须列出所有非聚合列
...
sed -i 's/` /"/g' mysql_dump.sql  # 反引号转双引号
...
# 主要差异
# 1. 占位符: %s (MySQL) → %s (PostgreSQL) ✓
# 2. 自动提交: MySQL默认开启 → PostgreSQL默认关闭
conn.autocommit = False  # 显式设置
...
-- 日期时间
DATE            → TIMESTAMP  (注意: Oracle DATE包含时间)
TIMESTAMP       → TIMESTAMP
