...
```mermaid
graph TB
    A[云原生PostgreSQL<br/>核心原则] --> B[容器化]
    A --> C[编排自动化]
    A --> D[弹性伸缩]
    A --> E[可观测性]
    A --> F[不可变基础设施]

    B --> B1[Docker镜像<br/>标准化部署]
    B --> B2[资源隔离<br/>cgroup限制]

    C --> C1[Kubernetes<br/>StatefulSet]
    C --> C2[Operator模式<br/>自动运维]

    D --> D1[水平扩展<br/>只读副本]
    D --> D2[垂直扩展<br/>动态资源]

    E --> E1[Prometheus<br/>指标采集]
    E --> E2[分布式追踪<br/>OpenTelemetry]

    F --> F1[配置即代码<br/>GitOps]
    F --> F2[声明式管理<br/>Yaml定义]
...
# 2. PostgreSQL实际测试
echo -e "\n测试PostgreSQL I/O性能..."
psql -c "DROP TABLE IF EXISTS io_test;"
psql -c "CREATE TABLE io_test AS SELECT * FROM generate_series(1, 10000000);"

# 清理缓存
sudo sh -c "echo 3 > /proc/sys/vm/drop_caches"

# 测试顺序扫描
psql -c "\timing on" -c "SELECT count(*) FROM io_test;"
...
# 方案B：Kubernetes Pod配置
# 添加到Pod spec:
# spec:
#   nodeSelector:
#     node.kubernetes.io/instance-type: numa-optimized
#   resources:
#     requests:
#       cpu: "16"
#       memory: "128Gi"
...
sequenceDiagram
    ...
    Note over Req,Storage: PostgreSQL 17冷启动
    Req->>K8s: 触发扩容
    K8s->>Pod: 创建Pod (30s)
    Pod->>Storage: 挂载卷 (5s)
    Pod->>Pod: 启动PostgreSQL (10s)
    Pod->>Pod: 执行ANALYZE (120s) ⚠️
    Pod-->>Req: 就绪 (总计165s)

    Note over Req,Storage: PostgreSQL 18冷启动
    Req->>K8s: 触发扩容
    K8s->>Pod: 创建Pod (30s)
    Pod->>Storage: 挂载卷 (5s)
    Pod->>Pod: 启动PostgreSQL (10s)
    Pod->>Pod: ✅ 加载保留的统计信息 (2s)
    Pod-->>Req: 就绪 (总计47s)
```
