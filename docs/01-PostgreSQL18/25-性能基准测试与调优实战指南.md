...
./runBenchmark.sh props.pg 2>&1 | tee tpcc_pg17.log
...
./runBenchmark.sh props.pg 2>&1 | tee tpcc_pg18.log

# 8. 对比结果
./generateReport.py tpcc_pg17.log tpcc_pg18.log
```
...
for table in customer lineitem nation orders part partsupp region supplier; do
    echo "导入表: $table..."
    psql -c "\COPY $table FROM '${table}.tbl' DELIMITER '|'"
done
...
pgbench -i -s $SCALE -F 90 $DB_NAME
# -F 90: fillfactor=90，为HOT更新预留空间

# 2. 创建额外索引
psql -d $DB_NAME <<EOF
CREATE INDEX ON pgbench_accounts (abalance);
CREATE INDEX ON pgbench_branches (bbalance);
CREATE INDEX ON pgbench_tellers (tbalance);
VACUUM ANALYZE;
EOF

# 3. 预热缓存
echo "【2/6】预热缓存..."
pgbench -c 20 -j 4 -T 60 -S $DB_NAME > /dev/null
