...
- [PostgreSQL 18 查询优化器增强完整指南](#postgresql-18-查询优化器增强完整指南)
  - [📑 目录](#-目录)
  - [1. 概述与核心突破](#1-概述与核心突破)
    - [1.1 PostgreSQL 18 优化器革命](#11-postgresql-18-优化器革命)
    - [1.2 性能提升概览](#12-性能提升概览)
    - [1.3 优化器架构演进](#13-优化器架构演进)
  - [2. 自连接消除（Self-Join Elimination）](#2-自连接消除self-join-elimination)
    - [2.1 技术原理](#21-技术原理)
      - [2.1.1 问题场景](#211-问题场景)
      - [2.1.2 消除算法](#212-消除算法)
    - [2.2 实战案例](#22-实战案例)
      - [2.2.1 案例1：电商订单查询](#221-案例1电商订单查询)
      - [2.2.2 案例2：视图嵌套](#222-案例2视图嵌套)
    - [2.3 监控与验证](#23-监控与验证)
  - [3. IN/OR表达式优化重写](#3-inor表达式优化重写)
    - [3.1 IN列表优化算法](#31-in列表优化算法)
      - [3.1.1 IN-to-ANY转换](#311-in-to-any转换)
      - [3.1.2 OR-to-IN规范化](#312-or-to-in规范化)
    - [3.2 大规模IN列表性能](#32-大规模in列表性能)
      - [3.2.1 性能测试](#321-性能测试)
      - [3.2.2 大IN列表优化策略](#322-大in列表优化策略)
    - [3.3 OR条件智能重写](#33-or条件智能重写)
      - [3.3.1 场景1：同列多值OR](#331-场景1同列多值or)
      - [3.3.2 场景2：多列OR组合](#332-场景2多列or组合)
    - [3.4 最佳实践与陷阱](#34-最佳实践与陷阱)
      - [3.4.1 ✅ 推荐写法](#341--推荐写法)
      - [3.4.2 ⚠️ 常见陷阱](#342-️-常见陷阱)
  - [4. 分区表智能裁剪增强](#4-分区表智能裁剪增强)
    - [4.1 分区裁剪算法升级](#41-分区裁剪算法升级)
      - [4.1.1 技术原理](#411-技术原理)
      - [4.1.2 性能对比](#412-性能对比)
    - [4.2 复杂条件裁剪](#42-复杂条件裁剪)
      - [4.2.1 多列分区裁剪](#421-多列分区裁剪)
      - [4.2.2 表达式分区裁剪](#422-表达式分区裁剪)
    - [4.3 分区表JOIN优化](#43-分区表join优化)
      - [4.3.1 分区智能连接（Partition-Wise Join）](#431-分区智能连接partition-wise-join)
      - [4.3.2 启用分区智能连接](#432-启用分区智能连接)
    - [4.4 分区表监控](#44-分区表监控)
  - [5. 连接下推优化（Join Pushdown）](#5-连接下推优化join-pushdown)
    - [5.1 外部表JOIN本地化](#51-外部表join本地化)
      - [5.1.1 技术原理](#511-技术原理)
      - [5.1.2 启用连接下推](#512-启用连接下推)
    - [5.2 实战案例：跨数据库查询](#52-实战案例跨数据库查询)
      - [5.2.1 场景：订单系统 + 用户中心](#521-场景订单系统--用户中心)
    - [5.3 下推限制与优化](#53-下推限制与优化)
      - [5.3.1 不支持下推的场景](#531-不支持下推的场景)
      - [5.3.2 优化下推性能](#532-优化下推性能)
  - [6. 子查询去相关化（Decorrelation）](#6-子查询去相关化decorrelation)
    - [6.1 相关子查询的性能灾难](#61-相关子查询的性能灾难)
      - [6.1.1 问题本质](#611-问题本质)
      - [6.1.2 去相关化技术](#612-去相关化技术)
    - [6.2 多层嵌套子查询优化](#62-多层嵌套子查询优化)
      - [6.2.1 复杂场景](#621-复杂场景)
    - [6.3 EXISTS子查询优化](#63-exists子查询优化)
      - [6.3.1 Semi-Join转换](#631-semi-join转换)
      - [6.3.2 NOT EXISTS优化](#632-not-exists优化)
    - [6.4 最佳实践与陷阱](#64-最佳实践与陷阱)
      - [6.4.1 ✅ 推荐写法](#641--推荐写法)
      - [6.4.2 ⚠️ 去相关化失败的场景](#642-️-去相关化失败的场景)
  - [7. 多表JOIN顺序优化](#7-多表join顺序优化)
    - [7.1 动态规划算法增强](#71-动态规划算法增强)
      - [7.1.1 问题规模](#711-问题规模)
      - [7.1.2 PostgreSQL 18改进](#712-postgresql-18改进)
      - [7.1.3 性能对比](#713-性能对比)
    - [7.2 星型模型优化](#72-星型模型优化)
      - [7.2.1 数据仓库场景](#721-数据仓库场景)
      - [7.2.2 启用星型优化](#722-启用星型优化)
    - [7.3 JOIN方法选择优化](#73-join方法选择优化)
      - [7.3.1 三种JOIN算法](#731-三种join算法)
      - [7.3.2 Memoize节点（PG18新增）](#732-memoize节点pg18新增)
  - [文档续写中](#文档续写中)
  - [8. 统计信息驱动的代价模型](#8-统计信息驱动的代价模型)
    - [8.1 统计信息精度提升](#81-统计信息精度提升)
      - [8.1.1 扩展统计信息增强](#811-扩展统计信息增强)
      - [8.1.2 代价估算改进](#812-代价估算改进)
    - [8.2 动态采样优化](#82-动态采样优化)
  - [9. 并行查询规划增强](#9-并行查询规划增强)
    - [9.1 并行度智能评估](#91-并行度智能评估)
      - [9.1.1 并行度决策模型](#911-并行度决策模型)
      - [9.1.2 并行配置优化](#912-并行配置优化)
    - [9.2 并行聚合优化](#92-并行聚合优化)
  - [10. TPC-H性能深度分析](#10-tpc-h性能深度分析)
    - [10.1 TPC-H基准测试概述](#101-tpc-h基准测试概述)
      - [10.1.1 测试环境](#1011-测试环境)
    - [10.2 关键查询性能对比](#102-关键查询性能对比)
      - [10.2.1 Q1: 聚合查询](#1021-q1-聚合查询)
      - [10.2.2 Q2: 子查询去相关化](#1022-q2-子查询去相关化)
      - [10.2.3 Q20: IN列表优化](#1023-q20-in列表优化)
    - [10.3 TPC-H综合性能](#103-tpc-h综合性能)
  - [11. 生产环境调优实战](#11-生产环境调优实战)
    - [11.1 优化器配置策略](#111-优化器配置策略)
      - [11.1.1 开发环境](#1111-开发环境)
      - [11.1.2 生产环境（OLTP）](#1112-生产环境oltp)
      - [11.1.3 生产环境（OLAP）](#1113-生产环境olap)
...
-- 按区域子分区
CREATE TABLE logs_202501_north PARTITION OF logs_202501
    FOR VALUES IN ('North');
CREATE TABLE logs_202501_south PARTITION OF logs_202501
    FOR VALUES IN ('South');
CREATE TABLE logs_202501_east PARTITION OF logs_202501
    FOR VALUES IN ('East');
CREATE TABLE logs_202501_west PARTITION OF logs_202501
    FOR VALUES IN ('West');
...
**暂存点**: 文档将继续扩展至40,000字（4000行）完整版本...
...
CREATE INDEX ON sales_data(product_id, sale_date);
VACUUM ANALYZE sales_data;
