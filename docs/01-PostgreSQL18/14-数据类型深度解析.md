...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'DOUBLE PRECISION累加查询失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'NUMERIC累加查询失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
```
...
EXCEPTION
    WHEN undefined_table THEN
        RAISE NOTICE '表text_test不存在';
    WHEN OTHERS THEN
        RAISE NOTICE '存储空间查询失败: %', SQLERRM;
        ROLLBACK;
        RAISE;
...
EXCEPTION
    WHEN duplicate_table THEN
        RAISE NOTICE '索引idx_search已存在';
    WHEN OTHERS THEN
        RAISE NOTICE '创建GIN索引失败: %', SQLERRM;
        ROLLBACK;
        RAISE;

-- 性能测试：全文搜索（带错误处理和性能分析）
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
