...
```mermaid
graph TB
    A[分区主表<br/>Partitioned Table] --> B1[分区1<br/>Partition 2024-01]
    A --> B2[分区2<br/>Partition 2024-02]
    A --> B3[分区3<br/>Partition 2024-03]
    A --> B4[...<br/>更多分区]
    A --> B5[默认分区<br/>Default Partition]

    B1 --> C1[索引1]
    B2 --> C2[索引2]
    B3 --> C3[索引3]

    Query[查询请求] --> Planner[查询规划器]
    Planner --> Prune[分区裁剪<br/>Partition Pruning]

    Prune -.仅扫描相关分区.-> B2
    Prune -.跳过无关分区.-> Skip1[❌ 2024-01]
    Prune -.跳过无关分区.-> Skip2[❌ 2024-03]
...
flowchart TD
    ...
    D -->|简单比较| E1[快速裁剪<br/>O1算法]
    D -->|复杂表达式| E2[表达式求值<br/>PG18增强]
    D -->|多列条件| E3[多维裁剪<br/>PG18新增]
...
-- 创建分区（区域 + 日期）
CREATE TABLE sales_region1_2024_q1 PARTITION OF sales
    FOR VALUES FROM (1, '2024-01-01') TO (1, '2024-04-01');

CREATE TABLE sales_region1_2024_q2 PARTITION OF sales
    FOR VALUES FROM (1, '2024-04-01') TO (1, '2024-07-01');

CREATE TABLE sales_region2_2024_q1 PARTITION OF sales
...
```mermaid
graph TB
    A[主表 logs] --> B1[2024年<br/>logs_2024]
    A --> B2[2025年<br/>logs_2025]
    ...
    C1 --> D1[01月<br/>logs_2024_01]
    C1 --> D2[02月<br/>logs_2024_02]
    C1 --> D3[03月<br/>logs_2024_03]

    D1 --> E1[华北<br/>logs_2024_01_north]
    D1 --> E2[华东<br/>logs_2024_01_east]
    D1 --> E3[华南<br/>logs_2024_01_south]
...
flowchart TD
    ...
    Q1 -->|时间范围查询| Time[时间列分区<br/>order_date, created_at]
    Q1 -->|区域查询| Region[区域列分区<br/>region, country]
    Q1 -->|均匀分布| Hash[哈希分区<br/>user_id, order_id]
    ...
    Region --> Q3{区域数量?}
    Q3 -->|<20| ListPart[List分区]
    Q3 -->|>20| RangePart[Range分区<br/>region_id]

    Hash --> Q4{并发写入?}
    Q4 -->|高| Hash16[16-32个分区]
    Q4 -->|中| Hash8[8-16个分区]
