...
-- 索引策略2：GIN jsonb_path_ops（优化版，仅支持@>）
CREATE INDEX idx_data_path_ops ON json_table USING gin (data jsonb_path_ops);
...
-- 索引策略3：表达式索引（针对性）
CREATE INDEX idx_data_status ON json_table ((data->>'status'));
...
-- 索引策略4：部分索引
CREATE INDEX idx_data_active ON json_table USING gin (data)
WHERE data->>'status' = 'active';
...
sequenceDiagram
    ...
    User->>App: 提问："PostgreSQL的向量索引有哪些？"
    ...
    PG-->>App: 3. 返回相关文档（Top-5）

    App->>App: 4. 构建prompt
    Note over App: context = concat(docs)<br/>prompt = f"基于上下文{context}回答{question}"
...
```mermaid
flowchart TD
    Start[选择数据库方案] --> Q1{需要ACID事务?}

    Q1 -->|是| Q2{数据模型复杂度?}
    Q1 -->|否| Specialized[专用数据库]

    Q2 -->|单一模型| Q3{性能要求?}
    Q2 -->|多种模型| MultiModal[多模态PostgreSQL]

    Q3 -->|极致性能| Specialized
    Q3 -->|性能够用| MultiModal

    Specialized --> S1[向量: Milvus]
    Specialized --> S2[图: Neo4j]
    Specialized --> S3[时序: InfluxDB]
    ...
    M5 --> Eval[评估性能]
    Eval --> OK{性能满足?}
    OK -->|是| Deploy[部署PostgreSQL]
    OK -->|否| Hybrid[混合方案]

    Hybrid --> H1[热数据: 专用DB]
    Hybrid --> H2[冷数据: PostgreSQL]
