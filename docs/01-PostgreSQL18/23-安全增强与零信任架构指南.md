...
- [PostgreSQL 18 安全增强与零信任架构指南](#postgresql-18-安全增强与零信任架构指南)
  - [📑 目录](#-目录)
  - [1. PostgreSQL安全架构全景](#1-postgresql安全架构全景)
    - [1.1 多层安全模型](#11-多层安全模型)
    - [1.2 安全威胁模型](#12-安全威胁模型)
  - [2. PostgreSQL 18安全增强](#2-postgresql-18安全增强)
...
```mermaid
graph TB
    A[PostgreSQL<br/>多层安全模型] --> B[网络层安全]
    A --> C[认证层安全]
    A --> D[授权层安全]
    A --> E[数据层安全]
    A --> F[审计层安全]
...
ssl_min_protocol_version = 'TLSv1.3'  # ← PG 18强制TLS 1.3
...
# 4. 客户端连接（强制SSL）
psql "postgresql://user@host/db?sslmode=require&sslrootcert=/path/to/ca.crt"
```
...
passwordcheck.min_upper = 2
passwordcheck.min_lower = 2
passwordcheck.min_digit = 2
passwordcheck.min_special = 1

-- 3. 创建用户（自动使用SCRAM）
CREATE ROLE app_user WITH LOGIN PASSWORD 'SecureP@ssw0rd123!';
...
```mermaid
graph TB
    A[PostgreSQL<br/>认证方法] --> B[密码认证]
    A --> C[证书认证]
    A --> D[外部认证]
    A --> E[特殊认证]

    B --> B1[SCRAM-SHA-256<br/>✅ 推荐]
    B --> B2[MD5<br/>❌ 已弃用]
    B --> B3[Password<br/>❌ 明文，危险]

    C --> C1[SSL Certificate<br/>双向认证]
    C --> C2[clientcert=verify-full]

    D --> D1[LDAP<br/>企业目录]
    D --> D2[Kerberos/GSSAPI<br/>域认证]
    D --> D3[RADIUS<br/>网络设备]
    D --> D4[PAM<br/>系统认证]
    D --> D5[OAuth 2.0<br/>PG18新增]

    E --> E1[peer<br/>本地Unix用户]
    E --> E2[ident<br/>身份映射]
    E --> E3[trust<br/>⚠️ 无密码]
...
```mermaid
graph TB
    A[零信任架构<br/>Zero Trust] --> B[永不信任<br/>Never Trust]
    A --> C[始终验证<br/>Always Verify]
    A --> D[最小权限<br/>Least Privilege]
