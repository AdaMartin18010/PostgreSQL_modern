...
2. **2022 年**: Neon 实现基于 COW 的数据库分支
3. **2023 年**: Supabase 添加分支支持
4. **2025 年**: 数据库分支技术成为 Serverless 数据库标准特性
...
2. **数据读取**: 读取时从父分支读取，无需复制
3. **数据写入**: 写入时创建数据副本，实现隔离
...
2. **创建分支元数据**: 创建分支记录，关联父分支快照
3. **初始化 COW 存储**: 创建增量存储空间
4. **注册分支**: 将分支注册到分支管理器
...
- [Copy-on-Write 技术](https://en.wikipedia.org/wiki/Copy-on-write)
