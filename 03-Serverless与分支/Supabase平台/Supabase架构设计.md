...
- [3.3 Supabase 架构设计](#33-supabase-架构设计)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
    - [1.1 技术背景](#11-技术背景)
    - [1.2 架构定位](#12-架构定位)
...
2. **混合搜索**: 集成向量搜索和全文搜索
3. **认证授权**: 完整的用户认证和权限管理
4. **API 自动生成**: 基于数据库自动生成 REST API

**技术演进**:

1. **2020 年**: Supabase 项目启动
2. **2022 年**: 集成 pgvector 支持向量搜索
3. **2024 年**: 支持混合搜索和实时功能
4. **2025 年**: 成为 AI 应用的主流后端平台
...
1. **订阅优化**: 只订阅需要的表和事件
2. **连接管理**: 合理管理 WebSocket 连接
3. **错误处理**: 实现重连和错误处理机制

### 6.2 混合搜索优化

1. **索引优化**: 为向量和全文搜索创建合适索引
2. **查询优化**: 使用 RRF 算法融合结果
3. **缓存策略**: 缓存热点查询结果

### 6.3 安全最佳实践

1. **RLS 策略**: 为所有表启用 RLS
2. **API 密钥**: 安全管理 API 密钥
3. **权限控制**: 最小权限原则
