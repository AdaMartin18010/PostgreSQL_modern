...
时钟同步是分布式系统的核心问题，从1970年代Lamport提出逻辑时钟开始，时钟同步就一直是分布式系统研究的重点。
2010年代，Google提出了TrueTime，CockroachDB提出了HLC，为分布式系统提供了不同的时钟同步方案。
理解时钟同步，有助于掌握分布式系统时间管理方法、理解不同时钟方案的权衡、避免常见的设计错误。
