...
print(f"平均TPS误差: {backtest_results['avg_tps_error']*100:.1f}%")
print(f"平均延迟误差: {backtest_results['avg_latency_error']*100:.1f}%")
```

**回测结果统计**:

| 案例类型 | TPS误差 | P99延迟误差 | 样本数 |
|---------|---------|------------|--------|
| 电商秒杀 | 12% | 18% | 5 |
| OLAP分析 | 15% | 22% | 3 |
| 金融交易 | 10% | 15% | 4 |
