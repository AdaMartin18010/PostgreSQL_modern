...
- [02 | 隔离级别选择器](#02--隔离级别选择器)
  ...
  - [七、部署与使用](#七部署与使用)
    - [7.1 快速部署](#71-快速部署)
    - [7.2 预设场景使用](#72-预设场景使用)
    - [7.3 性能数据引用](#73-性能数据引用)
...
PRESET_SCENARIOS = {
    'seckill': {
        'name': '电商秒杀系统',
        'description': '高并发秒杀场景，库存竞争激烈',
        'has_concurrent_update': True,
        'tolerate_lost_update': False,
        'allow_phantom': True,
        'accept_performance_loss': False,
        'concurrency': 50000,
        'data_type': 'seckill',
        'expected_recommendation': 'Read Committed + 乐观锁'
    },
    'financial': {
        'name': '金融转账系统',
        'description': '金融交易系统，要求强一致性',
        'has_concurrent_update': True,
        'tolerate_lost_update': False,
        'allow_phantom': False,
        'accept_performance_loss': True,
        'concurrency': 1000,
        'data_type': 'financial',
        'expected_recommendation': 'Serializable'
    },
    'social': {
        'name': '社交网络系统',
        'description': '社交平台，读多写少，允许最终一致性',
        'has_concurrent_update': False,
        'tolerate_lost_update': True,
        'allow_phantom': True,
        'accept_performance_loss': False,
        'concurrency': 100000,
        'data_type': 'social',
        'expected_recommendation': 'Read Committed'
    },
    'analytics': {
        'name': 'OLAP分析系统',
        'description': '数据分析系统，主要是查询操作',
        'has_concurrent_update': False,
        'tolerate_lost_update': True,
        'allow_phantom': True,
        'accept_performance_loss': False,
...
class IsolationRequest(BaseModel):
    ...
    concurrency: int
    data_type: str  # 'financial' | 'social' | 'web' | 'seckill' | 'analytics' | 'ecommerce'
...
@app.get("/preset-scenarios")
async def get_preset_scenarios() -> Dict[str, Dict]:
    """获取所有预设场景"""
    return {
...
@app.post("/recommend-from-preset/{scenario_name}")
async def recommend_from_preset(scenario_name: str) -> IsolationResponse:
    """从预设场景推荐隔离级别"""
    ...
    req = IsolationRequest(**{k: v for k, v in scenario.items()
                              if k not in ['name', 'description', 'expected_recommendation']})
    return await recommend_isolation(req)
...
```bash
# 1. 克隆仓库
git clone https://github.com/db-theory/isolation-level-selector.git
cd isolation-level-selector
...
```bash
# 获取所有预设场景
curl http://localhost:8000/preset-scenarios

# 从预设场景推荐
curl -X POST http://localhost:8000/recommend-from-preset/seckill
...
- `06-性能分析/04-量化对比实验.md`: 隔离级别性能对比数据
- `09-工业案例库/`: 真实系统性能数据
- `11-工具与自动化/01-并发控制决策助手.md`: 综合性能评估

**数据使用**:

- 推荐结果中的性能影响数据来自量化对比实验
- 预设场景的性能数据来自工业案例库
- 权衡分析基于实际测试数据
