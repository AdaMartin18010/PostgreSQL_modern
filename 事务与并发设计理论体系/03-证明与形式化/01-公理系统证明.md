...
- [01 | 公理系统证明](#01--公理系统证明)
  ...
  - [八、证明树可视化](#八证明树可视化)
    - [8.1 公理1证明树](#81-公理1证明树)
    - [8.2 公理2证明树](#82-公理2证明树)
    - [8.3 公理3证明树](#83-公理3证明树)
    - [8.4 公理独立性证明树](#84-公理独立性证明树)
    - [8.5 基础定理推导树](#85-基础定理推导树)
  - [九、Coq形式化](#九coq形式化)
    - [9.1 公理定义](#91-公理定义)
...
在数学和计算机科学的发展中，公理化方法一直是建立严格理论体系的基础。
从欧几里得几何（公元前300年）到现代集合论（1900年代），公理化方法帮助我们将复杂的理论建立在少数几个基本公理之上。
在数据库理论中，ACID特性、事务隔离级别等概念需要严格的数学基础，公理系统为此提供了形式化框架。
...
```mermaid
graph TD
    A[公理1: 状态原子性] --> B[定理2.1: WAL保证原子性]
    A --> C[定理2.2: 所有权保证内存安全]
    A --> D[定理2.3: Raft保证日志原子性]

    B --> B1[引理2.1: WAL记录不可分]
    B --> B2[引理2.2: 提交前所有修改已写入WAL]
    B --> B3[引理2.3: 崩溃恢复重放WAL]
    B1 --> B4[组合引理2.1-2.3]
    B2 --> B4
    B3 --> B4
    B4 --> B5[∴ Atomic T]
    ...
    C --> C1[引理2.4: 所有权唯一性]
    C --> C2[引理2.5: 借用检查保证无别名]
    C --> C3[引理2.6: 生命周期保证引用有效]
    C1 --> C4[组合引理2.4-2.6]
    C2 --> C4
    C3 --> C4
    C4 --> C5[∴ No dangling pointers, No data races]

    D --> D1[引理2.7: Leader只在多数派确认后提交]
    D --> D2[引理2.8: 已提交的日志不会丢失]
    D1 --> D3[组合引理2.7-2.8]
    D2 --> D3
    D3 --> D4[∴ Atomic e]
...
```mermaid
graph TD
    A[公理2: 可见性偏序] --> B[定理3.1: L0可见性满足偏序]
    A --> C[定理3.2: happens-before是偏序]
    A --> D[定理3.3: HLC保持偏序]

    B --> B1[非自反性: ¬Visible txn txn]
    B --> B2[传递性: Visible T1 T2 ∧ Visible T2 T3 → Visible T1 T3]
    B --> B3[反对称性: Visible T1 T2 → ¬Visible T2 T1]
    B1 --> B4[组合性质]
    B2 --> B4
    B3 --> B4
    B4 --> B5[∴ 严格偏序]

    C --> C1[非自反性: 事件不能发生在自己之前]
    C --> C2[传递性: 传递闭包保证]
    C --> C3[反对称性: 时间不可逆]
    C1 --> C4[组合性质]
    C2 --> C4
    C3 --> C4
    C4 --> C5[∴ happens-before is partial order]
...
```mermaid
graph TD
    A[公理3: 冲突可串行化] --> B[定理4.1: SSI检测所有违反]
    A --> C[定理4.2: 借用检查杜绝数据竞争]

    B --> B1[引理4.1: 存在环当且仅当不可串行化]
    B --> B2[引理4.2: SSI检测所有危险结构]
    B1 --> B3[SSI跟踪三种依赖]
    B2 --> B3
    B3 --> B4[检测到环 → 中止事务]
    B4 --> B5[∴ SSI ensures Serializability]

    C --> C1[引理4.3: 可变引用唯一]
    C --> C2[引理4.4: 数据竞争需要并发可变访问]
    C1 --> C3[组合引理4.3-4.4]
    C2 --> C3
    C3 --> C4[∴ No data races]
...
graph TD
    ...
    D --> D1[基于三大公理]
    D1 --> D2[步骤1: 日志条目提交是原子的]
    D1 --> D3[步骤2: 提交顺序形成全序]
    D1 --> D4[步骤3: 冲突通过Leader序列化]
    D2 --> D5[∴ Linearizable]
    D3 --> D5
    D4 --> D5
