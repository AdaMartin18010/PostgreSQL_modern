...
- [06 | 多维矩阵集（完整版）](#06--多维矩阵集完整版)
  ...
  - [十二、反例与错误使用](#十二反例与错误使用)
    ...
    - [反例2: 评分不客观](#反例2-评分不客观)
  - [十二、无锁算法对比矩阵](#十二无锁算法对比矩阵)
    - [12.1 无锁算法进度保证对比矩阵](#121-无锁算法进度保证对比矩阵)
    - [12.2 无锁数据结构性能对比矩阵](#122-无锁数据结构性能对比矩阵)
    - [12.3 无锁算法与锁机制对比矩阵](#123-无锁算法与锁机制对比矩阵)
    - [12.4 ABA问题解决方案对比矩阵](#124-aba问题解决方案对比矩阵)
    - [12.5 原子操作原语对比矩阵](#125-原子操作原语对比矩阵)
    - [12.6 内存排序语义对比矩阵](#126-内存排序语义对比矩阵)
    - [12.7 无锁算法正确性条件对比矩阵](#127-无锁算法正确性条件对比矩阵)
    - [12.8 无锁算法设计模式对比矩阵](#128-无锁算法设计模式对比矩阵)
    - [12.9 无锁算法硬件平台对比矩阵](#129-无锁算法硬件平台对比矩阵)
    - [12.10 无锁算法语言支持对比矩阵](#1210-无锁算法语言支持对比矩阵)
...
| 原语 | 硬件实现 | 性能 (cycles) | 适用场景 | 复杂度 | 平台支持 |
|------|---------|--------------|---------|--------|---------|
| **CAS** | x86: CMPXCHG<br>ARM: LL/SC | 10-50 (x86)<br>20-100 (ARM) | 通用无锁算法 | O(1) | 所有平台 |
| **FAA** | x86: XADD<br>ARM: LDADD | 10-50 (x86)<br>20-100 (ARM) | 计数器、累加器 | O(1) | 所有平台 |
| **Exchange** | x86: XCHG | 10-50 | 交换操作 | O(1) | x86 |
| **LL/SC** | ARM, RISC-V | 20-100 | 实现CAS | O(1) | ARM, RISC-V |

**性能对比** (Intel Xeon, 1000万次操作):

```text
CAS (CMPXCHG):     ████████████████████ 50M ops/sec
FAA (XADD):        ████████████████████ 50M ops/sec
Exchange (XCHG):   ████████████████████ 50M ops/sec
```

### 12.6 内存排序语义对比矩阵

| 排序语义 | 顺序保证 | 性能开销 | 适用场景 | 硬件屏障 | 编译器优化 |
|---------|---------|---------|---------|---------|-----------|
| **Relaxed** | 仅原子性 | 最低 | 计数器、标志位 | 无 | 完全优化 |
| **Acquire** | 后续操作不重排 | 低 | 读取共享数据 | Load屏障 | 部分优化 |
| **Release** | 之前操作不重排 | 低 | 发布共享数据 | Store屏障 | 部分优化 |
| **AcqRel** | Acquire + Release | 中 | 读-修改-写 | Load+Store屏障 | 部分优化 |
| **SeqCst** | 全局顺序一致性 | 最高 | 需要全局顺序 | 全屏障 | 最少优化 |
...
| 正确性条件 | 实时顺序 | 全局顺序 | 因果顺序 | 实现复杂度 | 性能 | 适用场景 |
|-----------|---------|---------|---------|-----------|------|---------|
| **线性化性** | 保持 | 是 | 保持 | 高 | 低 | 大多数无锁算法 |
| **顺序一致性** | 不保持 | 是 | 保持 | 中 | 中 | 部分无锁算法 |
| **因果一致性** | 不保持 | 否 | 保持 | 低 | 高 | 分布式无锁算法 |

**层次关系**:

$$\text{线性化性} \implies \text{顺序一致性} \implies \text{因果一致性}$$

### 12.8 无锁算法设计模式对比矩阵

| 设计模式 | 实现复杂度 | 性能 | 内存开销 | 适用场景 | 典型应用 |
|---------|-----------|------|---------|---------|---------|
| **CAS循环** | 低 | 高（低竞争） | 低 | 通用 | 计数器、栈 |
| **帮助机制** | 中 | 高 | 低 | 队列、哈希表 | Michael & Scott队列 |
| **标记指针** | 低 | 高 | 低 | ABA问题解决 | 无锁栈（ABA安全） |
| **危险指针** | 中 | 中 | 中 | 内存管理 | 无锁数据结构 |
| **引用计数** | 中 | 中 | 中 | 自动内存管理 | GC语言无锁算法 |
| **工作窃取** | 高 | 高 | 中 | 任务调度 | 并行计算框架 |

### 12.9 无锁算法硬件平台对比矩阵

| 平台 | CAS实现 | CAS性能 | FAA支持 | 内存模型 | 扩展性 | 典型应用 |
|------|---------|---------|---------|---------|--------|---------|
| **x86-64** | CMPXCHG | 快 (10-50 cycles) | 是 (XADD) | TSO | 好 | 服务器、PC |
| **ARM64** | LL/SC | 中 (20-100 cycles) | 是 (LDADD) | 弱内存模型 | 中 | 移动设备、服务器 |
| **RISC-V** | LL/SC | 中 (20-100 cycles) | 部分 | 弱内存模型 | 中 | 嵌入式、研究 |
| **PowerPC** | LL/SC | 中 (20-100 cycles) | 是 | 弱内存模型 | 中 | 服务器 |

**性能对比** (CAS操作, 1000万次):

```text
x86-64:     ████████████████████ 50M ops/sec
ARM64:      ████████████████     35M ops/sec (-30%)
RISC-V:     ███████████████      35M ops/sec (-30%)
PowerPC:    ██████████████       30M ops/sec (-40%)
```

### 12.10 无锁算法语言支持对比矩阵

| 语言 | 原子操作 | 内存模型 | 内存安全 | 性能 | 易用性 | 典型应用 |
|------|---------|---------|---------|------|--------|---------|
| **Rust** | std::sync::atomic | C++11兼容 | 编译期保证 | 最优 | 中 | 系统编程 |
| **C++** | std::atomic | C++11标准 | 运行时 | 最优 | 中 | 系统编程 |
| **Java** | java.util.concurrent.atomic | JMM | GC保证 | 中 | 高 | 应用编程 |
| **Go** | sync/atomic | 弱保证 | GC保证 | 中 | 高 | 应用编程 |
| **C** | 编译器内置 | 平台相关 | 无 | 最优 | 低 | 底层编程 |
