...
> **📖 概念词典引用**：本文档中的 happens-before、内存排序等概念定义与 [核心概念词典](../00-理论框架总览/01-核心概念词典.md) 保持一致。如发现不一致，请以核心概念词典为准。
...
- [07 | 内存模型与排序](#07--内存模型与排序)
  ...
  - [四、与C++内存模型对比](#四与c内存模型对比)
    ...
    - [4.2 关键差异](#42-关键差异)
  - [五、形式化语义](#五形式化语义)
    - [5.1 内存模型一致性定理](#51-内存模型一致性定理)
      - [5.1.1 形式化定义](#511-形式化定义)
      - [5.1.2 内存模型一致性定理](#512-内存模型一致性定理)
    - [5.2 不同内存模型之间关系的证明](#52-不同内存模型之间关系的证明)
      - [5.2.1 内存模型层次关系](#521-内存模型层次关系)
    - [5.3 Promising语义](#53-promising语义)
...
在多核处理器普及之前（2000年代之前），单核处理器时代，程序执行顺序与代码顺序基本一致。
但随着多核处理器的普及，CPU缓存、指令重排序、内存可见性等问题使得多线程程序的执行结果变得不可预测。
这促使编程语言和硬件定义了内存模型来规范多线程程序的语义。
...
**定义5.1.1 (内存模型一致性 - Lamport, 1979)**:

内存模型一致性是指：所有线程观察到的操作顺序是一致的。

**形式化表示**:

$$\forall \text{thread } T_1, T_2: \forall \text{operation } op:$$

$$\text{Observed}(T_1, op) \land \text{Observed}(T_2, op) \implies$$

$$\text{Order}(T_1, op) = \text{Order}(T_2, op)$$

其中：

- $\text{Observed}(T, op)$: 线程$T$观察到操作$op$
- $\text{Order}(T, op)$: 线程$T$观察到的操作$op$的顺序

**定义5.1.2 (顺序一致性 - Lamport, 1979)**:

顺序一致性是最强的内存模型一致性，要求：

1. **程序顺序**: 每个线程的操作按程序顺序执行
2. **全局顺序**: 所有线程的操作形成一个全局顺序
3. **一致性**: 所有线程观察到的全局顺序一致

**形式化表示**:

$$\exists \text{global order } G: \forall \text{thread } T:$$

$$(\forall op_1, op_2 \in T: op_1 \prec_{po} op_2 \implies op_1 \prec_G op_2) \land$$

$$(\forall \text{thread } T': \text{Observed}(T', G))$$

#### 5.1.2 内存模型一致性定理

**定理5.1 (内存模型一致性 - Lamport, 1979)**:

如果内存模型满足happens-before关系，则保证内存模型一致性。

$$\text{HappensBefore}(MemoryModel) \implies \text{Consistency}(MemoryModel)$$

**证明**:

**引理5.1.1**: happens-before关系是偏序

**证明**:

- 根据定义1.2：happens-before关系满足传递性、非自反性、反对称性
- 因此，happens-before关系是偏序 ∎

**引理5.1.2**: 偏序关系保证一致性

**证明**:

- 偏序关系定义：如果$e_1 \xrightarrow{hb} e_2$，则$e_1$在$e_2$之前
- 所有线程观察到的happens-before关系一致
- 因此，偏序关系保证一致性 ∎
...
1. happens-before关系是偏序（引理5.1.1）
2. 偏序关系保证一致性（引理5.1.2）

**因此**: 如果内存模型满足happens-before关系，则保证内存模型一致性 ∎
...
1. **全局顺序存在**: 所有操作形成全局顺序$G$
2. **程序顺序保持**: 每个线程的操作按程序顺序在$G$中
3. **一致性观察**: 所有线程观察到的全局顺序一致
...
**引理5.3.1**: SeqCst保证顺序一致性

**证明**:

- SeqCst（顺序一致性）保证所有操作形成全局顺序
- 因此，SeqCst保证顺序一致性 ∎

**引理5.3.2**: AcqRel保证Release-Acquire一致性

**证明**:

- AcqRel = Acquire + Release
- 因此，AcqRel保证Release-Acquire一致性 ∎

**引理5.3.3**: Release-Acquire保证Relaxed一致性

**证明**:

- Release-Acquire建立同步点，保证happens-before关系
- Relaxed仅保证原子性，无同步保证
- 因此，Release-Acquire保证Relaxed一致性 ∎

**不同内存模型一致性强度证明**:

根据引理5.3.1-5.3.3：

1. SeqCst保证顺序一致性（引理5.3.1）
2. AcqRel保证Release-Acquire一致性（引理5.3.2）
3. Release-Acquire保证Relaxed一致性（引理5.3.3）

**因此**: $SeqCst > AcqRel > Release/Acquire > Relaxed$ ∎
...
$$SeqCst \supset AcqRel \supset Release/Acquire \supset Relaxed$$

其中 $\supset$ 表示"强于"（保证更多）。

**证明**:

**定义5.2.1 (内存模型强度)**:

内存模型$M_1$强于$M_2$（$M_1 \supset M_2$），当且仅当：

$$\forall \text{program } P: \text{Valid}(P, M_1) \implies \text{Valid}(P, M_2)$$

即：在$M_1$下有效的程序，在$M_2$下也有效。

**引理5.4.1**: $SeqCst \supset AcqRel$

**证明**:

- SeqCst保证全局顺序，所有操作形成全序
- AcqRel仅保证Release-Acquire同步点
- 因此，SeqCst保证更多，$SeqCst \supset AcqRel$ ∎

**引理5.4.2**: $AcqRel \supset Release/Acquire$

**证明**:

- AcqRel = Acquire + Release（同时保证读写同步）
- Release/Acquire仅保证单向同步
- 因此，AcqRel保证更多，$AcqRel \supset Release/Acquire$ ∎

**引理5.4.3**: $Release/Acquire \supset Relaxed$

**证明**:

- Release-Acquire建立同步点，保证happens-before关系
- Relaxed仅保证原子性，无同步保证
- 因此，Release-Acquire保证更多，$Release/Acquire \supset Relaxed$ ∎

**内存模型层次关系证明**:

根据引理5.4.1-5.4.3：

$$SeqCst \supset AcqRel \supset Release/Acquire \supset Relaxed \quad \square$$
...
内存模型$M_1$和$M_2$等价（$M_1 \equiv M_2$），当且仅当：

$$\forall \text{program } P: \text{Valid}(P, M_1) \iff \text{Valid}(P, M_2)$$

**等价条件**:

1. **单线程程序**: 所有内存模型等价（无并发）
2. **无共享数据**: 所有内存模型等价（无竞争）
3. **仅Relaxed操作**: 所有内存模型等价（无同步）

**因此**: 在某些条件下，不同的内存模型可以等价 ∎
...
graph TB
    subgraph "内存排序层次"
        SEQ[SeqCst<br/>顺序一致性<br/>最强保证]
        ACQ[AcqRel<br/>获取-释放<br/>中等保证]
        REL[Release-Acquire<br/>释放-获取<br/>同步保证]
        RELAX[Relaxed<br/>宽松排序<br/>最弱保证]
    end

    subgraph "硬件支持"
        MB[内存屏障<br/>Memory Barrier]
        CAS[原子操作<br/>Compare-and-Swap]
        CACHE[缓存一致性<br/>MESI协议]
    end

    subgraph "应用场景"
        SYNC[同步原语<br/>Mutex/RwLock]
        LOCKFREE[无锁数据结构<br/>Lock-Free]
        COUNTER[原子计数器<br/>Atomic Counter]
    end
...
graph LR
    subgraph "线程1"
        T1_W1[写x=1<br/>Release]
        T1_W2[写y=2<br/>Release]
    end
