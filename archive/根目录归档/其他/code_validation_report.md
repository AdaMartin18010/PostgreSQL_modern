...
### 02-查询与优化\02.08-联邦查询\FDW外部数据包装器完整实战指南.md
...
-- 在低峰期手动VACUUM
SELECT cron.schedule('vacuum-fla
```
...
### 19-实战案例\02-OLAP分析系统\04-核心实现.md
...
### 19-实战案例\03-IoT时序数据系统\02-架构设计.md
...
### 19-实战案例\08-知识图谱问答系统\05-性能测试.md
...
### 19-实战案例\10-金融反欺诈系统\03-数据库设计.md

**行 441** (sql):

```sql
-- 自动创建下月分区（定时任务）
SELECT create_detection_partition(
    DATE_TRUNC('month', CURRENT_DATE + INTERVAL '1 month')::DATE
);

-- 删除旧分区（保留6个月）
DROP TABLE IF EXISTS detection_results_2024_06;
...
SELECT upsert_fraud_rule(
    ...
    p_actions := '{"score": 80, "reason": "大额交易"}':
```
