...
- [分布式数据库一致性模型与共识协议](#分布式数据库一致性模型与共识协议)
  ...
  - [2. 共识协议](#2-共识协议)
    - [2.1 Paxos 协议族](#21-paxos-协议族)
    - [2.2 Raft 协议](#22-raft-协议)
    - [2.3 拜占庭容错](#23-拜占庭容错)
  - [3. 仲裁与读写策略](#3-仲裁与读写策略)
    - [3.1 Quorum 机制](#31-quorum-机制)
    - [3.2 读写策略](#32-读写策略)
  - [4. PostgreSQL 生态实践](#4-postgresql-生态实践)
    - [4.1 基于 PostgreSQL 的一致性实现](#41-基于-postgresql-的一致性实现)
    - [4.2 Citus 分布式一致性](#42-citus-分布式一致性)
...
- 定义：保持因果关系的一致性，如果 A 操作在 B 操作之前发生，则所有节点都看到 A 在 B 之前
- 特点：保持逻辑因果关系，允许并发操作
- 实现：向量时钟、依赖追踪
- 适用：社交网络、协作编辑
...
- Tendermint：基于 PBFT 的改进
- HotStuff：线性视图变更的 BFT
- 异步 BFT：处理异步网络环境
...
- 读 Quorum（R）：读取操作需要的最小节点数
- 写 Quorum（W）：写入操作需要的最小节点数
- 总节点数（N）：集群总节点数
- 约束：R + W > N
