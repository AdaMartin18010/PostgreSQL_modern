...
-- 详细验证（返回详细信息）
SELECT * FROM bt_index_check('idx_orders_customer_id', true);

-- 验证表的所有索引
SELECT bt_index_parent_check('orders');
```
...
-- B-Tree索引：创建时间（时间序列查询）
CREATE INDEX idx_logs_created_at ON logs(created_at)
WITH (fillfactor = 75);

-- 复合B-Tree索引：应用ID+时间（组合查询）
