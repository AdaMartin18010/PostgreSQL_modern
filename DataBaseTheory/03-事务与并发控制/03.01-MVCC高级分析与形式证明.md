...
- ⭐⭐⭐ [并发控制与MVCC机制](../PostgreSQL/01-核心课程/01.05-并发控制与MVCC机制.md) - MVCC实现详解
- ⭐⭐⭐ [事务管理与ACID特性](../PostgreSQL/01-核心课程/01.04-事务管理与ACID特性.md) - 事务管理基础
...
- 📖 [核心理论模型总览](../../90-事务与并发设计理论体系/01-核心理论模型/README.md) - 事务与并发设计的核心理论基础

**形式化证明**:

- 📖 [MVCC正确性证明](../../90-事务与并发设计理论体系/03-证明与形式化/02-MVCC正确性证明.md) - PostgreSQL MVCC机制的完整正确性证明，包含Coq/TLA+形式化代码
- 📖 [串行化证明](../../90-事务与并发设计理论体系/03-证明与形式化/03-串行化证明.md) - 串行化理论的完整数学证明
- 📖 [公理系统证明](../../90-事务与并发设计理论体系/03-证明与形式化/01-公理系统证明.md) - 三大公理的严格数学证明

**实现机制**:

- 📖 [PostgreSQL MVCC实现](../../90-事务与并发设计理论体系/05-实现机制/01-PostgreSQL-MVCC实现.md) - PostgreSQL MVCC源码级分析
- 📖 [PostgreSQL VACUUM机制](../../90-事务与并发设计理论体系/05-实现机制/03-PostgreSQL-VACUUM机制.md) - VACUUM机制源码分析

**可视化与思维模型**:

- 📖 [证明树图集](../../90-事务与并发设计理论体系/07-可视化与思维模型/08-证明树图集.md) - 47+个完整证明树，包括MVCC正确性证明树
- 📖 [决策树图集](../../90-事务与并发设计理论体系/07-可视化与思维模型/03-决策树图集.md) - 31+个实用决策树
...
3. **Snapshot isolation** (<https://en.wikipedia.org/wiki/Snapshot_isolation>)
   ...
   - **深度解读**: Wikipedia条目介绍了快照隔离的基本概念，包括事务快照、读写规则和异常类型（如写偏序、更新丢失）。本文档深入分析了快照隔离的异常谱系，提供了完整的异常分类和形式化定义。特别是，本文档展示了PostgreSQL如何通过SSI（Serializable Snapshot Isolation）来防止这些异常，并提供了严格的数学证明。
   - **批判性分析**:
...
4. **Serializability** (<https://en.wikipedia.org/wiki/Serializability>)
   ...
   - **深度解读**: Wikipedia条目介绍了可串行化的基本定义，即事务执行结果等价于某个串行执行。本文档深入分析了可串行化的检测方法（如冲突图检测）和实现方式（如2PL、SSI）。特别是，本文档详细证明了SSI如何通过跟踪读写依赖关系来实现可串行化，这是PostgreSQL Serializable隔离级别的理论基础。
   - **批判性分析**:
...
5. **Transaction isolation** (<https://en.wikipedia.org/wiki/Isolation_(database_systems)>)
   ...
   - **深度解读**: Wikipedia条目介绍了ANSI SQL定义的四个隔离级别：读未提交（Read Uncommitted）、读已提交（Read Committed）、可重复读（Repeatable Read）和可串行化（Serializable）。本文档通过统一形式化模型完整刻画了所有隔离级别，包括快照隔离和SSI。特别是，本文档展示了PostgreSQL如何实现这些隔离级别，并提供了严格的正确性证明。
   - **批判性分析**:
