...
在1980年代，随着多处理器系统的出现，传统的锁机制在多核环境下性能急剧下降。
研究者开始探索不使用锁的并发算法，提出了无锁（Lock-Free）和无等待（Wait-Free）的概念。
...
impl LockFreeCounter {
    ...
    pub fn increment(&self) {
        loop {
            ...
            let new = old.wrapping_add(1);  // 使用wrapping_add防止溢出
...
fn cas_loop_example(atomic: &AtomicUsize, compute: fn(usize) -> usize) -> usize {
    loop {
        ...
        match atomic.compare_exchange_weak(
            ...
            Err(_) => {
                // 失败，重试
                // 注意：compare_exchange_weak可能虚假失败（spurious failure）
                // 即使值未改变也可能返回Err，这是正常的
            }
        }
    }
}
...
fn main() {
    ...
    // 再次执行
    let result = cas_loop_example(&atomic, |x| x * 2);
    println!("Result: {}", result);  // 输出: Result: 0 (因为0*2=0)
...
impl LockFreeCounter {
    ...
    pub fn increment(&self) -> Result<usize, &'static str> {
        const MAX_RETRIES: usize = 1000;  // 防止无限重试
        let mut retries = 0;
...
无锁算法正确性可以通过以下三个维度统一证明：

$$Correctness = (Linearizability, Progress, MemorySafety)$$

其中：

- $Linearizability$: 线性化性保证（操作原子性）
- $Progress$: 进度保证（无锁性、无等待性）
- $MemorySafety$: 内存安全性（无悬垂指针、无内存泄漏）
...
**引理8.5.1**: 线性化性是正确性的必要条件

**证明**:

- 如果算法不满足线性化性，则存在操作顺序不一致
- 这会导致线程看到不同的操作顺序
- 因此，算法不正确
- 因此，线性化性是正确性的必要条件 ∎

**引理8.5.2**: 进度保证是正确性的必要条件

**证明**:

- 如果算法不满足进度保证（死锁），则某些线程永远无法完成操作
- 这会导致系统无法取得进展
- 因此，算法不正确
- 因此，进度保证是正确性的必要条件 ∎

**引理8.5.3**: 内存安全性是正确性的必要条件

**证明**:

- 如果算法不满足内存安全性（悬垂指针、内存泄漏），则会导致未定义行为
- 这会导致系统崩溃或数据损坏
- 因此，算法不正确
- 因此，内存安全性是正确性的必要条件 ∎

**引理8.5.4**: 线性化性、进度保证和内存安全性共同保证正确性

**证明**:

- 线性化性保证操作原子性（引理8.5.1）
- 进度保证保证系统能够取得进展（引理8.5.2）
- 内存安全性保证系统不会崩溃（引理8.5.3）
- 因此，三者共同保证算法正确性 ∎
...
1. 线性化性是正确性的必要条件（引理8.5.1）
2. 进度保证是正确性的必要条件（引理8.5.2）
3. 内存安全性是正确性的必要条件（引理8.5.3）
4. 三者共同保证正确性（引理8.5.4）

**因此**: 无锁算法正确，当且仅当满足线性化性、进度保证和内存安全性 ∎

**定理8.5.2 (线性化点存在性)**:

如果无锁算法的每个操作都有线性化点，则算法满足线性化性。
...
- 如果操作$op_1$在$op_2$之前完成，则$LinearizationPoint(op_1) < LinearizationPoint(op_2)$
- 因此，线性化点保持实时顺序 ∎
...
1. 线性化点定义全局顺序（引理8.5.5）
2. 线性化点保持实时顺序（引理8.5.6）

**因此**: 如果每个操作都有线性化点，则算法满足线性化性 ∎

**定理8.5.3 (CAS操作的线性化点)**:

CAS（Compare-And-Swap）操作的成功时刻是线性化点。
...
**证明**:
- 无锁算法不使用锁，因此不会因为锁竞争而死锁
- 即使某些线程被延迟，其他线程仍能继续执行
- 因此，无锁算法不会死锁 ∎

**引理8.5.8**: 无锁算法保证系统级进度

**证明**:
- 根据无锁性定义：至少有一个线程能够取得进展
- 即使某些线程被延迟，系统整体仍能取得进展
- 因此，无锁算法保证系统级进度 ∎
...
1. 无锁算法不会死锁（引理8.5.7）
2. 无锁算法保证系统级进度（引理8.5.8）

**因此**: 无锁算法保证至少有一个线程能够取得进展 ∎
...
1. **安全性**: 不会回收正在使用的内存（无悬垂指针）
2. **完整性**: 所有不再使用的内存最终被回收（无内存泄漏）
3. **无锁性**: 回收过程不阻塞其他线程
...
**定理14.1.1 (Hazard Pointer正确性 - Michael, 2004)**:

Hazard Pointer算法正确，满足安全性、完整性和无锁性。

**证明**:

**引理14.1.1**: Hazard Pointer保证安全性

**证明**:
- Hazard Pointer机制：线程在使用指针前，将其标记为"危险"
- 回收算法检查：如果指针被标记为危险，则不回收
- 因此，正在使用的内存不会被回收
- 因此，Hazard Pointer保证安全性 ∎

**引理14.1.2**: Hazard Pointer保证完整性

**证明**:
- 当线程不再使用指针时，清除危险标记
- 回收算法定期检查：如果指针未被标记为危险，则可以回收
- 因此，所有不再使用的内存最终被回收
- 因此，Hazard Pointer保证完整性 ∎

**引理14.1.3**: Hazard Pointer保证无锁性

**证明**:
- Hazard Pointer使用原子操作标记/清除危险指针
- 回收算法使用原子操作检查危险指针
- 因此，回收过程不阻塞其他线程
- 因此，Hazard Pointer保证无锁性 ∎

**Hazard Pointer正确性定理证明**:

根据引理14.1.1-14.1.3：

1. Hazard Pointer保证安全性（引理14.1.1）
2. Hazard Pointer保证完整性（引理14.1.2）
3. Hazard Pointer保证无锁性（引理14.1.3）

**因此**: Hazard Pointer算法正确 ∎

**定理14.1.2 (Epoch-Based Reclamation正确性 - Fraser, 2004)**:

Epoch-Based Reclamation算法正确，满足安全性、完整性和无锁性。
...
- 每个线程维护当前epoch
- 内存节点标记为某个epoch
- 回收算法：回收所有epoch小于当前全局epoch的节点

**引理14.1.4**: Epoch-Based保证安全性

**证明**:
- 线程在使用内存前，更新当前epoch
- 回收算法：只回收epoch小于全局epoch的节点
- 如果线程正在使用内存，其epoch >= 全局epoch
- 因此，正在使用的内存不会被回收
- 因此，Epoch-Based保证安全性 ∎

**引理14.1.5**: Epoch-Based保证完整性

**证明**:
- 当线程不再使用内存时，epoch可能小于全局epoch
- 回收算法：定期推进全局epoch，回收旧epoch的节点
- 因此，所有不再使用的内存最终被回收
- 因此，Epoch-Based保证完整性 ∎

**引理14.1.6**: Epoch-Based保证无锁性

**证明**:
- Epoch-Based使用原子操作更新epoch
- 回收算法使用原子操作推进全局epoch
- 因此，回收过程不阻塞其他线程
- 因此，Epoch-Based保证无锁性 ∎
...
1. Epoch-Based保证安全性（引理14.1.4）
2. Epoch-Based保证完整性（引理14.1.5）
3. Epoch-Based保证无锁性（引理14.1.6）

**因此**: Epoch-Based Reclamation算法正确 ∎

**定理14.1.3 (内存回收算法安全性比较)**:
