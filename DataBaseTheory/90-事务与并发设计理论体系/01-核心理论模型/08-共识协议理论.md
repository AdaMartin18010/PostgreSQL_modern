...
> **📖 概念词典引用**：本文档中的 Raft、Paxos 等概念定义与 [核心概念词典](../00-理论框架总览/01-核心概念词典.md) 保持一致。如发现不一致，请以核心概念词典为准。
...
- [08 | 共识协议理论](#08--共识协议理论)
  ...
  - [二、共识问题](#二共识问题)
    ...
    - [1.2 FLP不可能定理](#12-flp不可能定理)
      - [1.2.1 FLP不可能定理的详细证明](#121-flp不可能定理的详细证明)
  - [二、Raft协议](#二raft协议)
    - [2.1 核心思想](#21-核心思想)
  ...
  - [四、共识协议对比](#四共识协议对比)
    ...
    - [4.1.1 Raft和Paxos等价性定理](#411-raft和paxos等价性定理)
      - [4.1.1.1 形式化定义](#4111-形式化定义)
      - [4.1.1.2 Raft和Paxos等价性定理](#4112-raft和paxos等价性定理)
    - [4.2 性能对比](#42-性能对比)
...
确定性算法是指：给定相同的输入和状态，算法总是产生相同的输出。

**定义1.2.3 (临界配置 - Critical Configuration)**:

临界配置是指：存在一个配置$C$，使得：

- 如果某个消息延迟，算法可能决定值$v_1$
- 如果该消息立即到达，算法可能决定值$v_2$
- 其中$v_1 \neq v_2$

**严格证明（反证法）**:

**假设**: 存在确定性的共识算法$A$，在异步网络中即使有一个进程故障也能保证终止。

**步骤1: 证明存在初始双值配置**

**引理1.2.1**: 存在初始配置，使得算法可能决定不同的值

**证明**:

- 根据有效性要求：决定的值必须是某个进程提议的
- 如果所有进程提议相同值，则算法决定该值
- 如果进程提议不同值，则算法可能决定不同值
- 因此，存在初始配置使得算法可能决定不同值 ∎

**步骤2: 证明存在临界配置**

**引理1.2.2**: 如果算法保证终止，则存在临界配置

**证明（归纳法）**:

**基础情况**: 初始配置可能是双值的（引理1.2.1）

**归纳步骤**: 假设存在双值配置$C$，考虑所有可能的下一步：
...
**引理1.2.3**: 可以构造无限延迟的执行序列，使得算法永远无法终止
...
1. 从临界配置$C$开始
2. 选择可能决定值$v_1$的消息$m_1$，延迟它
3. 执行其他消息，可能决定值$v_2$（$v_2 \neq v_1$）
4. 如果算法决定$v_2$，则与假设矛盾（算法应该等待$m_1$）
5. 如果算法不决定，则继续延迟$m_1$，执行其他消息
6. 重复步骤2-5，构造无限延迟序列

**关键洞察**: 在临界配置中，算法的行为取决于消息到达的顺序。通过控制消息延迟，可以防止算法终止。

**因此**: 可以构造无限延迟的执行序列，使得算法永远无法终止 ∎
...
- 根据假设：算法$A$保证终止
- 根据引理1.2.3：可以构造无限延迟序列，使得算法无法终止
- 矛盾：算法既保证终止，又无法终止 ✗

**因此**: 假设不成立，不存在确定性的共识算法能够保证终止 ∎

**定理1.2 (FLP不可能定理的必然性)**:

FLP不可能定理是分布式系统的根本限制，无法完全避免。

**证明**:

**引理1.2.4**: 异步网络是分布式系统的必然特征

**证明**:

- 网络延迟不确定（网络拥塞、路由变化）
- 进程执行速度不确定（CPU调度、负载变化）
- 因此，异步网络是分布式系统的必然特征 ∎

**引理1.2.5**: 进程故障是分布式系统的必然特征
...
1. 异步网络是必然特征（引理1.2.4）
2. 进程故障是必然特征（引理1.2.5）
3. 根据FLP不可能定理：异步网络 + 进程故障 → 无法保证终止

**因此**: FLP不可能定理是分布式系统的根本限制 ∎
...
### 4.1.1 Raft和Paxos等价性定理

#### 4.1.1.1 形式化定义

**定义4.1.1 (共识协议等价性)**:

两个共识协议$P_1$和$P_2$等价，当且仅当：

$$\forall \text{execution } E: \text{Output}(E, P_1) = \text{Output}(E, P_2)$$

其中：

- $\text{Output}(E, P)$: 协议$P$在执行$E$下的输出（决定的值）

**定义4.1.2 (Raft和Multi-Paxos的关系)**:

$$Raft \approx Multi\text{-}Paxos + \text{Strong Leader} + \text{Log Matching}$$

其中：

- $\text{Strong Leader}$: 强Leader机制（Raft要求严格Leader）
- $\text{Log Matching}$: 日志匹配保证（Raft要求日志连续）

#### 4.1.1.2 Raft和Paxos等价性定理

**定理4.1.1 (Raft和Multi-Paxos等价性 - Ongaro & Ousterhout, 2014)**:

在相同的执行环境下，Raft和Multi-Paxos在安全性（Safety）方面等价，即它们保证相同的共识属性。

$$\text{Safety}(Raft) \equiv \text{Safety}(Multi\text{-}Paxos)$$

**证明**:

**引理4.1.1**: Raft和Multi-Paxos都保证一致性（Agreement）

**证明**:

**Raft一致性保证**:

- Raft要求：只有包含所有已提交日志的节点才能成为Leader
- 因此，所有已提交的日志在所有节点上一致
- 因此，Raft保证一致性 ∎

**Multi-Paxos一致性保证**:

- Multi-Paxos要求：只有多数派接受的提案才能被学习
- 因此，所有被学习的值在所有节点上一致
- 因此，Multi-Paxos保证一致性 ∎

**因此**: Raft和Multi-Paxos都保证一致性 ∎

**引理4.1.2**: Raft和Multi-Paxos都保证有效性（Validity）

**证明**:

**Raft有效性保证**:

- Raft要求：日志条目只能由Leader创建
- Leader从客户端接收值，写入日志
- 因此，决定的值必须是某个客户端提议的
- 因此，Raft保证有效性 ∎
...
- Multi-Paxos要求：提案值由Proposer提出
- Proposer从客户端接收值，提出提案
- 因此，决定的值必须是某个Proposer提议的
- 因此，Multi-Paxos保证有效性 ∎

**因此**: Raft和Multi-Paxos都保证有效性 ∎

**引理4.1.3**: Raft和Multi-Paxos都保证持久性（Durability）

**证明**:

**Raft持久性保证**:

- Raft要求：已提交的日志条目持久化到磁盘
- 即使节点故障，已提交的日志不会丢失
- 因此，Raft保证持久性 ∎

**Multi-Paxos持久性保证**:

- Multi-Paxos要求：已学习的值持久化到磁盘
- 即使节点故障，已学习的值不会丢失
- 因此，Multi-Paxos保证持久性 ∎

**因此**: Raft和Multi-Paxos都保证持久性 ∎

**Raft和Multi-Paxos等价性定理证明**:

根据引理4.1.1-4.1.3：

1. Raft和Multi-Paxos都保证一致性（引理4.1.1）
2. Raft和Multi-Paxos都保证有效性（引理4.1.2）
3. Raft和Multi-Paxos都保证持久性（引理4.1.3）

**因此**: Raft和Multi-Paxos在安全性方面等价 ∎

**定理4.1.2 (Raft和Multi-Paxos的差异)**:

虽然Raft和Multi-Paxos在安全性上等价，但在实现细节上存在差异。

**证明**:

**差异1: Leader机制**:

- **Raft**: 强Leader机制，要求严格Leader
- **Multi-Paxos**: 弱Leader机制，允许Leader变更
...
- **Raft**: 使用随机超时选举
- **Multi-Paxos**: 使用提案编号竞争

**因此**: Raft和Multi-Paxos在实现细节上存在差异 ∎

**定理4.1.3 (Raft和Multi-Paxos的性能等价性)**:

在稳定Leader的情况下，Raft和Multi-Paxos的性能等价。
...
1. **延迟**:
   - **Raft**: 1.5 RTT（选举 + 日志复制）
   - **Multi-Paxos**: 1 RTT（跳过Phase 1，直接Phase 2）
   - **稳定Leader**: 两者都是1 RTT（Raft跳过选举）

2. **吞吐量**:
   - **Raft**: 受限于Leader的日志复制速度
   - **Multi-Paxos**: 受限于Leader的提案接受速度
   - **稳定Leader**: 两者性能相似

**因此**: 在稳定Leader的情况下，Raft和Multi-Paxos的性能等价 ∎
