...
> **📖 概念词典引用**：本文档中涉及的 MVCC、VACUUM、Transaction 等概念定义与 [核心概念词典](../00-理论框架总览/01-核心概念词典.md) 保持一致。如发现不一致，请以核心概念词典为准。
...
- [05 | IoT时序数据系统](#05--iot时序数据系统)
  ...
  - [五、性能测试](#五性能测试)
    - [5.1 写入性能测试](#51-写入性能测试)
      - [5.1.1 不同写入模式下的性能表现](#511-不同写入模式下的性能表现)
      - [5.1.2 不同分区策略下的写入性能](#512-不同分区策略下的写入性能)
...
**测试场景**: 固定设备数10万，写入总速率100万条/秒，对比不同写入模式

| 写入模式 | TPS | P50延迟 | P99延迟 | WAL写入量 | CPU使用率 | 适用性 |
|---------|-----|---------|---------|-----------|----------|--------|
| 单条INSERT | 150K | 20ms | 180ms | 800MB/min | 85% | 仅适合低写入场景 |
| 100行批量INSERT | 650K | 12ms | 65ms | 420MB/min | 72% | 一般批量写入 |
| **1000行批量INSERT** | **1.2M** | **8ms** | **25ms** | **260MB/min** | **68%** | **推荐，用于高写入吞吐** |

**分析**:

- 批量大小从1 → 1000时，TPS 提升约 **8×**，P99延迟从180ms降至25ms
- WAL写入量显著下降，CPU使用率也从85%降至68%

#### 5.1.2 不同分区策略下的写入性能

**测试场景**: 固定写入速率100万条/秒，对比按日/按小时分区

| 分区策略 | 分区数量（1年） | TPS | P99延迟 | VACUUM开销 | 索引维护成本 | 适用性 |
|---------|----------------|-----|---------|------------|--------------|--------|
| 按月分区 | 12 | 900K | 35ms | 低 | 低 | 冷热分层简单，适合中等写入 |
| 按日分区 | 365 | **1.2M** | **25ms** | 中 | 中 | **推荐，写入/查询平衡** |
| 按小时分区 | 8760 | 1.25M | 22ms | 高 | 高 | 分区过多，管理复杂 ✗ |

**结论**:
按**日分区**在写入性能、管理复杂度和VACUUM开销之间取得最佳平衡，是IoT时序数据的推荐方案。
