...
- [08 | 知识图谱系统](#08--知识图谱系统)
  ...
  - [三、数据模型](#三数据模型)
    - [3.1 实体表](#31-实体表)
    - [3.2 关系表（边表）](#32-关系表边表)
    - [3.3 路径缓存表（物化）](#33-路径缓存表物化)
  - [四、查询实现](#四查询实现)
    - [4.1 单跳查询（邻居）](#41-单跳查询邻居)
    - [4.2 多跳查询（递归CTE）](#42-多跳查询递归cte)
    - [4.3 最短路径（双向BFS）](#43-最短路径双向bfs)
    - [4.4 子图提取](#44-子图提取)
  - [五、性能优化](#五性能优化)
    - [5.1 索引策略](#51-索引策略)
    - [5.2 物化路径](#52-物化路径)
    - [5.3 查询重写优化](#53-查询重写优化)
  - [六、实验评估](#六实验评估)
...
**创建日期**: 2025-12-05
**最后更新**: 2025-12-05
**新增内容**: 完整Python实现、查询优化器、物化路径维护、生产案例、反例分析
