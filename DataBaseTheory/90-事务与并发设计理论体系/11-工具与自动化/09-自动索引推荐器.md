...
class IndexBenefitAnalyzer:
    ...
    def _estimate_cost_with_index(self,
        ...
        selectivity = self._calculate_selectivity(
            index_candidate.table,
            index_candidate.columns,
            row_count
        )

        # 2. 选择性检查（最佳实践：选择性 < 1% 的索引通常不推荐）
        if selectivity < 0.01:
            # 选择性太低，索引效果差，返回高成本
            return current_cost * 0.95  # 几乎无改善

        # 3. 基于PostgreSQL成本模型估算索引扫描成本
        # 成本 = CPU成本 + IO成本
        # CPU成本: log2(行数) * cpu_index_tuple_cost
        # IO成本: 选择性 * 表大小 * random_page_cost

        cpu_index_tuple_cost = 0.005  # PostgreSQL默认值
        random_page_cost = 4.0  # PostgreSQL默认值

        index_cost = (
            np.log2(max(row_count * selectivity, 1)) * cpu_index_tuple_cost +  # CPU成本
            selectivity * (table_size / 8192) * random_page_cost  # IO成本（页数）
        )

        # 4. 考虑索引维护成本（写入开销）
        # 参考最佳实践：索引会增加写入成本，需要权衡
        write_overhead = self._estimate_write_overhead(index_candidate, row_count)

        # 最终成本 = 查询成本 + 写入开销权重
        final_cost = index_cost + write_overhead * 0.1  # 写入开销权重10%

        return min(final_cost, current_cost * 0.1)  # 至少降低90%
    ...
    def _calculate_selectivity(
        ...
        try:
            ...
            if len(columns) == 1:
                cur.execute(f"""
                    SELECT n_distinct
                    FROM pg_stats
                    WHERE tablename = '{table}' AND attname = '{columns[0]}';
                """)
                row = cur.fetchone()
                if row and row[0] > 0:
                    distinct_count = abs(row[0])  # n_distinct可能是负数（表示比例）
                    if distinct_count < 1:
                        distinct_count = row_count * distinct_count
        ...
        try:
            ...
            cur.execute(f"""
                SELECT COUNT(DISTINCT ({columns_str}))
                FROM {table};
            """)
            row = cur.fetchone()
            if row:
        ...
        # 默认：假设均匀分布
        return 1.0 / row_count if row_count > 0 else 1.0

    def _estimate_write_overhead(
        self,
        index_candidate: IndexCandidate,
        row_count: int
    ) -> float:
        ...
        base_overhead = 0.001  # 基础开销（毫秒）

        # 复合索引的开销通常小于多个单列索引
        if column_count > 1:
            overhead = base_overhead * (1 + column_count * 0.5)  # 复合索引折扣
        else:
            overhead = base_overhead * column_count
...
class IndexRecommender:
    ...
    def analyze_slow_queries(
        ...
        Args:
            min_time_ms: 最小执行时间（毫秒），低于此值的查询不分析
            use_slow_query_log: 是否使用慢查询日志
            slow_query_log_path: 慢查询日志文件路径
        """
...
**工具版本**: 2.1.0（数据源扩展和最佳实践联动）
**最后更新**: 2025-12-18
**新增内容**:
- 完整SQL解析、索引候选生成、收益评估、ROI计算
- 自动索引推荐器背景与演进（为什么需要自动索引推荐器、历史背景、理论基础、核心挑战）
- 自动索引推荐器反例补充（6个新增反例：自动索引推荐器使用不当、忽略推荐验证、推荐算法参数错误、自动索引推荐器监控不足）
- **新增**: 基于pg_stat_statements和慢查询日志的数据源支持
- **新增**: 与索引最佳实践联动（选择性检查、代价评估、写入开销权衡）
