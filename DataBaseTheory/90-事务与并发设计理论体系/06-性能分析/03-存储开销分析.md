...
在MVCC系统中，存储开销是一个关键问题。MVCC通过维护多个版本来实现并发控制，但这也带来了存储开销。从PostgreSQL早期版本开始，存储膨胀问题就一直困扰着用户。理解存储开销的构成和演化规律，有助于优化存储使用、避免存储膨胀、诊断存储问题。
...
graph TB
    subgraph "存储开销层"
        BASE[基础存储<br/>Base Storage]
        VERSIONS[版本链存储<br/>Version Chain]
        INDEXES[索引存储<br/>Index Storage]
        OVERHEAD[元数据开销<br/>Overhead]
        WAL[WAL日志<br/>Write-Ahead Log]
    end

    subgraph "优化层"
        VACUUM[VACUUM清理<br/>Dead Tuple Removal]
        HOT[HOT优化<br/>Heap-Only Tuple]
        FILLFACTOR[Fillfactor<br/>预留空间]
    end

    subgraph "监控层"
        MONITOR[存储监控<br/>Storage Monitor]
        ALERT[告警系统<br/>Alert System]
        RECOMMEND[优化建议<br/>Recommendations]
    end
...
```mermaid
flowchart TD
    START([INSERT操作]) --> CREATE[创建初始版本<br/>xmin=100, xmax=0]
    CREATE --> STORAGE1[存储开销: 1个元组]

    STORAGE1 --> UPDATE1{UPDATE操作?}
    UPDATE1 -->|是| NEW_VERSION1[创建新版本<br/>xmin=101, xmax=0]
    UPDATE1 -->|否| COMMIT1[COMMIT]

    NEW_VERSION1 --> OLD_VERSION1[标记旧版本<br/>xmax=101]
    OLD_VERSION1 --> STORAGE2[存储开销: 2个元组]

    STORAGE2 --> UPDATE2{继续UPDATE?}
    UPDATE2 -->|是| NEW_VERSION2[创建新版本<br/>xmin=102, xmax=0]
    UPDATE2 -->|否| VACUUM_CHECK{VACUUM检查}

    NEW_VERSION2 --> OLD_VERSION2[标记旧版本<br/>xmax=102]
    OLD_VERSION2 --> STORAGE3[存储开销: 3个元组]

    STORAGE3 --> VACUUM_CHECK
    VACUUM_CHECK -->|死元组可清理| VACUUM[VACUUM清理]
    VACUUM_CHECK -->|有活跃事务| WAIT[等待]

    VACUUM --> REMOVE[移除死元组]
    REMOVE --> STORAGE4[存储开销: 1个元组<br/>恢复]
