...
- **定义**: PostgreSQL MVCC中存储事务提交状态的核心数据结构（PostgreSQL 10+中称为pg_xact）。Clog使用位图结构存储每个事务的提交状态，每个事务占用2位，可以表示四种状态：进行中、已提交、已回滚、子事务已提交。Clog是MVCC可见性判断的权威数据源，当需要判断元组的可见性时，系统查询Clog获取创建或删除该元组的事务的提交状态
- **形式化**: $$\text{Clog}(xid) \in \{IN\_PROGRESS, COMMITTED, ABORTED, SUB\_COMMITTED\}$; $T_{clog\_query} = T_{page\_read} + T_{byte\_extract} = O(1)$; $S_{clog} = \frac{N_{transactions} \times 2}{8} \text{ bytes}$$
- **作用**: 提供事务状态的权威数据源，支持MVCC可见性判断，存储效率高（每个事务0.25字节）
- **存储**: pg_xact目录，按页面组织，每个页面存储32,768个事务状态（8KB页面）
- **查询性能**: O(1)直接访问，1-100μs（取决于页面位置）
- **空间效率**: 相比传统存储方式，空间节省94%（1,000,000事务仅需250KB）
- **相关**: [02-MVCC理论完整解析.md](../01-核心理论模型/02-MVCC理论完整解析.md#607-clog-提交日志-完整定义与分析), [01-PostgreSQL-MVCC实现.md](../05-实现机制/01-PostgreSQL-MVCC实现.md)
...
- **定义**: PostgreSQL MVCC中防止32位事务ID回卷的关键机制。Freeze操作将旧元组标记为永久可见，通过设置特殊的冻结标志位（PostgreSQL 9.4+）或替换xmin为FrozenTransactionId（PostgreSQL < 9.4），确保这些元组对所有事务始终可见，无论事务ID是否回卷。Freeze是VACUUM操作的重要组成部分，防止XID回卷导致的数据可见性错误和系统崩溃
- **形式化**:

$$
\text{NeedsFreeze}(\tau) \iff (\text{CurrentXid} - \tau.xmin) > \text{FreezeMaxAge} \land \tau.xmin < \text{OldestXmin}$; $\text{FrozenTransactionId} = 2$; $\forall \text{snapshot } s, \forall \text{transaction } T: \text{Visible}(\tau, s) \text{ if } \tau.xmin = \text{FrozenTransactionId}
$$

- **作用**: 防止32位事务ID回卷，标记旧元组为永久可见，保证系统稳定性
- **条件**: 元组年龄超过FreezeMaxAge（默认200M）且xmin < OldestXmin
- **方式**: 设置冻结标志位（PostgreSQL 9.4+）或替换xmin为FrozenTransactionId（PostgreSQL < 9.4）
- **性能**: Freeze开销很小（< 0.1μs per tuple），但对系统稳定性影响大
- **相关**: [02-MVCC理论完整解析.md](../01-核心理论模型/02-MVCC理论完整解析.md#53-freeze-完整定义与分析), [01-PostgreSQL-MVCC实现.md](../05-实现机制/01-PostgreSQL-MVCC实现.md)
...
- **定义**: PostgreSQL MVCC中减少索引写放大的性能优化机制。HOT优化允许UPDATE操作在满足特定条件时，在同页内创建新版本而不更新索引。HOT更新的新版本通过ctid指针与旧版本连接，形成HOT链。由于索引仍然指向旧版本的物理位置（ItemId），通过HOT链可以访问新版本，因此无需创建新的索引项，从而显著减少索引写放大，提升UPDATE性能
- **形式化**:

$$
\text{HOTUpdate}(old, new, page) \iff \neg \text{IndexedColumnsChanged}(old, new) \land \text{SamePage}(old, new) \land \text{HasFreeSpace}(page, new)$; $\text{IndexWrites}_{HOT} = 0$; $\text{IndexWrites}_{traditional} = O(n)$ for $n$ indexes
$$

- **作用**: 减少索引写放大，避免索引更新，提升UPDATE性能（2-10×，取决于索引数量）
- **条件**: 未修改索引列、新版本在同一页内、页面有足够空闲空间
- **机制**: HOT链通过ctid指针连接同页内的版本，索引指向链头，通过HOT链访问新版本
- **性能**: HOT更新将索引写操作从$O(n)$优化为$0$，更新性能提升2-10×（取决于索引数量）
- **优化**: 合理设计索引（只对查询需要的列建索引）、设置合适的fillfactor（70-80）提升HOT使用率
- **相关**: [02-MVCC理论完整解析.md](../01-核心理论模型/02-MVCC理论完整解析.md#61-hot-heap-only-tuple-完整定义与分析), [01-PostgreSQL-MVCC实现.md](../05-实现机制/01-PostgreSQL-MVCC实现.md)
...
$$
\text{HintBits}(\tau) = \{XMIN\_COMMITTED, XMIN\_INVALID, XMAX\_COMMITTED, XMAX\_INVALID\}$; $\text{UseHintBit}(\tau, \text{bit}) \iff \text{HintBitSet}(\tau, \text{bit}) \land \text{Valid}(\tau, \text{bit})
$$
...
- **定义**: PostgreSQL MVCC中优化覆盖索引查询的性能优化机制。
  Index-Only Scan允许查询直接从索引获取数据，无需访问堆表，从而显著减少I/O操作。
  然而，由于MVCC需要检查元组的可见性，系统必须访问堆表或使用Visibility Map来优化可见性检查。
  当Visibility Map标记页面为全可见时，Index-Only Scan可以跳过堆访问，直接从索引返回数据，从而显著提升查询性能

- **形式化**:

$$
\text{IndexOnlyScan}(q, I) \iff \text{CoveringIndex}(I, q) \land \text{AllColumnsInIndex}(q, I)$; $\text{IndexOnlyScan}(I, q) = \begin{cases}\text{IndexData}(I, q) & \text{if } \text{AllVisible}(\text{Page}(I, q)) = 1 \\\text{IndexData}(I, q) \cup \text{HeapCheck}(I, q) & \text{otherwise}\end{cases}
$$

- **作用**: 优化覆盖索引查询性能，允许查询直接从索引获取数据，查询性能提升2-10×
- **前提**: 覆盖索引（所有查询列在索引中）
- **优化**: Visibility Map标记全可见页面，允许跳过堆访问
- **性能**: Index-Only Scan将查询延迟降低2-10×（取决于Visibility Map覆盖率）
- **设计**: 创建覆盖索引包含所有查询列，或使用INCLUDE子句（PostgreSQL 11+）
- **相关**: [02-MVCC理论完整解析.md](../01-核心理论模型/02-MVCC理论完整解析.md#62-index-only-scan-完整定义与分析), [01-PostgreSQL-MVCC实现.md](../05-实现机制/01-PostgreSQL-MVCC实现.md)
...
- **定义**: 悲观并发控制的核心协议，通过将事务执行分为增长阶段（只获取锁）和收缩阶段（只释放锁）来保证可串行化。PostgreSQL在写操作中使用严格2PL，所有锁在事务提交或回滚时才释放，防止级联回滚
- **形式化**: $$\forall T: \exists t_{\text{lock\_point}}: \forall t < t_{\text{lock\_point}}: \text{Acquire}(T) \land \neg\text{Release}(T) \land \forall t > t_{\text{lock\_point}}: \text{Release}(T) \land \neg\text{Acquire}(T)$; $Strict\_2PL \equiv 2PL \land (\forall \text{lock } L: \text{Release}(L) \text{ only at commit/abort})$$
- **作用**: 保证可串行化，预防冲突，适合高冲突场景
- **阶段**: 增长阶段（只获取锁）、收缩阶段（只释放锁）
- **锁类型**: 共享锁（S-lock，读操作）、排他锁（X-lock，写操作）
- **死锁**: 2PL可能导致死锁，需要死锁检测或死锁预防机制
- **性能**: 2PL适合高冲突场景（TPS = 10,000+），不适合读多场景（读操作需要加锁，性能差）
- **PostgreSQL实现**: 写操作使用严格2PL，所有锁在事务提交/回滚时释放
- **相关**: [05-并发控制理论统一框架.md](../01-核心理论模型/05-并发控制理论统一框架.md#21-两阶段锁-2pl-完整定义与分析), [02-PostgreSQL-锁机制.md](../05-实现机制/02-PostgreSQL-锁机制.md)
