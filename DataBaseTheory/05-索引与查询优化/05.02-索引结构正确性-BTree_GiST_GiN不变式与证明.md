...
```sql
-- PostgreSQL 18：使用UUIDv7作为主键
-- 1. 安装uuid-ossp扩展（如果使用uuid_generate_v7）
-- 或者使用pg_uuidv7扩展
...
-- 3. 创建索引（UUIDv7的时间顺序性提高索引性能）
CREATE INDEX idx_events_created_at ON events(created_at);

-- 4. 插入数据（UUIDv7按时间顺序生成）
...
-- PostgreSQL 18：UUIDv7范围查询优化
-- UUIDv7的时间顺序性支持基于时间戳的范围查询
SELECT * FROM events
