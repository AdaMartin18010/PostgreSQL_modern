...
flowchart TD
    ...
    C -->|计算资源| F[资源复用<br/>共享计算]
    C -->|存储资源| G[数据压缩<br/>冷热分离]
    C -->|网络资源| H[批量传输<br/>数据本地化]

    D -->|高性能| I[性能优先<br/>成本次之]
    D -->|低成本| J[成本优先<br/>性能次之]
    D -->|平衡| K[成本性能平衡]

    E --> L{预算类型?}
    L -->|固定预算| M[资源配额<br/>预算限制]
    L -->|弹性预算| N[动态调整<br/>按需分配]

    F --> O[PostgreSQL 18<br/>连接池复用]
    G --> P[PostgreSQL 18<br/>表压缩]
    K --> Q[PostgreSQL 18<br/>查询优化]
    M --> R[PostgreSQL 18<br/>资源限制]
