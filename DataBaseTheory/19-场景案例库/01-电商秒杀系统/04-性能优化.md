...
-- 3. 监控VACUUM性能（PostgreSQL 18新增字段）
SELECT
    relname,
    last_autovacuum,
    autovacuum_count,
...
BEGIN
    ...
    SELECT
        '连接数'::TEXT,
        CASE
            WHEN COUNT(*) > 1900 THEN '严重'
            WHEN COUNT(*) > 1700 THEN '警告'
            ELSE '正常'
        END,
        format('当前连接数: %s', COUNT(*)),
        COUNT(*)::NUMERIC,
        2000::NUMERIC
    FROM pg_stat_activity
    HAVING COUNT(*) > 1700;
    ...
    SELECT
        '锁等待'::TEXT,
        CASE
            WHEN COUNT(*) > 100 THEN '严重'
            WHEN COUNT(*) > 50 THEN '警告'
            ELSE '正常'
        END,
        format('等待事务数: %s', COUNT(*)),
        COUNT(*)::NUMERIC,
        50::NUMERIC
    FROM pg_stat_activity
