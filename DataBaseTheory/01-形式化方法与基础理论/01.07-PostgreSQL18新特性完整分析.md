...
-- 查询只用到第二列，索引无法使用（PostgreSQL 17）
EXPLAIN SELECT * FROM orders WHERE created_at > '2024-01-01';
-- Seq Scan on orders  (cost=...)  ❌ 全表扫描

-- PostgreSQL 18自动跳过前导列
EXPLAIN SELECT * FROM orders WHERE created_at > '2024-01-01';
-- Index Skip Scan using idx_orders  (cost=...)  ✅ 索引扫描
```
...
**当前进度**: 已完成核心引擎8项（100%），开始查询优化器部分（2/6项）

**文档统计**：

- 当前字数：约12,000字
- 完成度：约50%
- 预计总长度：25,000-30,000字
