...
- **[PostgreSQL 逻辑复制文档](https://www.postgresql.org/docs/current/logical-replication.html)**
  - 版本: PostgreSQL 10+
  - 内容: PostgreSQL 逻辑复制的完整文档，用于实现事件驱动架构
  - 最后更新: 2025年

- **[PostgreSQL 发布订阅文档](https://www.postgresql.org/docs/current/sql-createpublication.html)**
  - 内容: PostgreSQL 发布订阅机制的详细说明

### 7.2 学术论文

- **Fowler, M. (2017). "Event-Driven Architecture."**
  - 来源: [Martin Fowler's Blog](https://martinfowler.com/articles/201701-event-driven.html)
  - **重要性**: 事件驱动架构的经典文章，详细阐述了事件驱动架构的设计原则和模式

- **Fowler, M. (2005). "Event Sourcing."**
  - 来源: [Martin Fowler's Blog](https://martinfowler.com/eaaDev/EventSourcing.html)
  - **重要性**: 事件溯源模式的详细说明，为事件存储提供了理论基础

### 7.3 技术博客

- **[事件驱动架构模式](https://martinfowler.com/articles/201701-event-driven.html)**
  - 作者: Martin Fowler
  - 内容: 事件驱动架构的完整指南，包括模式、优缺点和实现建议

- **[事件溯源模式](https://martinfowler.com/eaaDev/EventSourcing.html)**
  - 作者: Martin Fowler
  - 内容: 事件溯源模式的详细说明和实现指南

- **[PostgreSQL 事件驱动架构实践](https://www.postgresql.org/docs/current/logical-replication.html)**
  - 内容: 使用 PostgreSQL 实现事件驱动架构的实践指南

### 7.4 相关资源

- **[CQRS 模式](https://martinfowler.com/bliki/CQRS.html)**
  - 作者: Martin Fowler
  - 内容: 命令查询职责分离模式，常与事件驱动架构结合使用

- **[事件存储最佳实践](https://eventstore.org/docs/)**
  - 来源: EventStore
  - 内容: 事件存储的设计和实现最佳实践
...
def handle_user_created(event: Dict):
    print(f"用户已创建: {event['event_data']}")

def handle_order_placed(event: Dict):
    print(f"订单已下单: {event['event_data']}")
