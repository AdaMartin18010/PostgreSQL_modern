# 边缘-云端数据同步

> **更新时间**: 2025年1月
> **技术版本**: PostgreSQL 18 with 流复制
> **文档编号**: 06-04-02

---

## 📑 目录

- [边缘-云端数据同步](#边缘-云端数据同步)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
    - [1.1 技术背景](#11-技术背景)
    - [1.2 技术定位](#12-技术定位)
    - [1.3 核心价值](#13-核心价值)
  - [2. 技术原理](#2-技术原理)
    - [2.1 同步架构设计](#21-同步架构设计)
    - [2.2 同步策略](#22-同步策略)
    - [2.3 冲突解决机制](#23-冲突解决机制)
  - [3. 实现方案](#3-实现方案)
    - [3.1 流复制同步](#31-流复制同步)
    - [3.2 逻辑复制同步](#32-逻辑复制同步)
    - [3.3 双向同步](#33-双向同步)
  - [4. 配置与部署](#4-配置与部署)
    - [4.1 边缘节点配置](#41-边缘节点配置)
    - [4.2 云端中心配置](#42-云端中心配置)
    - [4.3 同步链路配置](#43-同步链路配置)
  - [5. 性能优化](#5-性能优化)
    - [5.1 带宽优化](#51-带宽优化)
    - [5.2 延迟优化](#52-延迟优化)
    - [5.3 存储优化](#53-存储优化)
  - [6. 监控与诊断](#6-监控与诊断)
    - [6.1 同步状态监控](#61-同步状态监控)
    - [6.2 性能指标监控](#62-性能指标监控)
    - [6.3 故障诊断](#63-故障诊断)
  - [7. 实际应用案例](#7-实际应用案例)
    - [7.1 IoT边缘数据同步](#71-iot边缘数据同步)
    - [7.2 零售边缘数据同步](#72-零售边缘数据同步)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 同步策略选择](#81-同步策略选择)
    - [8.2 注意事项](#82-注意事项)
  - [9. 参考资料](#9-参考资料)
    - [官方文档](#官方文档)
    - [技术博客](#技术博客)

---

## 1. 概述

### 1.1 技术背景

在边缘计算场景中，边缘节点和云端中心需要保持数据同步：

- **边缘节点**：处理本地数据，提供低延迟服务
- **云端中心**：集中存储，提供全局视图和分析
- **数据同步**：确保边缘和云端数据一致性

**挑战**：

- 网络不稳定：边缘节点可能间歇性断网
- 带宽限制：边缘到云端的带宽有限
- 延迟要求：需要平衡一致性和延迟
- 冲突处理：多边缘节点可能产生数据冲突

### 1.2 技术定位

边缘-云端数据同步是PostgreSQL在边缘计算场景中的核心功能，提供：

- **流复制同步**：基于WAL的物理复制
- **逻辑复制同步**：基于表级别的逻辑复制
- **双向同步**：支持边缘到云端和云端到边缘的双向同步

### 1.3 核心价值

- **数据一致性**：保证边缘和云端数据一致性
- **高可用性**：边缘节点故障时云端可接管
- **低延迟**：边缘节点提供本地低延迟服务
- **灵活同步**：支持多种同步策略和冲突解决机制

---

## 2. 技术原理

### 2.1 同步架构设计

**同步架构**：

```text
边缘节点1 (Edge Node 1)
    ↓ (流复制/逻辑复制)
云端中心 (Cloud Center)
    ↓ (流复制/逻辑复制)
边缘节点2 (Edge Node 2)
```

**同步模式**：

1. **单向同步**：边缘 → 云端（数据上传）
2. **单向同步**：云端 → 边缘（配置下发）
3. **双向同步**：边缘 ↔ 云端（双向数据同步）

### 2.2 同步策略

**同步策略类型**：

1. **实时同步**：
   - 数据变更立即同步
   - 低延迟，高带宽消耗

2. **批量同步**：
   - 定期批量同步
   - 低带宽消耗，较高延迟

3. **按需同步**：
   - 根据需求触发同步
   - 平衡带宽和延迟

4. **增量同步**：
   - 只同步变更数据
   - 优化带宽使用

### 2.3 冲突解决机制

**冲突类型**：

1. **写冲突**：多个节点同时修改同一数据
2. **删除冲突**：一个节点删除，另一个节点修改
3. **插入冲突**：多个节点插入相同主键

**解决策略**：

1. **最后写入获胜（LWW）**：基于时间戳
2. **应用层解决**：应用层定义解决规则
3. **手动解决**：人工介入解决冲突

---

## 3. 实现方案

### 3.1 流复制同步

**流复制配置（边缘 → 云端）**：

```sql
-- 云端中心配置（主节点）
-- postgresql.conf
wal_level = replica
max_wal_senders = 10
max_replication_slots = 10

-- pg_hba.conf
host replication edge_user 0.0.0.0/0 md5

-- 创建复制用户
CREATE USER edge_user REPLICATION LOGIN PASSWORD 'password';

-- 边缘节点配置（从节点）
-- postgresql.conf
hot_standby = on

-- recovery.conf (PostgreSQL 12+)
primary_conninfo = 'host=cloud-center.example.com port=5432 user=edge_user password=password'
primary_slot_name = 'edge_node_1_slot'
```

**流复制监控**：

```sql
-- 查看复制状态
SELECT
    application_name,
    client_addr,
    state,
    sync_state,
    sync_priority,
    sync_standby_names
FROM pg_stat_replication;

-- 查看复制延迟
SELECT
    application_name,
    pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS sent_lag,
    pg_wal_lsn_diff(sent_lsn, write_lsn) AS write_lag,
    pg_wal_lsn_diff(write_lsn, flush_lsn) AS flush_lag,
    pg_wal_lsn_diff(flush_lsn, replay_lsn) AS replay_lag
FROM pg_stat_replication;
```

### 3.2 逻辑复制同步

**逻辑复制配置（边缘 → 云端）**：

```sql
-- 云端中心配置（订阅者）
-- postgresql.conf
wal_level = logical
max_replication_slots = 10
max_logical_replication_workers = 10

-- 创建发布
CREATE PUBLICATION edge_publication FOR TABLE sensor_data, device_status;

-- 边缘节点配置（发布者）
-- postgresql.conf
wal_level = logical
max_replication_slots = 10

-- 创建订阅
CREATE SUBSCRIPTION edge_subscription
CONNECTION 'host=cloud-center.example.com port=5432 dbname=postgres user=replication_user password=password'
PUBLICATION edge_publication
WITH (copy_data = false);
```

**逻辑复制监控**：

```sql
-- 查看订阅状态
SELECT
    subname,
    subenabled,
    subpublications,
    subslotname
FROM pg_subscription;

-- 查看复制槽状态
SELECT
    slot_name,
    slot_type,
    active,
    pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) AS lag_bytes
FROM pg_replication_slots;
```

### 3.3 双向同步

**双向同步架构**：

```
边缘节点 (Edge Node)
    ├── 发布者 (Publisher) → 云端订阅
    └── 订阅者 (Subscriber) ← 云端发布
云端中心 (Cloud Center)
    ├── 发布者 (Publisher) → 边缘订阅
    └── 订阅者 (Subscriber) ← 边缘发布
```

**双向同步配置**：

```sql
-- 云端中心：发布和订阅
CREATE PUBLICATION cloud_publication FOR TABLE config, updates;
CREATE SUBSCRIPTION edge_subscription
CONNECTION 'host=edge-node.example.com port=5432 dbname=postgres user=replication_user password=password'
PUBLICATION edge_publication;

-- 边缘节点：发布和订阅
CREATE PUBLICATION edge_publication FOR TABLE sensor_data, local_config;
CREATE SUBSCRIPTION cloud_subscription
CONNECTION 'host=cloud-center.example.com port=5432 dbname=postgres user=replication_user password=password'
PUBLICATION cloud_publication;
```

---

## 4. 配置与部署

### 4.1 边缘节点配置

**边缘节点配置示例**：

```bash
# postgresql.conf
# 流复制配置
wal_level = replica
max_wal_senders = 3
max_replication_slots = 3
hot_standby = on

# 逻辑复制配置
wal_level = logical
max_replication_slots = 5
max_logical_replication_workers = 5

# 性能优化
shared_buffers = 256MB
effective_cache_size = 1GB
maintenance_work_mem = 64MB
checkpoint_completion_target = 0.9
wal_buffers = 16MB
default_statistics_target = 100
random_page_cost = 1.1
effective_io_concurrency = 200
work_mem = 4MB
min_wal_size = 1GB
max_wal_size = 4GB
```

### 4.2 云端中心配置

**云端中心配置示例**：

```bash
# postgresql.conf
# 流复制配置
wal_level = replica
max_wal_senders = 20
max_replication_slots = 20

# 逻辑复制配置
wal_level = logical
max_replication_slots = 50
max_logical_replication_workers = 50

# 性能优化
shared_buffers = 8GB
effective_cache_size = 24GB
maintenance_work_mem = 1GB
checkpoint_completion_target = 0.9
wal_buffers = 64MB
default_statistics_target = 100
random_page_cost = 1.1
effective_io_concurrency = 200
work_mem = 64MB
min_wal_size = 4GB
max_wal_size = 16GB
```

### 4.3 同步链路配置

**同步链路配置脚本**：

```bash
#!/bin/bash
# configure_sync.sh

# 配置边缘节点到云端的同步
EDGE_NODE=$1
CLOUD_CENTER=$2

# 在云端创建复制用户
psql -h $CLOUD_CENTER -U postgres <<EOF
CREATE USER edge_replication REPLICATION LOGIN PASSWORD 'secure_password';
CREATE PUBLICATION edge_publication FOR ALL TABLES;
EOF

# 在边缘节点创建订阅
psql -h $EDGE_NODE -U postgres <<EOF
CREATE SUBSCRIPTION cloud_subscription
CONNECTION 'host=$CLOUD_CENTER port=5432 dbname=postgres user=edge_replication password=secure_password'
PUBLICATION edge_publication;
EOF
```

---

## 5. 性能优化

### 5.1 带宽优化

**带宽优化策略**：

1. **压缩WAL**：

   ```sql
   ALTER SYSTEM SET wal_compression = on;
   SELECT pg_reload_conf();
   ```

2. **批量同步**：

   ```sql
   -- 使用批量插入减少网络往返
   INSERT INTO target_table SELECT * FROM source_table;
   ```

3. **增量同步**：

   ```sql
   -- 只同步变更数据
   SELECT * FROM source_table
   WHERE updated_at > (SELECT MAX(updated_at) FROM target_table);
   ```

### 5.2 延迟优化

**延迟优化策略**：

1. **异步复制**：

   ```sql
   -- 使用异步复制减少延迟
   ALTER SYSTEM SET synchronous_standby_names = '';
   SELECT pg_reload_conf();
   ```

2. **本地缓存**：

   ```sql
   -- 使用本地缓存减少查询延迟
   CREATE MATERIALIZED VIEW local_cache AS
   SELECT * FROM remote_table;
   ```

### 5.3 存储优化

**存储优化策略**：

1. **分区表**：

   ```sql
   -- 使用分区表优化存储
   CREATE TABLE sensor_data (
       id SERIAL,
       sensor_id INT,
       value NUMERIC,
       timestamp TIMESTAMP
   ) PARTITION BY RANGE (timestamp);
   ```

2. **数据归档**：

   ```sql
   -- 归档旧数据
   CREATE TABLE sensor_data_archive (
       LIKE sensor_data INCLUDING ALL
   );
   ```

---

## 6. 监控与诊断

### 6.1 同步状态监控

**同步状态查询**：

```sql
-- 流复制状态
SELECT
    application_name,
    client_addr,
    state,
    sync_state,
    pg_wal_lsn_diff(pg_current_wal_lsn(), sent_lsn) AS lag_bytes
FROM pg_stat_replication;

-- 逻辑复制状态
SELECT
    subname,
    subenabled,
    subslotname,
    (SELECT pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn)
     FROM pg_replication_slots
     WHERE slot_name = subslotname) AS lag_bytes
FROM pg_subscription;
```

### 6.2 性能指标监控

**性能指标查询**：

```sql
-- 复制延迟
SELECT
    application_name,
    EXTRACT(EPOCH FROM (now() - write_lag)) AS write_lag_seconds,
    EXTRACT(EPOCH FROM (now() - flush_lag)) AS flush_lag_seconds,
    EXTRACT(EPOCH FROM (now() - replay_lag)) AS replay_lag_seconds
FROM pg_stat_replication;

-- 复制吞吐量
SELECT
    application_name,
    sent_lsn,
    write_lsn,
    flush_lsn,
    replay_lsn
FROM pg_stat_replication;
```

### 6.3 故障诊断

**故障诊断步骤**：

1. **检查网络连接**：

   ```bash
   psql -h cloud-center.example.com -U replication_user -d postgres -c "SELECT 1;"
   ```

2. **检查复制槽**：

   ```sql
   SELECT * FROM pg_replication_slots;
   ```

3. **检查WAL文件**：

   ```sql
   SELECT * FROM pg_ls_waldir() ORDER BY modification DESC LIMIT 10;
   ```

---

## 7. 实际应用案例

### 7.1 IoT边缘数据同步

**场景**：IoT设备在边缘节点收集数据，同步到云端进行分析

**实现**：

```sql
-- 边缘节点：发布传感器数据
CREATE PUBLICATION iot_publication FOR TABLE sensor_readings;

-- 云端中心：订阅传感器数据
CREATE SUBSCRIPTION iot_subscription
CONNECTION 'host=edge-node.example.com port=5432 dbname=iot user=replication password=password'
PUBLICATION iot_publication;

-- 云端分析查询
SELECT
    sensor_id,
    AVG(value) AS avg_value,
    MAX(value) AS max_value,
    MIN(value) AS min_value
FROM sensor_readings
WHERE timestamp > NOW() - INTERVAL '1 hour'
GROUP BY sensor_id;
```

### 7.2 零售边缘数据同步

**场景**：零售门店在边缘节点处理交易，同步到云端进行库存管理

**实现**：

```sql
-- 边缘节点：发布交易数据
CREATE PUBLICATION retail_publication FOR TABLE transactions, inventory_changes;

-- 云端中心：订阅交易数据
CREATE SUBSCRIPTION retail_subscription
CONNECTION 'host=store-edge.example.com port=5432 dbname=retail user=replication password=password'
PUBLICATION retail_publication;

-- 云端库存管理
SELECT
    product_id,
    SUM(quantity) AS total_sold,
    SUM(amount) AS total_revenue
FROM transactions
WHERE transaction_date = CURRENT_DATE
GROUP BY product_id;
```

---

## 8. 最佳实践

### 8.1 同步策略选择

**策略选择指南**：

| 场景 | 推荐策略 | 原因 |
|------|---------|------|
| **实时数据采集** | 流复制 | 低延迟，高一致性 |
| **批量数据上传** | 逻辑复制 | 灵活，可选择性同步 |
| **配置下发** | 逻辑复制 | 按需同步，减少带宽 |
| **双向同步** | 逻辑复制 | 支持双向，冲突解决 |

### 8.2 注意事项

**注意事项**：

1. **网络稳定性**：确保网络连接稳定，配置重连机制
2. **冲突处理**：定义清晰的冲突解决策略
3. **性能监控**：持续监控同步延迟和吞吐量
4. **故障恢复**：制定故障恢复和回滚策略

---

## 9. 参考资料

### 官方文档

1. **PostgreSQL流复制**：
   - [Streaming Replication](https://www.postgresql.org/docs/current/high-availability.html)
   - [Replication Slots](https://www.postgresql.org/docs/current/replication-slots.html)

2. **PostgreSQL逻辑复制**：
   - [Logical Replication](https://www.postgresql.org/docs/current/logical-replication.html)
   - [Publication and Subscription](https://www.postgresql.org/docs/current/logical-replication-publication.html)

### 技术博客

1. **边缘计算数据同步**：
   - PostgreSQL边缘计算最佳实践
   - 边缘-云端数据同步案例

---

**最后更新**: 2025年1月
**维护状态**: ✅ 持续更新
