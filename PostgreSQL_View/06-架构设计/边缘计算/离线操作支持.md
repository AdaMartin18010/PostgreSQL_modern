# 离线操作支持

> **更新时间**: 2025年1月
> **技术版本**: PostgreSQL 18
> **文档编号**: 06-04-03

---

## 📑 目录

- [离线操作支持](#离线操作支持)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
    - [1.1 技术背景](#11-技术背景)
    - [1.2 技术定位](#12-技术定位)
    - [1.3 核心价值](#13-核心价值)
  - [2. 技术原理](#2-技术原理)
    - [2.1 离线操作场景](#21-离线操作场景)
    - [2.2 本地数据库设计](#22-本地数据库设计)
    - [2.3 冲突检测与解决](#23-冲突检测与解决)
  - [3. 实现方案](#3-实现方案)
    - [3.1 本地数据库架构](#31-本地数据库架构)
    - [3.2 操作队列机制](#32-操作队列机制)
    - [3.3 同步机制](#33-同步机制)
  - [4. 配置与部署](#4-配置与部署)
    - [4.1 本地数据库配置](#41-本地数据库配置)
    - [4.2 同步配置](#42-同步配置)
    - [4.3 冲突解决配置](#43-冲突解决配置)
  - [5. 应用开发](#5-应用开发)
    - [5.1 离线操作API](#51-离线操作api)
    - [5.2 同步状态管理](#52-同步状态管理)
    - [5.3 冲突处理](#53-冲突处理)
  - [6. 性能优化](#6-性能优化)
    - [6.1 本地性能优化](#61-本地性能优化)
    - [6.2 同步性能优化](#62-同步性能优化)
  - [7. 实际应用案例](#7-实际应用案例)
    - [7.1 移动应用离线支持](#71-移动应用离线支持)
    - [7.2 边缘设备离线支持](#72-边缘设备离线支持)
  - [8. 最佳实践](#8-最佳实践)
    - [8.1 设计原则](#81-设计原则)
    - [8.2 注意事项](#82-注意事项)
  - [9. 参考资料](#9-参考资料)
    - [官方文档](#官方文档)
    - [技术博客](#技术博客)

---

## 1. 概述

### 1.1 技术背景

在边缘计算和移动应用场景中，设备可能处于离线状态：

- **网络不稳定**：边缘设备可能间歇性断网
- **移动应用**：移动设备经常处于离线状态
- **离线操作需求**：用户需要在离线状态下继续操作

**挑战**：

- **数据一致性**：离线操作与在线数据的一致性
- **冲突处理**：多设备离线操作产生的冲突
- **同步机制**：离线操作的上传和同步
- **用户体验**：保证离线操作的流畅体验

### 1.2 技术定位

离线操作支持是PostgreSQL在边缘计算和移动应用场景中的关键功能，提供：

- **本地数据库**：边缘设备本地PostgreSQL数据库
- **操作队列**：离线操作队列和同步机制
- **冲突解决**：自动和手动冲突解决机制

### 1.3 核心价值

- **离线可用性**：设备离线时仍可正常操作
- **数据一致性**：保证离线操作与在线数据的一致性
- **冲突处理**：自动检测和解决数据冲突
- **用户体验**：提供流畅的离线操作体验

---

## 2. 技术原理

### 2.1 离线操作场景

**离线操作类型**：

1. **读操作**：
   - 查询本地数据库
   - 使用本地缓存数据

2. **写操作**：
   - 插入、更新、删除操作
   - 操作进入队列等待同步

3. **同步操作**：
   - 网络恢复时自动同步
   - 手动触发同步

### 2.2 本地数据库设计

**本地数据库架构**：

```text
应用层
    ↓
同步层
    ├── 操作队列
    ├── 冲突检测
    └── 同步管理
    ↓
本地PostgreSQL数据库
    ├── 业务表
    ├── 同步状态表
    └── 操作日志表
```

**核心表设计**：

```sql
-- 业务表（示例）
CREATE TABLE local_data (
    id SERIAL PRIMARY KEY,
    data_value TEXT,
    updated_at TIMESTAMP DEFAULT NOW(),
    sync_status TEXT DEFAULT 'pending'  -- pending, synced, conflict
);

-- 同步状态表
CREATE TABLE sync_status (
    id SERIAL PRIMARY KEY,
    table_name TEXT,
    last_sync_time TIMESTAMP,
    sync_state TEXT  -- online, offline, syncing
);

-- 操作日志表
CREATE TABLE operation_log (
    id SERIAL PRIMARY KEY,
    table_name TEXT,
    operation_type TEXT,  -- INSERT, UPDATE, DELETE
    record_id INT,
    operation_data JSONB,
    created_at TIMESTAMP DEFAULT NOW(),
    synced_at TIMESTAMP,
    sync_status TEXT DEFAULT 'pending'
);
```

### 2.3 冲突检测与解决

**冲突类型**：

1. **更新冲突**：多个设备同时更新同一记录
2. **删除冲突**：一个设备删除，另一个设备更新
3. **插入冲突**：多个设备插入相同主键

**冲突检测机制**：

```sql
-- 冲突检测函数
CREATE OR REPLACE FUNCTION detect_conflict(
    p_table_name TEXT,
    p_record_id INT,
    p_local_version INT,
    p_remote_version INT
) RETURNS BOOLEAN AS $$
BEGIN
    IF p_local_version != p_remote_version THEN
        RETURN TRUE;  -- 检测到冲突
    END IF;
    RETURN FALSE;
END;
$$ LANGUAGE plpgsql;
```

**冲突解决策略**：

1. **最后写入获胜（LWW）**：基于时间戳
2. **版本号比较**：基于版本号
3. **应用层规则**：应用层定义解决规则
4. **手动解决**：人工介入解决冲突

---

## 3. 实现方案

### 3.1 本地数据库架构

**本地数据库配置**：

```sql
-- 创建本地数据库
CREATE DATABASE local_db;

-- 配置本地数据库参数
ALTER DATABASE local_db SET synchronous_commit = off;  -- 提高写入性能
ALTER DATABASE local_db SET wal_level = minimal;  -- 减少WAL开销
```

**本地表结构**：

```sql
-- 业务表（带同步字段）
CREATE TABLE products (
    id SERIAL PRIMARY KEY,
    name TEXT,
    price NUMERIC,
    version INT DEFAULT 1,  -- 版本号用于冲突检测
    updated_at TIMESTAMP DEFAULT NOW(),
    sync_status TEXT DEFAULT 'pending',
    sync_timestamp TIMESTAMP
);

-- 创建索引优化同步查询
CREATE INDEX idx_products_sync_status ON products(sync_status, sync_timestamp);
CREATE INDEX idx_products_version ON products(id, version);
```

### 3.2 操作队列机制

**操作队列实现**：

```sql
-- 操作队列表
CREATE TABLE operation_queue (
    id SERIAL PRIMARY KEY,
    table_name TEXT NOT NULL,
    operation_type TEXT NOT NULL,  -- INSERT, UPDATE, DELETE
    record_id INT,
    operation_data JSONB,
    created_at TIMESTAMP DEFAULT NOW(),
    priority INT DEFAULT 0,  -- 优先级
    retry_count INT DEFAULT 0,
    status TEXT DEFAULT 'pending'  -- pending, processing, completed, failed
);

-- 操作队列处理函数
CREATE OR REPLACE FUNCTION process_operation_queue()
RETURNS void AS $$
DECLARE
    op_record RECORD;
BEGIN
    FOR op_record IN
        SELECT * FROM operation_queue
        WHERE status = 'pending'
        ORDER BY priority DESC, created_at ASC
        LIMIT 100
    LOOP
        -- 处理操作
        PERFORM sync_operation(op_record);

        -- 更新状态
        UPDATE operation_queue
        SET status = 'processing'
        WHERE id = op_record.id;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

**操作同步函数**：

```sql
-- 同步操作函数
CREATE OR REPLACE FUNCTION sync_operation(p_operation operation_queue)
RETURNS void AS $$
DECLARE
    v_result BOOLEAN;
BEGIN
    -- 尝试同步到远程数据库
    v_result := sync_to_remote(p_operation);

    IF v_result THEN
        -- 同步成功
        UPDATE operation_queue
        SET status = 'completed', synced_at = NOW()
        WHERE id = p_operation.id;

        -- 更新业务表同步状态
        EXECUTE format('UPDATE %I SET sync_status = ''synced'', sync_timestamp = NOW() WHERE id = $1',
                       p_operation.table_name) USING p_operation.record_id;
    ELSE
        -- 同步失败，增加重试次数
        UPDATE operation_queue
        SET retry_count = retry_count + 1,
            status = CASE
                WHEN retry_count >= 3 THEN 'failed'
                ELSE 'pending'
            END
        WHERE id = p_operation.id;
    END IF;
END;
$$ LANGUAGE plpgsql;
```

### 3.3 同步机制

**同步触发机制**：

```sql
-- 网络状态检测函数
CREATE OR REPLACE FUNCTION check_network_status()
RETURNS BOOLEAN AS $$
BEGIN
    -- 尝试连接远程数据库
    BEGIN
        PERFORM dblink_connect('remote_db', 'host=remote-server port=5432 dbname=remote_db user=user password=password');
        PERFORM dblink_disconnect('remote_db');
        RETURN TRUE;
    EXCEPTION
        WHEN OTHERS THEN
            RETURN FALSE;
    END;
END;
$$ LANGUAGE plpgsql;

-- 自动同步触发器
CREATE OR REPLACE FUNCTION auto_sync_trigger()
RETURNS void AS $$
BEGIN
    IF check_network_status() THEN
        PERFORM process_operation_queue();
    END IF;
END;
$$ LANGUAGE plpgsql;

-- 定时同步任务（使用pg_cron扩展）
SELECT cron.schedule('sync-operations', '*/5 * * * *', 'SELECT auto_sync_trigger();');
```

**双向同步实现**：

```sql
-- 从远程同步数据
CREATE OR REPLACE FUNCTION sync_from_remote()
RETURNS void AS $$
DECLARE
    remote_record RECORD;
    local_record RECORD;
    conflict_detected BOOLEAN;
BEGIN
    -- 获取远程变更数据
    FOR remote_record IN
        SELECT * FROM dblink('remote_db',
            'SELECT id, name, price, version, updated_at FROM products WHERE updated_at > $1',
            (SELECT MAX(sync_timestamp) FROM products WHERE sync_status = 'synced')::TEXT
        ) AS t(id INT, name TEXT, price NUMERIC, version INT, updated_at TIMESTAMP)
    LOOP
        -- 检查本地记录
        SELECT * INTO local_record FROM products WHERE id = remote_record.id;

        IF local_record IS NULL THEN
            -- 插入新记录
            INSERT INTO products (id, name, price, version, updated_at, sync_status)
            VALUES (remote_record.id, remote_record.name, remote_record.price,
                   remote_record.version, remote_record.updated_at, 'synced');
        ELSIF local_record.sync_status = 'pending' THEN
            -- 检测冲突
            conflict_detected := detect_conflict('products', remote_record.id,
                                                local_record.version, remote_record.version);
            IF conflict_detected THEN
                -- 标记冲突
                UPDATE products
                SET sync_status = 'conflict'
                WHERE id = remote_record.id;
            ELSE
                -- 更新记录
                UPDATE products
                SET name = remote_record.name,
                    price = remote_record.price,
                    version = remote_record.version,
                    updated_at = remote_record.updated_at,
                    sync_status = 'synced'
                WHERE id = remote_record.id;
            END IF;
        END IF;
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

---

## 4. 配置与部署

### 4.1 本地数据库配置

**本地数据库优化配置**：

```sql
-- postgresql.conf (本地数据库)
# 性能优化
shared_buffers = 256MB
effective_cache_size = 1GB
maintenance_work_mem = 64MB
checkpoint_completion_target = 0.9
wal_buffers = 16MB
default_statistics_target = 100
random_page_cost = 1.1
effective_io_concurrency = 200
work_mem = 4MB

# 同步优化
synchronous_commit = off  -- 离线场景不需要同步提交
wal_level = minimal  -- 减少WAL开销
```

### 4.2 同步配置

**同步配置表**：

```sql
-- 同步配置表
CREATE TABLE sync_config (
    id SERIAL PRIMARY KEY,
    config_key TEXT UNIQUE,
    config_value TEXT,
    updated_at TIMESTAMP DEFAULT NOW()
);

-- 插入同步配置
INSERT INTO sync_config (config_key, config_value) VALUES
    ('sync_interval', '300'),  -- 5分钟同步一次
    ('max_retry_count', '3'),
    ('conflict_resolution', 'lww'),  -- 最后写入获胜
    ('remote_host', 'remote-server.example.com'),
    ('remote_port', '5432'),
    ('remote_database', 'remote_db'),
    ('remote_user', 'sync_user'),
    ('remote_password', 'secure_password');
```

### 4.3 冲突解决配置

**冲突解决配置**：

```sql
-- 冲突解决规则表
CREATE TABLE conflict_resolution_rules (
    id SERIAL PRIMARY KEY,
    table_name TEXT,
    conflict_type TEXT,  -- update_update, update_delete, insert_insert
    resolution_strategy TEXT,  -- lww, manual, custom
    custom_function TEXT,  -- 自定义解决函数
    priority INT DEFAULT 0
);

-- 插入冲突解决规则
INSERT INTO conflict_resolution_rules (table_name, conflict_type, resolution_strategy) VALUES
    ('products', 'update_update', 'lww'),
    ('products', 'update_delete', 'manual'),
    ('products', 'insert_insert', 'manual');
```

---

## 5. 应用开发

### 5.1 离线操作API

**Python离线操作API示例**：

```python
import psycopg2
from datetime import datetime

class OfflineDatabase:
    def __init__(self, connection_string):
        self.conn = psycopg2.connect(connection_string)
        self.is_online = self.check_network_status()

    def check_network_status(self):
        """检查网络状态"""
        try:
            # 尝试连接远程数据库
            remote_conn = psycopg2.connect("host=remote-server dbname=remote_db user=user password=password")
            remote_conn.close()
            return True
        except:
            return False

    def insert_offline(self, table_name, data):
        """离线插入操作"""
        cur = self.conn.cursor()

        # 插入到本地数据库
        columns = ', '.join(data.keys())
        values = ', '.join(['%s'] * len(data))
        query = f"INSERT INTO {table_name} ({columns}) VALUES ({values}) RETURNING id"

        cur.execute(query, list(data.values()))
        record_id = cur.fetchone()[0]

        # 添加到操作队列
        cur.execute("""
            INSERT INTO operation_queue (table_name, operation_type, record_id, operation_data)
            VALUES (%s, 'INSERT', %s, %s)
        """, (table_name, record_id, json.dumps(data)))

        # 更新业务表同步状态
        cur.execute(f"""
            UPDATE {table_name}
            SET sync_status = 'pending'
            WHERE id = %s
        """, (record_id,))

        self.conn.commit()
        return record_id

    def update_offline(self, table_name, record_id, data):
        """离线更新操作"""
        cur = self.conn.cursor()

        # 更新本地数据库
        set_clause = ', '.join([f"{k} = %s" for k in data.keys()])
        query = f"UPDATE {table_name} SET {set_clause}, version = version + 1 WHERE id = %s"

        cur.execute(query, list(data.values()) + [record_id])

        # 添加到操作队列
        cur.execute("""
            INSERT INTO operation_queue (table_name, operation_type, record_id, operation_data)
            VALUES (%s, 'UPDATE', %s, %s)
        """, (table_name, record_id, json.dumps(data)))

        # 更新同步状态
        cur.execute(f"""
            UPDATE {table_name}
            SET sync_status = 'pending'
            WHERE id = %s
        """, (record_id,))

        self.conn.commit()

    def sync_operations(self):
        """同步操作队列"""
        if not self.is_online:
            return False

        cur = self.conn.cursor()
        cur.execute("SELECT process_operation_queue()")
        self.conn.commit()
        return True
```

### 5.2 同步状态管理

**同步状态查询**：

```sql
-- 查询待同步操作数量
SELECT COUNT(*) FROM operation_queue WHERE status = 'pending';

-- 查询同步状态统计
SELECT
    sync_status,
    COUNT(*) AS count
FROM products
GROUP BY sync_status;

-- 查询冲突记录
SELECT * FROM products WHERE sync_status = 'conflict';
```

### 5.3 冲突处理

**冲突处理API**：

```python
def resolve_conflict(self, table_name, record_id, resolution_data):
    """解决冲突"""
    cur = self.conn.cursor()

    # 应用解决策略
    if resolution_data['strategy'] == 'lww':
        # 最后写入获胜
        cur.execute(f"""
            UPDATE {table_name}
            SET name = %s, price = %s, version = %s,
                sync_status = 'synced', sync_timestamp = NOW()
            WHERE id = %s
        """, (resolution_data['name'], resolution_data['price'],
              resolution_data['version'], record_id))

    elif resolution_data['strategy'] == 'manual':
        # 手动解决
        cur.execute(f"""
            UPDATE {table_name}
            SET name = %s, price = %s, version = version + 1,
                sync_status = 'pending'
            WHERE id = %s
        """, (resolution_data['name'], resolution_data['price'], record_id))

    self.conn.commit()
```

---

## 6. 性能优化

### 6.1 本地性能优化

**本地数据库优化**：

```sql
-- 创建索引优化查询
CREATE INDEX idx_products_sync_status ON products(sync_status, sync_timestamp);
CREATE INDEX idx_operation_queue_status ON operation_queue(status, created_at);

-- 定期清理已完成的操作
DELETE FROM operation_queue
WHERE status = 'completed'
AND synced_at < NOW() - INTERVAL '7 days';
```

### 6.2 同步性能优化

**批量同步优化**：

```sql
-- 批量同步函数
CREATE OR REPLACE FUNCTION batch_sync_operations(batch_size INT DEFAULT 100)
RETURNS INT AS $$
DECLARE
    synced_count INT := 0;
BEGIN
    -- 批量处理操作
    FOR i IN 1..batch_size LOOP
        PERFORM process_operation_queue();
        synced_count := synced_count + 1;
    END LOOP;

    RETURN synced_count;
END;
$$ LANGUAGE plpgsql;
```

---

## 7. 实际应用案例

### 7.1 移动应用离线支持

**场景**：移动应用在离线状态下继续操作，网络恢复后自动同步

**实现**：

```python
# 移动应用离线数据库操作
db = OfflineDatabase("dbname=mobile_app user=app_user")

# 离线插入订单
order_id = db.insert_offline('orders', {
    'user_id': 123,
    'product_id': 456,
    'quantity': 2,
    'total_amount': 199.99
})

# 网络恢复后自动同步
if db.check_network_status():
    db.sync_operations()
```

### 7.2 边缘设备离线支持

**场景**：IoT边缘设备在离线状态下收集数据，网络恢复后批量上传

**实现**：

```python
# 边缘设备数据收集
device_db = OfflineDatabase("dbname=iot_device user=device_user")

# 离线收集传感器数据
for reading in sensor_readings:
    device_db.insert_offline('sensor_data', {
        'sensor_id': reading.sensor_id,
        'value': reading.value,
        'timestamp': reading.timestamp
    })

# 定时批量同步
if device_db.check_network_status():
    device_db.sync_operations()
```

---

## 8. 最佳实践

### 8.1 设计原则

**设计原则**：

1. **本地优先**：优先使用本地数据库，保证离线可用性
2. **异步同步**：使用异步同步机制，不阻塞用户操作
3. **冲突检测**：自动检测冲突，提供解决机制
4. **状态管理**：清晰的状态管理，便于监控和调试

### 8.2 注意事项

**注意事项**：

1. **数据一致性**：平衡一致性和可用性
2. **冲突处理**：定义清晰的冲突解决策略
3. **性能优化**：优化本地数据库和同步性能
4. **错误处理**：完善的错误处理和重试机制

---

## 9. 参考资料

### 官方文档

1. **PostgreSQL本地数据库**：
   - [PostgreSQL Configuration](https://www.postgresql.org/docs/current/runtime-config.html)
   - [PostgreSQL Performance Tuning](https://www.postgresql.org/docs/current/performance-tips.html)

2. **同步机制**：
   - [PostgreSQL Logical Replication](https://www.postgresql.org/docs/current/logical-replication.html)
   - [PostgreSQL dblink Extension](https://www.postgresql.org/docs/current/dblink.html)

### 技术博客

1. **离线操作最佳实践**：
   - PostgreSQL离线操作设计模式
   - 移动应用离线数据同步案例

---

**最后更新**: 2025年1月
**维护状态**: ✅ 持续更新
