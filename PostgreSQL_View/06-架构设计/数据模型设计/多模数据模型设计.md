...
- [多模数据模型设计](#多模数据模型设计)
  ...
  - [3. 数据模型架构](#3-数据模型架构)
  - [4. 设计模式](#4-设计模式)
    - [4.1 模式 1: IoT 设备数据模型](#41-模式-1-iot-设备数据模型)
    - [4.2 模式 2: 电商商品数据模型](#42-模式-2-电商商品数据模型)
    - [4.3 模式 3: 知识图谱数据模型](#43-模式-3-知识图谱数据模型)
  - [5. 混合查询模式](#5-混合查询模式)
    - [5.1 JSONB + 向量 + 全文搜索](#51-jsonb--向量--全文搜索)
    - [5.2 时序 + 向量混合分析](#52-时序--向量混合分析)
    - [5.3 图 + 向量混合检索](#53-图--向量混合检索)
  - [6. 性能优化策略](#6-性能优化策略)
    - [6.1 分区策略](#61-分区策略)
    - [6.2 物化视图优化](#62-物化视图优化)
  - [7. 最佳实践](#7-最佳实践)
    - [7.1 数据类型选择](#71-数据类型选择)
    - [7.2 索引策略](#72-索引策略)
    - [7.3 查询优化](#73-查询优化)
  - [8. 参考资料](#8-参考资料)
...
本文档提供 PostgreSQL 18 多模数据模型设计指南，涵盖 JSONB、时序、图、向量四种数据模型的统一设计。
...
└─────────────────────────────────────────────────┘
                      │
┌─────────────────────────────────────────────────┐
│         PostgreSQL 18 Multi-Model               │
│  ┌──────────────────────────────────────────┐   │
│  │      Unified Table Schema                │   │
│  │  - JSONB Fields (Document)               │   │
│  │  - Time-Series Fields                    │   │
│  │  - Vector Fields                         │   │
│  │  - Graph Relationships                   │   │
│  └──────────────────────────────────────────┘   │
...
- **JSONB**: 半结构化数据、动态字段、配置信息
- **Timescale**: 时序数据、IoT 指标、监控数据
- **pgvector**: 向量数据、语义搜索、推荐系统
- **Apache AGE**: 图数据、关系分析、知识图谱

### 7.2 索引策略

- **JSONB**: GIN 索引用于 JSONB 查询
- **时序**: 时间字段索引 + TimescaleDB 分区
- **向量**: HNSW 或 IVFFlat 索引
- **图**: AGE 自动管理图索引

### 7.3 查询优化

- **避免跨模型 JOIN**: 尽量在单表内完成查询
- **使用物化视图**: 预计算复杂混合查询
- **分区裁剪**: 利用 TimescaleDB 分区优化
- **索引组合**: 合理组合不同索引类型

## 8. 参考资料

- [PostgreSQL 18 新特性](../04-多模一体化/PostgreSQL-18新特性/异步I-O机制.md)
...
CREATE TABLE device_metrics (
    time TIMESTAMPTZ NOT NULL,
    device_id TEXT NOT NULL,
    sensor_data JSONB,  -- JSONB 存储传感器数据
    state_vector vector(768),  -- 向量存储设备状态
    metadata JSONB,
...
for result in results:
    print(f"时间: {result['time']}, 相似度: {result['similarity']}")
