...
- [Prometheus-Rust监控集成](#prometheus-rust监控集成)
  ...
  - [🔧 第一部分：Rust应用指标导出](#-第一部分rust应用指标导出)
    - [1.1 Prometheus客户端集成](#11-prometheus客户端集成)
      - [1.1.1 添加依赖](#111-添加依赖)
      - [1.1.2 创建指标注册表](#112-创建指标注册表)
    - [1.2 指标注册](#12-指标注册)
      - [1.2.1 连接池指标注册](#121-连接池指标注册)
    - [1.3 HTTP端点暴露](#13-http端点暴露)
      - [1.3.1 Axum集成](#131-axum集成)
  - [📊 第二部分：Prometheus服务器配置](#-第二部分prometheus服务器配置)
    - [2.1 抓取配置](#21-抓取配置)
      - [2.1.1 prometheus.yml配置](#211-prometheusyml配置)
    - [2.2 服务发现](#22-服务发现)
      - [2.2.1 Kubernetes服务发现](#221-kubernetes服务发现)
    - [2.3 标签配置](#23-标签配置)
      - [2.3.1 标签重写](#231-标签重写)
  - [🚨 第三部分：告警规则配置](#-第三部分告警规则配置)
    - [3.1 连接池告警](#31-连接池告警)
      - [3.1.1 连接池告警规则](#311-连接池告警规则)
    - [3.2 事务告警](#32-事务告警)
      - [3.2.1 事务告警规则](#321-事务告警规则)
    - [3.3 性能告警](#33-性能告警)
      - [3.3.1 性能告警规则](#331-性能告警规则)
  - [📈 第四部分：Grafana仪表盘集成](#-第四部分grafana仪表盘集成)
    - [4.1 数据源配置](#41-数据源配置)
      - [4.1.1 Prometheus数据源](#411-prometheus数据源)
    - [4.2 仪表盘配置](#42-仪表盘配置)
      - [4.2.1 连接池监控面板](#421-连接池监控面板)
    - [4.3 面板配置](#43-面板配置)
      - [4.3.1 事务监控面板](#431-事务监控面板)
