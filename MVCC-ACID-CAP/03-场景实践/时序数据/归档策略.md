...
DECLARE
    ...
    part_name TEXT;
BEGIN
    archive_date := CURRENT_DATE - INTERVAL '3 months';
    ...
    FOR part_name IN
        ...
        AND relkind = 'r'
        AND relname < format('metrics_%s', to_char(archive_date, 'YYYY_MM'))
    LOOP
        -- 归档分区
        PERFORM archive_partition(part_name);
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    part_name TEXT;
    part_size BIGINT;
    size_threshold BIGINT := 10 * 1024 * 1024 * 1024;  -- 10GB
BEGIN
    -- 查找超过阈值的分区
    FOR part_name, part_size IN
        SELECT
            relname,
            pg_total_relation_size(relid)
        FROM pg_stat_user_tables
        WHERE relname LIKE 'metrics_%'
        AND pg_total_relation_size(relid) > size_threshold
