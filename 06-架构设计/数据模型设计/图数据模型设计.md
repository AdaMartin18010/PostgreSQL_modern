# 图数据模型设计

> **更新时间**: 2025 年 11 月 1 日
> **技术版本**: PostgreSQL 14+, Apache AGE 1.0+
> **文档编号**: 06-02-03

## 📑 目录

- [图数据模型设计](#图数据模型设计)
  - [📑 目录](#-目录)
  - [1. 概述](#1-概述)
    - [1.1 技术背景](#11-技术背景)
    - [1.2 核心价值](#12-核心价值)
  - [2. 数据模型设计](#2-数据模型设计)
    - [2.1 节点设计](#21-节点设计)
    - [2.2 边设计](#22-边设计)
    - [2.3 图模式设计](#23-图模式设计)
  - [3. Apache AGE 集成](#3-apache-age-集成)
    - [3.1 AGE 安装](#31-age-安装)
    - [3.2 图创建](#32-图创建)
    - [3.3 Cypher 查询](#33-cypher-查询)
  - [4. 查询优化](#4-查询优化)
    - [4.1 图查询优化](#41-图查询优化)
    - [4.2 索引策略](#42-索引策略)
  - [5. 最佳实践](#5-最佳实践)
  - [6. 参考资料](#6-参考资料)

---

## 1. 概述

### 1.1 技术背景

**问题需求**:

图数据模型设计需要：

- **关系建模**: 表示实体之间的关系
- **图查询**: 支持复杂的图查询（路径查询、社区发现）
- **性能优化**: 高效的图查询性能
- **与 SQL 集成**: 与 PostgreSQL SQL 无缝集成

**技术方案**:

- **Apache AGE**: PostgreSQL 图扩展
- **Cypher**: 图查询语言
- **关系表**: 传统关系表存储图数据

### 1.2 核心价值

- **查询能力**: 支持复杂图查询
- **性能**: 高效的图查询性能
- **集成**: 与 PostgreSQL 无缝集成

## 2. 数据模型设计

### 2.1 节点设计

```sql
-- 使用 Apache AGE 创建节点
SELECT * FROM cypher('knowledge_graph', $$
    CREATE (p:Person {
        id: 'person_001',
        name: 'Alice',
        age: 30,
        email: 'alice@example.com'
    })
$$) AS (p agtype);

-- 使用关系表存储节点（备选方案）
CREATE TABLE nodes (
    id TEXT PRIMARY KEY,
    label TEXT NOT NULL,
    properties JSONB,
    created_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX ON nodes (label);
CREATE INDEX ON nodes USING GIN (properties);
```

### 2.2 边设计

```sql
-- 使用 Apache AGE 创建边
SELECT * FROM cypher('knowledge_graph', $$
    MATCH (a:Person {id: 'person_001'}), (b:Person {id: 'person_002'})
    CREATE (a)-[r:KNOWS {
        since: '2020-01-01',
        strength: 0.8
    }]->(b)
$$) AS (r agtype);

-- 使用关系表存储边（备选方案）
CREATE TABLE edges (
    id SERIAL PRIMARY KEY,
    source_id TEXT NOT NULL REFERENCES nodes(id),
    target_id TEXT NOT NULL REFERENCES nodes(id),
    label TEXT NOT NULL,
    properties JSONB,
    created_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX ON edges (source_id, target_id);
CREATE INDEX ON edges (label);
```

### 2.3 图模式设计

```sql
-- 知识图谱模式设计
-- 节点类型：Person, Organization, Document
-- 边类型：KNOWS, WORKS_FOR, AUTHORED

-- 创建图
SELECT create_graph('knowledge_graph');

-- 创建节点
SELECT * FROM cypher('knowledge_graph', $$
    CREATE
        (p:Person {id: 'p1', name: 'Alice'}),
        (o:Organization {id: 'o1', name: 'Company A'}),
        (d:Document {id: 'd1', title: 'Paper 1'})
$$) AS (result agtype);

-- 创建边
SELECT * FROM cypher('knowledge_graph', $$
    MATCH (p:Person {id: 'p1'}), (o:Organization {id: 'o1'})
    CREATE (p)-[:WORKS_FOR {since: '2020-01-01'}]->(o)
$$) AS (result agtype);
```

## 3. Apache AGE 集成

### 3.1 AGE 安装

```sql
-- 安装 Apache AGE 扩展
CREATE EXTENSION IF NOT EXISTS age;

-- 加载 AGE
LOAD 'age';

-- 验证安装
SELECT * FROM pg_extension WHERE extname = 'age';
```

### 3.2 图创建

```sql
-- 创建图
SELECT create_graph('my_graph');

-- 列出所有图
SELECT * FROM ag_catalog.ag_graph;

-- 删除图
SELECT drop_graph('my_graph', true);
```

### 3.3 Cypher 查询

```sql
-- 1. 节点查询
SELECT * FROM cypher('knowledge_graph', $$
    MATCH (p:Person)
    RETURN p.name, p.age
    LIMIT 10
$$) AS (name agtype, age agtype);

-- 2. 路径查询
SELECT * FROM cypher('knowledge_graph', $$
    MATCH path = (a:Person)-[:KNOWS*..3]->(b:Person)
    WHERE a.id = 'person_001'
    RETURN path, length(path) AS path_length
    LIMIT 10
$$) AS (path agtype, path_length agtype);

-- 3. 最短路径查询
SELECT * FROM cypher('knowledge_graph', $$
    MATCH path = shortestPath(
        (a:Person {id: 'person_001'})-[*..10]-(b:Person {id: 'person_002'})
    )
    RETURN path
$$) AS (path agtype);
```

## 4. 查询优化

### 4.1 图查询优化

```sql
-- 1. 限制查询深度
SELECT * FROM cypher('knowledge_graph', $$
    MATCH (a:Person)-[*..2]-(b:Person)
    WHERE a.id = 'person_001'
    RETURN b
    LIMIT 100
$$) AS (b agtype);

-- 2. 使用索引加速查询
-- 在节点属性上创建索引
CREATE INDEX ON nodes USING GIN (properties);

-- 3. 过滤条件前置
SELECT * FROM cypher('knowledge_graph', $$
    MATCH (p:Person)
    WHERE p.age > 25 AND p.age < 35
    RETURN p
    LIMIT 100
$$) AS (p agtype);
```

### 4.2 索引策略

```sql
-- 1. 节点标签索引
CREATE INDEX ON nodes (label);

-- 2. 节点属性索引
CREATE INDEX ON nodes USING GIN (properties);

-- 3. 边索引
CREATE INDEX ON edges (source_id, target_id);
CREATE INDEX ON edges (label);
```

## 5. 最佳实践

### 5.1 图设计建议

1. **节点设计**:
   - 使用有意义的标签（Person, Organization）
   - 节点属性使用 JSONB 存储

2. **边设计**:
   - 使用有意义的边类型（KNOWS, WORKS_FOR）
   - 边属性存储关系元数据

3. **查询优化**:
   - 限制查询深度（*..3）
   - 使用索引加速查询
   - 过滤条件前置

### 5.2 性能优化建议

1. **索引**: 在常用查询属性上创建索引
2. **查询深度**: 限制图查询深度
3. **结果限制**: 使用 LIMIT 限制返回结果

## 6. 参考资料

- [多模数据模型设计](./多模数据模型设计.md)
- [向量数据模型设计](./向量数据模型设计.md)
- [多模数据模型设计](../../04-多模一体化/技术原理/多模数据模型设计.md)

---

**最后更新**: 2025 年 11 月 1 日
**维护者**: PostgreSQL Modern Team
**文档编号**: 06-02-03
